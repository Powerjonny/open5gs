...
OpenAPI_ue_location_info_t *OpenAPI_ue_location_info_create(
	...
	OpenAPI_ue_location_info_t *ue_location_info_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_location_info_t));
...
void OpenAPI_ue_location_info_free(
...
cJSON *OpenAPI_ue_location_info_convertToJSON(
	OpenAPI_ue_location_info_t *ue_location_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(ue_location_info == NULL) {
		ogs_error(
			"OpenAPI_ue_location_info_convertToJSON() failed [UELocationInfo]");
		return NULL;
	}
	...
	if(ue_location_info->location_estimate) {
		...
		if(location_estimate_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_location_info_convertToJSON() failed [location_estimate]");
			goto end;
		}
	...
	if(ue_location_info->is_age_of_location_estimate) {
		...
		{
			ogs_error(
				"OpenAPI_ue_location_info_convertToJSON() failed [age_of_location_estimate]");
			goto end;
		}
	}
	...
	if(ue_location_info->timestamp_of_location_estimate) {
		...
		{
			ogs_error(
				"OpenAPI_ue_location_info_convertToJSON() failed [timestamp_of_location_estimate]");
			goto end;
		}
	}
	...
	if(ue_location_info->velocity_estimate) {
		...
		if(velocity_estimate_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_location_info_convertToJSON() failed [velocity_estimate]");
			goto end;
		}
	...
	if(ue_location_info->is_age_of_velocity_estimate) {
		...
		{
			ogs_error(
				"OpenAPI_ue_location_info_convertToJSON() failed [age_of_velocity_estimate]");
			goto end;
		}
	}
	...
	if(ue_location_info->timestamp_of_velocity_estimate) {
		...
		{
			ogs_error(
				"OpenAPI_ue_location_info_convertToJSON() failed [timestamp_of_velocity_estimate]");
			goto end;
		}
	}
...
OpenAPI_ue_location_info_t *OpenAPI_ue_location_info_parseFromJSON(
	cJSON *ue_location_infoJSON) {
	...
	location_estimate =
		cJSON_GetObjectItemCaseSensitive(ue_location_infoJSON,
		                                 "locationEstimate");
	...
	if(age_of_location_estimate) {
		if(!cJSON_IsNumber(age_of_location_estimate)) {
			ogs_error(
				"OpenAPI_ue_location_info_parseFromJSON() failed [age_of_location_estimate]");
			goto end;
		}
	}
	...
	if(timestamp_of_location_estimate) {
		...
		{
			ogs_error(
				"OpenAPI_ue_location_info_parseFromJSON() failed [timestamp_of_location_estimate]");
			goto end;
		}
	}
	...
	if(age_of_velocity_estimate) {
		if(!cJSON_IsNumber(age_of_velocity_estimate)) {
			ogs_error(
				"OpenAPI_ue_location_info_parseFromJSON() failed [age_of_velocity_estimate]");
			goto end;
		}
	}
	...
	if(timestamp_of_velocity_estimate) {
		...
		{
			ogs_error(
				"OpenAPI_ue_location_info_parseFromJSON() failed [timestamp_of_velocity_estimate]");
			goto end;
		}
	}
	...
	ue_location_info_local_var = OpenAPI_ue_location_info_create(
		...
		age_of_velocity_estimate ?
		age_of_velocity_estimate->valuedouble : 0,
		timestamp_of_velocity_estimate &&
		!cJSON_IsNull(timestamp_of_velocity_estimate) ?
		ogs_strdup(timestamp_of_velocity_estimate->valuestring) : NULL
		);
...
OpenAPI_ue_location_info_t *OpenAPI_ue_location_info_copy(
	OpenAPI_ue_location_info_t *dst, OpenAPI_ue_location_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
