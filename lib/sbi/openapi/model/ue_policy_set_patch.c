...
OpenAPI_ue_policy_set_patch_t *OpenAPI_ue_policy_set_patch_create(
	OpenAPI_list_t *ue_policy_sections, OpenAPI_list_t *upsis,
	bool is_andsp_ind, int andsp_ind, char *pei, OpenAPI_list_t *os_ids) {
...
void OpenAPI_ue_policy_set_patch_free(
	OpenAPI_ue_policy_set_patch_t *ue_policy_set_patch) {
	...
	if(ue_policy_set_patch->ue_policy_sections) {
		OpenAPI_list_for_each(ue_policy_set_patch->ue_policy_sections,
		                      node) {
...
cJSON *OpenAPI_ue_policy_set_patch_convertToJSON(
	OpenAPI_ue_policy_set_patch_t *ue_policy_set_patch) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(ue_policy_set_patch == NULL) {
		ogs_error(
			"OpenAPI_ue_policy_set_patch_convertToJSON() failed [UePolicySetPatch]");
		return NULL;
	}
	...
	if(ue_policy_set_patch->ue_policy_sections) {
		...
		if(ue_policy_sections == NULL) {
			ogs_error(
				"OpenAPI_ue_policy_set_patch_convertToJSON() failed [ue_policy_sections]");
			goto end;
		}
		cJSON *localMapObject = ue_policy_sections;
		if(ue_policy_set_patch->ue_policy_sections) {
			OpenAPI_list_for_each(
				ue_policy_set_patch->ue_policy_sections, node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_ue_policy_set_patch_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(ue_policy_set_patch->upsis) {
		...
		if(upsisList == NULL) {
			ogs_error(
				"OpenAPI_ue_policy_set_patch_convertToJSON() failed [upsis]");
			goto end;
		}
		OpenAPI_list_for_each(ue_policy_set_patch->upsis, node) {
	...
	if(ue_policy_set_patch->is_andsp_ind) {
		if(cJSON_AddBoolToObject(item, "andspInd",
		                         ue_policy_set_patch->andsp_ind) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_ue_policy_set_patch_convertToJSON() failed [andsp_ind]");
			goto end;
		}
	}
	...
	if(ue_policy_set_patch->pei) {
		...
		{
			ogs_error(
				"OpenAPI_ue_policy_set_patch_convertToJSON() failed [pei]");
			goto end;
		}
	}
	...
	if(ue_policy_set_patch->os_ids) {
		...
		if(os_idsList == NULL) {
			ogs_error(
				"OpenAPI_ue_policy_set_patch_convertToJSON() failed [os_ids]");
			goto end;
		}
		OpenAPI_list_for_each(ue_policy_set_patch->os_ids, node) {
...
OpenAPI_ue_policy_set_patch_t *OpenAPI_ue_policy_set_patch_parseFromJSON(
	cJSON *ue_policy_set_patchJSON) {
	...
	ue_policy_sections =
		cJSON_GetObjectItemCaseSensitive(ue_policy_set_patchJSON,
		                                 "uePolicySections");
	if(ue_policy_sections) {
		...
		{
			ogs_error(
				"OpenAPI_ue_policy_set_patch_parseFromJSON() failed [ue_policy_sections]");
			goto end;
		}
		if(cJSON_IsObject(ue_policy_sections)) {
			...
			cJSON_ArrayForEach(ue_policy_sections_local_map,
				...
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_ue_policy_set_patch_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	upsis = cJSON_GetObjectItemCaseSensitive(ue_policy_set_patchJSON,
	                                         "upsis");
	if(upsis) {
		cJSON *upsis_local = NULL;
		if(!cJSON_IsArray(upsis)) {
			ogs_error(
				"OpenAPI_ue_policy_set_patch_parseFromJSON() failed [upsis]");
			goto end;
		}
	...
	andsp_ind = cJSON_GetObjectItemCaseSensitive(ue_policy_set_patchJSON,
	                                             "andspInd");
	if(andsp_ind) {
		if(!cJSON_IsBool(andsp_ind)) {
			ogs_error(
				"OpenAPI_ue_policy_set_patch_parseFromJSON() failed [andsp_ind]");
			goto end;
		}
	}
	...
	if(pei) {
		...
		{
			ogs_error(
				"OpenAPI_ue_policy_set_patch_parseFromJSON() failed [pei]");
			goto end;
		}
	}

	os_ids = cJSON_GetObjectItemCaseSensitive(ue_policy_set_patchJSON,
	                                          "osIds");
	if(os_ids) {
		cJSON *os_ids_local = NULL;
		if(!cJSON_IsArray(os_ids)) {
			ogs_error(
				"OpenAPI_ue_policy_set_patch_parseFromJSON() failed [os_ids]");
			goto end;
		}
...
OpenAPI_ue_policy_set_patch_t *OpenAPI_ue_policy_set_patch_copy(
	OpenAPI_ue_policy_set_patch_t	*dst,
	OpenAPI_ue_policy_set_patch_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
