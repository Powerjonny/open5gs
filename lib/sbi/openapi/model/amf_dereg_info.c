...
OpenAPI_amf_dereg_info_t *OpenAPI_amf_dereg_info_create(
	...
	OpenAPI_amf_dereg_info_t *amf_dereg_info_local_var =
		ogs_malloc(sizeof(OpenAPI_amf_dereg_info_t));
...
cJSON *OpenAPI_amf_dereg_info_convertToJSON(
	OpenAPI_amf_dereg_info_t *amf_dereg_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(amf_dereg_info == NULL) {
		ogs_error(
			"OpenAPI_amf_dereg_info_convertToJSON() failed [AmfDeregInfo]");
		return NULL;
	}
	...
	if(amf_dereg_info->dereg_reason == OpenAPI_deregistration_reason_NULL) {
		ogs_error(
			"OpenAPI_amf_dereg_info_convertToJSON() failed [dereg_reason]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "deregReason",
	                           OpenAPI_deregistration_reason_ToString(
					   amf_dereg_info->dereg_reason)) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_amf_dereg_info_convertToJSON() failed [dereg_reason]");
...
OpenAPI_amf_dereg_info_t *OpenAPI_amf_dereg_info_parseFromJSON(
	cJSON *amf_dereg_infoJSON) {
	...
	dereg_reason = cJSON_GetObjectItemCaseSensitive(amf_dereg_infoJSON,
	                                                "deregReason");
	if(!dereg_reason) {
		ogs_error(
			"OpenAPI_amf_dereg_info_parseFromJSON() failed [dereg_reason]");
		goto end;
	}
...
OpenAPI_amf_dereg_info_t *OpenAPI_amf_dereg_info_copy(
	OpenAPI_amf_dereg_info_t *dst, OpenAPI_amf_dereg_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
