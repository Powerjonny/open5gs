...
OpenAPI_amf_info_t *OpenAPI_amf_info_create(char		*amf_set_id,
                                            char		*amf_region_id,
	...
	OpenAPI_amf_info_t *amf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_amf_info_t));
	...
	amf_info_local_var->is_amf_onboarding_capability =
		is_amf_onboarding_capability;
	amf_info_local_var->amf_onboarding_capability =
		amf_onboarding_capability;
...
void OpenAPI_amf_info_free(OpenAPI_amf_info_t *amf_info) {
	...
	if(amf_info->n2_interface_amf_info) {
		OpenAPI_n2_interface_amf_info_free(
			amf_info->n2_interface_amf_info);
...
cJSON *OpenAPI_amf_info_convertToJSON(OpenAPI_amf_info_t *amf_info) {
	...
	if(!amf_info->amf_set_id) {
		ogs_error(
			"OpenAPI_amf_info_convertToJSON() failed [amf_set_id]");
		return NULL;
	}
	...
	if(!amf_info->amf_region_id) {
		ogs_error(
			"OpenAPI_amf_info_convertToJSON() failed [amf_region_id]");
		return NULL;
	}
	...
	if(!amf_info->guami_list) {
		ogs_error(
			"OpenAPI_amf_info_convertToJSON() failed [guami_list]");
		return NULL;
	}
	...
	if(amf_info->tai_list) {
		...
		if(tai_listList == NULL) {
			ogs_error(
				"OpenAPI_amf_info_convertToJSON() failed [tai_list]");
			goto end;
		}
		OpenAPI_list_for_each(amf_info->tai_list, node) {
			cJSON *itemLocal =
	...
	if(amf_info->tai_range_list) {
		...
		if(tai_range_listList == NULL) {
			ogs_error(
				"OpenAPI_amf_info_convertToJSON() failed [tai_range_list]");
			goto end;
		}
		OpenAPI_list_for_each(amf_info->tai_range_list, node) {
			cJSON *itemLocal =
	...
	if(amf_info->backup_info_amf_failure) {
		...
		if(backup_info_amf_failureList == NULL) {
			ogs_error(
				"OpenAPI_amf_info_convertToJSON() failed [backup_info_amf_failure]");
			goto end;
		}
		OpenAPI_list_for_each(amf_info->backup_info_amf_failure, node) {
			cJSON *itemLocal =
	...
	if(amf_info->backup_info_amf_removal) {
		...
		if(backup_info_amf_removalList == NULL) {
			ogs_error(
				"OpenAPI_amf_info_convertToJSON() failed [backup_info_amf_removal]");
			goto end;
		}
		OpenAPI_list_for_each(amf_info->backup_info_amf_removal, node) {
			cJSON *itemLocal =
	...
	if(amf_info->n2_interface_amf_info) {
		...
		if(n2_interface_amf_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_amf_info_convertToJSON() failed [n2_interface_amf_info]");
			goto end;
		}
	...
	if(amf_info->is_amf_onboarding_capability) {
		if(cJSON_AddBoolToObject(item, "amfOnboardingCapability",
		                         amf_info->amf_onboarding_capability) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_amf_info_convertToJSON() failed [amf_onboarding_capability]");
			goto end;
		}
	}
...
OpenAPI_amf_info_t *OpenAPI_amf_info_parseFromJSON(cJSON *amf_infoJSON) {
	...
	if(!amf_set_id) {
		ogs_error(
			"OpenAPI_amf_info_parseFromJSON() failed [amf_set_id]");
		goto end;
	}
	...
	amf_region_id = cJSON_GetObjectItemCaseSensitive(amf_infoJSON,
	                                                 "amfRegionId");
	if(!amf_region_id) {
		ogs_error(
			"OpenAPI_amf_info_parseFromJSON() failed [amf_region_id]");
		goto end;
	}
	...
	guami_list = cJSON_GetObjectItemCaseSensitive(amf_infoJSON,
	                                              "guamiList");
	if(!guami_list) {
		ogs_error(
			"OpenAPI_amf_info_parseFromJSON() failed [guami_list]");
		goto end;
	}
	...
	if(tai_list) {
		...
		if(!cJSON_IsArray(tai_list)) {
			ogs_error(
				"OpenAPI_amf_info_parseFromJSON() failed [tai_list]");
			goto end;
		}
	...
	tai_range_list = cJSON_GetObjectItemCaseSensitive(amf_infoJSON,
	                                                  "taiRangeList");
	if(tai_range_list) {
		cJSON *tai_range_list_local = NULL;
		if(!cJSON_IsArray(tai_range_list)) {
			ogs_error(
				"OpenAPI_amf_info_parseFromJSON() failed [tai_range_list]");
			goto end;
		}
	...
	backup_info_amf_failure = cJSON_GetObjectItemCaseSensitive(amf_infoJSON,
	...
	if(backup_info_amf_failure) {
		...
		if(!cJSON_IsArray(backup_info_amf_failure)) {
			ogs_error(
				"OpenAPI_amf_info_parseFromJSON() failed [backup_info_amf_failure]");
			goto end;
		}
	...
	backup_info_amf_removal = cJSON_GetObjectItemCaseSensitive(amf_infoJSON,
	...
	if(backup_info_amf_removal) {
		...
		if(!cJSON_IsArray(backup_info_amf_removal)) {
			ogs_error(
				"OpenAPI_amf_info_parseFromJSON() failed [backup_info_amf_removal]");
			goto end;
		}
	...
	n2_interface_amf_info = cJSON_GetObjectItemCaseSensitive(amf_infoJSON,
	                                                         "n2InterfaceAmfInfo");
	...
	amf_onboarding_capability =
		cJSON_GetObjectItemCaseSensitive(amf_infoJSON,
		                                 "amfOnboardingCapability");
	if(amf_onboarding_capability) {
		if(!cJSON_IsBool(amf_onboarding_capability)) {
			ogs_error(
				"OpenAPI_amf_info_parseFromJSON() failed [amf_onboarding_capability]");
			goto end;
		}
	}
	...
	amf_info_local_var = OpenAPI_amf_info_create(
		...
		n2_interface_amf_info ? n2_interface_amf_info_local_nonprim :
		NULL,
...
OpenAPI_amf_info_t *OpenAPI_amf_info_copy(OpenAPI_amf_info_t	*dst,
                                          OpenAPI_amf_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
