...
OpenAPI_pcscf_info_t *OpenAPI_pcscf_info_create(OpenAPI_list_t	*access_type,
	...
	OpenAPI_pcscf_info_t *pcscf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_pcscf_info_t));
	...
	pcscf_info_local_var->served_ipv4_address_ranges =
		served_ipv4_address_ranges;
	pcscf_info_local_var->served_ipv6_prefix_ranges =
		served_ipv6_prefix_ranges;
...
void OpenAPI_pcscf_info_free(OpenAPI_pcscf_info_t *pcscf_info) {
	...
	if(pcscf_info->served_ipv4_address_ranges) {
		OpenAPI_list_for_each(pcscf_info->served_ipv4_address_ranges,
		                      node) {
	...
	if(pcscf_info->served_ipv6_prefix_ranges) {
		OpenAPI_list_for_each(pcscf_info->served_ipv6_prefix_ranges,
		                      node) {
...
cJSON *OpenAPI_pcscf_info_convertToJSON(OpenAPI_pcscf_info_t *pcscf_info) {
	...
	if(pcscf_info == NULL) {
		ogs_error(
			"OpenAPI_pcscf_info_convertToJSON() failed [PcscfInfo]");
		return NULL;
	}
	...
	if(pcscf_info->access_type != OpenAPI_access_type_NULL) {
		...
		if(access_typeList == NULL) {
			ogs_error(
				"OpenAPI_pcscf_info_convertToJSON() failed [access_type]");
			goto end;
		}
	...
	if(pcscf_info->dnn_list) {
		...
		if(dnn_listList == NULL) {
			ogs_error(
				"OpenAPI_pcscf_info_convertToJSON() failed [dnn_list]");
			goto end;
		}
		OpenAPI_list_for_each(pcscf_info->dnn_list, node) {
	...
	if(pcscf_info->gm_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_pcscf_info_convertToJSON() failed [gm_fqdn]");
			goto end;
		}
	}
	...
	if(pcscf_info->gm_ipv4_addresses) {
		...
		if(gm_ipv4_addressesList == NULL) {
			ogs_error(
				"OpenAPI_pcscf_info_convertToJSON() failed [gm_ipv4_addresses]");
			goto end;
		}
		OpenAPI_list_for_each(pcscf_info->gm_ipv4_addresses, node) {
	...
	if(pcscf_info->gm_ipv6_addresses) {
		...
		if(gm_ipv6_addressesList == NULL) {
			ogs_error(
				"OpenAPI_pcscf_info_convertToJSON() failed [gm_ipv6_addresses]");
			goto end;
		}
		OpenAPI_list_for_each(pcscf_info->gm_ipv6_addresses, node) {
	...
	if(pcscf_info->mw_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_pcscf_info_convertToJSON() failed [mw_fqdn]");
			goto end;
		}
	}
	...
	if(pcscf_info->mw_ipv4_addresses) {
		...
		if(mw_ipv4_addressesList == NULL) {
			ogs_error(
				"OpenAPI_pcscf_info_convertToJSON() failed [mw_ipv4_addresses]");
			goto end;
		}
		OpenAPI_list_for_each(pcscf_info->mw_ipv4_addresses, node) {
	...
	if(pcscf_info->mw_ipv6_addresses) {
		...
		if(mw_ipv6_addressesList == NULL) {
			ogs_error(
				"OpenAPI_pcscf_info_convertToJSON() failed [mw_ipv6_addresses]");
			goto end;
		}
		OpenAPI_list_for_each(pcscf_info->mw_ipv6_addresses, node) {
	...
	if(pcscf_info->served_ipv4_address_ranges) {
		...
		if(served_ipv4_address_rangesList == NULL) {
			ogs_error(
				"OpenAPI_pcscf_info_convertToJSON() failed [served_ipv4_address_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(pcscf_info->served_ipv4_address_ranges,
		                      node) {
	...
	if(pcscf_info->served_ipv6_prefix_ranges) {
		...
		if(served_ipv6_prefix_rangesList == NULL) {
			ogs_error(
				"OpenAPI_pcscf_info_convertToJSON() failed [served_ipv6_prefix_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(pcscf_info->served_ipv6_prefix_ranges,
		                      node) {
...
OpenAPI_pcscf_info_t *OpenAPI_pcscf_info_parseFromJSON(cJSON *pcscf_infoJSON) {
	...
	access_type = cJSON_GetObjectItemCaseSensitive(pcscf_infoJSON,
	                                               "accessType");
	if(access_type) {
		cJSON *access_type_local = NULL;
		if(!cJSON_IsArray(access_type)) {
			ogs_error(
				"OpenAPI_pcscf_info_parseFromJSON() failed [access_type]");
			goto end;
		}
		...
		if(access_typeList->count == 0) {
			ogs_error(
				"OpenAPI_pcscf_info_parseFromJSON() failed: Expected access_typeList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(dnn_list) {
		...
		if(!cJSON_IsArray(dnn_list)) {
			ogs_error(
				"OpenAPI_pcscf_info_parseFromJSON() failed [dnn_list]");
			goto end;
		}
	...
	if(gm_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_pcscf_info_parseFromJSON() failed [gm_fqdn]");
			goto end;
		}
	}

	gm_ipv4_addresses = cJSON_GetObjectItemCaseSensitive(pcscf_infoJSON,
	                                                     "gmIpv4Addresses");
	if(gm_ipv4_addresses) {
		cJSON *gm_ipv4_addresses_local = NULL;
		if(!cJSON_IsArray(gm_ipv4_addresses)) {
			ogs_error(
				"OpenAPI_pcscf_info_parseFromJSON() failed [gm_ipv4_addresses]");
			goto end;
		}
	...
	gm_ipv6_addresses = cJSON_GetObjectItemCaseSensitive(pcscf_infoJSON,
	                                                     "gmIpv6Addresses");
	if(gm_ipv6_addresses) {
		cJSON *gm_ipv6_addresses_local = NULL;
		if(!cJSON_IsArray(gm_ipv6_addresses)) {
			ogs_error(
				"OpenAPI_pcscf_info_parseFromJSON() failed [gm_ipv6_addresses]");
			goto end;
		}
	...
	if(mw_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_pcscf_info_parseFromJSON() failed [mw_fqdn]");
			goto end;
		}
	}

	mw_ipv4_addresses = cJSON_GetObjectItemCaseSensitive(pcscf_infoJSON,
	                                                     "mwIpv4Addresses");
	if(mw_ipv4_addresses) {
		cJSON *mw_ipv4_addresses_local = NULL;
		if(!cJSON_IsArray(mw_ipv4_addresses)) {
			ogs_error(
				"OpenAPI_pcscf_info_parseFromJSON() failed [mw_ipv4_addresses]");
			goto end;
		}
	...
	mw_ipv6_addresses = cJSON_GetObjectItemCaseSensitive(pcscf_infoJSON,
	                                                     "mwIpv6Addresses");
	if(mw_ipv6_addresses) {
		cJSON *mw_ipv6_addresses_local = NULL;
		if(!cJSON_IsArray(mw_ipv6_addresses)) {
			ogs_error(
				"OpenAPI_pcscf_info_parseFromJSON() failed [mw_ipv6_addresses]");
			goto end;
		}
	...
	served_ipv4_address_ranges =
		cJSON_GetObjectItemCaseSensitive(pcscf_infoJSON,
	...
	if(served_ipv4_address_ranges) {
		...
		if(!cJSON_IsArray(served_ipv4_address_ranges)) {
			ogs_error(
				"OpenAPI_pcscf_info_parseFromJSON() failed [served_ipv4_address_ranges]");
			goto end;
		}
	...
	if(served_ipv6_prefix_ranges) {
		...
		if(!cJSON_IsArray(served_ipv6_prefix_ranges)) {
			ogs_error(
				"OpenAPI_pcscf_info_parseFromJSON() failed [served_ipv6_prefix_ranges]");
			goto end;
		}
