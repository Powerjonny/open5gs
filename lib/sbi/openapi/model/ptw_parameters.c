...
OpenAPI_ptw_parameters_t *OpenAPI_ptw_parameters_create(
	...
	OpenAPI_ptw_parameters_t *ptw_parameters_local_var =
		ogs_malloc(sizeof(OpenAPI_ptw_parameters_t));
...
cJSON *OpenAPI_ptw_parameters_convertToJSON(
	OpenAPI_ptw_parameters_t *ptw_parameters) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(ptw_parameters == NULL) {
		ogs_error(
			"OpenAPI_ptw_parameters_convertToJSON() failed [PtwParameters]");
		return NULL;
	}
	...
	if(ptw_parameters->operation_mode == OpenAPI_operation_mode_NULL) {
		ogs_error(
			"OpenAPI_ptw_parameters_convertToJSON() failed [operation_mode]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "operationMode",
	                           OpenAPI_operation_mode_ToString(
					   ptw_parameters->operation_mode)) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_ptw_parameters_convertToJSON() failed [operation_mode]");
	...
	if(!ptw_parameters->ptw_value) {
		ogs_error(
			"OpenAPI_ptw_parameters_convertToJSON() failed [ptw_value]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "ptwValue",
	                           ptw_parameters->ptw_value) == NULL)
	{
		ogs_error(
			"OpenAPI_ptw_parameters_convertToJSON() failed [ptw_value]");
...
OpenAPI_ptw_parameters_t *OpenAPI_ptw_parameters_parseFromJSON(
	cJSON *ptw_parametersJSON) {
	...
	operation_mode = cJSON_GetObjectItemCaseSensitive(ptw_parametersJSON,
	                                                  "operationMode");
	if(!operation_mode) {
		ogs_error(
			"OpenAPI_ptw_parameters_parseFromJSON() failed [operation_mode]");
		goto end;
	}
	...
	operation_modeVariable =
		OpenAPI_operation_mode_FromString(operation_mode->valuestring);

	ptw_value = cJSON_GetObjectItemCaseSensitive(ptw_parametersJSON,
	                                             "ptwValue");
	if(!ptw_value) {
		ogs_error(
			"OpenAPI_ptw_parameters_parseFromJSON() failed [ptw_value]");
		goto end;
	}
...
OpenAPI_ptw_parameters_t *OpenAPI_ptw_parameters_copy(
	OpenAPI_ptw_parameters_t *dst, OpenAPI_ptw_parameters_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
