...
OpenAPI_binding_resp_t *OpenAPI_binding_resp_create(
	...
	OpenAPI_binding_resp_t *binding_resp_local_var =
		ogs_malloc(sizeof(OpenAPI_binding_resp_t));
...
void OpenAPI_binding_resp_free(OpenAPI_binding_resp_t *binding_resp) {
	...
	if(binding_resp->pcf_sm_ip_end_points) {
		OpenAPI_list_for_each(binding_resp->pcf_sm_ip_end_points,
		                      node) {
...
cJSON *OpenAPI_binding_resp_convertToJSON(
	OpenAPI_binding_resp_t *binding_resp) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(binding_resp == NULL) {
		ogs_error(
			"OpenAPI_binding_resp_convertToJSON() failed [BindingResp]");
		return NULL;
	}
	...
	if(binding_resp->pcf_sm_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_binding_resp_convertToJSON() failed [pcf_sm_fqdn]");
			goto end;
		}
	}
	...
	if(binding_resp->pcf_sm_ip_end_points) {
		...
		if(pcf_sm_ip_end_pointsList == NULL) {
			ogs_error(
				"OpenAPI_binding_resp_convertToJSON() failed [pcf_sm_ip_end_points]");
			goto end;
		}
		OpenAPI_list_for_each(binding_resp->pcf_sm_ip_end_points,
		                      node) {
...
OpenAPI_binding_resp_t *OpenAPI_binding_resp_parseFromJSON(
	...
	pcf_sm_fqdn = cJSON_GetObjectItemCaseSensitive(binding_respJSON,
	                                               "pcfSmFqdn");
	if(pcf_sm_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_binding_resp_parseFromJSON() failed [pcf_sm_fqdn]");
			goto end;
		}
	}

	pcf_sm_ip_end_points =
		cJSON_GetObjectItemCaseSensitive(binding_respJSON,
		                                 "pcfSmIpEndPoints");
	if(pcf_sm_ip_end_points) {
		cJSON *pcf_sm_ip_end_points_local = NULL;
		if(!cJSON_IsArray(pcf_sm_ip_end_points)) {
			ogs_error(
				"OpenAPI_binding_resp_parseFromJSON() failed [pcf_sm_ip_end_points]");
			goto end;
		}
...
OpenAPI_binding_resp_t *OpenAPI_binding_resp_copy(
	OpenAPI_binding_resp_t	*dst,
