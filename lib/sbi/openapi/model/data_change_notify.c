...
OpenAPI_data_change_notify_t *OpenAPI_data_change_notify_create(
	...
	OpenAPI_data_change_notify_t *data_change_notify_local_var =
		ogs_malloc(sizeof(OpenAPI_data_change_notify_t));
	ogs_assert(data_change_notify_local_var);

	data_change_notify_local_var->original_callback_reference =
	...
	data_change_notify_local_var->additional_sdm_subscriptions =
		additional_sdm_subscriptions;
	data_change_notify_local_var->subscription_data_subscriptions =
...
void OpenAPI_data_change_notify_free(
	...
	if(data_change_notify->original_callback_reference) {
		OpenAPI_list_for_each(
			data_change_notify->original_callback_reference, node) {
			ogs_free(node->data);
		}
		OpenAPI_list_free(
	...
	if(data_change_notify->subscription_data_subscriptions) {
		OpenAPI_list_for_each(
			data_change_notify->subscription_data_subscriptions,
			node) {
...
cJSON *OpenAPI_data_change_notify_convertToJSON(
	...
	if(data_change_notify == NULL) {
		ogs_error(
			"OpenAPI_data_change_notify_convertToJSON() failed [DataChangeNotify]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(data_change_notify->original_callback_reference) {
		cJSON *original_callback_referenceList =
			cJSON_AddArrayToObject(item,
			                       "originalCallbackReference");
		if(original_callback_referenceList == NULL) {
			ogs_error(
				"OpenAPI_data_change_notify_convertToJSON() failed [original_callback_reference]");
			goto end;
		}
		OpenAPI_list_for_each(
			data_change_notify->original_callback_reference, node) {
			if(cJSON_AddStringToObject(
				   original_callback_referenceList, "",
				   (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_data_change_notify_convertToJSON() failed [original_callback_reference]");
	...
	if(data_change_notify->ue_id) {
		...
		{
			ogs_error(
				"OpenAPI_data_change_notify_convertToJSON() failed [ue_id]");
			goto end;
		}
	}

	if(data_change_notify->notify_items) {
		cJSON *notify_itemsList = cJSON_AddArrayToObject(item,
		                                                 "notifyItems");
		if(notify_itemsList == NULL) {
			ogs_error(
				"OpenAPI_data_change_notify_convertToJSON() failed [notify_items]");
			goto end;
		}
		OpenAPI_list_for_each(data_change_notify->notify_items, node) {
			cJSON *itemLocal =
				OpenAPI_notify_item_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_data_change_notify_convertToJSON() failed [notify_items]");
				goto end;
	...
	if(data_change_notify->sdm_subscription) {
		...
		if(sdm_subscription_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_data_change_notify_convertToJSON() failed [sdm_subscription]");
			goto end;
		}
	...
	if(data_change_notify->additional_sdm_subscriptions) {
		cJSON *additional_sdm_subscriptionsList =
			cJSON_AddArrayToObject(item,
			                       "additionalSdmSubscriptions");
		if(additional_sdm_subscriptionsList == NULL) {
			ogs_error(
				"OpenAPI_data_change_notify_convertToJSON() failed [additional_sdm_subscriptions]");
			goto end;
		}
		OpenAPI_list_for_each(
			data_change_notify->additional_sdm_subscriptions,
			...
			cJSON_AddItemToArray(additional_sdm_subscriptionsList,
			                     itemLocal);
		}
	}
	...
	if(data_change_notify->subscription_data_subscriptions) {
		...
		if(subscription_data_subscriptionsList == NULL) {
			ogs_error(
				"OpenAPI_data_change_notify_convertToJSON() failed [subscription_data_subscriptions]");
			goto end;
		}
		OpenAPI_list_for_each(
			data_change_notify->subscription_data_subscriptions,
...
OpenAPI_data_change_notify_t *OpenAPI_data_change_notify_parseFromJSON(
	...
	original_callback_reference =
		cJSON_GetObjectItemCaseSensitive(data_change_notifyJSON,
		                                 "originalCallbackReference");
	if(original_callback_reference) {
		cJSON *original_callback_reference_local = NULL;
		if(!cJSON_IsArray(original_callback_reference)) {
			ogs_error(
				"OpenAPI_data_change_notify_parseFromJSON() failed [original_callback_reference]");
			goto end;
		}

		original_callback_referenceList = OpenAPI_list_create();

		cJSON_ArrayForEach(original_callback_reference_local,
		                   original_callback_reference) {
			...
			OpenAPI_list_add(original_callback_referenceList,
			                 ogs_strdup(
						 original_callback_reference_local
						 ->valuestring));
		}
	}

	ue_id = cJSON_GetObjectItemCaseSensitive(data_change_notifyJSON,
	                                         "ueId");
	if(ue_id) {
		...
		{
			ogs_error(
				"OpenAPI_data_change_notify_parseFromJSON() failed [ue_id]");
			goto end;
		}
	}

	notify_items = cJSON_GetObjectItemCaseSensitive(data_change_notifyJSON,
	                                                "notifyItems");
	if(notify_items) {
		cJSON *notify_items_local = NULL;
		if(!cJSON_IsArray(notify_items)) {
			ogs_error(
				"OpenAPI_data_change_notify_parseFromJSON() failed [notify_items]");
			goto end;
		}
		...
		cJSON_ArrayForEach(notify_items_local, notify_items) {
			...
			OpenAPI_notify_item_t *notify_itemsItem =
				OpenAPI_notify_item_parseFromJSON(
					notify_items_local);
			if(!notify_itemsItem) {
	...
	if(additional_sdm_subscriptions) {
		...
		if(!cJSON_IsArray(additional_sdm_subscriptions)) {
			ogs_error(
				"OpenAPI_data_change_notify_parseFromJSON() failed [additional_sdm_subscriptions]");
			goto end;
		}

		additional_sdm_subscriptionsList = OpenAPI_list_create();

		cJSON_ArrayForEach(additional_sdm_subscriptions_local,
		                   additional_sdm_subscriptions) {
			if(!cJSON_IsObject(
				   additional_sdm_subscriptions_local))
			{
				ogs_error(
					"OpenAPI_data_change_notify_parseFromJSON() failed [additional_sdm_subscriptions]");
				goto end;
			...
			OpenAPI_sdm_subscription_1_t *
			        additional_sdm_subscriptionsItem =
				OpenAPI_sdm_subscription_1_parseFromJSON(
					additional_sdm_subscriptions_local);
			if(!additional_sdm_subscriptionsItem) {
				ogs_error(
					"No additional_sdm_subscriptionsItem");
				goto end;
			}
			OpenAPI_list_add(additional_sdm_subscriptionsList,
			                 additional_sdm_subscriptionsItem);
		}
	}
	...
	if(subscription_data_subscriptions) {
		...
		if(!cJSON_IsArray(subscription_data_subscriptions)) {
			ogs_error(
				"OpenAPI_data_change_notify_parseFromJSON() failed [subscription_data_subscriptions]");
			goto end;
		}
	...
	data_change_notify_local_var = OpenAPI_data_change_notify_create(
		...
		NULL,
		ue_id &&
		!cJSON_IsNull(ue_id) ? ogs_strdup(ue_id->valuestring) : NULL,
