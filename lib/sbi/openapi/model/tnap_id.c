...
OpenAPI_tnap_id_t *OpenAPI_tnap_id_create(char *ss_id, char *bss_id,
                                          char *civic_address) {
	OpenAPI_tnap_id_t *tnap_id_local_var =
		ogs_malloc(sizeof(OpenAPI_tnap_id_t));
...
cJSON *OpenAPI_tnap_id_convertToJSON(OpenAPI_tnap_id_t *tnap_id) {
	...
	if(tnap_id->ss_id) {
		...
		{
			ogs_error(
				"OpenAPI_tnap_id_convertToJSON() failed [ss_id]");
			goto end;
		}
	}
	...
	if(tnap_id->bss_id) {
		...
		{
			ogs_error(
				"OpenAPI_tnap_id_convertToJSON() failed [bss_id]");
			goto end;
		}
	}
	...
	if(tnap_id->civic_address) {
		...
		{
			ogs_error(
				"OpenAPI_tnap_id_convertToJSON() failed [civic_address]");
			goto end;
		}
	}
...
OpenAPI_tnap_id_t *OpenAPI_tnap_id_parseFromJSON(cJSON *tnap_idJSON) {
	...
	if(ss_id) {
		...
		{
			ogs_error(
				"OpenAPI_tnap_id_parseFromJSON() failed [ss_id]");
			goto end;
		}
	}
	...
	if(bss_id) {
		...
		{
			ogs_error(
				"OpenAPI_tnap_id_parseFromJSON() failed [bss_id]");
			goto end;
		}
	}
	...
	if(civic_address) {
		...
		{
			ogs_error(
				"OpenAPI_tnap_id_parseFromJSON() failed [civic_address]");
			goto end;
		}
	}
	...
	tnap_id_local_var = OpenAPI_tnap_id_create(
		ss_id &&
		!cJSON_IsNull(ss_id) ? ogs_strdup(ss_id->valuestring) : NULL,
		bss_id &&
		!cJSON_IsNull(bss_id) ? ogs_strdup(bss_id->valuestring) : NULL,
		civic_address &&
		!cJSON_IsNull(civic_address) ?
		ogs_strdup(civic_address->valuestring) : NULL
		);
...
OpenAPI_tnap_id_t *OpenAPI_tnap_id_copy(OpenAPI_tnap_id_t	*dst,
                                        OpenAPI_tnap_id_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
