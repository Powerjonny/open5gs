...
OpenAPI_mbs_subscription_data_1_t *OpenAPI_mbs_subscription_data_1_create(
	...
	OpenAPI_mbs_subscription_data_1_t *mbs_subscription_data_1_local_var =
		ogs_malloc(sizeof(OpenAPI_mbs_subscription_data_1_t));
	...
	mbs_subscription_data_1_local_var->mbs_session_id_list =
		mbs_session_id_list;
...
void OpenAPI_mbs_subscription_data_1_free(
	...
	if(mbs_subscription_data_1->mbs_session_id_list) {
		OpenAPI_list_for_each(
			mbs_subscription_data_1->mbs_session_id_list, node) {
...
cJSON *OpenAPI_mbs_subscription_data_1_convertToJSON(
	...
	if(mbs_subscription_data_1 == NULL) {
		ogs_error(
			"OpenAPI_mbs_subscription_data_1_convertToJSON() failed [MbsSubscriptionData_1]");
		return NULL;
	}
	...
	if(mbs_subscription_data_1->is_mbs_allowed) {
		if(cJSON_AddBoolToObject(item, "mbsAllowed",
		                         mbs_subscription_data_1->mbs_allowed)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_mbs_subscription_data_1_convertToJSON() failed [mbs_allowed]");
			goto end;
		}
	}
	...
	if(mbs_subscription_data_1->mbs_session_id_list) {
		...
		if(mbs_session_id_listList == NULL) {
			ogs_error(
				"OpenAPI_mbs_subscription_data_1_convertToJSON() failed [mbs_session_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			mbs_subscription_data_1->mbs_session_id_list, node) {
			cJSON *itemLocal =
				OpenAPI_mbs_session_id_1_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_mbs_subscription_data_1_convertToJSON() failed [mbs_session_id_list]");
...
OpenAPI_mbs_subscription_data_1_t *OpenAPI_mbs_subscription_data_1_parseFromJSON
	...
	mbs_allowed =
		cJSON_GetObjectItemCaseSensitive(mbs_subscription_data_1JSON,
		                                 "mbsAllowed");
	if(mbs_allowed) {
		if(!cJSON_IsBool(mbs_allowed)) {
			ogs_error(
				"OpenAPI_mbs_subscription_data_1_parseFromJSON() failed [mbs_allowed]");
			goto end;
		}
	}
	...
	if(mbs_session_id_list) {
		...
		if(!cJSON_IsArray(mbs_session_id_list)) {
			ogs_error(
				"OpenAPI_mbs_subscription_data_1_parseFromJSON() failed [mbs_session_id_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(mbs_session_id_list_local,
			...
			OpenAPI_mbs_session_id_1_t *mbs_session_id_listItem =
	...
	mbs_subscription_data_1_local_var =
		OpenAPI_mbs_subscription_data_1_create(
...
OpenAPI_mbs_subscription_data_1_t *OpenAPI_mbs_subscription_data_1_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_mbs_subscription_data_1_convertToJSON() failed");
		return NULL;
	}
