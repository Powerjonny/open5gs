...
OpenAPI_redundant_transmission_exp_info_create(
	...
	OpenAPI_redundant_transmission_exp_info_t *
		...
		ogs_malloc(sizeof(OpenAPI_redundant_transmission_exp_info_t));
...
void OpenAPI_redundant_transmission_exp_info_free(
	...
	if(redundant_transmission_exp_info->red_trans_exps) {
		OpenAPI_list_for_each(
			redundant_transmission_exp_info->red_trans_exps, node) {
			OpenAPI_redundant_transmission_exp_per_ts_free(
				node->data);
		}
		OpenAPI_list_free(
			redundant_transmission_exp_info->red_trans_exps);
...
cJSON *OpenAPI_redundant_transmission_exp_info_convertToJSON(
	...
	if(redundant_transmission_exp_info == NULL) {
		ogs_error(
			"OpenAPI_redundant_transmission_exp_info_convertToJSON() failed [RedundantTransmissionExpInfo]");
		return NULL;
	}
	...
	if(redundant_transmission_exp_info->spatial_valid_con) {
		...
		if(spatial_valid_con_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_redundant_transmission_exp_info_convertToJSON() failed [spatial_valid_con]");
			goto end;
		}
	...
	if(redundant_transmission_exp_info->dnn) {
		...
		{
			ogs_error(
				"OpenAPI_redundant_transmission_exp_info_convertToJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(!redundant_transmission_exp_info->red_trans_exps) {
		ogs_error(
			"OpenAPI_redundant_transmission_exp_info_convertToJSON() failed [red_trans_exps]");
		return NULL;
	}
	cJSON *red_trans_expsList = cJSON_AddArrayToObject(item,
	                                                   "redTransExps");
	...
	OpenAPI_list_for_each(redundant_transmission_exp_info->red_trans_exps,
	                      node) {
		cJSON *itemLocal =
			OpenAPI_redundant_transmission_exp_per_ts_convertToJSON(
				node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_redundant_transmission_exp_info_convertToJSON() failed [red_trans_exps]");
...
OpenAPI_redundant_transmission_exp_info_parseFromJSON(
	...
	if(dnn) {
		...
		{
			ogs_error(
				"OpenAPI_redundant_transmission_exp_info_parseFromJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(!red_trans_exps) {
		ogs_error(
			"OpenAPI_redundant_transmission_exp_info_parseFromJSON() failed [red_trans_exps]");
		goto end;
	}
	...
	cJSON_ArrayForEach(red_trans_exps_local, red_trans_exps) {
		...
		OpenAPI_redundant_transmission_exp_per_ts_t *red_trans_expsItem
			=
...
OpenAPI_redundant_transmission_exp_info_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_redundant_transmission_exp_info_convertToJSON() failed");
		return NULL;
	}
