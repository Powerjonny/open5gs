...
OpenAPI_congestion_type_t *OpenAPI_congestion_type_create() {
	OpenAPI_congestion_type_t *congestion_type_local_var =
		ogs_malloc(sizeof(OpenAPI_congestion_type_t));
...
cJSON *OpenAPI_congestion_type_convertToJSON(
	...
	if(congestion_type == NULL) {
		ogs_error(
			"OpenAPI_congestion_type_convertToJSON() failed [CongestionType]");
		return NULL;
	}
