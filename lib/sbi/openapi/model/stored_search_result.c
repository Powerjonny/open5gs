...
OpenAPI_stored_search_result_t *OpenAPI_stored_search_result_create(
	...
	OpenAPI_stored_search_result_t *stored_search_result_local_var =
		ogs_malloc(sizeof(OpenAPI_stored_search_result_t));
...
void OpenAPI_stored_search_result_free(
	...
	if(stored_search_result->nf_instances) {
		OpenAPI_list_for_each(stored_search_result->nf_instances,
		                      node) {
...
cJSON *OpenAPI_stored_search_result_convertToJSON(
	...
	if(stored_search_result == NULL) {
		ogs_error(
			"OpenAPI_stored_search_result_convertToJSON() failed [StoredSearchResult]");
		return NULL;
	}
	...
	if(!stored_search_result->nf_instances) {
		ogs_error(
			"OpenAPI_stored_search_result_convertToJSON() failed [nf_instances]");
		return NULL;
	}
...
OpenAPI_stored_search_result_t *OpenAPI_stored_search_result_parseFromJSON(
	...
	nf_instances =
		cJSON_GetObjectItemCaseSensitive(stored_search_resultJSON,
		                                 "nfInstances");
	if(!nf_instances) {
		ogs_error(
			"OpenAPI_stored_search_result_parseFromJSON() failed [nf_instances]");
		goto end;
	}
...
OpenAPI_stored_search_result_t *OpenAPI_stored_search_result_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_stored_search_result_convertToJSON() failed");
		return NULL;
	}
