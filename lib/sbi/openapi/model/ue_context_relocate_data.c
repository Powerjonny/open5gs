...
OpenAPI_ue_context_relocate_data_t *OpenAPI_ue_context_relocate_data_create(
	...
	OpenAPI_ue_context_relocate_data_t *ue_context_relocate_data_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_context_relocate_data_t));
...
void OpenAPI_ue_context_relocate_data_free(
	OpenAPI_ue_context_relocate_data_t *ue_context_relocate_data) {
	...
	if(ue_context_relocate_data->pdu_session_list) {
		OpenAPI_list_for_each(
			ue_context_relocate_data->pdu_session_list, node) {
...
cJSON *OpenAPI_ue_context_relocate_data_convertToJSON(
	OpenAPI_ue_context_relocate_data_t *ue_context_relocate_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(ue_context_relocate_data == NULL) {
		ogs_error(
			"OpenAPI_ue_context_relocate_data_convertToJSON() failed [UeContextRelocateData]");
		return NULL;
	}
	...
	if(!ue_context_relocate_data->ue_context) {
		ogs_error(
			"OpenAPI_ue_context_relocate_data_convertToJSON() failed [ue_context]");
		return NULL;
	}
	...
	if(!ue_context_relocate_data->target_id) {
		ogs_error(
			"OpenAPI_ue_context_relocate_data_convertToJSON() failed [target_id]");
		return NULL;
	}
	...
	if(!ue_context_relocate_data->source_to_target_data) {
		ogs_error(
			"OpenAPI_ue_context_relocate_data_convertToJSON() failed [source_to_target_data]");
		return NULL;
	}
	...
	if(!ue_context_relocate_data->forward_relocation_request) {
		ogs_error(
			"OpenAPI_ue_context_relocate_data_convertToJSON() failed [forward_relocation_request]");
		return NULL;
	}
	...
	if(ue_context_relocate_data->pdu_session_list) {
		...
		if(pdu_session_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_relocate_data_convertToJSON() failed [pdu_session_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			ue_context_relocate_data->pdu_session_list, node) {
			cJSON *itemLocal =
				OpenAPI_n2_sm_information_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_ue_context_relocate_data_convertToJSON() failed [pdu_session_list]");
	...
	if(ue_context_relocate_data->ue_radio_capability) {
		...
		if(ue_radio_capability_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_relocate_data_convertToJSON() failed [ue_radio_capability]");
			goto end;
		}
	...
	if(ue_context_relocate_data->ngap_cause) {
		...
		if(ngap_cause_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_relocate_data_convertToJSON() failed [ngap_cause]");
			goto end;
		}
	...
	if(ue_context_relocate_data->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_relocate_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
...
OpenAPI_ue_context_relocate_data_parseFromJSON(
	...
	OpenAPI_ref_to_binary_data_t *forward_relocation_request_local_nonprim =
		NULL;
	...
	ue_context =
		cJSON_GetObjectItemCaseSensitive(ue_context_relocate_dataJSON,
		                                 "ueContext");
	if(!ue_context) {
		ogs_error(
			"OpenAPI_ue_context_relocate_data_parseFromJSON() failed [ue_context]");
		goto end;
	}
	...
	if(!target_id) {
		ogs_error(
			"OpenAPI_ue_context_relocate_data_parseFromJSON() failed [target_id]");
		goto end;
	}
	...
	if(!source_to_target_data) {
		ogs_error(
			"OpenAPI_ue_context_relocate_data_parseFromJSON() failed [source_to_target_data]");
		goto end;
	}
	...
	if(!forward_relocation_request) {
		ogs_error(
			"OpenAPI_ue_context_relocate_data_parseFromJSON() failed [forward_relocation_request]");
		goto end;
	}
	...
	if(!forward_relocation_request_local_nonprim) {
		ogs_error(
			"OpenAPI_ref_to_binary_data_parseFromJSON failed [forward_relocation_request]");
		goto end;
	}
	...
	if(pdu_session_list) {
		...
		if(!cJSON_IsArray(pdu_session_list)) {
			ogs_error(
				"OpenAPI_ue_context_relocate_data_parseFromJSON() failed [pdu_session_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(pdu_session_list_local, pdu_session_list) {
			...
			OpenAPI_list_add(pdu_session_listList,
			                 pdu_session_listItem);
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_relocate_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}

	ue_context_relocate_data_local_var =
		OpenAPI_ue_context_relocate_data_create(
...
OpenAPI_ue_context_relocate_data_t *OpenAPI_ue_context_relocate_data_copy(
	OpenAPI_ue_context_relocate_data_t	*dst,
	OpenAPI_ue_context_relocate_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ue_context_relocate_data_convertToJSON() failed");
		return NULL;
	}
