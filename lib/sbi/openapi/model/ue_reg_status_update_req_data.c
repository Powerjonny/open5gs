...
OpenAPI_ue_reg_status_update_req_data_create(
	OpenAPI_ue_context_transfer_status_e transfer_status,
	OpenAPI_list_t *to_release_session_list, bool is_pcf_reselected_ind,
	int pcf_reselected_ind, OpenAPI_list_t *smf_change_info_list,
	OpenAPI_list_t *analytics_not_used_list,
	OpenAPI_list_t *to_release_session_info) {
	OpenAPI_ue_reg_status_update_req_data_t *
	        ue_reg_status_update_req_data_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_reg_status_update_req_data_t));
...
void OpenAPI_ue_reg_status_update_req_data_free(
...
cJSON *OpenAPI_ue_reg_status_update_req_data_convertToJSON(
	OpenAPI_ue_reg_status_update_req_data_t *ue_reg_status_update_req_data)
{
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(ue_reg_status_update_req_data == NULL) {
		ogs_error(
			"OpenAPI_ue_reg_status_update_req_data_convertToJSON() failed [UeRegStatusUpdateReqData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(ue_reg_status_update_req_data->transfer_status ==
	   OpenAPI_ue_context_transfer_status_NULL)
	{
		ogs_error(
			"OpenAPI_ue_reg_status_update_req_data_convertToJSON() failed [transfer_status]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "transferStatus",
	                           OpenAPI_ue_context_transfer_status_ToString(
	...
	if(ue_reg_status_update_req_data->to_release_session_list) {
		...
		if(to_release_session_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_reg_status_update_req_data_convertToJSON() failed [to_release_session_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			ue_reg_status_update_req_data->to_release_session_list,
	...
	if(ue_reg_status_update_req_data->is_pcf_reselected_ind) {
		...
		{
			ogs_error(
				"OpenAPI_ue_reg_status_update_req_data_convertToJSON() failed [pcf_reselected_ind]");
			goto end;
		}
	}
	...
	if(ue_reg_status_update_req_data->smf_change_info_list) {
		...
		if(smf_change_info_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_reg_status_update_req_data_convertToJSON() failed [smf_change_info_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			ue_reg_status_update_req_data->smf_change_info_list,
	...
	if(ue_reg_status_update_req_data->analytics_not_used_list) {
		cJSON *analytics_not_used_listList =
			cJSON_AddArrayToObject(item, "analyticsNotUsedList");
		if(analytics_not_used_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_reg_status_update_req_data_convertToJSON() failed [analytics_not_used_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			ue_reg_status_update_req_data->analytics_not_used_list,
	...
	if(ue_reg_status_update_req_data->to_release_session_info) {
		cJSON *to_release_session_infoList =
			cJSON_AddArrayToObject(item, "toReleaseSessionInfo");
		if(to_release_session_infoList == NULL) {
			ogs_error(
				"OpenAPI_ue_reg_status_update_req_data_convertToJSON() failed [to_release_session_info]");
			goto end;
		}
		OpenAPI_list_for_each(
			ue_reg_status_update_req_data->to_release_session_info,
			node) {
			cJSON *itemLocal =
				OpenAPI_release_session_info_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_ue_reg_status_update_req_data_convertToJSON() failed [to_release_session_info]");
...
OpenAPI_ue_reg_status_update_req_data_parseFromJSON(
	...
	OpenAPI_ue_reg_status_update_req_data_t *
	        ue_reg_status_update_req_data_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(!transfer_status) {
		ogs_error(
			"OpenAPI_ue_reg_status_update_req_data_parseFromJSON() failed [transfer_status]");
		goto end;
	}
	...
	to_release_session_list =
		cJSON_GetObjectItemCaseSensitive(
			ue_reg_status_update_req_dataJSON,
	...
	if(to_release_session_list) {
		...
		if(!cJSON_IsArray(to_release_session_list)) {
			ogs_error(
				"OpenAPI_ue_reg_status_update_req_data_parseFromJSON() failed [to_release_session_list]");
			goto end;
		}
	...
	if(pcf_reselected_ind) {
		if(!cJSON_IsBool(pcf_reselected_ind)) {
			ogs_error(
				"OpenAPI_ue_reg_status_update_req_data_parseFromJSON() failed [pcf_reselected_ind]");
			goto end;
		}
	}
	...
	if(smf_change_info_list) {
		...
		if(!cJSON_IsArray(smf_change_info_list)) {
			ogs_error(
				"OpenAPI_ue_reg_status_update_req_data_parseFromJSON() failed [smf_change_info_list]");
			goto end;
		}
	...
	if(analytics_not_used_list) {
		...
		if(!cJSON_IsArray(analytics_not_used_list)) {
			ogs_error(
				"OpenAPI_ue_reg_status_update_req_data_parseFromJSON() failed [analytics_not_used_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(analytics_not_used_list_local,
			...
			OpenAPI_list_add(analytics_not_used_listList,
			                 ogs_strdup(
						 analytics_not_used_list_local->
						 valuestring));
		}
	}
	...
	if(to_release_session_info) {
		...
		if(!cJSON_IsArray(to_release_session_info)) {
			ogs_error(
				"OpenAPI_ue_reg_status_update_req_data_parseFromJSON() failed [to_release_session_info]");
			goto end;
		}
		...
		cJSON_ArrayForEach(to_release_session_info_local,
			...
			OpenAPI_list_add(to_release_session_infoList,
			                 to_release_session_infoItem);
		}
	}
	...
	ue_reg_status_update_req_data_local_var =
		OpenAPI_ue_reg_status_update_req_data_create(
			...
			analytics_not_used_list ? analytics_not_used_listList :
			NULL,
			to_release_session_info ? to_release_session_infoList :
			NULL
			);
...
OpenAPI_ue_reg_status_update_req_data_t *
OpenAPI_ue_reg_status_update_req_data_copy(
	OpenAPI_ue_reg_status_update_req_data_t *dst,
	OpenAPI_ue_reg_status_update_req_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ue_reg_status_update_req_data_convertToJSON() failed");
		return NULL;
	}
