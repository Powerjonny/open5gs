...
OpenAPI_nssai_ack_data_t *OpenAPI_nssai_ack_data_create(
	...
	ue_update_status) {
	OpenAPI_nssai_ack_data_t *nssai_ack_data_local_var =
		ogs_malloc(sizeof(OpenAPI_nssai_ack_data_t));
...
cJSON *OpenAPI_nssai_ack_data_convertToJSON(
	...
	if(nssai_ack_data == NULL) {
		ogs_error(
			"OpenAPI_nssai_ack_data_convertToJSON() failed [NssaiAckData]");
		return NULL;
	}
	...
	if(!nssai_ack_data->provisioning_time) {
		ogs_error(
			"OpenAPI_nssai_ack_data_convertToJSON() failed [provisioning_time]");
		return NULL;
	}
	...
	if(nssai_ack_data->ue_update_status == OpenAPI_ue_update_status_NULL) {
		ogs_error(
			"OpenAPI_nssai_ack_data_convertToJSON() failed [ue_update_status]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "ueUpdateStatus",
	                           OpenAPI_ue_update_status_ToString(
					   nssai_ack_data->ue_update_status)) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_nssai_ack_data_convertToJSON() failed [ue_update_status]");
...
OpenAPI_nssai_ack_data_t *OpenAPI_nssai_ack_data_parseFromJSON(
	...
	provisioning_time = cJSON_GetObjectItemCaseSensitive(nssai_ack_dataJSON,
	                                                     "provisioningTime");
	if(!provisioning_time) {
		ogs_error(
			"OpenAPI_nssai_ack_data_parseFromJSON() failed [provisioning_time]");
		goto end;
	}
	...
	ue_update_status = cJSON_GetObjectItemCaseSensitive(nssai_ack_dataJSON,
	                                                    "ueUpdateStatus");
	if(!ue_update_status) {
		ogs_error(
			"OpenAPI_nssai_ack_data_parseFromJSON() failed [ue_update_status]");
		goto end;
	}
