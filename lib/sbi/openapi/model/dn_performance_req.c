...
OpenAPI_dn_performance_req_t *OpenAPI_dn_performance_req_create(
	...
	OpenAPI_dn_performance_req_t *dn_performance_req_local_var =
		ogs_malloc(sizeof(OpenAPI_dn_performance_req_t));
...
void OpenAPI_dn_performance_req_free(
	...
	if(dn_performance_req->dn_perf_order_criter) {
		OpenAPI_dn_perf_ordering_criterion_free(
	...
	if(dn_performance_req->report_thresholds) {
		OpenAPI_list_for_each(dn_performance_req->report_thresholds,
		                      node) {
...
cJSON *OpenAPI_dn_performance_req_convertToJSON(
	...
	if(dn_performance_req == NULL) {
		ogs_error(
			"OpenAPI_dn_performance_req_convertToJSON() failed [DnPerformanceReq]");
		return NULL;
	}
	...
	if(dn_performance_req->dn_perf_order_criter) {
		cJSON *dn_perf_order_criter_local_JSON =
			OpenAPI_dn_perf_ordering_criterion_convertToJSON(
				dn_performance_req->dn_perf_order_criter);
		if(dn_perf_order_criter_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_dn_performance_req_convertToJSON() failed [dn_perf_order_criter]");
			goto end;
		}
	...
	if(dn_performance_req->order) {
		...
		if(order_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_dn_performance_req_convertToJSON() failed [order]");
			goto end;
		}
	...
	if(dn_performance_req->report_thresholds) {
		...
		if(report_thresholdsList == NULL) {
			ogs_error(
				"OpenAPI_dn_performance_req_convertToJSON() failed [report_thresholds]");
			goto end;
		}
		OpenAPI_list_for_each(dn_performance_req->report_thresholds,
		                      node) {
...
OpenAPI_dn_performance_req_t *OpenAPI_dn_performance_req_parseFromJSON(
	...
	OpenAPI_dn_perf_ordering_criterion_t *dn_perf_order_criter_local_nonprim
	        = NULL;
	cJSON *order = NULL;
	...
	if(dn_perf_order_criter) {
		...
		if(!dn_perf_order_criter_local_nonprim) {
			ogs_error(
				"OpenAPI_dn_perf_ordering_criterion_parseFromJSON failed [dn_perf_order_criter]");
			goto end;
		}
	}

	order = cJSON_GetObjectItemCaseSensitive(dn_performance_reqJSON,
	                                         "order");
	if(order) {
		order_local_nonprim =
	...
	if(report_thresholds) {
		...
		if(!cJSON_IsArray(report_thresholds)) {
			ogs_error(
				"OpenAPI_dn_performance_req_parseFromJSON() failed [report_thresholds]");
			goto end;
		}
		...
		cJSON_ArrayForEach(report_thresholds_local, report_thresholds) {
			...
			OpenAPI_threshold_level_t *report_thresholdsItem =
			...
			OpenAPI_list_add(report_thresholdsList,
			                 report_thresholdsItem);
		}
	}

	dn_performance_req_local_var = OpenAPI_dn_performance_req_create(
		dn_perf_order_criter ? dn_perf_order_criter_local_nonprim :
		NULL,
