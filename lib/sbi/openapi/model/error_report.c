...
OpenAPI_error_report_t *OpenAPI_error_report_create(
	OpenAPI_problem_details_t *error, OpenAPI_list_t *rule_reports,
	OpenAPI_list_t *sess_rule_reports,
	OpenAPI_list_t *pol_dec_failure_reports,
	OpenAPI_list_t *invalid_policy_decs) {
	OpenAPI_error_report_t *error_report_local_var =
		ogs_malloc(sizeof(OpenAPI_error_report_t));
	...
	error_report_local_var->pol_dec_failure_reports =
		pol_dec_failure_reports;
...
cJSON *OpenAPI_error_report_convertToJSON(
	OpenAPI_error_report_t *error_report) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(error_report == NULL) {
		ogs_error(
			"OpenAPI_error_report_convertToJSON() failed [ErrorReport]");
		return NULL;
	}
	...
	if(error_report->error) {
		cJSON *error_local_JSON =
			OpenAPI_problem_details_convertToJSON(
				error_report->error);
		if(error_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_error_report_convertToJSON() failed [error]");
			goto end;
		}
	...
	if(error_report->rule_reports) {
		...
		if(rule_reportsList == NULL) {
			ogs_error(
				"OpenAPI_error_report_convertToJSON() failed [rule_reports]");
			goto end;
		}
		OpenAPI_list_for_each(error_report->rule_reports, node) {
			cJSON *itemLocal =
	...
	if(error_report->sess_rule_reports) {
		...
		if(sess_rule_reportsList == NULL) {
			ogs_error(
				"OpenAPI_error_report_convertToJSON() failed [sess_rule_reports]");
			goto end;
		}
		OpenAPI_list_for_each(error_report->sess_rule_reports, node) {
			cJSON *itemLocal =
	...
	{
		cJSON *pol_dec_failure_reportsList =
			cJSON_AddArrayToObject(item, "polDecFailureReports");
		if(pol_dec_failure_reportsList == NULL) {
			ogs_error(
				"OpenAPI_error_report_convertToJSON() failed [pol_dec_failure_reports]");
			goto end;
		}
		OpenAPI_list_for_each(error_report->pol_dec_failure_reports,
		                      node) {
			if(cJSON_AddStringToObject(pol_dec_failure_reportsList,
			                           "",
	...
	if(error_report->invalid_policy_decs) {
		...
		if(invalid_policy_decsList == NULL) {
			ogs_error(
				"OpenAPI_error_report_convertToJSON() failed [invalid_policy_decs]");
			goto end;
		}
		OpenAPI_list_for_each(error_report->invalid_policy_decs, node) {
			cJSON *itemLocal =
...
OpenAPI_error_report_t *OpenAPI_error_report_parseFromJSON(
	cJSON *error_reportJSON) {
	...
	if(error) {
		error_local_nonprim =
			OpenAPI_problem_details_parseFromJSON(error);
		if(!error_local_nonprim) {
			ogs_error(
				"OpenAPI_problem_details_parseFromJSON failed [error]");
			goto end;
		}
	}

	rule_reports = cJSON_GetObjectItemCaseSensitive(error_reportJSON,
	                                                "ruleReports");
	if(rule_reports) {
		cJSON *rule_reports_local = NULL;
		if(!cJSON_IsArray(rule_reports)) {
			ogs_error(
				"OpenAPI_error_report_parseFromJSON() failed [rule_reports]");
			goto end;
		}
	...
	sess_rule_reports = cJSON_GetObjectItemCaseSensitive(error_reportJSON,
	                                                     "sessRuleReports");
	if(sess_rule_reports) {
		cJSON *sess_rule_reports_local = NULL;
		if(!cJSON_IsArray(sess_rule_reports)) {
			ogs_error(
				"OpenAPI_error_report_parseFromJSON() failed [sess_rule_reports]");
			goto end;
		}
	...
	pol_dec_failure_reports =
		cJSON_GetObjectItemCaseSensitive(error_reportJSON,
	...
	if(pol_dec_failure_reports) {
		...
		if(!cJSON_IsArray(pol_dec_failure_reports)) {
			ogs_error(
				"OpenAPI_error_report_parseFromJSON() failed [pol_dec_failure_reports]");
			goto end;
		}

		pol_dec_failure_reportsList = OpenAPI_list_create();

		cJSON_ArrayForEach(pol_dec_failure_reports_local,
		                   pol_dec_failure_reports) {
			...
			if(!localEnum) {
				ogs_info(
					"Enum value \"%s\" for field \"pol_dec_failure_reports\" is not supported. Ignoring it ...",
					pol_dec_failure_reports_local->
					valuestring);
			} else {
				OpenAPI_list_add(pol_dec_failure_reportsList,
				                 (void *) localEnum);
			}
		}
		if(pol_dec_failure_reportsList->count == 0) {
			ogs_error(
				"OpenAPI_error_report_parseFromJSON() failed: Expected pol_dec_failure_reportsList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}

	invalid_policy_decs = cJSON_GetObjectItemCaseSensitive(error_reportJSON,
	                                                       "invalidPolicyDecs");
	if(invalid_policy_decs) {
		cJSON *invalid_policy_decs_local = NULL;
		if(!cJSON_IsArray(invalid_policy_decs)) {
			ogs_error(
				"OpenAPI_error_report_parseFromJSON() failed [invalid_policy_decs]");
			goto end;
		}
...
OpenAPI_error_report_t *OpenAPI_error_report_copy(
	OpenAPI_error_report_t	*dst,
