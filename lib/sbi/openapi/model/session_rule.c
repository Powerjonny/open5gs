...
OpenAPI_session_rule_t *OpenAPI_session_rule_create(
	...
	OpenAPI_session_rule_t *session_rule_local_var =
		ogs_malloc(sizeof(OpenAPI_session_rule_t));
	...
	session_rule_local_var->is_ref_um_n3g_data_null =
		is_ref_um_n3g_data_null;
...
cJSON *OpenAPI_session_rule_convertToJSON(
	...
	if(session_rule == NULL) {
		ogs_error(
			"OpenAPI_session_rule_convertToJSON() failed [SessionRule]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(session_rule->auth_sess_ambr) {
		cJSON *auth_sess_ambr_local_JSON =
			OpenAPI_ambr_convertToJSON(
				session_rule->auth_sess_ambr);
		if(auth_sess_ambr_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_session_rule_convertToJSON() failed [auth_sess_ambr]");
			goto end;
		}
		cJSON_AddItemToObject(item, "authSessAmbr",
		                      auth_sess_ambr_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_session_rule_convertToJSON() failed [auth_sess_ambr]");
			goto end;
	...
	if(session_rule->auth_def_qos) {
		cJSON *auth_def_qos_local_JSON =
			OpenAPI_authorized_default_qos_convertToJSON(
				session_rule->auth_def_qos);
		if(auth_def_qos_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_session_rule_convertToJSON() failed [auth_def_qos]");
			goto end;
		}
		cJSON_AddItemToObject(item, "authDefQos",
		                      auth_def_qos_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_session_rule_convertToJSON() failed [auth_def_qos]");
			goto end;
	...
	if(!session_rule->sess_rule_id) {
		ogs_error(
			"OpenAPI_session_rule_convertToJSON() failed [sess_rule_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "sessRuleId",
	                           session_rule->sess_rule_id) == NULL)
	{
		ogs_error(
			"OpenAPI_session_rule_convertToJSON() failed [sess_rule_id]");
	...
	if(session_rule->ref_um_data) {
		if(cJSON_AddStringToObject(item, "refUmData",
		                           session_rule->ref_um_data) == NULL)
		{
			ogs_error(
				"OpenAPI_session_rule_convertToJSON() failed [ref_um_data]");
			goto end;
		}
	...
	if(session_rule->ref_um_n3g_data) {
		if(cJSON_AddStringToObject(item, "refUmN3gData",
		                           session_rule->ref_um_n3g_data) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_session_rule_convertToJSON() failed [ref_um_n3g_data]");
			goto end;
		}
	...
	if(session_rule->ref_cond_data) {
		...
		{
			ogs_error(
				"OpenAPI_session_rule_convertToJSON() failed [ref_cond_data]");
			goto end;
		}
...
OpenAPI_session_rule_t *OpenAPI_session_rule_parseFromJSON(
	...
	auth_sess_ambr = cJSON_GetObjectItemCaseSensitive(session_ruleJSON,
	                                                  "authSessAmbr");
	if(auth_sess_ambr) {
		auth_sess_ambr_local_nonprim =
			OpenAPI_ambr_parseFromJSON(auth_sess_ambr);
		if(!auth_sess_ambr_local_nonprim) {
			ogs_error(
				"OpenAPI_ambr_parseFromJSON failed [auth_sess_ambr]");
			goto end;
		}
	}

	auth_def_qos = cJSON_GetObjectItemCaseSensitive(session_ruleJSON,
	                                                "authDefQos");
	if(auth_def_qos) {
		auth_def_qos_local_nonprim =
			OpenAPI_authorized_default_qos_parseFromJSON(
				auth_def_qos);
		if(!auth_def_qos_local_nonprim) {
			ogs_error(
				"OpenAPI_authorized_default_qos_parseFromJSON failed [auth_def_qos]");
			goto end;
		}
	}

	sess_rule_id = cJSON_GetObjectItemCaseSensitive(session_ruleJSON,
	                                                "sessRuleId");
	if(!sess_rule_id) {
		ogs_error(
			"OpenAPI_session_rule_parseFromJSON() failed [sess_rule_id]");
		goto end;
	}
	...
	ref_um_data = cJSON_GetObjectItemCaseSensitive(session_ruleJSON,
	                                               "refUmData");
	if(ref_um_data) {
		if(!cJSON_IsNull(ref_um_data)) {
			if(!cJSON_IsString(ref_um_data) &&
			   !cJSON_IsNull(ref_um_data))
			{
				ogs_error(
					"OpenAPI_session_rule_parseFromJSON() failed [ref_um_data]");
				goto end;
			}
		}
	}

	ref_um_n3g_data = cJSON_GetObjectItemCaseSensitive(session_ruleJSON,
	                                                   "refUmN3gData");
	if(ref_um_n3g_data) {
		if(!cJSON_IsNull(ref_um_n3g_data)) {
			if(!cJSON_IsString(ref_um_n3g_data) &&
			   !cJSON_IsNull(ref_um_n3g_data))
			{
				ogs_error(
					"OpenAPI_session_rule_parseFromJSON() failed [ref_um_n3g_data]");
				goto end;
			}
		}
	}

	ref_cond_data = cJSON_GetObjectItemCaseSensitive(session_ruleJSON,
	                                                 "refCondData");
	if(ref_cond_data) {
		if(!cJSON_IsNull(ref_cond_data)) {
			...
			{
				ogs_error(
					"OpenAPI_session_rule_parseFromJSON() failed [ref_cond_data]");
				goto end;
			}
		}
	}
	...
	session_rule_local_var = OpenAPI_session_rule_create(
		...
		ref_um_data &&
		!cJSON_IsNull(ref_um_data) ?
		ogs_strdup(ref_um_data->valuestring) : NULL,
		ref_um_n3g_data &&
		cJSON_IsNull(ref_um_n3g_data) ? true : false,
		ref_um_n3g_data &&
		!cJSON_IsNull(ref_um_n3g_data) ?
		ogs_strdup(ref_um_n3g_data->valuestring) : NULL,
		ref_cond_data &&
