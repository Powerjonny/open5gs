...
OpenAPI_ncgi_tai_t *OpenAPI_ncgi_tai_create(OpenAPI_tai_t	*tai,
	...
	OpenAPI_ncgi_tai_t *ncgi_tai_local_var =
		ogs_malloc(sizeof(OpenAPI_ncgi_tai_t));
...
cJSON *OpenAPI_ncgi_tai_convertToJSON(OpenAPI_ncgi_tai_t *ncgi_tai) {
	...
	if(!ncgi_tai->cell_list) {
		ogs_error(
			"OpenAPI_ncgi_tai_convertToJSON() failed [cell_list]");
		return NULL;
	}
...
OpenAPI_ncgi_tai_t *OpenAPI_ncgi_tai_parseFromJSON(cJSON *ncgi_taiJSON) {
	...
	if(!cell_list) {
		ogs_error(
			"OpenAPI_ncgi_tai_parseFromJSON() failed [cell_list]");
		goto end;
	}
	...
	cJSON_ArrayForEach(cell_list_local, cell_list) {
		...
		OpenAPI_ncgi_t *cell_listItem =
			OpenAPI_ncgi_parseFromJSON(cell_list_local);
		if(!cell_listItem) {
