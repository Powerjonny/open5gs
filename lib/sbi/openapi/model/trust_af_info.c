...
OpenAPI_trust_af_info_t *OpenAPI_trust_af_info_create(
	OpenAPI_list_t *s_nssai_info_list, OpenAPI_list_t *af_events,
	OpenAPI_list_t *app_ids, OpenAPI_list_t *internal_group_id,
	bool is_mapping_ind, int mapping_ind) {
	OpenAPI_trust_af_info_t *trust_af_info_local_var =
		ogs_malloc(sizeof(OpenAPI_trust_af_info_t));
...
cJSON *OpenAPI_trust_af_info_convertToJSON(
	...
	if(trust_af_info == NULL) {
		ogs_error(
			"OpenAPI_trust_af_info_convertToJSON() failed [TrustAfInfo]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(trust_af_info->s_nssai_info_list) {
		cJSON *s_nssai_info_listList = cJSON_AddArrayToObject(item,
		                                                      "sNssaiInfoList");
		if(s_nssai_info_listList == NULL) {
			ogs_error(
				"OpenAPI_trust_af_info_convertToJSON() failed [s_nssai_info_list]");
			goto end;
		}
		OpenAPI_list_for_each(trust_af_info->s_nssai_info_list, node) {
			cJSON *itemLocal =
				OpenAPI_snssai_info_item_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_trust_af_info_convertToJSON() failed [s_nssai_info_list]");
	...
	if(trust_af_info->af_events != OpenAPI_af_event_NULL) {
		...
		if(af_eventsList == NULL) {
			ogs_error(
				"OpenAPI_trust_af_info_convertToJSON() failed [af_events]");
			goto end;
		}
	...
	if(trust_af_info->app_ids) {
		...
		if(app_idsList == NULL) {
			ogs_error(
				"OpenAPI_trust_af_info_convertToJSON() failed [app_ids]");
			goto end;
		}
		OpenAPI_list_for_each(trust_af_info->app_ids, node) {
	...
	if(trust_af_info->internal_group_id) {
		cJSON *internal_group_idList = cJSON_AddArrayToObject(item,
		                                                      "internalGroupId");
		if(internal_group_idList == NULL) {
			ogs_error(
				"OpenAPI_trust_af_info_convertToJSON() failed [internal_group_id]");
			goto end;
		}
		OpenAPI_list_for_each(trust_af_info->internal_group_id, node) {
			if(cJSON_AddStringToObject(internal_group_idList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_trust_af_info_convertToJSON() failed [internal_group_id]");
	...
	if(trust_af_info->is_mapping_ind) {
		if(cJSON_AddBoolToObject(item, "mappingInd",
		                         trust_af_info->mapping_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_trust_af_info_convertToJSON() failed [mapping_ind]");
			goto end;
		}
	}
...
OpenAPI_trust_af_info_t *OpenAPI_trust_af_info_parseFromJSON(
	cJSON *trust_af_infoJSON) {
	...
	s_nssai_info_list = cJSON_GetObjectItemCaseSensitive(trust_af_infoJSON,
	                                                     "sNssaiInfoList");
	if(s_nssai_info_list) {
		cJSON *s_nssai_info_list_local = NULL;
		if(!cJSON_IsArray(s_nssai_info_list)) {
			ogs_error(
				"OpenAPI_trust_af_info_parseFromJSON() failed [s_nssai_info_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(s_nssai_info_list_local, s_nssai_info_list) {
			...
			OpenAPI_snssai_info_item_t *s_nssai_info_listItem =
				OpenAPI_snssai_info_item_parseFromJSON(
					s_nssai_info_list_local);
			if(!s_nssai_info_listItem) {
				ogs_error("No s_nssai_info_listItem");
				goto end;
			}
			OpenAPI_list_add(s_nssai_info_listList,
			                 s_nssai_info_listItem);
		}
	}

	af_events = cJSON_GetObjectItemCaseSensitive(trust_af_infoJSON,
	                                             "afEvents");
	if(af_events) {
		cJSON *af_events_local = NULL;
		if(!cJSON_IsArray(af_events)) {
			ogs_error(
				"OpenAPI_trust_af_info_parseFromJSON() failed [af_events]");
			goto end;
		}
		...
		if(af_eventsList->count == 0) {
			ogs_error(
				"OpenAPI_trust_af_info_parseFromJSON() failed: Expected af_eventsList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(app_ids) {
		...
		if(!cJSON_IsArray(app_ids)) {
			ogs_error(
				"OpenAPI_trust_af_info_parseFromJSON() failed [app_ids]");
			goto end;
		}
	...
	internal_group_id = cJSON_GetObjectItemCaseSensitive(trust_af_infoJSON,
	                                                     "internalGroupId");
	if(internal_group_id) {
		...
		cJSON_ArrayForEach(internal_group_id_local, internal_group_id) {
			...
			OpenAPI_list_add(internal_group_idList,
			                 ogs_strdup(
						 internal_group_id_local->
						 valuestring));
		}
	}

	mapping_ind = cJSON_GetObjectItemCaseSensitive(trust_af_infoJSON,
	                                               "mappingInd");
	if(mapping_ind) {
		if(!cJSON_IsBool(mapping_ind)) {
			ogs_error(
				"OpenAPI_trust_af_info_parseFromJSON() failed [mapping_ind]");
			goto end;
		}
	}
...
OpenAPI_trust_af_info_t *OpenAPI_trust_af_info_copy(
	OpenAPI_trust_af_info_t *dst, OpenAPI_trust_af_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
