...
OpenAPI_cm_info_t *OpenAPI_cm_info_create(OpenAPI_cm_state_t	*cm_state,
                                          OpenAPI_access_type_e access_type) {
	OpenAPI_cm_info_t *cm_info_local_var =
		ogs_malloc(sizeof(OpenAPI_cm_info_t));
...
cJSON *OpenAPI_cm_info_convertToJSON(OpenAPI_cm_info_t *cm_info) {
	...
	cJSON *cm_state_local_JSON =
		OpenAPI_cm_state_convertToJSON(cm_info->cm_state);
	...
	if(cm_info->access_type == OpenAPI_access_type_NULL) {
		ogs_error(
			"OpenAPI_cm_info_convertToJSON() failed [access_type]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "accessType",
	                           OpenAPI_access_type_ToString(
					   cm_info->access_type)) == NULL)
	{
		ogs_error(
			"OpenAPI_cm_info_convertToJSON() failed [access_type]");
		goto end;
...
OpenAPI_cm_info_t *OpenAPI_cm_info_parseFromJSON(cJSON *cm_infoJSON) {
	...
	access_type = cJSON_GetObjectItemCaseSensitive(cm_infoJSON,
	                                               "accessType");
	if(!access_type) {
		ogs_error(
			"OpenAPI_cm_info_parseFromJSON() failed [access_type]");
		goto end;
	}
...
OpenAPI_cm_info_t *OpenAPI_cm_info_copy(OpenAPI_cm_info_t	*dst,
                                        OpenAPI_cm_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
