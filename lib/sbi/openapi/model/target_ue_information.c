...
OpenAPI_target_ue_information_t *OpenAPI_target_ue_information_create(
	...
	OpenAPI_target_ue_information_t *target_ue_information_local_var =
		ogs_malloc(sizeof(OpenAPI_target_ue_information_t));
...
void OpenAPI_target_ue_information_free(
	...
	if(target_ue_information->int_group_ids) {
		OpenAPI_list_for_each(target_ue_information->int_group_ids,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_target_ue_information_convertToJSON(
	...
	if(target_ue_information == NULL) {
		ogs_error(
			"OpenAPI_target_ue_information_convertToJSON() failed [TargetUeInformation]");
		return NULL;
	}
	...
	if(target_ue_information->is_any_ue) {
		...
		{
			ogs_error(
				"OpenAPI_target_ue_information_convertToJSON() failed [any_ue]");
			goto end;
		}
	}
	...
	if(target_ue_information->supis) {
		...
		if(supisList == NULL) {
			ogs_error(
				"OpenAPI_target_ue_information_convertToJSON() failed [supis]");
			goto end;
		}
		OpenAPI_list_for_each(target_ue_information->supis, node) {
	...
	if(target_ue_information->gpsis) {
		...
		if(gpsisList == NULL) {
			ogs_error(
				"OpenAPI_target_ue_information_convertToJSON() failed [gpsis]");
			goto end;
		}
		OpenAPI_list_for_each(target_ue_information->gpsis, node) {
	...
	if(target_ue_information->int_group_ids) {
		...
		if(int_group_idsList == NULL) {
			ogs_error(
				"OpenAPI_target_ue_information_convertToJSON() failed [int_group_ids]");
			goto end;
		}
		OpenAPI_list_for_each(target_ue_information->int_group_ids,
		                      node) {
...
OpenAPI_target_ue_information_t *OpenAPI_target_ue_information_parseFromJSON(
	...
	if(any_ue) {
		if(!cJSON_IsBool(any_ue)) {
			ogs_error(
				"OpenAPI_target_ue_information_parseFromJSON() failed [any_ue]");
			goto end;
		}
	}
	...
	if(supis) {
		...
		if(!cJSON_IsArray(supis)) {
			ogs_error(
				"OpenAPI_target_ue_information_parseFromJSON() failed [supis]");
			goto end;
		}
	...
	if(gpsis) {
		...
		if(!cJSON_IsArray(gpsis)) {
			ogs_error(
				"OpenAPI_target_ue_information_parseFromJSON() failed [gpsis]");
			goto end;
		}
	...
	int_group_ids =
		cJSON_GetObjectItemCaseSensitive(target_ue_informationJSON,
		                                 "intGroupIds");
	if(int_group_ids) {
		cJSON *int_group_ids_local = NULL;
		if(!cJSON_IsArray(int_group_ids)) {
			ogs_error(
				"OpenAPI_target_ue_information_parseFromJSON() failed [int_group_ids]");
			goto end;
		}
		...
		cJSON_ArrayForEach(int_group_ids_local, int_group_ids) {
			...
			OpenAPI_list_add(int_group_idsList,
			                 ogs_strdup(
						 int_group_ids_local->
						 valuestring));
		}
...
OpenAPI_target_ue_information_t *OpenAPI_target_ue_information_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_target_ue_information_convertToJSON() failed");
		return NULL;
	}
