...
OpenAPI_sm_context_retrieve_data_t *OpenAPI_sm_context_retrieve_data_create(
	...
	OpenAPI_list_t *not_to_transfer_ebi_list, bool is_ran_unchanged_ind,
	int ran_unchanged_ind) {
	OpenAPI_sm_context_retrieve_data_t *sm_context_retrieve_data_local_var =
		ogs_malloc(sizeof(OpenAPI_sm_context_retrieve_data_t));
	...
	sm_context_retrieve_data_local_var->not_to_transfer_ebi_list =
		not_to_transfer_ebi_list;
	sm_context_retrieve_data_local_var->is_ran_unchanged_ind =
		is_ran_unchanged_ind;
	sm_context_retrieve_data_local_var->ran_unchanged_ind =
		ran_unchanged_ind;

	return sm_context_retrieve_data_local_var;
}

void OpenAPI_sm_context_retrieve_data_free(
	OpenAPI_sm_context_retrieve_data_t *sm_context_retrieve_data) {
	...
	if(sm_context_retrieve_data->target_mme_cap) {
		OpenAPI_mme_capabilities_free(
			sm_context_retrieve_data->target_mme_cap);
...
cJSON *OpenAPI_sm_context_retrieve_data_convertToJSON(
	...
	if(sm_context_retrieve_data == NULL) {
		ogs_error(
			"OpenAPI_sm_context_retrieve_data_convertToJSON() failed [SmContextRetrieveData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(sm_context_retrieve_data->target_mme_cap) {
		cJSON *target_mme_cap_local_JSON =
			OpenAPI_mme_capabilities_convertToJSON(
				sm_context_retrieve_data->target_mme_cap);
		if(target_mme_cap_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_context_retrieve_data_convertToJSON() failed [target_mme_cap]");
			goto end;
		}
		cJSON_AddItemToObject(item, "targetMmeCap",
		                      target_mme_cap_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_sm_context_retrieve_data_convertToJSON() failed [target_mme_cap]");
			goto end;
	...
	if(sm_context_retrieve_data->sm_context_type !=
	   OpenAPI_sm_context_type_NULL)
	{
		if(cJSON_AddStringToObject(item, "smContextType",
		                           OpenAPI_sm_context_type_ToString(
						   sm_context_retrieve_data->
						   sm_context_type)) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_retrieve_data_convertToJSON() failed [sm_context_type]");
			goto end;
		}
	}
	...
	if(sm_context_retrieve_data->serving_network) {
		...
		if(serving_network_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_context_retrieve_data_convertToJSON() failed [serving_network]");
			goto end;
		}
	...
	if(sm_context_retrieve_data->not_to_transfer_ebi_list) {
		cJSON *not_to_transfer_ebi_listList =
			cJSON_AddArrayToObject(item, "notToTransferEbiList");
		if(not_to_transfer_ebi_listList == NULL) {
			ogs_error(
				"OpenAPI_sm_context_retrieve_data_convertToJSON() failed [not_to_transfer_ebi_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			sm_context_retrieve_data->not_to_transfer_ebi_list,
			...
			if(cJSON_AddNumberToObject(not_to_transfer_ebi_listList,
			                           "",
			                           *(double *) node->data) ==
			   NULL)
			{
	...
	if(sm_context_retrieve_data->is_ran_unchanged_ind) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_retrieve_data_convertToJSON() failed [ran_unchanged_ind]");
			goto end;
		}
	}
...
OpenAPI_sm_context_retrieve_data_parseFromJSON(
	cJSON *sm_context_retrieve_dataJSON) {
	OpenAPI_sm_context_retrieve_data_t *sm_context_retrieve_data_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	target_mme_cap =
		cJSON_GetObjectItemCaseSensitive(sm_context_retrieve_dataJSON,
		                                 "targetMmeCap");
	if(target_mme_cap) {
		target_mme_cap_local_nonprim =
			OpenAPI_mme_capabilities_parseFromJSON(target_mme_cap);
		if(!target_mme_cap_local_nonprim) {
			ogs_error(
				"OpenAPI_mme_capabilities_parseFromJSON failed [target_mme_cap]");
			goto end;
		}
	}
	...
	if(sm_context_type) {
		if(!cJSON_IsString(sm_context_type)) {
			ogs_error(
				"OpenAPI_sm_context_retrieve_data_parseFromJSON() failed [sm_context_type]");
			goto end;
		}
		sm_context_typeVariable =
			OpenAPI_sm_context_type_FromString(
				sm_context_type->valuestring);
	}
	...
	if(serving_network) {
		serving_network_local_nonprim =
			OpenAPI_plmn_id_parseFromJSON(serving_network);
		if(!serving_network_local_nonprim) {
			ogs_error(
				"OpenAPI_plmn_id_parseFromJSON failed [serving_network]");
			goto end;
		}
	}
	...
	if(not_to_transfer_ebi_list) {
		...
		if(!cJSON_IsArray(not_to_transfer_ebi_list)) {
			ogs_error(
				"OpenAPI_sm_context_retrieve_data_parseFromJSON() failed [not_to_transfer_ebi_list]");
			goto end;
		}

		not_to_transfer_ebi_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(not_to_transfer_ebi_list_local,
		                   not_to_transfer_ebi_list) {
			...
			*localDouble =
				not_to_transfer_ebi_list_local->valuedouble;
			OpenAPI_list_add(not_to_transfer_ebi_listList,
			                 localDouble);
		}
	}
	...
	if(ran_unchanged_ind) {
		if(!cJSON_IsBool(ran_unchanged_ind)) {
			ogs_error(
				"OpenAPI_sm_context_retrieve_data_parseFromJSON() failed [ran_unchanged_ind]");
			goto end;
		}
	}
...
OpenAPI_sm_context_retrieve_data_t *OpenAPI_sm_context_retrieve_data_copy(
	OpenAPI_sm_context_retrieve_data_t	*dst,
	OpenAPI_sm_context_retrieve_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_sm_context_retrieve_data_convertToJSON() failed");
		return NULL;
	}
