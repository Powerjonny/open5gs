...
OpenAPI_access_and_mobility_data_t *OpenAPI_access_and_mobility_data_create(
	OpenAPI_user_location_t *location, char *location_ts, char *time_zone,
	char *time_zone_ts, OpenAPI_access_type_e access_type,
	OpenAPI_list_t *reg_states, char *reg_states_ts,
	OpenAPI_list_t *conn_states, char *conn_states_ts,
	OpenAPI_ue_reachability_t *reachability_status,
	char *reachability_status_ts, OpenAPI_sms_support_e sms_over_nas_status,
	char *sms_over_nas_status_ts, bool is_roaming_status,
	int roaming_status, char *roaming_status_ts,
	OpenAPI_plmn_id_1_t *current_plmn, char *current_plmn_ts,
	OpenAPI_list_t *rat_type, char *rat_types_ts, char *supp_feat,
	OpenAPI_list_t *reset_ids) {
	OpenAPI_access_and_mobility_data_t *access_and_mobility_data_local_var =
		ogs_malloc(sizeof(OpenAPI_access_and_mobility_data_t));
	...
	access_and_mobility_data_local_var->reachability_status =
		reachability_status;
	access_and_mobility_data_local_var->reachability_status_ts =
		reachability_status_ts;
	access_and_mobility_data_local_var->sms_over_nas_status =
		sms_over_nas_status;
	access_and_mobility_data_local_var->sms_over_nas_status_ts =
		sms_over_nas_status_ts;
	access_and_mobility_data_local_var->is_roaming_status =
		is_roaming_status;
	access_and_mobility_data_local_var->roaming_status = roaming_status;
	access_and_mobility_data_local_var->roaming_status_ts =
		roaming_status_ts;
...
void OpenAPI_access_and_mobility_data_free(
	...
	if(access_and_mobility_data->conn_states) {
		OpenAPI_list_for_each(access_and_mobility_data->conn_states,
		                      node) {
	...
	if(access_and_mobility_data->reachability_status) {
		OpenAPI_ue_reachability_free(
			access_and_mobility_data->reachability_status);
	...
	if(access_and_mobility_data->reset_ids) {
		OpenAPI_list_for_each(access_and_mobility_data->reset_ids,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_access_and_mobility_data_convertToJSON(
	...
	if(access_and_mobility_data == NULL) {
		ogs_error(
			"OpenAPI_access_and_mobility_data_convertToJSON() failed [AccessAndMobilityData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(access_and_mobility_data->location) {
		cJSON *location_local_JSON =
			OpenAPI_user_location_convertToJSON(
				access_and_mobility_data->location);
		if(location_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [location]");
			goto end;
		}
	...
	if(access_and_mobility_data->location_ts) {
		if(cJSON_AddStringToObject(item, "locationTs",
		                           access_and_mobility_data->location_ts)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [location_ts]");
			goto end;
		}
	}
	...
	if(access_and_mobility_data->time_zone) {
		if(cJSON_AddStringToObject(item, "timeZone",
		                           access_and_mobility_data->time_zone)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [time_zone]");
			goto end;
		}
	}

	if(access_and_mobility_data->time_zone_ts) {
		if(cJSON_AddStringToObject(item, "timeZoneTs",
		                           access_and_mobility_data->
		                           time_zone_ts) == NULL)
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [time_zone_ts]");
			goto end;
		}
	}
	...
	if(access_and_mobility_data->access_type != OpenAPI_access_type_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [access_type]");
			goto end;
		}
	}

	if(access_and_mobility_data->reg_states) {
		cJSON *reg_statesList = cJSON_AddArrayToObject(item,
		                                               "regStates");
		if(reg_statesList == NULL) {
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [reg_states]");
			goto end;
		}
		OpenAPI_list_for_each(access_and_mobility_data->reg_states,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_rm_info_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_access_and_mobility_data_convertToJSON() failed [reg_states]");
				goto end;
	...
	if(access_and_mobility_data->reg_states_ts) {
		if(cJSON_AddStringToObject(item, "regStatesTs",
		                           access_and_mobility_data->
		                           reg_states_ts) == NULL)
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [reg_states_ts]");
			goto end;
		}
	}

	if(access_and_mobility_data->conn_states) {
		cJSON *conn_statesList = cJSON_AddArrayToObject(item,
		                                                "connStates");
		if(conn_statesList == NULL) {
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [conn_states]");
			goto end;
		}
		OpenAPI_list_for_each(access_and_mobility_data->conn_states,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_cm_info_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_access_and_mobility_data_convertToJSON() failed [conn_states]");
				goto end;
	...
	if(access_and_mobility_data->conn_states_ts) {
		if(cJSON_AddStringToObject(item, "connStatesTs",
		                           access_and_mobility_data->
		                           conn_states_ts) == NULL)
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [conn_states_ts]");
			goto end;
		}
	}

	if(access_and_mobility_data->reachability_status) {
		cJSON *reachability_status_local_JSON =
			OpenAPI_ue_reachability_convertToJSON(
				access_and_mobility_data->reachability_status);
		if(reachability_status_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [reachability_status]");
			goto end;
		}
		cJSON_AddItemToObject(item, "reachabilityStatus",
		                      reachability_status_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [reachability_status]");
			goto end;
	...
	if(access_and_mobility_data->reachability_status_ts) {
		if(cJSON_AddStringToObject(item, "reachabilityStatusTs",
		                           access_and_mobility_data->
		                           reachability_status_ts) == NULL)
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [reachability_status_ts]");
			goto end;
		}
	}

	if(access_and_mobility_data->sms_over_nas_status !=
	   OpenAPI_sms_support_NULL)
	{
		if(cJSON_AddStringToObject(item, "smsOverNasStatus",
		                           OpenAPI_sms_support_ToString(
						   access_and_mobility_data->
						   sms_over_nas_status)) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [sms_over_nas_status]");
			goto end;
		}
	}

	if(access_and_mobility_data->sms_over_nas_status_ts) {
		if(cJSON_AddStringToObject(item, "smsOverNasStatusTs",
		                           access_and_mobility_data->
		                           sms_over_nas_status_ts) == NULL)
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [sms_over_nas_status_ts]");
			goto end;
		}
	}

	if(access_and_mobility_data->is_roaming_status) {
		if(cJSON_AddBoolToObject(item, "roamingStatus",
		                         access_and_mobility_data->
		                         roaming_status) == NULL)
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [roaming_status]");
			goto end;
		}
	}

	if(access_and_mobility_data->roaming_status_ts) {
		if(cJSON_AddStringToObject(item, "roamingStatusTs",
		                           access_and_mobility_data->
		                           roaming_status_ts) == NULL)
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [roaming_status_ts]");
			goto end;
		}
	}

	if(access_and_mobility_data->current_plmn) {
		cJSON *current_plmn_local_JSON =
			OpenAPI_plmn_id_1_convertToJSON(
				access_and_mobility_data->current_plmn);
		if(current_plmn_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [current_plmn]");
			goto end;
		}
		cJSON_AddItemToObject(item, "currentPlmn",
		                      current_plmn_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [current_plmn]");
			goto end;
	...
	if(access_and_mobility_data->current_plmn_ts) {
		if(cJSON_AddStringToObject(item, "currentPlmnTs",
		                           access_and_mobility_data->
		                           current_plmn_ts) == NULL)
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [current_plmn_ts]");
			goto end;
		}
	}
	...
	if(access_and_mobility_data->rat_type != OpenAPI_rat_type_NULL) {
		...
		if(rat_typeList == NULL) {
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [rat_type]");
			goto end;
		}
		OpenAPI_list_for_each(access_and_mobility_data->rat_type,
		                      node) {
			if(cJSON_AddStringToObject(rat_typeList, "",
	...
	if(access_and_mobility_data->rat_types_ts) {
		if(cJSON_AddStringToObject(item, "ratTypesTs",
		                           access_and_mobility_data->
		                           rat_types_ts) == NULL)
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [rat_types_ts]");
			goto end;
		}
	}
	...
	if(access_and_mobility_data->supp_feat) {
		if(cJSON_AddStringToObject(item, "suppFeat",
		                           access_and_mobility_data->supp_feat)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [supp_feat]");
			goto end;
		}
	}
	...
	if(access_and_mobility_data->reset_ids) {
		...
		if(reset_idsList == NULL) {
			ogs_error(
				"OpenAPI_access_and_mobility_data_convertToJSON() failed [reset_ids]");
			goto end;
		}
		OpenAPI_list_for_each(access_and_mobility_data->reset_ids,
		                      node) {
...
OpenAPI_access_and_mobility_data_parseFromJSON(
	cJSON *access_and_mobility_dataJSON) {
	OpenAPI_access_and_mobility_data_t *access_and_mobility_data_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *location = NULL;
	...
	if(location) {
		location_local_nonprim =
			OpenAPI_user_location_parseFromJSON(location);
		if(!location_local_nonprim) {
			ogs_error(
				"OpenAPI_user_location_parseFromJSON failed [location]");
			goto end;
		}
	}
	...
	if(location_ts) {
		if(!cJSON_IsString(location_ts) &&
		   !cJSON_IsNull(location_ts))
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_parseFromJSON() failed [location_ts]");
			goto end;
		}
	}
	...
	if(time_zone) {
		...
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_parseFromJSON() failed [time_zone]");
			goto end;
		}
	}
	...
	if(time_zone_ts) {
		if(!cJSON_IsString(time_zone_ts) &&
		   !cJSON_IsNull(time_zone_ts))
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_parseFromJSON() failed [time_zone_ts]");
			goto end;
		}
	}
	...
	if(access_type) {
		if(!cJSON_IsString(access_type)) {
			ogs_error(
				"OpenAPI_access_and_mobility_data_parseFromJSON() failed [access_type]");
			goto end;
		}
		access_typeVariable =
	...
	if(reg_states) {
		...
		cJSON_ArrayForEach(reg_states_local, reg_states) {
			...
			OpenAPI_rm_info_t *reg_statesItem =
				OpenAPI_rm_info_parseFromJSON(reg_states_local);
			if(!reg_statesItem) {
	...
	if(reg_states_ts) {
		if(!cJSON_IsString(reg_states_ts) &&
		   !cJSON_IsNull(reg_states_ts))
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_parseFromJSON() failed [reg_states_ts]");
			goto end;
		}
	}
	...
	if(conn_states) {
		...
		if(!cJSON_IsArray(conn_states)) {
			ogs_error(
				"OpenAPI_access_and_mobility_data_parseFromJSON() failed [conn_states]");
			goto end;
		}
		...
		cJSON_ArrayForEach(conn_states_local, conn_states) {
			...
			OpenAPI_cm_info_t *conn_statesItem =
				OpenAPI_cm_info_parseFromJSON(
					conn_states_local);
			if(!conn_statesItem) {
	...
	if(conn_states_ts) {
		if(!cJSON_IsString(conn_states_ts) &&
		   !cJSON_IsNull(conn_states_ts))
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_parseFromJSON() failed [conn_states_ts]");
			goto end;
		}
	}
	...
	if(reachability_status) {
		reachability_status_local_nonprim =
			OpenAPI_ue_reachability_parseFromJSON(
				reachability_status);
		if(!reachability_status_local_nonprim) {
			ogs_error(
				"OpenAPI_ue_reachability_parseFromJSON failed [reachability_status]");
			goto end;
		}
	}
	...
	if(reachability_status_ts) {
		if(!cJSON_IsString(reachability_status_ts) &&
		   !cJSON_IsNull(reachability_status_ts))
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_parseFromJSON() failed [reachability_status_ts]");
			goto end;
		}
	}
	...
	if(sms_over_nas_status) {
		if(!cJSON_IsString(sms_over_nas_status)) {
			ogs_error(
				"OpenAPI_access_and_mobility_data_parseFromJSON() failed [sms_over_nas_status]");
			goto end;
		}
		sms_over_nas_statusVariable =
			OpenAPI_sms_support_FromString(
				sms_over_nas_status->valuestring);
	}
	...
	if(sms_over_nas_status_ts) {
		if(!cJSON_IsString(sms_over_nas_status_ts) &&
		   !cJSON_IsNull(sms_over_nas_status_ts))
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_parseFromJSON() failed [sms_over_nas_status_ts]");
			goto end;
		}
	}
	...
	if(roaming_status) {
		if(!cJSON_IsBool(roaming_status)) {
			ogs_error(
				"OpenAPI_access_and_mobility_data_parseFromJSON() failed [roaming_status]");
			goto end;
		}
	}
	...
	if(roaming_status_ts) {
		if(!cJSON_IsString(roaming_status_ts) &&
		   !cJSON_IsNull(roaming_status_ts))
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_parseFromJSON() failed [roaming_status_ts]");
			goto end;
		}
	}
	...
	current_plmn =
		cJSON_GetObjectItemCaseSensitive(access_and_mobility_dataJSON,
		                                 "currentPlmn");
	if(current_plmn) {
		current_plmn_local_nonprim =
			OpenAPI_plmn_id_1_parseFromJSON(current_plmn);
		if(!current_plmn_local_nonprim) {
			ogs_error(
				"OpenAPI_plmn_id_1_parseFromJSON failed [current_plmn]");
			goto end;
		}
	}
	...
	if(current_plmn_ts) {
		if(!cJSON_IsString(current_plmn_ts) &&
		   !cJSON_IsNull(current_plmn_ts))
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_parseFromJSON() failed [current_plmn_ts]");
			goto end;
		}
	}
	...
	if(rat_type) {
		...
		if(!cJSON_IsArray(rat_type)) {
			ogs_error(
				"OpenAPI_access_and_mobility_data_parseFromJSON() failed [rat_type]");
			goto end;
		}
		...
		cJSON_ArrayForEach(rat_type_local, rat_type) {
			...
			if(!localEnum) {
				ogs_info(
					"Enum value \"%s\" for field \"rat_type\" is not supported. Ignoring it ...",
					rat_type_local->valuestring);
			} else {
				OpenAPI_list_add(rat_typeList,
				                 (void *) localEnum);
			}
		}
		if(rat_typeList->count == 0) {
			ogs_error(
				"OpenAPI_access_and_mobility_data_parseFromJSON() failed: Expected rat_typeList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(rat_types_ts) {
		if(!cJSON_IsString(rat_types_ts) &&
		   !cJSON_IsNull(rat_types_ts))
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_parseFromJSON() failed [rat_types_ts]");
			goto end;
		}
	}
	...
	if(supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_access_and_mobility_data_parseFromJSON() failed [supp_feat]");
			goto end;
		}
	}
	...
	if(reset_ids) {
		...
		if(!cJSON_IsArray(reset_ids)) {
			ogs_error(
				"OpenAPI_access_and_mobility_data_parseFromJSON() failed [reset_ids]");
			goto end;
		}
	...
	access_and_mobility_data_local_var =
		OpenAPI_access_and_mobility_data_create(
			...
			location_ts &&
			!cJSON_IsNull(location_ts) ?
			ogs_strdup(location_ts->valuestring) : NULL,
			time_zone &&
			!cJSON_IsNull(time_zone) ?
			...
			time_zone_ts &&
			!cJSON_IsNull(time_zone_ts) ?
			ogs_strdup(time_zone_ts->valuestring) : NULL,
			access_type ? access_typeVariable : 0,
			reg_states ? reg_statesList : NULL,
			reg_states_ts &&
			!cJSON_IsNull(reg_states_ts) ?
			ogs_strdup(reg_states_ts->valuestring) : NULL,
			conn_states ? conn_statesList : NULL,
			conn_states_ts &&
			!cJSON_IsNull(conn_states_ts) ?
			ogs_strdup(conn_states_ts->valuestring) : NULL,
			reachability_status ?
			reachability_status_local_nonprim : NULL,
			reachability_status_ts &&
			!cJSON_IsNull(reachability_status_ts) ?
			ogs_strdup(reachability_status_ts->valuestring) : NULL,
			sms_over_nas_status ? sms_over_nas_statusVariable : 0,
			sms_over_nas_status_ts &&
			!cJSON_IsNull(sms_over_nas_status_ts) ?
			ogs_strdup(sms_over_nas_status_ts->valuestring) : NULL,
			roaming_status ? true : false,
			roaming_status ? roaming_status->valueint : 0,
			roaming_status_ts &&
			!cJSON_IsNull(roaming_status_ts) ?
			ogs_strdup(roaming_status_ts->valuestring) : NULL,
			current_plmn ? current_plmn_local_nonprim : NULL,
			current_plmn_ts &&
			!cJSON_IsNull(current_plmn_ts) ?
			ogs_strdup(current_plmn_ts->valuestring) : NULL,
			rat_type ? rat_typeList : NULL,
			rat_types_ts &&
			!cJSON_IsNull(rat_types_ts) ?
			ogs_strdup(rat_types_ts->valuestring) : NULL,
			supp_feat &&
			!cJSON_IsNull(supp_feat) ?
...
OpenAPI_access_and_mobility_data_t *OpenAPI_access_and_mobility_data_copy(
	OpenAPI_access_and_mobility_data_t	*dst,
	OpenAPI_access_and_mobility_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_access_and_mobility_data_convertToJSON() failed");
		return NULL;
	}
