...
OpenAPI_anchor_smf_features_t *OpenAPI_anchor_smf_features_create(
	...
	OpenAPI_anchor_smf_features_t *anchor_smf_features_local_var =
		ogs_malloc(sizeof(OpenAPI_anchor_smf_features_t));
	ogs_assert(anchor_smf_features_local_var);

	anchor_smf_features_local_var->is_psetr_support_ind =
		is_psetr_support_ind;
...
cJSON *OpenAPI_anchor_smf_features_convertToJSON(
	...
	if(anchor_smf_features == NULL) {
		ogs_error(
			"OpenAPI_anchor_smf_features_convertToJSON() failed [AnchorSmfFeatures]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(anchor_smf_features->is_psetr_support_ind) {
		if(cJSON_AddBoolToObject(item, "psetrSupportInd",
		                         anchor_smf_features->psetr_support_ind)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_anchor_smf_features_convertToJSON() failed [psetr_support_ind]");
			goto end;
		}
	}
...
OpenAPI_anchor_smf_features_t *OpenAPI_anchor_smf_features_parseFromJSON(
	...
	psetr_support_ind =
		cJSON_GetObjectItemCaseSensitive(anchor_smf_featuresJSON,
		                                 "psetrSupportInd");
	if(psetr_support_ind) {
		if(!cJSON_IsBool(psetr_support_ind)) {
			ogs_error(
				"OpenAPI_anchor_smf_features_parseFromJSON() failed [psetr_support_ind]");
			goto end;
		}
	}
...
OpenAPI_anchor_smf_features_t *OpenAPI_anchor_smf_features_copy(
	OpenAPI_anchor_smf_features_t	*dst,
	OpenAPI_anchor_smf_features_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
