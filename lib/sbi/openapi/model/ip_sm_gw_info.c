...
OpenAPI_ip_sm_gw_info_t *OpenAPI_ip_sm_gw_info_create(
	...
	OpenAPI_ip_sm_gw_info_t *ip_sm_gw_info_local_var =
		ogs_malloc(sizeof(OpenAPI_ip_sm_gw_info_t));
...
void OpenAPI_ip_sm_gw_info_free(OpenAPI_ip_sm_gw_info_t *ip_sm_gw_info) {
	...
	if(ip_sm_gw_info->ip_sm_gw_registration) {
		OpenAPI_ip_sm_gw_registration_free(
			ip_sm_gw_info->ip_sm_gw_registration);
		ip_sm_gw_info->ip_sm_gw_registration = NULL;
	}
	if(ip_sm_gw_info->ip_sm_gw_guidance) {
		OpenAPI_ip_sm_gw_guidance_free(
			ip_sm_gw_info->ip_sm_gw_guidance);
...
cJSON *OpenAPI_ip_sm_gw_info_convertToJSON(
	...
	if(ip_sm_gw_info == NULL) {
		ogs_error(
			"OpenAPI_ip_sm_gw_info_convertToJSON() failed [IpSmGwInfo]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(ip_sm_gw_info->ip_sm_gw_registration) {
		cJSON *ip_sm_gw_registration_local_JSON =
			OpenAPI_ip_sm_gw_registration_convertToJSON(
				ip_sm_gw_info->ip_sm_gw_registration);
		if(ip_sm_gw_registration_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ip_sm_gw_info_convertToJSON() failed [ip_sm_gw_registration]");
			goto end;
		}
		cJSON_AddItemToObject(item, "ipSmGwRegistration",
		                      ip_sm_gw_registration_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_ip_sm_gw_info_convertToJSON() failed [ip_sm_gw_registration]");
			goto end;
	...
	if(ip_sm_gw_info->ip_sm_gw_guidance) {
		cJSON *ip_sm_gw_guidance_local_JSON =
			OpenAPI_ip_sm_gw_guidance_convertToJSON(
				ip_sm_gw_info->ip_sm_gw_guidance);
		if(ip_sm_gw_guidance_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ip_sm_gw_info_convertToJSON() failed [ip_sm_gw_guidance]");
			goto end;
		}
		cJSON_AddItemToObject(item, "ipSmGwGuidance",
		                      ip_sm_gw_guidance_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_ip_sm_gw_info_convertToJSON() failed [ip_sm_gw_guidance]");
			goto end;
...
OpenAPI_ip_sm_gw_info_t *OpenAPI_ip_sm_gw_info_parseFromJSON(
	...
	OpenAPI_ip_sm_gw_registration_t *ip_sm_gw_registration_local_nonprim =
		NULL;
	cJSON *ip_sm_gw_guidance = NULL;
	OpenAPI_ip_sm_gw_guidance_t *ip_sm_gw_guidance_local_nonprim = NULL;
	ip_sm_gw_registration =
		cJSON_GetObjectItemCaseSensitive(ip_sm_gw_infoJSON,
		                                 "ipSmGwRegistration");
	if(ip_sm_gw_registration) {
		ip_sm_gw_registration_local_nonprim =
			OpenAPI_ip_sm_gw_registration_parseFromJSON(
				ip_sm_gw_registration);
		if(!ip_sm_gw_registration_local_nonprim) {
			ogs_error(
				"OpenAPI_ip_sm_gw_registration_parseFromJSON failed [ip_sm_gw_registration]");
			goto end;
		}
	}

	ip_sm_gw_guidance = cJSON_GetObjectItemCaseSensitive(ip_sm_gw_infoJSON,
	                                                     "ipSmGwGuidance");
	if(ip_sm_gw_guidance) {
		ip_sm_gw_guidance_local_nonprim =
			OpenAPI_ip_sm_gw_guidance_parseFromJSON(
				ip_sm_gw_guidance);
		if(!ip_sm_gw_guidance_local_nonprim) {
			ogs_error(
				"OpenAPI_ip_sm_gw_guidance_parseFromJSON failed [ip_sm_gw_guidance]");
			goto end;
		}
	}

	ip_sm_gw_info_local_var = OpenAPI_ip_sm_gw_info_create(
		ip_sm_gw_registration ? ip_sm_gw_registration_local_nonprim :
		NULL,
