...
OpenAPI_hss_info_t *OpenAPI_hss_info_create(char		*group_id,
	...
	OpenAPI_hss_info_t *hss_info_local_var =
		ogs_malloc(sizeof(OpenAPI_hss_info_t));
	ogs_assert(hss_info_local_var);

	hss_info_local_var->group_id = group_id;
	hss_info_local_var->imsi_ranges = imsi_ranges;
	hss_info_local_var->ims_private_identity_ranges =
		ims_private_identity_ranges;
	hss_info_local_var->ims_public_identity_ranges =
		ims_public_identity_ranges;
	hss_info_local_var->msisdn_ranges = msisdn_ranges;
	hss_info_local_var->external_group_identifiers_ranges =
		external_group_identifiers_ranges;
...
void OpenAPI_hss_info_free(OpenAPI_hss_info_t *hss_info) {
	...
	if(hss_info->ims_public_identity_ranges) {
		OpenAPI_list_for_each(hss_info->ims_public_identity_ranges,
		                      node) {
			OpenAPI_identity_range_free(node->data);
		}
	...
	if(hss_info->external_group_identifiers_ranges) {
		OpenAPI_list_for_each(
			hss_info->external_group_identifiers_ranges, node) {
			OpenAPI_identity_range_free(node->data);
		}
...
cJSON *OpenAPI_hss_info_convertToJSON(OpenAPI_hss_info_t *hss_info) {
	...
	if(hss_info->group_id) {
		...
		{
			ogs_error(
				"OpenAPI_hss_info_convertToJSON() failed [group_id]");
			goto end;
		}
	}
	...
	if(hss_info->imsi_ranges) {
		...
		if(imsi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_hss_info_convertToJSON() failed [imsi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(hss_info->imsi_ranges, node) {
			cJSON *itemLocal =
	...
	if(hss_info->ims_private_identity_ranges) {
		...
		if(ims_private_identity_rangesList == NULL) {
			ogs_error(
				"OpenAPI_hss_info_convertToJSON() failed [ims_private_identity_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(hss_info->ims_private_identity_ranges,
		                      node) {
	...
	if(hss_info->ims_public_identity_ranges) {
		...
		if(ims_public_identity_rangesList == NULL) {
			ogs_error(
				"OpenAPI_hss_info_convertToJSON() failed [ims_public_identity_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(hss_info->ims_public_identity_ranges,
		                      node) {
	...
	if(hss_info->msisdn_ranges) {
		...
		if(msisdn_rangesList == NULL) {
			ogs_error(
				"OpenAPI_hss_info_convertToJSON() failed [msisdn_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(hss_info->msisdn_ranges, node) {
			cJSON *itemLocal =
	...
	if(hss_info->external_group_identifiers_ranges) {
		...
		if(external_group_identifiers_rangesList == NULL) {
			ogs_error(
				"OpenAPI_hss_info_convertToJSON() failed [external_group_identifiers_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			hss_info->external_group_identifiers_ranges, node) {
	...
	if(hss_info->hss_diameter_address) {
		...
		if(hss_diameter_address_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_hss_info_convertToJSON() failed [hss_diameter_address]");
			goto end;
		}
...
OpenAPI_hss_info_t *OpenAPI_hss_info_parseFromJSON(cJSON *hss_infoJSON) {
	...
	if(group_id) {
		...
		{
			ogs_error(
				"OpenAPI_hss_info_parseFromJSON() failed [group_id]");
			goto end;
		}
	}

	imsi_ranges = cJSON_GetObjectItemCaseSensitive(hss_infoJSON,
	                                               "imsiRanges");
	if(imsi_ranges) {
		cJSON *imsi_ranges_local = NULL;
		if(!cJSON_IsArray(imsi_ranges)) {
			ogs_error(
				"OpenAPI_hss_info_parseFromJSON() failed [imsi_ranges]");
			goto end;
		}
	...
	if(ims_public_identity_ranges) {
		...
		if(!cJSON_IsArray(ims_public_identity_ranges)) {
			ogs_error(
				"OpenAPI_hss_info_parseFromJSON() failed [ims_public_identity_ranges]");
			goto end;
		}
	...
	msisdn_ranges = cJSON_GetObjectItemCaseSensitive(hss_infoJSON,
	                                                 "msisdnRanges");
	if(msisdn_ranges) {
		cJSON *msisdn_ranges_local = NULL;
		if(!cJSON_IsArray(msisdn_ranges)) {
			ogs_error(
				"OpenAPI_hss_info_parseFromJSON() failed [msisdn_ranges]");
			goto end;
		}
	...
	if(external_group_identifiers_ranges) {
		...
		if(!cJSON_IsArray(external_group_identifiers_ranges)) {
			ogs_error(
				"OpenAPI_hss_info_parseFromJSON() failed [external_group_identifiers_ranges]");
			goto end;
		}
	...
	hss_diameter_address = cJSON_GetObjectItemCaseSensitive(hss_infoJSON,
	                                                        "hssDiameterAddress");
	...
	hss_info_local_var = OpenAPI_hss_info_create(
		...
		ims_private_identity_ranges ? ims_private_identity_rangesList :
		NULL,
		ims_public_identity_ranges ? ims_public_identity_rangesList :
		NULL,
...
OpenAPI_hss_info_t *OpenAPI_hss_info_copy(OpenAPI_hss_info_t	*dst,
                                          OpenAPI_hss_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
