...
OpenAPI_mm_context_t *OpenAPI_mm_context_create(
	...
	bool is_nas_downlink_count, int nas_downlink_count,
	bool is_nas_uplink_count, int nas_uplink_count,
	char *ue_security_capability, char *s1_ue_network_capability,
	OpenAPI_list_t *allowed_nssai, OpenAPI_list_t *nssai_mapping_list,
	OpenAPI_list_t *allowed_home_nssai, OpenAPI_list_t *ns_instance_list,
	OpenAPI_expected_ue_behavior_t *expected_u_ebehavior,
	OpenAPI_ue_differentiation_info_t *ue_differentiation_info,
	char *plmn_assi_ue_radio_cap_id, char *man_assi_ue_radio_cap_id,
	char *ucmf_dic_entry_id, OpenAPI_global_ran_node_id_t *n3_iwf_id,
	OpenAPI_global_ran_node_id_t *wagf_id,
	OpenAPI_global_ran_node_id_t *tngf_id, bool is_an_n2_ap_id,
	int an_n2_ap_id, OpenAPI_list_t *nssaa_status_list,
	OpenAPI_list_t *pending_nssai_mapping_list,
	OpenAPI_uuaa_mm_status_e uuaa_mm_status) {
	OpenAPI_mm_context_t *mm_context_local_var =
		ogs_malloc(sizeof(OpenAPI_mm_context_t));
...
void OpenAPI_mm_context_free(OpenAPI_mm_context_t *mm_context) {
	...
	if(mm_context->eps_nas_security_mode) {
		OpenAPI_eps_nas_security_mode_free(
	...
	if(mm_context->pending_nssai_mapping_list) {
		OpenAPI_list_for_each(mm_context->pending_nssai_mapping_list,
		                      node) {
			OpenAPI_nssai_mapping_free(node->data);
		}
...
cJSON *OpenAPI_mm_context_convertToJSON(OpenAPI_mm_context_t *mm_context) {
	...
	if(mm_context == NULL) {
		ogs_error(
			"OpenAPI_mm_context_convertToJSON() failed [MmContext]");
		return NULL;
	}
	...
	if(mm_context->access_type == OpenAPI_access_type_NULL) {
		ogs_error(
			"OpenAPI_mm_context_convertToJSON() failed [access_type]");
		return NULL;
	}
	...
	if(mm_context->nas_security_mode) {
		cJSON *nas_security_mode_local_JSON =
			OpenAPI_nas_security_mode_convertToJSON(
				mm_context->nas_security_mode);
		if(nas_security_mode_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [nas_security_mode]");
			goto end;
		}
	...
	if(mm_context->eps_nas_security_mode) {
		cJSON *eps_nas_security_mode_local_JSON =
			OpenAPI_eps_nas_security_mode_convertToJSON(
				mm_context->eps_nas_security_mode);
		if(eps_nas_security_mode_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [eps_nas_security_mode]");
			goto end;
		}
	...
	if(mm_context->is_nas_downlink_count) {
		...
		{
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [nas_downlink_count]");
			goto end;
		}
	}
	...
	if(mm_context->is_nas_uplink_count) {
		...
		{
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [nas_uplink_count]");
			goto end;
		}
	}
	...
	if(mm_context->ue_security_capability) {
		...
		{
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [ue_security_capability]");
			goto end;
		}
	}
	...
	if(mm_context->s1_ue_network_capability) {
		...
		{
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [s1_ue_network_capability]");
			goto end;
		}
	}
	...
	if(mm_context->allowed_nssai) {
		...
		if(allowed_nssaiList == NULL) {
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [allowed_nssai]");
			goto end;
		}
		OpenAPI_list_for_each(mm_context->allowed_nssai, node) {
			cJSON *itemLocal =
	...
	if(mm_context->nssai_mapping_list) {
		...
		if(nssai_mapping_listList == NULL) {
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [nssai_mapping_list]");
			goto end;
		}
		OpenAPI_list_for_each(mm_context->nssai_mapping_list, node) {
			cJSON *itemLocal =
				OpenAPI_nssai_mapping_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_mm_context_convertToJSON() failed [nssai_mapping_list]");
				goto end;
	...
	if(mm_context->allowed_home_nssai) {
		...
		if(allowed_home_nssaiList == NULL) {
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [allowed_home_nssai]");
			goto end;
		}
		OpenAPI_list_for_each(mm_context->allowed_home_nssai, node) {
			cJSON *itemLocal =
	...
	if(mm_context->ns_instance_list) {
		...
		if(ns_instance_listList == NULL) {
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [ns_instance_list]");
			goto end;
		}
		OpenAPI_list_for_each(mm_context->ns_instance_list, node) {
	...
	if(mm_context->expected_u_ebehavior) {
		cJSON *expected_u_ebehavior_local_JSON =
			OpenAPI_expected_ue_behavior_convertToJSON(
				mm_context->expected_u_ebehavior);
		if(expected_u_ebehavior_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [expected_u_ebehavior]");
			goto end;
		}
	...
	if(mm_context->ue_differentiation_info) {
		cJSON *ue_differentiation_info_local_JSON =
			OpenAPI_ue_differentiation_info_convertToJSON(
				mm_context->ue_differentiation_info);
		if(ue_differentiation_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [ue_differentiation_info]");
			goto end;
		}
	...
	if(mm_context->plmn_assi_ue_radio_cap_id) {
		...
		{
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [plmn_assi_ue_radio_cap_id]");
			goto end;
		}
	}
	...
	if(mm_context->man_assi_ue_radio_cap_id) {
		...
		{
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [man_assi_ue_radio_cap_id]");
			goto end;
		}
	}
	...
	if(mm_context->ucmf_dic_entry_id) {
		...
		{
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [ucmf_dic_entry_id]");
			goto end;
		}
	}

	if(mm_context->n3_iwf_id) {
		cJSON *n3_iwf_id_local_JSON =
			OpenAPI_global_ran_node_id_convertToJSON(
				mm_context->n3_iwf_id);
		if(n3_iwf_id_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [n3_iwf_id]");
			goto end;
		}
	...
	if(mm_context->wagf_id) {
		cJSON *wagf_id_local_JSON =
			OpenAPI_global_ran_node_id_convertToJSON(
				mm_context->wagf_id);
		if(wagf_id_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [wagf_id]");
			goto end;
		}
	...
	if(mm_context->tngf_id) {
		cJSON *tngf_id_local_JSON =
			OpenAPI_global_ran_node_id_convertToJSON(
				mm_context->tngf_id);
		if(tngf_id_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [tngf_id]");
			goto end;
		}
	...
	if(mm_context->is_an_n2_ap_id) {
		...
		{
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [an_n2_ap_id]");
			goto end;
		}
	}
	...
	if(mm_context->nssaa_status_list) {
		...
		if(nssaa_status_listList == NULL) {
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [nssaa_status_list]");
			goto end;
		}
		OpenAPI_list_for_each(mm_context->nssaa_status_list, node) {
			cJSON *itemLocal =
				OpenAPI_nssaa_status_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_mm_context_convertToJSON() failed [nssaa_status_list]");
				goto end;
	...
	if(mm_context->pending_nssai_mapping_list) {
		cJSON *pending_nssai_mapping_listList =
			cJSON_AddArrayToObject(item, "pendingNssaiMappingList");
		if(pending_nssai_mapping_listList == NULL) {
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [pending_nssai_mapping_list]");
			goto end;
		}
		OpenAPI_list_for_each(mm_context->pending_nssai_mapping_list,
		                      node) {
	...
	if(mm_context->uuaa_mm_status != OpenAPI_uuaa_mm_status_NULL) {
		if(cJSON_AddStringToObject(item, "uuaaMmStatus",
		                           OpenAPI_uuaa_mm_status_ToString(
						   mm_context->uuaa_mm_status))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_mm_context_convertToJSON() failed [uuaa_mm_status]");
			goto end;
		}
	}
...
OpenAPI_mm_context_t *OpenAPI_mm_context_parseFromJSON(cJSON *mm_contextJSON) {
	...
	OpenAPI_eps_nas_security_mode_t *eps_nas_security_mode_local_nonprim =
		NULL;
	...
	OpenAPI_expected_ue_behavior_t *expected_u_ebehavior_local_nonprim =
		NULL;
	cJSON *ue_differentiation_info = NULL;
	OpenAPI_ue_differentiation_info_t *ue_differentiation_info_local_nonprim
	        = NULL;
	...
	if(!access_type) {
		ogs_error(
			"OpenAPI_mm_context_parseFromJSON() failed [access_type]");
		goto end;
	}
	...
	if(nas_security_mode) {
		nas_security_mode_local_nonprim =
			OpenAPI_nas_security_mode_parseFromJSON(
				nas_security_mode);
		if(!nas_security_mode_local_nonprim) {
			ogs_error(
				"OpenAPI_nas_security_mode_parseFromJSON failed [nas_security_mode]");
			goto end;
		}
	}
	...
	if(eps_nas_security_mode) {
		...
		if(!eps_nas_security_mode_local_nonprim) {
			ogs_error(
				"OpenAPI_eps_nas_security_mode_parseFromJSON failed [eps_nas_security_mode]");
			goto end;
		}
	}
	...
	if(nas_downlink_count) {
		if(!cJSON_IsNumber(nas_downlink_count)) {
			ogs_error(
				"OpenAPI_mm_context_parseFromJSON() failed [nas_downlink_count]");
			goto end;
		}
	}
	...
	if(nas_uplink_count) {
		if(!cJSON_IsNumber(nas_uplink_count)) {
			ogs_error(
				"OpenAPI_mm_context_parseFromJSON() failed [nas_uplink_count]");
			goto end;
		}
	}

	ue_security_capability =
		cJSON_GetObjectItemCaseSensitive(mm_contextJSON,
	...
	if(ue_security_capability) {
		...
		{
			ogs_error(
				"OpenAPI_mm_context_parseFromJSON() failed [ue_security_capability]");
			goto end;
		}
	}
	...
	if(s1_ue_network_capability) {
		...
		{
			ogs_error(
				"OpenAPI_mm_context_parseFromJSON() failed [s1_ue_network_capability]");
			goto end;
		}
	}
	...
	if(allowed_nssai) {
		...
		if(!cJSON_IsArray(allowed_nssai)) {
			ogs_error(
				"OpenAPI_mm_context_parseFromJSON() failed [allowed_nssai]");
			goto end;
		}
		...
		cJSON_ArrayForEach(allowed_nssai_local, allowed_nssai) {
			...
			OpenAPI_snssai_t *allowed_nssaiItem =
	...
	if(nssai_mapping_list) {
		...
		if(!cJSON_IsArray(nssai_mapping_list)) {
			ogs_error(
				"OpenAPI_mm_context_parseFromJSON() failed [nssai_mapping_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(nssai_mapping_list_local,
			...
			OpenAPI_nssai_mapping_t *nssai_mapping_listItem =
				OpenAPI_nssai_mapping_parseFromJSON(
					nssai_mapping_list_local);
			if(!nssai_mapping_listItem) {
				ogs_error("No nssai_mapping_listItem");
				goto end;
			}
			OpenAPI_list_add(nssai_mapping_listList,
	...
	if(allowed_home_nssai) {
		...
		if(!cJSON_IsArray(allowed_home_nssai)) {
			ogs_error(
				"OpenAPI_mm_context_parseFromJSON() failed [allowed_home_nssai]");
			goto end;
		}
		...
		cJSON_ArrayForEach(allowed_home_nssai_local,
			...
			OpenAPI_snssai_t *allowed_home_nssaiItem =
				OpenAPI_snssai_parseFromJSON(
			...
			OpenAPI_list_add(allowed_home_nssaiList,
	...
	if(ns_instance_list) {
		...
		if(!cJSON_IsArray(ns_instance_list)) {
			ogs_error(
				"OpenAPI_mm_context_parseFromJSON() failed [ns_instance_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ns_instance_list_local, ns_instance_list) {
			...
			OpenAPI_list_add(ns_instance_listList,
			                 ogs_strdup(
						 ns_instance_list_local->
						 valuestring));
		}
	}
	...
	if(expected_u_ebehavior) {
		expected_u_ebehavior_local_nonprim =
			OpenAPI_expected_ue_behavior_parseFromJSON(
				expected_u_ebehavior);
		if(!expected_u_ebehavior_local_nonprim) {
			ogs_error(
				"OpenAPI_expected_ue_behavior_parseFromJSON failed [expected_u_ebehavior]");
			goto end;
		}
	}
	...
	if(ue_differentiation_info) {
		ue_differentiation_info_local_nonprim =
			OpenAPI_ue_differentiation_info_parseFromJSON(
				ue_differentiation_info);
		if(!ue_differentiation_info_local_nonprim) {
			ogs_error(
				"OpenAPI_ue_differentiation_info_parseFromJSON failed [ue_differentiation_info]");
			goto end;
		}
	}
	...
	if(plmn_assi_ue_radio_cap_id) {
		...
		{
			ogs_error(
				"OpenAPI_mm_context_parseFromJSON() failed [plmn_assi_ue_radio_cap_id]");
			goto end;
		}
	}
	...
	if(man_assi_ue_radio_cap_id) {
		...
		{
			ogs_error(
				"OpenAPI_mm_context_parseFromJSON() failed [man_assi_ue_radio_cap_id]");
			goto end;
		}
	}
	...
	if(ucmf_dic_entry_id) {
		...
		{
			ogs_error(
				"OpenAPI_mm_context_parseFromJSON() failed [ucmf_dic_entry_id]");
			goto end;
		}
	}
	...
	if(n3_iwf_id) {
		n3_iwf_id_local_nonprim =
			OpenAPI_global_ran_node_id_parseFromJSON(n3_iwf_id);
		if(!n3_iwf_id_local_nonprim) {
			ogs_error(
				"OpenAPI_global_ran_node_id_parseFromJSON failed [n3_iwf_id]");
			goto end;
		}
	}
	...
	if(wagf_id) {
		wagf_id_local_nonprim =
			OpenAPI_global_ran_node_id_parseFromJSON(wagf_id);
		if(!wagf_id_local_nonprim) {
			ogs_error(
				"OpenAPI_global_ran_node_id_parseFromJSON failed [wagf_id]");
			goto end;
		}
	}
	...
	if(tngf_id) {
		tngf_id_local_nonprim =
			OpenAPI_global_ran_node_id_parseFromJSON(tngf_id);
		if(!tngf_id_local_nonprim) {
			ogs_error(
				"OpenAPI_global_ran_node_id_parseFromJSON failed [tngf_id]");
			goto end;
		}
	}
	...
	if(an_n2_ap_id) {
		if(!cJSON_IsNumber(an_n2_ap_id)) {
			ogs_error(
				"OpenAPI_mm_context_parseFromJSON() failed [an_n2_ap_id]");
			goto end;
		}
	}
	...
	if(nssaa_status_list) {
		...
		if(!cJSON_IsArray(nssaa_status_list)) {
			ogs_error(
				"OpenAPI_mm_context_parseFromJSON() failed [nssaa_status_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(nssaa_status_list_local, nssaa_status_list) {
			...
			OpenAPI_nssaa_status_t *nssaa_status_listItem =
				OpenAPI_nssaa_status_parseFromJSON(
					nssaa_status_list_local);
			if(!nssaa_status_listItem) {
				ogs_error("No nssaa_status_listItem");
				goto end;
			}
			OpenAPI_list_add(nssaa_status_listList,
			                 nssaa_status_listItem);
		}
	}
	...
	if(pending_nssai_mapping_list) {
		...
		if(!cJSON_IsArray(pending_nssai_mapping_list)) {
			ogs_error(
				"OpenAPI_mm_context_parseFromJSON() failed [pending_nssai_mapping_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(pending_nssai_mapping_list_local,
			...
			OpenAPI_nssai_mapping_t *pending_nssai_mapping_listItem
			        = OpenAPI_nssai_mapping_parseFromJSON(
			...
			OpenAPI_list_add(pending_nssai_mapping_listList,
	...
	if(uuaa_mm_status) {
		if(!cJSON_IsString(uuaa_mm_status)) {
			ogs_error(
				"OpenAPI_mm_context_parseFromJSON() failed [uuaa_mm_status]");
			goto end;
		}
		uuaa_mm_statusVariable =
			OpenAPI_uuaa_mm_status_FromString(
	...
	mm_context_local_var = OpenAPI_mm_context_create(
		...
		ue_security_capability &&
		!cJSON_IsNull(ue_security_capability) ?
		ogs_strdup(ue_security_capability->valuestring) : NULL,
		s1_ue_network_capability &&
		!cJSON_IsNull(s1_ue_network_capability) ?
		ogs_strdup(s1_ue_network_capability->valuestring) : NULL,
		...
		expected_u_ebehavior ? expected_u_ebehavior_local_nonprim :
		NULL,
		ue_differentiation_info ?
		ue_differentiation_info_local_nonprim : NULL,
		plmn_assi_ue_radio_cap_id &&
		!cJSON_IsNull(plmn_assi_ue_radio_cap_id) ?
		ogs_strdup(plmn_assi_ue_radio_cap_id->valuestring) : NULL,
		man_assi_ue_radio_cap_id &&
		!cJSON_IsNull(man_assi_ue_radio_cap_id) ?
		ogs_strdup(man_assi_ue_radio_cap_id->valuestring) : NULL,
		ucmf_dic_entry_id &&
		!cJSON_IsNull(ucmf_dic_entry_id) ?
		ogs_strdup(ucmf_dic_entry_id->valuestring) : NULL,
		...
		pending_nssai_mapping_list ? pending_nssai_mapping_listList :
		NULL,
...
OpenAPI_mm_context_t *OpenAPI_mm_context_copy(OpenAPI_mm_context_t	*dst,
                                              OpenAPI_mm_context_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
