...
OpenAPI_addr_fqdn_t *OpenAPI_addr_fqdn_create(OpenAPI_ip_addr_t *ip_addr,
                                              char		*fqdn) {
	OpenAPI_addr_fqdn_t *addr_fqdn_local_var =
		ogs_malloc(sizeof(OpenAPI_addr_fqdn_t));
...
cJSON *OpenAPI_addr_fqdn_convertToJSON(OpenAPI_addr_fqdn_t *addr_fqdn) {
	...
	if(addr_fqdn == NULL) {
		ogs_error(
			"OpenAPI_addr_fqdn_convertToJSON() failed [AddrFqdn]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(addr_fqdn->ip_addr) {
		cJSON *ip_addr_local_JSON =
			OpenAPI_ip_addr_convertToJSON(addr_fqdn->ip_addr);
		if(ip_addr_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_addr_fqdn_convertToJSON() failed [ip_addr]");
			goto end;
		}
	...
	if(addr_fqdn->fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_addr_fqdn_convertToJSON() failed [fqdn]");
			goto end;
		}
	}
...
OpenAPI_addr_fqdn_t *OpenAPI_addr_fqdn_parseFromJSON(cJSON *addr_fqdnJSON) {
	...
	if(ip_addr) {
		...
		if(!ip_addr_local_nonprim) {
			ogs_error(
				"OpenAPI_ip_addr_parseFromJSON failed [ip_addr]");
			goto end;
		}
	}
	...
	if(fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_addr_fqdn_parseFromJSON() failed [fqdn]");
			goto end;
		}
	}
