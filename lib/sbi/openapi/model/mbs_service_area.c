...
OpenAPI_mbs_service_area_t *OpenAPI_mbs_service_area_create(
	...
	OpenAPI_mbs_service_area_t *mbs_service_area_local_var =
		ogs_malloc(sizeof(OpenAPI_mbs_service_area_t));
...
cJSON *OpenAPI_mbs_service_area_convertToJSON(
	...
	if(mbs_service_area == NULL) {
		ogs_error(
			"OpenAPI_mbs_service_area_convertToJSON() failed [MbsServiceArea]");
		return NULL;
	}
	...
	if(mbs_service_area->ncgi_list) {
		...
		if(ncgi_listList == NULL) {
			ogs_error(
				"OpenAPI_mbs_service_area_convertToJSON() failed [ncgi_list]");
			goto end;
		}
		OpenAPI_list_for_each(mbs_service_area->ncgi_list, node) {
			cJSON *itemLocal =
				OpenAPI_ncgi_tai_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_mbs_service_area_convertToJSON() failed [ncgi_list]");
				goto end;
	...
	if(mbs_service_area->tai_list) {
		...
		if(tai_listList == NULL) {
			ogs_error(
				"OpenAPI_mbs_service_area_convertToJSON() failed [tai_list]");
			goto end;
		}
		OpenAPI_list_for_each(mbs_service_area->tai_list, node) {
			cJSON *itemLocal =
...
OpenAPI_mbs_service_area_t *OpenAPI_mbs_service_area_parseFromJSON(
	...
	ncgi_list = cJSON_GetObjectItemCaseSensitive(mbs_service_areaJSON,
	                                             "ncgiList");
	if(ncgi_list) {
		cJSON *ncgi_list_local = NULL;
		if(!cJSON_IsArray(ncgi_list)) {
			ogs_error(
				"OpenAPI_mbs_service_area_parseFromJSON() failed [ncgi_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ncgi_list_local, ncgi_list) {
			...
			OpenAPI_ncgi_tai_t *ncgi_listItem =
				OpenAPI_ncgi_tai_parseFromJSON(ncgi_list_local);
			if(!ncgi_listItem) {
	...
	tai_list = cJSON_GetObjectItemCaseSensitive(mbs_service_areaJSON,
	                                            "taiList");
	if(tai_list) {
		cJSON *tai_list_local = NULL;
		if(!cJSON_IsArray(tai_list)) {
			ogs_error(
				"OpenAPI_mbs_service_area_parseFromJSON() failed [tai_list]");
			goto end;
		}
