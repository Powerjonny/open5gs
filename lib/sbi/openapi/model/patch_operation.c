...
char * OpenAPI_patch_operation_ToString(
	...
	const char *patch_operationArray[] = { "NULL", "add", "copy", "move",
		                               "remove", "replace", "test" };
	size_t sizeofArray = sizeof(patch_operationArray) /
	                     sizeof(patch_operationArray[0]);
...
OpenAPI_patch_operation_e OpenAPI_patch_operation_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(patch_operation,
		          patch_operationArray[stringToReturn]) == 0) {
