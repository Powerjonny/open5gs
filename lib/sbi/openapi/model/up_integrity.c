...
char * OpenAPI_up_integrity_ToString(OpenAPI_up_integrity_e up_integrity) {
	const char *up_integrityArray[] = { "NULL", "REQUIRED", "PREFERRED",
		                            "NOT_NEEDED" };
	size_t sizeofArray = sizeof(up_integrityArray) /
	                     sizeof(up_integrityArray[0]);
...
OpenAPI_up_integrity_e OpenAPI_up_integrity_FromString(char *up_integrity) {
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(up_integrity,
		          up_integrityArray[stringToReturn]) == 0) {
