...
OpenAPI_session_rule_report_t *OpenAPI_session_rule_report_create(
	...
	OpenAPI_session_rule_report_t *session_rule_report_local_var =
		ogs_malloc(sizeof(OpenAPI_session_rule_report_t));
	...
	session_rule_report_local_var->sess_rule_failure_code =
		sess_rule_failure_code;
	session_rule_report_local_var->policy_dec_failure_reports =
...
cJSON *OpenAPI_session_rule_report_convertToJSON(
	...
	if(session_rule_report == NULL) {
		ogs_error(
			"OpenAPI_session_rule_report_convertToJSON() failed [SessionRuleReport]");
		return NULL;
	}
	...
	if(!session_rule_report->rule_ids) {
		ogs_error(
			"OpenAPI_session_rule_report_convertToJSON() failed [rule_ids]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(session_rule_report->rule_ids, node) {
		if(cJSON_AddStringToObject(rule_idsList, "",
		                           (char *) node->data) == NULL)
		{
			ogs_error(
				"OpenAPI_session_rule_report_convertToJSON() failed [rule_ids]");
	...
	if(session_rule_report->rule_status == OpenAPI_rule_status_NULL) {
		ogs_error(
			"OpenAPI_session_rule_report_convertToJSON() failed [rule_status]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "ruleStatus",
	                           OpenAPI_rule_status_ToString(
					   session_rule_report->rule_status)) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_session_rule_report_convertToJSON() failed [rule_status]");
	...
	{
		if(cJSON_AddStringToObject(item, "sessRuleFailureCode",
		                           OpenAPI_session_rule_failure_code_ToString
		                                   (session_rule_report->
		                                   sess_rule_failure_code)) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_session_rule_report_convertToJSON() failed [sess_rule_failure_code]");
			goto end;
		}
	}
	   ...
	   OpenAPI_policy_decision_failure_code_NULL)
	{
		...
		if(policy_dec_failure_reportsList == NULL) {
			ogs_error(
				"OpenAPI_session_rule_report_convertToJSON() failed [policy_dec_failure_reports]");
			goto end;
		}
		OpenAPI_list_for_each(
			session_rule_report->policy_dec_failure_reports, node) {
...
OpenAPI_session_rule_report_t *OpenAPI_session_rule_report_parseFromJSON(
	...
	rule_ids = cJSON_GetObjectItemCaseSensitive(session_rule_reportJSON,
	                                            "ruleIds");
	if(!rule_ids) {
		ogs_error(
			"OpenAPI_session_rule_report_parseFromJSON() failed [rule_ids]");
		goto end;
	}
	...
	cJSON_ArrayForEach(rule_ids_local, rule_ids) {
		...
		OpenAPI_list_add(rule_idsList,
		                 ogs_strdup(rule_ids_local->valuestring));
	}

	rule_status = cJSON_GetObjectItemCaseSensitive(session_rule_reportJSON,
	                                               "ruleStatus");
	if(!rule_status) {
		ogs_error(
			"OpenAPI_session_rule_report_parseFromJSON() failed [rule_status]");
		goto end;
	}
	...
	sess_rule_failure_code =
		cJSON_GetObjectItemCaseSensitive(session_rule_reportJSON,
		                                 "sessRuleFailureCode");
	if(sess_rule_failure_code) {
		if(!cJSON_IsString(sess_rule_failure_code)) {
			ogs_error(
				"OpenAPI_session_rule_report_parseFromJSON() failed [sess_rule_failure_code]");
			goto end;
		}
		sess_rule_failure_codeVariable =
			OpenAPI_session_rule_failure_code_FromString(
				sess_rule_failure_code->valuestring);
	}
	...
	if(policy_dec_failure_reports) {
		...
		if(!cJSON_IsArray(policy_dec_failure_reports)) {
			ogs_error(
				"OpenAPI_session_rule_report_parseFromJSON() failed [policy_dec_failure_reports]");
			goto end;
		}
		...
		if(policy_dec_failure_reportsList->count == 0) {
			ogs_error(
				"OpenAPI_session_rule_report_parseFromJSON() failed: Expected policy_dec_failure_reportsList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	session_rule_report_local_var = OpenAPI_session_rule_report_create(
		...
		policy_dec_failure_reports ? policy_dec_failure_reportsList :
		NULL
		);
...
OpenAPI_session_rule_report_t *OpenAPI_session_rule_report_copy(
	OpenAPI_session_rule_report_t	*dst,
	OpenAPI_session_rule_report_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
