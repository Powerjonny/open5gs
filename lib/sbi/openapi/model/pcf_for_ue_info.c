...
void OpenAPI_pcf_for_ue_info_free(OpenAPI_pcf_for_ue_info_t *pcf_for_ue_info) {
	...
	if(pcf_for_ue_info->pcf_ip_end_points) {
		OpenAPI_list_for_each(pcf_for_ue_info->pcf_ip_end_points,
		                      node) {
...
cJSON *OpenAPI_pcf_for_ue_info_convertToJSON(
	...
	if(pcf_for_ue_info == NULL) {
		ogs_error(
			"OpenAPI_pcf_for_ue_info_convertToJSON() failed [PcfForUeInfo]");
		return NULL;
	}
	...
	if(pcf_for_ue_info->pcf_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_for_ue_info_convertToJSON() failed [pcf_fqdn]");
			goto end;
		}
	}
	...
	if(pcf_for_ue_info->pcf_ip_end_points) {
		...
		if(pcf_ip_end_pointsList == NULL) {
			ogs_error(
				"OpenAPI_pcf_for_ue_info_convertToJSON() failed [pcf_ip_end_points]");
			goto end;
		}
		OpenAPI_list_for_each(pcf_for_ue_info->pcf_ip_end_points,
		                      node) {
	...
	if(pcf_for_ue_info->pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_for_ue_info_convertToJSON() failed [pcf_id]");
			goto end;
		}
	}
	...
	if(pcf_for_ue_info->pcf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_for_ue_info_convertToJSON() failed [pcf_set_id]");
			goto end;
		}
	}
	...
	if(pcf_for_ue_info->bind_level != OpenAPI_binding_level_NULL) {
		if(cJSON_AddStringToObject(item, "bindLevel",
						   ...
						   pcf_for_ue_info->bind_level))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_pcf_for_ue_info_convertToJSON() failed [bind_level]");
			goto end;
		}
	}
...
OpenAPI_pcf_for_ue_info_t *OpenAPI_pcf_for_ue_info_parseFromJSON(
	...
	pcf_fqdn = cJSON_GetObjectItemCaseSensitive(pcf_for_ue_infoJSON,
	                                            "pcfFqdn");
	if(pcf_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_for_ue_info_parseFromJSON() failed [pcf_fqdn]");
			goto end;
		}
	}

	pcf_ip_end_points =
		cJSON_GetObjectItemCaseSensitive(pcf_for_ue_infoJSON,
		                                 "pcfIpEndPoints");
	if(pcf_ip_end_points) {
		cJSON *pcf_ip_end_points_local = NULL;
		if(!cJSON_IsArray(pcf_ip_end_points)) {
			ogs_error(
				"OpenAPI_pcf_for_ue_info_parseFromJSON() failed [pcf_ip_end_points]");
			goto end;
		}
	...
	if(pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_for_ue_info_parseFromJSON() failed [pcf_id]");
			goto end;
		}
	}

	pcf_set_id = cJSON_GetObjectItemCaseSensitive(pcf_for_ue_infoJSON,
	                                              "pcfSetId");
	if(pcf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_for_ue_info_parseFromJSON() failed [pcf_set_id]");
			goto end;
		}
	}

	bind_level = cJSON_GetObjectItemCaseSensitive(pcf_for_ue_infoJSON,
	                                              "bindLevel");
	if(bind_level) {
		if(!cJSON_IsString(bind_level)) {
			ogs_error(
				"OpenAPI_pcf_for_ue_info_parseFromJSON() failed [bind_level]");
			goto end;
		}
		bind_levelVariable =
