...
OpenAPI_snssai_info_t *OpenAPI_snssai_info_create(OpenAPI_list_t *dnn_infos) {
	OpenAPI_snssai_info_t *snssai_info_local_var =
		ogs_malloc(sizeof(OpenAPI_snssai_info_t));
...
cJSON *OpenAPI_snssai_info_convertToJSON(OpenAPI_snssai_info_t *snssai_info) {
	...
	if(snssai_info == NULL) {
		ogs_error(
			"OpenAPI_snssai_info_convertToJSON() failed [SnssaiInfo]");
		return NULL;
	}
	...
	if(!snssai_info->dnn_infos) {
		ogs_error(
			"OpenAPI_snssai_info_convertToJSON() failed [dnn_infos]");
		return NULL;
	}
...
OpenAPI_snssai_info_t *OpenAPI_snssai_info_parseFromJSON(
	...
	dnn_infos = cJSON_GetObjectItemCaseSensitive(snssai_infoJSON,
	                                             "dnnInfos");
	if(!dnn_infos) {
		ogs_error(
			"OpenAPI_snssai_info_parseFromJSON() failed [dnn_infos]");
		goto end;
	}
	...
	cJSON_ArrayForEach(dnn_infos_local, dnn_infos) {
		...
		OpenAPI_dnn_info_t *dnn_infosItem =
			OpenAPI_dnn_info_parseFromJSON(dnn_infos_local);
		if(!dnn_infosItem) {
