...
OpenAPI_resource_usage_t *OpenAPI_resource_usage_create(bool	is_cpu_usage,
                                                        int	cpu_usage,
                                                        bool	is_memory_usage,
                                                        int	memory_usage,
                                                        bool	is_storage_usage,
                                                        int	storage_usage) {
	OpenAPI_resource_usage_t *resource_usage_local_var =
		ogs_malloc(sizeof(OpenAPI_resource_usage_t));
...
cJSON *OpenAPI_resource_usage_convertToJSON(
	OpenAPI_resource_usage_t *resource_usage) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(resource_usage == NULL) {
		ogs_error(
			"OpenAPI_resource_usage_convertToJSON() failed [ResourceUsage]");
		return NULL;
	}
	...
	if(resource_usage->is_cpu_usage) {
		...
		{
			ogs_error(
				"OpenAPI_resource_usage_convertToJSON() failed [cpu_usage]");
			goto end;
		}
	}
	...
	if(resource_usage->is_memory_usage) {
		...
		{
			ogs_error(
				"OpenAPI_resource_usage_convertToJSON() failed [memory_usage]");
			goto end;
		}
	}
	...
	if(resource_usage->is_storage_usage) {
		...
		{
			ogs_error(
				"OpenAPI_resource_usage_convertToJSON() failed [storage_usage]");
			goto end;
		}
	}
...
OpenAPI_resource_usage_t *OpenAPI_resource_usage_parseFromJSON(
	cJSON *resource_usageJSON) {
	...
	if(cpu_usage) {
		if(!cJSON_IsNumber(cpu_usage)) {
			ogs_error(
				"OpenAPI_resource_usage_parseFromJSON() failed [cpu_usage]");
			goto end;
		}
	}
	...
	if(memory_usage) {
		if(!cJSON_IsNumber(memory_usage)) {
			ogs_error(
				"OpenAPI_resource_usage_parseFromJSON() failed [memory_usage]");
			goto end;
		}
	}
	...
	if(storage_usage) {
		if(!cJSON_IsNumber(storage_usage)) {
			ogs_error(
				"OpenAPI_resource_usage_parseFromJSON() failed [storage_usage]");
			goto end;
		}
	}
...
OpenAPI_resource_usage_t *OpenAPI_resource_usage_copy(
	OpenAPI_resource_usage_t *dst, OpenAPI_resource_usage_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
