...
OpenAPI_nsacf_capability_t *OpenAPI_nsacf_capability_create(
	bool is_support_ue_sac, int support_ue_sac, bool is_support_pdu_sac,
	int support_pdu_sac) {
	OpenAPI_nsacf_capability_t *nsacf_capability_local_var =
		ogs_malloc(sizeof(OpenAPI_nsacf_capability_t));
...
void OpenAPI_nsacf_capability_free(
...
cJSON *OpenAPI_nsacf_capability_convertToJSON(
	OpenAPI_nsacf_capability_t *nsacf_capability) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(nsacf_capability == NULL) {
		ogs_error(
			"OpenAPI_nsacf_capability_convertToJSON() failed [NsacfCapability]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(nsacf_capability->is_support_ue_sac) {
		if(cJSON_AddBoolToObject(item, "supportUeSAC",
		                         nsacf_capability->support_ue_sac) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_nsacf_capability_convertToJSON() failed [support_ue_sac]");
			goto end;
		}
	}

	if(nsacf_capability->is_support_pdu_sac) {
		if(cJSON_AddBoolToObject(item, "supportPduSAC",
		                         nsacf_capability->support_pdu_sac) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_nsacf_capability_convertToJSON() failed [support_pdu_sac]");
			goto end;
		}
	}
...
OpenAPI_nsacf_capability_t *OpenAPI_nsacf_capability_parseFromJSON(
	cJSON *nsacf_capabilityJSON) {
	OpenAPI_nsacf_capability_t *nsacf_capability_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *support_ue_sac = NULL;
	cJSON *support_pdu_sac = NULL;
	support_ue_sac = cJSON_GetObjectItemCaseSensitive(nsacf_capabilityJSON,
	                                                  "supportUeSAC");
	if(support_ue_sac) {
		if(!cJSON_IsBool(support_ue_sac)) {
			ogs_error(
				"OpenAPI_nsacf_capability_parseFromJSON() failed [support_ue_sac]");
			goto end;
		}
	}

	support_pdu_sac = cJSON_GetObjectItemCaseSensitive(nsacf_capabilityJSON,
	                                                   "supportPduSAC");
	if(support_pdu_sac) {
		if(!cJSON_IsBool(support_pdu_sac)) {
			ogs_error(
				"OpenAPI_nsacf_capability_parseFromJSON() failed [support_pdu_sac]");
			goto end;
		}
	}
...
OpenAPI_nsacf_capability_t *OpenAPI_nsacf_capability_copy(
	OpenAPI_nsacf_capability_t *dst, OpenAPI_nsacf_capability_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
