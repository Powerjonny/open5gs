...
OpenAPI_tsn_qos_container_t *OpenAPI_tsn_qos_container_create(
	...
	OpenAPI_tsn_qos_container_t *tsn_qos_container_local_var =
		ogs_malloc(sizeof(OpenAPI_tsn_qos_container_t));
	ogs_assert(tsn_qos_container_local_var);

	tsn_qos_container_local_var->is_max_tsc_burst_size =
...
cJSON *OpenAPI_tsn_qos_container_convertToJSON(
	...
	if(tsn_qos_container == NULL) {
		ogs_error(
			"OpenAPI_tsn_qos_container_convertToJSON() failed [TsnQosContainer]");
		return NULL;
	}
	...
	if(tsn_qos_container->is_max_tsc_burst_size) {
		...
		{
			ogs_error(
				"OpenAPI_tsn_qos_container_convertToJSON() failed [max_tsc_burst_size]");
			goto end;
		}
	}
	...
	if(tsn_qos_container->is_tsc_pack_delay) {
		if(cJSON_AddNumberToObject(item, "tscPackDelay",
		                           tsn_qos_container->tsc_pack_delay) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_tsn_qos_container_convertToJSON() failed [tsc_pack_delay]");
			goto end;
		}
	}
	...
	if(tsn_qos_container->is_tsc_prio_level) {
		if(cJSON_AddNumberToObject(item, "tscPrioLevel",
		                           tsn_qos_container->tsc_prio_level) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_tsn_qos_container_convertToJSON() failed [tsc_prio_level]");
			goto end;
		}
	}
...
OpenAPI_tsn_qos_container_t *OpenAPI_tsn_qos_container_parseFromJSON(
	...
	if(max_tsc_burst_size) {
		if(!cJSON_IsNumber(max_tsc_burst_size)) {
			ogs_error(
				"OpenAPI_tsn_qos_container_parseFromJSON() failed [max_tsc_burst_size]");
			goto end;
		}
	}

	tsc_pack_delay = cJSON_GetObjectItemCaseSensitive(tsn_qos_containerJSON,
	                                                  "tscPackDelay");
	if(tsc_pack_delay) {
		if(!cJSON_IsNumber(tsc_pack_delay)) {
			ogs_error(
				"OpenAPI_tsn_qos_container_parseFromJSON() failed [tsc_pack_delay]");
			goto end;
		}
	}

	tsc_prio_level = cJSON_GetObjectItemCaseSensitive(tsn_qos_containerJSON,
	                                                  "tscPrioLevel");
	if(tsc_prio_level) {
		if(!cJSON_IsNumber(tsc_prio_level)) {
			ogs_error(
				"OpenAPI_tsn_qos_container_parseFromJSON() failed [tsc_prio_level]");
			goto end;
		}
	}
