...
OpenAPI_rat_freq_information_t *OpenAPI_rat_freq_information_create(
	bool is_all_freq, int all_freq, bool is_all_rat, int all_rat,
	bool is_freq, int freq, OpenAPI_rat_type_e rat_type,
	OpenAPI_threshold_level_t *svc_exp_threshold,
	OpenAPI_matching_direction_t *matching_dir) {
	OpenAPI_rat_freq_information_t *rat_freq_information_local_var =
		ogs_malloc(sizeof(OpenAPI_rat_freq_information_t));
...
void OpenAPI_rat_freq_information_free(
	OpenAPI_rat_freq_information_t *rat_freq_information) {
...
cJSON *OpenAPI_rat_freq_information_convertToJSON(
	OpenAPI_rat_freq_information_t *rat_freq_information) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(rat_freq_information == NULL) {
		ogs_error(
			"OpenAPI_rat_freq_information_convertToJSON() failed [RatFreqInformation]");
		return NULL;
	}
	...
	if(rat_freq_information->is_all_freq) {
		...
		{
			ogs_error(
				"OpenAPI_rat_freq_information_convertToJSON() failed [all_freq]");
			goto end;
		}
	}
	...
	if(rat_freq_information->is_all_rat) {
		...
		{
			ogs_error(
				"OpenAPI_rat_freq_information_convertToJSON() failed [all_rat]");
			goto end;
		}
	}
	...
	if(rat_freq_information->is_freq) {
		...
		{
			ogs_error(
				"OpenAPI_rat_freq_information_convertToJSON() failed [freq]");
			goto end;
		}
	}
	...
	if(rat_freq_information->rat_type != OpenAPI_rat_type_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_rat_freq_information_convertToJSON() failed [rat_type]");
			goto end;
		}
	}
	...
	if(rat_freq_information->svc_exp_threshold) {
		...
		if(svc_exp_threshold_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_rat_freq_information_convertToJSON() failed [svc_exp_threshold]");
			goto end;
		}
	...
	if(rat_freq_information->matching_dir) {
		...
		if(matching_dir_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_rat_freq_information_convertToJSON() failed [matching_dir]");
			goto end;
		}
...
OpenAPI_rat_freq_information_t *OpenAPI_rat_freq_information_parseFromJSON(
	...
	if(all_freq) {
		if(!cJSON_IsBool(all_freq)) {
			ogs_error(
				"OpenAPI_rat_freq_information_parseFromJSON() failed [all_freq]");
			goto end;
		}
	}
	...
	if(all_rat) {
		if(!cJSON_IsBool(all_rat)) {
			ogs_error(
				"OpenAPI_rat_freq_information_parseFromJSON() failed [all_rat]");
			goto end;
		}
	}

	freq = cJSON_GetObjectItemCaseSensitive(rat_freq_informationJSON,
	                                        "freq");
	if(freq) {
		if(!cJSON_IsNumber(freq)) {
			ogs_error(
				"OpenAPI_rat_freq_information_parseFromJSON() failed [freq]");
			goto end;
		}
	}
	...
	if(rat_type) {
		if(!cJSON_IsString(rat_type)) {
			ogs_error(
				"OpenAPI_rat_freq_information_parseFromJSON() failed [rat_type]");
			goto end;
		}
		rat_typeVariable =
	...
	if(svc_exp_threshold) {
		svc_exp_threshold_local_nonprim =
			OpenAPI_threshold_level_parseFromJSON(
				svc_exp_threshold);
		if(!svc_exp_threshold_local_nonprim) {
			ogs_error(
				"OpenAPI_threshold_level_parseFromJSON failed [svc_exp_threshold]");
			goto end;
		}
	}
...
OpenAPI_rat_freq_information_t *OpenAPI_rat_freq_information_copy(
	OpenAPI_rat_freq_information_t	*dst,
	OpenAPI_rat_freq_information_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_rat_freq_information_convertToJSON() failed");
		return NULL;
	}
