...
OpenAPI_ue_context_in_amf_data_t *OpenAPI_ue_context_in_amf_data_create(
	...
	OpenAPI_ue_context_in_amf_data_t *ue_context_in_amf_data_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_context_in_amf_data_t));
	ogs_assert(ue_context_in_amf_data_local_var);

	ue_context_in_amf_data_local_var->eps_interworking_info =
...
void OpenAPI_ue_context_in_amf_data_free(
	OpenAPI_ue_context_in_amf_data_t *ue_context_in_amf_data) {
	...
	if(ue_context_in_amf_data->eps_interworking_info) {
		OpenAPI_eps_interworking_info_free(
			ue_context_in_amf_data->eps_interworking_info);
...
cJSON *OpenAPI_ue_context_in_amf_data_convertToJSON(
	...
	if(ue_context_in_amf_data == NULL) {
		ogs_error(
			"OpenAPI_ue_context_in_amf_data_convertToJSON() failed [UeContextInAmfData]");
		return NULL;
	}
	...
	if(ue_context_in_amf_data->eps_interworking_info) {
		...
		if(eps_interworking_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_in_amf_data_convertToJSON() failed [eps_interworking_info]");
			goto end;
		}
	...
	if(ue_context_in_amf_data->amf_info) {
		...
		if(amf_infoList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_in_amf_data_convertToJSON() failed [amf_info]");
			goto end;
		}
		OpenAPI_list_for_each(ue_context_in_amf_data->amf_info, node) {
			cJSON *itemLocal =
				OpenAPI_nudm_amf_info_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_ue_context_in_amf_data_convertToJSON() failed [amf_info]");
				goto end;
...
OpenAPI_ue_context_in_amf_data_t *OpenAPI_ue_context_in_amf_data_parseFromJSON(
	...
	eps_interworking_info =
		cJSON_GetObjectItemCaseSensitive(ue_context_in_amf_dataJSON,
		                                 "epsInterworkingInfo");
	...
	amf_info = cJSON_GetObjectItemCaseSensitive(ue_context_in_amf_dataJSON,
	                                            "amfInfo");
	if(amf_info) {
		cJSON *amf_info_local = NULL;
		if(!cJSON_IsArray(amf_info)) {
			ogs_error(
				"OpenAPI_ue_context_in_amf_data_parseFromJSON() failed [amf_info]");
			goto end;
		}
		...
		cJSON_ArrayForEach(amf_info_local, amf_info) {
			...
			OpenAPI_nudm_amf_info_t *amf_infoItem =
	...
	ue_context_in_amf_data_local_var =
		OpenAPI_ue_context_in_amf_data_create(
			eps_interworking_info ?
			eps_interworking_info_local_nonprim :
			NULL,
...
OpenAPI_ue_context_in_amf_data_t *OpenAPI_ue_context_in_amf_data_copy(
	OpenAPI_ue_context_in_amf_data_t	*dst,
	OpenAPI_ue_context_in_amf_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ue_context_in_amf_data_convertToJSON() failed");
		return NULL;
	}
