...
OpenAPI_sm_policy_snssai_data_t *OpenAPI_sm_policy_snssai_data_create(
	OpenAPI_snssai_t *snssai, OpenAPI_list_t *sm_policy_dnn_data,
	OpenAPI_slice_mbr_1_t *ue_slice_mbr) {
	OpenAPI_sm_policy_snssai_data_t *sm_policy_snssai_data_local_var =
		ogs_malloc(sizeof(OpenAPI_sm_policy_snssai_data_t));
	ogs_assert(sm_policy_snssai_data_local_var);

	sm_policy_snssai_data_local_var->snssai = snssai;
	sm_policy_snssai_data_local_var->sm_policy_dnn_data =
		sm_policy_dnn_data;
...
void OpenAPI_sm_policy_snssai_data_free(
	OpenAPI_sm_policy_snssai_data_t *sm_policy_snssai_data) {
	...
	if(sm_policy_snssai_data->sm_policy_dnn_data) {
		OpenAPI_list_for_each(sm_policy_snssai_data->sm_policy_dnn_data,
		                      node) {
...
cJSON *OpenAPI_sm_policy_snssai_data_convertToJSON(
	OpenAPI_sm_policy_snssai_data_t *sm_policy_snssai_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(sm_policy_snssai_data == NULL) {
		ogs_error(
			"OpenAPI_sm_policy_snssai_data_convertToJSON() failed [SmPolicySnssaiData]");
		return NULL;
	}
	...
	if(!sm_policy_snssai_data->snssai) {
		ogs_error(
			"OpenAPI_sm_policy_snssai_data_convertToJSON() failed [snssai]");
		return NULL;
	}
	cJSON *snssai_local_JSON =
		OpenAPI_snssai_convertToJSON(sm_policy_snssai_data->snssai);
	...
	if(sm_policy_snssai_data->sm_policy_dnn_data) {
		cJSON *sm_policy_dnn_data = cJSON_AddObjectToObject(item,
		                                                    "smPolicyDnnData");
		if(sm_policy_dnn_data == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_snssai_data_convertToJSON() failed [sm_policy_dnn_data]");
			goto end;
		}
		cJSON *localMapObject = sm_policy_dnn_data;
		if(sm_policy_snssai_data->sm_policy_dnn_data) {
			OpenAPI_list_for_each(
				sm_policy_snssai_data->sm_policy_dnn_data,
				node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_sm_policy_dnn_data_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_sm_policy_snssai_data_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(sm_policy_snssai_data->ue_slice_mbr) {
		cJSON *ue_slice_mbr_local_JSON =
			OpenAPI_slice_mbr_1_convertToJSON(
				sm_policy_snssai_data->ue_slice_mbr);
		if(ue_slice_mbr_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_snssai_data_convertToJSON() failed [ue_slice_mbr]");
			goto end;
		}
		cJSON_AddItemToObject(item, "ueSliceMbr",
		                      ue_slice_mbr_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_snssai_data_convertToJSON() failed [ue_slice_mbr]");
			goto end;
...
OpenAPI_sm_policy_snssai_data_t *OpenAPI_sm_policy_snssai_data_parseFromJSON(
	cJSON *sm_policy_snssai_dataJSON) {
	...
	snssai = cJSON_GetObjectItemCaseSensitive(sm_policy_snssai_dataJSON,
	                                          "snssai");
	if(!snssai) {
		ogs_error(
			"OpenAPI_sm_policy_snssai_data_parseFromJSON() failed [snssai]");
		goto end;
	}
	...
	sm_policy_dnn_data =
		cJSON_GetObjectItemCaseSensitive(sm_policy_snssai_dataJSON,
		                                 "smPolicyDnnData");
	if(sm_policy_dnn_data) {
		cJSON *sm_policy_dnn_data_local_map = NULL;
		if(!cJSON_IsObject(sm_policy_dnn_data) &&
		   !cJSON_IsNull(sm_policy_dnn_data))
		{
			ogs_error(
				"OpenAPI_sm_policy_snssai_data_parseFromJSON() failed [sm_policy_dnn_data]");
			goto end;
		}
		if(cJSON_IsObject(sm_policy_dnn_data)) {
			sm_policy_dnn_dataList = OpenAPI_list_create();
			OpenAPI_map_t *localMapKeyPair = NULL;
			cJSON_ArrayForEach(sm_policy_dnn_data_local_map,
			                   sm_policy_dnn_data) {
				cJSON *localMapObject =
					sm_policy_dnn_data_local_map;
				if(cJSON_IsObject(localMapObject)) {
					localMapKeyPair = OpenAPI_map_create(
						ogs_strdup(
							localMapObject->string),
						OpenAPI_sm_policy_dnn_data_parseFromJSON
						        (localMapObject));
					...
					ogs_error(
						"OpenAPI_sm_policy_snssai_data_parseFromJSON() failed [inner]");
					goto end;
				}
				OpenAPI_list_add(sm_policy_dnn_dataList,
				                 localMapKeyPair);
			}
		}
	}
	...
	ue_slice_mbr =
		cJSON_GetObjectItemCaseSensitive(sm_policy_snssai_dataJSON,
		                                 "ueSliceMbr");
	if(ue_slice_mbr) {
		ue_slice_mbr_local_nonprim =
			OpenAPI_slice_mbr_1_parseFromJSON(ue_slice_mbr);
		if(!ue_slice_mbr_local_nonprim) {
			ogs_error(
				"OpenAPI_slice_mbr_1_parseFromJSON failed [ue_slice_mbr]");
			goto end;
		}
	}
...
OpenAPI_sm_policy_snssai_data_t *OpenAPI_sm_policy_snssai_data_copy(
	OpenAPI_sm_policy_snssai_data_t *dst,
	OpenAPI_sm_policy_snssai_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_sm_policy_snssai_data_convertToJSON() failed");
		return NULL;
	}
