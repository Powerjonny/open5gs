...
OpenAPI_inter_freq_target_info_1_t *OpenAPI_inter_freq_target_info_1_create(
	int dl_carrier_freq, OpenAPI_list_t *cell_id_list) {
	OpenAPI_inter_freq_target_info_1_t *inter_freq_target_info_1_local_var =
		ogs_malloc(sizeof(OpenAPI_inter_freq_target_info_1_t));
...
void OpenAPI_inter_freq_target_info_1_free(
	OpenAPI_inter_freq_target_info_1_t *inter_freq_target_info_1) {
	...
	if(inter_freq_target_info_1->cell_id_list) {
		OpenAPI_list_for_each(inter_freq_target_info_1->cell_id_list,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_inter_freq_target_info_1_convertToJSON(
	OpenAPI_inter_freq_target_info_1_t *inter_freq_target_info_1) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(inter_freq_target_info_1 == NULL) {
		ogs_error(
			"OpenAPI_inter_freq_target_info_1_convertToJSON() failed [InterFreqTargetInfo_1]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(cJSON_AddNumberToObject(item, "dlCarrierFreq",
	                           inter_freq_target_info_1->dl_carrier_freq) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_inter_freq_target_info_1_convertToJSON() failed [dl_carrier_freq]");
		goto end;
	}

	if(inter_freq_target_info_1->cell_id_list) {
		cJSON *cell_id_listList = cJSON_AddArrayToObject(item,
		                                                 "cellIdList");
		if(cell_id_listList == NULL) {
			ogs_error(
				"OpenAPI_inter_freq_target_info_1_convertToJSON() failed [cell_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(inter_freq_target_info_1->cell_id_list,
		                      node) {
			...
			if(cJSON_AddNumberToObject(cell_id_listList, "",
			                           *(double *) node->data) ==
			   NULL)
			{
				ogs_error(
...
OpenAPI_inter_freq_target_info_1_parseFromJSON(
	cJSON *inter_freq_target_info_1JSON) {
	OpenAPI_inter_freq_target_info_1_t *inter_freq_target_info_1_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	dl_carrier_freq =
		cJSON_GetObjectItemCaseSensitive(inter_freq_target_info_1JSON,
		                                 "dlCarrierFreq");
	if(!dl_carrier_freq) {
		ogs_error(
			"OpenAPI_inter_freq_target_info_1_parseFromJSON() failed [dl_carrier_freq]");
		goto end;
	}
	...
	if(cell_id_list) {
		...
		if(!cJSON_IsArray(cell_id_list)) {
			ogs_error(
				"OpenAPI_inter_freq_target_info_1_parseFromJSON() failed [cell_id_list]");
			goto end;
		}
	...
	inter_freq_target_info_1_local_var =
...
OpenAPI_inter_freq_target_info_1_t *OpenAPI_inter_freq_target_info_1_copy(
	OpenAPI_inter_freq_target_info_1_t	*dst,
	OpenAPI_inter_freq_target_info_1_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_inter_freq_target_info_1_convertToJSON() failed");
		return NULL;
	}
