...
OpenAPI_iptv_config_data_t *OpenAPI_iptv_config_data_create(char *supi,
                                                            bool
                                                            is_inter_group_id_null,
                                                            OpenAPI_any_type_t *
                                                            inter_group_id,
                                                            char
                                                                *dnn,
                                                            ...
                                                            OpenAPI_list_t
                                                                *reset_ids) {
	OpenAPI_iptv_config_data_t *iptv_config_data_local_var =
		ogs_malloc(sizeof(OpenAPI_iptv_config_data_t));
	ogs_assert(iptv_config_data_local_var);

	iptv_config_data_local_var->supi = supi;
	iptv_config_data_local_var->is_inter_group_id_null =
		is_inter_group_id_null;
...
void OpenAPI_iptv_config_data_free(
	...
	if(iptv_config_data->multi_acc_ctrls) {
		OpenAPI_list_for_each(iptv_config_data->multi_acc_ctrls, node) {
			...
			OpenAPI_multicast_access_control_free(
				localKeyValue->value);
...
cJSON *OpenAPI_iptv_config_data_convertToJSON(
	OpenAPI_iptv_config_data_t *iptv_config_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(iptv_config_data == NULL) {
		ogs_error(
			"OpenAPI_iptv_config_data_convertToJSON() failed [IptvConfigData]");
		return NULL;
	}
	...
	if(iptv_config_data->supi) {
		...
		{
			ogs_error(
				"OpenAPI_iptv_config_data_convertToJSON() failed [supi]");
			goto end;
		}
	}

	if(iptv_config_data->inter_group_id) {
		cJSON *inter_group_id_object =
			OpenAPI_any_type_convertToJSON(
				iptv_config_data->inter_group_id);
		if(inter_group_id_object == NULL) {
			ogs_error(
				"OpenAPI_iptv_config_data_convertToJSON() failed [inter_group_id]");
			goto end;
		}
		cJSON_AddItemToObject(item, "interGroupId",
		                      inter_group_id_object);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_iptv_config_data_convertToJSON() failed [inter_group_id]");
			goto end;
	...
	if(iptv_config_data->dnn) {
		...
		{
			ogs_error(
				"OpenAPI_iptv_config_data_convertToJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(iptv_config_data->snssai) {
		cJSON *snssai_local_JSON =
			OpenAPI_snssai_convertToJSON(iptv_config_data->snssai);
		if(snssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_iptv_config_data_convertToJSON() failed [snssai]");
			goto end;
		}
	...
	if(!iptv_config_data->af_app_id) {
		ogs_error(
			"OpenAPI_iptv_config_data_convertToJSON() failed [af_app_id]");
		return NULL;
	}
	...
	if(!iptv_config_data->multi_acc_ctrls) {
		ogs_error(
			"OpenAPI_iptv_config_data_convertToJSON() failed [multi_acc_ctrls]");
		return NULL;
	}
	...
	if(iptv_config_data->multi_acc_ctrls) {
		OpenAPI_list_for_each(iptv_config_data->multi_acc_ctrls, node) {
			...
			cJSON *itemLocal = localKeyValue->value ?
			                   OpenAPI_multicast_access_control_convertToJSON
			                           (localKeyValue->value) :
			                   cJSON_CreateNull();
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_iptv_config_data_convertToJSON() failed [inner]");
				goto end;
			}
	...
	if(iptv_config_data->supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_iptv_config_data_convertToJSON() failed [supp_feat]");
			goto end;
		}
	}
	...
	if(iptv_config_data->res_uri) {
		...
		{
			ogs_error(
				"OpenAPI_iptv_config_data_convertToJSON() failed [res_uri]");
			goto end;
		}
	}
	...
	if(iptv_config_data->reset_ids) {
		...
		if(reset_idsList == NULL) {
			ogs_error(
				"OpenAPI_iptv_config_data_convertToJSON() failed [reset_ids]");
			goto end;
		}
		OpenAPI_list_for_each(iptv_config_data->reset_ids, node) {
...
OpenAPI_iptv_config_data_t *OpenAPI_iptv_config_data_parseFromJSON(
	cJSON *iptv_config_dataJSON) {
	...
	if(supi) {
		...
		{
			ogs_error(
				"OpenAPI_iptv_config_data_parseFromJSON() failed [supi]");
			goto end;
		}
	}

	inter_group_id = cJSON_GetObjectItemCaseSensitive(iptv_config_dataJSON,
	                                                  "interGroupId");
	if(inter_group_id) {
		if(!cJSON_IsNull(inter_group_id)) {
			inter_group_id_local_object =
				OpenAPI_any_type_parseFromJSON(inter_group_id);
		}
	}
	...
	if(dnn) {
		...
		{
			ogs_error(
				"OpenAPI_iptv_config_data_parseFromJSON() failed [dnn]");
			goto end;
		}
	}

	snssai = cJSON_GetObjectItemCaseSensitive(iptv_config_dataJSON,
	                                          "snssai");
	if(snssai) {
	...
	af_app_id = cJSON_GetObjectItemCaseSensitive(iptv_config_dataJSON,
	                                             "afAppId");
	if(!af_app_id) {
		ogs_error(
			"OpenAPI_iptv_config_data_parseFromJSON() failed [af_app_id]");
		goto end;
	}
	...
	multi_acc_ctrls = cJSON_GetObjectItemCaseSensitive(iptv_config_dataJSON,
	                                                   "multiAccCtrls");
	if(!multi_acc_ctrls) {
		ogs_error(
			"OpenAPI_iptv_config_data_parseFromJSON() failed [multi_acc_ctrls]");
		goto end;
	}
	cJSON *multi_acc_ctrls_local_map = NULL;
	if(!cJSON_IsObject(multi_acc_ctrls) &&
	   !cJSON_IsNull(multi_acc_ctrls))
	{
		ogs_error(
			"OpenAPI_iptv_config_data_parseFromJSON() failed [multi_acc_ctrls]");
		goto end;
		...
		cJSON_ArrayForEach(multi_acc_ctrls_local_map, multi_acc_ctrls) {
			...
			if(cJSON_IsObject(localMapObject)) {
				localMapKeyPair = OpenAPI_map_create(
					...
					OpenAPI_multicast_access_control_parseFromJSON
					        (localMapObject));
				...
				ogs_error(
					"OpenAPI_iptv_config_data_parseFromJSON() failed [inner]");
				goto end;
			}
	...
	supp_feat = cJSON_GetObjectItemCaseSensitive(iptv_config_dataJSON,
	                                             "suppFeat");
	if(supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_iptv_config_data_parseFromJSON() failed [supp_feat]");
			goto end;
		}
	}

	res_uri = cJSON_GetObjectItemCaseSensitive(iptv_config_dataJSON,
	                                           "resUri");
	if(res_uri) {
		...
		{
			ogs_error(
				"OpenAPI_iptv_config_data_parseFromJSON() failed [res_uri]");
			goto end;
		}
	}

	reset_ids = cJSON_GetObjectItemCaseSensitive(iptv_config_dataJSON,
	                                             "resetIds");
	if(reset_ids) {
		cJSON *reset_ids_local = NULL;
		if(!cJSON_IsArray(reset_ids)) {
			ogs_error(
				"OpenAPI_iptv_config_data_parseFromJSON() failed [reset_ids]");
			goto end;
		}
	...
	iptv_config_data_local_var = OpenAPI_iptv_config_data_create(
		...
		inter_group_id &&
		cJSON_IsNull(inter_group_id) ? true : false,
...
OpenAPI_iptv_config_data_t *OpenAPI_iptv_config_data_copy(
	OpenAPI_iptv_config_data_t *dst, OpenAPI_iptv_config_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
