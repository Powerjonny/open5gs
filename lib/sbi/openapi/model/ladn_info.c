...
OpenAPI_ladn_info_t *OpenAPI_ladn_info_create(char			*ladn,
                                              OpenAPI_presence_state_e	presence)
{
	OpenAPI_ladn_info_t *ladn_info_local_var =
		ogs_malloc(sizeof(OpenAPI_ladn_info_t));
...
cJSON *OpenAPI_ladn_info_convertToJSON(OpenAPI_ladn_info_t *ladn_info) {
	...
	if(ladn_info == NULL) {
		ogs_error(
			"OpenAPI_ladn_info_convertToJSON() failed [LadnInfo]");
		return NULL;
	}
	...
	if(ladn_info->presence != OpenAPI_presence_state_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_ladn_info_convertToJSON() failed [presence]");
			goto end;
		}
	}
...
OpenAPI_ladn_info_t *OpenAPI_ladn_info_parseFromJSON(cJSON *ladn_infoJSON) {
	...
	if(presence) {
		if(!cJSON_IsString(presence)) {
			ogs_error(
				"OpenAPI_ladn_info_parseFromJSON() failed [presence]");
			goto end;
		}
		presenceVariable =
			OpenAPI_presence_state_FromString(
				presence->valuestring);
	}
...
OpenAPI_ladn_info_t *OpenAPI_ladn_info_copy(OpenAPI_ladn_info_t *dst,
                                            OpenAPI_ladn_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
