...
OpenAPI_traffic_control_data_t *OpenAPI_traffic_control_data_create(char *tc_id,
                                                                    ...
                                                                    *
                                                                    ...
                                                                    mute_notif,
                                                                    bool
                                                                    is_traffic_steering_pol_id_dl_null,
                                                                    char *
                                                                    traffic_steering_pol_id_dl,
                                                                    bool
                                                                    is_traffic_steering_pol_id_ul_null,
                                                                    char *
                                                                    traffic_steering_pol_id_ul,
                                                                    bool
                                                                    is_route_to_locs_null,
                                                                    OpenAPI_list_t
                                                                    *
                                                                    route_to_locs,
                                                                    bool
                                                                    is_max_allowed_up_lat_null,
                                                                    bool
                                                                    ...
                                                                    *
                                                                    ...
                                                                    sim_conn_term,
                                                                    bool
                                                                    is_up_path_chg_event_null,
                                                                    OpenAPI_up_path_chg_event_t
                                                                    *
                                                                    up_path_chg_event,
                                                                    OpenAPI_steering_functionality_e
                                                                    steer_fun,
                                                                    ...
                                                                    *
                                                                    ...
                                                                    mul_acc_ctrl)
{
	OpenAPI_traffic_control_data_t *traffic_control_data_local_var =
		ogs_malloc(sizeof(OpenAPI_traffic_control_data_t));
	...
	traffic_control_data_local_var->is_traffic_steering_pol_id_dl_null =
		is_traffic_steering_pol_id_dl_null;
	traffic_control_data_local_var->traffic_steering_pol_id_dl =
		traffic_steering_pol_id_dl;
	traffic_control_data_local_var->is_traffic_steering_pol_id_ul_null =
		is_traffic_steering_pol_id_ul_null;
	traffic_control_data_local_var->traffic_steering_pol_id_ul =
		traffic_steering_pol_id_ul;
	traffic_control_data_local_var->is_route_to_locs_null =
		is_route_to_locs_null;
	traffic_control_data_local_var->route_to_locs = route_to_locs;
	traffic_control_data_local_var->is_max_allowed_up_lat_null =
		is_max_allowed_up_lat_null;
	traffic_control_data_local_var->is_max_allowed_up_lat =
		is_max_allowed_up_lat;
	traffic_control_data_local_var->max_allowed_up_lat = max_allowed_up_lat;
	traffic_control_data_local_var->is_eas_ip_replace_infos_null =
		is_eas_ip_replace_infos_null;
	traffic_control_data_local_var->eas_ip_replace_infos =
		eas_ip_replace_infos;
	...
	traffic_control_data_local_var->is_up_path_chg_event_null =
		is_up_path_chg_event_null;
...
void OpenAPI_traffic_control_data_free(
	OpenAPI_traffic_control_data_t *traffic_control_data) {
	...
	if(traffic_control_data->redirect_info) {
		OpenAPI_redirect_information_free(
			traffic_control_data->redirect_info);
		traffic_control_data->redirect_info = NULL;
	}
	if(traffic_control_data->add_redirect_info) {
		OpenAPI_list_for_each(traffic_control_data->add_redirect_info,
		                      node) {
	...
	if(traffic_control_data->route_to_locs) {
		OpenAPI_list_for_each(traffic_control_data->route_to_locs,
		                      node) {
	...
	if(traffic_control_data->eas_ip_replace_infos) {
		OpenAPI_list_for_each(
			traffic_control_data->eas_ip_replace_infos, node) {
	...
	if(traffic_control_data->up_path_chg_event) {
		OpenAPI_up_path_chg_event_free(
			traffic_control_data->up_path_chg_event);
...
cJSON *OpenAPI_traffic_control_data_convertToJSON(
	OpenAPI_traffic_control_data_t *traffic_control_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(traffic_control_data == NULL) {
		ogs_error(
			"OpenAPI_traffic_control_data_convertToJSON() failed [TrafficControlData]");
		return NULL;
	}
	...
	if(!traffic_control_data->tc_id) {
		ogs_error(
			"OpenAPI_traffic_control_data_convertToJSON() failed [tc_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "tcId",
	                           traffic_control_data->tc_id) == NULL)
	{
		ogs_error(
			"OpenAPI_traffic_control_data_convertToJSON() failed [tc_id]");
	...
	if(traffic_control_data->flow_status != OpenAPI_flow_status_NULL) {
		if(cJSON_AddStringToObject(item, "flowStatus",
		                           OpenAPI_flow_status_ToString(
						   traffic_control_data->
						   flow_status)) == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [flow_status]");
			goto end;
		}
	}

	if(traffic_control_data->redirect_info) {
		cJSON *redirect_info_local_JSON =
			OpenAPI_redirect_information_convertToJSON(
				traffic_control_data->redirect_info);
		if(redirect_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [redirect_info]");
			goto end;
		}
		cJSON_AddItemToObject(item, "redirectInfo",
		                      redirect_info_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [redirect_info]");
			goto end;
	...
	if(traffic_control_data->add_redirect_info) {
		cJSON *add_redirect_infoList = cJSON_AddArrayToObject(item,
		                                                      "addRedirectInfo");
		if(add_redirect_infoList == NULL) {
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [add_redirect_info]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_control_data->add_redirect_info,
		                      node) {
	...
	if(traffic_control_data->is_mute_notif) {
		if(cJSON_AddBoolToObject(item, "muteNotif",
		                         traffic_control_data->mute_notif) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [mute_notif]");
			goto end;
		}
	}

	if(traffic_control_data->traffic_steering_pol_id_dl) {
		if(cJSON_AddStringToObject(item, "trafficSteeringPolIdDl",
		                           traffic_control_data->
		                           traffic_steering_pol_id_dl) == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [traffic_steering_pol_id_dl]");
			goto end;
		}
	} else if(traffic_control_data->is_traffic_steering_pol_id_dl_null) {
		if(cJSON_AddNullToObject(item,
		                         "trafficSteeringPolIdDl") == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [traffic_steering_pol_id_dl]");
			goto end;
	...
	if(traffic_control_data->traffic_steering_pol_id_ul) {
		if(cJSON_AddStringToObject(item, "trafficSteeringPolIdUl",
		                           traffic_control_data->
		                           traffic_steering_pol_id_ul) == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [traffic_steering_pol_id_ul]");
			goto end;
		}
	} else if(traffic_control_data->is_traffic_steering_pol_id_ul_null) {
		if(cJSON_AddNullToObject(item,
		                         "trafficSteeringPolIdUl") == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [traffic_steering_pol_id_ul]");
			goto end;
	...
	if(traffic_control_data->route_to_locs) {
		cJSON *route_to_locsList = cJSON_AddArrayToObject(item,
		                                                  "routeToLocs");
		if(route_to_locsList == NULL) {
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [route_to_locs]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_control_data->route_to_locs,
		                      node) {
	...
	if(traffic_control_data->is_max_allowed_up_lat) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [max_allowed_up_lat]");
			goto end;
		}
	...
	if(traffic_control_data->eas_ip_replace_infos) {
		cJSON *eas_ip_replace_infosList = cJSON_AddArrayToObject(item,
		                                                         "easIpReplaceInfos");
		if(eas_ip_replace_infosList == NULL) {
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [eas_ip_replace_infos]");
			goto end;
		}
		OpenAPI_list_for_each(
			traffic_control_data->eas_ip_replace_infos, node) {
			cJSON *itemLocal =
				OpenAPI_eas_ip_replacement_info_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_traffic_control_data_convertToJSON() failed [eas_ip_replace_infos]");
			...
			cJSON_AddItemToArray(eas_ip_replace_infosList,
			                     itemLocal);
		}
	...
	if(traffic_control_data->is_traff_corre_ind) {
		if(cJSON_AddBoolToObject(item, "traffCorreInd",
		                         traffic_control_data->traff_corre_ind)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [traff_corre_ind]");
			goto end;
		}
	}
	...
	if(traffic_control_data->is_sim_conn_ind) {
		if(cJSON_AddBoolToObject(item, "simConnInd",
		                         traffic_control_data->sim_conn_ind) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [sim_conn_ind]");
			goto end;
		}
	}
	...
	if(traffic_control_data->is_sim_conn_term) {
		if(cJSON_AddNumberToObject(item, "simConnTerm",
		                           traffic_control_data->sim_conn_term)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [sim_conn_term]");
			goto end;
		}
	}

	if(traffic_control_data->up_path_chg_event) {
		cJSON *up_path_chg_event_local_JSON =
			OpenAPI_up_path_chg_event_convertToJSON(
				traffic_control_data->up_path_chg_event);
		if(up_path_chg_event_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [up_path_chg_event]");
			goto end;
		}
		cJSON_AddItemToObject(item, "upPathChgEvent",
	...
	if(traffic_control_data->steer_fun !=
	   OpenAPI_steering_functionality_NULL)
	{
		if(cJSON_AddStringToObject(item, "steerFun",
		                           OpenAPI_steering_functionality_ToString
		                                   (traffic_control_data->
		                                   steer_fun)) == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [steer_fun]");
			goto end;
		}
	}

	if(traffic_control_data->steer_mode_dl) {
		cJSON *steer_mode_dl_local_JSON =
			OpenAPI_steering_mode_convertToJSON(
				traffic_control_data->steer_mode_dl);
		if(steer_mode_dl_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [steer_mode_dl]");
			goto end;
		}
		cJSON_AddItemToObject(item, "steerModeDl",
		                      steer_mode_dl_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [steer_mode_dl]");
			goto end;
	...
	if(traffic_control_data->steer_mode_ul) {
		cJSON *steer_mode_ul_local_JSON =
			OpenAPI_steering_mode_convertToJSON(
				traffic_control_data->steer_mode_ul);
		if(steer_mode_ul_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [steer_mode_ul]");
			goto end;
		}
		cJSON_AddItemToObject(item, "steerModeUl",
		                      steer_mode_ul_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [steer_mode_ul]");
			goto end;
	...
	if(traffic_control_data->mul_acc_ctrl !=
	   OpenAPI_npcf_multicast_access_control_NULL)
	{
		if(cJSON_AddStringToObject(item, "mulAccCtrl",
		                           OpenAPI_npcf_multicast_access_control_ToString
		                                   (traffic_control_data->
		                                   mul_acc_ctrl)) == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_control_data_convertToJSON() failed [mul_acc_ctrl]");
			goto end;
		}
	}
...
OpenAPI_traffic_control_data_t *OpenAPI_traffic_control_data_parseFromJSON(
	cJSON *traffic_control_dataJSON) {
	...
	tc_id = cJSON_GetObjectItemCaseSensitive(traffic_control_dataJSON,
	                                         "tcId");
	if(!tc_id) {
		ogs_error(
			"OpenAPI_traffic_control_data_parseFromJSON() failed [tc_id]");
		goto end;
	}
	...
	flow_status = cJSON_GetObjectItemCaseSensitive(traffic_control_dataJSON,
	                                               "flowStatus");
	if(flow_status) {
		if(!cJSON_IsString(flow_status)) {
			ogs_error(
				"OpenAPI_traffic_control_data_parseFromJSON() failed [flow_status]");
			goto end;
		}
		flow_statusVariable =
			OpenAPI_flow_status_FromString(
				flow_status->valuestring);
	}

	redirect_info =
		cJSON_GetObjectItemCaseSensitive(traffic_control_dataJSON,
		                                 "redirectInfo");
	if(redirect_info) {
		redirect_info_local_nonprim =
			OpenAPI_redirect_information_parseFromJSON(
				redirect_info);
		if(!redirect_info_local_nonprim) {
			ogs_error(
				"OpenAPI_redirect_information_parseFromJSON failed [redirect_info]");
			goto end;
		}
	}
	...
	if(add_redirect_info) {
		...
		if(!cJSON_IsArray(add_redirect_info)) {
			ogs_error(
				"OpenAPI_traffic_control_data_parseFromJSON() failed [add_redirect_info]");
			goto end;
		}
		...
		cJSON_ArrayForEach(add_redirect_info_local, add_redirect_info) {
			...
			OpenAPI_redirect_information_t *add_redirect_infoItem =
				OpenAPI_redirect_information_parseFromJSON(
					add_redirect_info_local);
	...
	mute_notif = cJSON_GetObjectItemCaseSensitive(traffic_control_dataJSON,
	                                              "muteNotif");
	if(mute_notif) {
		if(!cJSON_IsBool(mute_notif)) {
			ogs_error(
				"OpenAPI_traffic_control_data_parseFromJSON() failed [mute_notif]");
			goto end;
		}
	}
	...
	if(traffic_steering_pol_id_dl) {
		if(!cJSON_IsNull(traffic_steering_pol_id_dl)) {
			if(!cJSON_IsString(traffic_steering_pol_id_dl) &&
			   !cJSON_IsNull(traffic_steering_pol_id_dl))
			{
				ogs_error(
					"OpenAPI_traffic_control_data_parseFromJSON() failed [traffic_steering_pol_id_dl]");
				goto end;
			}
		}
	}
	...
	if(traffic_steering_pol_id_ul) {
		if(!cJSON_IsNull(traffic_steering_pol_id_ul)) {
			...
			{
				ogs_error(
					"OpenAPI_traffic_control_data_parseFromJSON() failed [traffic_steering_pol_id_ul]");
				goto end;
			}
		}
	}
	...
	if(route_to_locs) {
		if(!cJSON_IsNull(route_to_locs)) {
			...
			if(!cJSON_IsArray(route_to_locs)) {
				ogs_error(
					"OpenAPI_traffic_control_data_parseFromJSON() failed [route_to_locs]");
				goto end;
			}
			...
			cJSON_ArrayForEach(route_to_locs_local, route_to_locs) {
				...
				OpenAPI_route_to_location_t *route_to_locsItem =
				...
				OpenAPI_list_add(route_to_locsList,
				                 route_to_locsItem);
			}
		}
	}
	...
	if(max_allowed_up_lat) {
		if(!cJSON_IsNull(max_allowed_up_lat)) {
			if(!cJSON_IsNumber(max_allowed_up_lat)) {
				ogs_error(
					"OpenAPI_traffic_control_data_parseFromJSON() failed [max_allowed_up_lat]");
				goto end;
			}
		}
	}
	...
	if(eas_ip_replace_infos) {
		if(!cJSON_IsNull(eas_ip_replace_infos)) {
			...
			if(!cJSON_IsArray(eas_ip_replace_infos)) {
				ogs_error(
					"OpenAPI_traffic_control_data_parseFromJSON() failed [eas_ip_replace_infos]");
				goto end;
			}

			eas_ip_replace_infosList = OpenAPI_list_create();

			cJSON_ArrayForEach(eas_ip_replace_infos_local,
			                   eas_ip_replace_infos) {
				if(!cJSON_IsObject(
					   eas_ip_replace_infos_local))
				{
					ogs_error(
						"OpenAPI_traffic_control_data_parseFromJSON() failed [eas_ip_replace_infos]");
					goto end;
				...
				OpenAPI_eas_ip_replacement_info_t *
				        eas_ip_replace_infosItem =
					OpenAPI_eas_ip_replacement_info_parseFromJSON
					        (eas_ip_replace_infos_local);
				if(!eas_ip_replace_infosItem) {
					ogs_error(
						"No eas_ip_replace_infosItem");
					goto end;
				}
				OpenAPI_list_add(eas_ip_replace_infosList,
				                 eas_ip_replace_infosItem);
			}
		}
	}
	...
	if(sim_conn_ind) {
		if(!cJSON_IsBool(sim_conn_ind)) {
			ogs_error(
				"OpenAPI_traffic_control_data_parseFromJSON() failed [sim_conn_ind]");
			goto end;
		}
	}
	...
	if(sim_conn_term) {
		if(!cJSON_IsNumber(sim_conn_term)) {
			ogs_error(
				"OpenAPI_traffic_control_data_parseFromJSON() failed [sim_conn_term]");
			goto end;
		}
	}
	...
	if(up_path_chg_event) {
		if(!cJSON_IsNull(up_path_chg_event)) {
			...
			if(!up_path_chg_event_local_nonprim) {
				ogs_error(
					"OpenAPI_up_path_chg_event_parseFromJSON failed [up_path_chg_event]");
				goto end;
			}
		}
	}

	steer_fun = cJSON_GetObjectItemCaseSensitive(traffic_control_dataJSON,
	                                             "steerFun");
	if(steer_fun) {
		if(!cJSON_IsString(steer_fun)) {
			ogs_error(
				"OpenAPI_traffic_control_data_parseFromJSON() failed [steer_fun]");
			goto end;
		}
		steer_funVariable =
			OpenAPI_steering_functionality_FromString(
				steer_fun->valuestring);
	}
	...
	steer_mode_dl =
		cJSON_GetObjectItemCaseSensitive(traffic_control_dataJSON,
		                                 "steerModeDl");
	if(steer_mode_dl) {
		steer_mode_dl_local_nonprim =
			OpenAPI_steering_mode_parseFromJSON(steer_mode_dl);
		if(!steer_mode_dl_local_nonprim) {
			ogs_error(
				"OpenAPI_steering_mode_parseFromJSON failed [steer_mode_dl]");
			goto end;
		}
	}
	...
	steer_mode_ul =
		cJSON_GetObjectItemCaseSensitive(traffic_control_dataJSON,
		                                 "steerModeUl");
	if(steer_mode_ul) {
		steer_mode_ul_local_nonprim =
			OpenAPI_steering_mode_parseFromJSON(steer_mode_ul);
		if(!steer_mode_ul_local_nonprim) {
			ogs_error(
				"OpenAPI_steering_mode_parseFromJSON failed [steer_mode_ul]");
			goto end;
		}
	}
	...
	if(mul_acc_ctrl) {
		if(!cJSON_IsString(mul_acc_ctrl)) {
			ogs_error(
				"OpenAPI_traffic_control_data_parseFromJSON() failed [mul_acc_ctrl]");
			goto end;
		}
		mul_acc_ctrlVariable =
			OpenAPI_npcf_multicast_access_control_FromString(
				mul_acc_ctrl->valuestring);
	}
	...
	traffic_control_data_local_var = OpenAPI_traffic_control_data_create(
		...
		traffic_steering_pol_id_dl &&
		cJSON_IsNull(traffic_steering_pol_id_dl) ? true : false,
		traffic_steering_pol_id_dl &&
		!cJSON_IsNull(traffic_steering_pol_id_dl) ?
		ogs_strdup(traffic_steering_pol_id_dl->valuestring) : NULL,
		traffic_steering_pol_id_ul &&
		cJSON_IsNull(traffic_steering_pol_id_ul) ? true : false,
		traffic_steering_pol_id_ul &&
		!cJSON_IsNull(traffic_steering_pol_id_ul) ?
		ogs_strdup(traffic_steering_pol_id_ul->valuestring) : NULL,
		route_to_locs &&
		cJSON_IsNull(route_to_locs) ? true : false,
		route_to_locs ? route_to_locsList : NULL,
		max_allowed_up_lat &&
		cJSON_IsNull(max_allowed_up_lat) ? true : false,
		max_allowed_up_lat ? true : false,
		max_allowed_up_lat ? max_allowed_up_lat->valuedouble : 0,
		eas_ip_replace_infos &&
		cJSON_IsNull(eas_ip_replace_infos) ? true : false,
		...
		up_path_chg_event &&
		cJSON_IsNull(up_path_chg_event) ? true : false,
...
OpenAPI_traffic_control_data_t *OpenAPI_traffic_control_data_copy(
	OpenAPI_traffic_control_data_t	*dst,
	OpenAPI_traffic_control_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_traffic_control_data_convertToJSON() failed");
		return NULL;
	}
