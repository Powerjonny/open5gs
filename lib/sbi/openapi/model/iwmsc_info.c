...
OpenAPI_iwmsc_info_t *OpenAPI_iwmsc_info_create(OpenAPI_list_t	*msisdn_ranges,
                                                ...
                                                char		*sc_number) {
	OpenAPI_iwmsc_info_t *iwmsc_info_local_var =
		ogs_malloc(sizeof(OpenAPI_iwmsc_info_t));
...
cJSON *OpenAPI_iwmsc_info_convertToJSON(OpenAPI_iwmsc_info_t *iwmsc_info) {
	...
	if(iwmsc_info == NULL) {
		ogs_error(
			"OpenAPI_iwmsc_info_convertToJSON() failed [IwmscInfo]");
		return NULL;
	}
	...
	if(iwmsc_info->msisdn_ranges) {
		...
		if(msisdn_rangesList == NULL) {
			ogs_error(
				"OpenAPI_iwmsc_info_convertToJSON() failed [msisdn_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(iwmsc_info->msisdn_ranges, node) {
			cJSON *itemLocal =
	...
	if(iwmsc_info->supi_ranges) {
		...
		if(supi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_iwmsc_info_convertToJSON() failed [supi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(iwmsc_info->supi_ranges, node) {
			cJSON *itemLocal =
	...
	if(iwmsc_info->tai_range_list) {
		...
		if(tai_range_listList == NULL) {
			ogs_error(
				"OpenAPI_iwmsc_info_convertToJSON() failed [tai_range_list]");
			goto end;
		}
		OpenAPI_list_for_each(iwmsc_info->tai_range_list, node) {
			cJSON *itemLocal =
	...
	if(iwmsc_info->sc_number) {
		if(cJSON_AddStringToObject(item, "scNumber",
		                           iwmsc_info->sc_number) == NULL)
		{
			ogs_error(
				"OpenAPI_iwmsc_info_convertToJSON() failed [sc_number]");
			goto end;
		}
	}
...
OpenAPI_iwmsc_info_t *OpenAPI_iwmsc_info_parseFromJSON(cJSON *iwmsc_infoJSON) {
	...
	msisdn_ranges = cJSON_GetObjectItemCaseSensitive(iwmsc_infoJSON,
	                                                 "msisdnRanges");
	if(msisdn_ranges) {
		cJSON *msisdn_ranges_local = NULL;
		if(!cJSON_IsArray(msisdn_ranges)) {
			ogs_error(
				"OpenAPI_iwmsc_info_parseFromJSON() failed [msisdn_ranges]");
			goto end;
		}
	...
	supi_ranges = cJSON_GetObjectItemCaseSensitive(iwmsc_infoJSON,
	                                               "supiRanges");
	if(supi_ranges) {
		cJSON *supi_ranges_local = NULL;
		if(!cJSON_IsArray(supi_ranges)) {
			ogs_error(
				"OpenAPI_iwmsc_info_parseFromJSON() failed [supi_ranges]");
			goto end;
		}
	...
	tai_range_list = cJSON_GetObjectItemCaseSensitive(iwmsc_infoJSON,
	                                                  "taiRangeList");
	if(tai_range_list) {
		cJSON *tai_range_list_local = NULL;
		if(!cJSON_IsArray(tai_range_list)) {
			ogs_error(
				"OpenAPI_iwmsc_info_parseFromJSON() failed [tai_range_list]");
			goto end;
		}
	...
	sc_number = cJSON_GetObjectItemCaseSensitive(iwmsc_infoJSON,
	                                             "scNumber");
	if(sc_number) {
		if(!cJSON_IsString(sc_number) &&
		   !cJSON_IsNull(sc_number))
		{
			ogs_error(
				"OpenAPI_iwmsc_info_parseFromJSON() failed [sc_number]");
			goto end;
		}
	}
	...
	iwmsc_info_local_var = OpenAPI_iwmsc_info_create(
		...
		sc_number &&
		!cJSON_IsNull(sc_number) ? ogs_strdup(sc_number->valuestring) :
		NULL
		);
