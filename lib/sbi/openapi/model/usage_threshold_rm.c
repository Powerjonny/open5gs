...
OpenAPI_usage_threshold_rm_t *OpenAPI_usage_threshold_rm_create(
	bool is_duration_null, bool is_duration, int duration,
	bool is_total_volume_null, bool is_total_volume, long total_volume,
	bool is_downlink_volume_null, bool is_downlink_volume,
	long downlink_volume, bool is_uplink_volume_null, bool is_uplink_volume,
	long uplink_volume) {
	OpenAPI_usage_threshold_rm_t *usage_threshold_rm_local_var =
		ogs_malloc(sizeof(OpenAPI_usage_threshold_rm_t));
	...
	usage_threshold_rm_local_var->is_total_volume_null =
	...
	usage_threshold_rm_local_var->is_downlink_volume_null =
	...
	usage_threshold_rm_local_var->is_uplink_volume_null =
		is_uplink_volume_null;
...
void OpenAPI_usage_threshold_rm_free(
	OpenAPI_usage_threshold_rm_t *usage_threshold_rm) {
...
cJSON *OpenAPI_usage_threshold_rm_convertToJSON(
	...
	if(usage_threshold_rm == NULL) {
		ogs_error(
			"OpenAPI_usage_threshold_rm_convertToJSON() failed [UsageThresholdRm]");
		return NULL;
	}
	...
	if(usage_threshold_rm->is_duration) {
		if(cJSON_AddNumberToObject(item, "duration",
		                           usage_threshold_rm->duration) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_usage_threshold_rm_convertToJSON() failed [duration]");
			goto end;
		}
	...
	if(usage_threshold_rm->is_total_volume) {
		if(cJSON_AddNumberToObject(item, "totalVolume",
		                           usage_threshold_rm->total_volume) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_usage_threshold_rm_convertToJSON() failed [total_volume]");
			goto end;
		}
	...
	if(usage_threshold_rm->is_downlink_volume) {
		if(cJSON_AddNumberToObject(item, "downlinkVolume",
		                           usage_threshold_rm->downlink_volume)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_usage_threshold_rm_convertToJSON() failed [downlink_volume]");
			goto end;
		}
	...
	if(usage_threshold_rm->is_uplink_volume) {
		if(cJSON_AddNumberToObject(item, "uplinkVolume",
		                           usage_threshold_rm->uplink_volume) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_usage_threshold_rm_convertToJSON() failed [uplink_volume]");
			goto end;
		}
...
OpenAPI_usage_threshold_rm_t *OpenAPI_usage_threshold_rm_parseFromJSON(
	cJSON *usage_threshold_rmJSON) {
	...
	duration = cJSON_GetObjectItemCaseSensitive(usage_threshold_rmJSON,
	                                            "duration");
	if(duration) {
		if(!cJSON_IsNull(duration)) {
			if(!cJSON_IsNumber(duration)) {
				ogs_error(
					"OpenAPI_usage_threshold_rm_parseFromJSON() failed [duration]");
				goto end;
			}
		}
	}

	total_volume = cJSON_GetObjectItemCaseSensitive(usage_threshold_rmJSON,
	                                                "totalVolume");
	if(total_volume) {
		if(!cJSON_IsNull(total_volume)) {
			if(!cJSON_IsNumber(total_volume)) {
				ogs_error(
					"OpenAPI_usage_threshold_rm_parseFromJSON() failed [total_volume]");
				goto end;
			}
		}
	}
	...
	downlink_volume =
		cJSON_GetObjectItemCaseSensitive(usage_threshold_rmJSON,
		                                 "downlinkVolume");
	if(downlink_volume) {
		if(!cJSON_IsNull(downlink_volume)) {
			if(!cJSON_IsNumber(downlink_volume)) {
				ogs_error(
					"OpenAPI_usage_threshold_rm_parseFromJSON() failed [downlink_volume]");
				goto end;
			}
		}
	}

	uplink_volume = cJSON_GetObjectItemCaseSensitive(usage_threshold_rmJSON,
	                                                 "uplinkVolume");
	if(uplink_volume) {
		if(!cJSON_IsNull(uplink_volume)) {
			if(!cJSON_IsNumber(uplink_volume)) {
				ogs_error(
					"OpenAPI_usage_threshold_rm_parseFromJSON() failed [uplink_volume]");
				goto end;
			}
		}
	}
...
OpenAPI_usage_threshold_rm_t *OpenAPI_usage_threshold_rm_copy(
	OpenAPI_usage_threshold_rm_t *dst, OpenAPI_usage_threshold_rm_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
