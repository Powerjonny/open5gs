...
OpenAPI_amf_event_area_t *OpenAPI_amf_event_area_create(
	OpenAPI_presence_info_t *presence_info, OpenAPI_ladn_info_t *ladn_info,
	OpenAPI_snssai_t *s_nssai, char *nsi_id) {
	OpenAPI_amf_event_area_t *amf_event_area_local_var =
		ogs_malloc(sizeof(OpenAPI_amf_event_area_t));
...
cJSON *OpenAPI_amf_event_area_convertToJSON(
	OpenAPI_amf_event_area_t *amf_event_area) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(amf_event_area == NULL) {
		ogs_error(
			"OpenAPI_amf_event_area_convertToJSON() failed [AmfEventArea]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(amf_event_area->presence_info) {
		cJSON *presence_info_local_JSON =
			OpenAPI_presence_info_convertToJSON(
				amf_event_area->presence_info);
		if(presence_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_amf_event_area_convertToJSON() failed [presence_info]");
			goto end;
		}
	...
	if(amf_event_area->ladn_info) {
		cJSON *ladn_info_local_JSON =
			OpenAPI_ladn_info_convertToJSON(
				amf_event_area->ladn_info);
		if(ladn_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_amf_event_area_convertToJSON() failed [ladn_info]");
			goto end;
		}
	...
	if(amf_event_area->s_nssai) {
		cJSON *s_nssai_local_JSON =
			OpenAPI_snssai_convertToJSON(amf_event_area->s_nssai);
		if(s_nssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_amf_event_area_convertToJSON() failed [s_nssai]");
			goto end;
		}
	...
	if(amf_event_area->nsi_id) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_area_convertToJSON() failed [nsi_id]");
			goto end;
		}
	}
...
OpenAPI_amf_event_area_t *OpenAPI_amf_event_area_parseFromJSON(
	cJSON *amf_event_areaJSON) {
	...
	presence_info = cJSON_GetObjectItemCaseSensitive(amf_event_areaJSON,
	                                                 "presenceInfo");
	if(presence_info) {
		presence_info_local_nonprim =
			OpenAPI_presence_info_parseFromJSON(presence_info);
		if(!presence_info_local_nonprim) {
			ogs_error(
				"OpenAPI_presence_info_parseFromJSON failed [presence_info]");
			goto end;
		}
	}
	...
	ladn_info = cJSON_GetObjectItemCaseSensitive(amf_event_areaJSON,
	                                             "ladnInfo");
	if(ladn_info) {
		ladn_info_local_nonprim =
			OpenAPI_ladn_info_parseFromJSON(ladn_info);
		if(!ladn_info_local_nonprim) {
			ogs_error(
				"OpenAPI_ladn_info_parseFromJSON failed [ladn_info]");
			goto end;
		}
	}

	s_nssai = cJSON_GetObjectItemCaseSensitive(amf_event_areaJSON,
	                                           "sNssai");
	if(s_nssai) {
	...
	if(nsi_id) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_area_parseFromJSON() failed [nsi_id]");
			goto end;
		}
	}
...
OpenAPI_amf_event_area_t *OpenAPI_amf_event_area_copy(
	OpenAPI_amf_event_area_t *dst, OpenAPI_amf_event_area_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
