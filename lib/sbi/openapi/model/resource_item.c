...
{
	OpenAPI_resource_item_t *resource_item_local_var =
		ogs_malloc(sizeof(OpenAPI_resource_item_t));
...
cJSON *OpenAPI_resource_item_convertToJSON(
	OpenAPI_resource_item_t *resource_item) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(resource_item == NULL) {
		ogs_error(
			"OpenAPI_resource_item_convertToJSON() failed [ResourceItem]");
		return NULL;
	}
	...
	if(!resource_item->mon_resource_uri) {
		ogs_error(
			"OpenAPI_resource_item_convertToJSON() failed [mon_resource_uri]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "monResourceUri",
	                           resource_item->mon_resource_uri) == NULL)
	{
		ogs_error(
			"OpenAPI_resource_item_convertToJSON() failed [mon_resource_uri]");
	...
	if(!resource_item->items) {
		ogs_error(
			"OpenAPI_resource_item_convertToJSON() failed [items]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(resource_item->items, node) {
		if(cJSON_AddStringToObject(itemsList, "",
		                           (char *) node->data) == NULL)
		{
			ogs_error(
				"OpenAPI_resource_item_convertToJSON() failed [items]");
...
OpenAPI_resource_item_t *OpenAPI_resource_item_parseFromJSON(
	cJSON *resource_itemJSON) {
	OpenAPI_resource_item_t *resource_item_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *mon_resource_uri = NULL;
	cJSON *items = NULL;
	OpenAPI_list_t *itemsList = NULL;
	mon_resource_uri = cJSON_GetObjectItemCaseSensitive(resource_itemJSON,
	                                                    "monResourceUri");
	if(!mon_resource_uri) {
		ogs_error(
			"OpenAPI_resource_item_parseFromJSON() failed [mon_resource_uri]");
		goto end;
	}
	...
	if(!items) {
		ogs_error(
			"OpenAPI_resource_item_parseFromJSON() failed [items]");
		goto end;
	}
	...
	cJSON_ArrayForEach(items_local, items) {
		...
		OpenAPI_list_add(itemsList,
		                 ogs_strdup(items_local->valuestring));
	}
...
OpenAPI_resource_item_t *OpenAPI_resource_item_copy(
	OpenAPI_resource_item_t *dst, OpenAPI_resource_item_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
