...
OpenAPI_pp_data_entry_list_t *OpenAPI_pp_data_entry_list_create(
	...
	OpenAPI_pp_data_entry_list_t *pp_data_entry_list_local_var =
		ogs_malloc(sizeof(OpenAPI_pp_data_entry_list_t));
...
void OpenAPI_pp_data_entry_list_free(
	...
	if(pp_data_entry_list->pp_data_entry_list) {
		OpenAPI_list_for_each(pp_data_entry_list->pp_data_entry_list,
		                      node) {
...
cJSON *OpenAPI_pp_data_entry_list_convertToJSON(
	...
	if(pp_data_entry_list == NULL) {
		ogs_error(
			"OpenAPI_pp_data_entry_list_convertToJSON() failed [PpDataEntryList]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(pp_data_entry_list->pp_data_entry_list) {
		cJSON *pp_data_entry_listList = cJSON_AddArrayToObject(item,
		                                                       "ppDataEntryList");
		if(pp_data_entry_listList == NULL) {
			ogs_error(
				"OpenAPI_pp_data_entry_list_convertToJSON() failed [pp_data_entry_list]");
			goto end;
		}
		OpenAPI_list_for_each(pp_data_entry_list->pp_data_entry_list,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_pp_data_entry_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_pp_data_entry_list_convertToJSON() failed [pp_data_entry_list]");
				goto end;
	...
	if(pp_data_entry_list->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_pp_data_entry_list_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
...
OpenAPI_pp_data_entry_list_t *OpenAPI_pp_data_entry_list_parseFromJSON(
	...
	pp_data_entry_list =
		cJSON_GetObjectItemCaseSensitive(pp_data_entry_listJSON,
		                                 "ppDataEntryList");
	if(pp_data_entry_list) {
		cJSON *pp_data_entry_list_local = NULL;
		if(!cJSON_IsArray(pp_data_entry_list)) {
			ogs_error(
				"OpenAPI_pp_data_entry_list_parseFromJSON() failed [pp_data_entry_list]");
			goto end;
		}

		pp_data_entry_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(pp_data_entry_list_local,
		                   pp_data_entry_list) {
			...
			OpenAPI_pp_data_entry_t *pp_data_entry_listItem =
				OpenAPI_pp_data_entry_parseFromJSON(
					pp_data_entry_list_local);
			if(!pp_data_entry_listItem) {
				ogs_error("No pp_data_entry_listItem");
				goto end;
			}
			OpenAPI_list_add(pp_data_entry_listList,
			                 pp_data_entry_listItem);
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_pp_data_entry_list_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
