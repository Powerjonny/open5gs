...
OpenAPI_amf_cond_t *OpenAPI_amf_cond_create(char	*amf_set_id,
                                            char	*amf_region_id) {
	OpenAPI_amf_cond_t *amf_cond_local_var =
		ogs_malloc(sizeof(OpenAPI_amf_cond_t));
...
cJSON *OpenAPI_amf_cond_convertToJSON(OpenAPI_amf_cond_t *amf_cond) {
	...
	if(amf_cond->amf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_amf_cond_convertToJSON() failed [amf_set_id]");
			goto end;
		}
	}
	...
	if(amf_cond->amf_region_id) {
		...
		{
			ogs_error(
				"OpenAPI_amf_cond_convertToJSON() failed [amf_region_id]");
			goto end;
		}
	}
...
OpenAPI_amf_cond_t *OpenAPI_amf_cond_parseFromJSON(cJSON *amf_condJSON) {
	...
	if(amf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_amf_cond_parseFromJSON() failed [amf_set_id]");
			goto end;
		}
	}

	amf_region_id = cJSON_GetObjectItemCaseSensitive(amf_condJSON,
	                                                 "amfRegionId");
	if(amf_region_id) {
		...
		{
			ogs_error(
				"OpenAPI_amf_cond_parseFromJSON() failed [amf_region_id]");
			goto end;
		}
	}
