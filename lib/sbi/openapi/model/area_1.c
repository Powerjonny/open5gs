...
OpenAPI_area_1_t *OpenAPI_area_1_create(OpenAPI_list_t *tacs, char *area_code) {
	OpenAPI_area_1_t *area_1_local_var =
		ogs_malloc(sizeof(OpenAPI_area_1_t));
...
cJSON *OpenAPI_area_1_convertToJSON(OpenAPI_area_1_t *area_1) {
	...
	if(area_1->tacs) {
		...
		if(tacsList == NULL) {
			ogs_error(
				"OpenAPI_area_1_convertToJSON() failed [tacs]");
			goto end;
		}
		OpenAPI_list_for_each(area_1->tacs, node) {
	...
	if(area_1->area_code) {
		...
		{
			ogs_error(
				"OpenAPI_area_1_convertToJSON() failed [area_code]");
			goto end;
		}
	}
...
OpenAPI_area_1_t *OpenAPI_area_1_parseFromJSON(cJSON *area_1JSON) {
	...
	if(tacs) {
		...
		if(!cJSON_IsArray(tacs)) {
			ogs_error(
				"OpenAPI_area_1_parseFromJSON() failed [tacs]");
			goto end;
		}
		...
		cJSON_ArrayForEach(tacs_local, tacs) {
			...
			OpenAPI_list_add(tacsList,
			                 ogs_strdup(tacs_local->valuestring));
		}
	}
	...
	if(area_code) {
		...
		{
			ogs_error(
				"OpenAPI_area_1_parseFromJSON() failed [area_code]");
			goto end;
		}
	}
	...
	area_1_local_var = OpenAPI_area_1_create(
		tacs ? tacsList : NULL,
		area_code &&
		!cJSON_IsNull(area_code) ? ogs_strdup(area_code->valuestring) :
		NULL
		);
...
OpenAPI_area_1_t *OpenAPI_area_1_copy(OpenAPI_area_1_t	*dst,
                                      OpenAPI_area_1_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
