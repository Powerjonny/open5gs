...
OpenAPI_af_external_t *OpenAPI_af_external_create(char *af_id,
                                                  ...
                                                  valid_time_period) {
	OpenAPI_af_external_t *af_external_local_var =
		ogs_malloc(sizeof(OpenAPI_af_external_t));
	ogs_assert(af_external_local_var);

	af_external_local_var->af_id = af_id;
	af_external_local_var->allowed_geographic_area =
		allowed_geographic_area;
	af_external_local_var->privacy_check_related_action =
		privacy_check_related_action;
...
cJSON *OpenAPI_af_external_convertToJSON(OpenAPI_af_external_t *af_external) {
	...
	if(af_external == NULL) {
		ogs_error(
			"OpenAPI_af_external_convertToJSON() failed [AfExternal]");
		return NULL;
	}
	...
	if(af_external->af_id) {
		...
		{
			ogs_error(
				"OpenAPI_af_external_convertToJSON() failed [af_id]");
			goto end;
		}
	}
	...
	if(af_external->allowed_geographic_area) {
		...
		if(allowed_geographic_areaList == NULL) {
			ogs_error(
				"OpenAPI_af_external_convertToJSON() failed [allowed_geographic_area]");
			goto end;
		}
		OpenAPI_list_for_each(af_external->allowed_geographic_area,
		                      node) {
	   ...
	   OpenAPI_privacy_check_related_action_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_af_external_convertToJSON() failed [privacy_check_related_action]");
			goto end;
		}
	}
	...
	if(af_external->valid_time_period) {
		cJSON *valid_time_period_local_JSON =
			OpenAPI_valid_time_period_convertToJSON(
				af_external->valid_time_period);
		if(valid_time_period_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_af_external_convertToJSON() failed [valid_time_period]");
			goto end;
		}
...
OpenAPI_af_external_t *OpenAPI_af_external_parseFromJSON(
	...
	if(af_id) {
		...
		{
			ogs_error(
				"OpenAPI_af_external_parseFromJSON() failed [af_id]");
			goto end;
		}
	}

	allowed_geographic_area =
		cJSON_GetObjectItemCaseSensitive(af_externalJSON,
	...
	if(allowed_geographic_area) {
		...
		if(!cJSON_IsArray(allowed_geographic_area)) {
			ogs_error(
				"OpenAPI_af_external_parseFromJSON() failed [allowed_geographic_area]");
			goto end;
		}
	...
	if(privacy_check_related_action) {
		if(!cJSON_IsString(privacy_check_related_action)) {
			ogs_error(
				"OpenAPI_af_external_parseFromJSON() failed [privacy_check_related_action]");
			goto end;
		}
	...
	valid_time_period = cJSON_GetObjectItemCaseSensitive(af_externalJSON,
	                                                     "validTimePeriod");
