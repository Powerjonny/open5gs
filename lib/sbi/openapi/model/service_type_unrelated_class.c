...
OpenAPI_service_type_unrelated_class_create(int service_type,
	...
	OpenAPI_service_type_unrelated_class_t *
	        service_type_unrelated_class_local_var =
		ogs_malloc(sizeof(OpenAPI_service_type_unrelated_class_t));
	ogs_assert(service_type_unrelated_class_local_var);

	service_type_unrelated_class_local_var->service_type = service_type;
	service_type_unrelated_class_local_var->allowed_geographic_area =
		allowed_geographic_area;
	...
	service_type_unrelated_class_local_var->valid_time_period =
		valid_time_period;
...
void OpenAPI_service_type_unrelated_class_free(
	OpenAPI_service_type_unrelated_class_t *service_type_unrelated_class) {
	...
	if(service_type_unrelated_class->allowed_geographic_area) {
		OpenAPI_list_for_each(
			service_type_unrelated_class->allowed_geographic_area,
			node) {
	...
	if(service_type_unrelated_class->valid_time_period) {
		OpenAPI_valid_time_period_free(
			service_type_unrelated_class->valid_time_period);
		service_type_unrelated_class->valid_time_period = NULL;
	}
	if(service_type_unrelated_class->code_word_list) {
		OpenAPI_list_for_each(
			service_type_unrelated_class->code_word_list, node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_service_type_unrelated_class_convertToJSON(
	OpenAPI_service_type_unrelated_class_t *service_type_unrelated_class) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(service_type_unrelated_class == NULL) {
		ogs_error(
			"OpenAPI_service_type_unrelated_class_convertToJSON() failed [ServiceTypeUnrelatedClass]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(cJSON_AddNumberToObject(item, "serviceType",
	                           service_type_unrelated_class->service_type)
	   == NULL)
	{
		ogs_error(
			"OpenAPI_service_type_unrelated_class_convertToJSON() failed [service_type]");
		goto end;
	}

	if(service_type_unrelated_class->allowed_geographic_area) {
		cJSON *allowed_geographic_areaList =
			cJSON_AddArrayToObject(item, "allowedGeographicArea");
		if(allowed_geographic_areaList == NULL) {
			ogs_error(
				"OpenAPI_service_type_unrelated_class_convertToJSON() failed [allowed_geographic_area]");
			goto end;
		}
		OpenAPI_list_for_each(
			service_type_unrelated_class->allowed_geographic_area,
			...
			cJSON_AddItemToArray(allowed_geographic_areaList,
			                     itemLocal);
		}
	}
	...
	{
		if(cJSON_AddStringToObject(item, "privacyCheckRelatedAction",
		                           OpenAPI_privacy_check_related_action_ToString
		                                   (service_type_unrelated_class
		                                   ->
		                                   privacy_check_related_action))
		   ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_service_type_unrelated_class_convertToJSON() failed [privacy_check_related_action]");
			goto end;
		}
	}

	if(service_type_unrelated_class->code_word_ind !=
	   OpenAPI_code_word_ind_NULL)
	{
		if(cJSON_AddStringToObject(item, "codeWordInd",
		                           OpenAPI_code_word_ind_ToString(
						   service_type_unrelated_class
						   ->code_word_ind)) == NULL)
		{
			ogs_error(
				"OpenAPI_service_type_unrelated_class_convertToJSON() failed [code_word_ind]");
			goto end;
		}
	}
	...
	if(service_type_unrelated_class->valid_time_period) {
		cJSON *valid_time_period_local_JSON =
			OpenAPI_valid_time_period_convertToJSON(
				service_type_unrelated_class->valid_time_period);
		if(valid_time_period_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_service_type_unrelated_class_convertToJSON() failed [valid_time_period]");
			goto end;
		}
	...
	if(service_type_unrelated_class->code_word_list) {
		cJSON *code_word_listList = cJSON_AddArrayToObject(item,
		                                                   "codeWordList");
		if(code_word_listList == NULL) {
			ogs_error(
				"OpenAPI_service_type_unrelated_class_convertToJSON() failed [code_word_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			service_type_unrelated_class->code_word_list, node) {
			if(cJSON_AddStringToObject(code_word_listList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_service_type_unrelated_class_convertToJSON() failed [code_word_list]");
...
OpenAPI_service_type_unrelated_class_t *
OpenAPI_service_type_unrelated_class_parseFromJSON(
	cJSON *service_type_unrelated_classJSON) {
	OpenAPI_service_type_unrelated_class_t *
	        service_type_unrelated_class_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	OpenAPI_privacy_check_related_action_e
	        privacy_check_related_actionVariable = 0;
	...
	service_type =
		cJSON_GetObjectItemCaseSensitive(
			service_type_unrelated_classJSON, "serviceType");
	if(!service_type) {
		ogs_error(
			"OpenAPI_service_type_unrelated_class_parseFromJSON() failed [service_type]");
		goto end;
	}
	...
	allowed_geographic_area =
		cJSON_GetObjectItemCaseSensitive(
			service_type_unrelated_classJSON,
	...
	if(allowed_geographic_area) {
		...
		if(!cJSON_IsArray(allowed_geographic_area)) {
			ogs_error(
				"OpenAPI_service_type_unrelated_class_parseFromJSON() failed [allowed_geographic_area]");
			goto end;
		}

		allowed_geographic_areaList = OpenAPI_list_create();

		cJSON_ArrayForEach(allowed_geographic_area_local,
		                   allowed_geographic_area) {
			...
			OpenAPI_geographic_area_t *allowed_geographic_areaItem =
				OpenAPI_geographic_area_parseFromJSON(
					allowed_geographic_area_local);
			if(!allowed_geographic_areaItem) {
				ogs_error("No allowed_geographic_areaItem");
				goto end;
			}
			OpenAPI_list_add(allowed_geographic_areaList,
			                 allowed_geographic_areaItem);
		}
	}
	...
	if(privacy_check_related_action) {
		if(!cJSON_IsString(privacy_check_related_action)) {
			ogs_error(
				"OpenAPI_service_type_unrelated_class_parseFromJSON() failed [privacy_check_related_action]");
			goto end;
		}
		privacy_check_related_actionVariable =
			OpenAPI_privacy_check_related_action_FromString(
				privacy_check_related_action->valuestring);
	}

	code_word_ind =
		cJSON_GetObjectItemCaseSensitive(
			service_type_unrelated_classJSON, "codeWordInd");
	if(code_word_ind) {
		if(!cJSON_IsString(code_word_ind)) {
			ogs_error(
				"OpenAPI_service_type_unrelated_class_parseFromJSON() failed [code_word_ind]");
			goto end;
		}
		code_word_indVariable =
			OpenAPI_code_word_ind_FromString(
				code_word_ind->valuestring);
	}

	valid_time_period =
		cJSON_GetObjectItemCaseSensitive(
			service_type_unrelated_classJSON, "validTimePeriod");
	if(valid_time_period) {
		valid_time_period_local_nonprim =
			OpenAPI_valid_time_period_parseFromJSON(
				valid_time_period);
		if(!valid_time_period_local_nonprim) {
			ogs_error(
				"OpenAPI_valid_time_period_parseFromJSON failed [valid_time_period]");
			goto end;
		}
	}
	...
	code_word_list =
		cJSON_GetObjectItemCaseSensitive(
			service_type_unrelated_classJSON, "codeWordList");
	if(code_word_list) {
		cJSON *code_word_list_local = NULL;
		if(!cJSON_IsArray(code_word_list)) {
			ogs_error(
				"OpenAPI_service_type_unrelated_class_parseFromJSON() failed [code_word_list]");
			goto end;
		}
	...
	service_type_unrelated_class_local_var =
		OpenAPI_service_type_unrelated_class_create(

			service_type->valuedouble,
			allowed_geographic_area ? allowed_geographic_areaList :
			NULL,
			privacy_check_related_action ?
			privacy_check_related_actionVariable : 0,
...
OpenAPI_service_type_unrelated_class_t *
OpenAPI_service_type_unrelated_class_copy(
	OpenAPI_service_type_unrelated_class_t	*dst,
	OpenAPI_service_type_unrelated_class_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_service_type_unrelated_class_convertToJSON() failed");
		return NULL;
	}
