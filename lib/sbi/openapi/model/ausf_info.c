...
OpenAPI_ausf_info_t *OpenAPI_ausf_info_create(char		*group_id,
                                              OpenAPI_list_t	*supi_ranges,
                                              OpenAPI_list_t	*
                                              routing_indicators,
                                              OpenAPI_list_t *suci_infos) {
	OpenAPI_ausf_info_t *ausf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_ausf_info_t));
...
cJSON *OpenAPI_ausf_info_convertToJSON(OpenAPI_ausf_info_t *ausf_info) {
	...
	if(ausf_info == NULL) {
		ogs_error(
			"OpenAPI_ausf_info_convertToJSON() failed [AusfInfo]");
		return NULL;
	}
	...
	if(ausf_info->group_id) {
		...
		{
			ogs_error(
				"OpenAPI_ausf_info_convertToJSON() failed [group_id]");
			goto end;
		}
	}
	...
	if(ausf_info->supi_ranges) {
		...
		if(supi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_ausf_info_convertToJSON() failed [supi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(ausf_info->supi_ranges, node) {
			cJSON *itemLocal =
	...
	if(ausf_info->routing_indicators) {
		...
		if(routing_indicatorsList == NULL) {
			ogs_error(
				"OpenAPI_ausf_info_convertToJSON() failed [routing_indicators]");
			goto end;
		}
		OpenAPI_list_for_each(ausf_info->routing_indicators, node) {
	...
	if(ausf_info->suci_infos) {
		...
		if(suci_infosList == NULL) {
			ogs_error(
				"OpenAPI_ausf_info_convertToJSON() failed [suci_infos]");
			goto end;
		}
		OpenAPI_list_for_each(ausf_info->suci_infos, node) {
			cJSON *itemLocal =
...
OpenAPI_ausf_info_t *OpenAPI_ausf_info_parseFromJSON(cJSON *ausf_infoJSON) {
	...
	if(group_id) {
		...
		{
			ogs_error(
				"OpenAPI_ausf_info_parseFromJSON() failed [group_id]");
			goto end;
		}
	}

	supi_ranges = cJSON_GetObjectItemCaseSensitive(ausf_infoJSON,
	                                               "supiRanges");
	if(supi_ranges) {
		cJSON *supi_ranges_local = NULL;
		if(!cJSON_IsArray(supi_ranges)) {
			ogs_error(
				"OpenAPI_ausf_info_parseFromJSON() failed [supi_ranges]");
			goto end;
		}
	...
	routing_indicators = cJSON_GetObjectItemCaseSensitive(ausf_infoJSON,
	                                                      "routingIndicators");
	if(routing_indicators) {
		cJSON *routing_indicators_local = NULL;
		if(!cJSON_IsArray(routing_indicators)) {
			ogs_error(
				"OpenAPI_ausf_info_parseFromJSON() failed [routing_indicators]");
			goto end;
		}
	...
	suci_infos = cJSON_GetObjectItemCaseSensitive(ausf_infoJSON,
	                                              "suciInfos");
	if(suci_infos) {
		cJSON *suci_infos_local = NULL;
		if(!cJSON_IsArray(suci_infos)) {
			ogs_error(
				"OpenAPI_ausf_info_parseFromJSON() failed [suci_infos]");
			goto end;
		}
...
OpenAPI_ausf_info_t *OpenAPI_ausf_info_copy(OpenAPI_ausf_info_t *dst,
                                            OpenAPI_ausf_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
