...
OpenAPI_wireline_service_area_restriction_create(
	...
	OpenAPI_wireline_service_area_restriction_t *
		...
		ogs_malloc(sizeof(OpenAPI_wireline_service_area_restriction_t));
...
void OpenAPI_wireline_service_area_restriction_free(
	...
	if(wireline_service_area_restriction->areas) {
		OpenAPI_list_for_each(wireline_service_area_restriction->areas,
		                      node) {
...
cJSON *OpenAPI_wireline_service_area_restriction_convertToJSON(
	...
	if(wireline_service_area_restriction == NULL) {
		ogs_error(
			"OpenAPI_wireline_service_area_restriction_convertToJSON() failed [WirelineServiceAreaRestriction]");
		return NULL;
	}
	   ...
	   OpenAPI_restriction_type_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_wireline_service_area_restriction_convertToJSON() failed [restriction_type]");
			goto end;
		}
	}
	...
	if(wireline_service_area_restriction->areas) {
		...
		if(areasList == NULL) {
			ogs_error(
				"OpenAPI_wireline_service_area_restriction_convertToJSON() failed [areas]");
			goto end;
		}
		OpenAPI_list_for_each(wireline_service_area_restriction->areas,
		                      node) {
...
OpenAPI_wireline_service_area_restriction_parseFromJSON(
	...
	if(restriction_type) {
		if(!cJSON_IsString(restriction_type)) {
			ogs_error(
				"OpenAPI_wireline_service_area_restriction_parseFromJSON() failed [restriction_type]");
			goto end;
		}
	...
	if(areas) {
		...
		if(!cJSON_IsArray(areas)) {
			ogs_error(
				"OpenAPI_wireline_service_area_restriction_parseFromJSON() failed [areas]");
			goto end;
		}
		...
		cJSON_ArrayForEach(areas_local, areas) {
			...
			OpenAPI_wireline_area_t *areasItem =
...
OpenAPI_wireline_service_area_restriction_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_wireline_service_area_restriction_convertToJSON() failed");
		return NULL;
	}
