...
OpenAPI_pro_se_information_t *OpenAPI_pro_se_information_create(
	...
	OpenAPI_pro_se_information_t *pro_se_information_local_var =
...
void OpenAPI_pro_se_information_free(
...
cJSON *OpenAPI_pro_se_information_convertToJSON(
	OpenAPI_pro_se_information_t *pro_se_information) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(pro_se_information == NULL) {
		ogs_error(
			"OpenAPI_pro_se_information_convertToJSON() failed [ProSeInformation]");
		return NULL;
	}
	...
	if(pro_se_information->n2_pc5_pro_se_pol) {
		...
		if(n2_pc5_pro_se_pol_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_pro_se_information_convertToJSON() failed [n2_pc5_pro_se_pol]");
			goto end;
		}
...
OpenAPI_pro_se_information_t *OpenAPI_pro_se_information_parseFromJSON(
	...
	n2_pc5_pro_se_pol =
		cJSON_GetObjectItemCaseSensitive(pro_se_informationJSON,
		                                 "n2Pc5ProSePol");
...
OpenAPI_pro_se_information_t *OpenAPI_pro_se_information_copy(
	OpenAPI_pro_se_information_t *dst, OpenAPI_pro_se_information_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
