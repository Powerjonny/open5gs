...
OpenAPI_route_to_location_t *OpenAPI_route_to_location_create(char *dnai,
                                                              bool
                                                              is_route_info_null,
                                                              ...
                                                              *
                                                                route_info,
                                                              bool
                                                                is_route_prof_id_null,
                                                              char *
                                                              route_prof_id) {
	OpenAPI_route_to_location_t *route_to_location_local_var =
		ogs_malloc(sizeof(OpenAPI_route_to_location_t));
	...
	route_to_location_local_var->is_route_prof_id_null =
		is_route_prof_id_null;
...
cJSON *OpenAPI_route_to_location_convertToJSON(
	OpenAPI_route_to_location_t *route_to_location) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(route_to_location == NULL) {
		ogs_error(
			"OpenAPI_route_to_location_convertToJSON() failed [RouteToLocation]");
		return NULL;
	}
	...
	if(!route_to_location->dnai) {
		ogs_error(
			"OpenAPI_route_to_location_convertToJSON() failed [dnai]");
		return NULL;
	}
	...
	if(route_to_location->route_info) {
		cJSON *route_info_local_JSON =
			OpenAPI_route_information_convertToJSON(
				route_to_location->route_info);
		if(route_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_route_to_location_convertToJSON() failed [route_info]");
			goto end;
		}
	...
	if(route_to_location->route_prof_id) {
		if(cJSON_AddStringToObject(item, "routeProfId",
		                           route_to_location->route_prof_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_route_to_location_convertToJSON() failed [route_prof_id]");
			goto end;
		}
...
OpenAPI_route_to_location_t *OpenAPI_route_to_location_parseFromJSON(
	cJSON *route_to_locationJSON) {
	...
	if(!dnai) {
		ogs_error(
			"OpenAPI_route_to_location_parseFromJSON() failed [dnai]");
		goto end;
	}
	...
	route_info = cJSON_GetObjectItemCaseSensitive(route_to_locationJSON,
	                                              "routeInfo");
	if(route_info) {
		if(!cJSON_IsNull(route_info)) {
			route_info_local_nonprim =
				OpenAPI_route_information_parseFromJSON(
					route_info);
			if(!route_info_local_nonprim) {
				ogs_error(
					"OpenAPI_route_information_parseFromJSON failed [route_info]");
				goto end;
			}
		}
	}

	route_prof_id = cJSON_GetObjectItemCaseSensitive(route_to_locationJSON,
	                                                 "routeProfId");
	if(route_prof_id) {
		if(!cJSON_IsNull(route_prof_id)) {
			if(!cJSON_IsString(route_prof_id) &&
			   !cJSON_IsNull(route_prof_id))
			{
				ogs_error(
					"OpenAPI_route_to_location_parseFromJSON() failed [route_prof_id]");
				goto end;
			}
		}
	}
	...
	route_to_location_local_var = OpenAPI_route_to_location_create(
		...
		route_info &&
		cJSON_IsNull(route_info) ? true : false,
		route_info ? route_info_local_nonprim : NULL,
		route_prof_id &&
		cJSON_IsNull(route_prof_id) ? true : false,
		route_prof_id &&
		!cJSON_IsNull(route_prof_id) ?
		ogs_strdup(route_prof_id->valuestring) : NULL
		);
...
OpenAPI_route_to_location_t *OpenAPI_route_to_location_copy(
	OpenAPI_route_to_location_t *dst, OpenAPI_route_to_location_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
