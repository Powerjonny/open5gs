...
OpenAPI_ssc_modes_t *OpenAPI_ssc_modes_create(
	...
	OpenAPI_ssc_modes_t *ssc_modes_local_var =
		ogs_malloc(sizeof(OpenAPI_ssc_modes_t));
...
cJSON *OpenAPI_ssc_modes_convertToJSON(OpenAPI_ssc_modes_t *ssc_modes) {
	...
	if(ssc_modes == NULL) {
		ogs_error(
			"OpenAPI_ssc_modes_convertToJSON() failed [SscModes]");
		return NULL;
	}
	...
	if(ssc_modes->default_ssc_mode == OpenAPI_ssc_mode_NULL) {
		ogs_error(
			"OpenAPI_ssc_modes_convertToJSON() failed [default_ssc_mode]");
		return NULL;
	}
	...
	if(ssc_modes->allowed_ssc_modes != OpenAPI_ssc_mode_NULL) {
		...
		if(allowed_ssc_modesList == NULL) {
			ogs_error(
				"OpenAPI_ssc_modes_convertToJSON() failed [allowed_ssc_modes]");
			goto end;
		}
...
OpenAPI_ssc_modes_t *OpenAPI_ssc_modes_parseFromJSON(cJSON *ssc_modesJSON) {
	...
	default_ssc_mode = cJSON_GetObjectItemCaseSensitive(ssc_modesJSON,
	                                                    "defaultSscMode");
	if(!default_ssc_mode) {
		ogs_error(
			"OpenAPI_ssc_modes_parseFromJSON() failed [default_ssc_mode]");
		goto end;
	}
	...
	allowed_ssc_modes = cJSON_GetObjectItemCaseSensitive(ssc_modesJSON,
	                                                     "allowedSscModes");
	if(allowed_ssc_modes) {
		cJSON *allowed_ssc_modes_local = NULL;
		if(!cJSON_IsArray(allowed_ssc_modes)) {
			ogs_error(
				"OpenAPI_ssc_modes_parseFromJSON() failed [allowed_ssc_modes]");
			goto end;
		}
		...
		if(allowed_ssc_modesList->count == 0) {
			ogs_error(
				"OpenAPI_ssc_modes_parseFromJSON() failed: Expected allowed_ssc_modesList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
...
OpenAPI_ssc_modes_t *OpenAPI_ssc_modes_copy(OpenAPI_ssc_modes_t *dst,
                                            OpenAPI_ssc_modes_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
