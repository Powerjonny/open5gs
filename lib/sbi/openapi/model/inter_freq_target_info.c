...
OpenAPI_inter_freq_target_info_t *OpenAPI_inter_freq_target_info_create(
	...
	OpenAPI_inter_freq_target_info_t *inter_freq_target_info_local_var =
		ogs_malloc(sizeof(OpenAPI_inter_freq_target_info_t));
...
void OpenAPI_inter_freq_target_info_free(
	OpenAPI_inter_freq_target_info_t *inter_freq_target_info) {
	...
	if(inter_freq_target_info->cell_id_list) {
		OpenAPI_list_for_each(inter_freq_target_info->cell_id_list,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_inter_freq_target_info_convertToJSON(
	OpenAPI_inter_freq_target_info_t *inter_freq_target_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(inter_freq_target_info == NULL) {
		ogs_error(
			"OpenAPI_inter_freq_target_info_convertToJSON() failed [InterFreqTargetInfo]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(cJSON_AddNumberToObject(item, "dlCarrierFreq",
	                           inter_freq_target_info->dl_carrier_freq) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_inter_freq_target_info_convertToJSON() failed [dl_carrier_freq]");
		goto end;
	}
	...
	if(inter_freq_target_info->cell_id_list) {
		...
		if(cell_id_listList == NULL) {
			ogs_error(
				"OpenAPI_inter_freq_target_info_convertToJSON() failed [cell_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(inter_freq_target_info->cell_id_list,
		                      node) {
...
OpenAPI_inter_freq_target_info_t *OpenAPI_inter_freq_target_info_parseFromJSON(
	cJSON *inter_freq_target_infoJSON) {
	OpenAPI_inter_freq_target_info_t *inter_freq_target_info_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	dl_carrier_freq =
		cJSON_GetObjectItemCaseSensitive(inter_freq_target_infoJSON,
		                                 "dlCarrierFreq");
	if(!dl_carrier_freq) {
		ogs_error(
			"OpenAPI_inter_freq_target_info_parseFromJSON() failed [dl_carrier_freq]");
		goto end;
	}
	...
	if(cell_id_list) {
		...
		if(!cJSON_IsArray(cell_id_list)) {
			ogs_error(
				"OpenAPI_inter_freq_target_info_parseFromJSON() failed [cell_id_list]");
			goto end;
		}
...
OpenAPI_inter_freq_target_info_t *OpenAPI_inter_freq_target_info_copy(
	OpenAPI_inter_freq_target_info_t	*dst,
	OpenAPI_inter_freq_target_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_inter_freq_target_info_convertToJSON() failed");
		return NULL;
	}
