...
OpenAPI_pdu_session_t *OpenAPI_pdu_session_create(char *dnn,
                                                  char			*
                                                  smf_instance_id,
                                                  OpenAPI_plmn_id_t *plmn_id,
                                                  OpenAPI_snssai_t	*
                                                  single_nssai) {
	OpenAPI_pdu_session_t *pdu_session_local_var =
		ogs_malloc(sizeof(OpenAPI_pdu_session_t));
...
cJSON *OpenAPI_pdu_session_convertToJSON(OpenAPI_pdu_session_t *pdu_session) {
	...
	if(pdu_session == NULL) {
		ogs_error(
			"OpenAPI_pdu_session_convertToJSON() failed [PduSession]");
		return NULL;
	}
	...
	if(!pdu_session->smf_instance_id) {
		ogs_error(
			"OpenAPI_pdu_session_convertToJSON() failed [smf_instance_id]");
		return NULL;
	}
	...
	if(!pdu_session->plmn_id) {
		ogs_error(
			"OpenAPI_pdu_session_convertToJSON() failed [plmn_id]");
		return NULL;
	}
	cJSON *plmn_id_local_JSON =
		OpenAPI_plmn_id_convertToJSON(pdu_session->plmn_id);
	...
	if(pdu_session->single_nssai) {
		cJSON *single_nssai_local_JSON =
			OpenAPI_snssai_convertToJSON(pdu_session->single_nssai);
		if(single_nssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_pdu_session_convertToJSON() failed [single_nssai]");
			goto end;
		}
...
OpenAPI_pdu_session_t *OpenAPI_pdu_session_parseFromJSON(
	cJSON *pdu_sessionJSON) {
	...
	smf_instance_id = cJSON_GetObjectItemCaseSensitive(pdu_sessionJSON,
	                                                   "smfInstanceId");
	if(!smf_instance_id) {
		ogs_error(
			"OpenAPI_pdu_session_parseFromJSON() failed [smf_instance_id]");
		goto end;
	}
	...
	if(!plmn_id) {
		ogs_error(
			"OpenAPI_pdu_session_parseFromJSON() failed [plmn_id]");
		goto end;
	}
	...
	single_nssai = cJSON_GetObjectItemCaseSensitive(pdu_sessionJSON,
	                                                "singleNssai");
	if(single_nssai) {
...
OpenAPI_pdu_session_t *OpenAPI_pdu_session_copy(OpenAPI_pdu_session_t	*dst,
                                                OpenAPI_pdu_session_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
