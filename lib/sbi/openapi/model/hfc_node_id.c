...
OpenAPI_hfc_node_id_t *OpenAPI_hfc_node_id_create(char *hfc_nid) {
	OpenAPI_hfc_node_id_t *hfc_node_id_local_var =
		ogs_malloc(sizeof(OpenAPI_hfc_node_id_t));
...
cJSON *OpenAPI_hfc_node_id_convertToJSON(OpenAPI_hfc_node_id_t *hfc_node_id) {
	...
	if(hfc_node_id == NULL) {
		ogs_error(
			"OpenAPI_hfc_node_id_convertToJSON() failed [HfcNodeId]");
		return NULL;
	}
	...
	if(!hfc_node_id->hfc_nid) {
		ogs_error(
			"OpenAPI_hfc_node_id_convertToJSON() failed [hfc_nid]");
		return NULL;
	}
...
OpenAPI_hfc_node_id_t *OpenAPI_hfc_node_id_parseFromJSON(
	...
	if(!hfc_nid) {
		ogs_error(
			"OpenAPI_hfc_node_id_parseFromJSON() failed [hfc_nid]");
		goto end;
	}
