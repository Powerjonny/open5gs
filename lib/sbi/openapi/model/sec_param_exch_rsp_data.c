...
OpenAPI_sec_param_exch_rsp_data_t *OpenAPI_sec_param_exch_rsp_data_create(
	...
	OpenAPI_sec_param_exch_rsp_data_t *sec_param_exch_rsp_data_local_var =
		ogs_malloc(sizeof(OpenAPI_sec_param_exch_rsp_data_t));
	ogs_assert(sec_param_exch_rsp_data_local_var);

	sec_param_exch_rsp_data_local_var->n32f_context_id = n32f_context_id;
	sec_param_exch_rsp_data_local_var->selected_jwe_cipher_suite =
		selected_jwe_cipher_suite;
	sec_param_exch_rsp_data_local_var->selected_jws_cipher_suite =
		selected_jws_cipher_suite;
	sec_param_exch_rsp_data_local_var->sel_protection_policy_info =
		sel_protection_policy_info;
	sec_param_exch_rsp_data_local_var->ipx_provider_sec_info_list =
...
cJSON *OpenAPI_sec_param_exch_rsp_data_convertToJSON(
	...
	if(sec_param_exch_rsp_data == NULL) {
		ogs_error(
			"OpenAPI_sec_param_exch_rsp_data_convertToJSON() failed [SecParamExchRspData]");
		return NULL;
	}
	...
	if(!sec_param_exch_rsp_data->n32f_context_id) {
		ogs_error(
			"OpenAPI_sec_param_exch_rsp_data_convertToJSON() failed [n32f_context_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "n32fContextId",
	                           sec_param_exch_rsp_data->n32f_context_id) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_sec_param_exch_rsp_data_convertToJSON() failed [n32f_context_id]");
	...
	if(sec_param_exch_rsp_data->selected_jwe_cipher_suite) {
		if(cJSON_AddStringToObject(item, "selectedJweCipherSuite",
		                           sec_param_exch_rsp_data->
		                           selected_jwe_cipher_suite) == NULL)
		{
			ogs_error(
				"OpenAPI_sec_param_exch_rsp_data_convertToJSON() failed [selected_jwe_cipher_suite]");
			goto end;
		}
	}

	if(sec_param_exch_rsp_data->selected_jws_cipher_suite) {
		if(cJSON_AddStringToObject(item, "selectedJwsCipherSuite",
		                           sec_param_exch_rsp_data->
		                           selected_jws_cipher_suite) == NULL)
		{
			ogs_error(
				"OpenAPI_sec_param_exch_rsp_data_convertToJSON() failed [selected_jws_cipher_suite]");
			goto end;
		}
	}

	if(sec_param_exch_rsp_data->sel_protection_policy_info) {
		cJSON *sel_protection_policy_info_local_JSON =
			OpenAPI_protection_policy_convertToJSON(
				sec_param_exch_rsp_data->
				sel_protection_policy_info);
		if(sel_protection_policy_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sec_param_exch_rsp_data_convertToJSON() failed [sel_protection_policy_info]");
			goto end;
		}
		cJSON_AddItemToObject(item, "selProtectionPolicyInfo",
		                      sel_protection_policy_info_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_sec_param_exch_rsp_data_convertToJSON() failed [sel_protection_policy_info]");
			goto end;
	...
	if(sec_param_exch_rsp_data->ipx_provider_sec_info_list) {
		...
		if(ipx_provider_sec_info_listList == NULL) {
			ogs_error(
				"OpenAPI_sec_param_exch_rsp_data_convertToJSON() failed [ipx_provider_sec_info_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			sec_param_exch_rsp_data->ipx_provider_sec_info_list,
	...
	if(sec_param_exch_rsp_data->sender) {
		if(cJSON_AddStringToObject(item, "sender",
		                           sec_param_exch_rsp_data->sender) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sec_param_exch_rsp_data_convertToJSON() failed [sender]");
			goto end;
		}
	}
...
OpenAPI_sec_param_exch_rsp_data_t *OpenAPI_sec_param_exch_rsp_data_parseFromJSON
	...
	OpenAPI_protection_policy_t *sel_protection_policy_info_local_nonprim =
		NULL;
	cJSON *ipx_provider_sec_info_list = NULL;
	OpenAPI_list_t *ipx_provider_sec_info_listList = NULL;
	cJSON *sender = NULL;
	n32f_context_id =
		cJSON_GetObjectItemCaseSensitive(sec_param_exch_rsp_dataJSON,
		                                 "n32fContextId");
	if(!n32f_context_id) {
		ogs_error(
			"OpenAPI_sec_param_exch_rsp_data_parseFromJSON() failed [n32f_context_id]");
		goto end;
	}
	...
	if(selected_jwe_cipher_suite) {
		...
		{
			ogs_error(
				"OpenAPI_sec_param_exch_rsp_data_parseFromJSON() failed [selected_jwe_cipher_suite]");
			goto end;
		}
	}
	...
	if(selected_jws_cipher_suite) {
		if(!cJSON_IsString(selected_jws_cipher_suite) &&
		   !cJSON_IsNull(selected_jws_cipher_suite))
		{
			ogs_error(
				"OpenAPI_sec_param_exch_rsp_data_parseFromJSON() failed [selected_jws_cipher_suite]");
			goto end;
		}
	}
	...
	if(sel_protection_policy_info) {
		sel_protection_policy_info_local_nonprim =
			OpenAPI_protection_policy_parseFromJSON(
				sel_protection_policy_info);
		if(!sel_protection_policy_info_local_nonprim) {
			ogs_error(
				"OpenAPI_protection_policy_parseFromJSON failed [sel_protection_policy_info]");
			goto end;
		}
	}
	...
	if(ipx_provider_sec_info_list) {
		...
		if(!cJSON_IsArray(ipx_provider_sec_info_list)) {
			ogs_error(
				"OpenAPI_sec_param_exch_rsp_data_parseFromJSON() failed [ipx_provider_sec_info_list]");
			goto end;
		}
	...
	sender = cJSON_GetObjectItemCaseSensitive(sec_param_exch_rsp_dataJSON,
	                                          "sender");
	if(sender) {
		...
		{
			ogs_error(
				"OpenAPI_sec_param_exch_rsp_data_parseFromJSON() failed [sender]");
			goto end;
		}
	}

	sec_param_exch_rsp_data_local_var =
		OpenAPI_sec_param_exch_rsp_data_create(
			ogs_strdup(n32f_context_id->valuestring),
			selected_jwe_cipher_suite &&
			!cJSON_IsNull(selected_jwe_cipher_suite) ?
			ogs_strdup(selected_jwe_cipher_suite->valuestring) :
			NULL,
			selected_jws_cipher_suite &&
			!cJSON_IsNull(selected_jws_cipher_suite) ?
			ogs_strdup(selected_jws_cipher_suite->valuestring) :
			NULL,
			sel_protection_policy_info ?
...
OpenAPI_sec_param_exch_rsp_data_t *OpenAPI_sec_param_exch_rsp_data_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_sec_param_exch_rsp_data_convertToJSON() failed");
		return NULL;
	}
