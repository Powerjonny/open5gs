...
OpenAPI_mbs_binding_resp_t *OpenAPI_mbs_binding_resp_create(
	...
	OpenAPI_list_t
	        *pcf_ip_end_points) {
	OpenAPI_mbs_binding_resp_t *mbs_binding_resp_local_var =
		ogs_malloc(sizeof(OpenAPI_mbs_binding_resp_t));
...
void OpenAPI_mbs_binding_resp_free(
	OpenAPI_mbs_binding_resp_t *mbs_binding_resp) {
...
cJSON *OpenAPI_mbs_binding_resp_convertToJSON(
	OpenAPI_mbs_binding_resp_t *mbs_binding_resp) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(mbs_binding_resp == NULL) {
		ogs_error(
			"OpenAPI_mbs_binding_resp_convertToJSON() failed [MbsBindingResp]");
		return NULL;
	}
	...
	if(mbs_binding_resp->pcf_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_mbs_binding_resp_convertToJSON() failed [pcf_fqdn]");
			goto end;
		}
	}
	...
	if(mbs_binding_resp->pcf_ip_end_points) {
		...
		if(pcf_ip_end_pointsList == NULL) {
			ogs_error(
				"OpenAPI_mbs_binding_resp_convertToJSON() failed [pcf_ip_end_points]");
			goto end;
		}
		OpenAPI_list_for_each(mbs_binding_resp->pcf_ip_end_points,
		                      node) {
...
OpenAPI_mbs_binding_resp_t *OpenAPI_mbs_binding_resp_parseFromJSON(
	cJSON *mbs_binding_respJSON) {
	...
	pcf_fqdn = cJSON_GetObjectItemCaseSensitive(mbs_binding_respJSON,
	                                            "pcfFqdn");
	if(pcf_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_mbs_binding_resp_parseFromJSON() failed [pcf_fqdn]");
			goto end;
		}
	}

	pcf_ip_end_points =
		cJSON_GetObjectItemCaseSensitive(mbs_binding_respJSON,
		                                 "pcfIpEndPoints");
	if(pcf_ip_end_points) {
		cJSON *pcf_ip_end_points_local = NULL;
		if(!cJSON_IsArray(pcf_ip_end_points)) {
			ogs_error(
				"OpenAPI_mbs_binding_resp_parseFromJSON() failed [pcf_ip_end_points]");
			goto end;
		}
...
OpenAPI_mbs_binding_resp_t *OpenAPI_mbs_binding_resp_copy(
	OpenAPI_mbs_binding_resp_t *dst, OpenAPI_mbs_binding_resp_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
