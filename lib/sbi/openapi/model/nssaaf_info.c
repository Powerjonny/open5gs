...
OpenAPI_nssaaf_info_t *OpenAPI_nssaaf_info_create(
	...
	OpenAPI_nssaaf_info_t *nssaaf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_nssaaf_info_t));
	ogs_assert(nssaaf_info_local_var);

	nssaaf_info_local_var->supi_ranges = supi_ranges;
	nssaaf_info_local_var->internal_group_identifiers_ranges =
		internal_group_identifiers_ranges;
...
void OpenAPI_nssaaf_info_free(OpenAPI_nssaaf_info_t *nssaaf_info) {
	...
	if(nssaaf_info->internal_group_identifiers_ranges) {
		OpenAPI_list_for_each(
			nssaaf_info->internal_group_identifiers_ranges, node) {
...
cJSON *OpenAPI_nssaaf_info_convertToJSON(OpenAPI_nssaaf_info_t *nssaaf_info) {
	...
	if(nssaaf_info == NULL) {
		ogs_error(
			"OpenAPI_nssaaf_info_convertToJSON() failed [NssaafInfo]");
		return NULL;
	}
	...
	if(nssaaf_info->supi_ranges) {
		...
		if(supi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_nssaaf_info_convertToJSON() failed [supi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(nssaaf_info->supi_ranges, node) {
			cJSON *itemLocal =
	...
	if(nssaaf_info->internal_group_identifiers_ranges) {
		...
		if(internal_group_identifiers_rangesList == NULL) {
			ogs_error(
				"OpenAPI_nssaaf_info_convertToJSON() failed [internal_group_identifiers_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			nssaaf_info->internal_group_identifiers_ranges, node) {
...
OpenAPI_nssaaf_info_t *OpenAPI_nssaaf_info_parseFromJSON(
	cJSON *nssaaf_infoJSON) {
	...
	supi_ranges = cJSON_GetObjectItemCaseSensitive(nssaaf_infoJSON,
	                                               "supiRanges");
	if(supi_ranges) {
		cJSON *supi_ranges_local = NULL;
		if(!cJSON_IsArray(supi_ranges)) {
			ogs_error(
				"OpenAPI_nssaaf_info_parseFromJSON() failed [supi_ranges]");
			goto end;
		}
	...
	internal_group_identifiers_ranges =
		cJSON_GetObjectItemCaseSensitive(nssaaf_infoJSON,
		                                 "internalGroupIdentifiersRanges");
	if(internal_group_identifiers_ranges) {
		...
		if(!cJSON_IsArray(internal_group_identifiers_ranges)) {
			ogs_error(
				"OpenAPI_nssaaf_info_parseFromJSON() failed [internal_group_identifiers_ranges]");
			goto end;
		}
...
OpenAPI_nssaaf_info_t *OpenAPI_nssaaf_info_copy(OpenAPI_nssaaf_info_t	*dst,
                                                OpenAPI_nssaaf_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
