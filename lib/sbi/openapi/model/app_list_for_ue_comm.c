...
OpenAPI_app_list_for_ue_comm_t *OpenAPI_app_list_for_ue_comm_create(
	char *app_id, char *start_time, bool is_app_dur, int app_dur,
	bool is_occur_ratio, int occur_ratio,
	OpenAPI_network_area_info_t *spatial_validity) {
	OpenAPI_app_list_for_ue_comm_t *app_list_for_ue_comm_local_var =
		ogs_malloc(sizeof(OpenAPI_app_list_for_ue_comm_t));
...
void OpenAPI_app_list_for_ue_comm_free(
	OpenAPI_app_list_for_ue_comm_t *app_list_for_ue_comm) {
...
cJSON *OpenAPI_app_list_for_ue_comm_convertToJSON(
	OpenAPI_app_list_for_ue_comm_t *app_list_for_ue_comm) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(app_list_for_ue_comm == NULL) {
		ogs_error(
			"OpenAPI_app_list_for_ue_comm_convertToJSON() failed [AppListForUeComm]");
		return NULL;
	}
	...
	if(!app_list_for_ue_comm->app_id) {
		ogs_error(
			"OpenAPI_app_list_for_ue_comm_convertToJSON() failed [app_id]");
		return NULL;
	}
	...
	if(app_list_for_ue_comm->start_time) {
		...
		{
			ogs_error(
				"OpenAPI_app_list_for_ue_comm_convertToJSON() failed [start_time]");
			goto end;
		}
	}
	...
	if(app_list_for_ue_comm->is_app_dur) {
		...
		{
			ogs_error(
				"OpenAPI_app_list_for_ue_comm_convertToJSON() failed [app_dur]");
			goto end;
		}
	}
	...
	if(app_list_for_ue_comm->is_occur_ratio) {
		...
		{
			ogs_error(
				"OpenAPI_app_list_for_ue_comm_convertToJSON() failed [occur_ratio]");
			goto end;
		}
	}
	...
	if(app_list_for_ue_comm->spatial_validity) {
		...
		if(spatial_validity_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_app_list_for_ue_comm_convertToJSON() failed [spatial_validity]");
			goto end;
		}
...
OpenAPI_app_list_for_ue_comm_t *OpenAPI_app_list_for_ue_comm_parseFromJSON(
	...
	if(!app_id) {
		ogs_error(
			"OpenAPI_app_list_for_ue_comm_parseFromJSON() failed [app_id]");
		goto end;
	}
	...
	if(start_time) {
		...
		{
			ogs_error(
				"OpenAPI_app_list_for_ue_comm_parseFromJSON() failed [start_time]");
			goto end;
		}
	}
	...
	if(app_dur) {
		if(!cJSON_IsNumber(app_dur)) {
			ogs_error(
				"OpenAPI_app_list_for_ue_comm_parseFromJSON() failed [app_dur]");
			goto end;
		}
	}
	...
	if(occur_ratio) {
		if(!cJSON_IsNumber(occur_ratio)) {
			ogs_error(
				"OpenAPI_app_list_for_ue_comm_parseFromJSON() failed [occur_ratio]");
			goto end;
		}
	}

	spatial_validity =
		cJSON_GetObjectItemCaseSensitive(app_list_for_ue_commJSON,
		                                 "spatialValidity");
	if(spatial_validity) {
		...
		if(!spatial_validity_local_nonprim) {
			ogs_error(
				"OpenAPI_network_area_info_parseFromJSON failed [spatial_validity]");
			goto end;
		}
	}
	...
	app_list_for_ue_comm_local_var = OpenAPI_app_list_for_ue_comm_create(
		...
		start_time &&
		!cJSON_IsNull(start_time) ?
		ogs_strdup(start_time->valuestring) : NULL,
		app_dur ? true : false,
...
OpenAPI_app_list_for_ue_comm_t *OpenAPI_app_list_for_ue_comm_copy(
	OpenAPI_app_list_for_ue_comm_t	*dst,
	OpenAPI_app_list_for_ue_comm_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_app_list_for_ue_comm_convertToJSON() failed");
		return NULL;
	}
