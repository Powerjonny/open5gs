...
OpenAPI_guami_list_cond_t *OpenAPI_guami_list_cond_create(
	...
	OpenAPI_guami_list_cond_t *guami_list_cond_local_var =
		ogs_malloc(sizeof(OpenAPI_guami_list_cond_t));
...
cJSON *OpenAPI_guami_list_cond_convertToJSON(
	...
	if(guami_list_cond == NULL) {
		ogs_error(
			"OpenAPI_guami_list_cond_convertToJSON() failed [GuamiListCond]");
		return NULL;
	}
	...
	if(!guami_list_cond->guami_list) {
		ogs_error(
			"OpenAPI_guami_list_cond_convertToJSON() failed [guami_list]");
		return NULL;
	}
...
OpenAPI_guami_list_cond_t *OpenAPI_guami_list_cond_parseFromJSON(
	...
	guami_list = cJSON_GetObjectItemCaseSensitive(guami_list_condJSON,
	                                              "guamiList");
	if(!guami_list) {
		ogs_error(
			"OpenAPI_guami_list_cond_parseFromJSON() failed [guami_list]");
		goto end;
	}
