...
OpenAPI_location_info_t *OpenAPI_location_info_create(
	OpenAPI_user_location_t *loc, bool is_ratio, int ratio,
	bool is_confidence, int confidence) {
	OpenAPI_location_info_t *location_info_local_var =
		ogs_malloc(sizeof(OpenAPI_location_info_t));
...
cJSON *OpenAPI_location_info_convertToJSON(
	OpenAPI_location_info_t *location_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(location_info == NULL) {
		ogs_error(
			"OpenAPI_location_info_convertToJSON() failed [LocationInfo]");
		return NULL;
	}
	...
	cJSON *loc_local_JSON =
		OpenAPI_user_location_convertToJSON(location_info->loc);
	...
	if(location_info->is_ratio) {
		...
		{
			ogs_error(
				"OpenAPI_location_info_convertToJSON() failed [ratio]");
			goto end;
		}
	}
	...
	if(location_info->is_confidence) {
		...
		{
			ogs_error(
				"OpenAPI_location_info_convertToJSON() failed [confidence]");
			goto end;
		}
	}
...
OpenAPI_location_info_t *OpenAPI_location_info_parseFromJSON(
	cJSON *location_infoJSON) {
	...
	if(ratio) {
		if(!cJSON_IsNumber(ratio)) {
			ogs_error(
				"OpenAPI_location_info_parseFromJSON() failed [ratio]");
			goto end;
		}
	}
	...
	if(confidence) {
		if(!cJSON_IsNumber(confidence)) {
			ogs_error(
				"OpenAPI_location_info_parseFromJSON() failed [confidence]");
			goto end;
		}
	}
...
OpenAPI_location_info_t *OpenAPI_location_info_copy(
	OpenAPI_location_info_t *dst, OpenAPI_location_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
