...
OpenAPI_ip_multicast_address_info_t *OpenAPI_ip_multicast_address_info_create(
	...
	char *ipv6_mul_addr) {
	OpenAPI_ip_multicast_address_info_t *ip_multicast_address_info_local_var
	        = ogs_malloc(sizeof(OpenAPI_ip_multicast_address_info_t));
...
cJSON *OpenAPI_ip_multicast_address_info_convertToJSON(
	...
	if(ip_multicast_address_info == NULL) {
		ogs_error(
			"OpenAPI_ip_multicast_address_info_convertToJSON() failed [IpMulticastAddressInfo]");
		return NULL;
	}
	...
	if(ip_multicast_address_info->src_ipv4_addr) {
		if(cJSON_AddStringToObject(item, "srcIpv4Addr",
		                           ip_multicast_address_info->
		                           src_ipv4_addr) == NULL)
		{
			ogs_error(
				"OpenAPI_ip_multicast_address_info_convertToJSON() failed [src_ipv4_addr]");
			goto end;
		}
	}

	if(ip_multicast_address_info->ipv4_mul_addr) {
		if(cJSON_AddStringToObject(item, "ipv4MulAddr",
		                           ip_multicast_address_info->
		                           ipv4_mul_addr) == NULL)
		{
			ogs_error(
				"OpenAPI_ip_multicast_address_info_convertToJSON() failed [ipv4_mul_addr]");
			goto end;
		}
	}
	...
	if(ip_multicast_address_info->src_ipv6_addr) {
		...
		{
			ogs_error(
				"OpenAPI_ip_multicast_address_info_convertToJSON() failed [src_ipv6_addr]");
			goto end;
		}
	}

	if(ip_multicast_address_info->ipv6_mul_addr) {
		if(cJSON_AddStringToObject(item, "ipv6MulAddr",
		                           ip_multicast_address_info->
		                           ipv6_mul_addr) == NULL)
		{
			ogs_error(
				"OpenAPI_ip_multicast_address_info_convertToJSON() failed [ipv6_mul_addr]");
			goto end;
		}
	}
...
OpenAPI_ip_multicast_address_info_parseFromJSON(
	...
	src_ipv4_addr =
		cJSON_GetObjectItemCaseSensitive(ip_multicast_address_infoJSON,
		                                 "srcIpv4Addr");
	if(src_ipv4_addr) {
		...
		{
			ogs_error(
				"OpenAPI_ip_multicast_address_info_parseFromJSON() failed [src_ipv4_addr]");
			goto end;
		}
	}
	...
	if(ipv4_mul_addr) {
		if(!cJSON_IsString(ipv4_mul_addr) &&
		   !cJSON_IsNull(ipv4_mul_addr))
		{
			ogs_error(
				"OpenAPI_ip_multicast_address_info_parseFromJSON() failed [ipv4_mul_addr]");
			goto end;
		}
	}
	...
	if(src_ipv6_addr) {
		...
		{
			ogs_error(
				"OpenAPI_ip_multicast_address_info_parseFromJSON() failed [src_ipv6_addr]");
			goto end;
		}
	}
	...
	if(ipv6_mul_addr) {
		if(!cJSON_IsString(ipv6_mul_addr) &&
		   !cJSON_IsNull(ipv6_mul_addr))
		{
			ogs_error(
				"OpenAPI_ip_multicast_address_info_parseFromJSON() failed [ipv6_mul_addr]");
			goto end;
		}
	}

	ip_multicast_address_info_local_var =
		OpenAPI_ip_multicast_address_info_create(
			src_ipv4_addr &&
			!cJSON_IsNull(src_ipv4_addr) ?
			ogs_strdup(src_ipv4_addr->valuestring) : NULL,
			ipv4_mul_addr &&
			!cJSON_IsNull(ipv4_mul_addr) ?
			ogs_strdup(ipv4_mul_addr->valuestring) : NULL,
			src_ipv6_addr &&
			!cJSON_IsNull(src_ipv6_addr) ?
			ogs_strdup(src_ipv6_addr->valuestring) : NULL,
			ipv6_mul_addr &&
			!cJSON_IsNull(ipv6_mul_addr) ?
			ogs_strdup(ipv6_mul_addr->valuestring) : NULL
			);
...
OpenAPI_ip_multicast_address_info_t *OpenAPI_ip_multicast_address_info_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ip_multicast_address_info_convertToJSON() failed");
		return NULL;
	}
