...
OpenAPI_cag_data_t *OpenAPI_cag_data_create(OpenAPI_list_t *cag_infos,
                                            char		*
                                            provisioning_time) {
	OpenAPI_cag_data_t *cag_data_local_var =
		ogs_malloc(sizeof(OpenAPI_cag_data_t));
...
cJSON *OpenAPI_cag_data_convertToJSON(OpenAPI_cag_data_t *cag_data) {
	...
	if(!cag_data->cag_infos) {
		ogs_error(
			"OpenAPI_cag_data_convertToJSON() failed [cag_infos]");
		return NULL;
	}
	...
	if(cag_data->cag_infos) {
		OpenAPI_list_for_each(cag_data->cag_infos, node) {
			...
			cJSON *itemLocal = localKeyValue->value ?
			                   OpenAPI_cag_info_convertToJSON(
				localKeyValue->value) :
			                   cJSON_CreateNull();
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_cag_data_convertToJSON() failed [inner]");
				goto end;
			}
	...
	if(cag_data->provisioning_time) {
		...
		{
			ogs_error(
				"OpenAPI_cag_data_convertToJSON() failed [provisioning_time]");
			goto end;
		}
	}
...
OpenAPI_cag_data_t *OpenAPI_cag_data_parseFromJSON(cJSON *cag_dataJSON) {
	...
	if(!cag_infos) {
		ogs_error(
			"OpenAPI_cag_data_parseFromJSON() failed [cag_infos]");
		goto end;
	}
		...
		cJSON_ArrayForEach(cag_infos_local_map, cag_infos) {
			...
			if(cJSON_IsObject(localMapObject)) {
				localMapKeyPair = OpenAPI_map_create(
					...
					OpenAPI_cag_info_parseFromJSON(
						localMapObject));
				...
				ogs_error(
					"OpenAPI_cag_data_parseFromJSON() failed [inner]");
				goto end;
			}
	...
	if(provisioning_time) {
		...
		{
			ogs_error(
				"OpenAPI_cag_data_parseFromJSON() failed [provisioning_time]");
			goto end;
		}
	}
...
OpenAPI_cag_data_t *OpenAPI_cag_data_copy(OpenAPI_cag_data_t	*dst,
                                          OpenAPI_cag_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
