...
OpenAPI_service_experience_info_t *OpenAPI_service_experience_info_create(
	OpenAPI_svc_experience_t *svc_exprc, bool is_svc_exprc_variance,
	float svc_exprc_variance, OpenAPI_list_t *supis,
	OpenAPI_snssai_t *snssai, char *app_id,
	OpenAPI_service_experience_type_t *srv_expc_type,
	OpenAPI_list_t *ue_locs, OpenAPI_upf_information_t *upf_info,
	char *dnai, OpenAPI_addr_fqdn_t *app_server_inst, bool is_confidence,
	int confidence, char *dnn, OpenAPI_network_area_info_t *network_area,
	char *nsi_id, bool is_ratio, int ratio,
	OpenAPI_rat_freq_information_t *rat_freq) {
	OpenAPI_service_experience_info_t *service_experience_info_local_var =
		ogs_malloc(sizeof(OpenAPI_service_experience_info_t));
...
void OpenAPI_service_experience_info_free(
	OpenAPI_service_experience_info_t *service_experience_info) {
	...
	if(service_experience_info->srv_expc_type) {
		OpenAPI_service_experience_type_free(
	...
	if(service_experience_info->rat_freq) {
		OpenAPI_rat_freq_information_free(
...
cJSON *OpenAPI_service_experience_info_convertToJSON(
	OpenAPI_service_experience_info_t *service_experience_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(service_experience_info == NULL) {
		ogs_error(
			"OpenAPI_service_experience_info_convertToJSON() failed [ServiceExperienceInfo]");
		return NULL;
	}
	...
	if(!service_experience_info->svc_exprc) {
		ogs_error(
			"OpenAPI_service_experience_info_convertToJSON() failed [svc_exprc]");
		return NULL;
	}
	cJSON *svc_exprc_local_JSON =
		OpenAPI_svc_experience_convertToJSON(
	...
	if(service_experience_info->is_svc_exprc_variance) {
		...
		{
			ogs_error(
				"OpenAPI_service_experience_info_convertToJSON() failed [svc_exprc_variance]");
			goto end;
		}
	}
	...
	if(service_experience_info->supis) {
		...
		if(supisList == NULL) {
			ogs_error(
				"OpenAPI_service_experience_info_convertToJSON() failed [supis]");
			goto end;
		}
		OpenAPI_list_for_each(service_experience_info->supis, node) {
	...
	if(service_experience_info->snssai) {
		...
		if(snssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_service_experience_info_convertToJSON() failed [snssai]");
			goto end;
		}
	...
	if(service_experience_info->app_id) {
		...
		{
			ogs_error(
				"OpenAPI_service_experience_info_convertToJSON() failed [app_id]");
			goto end;
		}
	}
	...
	if(service_experience_info->srv_expc_type) {
		cJSON *srv_expc_type_local_JSON =
			OpenAPI_service_experience_type_convertToJSON(
				service_experience_info->srv_expc_type);
		if(srv_expc_type_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_service_experience_info_convertToJSON() failed [srv_expc_type]");
			goto end;
		}
	...
	if(service_experience_info->ue_locs) {
		...
		if(ue_locsList == NULL) {
			ogs_error(
				"OpenAPI_service_experience_info_convertToJSON() failed [ue_locs]");
			goto end;
		}
		OpenAPI_list_for_each(service_experience_info->ue_locs, node) {
			cJSON *itemLocal =
				OpenAPI_location_info_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_service_experience_info_convertToJSON() failed [ue_locs]");
				goto end;
	...
	if(service_experience_info->upf_info) {
		...
		if(upf_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_service_experience_info_convertToJSON() failed [upf_info]");
			goto end;
		}
	...
	if(service_experience_info->dnai) {
		...
		{
			ogs_error(
				"OpenAPI_service_experience_info_convertToJSON() failed [dnai]");
			goto end;
		}
	}

	if(service_experience_info->app_server_inst) {
		cJSON *app_server_inst_local_JSON =
			OpenAPI_addr_fqdn_convertToJSON(
				service_experience_info->app_server_inst);
		if(app_server_inst_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_service_experience_info_convertToJSON() failed [app_server_inst]");
			goto end;
		}
	...
	if(service_experience_info->is_confidence) {
		...
		{
			ogs_error(
				"OpenAPI_service_experience_info_convertToJSON() failed [confidence]");
			goto end;
		}
	}
	...
	if(service_experience_info->dnn) {
		...
		{
			ogs_error(
				"OpenAPI_service_experience_info_convertToJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(service_experience_info->network_area) {
		...
		if(network_area_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_service_experience_info_convertToJSON() failed [network_area]");
			goto end;
		}
	...
	if(service_experience_info->nsi_id) {
		...
		{
			ogs_error(
				"OpenAPI_service_experience_info_convertToJSON() failed [nsi_id]");
			goto end;
		}
	}
	...
	if(service_experience_info->is_ratio) {
		...
		{
			ogs_error(
				"OpenAPI_service_experience_info_convertToJSON() failed [ratio]");
			goto end;
		}
	}

	if(service_experience_info->rat_freq) {
		cJSON *rat_freq_local_JSON =
			OpenAPI_rat_freq_information_convertToJSON(
				service_experience_info->rat_freq);
		if(rat_freq_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_service_experience_info_convertToJSON() failed [rat_freq]");
			goto end;
		}
...
OpenAPI_service_experience_info_t *OpenAPI_service_experience_info_parseFromJSON
	...
	if(!svc_exprc) {
		ogs_error(
			"OpenAPI_service_experience_info_parseFromJSON() failed [svc_exprc]");
		goto end;
	}
	svc_exprc_local_nonprim =
		OpenAPI_svc_experience_parseFromJSON(svc_exprc);
	if(!svc_exprc_local_nonprim) {
		ogs_error(
			"OpenAPI_svc_experience_parseFromJSON failed [svc_exprc]");
		goto end;
	}
	...
	if(svc_exprc_variance) {
		if(!cJSON_IsNumber(svc_exprc_variance)) {
			ogs_error(
				"OpenAPI_service_experience_info_parseFromJSON() failed [svc_exprc_variance]");
			goto end;
		}
	}
	...
	if(supis) {
		...
		if(!cJSON_IsArray(supis)) {
			ogs_error(
				"OpenAPI_service_experience_info_parseFromJSON() failed [supis]");
			goto end;
		}
		...
		cJSON_ArrayForEach(supis_local, supis) {
			...
			OpenAPI_list_add(supisList,
			                 ogs_strdup(supis_local->valuestring));
		}
	}
	...
	if(app_id) {
		...
		{
			ogs_error(
				"OpenAPI_service_experience_info_parseFromJSON() failed [app_id]");
			goto end;
		}
	}
	...
	if(srv_expc_type) {
		...
		if(!srv_expc_type_local_nonprim) {
			ogs_error(
				"OpenAPI_service_experience_type_parseFromJSON failed [srv_expc_type]");
			goto end;
		}
	}
	...
	if(ue_locs) {
		...
		if(!cJSON_IsArray(ue_locs)) {
			ogs_error(
				"OpenAPI_service_experience_info_parseFromJSON() failed [ue_locs]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ue_locs_local, ue_locs) {
			...
			OpenAPI_location_info_t *ue_locsItem =
	...
	if(dnai) {
		...
		{
			ogs_error(
				"OpenAPI_service_experience_info_parseFromJSON() failed [dnai]");
			goto end;
		}
	}
	...
	if(app_server_inst) {
		app_server_inst_local_nonprim =
			OpenAPI_addr_fqdn_parseFromJSON(app_server_inst);
		if(!app_server_inst_local_nonprim) {
			ogs_error(
				"OpenAPI_addr_fqdn_parseFromJSON failed [app_server_inst]");
			goto end;
		}
	}
	...
	if(confidence) {
		if(!cJSON_IsNumber(confidence)) {
			ogs_error(
				"OpenAPI_service_experience_info_parseFromJSON() failed [confidence]");
			goto end;
		}
	}
	...
	if(dnn) {
		...
		{
			ogs_error(
				"OpenAPI_service_experience_info_parseFromJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(nsi_id) {
		...
		{
			ogs_error(
				"OpenAPI_service_experience_info_parseFromJSON() failed [nsi_id]");
			goto end;
		}
	}
	...
	if(ratio) {
		if(!cJSON_IsNumber(ratio)) {
			ogs_error(
				"OpenAPI_service_experience_info_parseFromJSON() failed [ratio]");
			goto end;
		}
	}
	...
	rat_freq = cJSON_GetObjectItemCaseSensitive(service_experience_infoJSON,
	                                            "ratFreq");
	if(rat_freq) {
		rat_freq_local_nonprim =
			OpenAPI_rat_freq_information_parseFromJSON(rat_freq);
		if(!rat_freq_local_nonprim) {
			ogs_error(
				"OpenAPI_rat_freq_information_parseFromJSON failed [rat_freq]");
			goto end;
		}
	}
	...
	service_experience_info_local_var =
		OpenAPI_service_experience_info_create(
			...
			svc_exprc_variance ? svc_exprc_variance->valuedouble :
			0,
			...
			nsi_id &&
			!cJSON_IsNull(nsi_id) ?
			ogs_strdup(nsi_id->valuestring) : NULL,
			ratio ? true : false,
...
OpenAPI_service_experience_info_t *OpenAPI_service_experience_info_copy(
	OpenAPI_service_experience_info_t	*dst,
	OpenAPI_service_experience_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_service_experience_info_convertToJSON() failed");
		return NULL;
	}
