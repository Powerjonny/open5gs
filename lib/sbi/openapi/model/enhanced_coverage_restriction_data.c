...
OpenAPI_enhanced_coverage_restriction_data_create(
	OpenAPI_list_t *plmn_ec_info_list) {
	OpenAPI_enhanced_coverage_restriction_data_t *
	        enhanced_coverage_restriction_data_local_var =
		ogs_malloc(
			sizeof(OpenAPI_enhanced_coverage_restriction_data_t));
	ogs_assert(enhanced_coverage_restriction_data_local_var);

	enhanced_coverage_restriction_data_local_var->plmn_ec_info_list =
		plmn_ec_info_list;

	return enhanced_coverage_restriction_data_local_var;
}

void OpenAPI_enhanced_coverage_restriction_data_free(
	OpenAPI_enhanced_coverage_restriction_data_t *
	...
	if(enhanced_coverage_restriction_data->plmn_ec_info_list) {
		OpenAPI_list_for_each(
			enhanced_coverage_restriction_data->plmn_ec_info_list,
			node) {
...
cJSON *OpenAPI_enhanced_coverage_restriction_data_convertToJSON(
	OpenAPI_enhanced_coverage_restriction_data_t *
	enhanced_coverage_restriction_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(enhanced_coverage_restriction_data == NULL) {
		ogs_error(
			"OpenAPI_enhanced_coverage_restriction_data_convertToJSON() failed [EnhancedCoverageRestrictionData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(enhanced_coverage_restriction_data->plmn_ec_info_list) {
		cJSON *plmn_ec_info_listList = cJSON_AddArrayToObject(item,
		                                                      "plmnEcInfoList");
		if(plmn_ec_info_listList == NULL) {
			ogs_error(
				"OpenAPI_enhanced_coverage_restriction_data_convertToJSON() failed [plmn_ec_info_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			enhanced_coverage_restriction_data->plmn_ec_info_list,
...
OpenAPI_enhanced_coverage_restriction_data_parseFromJSON(
	cJSON *enhanced_coverage_restriction_dataJSON) {
	OpenAPI_enhanced_coverage_restriction_data_t *
	        enhanced_coverage_restriction_data_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *plmn_ec_info_list = NULL;
	OpenAPI_list_t *plmn_ec_info_listList = NULL;
	plmn_ec_info_list =
		cJSON_GetObjectItemCaseSensitive(
			enhanced_coverage_restriction_dataJSON,
			"plmnEcInfoList");
	if(plmn_ec_info_list) {
		cJSON *plmn_ec_info_list_local = NULL;
		if(!cJSON_IsArray(plmn_ec_info_list)) {
			ogs_error(
				"OpenAPI_enhanced_coverage_restriction_data_parseFromJSON() failed [plmn_ec_info_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(plmn_ec_info_list_local, plmn_ec_info_list) {
			...
			OpenAPI_plmn_ec_info_t *plmn_ec_info_listItem =
				OpenAPI_plmn_ec_info_parseFromJSON(
					plmn_ec_info_list_local);
			if(!plmn_ec_info_listItem) {
				ogs_error("No plmn_ec_info_listItem");
				goto end;
			}
			OpenAPI_list_add(plmn_ec_info_listList,
			                 plmn_ec_info_listItem);
		}
	}
...
OpenAPI_enhanced_coverage_restriction_data_t *
OpenAPI_enhanced_coverage_restriction_data_copy(
	OpenAPI_enhanced_coverage_restriction_data_t	*dst,
	OpenAPI_enhanced_coverage_restriction_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_enhanced_coverage_restriction_data_convertToJSON() failed");
		return NULL;
	}
