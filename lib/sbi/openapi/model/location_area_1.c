...
OpenAPI_location_area_1_t *OpenAPI_location_area_1_create(
	...
	OpenAPI_location_area_1_t *location_area_1_local_var =
		ogs_malloc(sizeof(OpenAPI_location_area_1_t));
...
cJSON *OpenAPI_location_area_1_convertToJSON(
	OpenAPI_location_area_1_t *location_area_1) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(location_area_1 == NULL) {
		ogs_error(
			"OpenAPI_location_area_1_convertToJSON() failed [LocationArea_1]");
		return NULL;
	}
	...
	if(location_area_1->geographic_areas) {
		...
		if(geographic_areasList == NULL) {
			ogs_error(
				"OpenAPI_location_area_1_convertToJSON() failed [geographic_areas]");
			goto end;
		}
		OpenAPI_list_for_each(location_area_1->geographic_areas, node) {
			cJSON *itemLocal =
	...
	if(location_area_1->civic_addresses) {
		...
		if(civic_addressesList == NULL) {
			ogs_error(
				"OpenAPI_location_area_1_convertToJSON() failed [civic_addresses]");
			goto end;
		}
		OpenAPI_list_for_each(location_area_1->civic_addresses, node) {
			cJSON *itemLocal =
				OpenAPI_civic_address_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_location_area_1_convertToJSON() failed [civic_addresses]");
				goto end;
	...
	if(location_area_1->nw_area_info) {
		cJSON *nw_area_info_local_JSON =
			OpenAPI_network_area_info_1_convertToJSON(
				location_area_1->nw_area_info);
		if(nw_area_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_location_area_1_convertToJSON() failed [nw_area_info]");
			goto end;
		}
	...
	if(location_area_1->umt_time) {
		cJSON *umt_time_local_JSON =
			OpenAPI_umt_time_1_convertToJSON(
				location_area_1->umt_time);
		if(umt_time_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_location_area_1_convertToJSON() failed [umt_time]");
			goto end;
		}
...
OpenAPI_location_area_1_t *OpenAPI_location_area_1_parseFromJSON(
	cJSON *location_area_1JSON) {
	...
	geographic_areas = cJSON_GetObjectItemCaseSensitive(location_area_1JSON,
	                                                    "geographicAreas");
	if(geographic_areas) {
		cJSON *geographic_areas_local = NULL;
		if(!cJSON_IsArray(geographic_areas)) {
			ogs_error(
				"OpenAPI_location_area_1_parseFromJSON() failed [geographic_areas]");
			goto end;
		}
		...
		cJSON_ArrayForEach(geographic_areas_local, geographic_areas) {
			...
			OpenAPI_geographic_area_t *geographic_areasItem =
			...
			OpenAPI_list_add(geographic_areasList,
			                 geographic_areasItem);
		}
	}

	civic_addresses = cJSON_GetObjectItemCaseSensitive(location_area_1JSON,
	                                                   "civicAddresses");
	if(civic_addresses) {
		cJSON *civic_addresses_local = NULL;
		if(!cJSON_IsArray(civic_addresses)) {
			ogs_error(
				"OpenAPI_location_area_1_parseFromJSON() failed [civic_addresses]");
			goto end;
		}
		...
		cJSON_ArrayForEach(civic_addresses_local, civic_addresses) {
			...
			OpenAPI_civic_address_t *civic_addressesItem =
				OpenAPI_civic_address_parseFromJSON(
					civic_addresses_local);
			if(!civic_addressesItem) {
				ogs_error("No civic_addressesItem");
				goto end;
			}
			OpenAPI_list_add(civic_addressesList,
			                 civic_addressesItem);
		}
	}

	nw_area_info = cJSON_GetObjectItemCaseSensitive(location_area_1JSON,
	                                                "nwAreaInfo");
	if(nw_area_info) {
		nw_area_info_local_nonprim =
			OpenAPI_network_area_info_1_parseFromJSON(nw_area_info);
		if(!nw_area_info_local_nonprim) {
			ogs_error(
				"OpenAPI_network_area_info_1_parseFromJSON failed [nw_area_info]");
			goto end;
		}
	}

	umt_time = cJSON_GetObjectItemCaseSensitive(location_area_1JSON,
	                                            "umtTime");
	if(umt_time) {
		umt_time_local_nonprim =
			OpenAPI_umt_time_1_parseFromJSON(umt_time);
		if(!umt_time_local_nonprim) {
			ogs_error(
				"OpenAPI_umt_time_1_parseFromJSON failed [umt_time]");
			goto end;
		}
	}
...
OpenAPI_location_area_1_t *OpenAPI_location_area_1_copy(
	OpenAPI_location_area_1_t *dst, OpenAPI_location_area_1_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
