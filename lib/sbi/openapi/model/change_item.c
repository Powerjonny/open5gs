...
{
	OpenAPI_change_item_t *change_item_local_var =
		ogs_malloc(sizeof(OpenAPI_change_item_t));
...
cJSON *OpenAPI_change_item_convertToJSON(OpenAPI_change_item_t *change_item) {
	...
	if(change_item == NULL) {
		ogs_error(
			"OpenAPI_change_item_convertToJSON() failed [ChangeItem]");
		return NULL;
	}
	...
	if(cJSON_AddStringToObject(item, "op",
	                           OpenAPI_change_type_ToString(
					   change_item->op)) == NULL)
	{
		ogs_error("OpenAPI_change_item_convertToJSON() failed [op]");
		goto end;
	...
	if(change_item->from) {
		if(cJSON_AddStringToObject(item, "from",
		                           change_item->from) == NULL)
		{
			ogs_error(
				"OpenAPI_change_item_convertToJSON() failed [from]");
			goto end;
		}
	}

	if(change_item->orig_value) {
		cJSON *orig_value_object =
			OpenAPI_any_type_convertToJSON(change_item->orig_value);
		if(orig_value_object == NULL) {
			ogs_error(
				"OpenAPI_change_item_convertToJSON() failed [orig_value]");
			goto end;
		}
	...
	if(change_item->new_value) {
		cJSON *new_value_object =
			OpenAPI_any_type_convertToJSON(change_item->new_value);
		if(new_value_object == NULL) {
			ogs_error(
				"OpenAPI_change_item_convertToJSON() failed [new_value]");
			goto end;
		}
...
OpenAPI_change_item_t *OpenAPI_change_item_parseFromJSON(
	...
	if(from) {
		...
		{
			ogs_error(
				"OpenAPI_change_item_parseFromJSON() failed [from]");
			goto end;
		}
	}

	orig_value = cJSON_GetObjectItemCaseSensitive(change_itemJSON,
	                                              "origValue");
	if(orig_value) {
		if(!cJSON_IsNull(orig_value)) {
			orig_value_local_object =
				OpenAPI_any_type_parseFromJSON(orig_value);
		}
	}

	new_value = cJSON_GetObjectItemCaseSensitive(change_itemJSON,
	                                             "newValue");
	if(new_value) {
		if(!cJSON_IsNull(new_value)) {
			new_value_local_object =
				OpenAPI_any_type_parseFromJSON(new_value);
		}
	}
	...
	change_item_local_var = OpenAPI_change_item_create(
		...
		from &&
		!cJSON_IsNull(from) ? ogs_strdup(from->valuestring) : NULL,
		orig_value &&
		cJSON_IsNull(orig_value) ? true : false,
		orig_value ? orig_value_local_object : NULL,
		new_value &&
		cJSON_IsNull(new_value) ? true : false,
