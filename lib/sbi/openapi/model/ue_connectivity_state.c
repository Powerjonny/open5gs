...
OpenAPI_ue_connectivity_state_t *OpenAPI_ue_connectivity_state_create(
	...
	OpenAPI_ue_connectivity_state_t *ue_connectivity_state_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_connectivity_state_t));
...
void OpenAPI_ue_connectivity_state_free(
	OpenAPI_ue_connectivity_state_t *ue_connectivity_state) {
...
cJSON *OpenAPI_ue_connectivity_state_convertToJSON(
	OpenAPI_ue_connectivity_state_t *ue_connectivity_state) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(ue_connectivity_state == NULL) {
		ogs_error(
			"OpenAPI_ue_connectivity_state_convertToJSON() failed [UeConnectivityState]");
		return NULL;
	}
	...
	if(ue_connectivity_state->access_type == OpenAPI_access_type_NULL) {
		ogs_error(
			"OpenAPI_ue_connectivity_state_convertToJSON() failed [access_type]");
		return NULL;
	}
...
OpenAPI_ue_connectivity_state_t *OpenAPI_ue_connectivity_state_parseFromJSON(
	...
	access_type =
		cJSON_GetObjectItemCaseSensitive(ue_connectivity_stateJSON,
		                                 "accessType");
	if(!access_type) {
		ogs_error(
			"OpenAPI_ue_connectivity_state_parseFromJSON() failed [access_type]");
		goto end;
	}
	...
	if(connectivitystate) {
		connectivitystate_local_nonprim =
			OpenAPI_cm_state_parseFromJSON(connectivitystate);
		if(!connectivitystate_local_nonprim) {
			ogs_error(
				"OpenAPI_cm_state_parseFromJSON failed [connectivitystate]");
			goto end;
		}
	}
...
OpenAPI_ue_connectivity_state_t *OpenAPI_ue_connectivity_state_copy(
	OpenAPI_ue_connectivity_state_t *dst,
	OpenAPI_ue_connectivity_state_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ue_connectivity_state_convertToJSON() failed");
		return NULL;
	}
