...
OpenAPI_smf_info_t *OpenAPI_smf_info_create(
	...
	OpenAPI_smf_info_t *smf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_smf_info_t));
	...
	smf_info_local_var->is_smf_onboarding_capability =
		is_smf_onboarding_capability;
	smf_info_local_var->smf_onboarding_capability =
...
cJSON *OpenAPI_smf_info_convertToJSON(OpenAPI_smf_info_t *smf_info) {
	...
	if(!smf_info->s_nssai_smf_info_list) {
		ogs_error(
			"OpenAPI_smf_info_convertToJSON() failed [s_nssai_smf_info_list]");
		return NULL;
	}
	...
	if(smf_info->tai_list) {
		...
		if(tai_listList == NULL) {
			ogs_error(
				"OpenAPI_smf_info_convertToJSON() failed [tai_list]");
			goto end;
		}
		OpenAPI_list_for_each(smf_info->tai_list, node) {
			cJSON *itemLocal =
	...
	if(smf_info->tai_range_list) {
		...
		if(tai_range_listList == NULL) {
			ogs_error(
				"OpenAPI_smf_info_convertToJSON() failed [tai_range_list]");
			goto end;
		}
		OpenAPI_list_for_each(smf_info->tai_range_list, node) {
			cJSON *itemLocal =
	...
	if(smf_info->pgw_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_smf_info_convertToJSON() failed [pgw_fqdn]");
			goto end;
		}
	}
	...
	if(smf_info->pgw_ip_addr_list) {
		...
		if(pgw_ip_addr_listList == NULL) {
			ogs_error(
				"OpenAPI_smf_info_convertToJSON() failed [pgw_ip_addr_list]");
			goto end;
		}
		OpenAPI_list_for_each(smf_info->pgw_ip_addr_list, node) {
			cJSON *itemLocal =
	...
	if(smf_info->access_type != OpenAPI_access_type_NULL) {
		...
		if(access_typeList == NULL) {
			ogs_error(
				"OpenAPI_smf_info_convertToJSON() failed [access_type]");
			goto end;
		}
	...
	if(smf_info->is_priority) {
		if(cJSON_AddNumberToObject(item, "priority",
		                           smf_info->priority) == NULL)
		{
			ogs_error(
				"OpenAPI_smf_info_convertToJSON() failed [priority]");
			goto end;
		}
	}
	...
	if(smf_info->is_vsmf_support_ind) {
		if(cJSON_AddBoolToObject(item, "vsmfSupportInd",
		                         smf_info->vsmf_support_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_smf_info_convertToJSON() failed [vsmf_support_ind]");
			goto end;
		}
	}
	...
	if(smf_info->pgw_fqdn_list) {
		...
		if(pgw_fqdn_listList == NULL) {
			ogs_error(
				"OpenAPI_smf_info_convertToJSON() failed [pgw_fqdn_list]");
			goto end;
		}
		OpenAPI_list_for_each(smf_info->pgw_fqdn_list, node) {
	...
	if(smf_info->is_smf_onboarding_capability) {
		if(cJSON_AddBoolToObject(item, "smfOnboardingCapability",
		                         smf_info->smf_onboarding_capability) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_smf_info_convertToJSON() failed [smf_onboarding_capability]");
			goto end;
		}
	}
	...
	if(smf_info->is_ismf_support_ind) {
		if(cJSON_AddBoolToObject(item, "ismfSupportInd",
		                         smf_info->ismf_support_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_smf_info_convertToJSON() failed [ismf_support_ind]");
			goto end;
		}
	}
	...
	if(smf_info->is_smf_uprp_capability) {
		if(cJSON_AddBoolToObject(item, "smfUPRPCapability",
		                         smf_info->smf_uprp_capability) == NULL)
		{
			ogs_error(
				"OpenAPI_smf_info_convertToJSON() failed [smf_uprp_capability]");
			goto end;
		}
	}
...
OpenAPI_smf_info_t *OpenAPI_smf_info_parseFromJSON(cJSON *smf_infoJSON) {
	...
	s_nssai_smf_info_list = cJSON_GetObjectItemCaseSensitive(smf_infoJSON,
	                                                         "sNssaiSmfInfoList");
	if(!s_nssai_smf_info_list) {
		ogs_error(
			"OpenAPI_smf_info_parseFromJSON() failed [s_nssai_smf_info_list]");
		goto end;
	}
	...
	if(tai_list) {
		...
		if(!cJSON_IsArray(tai_list)) {
			ogs_error(
				"OpenAPI_smf_info_parseFromJSON() failed [tai_list]");
			goto end;
		}
	...
	tai_range_list = cJSON_GetObjectItemCaseSensitive(smf_infoJSON,
	                                                  "taiRangeList");
	if(tai_range_list) {
		cJSON *tai_range_list_local = NULL;
		if(!cJSON_IsArray(tai_range_list)) {
			ogs_error(
				"OpenAPI_smf_info_parseFromJSON() failed [tai_range_list]");
			goto end;
		}
	...
	if(pgw_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_smf_info_parseFromJSON() failed [pgw_fqdn]");
			goto end;
		}
	}

	pgw_ip_addr_list = cJSON_GetObjectItemCaseSensitive(smf_infoJSON,
	                                                    "pgwIpAddrList");
	if(pgw_ip_addr_list) {
		cJSON *pgw_ip_addr_list_local = NULL;
		if(!cJSON_IsArray(pgw_ip_addr_list)) {
			ogs_error(
				"OpenAPI_smf_info_parseFromJSON() failed [pgw_ip_addr_list]");
			goto end;
		}
	...
	access_type = cJSON_GetObjectItemCaseSensitive(smf_infoJSON,
	                                               "accessType");
	if(access_type) {
		cJSON *access_type_local = NULL;
		if(!cJSON_IsArray(access_type)) {
			ogs_error(
				"OpenAPI_smf_info_parseFromJSON() failed [access_type]");
			goto end;
		}
		...
		if(access_typeList->count == 0) {
			ogs_error(
				"OpenAPI_smf_info_parseFromJSON() failed: Expected access_typeList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(priority) {
		if(!cJSON_IsNumber(priority)) {
			ogs_error(
				"OpenAPI_smf_info_parseFromJSON() failed [priority]");
			goto end;
		}
	}

	vsmf_support_ind = cJSON_GetObjectItemCaseSensitive(smf_infoJSON,
	                                                    "vsmfSupportInd");
	if(vsmf_support_ind) {
		if(!cJSON_IsBool(vsmf_support_ind)) {
			ogs_error(
				"OpenAPI_smf_info_parseFromJSON() failed [vsmf_support_ind]");
			goto end;
		}
	}

	pgw_fqdn_list = cJSON_GetObjectItemCaseSensitive(smf_infoJSON,
	                                                 "pgwFqdnList");
	if(pgw_fqdn_list) {
		cJSON *pgw_fqdn_list_local = NULL;
		if(!cJSON_IsArray(pgw_fqdn_list)) {
			ogs_error(
				"OpenAPI_smf_info_parseFromJSON() failed [pgw_fqdn_list]");
			goto end;
		}
	...
	smf_onboarding_capability =
		cJSON_GetObjectItemCaseSensitive(smf_infoJSON,
		                                 "smfOnboardingCapability");
	if(smf_onboarding_capability) {
		if(!cJSON_IsBool(smf_onboarding_capability)) {
			ogs_error(
				"OpenAPI_smf_info_parseFromJSON() failed [smf_onboarding_capability]");
			goto end;
		}
	}

	ismf_support_ind = cJSON_GetObjectItemCaseSensitive(smf_infoJSON,
	                                                    "ismfSupportInd");
	if(ismf_support_ind) {
		if(!cJSON_IsBool(ismf_support_ind)) {
			ogs_error(
				"OpenAPI_smf_info_parseFromJSON() failed [ismf_support_ind]");
			goto end;
		}
	}

	smf_uprp_capability = cJSON_GetObjectItemCaseSensitive(smf_infoJSON,
	                                                       "smfUPRPCapability");
	if(smf_uprp_capability) {
		if(!cJSON_IsBool(smf_uprp_capability)) {
			ogs_error(
				"OpenAPI_smf_info_parseFromJSON() failed [smf_uprp_capability]");
			goto end;
		}
	}
...
OpenAPI_smf_info_t *OpenAPI_smf_info_copy(OpenAPI_smf_info_t	*dst,
                                          OpenAPI_smf_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
