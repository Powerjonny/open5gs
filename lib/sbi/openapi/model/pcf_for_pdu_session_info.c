...
OpenAPI_pcf_for_pdu_session_info_t *OpenAPI_pcf_for_pdu_session_info_create(
	char *dnn, OpenAPI_snssai_t *snssai, char *pcf_fqdn,
	OpenAPI_list_t *pcf_ip_end_points, bool is_ipv4_addr_null,
	char *ipv4_addr, char *ip_domain, OpenAPI_list_t *ipv6_prefixes,
	OpenAPI_list_t *mac_addrs, char *pcf_id, char *pcf_set_id,
	OpenAPI_binding_level_e bind_level) {
	OpenAPI_pcf_for_pdu_session_info_t *pcf_for_pdu_session_info_local_var =
		ogs_malloc(sizeof(OpenAPI_pcf_for_pdu_session_info_t));
	...
	pcf_for_pdu_session_info_local_var->pcf_ip_end_points =
		pcf_ip_end_points;
	pcf_for_pdu_session_info_local_var->is_ipv4_addr_null =
		is_ipv4_addr_null;
...
void OpenAPI_pcf_for_pdu_session_info_free(
	OpenAPI_pcf_for_pdu_session_info_t *pcf_for_pdu_session_info) {
	...
	if(pcf_for_pdu_session_info->pcf_ip_end_points) {
		OpenAPI_list_for_each(
			pcf_for_pdu_session_info->pcf_ip_end_points, node) {
	...
	if(pcf_for_pdu_session_info->ipv6_prefixes) {
		OpenAPI_list_for_each(pcf_for_pdu_session_info->ipv6_prefixes,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_pcf_for_pdu_session_info_convertToJSON(
	OpenAPI_pcf_for_pdu_session_info_t *pcf_for_pdu_session_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(pcf_for_pdu_session_info == NULL) {
		ogs_error(
			"OpenAPI_pcf_for_pdu_session_info_convertToJSON() failed [PcfForPduSessionInfo]");
		return NULL;
	}
	...
	if(!pcf_for_pdu_session_info->dnn) {
		ogs_error(
			"OpenAPI_pcf_for_pdu_session_info_convertToJSON() failed [dnn]");
		return NULL;
	}
	...
	if(!pcf_for_pdu_session_info->snssai) {
		ogs_error(
			"OpenAPI_pcf_for_pdu_session_info_convertToJSON() failed [snssai]");
		return NULL;
	}
	cJSON *snssai_local_JSON =
		OpenAPI_snssai_convertToJSON(pcf_for_pdu_session_info->snssai);
	...
	if(pcf_for_pdu_session_info->pcf_fqdn) {
		if(cJSON_AddStringToObject(item, "pcfFqdn",
		                           pcf_for_pdu_session_info->pcf_fqdn)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_pcf_for_pdu_session_info_convertToJSON() failed [pcf_fqdn]");
			goto end;
		}
	}
	...
	if(pcf_for_pdu_session_info->pcf_ip_end_points) {
		...
		if(pcf_ip_end_pointsList == NULL) {
			ogs_error(
				"OpenAPI_pcf_for_pdu_session_info_convertToJSON() failed [pcf_ip_end_points]");
			goto end;
		}
		OpenAPI_list_for_each(
			pcf_for_pdu_session_info->pcf_ip_end_points, node) {
	...
	if(pcf_for_pdu_session_info->ipv4_addr) {
		if(cJSON_AddStringToObject(item, "ipv4Addr",
		                           pcf_for_pdu_session_info->ipv4_addr)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_pcf_for_pdu_session_info_convertToJSON() failed [ipv4_addr]");
			goto end;
		}
	...
	if(pcf_for_pdu_session_info->ip_domain) {
		if(cJSON_AddStringToObject(item, "ipDomain",
		                           pcf_for_pdu_session_info->ip_domain)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_pcf_for_pdu_session_info_convertToJSON() failed [ip_domain]");
			goto end;
		}
	}
	...
	if(pcf_for_pdu_session_info->ipv6_prefixes) {
		...
		if(ipv6_prefixesList == NULL) {
			ogs_error(
				"OpenAPI_pcf_for_pdu_session_info_convertToJSON() failed [ipv6_prefixes]");
			goto end;
		}
		OpenAPI_list_for_each(pcf_for_pdu_session_info->ipv6_prefixes,
		                      node) {
	...
	if(pcf_for_pdu_session_info->mac_addrs) {
		...
		if(mac_addrsList == NULL) {
			ogs_error(
				"OpenAPI_pcf_for_pdu_session_info_convertToJSON() failed [mac_addrs]");
			goto end;
		}
		OpenAPI_list_for_each(pcf_for_pdu_session_info->mac_addrs,
		                      node) {
			if(cJSON_AddStringToObject(mac_addrsList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_pcf_for_pdu_session_info_convertToJSON() failed [mac_addrs]");
	...
	if(pcf_for_pdu_session_info->pcf_id) {
		if(cJSON_AddStringToObject(item, "pcfId",
		                           pcf_for_pdu_session_info->pcf_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pcf_for_pdu_session_info_convertToJSON() failed [pcf_id]");
			goto end;
		}
	}
	...
	if(pcf_for_pdu_session_info->pcf_set_id) {
		if(cJSON_AddStringToObject(item, "pcfSetId",
		                           pcf_for_pdu_session_info->pcf_set_id)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_pcf_for_pdu_session_info_convertToJSON() failed [pcf_set_id]");
			goto end;
		}
	}
	...
	if(pcf_for_pdu_session_info->bind_level != OpenAPI_binding_level_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_for_pdu_session_info_convertToJSON() failed [bind_level]");
			goto end;
		}
	}
...
OpenAPI_pcf_for_pdu_session_info_parseFromJSON(
	cJSON *pcf_for_pdu_session_infoJSON) {
	OpenAPI_pcf_for_pdu_session_info_t *pcf_for_pdu_session_info_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *dnn = NULL;
	...
	dnn = cJSON_GetObjectItemCaseSensitive(pcf_for_pdu_session_infoJSON,
	                                       "dnn");
	if(!dnn) {
		ogs_error(
			"OpenAPI_pcf_for_pdu_session_info_parseFromJSON() failed [dnn]");
		goto end;
	}
	...
	snssai = cJSON_GetObjectItemCaseSensitive(pcf_for_pdu_session_infoJSON,
	                                          "snssai");
	if(!snssai) {
		ogs_error(
			"OpenAPI_pcf_for_pdu_session_info_parseFromJSON() failed [snssai]");
		goto end;
	}
	...
	pcf_fqdn =
		cJSON_GetObjectItemCaseSensitive(pcf_for_pdu_session_infoJSON,
		                                 "pcfFqdn");
	if(pcf_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_for_pdu_session_info_parseFromJSON() failed [pcf_fqdn]");
			goto end;
		}
	}
	...
	if(pcf_ip_end_points) {
		...
		if(!cJSON_IsArray(pcf_ip_end_points)) {
			ogs_error(
				"OpenAPI_pcf_for_pdu_session_info_parseFromJSON() failed [pcf_ip_end_points]");
			goto end;
		}
	...
	if(ipv4_addr) {
		if(!cJSON_IsNull(ipv4_addr)) {
			...
			{
				ogs_error(
					"OpenAPI_pcf_for_pdu_session_info_parseFromJSON() failed [ipv4_addr]");
				goto end;
			}
		}
	}
	...
	if(ip_domain) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_for_pdu_session_info_parseFromJSON() failed [ip_domain]");
			goto end;
		}
	}
	...
	if(ipv6_prefixes) {
		...
		if(!cJSON_IsArray(ipv6_prefixes)) {
			ogs_error(
				"OpenAPI_pcf_for_pdu_session_info_parseFromJSON() failed [ipv6_prefixes]");
			goto end;
		}
	...
	if(mac_addrs) {
		...
		if(!cJSON_IsArray(mac_addrs)) {
			ogs_error(
				"OpenAPI_pcf_for_pdu_session_info_parseFromJSON() failed [mac_addrs]");
			goto end;
		}
		...
		cJSON_ArrayForEach(mac_addrs_local, mac_addrs) {
			...
			OpenAPI_list_add(mac_addrsList,
			                 ogs_strdup(
						 mac_addrs_local->valuestring));
		}
	}

	pcf_id = cJSON_GetObjectItemCaseSensitive(pcf_for_pdu_session_infoJSON,
	                                          "pcfId");
	if(pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_for_pdu_session_info_parseFromJSON() failed [pcf_id]");
			goto end;
		}
	}
	...
	if(pcf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_for_pdu_session_info_parseFromJSON() failed [pcf_set_id]");
			goto end;
		}
	}
	...
	if(bind_level) {
		if(!cJSON_IsString(bind_level)) {
			ogs_error(
				"OpenAPI_pcf_for_pdu_session_info_parseFromJSON() failed [bind_level]");
			goto end;
		}
		bind_levelVariable =
...
OpenAPI_pcf_for_pdu_session_info_t *OpenAPI_pcf_for_pdu_session_info_copy(
	OpenAPI_pcf_for_pdu_session_info_t	*dst,
	OpenAPI_pcf_for_pdu_session_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_pcf_for_pdu_session_info_convertToJSON() failed");
		return NULL;
	}
