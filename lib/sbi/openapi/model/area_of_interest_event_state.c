...
OpenAPI_area_of_interest_event_state_create(
	...
	OpenAPI_area_of_interest_event_state_t *
		...
		ogs_malloc(sizeof(OpenAPI_area_of_interest_event_state_t));
...
cJSON *OpenAPI_area_of_interest_event_state_convertToJSON(
	...
	if(area_of_interest_event_state == NULL) {
		ogs_error(
			"OpenAPI_area_of_interest_event_state_convertToJSON() failed [AreaOfInterestEventState]");
		return NULL;
	}
	...
	{
		ogs_error(
			"OpenAPI_area_of_interest_event_state_convertToJSON() failed [presence]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "presence",
	...
	if(area_of_interest_event_state->individual_pra_id_list) {
		...
		if(individual_pra_id_listList == NULL) {
			ogs_error(
				"OpenAPI_area_of_interest_event_state_convertToJSON() failed [individual_pra_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			area_of_interest_event_state->individual_pra_id_list,
...
OpenAPI_area_of_interest_event_state_parseFromJSON(
	...
	OpenAPI_area_of_interest_event_state_t *
	        area_of_interest_event_state_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *presence = NULL;
	...
	if(!presence) {
		ogs_error(
			"OpenAPI_area_of_interest_event_state_parseFromJSON() failed [presence]");
		goto end;
	}
	...
	presenceVariable =
		OpenAPI_presence_state_FromString(presence->valuestring);

	individual_pra_id_list =
		cJSON_GetObjectItemCaseSensitive(
			area_of_interest_event_stateJSON,
			"individualPraIdList");
	if(individual_pra_id_list) {
		cJSON *individual_pra_id_list_local = NULL;
		if(!cJSON_IsArray(individual_pra_id_list)) {
			ogs_error(
				"OpenAPI_area_of_interest_event_state_parseFromJSON() failed [individual_pra_id_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(individual_pra_id_list_local,
			...
			OpenAPI_list_add(individual_pra_id_listList,
			                 ogs_strdup(
						 individual_pra_id_list_local->
						 valuestring));
		}
	}
	...
	area_of_interest_event_state_local_var =
		OpenAPI_area_of_interest_event_state_create(
			presenceVariable,
			individual_pra_id_list ? individual_pra_id_listList :
			NULL
			);
...
OpenAPI_area_of_interest_event_state_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_area_of_interest_event_state_convertToJSON() failed");
		return NULL;
	}
