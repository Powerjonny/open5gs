...
OpenAPI_time_period_t *OpenAPI_time_period_create(OpenAPI_periodicity_t *period,
                                                  ...
                                                  int
                                                  max_num_period) {
	OpenAPI_time_period_t *time_period_local_var =
		ogs_malloc(sizeof(OpenAPI_time_period_t));
...
cJSON *OpenAPI_time_period_convertToJSON(OpenAPI_time_period_t *time_period) {
	...
	if(time_period == NULL) {
		ogs_error(
			"OpenAPI_time_period_convertToJSON() failed [TimePeriod]");
		return NULL;
	}
	...
	if(!time_period->period) {
		ogs_error(
			"OpenAPI_time_period_convertToJSON() failed [period]");
		return NULL;
	}
	cJSON *period_local_JSON =
		OpenAPI_periodicity_convertToJSON(time_period->period);
	...
	if(time_period->is_max_num_period) {
		if(cJSON_AddNumberToObject(item, "maxNumPeriod",
		                           time_period->max_num_period) == NULL)
		{
			ogs_error(
				"OpenAPI_time_period_convertToJSON() failed [max_num_period]");
			goto end;
		}
	}
...
OpenAPI_time_period_t *OpenAPI_time_period_parseFromJSON(
	...
	if(!period) {
		ogs_error(
			"OpenAPI_time_period_parseFromJSON() failed [period]");
		goto end;
	}
	...
	max_num_period = cJSON_GetObjectItemCaseSensitive(time_periodJSON,
	                                                  "maxNumPeriod");
	if(max_num_period) {
		if(!cJSON_IsNumber(max_num_period)) {
			ogs_error(
				"OpenAPI_time_period_parseFromJSON() failed [max_num_period]");
			goto end;
		}
	}
