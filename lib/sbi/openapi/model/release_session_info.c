...
OpenAPI_release_session_info_t *OpenAPI_release_session_info_create(
	...
	OpenAPI_release_session_info_t *release_session_info_local_var =
		ogs_malloc(sizeof(OpenAPI_release_session_info_t));
...
void OpenAPI_release_session_info_free(
	...
	if(release_session_info->release_session_list) {
		OpenAPI_list_for_each(
			release_session_info->release_session_list, node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_release_session_info_convertToJSON(
	...
	if(release_session_info == NULL) {
		ogs_error(
			"OpenAPI_release_session_info_convertToJSON() failed [ReleaseSessionInfo]");
		return NULL;
	}
	...
	if(!release_session_info->release_session_list) {
		ogs_error(
			"OpenAPI_release_session_info_convertToJSON() failed [release_session_list]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(release_session_info->release_session_list,
	                      node) {
		if(node->data == NULL) {
			ogs_error(
				"OpenAPI_release_session_info_convertToJSON() failed [release_session_list]");
	...
	if(release_session_info->release_cause == OpenAPI_release_cause_NULL) {
		ogs_error(
			"OpenAPI_release_session_info_convertToJSON() failed [release_cause]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "releaseCause",
	                           OpenAPI_release_cause_ToString(
					   release_session_info->release_cause))
	   == NULL)
	{
		ogs_error(
			"OpenAPI_release_session_info_convertToJSON() failed [release_cause]");
...
OpenAPI_release_session_info_t *OpenAPI_release_session_info_parseFromJSON(
	...
	release_session_list =
		cJSON_GetObjectItemCaseSensitive(release_session_infoJSON,
		                                 "releaseSessionList");
	if(!release_session_list) {
		ogs_error(
			"OpenAPI_release_session_info_parseFromJSON() failed [release_session_list]");
		goto end;
	}
	...
	if(!release_cause) {
		ogs_error(
			"OpenAPI_release_session_info_parseFromJSON() failed [release_cause]");
		goto end;
	}
	...
	release_causeVariable =
		OpenAPI_release_cause_FromString(release_cause->valuestring);
...
OpenAPI_release_session_info_t *OpenAPI_release_session_info_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_release_session_info_convertToJSON() failed");
		return NULL;
	}
