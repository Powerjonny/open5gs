...
OpenAPI_smf_registration_info_t *OpenAPI_smf_registration_info_create(
	...
	OpenAPI_smf_registration_info_t *smf_registration_info_local_var =
		ogs_malloc(sizeof(OpenAPI_smf_registration_info_t));
	ogs_assert(smf_registration_info_local_var);

	smf_registration_info_local_var->smf_registration_list =
		smf_registration_list;

	return smf_registration_info_local_var;
}

void OpenAPI_smf_registration_info_free(
	OpenAPI_smf_registration_info_t *smf_registration_info) {
	...
	if(smf_registration_info->smf_registration_list) {
		OpenAPI_list_for_each(
			smf_registration_info->smf_registration_list, node) {
...
cJSON *OpenAPI_smf_registration_info_convertToJSON(
	OpenAPI_smf_registration_info_t *smf_registration_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(smf_registration_info == NULL) {
		ogs_error(
			"OpenAPI_smf_registration_info_convertToJSON() failed [SmfRegistrationInfo]");
		return NULL;
	}
	...
	if(!smf_registration_info->smf_registration_list) {
		ogs_error(
			"OpenAPI_smf_registration_info_convertToJSON() failed [smf_registration_list]");
		return NULL;
	}
	cJSON *smf_registration_listList = cJSON_AddArrayToObject(item,
	                                                          "smfRegistrationList");
	...
	OpenAPI_list_for_each(smf_registration_info->smf_registration_list,
	                      node) {
		cJSON *itemLocal =
			OpenAPI_smf_registration_convertToJSON(node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_smf_registration_info_convertToJSON() failed [smf_registration_list]");
			goto end;
...
OpenAPI_smf_registration_info_t *OpenAPI_smf_registration_info_parseFromJSON(
	cJSON *smf_registration_infoJSON) {
	...
	smf_registration_list =
		cJSON_GetObjectItemCaseSensitive(smf_registration_infoJSON,
		                                 "smfRegistrationList");
	if(!smf_registration_list) {
		ogs_error(
			"OpenAPI_smf_registration_info_parseFromJSON() failed [smf_registration_list]");
		goto end;
	}
	...
	cJSON_ArrayForEach(smf_registration_list_local, smf_registration_list) {
		...
		OpenAPI_smf_registration_t *smf_registration_listItem =
			OpenAPI_smf_registration_parseFromJSON(
				smf_registration_list_local);
		if(!smf_registration_listItem) {
			ogs_error("No smf_registration_listItem");
			goto end;
		}
		OpenAPI_list_add(smf_registration_listList,
		                 smf_registration_listItem);
	}
...
OpenAPI_smf_registration_info_t *OpenAPI_smf_registration_info_copy(
	OpenAPI_smf_registration_info_t *dst,
	OpenAPI_smf_registration_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
