...
OpenAPI_target_dnai_info_t *OpenAPI_target_dnai_info_create(
	...
	smf_selection_type) {
	OpenAPI_target_dnai_info_t *target_dnai_info_local_var =
		ogs_malloc(sizeof(OpenAPI_target_dnai_info_t));
...
cJSON *OpenAPI_target_dnai_info_convertToJSON(
	...
	if(target_dnai_info == NULL) {
		ogs_error(
			"OpenAPI_target_dnai_info_convertToJSON() failed [TargetDnaiInfo]");
		return NULL;
	}
	...
	if(target_dnai_info->target_dnai) {
		if(cJSON_AddStringToObject(item, "targetDnai",
		                           target_dnai_info->target_dnai) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_target_dnai_info_convertToJSON() failed [target_dnai]");
			goto end;
		}
	}

	if(target_dnai_info->smf_selection_type ==
	   OpenAPI_smf_selection_type_NULL)
	{
		ogs_error(
			"OpenAPI_target_dnai_info_convertToJSON() failed [smf_selection_type]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "smfSelectionType",
	                           OpenAPI_smf_selection_type_ToString(
					   target_dnai_info->smf_selection_type))
	   == NULL)
	{
		ogs_error(
			"OpenAPI_target_dnai_info_convertToJSON() failed [smf_selection_type]");
...
OpenAPI_target_dnai_info_t *OpenAPI_target_dnai_info_parseFromJSON(
	...
	target_dnai = cJSON_GetObjectItemCaseSensitive(target_dnai_infoJSON,
	                                               "targetDnai");
	if(target_dnai) {
		...
		{
			ogs_error(
				"OpenAPI_target_dnai_info_parseFromJSON() failed [target_dnai]");
			goto end;
		}
	}

	smf_selection_type =
		cJSON_GetObjectItemCaseSensitive(target_dnai_infoJSON,
		                                 "smfSelectionType");
	if(!smf_selection_type) {
		ogs_error(
			"OpenAPI_target_dnai_info_parseFromJSON() failed [smf_selection_type]");
		goto end;
	}
	...
	smf_selection_typeVariable =
		OpenAPI_smf_selection_type_FromString(
			smf_selection_type->valuestring);
