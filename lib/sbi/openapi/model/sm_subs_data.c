...
OpenAPI_sm_subs_data_t *OpenAPI_sm_subs_data_create(
	...
	OpenAPI_sm_subs_data_t *sm_subs_data_local_var =
		ogs_malloc(sizeof(OpenAPI_sm_subs_data_t));
	ogs_assert(sm_subs_data_local_var);

	sm_subs_data_local_var->session_management_subscription_data_list =
		session_management_subscription_data_list;
...
void OpenAPI_sm_subs_data_free(OpenAPI_sm_subs_data_t *sm_subs_data) {
	...
	if(sm_subs_data->session_management_subscription_data_list) {
		OpenAPI_list_for_each(
			sm_subs_data->session_management_subscription_data_list,
			node) {
	...
	if(sm_subs_data->extended_sm_subs_data) {
		OpenAPI_extended_sm_subs_data_free(
			sm_subs_data->extended_sm_subs_data);
		sm_subs_data->extended_sm_subs_data = NULL;
	}
	ogs_free(sm_subs_data);
}

cJSON *OpenAPI_sm_subs_data_convertToJSON(
	OpenAPI_sm_subs_data_t *sm_subs_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(sm_subs_data == NULL) {
		ogs_error(
			"OpenAPI_sm_subs_data_convertToJSON() failed [SmSubsData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(sm_subs_data->session_management_subscription_data_list) {
		cJSON *session_management_subscription_data_listList =
			cJSON_AddArrayToObject(item,
			                       "SessionManagementSubscriptionDataList");
		if(session_management_subscription_data_listList == NULL) {
			ogs_error(
				"OpenAPI_sm_subs_data_convertToJSON() failed [session_management_subscription_data_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			sm_subs_data->session_management_subscription_data_list,
	...
	if(sm_subs_data->extended_sm_subs_data) {
		cJSON *extended_sm_subs_data_local_JSON =
			OpenAPI_extended_sm_subs_data_convertToJSON(
				sm_subs_data->extended_sm_subs_data);
		if(extended_sm_subs_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_subs_data_convertToJSON() failed [extended_sm_subs_data]");
			goto end;
		}
		cJSON_AddItemToObject(item, "ExtendedSmSubsData",
		                      extended_sm_subs_data_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_sm_subs_data_convertToJSON() failed [extended_sm_subs_data]");
			goto end;
...
OpenAPI_sm_subs_data_t *OpenAPI_sm_subs_data_parseFromJSON(
	...
	OpenAPI_extended_sm_subs_data_t *extended_sm_subs_data_local_nonprim =
		NULL;
	session_management_subscription_data_list =
		cJSON_GetObjectItemCaseSensitive(sm_subs_dataJSON,
		                                 "SessionManagementSubscriptionDataList");
	if(session_management_subscription_data_list) {
		...
		if(!cJSON_IsArray(session_management_subscription_data_list)) {
			ogs_error(
				"OpenAPI_sm_subs_data_parseFromJSON() failed [session_management_subscription_data_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(
			session_management_subscription_data_list_local,
			session_management_subscription_data_list) {
			if(!cJSON_IsObject(
				   session_management_subscription_data_list_local))
			{
	...
	if(extended_sm_subs_data) {
		extended_sm_subs_data_local_nonprim =
			OpenAPI_extended_sm_subs_data_parseFromJSON(
				extended_sm_subs_data);
		if(!extended_sm_subs_data_local_nonprim) {
			ogs_error(
				"OpenAPI_extended_sm_subs_data_parseFromJSON failed [extended_sm_subs_data]");
			goto end;
		}
	}
	...
	sm_subs_data_local_var = OpenAPI_sm_subs_data_create(
		...
		extended_sm_subs_data ? extended_sm_subs_data_local_nonprim :
		NULL
		);
	...
	if(session_management_subscription_data_listList) {
		OpenAPI_list_for_each(
			session_management_subscription_data_listList, node) {
			OpenAPI_session_management_subscription_data_free(
				node->data);
		}
		OpenAPI_list_free(
