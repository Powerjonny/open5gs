...
OpenAPI_vsmf_updated_data_t *OpenAPI_vsmf_updated_data_create(
	...
	OpenAPI_vsmf_updated_data_t *vsmf_updated_data_local_var =
		ogs_malloc(sizeof(OpenAPI_vsmf_updated_data_t));
	ogs_assert(vsmf_updated_data_local_var);

	vsmf_updated_data_local_var->qos_flows_add_mod_list =
		qos_flows_add_mod_list;
	vsmf_updated_data_local_var->qos_flows_rel_list = qos_flows_rel_list;
	vsmf_updated_data_local_var->qos_flows_failedto_add_mod_list =
		qos_flows_failedto_add_mod_list;
	vsmf_updated_data_local_var->qos_flows_failedto_rel_list =
	...
	vsmf_updated_data_local_var->failed_to_assign_ebi_list =
		failed_to_assign_ebi_list;
	vsmf_updated_data_local_var->released_ebi_list = released_ebi_list;
	vsmf_updated_data_local_var->secondary_rat_usage_report =
		secondary_rat_usage_report;
	vsmf_updated_data_local_var->secondary_rat_usage_info =
...
void OpenAPI_vsmf_updated_data_free(
	OpenAPI_vsmf_updated_data_t *vsmf_updated_data) {
	...
	if(vsmf_updated_data->qos_flows_add_mod_list) {
		OpenAPI_list_for_each(vsmf_updated_data->qos_flows_add_mod_list,
		                      node) {
	...
	if(vsmf_updated_data->qos_flows_failedto_add_mod_list) {
		OpenAPI_list_for_each(
			vsmf_updated_data->qos_flows_failedto_add_mod_list,
			node) {
			OpenAPI_qos_flow_item_free(node->data);
		}
		OpenAPI_list_free(
	...
	if(vsmf_updated_data->assigned_ebi_list) {
		OpenAPI_list_for_each(vsmf_updated_data->assigned_ebi_list,
		                      node) {
	...
	if(vsmf_updated_data->failed_to_assign_ebi_list) {
		OpenAPI_list_for_each(
			vsmf_updated_data->failed_to_assign_ebi_list, node) {
	...
	if(vsmf_updated_data->released_ebi_list) {
		OpenAPI_list_for_each(vsmf_updated_data->released_ebi_list,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_vsmf_updated_data_convertToJSON(
	OpenAPI_vsmf_updated_data_t *vsmf_updated_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(vsmf_updated_data == NULL) {
		ogs_error(
			"OpenAPI_vsmf_updated_data_convertToJSON() failed [VsmfUpdatedData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(vsmf_updated_data->qos_flows_add_mod_list) {
		cJSON *qos_flows_add_mod_listList = cJSON_AddArrayToObject(item,
		                                                           "qosFlowsAddModList");
		if(qos_flows_add_mod_listList == NULL) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_convertToJSON() failed [qos_flows_add_mod_list]");
			goto end;
		}
		OpenAPI_list_for_each(vsmf_updated_data->qos_flows_add_mod_list,
		                      node) {
			...
			cJSON_AddItemToArray(qos_flows_add_mod_listList,
			                     itemLocal);
		}
	}

	if(vsmf_updated_data->qos_flows_rel_list) {
		cJSON *qos_flows_rel_listList = cJSON_AddArrayToObject(item,
		                                                       "qosFlowsRelList");
		if(qos_flows_rel_listList == NULL) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_convertToJSON() failed [qos_flows_rel_list]");
			goto end;
		}
		OpenAPI_list_for_each(vsmf_updated_data->qos_flows_rel_list,
		                      node) {
	...
	if(vsmf_updated_data->qos_flows_failedto_add_mod_list) {
		cJSON *qos_flows_failedto_add_mod_listList =
			cJSON_AddArrayToObject(item,
			                       "qosFlowsFailedtoAddModList");
		if(qos_flows_failedto_add_mod_listList == NULL) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_convertToJSON() failed [qos_flows_failedto_add_mod_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			vsmf_updated_data->qos_flows_failedto_add_mod_list,
	...
	if(vsmf_updated_data->qos_flows_failedto_rel_list) {
		cJSON *qos_flows_failedto_rel_listList =
			cJSON_AddArrayToObject(item, "qosFlowsFailedtoRelList");
		if(qos_flows_failedto_rel_listList == NULL) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_convertToJSON() failed [qos_flows_failedto_rel_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			vsmf_updated_data->qos_flows_failedto_rel_list, node) {
			...
			cJSON_AddItemToArray(qos_flows_failedto_rel_listList,
			                     itemLocal);
		}
	}
	...
	if(vsmf_updated_data->n1_sm_info_from_ue) {
		...
		if(n1_sm_info_from_ue_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_convertToJSON() failed [n1_sm_info_from_ue]");
			goto end;
		}
	...
	if(vsmf_updated_data->unknown_n1_sm_info) {
		...
		if(unknown_n1_sm_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_convertToJSON() failed [unknown_n1_sm_info]");
			goto end;
		}
	...
	if(vsmf_updated_data->ue_location) {
		cJSON *ue_location_local_JSON =
			OpenAPI_user_location_convertToJSON(
				vsmf_updated_data->ue_location);
		if(ue_location_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_convertToJSON() failed [ue_location]");
			goto end;
		}
	...
	if(vsmf_updated_data->ue_time_zone) {
		if(cJSON_AddStringToObject(item, "ueTimeZone",
		                           vsmf_updated_data->ue_time_zone) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_vsmf_updated_data_convertToJSON() failed [ue_time_zone]");
			goto end;
		}
	}
	...
	if(vsmf_updated_data->add_ue_location) {
		cJSON *add_ue_location_local_JSON =
			OpenAPI_user_location_convertToJSON(
				vsmf_updated_data->add_ue_location);
		if(add_ue_location_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_convertToJSON() failed [add_ue_location]");
			goto end;
		}
	...
	if(vsmf_updated_data->assigned_ebi_list) {
		...
		if(assigned_ebi_listList == NULL) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_convertToJSON() failed [assigned_ebi_list]");
			goto end;
		}
		OpenAPI_list_for_each(vsmf_updated_data->assigned_ebi_list,
		                      node) {
	...
	if(vsmf_updated_data->failed_to_assign_ebi_list) {
		cJSON *failed_to_assign_ebi_listList =
			cJSON_AddArrayToObject(item, "failedToAssignEbiList");
		if(failed_to_assign_ebi_listList == NULL) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_convertToJSON() failed [failed_to_assign_ebi_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			vsmf_updated_data->failed_to_assign_ebi_list, node) {
			...
			cJSON_AddItemToArray(failed_to_assign_ebi_listList,
			                     itemLocal);
		}
	}
	...
	if(vsmf_updated_data->released_ebi_list) {
		...
		if(released_ebi_listList == NULL) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_convertToJSON() failed [released_ebi_list]");
			goto end;
		}
		OpenAPI_list_for_each(vsmf_updated_data->released_ebi_list,
		                      node) {
	...
	if(vsmf_updated_data->secondary_rat_usage_report) {
		...
		if(secondary_rat_usage_reportList == NULL) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_convertToJSON() failed [secondary_rat_usage_report]");
			goto end;
		}
		OpenAPI_list_for_each(
			vsmf_updated_data->secondary_rat_usage_report, node) {
	...
	if(vsmf_updated_data->secondary_rat_usage_info) {
		...
		if(secondary_rat_usage_infoList == NULL) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_convertToJSON() failed [secondary_rat_usage_info]");
			goto end;
		}
		OpenAPI_list_for_each(
			vsmf_updated_data->secondary_rat_usage_info, node) {
	...
	if(vsmf_updated_data->n4_info) {
		cJSON *n4_info_local_JSON =
			OpenAPI_n4_information_convertToJSON(
				vsmf_updated_data->n4_info);
		if(n4_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_convertToJSON() failed [n4_info]");
			goto end;
		}
	...
	if(vsmf_updated_data->n4_info_ext1) {
		...
		if(n4_info_ext1_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_convertToJSON() failed [n4_info_ext1]");
			goto end;
		}
	...
	if(vsmf_updated_data->n4_info_ext2) {
		cJSON *n4_info_ext2_local_JSON =
			OpenAPI_n4_information_convertToJSON(
				vsmf_updated_data->n4_info_ext2);
		if(n4_info_ext2_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_convertToJSON() failed [n4_info_ext2]");
			goto end;
		}
	...
	if(vsmf_updated_data->n4_info_ext3) {
		cJSON *n4_info_ext3_local_JSON =
			OpenAPI_n4_information_convertToJSON(
				vsmf_updated_data->n4_info_ext3);
		if(n4_info_ext3_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_convertToJSON() failed [n4_info_ext3]");
			goto end;
		}
...
OpenAPI_vsmf_updated_data_t *OpenAPI_vsmf_updated_data_parseFromJSON(
	cJSON *vsmf_updated_dataJSON) {
	...
	qos_flows_add_mod_list =
		cJSON_GetObjectItemCaseSensitive(vsmf_updated_dataJSON,
		                                 "qosFlowsAddModList");
	if(qos_flows_add_mod_list) {
		cJSON *qos_flows_add_mod_list_local = NULL;
		if(!cJSON_IsArray(qos_flows_add_mod_list)) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_parseFromJSON() failed [qos_flows_add_mod_list]");
			goto end;
		}

		qos_flows_add_mod_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(qos_flows_add_mod_list_local,
		                   qos_flows_add_mod_list) {
			...
			OpenAPI_qos_flow_item_t *qos_flows_add_mod_listItem =
				OpenAPI_qos_flow_item_parseFromJSON(
					qos_flows_add_mod_list_local);
			if(!qos_flows_add_mod_listItem) {
				ogs_error("No qos_flows_add_mod_listItem");
				goto end;
			}
			OpenAPI_list_add(qos_flows_add_mod_listList,
			                 qos_flows_add_mod_listItem);
		}
	}
	...
	if(qos_flows_rel_list) {
		...
		if(!cJSON_IsArray(qos_flows_rel_list)) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_parseFromJSON() failed [qos_flows_rel_list]");
			goto end;
		}

		qos_flows_rel_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(qos_flows_rel_list_local,
		                   qos_flows_rel_list) {
			...
			OpenAPI_qos_flow_item_t *qos_flows_rel_listItem =
				OpenAPI_qos_flow_item_parseFromJSON(
					qos_flows_rel_list_local);
			if(!qos_flows_rel_listItem) {
				ogs_error("No qos_flows_rel_listItem");
				goto end;
			}
			OpenAPI_list_add(qos_flows_rel_listList,
			                 qos_flows_rel_listItem);
		}
	}
	...
	if(qos_flows_failedto_add_mod_list) {
		...
		if(!cJSON_IsArray(qos_flows_failedto_add_mod_list)) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_parseFromJSON() failed [qos_flows_failedto_add_mod_list]");
			goto end;
		}

		qos_flows_failedto_add_mod_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(qos_flows_failedto_add_mod_list_local,
		                   qos_flows_failedto_add_mod_list) {
			if(!cJSON_IsObject(
				   qos_flows_failedto_add_mod_list_local))
			{
				ogs_error(
					"OpenAPI_vsmf_updated_data_parseFromJSON() failed [qos_flows_failedto_add_mod_list]");
				goto end;
			...
			OpenAPI_qos_flow_item_t *
			        qos_flows_failedto_add_mod_listItem =
				OpenAPI_qos_flow_item_parseFromJSON(
					qos_flows_failedto_add_mod_list_local);
			if(!qos_flows_failedto_add_mod_listItem) {
				ogs_error(
					"No qos_flows_failedto_add_mod_listItem");
				goto end;
			}
			OpenAPI_list_add(qos_flows_failedto_add_mod_listList,
			                 qos_flows_failedto_add_mod_listItem);
		}
	}
	...
	if(qos_flows_failedto_rel_list) {
		...
		if(!cJSON_IsArray(qos_flows_failedto_rel_list)) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_parseFromJSON() failed [qos_flows_failedto_rel_list]");
			goto end;
		}

		qos_flows_failedto_rel_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(qos_flows_failedto_rel_list_local,
		                   qos_flows_failedto_rel_list) {
			...
			OpenAPI_qos_flow_item_t *qos_flows_failedto_rel_listItem
			        = OpenAPI_qos_flow_item_parseFromJSON(
					  qos_flows_failedto_rel_list_local);
	...
	ue_location = cJSON_GetObjectItemCaseSensitive(vsmf_updated_dataJSON,
	                                               "ueLocation");
	if(ue_location) {
	...
	ue_time_zone = cJSON_GetObjectItemCaseSensitive(vsmf_updated_dataJSON,
	                                                "ueTimeZone");
	if(ue_time_zone) {
		...
		{
			ogs_error(
				"OpenAPI_vsmf_updated_data_parseFromJSON() failed [ue_time_zone]");
			goto end;
		}
	}
	...
	if(assigned_ebi_list) {
		...
		if(!cJSON_IsArray(assigned_ebi_list)) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_parseFromJSON() failed [assigned_ebi_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(assigned_ebi_list_local, assigned_ebi_list) {
			...
			OpenAPI_ebi_arp_mapping_t *assigned_ebi_listItem =
				OpenAPI_ebi_arp_mapping_parseFromJSON(
					assigned_ebi_list_local);
			if(!assigned_ebi_listItem) {
				ogs_error("No assigned_ebi_listItem");
				goto end;
			}
			OpenAPI_list_add(assigned_ebi_listList,
			                 assigned_ebi_listItem);
		}
	}
	...
	if(failed_to_assign_ebi_list) {
		...
		if(!cJSON_IsArray(failed_to_assign_ebi_list)) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_parseFromJSON() failed [failed_to_assign_ebi_list]");
			goto end;
		}

		failed_to_assign_ebi_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(failed_to_assign_ebi_list_local,
		                   failed_to_assign_ebi_list) {
			...
			OpenAPI_arp_t *failed_to_assign_ebi_listItem =
				OpenAPI_arp_parseFromJSON(
					failed_to_assign_ebi_list_local);
			if(!failed_to_assign_ebi_listItem) {
				ogs_error("No failed_to_assign_ebi_listItem");
				goto end;
			}
			OpenAPI_list_add(failed_to_assign_ebi_listList,
			                 failed_to_assign_ebi_listItem);
		}
	}
	...
	if(released_ebi_list) {
		...
		if(!cJSON_IsArray(released_ebi_list)) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_parseFromJSON() failed [released_ebi_list]");
			goto end;
		}
	...
	if(secondary_rat_usage_report) {
		...
		if(!cJSON_IsArray(secondary_rat_usage_report)) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_parseFromJSON() failed [secondary_rat_usage_report]");
			goto end;
		}
	...
	if(secondary_rat_usage_info) {
		...
		if(!cJSON_IsArray(secondary_rat_usage_info)) {
			ogs_error(
				"OpenAPI_vsmf_updated_data_parseFromJSON() failed [secondary_rat_usage_info]");
			goto end;
		}
	...
	n4_info = cJSON_GetObjectItemCaseSensitive(vsmf_updated_dataJSON,
	                                           "n4Info");
	if(n4_info) {
	...
	n4_info_ext1 = cJSON_GetObjectItemCaseSensitive(vsmf_updated_dataJSON,
	                                                "n4InfoExt1");
	if(n4_info_ext1) {
	...
	n4_info_ext2 = cJSON_GetObjectItemCaseSensitive(vsmf_updated_dataJSON,
	                                                "n4InfoExt2");
	if(n4_info_ext2) {
	...
	n4_info_ext3 = cJSON_GetObjectItemCaseSensitive(vsmf_updated_dataJSON,
	                                                "n4InfoExt3");
	if(n4_info_ext3) {
	...
	vsmf_updated_data_local_var = OpenAPI_vsmf_updated_data_create(
		...
		qos_flows_failedto_rel_list ? qos_flows_failedto_rel_listList :
		NULL,
		...
		failed_to_assign_ebi_list ? failed_to_assign_ebi_listList :
		NULL,
	...
	if(qos_flows_failedto_add_mod_listList) {
		OpenAPI_list_for_each(qos_flows_failedto_add_mod_listList,
		                      node) {
			OpenAPI_qos_flow_item_free(node->data);
		}
...
OpenAPI_vsmf_updated_data_t *OpenAPI_vsmf_updated_data_copy(
	OpenAPI_vsmf_updated_data_t *dst, OpenAPI_vsmf_updated_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
