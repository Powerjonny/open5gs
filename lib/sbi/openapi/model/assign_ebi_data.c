...
OpenAPI_assign_ebi_data_t *OpenAPI_assign_ebi_data_create(int pdu_session_id,
                                                          OpenAPI_list_t *
                                                          arp_list,
                                                          OpenAPI_list_t *
                                                          released_ebi_list,
                                                          ...
                                                          OpenAPI_list_t *
                                                          modified_ebi_list) {
	OpenAPI_assign_ebi_data_t *assign_ebi_data_local_var =
		ogs_malloc(sizeof(OpenAPI_assign_ebi_data_t));
...
void OpenAPI_assign_ebi_data_free(OpenAPI_assign_ebi_data_t *assign_ebi_data) {
	...
	if(assign_ebi_data->released_ebi_list) {
		OpenAPI_list_for_each(assign_ebi_data->released_ebi_list,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(assign_ebi_data->modified_ebi_list) {
		OpenAPI_list_for_each(assign_ebi_data->modified_ebi_list,
		                      node) {
...
cJSON *OpenAPI_assign_ebi_data_convertToJSON(
	OpenAPI_assign_ebi_data_t *assign_ebi_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(assign_ebi_data == NULL) {
		ogs_error(
			"OpenAPI_assign_ebi_data_convertToJSON() failed [AssignEbiData]");
		return NULL;
	}
	...
	{
		ogs_error(
			"OpenAPI_assign_ebi_data_convertToJSON() failed [pdu_session_id]");
		goto end;
	}
	...
	if(assign_ebi_data->arp_list) {
		...
		if(arp_listList == NULL) {
			ogs_error(
				"OpenAPI_assign_ebi_data_convertToJSON() failed [arp_list]");
			goto end;
		}
		OpenAPI_list_for_each(assign_ebi_data->arp_list, node) {
			cJSON *itemLocal =
	...
	if(assign_ebi_data->released_ebi_list) {
		...
		if(released_ebi_listList == NULL) {
			ogs_error(
				"OpenAPI_assign_ebi_data_convertToJSON() failed [released_ebi_list]");
			goto end;
		}
		OpenAPI_list_for_each(assign_ebi_data->released_ebi_list,
		                      node) {
	...
	if(assign_ebi_data->old_guami) {
		cJSON *old_guami_local_JSON =
			OpenAPI_guami_convertToJSON(assign_ebi_data->old_guami);
		if(old_guami_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_assign_ebi_data_convertToJSON() failed [old_guami]");
			goto end;
		}
	...
	if(assign_ebi_data->modified_ebi_list) {
		...
		if(modified_ebi_listList == NULL) {
			ogs_error(
				"OpenAPI_assign_ebi_data_convertToJSON() failed [modified_ebi_list]");
			goto end;
		}
		OpenAPI_list_for_each(assign_ebi_data->modified_ebi_list,
		                      node) {
...
OpenAPI_assign_ebi_data_t *OpenAPI_assign_ebi_data_parseFromJSON(
	cJSON *assign_ebi_dataJSON) {
	...
	if(!pdu_session_id) {
		ogs_error(
			"OpenAPI_assign_ebi_data_parseFromJSON() failed [pdu_session_id]");
		goto end;
	}
	...
	if(arp_list) {
		...
		if(!cJSON_IsArray(arp_list)) {
			ogs_error(
				"OpenAPI_assign_ebi_data_parseFromJSON() failed [arp_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(arp_list_local, arp_list) {
			...
			OpenAPI_arp_t *arp_listItem =
				OpenAPI_arp_parseFromJSON(arp_list_local);
			if(!arp_listItem) {
	...
	released_ebi_list =
		cJSON_GetObjectItemCaseSensitive(assign_ebi_dataJSON,
		                                 "releasedEbiList");
	if(released_ebi_list) {
		cJSON *released_ebi_list_local = NULL;
		if(!cJSON_IsArray(released_ebi_list)) {
			ogs_error(
				"OpenAPI_assign_ebi_data_parseFromJSON() failed [released_ebi_list]");
			goto end;
		}
	...
	if(modified_ebi_list) {
		...
		if(!cJSON_IsArray(modified_ebi_list)) {
			ogs_error(
				"OpenAPI_assign_ebi_data_parseFromJSON() failed [modified_ebi_list]");
			goto end;
		}
...
OpenAPI_assign_ebi_data_t *OpenAPI_assign_ebi_data_copy(
	OpenAPI_assign_ebi_data_t *dst, OpenAPI_assign_ebi_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
