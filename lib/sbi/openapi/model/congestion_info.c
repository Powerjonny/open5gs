...
OpenAPI_congestion_info_t *OpenAPI_congestion_info_create(
	OpenAPI_congestion_type_t *cong_type, OpenAPI_time_window_t *time_intev,
	OpenAPI_threshold_level_t *nsi, bool is_confidence, int confidence,
	OpenAPI_list_t *top_app_list_ul, OpenAPI_list_t *top_app_list_dl) {
	OpenAPI_congestion_info_t *congestion_info_local_var =
		ogs_malloc(sizeof(OpenAPI_congestion_info_t));
...
cJSON *OpenAPI_congestion_info_convertToJSON(
	...
	if(congestion_info == NULL) {
		ogs_error(
			"OpenAPI_congestion_info_convertToJSON() failed [CongestionInfo]");
		return NULL;
	}
	...
	cJSON *cong_type_local_JSON =
		OpenAPI_congestion_type_convertToJSON(
	...
	if(!congestion_info->time_intev) {
		ogs_error(
			"OpenAPI_congestion_info_convertToJSON() failed [time_intev]");
		return NULL;
	}
	cJSON *time_intev_local_JSON =
		OpenAPI_time_window_convertToJSON(congestion_info->time_intev);
	...
	if(!congestion_info->nsi) {
		ogs_error(
			"OpenAPI_congestion_info_convertToJSON() failed [nsi]");
		return NULL;
	}
	cJSON *nsi_local_JSON =
		OpenAPI_threshold_level_convertToJSON(congestion_info->nsi);
	...
	if(congestion_info->is_confidence) {
		...
		{
			ogs_error(
				"OpenAPI_congestion_info_convertToJSON() failed [confidence]");
			goto end;
		}
	}
	...
	if(congestion_info->top_app_list_ul) {
		...
		if(top_app_list_ulList == NULL) {
			ogs_error(
				"OpenAPI_congestion_info_convertToJSON() failed [top_app_list_ul]");
			goto end;
		}
		OpenAPI_list_for_each(congestion_info->top_app_list_ul, node) {
			cJSON *itemLocal =
				OpenAPI_top_application_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_congestion_info_convertToJSON() failed [top_app_list_ul]");
	...
	if(congestion_info->top_app_list_dl) {
		...
		if(top_app_list_dlList == NULL) {
			ogs_error(
				"OpenAPI_congestion_info_convertToJSON() failed [top_app_list_dl]");
			goto end;
		}
		OpenAPI_list_for_each(congestion_info->top_app_list_dl, node) {
			cJSON *itemLocal =
...
OpenAPI_congestion_info_t *OpenAPI_congestion_info_parseFromJSON(
	cJSON *congestion_infoJSON) {
	...
	cong_type_local_nonprim =
		OpenAPI_congestion_type_parseFromJSON(cong_type);
	if(!cong_type_local_nonprim) {
		ogs_error(
			"OpenAPI_congestion_type_parseFromJSON failed [cong_type]");
		goto end;
	}
	...
	if(!time_intev) {
		ogs_error(
			"OpenAPI_congestion_info_parseFromJSON() failed [time_intev]");
		goto end;
	}
	time_intev_local_nonprim =
		OpenAPI_time_window_parseFromJSON(time_intev);
	if(!time_intev_local_nonprim) {
		ogs_error(
			"OpenAPI_time_window_parseFromJSON failed [time_intev]");
		goto end;
	}
	...
	if(!nsi) {
		ogs_error(
			"OpenAPI_congestion_info_parseFromJSON() failed [nsi]");
		goto end;
	}
	...
	if(confidence) {
		if(!cJSON_IsNumber(confidence)) {
			ogs_error(
				"OpenAPI_congestion_info_parseFromJSON() failed [confidence]");
			goto end;
		}
	}
	...
	if(top_app_list_ul) {
		...
		if(!cJSON_IsArray(top_app_list_ul)) {
			ogs_error(
				"OpenAPI_congestion_info_parseFromJSON() failed [top_app_list_ul]");
			goto end;
		}
		...
		cJSON_ArrayForEach(top_app_list_ul_local, top_app_list_ul) {
			...
			OpenAPI_top_application_t *top_app_list_ulItem =
			...
			OpenAPI_list_add(top_app_list_ulList,
			                 top_app_list_ulItem);
		}
	}
	...
	if(top_app_list_dl) {
		...
		if(!cJSON_IsArray(top_app_list_dl)) {
			ogs_error(
				"OpenAPI_congestion_info_parseFromJSON() failed [top_app_list_dl]");
			goto end;
		}
		...
		cJSON_ArrayForEach(top_app_list_dl_local, top_app_list_dl) {
			...
			OpenAPI_top_application_t *top_app_list_dlItem =
				OpenAPI_top_application_parseFromJSON(
			...
			OpenAPI_list_add(top_app_list_dlList,
			                 top_app_list_dlItem);
		}
	}
...
OpenAPI_congestion_info_t *OpenAPI_congestion_info_copy(
	OpenAPI_congestion_info_t *dst, OpenAPI_congestion_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
