...
OpenAPI_amf_event_t *OpenAPI_amf_event_create(OpenAPI_amf_event_type_t *type,
                                              bool
                                              is_immediate_flag,
                                              int
                                              immediate_flag,
                                              ...
                                              dispersion_area) {
	OpenAPI_amf_event_t *amf_event_local_var =
		ogs_malloc(sizeof(OpenAPI_amf_event_t));
...
void OpenAPI_amf_event_free(OpenAPI_amf_event_t *amf_event) {
	...
	if(amf_event->traffic_descriptor_list) {
		OpenAPI_list_for_each(amf_event->traffic_descriptor_list,
		                      node) {
	...
	if(amf_event->reachability_filter) {
		OpenAPI_reachability_filter_free(
...
cJSON *OpenAPI_amf_event_convertToJSON(OpenAPI_amf_event_t *amf_event) {
	...
	if(amf_event == NULL) {
		ogs_error(
			"OpenAPI_amf_event_convertToJSON() failed [AmfEvent]");
		return NULL;
	}
	...
	cJSON *type_local_JSON =
		OpenAPI_amf_event_type_convertToJSON(amf_event->type);
	...
	if(amf_event->is_immediate_flag) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_convertToJSON() failed [immediate_flag]");
			goto end;
		}
	}
	...
	if(amf_event->area_list) {
		...
		if(area_listList == NULL) {
			ogs_error(
				"OpenAPI_amf_event_convertToJSON() failed [area_list]");
			goto end;
		}
		OpenAPI_list_for_each(amf_event->area_list, node) {
			cJSON *itemLocal =
				OpenAPI_amf_event_area_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_amf_event_convertToJSON() failed [area_list]");
	...
	if(amf_event->location_filter_list) {
		...
		if(location_filter_listList == NULL) {
			ogs_error(
				"OpenAPI_amf_event_convertToJSON() failed [location_filter_list]");
			goto end;
		}
		OpenAPI_list_for_each(amf_event->location_filter_list, node) {
			cJSON *itemLocal =
				OpenAPI_location_filter_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_amf_event_convertToJSON() failed [location_filter_list]");
	...
	if(amf_event->is_ref_id) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_convertToJSON() failed [ref_id]");
			goto end;
		}
	}

	if(amf_event->traffic_descriptor_list) {
		cJSON *traffic_descriptor_listList =
			cJSON_AddArrayToObject(item, "trafficDescriptorList");
		if(traffic_descriptor_listList == NULL) {
			ogs_error(
				"OpenAPI_amf_event_convertToJSON() failed [traffic_descriptor_list]");
			goto end;
		}
		OpenAPI_list_for_each(amf_event->traffic_descriptor_list,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_traffic_descriptor_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_amf_event_convertToJSON() failed [traffic_descriptor_list]");
	...
	if(amf_event->is_report_ue_reachable) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_convertToJSON() failed [report_ue_reachable]");
			goto end;
		}
	}
	...
	if(amf_event->reachability_filter) {
		cJSON *reachability_filter_local_JSON =
			OpenAPI_reachability_filter_convertToJSON(
				amf_event->reachability_filter);
		if(reachability_filter_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_amf_event_convertToJSON() failed [reachability_filter]");
			goto end;
		}
	...
	if(amf_event->is_udm_detect_ind) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_convertToJSON() failed [udm_detect_ind]");
			goto end;
		}
	}
	...
	if(amf_event->is_max_reports) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_convertToJSON() failed [max_reports]");
			goto end;
		}
	}
	...
	if(amf_event->presence_info_list) {
		...
		if(presence_info_list == NULL) {
			ogs_error(
				"OpenAPI_amf_event_convertToJSON() failed [presence_info_list]");
			goto end;
		}
		cJSON *localMapObject = presence_info_list;
		if(amf_event->presence_info_list) {
			OpenAPI_list_for_each(amf_event->presence_info_list,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_amf_event_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(amf_event->is_max_response_time) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_convertToJSON() failed [max_response_time]");
			goto end;
		}
	}

	if(amf_event->target_area) {
		cJSON *target_area_local_JSON =
			OpenAPI_target_area_convertToJSON(
				amf_event->target_area);
		if(target_area_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_amf_event_convertToJSON() failed [target_area]");
			goto end;
		}
	...
	if(amf_event->snssai_filter) {
		...
		if(snssai_filterList == NULL) {
			ogs_error(
				"OpenAPI_amf_event_convertToJSON() failed [snssai_filter]");
			goto end;
		}
		OpenAPI_list_for_each(amf_event->snssai_filter, node) {
			cJSON *itemLocal =
	...
	if(amf_event->ue_in_area_filter) {
		...
		if(ue_in_area_filter_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_amf_event_convertToJSON() failed [ue_in_area_filter]");
			goto end;
		}
	...
	if(amf_event->is_min_interval) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_convertToJSON() failed [min_interval]");
			goto end;
		}
	}
	...
	if(amf_event->next_report) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_convertToJSON() failed [next_report]");
			goto end;
		}
	}
	...
	if(amf_event->is_idle_status_ind) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_convertToJSON() failed [idle_status_ind]");
			goto end;
		}
	}
	...
	if(amf_event->dispersion_area) {
		cJSON *dispersion_area_local_JSON =
			OpenAPI_dispersion_area_convertToJSON(
				amf_event->dispersion_area);
		if(dispersion_area_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_amf_event_convertToJSON() failed [dispersion_area]");
			goto end;
		}
...
OpenAPI_amf_event_t *OpenAPI_amf_event_parseFromJSON(cJSON *amf_eventJSON) {
	...
	if(immediate_flag) {
		if(!cJSON_IsBool(immediate_flag)) {
			ogs_error(
				"OpenAPI_amf_event_parseFromJSON() failed [immediate_flag]");
			goto end;
		}
	}
	...
	if(area_list) {
		...
		if(!cJSON_IsArray(area_list)) {
			ogs_error(
				"OpenAPI_amf_event_parseFromJSON() failed [area_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(area_list_local, area_list) {
			...
			OpenAPI_amf_event_area_t *area_listItem =
				OpenAPI_amf_event_area_parseFromJSON(
					area_list_local);
			if(!area_listItem) {
	...
	if(location_filter_list) {
		...
		if(!cJSON_IsArray(location_filter_list)) {
			ogs_error(
				"OpenAPI_amf_event_parseFromJSON() failed [location_filter_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(location_filter_list_local,
			...
			OpenAPI_location_filter_t *location_filter_listItem =
				OpenAPI_location_filter_parseFromJSON(
					location_filter_list_local);
			if(!location_filter_listItem) {
				ogs_error("No location_filter_listItem");
				goto end;
			}
			OpenAPI_list_add(location_filter_listList,
	...
	if(ref_id) {
		if(!cJSON_IsNumber(ref_id)) {
			ogs_error(
				"OpenAPI_amf_event_parseFromJSON() failed [ref_id]");
			goto end;
		}
	}

	traffic_descriptor_list =
		cJSON_GetObjectItemCaseSensitive(amf_eventJSON,
	...
	if(traffic_descriptor_list) {
		...
		if(!cJSON_IsArray(traffic_descriptor_list)) {
			ogs_error(
				"OpenAPI_amf_event_parseFromJSON() failed [traffic_descriptor_list]");
			goto end;
		}
	...
	if(report_ue_reachable) {
		if(!cJSON_IsBool(report_ue_reachable)) {
			ogs_error(
				"OpenAPI_amf_event_parseFromJSON() failed [report_ue_reachable]");
			goto end;
		}
	}
	...
	if(reachability_filter) {
		reachability_filter_local_nonprim =
			OpenAPI_reachability_filter_parseFromJSON(
				reachability_filter);
		if(!reachability_filter_local_nonprim) {
			ogs_error(
				"OpenAPI_reachability_filter_parseFromJSON failed [reachability_filter]");
			goto end;
		}
	}
	...
	if(udm_detect_ind) {
		if(!cJSON_IsBool(udm_detect_ind)) {
			ogs_error(
				"OpenAPI_amf_event_parseFromJSON() failed [udm_detect_ind]");
			goto end;
		}
	}
	...
	if(max_reports) {
		if(!cJSON_IsNumber(max_reports)) {
			ogs_error(
				"OpenAPI_amf_event_parseFromJSON() failed [max_reports]");
			goto end;
		}
	}
	...
	if(presence_info_list) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_parseFromJSON() failed [presence_info_list]");
			goto end;
		}
		if(cJSON_IsObject(presence_info_list)) {
			...
			cJSON_ArrayForEach(presence_info_list_local_map,
				...
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_amf_event_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	if(max_response_time) {
		if(!cJSON_IsNumber(max_response_time)) {
			ogs_error(
				"OpenAPI_amf_event_parseFromJSON() failed [max_response_time]");
			goto end;
		}
	}
	...
	target_area = cJSON_GetObjectItemCaseSensitive(amf_eventJSON,
	                                               "targetArea");
	if(target_area) {
		target_area_local_nonprim =
			OpenAPI_target_area_parseFromJSON(target_area);
		if(!target_area_local_nonprim) {
			ogs_error(
				"OpenAPI_target_area_parseFromJSON failed [target_area]");
			goto end;
		}
	}
	...
	if(snssai_filter) {
		...
		if(!cJSON_IsArray(snssai_filter)) {
			ogs_error(
				"OpenAPI_amf_event_parseFromJSON() failed [snssai_filter]");
			goto end;
		}
		...
		cJSON_ArrayForEach(snssai_filter_local, snssai_filter) {
			...
			OpenAPI_ext_snssai_t *snssai_filterItem =
	...
	if(ue_in_area_filter) {
		ue_in_area_filter_local_nonprim =
			OpenAPI_ue_in_area_filter_parseFromJSON(
				ue_in_area_filter);
		if(!ue_in_area_filter_local_nonprim) {
			ogs_error(
				"OpenAPI_ue_in_area_filter_parseFromJSON failed [ue_in_area_filter]");
			goto end;
		}
	}
	...
	if(min_interval) {
		if(!cJSON_IsNumber(min_interval)) {
			ogs_error(
				"OpenAPI_amf_event_parseFromJSON() failed [min_interval]");
			goto end;
		}
	}
	...
	if(next_report) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_parseFromJSON() failed [next_report]");
			goto end;
		}
	}
	...
	if(idle_status_ind) {
		if(!cJSON_IsBool(idle_status_ind)) {
			ogs_error(
				"OpenAPI_amf_event_parseFromJSON() failed [idle_status_ind]");
			goto end;
		}
	}
	...
	dispersion_area = cJSON_GetObjectItemCaseSensitive(amf_eventJSON,
	                                                   "dispersionArea");
	if(dispersion_area) {
		dispersion_area_local_nonprim =
			OpenAPI_dispersion_area_parseFromJSON(dispersion_area);
		if(!dispersion_area_local_nonprim) {
			ogs_error(
				"OpenAPI_dispersion_area_parseFromJSON failed [dispersion_area]");
			goto end;
		}
	}
	...
	amf_event_local_var = OpenAPI_amf_event_create(
		...
		next_report &&
		!cJSON_IsNull(next_report) ?
		ogs_strdup(next_report->valuestring) : NULL,
...
OpenAPI_amf_event_t *OpenAPI_amf_event_copy(OpenAPI_amf_event_t *dst,
                                            OpenAPI_amf_event_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
