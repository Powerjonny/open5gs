...
char *OpenAPI_condition_typeupf_cond_ToString(
...
OpenAPI_upf_cond_condition_type_e OpenAPI_condition_typeupf_cond_FromString(
	char *condition_type) {
...
OpenAPI_upf_cond_t *OpenAPI_upf_cond_create(
	OpenAPI_upf_cond_condition_type_e condition_type,
	OpenAPI_list_t *smf_serving_area, OpenAPI_list_t *tai_list) {
	OpenAPI_upf_cond_t *upf_cond_local_var =
		ogs_malloc(sizeof(OpenAPI_upf_cond_t));
...
cJSON *OpenAPI_upf_cond_convertToJSON(OpenAPI_upf_cond_t *upf_cond) {
	...
	if(upf_cond->condition_type == OpenAPI_upf_cond_CONDITIONTYPE_NULL) {
		ogs_error(
			"OpenAPI_upf_cond_convertToJSON() failed [condition_type]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "conditionType",
	                           OpenAPI_condition_typeupf_cond_ToString(
					   upf_cond->condition_type)) == NULL)
	{
		ogs_error(
			"OpenAPI_upf_cond_convertToJSON() failed [condition_type]");
		goto end;
	...
	if(upf_cond->smf_serving_area) {
		...
		if(smf_serving_areaList == NULL) {
			ogs_error(
				"OpenAPI_upf_cond_convertToJSON() failed [smf_serving_area]");
			goto end;
		}
		OpenAPI_list_for_each(upf_cond->smf_serving_area, node) {
	...
	if(upf_cond->tai_list) {
		...
		if(tai_listList == NULL) {
			ogs_error(
				"OpenAPI_upf_cond_convertToJSON() failed [tai_list]");
			goto end;
		}
		OpenAPI_list_for_each(upf_cond->tai_list, node) {
			cJSON *itemLocal =
...
OpenAPI_upf_cond_t *OpenAPI_upf_cond_parseFromJSON(cJSON *upf_condJSON) {
	...
	condition_type = cJSON_GetObjectItemCaseSensitive(upf_condJSON,
	                                                  "conditionType");
	if(!condition_type) {
		ogs_error(
			"OpenAPI_upf_cond_parseFromJSON() failed [condition_type]");
		goto end;
	}
	...
	condition_typeVariable =
		OpenAPI_condition_typeupf_cond_FromString(
			condition_type->valuestring);

	smf_serving_area = cJSON_GetObjectItemCaseSensitive(upf_condJSON,
	                                                    "smfServingArea");
	if(smf_serving_area) {
		cJSON *smf_serving_area_local = NULL;
		if(!cJSON_IsArray(smf_serving_area)) {
			ogs_error(
				"OpenAPI_upf_cond_parseFromJSON() failed [smf_serving_area]");
			goto end;
		}
	...
	if(tai_list) {
		...
		if(!cJSON_IsArray(tai_list)) {
			ogs_error(
				"OpenAPI_upf_cond_parseFromJSON() failed [tai_list]");
			goto end;
		}
...
OpenAPI_upf_cond_t *OpenAPI_upf_cond_copy(OpenAPI_upf_cond_t	*dst,
                                          OpenAPI_upf_cond_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
