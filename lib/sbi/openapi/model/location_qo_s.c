...
OpenAPI_location_qo_s_t *OpenAPI_location_qo_s_create(bool	is_h_accuracy,
                                                      float	h_accuracy,
                                                      bool	is_v_accuracy,
                                                      float	v_accuracy,
                                                      bool
                                                      is_vertical_requested,
                                                      int
                                                      vertical_requested,
                                                      OpenAPI_response_time_e
                                                      response_time,
                                                      OpenAPI_list_t *
                                                      minor_loc_qoses,
                                                      OpenAPI_lcs_qos_class_e
                                                      lcs_qos_class) {
	OpenAPI_location_qo_s_t *location_qo_s_local_var =
		ogs_malloc(sizeof(OpenAPI_location_qo_s_t));
...
cJSON *OpenAPI_location_qo_s_convertToJSON(
	...
	if(location_qo_s == NULL) {
		ogs_error(
			"OpenAPI_location_qo_s_convertToJSON() failed [LocationQoS]");
		return NULL;
	}
	...
	if(location_qo_s->is_h_accuracy) {
		...
		{
			ogs_error(
				"OpenAPI_location_qo_s_convertToJSON() failed [h_accuracy]");
			goto end;
		}
	}
	...
	if(location_qo_s->is_v_accuracy) {
		...
		{
			ogs_error(
				"OpenAPI_location_qo_s_convertToJSON() failed [v_accuracy]");
			goto end;
		}
	}
	...
	if(location_qo_s->is_vertical_requested) {
		...
		{
			ogs_error(
				"OpenAPI_location_qo_s_convertToJSON() failed [vertical_requested]");
			goto end;
		}
	}
	...
	if(location_qo_s->response_time != OpenAPI_response_time_NULL) {
		if(cJSON_AddStringToObject(item, "responseTime",
		                           OpenAPI_response_time_ToString(
						   location_qo_s->response_time))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_location_qo_s_convertToJSON() failed [response_time]");
			goto end;
		}
	}
	...
	if(location_qo_s->minor_loc_qoses) {
		...
		if(minor_loc_qosesList == NULL) {
			ogs_error(
				"OpenAPI_location_qo_s_convertToJSON() failed [minor_loc_qoses]");
			goto end;
		}
		OpenAPI_list_for_each(location_qo_s->minor_loc_qoses, node) {
			cJSON *itemLocal =
	...
	if(location_qo_s->lcs_qos_class != OpenAPI_lcs_qos_class_NULL) {
		if(cJSON_AddStringToObject(item, "lcsQosClass",
		                           OpenAPI_lcs_qos_class_ToString(
						   location_qo_s->lcs_qos_class))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_location_qo_s_convertToJSON() failed [lcs_qos_class]");
			goto end;
		}
	}
...
OpenAPI_location_qo_s_t *OpenAPI_location_qo_s_parseFromJSON(
	cJSON *location_qo_sJSON) {
	...
	if(h_accuracy) {
		if(!cJSON_IsNumber(h_accuracy)) {
			ogs_error(
				"OpenAPI_location_qo_s_parseFromJSON() failed [h_accuracy]");
			goto end;
		}
	}
	...
	if(v_accuracy) {
		if(!cJSON_IsNumber(v_accuracy)) {
			ogs_error(
				"OpenAPI_location_qo_s_parseFromJSON() failed [v_accuracy]");
			goto end;
		}
	}
	...
	if(vertical_requested) {
		if(!cJSON_IsBool(vertical_requested)) {
			ogs_error(
				"OpenAPI_location_qo_s_parseFromJSON() failed [vertical_requested]");
			goto end;
		}
	}
	...
	if(response_time) {
		if(!cJSON_IsString(response_time)) {
			ogs_error(
				"OpenAPI_location_qo_s_parseFromJSON() failed [response_time]");
			goto end;
		}
		response_timeVariable =
			OpenAPI_response_time_FromString(
	...
	if(minor_loc_qoses) {
		...
		if(!cJSON_IsArray(minor_loc_qoses)) {
			ogs_error(
				"OpenAPI_location_qo_s_parseFromJSON() failed [minor_loc_qoses]");
			goto end;
		}
		...
		cJSON_ArrayForEach(minor_loc_qoses_local, minor_loc_qoses) {
			...
			OpenAPI_minor_location_qo_s_t *minor_loc_qosesItem =
				OpenAPI_minor_location_qo_s_parseFromJSON(
					minor_loc_qoses_local);
			if(!minor_loc_qosesItem) {
				ogs_error("No minor_loc_qosesItem");
				goto end;
			}
			OpenAPI_list_add(minor_loc_qosesList,
			                 minor_loc_qosesItem);
		}
	}
	...
	if(lcs_qos_class) {
		if(!cJSON_IsString(lcs_qos_class)) {
			ogs_error(
				"OpenAPI_location_qo_s_parseFromJSON() failed [lcs_qos_class]");
			goto end;
		}
		lcs_qos_classVariable =
			OpenAPI_lcs_qos_class_FromString(
...
OpenAPI_location_qo_s_t *OpenAPI_location_qo_s_copy(
	OpenAPI_location_qo_s_t *dst, OpenAPI_location_qo_s_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
