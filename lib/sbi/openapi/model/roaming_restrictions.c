...
OpenAPI_roaming_restrictions_t *OpenAPI_roaming_restrictions_create(
	...
	OpenAPI_roaming_restrictions_t *roaming_restrictions_local_var =
		ogs_malloc(sizeof(OpenAPI_roaming_restrictions_t));
...
cJSON *OpenAPI_roaming_restrictions_convertToJSON(
	...
	if(roaming_restrictions == NULL) {
		ogs_error(
			"OpenAPI_roaming_restrictions_convertToJSON() failed [RoamingRestrictions]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(roaming_restrictions->is_access_allowed) {
		if(cJSON_AddBoolToObject(item, "accessAllowed",
		                         roaming_restrictions->access_allowed)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_roaming_restrictions_convertToJSON() failed [access_allowed]");
			goto end;
		}
	}
...
OpenAPI_roaming_restrictions_t *OpenAPI_roaming_restrictions_parseFromJSON(
	...
	access_allowed =
		cJSON_GetObjectItemCaseSensitive(roaming_restrictionsJSON,
		                                 "accessAllowed");
	if(access_allowed) {
		if(!cJSON_IsBool(access_allowed)) {
			ogs_error(
				"OpenAPI_roaming_restrictions_parseFromJSON() failed [access_allowed]");
			goto end;
		}
	}
...
OpenAPI_roaming_restrictions_t *OpenAPI_roaming_restrictions_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_roaming_restrictions_convertToJSON() failed");
		return NULL;
	}
