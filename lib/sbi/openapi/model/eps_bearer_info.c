...
OpenAPI_eps_bearer_info_t *OpenAPI_eps_bearer_info_create(int	ebi,
                                                          ...
                                                          char	*
                                                          bearer_level_qo_s) {
	OpenAPI_eps_bearer_info_t *eps_bearer_info_local_var =
		ogs_malloc(sizeof(OpenAPI_eps_bearer_info_t));
...
cJSON *OpenAPI_eps_bearer_info_convertToJSON(
	...
	if(eps_bearer_info == NULL) {
		ogs_error(
			"OpenAPI_eps_bearer_info_convertToJSON() failed [EpsBearerInfo]");
		return NULL;
	}
	...
	if(cJSON_AddNumberToObject(item, "ebi", eps_bearer_info->ebi) == NULL) {
		ogs_error(
			"OpenAPI_eps_bearer_info_convertToJSON() failed [ebi]");
		goto end;
	}

	if(!eps_bearer_info->pgw_s8u_fteid) {
		ogs_error(
			"OpenAPI_eps_bearer_info_convertToJSON() failed [pgw_s8u_fteid]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "pgwS8uFteid",
	                           eps_bearer_info->pgw_s8u_fteid) == NULL)
	{
		ogs_error(
			"OpenAPI_eps_bearer_info_convertToJSON() failed [pgw_s8u_fteid]");
	...
	if(!eps_bearer_info->bearer_level_qo_s) {
		ogs_error(
			"OpenAPI_eps_bearer_info_convertToJSON() failed [bearer_level_qo_s]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "bearerLevelQoS",
	                           eps_bearer_info->bearer_level_qo_s) == NULL)
	{
		ogs_error(
			"OpenAPI_eps_bearer_info_convertToJSON() failed [bearer_level_qo_s]");
...
OpenAPI_eps_bearer_info_t *OpenAPI_eps_bearer_info_parseFromJSON(
	...
	if(!ebi) {
		ogs_error(
			"OpenAPI_eps_bearer_info_parseFromJSON() failed [ebi]");
		goto end;
	}
	...
	pgw_s8u_fteid = cJSON_GetObjectItemCaseSensitive(eps_bearer_infoJSON,
	                                                 "pgwS8uFteid");
	if(!pgw_s8u_fteid) {
		ogs_error(
			"OpenAPI_eps_bearer_info_parseFromJSON() failed [pgw_s8u_fteid]");
		goto end;
	}
	...
	bearer_level_qo_s =
		cJSON_GetObjectItemCaseSensitive(eps_bearer_infoJSON,
		                                 "bearerLevelQoS");
	if(!bearer_level_qo_s) {
		ogs_error(
			"OpenAPI_eps_bearer_info_parseFromJSON() failed [bearer_level_qo_s]");
		goto end;
	}
