...
OpenAPI_pdu_session_types_t *OpenAPI_pdu_session_types_create(
	...
	OpenAPI_pdu_session_types_t *pdu_session_types_local_var =
		ogs_malloc(sizeof(OpenAPI_pdu_session_types_t));
	ogs_assert(pdu_session_types_local_var);

	pdu_session_types_local_var->default_session_type =
		default_session_type;
	pdu_session_types_local_var->allowed_session_types =
		allowed_session_types;

	return pdu_session_types_local_var;
}

void OpenAPI_pdu_session_types_free(
...
cJSON *OpenAPI_pdu_session_types_convertToJSON(
	OpenAPI_pdu_session_types_t *pdu_session_types) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(pdu_session_types == NULL) {
		ogs_error(
			"OpenAPI_pdu_session_types_convertToJSON() failed [PduSessionTypes]");
		return NULL;
	}
	   ...
	   OpenAPI_pdu_session_type_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_pdu_session_types_convertToJSON() failed [default_session_type]");
			goto end;
		}
	}
	   ...
	   OpenAPI_pdu_session_type_NULL)
	{
		...
		if(allowed_session_typesList == NULL) {
			ogs_error(
				"OpenAPI_pdu_session_types_convertToJSON() failed [allowed_session_types]");
			goto end;
		}
...
OpenAPI_pdu_session_types_t *OpenAPI_pdu_session_types_parseFromJSON(
	cJSON *pdu_session_typesJSON) {
	...
	default_session_type =
		cJSON_GetObjectItemCaseSensitive(pdu_session_typesJSON,
		                                 "defaultSessionType");
	if(default_session_type) {
		if(!cJSON_IsString(default_session_type)) {
			ogs_error(
				"OpenAPI_pdu_session_types_parseFromJSON() failed [default_session_type]");
			goto end;
		}
	...
	if(allowed_session_types) {
		...
		if(!cJSON_IsArray(allowed_session_types)) {
			ogs_error(
				"OpenAPI_pdu_session_types_parseFromJSON() failed [allowed_session_types]");
			goto end;
		}
		...
		if(allowed_session_typesList->count == 0) {
			ogs_error(
				"OpenAPI_pdu_session_types_parseFromJSON() failed: Expected allowed_session_typesList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
...
OpenAPI_pdu_session_types_t *OpenAPI_pdu_session_types_copy(
	OpenAPI_pdu_session_types_t *dst, OpenAPI_pdu_session_types_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
