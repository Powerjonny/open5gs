...
OpenAPI_data_filter_t *OpenAPI_data_filter_create(
	...
	OpenAPI_data_filter_t *data_filter_local_var =
		ogs_malloc(sizeof(OpenAPI_data_filter_t));
...
cJSON *OpenAPI_data_filter_convertToJSON(OpenAPI_data_filter_t *data_filter) {
	...
	if(data_filter == NULL) {
		ogs_error(
			"OpenAPI_data_filter_convertToJSON() failed [DataFilter]");
		return NULL;
	}
	...
	if(!data_filter->data_ind) {
		ogs_error(
			"OpenAPI_data_filter_convertToJSON() failed [data_ind]");
		return NULL;
	}
	cJSON *data_ind_local_JSON =
		OpenAPI_data_ind_convertToJSON(data_filter->data_ind);
	...
	if(data_filter->dnns) {
		...
		if(dnnsList == NULL) {
			ogs_error(
				"OpenAPI_data_filter_convertToJSON() failed [dnns]");
			goto end;
		}
		OpenAPI_list_for_each(data_filter->dnns, node) {
	...
	if(data_filter->snssais) {
		...
		if(snssaisList == NULL) {
			ogs_error(
				"OpenAPI_data_filter_convertToJSON() failed [snssais]");
			goto end;
		}
		OpenAPI_list_for_each(data_filter->snssais, node) {
			cJSON *itemLocal =
	...
	if(data_filter->internal_group_ids) {
		...
		if(internal_group_idsList == NULL) {
			ogs_error(
				"OpenAPI_data_filter_convertToJSON() failed [internal_group_ids]");
			goto end;
		}
		OpenAPI_list_for_each(data_filter->internal_group_ids, node) {
	...
	if(data_filter->supis) {
		...
		if(supisList == NULL) {
			ogs_error(
				"OpenAPI_data_filter_convertToJSON() failed [supis]");
			goto end;
		}
		OpenAPI_list_for_each(data_filter->supis, node) {
	...
	if(data_filter->app_ids) {
		...
		if(app_idsList == NULL) {
			ogs_error(
				"OpenAPI_data_filter_convertToJSON() failed [app_ids]");
			goto end;
		}
		OpenAPI_list_for_each(data_filter->app_ids, node) {
	...
	if(data_filter->ue_ipv4s) {
		...
		if(ue_ipv4sList == NULL) {
			ogs_error(
				"OpenAPI_data_filter_convertToJSON() failed [ue_ipv4s]");
			goto end;
		}
		OpenAPI_list_for_each(data_filter->ue_ipv4s, node) {
			if(cJSON_AddStringToObject(ue_ipv4sList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_data_filter_convertToJSON() failed [ue_ipv4s]");
	...
	if(data_filter->ue_ipv6s) {
		...
		if(ue_ipv6sList == NULL) {
			ogs_error(
				"OpenAPI_data_filter_convertToJSON() failed [ue_ipv6s]");
			goto end;
		}
		OpenAPI_list_for_each(data_filter->ue_ipv6s, node) {
			if(cJSON_AddStringToObject(ue_ipv6sList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_data_filter_convertToJSON() failed [ue_ipv6s]");
	...
	if(data_filter->ue_macs) {
		...
		if(ue_macsList == NULL) {
			ogs_error(
				"OpenAPI_data_filter_convertToJSON() failed [ue_macs]");
			goto end;
		}
		OpenAPI_list_for_each(data_filter->ue_macs, node) {
			if(cJSON_AddStringToObject(ue_macsList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_data_filter_convertToJSON() failed [ue_macs]");
	...
	if(data_filter->is_any_ue_ind) {
		if(cJSON_AddBoolToObject(item, "anyUeInd",
		                         data_filter->any_ue_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_data_filter_convertToJSON() failed [any_ue_ind]");
			goto end;
		}
	}
	...
	if(data_filter->dnn_snssai_infos) {
		...
		if(dnn_snssai_infosList == NULL) {
			ogs_error(
				"OpenAPI_data_filter_convertToJSON() failed [dnn_snssai_infos]");
			goto end;
		}
		OpenAPI_list_for_each(data_filter->dnn_snssai_infos, node) {
			cJSON *itemLocal =
...
OpenAPI_data_filter_t *OpenAPI_data_filter_parseFromJSON(
	...
	if(!data_ind) {
		ogs_error(
			"OpenAPI_data_filter_parseFromJSON() failed [data_ind]");
		goto end;
	}
	...
	if(dnns) {
		...
		if(!cJSON_IsArray(dnns)) {
			ogs_error(
				"OpenAPI_data_filter_parseFromJSON() failed [dnns]");
			goto end;
		}
	...
	if(snssais) {
		...
		if(!cJSON_IsArray(snssais)) {
			ogs_error(
				"OpenAPI_data_filter_parseFromJSON() failed [snssais]");
			goto end;
		}
		...
		cJSON_ArrayForEach(snssais_local, snssais) {
			...
			OpenAPI_snssai_t *snssaisItem =
				OpenAPI_snssai_parseFromJSON(snssais_local);
			if(!snssaisItem) {
	...
	internal_group_ids = cJSON_GetObjectItemCaseSensitive(data_filterJSON,
	                                                      "internalGroupIds");
	if(internal_group_ids) {
		cJSON *internal_group_ids_local = NULL;
		if(!cJSON_IsArray(internal_group_ids)) {
			ogs_error(
				"OpenAPI_data_filter_parseFromJSON() failed [internal_group_ids]");
			goto end;
		}
	...
	if(supis) {
		...
		if(!cJSON_IsArray(supis)) {
			ogs_error(
				"OpenAPI_data_filter_parseFromJSON() failed [supis]");
			goto end;
		}
	...
	if(app_ids) {
		...
		if(!cJSON_IsArray(app_ids)) {
			ogs_error(
				"OpenAPI_data_filter_parseFromJSON() failed [app_ids]");
			goto end;
		}
	...
	if(ue_ipv4s) {
		...
		if(!cJSON_IsArray(ue_ipv4s)) {
			ogs_error(
				"OpenAPI_data_filter_parseFromJSON() failed [ue_ipv4s]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ue_ipv4s_local, ue_ipv4s) {
			...
			OpenAPI_list_add(ue_ipv4sList,
			                 ogs_strdup(
						 ue_ipv4s_local->valuestring));
		}
	}
	...
	if(ue_ipv6s) {
		...
		if(!cJSON_IsArray(ue_ipv6s)) {
			ogs_error(
				"OpenAPI_data_filter_parseFromJSON() failed [ue_ipv6s]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ue_ipv6s_local, ue_ipv6s) {
			...
			OpenAPI_list_add(ue_ipv6sList,
			                 ogs_strdup(
						 ue_ipv6s_local->valuestring));
		}
	}
	...
	if(ue_macs) {
		...
		if(!cJSON_IsArray(ue_macs)) {
			ogs_error(
				"OpenAPI_data_filter_parseFromJSON() failed [ue_macs]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ue_macs_local, ue_macs) {
			...
			OpenAPI_list_add(ue_macsList,
			                 ogs_strdup(
						 ue_macs_local->valuestring));
		}
	}

	any_ue_ind = cJSON_GetObjectItemCaseSensitive(data_filterJSON,
	                                              "anyUeInd");
	if(any_ue_ind) {
		if(!cJSON_IsBool(any_ue_ind)) {
			ogs_error(
				"OpenAPI_data_filter_parseFromJSON() failed [any_ue_ind]");
			goto end;
		}
	}

	dnn_snssai_infos = cJSON_GetObjectItemCaseSensitive(data_filterJSON,
	                                                    "dnnSnssaiInfos");
	if(dnn_snssai_infos) {
		cJSON *dnn_snssai_infos_local = NULL;
		if(!cJSON_IsArray(dnn_snssai_infos)) {
			ogs_error(
				"OpenAPI_data_filter_parseFromJSON() failed [dnn_snssai_infos]");
			goto end;
		}
