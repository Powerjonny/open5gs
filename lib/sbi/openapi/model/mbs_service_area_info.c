...
OpenAPI_mbs_service_area_info_t *OpenAPI_mbs_service_area_info_create(
	...
	OpenAPI_mbs_service_area_info_t *mbs_service_area_info_local_var =
		ogs_malloc(sizeof(OpenAPI_mbs_service_area_info_t));
...
void OpenAPI_mbs_service_area_info_free(
	...
	if(mbs_service_area_info->mbs_service_area) {
		OpenAPI_mbs_service_area_free(
			mbs_service_area_info->mbs_service_area);
...
cJSON *OpenAPI_mbs_service_area_info_convertToJSON(
	...
	if(mbs_service_area_info == NULL) {
		ogs_error(
			"OpenAPI_mbs_service_area_info_convertToJSON() failed [MbsServiceAreaInfo]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(cJSON_AddNumberToObject(item, "areaSessionId",
	                           mbs_service_area_info->area_session_id) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_mbs_service_area_info_convertToJSON() failed [area_session_id]");
		goto end;
	}

	if(!mbs_service_area_info->mbs_service_area) {
		ogs_error(
			"OpenAPI_mbs_service_area_info_convertToJSON() failed [mbs_service_area]");
		return NULL;
	}
	cJSON *mbs_service_area_local_JSON =
		OpenAPI_mbs_service_area_convertToJSON(
			mbs_service_area_info->mbs_service_area);
	...
	cJSON_AddItemToObject(item, "mbsServiceArea",
	                      mbs_service_area_local_JSON);
	if(item->child == NULL) {
		ogs_error(
			"OpenAPI_mbs_service_area_info_convertToJSON() failed [mbs_service_area]");
		goto end;
...
OpenAPI_mbs_service_area_info_t *OpenAPI_mbs_service_area_info_parseFromJSON(
	...
	area_session_id =
		cJSON_GetObjectItemCaseSensitive(mbs_service_area_infoJSON,
		                                 "areaSessionId");
	if(!area_session_id) {
		ogs_error(
			"OpenAPI_mbs_service_area_info_parseFromJSON() failed [area_session_id]");
		goto end;
	}
	...
	if(!mbs_service_area) {
		ogs_error(
			"OpenAPI_mbs_service_area_info_parseFromJSON() failed [mbs_service_area]");
		goto end;
	}
	mbs_service_area_local_nonprim =
		OpenAPI_mbs_service_area_parseFromJSON(mbs_service_area);
	if(!mbs_service_area_local_nonprim) {
		ogs_error(
			"OpenAPI_mbs_service_area_parseFromJSON failed [mbs_service_area]");
		goto end;
	}
...
OpenAPI_mbs_service_area_info_t *OpenAPI_mbs_service_area_info_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_mbs_service_area_info_convertToJSON() failed");
		return NULL;
	}
