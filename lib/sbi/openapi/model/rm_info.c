...
OpenAPI_rm_info_t *OpenAPI_rm_info_create(OpenAPI_rm_state_t	*rm_state,
	...
	OpenAPI_rm_info_t *rm_info_local_var =
		ogs_malloc(sizeof(OpenAPI_rm_info_t));
...
cJSON *OpenAPI_rm_info_convertToJSON(OpenAPI_rm_info_t *rm_info) {
	...
	cJSON *rm_state_local_JSON =
		OpenAPI_rm_state_convertToJSON(rm_info->rm_state);
	...
	if(rm_info->access_type == OpenAPI_access_type_NULL) {
		ogs_error(
			"OpenAPI_rm_info_convertToJSON() failed [access_type]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "accessType",
	                           OpenAPI_access_type_ToString(
					   rm_info->access_type)) == NULL)
	{
		ogs_error(
			"OpenAPI_rm_info_convertToJSON() failed [access_type]");
		goto end;
...
OpenAPI_rm_info_t *OpenAPI_rm_info_parseFromJSON(cJSON *rm_infoJSON) {
	...
	access_type = cJSON_GetObjectItemCaseSensitive(rm_infoJSON,
	                                               "accessType");
	if(!access_type) {
		ogs_error(
			"OpenAPI_rm_info_parseFromJSON() failed [access_type]");
		goto end;
	}
