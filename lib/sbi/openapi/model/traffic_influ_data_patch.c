...
OpenAPI_traffic_influ_data_patch_t *OpenAPI_traffic_influ_data_patch_create(
	...
	int app_relo_ind, OpenAPI_list_t *eth_traffic_filters,
	OpenAPI_list_t *traffic_filters, OpenAPI_list_t *traffic_routes,
	bool is_traff_corre_ind, int traff_corre_ind, char *valid_start_time,
	char *valid_end_time, bool is_temp_validities_null,
	...
	bool is_af_ack_ind, int af_ack_ind, bool is_addr_preser_ind,
	int addr_preser_ind, bool is_max_allowed_up_lat_null,
	bool is_max_allowed_up_lat, int max_allowed_up_lat,
	bool is_sim_conn_ind, int sim_conn_ind, bool is_sim_conn_term_null,
	bool is_sim_conn_term, int sim_conn_term) {
	OpenAPI_traffic_influ_data_patch_t *traffic_influ_data_patch_local_var =
		ogs_malloc(sizeof(OpenAPI_traffic_influ_data_patch_t));
	ogs_assert(traffic_influ_data_patch_local_var);

	traffic_influ_data_patch_local_var->up_path_chg_notif_corre_id =
	...
	traffic_influ_data_patch_local_var->eth_traffic_filters =
		eth_traffic_filters;
	...
	traffic_influ_data_patch_local_var->is_traff_corre_ind =
		is_traff_corre_ind;
	...
	traffic_influ_data_patch_local_var->is_temp_validities_null =
		is_temp_validities_null;
	...
	traffic_influ_data_patch_local_var->up_path_chg_notif_uri =
	...
	traffic_influ_data_patch_local_var->is_addr_preser_ind =
		is_addr_preser_ind;
	traffic_influ_data_patch_local_var->addr_preser_ind = addr_preser_ind;
	traffic_influ_data_patch_local_var->is_max_allowed_up_lat_null =
		is_max_allowed_up_lat_null;
	traffic_influ_data_patch_local_var->is_max_allowed_up_lat =
		is_max_allowed_up_lat;
	traffic_influ_data_patch_local_var->max_allowed_up_lat =
		max_allowed_up_lat;
	...
	traffic_influ_data_patch_local_var->is_sim_conn_term_null =
		is_sim_conn_term_null;
...
void OpenAPI_traffic_influ_data_patch_free(
	OpenAPI_traffic_influ_data_patch_t *traffic_influ_data_patch) {
	...
	if(traffic_influ_data_patch->eth_traffic_filters) {
		OpenAPI_list_for_each(
			traffic_influ_data_patch->eth_traffic_filters, node) {
			OpenAPI_eth_flow_description_free(node->data);
		}
		OpenAPI_list_free(
			traffic_influ_data_patch->eth_traffic_filters);
		traffic_influ_data_patch->eth_traffic_filters = NULL;
	}
	if(traffic_influ_data_patch->traffic_filters) {
		OpenAPI_list_for_each(traffic_influ_data_patch->traffic_filters,
		                      node) {
	...
	if(traffic_influ_data_patch->traffic_routes) {
		OpenAPI_list_for_each(traffic_influ_data_patch->traffic_routes,
		                      node) {
	...
	if(traffic_influ_data_patch->temp_validities) {
		OpenAPI_list_for_each(traffic_influ_data_patch->temp_validities,
		                      node) {
	...
	if(traffic_influ_data_patch->nw_area_info) {
		OpenAPI_network_area_info_2_free(
			traffic_influ_data_patch->nw_area_info);
...
cJSON *OpenAPI_traffic_influ_data_patch_convertToJSON(
	OpenAPI_traffic_influ_data_patch_t *traffic_influ_data_patch) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(traffic_influ_data_patch == NULL) {
		ogs_error(
			"OpenAPI_traffic_influ_data_patch_convertToJSON() failed [TrafficInfluDataPatch]");
		return NULL;
	}
	...
	if(traffic_influ_data_patch->up_path_chg_notif_corre_id) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_convertToJSON() failed [up_path_chg_notif_corre_id]");
			goto end;
		}
	}
	...
	if(traffic_influ_data_patch->is_app_relo_ind) {
		if(cJSON_AddBoolToObject(item, "appReloInd",
		                         traffic_influ_data_patch->app_relo_ind)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_convertToJSON() failed [app_relo_ind]");
			goto end;
		}
	}
	...
	if(traffic_influ_data_patch->eth_traffic_filters) {
		...
		if(eth_traffic_filtersList == NULL) {
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_convertToJSON() failed [eth_traffic_filters]");
			goto end;
		}
		OpenAPI_list_for_each(
			traffic_influ_data_patch->eth_traffic_filters, node) {
	...
	if(traffic_influ_data_patch->traffic_filters) {
		...
		if(traffic_filtersList == NULL) {
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_convertToJSON() failed [traffic_filters]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_influ_data_patch->traffic_filters,
		                      node) {
	...
	if(traffic_influ_data_patch->traffic_routes) {
		...
		if(traffic_routesList == NULL) {
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_convertToJSON() failed [traffic_routes]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_influ_data_patch->traffic_routes,
		                      node) {
	...
	if(traffic_influ_data_patch->is_traff_corre_ind) {
		if(cJSON_AddBoolToObject(item, "traffCorreInd",
		                         traffic_influ_data_patch->
		                         traff_corre_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_convertToJSON() failed [traff_corre_ind]");
			goto end;
		}
	}
	...
	if(traffic_influ_data_patch->valid_start_time) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_convertToJSON() failed [valid_start_time]");
			goto end;
		}
	}
	...
	if(traffic_influ_data_patch->valid_end_time) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_convertToJSON() failed [valid_end_time]");
			goto end;
		}
	}
	...
	if(traffic_influ_data_patch->temp_validities) {
		...
		if(temp_validitiesList == NULL) {
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_convertToJSON() failed [temp_validities]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_influ_data_patch->temp_validities,
		                      node) {
	...
	if(traffic_influ_data_patch->nw_area_info) {
		...
		if(nw_area_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_convertToJSON() failed [nw_area_info]");
			goto end;
		}
	...
	if(traffic_influ_data_patch->up_path_chg_notif_uri) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_convertToJSON() failed [up_path_chg_notif_uri]");
			goto end;
		}
	}
	...
	if(traffic_influ_data_patch->headers) {
		...
		if(headersList == NULL) {
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_convertToJSON() failed [headers]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_influ_data_patch->headers, node) {
	...
	if(traffic_influ_data_patch->is_af_ack_ind) {
		if(cJSON_AddBoolToObject(item, "afAckInd",
		                         traffic_influ_data_patch->af_ack_ind)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_convertToJSON() failed [af_ack_ind]");
			goto end;
		}
	}
	...
	if(traffic_influ_data_patch->is_addr_preser_ind) {
		if(cJSON_AddBoolToObject(item, "addrPreserInd",
		                         traffic_influ_data_patch->
		                         addr_preser_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_convertToJSON() failed [addr_preser_ind]");
			goto end;
		}
	}
	...
	if(traffic_influ_data_patch->is_max_allowed_up_lat) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_convertToJSON() failed [max_allowed_up_lat]");
			goto end;
		}
	...
	if(traffic_influ_data_patch->is_sim_conn_ind) {
		if(cJSON_AddBoolToObject(item, "simConnInd",
		                         traffic_influ_data_patch->sim_conn_ind)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_convertToJSON() failed [sim_conn_ind]");
			goto end;
		}
	}
	...
	if(traffic_influ_data_patch->is_sim_conn_term) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_convertToJSON() failed [sim_conn_term]");
			goto end;
		}
...
OpenAPI_traffic_influ_data_patch_parseFromJSON(
	cJSON *traffic_influ_data_patchJSON) {
	OpenAPI_traffic_influ_data_patch_t *traffic_influ_data_patch_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(up_path_chg_notif_corre_id) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_parseFromJSON() failed [up_path_chg_notif_corre_id]");
			goto end;
		}
	}
	...
	if(app_relo_ind) {
		if(!cJSON_IsBool(app_relo_ind)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_parseFromJSON() failed [app_relo_ind]");
			goto end;
		}
	}
	...
	if(eth_traffic_filters) {
		...
		if(!cJSON_IsArray(eth_traffic_filters)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_parseFromJSON() failed [eth_traffic_filters]");
			goto end;
		}
	...
	if(traffic_filters) {
		...
		if(!cJSON_IsArray(traffic_filters)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_parseFromJSON() failed [traffic_filters]");
			goto end;
		}
	...
	if(traffic_routes) {
		...
		if(!cJSON_IsArray(traffic_routes)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_parseFromJSON() failed [traffic_routes]");
			goto end;
		}
	...
	if(valid_start_time) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_parseFromJSON() failed [valid_start_time]");
			goto end;
		}
	}
	...
	if(valid_end_time) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_parseFromJSON() failed [valid_end_time]");
			goto end;
		}
	}
	...
	if(temp_validities) {
		if(!cJSON_IsNull(temp_validities)) {
			...
			if(!cJSON_IsArray(temp_validities)) {
				ogs_error(
					"OpenAPI_traffic_influ_data_patch_parseFromJSON() failed [temp_validities]");
				goto end;
			}

			temp_validitiesList = OpenAPI_list_create();

			cJSON_ArrayForEach(temp_validities_local,
			                   temp_validities) {
	...
	if(up_path_chg_notif_uri) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_parseFromJSON() failed [up_path_chg_notif_uri]");
			goto end;
		}
	}

	headers = cJSON_GetObjectItemCaseSensitive(traffic_influ_data_patchJSON,
	                                           "headers");
	if(headers) {
		cJSON *headers_local = NULL;
		if(!cJSON_IsArray(headers)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_parseFromJSON() failed [headers]");
			goto end;
		}
	...
	if(af_ack_ind) {
		if(!cJSON_IsBool(af_ack_ind)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_parseFromJSON() failed [af_ack_ind]");
			goto end;
		}
	}
	...
	if(addr_preser_ind) {
		if(!cJSON_IsBool(addr_preser_ind)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_parseFromJSON() failed [addr_preser_ind]");
			goto end;
		}
	}
	...
	if(max_allowed_up_lat) {
		if(!cJSON_IsNull(max_allowed_up_lat)) {
			if(!cJSON_IsNumber(max_allowed_up_lat)) {
				ogs_error(
					"OpenAPI_traffic_influ_data_patch_parseFromJSON() failed [max_allowed_up_lat]");
				goto end;
			}
		}
	}
	...
	if(sim_conn_ind) {
		if(!cJSON_IsBool(sim_conn_ind)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_patch_parseFromJSON() failed [sim_conn_ind]");
			goto end;
		}
	}
	...
	if(sim_conn_term) {
		if(!cJSON_IsNull(sim_conn_term)) {
			if(!cJSON_IsNumber(sim_conn_term)) {
				ogs_error(
					"OpenAPI_traffic_influ_data_patch_parseFromJSON() failed [sim_conn_term]");
				goto end;
			}
		}
	}

	traffic_influ_data_patch_local_var =
		OpenAPI_traffic_influ_data_patch_create(
			up_path_chg_notif_corre_id &&
			!cJSON_IsNull(up_path_chg_notif_corre_id) ?
			ogs_strdup(up_path_chg_notif_corre_id->valuestring) :
			NULL,
			...
			temp_validities &&
			cJSON_IsNull(temp_validities) ? true : false,
			...
			max_allowed_up_lat ? max_allowed_up_lat->valuedouble :
			0,
			sim_conn_ind ? true : false,
			sim_conn_ind ? sim_conn_ind->valueint : 0,
			sim_conn_term &&
			cJSON_IsNull(sim_conn_term) ? true : false,
...
OpenAPI_traffic_influ_data_patch_t *OpenAPI_traffic_influ_data_patch_copy(
	OpenAPI_traffic_influ_data_patch_t	*dst,
	OpenAPI_traffic_influ_data_patch_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_traffic_influ_data_patch_convertToJSON() failed");
		return NULL;
	}
