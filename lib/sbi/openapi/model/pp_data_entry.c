...
OpenAPI_pp_data_entry_t *OpenAPI_pp_data_entry_create(
	...
	communication_characteristics, bool is_reference_id, int reference_id,
	char *validity_time, char *mtc_provider_information,
	char *supported_features, bool is_ecs_addr_config_info_null,
	...
	OpenAPI_pp_data_entry_t *pp_data_entry_local_var =
		ogs_malloc(sizeof(OpenAPI_pp_data_entry_t));
	ogs_assert(pp_data_entry_local_var);

	pp_data_entry_local_var->is_communication_characteristics_null =
		is_communication_characteristics_null;
	pp_data_entry_local_var->communication_characteristics =
...
void OpenAPI_pp_data_entry_free(OpenAPI_pp_data_entry_t *pp_data_entry) {
	...
	if(pp_data_entry->communication_characteristics) {
		OpenAPI_communication_characteristics_af_free(
...
cJSON *OpenAPI_pp_data_entry_convertToJSON(
	OpenAPI_pp_data_entry_t *pp_data_entry) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(pp_data_entry == NULL) {
		ogs_error(
			"OpenAPI_pp_data_entry_convertToJSON() failed [PpDataEntry]");
		return NULL;
	}
	...
	if(pp_data_entry->communication_characteristics) {
		cJSON *communication_characteristics_local_JSON =
			OpenAPI_communication_characteristics_af_convertToJSON(
				pp_data_entry->communication_characteristics);
		if(communication_characteristics_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_pp_data_entry_convertToJSON() failed [communication_characteristics]");
			goto end;
		}
		cJSON_AddItemToObject(item, "communicationCharacteristics",
		                      communication_characteristics_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_pp_data_entry_convertToJSON() failed [communication_characteristics]");
			goto end;
	...
	if(pp_data_entry->is_reference_id) {
		if(cJSON_AddNumberToObject(item, "referenceId",
		                           pp_data_entry->reference_id) == NULL)
		{
			ogs_error(
				"OpenAPI_pp_data_entry_convertToJSON() failed [reference_id]");
			goto end;
		}
	}
	...
	if(pp_data_entry->validity_time) {
		if(cJSON_AddStringToObject(item, "validityTime",
		                           pp_data_entry->validity_time) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pp_data_entry_convertToJSON() failed [validity_time]");
			goto end;
		}
	}
	...
	if(pp_data_entry->mtc_provider_information) {
		...
		{
			ogs_error(
				"OpenAPI_pp_data_entry_convertToJSON() failed [mtc_provider_information]");
			goto end;
		}
	}
	...
	if(pp_data_entry->supported_features) {
		if(cJSON_AddStringToObject(item, "supportedFeatures",
		                           pp_data_entry->supported_features) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pp_data_entry_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(pp_data_entry->ecs_addr_config_info) {
		...
		if(ecs_addr_config_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_pp_data_entry_convertToJSON() failed [ecs_addr_config_info]");
			goto end;
		}
	...
	if(pp_data_entry->additional_ecs_addr_config_infos) {
		...
		if(additional_ecs_addr_config_infosList == NULL) {
			ogs_error(
				"OpenAPI_pp_data_entry_convertToJSON() failed [additional_ecs_addr_config_infos]");
			goto end;
		}
		OpenAPI_list_for_each(
			pp_data_entry->additional_ecs_addr_config_infos, node) {
	...
	if(pp_data_entry->ec_restriction) {
		cJSON *ec_restriction_local_JSON =
			OpenAPI_ec_restriction_1_convertToJSON(
				pp_data_entry->ec_restriction);
		if(ec_restriction_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_pp_data_entry_convertToJSON() failed [ec_restriction]");
			goto end;
		}
		cJSON_AddItemToObject(item, "ecRestriction",
		                      ec_restriction_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_pp_data_entry_convertToJSON() failed [ec_restriction]");
			goto end;
...
OpenAPI_pp_data_entry_t *OpenAPI_pp_data_entry_parseFromJSON(
	cJSON *pp_data_entryJSON) {
	...
	if(communication_characteristics) {
		if(!cJSON_IsNull(communication_characteristics)) {
			...
			if(!communication_characteristics_local_nonprim) {
				ogs_error(
					"OpenAPI_communication_characteristics_af_parseFromJSON failed [communication_characteristics]");
				goto end;
			}
		}
	}

	reference_id = cJSON_GetObjectItemCaseSensitive(pp_data_entryJSON,
	                                                "referenceId");
	if(reference_id) {
		if(!cJSON_IsNumber(reference_id)) {
			ogs_error(
				"OpenAPI_pp_data_entry_parseFromJSON() failed [reference_id]");
			goto end;
		}
	}

	validity_time = cJSON_GetObjectItemCaseSensitive(pp_data_entryJSON,
	                                                 "validityTime");
	if(validity_time) {
		...
		{
			ogs_error(
				"OpenAPI_pp_data_entry_parseFromJSON() failed [validity_time]");
			goto end;
		}
	}
	...
	if(mtc_provider_information) {
		...
		{
			ogs_error(
				"OpenAPI_pp_data_entry_parseFromJSON() failed [mtc_provider_information]");
			goto end;
		}
	}

	supported_features = cJSON_GetObjectItemCaseSensitive(pp_data_entryJSON,
	                                                      "supportedFeatures");
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_pp_data_entry_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(additional_ecs_addr_config_infos) {
		...
		if(!cJSON_IsArray(additional_ecs_addr_config_infos)) {
			ogs_error(
				"OpenAPI_pp_data_entry_parseFromJSON() failed [additional_ecs_addr_config_infos]");
			goto end;
		}
	...
	ec_restriction = cJSON_GetObjectItemCaseSensitive(pp_data_entryJSON,
	                                                  "ecRestriction");
	if(ec_restriction) {
		if(!cJSON_IsNull(ec_restriction)) {
			ec_restriction_local_nonprim =
				OpenAPI_ec_restriction_1_parseFromJSON(
					ec_restriction);
			if(!ec_restriction_local_nonprim) {
				ogs_error(
					"OpenAPI_ec_restriction_1_parseFromJSON failed [ec_restriction]");
				goto end;
			}
		}
	}
	...
	pp_data_entry_local_var = OpenAPI_pp_data_entry_create(
		communication_characteristics &&
		cJSON_IsNull(communication_characteristics) ? true : false,
		communication_characteristics ?
		communication_characteristics_local_nonprim : NULL,
		...
		ec_restriction &&
		cJSON_IsNull(ec_restriction) ? true : false,
...
OpenAPI_pp_data_entry_t *OpenAPI_pp_data_entry_copy(
	OpenAPI_pp_data_entry_t *dst, OpenAPI_pp_data_entry_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
