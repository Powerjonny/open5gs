...
OpenAPI_aanf_info_t *OpenAPI_aanf_info_create(
	...
	OpenAPI_aanf_info_t *aanf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_aanf_info_t));
...
cJSON *OpenAPI_aanf_info_convertToJSON(OpenAPI_aanf_info_t *aanf_info) {
	...
	if(aanf_info == NULL) {
		ogs_error(
			"OpenAPI_aanf_info_convertToJSON() failed [AanfInfo]");
		return NULL;
	}
	...
	if(aanf_info->routing_indicators) {
		...
		if(routing_indicatorsList == NULL) {
			ogs_error(
				"OpenAPI_aanf_info_convertToJSON() failed [routing_indicators]");
			goto end;
		}
		OpenAPI_list_for_each(aanf_info->routing_indicators, node) {
...
OpenAPI_aanf_info_t *OpenAPI_aanf_info_parseFromJSON(cJSON *aanf_infoJSON) {
	...
	routing_indicators = cJSON_GetObjectItemCaseSensitive(aanf_infoJSON,
	                                                      "routingIndicators");
	if(routing_indicators) {
		cJSON *routing_indicators_local = NULL;
		if(!cJSON_IsArray(routing_indicators)) {
			ogs_error(
				"OpenAPI_aanf_info_parseFromJSON() failed [routing_indicators]");
			goto end;
		}
...
OpenAPI_aanf_info_t *OpenAPI_aanf_info_copy(OpenAPI_aanf_info_t *dst,
                                            OpenAPI_aanf_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
