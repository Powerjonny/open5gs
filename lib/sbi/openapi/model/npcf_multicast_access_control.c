...
char * OpenAPI_npcf_multicast_access_control_ToString(
	...
	const char *npcf_multicast_access_controlArray[] = { "NULL", "ALLOWED",
		                                             "NOT_ALLOWED" };
	size_t sizeofArray = sizeof(npcf_multicast_access_controlArray) /
	                     sizeof(npcf_multicast_access_controlArray[0]);
	if(npcf_multicast_access_control < sizeofArray) {
		return (char *) npcf_multicast_access_controlArray[
			npcf_multicast_access_control];
	} else {
		return (char *) "Unknown";
	}
}
...
OpenAPI_npcf_multicast_access_control_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(npcf_multicast_access_control,
		          npcf_multicast_access_controlArray[stringToReturn]) ==
		   0) {
