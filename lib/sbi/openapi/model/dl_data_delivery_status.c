...
char * OpenAPI_dl_data_delivery_status_ToString(
	...
	const char *dl_data_delivery_statusArray[] = { "NULL", "BUFFERED",
		                                       "TRANSMITTED",
		                                       "DISCARDED" };
	size_t sizeofArray = sizeof(dl_data_delivery_statusArray) /
	                     sizeof(dl_data_delivery_statusArray[0]);
	if(dl_data_delivery_status < sizeofArray) {
		return (char *) dl_data_delivery_statusArray[
...
OpenAPI_dl_data_delivery_status_e OpenAPI_dl_data_delivery_status_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(dl_data_delivery_status,
		          dl_data_delivery_statusArray[stringToReturn]) == 0) {
