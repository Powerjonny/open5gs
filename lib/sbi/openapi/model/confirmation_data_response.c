...
OpenAPI_confirmation_data_response_t *OpenAPI_confirmation_data_response_create(
	OpenAPI_auth_result_e auth_result, char *supi, char *kseaf,
	OpenAPI_list_t *pvs_info) {
	OpenAPI_confirmation_data_response_t *
	        confirmation_data_response_local_var =
		ogs_malloc(sizeof(OpenAPI_confirmation_data_response_t));
...
void OpenAPI_confirmation_data_response_free(
	OpenAPI_confirmation_data_response_t *confirmation_data_response) {
	...
	if(confirmation_data_response->pvs_info) {
		OpenAPI_list_for_each(confirmation_data_response->pvs_info,
		                      node) {
...
cJSON *OpenAPI_confirmation_data_response_convertToJSON(
	OpenAPI_confirmation_data_response_t *confirmation_data_response) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(confirmation_data_response == NULL) {
		ogs_error(
			"OpenAPI_confirmation_data_response_convertToJSON() failed [ConfirmationDataResponse]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(confirmation_data_response->auth_result ==
	   OpenAPI_auth_result_NULL)
	{
		ogs_error(
			"OpenAPI_confirmation_data_response_convertToJSON() failed [auth_result]");
		return NULL;
	}
	...
	if(confirmation_data_response->supi) {
		if(cJSON_AddStringToObject(item, "supi",
		                           confirmation_data_response->supi) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_confirmation_data_response_convertToJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(confirmation_data_response->kseaf) {
		...
		{
			ogs_error(
				"OpenAPI_confirmation_data_response_convertToJSON() failed [kseaf]");
			goto end;
		}
	}
	...
	if(confirmation_data_response->pvs_info) {
		...
		if(pvs_infoList == NULL) {
			ogs_error(
				"OpenAPI_confirmation_data_response_convertToJSON() failed [pvs_info]");
			goto end;
		}
		OpenAPI_list_for_each(confirmation_data_response->pvs_info,
		                      node) {
...
OpenAPI_confirmation_data_response_parseFromJSON(
	cJSON *confirmation_data_responseJSON) {
	OpenAPI_confirmation_data_response_t *
	        confirmation_data_response_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	auth_result =
		cJSON_GetObjectItemCaseSensitive(confirmation_data_responseJSON,
		                                 "authResult");
	if(!auth_result) {
		ogs_error(
			"OpenAPI_confirmation_data_response_parseFromJSON() failed [auth_result]");
		goto end;
	}
	...
	auth_resultVariable =
		OpenAPI_auth_result_FromString(auth_result->valuestring);

	supi = cJSON_GetObjectItemCaseSensitive(confirmation_data_responseJSON,
	                                        "supi");
	if(supi) {
		...
		{
			ogs_error(
				"OpenAPI_confirmation_data_response_parseFromJSON() failed [supi]");
			goto end;
		}
	}

	kseaf = cJSON_GetObjectItemCaseSensitive(confirmation_data_responseJSON,
	                                         "kseaf");
	if(kseaf) {
		...
		{
			ogs_error(
				"OpenAPI_confirmation_data_response_parseFromJSON() failed [kseaf]");
			goto end;
		}
	}
	...
	if(pvs_info) {
		...
		if(!cJSON_IsArray(pvs_info)) {
			ogs_error(
				"OpenAPI_confirmation_data_response_parseFromJSON() failed [pvs_info]");
			goto end;
		}
	...
	confirmation_data_response_local_var =
		OpenAPI_confirmation_data_response_create(
			auth_resultVariable,
			supi &&
			!cJSON_IsNull(supi) ? ogs_strdup(supi->valuestring) :
			NULL,
			kseaf &&
			!cJSON_IsNull(kseaf) ? ogs_strdup(kseaf->valuestring) :
			NULL,
...
OpenAPI_confirmation_data_response_t *OpenAPI_confirmation_data_response_copy(
	OpenAPI_confirmation_data_response_t	*dst,
	OpenAPI_confirmation_data_response_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_confirmation_data_response_convertToJSON() failed");
		return NULL;
	}
