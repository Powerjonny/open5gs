...
OpenAPI_psa_information_t *OpenAPI_psa_information_create(
	OpenAPI_psa_indication_e psa_ind, OpenAPI_list_t *dnai_list,
	char *ue_ipv6_prefix, char *psa_upf_id) {
	OpenAPI_psa_information_t *psa_information_local_var =
		ogs_malloc(sizeof(OpenAPI_psa_information_t));
...
cJSON *OpenAPI_psa_information_convertToJSON(
	...
	if(psa_information == NULL) {
		ogs_error(
			"OpenAPI_psa_information_convertToJSON() failed [PsaInformation]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(psa_information->psa_ind != OpenAPI_psa_indication_NULL) {
		if(cJSON_AddStringToObject(item, "psaInd",
		                           OpenAPI_psa_indication_ToString(
						   psa_information->psa_ind)) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_psa_information_convertToJSON() failed [psa_ind]");
			goto end;
		}
	}
	...
	if(psa_information->dnai_list) {
		...
		if(dnai_listList == NULL) {
			ogs_error(
				"OpenAPI_psa_information_convertToJSON() failed [dnai_list]");
			goto end;
		}
		OpenAPI_list_for_each(psa_information->dnai_list, node) {
	...
	if(psa_information->ue_ipv6_prefix) {
		...
		{
			ogs_error(
				"OpenAPI_psa_information_convertToJSON() failed [ue_ipv6_prefix]");
			goto end;
		}
	}
	...
	if(psa_information->psa_upf_id) {
		...
		{
			ogs_error(
				"OpenAPI_psa_information_convertToJSON() failed [psa_upf_id]");
			goto end;
		}
	}
...
OpenAPI_psa_information_t *OpenAPI_psa_information_parseFromJSON(
	cJSON *psa_informationJSON) {
	...
	psa_ind = cJSON_GetObjectItemCaseSensitive(psa_informationJSON,
	                                           "psaInd");
	if(psa_ind) {
		if(!cJSON_IsString(psa_ind)) {
			ogs_error(
				"OpenAPI_psa_information_parseFromJSON() failed [psa_ind]");
			goto end;
		}
		psa_indVariable =
			OpenAPI_psa_indication_FromString(psa_ind->valuestring);
	}

	dnai_list = cJSON_GetObjectItemCaseSensitive(psa_informationJSON,
	                                             "dnaiList");
	if(dnai_list) {
		cJSON *dnai_list_local = NULL;
		if(!cJSON_IsArray(dnai_list)) {
			ogs_error(
				"OpenAPI_psa_information_parseFromJSON() failed [dnai_list]");
			goto end;
		}
	...
	ue_ipv6_prefix = cJSON_GetObjectItemCaseSensitive(psa_informationJSON,
	                                                  "ueIpv6Prefix");
	if(ue_ipv6_prefix) {
		...
		{
			ogs_error(
				"OpenAPI_psa_information_parseFromJSON() failed [ue_ipv6_prefix]");
			goto end;
		}
	}

	psa_upf_id = cJSON_GetObjectItemCaseSensitive(psa_informationJSON,
	                                              "psaUpfId");
	if(psa_upf_id) {
		...
		{
			ogs_error(
				"OpenAPI_psa_information_parseFromJSON() failed [psa_upf_id]");
			goto end;
		}
	}
...
OpenAPI_psa_information_t *OpenAPI_psa_information_copy(
	OpenAPI_psa_information_t *dst, OpenAPI_psa_information_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
