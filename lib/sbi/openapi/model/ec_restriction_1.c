...
OpenAPI_ec_restriction_1_t *OpenAPI_ec_restriction_1_create(
	...
	OpenAPI_ec_restriction_1_t *ec_restriction_1_local_var =
		ogs_malloc(sizeof(OpenAPI_ec_restriction_1_t));
	...
	ec_restriction_1_local_var->mtc_provider_information =
...
cJSON *OpenAPI_ec_restriction_1_convertToJSON(
	...
	if(ec_restriction_1 == NULL) {
		ogs_error(
			"OpenAPI_ec_restriction_1_convertToJSON() failed [EcRestriction_1]");
		return NULL;
	}
	...
	if(!ec_restriction_1->af_instance_id) {
		ogs_error(
			"OpenAPI_ec_restriction_1_convertToJSON() failed [af_instance_id]");
		return NULL;
	}
	...
	if(cJSON_AddNumberToObject(item, "referenceId",
	                           ec_restriction_1->reference_id) == NULL)
	{
		ogs_error(
			"OpenAPI_ec_restriction_1_convertToJSON() failed [reference_id]");
		goto end;
	}
	...
	if(ec_restriction_1->plmn_ec_infos) {
		...
		if(plmn_ec_infosList == NULL) {
			ogs_error(
				"OpenAPI_ec_restriction_1_convertToJSON() failed [plmn_ec_infos]");
			goto end;
		}
		OpenAPI_list_for_each(ec_restriction_1->plmn_ec_infos, node) {
			cJSON *itemLocal =
				OpenAPI_plmn_ec_info_1_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_ec_restriction_1_convertToJSON() failed [plmn_ec_infos]");
	...
	if(ec_restriction_1->mtc_provider_information) {
		...
		{
			ogs_error(
				"OpenAPI_ec_restriction_1_convertToJSON() failed [mtc_provider_information]");
			goto end;
		}
	}
...
OpenAPI_ec_restriction_1_t *OpenAPI_ec_restriction_1_parseFromJSON(
	...
	af_instance_id = cJSON_GetObjectItemCaseSensitive(ec_restriction_1JSON,
	                                                  "afInstanceId");
	if(!af_instance_id) {
		ogs_error(
			"OpenAPI_ec_restriction_1_parseFromJSON() failed [af_instance_id]");
		goto end;
	}
	...
	reference_id = cJSON_GetObjectItemCaseSensitive(ec_restriction_1JSON,
	                                                "referenceId");
	if(!reference_id) {
		ogs_error(
			"OpenAPI_ec_restriction_1_parseFromJSON() failed [reference_id]");
		goto end;
	}
	...
	plmn_ec_infos = cJSON_GetObjectItemCaseSensitive(ec_restriction_1JSON,
	                                                 "plmnEcInfos");
	if(plmn_ec_infos) {
		cJSON *plmn_ec_infos_local = NULL;
		if(!cJSON_IsArray(plmn_ec_infos)) {
			ogs_error(
				"OpenAPI_ec_restriction_1_parseFromJSON() failed [plmn_ec_infos]");
			goto end;
		}
		...
		cJSON_ArrayForEach(plmn_ec_infos_local, plmn_ec_infos) {
			...
			OpenAPI_plmn_ec_info_1_t *plmn_ec_infosItem =
				OpenAPI_plmn_ec_info_1_parseFromJSON(
					plmn_ec_infos_local);
	...
	mtc_provider_information =
		cJSON_GetObjectItemCaseSensitive(ec_restriction_1JSON,
	...
	if(mtc_provider_information) {
		...
		{
			ogs_error(
				"OpenAPI_ec_restriction_1_parseFromJSON() failed [mtc_provider_information]");
			goto end;
		}
	}
