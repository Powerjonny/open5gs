...
OpenAPI_upu_data_t *OpenAPI_upu_data_create(char *sec_packet,
                                            ...
                                            default_conf_nssai,
                                            char *routing_id) {
	OpenAPI_upu_data_t *upu_data_local_var =
		ogs_malloc(sizeof(OpenAPI_upu_data_t));
...
cJSON *OpenAPI_upu_data_convertToJSON(OpenAPI_upu_data_t *upu_data) {
	...
	if(upu_data->sec_packet) {
		...
		{
			ogs_error(
				"OpenAPI_upu_data_convertToJSON() failed [sec_packet]");
			goto end;
		}
	}
	...
	if(upu_data->default_conf_nssai) {
		...
		if(default_conf_nssaiList == NULL) {
			ogs_error(
				"OpenAPI_upu_data_convertToJSON() failed [default_conf_nssai]");
			goto end;
		}
		OpenAPI_list_for_each(upu_data->default_conf_nssai, node) {
			cJSON *itemLocal =
	...
	if(upu_data->routing_id) {
		...
		{
			ogs_error(
				"OpenAPI_upu_data_convertToJSON() failed [routing_id]");
			goto end;
		}
	}
...
OpenAPI_upu_data_t *OpenAPI_upu_data_parseFromJSON(cJSON *upu_dataJSON) {
	...
	sec_packet = cJSON_GetObjectItemCaseSensitive(upu_dataJSON,
	                                              "secPacket");
	if(sec_packet) {
		...
		{
			ogs_error(
				"OpenAPI_upu_data_parseFromJSON() failed [sec_packet]");
			goto end;
		}
	}

	default_conf_nssai = cJSON_GetObjectItemCaseSensitive(upu_dataJSON,
	                                                      "defaultConfNssai");
	if(default_conf_nssai) {
		cJSON *default_conf_nssai_local = NULL;
		if(!cJSON_IsArray(default_conf_nssai)) {
			ogs_error(
				"OpenAPI_upu_data_parseFromJSON() failed [default_conf_nssai]");
			goto end;
		}
	...
	routing_id = cJSON_GetObjectItemCaseSensitive(upu_dataJSON,
	                                              "routingId");
	if(routing_id) {
		...
		{
			ogs_error(
				"OpenAPI_upu_data_parseFromJSON() failed [routing_id]");
			goto end;
		}
	}
