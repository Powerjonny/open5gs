...
{
	OpenAPI_sdm_subscription_t *sdm_subscription_local_var =
		ogs_malloc(sizeof(OpenAPI_sdm_subscription_t));
	ogs_assert(sdm_subscription_local_var);

	sdm_subscription_local_var->nf_instance_id = nf_instance_id;
	sdm_subscription_local_var->is_implicit_unsubscribe =
	...
	sdm_subscription_local_var->monitored_resource_uris =
	...
	sdm_subscription_local_var->is_unique_subscription =
	...
	sdm_subscription_local_var->ue_con_smf_data_sub_filter =
		ue_con_smf_data_sub_filter;

	return sdm_subscription_local_var;
}

void OpenAPI_sdm_subscription_free(
	...
	if(sdm_subscription->monitored_resource_uris) {
		OpenAPI_list_for_each(sdm_subscription->monitored_resource_uris,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(sdm_subscription->ue_con_smf_data_sub_filter) {
		OpenAPI_ue_context_in_smf_data_sub_filter_free(
			sdm_subscription->ue_con_smf_data_sub_filter);
		sdm_subscription->ue_con_smf_data_sub_filter = NULL;
	}
	ogs_free(sdm_subscription);
}

cJSON *OpenAPI_sdm_subscription_convertToJSON(
	OpenAPI_sdm_subscription_t *sdm_subscription) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(sdm_subscription == NULL) {
		ogs_error(
			"OpenAPI_sdm_subscription_convertToJSON() failed [SdmSubscription]");
		return NULL;
	}
	...
	if(!sdm_subscription->nf_instance_id) {
		ogs_error(
			"OpenAPI_sdm_subscription_convertToJSON() failed [nf_instance_id]");
		return NULL;
	}
	...
	if(sdm_subscription->is_implicit_unsubscribe) {
		if(cJSON_AddBoolToObject(item, "implicitUnsubscribe",
		                         sdm_subscription->implicit_unsubscribe)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_sdm_subscription_convertToJSON() failed [implicit_unsubscribe]");
			goto end;
		}
	}
	...
	if(sdm_subscription->expires) {
		...
		{
			ogs_error(
				"OpenAPI_sdm_subscription_convertToJSON() failed [expires]");
			goto end;
		}
	}
	...
	if(!sdm_subscription->callback_reference) {
		ogs_error(
			"OpenAPI_sdm_subscription_convertToJSON() failed [callback_reference]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "callbackReference",
	                           sdm_subscription->callback_reference) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_sdm_subscription_convertToJSON() failed [callback_reference]");
	...
	if(sdm_subscription->amf_service_name) {
		if(cJSON_AddStringToObject(item, "amfServiceName",
		                           sdm_subscription->amf_service_name)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_sdm_subscription_convertToJSON() failed [amf_service_name]");
			goto end;
		}
	}
	...
	if(!sdm_subscription->monitored_resource_uris) {
		ogs_error(
			"OpenAPI_sdm_subscription_convertToJSON() failed [monitored_resource_uris]");
		return NULL;
	}
	...
	if(sdm_subscription->single_nssai) {
		cJSON *single_nssai_local_JSON =
			OpenAPI_snssai_convertToJSON(
				sdm_subscription->single_nssai);
		if(single_nssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sdm_subscription_convertToJSON() failed [single_nssai]");
			goto end;
		}
	...
	if(sdm_subscription->dnn) {
		...
		{
			ogs_error(
				"OpenAPI_sdm_subscription_convertToJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(sdm_subscription->subscription_id) {
		if(cJSON_AddStringToObject(item, "subscriptionId",
		                           sdm_subscription->subscription_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sdm_subscription_convertToJSON() failed [subscription_id]");
			goto end;
		}
	}
	...
	if(sdm_subscription->plmn_id) {
		...
		if(plmn_id_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sdm_subscription_convertToJSON() failed [plmn_id]");
			goto end;
		}
	...
	if(sdm_subscription->is_immediate_report) {
		if(cJSON_AddBoolToObject(item, "immediateReport",
		                         sdm_subscription->immediate_report) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sdm_subscription_convertToJSON() failed [immediate_report]");
			goto end;
		}
	}
	...
	if(sdm_subscription->report) {
		cJSON *report_local_JSON =
			OpenAPI_immediate_report_convertToJSON(
				sdm_subscription->report);
		if(report_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sdm_subscription_convertToJSON() failed [report]");
			goto end;
		}
	...
	if(sdm_subscription->supported_features) {
		if(cJSON_AddStringToObject(item, "supportedFeatures",
		                           sdm_subscription->supported_features)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_sdm_subscription_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(sdm_subscription->context_info) {
		cJSON *context_info_local_JSON =
			OpenAPI_context_info_convertToJSON(
				sdm_subscription->context_info);
		if(context_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sdm_subscription_convertToJSON() failed [context_info]");
			goto end;
		}
	...
	if(sdm_subscription->is_nf_change_filter) {
		if(cJSON_AddBoolToObject(item, "nfChangeFilter",
		                         sdm_subscription->nf_change_filter) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sdm_subscription_convertToJSON() failed [nf_change_filter]");
			goto end;
		}
	}
	...
	if(sdm_subscription->is_unique_subscription) {
		if(cJSON_AddBoolToObject(item, "uniqueSubscription",
		                         sdm_subscription->unique_subscription)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_sdm_subscription_convertToJSON() failed [unique_subscription]");
			goto end;
		}
	}
	...
	if(sdm_subscription->reset_ids) {
		...
		if(reset_idsList == NULL) {
			ogs_error(
				"OpenAPI_sdm_subscription_convertToJSON() failed [reset_ids]");
			goto end;
		}
		OpenAPI_list_for_each(sdm_subscription->reset_ids, node) {
	...
	if(sdm_subscription->ue_con_smf_data_sub_filter) {
		...
		if(ue_con_smf_data_sub_filter_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sdm_subscription_convertToJSON() failed [ue_con_smf_data_sub_filter]");
			goto end;
		}
...
OpenAPI_sdm_subscription_t *OpenAPI_sdm_subscription_parseFromJSON(
	cJSON *sdm_subscriptionJSON) {
	...
	nf_instance_id = cJSON_GetObjectItemCaseSensitive(sdm_subscriptionJSON,
	                                                  "nfInstanceId");
	if(!nf_instance_id) {
		ogs_error(
			"OpenAPI_sdm_subscription_parseFromJSON() failed [nf_instance_id]");
		goto end;
	}
	...
	implicit_unsubscribe =
		cJSON_GetObjectItemCaseSensitive(sdm_subscriptionJSON,
		                                 "implicitUnsubscribe");
	if(implicit_unsubscribe) {
		if(!cJSON_IsBool(implicit_unsubscribe)) {
			ogs_error(
				"OpenAPI_sdm_subscription_parseFromJSON() failed [implicit_unsubscribe]");
			goto end;
		}
	}

	expires = cJSON_GetObjectItemCaseSensitive(sdm_subscriptionJSON,
	                                           "expires");
	if(expires) {
		...
		{
			ogs_error(
				"OpenAPI_sdm_subscription_parseFromJSON() failed [expires]");
			goto end;
		}
	}
	...
	if(!callback_reference) {
		ogs_error(
			"OpenAPI_sdm_subscription_parseFromJSON() failed [callback_reference]");
		goto end;
	}
	...
	if(amf_service_name) {
		...
		{
			ogs_error(
				"OpenAPI_sdm_subscription_parseFromJSON() failed [amf_service_name]");
			goto end;
		}
	}
	...
	if(!monitored_resource_uris) {
		ogs_error(
			"OpenAPI_sdm_subscription_parseFromJSON() failed [monitored_resource_uris]");
		goto end;
	}
	...
	single_nssai = cJSON_GetObjectItemCaseSensitive(sdm_subscriptionJSON,
	                                                "singleNssai");
	if(single_nssai) {
	...
	if(dnn) {
		...
		{
			ogs_error(
				"OpenAPI_sdm_subscription_parseFromJSON() failed [dnn]");
			goto end;
		}
	}

	subscription_id = cJSON_GetObjectItemCaseSensitive(sdm_subscriptionJSON,
	                                                   "subscriptionId");
	if(subscription_id) {
		...
		{
			ogs_error(
				"OpenAPI_sdm_subscription_parseFromJSON() failed [subscription_id]");
			goto end;
		}
	}

	plmn_id = cJSON_GetObjectItemCaseSensitive(sdm_subscriptionJSON,
	                                           "plmnId");
	if(plmn_id) {
	...
	report = cJSON_GetObjectItemCaseSensitive(sdm_subscriptionJSON,
	                                          "report");
	if(report) {
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_sdm_subscription_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}

	context_info = cJSON_GetObjectItemCaseSensitive(sdm_subscriptionJSON,
	                                                "contextInfo");
	if(context_info) {
	...
	if(nf_change_filter) {
		if(!cJSON_IsBool(nf_change_filter)) {
			ogs_error(
				"OpenAPI_sdm_subscription_parseFromJSON() failed [nf_change_filter]");
			goto end;
		}
	}
	...
	if(unique_subscription) {
		if(!cJSON_IsBool(unique_subscription)) {
			ogs_error(
				"OpenAPI_sdm_subscription_parseFromJSON() failed [unique_subscription]");
			goto end;
		}
	}

	reset_ids = cJSON_GetObjectItemCaseSensitive(sdm_subscriptionJSON,
	                                             "resetIds");
	if(reset_ids) {
		cJSON *reset_ids_local = NULL;
		if(!cJSON_IsArray(reset_ids)) {
			ogs_error(
				"OpenAPI_sdm_subscription_parseFromJSON() failed [reset_ids]");
			goto end;
		}
...
OpenAPI_sdm_subscription_t *OpenAPI_sdm_subscription_copy(
	OpenAPI_sdm_subscription_t *dst, OpenAPI_sdm_subscription_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
