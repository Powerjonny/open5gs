...
OpenAPI_relative_cartesian_location_create(float x, float y, bool is_z,
	...
	OpenAPI_relative_cartesian_location_t *
		...
		ogs_malloc(sizeof(OpenAPI_relative_cartesian_location_t));
...
cJSON *OpenAPI_relative_cartesian_location_convertToJSON(
	...
	if(relative_cartesian_location == NULL) {
		ogs_error(
			"OpenAPI_relative_cartesian_location_convertToJSON() failed [RelativeCartesianLocation]");
		return NULL;
	}
	...
	{
		ogs_error(
			"OpenAPI_relative_cartesian_location_convertToJSON() failed [x]");
		goto end;
	}
	...
	{
		ogs_error(
			"OpenAPI_relative_cartesian_location_convertToJSON() failed [y]");
		goto end;
	}
	...
	if(relative_cartesian_location->is_z) {
		...
		{
			ogs_error(
				"OpenAPI_relative_cartesian_location_convertToJSON() failed [z]");
			goto end;
		}
	}
...
OpenAPI_relative_cartesian_location_parseFromJSON(
	...
	OpenAPI_relative_cartesian_location_t *
	        relative_cartesian_location_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *x = NULL;
	cJSON *y = NULL;
	cJSON *z = NULL;
	...
	if(!x) {
		ogs_error(
			"OpenAPI_relative_cartesian_location_parseFromJSON() failed [x]");
		goto end;
	}
	if(!cJSON_IsNumber(x)) {
		ogs_error(
	...
	if(!y) {
		ogs_error(
			"OpenAPI_relative_cartesian_location_parseFromJSON() failed [y]");
		goto end;
	}
	if(!cJSON_IsNumber(y)) {
		ogs_error(
	...
	if(z) {
		if(!cJSON_IsNumber(z)) {
			ogs_error(
				"OpenAPI_relative_cartesian_location_parseFromJSON() failed [z]");
			goto end;
		}
	}

	relative_cartesian_location_local_var =
		OpenAPI_relative_cartesian_location_create(
...
OpenAPI_relative_cartesian_location_t *OpenAPI_relative_cartesian_location_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_relative_cartesian_location_convertToJSON() failed");
		return NULL;
	}
