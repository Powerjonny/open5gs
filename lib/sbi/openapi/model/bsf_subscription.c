...
OpenAPI_bsf_subscription_t *OpenAPI_bsf_subscription_create(
	...
	OpenAPI_list_t *add_snssai_dnn_pairs, char *supp_feat) {
	OpenAPI_bsf_subscription_t *bsf_subscription_local_var =
		ogs_malloc(sizeof(OpenAPI_bsf_subscription_t));
...
void OpenAPI_bsf_subscription_free(
	OpenAPI_bsf_subscription_t *bsf_subscription) {
	...
	if(bsf_subscription->snssai_dnn_pairs) {
		OpenAPI_snssai_dnn_pair_free(
			bsf_subscription->snssai_dnn_pairs);
		bsf_subscription->snssai_dnn_pairs = NULL;
	}
	if(bsf_subscription->add_snssai_dnn_pairs) {
		OpenAPI_list_for_each(bsf_subscription->add_snssai_dnn_pairs,
		                      node) {
...
cJSON *OpenAPI_bsf_subscription_convertToJSON(
	OpenAPI_bsf_subscription_t *bsf_subscription) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(bsf_subscription == NULL) {
		ogs_error(
			"OpenAPI_bsf_subscription_convertToJSON() failed [BsfSubscription]");
		return NULL;
	}
	...
	if(bsf_subscription->events == OpenAPI_bsf_event_NULL) {
		ogs_error(
			"OpenAPI_bsf_subscription_convertToJSON() failed [events]");
		return NULL;
	}
	...
	if(!bsf_subscription->notif_uri) {
		ogs_error(
			"OpenAPI_bsf_subscription_convertToJSON() failed [notif_uri]");
		return NULL;
	}
	...
	if(!bsf_subscription->notif_corre_id) {
		ogs_error(
			"OpenAPI_bsf_subscription_convertToJSON() failed [notif_corre_id]");
		return NULL;
	}
	...
	if(!bsf_subscription->supi) {
		ogs_error(
			"OpenAPI_bsf_subscription_convertToJSON() failed [supi]");
		return NULL;
	}
	...
	if(bsf_subscription->gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_bsf_subscription_convertToJSON() failed [gpsi]");
			goto end;
		}
	}
	...
	if(bsf_subscription->snssai_dnn_pairs) {
		...
		if(snssai_dnn_pairs_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_bsf_subscription_convertToJSON() failed [snssai_dnn_pairs]");
			goto end;
		}
	...
	if(bsf_subscription->add_snssai_dnn_pairs) {
		...
		if(add_snssai_dnn_pairsList == NULL) {
			ogs_error(
				"OpenAPI_bsf_subscription_convertToJSON() failed [add_snssai_dnn_pairs]");
			goto end;
		}
		OpenAPI_list_for_each(bsf_subscription->add_snssai_dnn_pairs,
		                      node) {
	...
	if(bsf_subscription->supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_bsf_subscription_convertToJSON() failed [supp_feat]");
			goto end;
		}
	}
...
OpenAPI_bsf_subscription_t *OpenAPI_bsf_subscription_parseFromJSON(
	cJSON *bsf_subscriptionJSON) {
	...
	events = cJSON_GetObjectItemCaseSensitive(bsf_subscriptionJSON,
	                                          "events");
	if(!events) {
		ogs_error(
			"OpenAPI_bsf_subscription_parseFromJSON() failed [events]");
		goto end;
	}
	...
	if(eventsList->count == 0) {
		ogs_error(
			"OpenAPI_bsf_subscription_parseFromJSON() failed: Expected eventsList to not be empty (after ignoring unsupported enum values).");
		goto end;
	}

	notif_uri = cJSON_GetObjectItemCaseSensitive(bsf_subscriptionJSON,
	                                             "notifUri");
	if(!notif_uri) {
		ogs_error(
			"OpenAPI_bsf_subscription_parseFromJSON() failed [notif_uri]");
		goto end;
	}
	...
	notif_corre_id = cJSON_GetObjectItemCaseSensitive(bsf_subscriptionJSON,
	                                                  "notifCorreId");
	if(!notif_corre_id) {
		ogs_error(
			"OpenAPI_bsf_subscription_parseFromJSON() failed [notif_corre_id]");
		goto end;
	}
	...
	if(!supi) {
		ogs_error(
			"OpenAPI_bsf_subscription_parseFromJSON() failed [supi]");
		goto end;
	}
	...
	if(gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_bsf_subscription_parseFromJSON() failed [gpsi]");
			goto end;
		}
	}

	snssai_dnn_pairs =
		cJSON_GetObjectItemCaseSensitive(bsf_subscriptionJSON,
		                                 "snssaiDnnPairs");
	if(snssai_dnn_pairs) {
	...
	if(add_snssai_dnn_pairs) {
		...
		if(!cJSON_IsArray(add_snssai_dnn_pairs)) {
			ogs_error(
				"OpenAPI_bsf_subscription_parseFromJSON() failed [add_snssai_dnn_pairs]");
			goto end;
		}
	...
	supp_feat = cJSON_GetObjectItemCaseSensitive(bsf_subscriptionJSON,
	                                             "suppFeat");
	if(supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_bsf_subscription_parseFromJSON() failed [supp_feat]");
			goto end;
		}
	}
...
OpenAPI_bsf_subscription_t *OpenAPI_bsf_subscription_copy(
	OpenAPI_bsf_subscription_t *dst, OpenAPI_bsf_subscription_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
