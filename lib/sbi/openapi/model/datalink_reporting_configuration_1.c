...
void OpenAPI_datalink_reporting_configuration_1_free(
	...
	if(datalink_reporting_configuration_1->ddd_traffic_des) {
		OpenAPI_list_for_each(
			datalink_reporting_configuration_1->ddd_traffic_des,
			node) {
	...
	if(datalink_reporting_configuration_1->ddd_status_list) {
		OpenAPI_list_free(
			datalink_reporting_configuration_1->ddd_status_list);
...
cJSON *OpenAPI_datalink_reporting_configuration_1_convertToJSON(
	...
	if(datalink_reporting_configuration_1 == NULL) {
		ogs_error(
			"OpenAPI_datalink_reporting_configuration_1_convertToJSON() failed [DatalinkReportingConfiguration_1]");
		return NULL;
	}
	...
	if(datalink_reporting_configuration_1->ddd_traffic_des) {
		...
		if(ddd_traffic_desList == NULL) {
			ogs_error(
				"OpenAPI_datalink_reporting_configuration_1_convertToJSON() failed [ddd_traffic_des]");
			goto end;
		}
		OpenAPI_list_for_each(
			datalink_reporting_configuration_1->ddd_traffic_des,
			node) {
			cJSON *itemLocal =
				OpenAPI_ddd_traffic_descriptor_1_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_datalink_reporting_configuration_1_convertToJSON() failed [ddd_traffic_des]");
	...
	if(datalink_reporting_configuration_1->dnn) {
		...
		{
			ogs_error(
				"OpenAPI_datalink_reporting_configuration_1_convertToJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(datalink_reporting_configuration_1->slice) {
		cJSON *slice_local_JSON =
			OpenAPI_snssai_convertToJSON(
				datalink_reporting_configuration_1->slice);
		if(slice_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_datalink_reporting_configuration_1_convertToJSON() failed [slice]");
			goto end;
		}
	   ...
	   OpenAPI_dl_data_delivery_status_NULL)
	{
		...
		if(ddd_status_listList == NULL) {
			ogs_error(
				"OpenAPI_datalink_reporting_configuration_1_convertToJSON() failed [ddd_status_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			datalink_reporting_configuration_1->ddd_status_list,
...
OpenAPI_datalink_reporting_configuration_1_parseFromJSON(
	...
	OpenAPI_datalink_reporting_configuration_1_t *
	        datalink_reporting_configuration_1_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	ddd_traffic_des =
		cJSON_GetObjectItemCaseSensitive(
			datalink_reporting_configuration_1JSON,
			"dddTrafficDes");
	if(ddd_traffic_des) {
		cJSON *ddd_traffic_des_local = NULL;
		if(!cJSON_IsArray(ddd_traffic_des)) {
			ogs_error(
				"OpenAPI_datalink_reporting_configuration_1_parseFromJSON() failed [ddd_traffic_des]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ddd_traffic_des_local, ddd_traffic_des) {
			...
			OpenAPI_ddd_traffic_descriptor_1_t *ddd_traffic_desItem
			        = OpenAPI_ddd_traffic_descriptor_1_parseFromJSON
	...
	dnn =
		cJSON_GetObjectItemCaseSensitive(
			datalink_reporting_configuration_1JSON, "dnn");
	if(dnn) {
		if(!cJSON_IsString(dnn) &&
		   !cJSON_IsNull(dnn))
		{
			ogs_error(
				"OpenAPI_datalink_reporting_configuration_1_parseFromJSON() failed [dnn]");
			goto end;
		}
	}
	...
	slice =
		cJSON_GetObjectItemCaseSensitive(
			datalink_reporting_configuration_1JSON, "slice");
	if(slice) {
	...
	if(ddd_status_list) {
		...
		if(!cJSON_IsArray(ddd_status_list)) {
			ogs_error(
				"OpenAPI_datalink_reporting_configuration_1_parseFromJSON() failed [ddd_status_list]");
			goto end;
		}
		...
		if(ddd_status_listList->count == 0) {
			ogs_error(
				"OpenAPI_datalink_reporting_configuration_1_parseFromJSON() failed: Expected ddd_status_listList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
...
OpenAPI_datalink_reporting_configuration_1_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_datalink_reporting_configuration_1_convertToJSON() failed");
		return NULL;
	}
