...
OpenAPI_pro_se_eap_session_t *OpenAPI_pro_se_eap_session_create(
	...
	char *_5g_pruk_id) {
	OpenAPI_pro_se_eap_session_t *pro_se_eap_session_local_var =
		ogs_malloc(sizeof(OpenAPI_pro_se_eap_session_t));
...
cJSON *OpenAPI_pro_se_eap_session_convertToJSON(
	...
	if(pro_se_eap_session == NULL) {
		ogs_error(
			"OpenAPI_pro_se_eap_session_convertToJSON() failed [ProSeEapSession]");
		return NULL;
	}
	...
	if(!pro_se_eap_session->eap_payload) {
		ogs_error(
			"OpenAPI_pro_se_eap_session_convertToJSON() failed [eap_payload]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "eapPayload",
	                           pro_se_eap_session->eap_payload) == NULL)
	{
		ogs_error(
			"OpenAPI_pro_se_eap_session_convertToJSON() failed [eap_payload]");
	...
	if(pro_se_eap_session->knr_pro_se) {
		if(cJSON_AddStringToObject(item, "knrProSe",
		                           pro_se_eap_session->knr_pro_se) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pro_se_eap_session_convertToJSON() failed [knr_pro_se]");
			goto end;
		}
	}
	...
	if(pro_se_eap_session->_links) {
		...
		if(_links == NULL) {
			ogs_error(
				"OpenAPI_pro_se_eap_session_convertToJSON() failed [_links]");
			goto end;
		}
		cJSON *localMapObject = _links;
		if(pro_se_eap_session->_links) {
			OpenAPI_list_for_each(pro_se_eap_session->_links,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_links_value_schema_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_pro_se_eap_session_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(pro_se_eap_session->auth_result != OpenAPI_auth_result_NULL) {
		if(cJSON_AddStringToObject(item, "authResult",
		                           OpenAPI_auth_result_ToString(
						   pro_se_eap_session->
						   auth_result)) == NULL)
		{
			ogs_error(
				"OpenAPI_pro_se_eap_session_convertToJSON() failed [auth_result]");
			goto end;
		}
	}
	...
	if(pro_se_eap_session->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_pro_se_eap_session_convertToJSON() failed [supported_features]");
			goto end;
		}
	}

	if(pro_se_eap_session->nonce2) {
		if(cJSON_AddStringToObject(item, "nonce2",
		                           pro_se_eap_session->nonce2) == NULL)
		{
			ogs_error(
				"OpenAPI_pro_se_eap_session_convertToJSON() failed [nonce2]");
			goto end;
		}
	...
	if(pro_se_eap_session->_5g_pruk_id) {
		if(cJSON_AddStringToObject(item, "5gPrukId",
		                           pro_se_eap_session->_5g_pruk_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pro_se_eap_session_convertToJSON() failed [_5g_pruk_id]");
			goto end;
		}
	}
...
OpenAPI_pro_se_eap_session_t *OpenAPI_pro_se_eap_session_parseFromJSON(
	...
	eap_payload = cJSON_GetObjectItemCaseSensitive(pro_se_eap_sessionJSON,
	                                               "eapPayload");
	if(!eap_payload) {
		ogs_error(
			"OpenAPI_pro_se_eap_session_parseFromJSON() failed [eap_payload]");
		goto end;
	}
	...
	knr_pro_se = cJSON_GetObjectItemCaseSensitive(pro_se_eap_sessionJSON,
	                                              "knrProSe");
	if(knr_pro_se) {
		if(!cJSON_IsString(knr_pro_se) &&
		   !cJSON_IsNull(knr_pro_se))
		{
			ogs_error(
				"OpenAPI_pro_se_eap_session_parseFromJSON() failed [knr_pro_se]");
			goto end;
		}
	}

	_links = cJSON_GetObjectItemCaseSensitive(pro_se_eap_sessionJSON,
	                                          "_links");
	if(_links) {
		...
		{
			ogs_error(
				"OpenAPI_pro_se_eap_session_parseFromJSON() failed [_links]");
			goto end;
		}
		if(cJSON_IsObject(_links)) {
			...
			cJSON_ArrayForEach(_links_local_map, _links) {
				...
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_pro_se_eap_session_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	auth_result = cJSON_GetObjectItemCaseSensitive(pro_se_eap_sessionJSON,
	                                               "authResult");
	if(auth_result) {
		if(!cJSON_IsString(auth_result)) {
			ogs_error(
				"OpenAPI_pro_se_eap_session_parseFromJSON() failed [auth_result]");
			goto end;
		}
		auth_resultVariable =
			OpenAPI_auth_result_FromString(
				auth_result->valuestring);
	}

	supported_features =
		cJSON_GetObjectItemCaseSensitive(pro_se_eap_sessionJSON,
		                                 "supportedFeatures");
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_pro_se_eap_session_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}

	nonce2 = cJSON_GetObjectItemCaseSensitive(pro_se_eap_sessionJSON,
	                                          "nonce2");
	if(nonce2) {
		if(!cJSON_IsNull(nonce2)) {
			if(!cJSON_IsString(nonce2) &&
			   !cJSON_IsNull(nonce2))
			{
				ogs_error(
					"OpenAPI_pro_se_eap_session_parseFromJSON() failed [nonce2]");
				goto end;
			}
		}
	}

	_5g_pruk_id = cJSON_GetObjectItemCaseSensitive(pro_se_eap_sessionJSON,
	                                               "5gPrukId");
	if(_5g_pruk_id) {
		if(!cJSON_IsString(_5g_pruk_id) &&
		   !cJSON_IsNull(_5g_pruk_id))
		{
			ogs_error(
				"OpenAPI_pro_se_eap_session_parseFromJSON() failed [_5g_pruk_id]");
			goto end;
		}
	}
	...
	pro_se_eap_session_local_var = OpenAPI_pro_se_eap_session_create(
		eap_payload &&
		cJSON_IsNull(eap_payload) ? true : false,
		ogs_strdup(eap_payload->valuestring),
		knr_pro_se &&
		!cJSON_IsNull(knr_pro_se) ?
		ogs_strdup(knr_pro_se->valuestring) : NULL,
		...
		nonce2 &&
		cJSON_IsNull(nonce2) ? true : false,
		nonce2 &&
		!cJSON_IsNull(nonce2) ? ogs_strdup(nonce2->valuestring) : NULL,
		_5g_pruk_id &&
		!cJSON_IsNull(_5g_pruk_id) ?
		ogs_strdup(_5g_pruk_id->valuestring) : NULL
		);
