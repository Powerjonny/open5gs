...
OpenAPI_roaming_info_update_t *OpenAPI_roaming_info_update_create(
	bool is_roaming, int roaming, OpenAPI_plmn_id_t *serving_plmn) {
	OpenAPI_roaming_info_update_t *roaming_info_update_local_var =
		ogs_malloc(sizeof(OpenAPI_roaming_info_update_t));
...
void OpenAPI_roaming_info_update_free(
	OpenAPI_roaming_info_update_t *roaming_info_update) {
...
cJSON *OpenAPI_roaming_info_update_convertToJSON(
	OpenAPI_roaming_info_update_t *roaming_info_update) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(roaming_info_update == NULL) {
		ogs_error(
			"OpenAPI_roaming_info_update_convertToJSON() failed [RoamingInfoUpdate]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(roaming_info_update->is_roaming) {
		if(cJSON_AddBoolToObject(item, "roaming",
		                         roaming_info_update->roaming) == NULL)
		{
			ogs_error(
				"OpenAPI_roaming_info_update_convertToJSON() failed [roaming]");
			goto end;
		}
	}
	...
	if(!roaming_info_update->serving_plmn) {
		ogs_error(
			"OpenAPI_roaming_info_update_convertToJSON() failed [serving_plmn]");
		return NULL;
	}
	cJSON *serving_plmn_local_JSON =
		OpenAPI_plmn_id_convertToJSON(
			roaming_info_update->serving_plmn);
...
OpenAPI_roaming_info_update_t *OpenAPI_roaming_info_update_parseFromJSON(
	cJSON *roaming_info_updateJSON) {
	...
	roaming = cJSON_GetObjectItemCaseSensitive(roaming_info_updateJSON,
	                                           "roaming");
	if(roaming) {
		if(!cJSON_IsBool(roaming)) {
			ogs_error(
				"OpenAPI_roaming_info_update_parseFromJSON() failed [roaming]");
			goto end;
		}
	}

	serving_plmn = cJSON_GetObjectItemCaseSensitive(roaming_info_updateJSON,
	                                                "servingPlmn");
	if(!serving_plmn) {
		ogs_error(
			"OpenAPI_roaming_info_update_parseFromJSON() failed [serving_plmn]");
		goto end;
	}
	serving_plmn_local_nonprim =
		OpenAPI_plmn_id_parseFromJSON(serving_plmn);
	if(!serving_plmn_local_nonprim) {
		ogs_error(
			"OpenAPI_plmn_id_parseFromJSON failed [serving_plmn]");
		goto end;
	}
...
OpenAPI_roaming_info_update_t *OpenAPI_roaming_info_update_copy(
	OpenAPI_roaming_info_update_t	*dst,
	OpenAPI_roaming_info_update_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
