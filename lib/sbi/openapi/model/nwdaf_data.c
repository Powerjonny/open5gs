...
OpenAPI_nwdaf_data_t *OpenAPI_nwdaf_data_create(
	...
	OpenAPI_nwdaf_data_t *nwdaf_data_local_var =
		ogs_malloc(sizeof(OpenAPI_nwdaf_data_t));
...
cJSON *OpenAPI_nwdaf_data_convertToJSON(OpenAPI_nwdaf_data_t *nwdaf_data) {
	...
	if(nwdaf_data == NULL) {
		ogs_error(
			"OpenAPI_nwdaf_data_convertToJSON() failed [NwdafData]");
		return NULL;
	}
	...
	if(!nwdaf_data->nwdaf_instance_id) {
		ogs_error(
			"OpenAPI_nwdaf_data_convertToJSON() failed [nwdaf_instance_id]");
		return NULL;
	}
	...
	if(nwdaf_data->nwdaf_events) {
		...
		if(nwdaf_eventsList == NULL) {
			ogs_error(
				"OpenAPI_nwdaf_data_convertToJSON() failed [nwdaf_events]");
			goto end;
		}
		OpenAPI_list_for_each(nwdaf_data->nwdaf_events, node) {
			cJSON *itemLocal =
...
OpenAPI_nwdaf_data_t *OpenAPI_nwdaf_data_parseFromJSON(cJSON *nwdaf_dataJSON) {
	...
	nwdaf_instance_id = cJSON_GetObjectItemCaseSensitive(nwdaf_dataJSON,
	                                                     "nwdafInstanceId");
	if(!nwdaf_instance_id) {
		ogs_error(
			"OpenAPI_nwdaf_data_parseFromJSON() failed [nwdaf_instance_id]");
		goto end;
	}
	...
	nwdaf_events = cJSON_GetObjectItemCaseSensitive(nwdaf_dataJSON,
	                                                "nwdafEvents");
	if(nwdaf_events) {
