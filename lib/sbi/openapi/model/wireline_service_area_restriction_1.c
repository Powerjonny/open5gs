...
void OpenAPI_wireline_service_area_restriction_1_free(
	OpenAPI_wireline_service_area_restriction_1_t *
	...
	if(wireline_service_area_restriction_1->areas) {
		OpenAPI_list_for_each(
			wireline_service_area_restriction_1->areas, node) {
...
cJSON *OpenAPI_wireline_service_area_restriction_1_convertToJSON(
	OpenAPI_wireline_service_area_restriction_1_t *
	wireline_service_area_restriction_1) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(wireline_service_area_restriction_1 == NULL) {
		ogs_error(
			"OpenAPI_wireline_service_area_restriction_1_convertToJSON() failed [WirelineServiceAreaRestriction_1]");
		return NULL;
	}
	   ...
	   OpenAPI_restriction_type_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_wireline_service_area_restriction_1_convertToJSON() failed [restriction_type]");
			goto end;
		}
	}
	...
	if(wireline_service_area_restriction_1->areas) {
		...
		if(areasList == NULL) {
			ogs_error(
				"OpenAPI_wireline_service_area_restriction_1_convertToJSON() failed [areas]");
			goto end;
		}
		OpenAPI_list_for_each(
			wireline_service_area_restriction_1->areas, node) {
...
OpenAPI_wireline_service_area_restriction_1_parseFromJSON(
	cJSON *wireline_service_area_restriction_1JSON) {
	OpenAPI_wireline_service_area_restriction_1_t *
	        wireline_service_area_restriction_1_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(restriction_type) {
		if(!cJSON_IsString(restriction_type)) {
			ogs_error(
				"OpenAPI_wireline_service_area_restriction_1_parseFromJSON() failed [restriction_type]");
			goto end;
		}
	...
	areas =
		cJSON_GetObjectItemCaseSensitive(
			wireline_service_area_restriction_1JSON, "areas");
	if(areas) {
		cJSON *areas_local = NULL;
		if(!cJSON_IsArray(areas)) {
			ogs_error(
				"OpenAPI_wireline_service_area_restriction_1_parseFromJSON() failed [areas]");
			goto end;
		}
		...
		cJSON_ArrayForEach(areas_local, areas) {
			...
			OpenAPI_wireline_area_1_t *areasItem =
...
OpenAPI_wireline_service_area_restriction_1_t *
OpenAPI_wireline_service_area_restriction_1_copy(
	OpenAPI_wireline_service_area_restriction_1_t	*dst,
	OpenAPI_wireline_service_area_restriction_1_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_wireline_service_area_restriction_1_convertToJSON() failed");
		return NULL;
	}
