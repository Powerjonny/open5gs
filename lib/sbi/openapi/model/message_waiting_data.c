...
OpenAPI_message_waiting_data_t *OpenAPI_message_waiting_data_create(
	...
	OpenAPI_message_waiting_data_t *message_waiting_data_local_var =
		ogs_malloc(sizeof(OpenAPI_message_waiting_data_t));
...
void OpenAPI_message_waiting_data_free(
	OpenAPI_message_waiting_data_t *message_waiting_data) {
...
cJSON *OpenAPI_message_waiting_data_convertToJSON(
	OpenAPI_message_waiting_data_t *message_waiting_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(message_waiting_data == NULL) {
		ogs_error(
			"OpenAPI_message_waiting_data_convertToJSON() failed [MessageWaitingData]");
		return NULL;
	}
	...
	if(message_waiting_data->mwd_list) {
		...
		if(mwd_listList == NULL) {
			ogs_error(
				"OpenAPI_message_waiting_data_convertToJSON() failed [mwd_list]");
			goto end;
		}
		OpenAPI_list_for_each(message_waiting_data->mwd_list, node) {
			cJSON *itemLocal =
				OpenAPI_smsc_data_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_message_waiting_data_convertToJSON() failed [mwd_list]");
				goto end;
...
OpenAPI_message_waiting_data_t *OpenAPI_message_waiting_data_parseFromJSON(
	cJSON *message_waiting_dataJSON) {
	OpenAPI_message_waiting_data_t *message_waiting_data_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *mwd_list = NULL;
	OpenAPI_list_t *mwd_listList = NULL;
	mwd_list = cJSON_GetObjectItemCaseSensitive(message_waiting_dataJSON,
	                                            "mwdList");
	if(mwd_list) {
		cJSON *mwd_list_local = NULL;
		if(!cJSON_IsArray(mwd_list)) {
			ogs_error(
				"OpenAPI_message_waiting_data_parseFromJSON() failed [mwd_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(mwd_list_local, mwd_list) {
			...
			OpenAPI_smsc_data_t *mwd_listItem =
				OpenAPI_smsc_data_parseFromJSON(mwd_list_local);
			if(!mwd_listItem) {
...
OpenAPI_message_waiting_data_t *OpenAPI_message_waiting_data_copy(
	OpenAPI_message_waiting_data_t	*dst,
	OpenAPI_message_waiting_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_message_waiting_data_convertToJSON() failed");
		return NULL;
	}
