...
OpenAPI_retrieved_data_t *OpenAPI_retrieved_data_create(
	...
	OpenAPI_retrieved_data_t *retrieved_data_local_var =
		ogs_malloc(sizeof(OpenAPI_retrieved_data_t));
	ogs_assert(retrieved_data_local_var);

	retrieved_data_local_var->small_data_rate_status =
...
void OpenAPI_retrieved_data_free(OpenAPI_retrieved_data_t *retrieved_data) {
	...
	if(retrieved_data->af_coordination_info) {
		OpenAPI_af_coordination_info_free(
			retrieved_data->af_coordination_info);
...
cJSON *OpenAPI_retrieved_data_convertToJSON(
	...
	if(retrieved_data == NULL) {
		ogs_error(
			"OpenAPI_retrieved_data_convertToJSON() failed [RetrievedData]");
		return NULL;
	}
	...
	if(retrieved_data->small_data_rate_status) {
		...
		if(small_data_rate_status_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_retrieved_data_convertToJSON() failed [small_data_rate_status]");
			goto end;
		}
	...
	if(retrieved_data->af_coordination_info) {
		...
		if(af_coordination_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_retrieved_data_convertToJSON() failed [af_coordination_info]");
			goto end;
		}
...
OpenAPI_retrieved_data_t *OpenAPI_retrieved_data_parseFromJSON(
	...
	OpenAPI_af_coordination_info_t *af_coordination_info_local_nonprim =
		NULL;
	small_data_rate_status =
		cJSON_GetObjectItemCaseSensitive(retrieved_dataJSON,
		                                 "smallDataRateStatus");
