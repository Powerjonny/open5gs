...
OpenAPI_geo_service_area_t *OpenAPI_geo_service_area_create(
	...
	OpenAPI_geo_service_area_t *geo_service_area_local_var =
		ogs_malloc(sizeof(OpenAPI_geo_service_area_t));
...
void OpenAPI_geo_service_area_free(
	...
	if(geo_service_area->geographic_area_list) {
		OpenAPI_list_for_each(geo_service_area->geographic_area_list,
		                      node) {
	...
	if(geo_service_area->civic_address_list) {
		OpenAPI_list_for_each(geo_service_area->civic_address_list,
		                      node) {
...
cJSON *OpenAPI_geo_service_area_convertToJSON(
	...
	if(geo_service_area == NULL) {
		ogs_error(
			"OpenAPI_geo_service_area_convertToJSON() failed [GeoServiceArea]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(geo_service_area->geographic_area_list) {
		cJSON *geographic_area_listList = cJSON_AddArrayToObject(item,
		                                                         "geographicAreaList");
		if(geographic_area_listList == NULL) {
			ogs_error(
				"OpenAPI_geo_service_area_convertToJSON() failed [geographic_area_list]");
			goto end;
		}
		OpenAPI_list_for_each(geo_service_area->geographic_area_list,
		                      node) {
			...
			cJSON_AddItemToArray(geographic_area_listList,
			                     itemLocal);
		}
	}

	if(geo_service_area->civic_address_list) {
		cJSON *civic_address_listList = cJSON_AddArrayToObject(item,
		                                                       "civicAddressList");
		if(civic_address_listList == NULL) {
			ogs_error(
				"OpenAPI_geo_service_area_convertToJSON() failed [civic_address_list]");
			goto end;
		}
		OpenAPI_list_for_each(geo_service_area->civic_address_list,
		                      node) {
...
OpenAPI_geo_service_area_t *OpenAPI_geo_service_area_parseFromJSON(
	...
	geographic_area_list =
		cJSON_GetObjectItemCaseSensitive(geo_service_areaJSON,
		                                 "geographicAreaList");
	if(geographic_area_list) {
		cJSON *geographic_area_list_local = NULL;
		if(!cJSON_IsArray(geographic_area_list)) {
			ogs_error(
				"OpenAPI_geo_service_area_parseFromJSON() failed [geographic_area_list]");
			goto end;
		}

		geographic_area_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(geographic_area_list_local,
		                   geographic_area_list) {
			...
			OpenAPI_geographic_area_t *geographic_area_listItem =
				OpenAPI_geographic_area_parseFromJSON(
					geographic_area_list_local);
			if(!geographic_area_listItem) {
				ogs_error("No geographic_area_listItem");
				goto end;
			}
			OpenAPI_list_add(geographic_area_listList,
			                 geographic_area_listItem);
		}
	}
	...
	if(civic_address_list) {
		...
		if(!cJSON_IsArray(civic_address_list)) {
			ogs_error(
				"OpenAPI_geo_service_area_parseFromJSON() failed [civic_address_list]");
			goto end;
		}

		civic_address_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(civic_address_list_local,
		                   civic_address_list) {
			...
			OpenAPI_civic_address_t *civic_address_listItem =
				OpenAPI_civic_address_parseFromJSON(
					civic_address_list_local);
			if(!civic_address_listItem) {
				ogs_error("No civic_address_listItem");
				goto end;
			}
			OpenAPI_list_add(civic_address_listList,
			                 civic_address_listItem);
		}
	}
...
OpenAPI_geo_service_area_t *OpenAPI_geo_service_area_copy(
	OpenAPI_geo_service_area_t *dst, OpenAPI_geo_service_area_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
