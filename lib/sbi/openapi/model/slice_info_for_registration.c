...
OpenAPI_slice_info_for_registration_create(OpenAPI_list_t *subscribed_nssai,
                                           ...
                                           bool
                                           is_default_configured_snssai_ind,
                                           int
                                           ...
                                           int nsag_supported)
{
	OpenAPI_slice_info_for_registration_t *
		...
		ogs_malloc(sizeof(OpenAPI_slice_info_for_registration_t));
	ogs_assert(slice_info_for_registration_local_var);

	slice_info_for_registration_local_var->subscribed_nssai =
		subscribed_nssai;
	slice_info_for_registration_local_var->allowed_nssai_current_access =
		allowed_nssai_current_access;
	slice_info_for_registration_local_var->allowed_nssai_other_access =
		allowed_nssai_other_access;
	slice_info_for_registration_local_var->s_nssai_for_mapping =
		s_nssai_for_mapping;
	slice_info_for_registration_local_var->requested_nssai =
		requested_nssai;
	slice_info_for_registration_local_var->is_default_configured_snssai_ind
	        = is_default_configured_snssai_ind;
	slice_info_for_registration_local_var->default_configured_snssai_ind =
		default_configured_snssai_ind;
	slice_info_for_registration_local_var->mapping_of_nssai =
		mapping_of_nssai;
	slice_info_for_registration_local_var->is_request_mapping =
		is_request_mapping;
	slice_info_for_registration_local_var->request_mapping =
		request_mapping;
	slice_info_for_registration_local_var->is_ue_sup_nssrg_ind =
		is_ue_sup_nssrg_ind;
	slice_info_for_registration_local_var->ue_sup_nssrg_ind =
		ue_sup_nssrg_ind;
	slice_info_for_registration_local_var->is_suppress_nssrg_ind =
		is_suppress_nssrg_ind;
	slice_info_for_registration_local_var->suppress_nssrg_ind =
		suppress_nssrg_ind;
	slice_info_for_registration_local_var->is_nsag_supported =
		is_nsag_supported;
...
void OpenAPI_slice_info_for_registration_free(
	OpenAPI_slice_info_for_registration_t *slice_info_for_registration) {
	...
	if(slice_info_for_registration->subscribed_nssai) {
		OpenAPI_list_for_each(
			slice_info_for_registration->subscribed_nssai, node) {
	...
	if(slice_info_for_registration->requested_nssai) {
		OpenAPI_list_for_each(
			slice_info_for_registration->requested_nssai, node) {
			OpenAPI_snssai_free(node->data);
		}
	...
	if(slice_info_for_registration->mapping_of_nssai) {
		OpenAPI_list_for_each(
			slice_info_for_registration->mapping_of_nssai, node) {
...
cJSON *OpenAPI_slice_info_for_registration_convertToJSON(
	...
	if(slice_info_for_registration == NULL) {
		ogs_error(
			"OpenAPI_slice_info_for_registration_convertToJSON() failed [SliceInfoForRegistration]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(slice_info_for_registration->subscribed_nssai) {
		cJSON *subscribed_nssaiList = cJSON_AddArrayToObject(item,
		                                                     "subscribedNssai");
		if(subscribed_nssaiList == NULL) {
			ogs_error(
				"OpenAPI_slice_info_for_registration_convertToJSON() failed [subscribed_nssai]");
			goto end;
		}
		OpenAPI_list_for_each(
			slice_info_for_registration->subscribed_nssai, node) {
			cJSON *itemLocal =
				OpenAPI_subscribed_snssai_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_slice_info_for_registration_convertToJSON() failed [subscribed_nssai]");
	...
	if(slice_info_for_registration->allowed_nssai_current_access) {
		cJSON *allowed_nssai_current_access_local_JSON =
			OpenAPI_allowed_nssai_convertToJSON(
				slice_info_for_registration->
				allowed_nssai_current_access);
		if(allowed_nssai_current_access_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_slice_info_for_registration_convertToJSON() failed [allowed_nssai_current_access]");
			goto end;
		}
		cJSON_AddItemToObject(item, "allowedNssaiCurrentAccess",
		                      allowed_nssai_current_access_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_slice_info_for_registration_convertToJSON() failed [allowed_nssai_current_access]");
			goto end;
	...
	if(slice_info_for_registration->allowed_nssai_other_access) {
		...
		if(allowed_nssai_other_access_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_slice_info_for_registration_convertToJSON() failed [allowed_nssai_other_access]");
			goto end;
		}
		cJSON_AddItemToObject(item, "allowedNssaiOtherAccess",
		                      allowed_nssai_other_access_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_slice_info_for_registration_convertToJSON() failed [allowed_nssai_other_access]");
			goto end;
	...
	if(slice_info_for_registration->s_nssai_for_mapping) {
		cJSON *s_nssai_for_mappingList = cJSON_AddArrayToObject(item,
		                                                        "sNssaiForMapping");
		if(s_nssai_for_mappingList == NULL) {
			ogs_error(
				"OpenAPI_slice_info_for_registration_convertToJSON() failed [s_nssai_for_mapping]");
			goto end;
		}
		OpenAPI_list_for_each(
			slice_info_for_registration->s_nssai_for_mapping,
			...
			cJSON_AddItemToArray(s_nssai_for_mappingList,
			                     itemLocal);
		}
	}

	if(slice_info_for_registration->requested_nssai) {
		cJSON *requested_nssaiList = cJSON_AddArrayToObject(item,
		                                                    "requestedNssai");
		if(requested_nssaiList == NULL) {
			ogs_error(
				"OpenAPI_slice_info_for_registration_convertToJSON() failed [requested_nssai]");
			goto end;
		}
		OpenAPI_list_for_each(
			slice_info_for_registration->requested_nssai, node) {
	...
	if(slice_info_for_registration->is_default_configured_snssai_ind) {
		if(cJSON_AddBoolToObject(item, "defaultConfiguredSnssaiInd",
		                         slice_info_for_registration->
		                         default_configured_snssai_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_slice_info_for_registration_convertToJSON() failed [default_configured_snssai_ind]");
			goto end;
		}
	}

	if(slice_info_for_registration->mapping_of_nssai) {
		cJSON *mapping_of_nssaiList = cJSON_AddArrayToObject(item,
		                                                     "mappingOfNssai");
		if(mapping_of_nssaiList == NULL) {
			ogs_error(
				"OpenAPI_slice_info_for_registration_convertToJSON() failed [mapping_of_nssai]");
			goto end;
		}
		OpenAPI_list_for_each(
			slice_info_for_registration->mapping_of_nssai, node) {
	...
	if(slice_info_for_registration->is_request_mapping) {
		if(cJSON_AddBoolToObject(item, "requestMapping",
		                         slice_info_for_registration->
		                         request_mapping) == NULL)
		{
			ogs_error(
				"OpenAPI_slice_info_for_registration_convertToJSON() failed [request_mapping]");
			goto end;
		}
	}

	if(slice_info_for_registration->is_ue_sup_nssrg_ind) {
		if(cJSON_AddBoolToObject(item, "ueSupNssrgInd",
		                         slice_info_for_registration->
		                         ue_sup_nssrg_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_slice_info_for_registration_convertToJSON() failed [ue_sup_nssrg_ind]");
			goto end;
		}
	}

	if(slice_info_for_registration->is_suppress_nssrg_ind) {
		if(cJSON_AddBoolToObject(item, "suppressNssrgInd",
		                         slice_info_for_registration->
		                         suppress_nssrg_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_slice_info_for_registration_convertToJSON() failed [suppress_nssrg_ind]");
			goto end;
		}
	}

	if(slice_info_for_registration->is_nsag_supported) {
		if(cJSON_AddBoolToObject(item, "nsagSupported",
		                         slice_info_for_registration->
		                         nsag_supported) == NULL)
		{
			ogs_error(
				"OpenAPI_slice_info_for_registration_convertToJSON() failed [nsag_supported]");
			goto end;
		}
	}
...
OpenAPI_slice_info_for_registration_parseFromJSON(
	...
	OpenAPI_slice_info_for_registration_t *
	        slice_info_for_registration_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *subscribed_nssai = NULL;
	OpenAPI_list_t *subscribed_nssaiList = NULL;
	cJSON *allowed_nssai_current_access = NULL;
	OpenAPI_allowed_nssai_t *allowed_nssai_current_access_local_nonprim =
		NULL;
	cJSON *allowed_nssai_other_access = NULL;
	OpenAPI_allowed_nssai_t *allowed_nssai_other_access_local_nonprim =
		NULL;
	...
	subscribed_nssai =
		cJSON_GetObjectItemCaseSensitive(
			slice_info_for_registrationJSON, "subscribedNssai");
	if(subscribed_nssai) {
		cJSON *subscribed_nssai_local = NULL;
		if(!cJSON_IsArray(subscribed_nssai)) {
			ogs_error(
				"OpenAPI_slice_info_for_registration_parseFromJSON() failed [subscribed_nssai]");
			goto end;
		}
		...
		cJSON_ArrayForEach(subscribed_nssai_local, subscribed_nssai) {
			...
			OpenAPI_subscribed_snssai_t *subscribed_nssaiItem =
				OpenAPI_subscribed_snssai_parseFromJSON(
					subscribed_nssai_local);
	...
	allowed_nssai_current_access =
		cJSON_GetObjectItemCaseSensitive(
			slice_info_for_registrationJSON,
	...
	if(allowed_nssai_current_access) {
		...
		if(!allowed_nssai_current_access_local_nonprim) {
			ogs_error(
				"OpenAPI_allowed_nssai_parseFromJSON failed [allowed_nssai_current_access]");
			goto end;
		}
	}
	...
	if(allowed_nssai_other_access) {
		...
		if(!allowed_nssai_other_access_local_nonprim) {
			ogs_error(
				"OpenAPI_allowed_nssai_parseFromJSON failed [allowed_nssai_other_access]");
			goto end;
		}
	}
	...
	s_nssai_for_mapping =
		cJSON_GetObjectItemCaseSensitive(
			slice_info_for_registrationJSON, "sNssaiForMapping");
	if(s_nssai_for_mapping) {
		cJSON *s_nssai_for_mapping_local = NULL;
		if(!cJSON_IsArray(s_nssai_for_mapping)) {
			ogs_error(
				"OpenAPI_slice_info_for_registration_parseFromJSON() failed [s_nssai_for_mapping]");
			goto end;
		}

		s_nssai_for_mappingList = OpenAPI_list_create();

		cJSON_ArrayForEach(s_nssai_for_mapping_local,
		                   s_nssai_for_mapping) {
			...
			OpenAPI_snssai_t *s_nssai_for_mappingItem =
				OpenAPI_snssai_parseFromJSON(
					s_nssai_for_mapping_local);
			if(!s_nssai_for_mappingItem) {
				ogs_error("No s_nssai_for_mappingItem");
				goto end;
			}
			OpenAPI_list_add(s_nssai_for_mappingList,
			                 s_nssai_for_mappingItem);
		}
	}

	requested_nssai =
		cJSON_GetObjectItemCaseSensitive(
			slice_info_for_registrationJSON, "requestedNssai");
	if(requested_nssai) {
		cJSON *requested_nssai_local = NULL;
		if(!cJSON_IsArray(requested_nssai)) {
			ogs_error(
				"OpenAPI_slice_info_for_registration_parseFromJSON() failed [requested_nssai]");
			goto end;
		}
		...
		cJSON_ArrayForEach(requested_nssai_local, requested_nssai) {
			...
			OpenAPI_snssai_t *requested_nssaiItem =
				OpenAPI_snssai_parseFromJSON(
					requested_nssai_local);
			if(!requested_nssaiItem) {
				ogs_error("No requested_nssaiItem");
				goto end;
			}
			OpenAPI_list_add(requested_nssaiList,
			                 requested_nssaiItem);
		}
	}
	...
	if(default_configured_snssai_ind) {
		if(!cJSON_IsBool(default_configured_snssai_ind)) {
			ogs_error(
				"OpenAPI_slice_info_for_registration_parseFromJSON() failed [default_configured_snssai_ind]");
			goto end;
		}
	}
	...
	mapping_of_nssai =
		cJSON_GetObjectItemCaseSensitive(
			slice_info_for_registrationJSON, "mappingOfNssai");
	if(mapping_of_nssai) {
		cJSON *mapping_of_nssai_local = NULL;
		if(!cJSON_IsArray(mapping_of_nssai)) {
			ogs_error(
				"OpenAPI_slice_info_for_registration_parseFromJSON() failed [mapping_of_nssai]");
			goto end;
		}
		...
		cJSON_ArrayForEach(mapping_of_nssai_local, mapping_of_nssai) {
			...
			OpenAPI_mapping_of_snssai_t *mapping_of_nssaiItem =
				OpenAPI_mapping_of_snssai_parseFromJSON(
					mapping_of_nssai_local);
			if(!mapping_of_nssaiItem) {
				ogs_error("No mapping_of_nssaiItem");
				goto end;
			}
			OpenAPI_list_add(mapping_of_nssaiList,
			                 mapping_of_nssaiItem);
		}
	}

	request_mapping =
		cJSON_GetObjectItemCaseSensitive(
			slice_info_for_registrationJSON, "requestMapping");
	if(request_mapping) {
		if(!cJSON_IsBool(request_mapping)) {
			ogs_error(
				"OpenAPI_slice_info_for_registration_parseFromJSON() failed [request_mapping]");
			goto end;
		}
	}
	...
	ue_sup_nssrg_ind =
		cJSON_GetObjectItemCaseSensitive(
			slice_info_for_registrationJSON, "ueSupNssrgInd");
	if(ue_sup_nssrg_ind) {
		if(!cJSON_IsBool(ue_sup_nssrg_ind)) {
			ogs_error(
				"OpenAPI_slice_info_for_registration_parseFromJSON() failed [ue_sup_nssrg_ind]");
			goto end;
		}
	}
	...
	suppress_nssrg_ind =
		cJSON_GetObjectItemCaseSensitive(
			slice_info_for_registrationJSON, "suppressNssrgInd");
	if(suppress_nssrg_ind) {
		if(!cJSON_IsBool(suppress_nssrg_ind)) {
			ogs_error(
				"OpenAPI_slice_info_for_registration_parseFromJSON() failed [suppress_nssrg_ind]");
			goto end;
		}
	}
	...
	nsag_supported =
		cJSON_GetObjectItemCaseSensitive(
			slice_info_for_registrationJSON, "nsagSupported");
	if(nsag_supported) {
		if(!cJSON_IsBool(nsag_supported)) {
			ogs_error(
				"OpenAPI_slice_info_for_registration_parseFromJSON() failed [nsag_supported]");
			goto end;
		}
	}

	slice_info_for_registration_local_var =
		OpenAPI_slice_info_for_registration_create(
			...
			default_configured_snssai_ind ?
			default_configured_snssai_ind->valueint : 0,
...
OpenAPI_slice_info_for_registration_t *OpenAPI_slice_info_for_registration_copy(
	OpenAPI_slice_info_for_registration_t	*dst,
	OpenAPI_slice_info_for_registration_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_slice_info_for_registration_convertToJSON() failed");
		return NULL;
	}
