...
OpenAPI_service_area_coverage_info_t *OpenAPI_service_area_coverage_info_create(
	OpenAPI_list_t *tac_list, OpenAPI_plmn_id_nid_1_t *serving_network) {
	OpenAPI_service_area_coverage_info_t *
	        service_area_coverage_info_local_var =
		ogs_malloc(sizeof(OpenAPI_service_area_coverage_info_t));
...
void OpenAPI_service_area_coverage_info_free(
	OpenAPI_service_area_coverage_info_t *service_area_coverage_info) {
	...
	if(service_area_coverage_info->tac_list) {
		OpenAPI_list_for_each(service_area_coverage_info->tac_list,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_service_area_coverage_info_convertToJSON(
	OpenAPI_service_area_coverage_info_t *service_area_coverage_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(service_area_coverage_info == NULL) {
		ogs_error(
			"OpenAPI_service_area_coverage_info_convertToJSON() failed [ServiceAreaCoverageInfo]");
		return NULL;
	}
	...
	if(!service_area_coverage_info->tac_list) {
		ogs_error(
			"OpenAPI_service_area_coverage_info_convertToJSON() failed [tac_list]");
		return NULL;
	}
	...
	if(service_area_coverage_info->serving_network) {
		cJSON *serving_network_local_JSON =
			OpenAPI_plmn_id_nid_1_convertToJSON(
				service_area_coverage_info->serving_network);
		if(serving_network_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_service_area_coverage_info_convertToJSON() failed [serving_network]");
			goto end;
		}
...
OpenAPI_service_area_coverage_info_parseFromJSON(
	cJSON *service_area_coverage_infoJSON) {
	OpenAPI_service_area_coverage_info_t *
	        service_area_coverage_info_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *tac_list = NULL;
	...
	tac_list =
		cJSON_GetObjectItemCaseSensitive(service_area_coverage_infoJSON,
		                                 "tacList");
	if(!tac_list) {
		ogs_error(
			"OpenAPI_service_area_coverage_info_parseFromJSON() failed [tac_list]");
		goto end;
	}
	...
	cJSON_ArrayForEach(tac_list_local, tac_list) {
		...
		OpenAPI_list_add(tac_listList,
		                 ogs_strdup(tac_list_local->valuestring));
	}
	...
	if(serving_network) {
		serving_network_local_nonprim =
			OpenAPI_plmn_id_nid_1_parseFromJSON(serving_network);
		if(!serving_network_local_nonprim) {
			ogs_error(
				"OpenAPI_plmn_id_nid_1_parseFromJSON failed [serving_network]");
			goto end;
		}
	}

	service_area_coverage_info_local_var =
		OpenAPI_service_area_coverage_info_create(
			tac_listList,
...
OpenAPI_service_area_coverage_info_t *OpenAPI_service_area_coverage_info_copy(
	OpenAPI_service_area_coverage_info_t	*dst,
	OpenAPI_service_area_coverage_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_service_area_coverage_info_convertToJSON() failed");
		return NULL;
	}
