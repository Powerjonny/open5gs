...
OpenAPI_nwdaf_info_t *OpenAPI_nwdaf_info_create(OpenAPI_list_t	*event_ids,
                                                ...
                                                OpenAPI_list_t *
                                                ml_analytics_list) {
	OpenAPI_nwdaf_info_t *nwdaf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_nwdaf_info_t));
...
void OpenAPI_nwdaf_info_free(OpenAPI_nwdaf_info_t *nwdaf_info) {
	...
	if(nwdaf_info->serving_nf_set_id_list) {
		OpenAPI_list_for_each(nwdaf_info->serving_nf_set_id_list,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_nwdaf_info_convertToJSON(OpenAPI_nwdaf_info_t *nwdaf_info) {
	...
	if(nwdaf_info == NULL) {
		ogs_error(
			"OpenAPI_nwdaf_info_convertToJSON() failed [NwdafInfo]");
		return NULL;
	}
	...
	if(nwdaf_info->event_ids) {
		...
		if(event_idsList == NULL) {
			ogs_error(
				"OpenAPI_nwdaf_info_convertToJSON() failed [event_ids]");
			goto end;
		}
		OpenAPI_list_for_each(nwdaf_info->event_ids, node) {
			cJSON *itemLocal =
	...
	if(nwdaf_info->nwdaf_events) {
		...
		if(nwdaf_eventsList == NULL) {
			ogs_error(
				"OpenAPI_nwdaf_info_convertToJSON() failed [nwdaf_events]");
			goto end;
		}
		OpenAPI_list_for_each(nwdaf_info->nwdaf_events, node) {
			cJSON *itemLocal =
	...
	if(nwdaf_info->tai_list) {
		...
		if(tai_listList == NULL) {
			ogs_error(
				"OpenAPI_nwdaf_info_convertToJSON() failed [tai_list]");
			goto end;
		}
		OpenAPI_list_for_each(nwdaf_info->tai_list, node) {
			cJSON *itemLocal =
	...
	if(nwdaf_info->tai_range_list) {
		...
		if(tai_range_listList == NULL) {
			ogs_error(
				"OpenAPI_nwdaf_info_convertToJSON() failed [tai_range_list]");
			goto end;
		}
		OpenAPI_list_for_each(nwdaf_info->tai_range_list, node) {
			cJSON *itemLocal =
	...
	if(nwdaf_info->nwdaf_capability) {
		cJSON *nwdaf_capability_local_JSON =
			OpenAPI_nwdaf_capability_convertToJSON(
				nwdaf_info->nwdaf_capability);
		if(nwdaf_capability_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_nwdaf_info_convertToJSON() failed [nwdaf_capability]");
			goto end;
		}
	...
	if(nwdaf_info->is_analytics_delay) {
		if(cJSON_AddNumberToObject(item, "analyticsDelay",
		                           nwdaf_info->analytics_delay) == NULL)
		{
			ogs_error(
				"OpenAPI_nwdaf_info_convertToJSON() failed [analytics_delay]");
			goto end;
		}
	}
	...
	if(nwdaf_info->serving_nf_set_id_list) {
		...
		if(serving_nf_set_id_listList == NULL) {
			ogs_error(
				"OpenAPI_nwdaf_info_convertToJSON() failed [serving_nf_set_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(nwdaf_info->serving_nf_set_id_list,
		                      node) {
	...
	if(nwdaf_info->serving_nf_type_list != OpenAPI_nf_type_NULL) {
		...
		if(serving_nf_type_listList == NULL) {
			ogs_error(
				"OpenAPI_nwdaf_info_convertToJSON() failed [serving_nf_type_list]");
			goto end;
		}
	...
	if(nwdaf_info->ml_analytics_list) {
		...
		if(ml_analytics_listList == NULL) {
			ogs_error(
				"OpenAPI_nwdaf_info_convertToJSON() failed [ml_analytics_list]");
			goto end;
		}
		OpenAPI_list_for_each(nwdaf_info->ml_analytics_list, node) {
			cJSON *itemLocal =
...
OpenAPI_nwdaf_info_t *OpenAPI_nwdaf_info_parseFromJSON(cJSON *nwdaf_infoJSON) {
	...
	event_ids = cJSON_GetObjectItemCaseSensitive(nwdaf_infoJSON,
	                                             "eventIds");
	if(event_ids) {
		cJSON *event_ids_local = NULL;
		if(!cJSON_IsArray(event_ids)) {
			ogs_error(
				"OpenAPI_nwdaf_info_parseFromJSON() failed [event_ids]");
			goto end;
		}
	...
	nwdaf_events = cJSON_GetObjectItemCaseSensitive(nwdaf_infoJSON,
	                                                "nwdafEvents");
	if(nwdaf_events) {
	...
	if(tai_list) {
		...
		if(!cJSON_IsArray(tai_list)) {
			ogs_error(
				"OpenAPI_nwdaf_info_parseFromJSON() failed [tai_list]");
			goto end;
		}
	...
	tai_range_list = cJSON_GetObjectItemCaseSensitive(nwdaf_infoJSON,
	                                                  "taiRangeList");
	if(tai_range_list) {
		cJSON *tai_range_list_local = NULL;
		if(!cJSON_IsArray(tai_range_list)) {
			ogs_error(
				"OpenAPI_nwdaf_info_parseFromJSON() failed [tai_range_list]");
			goto end;
		}
	...
	nwdaf_capability = cJSON_GetObjectItemCaseSensitive(nwdaf_infoJSON,
	                                                    "nwdafCapability");
	if(nwdaf_capability) {
	...
	analytics_delay = cJSON_GetObjectItemCaseSensitive(nwdaf_infoJSON,
	                                                   "analyticsDelay");
	if(analytics_delay) {
		if(!cJSON_IsNumber(analytics_delay)) {
			ogs_error(
				"OpenAPI_nwdaf_info_parseFromJSON() failed [analytics_delay]");
			goto end;
		}
	}

	serving_nf_set_id_list =
		cJSON_GetObjectItemCaseSensitive(nwdaf_infoJSON,
		                                 "servingNfSetIdList");
	if(serving_nf_set_id_list) {
		cJSON *serving_nf_set_id_list_local = NULL;
		if(!cJSON_IsArray(serving_nf_set_id_list)) {
			ogs_error(
				"OpenAPI_nwdaf_info_parseFromJSON() failed [serving_nf_set_id_list]");
			goto end;
		}
	...
	serving_nf_type_list = cJSON_GetObjectItemCaseSensitive(nwdaf_infoJSON,
	                                                        "servingNfTypeList");
	if(serving_nf_type_list) {
		cJSON *serving_nf_type_list_local = NULL;
		if(!cJSON_IsArray(serving_nf_type_list)) {
			ogs_error(
				"OpenAPI_nwdaf_info_parseFromJSON() failed [serving_nf_type_list]");
			goto end;
		}
		...
		if(serving_nf_type_listList->count == 0) {
			ogs_error(
				"OpenAPI_nwdaf_info_parseFromJSON() failed: Expected serving_nf_type_listList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}

	ml_analytics_list = cJSON_GetObjectItemCaseSensitive(nwdaf_infoJSON,
	                                                     "mlAnalyticsList");
	if(ml_analytics_list) {
		cJSON *ml_analytics_list_local = NULL;
		if(!cJSON_IsArray(ml_analytics_list)) {
			ogs_error(
				"OpenAPI_nwdaf_info_parseFromJSON() failed [ml_analytics_list]");
			goto end;
		}
...
OpenAPI_nwdaf_info_t *OpenAPI_nwdaf_info_copy(OpenAPI_nwdaf_info_t	*dst,
                                              OpenAPI_nwdaf_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
