...
OpenAPI_ue_context_in_smf_data_t *OpenAPI_ue_context_in_smf_data_create(
	...
	OpenAPI_ue_context_in_smf_data_t *ue_context_in_smf_data_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_context_in_smf_data_t));
...
void OpenAPI_ue_context_in_smf_data_free(
	...
	if(ue_context_in_smf_data->emergency_info) {
		OpenAPI_emergency_info_free(
			ue_context_in_smf_data->emergency_info);
...
cJSON *OpenAPI_ue_context_in_smf_data_convertToJSON(
	...
	if(ue_context_in_smf_data == NULL) {
		ogs_error(
			"OpenAPI_ue_context_in_smf_data_convertToJSON() failed [UeContextInSmfData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(ue_context_in_smf_data->pdu_sessions) {
		cJSON *pdu_sessions = cJSON_AddObjectToObject(item,
		                                              "pduSessions");
		if(pdu_sessions == NULL) {
			ogs_error(
				"OpenAPI_ue_context_in_smf_data_convertToJSON() failed [pdu_sessions]");
			goto end;
		}
		cJSON *localMapObject = pdu_sessions;
		if(ue_context_in_smf_data->pdu_sessions) {
			OpenAPI_list_for_each(
				ue_context_in_smf_data->pdu_sessions, node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_ue_context_in_smf_data_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(ue_context_in_smf_data->pgw_info) {
		...
		if(pgw_infoList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_in_smf_data_convertToJSON() failed [pgw_info]");
			goto end;
		}
		OpenAPI_list_for_each(ue_context_in_smf_data->pgw_info, node) {
			cJSON *itemLocal =
				OpenAPI_pgw_info_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_ue_context_in_smf_data_convertToJSON() failed [pgw_info]");
				goto end;
	...
	if(ue_context_in_smf_data->emergency_info) {
		cJSON *emergency_info_local_JSON =
			OpenAPI_emergency_info_convertToJSON(
				ue_context_in_smf_data->emergency_info);
		if(emergency_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_in_smf_data_convertToJSON() failed [emergency_info]");
			goto end;
		}
		cJSON_AddItemToObject(item, "emergencyInfo",
		                      emergency_info_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_ue_context_in_smf_data_convertToJSON() failed [emergency_info]");
			goto end;
...
OpenAPI_ue_context_in_smf_data_t *OpenAPI_ue_context_in_smf_data_parseFromJSON(
	...
	if(pdu_sessions) {
		...
		if(!cJSON_IsObject(pdu_sessions) &&
		   !cJSON_IsNull(pdu_sessions))
		{
			ogs_error(
				"OpenAPI_ue_context_in_smf_data_parseFromJSON() failed [pdu_sessions]");
			goto end;
		}
		if(cJSON_IsObject(pdu_sessions)) {
			pdu_sessionsList = OpenAPI_list_create();
			OpenAPI_map_t *localMapKeyPair = NULL;
			cJSON_ArrayForEach(pdu_sessions_local_map,
			                   pdu_sessions) {
				...
				if(cJSON_IsObject(localMapObject)) {
					localMapKeyPair = OpenAPI_map_create(
						...
						OpenAPI_pdu_session_parseFromJSON
						        (localMapObject));
					...
					ogs_error(
						"OpenAPI_ue_context_in_smf_data_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	pgw_info = cJSON_GetObjectItemCaseSensitive(ue_context_in_smf_dataJSON,
	                                            "pgwInfo");
	if(pgw_info) {
		cJSON *pgw_info_local = NULL;
		if(!cJSON_IsArray(pgw_info)) {
			ogs_error(
				"OpenAPI_ue_context_in_smf_data_parseFromJSON() failed [pgw_info]");
			goto end;
		}
		...
		cJSON_ArrayForEach(pgw_info_local, pgw_info) {
			...
			OpenAPI_pgw_info_t *pgw_infoItem =
				OpenAPI_pgw_info_parseFromJSON(pgw_info_local);
			if(!pgw_infoItem) {
	...
	if(emergency_info) {
		emergency_info_local_nonprim =
			OpenAPI_emergency_info_parseFromJSON(emergency_info);
		if(!emergency_info_local_nonprim) {
			ogs_error(
				"OpenAPI_emergency_info_parseFromJSON failed [emergency_info]");
			goto end;
		}
	}

	ue_context_in_smf_data_local_var =
		OpenAPI_ue_context_in_smf_data_create(
...
OpenAPI_ue_context_in_smf_data_t *OpenAPI_ue_context_in_smf_data_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ue_context_in_smf_data_convertToJSON() failed");
		return NULL;
	}
