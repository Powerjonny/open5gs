...
OpenAPI_user_data_congestion_info_t *OpenAPI_user_data_congestion_info_create(
	...
	OpenAPI_congestion_info_t *congestion_info, OpenAPI_snssai_t *snssai) {
	OpenAPI_user_data_congestion_info_t *user_data_congestion_info_local_var
	        = ogs_malloc(sizeof(OpenAPI_user_data_congestion_info_t));
...
void OpenAPI_user_data_congestion_info_free(
	OpenAPI_user_data_congestion_info_t *user_data_congestion_info) {
...
cJSON *OpenAPI_user_data_congestion_info_convertToJSON(
	...
	if(user_data_congestion_info == NULL) {
		ogs_error(
			"OpenAPI_user_data_congestion_info_convertToJSON() failed [UserDataCongestionInfo]");
		return NULL;
	}
	...
	if(!user_data_congestion_info->network_area) {
		ogs_error(
			"OpenAPI_user_data_congestion_info_convertToJSON() failed [network_area]");
		return NULL;
	}
	...
	if(!user_data_congestion_info->congestion_info) {
		ogs_error(
			"OpenAPI_user_data_congestion_info_convertToJSON() failed [congestion_info]");
		return NULL;
	}
	cJSON *congestion_info_local_JSON =
		OpenAPI_congestion_info_convertToJSON(
			user_data_congestion_info->congestion_info);
	...
	if(user_data_congestion_info->snssai) {
		...
		if(snssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_user_data_congestion_info_convertToJSON() failed [snssai]");
			goto end;
		}
...
OpenAPI_user_data_congestion_info_parseFromJSON(
	...
	network_area =
		cJSON_GetObjectItemCaseSensitive(user_data_congestion_infoJSON,
		                                 "networkArea");
	if(!network_area) {
		ogs_error(
			"OpenAPI_user_data_congestion_info_parseFromJSON() failed [network_area]");
		goto end;
	}
	...
	if(!congestion_info) {
		ogs_error(
			"OpenAPI_user_data_congestion_info_parseFromJSON() failed [congestion_info]");
		goto end;
	}
	congestion_info_local_nonprim =
		OpenAPI_congestion_info_parseFromJSON(congestion_info);
	if(!congestion_info_local_nonprim) {
		ogs_error(
			"OpenAPI_congestion_info_parseFromJSON failed [congestion_info]");
		goto end;
	}
	...
	user_data_congestion_info_local_var =
		OpenAPI_user_data_congestion_info_create(
			network_area_local_nonprim,
			congestion_info_local_nonprim,
...
OpenAPI_user_data_congestion_info_t *OpenAPI_user_data_congestion_info_copy(
	OpenAPI_user_data_congestion_info_t	*dst,
	OpenAPI_user_data_congestion_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_user_data_congestion_info_convertToJSON() failed");
		return NULL;
	}
