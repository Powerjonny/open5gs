...
OpenAPI_resources_allocation_info_t *OpenAPI_resources_allocation_info_create(
	...
	OpenAPI_list_t *flows, char *alt_ser_req) {
	OpenAPI_resources_allocation_info_t *resources_allocation_info_local_var
	        = ogs_malloc(sizeof(OpenAPI_resources_allocation_info_t));
	ogs_assert(resources_allocation_info_local_var);

	resources_allocation_info_local_var->mc_resourc_status =
		mc_resourc_status;
...
void OpenAPI_resources_allocation_info_free(
	OpenAPI_resources_allocation_info_t *resources_allocation_info) {
...
cJSON *OpenAPI_resources_allocation_info_convertToJSON(
	...
	if(resources_allocation_info == NULL) {
		ogs_error(
			"OpenAPI_resources_allocation_info_convertToJSON() failed [ResourcesAllocationInfo]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(resources_allocation_info->mc_resourc_status !=
	   OpenAPI_media_component_resources_status_NULL)
	{
		if(cJSON_AddStringToObject(item, "mcResourcStatus",
		                           OpenAPI_media_component_resources_status_ToString
		                                   (resources_allocation_info->
		                                   mc_resourc_status)) == NULL)
		{
			ogs_error(
				"OpenAPI_resources_allocation_info_convertToJSON() failed [mc_resourc_status]");
			goto end;
		}
	}
	...
	if(resources_allocation_info->flows) {
		...
		if(flowsList == NULL) {
			ogs_error(
				"OpenAPI_resources_allocation_info_convertToJSON() failed [flows]");
			goto end;
		}
		OpenAPI_list_for_each(resources_allocation_info->flows, node) {
			cJSON *itemLocal =
	...
	if(resources_allocation_info->alt_ser_req) {
		if(cJSON_AddStringToObject(item, "altSerReq",
		                           resources_allocation_info->
		                           alt_ser_req) == NULL)
		{
			ogs_error(
				"OpenAPI_resources_allocation_info_convertToJSON() failed [alt_ser_req]");
			goto end;
		}
	}
...
OpenAPI_resources_allocation_info_parseFromJSON(
	cJSON *resources_allocation_infoJSON) {
	OpenAPI_resources_allocation_info_t *resources_allocation_info_local_var
	        = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	mc_resourc_status =
		cJSON_GetObjectItemCaseSensitive(resources_allocation_infoJSON,
		                                 "mcResourcStatus");
	if(mc_resourc_status) {
		if(!cJSON_IsString(mc_resourc_status)) {
			ogs_error(
				"OpenAPI_resources_allocation_info_parseFromJSON() failed [mc_resourc_status]");
			goto end;
		}
		mc_resourc_statusVariable =
			OpenAPI_media_component_resources_status_FromString(
				mc_resourc_status->valuestring);
	}

	flows = cJSON_GetObjectItemCaseSensitive(resources_allocation_infoJSON,
	                                         "flows");
	if(flows) {
		cJSON *flows_local = NULL;
		if(!cJSON_IsArray(flows)) {
			ogs_error(
				"OpenAPI_resources_allocation_info_parseFromJSON() failed [flows]");
			goto end;
		}
	...
	if(alt_ser_req) {
		if(!cJSON_IsString(alt_ser_req) &&
		   !cJSON_IsNull(alt_ser_req))
		{
			ogs_error(
				"OpenAPI_resources_allocation_info_parseFromJSON() failed [alt_ser_req]");
			goto end;
		}
	}

	resources_allocation_info_local_var =
		OpenAPI_resources_allocation_info_create(
			mc_resourc_status ? mc_resourc_statusVariable : 0,
			flows ? flowsList : NULL,
			alt_ser_req &&
			!cJSON_IsNull(alt_ser_req) ?
			ogs_strdup(alt_ser_req->valuestring) : NULL
			);
...
OpenAPI_resources_allocation_info_t *OpenAPI_resources_allocation_info_copy(
	OpenAPI_resources_allocation_info_t	*dst,
	OpenAPI_resources_allocation_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_resources_allocation_info_convertToJSON() failed");
		return NULL;
	}
