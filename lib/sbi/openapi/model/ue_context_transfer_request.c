...
OpenAPI_ue_context_transfer_request_create(
	...
	OpenAPI_ue_context_transfer_request_t *
		...
		ogs_malloc(sizeof(OpenAPI_ue_context_transfer_request_t));
...
void OpenAPI_ue_context_transfer_request_free(
	...
	if(ue_context_transfer_request->json_data) {
		OpenAPI_ue_context_transfer_req_data_free(
	...
	if(ue_context_transfer_request->binary_data_n1_message) {
		ogs_free(
			ue_context_transfer_request->binary_data_n1_message->
			data);
...
cJSON *OpenAPI_ue_context_transfer_request_convertToJSON(
	...
	if(ue_context_transfer_request == NULL) {
		ogs_error(
			"OpenAPI_ue_context_transfer_request_convertToJSON() failed [UEContextTransfer_request]");
		return NULL;
	}
	...
	if(ue_context_transfer_request->json_data) {
		cJSON *json_data_local_JSON =
			OpenAPI_ue_context_transfer_req_data_convertToJSON(
				ue_context_transfer_request->json_data);
		if(json_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_transfer_request_convertToJSON() failed [json_data]");
			goto end;
		}
	...
	if(ue_context_transfer_request->binary_data_n1_message) {
		char *encoded_str_binary_data_n1_message =
			OpenAPI_base64encode(
				...
				binary_data_n1_message->data,
				ue_context_transfer_request->
				binary_data_n1_message->len);
		if(cJSON_AddStringToObject(item, "binaryDataN1Message",
		                           encoded_str_binary_data_n1_message)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_ue_context_transfer_request_convertToJSON() failed [binary_data_n1_message]");
			goto end;
		}
...
OpenAPI_ue_context_transfer_request_parseFromJSON(
	...
	OpenAPI_ue_context_transfer_request_t *
	        ue_context_transfer_request_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(json_data) {
		...
		if(!json_data_local_nonprim) {
			ogs_error(
				"OpenAPI_ue_context_transfer_req_data_parseFromJSON failed [json_data]");
			goto end;
		}
	}
	...
	if(binary_data_n1_message) {
		...
		if(!cJSON_IsString(binary_data_n1_message)) {
			ogs_error(
				"OpenAPI_ue_context_transfer_request_parseFromJSON() failed [binary_data_n1_message]");
			goto end;
		}
...
OpenAPI_ue_context_transfer_request_t *OpenAPI_ue_context_transfer_request_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ue_context_transfer_request_convertToJSON() failed");
		return NULL;
	}
