...
OpenAPI_observed_redundant_trans_exp_create(bool	is_avg_pkt_drop_rate_ul,
	...
	OpenAPI_observed_redundant_trans_exp_t *
		...
		ogs_malloc(sizeof(OpenAPI_observed_redundant_trans_exp_t));
...
void OpenAPI_observed_redundant_trans_exp_free(
	OpenAPI_observed_redundant_trans_exp_t *observed_redundant_trans_exp) {
...
cJSON *OpenAPI_observed_redundant_trans_exp_convertToJSON(
	OpenAPI_observed_redundant_trans_exp_t *observed_redundant_trans_exp) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(observed_redundant_trans_exp == NULL) {
		ogs_error(
			"OpenAPI_observed_redundant_trans_exp_convertToJSON() failed [ObservedRedundantTransExp]");
		return NULL;
	}
	...
	if(observed_redundant_trans_exp->is_avg_pkt_drop_rate_ul) {
		...
		{
			ogs_error(
				"OpenAPI_observed_redundant_trans_exp_convertToJSON() failed [avg_pkt_drop_rate_ul]");
			goto end;
		}
	}
	...
	if(observed_redundant_trans_exp->is_var_pkt_drop_rate_ul) {
		...
		{
			ogs_error(
				"OpenAPI_observed_redundant_trans_exp_convertToJSON() failed [var_pkt_drop_rate_ul]");
			goto end;
		}
	}
	...
	if(observed_redundant_trans_exp->is_avg_pkt_drop_rate_dl) {
		...
		{
			ogs_error(
				"OpenAPI_observed_redundant_trans_exp_convertToJSON() failed [avg_pkt_drop_rate_dl]");
			goto end;
		}
	}
	...
	if(observed_redundant_trans_exp->is_var_pkt_drop_rate_dl) {
		...
		{
			ogs_error(
				"OpenAPI_observed_redundant_trans_exp_convertToJSON() failed [var_pkt_drop_rate_dl]");
			goto end;
		}
	}
	...
	if(observed_redundant_trans_exp->is_avg_pkt_delay_ul) {
		...
		{
			ogs_error(
				"OpenAPI_observed_redundant_trans_exp_convertToJSON() failed [avg_pkt_delay_ul]");
			goto end;
		}
	}
	...
	if(observed_redundant_trans_exp->is_var_pkt_delay_ul) {
		...
		{
			ogs_error(
				"OpenAPI_observed_redundant_trans_exp_convertToJSON() failed [var_pkt_delay_ul]");
			goto end;
		}
	}
	...
	if(observed_redundant_trans_exp->is_avg_pkt_delay_dl) {
		...
		{
			ogs_error(
				"OpenAPI_observed_redundant_trans_exp_convertToJSON() failed [avg_pkt_delay_dl]");
			goto end;
		}
	}
	...
	if(observed_redundant_trans_exp->is_var_pkt_delay_dl) {
		...
		{
			ogs_error(
				"OpenAPI_observed_redundant_trans_exp_convertToJSON() failed [var_pkt_delay_dl]");
			goto end;
		}
	}
...
OpenAPI_observed_redundant_trans_exp_t *
OpenAPI_observed_redundant_trans_exp_parseFromJSON(
	cJSON *observed_redundant_trans_expJSON) {
	OpenAPI_observed_redundant_trans_exp_t *
	        observed_redundant_trans_exp_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(avg_pkt_drop_rate_ul) {
		if(!cJSON_IsNumber(avg_pkt_drop_rate_ul)) {
			ogs_error(
				"OpenAPI_observed_redundant_trans_exp_parseFromJSON() failed [avg_pkt_drop_rate_ul]");
			goto end;
		}
	}
	...
	if(var_pkt_drop_rate_ul) {
		if(!cJSON_IsNumber(var_pkt_drop_rate_ul)) {
			ogs_error(
				"OpenAPI_observed_redundant_trans_exp_parseFromJSON() failed [var_pkt_drop_rate_ul]");
			goto end;
		}
	}
	...
	if(avg_pkt_drop_rate_dl) {
		if(!cJSON_IsNumber(avg_pkt_drop_rate_dl)) {
			ogs_error(
				"OpenAPI_observed_redundant_trans_exp_parseFromJSON() failed [avg_pkt_drop_rate_dl]");
			goto end;
		}
	}
	...
	if(var_pkt_drop_rate_dl) {
		if(!cJSON_IsNumber(var_pkt_drop_rate_dl)) {
			ogs_error(
				"OpenAPI_observed_redundant_trans_exp_parseFromJSON() failed [var_pkt_drop_rate_dl]");
			goto end;
		}
	}
	...
	if(avg_pkt_delay_ul) {
		if(!cJSON_IsNumber(avg_pkt_delay_ul)) {
			ogs_error(
				"OpenAPI_observed_redundant_trans_exp_parseFromJSON() failed [avg_pkt_delay_ul]");
			goto end;
		}
	}
	...
	if(var_pkt_delay_ul) {
		if(!cJSON_IsNumber(var_pkt_delay_ul)) {
			ogs_error(
				"OpenAPI_observed_redundant_trans_exp_parseFromJSON() failed [var_pkt_delay_ul]");
			goto end;
		}
	}
	...
	if(avg_pkt_delay_dl) {
		if(!cJSON_IsNumber(avg_pkt_delay_dl)) {
			ogs_error(
				"OpenAPI_observed_redundant_trans_exp_parseFromJSON() failed [avg_pkt_delay_dl]");
			goto end;
		}
	}
	...
	if(var_pkt_delay_dl) {
		if(!cJSON_IsNumber(var_pkt_delay_dl)) {
			ogs_error(
				"OpenAPI_observed_redundant_trans_exp_parseFromJSON() failed [var_pkt_delay_dl]");
			goto end;
		}
	}

	observed_redundant_trans_exp_local_var =
		OpenAPI_observed_redundant_trans_exp_create(
			avg_pkt_drop_rate_ul ? true : false,
			avg_pkt_drop_rate_ul ?
			avg_pkt_drop_rate_ul->valuedouble : 0,
			var_pkt_drop_rate_ul ? true : false,
			var_pkt_drop_rate_ul ?
			var_pkt_drop_rate_ul->valuedouble : 0,
			avg_pkt_drop_rate_dl ? true : false,
			avg_pkt_drop_rate_dl ?
			avg_pkt_drop_rate_dl->valuedouble : 0,
			var_pkt_drop_rate_dl ? true : false,
			var_pkt_drop_rate_dl ?
			var_pkt_drop_rate_dl->valuedouble : 0,
...
OpenAPI_observed_redundant_trans_exp_t *
OpenAPI_observed_redundant_trans_exp_copy(
	OpenAPI_observed_redundant_trans_exp_t	*dst,
	OpenAPI_observed_redundant_trans_exp_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_observed_redundant_trans_exp_convertToJSON() failed");
		return NULL;
	}
