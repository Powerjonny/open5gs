...
OpenAPI_ng_ksi_t *OpenAPI_ng_ksi_create(OpenAPI_sc_type_e tsc, int ksi) {
	OpenAPI_ng_ksi_t *ng_ksi_local_var =
		ogs_malloc(sizeof(OpenAPI_ng_ksi_t));
...
cJSON *OpenAPI_ng_ksi_convertToJSON(OpenAPI_ng_ksi_t *ng_ksi) {
	...
	if(cJSON_AddStringToObject(item, "tsc",
	                           OpenAPI_sc_type_ToString(ng_ksi->tsc)) ==
	   NULL)
	{
		ogs_error("OpenAPI_ng_ksi_convertToJSON() failed [tsc]");
		goto end;
...
OpenAPI_ng_ksi_t *OpenAPI_ng_ksi_copy(OpenAPI_ng_ksi_t	*dst,
                                      OpenAPI_ng_ksi_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
