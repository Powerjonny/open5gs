...
OpenAPI_mbsfn_area_t *OpenAPI_mbsfn_area_create(bool	is_mbsfn_area_id,
                                                int	mbsfn_area_id,
                                                bool	is_carrier_frequency,
                                                int	carrier_frequency) {
	OpenAPI_mbsfn_area_t *mbsfn_area_local_var =
		ogs_malloc(sizeof(OpenAPI_mbsfn_area_t));
...
cJSON *OpenAPI_mbsfn_area_convertToJSON(OpenAPI_mbsfn_area_t *mbsfn_area) {
	...
	if(mbsfn_area == NULL) {
		ogs_error(
			"OpenAPI_mbsfn_area_convertToJSON() failed [MbsfnArea]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(mbsfn_area->is_mbsfn_area_id) {
		if(cJSON_AddNumberToObject(item, "mbsfnAreaId",
		                           mbsfn_area->mbsfn_area_id) == NULL)
		{
			ogs_error(
				"OpenAPI_mbsfn_area_convertToJSON() failed [mbsfn_area_id]");
			goto end;
		}
	}

	if(mbsfn_area->is_carrier_frequency) {
		if(cJSON_AddNumberToObject(item, "carrierFrequency",
		                           mbsfn_area->carrier_frequency) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_mbsfn_area_convertToJSON() failed [carrier_frequency]");
			goto end;
		}
	}
...
OpenAPI_mbsfn_area_t *OpenAPI_mbsfn_area_parseFromJSON(cJSON *mbsfn_areaJSON) {
	...
	mbsfn_area_id = cJSON_GetObjectItemCaseSensitive(mbsfn_areaJSON,
	                                                 "mbsfnAreaId");
	if(mbsfn_area_id) {
		if(!cJSON_IsNumber(mbsfn_area_id)) {
			ogs_error(
				"OpenAPI_mbsfn_area_parseFromJSON() failed [mbsfn_area_id]");
			goto end;
		}
	}

	carrier_frequency = cJSON_GetObjectItemCaseSensitive(mbsfn_areaJSON,
	                                                     "carrierFrequency");
	if(carrier_frequency) {
		if(!cJSON_IsNumber(carrier_frequency)) {
			ogs_error(
				"OpenAPI_mbsfn_area_parseFromJSON() failed [carrier_frequency]");
			goto end;
		}
	}
...
OpenAPI_mbsfn_area_t *OpenAPI_mbsfn_area_copy(OpenAPI_mbsfn_area_t	*dst,
                                              OpenAPI_mbsfn_area_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
