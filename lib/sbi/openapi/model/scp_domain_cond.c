...
OpenAPI_scp_domain_cond_t *OpenAPI_scp_domain_cond_create(
	OpenAPI_list_t *scp_domains, OpenAPI_list_t *nf_type_list) {
	OpenAPI_scp_domain_cond_t *scp_domain_cond_local_var =
		ogs_malloc(sizeof(OpenAPI_scp_domain_cond_t));
...
cJSON *OpenAPI_scp_domain_cond_convertToJSON(
	OpenAPI_scp_domain_cond_t *scp_domain_cond) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(scp_domain_cond == NULL) {
		ogs_error(
			"OpenAPI_scp_domain_cond_convertToJSON() failed [ScpDomainCond]");
		return NULL;
	}
	...
	if(!scp_domain_cond->scp_domains) {
		ogs_error(
			"OpenAPI_scp_domain_cond_convertToJSON() failed [scp_domains]");
		return NULL;
	}
	...
	if(scp_domain_cond->nf_type_list != OpenAPI_nf_type_NULL) {
		cJSON *nf_type_listList = cJSON_AddArrayToObject(item,
		                                                 "nfTypeList");
		if(nf_type_listList == NULL) {
			ogs_error(
				"OpenAPI_scp_domain_cond_convertToJSON() failed [nf_type_list]");
			goto end;
		}
		OpenAPI_list_for_each(scp_domain_cond->nf_type_list, node) {
			if(cJSON_AddStringToObject(nf_type_listList, "",
...
OpenAPI_scp_domain_cond_t *OpenAPI_scp_domain_cond_parseFromJSON(
	cJSON *scp_domain_condJSON) {
	...
	scp_domains = cJSON_GetObjectItemCaseSensitive(scp_domain_condJSON,
	                                               "scpDomains");
	if(!scp_domains) {
		ogs_error(
			"OpenAPI_scp_domain_cond_parseFromJSON() failed [scp_domains]");
		goto end;
	}
	...
	cJSON_ArrayForEach(scp_domains_local, scp_domains) {
		...
		OpenAPI_list_add(scp_domainsList,
		                 ogs_strdup(scp_domains_local->valuestring));
	}

	nf_type_list = cJSON_GetObjectItemCaseSensitive(scp_domain_condJSON,
	                                                "nfTypeList");
	if(nf_type_list) {
		cJSON *nf_type_list_local = NULL;
		if(!cJSON_IsArray(nf_type_list)) {
			ogs_error(
				"OpenAPI_scp_domain_cond_parseFromJSON() failed [nf_type_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(nf_type_list_local, nf_type_list) {
			...
			if(!localEnum) {
				ogs_info(
					"Enum value \"%s\" for field \"nf_type_list\" is not supported. Ignoring it ...",
					nf_type_list_local->valuestring);
			} else {
				OpenAPI_list_add(nf_type_listList,
				                 (void *) localEnum);
			}
		}
		if(nf_type_listList->count == 0) {
			ogs_error(
				"OpenAPI_scp_domain_cond_parseFromJSON() failed: Expected nf_type_listList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
...
OpenAPI_scp_domain_cond_t *OpenAPI_scp_domain_cond_copy(
	OpenAPI_scp_domain_cond_t *dst, OpenAPI_scp_domain_cond_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
