...
OpenAPI_sm_policy_notification_t *OpenAPI_sm_policy_notification_create(
	...
	OpenAPI_sm_policy_notification_t *sm_policy_notification_local_var =
		ogs_malloc(sizeof(OpenAPI_sm_policy_notification_t));
	ogs_assert(sm_policy_notification_local_var);

	sm_policy_notification_local_var->resource_uri = resource_uri;
	sm_policy_notification_local_var->sm_policy_decision =
		sm_policy_decision;
...
void OpenAPI_sm_policy_notification_free(
	...
	if(sm_policy_notification->sm_policy_decision) {
		OpenAPI_sm_policy_decision_free(
			sm_policy_notification->sm_policy_decision);
...
cJSON *OpenAPI_sm_policy_notification_convertToJSON(
	...
	if(sm_policy_notification == NULL) {
		ogs_error(
			"OpenAPI_sm_policy_notification_convertToJSON() failed [SmPolicyNotification]");
		return NULL;
	}
	...
	if(sm_policy_notification->resource_uri) {
		if(cJSON_AddStringToObject(item, "resourceUri",
		                           sm_policy_notification->resource_uri)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_notification_convertToJSON() failed [resource_uri]");
			goto end;
		}
	}

	if(sm_policy_notification->sm_policy_decision) {
		cJSON *sm_policy_decision_local_JSON =
			OpenAPI_sm_policy_decision_convertToJSON(
				sm_policy_notification->sm_policy_decision);
		if(sm_policy_decision_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_notification_convertToJSON() failed [sm_policy_decision]");
			goto end;
		}
		cJSON_AddItemToObject(item, "smPolicyDecision",
		                      sm_policy_decision_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_notification_convertToJSON() failed [sm_policy_decision]");
			goto end;
...
OpenAPI_sm_policy_notification_t *OpenAPI_sm_policy_notification_parseFromJSON(
	...
	resource_uri =
		cJSON_GetObjectItemCaseSensitive(sm_policy_notificationJSON,
		                                 "resourceUri");
	if(resource_uri) {
		if(!cJSON_IsString(resource_uri) &&
		   !cJSON_IsNull(resource_uri))
		{
			ogs_error(
				"OpenAPI_sm_policy_notification_parseFromJSON() failed [resource_uri]");
			goto end;
		}
	}
	...
	if(sm_policy_decision) {
		sm_policy_decision_local_nonprim =
			OpenAPI_sm_policy_decision_parseFromJSON(
				sm_policy_decision);
		if(!sm_policy_decision_local_nonprim) {
			ogs_error(
				"OpenAPI_sm_policy_decision_parseFromJSON failed [sm_policy_decision]");
			goto end;
		}
	}

	sm_policy_notification_local_var =
		OpenAPI_sm_policy_notification_create(
			resource_uri &&
			!cJSON_IsNull(resource_uri) ?
			ogs_strdup(resource_uri->valuestring) : NULL,
			sm_policy_decision ? sm_policy_decision_local_nonprim :
			NULL
			);
...
OpenAPI_sm_policy_notification_t *OpenAPI_sm_policy_notification_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_sm_policy_notification_convertToJSON() failed");
		return NULL;
	}
