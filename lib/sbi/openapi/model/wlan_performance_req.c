...
OpenAPI_wlan_performance_req_t *OpenAPI_wlan_performance_req_create(
	OpenAPI_list_t *ss_ids, OpenAPI_list_t *bss_ids,
...
void OpenAPI_wlan_performance_req_free(
	OpenAPI_wlan_performance_req_t *wlan_performance_req) {
...
cJSON *OpenAPI_wlan_performance_req_convertToJSON(
	OpenAPI_wlan_performance_req_t *wlan_performance_req) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(wlan_performance_req == NULL) {
		ogs_error(
			"OpenAPI_wlan_performance_req_convertToJSON() failed [WlanPerformanceReq]");
		return NULL;
	}
	...
	if(wlan_performance_req->ss_ids) {
		...
		if(ss_idsList == NULL) {
			ogs_error(
				"OpenAPI_wlan_performance_req_convertToJSON() failed [ss_ids]");
			goto end;
		}
		OpenAPI_list_for_each(wlan_performance_req->ss_ids, node) {
	...
	if(wlan_performance_req->bss_ids) {
		...
		if(bss_idsList == NULL) {
			ogs_error(
				"OpenAPI_wlan_performance_req_convertToJSON() failed [bss_ids]");
			goto end;
		}
		OpenAPI_list_for_each(wlan_performance_req->bss_ids, node) {
	...
	if(wlan_performance_req->wlan_order_criter) {
		cJSON *wlan_order_criter_local_JSON =
			OpenAPI_wlan_ordering_criterion_convertToJSON(
				wlan_performance_req->wlan_order_criter);
		if(wlan_order_criter_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_wlan_performance_req_convertToJSON() failed [wlan_order_criter]");
			goto end;
		}
	...
	if(wlan_performance_req->order) {
		...
		if(order_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_wlan_performance_req_convertToJSON() failed [order]");
			goto end;
		}
...
OpenAPI_wlan_performance_req_t *OpenAPI_wlan_performance_req_parseFromJSON(
	...
	OpenAPI_wlan_ordering_criterion_t *wlan_order_criter_local_nonprim =
		NULL;
	cJSON *order = NULL;
	...
	if(ss_ids) {
		...
		if(!cJSON_IsArray(ss_ids)) {
			ogs_error(
				"OpenAPI_wlan_performance_req_parseFromJSON() failed [ss_ids]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ss_ids_local, ss_ids) {
			...
			OpenAPI_list_add(ss_idsList,
			                 ogs_strdup(ss_ids_local->valuestring));
		}
	}
	...
	if(bss_ids) {
		...
		if(!cJSON_IsArray(bss_ids)) {
			ogs_error(
				"OpenAPI_wlan_performance_req_parseFromJSON() failed [bss_ids]");
			goto end;
		}
		...
		cJSON_ArrayForEach(bss_ids_local, bss_ids) {
			...
			OpenAPI_list_add(bss_idsList,
			                 ogs_strdup(
						 bss_ids_local->valuestring));
		}
	}

	wlan_order_criter =
		cJSON_GetObjectItemCaseSensitive(wlan_performance_reqJSON,
		                                 "wlanOrderCriter");
	if(wlan_order_criter) {
		...
		if(!wlan_order_criter_local_nonprim) {
			ogs_error(
				"OpenAPI_wlan_ordering_criterion_parseFromJSON failed [wlan_order_criter]");
			goto end;
		}
	}
...
OpenAPI_wlan_performance_req_t *OpenAPI_wlan_performance_req_copy(
	OpenAPI_wlan_performance_req_t	*dst,
	OpenAPI_wlan_performance_req_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_wlan_performance_req_convertToJSON() failed");
		return NULL;
	}
