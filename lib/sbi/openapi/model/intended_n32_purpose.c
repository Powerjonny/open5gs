...
OpenAPI_intended_n32_purpose_t *OpenAPI_intended_n32_purpose_create(
	...
	char *cause) {
	OpenAPI_intended_n32_purpose_t *intended_n32_purpose_local_var =
		ogs_malloc(sizeof(OpenAPI_intended_n32_purpose_t));
...
cJSON *OpenAPI_intended_n32_purpose_convertToJSON(
	...
	if(intended_n32_purpose == NULL) {
		ogs_error(
			"OpenAPI_intended_n32_purpose_convertToJSON() failed [IntendedN32Purpose]");
		return NULL;
	}
	...
	if(intended_n32_purpose->usage_purpose == OpenAPI_n32_purpose_NULL) {
		ogs_error(
			"OpenAPI_intended_n32_purpose_convertToJSON() failed [usage_purpose]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "usagePurpose",
	                           OpenAPI_n32_purpose_ToString(
					   intended_n32_purpose->usage_purpose))
	   == NULL)
	{
		ogs_error(
			"OpenAPI_intended_n32_purpose_convertToJSON() failed [usage_purpose]");
	...
	if(intended_n32_purpose->additional_info) {
		if(cJSON_AddStringToObject(item, "additionalInfo",
		                           intended_n32_purpose->additional_info)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_intended_n32_purpose_convertToJSON() failed [additional_info]");
			goto end;
		}
	}
	...
	if(intended_n32_purpose->cause) {
		...
		{
			ogs_error(
				"OpenAPI_intended_n32_purpose_convertToJSON() failed [cause]");
			goto end;
		}
	}
...
OpenAPI_intended_n32_purpose_t *OpenAPI_intended_n32_purpose_parseFromJSON(
	...
	cJSON *cause = NULL;
	usage_purpose =
		cJSON_GetObjectItemCaseSensitive(intended_n32_purposeJSON,
		                                 "usagePurpose");
	if(!usage_purpose) {
		ogs_error(
			"OpenAPI_intended_n32_purpose_parseFromJSON() failed [usage_purpose]");
		goto end;
	}
	...
	usage_purposeVariable =
		OpenAPI_n32_purpose_FromString(usage_purpose->valuestring);

	additional_info =
		cJSON_GetObjectItemCaseSensitive(intended_n32_purposeJSON,
		                                 "additionalInfo");
	if(additional_info) {
		if(!cJSON_IsString(additional_info) &&
		   !cJSON_IsNull(additional_info))
		{
			ogs_error(
				"OpenAPI_intended_n32_purpose_parseFromJSON() failed [additional_info]");
			goto end;
		}
	}

	cause = cJSON_GetObjectItemCaseSensitive(intended_n32_purposeJSON,
	                                         "cause");
	if(cause) {
		...
		{
			ogs_error(
				"OpenAPI_intended_n32_purpose_parseFromJSON() failed [cause]");
			goto end;
		}
	}
	...
	intended_n32_purpose_local_var = OpenAPI_intended_n32_purpose_create(
		...
		additional_info &&
		!cJSON_IsNull(additional_info) ?
		ogs_strdup(additional_info->valuestring) : NULL,
		cause &&
...
OpenAPI_intended_n32_purpose_t *OpenAPI_intended_n32_purpose_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_intended_n32_purpose_convertToJSON() failed");
		return NULL;
	}
