...
OpenAPI_lcs_privacy_data_t *OpenAPI_lcs_privacy_data_create(
	...
	*
	unrelated_class,
	OpenAPI_list_t
	*plmn_operator_classes) {
	OpenAPI_lcs_privacy_data_t *lcs_privacy_data_local_var =
		ogs_malloc(sizeof(OpenAPI_lcs_privacy_data_t));
	...
	lcs_privacy_data_local_var->plmn_operator_classes =
		plmn_operator_classes;

	return lcs_privacy_data_local_var;
}

void OpenAPI_lcs_privacy_data_free(
	...
	if(lcs_privacy_data->plmn_operator_classes) {
		OpenAPI_list_for_each(lcs_privacy_data->plmn_operator_classes,
		                      node) {
...
cJSON *OpenAPI_lcs_privacy_data_convertToJSON(
	OpenAPI_lcs_privacy_data_t *lcs_privacy_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(lcs_privacy_data == NULL) {
		ogs_error(
			"OpenAPI_lcs_privacy_data_convertToJSON() failed [LcsPrivacyData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(lcs_privacy_data->lpi) {
		cJSON *lpi_local_JSON =
			OpenAPI_lpi_convertToJSON(lcs_privacy_data->lpi);
		if(lpi_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_lcs_privacy_data_convertToJSON() failed [lpi]");
			goto end;
		}
	...
	if(lcs_privacy_data->unrelated_class) {
		cJSON *unrelated_class_local_JSON =
			OpenAPI_unrelated_class_convertToJSON(
				lcs_privacy_data->unrelated_class);
		if(unrelated_class_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_lcs_privacy_data_convertToJSON() failed [unrelated_class]");
			goto end;
		}
		cJSON_AddItemToObject(item, "unrelatedClass",
		                      unrelated_class_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_lcs_privacy_data_convertToJSON() failed [unrelated_class]");
			goto end;
	...
	if(lcs_privacy_data->plmn_operator_classes) {
		cJSON *plmn_operator_classesList = cJSON_AddArrayToObject(item,
		                                                          "plmnOperatorClasses");
		if(plmn_operator_classesList == NULL) {
			ogs_error(
				"OpenAPI_lcs_privacy_data_convertToJSON() failed [plmn_operator_classes]");
			goto end;
		}
		OpenAPI_list_for_each(lcs_privacy_data->plmn_operator_classes,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_plmn_operator_class_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_lcs_privacy_data_convertToJSON() failed [plmn_operator_classes]");
			...
			cJSON_AddItemToArray(plmn_operator_classesList,
			                     itemLocal);
		}
	}
...
OpenAPI_lcs_privacy_data_t *OpenAPI_lcs_privacy_data_parseFromJSON(
	cJSON *lcs_privacy_dataJSON) {
	...
	unrelated_class = cJSON_GetObjectItemCaseSensitive(lcs_privacy_dataJSON,
	                                                   "unrelatedClass");
	if(unrelated_class) {
		unrelated_class_local_nonprim =
			OpenAPI_unrelated_class_parseFromJSON(unrelated_class);
		if(!unrelated_class_local_nonprim) {
			ogs_error(
				"OpenAPI_unrelated_class_parseFromJSON failed [unrelated_class]");
			goto end;
		}
	}

	plmn_operator_classes =
		cJSON_GetObjectItemCaseSensitive(lcs_privacy_dataJSON,
		                                 "plmnOperatorClasses");
	if(plmn_operator_classes) {
		cJSON *plmn_operator_classes_local = NULL;
		if(!cJSON_IsArray(plmn_operator_classes)) {
			ogs_error(
				"OpenAPI_lcs_privacy_data_parseFromJSON() failed [plmn_operator_classes]");
			goto end;
		}

		plmn_operator_classesList = OpenAPI_list_create();

		cJSON_ArrayForEach(plmn_operator_classes_local,
		                   plmn_operator_classes) {
			...
			OpenAPI_plmn_operator_class_t *plmn_operator_classesItem
			        = OpenAPI_plmn_operator_class_parseFromJSON(
					  plmn_operator_classes_local);
			if(!plmn_operator_classesItem) {
				ogs_error("No plmn_operator_classesItem");
				goto end;
			}
			OpenAPI_list_add(plmn_operator_classesList,
			                 plmn_operator_classesItem);
		}
	}
...
OpenAPI_lcs_privacy_data_t *OpenAPI_lcs_privacy_data_copy(
	OpenAPI_lcs_privacy_data_t *dst, OpenAPI_lcs_privacy_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
