...
OpenAPI_smcce_info_t *OpenAPI_smcce_info_create(char			*dnn,
                                                ...
                                                smcce_ue_list) {
	OpenAPI_smcce_info_t *smcce_info_local_var =
		ogs_malloc(sizeof(OpenAPI_smcce_info_t));
...
cJSON *OpenAPI_smcce_info_convertToJSON(OpenAPI_smcce_info_t *smcce_info) {
	...
	if(smcce_info == NULL) {
		ogs_error(
			"OpenAPI_smcce_info_convertToJSON() failed [SmcceInfo]");
		return NULL;
	}
	...
	if(smcce_info->dnn) {
		...
		{
			ogs_error(
				"OpenAPI_smcce_info_convertToJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(smcce_info->snssai) {
		cJSON *snssai_local_JSON =
			OpenAPI_snssai_convertToJSON(smcce_info->snssai);
		if(snssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_smcce_info_convertToJSON() failed [snssai]");
			goto end;
		}
	...
	if(!smcce_info->smcce_ue_list) {
		ogs_error(
			"OpenAPI_smcce_info_convertToJSON() failed [smcce_ue_list]");
		return NULL;
	}
	cJSON *smcce_ue_list_local_JSON =
		OpenAPI_smcce_ue_list_convertToJSON(smcce_info->smcce_ue_list);
...
OpenAPI_smcce_info_t *OpenAPI_smcce_info_parseFromJSON(cJSON *smcce_infoJSON) {
	...
	if(dnn) {
		...
		{
			ogs_error(
				"OpenAPI_smcce_info_parseFromJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(!smcce_ue_list) {
		ogs_error(
			"OpenAPI_smcce_info_parseFromJSON() failed [smcce_ue_list]");
		goto end;
	}
	smcce_ue_list_local_nonprim =
		OpenAPI_smcce_ue_list_parseFromJSON(smcce_ue_list);
	if(!smcce_ue_list_local_nonprim) {
		ogs_error(
			"OpenAPI_smcce_ue_list_parseFromJSON failed [smcce_ue_list]");
		goto end;
	}
