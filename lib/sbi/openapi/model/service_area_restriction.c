...
OpenAPI_service_area_restriction_t *OpenAPI_service_area_restriction_create(
	...
	OpenAPI_service_area_restriction_t *service_area_restriction_local_var =
		ogs_malloc(sizeof(OpenAPI_service_area_restriction_t));
...
cJSON *OpenAPI_service_area_restriction_convertToJSON(
	OpenAPI_service_area_restriction_t *service_area_restriction) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(service_area_restriction == NULL) {
		ogs_error(
			"OpenAPI_service_area_restriction_convertToJSON() failed [ServiceAreaRestriction]");
		return NULL;
	}
	   ...
	   OpenAPI_restriction_type_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_service_area_restriction_convertToJSON() failed [restriction_type]");
			goto end;
		}
	}
	...
	if(service_area_restriction->areas) {
		...
		if(areasList == NULL) {
			ogs_error(
				"OpenAPI_service_area_restriction_convertToJSON() failed [areas]");
			goto end;
		}
		OpenAPI_list_for_each(service_area_restriction->areas, node) {
			cJSON *itemLocal =
	...
	if(service_area_restriction->is_max_num_of_tas) {
		...
		{
			ogs_error(
				"OpenAPI_service_area_restriction_convertToJSON() failed [max_num_of_tas]");
			goto end;
		}
	}
	...
	if(service_area_restriction->is_max_num_of_tas_for_not_allowed_areas) {
		...
		{
			ogs_error(
				"OpenAPI_service_area_restriction_convertToJSON() failed [max_num_of_tas_for_not_allowed_areas]");
			goto end;
		}
	}
...
OpenAPI_service_area_restriction_parseFromJSON(
	...
	restriction_type =
		cJSON_GetObjectItemCaseSensitive(service_area_restrictionJSON,
		                                 "restrictionType");
	if(restriction_type) {
		if(!cJSON_IsString(restriction_type)) {
			ogs_error(
				"OpenAPI_service_area_restriction_parseFromJSON() failed [restriction_type]");
			goto end;
		}
	...
	if(areas) {
		...
		if(!cJSON_IsArray(areas)) {
			ogs_error(
				"OpenAPI_service_area_restriction_parseFromJSON() failed [areas]");
			goto end;
		}
		...
		cJSON_ArrayForEach(areas_local, areas) {
			...
			OpenAPI_area_t *areasItem =
				OpenAPI_area_parseFromJSON(areas_local);
			if(!areasItem) {
	...
	if(max_num_of_tas) {
		if(!cJSON_IsNumber(max_num_of_tas)) {
			ogs_error(
				"OpenAPI_service_area_restriction_parseFromJSON() failed [max_num_of_tas]");
			goto end;
		}
	}
	...
	if(max_num_of_tas_for_not_allowed_areas) {
		if(!cJSON_IsNumber(max_num_of_tas_for_not_allowed_areas)) {
			ogs_error(
				"OpenAPI_service_area_restriction_parseFromJSON() failed [max_num_of_tas_for_not_allowed_areas]");
			goto end;
		}
	}
...
OpenAPI_service_area_restriction_t *OpenAPI_service_area_restriction_copy(
	OpenAPI_service_area_restriction_t	*dst,
	OpenAPI_service_area_restriction_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_service_area_restriction_convertToJSON() failed");
		return NULL;
	}
