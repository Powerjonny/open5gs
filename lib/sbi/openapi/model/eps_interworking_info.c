...
OpenAPI_eps_interworking_info_t *OpenAPI_eps_interworking_info_create(
	...
	OpenAPI_eps_interworking_info_t *eps_interworking_info_local_var =
		ogs_malloc(sizeof(OpenAPI_eps_interworking_info_t));
...
void OpenAPI_eps_interworking_info_free(
...
cJSON *OpenAPI_eps_interworking_info_convertToJSON(
	OpenAPI_eps_interworking_info_t *eps_interworking_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(eps_interworking_info == NULL) {
		ogs_error(
			"OpenAPI_eps_interworking_info_convertToJSON() failed [EpsInterworkingInfo]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(eps_interworking_info->eps_iwk_pgws) {
		cJSON *eps_iwk_pgws = cJSON_AddObjectToObject(item,
		                                              "epsIwkPgws");
		if(eps_iwk_pgws == NULL) {
			ogs_error(
				"OpenAPI_eps_interworking_info_convertToJSON() failed [eps_iwk_pgws]");
			goto end;
		}
		cJSON *localMapObject = eps_iwk_pgws;
		if(eps_interworking_info->eps_iwk_pgws) {
			OpenAPI_list_for_each(
				eps_interworking_info->eps_iwk_pgws, node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_eps_iwk_pgw_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_eps_interworking_info_convertToJSON() failed [inner]");
					goto end;
				}
...
OpenAPI_eps_interworking_info_t *OpenAPI_eps_interworking_info_parseFromJSON(
	...
	eps_iwk_pgws =
		cJSON_GetObjectItemCaseSensitive(eps_interworking_infoJSON,
		                                 "epsIwkPgws");
	if(eps_iwk_pgws) {
		...
		{
			ogs_error(
				"OpenAPI_eps_interworking_info_parseFromJSON() failed [eps_iwk_pgws]");
			goto end;
		}
		if(cJSON_IsObject(eps_iwk_pgws)) {
			...
			cJSON_ArrayForEach(eps_iwk_pgws_local_map,
				...
				if(cJSON_IsObject(localMapObject)) {
					localMapKeyPair = OpenAPI_map_create(
						...
						OpenAPI_eps_iwk_pgw_parseFromJSON
						        (localMapObject));
					...
					ogs_error(
						"OpenAPI_eps_interworking_info_parseFromJSON() failed [inner]");
					goto end;
				}
...
OpenAPI_eps_interworking_info_t *OpenAPI_eps_interworking_info_copy(
	OpenAPI_eps_interworking_info_t *dst,
	OpenAPI_eps_interworking_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_eps_interworking_info_convertToJSON() failed");
		return NULL;
	}
