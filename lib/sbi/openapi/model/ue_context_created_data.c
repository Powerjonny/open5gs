...
OpenAPI_ue_context_created_data_t *OpenAPI_ue_context_created_data_create(
	...
	OpenAPI_ue_context_created_data_t *ue_context_created_data_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_context_created_data_t));
...
void OpenAPI_ue_context_created_data_free(
	...
	if(ue_context_created_data->pdu_session_list) {
		OpenAPI_list_for_each(ue_context_created_data->pdu_session_list,
		                      node) {
	...
	if(ue_context_created_data->failed_session_list) {
		OpenAPI_list_for_each(
			ue_context_created_data->failed_session_list, node) {
			OpenAPI_n2_sm_information_free(node->data);
		}
	...
	if(ue_context_created_data->analytics_not_used_list) {
		OpenAPI_list_for_each(
			ue_context_created_data->analytics_not_used_list,
			node) {
			ogs_free(node->data);
		}
		OpenAPI_list_free(
...
cJSON *OpenAPI_ue_context_created_data_convertToJSON(
	...
	if(ue_context_created_data == NULL) {
		ogs_error(
			"OpenAPI_ue_context_created_data_convertToJSON() failed [UeContextCreatedData]");
		return NULL;
	}
	...
	if(!ue_context_created_data->ue_context) {
		ogs_error(
			"OpenAPI_ue_context_created_data_convertToJSON() failed [ue_context]");
		return NULL;
	}
	...
	if(!ue_context_created_data->target_to_source_data) {
		ogs_error(
			"OpenAPI_ue_context_created_data_convertToJSON() failed [target_to_source_data]");
		return NULL;
	}
	cJSON *target_to_source_data_local_JSON =
	...
	if(!ue_context_created_data->pdu_session_list) {
		ogs_error(
			"OpenAPI_ue_context_created_data_convertToJSON() failed [pdu_session_list]");
		return NULL;
	}
	...
	if(ue_context_created_data->failed_session_list) {
		...
		if(failed_session_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_created_data_convertToJSON() failed [failed_session_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			ue_context_created_data->failed_session_list, node) {
	...
	if(ue_context_created_data->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_created_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(ue_context_created_data->is_pcf_reselected_ind) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_created_data_convertToJSON() failed [pcf_reselected_ind]");
			goto end;
		}
	}
	...
	if(ue_context_created_data->analytics_not_used_list) {
		...
		if(analytics_not_used_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_created_data_convertToJSON() failed [analytics_not_used_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			ue_context_created_data->analytics_not_used_list,
...
OpenAPI_ue_context_created_data_t *OpenAPI_ue_context_created_data_parseFromJSON
	...
	ue_context =
		cJSON_GetObjectItemCaseSensitive(ue_context_created_dataJSON,
		                                 "ueContext");
	if(!ue_context) {
		ogs_error(
			"OpenAPI_ue_context_created_data_parseFromJSON() failed [ue_context]");
		goto end;
	}
	...
	if(!target_to_source_data) {
		ogs_error(
			"OpenAPI_ue_context_created_data_parseFromJSON() failed [target_to_source_data]");
		goto end;
	}
	target_to_source_data_local_nonprim =
		OpenAPI_n2_info_content_parseFromJSON(target_to_source_data);
	if(!target_to_source_data_local_nonprim) {
		ogs_error(
			"OpenAPI_n2_info_content_parseFromJSON failed [target_to_source_data]");
		goto end;
	}
	...
	if(!pdu_session_list) {
		ogs_error(
			"OpenAPI_ue_context_created_data_parseFromJSON() failed [pdu_session_list]");
		goto end;
	}
	...
	if(failed_session_list) {
		...
		if(!cJSON_IsArray(failed_session_list)) {
			ogs_error(
				"OpenAPI_ue_context_created_data_parseFromJSON() failed [failed_session_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(failed_session_list_local,
			...
			OpenAPI_n2_sm_information_t *failed_session_listItem =
				OpenAPI_n2_sm_information_parseFromJSON(
			...
			OpenAPI_list_add(failed_session_listList,
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_created_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(pcf_reselected_ind) {
		if(!cJSON_IsBool(pcf_reselected_ind)) {
			ogs_error(
				"OpenAPI_ue_context_created_data_parseFromJSON() failed [pcf_reselected_ind]");
			goto end;
		}
	}
	...
	if(analytics_not_used_list) {
		...
		if(!cJSON_IsArray(analytics_not_used_list)) {
			ogs_error(
				"OpenAPI_ue_context_created_data_parseFromJSON() failed [analytics_not_used_list]");
			goto end;
		}
	...
	ue_context_created_data_local_var =
		OpenAPI_ue_context_created_data_create(
...
OpenAPI_ue_context_created_data_t *OpenAPI_ue_context_created_data_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ue_context_created_data_convertToJSON() failed");
		return NULL;
	}
