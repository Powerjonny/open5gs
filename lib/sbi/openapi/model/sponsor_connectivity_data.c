...
OpenAPI_sponsor_connectivity_data_t *OpenAPI_sponsor_connectivity_data_create(
	...
	OpenAPI_sponsor_connectivity_data_t *sponsor_connectivity_data_local_var
	        = ogs_malloc(sizeof(OpenAPI_sponsor_connectivity_data_t));
...
void OpenAPI_sponsor_connectivity_data_free(
	...
	if(sponsor_connectivity_data->asp_ids) {
		OpenAPI_list_for_each(sponsor_connectivity_data->asp_ids,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_sponsor_connectivity_data_convertToJSON(
	OpenAPI_sponsor_connectivity_data_t *sponsor_connectivity_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(sponsor_connectivity_data == NULL) {
		ogs_error(
			"OpenAPI_sponsor_connectivity_data_convertToJSON() failed [SponsorConnectivityData]");
		return NULL;
	}
	...
	if(!sponsor_connectivity_data->asp_ids) {
		ogs_error(
			"OpenAPI_sponsor_connectivity_data_convertToJSON() failed [asp_ids]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(sponsor_connectivity_data->asp_ids, node) {
		if(cJSON_AddStringToObject(asp_idsList, "",
		                           (char *) node->data) == NULL)
		{
			ogs_error(
				"OpenAPI_sponsor_connectivity_data_convertToJSON() failed [asp_ids]");
...
OpenAPI_sponsor_connectivity_data_parseFromJSON(
	cJSON *sponsor_connectivity_dataJSON) {
	OpenAPI_sponsor_connectivity_data_t *sponsor_connectivity_data_local_var
	        = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *asp_ids = NULL;
	...
	asp_ids =
		cJSON_GetObjectItemCaseSensitive(sponsor_connectivity_dataJSON,
		                                 "aspIds");
	if(!asp_ids) {
		ogs_error(
			"OpenAPI_sponsor_connectivity_data_parseFromJSON() failed [asp_ids]");
		goto end;
	}
	...
	cJSON_ArrayForEach(asp_ids_local, asp_ids) {
		...
		OpenAPI_list_add(asp_idsList,
		                 ogs_strdup(asp_ids_local->valuestring));
	}

	sponsor_connectivity_data_local_var =
		OpenAPI_sponsor_connectivity_data_create(
			asp_idsList
			);
...
OpenAPI_sponsor_connectivity_data_t *OpenAPI_sponsor_connectivity_data_copy(
	OpenAPI_sponsor_connectivity_data_t	*dst,
	OpenAPI_sponsor_connectivity_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_sponsor_connectivity_data_convertToJSON() failed");
		return NULL;
	}
