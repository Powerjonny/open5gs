...
{
	OpenAPI_nsag_info_t *nsag_info_local_var =
		ogs_malloc(sizeof(OpenAPI_nsag_info_t));
...
cJSON *OpenAPI_nsag_info_convertToJSON(OpenAPI_nsag_info_t *nsag_info) {
	...
	if(nsag_info == NULL) {
		ogs_error(
			"OpenAPI_nsag_info_convertToJSON() failed [NsagInfo]");
		return NULL;
	}
	...
	if(!nsag_info->nsag_ids) {
		ogs_error(
			"OpenAPI_nsag_info_convertToJSON() failed [nsag_ids]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(nsag_info->nsag_ids, node) {
		...
		if(cJSON_AddNumberToObject(nsag_idsList, "",
	...
	if(!nsag_info->snssai_list) {
		ogs_error(
			"OpenAPI_nsag_info_convertToJSON() failed [snssai_list]");
		return NULL;
	}
	...
	if(nsag_info->tai_list) {
		...
		if(tai_listList == NULL) {
			ogs_error(
				"OpenAPI_nsag_info_convertToJSON() failed [tai_list]");
			goto end;
		}
		OpenAPI_list_for_each(nsag_info->tai_list, node) {
			cJSON *itemLocal =
	...
	if(nsag_info->tai_range_list) {
		...
		if(tai_range_listList == NULL) {
			ogs_error(
				"OpenAPI_nsag_info_convertToJSON() failed [tai_range_list]");
			goto end;
		}
		OpenAPI_list_for_each(nsag_info->tai_range_list, node) {
			cJSON *itemLocal =
...
OpenAPI_nsag_info_t *OpenAPI_nsag_info_parseFromJSON(cJSON *nsag_infoJSON) {
	...
	if(!nsag_ids) {
		ogs_error(
			"OpenAPI_nsag_info_parseFromJSON() failed [nsag_ids]");
		goto end;
	}
	...
	snssai_list = cJSON_GetObjectItemCaseSensitive(nsag_infoJSON,
	                                               "snssaiList");
	if(!snssai_list) {
		ogs_error(
			"OpenAPI_nsag_info_parseFromJSON() failed [snssai_list]");
		goto end;
	}
	...
	if(tai_list) {
		...
		if(!cJSON_IsArray(tai_list)) {
			ogs_error(
				"OpenAPI_nsag_info_parseFromJSON() failed [tai_list]");
			goto end;
		}
	...
	tai_range_list = cJSON_GetObjectItemCaseSensitive(nsag_infoJSON,
	                                                  "taiRangeList");
	if(tai_range_list) {
		cJSON *tai_range_list_local = NULL;
		if(!cJSON_IsArray(tai_range_list)) {
			ogs_error(
				"OpenAPI_nsag_info_parseFromJSON() failed [tai_range_list]");
			goto end;
		}
