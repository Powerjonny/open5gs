...
OpenAPI_location_accuracy_t *OpenAPI_location_accuracy_create() {
	OpenAPI_location_accuracy_t *location_accuracy_local_var =
		ogs_malloc(sizeof(OpenAPI_location_accuracy_t));
...
cJSON *OpenAPI_location_accuracy_convertToJSON(
	...
	if(location_accuracy == NULL) {
		ogs_error(
			"OpenAPI_location_accuracy_convertToJSON() failed [LocationAccuracy]");
		return NULL;
	}
