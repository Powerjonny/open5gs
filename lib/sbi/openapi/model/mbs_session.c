...
OpenAPI_mbs_session_t *OpenAPI_mbs_session_create(
	...
	OpenAPI_mbs_session_t *mbs_session_local_var =
		ogs_malloc(sizeof(OpenAPI_mbs_session_t));
...
void OpenAPI_mbs_session_free(OpenAPI_mbs_session_t *mbs_session) {
	...
	if(mbs_session->mbs_area_sessions) {
		OpenAPI_list_for_each(mbs_session->mbs_area_sessions, node) {
			...
			OpenAPI_mbs_service_area_info_free(
				localKeyValue->value);
...
cJSON *OpenAPI_mbs_session_convertToJSON(OpenAPI_mbs_session_t *mbs_session) {
	...
	if(mbs_session == NULL) {
		ogs_error(
			"OpenAPI_mbs_session_convertToJSON() failed [MbsSession]");
		return NULL;
	}
	...
	if(!mbs_session->mbs_session_id) {
		ogs_error(
			"OpenAPI_mbs_session_convertToJSON() failed [mbs_session_id]");
		return NULL;
	}
	cJSON *mbs_session_id_local_JSON =
		OpenAPI_mbs_session_id_convertToJSON(
			mbs_session->mbs_session_id);
	...
	if(mbs_session->mbs_area_sessions) {
		cJSON *mbs_area_sessions = cJSON_AddObjectToObject(item,
		                                                   "mbsAreaSessions");
		if(mbs_area_sessions == NULL) {
			ogs_error(
				"OpenAPI_mbs_session_convertToJSON() failed [mbs_area_sessions]");
			goto end;
		}
		cJSON *localMapObject = mbs_area_sessions;
		if(mbs_session->mbs_area_sessions) {
			OpenAPI_list_for_each(mbs_session->mbs_area_sessions,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_mbs_service_area_info_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_mbs_session_convertToJSON() failed [inner]");
					goto end;
				}
...
OpenAPI_mbs_session_t *OpenAPI_mbs_session_parseFromJSON(
	...
	mbs_session_id = cJSON_GetObjectItemCaseSensitive(mbs_sessionJSON,
	                                                  "mbsSessionId");
	if(!mbs_session_id) {
		ogs_error(
			"OpenAPI_mbs_session_parseFromJSON() failed [mbs_session_id]");
		goto end;
	}
	...
	mbs_area_sessions = cJSON_GetObjectItemCaseSensitive(mbs_sessionJSON,
	                                                     "mbsAreaSessions");
	if(mbs_area_sessions) {
		cJSON *mbs_area_sessions_local_map = NULL;
		if(!cJSON_IsObject(mbs_area_sessions) &&
		   !cJSON_IsNull(mbs_area_sessions))
		{
			ogs_error(
				"OpenAPI_mbs_session_parseFromJSON() failed [mbs_area_sessions]");
			goto end;
		}
		if(cJSON_IsObject(mbs_area_sessions)) {
			mbs_area_sessionsList = OpenAPI_list_create();
			OpenAPI_map_t *localMapKeyPair = NULL;
			cJSON_ArrayForEach(mbs_area_sessions_local_map,
			                   mbs_area_sessions) {
				cJSON *localMapObject =
					mbs_area_sessions_local_map;
				if(cJSON_IsObject(localMapObject)) {
					localMapKeyPair = OpenAPI_map_create(
						ogs_strdup(
							localMapObject->string),
						OpenAPI_mbs_service_area_info_parseFromJSON
						        (localMapObject));
					...
					ogs_error(
						"OpenAPI_mbs_session_parseFromJSON() failed [inner]");
					goto end;
				}
				OpenAPI_list_add(mbs_area_sessionsList,
				                 localMapKeyPair);
			}
		}
	}
