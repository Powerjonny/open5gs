...
OpenAPI_ip_address_t *OpenAPI_ip_address_create(char	*ipv4_addr,
                                                char	*ipv6_addr,
                                                char	*ipv6_prefix) {
	OpenAPI_ip_address_t *ip_address_local_var =
		ogs_malloc(sizeof(OpenAPI_ip_address_t));
...
cJSON *OpenAPI_ip_address_convertToJSON(OpenAPI_ip_address_t *ip_address) {
	...
	if(ip_address == NULL) {
		ogs_error(
			"OpenAPI_ip_address_convertToJSON() failed [IpAddress]");
		return NULL;
	}
	...
	if(ip_address->ipv4_addr) {
		...
		{
			ogs_error(
				"OpenAPI_ip_address_convertToJSON() failed [ipv4_addr]");
			goto end;
		}
	}
	...
	if(ip_address->ipv6_addr) {
		...
		{
			ogs_error(
				"OpenAPI_ip_address_convertToJSON() failed [ipv6_addr]");
			goto end;
		}
	}
	...
	if(ip_address->ipv6_prefix) {
		...
		{
			ogs_error(
				"OpenAPI_ip_address_convertToJSON() failed [ipv6_prefix]");
			goto end;
		}
	}
...
OpenAPI_ip_address_t *OpenAPI_ip_address_parseFromJSON(cJSON *ip_addressJSON) {
	...
	ipv4_addr = cJSON_GetObjectItemCaseSensitive(ip_addressJSON,
	                                             "ipv4Addr");
	if(ipv4_addr) {
		...
		{
			ogs_error(
				"OpenAPI_ip_address_parseFromJSON() failed [ipv4_addr]");
			goto end;
		}
	}

	ipv6_addr = cJSON_GetObjectItemCaseSensitive(ip_addressJSON,
	                                             "ipv6Addr");
	if(ipv6_addr) {
		...
		{
			ogs_error(
				"OpenAPI_ip_address_parseFromJSON() failed [ipv6_addr]");
			goto end;
		}
	}
	...
	if(ipv6_prefix) {
		...
		{
			ogs_error(
				"OpenAPI_ip_address_parseFromJSON() failed [ipv6_prefix]");
			goto end;
		}
	}
