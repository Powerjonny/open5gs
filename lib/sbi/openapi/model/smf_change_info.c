...
OpenAPI_smf_change_info_t *OpenAPI_smf_change_info_create(
	...
	OpenAPI_smf_change_info_t *smf_change_info_local_var =
		ogs_malloc(sizeof(OpenAPI_smf_change_info_t));
...
void OpenAPI_smf_change_info_free(OpenAPI_smf_change_info_t *smf_change_info) {
	...
	if(smf_change_info->pdu_session_id_list) {
		OpenAPI_list_for_each(smf_change_info->pdu_session_id_list,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_smf_change_info_convertToJSON(
	OpenAPI_smf_change_info_t *smf_change_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(smf_change_info == NULL) {
		ogs_error(
			"OpenAPI_smf_change_info_convertToJSON() failed [SmfChangeInfo]");
		return NULL;
	}
	...
	if(!smf_change_info->pdu_session_id_list) {
		ogs_error(
			"OpenAPI_smf_change_info_convertToJSON() failed [pdu_session_id_list]");
		return NULL;
	}
	...
	if(smf_change_info->smf_change_ind ==
	   OpenAPI_smf_change_indication_NULL)
	{
		ogs_error(
			"OpenAPI_smf_change_info_convertToJSON() failed [smf_change_ind]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "smfChangeInd",
	                           OpenAPI_smf_change_indication_ToString(
...
OpenAPI_smf_change_info_t *OpenAPI_smf_change_info_parseFromJSON(
	cJSON *smf_change_infoJSON) {
	...
	pdu_session_id_list =
		cJSON_GetObjectItemCaseSensitive(smf_change_infoJSON,
		                                 "pduSessionIdList");
	if(!pdu_session_id_list) {
		ogs_error(
			"OpenAPI_smf_change_info_parseFromJSON() failed [pdu_session_id_list]");
		goto end;
	}
	...
	if(!smf_change_ind) {
		ogs_error(
			"OpenAPI_smf_change_info_parseFromJSON() failed [smf_change_ind]");
		goto end;
	}
	...
	smf_change_indVariable =
		OpenAPI_smf_change_indication_FromString(
...
OpenAPI_smf_change_info_t *OpenAPI_smf_change_info_copy(
	OpenAPI_smf_change_info_t *dst, OpenAPI_smf_change_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
