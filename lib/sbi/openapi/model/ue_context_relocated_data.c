...
cJSON *OpenAPI_ue_context_relocated_data_convertToJSON(
	...
	if(ue_context_relocated_data == NULL) {
		ogs_error(
			"OpenAPI_ue_context_relocated_data_convertToJSON() failed [UeContextRelocatedData]");
		return NULL;
	}
	...
	if(!ue_context_relocated_data->ue_context) {
		ogs_error(
			"OpenAPI_ue_context_relocated_data_convertToJSON() failed [ue_context]");
		return NULL;
	}
...
OpenAPI_ue_context_relocated_data_parseFromJSON(
	...
	ue_context =
		cJSON_GetObjectItemCaseSensitive(ue_context_relocated_dataJSON,
		                                 "ueContext");
	if(!ue_context) {
		ogs_error(
			"OpenAPI_ue_context_relocated_data_parseFromJSON() failed [ue_context]");
		goto end;
	}
...
OpenAPI_ue_context_relocated_data_t *OpenAPI_ue_context_relocated_data_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ue_context_relocated_data_convertToJSON() failed");
		return NULL;
	}
