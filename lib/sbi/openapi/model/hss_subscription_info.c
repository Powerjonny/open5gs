...
OpenAPI_hss_subscription_info_t *OpenAPI_hss_subscription_info_create(
	...
	OpenAPI_hss_subscription_info_t *hss_subscription_info_local_var =
		ogs_malloc(sizeof(OpenAPI_hss_subscription_info_t));
	ogs_assert(hss_subscription_info_local_var);

	hss_subscription_info_local_var->hss_subscription_list =
		hss_subscription_list;

	return hss_subscription_info_local_var;
}

void OpenAPI_hss_subscription_info_free(
	...
	if(hss_subscription_info->hss_subscription_list) {
		OpenAPI_list_for_each(
			hss_subscription_info->hss_subscription_list, node) {
...
cJSON *OpenAPI_hss_subscription_info_convertToJSON(
	OpenAPI_hss_subscription_info_t *hss_subscription_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(hss_subscription_info == NULL) {
		ogs_error(
			"OpenAPI_hss_subscription_info_convertToJSON() failed [HssSubscriptionInfo]");
		return NULL;
	}
	...
	if(!hss_subscription_info->hss_subscription_list) {
		ogs_error(
			"OpenAPI_hss_subscription_info_convertToJSON() failed [hss_subscription_list]");
		return NULL;
	}
	cJSON *hss_subscription_listList = cJSON_AddArrayToObject(item,
	                                                          "hssSubscriptionList");
	...
	OpenAPI_list_for_each(hss_subscription_info->hss_subscription_list,
	                      node) {
		cJSON *itemLocal =
			OpenAPI_hss_subscription_item_convertToJSON(node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_hss_subscription_info_convertToJSON() failed [hss_subscription_list]");
			goto end;
...
OpenAPI_hss_subscription_info_t *OpenAPI_hss_subscription_info_parseFromJSON(
	cJSON *hss_subscription_infoJSON) {
	...
	hss_subscription_list =
		cJSON_GetObjectItemCaseSensitive(hss_subscription_infoJSON,
		                                 "hssSubscriptionList");
	if(!hss_subscription_list) {
		ogs_error(
			"OpenAPI_hss_subscription_info_parseFromJSON() failed [hss_subscription_list]");
		goto end;
	}
	...
	cJSON_ArrayForEach(hss_subscription_list_local, hss_subscription_list) {
		...
		OpenAPI_hss_subscription_item_t *hss_subscription_listItem =
			OpenAPI_hss_subscription_item_parseFromJSON(
				hss_subscription_list_local);
		if(!hss_subscription_listItem) {
			ogs_error("No hss_subscription_listItem");
			goto end;
		}
		OpenAPI_list_add(hss_subscription_listList,
		                 hss_subscription_listItem);
	}
...
OpenAPI_hss_subscription_info_t *OpenAPI_hss_subscription_info_copy(
	OpenAPI_hss_subscription_info_t *dst,
	OpenAPI_hss_subscription_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_hss_subscription_info_convertToJSON() failed");
		return NULL;
	}
