...
OpenAPI_npn_access_info_t *OpenAPI_npn_access_info_create(
	...
	OpenAPI_npn_access_info_t *npn_access_info_local_var =
		ogs_malloc(sizeof(OpenAPI_npn_access_info_t));
...
cJSON *OpenAPI_npn_access_info_convertToJSON(
	...
	if(npn_access_info == NULL) {
		ogs_error(
			"OpenAPI_npn_access_info_convertToJSON() failed [NpnAccessInfo]");
		return NULL;
	}
	...
	if(npn_access_info->cell_cag_info) {
		...
		if(cell_cag_infoList == NULL) {
			ogs_error(
				"OpenAPI_npn_access_info_convertToJSON() failed [cell_cag_info]");
			goto end;
		}
		OpenAPI_list_for_each(npn_access_info->cell_cag_info, node) {
...
OpenAPI_npn_access_info_t *OpenAPI_npn_access_info_parseFromJSON(
	cJSON *npn_access_infoJSON) {
	...
	if(cell_cag_info) {
		...
		if(!cJSON_IsArray(cell_cag_info)) {
			ogs_error(
				"OpenAPI_npn_access_info_parseFromJSON() failed [cell_cag_info]");
			goto end;
		}
...
OpenAPI_npn_access_info_t *OpenAPI_npn_access_info_copy(
	OpenAPI_npn_access_info_t *dst, OpenAPI_npn_access_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
