...
char *OpenAPI_wildcard_sdsnssai_extension_ToString(
...
OpenAPI_wildcard_sdsnssai_extension_FromString(char *wildcard_sd) {
...
OpenAPI_snssai_extension_t *OpenAPI_snssai_extension_create(
	...
	OpenAPI_snssai_extension_wildcard_sd_e	wildcard_sd) {
	OpenAPI_snssai_extension_t *snssai_extension_local_var =
		ogs_malloc(sizeof(OpenAPI_snssai_extension_t));
...
void OpenAPI_snssai_extension_free(
	OpenAPI_snssai_extension_t *snssai_extension) {
...
cJSON *OpenAPI_snssai_extension_convertToJSON(
	OpenAPI_snssai_extension_t *snssai_extension) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(snssai_extension == NULL) {
		ogs_error(
			"OpenAPI_snssai_extension_convertToJSON() failed [SnssaiExtension]");
		return NULL;
	}
	...
	if(snssai_extension->sd_ranges) {
		...
		if(sd_rangesList == NULL) {
			ogs_error(
				"OpenAPI_snssai_extension_convertToJSON() failed [sd_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(snssai_extension->sd_ranges, node) {
			cJSON *itemLocal =
				OpenAPI_sd_range_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_snssai_extension_convertToJSON() failed [sd_ranges]");
				goto end;
	...
	if(snssai_extension->wildcard_sd !=
	   OpenAPI_snssai_extension_WILDCARDSD_NULL)
	{
		if(cJSON_AddStringToObject(item, "wildcardSd",
		                           OpenAPI_wildcard_sdsnssai_extension_ToString
		                                   (snssai_extension->
		                                   wildcard_sd)) == NULL)
		{
			ogs_error(
				"OpenAPI_snssai_extension_convertToJSON() failed [wildcard_sd]");
			goto end;
		}
	}
...
OpenAPI_snssai_extension_t *OpenAPI_snssai_extension_parseFromJSON(
	cJSON *snssai_extensionJSON) {
	...
	if(sd_ranges) {
		...
		if(!cJSON_IsArray(sd_ranges)) {
			ogs_error(
				"OpenAPI_snssai_extension_parseFromJSON() failed [sd_ranges]");
			goto end;
		}
		...
		cJSON_ArrayForEach(sd_ranges_local, sd_ranges) {
			...
			OpenAPI_sd_range_t *sd_rangesItem =
				OpenAPI_sd_range_parseFromJSON(sd_ranges_local);
			if(!sd_rangesItem) {
	...
	if(wildcard_sd) {
		if(!cJSON_IsString(wildcard_sd)) {
			ogs_error(
				"OpenAPI_snssai_extension_parseFromJSON() failed [wildcard_sd]");
			goto end;
		}
		wildcard_sdVariable =
			OpenAPI_wildcard_sdsnssai_extension_FromString(
...
OpenAPI_snssai_extension_t *OpenAPI_snssai_extension_copy(
	OpenAPI_snssai_extension_t *dst, OpenAPI_snssai_extension_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
