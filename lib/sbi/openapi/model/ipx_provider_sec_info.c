...
OpenAPI_ipx_provider_sec_info_t *OpenAPI_ipx_provider_sec_info_create(
	char *ipx_provider_id, OpenAPI_list_t *raw_public_key_list,
	OpenAPI_list_t *certificate_list) {
	OpenAPI_ipx_provider_sec_info_t *ipx_provider_sec_info_local_var =
		ogs_malloc(sizeof(OpenAPI_ipx_provider_sec_info_t));
	ogs_assert(ipx_provider_sec_info_local_var);

	ipx_provider_sec_info_local_var->ipx_provider_id = ipx_provider_id;
	ipx_provider_sec_info_local_var->raw_public_key_list =
		raw_public_key_list;
...
void OpenAPI_ipx_provider_sec_info_free(
	OpenAPI_ipx_provider_sec_info_t *ipx_provider_sec_info) {
	...
	if(ipx_provider_sec_info->raw_public_key_list) {
		OpenAPI_list_for_each(
			ipx_provider_sec_info->raw_public_key_list, node) {
			ogs_free(node->data);
		}
	...
	if(ipx_provider_sec_info->certificate_list) {
		OpenAPI_list_for_each(ipx_provider_sec_info->certificate_list,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_ipx_provider_sec_info_convertToJSON(
	OpenAPI_ipx_provider_sec_info_t *ipx_provider_sec_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(ipx_provider_sec_info == NULL) {
		ogs_error(
			"OpenAPI_ipx_provider_sec_info_convertToJSON() failed [IpxProviderSecInfo]");
		return NULL;
	}
	...
	if(!ipx_provider_sec_info->ipx_provider_id) {
		ogs_error(
			"OpenAPI_ipx_provider_sec_info_convertToJSON() failed [ipx_provider_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "ipxProviderId",
	                           ipx_provider_sec_info->ipx_provider_id) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_ipx_provider_sec_info_convertToJSON() failed [ipx_provider_id]");
	...
	if(ipx_provider_sec_info->raw_public_key_list) {
		cJSON *raw_public_key_listList = cJSON_AddArrayToObject(item,
		                                                        "rawPublicKeyList");
		if(raw_public_key_listList == NULL) {
			ogs_error(
				"OpenAPI_ipx_provider_sec_info_convertToJSON() failed [raw_public_key_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			ipx_provider_sec_info->raw_public_key_list, node) {
			if(cJSON_AddStringToObject(raw_public_key_listList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_ipx_provider_sec_info_convertToJSON() failed [raw_public_key_list]");
	...
	if(ipx_provider_sec_info->certificate_list) {
		cJSON *certificate_listList = cJSON_AddArrayToObject(item,
		                                                     "certificateList");
		if(certificate_listList == NULL) {
			ogs_error(
				"OpenAPI_ipx_provider_sec_info_convertToJSON() failed [certificate_list]");
			goto end;
		}
		OpenAPI_list_for_each(ipx_provider_sec_info->certificate_list,
		                      node) {
			if(cJSON_AddStringToObject(certificate_listList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_ipx_provider_sec_info_convertToJSON() failed [certificate_list]");
...
OpenAPI_ipx_provider_sec_info_t *OpenAPI_ipx_provider_sec_info_parseFromJSON(
	cJSON *ipx_provider_sec_infoJSON) {
	...
	ipx_provider_id =
		cJSON_GetObjectItemCaseSensitive(ipx_provider_sec_infoJSON,
		                                 "ipxProviderId");
	if(!ipx_provider_id) {
		ogs_error(
			"OpenAPI_ipx_provider_sec_info_parseFromJSON() failed [ipx_provider_id]");
		goto end;
	}
	...
	if(raw_public_key_list) {
		...
		if(!cJSON_IsArray(raw_public_key_list)) {
			ogs_error(
				"OpenAPI_ipx_provider_sec_info_parseFromJSON() failed [raw_public_key_list]");
			goto end;
		}

		raw_public_key_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(raw_public_key_list_local,
		                   raw_public_key_list) {
			...
			OpenAPI_list_add(raw_public_key_listList,
			                 ogs_strdup(
						 raw_public_key_list_local->
						 valuestring));
		}
	}
	...
	if(certificate_list) {
		...
		if(!cJSON_IsArray(certificate_list)) {
			ogs_error(
				"OpenAPI_ipx_provider_sec_info_parseFromJSON() failed [certificate_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(certificate_list_local, certificate_list) {
			...
			OpenAPI_list_add(certificate_listList,
			                 ogs_strdup(
						 certificate_list_local->
						 valuestring));
		}
	}
...
OpenAPI_ipx_provider_sec_info_t *OpenAPI_ipx_provider_sec_info_copy(
	OpenAPI_ipx_provider_sec_info_t *dst,
	OpenAPI_ipx_provider_sec_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ipx_provider_sec_info_convertToJSON() failed");
		return NULL;
	}
