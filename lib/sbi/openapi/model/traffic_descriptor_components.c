...
OpenAPI_traffic_descriptor_components_create(OpenAPI_list_t	*app_descs,
	...
	OpenAPI_traffic_descriptor_components_t *
	        traffic_descriptor_components_local_var =
		ogs_malloc(sizeof(OpenAPI_traffic_descriptor_components_t));
	...
	traffic_descriptor_components_local_var->eth_flow_descs =
		eth_flow_descs;
...
void OpenAPI_traffic_descriptor_components_free(
	OpenAPI_traffic_descriptor_components_t *traffic_descriptor_components)
{
	...
	if(traffic_descriptor_components->app_descs) {
		OpenAPI_list_for_each(traffic_descriptor_components->app_descs,
		                      node) {
	...
	if(traffic_descriptor_components->flow_descs) {
		OpenAPI_list_for_each(traffic_descriptor_components->flow_descs,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(traffic_descriptor_components->domain_descs) {
		OpenAPI_list_for_each(
			traffic_descriptor_components->domain_descs, node) {
			ogs_free(node->data);
		}
	...
	if(traffic_descriptor_components->eth_flow_descs) {
		OpenAPI_list_for_each(
			traffic_descriptor_components->eth_flow_descs, node) {
			OpenAPI_eth_flow_description_free(node->data);
		}
		OpenAPI_list_free(
			traffic_descriptor_components->eth_flow_descs);
		traffic_descriptor_components->eth_flow_descs = NULL;
	}
	if(traffic_descriptor_components->dnns) {
		OpenAPI_list_for_each(traffic_descriptor_components->dnns,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(traffic_descriptor_components->conn_caps) {
		OpenAPI_list_for_each(traffic_descriptor_components->conn_caps,
		                      node) {
...
cJSON *OpenAPI_traffic_descriptor_components_convertToJSON(
	OpenAPI_traffic_descriptor_components_t *traffic_descriptor_components)
{
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(traffic_descriptor_components == NULL) {
		ogs_error(
			"OpenAPI_traffic_descriptor_components_convertToJSON() failed [TrafficDescriptorComponents]");
		return NULL;
	}
	...
	if(traffic_descriptor_components->app_descs) {
		...
		if(app_descs == NULL) {
			ogs_error(
				"OpenAPI_traffic_descriptor_components_convertToJSON() failed [app_descs]");
			goto end;
		}
		cJSON *localMapObject = app_descs;
		if(traffic_descriptor_components->app_descs) {
			OpenAPI_list_for_each(
				traffic_descriptor_components->app_descs,
				node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_app_descriptor_1_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_traffic_descriptor_components_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(traffic_descriptor_components->flow_descs) {
		cJSON *flow_descsList = cJSON_AddArrayToObject(item,
		                                               "flowDescs");
		if(flow_descsList == NULL) {
			ogs_error(
				"OpenAPI_traffic_descriptor_components_convertToJSON() failed [flow_descs]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_descriptor_components->flow_descs,
		                      node) {
	...
	if(traffic_descriptor_components->domain_descs) {
		...
		if(domain_descsList == NULL) {
			ogs_error(
				"OpenAPI_traffic_descriptor_components_convertToJSON() failed [domain_descs]");
			goto end;
		}
		OpenAPI_list_for_each(
			traffic_descriptor_components->domain_descs, node) {
	...
	if(traffic_descriptor_components->eth_flow_descs) {
		cJSON *eth_flow_descsList = cJSON_AddArrayToObject(item,
		                                                   "ethFlowDescs");
		if(eth_flow_descsList == NULL) {
			ogs_error(
				"OpenAPI_traffic_descriptor_components_convertToJSON() failed [eth_flow_descs]");
			goto end;
		}
		OpenAPI_list_for_each(
			traffic_descriptor_components->eth_flow_descs, node) {
	...
	if(traffic_descriptor_components->dnns) {
		...
		if(dnnsList == NULL) {
			ogs_error(
				"OpenAPI_traffic_descriptor_components_convertToJSON() failed [dnns]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_descriptor_components->dnns,
		                      node) {
	...
	if(traffic_descriptor_components->conn_caps) {
		...
		if(conn_capsList == NULL) {
			ogs_error(
				"OpenAPI_traffic_descriptor_components_convertToJSON() failed [conn_caps]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_descriptor_components->conn_caps,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_connection_capabilities_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_traffic_descriptor_components_convertToJSON() failed [conn_caps]");
...
OpenAPI_traffic_descriptor_components_parseFromJSON(
	cJSON *traffic_descriptor_componentsJSON) {
	OpenAPI_traffic_descriptor_components_t *
	        traffic_descriptor_components_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	app_descs =
		cJSON_GetObjectItemCaseSensitive(
			traffic_descriptor_componentsJSON, "appDescs");
	if(app_descs) {
		cJSON *app_descs_local_map = NULL;
		if(!cJSON_IsObject(app_descs) &&
		   !cJSON_IsNull(app_descs))
		{
			ogs_error(
				"OpenAPI_traffic_descriptor_components_parseFromJSON() failed [app_descs]");
			goto end;
		}
		if(cJSON_IsObject(app_descs)) {
			...
			cJSON_ArrayForEach(app_descs_local_map, app_descs) {
				...
				if(cJSON_IsObject(localMapObject)) {
					localMapKeyPair = OpenAPI_map_create(
						...
						OpenAPI_app_descriptor_1_parseFromJSON
						        (localMapObject));
					...
					ogs_error(
						"OpenAPI_traffic_descriptor_components_parseFromJSON() failed [inner]");
					goto end;
				}
				OpenAPI_list_add(app_descsList,
				                 localMapKeyPair);
			}
		}
	}
	...
	flow_descs =
		cJSON_GetObjectItemCaseSensitive(
			traffic_descriptor_componentsJSON, "flowDescs");
	if(flow_descs) {
		cJSON *flow_descs_local = NULL;
		if(!cJSON_IsArray(flow_descs)) {
			ogs_error(
				"OpenAPI_traffic_descriptor_components_parseFromJSON() failed [flow_descs]");
			goto end;
		}
		...
		cJSON_ArrayForEach(flow_descs_local, flow_descs) {
			...
			OpenAPI_list_add(flow_descsList,
			                 ogs_strdup(
						 flow_descs_local->valuestring));
		}
	}

	domain_descs =
		cJSON_GetObjectItemCaseSensitive(
			traffic_descriptor_componentsJSON, "domainDescs");
	if(domain_descs) {
		cJSON *domain_descs_local = NULL;
		if(!cJSON_IsArray(domain_descs)) {
			ogs_error(
				"OpenAPI_traffic_descriptor_components_parseFromJSON() failed [domain_descs]");
			goto end;
		}
		...
		cJSON_ArrayForEach(domain_descs_local, domain_descs) {
			...
			OpenAPI_list_add(domain_descsList,
			                 ogs_strdup(
						 domain_descs_local->valuestring));
		}
	}

	eth_flow_descs =
		cJSON_GetObjectItemCaseSensitive(
			traffic_descriptor_componentsJSON, "ethFlowDescs");
	if(eth_flow_descs) {
		cJSON *eth_flow_descs_local = NULL;
		if(!cJSON_IsArray(eth_flow_descs)) {
			ogs_error(
				"OpenAPI_traffic_descriptor_components_parseFromJSON() failed [eth_flow_descs]");
			goto end;
		}
		...
		cJSON_ArrayForEach(eth_flow_descs_local, eth_flow_descs) {
			...
			OpenAPI_eth_flow_description_t *eth_flow_descsItem =
				OpenAPI_eth_flow_description_parseFromJSON(
					eth_flow_descs_local);
			if(!eth_flow_descsItem) {
				ogs_error("No eth_flow_descsItem");
				goto end;
			}
			OpenAPI_list_add(eth_flow_descsList,
			                 eth_flow_descsItem);
		}
	}
	...
	dnns =
		cJSON_GetObjectItemCaseSensitive(
			traffic_descriptor_componentsJSON, "dnns");
	if(dnns) {
		cJSON *dnns_local = NULL;
		if(!cJSON_IsArray(dnns)) {
			ogs_error(
				"OpenAPI_traffic_descriptor_components_parseFromJSON() failed [dnns]");
			goto end;
		}
	...
	conn_caps =
		cJSON_GetObjectItemCaseSensitive(
			traffic_descriptor_componentsJSON, "connCaps");
	if(conn_caps) {
		cJSON *conn_caps_local = NULL;
		if(!cJSON_IsArray(conn_caps)) {
			ogs_error(
				"OpenAPI_traffic_descriptor_components_parseFromJSON() failed [conn_caps]");
			goto end;
		}
		...
		cJSON_ArrayForEach(conn_caps_local, conn_caps) {
			...
			OpenAPI_connection_capabilities_t *conn_capsItem =
				OpenAPI_connection_capabilities_parseFromJSON(
					conn_caps_local);
			if(!conn_capsItem) {
...
OpenAPI_traffic_descriptor_components_t *
OpenAPI_traffic_descriptor_components_copy(
	OpenAPI_traffic_descriptor_components_t *dst,
	OpenAPI_traffic_descriptor_components_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_traffic_descriptor_components_convertToJSON() failed");
		return NULL;
	}
