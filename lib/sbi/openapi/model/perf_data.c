...
OpenAPI_perf_data_t *OpenAPI_perf_data_create(char	*avg_traffic_rate,
	...
	OpenAPI_perf_data_t *perf_data_local_var =
		ogs_malloc(sizeof(OpenAPI_perf_data_t));
...
cJSON *OpenAPI_perf_data_convertToJSON(OpenAPI_perf_data_t *perf_data) {
	...
	if(perf_data == NULL) {
		ogs_error(
			"OpenAPI_perf_data_convertToJSON() failed [PerfData]");
		return NULL;
	}
	...
	if(perf_data->avg_traffic_rate) {
		...
		{
			ogs_error(
				"OpenAPI_perf_data_convertToJSON() failed [avg_traffic_rate]");
			goto end;
		}
	}
	...
	if(perf_data->max_traffic_rate) {
		...
		{
			ogs_error(
				"OpenAPI_perf_data_convertToJSON() failed [max_traffic_rate]");
			goto end;
		}
	}
	...
	if(perf_data->is_ave_packet_delay) {
		...
		{
			ogs_error(
				"OpenAPI_perf_data_convertToJSON() failed [ave_packet_delay]");
			goto end;
		}
	}
	...
	if(perf_data->is_max_packet_delay) {
		...
		{
			ogs_error(
				"OpenAPI_perf_data_convertToJSON() failed [max_packet_delay]");
			goto end;
		}
	}
	...
	if(perf_data->is_avg_packet_loss_rate) {
		...
		{
			ogs_error(
				"OpenAPI_perf_data_convertToJSON() failed [avg_packet_loss_rate]");
			goto end;
		}
	}
...
OpenAPI_perf_data_t *OpenAPI_perf_data_parseFromJSON(cJSON *perf_dataJSON) {
	...
	if(avg_traffic_rate) {
		...
		{
			ogs_error(
				"OpenAPI_perf_data_parseFromJSON() failed [avg_traffic_rate]");
			goto end;
		}
	}
	...
	if(max_traffic_rate) {
		...
		{
			ogs_error(
				"OpenAPI_perf_data_parseFromJSON() failed [max_traffic_rate]");
			goto end;
		}
	}
	...
	if(ave_packet_delay) {
		if(!cJSON_IsNumber(ave_packet_delay)) {
			ogs_error(
				"OpenAPI_perf_data_parseFromJSON() failed [ave_packet_delay]");
			goto end;
		}
	}
	...
	if(max_packet_delay) {
		if(!cJSON_IsNumber(max_packet_delay)) {
			ogs_error(
				"OpenAPI_perf_data_parseFromJSON() failed [max_packet_delay]");
			goto end;
		}
	}
	...
	if(avg_packet_loss_rate) {
		if(!cJSON_IsNumber(avg_packet_loss_rate)) {
			ogs_error(
				"OpenAPI_perf_data_parseFromJSON() failed [avg_packet_loss_rate]");
			goto end;
		}
	}
...
OpenAPI_perf_data_t *OpenAPI_perf_data_copy(OpenAPI_perf_data_t *dst,
                                            OpenAPI_perf_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
