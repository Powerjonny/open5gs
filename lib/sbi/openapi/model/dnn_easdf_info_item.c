...
OpenAPI_dnn_easdf_info_item_t *OpenAPI_dnn_easdf_info_item_create(
	...
	OpenAPI_list_t
	        *dnai_list) {
	OpenAPI_dnn_easdf_info_item_t *dnn_easdf_info_item_local_var =
		ogs_malloc(sizeof(OpenAPI_dnn_easdf_info_item_t));
...
void OpenAPI_dnn_easdf_info_item_free(
	OpenAPI_dnn_easdf_info_item_t *dnn_easdf_info_item) {
...
cJSON *OpenAPI_dnn_easdf_info_item_convertToJSON(
	OpenAPI_dnn_easdf_info_item_t *dnn_easdf_info_item) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(dnn_easdf_info_item == NULL) {
		ogs_error(
			"OpenAPI_dnn_easdf_info_item_convertToJSON() failed [DnnEasdfInfoItem]");
		return NULL;
	}
	...
	if(!dnn_easdf_info_item->dnn) {
		ogs_error(
			"OpenAPI_dnn_easdf_info_item_convertToJSON() failed [dnn]");
		return NULL;
	}
	...
	if(dnn_easdf_info_item->dnai_list) {
		...
		if(dnai_listList == NULL) {
			ogs_error(
				"OpenAPI_dnn_easdf_info_item_convertToJSON() failed [dnai_list]");
			goto end;
		}
		OpenAPI_list_for_each(dnn_easdf_info_item->dnai_list, node) {
...
OpenAPI_dnn_easdf_info_item_t *OpenAPI_dnn_easdf_info_item_parseFromJSON(
	cJSON *dnn_easdf_info_itemJSON) {
	...
	if(!dnn) {
		ogs_error(
			"OpenAPI_dnn_easdf_info_item_parseFromJSON() failed [dnn]");
		goto end;
	}
	...
	dnai_list = cJSON_GetObjectItemCaseSensitive(dnn_easdf_info_itemJSON,
	                                             "dnaiList");
	if(dnai_list) {
		cJSON *dnai_list_local = NULL;
		if(!cJSON_IsArray(dnai_list)) {
			ogs_error(
				"OpenAPI_dnn_easdf_info_item_parseFromJSON() failed [dnai_list]");
			goto end;
		}
...
OpenAPI_dnn_easdf_info_item_t *OpenAPI_dnn_easdf_info_item_copy(
	OpenAPI_dnn_easdf_info_item_t	*dst,
	OpenAPI_dnn_easdf_info_item_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
