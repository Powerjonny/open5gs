...
OpenAPI_prose_context_t *OpenAPI_prose_context_create(
	OpenAPI_ue_auth_e direct_discovery, OpenAPI_ue_auth_e direct_comm,
	OpenAPI_ue_auth_e l2_relay, OpenAPI_ue_auth_e l3_relay,
	OpenAPI_ue_auth_e l2_remote, char *nr_ue_pc5_ambr,
	OpenAPI_pc5_qo_s_para_t *pc5_qo_s_para) {
	OpenAPI_prose_context_t *prose_context_local_var =
		ogs_malloc(sizeof(OpenAPI_prose_context_t));
...
cJSON *OpenAPI_prose_context_convertToJSON(
	...
	if(prose_context == NULL) {
		ogs_error(
			"OpenAPI_prose_context_convertToJSON() failed [ProseContext]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(prose_context->direct_discovery != OpenAPI_ue_auth_NULL) {
		if(cJSON_AddStringToObject(item, "directDiscovery",
		                           OpenAPI_ue_auth_ToString(
						   prose_context->
						   direct_discovery)) == NULL)
		{
			ogs_error(
				"OpenAPI_prose_context_convertToJSON() failed [direct_discovery]");
			goto end;
		}
	}

	if(prose_context->direct_comm != OpenAPI_ue_auth_NULL) {
		if(cJSON_AddStringToObject(item, "directComm",
		                           OpenAPI_ue_auth_ToString(
						   prose_context->direct_comm))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_prose_context_convertToJSON() failed [direct_comm]");
			goto end;
		}
	}

	if(prose_context->l2_relay != OpenAPI_ue_auth_NULL) {
		if(cJSON_AddStringToObject(item, "l2Relay",
		                           OpenAPI_ue_auth_ToString(
						   prose_context->l2_relay)) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_prose_context_convertToJSON() failed [l2_relay]");
			goto end;
		}
	}

	if(prose_context->l3_relay != OpenAPI_ue_auth_NULL) {
		if(cJSON_AddStringToObject(item, "l3Relay",
		                           OpenAPI_ue_auth_ToString(
						   prose_context->l3_relay)) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_prose_context_convertToJSON() failed [l3_relay]");
			goto end;
		}
	}

	if(prose_context->l2_remote != OpenAPI_ue_auth_NULL) {
		if(cJSON_AddStringToObject(item, "l2Remote",
		                           OpenAPI_ue_auth_ToString(
						   prose_context->l2_remote)) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_prose_context_convertToJSON() failed [l2_remote]");
			goto end;
		}
	}
	...
	if(prose_context->nr_ue_pc5_ambr) {
		...
		{
			ogs_error(
				"OpenAPI_prose_context_convertToJSON() failed [nr_ue_pc5_ambr]");
			goto end;
		}
	}

	if(prose_context->pc5_qo_s_para) {
		cJSON *pc5_qo_s_para_local_JSON =
			OpenAPI_pc5_qo_s_para_convertToJSON(
				prose_context->pc5_qo_s_para);
		if(pc5_qo_s_para_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_prose_context_convertToJSON() failed [pc5_qo_s_para]");
			goto end;
		}
...
OpenAPI_prose_context_t *OpenAPI_prose_context_parseFromJSON(
	cJSON *prose_contextJSON) {
	...
	if(direct_discovery) {
		if(!cJSON_IsString(direct_discovery)) {
			ogs_error(
				"OpenAPI_prose_context_parseFromJSON() failed [direct_discovery]");
			goto end;
		}
		direct_discoveryVariable =
			OpenAPI_ue_auth_FromString(
				direct_discovery->valuestring);
	}
	...
	if(direct_comm) {
		if(!cJSON_IsString(direct_comm)) {
			ogs_error(
				"OpenAPI_prose_context_parseFromJSON() failed [direct_comm]");
			goto end;
		}
		direct_commVariable =
			OpenAPI_ue_auth_FromString(direct_comm->valuestring);
	}

	l2_relay = cJSON_GetObjectItemCaseSensitive(prose_contextJSON,
	                                            "l2Relay");
	if(l2_relay) {
		if(!cJSON_IsString(l2_relay)) {
			ogs_error(
				"OpenAPI_prose_context_parseFromJSON() failed [l2_relay]");
			goto end;
		}
		l2_relayVariable =
			OpenAPI_ue_auth_FromString(l2_relay->valuestring);
	}

	l3_relay = cJSON_GetObjectItemCaseSensitive(prose_contextJSON,
	                                            "l3Relay");
	if(l3_relay) {
		if(!cJSON_IsString(l3_relay)) {
			ogs_error(
				"OpenAPI_prose_context_parseFromJSON() failed [l3_relay]");
			goto end;
		}
		l3_relayVariable =
			OpenAPI_ue_auth_FromString(l3_relay->valuestring);
	}
	...
	if(l2_remote) {
		if(!cJSON_IsString(l2_remote)) {
			ogs_error(
				"OpenAPI_prose_context_parseFromJSON() failed [l2_remote]");
			goto end;
		}
		l2_remoteVariable =
			OpenAPI_ue_auth_FromString(l2_remote->valuestring);
	}
	...
	if(nr_ue_pc5_ambr) {
		...
		{
			ogs_error(
				"OpenAPI_prose_context_parseFromJSON() failed [nr_ue_pc5_ambr]");
			goto end;
		}
	}
	...
	pc5_qo_s_para = cJSON_GetObjectItemCaseSensitive(prose_contextJSON,
	                                                 "pc5QoSPara");
	if(pc5_qo_s_para) {
		pc5_qo_s_para_local_nonprim =
			OpenAPI_pc5_qo_s_para_parseFromJSON(pc5_qo_s_para);
		if(!pc5_qo_s_para_local_nonprim) {
			ogs_error(
				"OpenAPI_pc5_qo_s_para_parseFromJSON failed [pc5_qo_s_para]");
			goto end;
		}
	}
	...
	prose_context_local_var = OpenAPI_prose_context_create(
		...
		nr_ue_pc5_ambr &&
		!cJSON_IsNull(nr_ue_pc5_ambr) ?
		ogs_strdup(nr_ue_pc5_ambr->valuestring) : NULL,
...
OpenAPI_prose_context_t *OpenAPI_prose_context_copy(
	OpenAPI_prose_context_t *dst, OpenAPI_prose_context_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
