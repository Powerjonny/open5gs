...
OpenAPI_registration_context_container_create(
	...
	OpenAPI_user_location_t *user_location, char *rrc_est_cause,
	bool is_ue_context_request, int ue_context_request,
	bool is_initial_amf_n2_ap_id, int initial_amf_n2_ap_id,
	char *an_n2_ipv4_addr, char *an_n2_ipv6_addr,
	...
	OpenAPI_plmn_id_t *selected_plmn_id, bool is_iab_node_ind,
	int iab_node_ind, OpenAPI_ce_mode_b_ind_t *ce_mode_b_ind,
	OpenAPI_lte_m_ind_t *lte_m_ind, bool is_authenticated_ind,
	int authenticated_ind, OpenAPI_npn_access_info_t *npn_access_info) {
	OpenAPI_registration_context_container_t *
	        registration_context_container_local_var =
		ogs_malloc(sizeof(OpenAPI_registration_context_container_t));
	...
	OpenAPI_registration_context_container_t *registration_context_container)
{
	...
	if(registration_context_container->ran_node_id) {
		OpenAPI_global_ran_node_id_free(
	...
	if(registration_context_container->configured_nssai) {
		OpenAPI_list_for_each(
			registration_context_container->configured_nssai,
			node) {
	...
	OpenAPI_registration_context_container_t *registration_context_container)
{
	...
	if(registration_context_container == NULL) {
		ogs_error(
			"OpenAPI_registration_context_container_convertToJSON() failed [RegistrationContextContainer]");
		return NULL;
	}
	...
	if(!registration_context_container->ue_context) {
		ogs_error(
			"OpenAPI_registration_context_container_convertToJSON() failed [ue_context]");
		return NULL;
	}
	cJSON *ue_context_local_JSON =
		OpenAPI_ue_context_convertToJSON(
			registration_context_container->ue_context);
	...
	if(registration_context_container->local_time_zone) {
		...
		{
			ogs_error(
				"OpenAPI_registration_context_container_convertToJSON() failed [local_time_zone]");
			goto end;
		}
	}
	...
	{
		ogs_error(
			"OpenAPI_registration_context_container_convertToJSON() failed [an_type]");
		return NULL;
	}
	...
	{
		ogs_error(
			"OpenAPI_registration_context_container_convertToJSON() failed [an_n2_ap_id]");
		goto end;
	}

	if(!registration_context_container->ran_node_id) {
		ogs_error(
			"OpenAPI_registration_context_container_convertToJSON() failed [ran_node_id]");
		return NULL;
	}
	...
	if(!registration_context_container->initial_amf_name) {
		ogs_error(
			"OpenAPI_registration_context_container_convertToJSON() failed [initial_amf_name]");
		return NULL;
	}
	...
	if(!registration_context_container->user_location) {
		ogs_error(
			"OpenAPI_registration_context_container_convertToJSON() failed [user_location]");
		return NULL;
	}
	cJSON *user_location_local_JSON =
	...
	if(registration_context_container->rrc_est_cause) {
		...
		{
			ogs_error(
				"OpenAPI_registration_context_container_convertToJSON() failed [rrc_est_cause]");
			goto end;
		}
	}
	...
	if(registration_context_container->is_ue_context_request) {
		...
		{
			ogs_error(
				"OpenAPI_registration_context_container_convertToJSON() failed [ue_context_request]");
			goto end;
		}
	}
	...
	if(registration_context_container->is_initial_amf_n2_ap_id) {
		...
		{
			ogs_error(
				"OpenAPI_registration_context_container_convertToJSON() failed [initial_amf_n2_ap_id]");
			goto end;
		}
	}
	...
	if(registration_context_container->an_n2_ipv4_addr) {
		...
		{
			ogs_error(
				"OpenAPI_registration_context_container_convertToJSON() failed [an_n2_ipv4_addr]");
			goto end;
		}
	}

	if(registration_context_container->an_n2_ipv6_addr) {
		if(cJSON_AddStringToObject(item, "anN2IPv6Addr",
		                           registration_context_container->
		                           an_n2_ipv6_addr) == NULL)
		{
			ogs_error(
				"OpenAPI_registration_context_container_convertToJSON() failed [an_n2_ipv6_addr]");
			goto end;
		}
	}

	if(registration_context_container->allowed_nssai) {
		cJSON *allowed_nssai_local_JSON =
			OpenAPI_allowed_nssai_convertToJSON(
				registration_context_container->allowed_nssai);
		if(allowed_nssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_registration_context_container_convertToJSON() failed [allowed_nssai]");
			goto end;
		}
	...
	if(registration_context_container->configured_nssai) {
		...
		if(configured_nssaiList == NULL) {
			ogs_error(
				"OpenAPI_registration_context_container_convertToJSON() failed [configured_nssai]");
			goto end;
		}
		OpenAPI_list_for_each(
			registration_context_container->configured_nssai,
	...
	if(registration_context_container->rejected_nssai_in_plmn) {
		...
		if(rejected_nssai_in_plmnList == NULL) {
			ogs_error(
				"OpenAPI_registration_context_container_convertToJSON() failed [rejected_nssai_in_plmn]");
			goto end;
		}
		OpenAPI_list_for_each(
			registration_context_container->rejected_nssai_in_plmn,
	...
	if(registration_context_container->rejected_nssai_in_ta) {
		...
		if(rejected_nssai_in_taList == NULL) {
			ogs_error(
				"OpenAPI_registration_context_container_convertToJSON() failed [rejected_nssai_in_ta]");
			goto end;
		}
		OpenAPI_list_for_each(
			registration_context_container->rejected_nssai_in_ta,
	...
	if(registration_context_container->selected_plmn_id) {
		cJSON *selected_plmn_id_local_JSON =
			OpenAPI_plmn_id_convertToJSON(
				registration_context_container->selected_plmn_id);
		if(selected_plmn_id_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_registration_context_container_convertToJSON() failed [selected_plmn_id]");
			goto end;
		}
	...
	if(registration_context_container->is_iab_node_ind) {
		...
		{
			ogs_error(
				"OpenAPI_registration_context_container_convertToJSON() failed [iab_node_ind]");
			goto end;
		}
	}

	if(registration_context_container->ce_mode_b_ind) {
		cJSON *ce_mode_b_ind_local_JSON =
			OpenAPI_ce_mode_b_ind_convertToJSON(
				registration_context_container->ce_mode_b_ind);
		if(ce_mode_b_ind_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_registration_context_container_convertToJSON() failed [ce_mode_b_ind]");
			goto end;
		}
	...
	if(registration_context_container->lte_m_ind) {
		cJSON *lte_m_ind_local_JSON =
			OpenAPI_lte_m_ind_convertToJSON(
				registration_context_container->lte_m_ind);
		if(lte_m_ind_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_registration_context_container_convertToJSON() failed [lte_m_ind]");
			goto end;
		}
	...
	if(registration_context_container->is_authenticated_ind) {
		...
		{
			ogs_error(
				"OpenAPI_registration_context_container_convertToJSON() failed [authenticated_ind]");
			goto end;
		}
	}
	...
	if(registration_context_container->npn_access_info) {
		cJSON *npn_access_info_local_JSON =
			OpenAPI_npn_access_info_convertToJSON(
				registration_context_container->npn_access_info);
		if(npn_access_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_registration_context_container_convertToJSON() failed [npn_access_info]");
			goto end;
		}
...
OpenAPI_registration_context_container_parseFromJSON(
	...
	if(!ue_context) {
		ogs_error(
			"OpenAPI_registration_context_container_parseFromJSON() failed [ue_context]");
		goto end;
	}
	ue_context_local_nonprim = OpenAPI_ue_context_parseFromJSON(ue_context);
	if(!ue_context_local_nonprim) {
		ogs_error(
			"OpenAPI_ue_context_parseFromJSON failed [ue_context]");
		goto end;
	}
	...
	if(local_time_zone) {
		...
		{
			ogs_error(
				"OpenAPI_registration_context_container_parseFromJSON() failed [local_time_zone]");
			goto end;
		}
	}
	...
	if(!an_type) {
		ogs_error(
			"OpenAPI_registration_context_container_parseFromJSON() failed [an_type]");
		goto end;
	}
	...
	if(!an_n2_ap_id) {
		ogs_error(
			"OpenAPI_registration_context_container_parseFromJSON() failed [an_n2_ap_id]");
		goto end;
	}
	...
	if(!ran_node_id) {
		ogs_error(
			"OpenAPI_registration_context_container_parseFromJSON() failed [ran_node_id]");
		goto end;
	}
	ran_node_id_local_nonprim =
		OpenAPI_global_ran_node_id_parseFromJSON(ran_node_id);
	if(!ran_node_id_local_nonprim) {
		ogs_error(
			"OpenAPI_global_ran_node_id_parseFromJSON failed [ran_node_id]");
		goto end;
	}
	...
	if(!initial_amf_name) {
		ogs_error(
			"OpenAPI_registration_context_container_parseFromJSON() failed [initial_amf_name]");
		goto end;
	}
	...
	if(!user_location) {
		ogs_error(
			"OpenAPI_registration_context_container_parseFromJSON() failed [user_location]");
		goto end;
	}
	user_location_local_nonprim =
		OpenAPI_user_location_parseFromJSON(user_location);
	if(!user_location_local_nonprim) {
		ogs_error(
			"OpenAPI_user_location_parseFromJSON failed [user_location]");
		goto end;
	}
	...
	if(rrc_est_cause) {
		...
		{
			ogs_error(
				"OpenAPI_registration_context_container_parseFromJSON() failed [rrc_est_cause]");
			goto end;
		}
	}
	...
	if(ue_context_request) {
		if(!cJSON_IsBool(ue_context_request)) {
			ogs_error(
				"OpenAPI_registration_context_container_parseFromJSON() failed [ue_context_request]");
			goto end;
		}
	}
	...
	if(initial_amf_n2_ap_id) {
		if(!cJSON_IsNumber(initial_amf_n2_ap_id)) {
			ogs_error(
				"OpenAPI_registration_context_container_parseFromJSON() failed [initial_amf_n2_ap_id]");
			goto end;
		}
	}
	...
	if(an_n2_ipv4_addr) {
		...
		{
			ogs_error(
				"OpenAPI_registration_context_container_parseFromJSON() failed [an_n2_ipv4_addr]");
			goto end;
		}
	}
	...
	if(an_n2_ipv6_addr) {
		if(!cJSON_IsString(an_n2_ipv6_addr) &&
		   !cJSON_IsNull(an_n2_ipv6_addr))
		{
			ogs_error(
				"OpenAPI_registration_context_container_parseFromJSON() failed [an_n2_ipv6_addr]");
			goto end;
		}
	}
	...
	if(allowed_nssai) {
		allowed_nssai_local_nonprim =
			OpenAPI_allowed_nssai_parseFromJSON(allowed_nssai);
		if(!allowed_nssai_local_nonprim) {
			ogs_error(
				"OpenAPI_allowed_nssai_parseFromJSON failed [allowed_nssai]");
			goto end;
		}
	}
	...
	if(configured_nssai) {
		...
		if(!cJSON_IsArray(configured_nssai)) {
			ogs_error(
				"OpenAPI_registration_context_container_parseFromJSON() failed [configured_nssai]");
			goto end;
		}
	...
	rejected_nssai_in_plmn =
		cJSON_GetObjectItemCaseSensitive(
			registration_context_containerJSON,
			"rejectedNssaiInPlmn");
	if(rejected_nssai_in_plmn) {
		cJSON *rejected_nssai_in_plmn_local = NULL;
		if(!cJSON_IsArray(rejected_nssai_in_plmn)) {
			ogs_error(
				"OpenAPI_registration_context_container_parseFromJSON() failed [rejected_nssai_in_plmn]");
			goto end;
		}
	...
	if(rejected_nssai_in_ta) {
		...
		if(!cJSON_IsArray(rejected_nssai_in_ta)) {
			ogs_error(
				"OpenAPI_registration_context_container_parseFromJSON() failed [rejected_nssai_in_ta]");
			goto end;
		}
	...
	if(selected_plmn_id) {
		selected_plmn_id_local_nonprim =
			OpenAPI_plmn_id_parseFromJSON(selected_plmn_id);
		if(!selected_plmn_id_local_nonprim) {
			ogs_error(
				"OpenAPI_plmn_id_parseFromJSON failed [selected_plmn_id]");
			goto end;
		}
	}
	...
	if(iab_node_ind) {
		if(!cJSON_IsBool(iab_node_ind)) {
			ogs_error(
				"OpenAPI_registration_context_container_parseFromJSON() failed [iab_node_ind]");
			goto end;
		}
	}
	...
	if(ce_mode_b_ind) {
		ce_mode_b_ind_local_nonprim =
			OpenAPI_ce_mode_b_ind_parseFromJSON(ce_mode_b_ind);
		if(!ce_mode_b_ind_local_nonprim) {
			ogs_error(
				"OpenAPI_ce_mode_b_ind_parseFromJSON failed [ce_mode_b_ind]");
			goto end;
		}
	}
	...
	if(lte_m_ind) {
		lte_m_ind_local_nonprim =
			OpenAPI_lte_m_ind_parseFromJSON(lte_m_ind);
		if(!lte_m_ind_local_nonprim) {
			ogs_error(
				"OpenAPI_lte_m_ind_parseFromJSON failed [lte_m_ind]");
			goto end;
		}
	}
	...
	if(authenticated_ind) {
		if(!cJSON_IsBool(authenticated_ind)) {
			ogs_error(
				"OpenAPI_registration_context_container_parseFromJSON() failed [authenticated_ind]");
			goto end;
		}
	}
	...
	if(npn_access_info) {
		npn_access_info_local_nonprim =
			OpenAPI_npn_access_info_parseFromJSON(npn_access_info);
		if(!npn_access_info_local_nonprim) {
			ogs_error(
				"OpenAPI_npn_access_info_parseFromJSON failed [npn_access_info]");
			goto end;
		}
	}
	...
	registration_context_container_local_var =
		OpenAPI_registration_context_container_create(
			...
			local_time_zone &&
			!cJSON_IsNull(local_time_zone) ?
			ogs_strdup(local_time_zone->valuestring) : NULL,
			an_typeVariable,
			...
			rrc_est_cause &&
			!cJSON_IsNull(rrc_est_cause) ?
			ogs_strdup(rrc_est_cause->valuestring) : NULL,
			ue_context_request ? true : false,
			ue_context_request ? ue_context_request->valueint : 0,
			initial_amf_n2_ap_id ? true : false,
			initial_amf_n2_ap_id ?
			initial_amf_n2_ap_id->valuedouble : 0,
			an_n2_ipv4_addr &&
			!cJSON_IsNull(an_n2_ipv4_addr) ?
			ogs_strdup(an_n2_ipv4_addr->valuestring) : NULL,
			an_n2_ipv6_addr &&
			!cJSON_IsNull(an_n2_ipv6_addr) ?
			ogs_strdup(an_n2_ipv6_addr->valuestring) : NULL,
			...
			selected_plmn_id ? selected_plmn_id_local_nonprim :
			NULL,
...
OpenAPI_registration_context_container_t *
OpenAPI_registration_context_container_copy(
	OpenAPI_registration_context_container_t	*dst,
	OpenAPI_registration_context_container_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_registration_context_container_convertToJSON() failed");
		return NULL;
	}
