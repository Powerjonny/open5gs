...
OpenAPI_qos_monitoring_info_t *OpenAPI_qos_monitoring_info_create(
	bool is_qos_monitoring_ind, int qos_monitoring_ind) {
	OpenAPI_qos_monitoring_info_t *qos_monitoring_info_local_var =
		ogs_malloc(sizeof(OpenAPI_qos_monitoring_info_t));
	ogs_assert(qos_monitoring_info_local_var);

	qos_monitoring_info_local_var->is_qos_monitoring_ind =
		is_qos_monitoring_ind;
...
cJSON *OpenAPI_qos_monitoring_info_convertToJSON(
	...
	if(qos_monitoring_info == NULL) {
		ogs_error(
			"OpenAPI_qos_monitoring_info_convertToJSON() failed [QosMonitoringInfo]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(qos_monitoring_info->is_qos_monitoring_ind) {
		if(cJSON_AddBoolToObject(item, "qosMonitoringInd",
		                         qos_monitoring_info->qos_monitoring_ind)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_qos_monitoring_info_convertToJSON() failed [qos_monitoring_ind]");
			goto end;
		}
	}
...
OpenAPI_qos_monitoring_info_t *OpenAPI_qos_monitoring_info_parseFromJSON(
	cJSON *qos_monitoring_infoJSON) {
	OpenAPI_qos_monitoring_info_t *qos_monitoring_info_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *qos_monitoring_ind = NULL;
	qos_monitoring_ind =
		cJSON_GetObjectItemCaseSensitive(qos_monitoring_infoJSON,
		                                 "qosMonitoringInd");
	if(qos_monitoring_ind) {
		if(!cJSON_IsBool(qos_monitoring_ind)) {
			ogs_error(
				"OpenAPI_qos_monitoring_info_parseFromJSON() failed [qos_monitoring_ind]");
			goto end;
		}
	}
...
OpenAPI_qos_monitoring_info_t *OpenAPI_qos_monitoring_info_copy(
	OpenAPI_qos_monitoring_info_t	*dst,
	OpenAPI_qos_monitoring_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
