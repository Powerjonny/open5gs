...
char * OpenAPI_up_cnx_state_ToString(OpenAPI_up_cnx_state_e up_cnx_state) {
	const char *up_cnx_stateArray[] = { "NULL", "ACTIVATED", "DEACTIVATED",
		                            "ACTIVATING", "SUSPENDED" };
	size_t sizeofArray = sizeof(up_cnx_stateArray) /
	                     sizeof(up_cnx_stateArray[0]);
...
OpenAPI_up_cnx_state_e OpenAPI_up_cnx_state_FromString(char *up_cnx_state) {
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(up_cnx_state,
		          up_cnx_stateArray[stringToReturn]) == 0) {
