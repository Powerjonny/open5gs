...
OpenAPI_key_amf_t *OpenAPI_key_amf_create(
	...
	char			*key_val)
{
	OpenAPI_key_amf_t *key_amf_local_var =
		ogs_malloc(sizeof(OpenAPI_key_amf_t));
...
cJSON *OpenAPI_key_amf_convertToJSON(OpenAPI_key_amf_t *key_amf) {
	...
	if(cJSON_AddStringToObject(item, "keyType",
	                           OpenAPI_key_amf_type_ToString(
					   key_amf->key_type)) == NULL)
	{
		ogs_error("OpenAPI_key_amf_convertToJSON() failed [key_type]");
		goto end;
	}
...
OpenAPI_key_amf_t *OpenAPI_key_amf_parseFromJSON(cJSON *key_amfJSON) {
	...
	key_typeVariable =
		OpenAPI_key_amf_type_FromString(key_type->valuestring);
...
OpenAPI_key_amf_t *OpenAPI_key_amf_copy(OpenAPI_key_amf_t	*dst,
                                        OpenAPI_key_amf_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
