...
OpenAPI_upf_info_t *OpenAPI_upf_info_create(
	...
	OpenAPI_upf_info_t *upf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_upf_info_t));
...
cJSON *OpenAPI_upf_info_convertToJSON(OpenAPI_upf_info_t *upf_info) {
	...
	if(!upf_info->s_nssai_upf_info_list) {
		ogs_error(
			"OpenAPI_upf_info_convertToJSON() failed [s_nssai_upf_info_list]");
		return NULL;
	}
	...
	if(upf_info->smf_serving_area) {
		...
		if(smf_serving_areaList == NULL) {
			ogs_error(
				"OpenAPI_upf_info_convertToJSON() failed [smf_serving_area]");
			goto end;
		}
		OpenAPI_list_for_each(upf_info->smf_serving_area, node) {
	...
	if(upf_info->interface_upf_info_list) {
		...
		if(interface_upf_info_listList == NULL) {
			ogs_error(
				"OpenAPI_upf_info_convertToJSON() failed [interface_upf_info_list]");
			goto end;
		}
		OpenAPI_list_for_each(upf_info->interface_upf_info_list, node) {
			cJSON *itemLocal =
	...
	if(upf_info->is_iwk_eps_ind) {
		if(cJSON_AddBoolToObject(item, "iwkEpsInd",
		                         upf_info->iwk_eps_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_upf_info_convertToJSON() failed [iwk_eps_ind]");
			goto end;
		}
	}
	...
	if(upf_info->pdu_session_types != OpenAPI_pdu_session_type_NULL) {
		...
		if(pdu_session_typesList == NULL) {
			ogs_error(
				"OpenAPI_upf_info_convertToJSON() failed [pdu_session_types]");
			goto end;
		}
	...
	if(upf_info->atsss_capability) {
		...
		if(atsss_capability_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_upf_info_convertToJSON() failed [atsss_capability]");
			goto end;
		}
	...
	if(upf_info->is_ue_ip_addr_ind) {
		if(cJSON_AddBoolToObject(item, "ueIpAddrInd",
		                         upf_info->ue_ip_addr_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_upf_info_convertToJSON() failed [ue_ip_addr_ind]");
			goto end;
		}
	}
	...
	if(upf_info->tai_list) {
		...
		if(tai_listList == NULL) {
			ogs_error(
				"OpenAPI_upf_info_convertToJSON() failed [tai_list]");
			goto end;
		}
		OpenAPI_list_for_each(upf_info->tai_list, node) {
			cJSON *itemLocal =
	...
	if(upf_info->tai_range_list) {
		...
		if(tai_range_listList == NULL) {
			ogs_error(
				"OpenAPI_upf_info_convertToJSON() failed [tai_range_list]");
			goto end;
		}
		OpenAPI_list_for_each(upf_info->tai_range_list, node) {
			cJSON *itemLocal =
	...
	if(upf_info->w_agf_info) {
		cJSON *w_agf_info_local_JSON =
			OpenAPI_w_agf_info_convertToJSON(upf_info->w_agf_info);
		if(w_agf_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_upf_info_convertToJSON() failed [w_agf_info]");
			goto end;
		}
	...
	if(upf_info->tngf_info) {
		cJSON *tngf_info_local_JSON =
			OpenAPI_tngf_info_convertToJSON(upf_info->tngf_info);
		if(tngf_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_upf_info_convertToJSON() failed [tngf_info]");
			goto end;
		}
	...
	if(upf_info->twif_info) {
		cJSON *twif_info_local_JSON =
			OpenAPI_twif_info_convertToJSON(upf_info->twif_info);
		if(twif_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_upf_info_convertToJSON() failed [twif_info]");
			goto end;
		}
	...
	if(upf_info->is_priority) {
		if(cJSON_AddNumberToObject(item, "priority",
		                           upf_info->priority) == NULL)
		{
			ogs_error(
				"OpenAPI_upf_info_convertToJSON() failed [priority]");
			goto end;
		}
	}
	...
	if(upf_info->is_redundant_gtpu) {
		if(cJSON_AddBoolToObject(item, "redundantGtpu",
		                         upf_info->redundant_gtpu) == NULL)
		{
			ogs_error(
				"OpenAPI_upf_info_convertToJSON() failed [redundant_gtpu]");
			goto end;
		}
	}
	...
	if(upf_info->is_ipups) {
		...
		{
			ogs_error(
				"OpenAPI_upf_info_convertToJSON() failed [ipups]");
			goto end;
		}
	}
	...
	if(upf_info->is_data_forwarding) {
		if(cJSON_AddBoolToObject(item, "dataForwarding",
		                         upf_info->data_forwarding) == NULL)
		{
			ogs_error(
				"OpenAPI_upf_info_convertToJSON() failed [data_forwarding]");
			goto end;
		}
	}
	...
	if(upf_info->supported_pfcp_features) {
		if(cJSON_AddStringToObject(item, "supportedPfcpFeatures",
		                           upf_info->supported_pfcp_features) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_upf_info_convertToJSON() failed [supported_pfcp_features]");
			goto end;
		}
	}
...
OpenAPI_upf_info_t *OpenAPI_upf_info_parseFromJSON(cJSON *upf_infoJSON) {
	...
	s_nssai_upf_info_list = cJSON_GetObjectItemCaseSensitive(upf_infoJSON,
	                                                         "sNssaiUpfInfoList");
	if(!s_nssai_upf_info_list) {
		ogs_error(
			"OpenAPI_upf_info_parseFromJSON() failed [s_nssai_upf_info_list]");
		goto end;
	}
	...
	smf_serving_area = cJSON_GetObjectItemCaseSensitive(upf_infoJSON,
	                                                    "smfServingArea");
	if(smf_serving_area) {
		cJSON *smf_serving_area_local = NULL;
		if(!cJSON_IsArray(smf_serving_area)) {
			ogs_error(
				"OpenAPI_upf_info_parseFromJSON() failed [smf_serving_area]");
			goto end;
		}
	...
	interface_upf_info_list = cJSON_GetObjectItemCaseSensitive(upf_infoJSON,
	                                                           "interfaceUpfInfoList");
	if(interface_upf_info_list) {
		cJSON *interface_upf_info_list_local = NULL;
		if(!cJSON_IsArray(interface_upf_info_list)) {
			ogs_error(
				"OpenAPI_upf_info_parseFromJSON() failed [interface_upf_info_list]");
			goto end;
		}
	...
	iwk_eps_ind = cJSON_GetObjectItemCaseSensitive(upf_infoJSON,
	                                               "iwkEpsInd");
	if(iwk_eps_ind) {
		if(!cJSON_IsBool(iwk_eps_ind)) {
			ogs_error(
				"OpenAPI_upf_info_parseFromJSON() failed [iwk_eps_ind]");
			goto end;
		}
	}

	pdu_session_types = cJSON_GetObjectItemCaseSensitive(upf_infoJSON,
	                                                     "pduSessionTypes");
	if(pdu_session_types) {
		cJSON *pdu_session_types_local = NULL;
		if(!cJSON_IsArray(pdu_session_types)) {
			ogs_error(
				"OpenAPI_upf_info_parseFromJSON() failed [pdu_session_types]");
			goto end;
		}
		...
		if(pdu_session_typesList->count == 0) {
			ogs_error(
				"OpenAPI_upf_info_parseFromJSON() failed: Expected pdu_session_typesList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}

	atsss_capability = cJSON_GetObjectItemCaseSensitive(upf_infoJSON,
	                                                    "atsssCapability");
	if(atsss_capability) {
	...
	ue_ip_addr_ind = cJSON_GetObjectItemCaseSensitive(upf_infoJSON,
	                                                  "ueIpAddrInd");
	if(ue_ip_addr_ind) {
		if(!cJSON_IsBool(ue_ip_addr_ind)) {
			ogs_error(
				"OpenAPI_upf_info_parseFromJSON() failed [ue_ip_addr_ind]");
			goto end;
		}
	}
	...
	if(tai_list) {
		...
		if(!cJSON_IsArray(tai_list)) {
			ogs_error(
				"OpenAPI_upf_info_parseFromJSON() failed [tai_list]");
			goto end;
		}
	...
	tai_range_list = cJSON_GetObjectItemCaseSensitive(upf_infoJSON,
	                                                  "taiRangeList");
	if(tai_range_list) {
		cJSON *tai_range_list_local = NULL;
		if(!cJSON_IsArray(tai_range_list)) {
			ogs_error(
				"OpenAPI_upf_info_parseFromJSON() failed [tai_range_list]");
			goto end;
		}
	...
	if(priority) {
		if(!cJSON_IsNumber(priority)) {
			ogs_error(
				"OpenAPI_upf_info_parseFromJSON() failed [priority]");
			goto end;
		}
	}

	redundant_gtpu = cJSON_GetObjectItemCaseSensitive(upf_infoJSON,
	                                                  "redundantGtpu");
	if(redundant_gtpu) {
		if(!cJSON_IsBool(redundant_gtpu)) {
			ogs_error(
				"OpenAPI_upf_info_parseFromJSON() failed [redundant_gtpu]");
			goto end;
		}
	}
	...
	if(ipups) {
		if(!cJSON_IsBool(ipups)) {
			ogs_error(
				"OpenAPI_upf_info_parseFromJSON() failed [ipups]");
			goto end;
		}
	}

	data_forwarding = cJSON_GetObjectItemCaseSensitive(upf_infoJSON,
	                                                   "dataForwarding");
	if(data_forwarding) {
		if(!cJSON_IsBool(data_forwarding)) {
			ogs_error(
				"OpenAPI_upf_info_parseFromJSON() failed [data_forwarding]");
			goto end;
		}
	}

	supported_pfcp_features = cJSON_GetObjectItemCaseSensitive(upf_infoJSON,
	...
	if(supported_pfcp_features) {
		...
		{
			ogs_error(
				"OpenAPI_upf_info_parseFromJSON() failed [supported_pfcp_features]");
			goto end;
		}
	}
...
OpenAPI_upf_info_t *OpenAPI_upf_info_copy(OpenAPI_upf_info_t	*dst,
                                          OpenAPI_upf_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
