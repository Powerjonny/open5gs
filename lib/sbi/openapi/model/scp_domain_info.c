...
OpenAPI_scp_domain_info_t *OpenAPI_scp_domain_info_create(char *scp_fqdn,
                                                          OpenAPI_list_t *
                                                          scp_ip_end_points,
                                                          char
                                                                *scp_prefix,
                                                          OpenAPI_list_t
                                                                *scp_ports) {
	OpenAPI_scp_domain_info_t *scp_domain_info_local_var =
		ogs_malloc(sizeof(OpenAPI_scp_domain_info_t));
...
void OpenAPI_scp_domain_info_free(OpenAPI_scp_domain_info_t *scp_domain_info) {
	...
	if(scp_domain_info->scp_ip_end_points) {
		OpenAPI_list_for_each(scp_domain_info->scp_ip_end_points,
		                      node) {
...
cJSON *OpenAPI_scp_domain_info_convertToJSON(
	OpenAPI_scp_domain_info_t *scp_domain_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(scp_domain_info == NULL) {
		ogs_error(
			"OpenAPI_scp_domain_info_convertToJSON() failed [ScpDomainInfo]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(scp_domain_info->scp_fqdn) {
		if(cJSON_AddStringToObject(item, "scpFqdn",
		                           scp_domain_info->scp_fqdn) == NULL)
		{
			ogs_error(
				"OpenAPI_scp_domain_info_convertToJSON() failed [scp_fqdn]");
			goto end;
		}
	}

	if(scp_domain_info->scp_ip_end_points) {
		cJSON *scp_ip_end_pointsList = cJSON_AddArrayToObject(item,
		                                                      "scpIpEndPoints");
		if(scp_ip_end_pointsList == NULL) {
			ogs_error(
				"OpenAPI_scp_domain_info_convertToJSON() failed [scp_ip_end_points]");
			goto end;
		}
		OpenAPI_list_for_each(scp_domain_info->scp_ip_end_points,
		                      node) {
	...
	if(scp_domain_info->scp_prefix) {
		...
		{
			ogs_error(
				"OpenAPI_scp_domain_info_convertToJSON() failed [scp_prefix]");
			goto end;
		}
	}
	...
	if(scp_domain_info->scp_ports) {
		...
		if(scp_ports == NULL) {
			ogs_error(
				"OpenAPI_scp_domain_info_convertToJSON() failed [scp_ports]");
			goto end;
		}
		cJSON *localMapObject = scp_ports;
		if(scp_domain_info->scp_ports) {
			OpenAPI_list_for_each(scp_domain_info->scp_ports,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				if(localKeyValue->value == NULL) {
					ogs_error(
						"OpenAPI_scp_domain_info_convertToJSON() failed [inner]");
					goto end;
				}
...
OpenAPI_scp_domain_info_t *OpenAPI_scp_domain_info_parseFromJSON(
	cJSON *scp_domain_infoJSON) {
	...
	scp_fqdn = cJSON_GetObjectItemCaseSensitive(scp_domain_infoJSON,
	                                            "scpFqdn");
	if(scp_fqdn) {
		if(!cJSON_IsString(scp_fqdn) &&
		   !cJSON_IsNull(scp_fqdn))
		{
			ogs_error(
				"OpenAPI_scp_domain_info_parseFromJSON() failed [scp_fqdn]");
			goto end;
		}
	}
	...
	if(scp_ip_end_points) {
		...
		if(!cJSON_IsArray(scp_ip_end_points)) {
			ogs_error(
				"OpenAPI_scp_domain_info_parseFromJSON() failed [scp_ip_end_points]");
			goto end;
		}
		...
		cJSON_ArrayForEach(scp_ip_end_points_local, scp_ip_end_points) {
			...
			OpenAPI_ip_end_point_t *scp_ip_end_pointsItem =
				OpenAPI_ip_end_point_parseFromJSON(
					scp_ip_end_points_local);
			if(!scp_ip_end_pointsItem) {
				ogs_error("No scp_ip_end_pointsItem");
				goto end;
			}
			OpenAPI_list_add(scp_ip_end_pointsList,
			                 scp_ip_end_pointsItem);
		}
	}

	scp_prefix = cJSON_GetObjectItemCaseSensitive(scp_domain_infoJSON,
	                                              "scpPrefix");
	if(scp_prefix) {
		...
		{
			ogs_error(
				"OpenAPI_scp_domain_info_parseFromJSON() failed [scp_prefix]");
			goto end;
		}
	}

	scp_ports = cJSON_GetObjectItemCaseSensitive(scp_domain_infoJSON,
	                                             "scpPorts");
	if(scp_ports) {
		...
		{
			ogs_error(
				"OpenAPI_scp_domain_info_parseFromJSON() failed [scp_ports]");
			goto end;
		}
		if(cJSON_IsObject(scp_ports)) {
			...
			cJSON_ArrayForEach(scp_ports_local_map, scp_ports) {
				...
				if(!cJSON_IsNumber(localMapObject)) {
					ogs_error(
						"OpenAPI_scp_domain_info_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	scp_domain_info_local_var = OpenAPI_scp_domain_info_create(
		scp_fqdn &&
		!cJSON_IsNull(scp_fqdn) ? ogs_strdup(scp_fqdn->valuestring) :
		NULL,
...
OpenAPI_scp_domain_info_t *OpenAPI_scp_domain_info_copy(
	OpenAPI_scp_domain_info_t *dst, OpenAPI_scp_domain_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
