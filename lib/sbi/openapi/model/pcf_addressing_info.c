...
OpenAPI_pcf_addressing_info_t *OpenAPI_pcf_addressing_info_create(
	...
	OpenAPI_pcf_addressing_info_t *pcf_addressing_info_local_var =
		ogs_malloc(sizeof(OpenAPI_pcf_addressing_info_t));
...
void OpenAPI_pcf_addressing_info_free(
	...
	if(pcf_addressing_info->pcf_ip_end_points) {
		OpenAPI_list_for_each(pcf_addressing_info->pcf_ip_end_points,
		                      node) {
...
cJSON *OpenAPI_pcf_addressing_info_convertToJSON(
	...
	if(pcf_addressing_info == NULL) {
		ogs_error(
			"OpenAPI_pcf_addressing_info_convertToJSON() failed [PcfAddressingInfo]");
		return NULL;
	}
	...
	if(pcf_addressing_info->pcf_fqdn) {
		if(cJSON_AddStringToObject(item, "pcfFqdn",
		                           pcf_addressing_info->pcf_fqdn) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pcf_addressing_info_convertToJSON() failed [pcf_fqdn]");
			goto end;
		}
	}
	...
	if(pcf_addressing_info->pcf_ip_end_points) {
		...
		if(pcf_ip_end_pointsList == NULL) {
			ogs_error(
				"OpenAPI_pcf_addressing_info_convertToJSON() failed [pcf_ip_end_points]");
			goto end;
		}
		OpenAPI_list_for_each(pcf_addressing_info->pcf_ip_end_points,
		                      node) {
	...
	if(pcf_addressing_info->binding_info) {
		if(cJSON_AddStringToObject(item, "bindingInfo",
		                           pcf_addressing_info->binding_info) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pcf_addressing_info_convertToJSON() failed [binding_info]");
			goto end;
		}
	}
...
OpenAPI_pcf_addressing_info_t *OpenAPI_pcf_addressing_info_parseFromJSON(
	...
	pcf_fqdn = cJSON_GetObjectItemCaseSensitive(pcf_addressing_infoJSON,
	                                            "pcfFqdn");
	if(pcf_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_addressing_info_parseFromJSON() failed [pcf_fqdn]");
			goto end;
		}
	}

	pcf_ip_end_points =
		cJSON_GetObjectItemCaseSensitive(pcf_addressing_infoJSON,
		                                 "pcfIpEndPoints");
	if(pcf_ip_end_points) {
		cJSON *pcf_ip_end_points_local = NULL;
		if(!cJSON_IsArray(pcf_ip_end_points)) {
			ogs_error(
				"OpenAPI_pcf_addressing_info_parseFromJSON() failed [pcf_ip_end_points]");
			goto end;
		}
	...
	binding_info = cJSON_GetObjectItemCaseSensitive(pcf_addressing_infoJSON,
	                                                "bindingInfo");
	if(binding_info) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_addressing_info_parseFromJSON() failed [binding_info]");
			goto end;
		}
	}
	...
	pcf_addressing_info_local_var = OpenAPI_pcf_addressing_info_create(
		...
		binding_info &&
		!cJSON_IsNull(binding_info) ?
		ogs_strdup(binding_info->valuestring) : NULL
		);
...
OpenAPI_pcf_addressing_info_t *OpenAPI_pcf_addressing_info_copy(
	OpenAPI_pcf_addressing_info_t	*dst,
	OpenAPI_pcf_addressing_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
