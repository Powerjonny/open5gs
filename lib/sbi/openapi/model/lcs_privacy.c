...
OpenAPI_lcs_privacy_t *OpenAPI_lcs_privacy_create(char		*af_instance_id,
                                                  bool		is_reference_id,
                                                  int		reference_id,
                                                  OpenAPI_lpi_t *lpi,
                                                  char		*
                                                  mtc_provider_information) {
	OpenAPI_lcs_privacy_t *lcs_privacy_local_var =
		ogs_malloc(sizeof(OpenAPI_lcs_privacy_t));
	...
	lcs_privacy_local_var->mtc_provider_information =
		mtc_provider_information;
...
cJSON *OpenAPI_lcs_privacy_convertToJSON(OpenAPI_lcs_privacy_t *lcs_privacy) {
	...
	if(lcs_privacy == NULL) {
		ogs_error(
			"OpenAPI_lcs_privacy_convertToJSON() failed [LcsPrivacy]");
		return NULL;
	}
	...
	if(lcs_privacy->af_instance_id) {
		...
		{
			ogs_error(
				"OpenAPI_lcs_privacy_convertToJSON() failed [af_instance_id]");
			goto end;
		}
	}
	...
	if(lcs_privacy->is_reference_id) {
		if(cJSON_AddNumberToObject(item, "referenceId",
		                           lcs_privacy->reference_id) == NULL)
		{
			ogs_error(
				"OpenAPI_lcs_privacy_convertToJSON() failed [reference_id]");
			goto end;
		}
	}
	...
	if(lcs_privacy->lpi) {
		cJSON *lpi_local_JSON =
			OpenAPI_lpi_convertToJSON(lcs_privacy->lpi);
		if(lpi_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_lcs_privacy_convertToJSON() failed [lpi]");
			goto end;
		}
	...
	if(lcs_privacy->mtc_provider_information) {
		if(cJSON_AddStringToObject(item, "mtcProviderInformation",
		                           lcs_privacy->mtc_provider_information)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_lcs_privacy_convertToJSON() failed [mtc_provider_information]");
			goto end;
		}
	}
...
OpenAPI_lcs_privacy_t *OpenAPI_lcs_privacy_parseFromJSON(
	...
	af_instance_id = cJSON_GetObjectItemCaseSensitive(lcs_privacyJSON,
	                                                  "afInstanceId");
	if(af_instance_id) {
		...
		{
			ogs_error(
				"OpenAPI_lcs_privacy_parseFromJSON() failed [af_instance_id]");
			goto end;
		}
	}

	reference_id = cJSON_GetObjectItemCaseSensitive(lcs_privacyJSON,
	                                                "referenceId");
	if(reference_id) {
		if(!cJSON_IsNumber(reference_id)) {
			ogs_error(
				"OpenAPI_lcs_privacy_parseFromJSON() failed [reference_id]");
			goto end;
		}
	}
	...
	mtc_provider_information =
		cJSON_GetObjectItemCaseSensitive(lcs_privacyJSON,
	...
	if(mtc_provider_information) {
		...
		{
			ogs_error(
				"OpenAPI_lcs_privacy_parseFromJSON() failed [mtc_provider_information]");
			goto end;
		}
	}
...
OpenAPI_lcs_privacy_t *OpenAPI_lcs_privacy_copy(OpenAPI_lcs_privacy_t	*dst,
                                                OpenAPI_lcs_privacy_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
