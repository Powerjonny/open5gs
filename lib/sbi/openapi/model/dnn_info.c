...
OpenAPI_dnn_info_t *OpenAPI_dnn_info_create(char *dnn,
                                            ...
                                            int same_smf_ind) {
	OpenAPI_dnn_info_t *dnn_info_local_var =
...
cJSON *OpenAPI_dnn_info_convertToJSON(OpenAPI_dnn_info_t *dnn_info) {
	...
	if(dnn_info->is_default_dnn_indicator) {
		if(cJSON_AddBoolToObject(item, "defaultDnnIndicator",
		                         dnn_info->default_dnn_indicator) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_dnn_info_convertToJSON() failed [default_dnn_indicator]");
			goto end;
		}
	}

	if(dnn_info->is_lbo_roaming_allowed) {
		if(cJSON_AddBoolToObject(item, "lboRoamingAllowed",
		                         dnn_info->lbo_roaming_allowed) == NULL)
		{
			ogs_error(
				"OpenAPI_dnn_info_convertToJSON() failed [lbo_roaming_allowed]");
			goto end;
		}
	}
	...
	if(dnn_info->is_iwk_eps_ind) {
		if(cJSON_AddBoolToObject(item, "iwkEpsInd",
		                         dnn_info->iwk_eps_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_dnn_info_convertToJSON() failed [iwk_eps_ind]");
			goto end;
		}
	}

	if(dnn_info->is_dnn_barred) {
		if(cJSON_AddBoolToObject(item, "dnnBarred",
		                         dnn_info->dnn_barred) == NULL)
		{
			ogs_error(
				"OpenAPI_dnn_info_convertToJSON() failed [dnn_barred]");
			goto end;
		}
	}

	if(dnn_info->is_invoke_nef_ind) {
		if(cJSON_AddBoolToObject(item, "invokeNefInd",
		                         dnn_info->invoke_nef_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_dnn_info_convertToJSON() failed [invoke_nef_ind]");
			goto end;
		}
	}
	...
	if(dnn_info->smf_list) {
		...
		if(smf_listList == NULL) {
			ogs_error(
				"OpenAPI_dnn_info_convertToJSON() failed [smf_list]");
			goto end;
		}
		OpenAPI_list_for_each(dnn_info->smf_list, node) {
			if(cJSON_AddStringToObject(smf_listList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_dnn_info_convertToJSON() failed [smf_list]");
	...
	if(dnn_info->is_same_smf_ind) {
		if(cJSON_AddBoolToObject(item, "sameSmfInd",
		                         dnn_info->same_smf_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_dnn_info_convertToJSON() failed [same_smf_ind]");
			goto end;
		}
	}
...
OpenAPI_dnn_info_t *OpenAPI_dnn_info_parseFromJSON(cJSON *dnn_infoJSON) {
	...
	default_dnn_indicator = cJSON_GetObjectItemCaseSensitive(dnn_infoJSON,
	                                                         "defaultDnnIndicator");
	if(default_dnn_indicator) {
		if(!cJSON_IsBool(default_dnn_indicator)) {
			ogs_error(
				"OpenAPI_dnn_info_parseFromJSON() failed [default_dnn_indicator]");
			goto end;
		}
	}

	lbo_roaming_allowed = cJSON_GetObjectItemCaseSensitive(dnn_infoJSON,
	                                                       "lboRoamingAllowed");
	if(lbo_roaming_allowed) {
		if(!cJSON_IsBool(lbo_roaming_allowed)) {
			ogs_error(
				"OpenAPI_dnn_info_parseFromJSON() failed [lbo_roaming_allowed]");
			goto end;
		}
	}

	iwk_eps_ind = cJSON_GetObjectItemCaseSensitive(dnn_infoJSON,
	                                               "iwkEpsInd");
	if(iwk_eps_ind) {
		if(!cJSON_IsBool(iwk_eps_ind)) {
			ogs_error(
				"OpenAPI_dnn_info_parseFromJSON() failed [iwk_eps_ind]");
			goto end;
		}
	}

	dnn_barred = cJSON_GetObjectItemCaseSensitive(dnn_infoJSON,
	                                              "dnnBarred");
	if(dnn_barred) {
		if(!cJSON_IsBool(dnn_barred)) {
			ogs_error(
				"OpenAPI_dnn_info_parseFromJSON() failed [dnn_barred]");
			goto end;
		}
	}

	invoke_nef_ind = cJSON_GetObjectItemCaseSensitive(dnn_infoJSON,
	                                                  "invokeNefInd");
	if(invoke_nef_ind) {
		if(!cJSON_IsBool(invoke_nef_ind)) {
			ogs_error(
				"OpenAPI_dnn_info_parseFromJSON() failed [invoke_nef_ind]");
			goto end;
		}
	}
	...
	if(smf_list) {
		...
		if(!cJSON_IsArray(smf_list)) {
			ogs_error(
				"OpenAPI_dnn_info_parseFromJSON() failed [smf_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(smf_list_local, smf_list) {
			...
			OpenAPI_list_add(smf_listList,
			                 ogs_strdup(
						 smf_list_local->valuestring));
		}
	}

	same_smf_ind = cJSON_GetObjectItemCaseSensitive(dnn_infoJSON,
	                                                "sameSmfInd");
	if(same_smf_ind) {
		if(!cJSON_IsBool(same_smf_ind)) {
			ogs_error(
				"OpenAPI_dnn_info_parseFromJSON() failed [same_smf_ind]");
			goto end;
		}
	}
...
OpenAPI_dnn_info_t *OpenAPI_dnn_info_copy(OpenAPI_dnn_info_t	*dst,
                                          OpenAPI_dnn_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
