...
OpenAPI_plmn_restriction_t *OpenAPI_plmn_restriction_create(
	...
	OpenAPI_plmn_restriction_t *plmn_restriction_local_var =
	...
	plmn_restriction_local_var->service_area_restriction =
		service_area_restriction;
	plmn_restriction_local_var->core_network_type_restrictions =
		core_network_type_restrictions;
	plmn_restriction_local_var->primary_rat_restrictions =
		primary_rat_restrictions;
	plmn_restriction_local_var->secondary_rat_restrictions =
		secondary_rat_restrictions;

	return plmn_restriction_local_var;
}

void OpenAPI_plmn_restriction_free(
	OpenAPI_plmn_restriction_t *plmn_restriction) {
...
cJSON *OpenAPI_plmn_restriction_convertToJSON(
	OpenAPI_plmn_restriction_t *plmn_restriction) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(plmn_restriction == NULL) {
		ogs_error(
			"OpenAPI_plmn_restriction_convertToJSON() failed [PlmnRestriction]");
		return NULL;
	}
	...
	if(plmn_restriction->rat_restrictions != OpenAPI_rat_type_NULL) {
		...
		if(rat_restrictionsList == NULL) {
			ogs_error(
				"OpenAPI_plmn_restriction_convertToJSON() failed [rat_restrictions]");
			goto end;
		}
		OpenAPI_list_for_each(plmn_restriction->rat_restrictions,
		                      node) {
	...
	if(plmn_restriction->forbidden_areas) {
		...
		if(forbidden_areasList == NULL) {
			ogs_error(
				"OpenAPI_plmn_restriction_convertToJSON() failed [forbidden_areas]");
			goto end;
		}
		OpenAPI_list_for_each(plmn_restriction->forbidden_areas, node) {
			cJSON *itemLocal =
	...
	if(plmn_restriction->service_area_restriction) {
		...
		if(service_area_restriction_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_plmn_restriction_convertToJSON() failed [service_area_restriction]");
			goto end;
		}
	   ...
	   OpenAPI_core_network_type_NULL)
	{
		...
		if(core_network_type_restrictionsList == NULL) {
			ogs_error(
				"OpenAPI_plmn_restriction_convertToJSON() failed [core_network_type_restrictions]");
			goto end;
		}
	   ...
	   OpenAPI_rat_type_NULL)
	{
		...
		if(primary_rat_restrictionsList == NULL) {
			ogs_error(
				"OpenAPI_plmn_restriction_convertToJSON() failed [primary_rat_restrictions]");
			goto end;
		}
		OpenAPI_list_for_each(
			plmn_restriction->primary_rat_restrictions, node) {
	   ...
	   OpenAPI_rat_type_NULL)
	{
		...
		if(secondary_rat_restrictionsList == NULL) {
			ogs_error(
				"OpenAPI_plmn_restriction_convertToJSON() failed [secondary_rat_restrictions]");
			goto end;
		}
		OpenAPI_list_for_each(
			plmn_restriction->secondary_rat_restrictions, node) {
			if(cJSON_AddStringToObject(
...
OpenAPI_plmn_restriction_t *OpenAPI_plmn_restriction_parseFromJSON(
	cJSON *plmn_restrictionJSON) {
	...
	rat_restrictions =
		cJSON_GetObjectItemCaseSensitive(plmn_restrictionJSON,
		                                 "ratRestrictions");
	if(rat_restrictions) {
		cJSON *rat_restrictions_local = NULL;
		if(!cJSON_IsArray(rat_restrictions)) {
			ogs_error(
				"OpenAPI_plmn_restriction_parseFromJSON() failed [rat_restrictions]");
			goto end;
		}
		...
		if(rat_restrictionsList->count == 0) {
			ogs_error(
				"OpenAPI_plmn_restriction_parseFromJSON() failed: Expected rat_restrictionsList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}

	forbidden_areas = cJSON_GetObjectItemCaseSensitive(plmn_restrictionJSON,
	                                                   "forbiddenAreas");
	if(forbidden_areas) {
		cJSON *forbidden_areas_local = NULL;
		if(!cJSON_IsArray(forbidden_areas)) {
			ogs_error(
				"OpenAPI_plmn_restriction_parseFromJSON() failed [forbidden_areas]");
			goto end;
		}
	...
	if(core_network_type_restrictions) {
		...
		if(!cJSON_IsArray(core_network_type_restrictions)) {
			ogs_error(
				"OpenAPI_plmn_restriction_parseFromJSON() failed [core_network_type_restrictions]");
			goto end;
		}
		...
		if(core_network_type_restrictionsList->count == 0) {
			ogs_error(
				"OpenAPI_plmn_restriction_parseFromJSON() failed: Expected core_network_type_restrictionsList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(primary_rat_restrictions) {
		...
		if(!cJSON_IsArray(primary_rat_restrictions)) {
			ogs_error(
				"OpenAPI_plmn_restriction_parseFromJSON() failed [primary_rat_restrictions]");
			goto end;
		}
		...
		if(primary_rat_restrictionsList->count == 0) {
			ogs_error(
				"OpenAPI_plmn_restriction_parseFromJSON() failed: Expected primary_rat_restrictionsList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(secondary_rat_restrictions) {
		...
		if(!cJSON_IsArray(secondary_rat_restrictions)) {
			ogs_error(
				"OpenAPI_plmn_restriction_parseFromJSON() failed [secondary_rat_restrictions]");
			goto end;
		}
		...
		if(secondary_rat_restrictionsList->count == 0) {
			ogs_error(
				"OpenAPI_plmn_restriction_parseFromJSON() failed: Expected secondary_rat_restrictionsList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	plmn_restriction_local_var = OpenAPI_plmn_restriction_create(
		...
		secondary_rat_restrictions ? secondary_rat_restrictionsList :
		NULL
		);
...
OpenAPI_plmn_restriction_t *OpenAPI_plmn_restriction_copy(
	OpenAPI_plmn_restriction_t *dst, OpenAPI_plmn_restriction_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
