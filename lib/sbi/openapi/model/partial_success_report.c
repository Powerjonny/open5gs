...
OpenAPI_partial_success_report_t *OpenAPI_partial_success_report_create(
	...
	OpenAPI_partial_success_report_t *partial_success_report_local_var =
		ogs_malloc(sizeof(OpenAPI_partial_success_report_t));
	...
	partial_success_report_local_var->policy_dec_failure_reports =
		policy_dec_failure_reports;
	partial_success_report_local_var->invalid_policy_decs =
		invalid_policy_decs;
...
void OpenAPI_partial_success_report_free(
	...
	if(partial_success_report->rule_reports) {
		OpenAPI_list_for_each(partial_success_report->rule_reports,
		                      node) {
	...
	if(partial_success_report->sess_rule_reports) {
		OpenAPI_list_for_each(partial_success_report->sess_rule_reports,
		                      node) {
	...
	if(partial_success_report->ue_camping_rep) {
		OpenAPI_ue_camping_rep_free(
			partial_success_report->ue_camping_rep);
	...
	if(partial_success_report->invalid_policy_decs) {
		OpenAPI_list_for_each(
			partial_success_report->invalid_policy_decs, node) {
...
cJSON *OpenAPI_partial_success_report_convertToJSON(
	...
	if(partial_success_report == NULL) {
		ogs_error(
			"OpenAPI_partial_success_report_convertToJSON() failed [PartialSuccessReport]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(partial_success_report->failure_cause ==
	   OpenAPI_failure_cause_NULL)
	{
		ogs_error(
			"OpenAPI_partial_success_report_convertToJSON() failed [failure_cause]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "failureCause",
	                           OpenAPI_failure_cause_ToString(
					   partial_success_report->failure_cause))
	   == NULL)
	{
		ogs_error(
			"OpenAPI_partial_success_report_convertToJSON() failed [failure_cause]");
	...
	if(partial_success_report->rule_reports) {
		...
		if(rule_reportsList == NULL) {
			ogs_error(
				"OpenAPI_partial_success_report_convertToJSON() failed [rule_reports]");
			goto end;
		}
		OpenAPI_list_for_each(partial_success_report->rule_reports,
		                      node) {
	...
	if(partial_success_report->sess_rule_reports) {
		...
		if(sess_rule_reportsList == NULL) {
			ogs_error(
				"OpenAPI_partial_success_report_convertToJSON() failed [sess_rule_reports]");
			goto end;
		}
		OpenAPI_list_for_each(partial_success_report->sess_rule_reports,
		                      node) {
	...
	if(partial_success_report->ue_camping_rep) {
		cJSON *ue_camping_rep_local_JSON =
			OpenAPI_ue_camping_rep_convertToJSON(
				partial_success_report->ue_camping_rep);
		if(ue_camping_rep_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_partial_success_report_convertToJSON() failed [ue_camping_rep]");
			goto end;
		}
		cJSON_AddItemToObject(item, "ueCampingRep",
		                      ue_camping_rep_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_partial_success_report_convertToJSON() failed [ue_camping_rep]");
			goto end;
	   ...
	   OpenAPI_policy_decision_failure_code_NULL)
	{
		...
		if(policy_dec_failure_reportsList == NULL) {
			ogs_error(
				"OpenAPI_partial_success_report_convertToJSON() failed [policy_dec_failure_reports]");
			goto end;
		}
		OpenAPI_list_for_each(
			partial_success_report->policy_dec_failure_reports,
	...
	if(partial_success_report->invalid_policy_decs) {
		...
		if(invalid_policy_decsList == NULL) {
			ogs_error(
				"OpenAPI_partial_success_report_convertToJSON() failed [invalid_policy_decs]");
			goto end;
		}
		OpenAPI_list_for_each(
			partial_success_report->invalid_policy_decs, node) {
...
OpenAPI_partial_success_report_t *OpenAPI_partial_success_report_parseFromJSON(
	...
	failure_cause =
		cJSON_GetObjectItemCaseSensitive(partial_success_reportJSON,
		                                 "failureCause");
	if(!failure_cause) {
		ogs_error(
			"OpenAPI_partial_success_report_parseFromJSON() failed [failure_cause]");
		goto end;
	}
	...
	failure_causeVariable =
		OpenAPI_failure_cause_FromString(failure_cause->valuestring);
	...
	if(rule_reports) {
		...
		if(!cJSON_IsArray(rule_reports)) {
			ogs_error(
				"OpenAPI_partial_success_report_parseFromJSON() failed [rule_reports]");
			goto end;
		}
	...
	if(sess_rule_reports) {
		...
		if(!cJSON_IsArray(sess_rule_reports)) {
			ogs_error(
				"OpenAPI_partial_success_report_parseFromJSON() failed [sess_rule_reports]");
			goto end;
		}
	...
	ue_camping_rep =
		cJSON_GetObjectItemCaseSensitive(partial_success_reportJSON,
		                                 "ueCampingRep");
	if(ue_camping_rep) {
		ue_camping_rep_local_nonprim =
			OpenAPI_ue_camping_rep_parseFromJSON(ue_camping_rep);
		if(!ue_camping_rep_local_nonprim) {
			ogs_error(
				"OpenAPI_ue_camping_rep_parseFromJSON failed [ue_camping_rep]");
			goto end;
		}
	}
	...
	if(policy_dec_failure_reports) {
		...
		if(!cJSON_IsArray(policy_dec_failure_reports)) {
			ogs_error(
				"OpenAPI_partial_success_report_parseFromJSON() failed [policy_dec_failure_reports]");
			goto end;
		}
		...
		if(policy_dec_failure_reportsList->count == 0) {
			ogs_error(
				"OpenAPI_partial_success_report_parseFromJSON() failed: Expected policy_dec_failure_reportsList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(invalid_policy_decs) {
		...
		if(!cJSON_IsArray(invalid_policy_decs)) {
			ogs_error(
				"OpenAPI_partial_success_report_parseFromJSON() failed [invalid_policy_decs]");
			goto end;
		}
	...
	partial_success_report_local_var =
		OpenAPI_partial_success_report_create(
			failure_causeVariable,
...
OpenAPI_partial_success_report_t *OpenAPI_partial_success_report_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_partial_success_report_convertToJSON() failed");
		return NULL;
	}
