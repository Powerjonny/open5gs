...
OpenAPI_traffic_influ_sub_t *OpenAPI_traffic_influ_sub_create(
	OpenAPI_list_t *dnns, OpenAPI_list_t *snssais,
	OpenAPI_list_t *internal_group_ids, OpenAPI_list_t *supis,
	char *notification_uri, char *expiry, char *supported_features,
	OpenAPI_list_t *reset_ids) {
	OpenAPI_traffic_influ_sub_t *traffic_influ_sub_local_var =
		ogs_malloc(sizeof(OpenAPI_traffic_influ_sub_t));
...
void OpenAPI_traffic_influ_sub_free(
	OpenAPI_traffic_influ_sub_t *traffic_influ_sub) {
	...
	if(traffic_influ_sub->internal_group_ids) {
		OpenAPI_list_for_each(traffic_influ_sub->internal_group_ids,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_traffic_influ_sub_convertToJSON(
	...
	if(traffic_influ_sub == NULL) {
		ogs_error(
			"OpenAPI_traffic_influ_sub_convertToJSON() failed [TrafficInfluSub]");
		return NULL;
	}
	...
	if(traffic_influ_sub->dnns) {
		...
		if(dnnsList == NULL) {
			ogs_error(
				"OpenAPI_traffic_influ_sub_convertToJSON() failed [dnns]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_influ_sub->dnns, node) {
	...
	if(traffic_influ_sub->snssais) {
		...
		if(snssaisList == NULL) {
			ogs_error(
				"OpenAPI_traffic_influ_sub_convertToJSON() failed [snssais]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_influ_sub->snssais, node) {
			cJSON *itemLocal =
	...
	if(traffic_influ_sub->internal_group_ids) {
		...
		if(internal_group_idsList == NULL) {
			ogs_error(
				"OpenAPI_traffic_influ_sub_convertToJSON() failed [internal_group_ids]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_influ_sub->internal_group_ids,
		                      node) {
	...
	if(traffic_influ_sub->supis) {
		...
		if(supisList == NULL) {
			ogs_error(
				"OpenAPI_traffic_influ_sub_convertToJSON() failed [supis]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_influ_sub->supis, node) {
	...
	if(!traffic_influ_sub->notification_uri) {
		ogs_error(
			"OpenAPI_traffic_influ_sub_convertToJSON() failed [notification_uri]");
		return NULL;
	}
	...
	if(traffic_influ_sub->expiry) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_influ_sub_convertToJSON() failed [expiry]");
			goto end;
		}
	}
	...
	if(traffic_influ_sub->supported_features) {
		if(cJSON_AddStringToObject(item, "supportedFeatures",
		                           traffic_influ_sub->supported_features)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_sub_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(traffic_influ_sub->reset_ids) {
		...
		if(reset_idsList == NULL) {
			ogs_error(
				"OpenAPI_traffic_influ_sub_convertToJSON() failed [reset_ids]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_influ_sub->reset_ids, node) {
...
OpenAPI_traffic_influ_sub_t *OpenAPI_traffic_influ_sub_parseFromJSON(
	cJSON *traffic_influ_subJSON) {
	...
	if(dnns) {
		...
		if(!cJSON_IsArray(dnns)) {
			ogs_error(
				"OpenAPI_traffic_influ_sub_parseFromJSON() failed [dnns]");
			goto end;
		}
	...
	snssais = cJSON_GetObjectItemCaseSensitive(traffic_influ_subJSON,
	                                           "snssais");
	if(snssais) {
		cJSON *snssais_local = NULL;
		if(!cJSON_IsArray(snssais)) {
			ogs_error(
				"OpenAPI_traffic_influ_sub_parseFromJSON() failed [snssais]");
			goto end;
		}
	...
	internal_group_ids =
		cJSON_GetObjectItemCaseSensitive(traffic_influ_subJSON,
		                                 "internalGroupIds");
	if(internal_group_ids) {
		cJSON *internal_group_ids_local = NULL;
		if(!cJSON_IsArray(internal_group_ids)) {
			ogs_error(
				"OpenAPI_traffic_influ_sub_parseFromJSON() failed [internal_group_ids]");
			goto end;
		}
	...
	supis = cJSON_GetObjectItemCaseSensitive(traffic_influ_subJSON,
	                                         "supis");
	if(supis) {
		cJSON *supis_local = NULL;
		if(!cJSON_IsArray(supis)) {
			ogs_error(
				"OpenAPI_traffic_influ_sub_parseFromJSON() failed [supis]");
			goto end;
		}
	...
	if(!notification_uri) {
		ogs_error(
			"OpenAPI_traffic_influ_sub_parseFromJSON() failed [notification_uri]");
		goto end;
	}
	...
	expiry = cJSON_GetObjectItemCaseSensitive(traffic_influ_subJSON,
	                                          "expiry");
	if(expiry) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_influ_sub_parseFromJSON() failed [expiry]");
			goto end;
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_influ_sub_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}

	reset_ids = cJSON_GetObjectItemCaseSensitive(traffic_influ_subJSON,
	                                             "resetIds");
	if(reset_ids) {
		cJSON *reset_ids_local = NULL;
		if(!cJSON_IsArray(reset_ids)) {
			ogs_error(
				"OpenAPI_traffic_influ_sub_parseFromJSON() failed [reset_ids]");
			goto end;
		}
...
OpenAPI_traffic_influ_sub_t *OpenAPI_traffic_influ_sub_copy(
	OpenAPI_traffic_influ_sub_t *dst, OpenAPI_traffic_influ_sub_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
