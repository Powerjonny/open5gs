...
OpenAPI_tsctsf_info_t *OpenAPI_tsctsf_info_create(
	...
	OpenAPI_tsctsf_info_t *tsctsf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_tsctsf_info_t));
	ogs_assert(tsctsf_info_local_var);

	tsctsf_info_local_var->s_nssai_info_list = s_nssai_info_list;
	tsctsf_info_local_var->external_group_identifiers_ranges =
...
void OpenAPI_tsctsf_info_free(OpenAPI_tsctsf_info_t *tsctsf_info) {
	...
	if(tsctsf_info->s_nssai_info_list) {
		OpenAPI_list_for_each(tsctsf_info->s_nssai_info_list, node) {
			...
			OpenAPI_snssai_tsctsf_info_item_free(
				localKeyValue->value);
	...
	if(tsctsf_info->external_group_identifiers_ranges) {
		OpenAPI_list_for_each(
			tsctsf_info->external_group_identifiers_ranges, node) {
	...
	if(tsctsf_info->internal_group_identifiers_ranges) {
		OpenAPI_list_for_each(
			tsctsf_info->internal_group_identifiers_ranges, node) {
			OpenAPI_internal_group_id_range_free(node->data);
		}
		OpenAPI_list_free(
			tsctsf_info->internal_group_identifiers_ranges);
...
cJSON *OpenAPI_tsctsf_info_convertToJSON(OpenAPI_tsctsf_info_t *tsctsf_info) {
	...
	if(tsctsf_info == NULL) {
		ogs_error(
			"OpenAPI_tsctsf_info_convertToJSON() failed [TsctsfInfo]");
		return NULL;
	}
	...
	if(tsctsf_info->s_nssai_info_list) {
		...
		if(s_nssai_info_list == NULL) {
			ogs_error(
				"OpenAPI_tsctsf_info_convertToJSON() failed [s_nssai_info_list]");
			goto end;
		}
		cJSON *localMapObject = s_nssai_info_list;
		if(tsctsf_info->s_nssai_info_list) {
			OpenAPI_list_for_each(tsctsf_info->s_nssai_info_list,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_snssai_tsctsf_info_item_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_tsctsf_info_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(tsctsf_info->external_group_identifiers_ranges) {
		...
		if(external_group_identifiers_rangesList == NULL) {
			ogs_error(
				"OpenAPI_tsctsf_info_convertToJSON() failed [external_group_identifiers_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			tsctsf_info->external_group_identifiers_ranges, node) {
	...
	if(tsctsf_info->supi_ranges) {
		...
		if(supi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_tsctsf_info_convertToJSON() failed [supi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(tsctsf_info->supi_ranges, node) {
			cJSON *itemLocal =
	...
	if(tsctsf_info->gpsi_ranges) {
		...
		if(gpsi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_tsctsf_info_convertToJSON() failed [gpsi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(tsctsf_info->gpsi_ranges, node) {
			cJSON *itemLocal =
	...
	if(tsctsf_info->internal_group_identifiers_ranges) {
		...
		if(internal_group_identifiers_rangesList == NULL) {
			ogs_error(
				"OpenAPI_tsctsf_info_convertToJSON() failed [internal_group_identifiers_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			tsctsf_info->internal_group_identifiers_ranges, node) {
...
OpenAPI_tsctsf_info_t *OpenAPI_tsctsf_info_parseFromJSON(
	...
	s_nssai_info_list = cJSON_GetObjectItemCaseSensitive(tsctsf_infoJSON,
	                                                     "sNssaiInfoList");
	if(s_nssai_info_list) {
		...
		{
			ogs_error(
				"OpenAPI_tsctsf_info_parseFromJSON() failed [s_nssai_info_list]");
			goto end;
		}
		if(cJSON_IsObject(s_nssai_info_list)) {
			...
			cJSON_ArrayForEach(s_nssai_info_list_local_map,
				...
				if(cJSON_IsObject(localMapObject)) {
					localMapKeyPair = OpenAPI_map_create(
						...
						OpenAPI_snssai_tsctsf_info_item_parseFromJSON
						        (localMapObject));
					...
					ogs_error(
						"OpenAPI_tsctsf_info_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	external_group_identifiers_ranges =
		cJSON_GetObjectItemCaseSensitive(tsctsf_infoJSON,
		                                 "externalGroupIdentifiersRanges");
	if(external_group_identifiers_ranges) {
		...
		if(!cJSON_IsArray(external_group_identifiers_ranges)) {
			ogs_error(
				"OpenAPI_tsctsf_info_parseFromJSON() failed [external_group_identifiers_ranges]");
			goto end;
		}
	...
	supi_ranges = cJSON_GetObjectItemCaseSensitive(tsctsf_infoJSON,
	                                               "supiRanges");
	if(supi_ranges) {
		cJSON *supi_ranges_local = NULL;
		if(!cJSON_IsArray(supi_ranges)) {
			ogs_error(
				"OpenAPI_tsctsf_info_parseFromJSON() failed [supi_ranges]");
			goto end;
		}
	...
	gpsi_ranges = cJSON_GetObjectItemCaseSensitive(tsctsf_infoJSON,
	                                               "gpsiRanges");
	if(gpsi_ranges) {
		cJSON *gpsi_ranges_local = NULL;
		if(!cJSON_IsArray(gpsi_ranges)) {
			ogs_error(
				"OpenAPI_tsctsf_info_parseFromJSON() failed [gpsi_ranges]");
			goto end;
		}
	...
	if(internal_group_identifiers_ranges) {
		...
		if(!cJSON_IsArray(internal_group_identifiers_ranges)) {
			ogs_error(
				"OpenAPI_tsctsf_info_parseFromJSON() failed [internal_group_identifiers_ranges]");
			goto end;
		}
