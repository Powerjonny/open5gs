...
OpenAPI_n2_information_transfer_rsp_data_create(
	OpenAPI_n2_information_transfer_result_e result,
	OpenAPI_pws_response_data_t *pws_rsp_data, char *supported_features) {
	OpenAPI_n2_information_transfer_rsp_data_t *
	        n2_information_transfer_rsp_data_local_var =
		ogs_malloc(sizeof(OpenAPI_n2_information_transfer_rsp_data_t));
...
void OpenAPI_n2_information_transfer_rsp_data_free(
	OpenAPI_n2_information_transfer_rsp_data_t *
	...
	if(n2_information_transfer_rsp_data->pws_rsp_data) {
		OpenAPI_pws_response_data_free(
...
cJSON *OpenAPI_n2_information_transfer_rsp_data_convertToJSON(
	...
	if(n2_information_transfer_rsp_data == NULL) {
		ogs_error(
			"OpenAPI_n2_information_transfer_rsp_data_convertToJSON() failed [N2InformationTransferRspData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(n2_information_transfer_rsp_data->result ==
	   OpenAPI_n2_information_transfer_result_NULL)
	{
		ogs_error(
			"OpenAPI_n2_information_transfer_rsp_data_convertToJSON() failed [result]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "result",
	                           OpenAPI_n2_information_transfer_result_ToString
	                                   (n2_information_transfer_rsp_data->
	                                   result)) == NULL)
	{
		ogs_error(
	...
	if(n2_information_transfer_rsp_data->pws_rsp_data) {
		cJSON *pws_rsp_data_local_JSON =
			OpenAPI_pws_response_data_convertToJSON(
				n2_information_transfer_rsp_data->pws_rsp_data);
		if(pws_rsp_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_n2_information_transfer_rsp_data_convertToJSON() failed [pws_rsp_data]");
			goto end;
		}
	...
	if(n2_information_transfer_rsp_data->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_n2_information_transfer_rsp_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
...
OpenAPI_n2_information_transfer_rsp_data_t *
OpenAPI_n2_information_transfer_rsp_data_parseFromJSON(
	...
	if(!result) {
		ogs_error(
			"OpenAPI_n2_information_transfer_rsp_data_parseFromJSON() failed [result]");
		goto end;
	}
	...
	resultVariable =
		OpenAPI_n2_information_transfer_result_FromString(
			result->valuestring);
	...
	if(pws_rsp_data) {
		pws_rsp_data_local_nonprim =
			OpenAPI_pws_response_data_parseFromJSON(pws_rsp_data);
		if(!pws_rsp_data_local_nonprim) {
			ogs_error(
				"OpenAPI_pws_response_data_parseFromJSON failed [pws_rsp_data]");
			goto end;
		}
	}
	...
	supported_features =
		cJSON_GetObjectItemCaseSensitive(
			n2_information_transfer_rsp_dataJSON,
			"supportedFeatures");
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_n2_information_transfer_rsp_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
...
OpenAPI_n2_information_transfer_rsp_data_t *
OpenAPI_n2_information_transfer_rsp_data_copy(
	OpenAPI_n2_information_transfer_rsp_data_t	*dst,
	OpenAPI_n2_information_transfer_rsp_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_n2_information_transfer_rsp_data_convertToJSON() failed");
		return NULL;
	}
