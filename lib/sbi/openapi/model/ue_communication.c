...
OpenAPI_ue_communication_t *OpenAPI_ue_communication_create(
	...
	float
	comm_dur_variance, bool is_perio_time, int perio_time,
	bool
	is_perio_time_variance, float perio_time_variance, char *ts,
	bool is_ts_variance, float ts_variance,
	OpenAPI_scheduled_communication_time_1_t *recurring_time,
	OpenAPI_traffic_characterization_t *traf_char, bool is_ratio, int ratio,
	bool is_perio_comm_ind, int perio_comm_ind, bool is_confidence,
	int confidence, OpenAPI_app_list_for_ue_comm_t *ana_of_app_list,
	OpenAPI_sess_inact_timer_for_ue_comm_t *sess_inact_timer) {
	OpenAPI_ue_communication_t *ue_communication_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_communication_t));
...
void OpenAPI_ue_communication_free(
	OpenAPI_ue_communication_t *ue_communication) {
...
cJSON *OpenAPI_ue_communication_convertToJSON(
	OpenAPI_ue_communication_t *ue_communication) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(ue_communication == NULL) {
		ogs_error(
			"OpenAPI_ue_communication_convertToJSON() failed [UeCommunication]");
		return NULL;
	}
	...
	if(ue_communication->is_comm_dur) {
		...
		{
			ogs_error(
				"OpenAPI_ue_communication_convertToJSON() failed [comm_dur]");
			goto end;
		}
	}
	...
	if(ue_communication->is_comm_dur_variance) {
		...
		{
			ogs_error(
				"OpenAPI_ue_communication_convertToJSON() failed [comm_dur_variance]");
			goto end;
		}
	}
	...
	if(ue_communication->is_perio_time) {
		...
		{
			ogs_error(
				"OpenAPI_ue_communication_convertToJSON() failed [perio_time]");
			goto end;
		}
	}
	...
	if(ue_communication->is_perio_time_variance) {
		...
		{
			ogs_error(
				"OpenAPI_ue_communication_convertToJSON() failed [perio_time_variance]");
			goto end;
		}
	}
	...
	if(ue_communication->ts) {
		...
		{
			ogs_error(
				"OpenAPI_ue_communication_convertToJSON() failed [ts]");
			goto end;
		}
	}
	...
	if(ue_communication->is_ts_variance) {
		...
		{
			ogs_error(
				"OpenAPI_ue_communication_convertToJSON() failed [ts_variance]");
			goto end;
		}
	}
	...
	if(ue_communication->recurring_time) {
		...
		if(recurring_time_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_communication_convertToJSON() failed [recurring_time]");
			goto end;
		}
	...
	if(ue_communication->traf_char) {
		cJSON *traf_char_local_JSON =
			OpenAPI_traffic_characterization_convertToJSON(
				ue_communication->traf_char);
		if(traf_char_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_communication_convertToJSON() failed [traf_char]");
			goto end;
		}
	...
	if(ue_communication->is_ratio) {
		...
		{
			ogs_error(
				"OpenAPI_ue_communication_convertToJSON() failed [ratio]");
			goto end;
		}
	}
	...
	if(ue_communication->is_perio_comm_ind) {
		...
		{
			ogs_error(
				"OpenAPI_ue_communication_convertToJSON() failed [perio_comm_ind]");
			goto end;
		}
	}
	...
	if(ue_communication->is_confidence) {
		...
		{
			ogs_error(
				"OpenAPI_ue_communication_convertToJSON() failed [confidence]");
			goto end;
		}
	}
	...
	if(ue_communication->ana_of_app_list) {
		cJSON *ana_of_app_list_local_JSON =
			OpenAPI_app_list_for_ue_comm_convertToJSON(
				ue_communication->ana_of_app_list);
		if(ana_of_app_list_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_communication_convertToJSON() failed [ana_of_app_list]");
			goto end;
		}
	...
	if(ue_communication->sess_inact_timer) {
		...
		if(sess_inact_timer_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_communication_convertToJSON() failed [sess_inact_timer]");
			goto end;
		}
...
OpenAPI_ue_communication_t *OpenAPI_ue_communication_parseFromJSON(
	cJSON *ue_communicationJSON) {
	...
	OpenAPI_scheduled_communication_time_1_t *recurring_time_local_nonprim =
		NULL;
	...
	OpenAPI_sess_inact_timer_for_ue_comm_t *sess_inact_timer_local_nonprim =
		NULL;
	...
	if(comm_dur) {
		if(!cJSON_IsNumber(comm_dur)) {
			ogs_error(
				"OpenAPI_ue_communication_parseFromJSON() failed [comm_dur]");
			goto end;
		}
	}

	comm_dur_variance =
		cJSON_GetObjectItemCaseSensitive(ue_communicationJSON,
		                                 "commDurVariance");
	if(comm_dur_variance) {
		if(!cJSON_IsNumber(comm_dur_variance)) {
			ogs_error(
				"OpenAPI_ue_communication_parseFromJSON() failed [comm_dur_variance]");
			goto end;
		}
	}
	...
	if(perio_time) {
		if(!cJSON_IsNumber(perio_time)) {
			ogs_error(
				"OpenAPI_ue_communication_parseFromJSON() failed [perio_time]");
			goto end;
		}
	}
	...
	if(perio_time_variance) {
		if(!cJSON_IsNumber(perio_time_variance)) {
			ogs_error(
				"OpenAPI_ue_communication_parseFromJSON() failed [perio_time_variance]");
			goto end;
		}
	}
	...
	if(ts) {
		...
		{
			ogs_error(
				"OpenAPI_ue_communication_parseFromJSON() failed [ts]");
			goto end;
		}
	}
	...
	if(ts_variance) {
		if(!cJSON_IsNumber(ts_variance)) {
			ogs_error(
				"OpenAPI_ue_communication_parseFromJSON() failed [ts_variance]");
			goto end;
		}
	}
	...
	if(recurring_time) {
		...
		if(!recurring_time_local_nonprim) {
			ogs_error(
				"OpenAPI_scheduled_communication_time_1_parseFromJSON failed [recurring_time]");
			goto end;
		}
	}
	...
	if(traf_char) {
		traf_char_local_nonprim =
			OpenAPI_traffic_characterization_parseFromJSON(
				traf_char);
		if(!traf_char_local_nonprim) {
			ogs_error(
				"OpenAPI_traffic_characterization_parseFromJSON failed [traf_char]");
			goto end;
		}
	}
	...
	if(ratio) {
		if(!cJSON_IsNumber(ratio)) {
			ogs_error(
				"OpenAPI_ue_communication_parseFromJSON() failed [ratio]");
			goto end;
		}
	}
	...
	if(perio_comm_ind) {
		if(!cJSON_IsBool(perio_comm_ind)) {
			ogs_error(
				"OpenAPI_ue_communication_parseFromJSON() failed [perio_comm_ind]");
			goto end;
		}
	}
	...
	if(confidence) {
		if(!cJSON_IsNumber(confidence)) {
			ogs_error(
				"OpenAPI_ue_communication_parseFromJSON() failed [confidence]");
			goto end;
		}
	}
	...
	if(ana_of_app_list) {
		ana_of_app_list_local_nonprim =
			OpenAPI_app_list_for_ue_comm_parseFromJSON(
				ana_of_app_list);
		if(!ana_of_app_list_local_nonprim) {
			ogs_error(
				"OpenAPI_app_list_for_ue_comm_parseFromJSON failed [ana_of_app_list]");
			goto end;
		}
	}
	...
	if(sess_inact_timer) {
		sess_inact_timer_local_nonprim =
			OpenAPI_sess_inact_timer_for_ue_comm_parseFromJSON(
				sess_inact_timer);
		if(!sess_inact_timer_local_nonprim) {
			ogs_error(
				"OpenAPI_sess_inact_timer_for_ue_comm_parseFromJSON failed [sess_inact_timer]");
			goto end;
		}
	}
	...
	ue_communication_local_var = OpenAPI_ue_communication_create(
		...
		ts &&
		!cJSON_IsNull(ts) ? ogs_strdup(ts->valuestring) : NULL,
...
OpenAPI_ue_communication_t *OpenAPI_ue_communication_copy(
	OpenAPI_ue_communication_t *dst, OpenAPI_ue_communication_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
