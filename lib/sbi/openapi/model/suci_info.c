...
OpenAPI_suci_info_t *OpenAPI_suci_info_create(OpenAPI_list_t *routing_inds,
                                              OpenAPI_list_t	*
                                              h_nw_pub_key_ids) {
	OpenAPI_suci_info_t *suci_info_local_var =
		ogs_malloc(sizeof(OpenAPI_suci_info_t));
...
cJSON *OpenAPI_suci_info_convertToJSON(OpenAPI_suci_info_t *suci_info) {
	...
	if(suci_info == NULL) {
		ogs_error(
			"OpenAPI_suci_info_convertToJSON() failed [SuciInfo]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(suci_info->routing_inds) {
		cJSON *routing_indsList = cJSON_AddArrayToObject(item,
		                                                 "routingInds");
		if(routing_indsList == NULL) {
			ogs_error(
				"OpenAPI_suci_info_convertToJSON() failed [routing_inds]");
			goto end;
		}
		OpenAPI_list_for_each(suci_info->routing_inds, node) {
			if(cJSON_AddStringToObject(routing_indsList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_suci_info_convertToJSON() failed [routing_inds]");
	...
	if(suci_info->h_nw_pub_key_ids) {
		cJSON *h_nw_pub_key_idsList = cJSON_AddArrayToObject(item,
		                                                     "hNwPubKeyIds");
		if(h_nw_pub_key_idsList == NULL) {
			ogs_error(
				"OpenAPI_suci_info_convertToJSON() failed [h_nw_pub_key_ids]");
			goto end;
		}
		OpenAPI_list_for_each(suci_info->h_nw_pub_key_ids, node) {
			if(node->data == NULL) {
			...
			if(cJSON_AddNumberToObject(h_nw_pub_key_idsList, "",
			                           *(double *) node->data) ==
			   NULL)
			{
				ogs_error(
...
OpenAPI_suci_info_t *OpenAPI_suci_info_parseFromJSON(cJSON *suci_infoJSON) {
	...
	routing_inds = cJSON_GetObjectItemCaseSensitive(suci_infoJSON,
	                                                "routingInds");
	if(routing_inds) {
		cJSON *routing_inds_local = NULL;
		if(!cJSON_IsArray(routing_inds)) {
			ogs_error(
				"OpenAPI_suci_info_parseFromJSON() failed [routing_inds]");
			goto end;
		}
		...
		cJSON_ArrayForEach(routing_inds_local, routing_inds) {
			...
			OpenAPI_list_add(routing_indsList,
			                 ogs_strdup(
						 routing_inds_local->valuestring));
		}
	}

	h_nw_pub_key_ids = cJSON_GetObjectItemCaseSensitive(suci_infoJSON,
	                                                    "hNwPubKeyIds");
	if(h_nw_pub_key_ids) {
		cJSON *h_nw_pub_key_ids_local = NULL;
		if(!cJSON_IsArray(h_nw_pub_key_ids)) {
			ogs_error(
				"OpenAPI_suci_info_parseFromJSON() failed [h_nw_pub_key_ids]");
			goto end;
		}
...
OpenAPI_suci_info_t *OpenAPI_suci_info_copy(OpenAPI_suci_info_t *dst,
                                            OpenAPI_suci_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
