...
OpenAPI_gmlc_info_t *OpenAPI_gmlc_info_create(
	...
	OpenAPI_gmlc_info_t *gmlc_info_local_var =
		ogs_malloc(sizeof(OpenAPI_gmlc_info_t));
...
cJSON *OpenAPI_gmlc_info_convertToJSON(OpenAPI_gmlc_info_t *gmlc_info) {
	...
	if(gmlc_info == NULL) {
		ogs_error(
			"OpenAPI_gmlc_info_convertToJSON() failed [GmlcInfo]");
		return NULL;
	}
	   ...
	   OpenAPI_external_client_type_NULL)
	{
		...
		if(serving_client_typesList == NULL) {
			ogs_error(
				"OpenAPI_gmlc_info_convertToJSON() failed [serving_client_types]");
			goto end;
		}
	...
	if(gmlc_info->gmlc_numbers) {
		...
		if(gmlc_numbersList == NULL) {
			ogs_error(
				"OpenAPI_gmlc_info_convertToJSON() failed [gmlc_numbers]");
			goto end;
		}
		OpenAPI_list_for_each(gmlc_info->gmlc_numbers, node) {
...
OpenAPI_gmlc_info_t *OpenAPI_gmlc_info_parseFromJSON(cJSON *gmlc_infoJSON) {
	...
	serving_client_types = cJSON_GetObjectItemCaseSensitive(gmlc_infoJSON,
	                                                        "servingClientTypes");
	if(serving_client_types) {
		cJSON *serving_client_types_local = NULL;
		if(!cJSON_IsArray(serving_client_types)) {
			ogs_error(
				"OpenAPI_gmlc_info_parseFromJSON() failed [serving_client_types]");
			goto end;
		}
		...
		if(serving_client_typesList->count == 0) {
			ogs_error(
				"OpenAPI_gmlc_info_parseFromJSON() failed: Expected serving_client_typesList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}

	gmlc_numbers = cJSON_GetObjectItemCaseSensitive(gmlc_infoJSON,
	                                                "gmlcNumbers");
	if(gmlc_numbers) {
		cJSON *gmlc_numbers_local = NULL;
		if(!cJSON_IsArray(gmlc_numbers)) {
			ogs_error(
				"OpenAPI_gmlc_info_parseFromJSON() failed [gmlc_numbers]");
			goto end;
		}
...
OpenAPI_gmlc_info_t *OpenAPI_gmlc_info_copy(OpenAPI_gmlc_info_t *dst,
                                            OpenAPI_gmlc_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
