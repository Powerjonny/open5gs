...
OpenAPI_sm_policy_dnn_data_t *OpenAPI_sm_policy_dnn_data_create(char *dnn,
                                                                OpenAPI_list_t *
                                                                allowed_services,
                                                                OpenAPI_list_t *
                                                                subsc_cats,
                                                                ...
                                                                adc_support,
                                                                bool
                                                                is_subsc_spending_limits,
                                                                int
                                                                subsc_spending_limits,
                                                                bool
                                                                is_ipv4_index,
                                                                int ipv4_index,
                                                                bool
                                                                ...
                                                                *
                                                                ...
                                                                mcs_priority,
                                                                bool
                                                                is_ims_signalling_prio,
                                                                int
                                                                ims_signalling_prio,
                                                                bool
                                                                is_mps_priority_level,
                                                                int
                                                                mps_priority_level,
                                                                bool
                                                                is_mcs_priority_level,
                                                                int
                                                                mcs_priority_level,
                                                                OpenAPI_list_t *
                                                                pra_infos,
                                                                ...
                                                                bdt_ref_ids,
                                                                bool
                                                                is_loc_rout_not_allowed,
                                                                int
                                                                loc_rout_not_allowed)
{
	OpenAPI_sm_policy_dnn_data_t *sm_policy_dnn_data_local_var =
		ogs_malloc(sizeof(OpenAPI_sm_policy_dnn_data_t));
	...
	sm_policy_dnn_data_local_var->is_subsc_spending_limits =
		is_subsc_spending_limits;
	sm_policy_dnn_data_local_var->subsc_spending_limits =
		subsc_spending_limits;
	...
	sm_policy_dnn_data_local_var->ref_um_data_limit_ids =
		ref_um_data_limit_ids;
	...
	sm_policy_dnn_data_local_var->is_ims_signalling_prio =
		is_ims_signalling_prio;
	sm_policy_dnn_data_local_var->ims_signalling_prio = ims_signalling_prio;
	sm_policy_dnn_data_local_var->is_mps_priority_level =
		is_mps_priority_level;
	sm_policy_dnn_data_local_var->mps_priority_level = mps_priority_level;
	sm_policy_dnn_data_local_var->is_mcs_priority_level =
		is_mcs_priority_level;
	...
	sm_policy_dnn_data_local_var->is_loc_rout_not_allowed =
		is_loc_rout_not_allowed;
	sm_policy_dnn_data_local_var->loc_rout_not_allowed =
		loc_rout_not_allowed;

	return sm_policy_dnn_data_local_var;
}

void OpenAPI_sm_policy_dnn_data_free(
	...
	if(sm_policy_dnn_data->allowed_services) {
		OpenAPI_list_for_each(sm_policy_dnn_data->allowed_services,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(sm_policy_dnn_data->ref_um_data_limit_ids) {
		OpenAPI_list_for_each(sm_policy_dnn_data->ref_um_data_limit_ids,
		                      node) {
			OpenAPI_map_t *localKeyValue =
				(OpenAPI_map_t *) node->data;
			ogs_free(localKeyValue->key);
			OpenAPI_limit_id_to_monitoring_key_free(
				localKeyValue->value);
...
cJSON *OpenAPI_sm_policy_dnn_data_convertToJSON(
	OpenAPI_sm_policy_dnn_data_t *sm_policy_dnn_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(sm_policy_dnn_data == NULL) {
		ogs_error(
			"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [SmPolicyDnnData]");
		return NULL;
	}
	...
	if(!sm_policy_dnn_data->dnn) {
		ogs_error(
			"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [dnn]");
		return NULL;
	}
	...
	if(sm_policy_dnn_data->allowed_services) {
		cJSON *allowed_servicesList = cJSON_AddArrayToObject(item,
		                                                     "allowedServices");
		if(allowed_servicesList == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [allowed_services]");
			goto end;
		}
		OpenAPI_list_for_each(sm_policy_dnn_data->allowed_services,
		                      node) {
			if(cJSON_AddStringToObject(allowed_servicesList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [allowed_services]");
	...
	if(sm_policy_dnn_data->subsc_cats) {
		cJSON *subsc_catsList = cJSON_AddArrayToObject(item,
		                                               "subscCats");
		if(subsc_catsList == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [subsc_cats]");
			goto end;
		}
		OpenAPI_list_for_each(sm_policy_dnn_data->subsc_cats, node) {
			if(cJSON_AddStringToObject(subsc_catsList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [subsc_cats]");
	...
	if(sm_policy_dnn_data->gbr_ul) {
		if(cJSON_AddStringToObject(item, "gbrUl",
		                           sm_policy_dnn_data->gbr_ul) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [gbr_ul]");
			goto end;
		}
	}

	if(sm_policy_dnn_data->gbr_dl) {
		if(cJSON_AddStringToObject(item, "gbrDl",
		                           sm_policy_dnn_data->gbr_dl) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [gbr_dl]");
			goto end;
		}
	}

	if(sm_policy_dnn_data->is_adc_support) {
		if(cJSON_AddBoolToObject(item, "adcSupport",
		                         sm_policy_dnn_data->adc_support) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [adc_support]");
			goto end;
		}
	}

	if(sm_policy_dnn_data->is_subsc_spending_limits) {
		if(cJSON_AddBoolToObject(item, "subscSpendingLimits",
		                         sm_policy_dnn_data->
		                         subsc_spending_limits) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [subsc_spending_limits]");
			goto end;
		}
	}
	...
	if(sm_policy_dnn_data->is_ipv4_index) {
		if(cJSON_AddNumberToObject(item, "ipv4Index",
		                           sm_policy_dnn_data->ipv4_index) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [ipv4_index]");
			goto end;
		}
	}
	...
	if(sm_policy_dnn_data->is_ipv6_index) {
		if(cJSON_AddNumberToObject(item, "ipv6Index",
		                           sm_policy_dnn_data->ipv6_index) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [ipv6_index]");
			goto end;
		}
	}
	...
	if(sm_policy_dnn_data->is_offline) {
		if(cJSON_AddBoolToObject(item, "offline",
		                         sm_policy_dnn_data->offline) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [offline]");
			goto end;
		}
	}
	...
	if(sm_policy_dnn_data->is_online) {
		if(cJSON_AddBoolToObject(item, "online",
		                         sm_policy_dnn_data->online) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [online]");
			goto end;
		}
	}
	...
	if(sm_policy_dnn_data->chf_info) {
		cJSON *chf_info_local_JSON =
			OpenAPI_charging_information_convertToJSON(
				sm_policy_dnn_data->chf_info);
		if(chf_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [chf_info]");
			goto end;
		}
	...
	if(sm_policy_dnn_data->ref_um_data_limit_ids) {
		cJSON *ref_um_data_limit_ids = cJSON_AddObjectToObject(item,
		                                                       "refUmDataLimitIds");
		if(ref_um_data_limit_ids == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [ref_um_data_limit_ids]");
			goto end;
		}
		cJSON *localMapObject = ref_um_data_limit_ids;
		if(sm_policy_dnn_data->ref_um_data_limit_ids) {
			OpenAPI_list_for_each(
				sm_policy_dnn_data->ref_um_data_limit_ids,
				node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(sm_policy_dnn_data->is_mps_priority) {
		if(cJSON_AddBoolToObject(item, "mpsPriority",
		                         sm_policy_dnn_data->mps_priority) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [mps_priority]");
			goto end;
		}
	}
	...
	if(sm_policy_dnn_data->is_mcs_priority) {
		if(cJSON_AddBoolToObject(item, "mcsPriority",
		                         sm_policy_dnn_data->mcs_priority) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [mcs_priority]");
			goto end;
		}
	}

	if(sm_policy_dnn_data->is_ims_signalling_prio) {
		if(cJSON_AddBoolToObject(item, "imsSignallingPrio",
		                         sm_policy_dnn_data->ims_signalling_prio)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [ims_signalling_prio]");
			goto end;
		}
	}
	...
	if(sm_policy_dnn_data->is_mps_priority_level) {
		if(cJSON_AddNumberToObject(item, "mpsPriorityLevel",
		                           sm_policy_dnn_data->
		                           mps_priority_level) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [mps_priority_level]");
			goto end;
		}
	}

	if(sm_policy_dnn_data->is_mcs_priority_level) {
		if(cJSON_AddNumberToObject(item, "mcsPriorityLevel",
		                           sm_policy_dnn_data->
		                           mcs_priority_level) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [mcs_priority_level]");
			goto end;
		}
	}
	...
	if(sm_policy_dnn_data->pra_infos) {
		...
		if(pra_infos == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [pra_infos]");
			goto end;
		}
		cJSON *localMapObject = pra_infos;
		if(sm_policy_dnn_data->pra_infos) {
			OpenAPI_list_for_each(sm_policy_dnn_data->pra_infos,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				if(localKeyValue == NULL) {
	...
	if(sm_policy_dnn_data->bdt_ref_ids) {
		...
		if(bdt_ref_ids == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [bdt_ref_ids]");
			goto end;
		}
		cJSON *localMapObject = bdt_ref_ids;
		if(sm_policy_dnn_data->bdt_ref_ids) {
			OpenAPI_list_for_each(sm_policy_dnn_data->bdt_ref_ids,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				if(localKeyValue == NULL) {
	...
	if(sm_policy_dnn_data->is_loc_rout_not_allowed) {
		if(cJSON_AddBoolToObject(item, "locRoutNotAllowed",
		                         sm_policy_dnn_data->
		                         loc_rout_not_allowed) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_convertToJSON() failed [loc_rout_not_allowed]");
			goto end;
		}
	}
...
OpenAPI_sm_policy_dnn_data_t *OpenAPI_sm_policy_dnn_data_parseFromJSON(
	cJSON *sm_policy_dnn_dataJSON) {
	...
	if(!dnn) {
		ogs_error(
			"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [dnn]");
		goto end;
	}
	...
	allowed_services =
		cJSON_GetObjectItemCaseSensitive(sm_policy_dnn_dataJSON,
		                                 "allowedServices");
	if(allowed_services) {
		cJSON *allowed_services_local = NULL;
		if(!cJSON_IsArray(allowed_services)) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [allowed_services]");
			goto end;
		}
		...
		cJSON_ArrayForEach(allowed_services_local, allowed_services) {
			...
			OpenAPI_list_add(allowed_servicesList,
			                 ogs_strdup(
						 allowed_services_local->
						 valuestring));
		}
	}

	subsc_cats = cJSON_GetObjectItemCaseSensitive(sm_policy_dnn_dataJSON,
	                                              "subscCats");
	if(subsc_cats) {
		cJSON *subsc_cats_local = NULL;
		if(!cJSON_IsArray(subsc_cats)) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [subsc_cats]");
			goto end;
		}
		...
		cJSON_ArrayForEach(subsc_cats_local, subsc_cats) {
			...
			OpenAPI_list_add(subsc_catsList,
			                 ogs_strdup(
						 subsc_cats_local->valuestring));
		}
	}

	gbr_ul = cJSON_GetObjectItemCaseSensitive(sm_policy_dnn_dataJSON,
	                                          "gbrUl");
	if(gbr_ul) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [gbr_ul]");
			goto end;
		}
	}

	gbr_dl = cJSON_GetObjectItemCaseSensitive(sm_policy_dnn_dataJSON,
	                                          "gbrDl");
	if(gbr_dl) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [gbr_dl]");
			goto end;
		}
	}

	adc_support = cJSON_GetObjectItemCaseSensitive(sm_policy_dnn_dataJSON,
	                                               "adcSupport");
	if(adc_support) {
		if(!cJSON_IsBool(adc_support)) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [adc_support]");
			goto end;
		}
	}
	...
	if(subsc_spending_limits) {
		if(!cJSON_IsBool(subsc_spending_limits)) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [subsc_spending_limits]");
			goto end;
		}
	}

	ipv4_index = cJSON_GetObjectItemCaseSensitive(sm_policy_dnn_dataJSON,
	                                              "ipv4Index");
	if(ipv4_index) {
		if(!cJSON_IsNumber(ipv4_index)) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [ipv4_index]");
			goto end;
		}
	}

	ipv6_index = cJSON_GetObjectItemCaseSensitive(sm_policy_dnn_dataJSON,
	                                              "ipv6Index");
	if(ipv6_index) {
		if(!cJSON_IsNumber(ipv6_index)) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [ipv6_index]");
			goto end;
		}
	}

	offline = cJSON_GetObjectItemCaseSensitive(sm_policy_dnn_dataJSON,
	                                           "offline");
	if(offline) {
		if(!cJSON_IsBool(offline)) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [offline]");
			goto end;
		}
	}

	online = cJSON_GetObjectItemCaseSensitive(sm_policy_dnn_dataJSON,
	                                          "online");
	if(online) {
		if(!cJSON_IsBool(online)) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [online]");
			goto end;
		}
	}

	chf_info = cJSON_GetObjectItemCaseSensitive(sm_policy_dnn_dataJSON,
	                                            "chfInfo");
	if(chf_info) {
	...
	if(ref_um_data_limit_ids) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [ref_um_data_limit_ids]");
			goto end;
		}
		if(cJSON_IsObject(ref_um_data_limit_ids)) {
			ref_um_data_limit_idsList = OpenAPI_list_create();
			OpenAPI_map_t *localMapKeyPair = NULL;
			cJSON_ArrayForEach(ref_um_data_limit_ids_local_map,
			                   ref_um_data_limit_ids) {
				cJSON *localMapObject =
					ref_um_data_limit_ids_local_map;
				if(cJSON_IsObject(localMapObject)) {
					localMapKeyPair = OpenAPI_map_create(
						ogs_strdup(
							localMapObject->string),
						OpenAPI_limit_id_to_monitoring_key_parseFromJSON
						        (localMapObject));
					...
					ogs_error(
						"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [inner]");
					goto end;
				}
				OpenAPI_list_add(ref_um_data_limit_idsList,
				                 localMapKeyPair);
			}
		}
	}

	mps_priority = cJSON_GetObjectItemCaseSensitive(sm_policy_dnn_dataJSON,
	                                                "mpsPriority");
	if(mps_priority) {
		if(!cJSON_IsBool(mps_priority)) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [mps_priority]");
			goto end;
		}
	}

	mcs_priority = cJSON_GetObjectItemCaseSensitive(sm_policy_dnn_dataJSON,
	                                                "mcsPriority");
	if(mcs_priority) {
		if(!cJSON_IsBool(mcs_priority)) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [mcs_priority]");
			goto end;
		}
	}
	...
	if(ims_signalling_prio) {
		if(!cJSON_IsBool(ims_signalling_prio)) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [ims_signalling_prio]");
			goto end;
		}
	}
	...
	if(mps_priority_level) {
		if(!cJSON_IsNumber(mps_priority_level)) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [mps_priority_level]");
			goto end;
		}
	}
	...
	if(mcs_priority_level) {
		if(!cJSON_IsNumber(mcs_priority_level)) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [mcs_priority_level]");
			goto end;
		}
	}

	pra_infos = cJSON_GetObjectItemCaseSensitive(sm_policy_dnn_dataJSON,
	                                             "praInfos");
	if(pra_infos) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [pra_infos]");
			goto end;
		}
		if(cJSON_IsObject(pra_infos)) {
	...
	bdt_ref_ids = cJSON_GetObjectItemCaseSensitive(sm_policy_dnn_dataJSON,
	                                               "bdtRefIds");
	if(bdt_ref_ids) {
		if(!cJSON_IsNull(bdt_ref_ids)) {
			cJSON *bdt_ref_ids_local_map = NULL;
			if(!cJSON_IsObject(bdt_ref_ids) &&
			   !cJSON_IsNull(bdt_ref_ids))
			{
				ogs_error(
					"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [bdt_ref_ids]");
				goto end;
			}
			if(cJSON_IsObject(bdt_ref_ids)) {
				bdt_ref_idsList = OpenAPI_list_create();
				OpenAPI_map_t *localMapKeyPair = NULL;
				cJSON_ArrayForEach(bdt_ref_ids_local_map,
				                   bdt_ref_ids) {
					cJSON *localMapObject =
						bdt_ref_ids_local_map;
					double *localDouble = NULL;
					int *localInt = NULL;
					...
					localMapKeyPair =
						OpenAPI_map_create(ogs_strdup(
									   ...
									   ->
									   valuestring));
					OpenAPI_list_add(bdt_ref_idsList,
					                 localMapKeyPair);
				}
			}
		}
	...
	if(loc_rout_not_allowed) {
		if(!cJSON_IsBool(loc_rout_not_allowed)) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_parseFromJSON() failed [loc_rout_not_allowed]");
			goto end;
		}
	}
	...
	sm_policy_dnn_data_local_var = OpenAPI_sm_policy_dnn_data_create(
		...
		gbr_ul &&
		!cJSON_IsNull(gbr_ul) ? ogs_strdup(gbr_ul->valuestring) : NULL,
		gbr_dl &&
		!cJSON_IsNull(gbr_dl) ? ogs_strdup(gbr_dl->valuestring) : NULL,
		...
		bdt_ref_ids &&
		cJSON_IsNull(bdt_ref_ids) ? true : false,
...
OpenAPI_sm_policy_dnn_data_t *OpenAPI_sm_policy_dnn_data_copy(
	OpenAPI_sm_policy_dnn_data_t *dst, OpenAPI_sm_policy_dnn_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
