...
OpenAPI_frame_route_info_1_t *OpenAPI_frame_route_info_1_create(
	...
	char	*
	ipv6_prefix) {
	OpenAPI_frame_route_info_1_t *frame_route_info_1_local_var =
		ogs_malloc(sizeof(OpenAPI_frame_route_info_1_t));
...
cJSON *OpenAPI_frame_route_info_1_convertToJSON(
	...
	if(frame_route_info_1 == NULL) {
		ogs_error(
			"OpenAPI_frame_route_info_1_convertToJSON() failed [FrameRouteInfo_1]");
		return NULL;
	}
	...
	if(frame_route_info_1->ipv4_mask) {
		if(cJSON_AddStringToObject(item, "ipv4Mask",
		                           frame_route_info_1->ipv4_mask) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_frame_route_info_1_convertToJSON() failed [ipv4_mask]");
			goto end;
		}
	}
	...
	if(frame_route_info_1->ipv6_prefix) {
		if(cJSON_AddStringToObject(item, "ipv6Prefix",
		                           frame_route_info_1->ipv6_prefix) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_frame_route_info_1_convertToJSON() failed [ipv6_prefix]");
			goto end;
		}
	}
...
OpenAPI_frame_route_info_1_t *OpenAPI_frame_route_info_1_parseFromJSON(
	...
	ipv4_mask = cJSON_GetObjectItemCaseSensitive(frame_route_info_1JSON,
	                                             "ipv4Mask");
	if(ipv4_mask) {
		...
		{
			ogs_error(
				"OpenAPI_frame_route_info_1_parseFromJSON() failed [ipv4_mask]");
			goto end;
		}
	}

	ipv6_prefix = cJSON_GetObjectItemCaseSensitive(frame_route_info_1JSON,
	                                               "ipv6Prefix");
	if(ipv6_prefix) {
		...
		{
			ogs_error(
				"OpenAPI_frame_route_info_1_parseFromJSON() failed [ipv6_prefix]");
			goto end;
		}
	}
