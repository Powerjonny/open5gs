...
OpenAPI_pcscf_address_t *OpenAPI_pcscf_address_create(
	...
	OpenAPI_pcscf_address_t *pcscf_address_local_var =
		ogs_malloc(sizeof(OpenAPI_pcscf_address_t));
...
cJSON *OpenAPI_pcscf_address_convertToJSON(
	OpenAPI_pcscf_address_t *pcscf_address) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(pcscf_address == NULL) {
		ogs_error(
			"OpenAPI_pcscf_address_convertToJSON() failed [PcscfAddress]");
		return NULL;
	}
	...
	if(pcscf_address->ipv4_addrs) {
		...
		if(ipv4_addrsList == NULL) {
			ogs_error(
				"OpenAPI_pcscf_address_convertToJSON() failed [ipv4_addrs]");
			goto end;
		}
		OpenAPI_list_for_each(pcscf_address->ipv4_addrs, node) {
	...
	if(pcscf_address->ipv6_addrs) {
		...
		if(ipv6_addrsList == NULL) {
			ogs_error(
				"OpenAPI_pcscf_address_convertToJSON() failed [ipv6_addrs]");
			goto end;
		}
		OpenAPI_list_for_each(pcscf_address->ipv6_addrs, node) {
	...
	if(pcscf_address->fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_pcscf_address_convertToJSON() failed [fqdn]");
			goto end;
		}
	}
...
OpenAPI_pcscf_address_t *OpenAPI_pcscf_address_parseFromJSON(
	...
	cJSON *fqdn = NULL;
	ipv4_addrs = cJSON_GetObjectItemCaseSensitive(pcscf_addressJSON,
	                                              "ipv4Addrs");
	if(ipv4_addrs) {
		cJSON *ipv4_addrs_local = NULL;
		if(!cJSON_IsArray(ipv4_addrs)) {
			ogs_error(
				"OpenAPI_pcscf_address_parseFromJSON() failed [ipv4_addrs]");
			goto end;
		}
	...
	ipv6_addrs = cJSON_GetObjectItemCaseSensitive(pcscf_addressJSON,
	                                              "ipv6Addrs");
	if(ipv6_addrs) {
		cJSON *ipv6_addrs_local = NULL;
		if(!cJSON_IsArray(ipv6_addrs)) {
			ogs_error(
				"OpenAPI_pcscf_address_parseFromJSON() failed [ipv6_addrs]");
			goto end;
		}
	...
	if(fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_pcscf_address_parseFromJSON() failed [fqdn]");
			goto end;
		}
	}
...
OpenAPI_pcscf_address_t *OpenAPI_pcscf_address_copy(
	OpenAPI_pcscf_address_t *dst, OpenAPI_pcscf_address_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
