...
OpenAPI_slice_mbr_rm_t *OpenAPI_slice_mbr_rm_create(char	*uplink,
                                                    char	*downlink) {
	OpenAPI_slice_mbr_rm_t *slice_mbr_rm_local_var =
		ogs_malloc(sizeof(OpenAPI_slice_mbr_rm_t));
...
cJSON *OpenAPI_slice_mbr_rm_convertToJSON(
	OpenAPI_slice_mbr_rm_t *slice_mbr_rm) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(slice_mbr_rm == NULL) {
		ogs_error(
			"OpenAPI_slice_mbr_rm_convertToJSON() failed [SliceMbrRm]");
		return NULL;
	}
	...
	if(!slice_mbr_rm->uplink) {
		ogs_error(
			"OpenAPI_slice_mbr_rm_convertToJSON() failed [uplink]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "uplink",
	                           slice_mbr_rm->uplink) == NULL)
	{
		ogs_error(
			"OpenAPI_slice_mbr_rm_convertToJSON() failed [uplink]");
	...
	if(!slice_mbr_rm->downlink) {
		ogs_error(
			"OpenAPI_slice_mbr_rm_convertToJSON() failed [downlink]");
		return NULL;
	}
...
OpenAPI_slice_mbr_rm_t *OpenAPI_slice_mbr_rm_parseFromJSON(
	cJSON *slice_mbr_rmJSON) {
	...
	if(!uplink) {
		ogs_error(
			"OpenAPI_slice_mbr_rm_parseFromJSON() failed [uplink]");
		goto end;
	}
	...
	downlink = cJSON_GetObjectItemCaseSensitive(slice_mbr_rmJSON,
	                                            "downlink");
	if(!downlink) {
		ogs_error(
			"OpenAPI_slice_mbr_rm_parseFromJSON() failed [downlink]");
		goto end;
	}
...
OpenAPI_slice_mbr_rm_t *OpenAPI_slice_mbr_rm_copy(
	OpenAPI_slice_mbr_rm_t	*dst,
