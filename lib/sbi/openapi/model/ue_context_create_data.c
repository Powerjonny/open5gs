...
OpenAPI_ue_context_create_data_t *OpenAPI_ue_context_create_data_create(
	OpenAPI_ue_context_t *ue_context, OpenAPI_ng_ran_target_id_t *target_id,
	OpenAPI_n2_info_content_t *source_to_target_data,
	OpenAPI_list_t *pdu_session_list, char *n2_notify_uri,
	OpenAPI_n2_info_content_t *ue_radio_capability,
	OpenAPI_n2_info_content_t *ue_radio_capability_for_paging,
	OpenAPI_ng_ap_cause_t *ngap_cause, char *supported_features,
	OpenAPI_plmn_id_nid_t *serving_network) {
	OpenAPI_ue_context_create_data_t *ue_context_create_data_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_context_create_data_t));
...
void OpenAPI_ue_context_create_data_free(
	OpenAPI_ue_context_create_data_t *ue_context_create_data) {
	...
	if(ue_context_create_data->pdu_session_list) {
		OpenAPI_list_for_each(ue_context_create_data->pdu_session_list,
		                      node) {
...
cJSON *OpenAPI_ue_context_create_data_convertToJSON(
	OpenAPI_ue_context_create_data_t *ue_context_create_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(ue_context_create_data == NULL) {
		ogs_error(
			"OpenAPI_ue_context_create_data_convertToJSON() failed [UeContextCreateData]");
		return NULL;
	}
	...
	if(!ue_context_create_data->ue_context) {
		ogs_error(
			"OpenAPI_ue_context_create_data_convertToJSON() failed [ue_context]");
		return NULL;
	}
	...
	if(!ue_context_create_data->target_id) {
		ogs_error(
			"OpenAPI_ue_context_create_data_convertToJSON() failed [target_id]");
		return NULL;
	}
	...
	if(!ue_context_create_data->source_to_target_data) {
		ogs_error(
			"OpenAPI_ue_context_create_data_convertToJSON() failed [source_to_target_data]");
		return NULL;
	}
	cJSON *source_to_target_data_local_JSON =
	...
	if(!ue_context_create_data->pdu_session_list) {
		ogs_error(
			"OpenAPI_ue_context_create_data_convertToJSON() failed [pdu_session_list]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(ue_context_create_data->pdu_session_list, node) {
		cJSON *itemLocal =
			OpenAPI_n2_sm_information_convertToJSON(node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_ue_context_create_data_convertToJSON() failed [pdu_session_list]");
			goto end;
	...
	if(ue_context_create_data->n2_notify_uri) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_create_data_convertToJSON() failed [n2_notify_uri]");
			goto end;
		}
	}
	...
	if(ue_context_create_data->ue_radio_capability) {
		...
		if(ue_radio_capability_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_create_data_convertToJSON() failed [ue_radio_capability]");
			goto end;
		}
	...
	if(ue_context_create_data->ue_radio_capability_for_paging) {
		...
		if(ue_radio_capability_for_paging_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_create_data_convertToJSON() failed [ue_radio_capability_for_paging]");
			goto end;
		}
	...
	if(ue_context_create_data->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_create_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(ue_context_create_data->serving_network) {
		...
		if(serving_network_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_create_data_convertToJSON() failed [serving_network]");
			goto end;
		}
...
OpenAPI_ue_context_create_data_t *OpenAPI_ue_context_create_data_parseFromJSON(
	...
	OpenAPI_n2_info_content_t *ue_radio_capability_for_paging_local_nonprim
	        = NULL;
	...
	ue_context =
		cJSON_GetObjectItemCaseSensitive(ue_context_create_dataJSON,
		                                 "ueContext");
	if(!ue_context) {
		ogs_error(
			"OpenAPI_ue_context_create_data_parseFromJSON() failed [ue_context]");
		goto end;
	}
	...
	if(!target_id) {
		ogs_error(
			"OpenAPI_ue_context_create_data_parseFromJSON() failed [target_id]");
		goto end;
	}
	...
	if(!source_to_target_data) {
		ogs_error(
			"OpenAPI_ue_context_create_data_parseFromJSON() failed [source_to_target_data]");
		goto end;
	}
	source_to_target_data_local_nonprim =
		OpenAPI_n2_info_content_parseFromJSON(source_to_target_data);
	if(!source_to_target_data_local_nonprim) {
		ogs_error(
			"OpenAPI_n2_info_content_parseFromJSON failed [source_to_target_data]");
		goto end;
	}
	...
	if(!pdu_session_list) {
		ogs_error(
			"OpenAPI_ue_context_create_data_parseFromJSON() failed [pdu_session_list]");
		goto end;
	}
	...
	cJSON_ArrayForEach(pdu_session_list_local, pdu_session_list) {
		...
		OpenAPI_n2_sm_information_t *pdu_session_listItem =
	...
	if(n2_notify_uri) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_create_data_parseFromJSON() failed [n2_notify_uri]");
			goto end;
		}
	}
	...
	if(ue_radio_capability) {
		...
		if(!ue_radio_capability_local_nonprim) {
			ogs_error(
				"OpenAPI_n2_info_content_parseFromJSON failed [ue_radio_capability]");
			goto end;
		}
	}
	...
	if(ue_radio_capability_for_paging) {
		...
		if(!ue_radio_capability_for_paging_local_nonprim) {
			ogs_error(
				"OpenAPI_n2_info_content_parseFromJSON failed [ue_radio_capability_for_paging]");
			goto end;
		}
	}
	...
	if(ngap_cause) {
		ngap_cause_local_nonprim =
			OpenAPI_ng_ap_cause_parseFromJSON(ngap_cause);
		if(!ngap_cause_local_nonprim) {
			ogs_error(
				"OpenAPI_ng_ap_cause_parseFromJSON failed [ngap_cause]");
			goto end;
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_create_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	ue_context_create_data_local_var =
		OpenAPI_ue_context_create_data_create(
			...
			n2_notify_uri &&
			!cJSON_IsNull(n2_notify_uri) ?
			ogs_strdup(n2_notify_uri->valuestring) : NULL,
			ue_radio_capability ?
...
OpenAPI_ue_context_create_data_t *OpenAPI_ue_context_create_data_copy(
	OpenAPI_ue_context_create_data_t	*dst,
	OpenAPI_ue_context_create_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ue_context_create_data_convertToJSON() failed");
		return NULL;
	}
