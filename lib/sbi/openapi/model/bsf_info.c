...
OpenAPI_bsf_info_t *OpenAPI_bsf_info_create(OpenAPI_list_t *dnn_list,
                                            OpenAPI_list_t *ip_domain_list,
                                            OpenAPI_list_t *ipv4_address_ranges,
                                            OpenAPI_list_t *ipv6_prefix_ranges,
                                            char *rx_diam_host,
                                            char *rx_diam_realm, char *group_id,
                                            OpenAPI_list_t *supi_ranges,
                                            OpenAPI_list_t *gpsi_ranges) {
	OpenAPI_bsf_info_t *bsf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_bsf_info_t));
...
cJSON *OpenAPI_bsf_info_convertToJSON(OpenAPI_bsf_info_t *bsf_info) {
	...
	if(bsf_info->dnn_list) {
		...
		if(dnn_listList == NULL) {
			ogs_error(
				"OpenAPI_bsf_info_convertToJSON() failed [dnn_list]");
			goto end;
		}
		OpenAPI_list_for_each(bsf_info->dnn_list, node) {
	...
	if(bsf_info->ip_domain_list) {
		...
		if(ip_domain_listList == NULL) {
			ogs_error(
				"OpenAPI_bsf_info_convertToJSON() failed [ip_domain_list]");
			goto end;
		}
		OpenAPI_list_for_each(bsf_info->ip_domain_list, node) {
	...
	if(bsf_info->ipv4_address_ranges) {
		...
		if(ipv4_address_rangesList == NULL) {
			ogs_error(
				"OpenAPI_bsf_info_convertToJSON() failed [ipv4_address_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(bsf_info->ipv4_address_ranges, node) {
			cJSON *itemLocal =
	...
	if(bsf_info->ipv6_prefix_ranges) {
		...
		if(ipv6_prefix_rangesList == NULL) {
			ogs_error(
				"OpenAPI_bsf_info_convertToJSON() failed [ipv6_prefix_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(bsf_info->ipv6_prefix_ranges, node) {
			cJSON *itemLocal =
	...
	if(bsf_info->rx_diam_host) {
		...
		{
			ogs_error(
				"OpenAPI_bsf_info_convertToJSON() failed [rx_diam_host]");
			goto end;
		}
	}
	...
	if(bsf_info->rx_diam_realm) {
		...
		{
			ogs_error(
				"OpenAPI_bsf_info_convertToJSON() failed [rx_diam_realm]");
			goto end;
		}
	}
	...
	if(bsf_info->group_id) {
		...
		{
			ogs_error(
				"OpenAPI_bsf_info_convertToJSON() failed [group_id]");
			goto end;
		}
	}
	...
	if(bsf_info->supi_ranges) {
		...
		if(supi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_bsf_info_convertToJSON() failed [supi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(bsf_info->supi_ranges, node) {
			cJSON *itemLocal =
	...
	if(bsf_info->gpsi_ranges) {
		...
		if(gpsi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_bsf_info_convertToJSON() failed [gpsi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(bsf_info->gpsi_ranges, node) {
			cJSON *itemLocal =
...
OpenAPI_bsf_info_t *OpenAPI_bsf_info_parseFromJSON(cJSON *bsf_infoJSON) {
	...
	if(dnn_list) {
		...
		if(!cJSON_IsArray(dnn_list)) {
			ogs_error(
				"OpenAPI_bsf_info_parseFromJSON() failed [dnn_list]");
			goto end;
		}
	...
	ip_domain_list = cJSON_GetObjectItemCaseSensitive(bsf_infoJSON,
	                                                  "ipDomainList");
	if(ip_domain_list) {
		cJSON *ip_domain_list_local = NULL;
		if(!cJSON_IsArray(ip_domain_list)) {
			ogs_error(
				"OpenAPI_bsf_info_parseFromJSON() failed [ip_domain_list]");
			goto end;
		}
	...
	ipv4_address_ranges = cJSON_GetObjectItemCaseSensitive(bsf_infoJSON,
	                                                       "ipv4AddressRanges");
	if(ipv4_address_ranges) {
		cJSON *ipv4_address_ranges_local = NULL;
		if(!cJSON_IsArray(ipv4_address_ranges)) {
			ogs_error(
				"OpenAPI_bsf_info_parseFromJSON() failed [ipv4_address_ranges]");
			goto end;
		}
	...
	ipv6_prefix_ranges = cJSON_GetObjectItemCaseSensitive(bsf_infoJSON,
	                                                      "ipv6PrefixRanges");
	if(ipv6_prefix_ranges) {
		cJSON *ipv6_prefix_ranges_local = NULL;
		if(!cJSON_IsArray(ipv6_prefix_ranges)) {
			ogs_error(
				"OpenAPI_bsf_info_parseFromJSON() failed [ipv6_prefix_ranges]");
			goto end;
		}
	...
	rx_diam_host = cJSON_GetObjectItemCaseSensitive(bsf_infoJSON,
	                                                "rxDiamHost");
	if(rx_diam_host) {
		...
		{
			ogs_error(
				"OpenAPI_bsf_info_parseFromJSON() failed [rx_diam_host]");
			goto end;
		}
	}

	rx_diam_realm = cJSON_GetObjectItemCaseSensitive(bsf_infoJSON,
	                                                 "rxDiamRealm");
	if(rx_diam_realm) {
		...
		{
			ogs_error(
				"OpenAPI_bsf_info_parseFromJSON() failed [rx_diam_realm]");
			goto end;
		}
	}
	...
	if(group_id) {
		...
		{
			ogs_error(
				"OpenAPI_bsf_info_parseFromJSON() failed [group_id]");
			goto end;
		}
	}

	supi_ranges = cJSON_GetObjectItemCaseSensitive(bsf_infoJSON,
	                                               "supiRanges");
	if(supi_ranges) {
		cJSON *supi_ranges_local = NULL;
		if(!cJSON_IsArray(supi_ranges)) {
			ogs_error(
				"OpenAPI_bsf_info_parseFromJSON() failed [supi_ranges]");
			goto end;
		}
	...
	gpsi_ranges = cJSON_GetObjectItemCaseSensitive(bsf_infoJSON,
	                                               "gpsiRanges");
	if(gpsi_ranges) {
		cJSON *gpsi_ranges_local = NULL;
		if(!cJSON_IsArray(gpsi_ranges)) {
			ogs_error(
				"OpenAPI_bsf_info_parseFromJSON() failed [gpsi_ranges]");
			goto end;
		}
	...
	bsf_info_local_var = OpenAPI_bsf_info_create(
		...
		group_id &&
		!cJSON_IsNull(group_id) ? ogs_strdup(group_id->valuestring) :
		NULL,
...
OpenAPI_bsf_info_t *OpenAPI_bsf_info_copy(OpenAPI_bsf_info_t	*dst,
                                          OpenAPI_bsf_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
