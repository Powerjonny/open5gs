...
OpenAPI_af_routing_requirement_t *OpenAPI_af_routing_requirement_create(
	bool is_app_reloc, int app_reloc, OpenAPI_list_t *route_to_locs,
	OpenAPI_spatial_validity_t *sp_val, OpenAPI_list_t *temp_vals,
	bool is_up_path_chg_sub_null,
	OpenAPI_up_path_chg_event_t *up_path_chg_sub, bool is_addr_preser_ind,
	int addr_preser_ind, bool is_sim_conn_ind, int sim_conn_ind,
	bool is_sim_conn_term, int sim_conn_term,
	OpenAPI_list_t *eas_ip_replace_infos, bool is_eas_redis_ind,
	int eas_redis_ind, bool is_max_allowed_up_lat, int max_allowed_up_lat) {
	OpenAPI_af_routing_requirement_t *af_routing_requirement_local_var =
		ogs_malloc(sizeof(OpenAPI_af_routing_requirement_t));
	...
	af_routing_requirement_local_var->is_up_path_chg_sub_null =
		is_up_path_chg_sub_null;
	af_routing_requirement_local_var->up_path_chg_sub = up_path_chg_sub;
	af_routing_requirement_local_var->is_addr_preser_ind =
		is_addr_preser_ind;
	...
	af_routing_requirement_local_var->eas_ip_replace_infos =
	...
	af_routing_requirement_local_var->is_max_allowed_up_lat =
		is_max_allowed_up_lat;
	af_routing_requirement_local_var->max_allowed_up_lat =
		max_allowed_up_lat;

	return af_routing_requirement_local_var;
}

void OpenAPI_af_routing_requirement_free(
	OpenAPI_af_routing_requirement_t *af_routing_requirement) {
	...
	if(af_routing_requirement->route_to_locs) {
		OpenAPI_list_for_each(af_routing_requirement->route_to_locs,
		                      node) {
...
cJSON *OpenAPI_af_routing_requirement_convertToJSON(
	OpenAPI_af_routing_requirement_t *af_routing_requirement) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(af_routing_requirement == NULL) {
		ogs_error(
			"OpenAPI_af_routing_requirement_convertToJSON() failed [AfRoutingRequirement]");
		return NULL;
	}
	...
	if(af_routing_requirement->is_app_reloc) {
		if(cJSON_AddBoolToObject(item, "appReloc",
		                         af_routing_requirement->app_reloc) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_af_routing_requirement_convertToJSON() failed [app_reloc]");
			goto end;
		}
	}
	...
	if(af_routing_requirement->route_to_locs) {
		...
		if(route_to_locsList == NULL) {
			ogs_error(
				"OpenAPI_af_routing_requirement_convertToJSON() failed [route_to_locs]");
			goto end;
		}
		OpenAPI_list_for_each(af_routing_requirement->route_to_locs,
		                      node) {
	...
	if(af_routing_requirement->sp_val) {
		cJSON *sp_val_local_JSON =
			OpenAPI_spatial_validity_convertToJSON(
				af_routing_requirement->sp_val);
		if(sp_val_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_af_routing_requirement_convertToJSON() failed [sp_val]");
			goto end;
		}
	...
	if(af_routing_requirement->temp_vals) {
		...
		if(temp_valsList == NULL) {
			ogs_error(
				"OpenAPI_af_routing_requirement_convertToJSON() failed [temp_vals]");
			goto end;
		}
		OpenAPI_list_for_each(af_routing_requirement->temp_vals, node) {
			cJSON *itemLocal =
	...
	if(af_routing_requirement->up_path_chg_sub) {
		...
		if(up_path_chg_sub_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_af_routing_requirement_convertToJSON() failed [up_path_chg_sub]");
			goto end;
		}
	...
	if(af_routing_requirement->is_addr_preser_ind) {
		if(cJSON_AddBoolToObject(item, "addrPreserInd",
		                         af_routing_requirement->addr_preser_ind)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_af_routing_requirement_convertToJSON() failed [addr_preser_ind]");
			goto end;
		}
	}
	...
	if(af_routing_requirement->is_sim_conn_ind) {
		if(cJSON_AddBoolToObject(item, "simConnInd",
		                         af_routing_requirement->sim_conn_ind)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_af_routing_requirement_convertToJSON() failed [sim_conn_ind]");
			goto end;
		}
	}
	...
	if(af_routing_requirement->is_sim_conn_term) {
		if(cJSON_AddNumberToObject(item, "simConnTerm",
		                           af_routing_requirement->sim_conn_term)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_af_routing_requirement_convertToJSON() failed [sim_conn_term]");
			goto end;
		}
	}
	...
	if(af_routing_requirement->eas_ip_replace_infos) {
		...
		if(eas_ip_replace_infosList == NULL) {
			ogs_error(
				"OpenAPI_af_routing_requirement_convertToJSON() failed [eas_ip_replace_infos]");
			goto end;
		}
		OpenAPI_list_for_each(
			af_routing_requirement->eas_ip_replace_infos, node) {
	...
	if(af_routing_requirement->is_eas_redis_ind) {
		if(cJSON_AddBoolToObject(item, "easRedisInd",
		                         af_routing_requirement->eas_redis_ind)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_af_routing_requirement_convertToJSON() failed [eas_redis_ind]");
			goto end;
		}
	}
	...
	if(af_routing_requirement->is_max_allowed_up_lat) {
		...
		{
			ogs_error(
				"OpenAPI_af_routing_requirement_convertToJSON() failed [max_allowed_up_lat]");
			goto end;
		}
	}
...
OpenAPI_af_routing_requirement_t *OpenAPI_af_routing_requirement_parseFromJSON(
	cJSON *af_routing_requirementJSON) {
	OpenAPI_af_routing_requirement_t *af_routing_requirement_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	app_reloc = cJSON_GetObjectItemCaseSensitive(af_routing_requirementJSON,
	                                             "appReloc");
	if(app_reloc) {
		if(!cJSON_IsBool(app_reloc)) {
			ogs_error(
				"OpenAPI_af_routing_requirement_parseFromJSON() failed [app_reloc]");
			goto end;
		}
	}

	route_to_locs =
		cJSON_GetObjectItemCaseSensitive(af_routing_requirementJSON,
		                                 "routeToLocs");
	if(route_to_locs) {
		cJSON *route_to_locs_local = NULL;
		if(!cJSON_IsArray(route_to_locs)) {
			ogs_error(
				"OpenAPI_af_routing_requirement_parseFromJSON() failed [route_to_locs]");
			goto end;
		}
	...
	sp_val = cJSON_GetObjectItemCaseSensitive(af_routing_requirementJSON,
	                                          "spVal");
	if(sp_val) {
		sp_val_local_nonprim =
			OpenAPI_spatial_validity_parseFromJSON(sp_val);
		if(!sp_val_local_nonprim) {
			ogs_error(
				"OpenAPI_spatial_validity_parseFromJSON failed [sp_val]");
			goto end;
		}
	}

	temp_vals = cJSON_GetObjectItemCaseSensitive(af_routing_requirementJSON,
	                                             "tempVals");
	if(temp_vals) {
		cJSON *temp_vals_local = NULL;
		if(!cJSON_IsArray(temp_vals)) {
			ogs_error(
				"OpenAPI_af_routing_requirement_parseFromJSON() failed [temp_vals]");
			goto end;
		}
	...
	if(addr_preser_ind) {
		if(!cJSON_IsBool(addr_preser_ind)) {
			ogs_error(
				"OpenAPI_af_routing_requirement_parseFromJSON() failed [addr_preser_ind]");
			goto end;
		}
	}
	...
	if(sim_conn_ind) {
		if(!cJSON_IsBool(sim_conn_ind)) {
			ogs_error(
				"OpenAPI_af_routing_requirement_parseFromJSON() failed [sim_conn_ind]");
			goto end;
		}
	}
	...
	if(sim_conn_term) {
		if(!cJSON_IsNumber(sim_conn_term)) {
			ogs_error(
				"OpenAPI_af_routing_requirement_parseFromJSON() failed [sim_conn_term]");
			goto end;
		}
	}
	...
	if(eas_ip_replace_infos) {
		...
		if(!cJSON_IsArray(eas_ip_replace_infos)) {
			ogs_error(
				"OpenAPI_af_routing_requirement_parseFromJSON() failed [eas_ip_replace_infos]");
			goto end;
		}
	...
	if(eas_redis_ind) {
		if(!cJSON_IsBool(eas_redis_ind)) {
			ogs_error(
				"OpenAPI_af_routing_requirement_parseFromJSON() failed [eas_redis_ind]");
			goto end;
		}
	}
	...
	if(max_allowed_up_lat) {
		if(!cJSON_IsNumber(max_allowed_up_lat)) {
			ogs_error(
				"OpenAPI_af_routing_requirement_parseFromJSON() failed [max_allowed_up_lat]");
			goto end;
		}
	}

	af_routing_requirement_local_var =
		OpenAPI_af_routing_requirement_create(
...
OpenAPI_af_routing_requirement_t *OpenAPI_af_routing_requirement_copy(
	OpenAPI_af_routing_requirement_t	*dst,
	OpenAPI_af_routing_requirement_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_af_routing_requirement_convertToJSON() failed");
		return NULL;
	}
