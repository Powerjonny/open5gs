...
OpenAPI_top_application_t *OpenAPI_top_application_create(char *app_id,
                                                          ...
                                                          bool	is_ratio,
                                                          int	ratio) {
	OpenAPI_top_application_t *top_application_local_var =
		ogs_malloc(sizeof(OpenAPI_top_application_t));
...
cJSON *OpenAPI_top_application_convertToJSON(
	...
	if(top_application == NULL) {
		ogs_error(
			"OpenAPI_top_application_convertToJSON() failed [TopApplication]");
		return NULL;
	}
	...
	if(top_application->app_id) {
		...
		{
			ogs_error(
				"OpenAPI_top_application_convertToJSON() failed [app_id]");
			goto end;
		}
	}

	if(top_application->ip_traffic_filter) {
		cJSON *ip_traffic_filter_local_JSON =
			OpenAPI_flow_info_convertToJSON(
				top_application->ip_traffic_filter);
		if(ip_traffic_filter_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_top_application_convertToJSON() failed [ip_traffic_filter]");
			goto end;
		}
	...
	if(top_application->is_ratio) {
		...
		{
			ogs_error(
				"OpenAPI_top_application_convertToJSON() failed [ratio]");
			goto end;
		}
	}
...
OpenAPI_top_application_t *OpenAPI_top_application_parseFromJSON(
	...
	if(app_id) {
		...
		{
			ogs_error(
				"OpenAPI_top_application_parseFromJSON() failed [app_id]");
			goto end;
		}
	}

	ip_traffic_filter =
		cJSON_GetObjectItemCaseSensitive(top_applicationJSON,
		                                 "ipTrafficFilter");
	if(ip_traffic_filter) {
		ip_traffic_filter_local_nonprim =
			OpenAPI_flow_info_parseFromJSON(ip_traffic_filter);
		if(!ip_traffic_filter_local_nonprim) {
			ogs_error(
				"OpenAPI_flow_info_parseFromJSON failed [ip_traffic_filter]");
			goto end;
		}
	}
	...
	if(ratio) {
		if(!cJSON_IsNumber(ratio)) {
			ogs_error(
				"OpenAPI_top_application_parseFromJSON() failed [ratio]");
			goto end;
		}
	}
