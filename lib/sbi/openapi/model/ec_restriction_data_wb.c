...
OpenAPI_ec_restriction_data_wb_t *OpenAPI_ec_restriction_data_wb_create(
	...
	OpenAPI_ec_restriction_data_wb_t *ec_restriction_data_wb_local_var =
		ogs_malloc(sizeof(OpenAPI_ec_restriction_data_wb_t));
...
cJSON *OpenAPI_ec_restriction_data_wb_convertToJSON(
	OpenAPI_ec_restriction_data_wb_t *ec_restriction_data_wb) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(ec_restriction_data_wb == NULL) {
		ogs_error(
			"OpenAPI_ec_restriction_data_wb_convertToJSON() failed [EcRestrictionDataWb]");
		return NULL;
	}
	...
	if(ec_restriction_data_wb->is_ec_mode_a_restricted) {
		...
		{
			ogs_error(
				"OpenAPI_ec_restriction_data_wb_convertToJSON() failed [ec_mode_a_restricted]");
			goto end;
		}
	}
	...
	{
		ogs_error(
			"OpenAPI_ec_restriction_data_wb_convertToJSON() failed [ec_mode_b_restricted]");
		goto end;
	}
...
OpenAPI_ec_restriction_data_wb_t *OpenAPI_ec_restriction_data_wb_parseFromJSON(
	...
	ec_mode_a_restricted =
		cJSON_GetObjectItemCaseSensitive(ec_restriction_data_wbJSON,
		                                 "ecModeARestricted");
	if(ec_mode_a_restricted) {
		if(!cJSON_IsBool(ec_mode_a_restricted)) {
			ogs_error(
				"OpenAPI_ec_restriction_data_wb_parseFromJSON() failed [ec_mode_a_restricted]");
			goto end;
		}
	}
	...
	ec_restriction_data_wb_local_var =
		OpenAPI_ec_restriction_data_wb_create(
			ec_mode_a_restricted ? true : false,
			ec_mode_a_restricted ? ec_mode_a_restricted->valueint :
			0,
...
OpenAPI_ec_restriction_data_wb_t *OpenAPI_ec_restriction_data_wb_copy(
	OpenAPI_ec_restriction_data_wb_t	*dst,
	OpenAPI_ec_restriction_data_wb_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ec_restriction_data_wb_convertToJSON() failed");
		return NULL;
	}
