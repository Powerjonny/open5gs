...
OpenAPI_scheduled_communication_time_rm_create(OpenAPI_list_t *days_of_week,
                                               char		*
                                               time_of_day_start,
                                               char *time_of_day_end)
{
	...
	scheduled_communication_time_rm_local_var->time_of_day_start =
		time_of_day_start;
	scheduled_communication_time_rm_local_var->time_of_day_end =
		time_of_day_end;

	return scheduled_communication_time_rm_local_var;
}

void OpenAPI_scheduled_communication_time_rm_free(
	OpenAPI_scheduled_communication_time_rm_t *
	...
	if(scheduled_communication_time_rm->days_of_week) {
		OpenAPI_list_for_each(
			scheduled_communication_time_rm->days_of_week, node) {
			ogs_free(node->data);
		}
		OpenAPI_list_free(
			scheduled_communication_time_rm->days_of_week);
...
cJSON *OpenAPI_scheduled_communication_time_rm_convertToJSON(
	OpenAPI_scheduled_communication_time_rm_t *
	scheduled_communication_time_rm) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(scheduled_communication_time_rm == NULL) {
		ogs_error(
			"OpenAPI_scheduled_communication_time_rm_convertToJSON() failed [ScheduledCommunicationTimeRm]");
		return NULL;
	}
	...
	if(scheduled_communication_time_rm->days_of_week) {
		...
		if(days_of_weekList == NULL) {
			ogs_error(
				"OpenAPI_scheduled_communication_time_rm_convertToJSON() failed [days_of_week]");
			goto end;
		}
		OpenAPI_list_for_each(
			scheduled_communication_time_rm->days_of_week, node) {
	...
	if(scheduled_communication_time_rm->time_of_day_start) {
		...
		{
			ogs_error(
				"OpenAPI_scheduled_communication_time_rm_convertToJSON() failed [time_of_day_start]");
			goto end;
		}
	}
	...
	if(scheduled_communication_time_rm->time_of_day_end) {
		...
		{
			ogs_error(
				"OpenAPI_scheduled_communication_time_rm_convertToJSON() failed [time_of_day_end]");
			goto end;
		}
	}
...
OpenAPI_scheduled_communication_time_rm_parseFromJSON(
	cJSON *scheduled_communication_time_rmJSON) {
	OpenAPI_scheduled_communication_time_rm_t *
	        scheduled_communication_time_rm_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	days_of_week =
		cJSON_GetObjectItemCaseSensitive(
			scheduled_communication_time_rmJSON, "daysOfWeek");
	if(days_of_week) {
		cJSON *days_of_week_local = NULL;
		if(!cJSON_IsArray(days_of_week)) {
			ogs_error(
				"OpenAPI_scheduled_communication_time_rm_parseFromJSON() failed [days_of_week]");
			goto end;
		}
	...
	time_of_day_start =
		cJSON_GetObjectItemCaseSensitive(
			scheduled_communication_time_rmJSON, "timeOfDayStart");
	if(time_of_day_start) {
		if(!cJSON_IsString(time_of_day_start) &&
		   !cJSON_IsNull(time_of_day_start))
		{
			ogs_error(
				"OpenAPI_scheduled_communication_time_rm_parseFromJSON() failed [time_of_day_start]");
			goto end;
		}
	}
	...
	time_of_day_end =
		cJSON_GetObjectItemCaseSensitive(
			scheduled_communication_time_rmJSON, "timeOfDayEnd");
	if(time_of_day_end) {
		if(!cJSON_IsString(time_of_day_end) &&
		   !cJSON_IsNull(time_of_day_end))
		{
			ogs_error(
				"OpenAPI_scheduled_communication_time_rm_parseFromJSON() failed [time_of_day_end]");
			goto end;
		}
	}

	scheduled_communication_time_rm_local_var =
		OpenAPI_scheduled_communication_time_rm_create(
			days_of_week ? days_of_weekList : NULL,
			time_of_day_start &&
			!cJSON_IsNull(time_of_day_start) ?
			ogs_strdup(time_of_day_start->valuestring) : NULL,
			time_of_day_end &&
			!cJSON_IsNull(time_of_day_end) ?
			ogs_strdup(time_of_day_end->valuestring) : NULL
			);
...
OpenAPI_scheduled_communication_time_rm_t *
OpenAPI_scheduled_communication_time_rm_copy(
	OpenAPI_scheduled_communication_time_rm_t	*dst,
	OpenAPI_scheduled_communication_time_rm_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_scheduled_communication_time_rm_convertToJSON() failed");
		return NULL;
	}
