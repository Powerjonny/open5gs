...
OpenAPI_sm_policy_snssai_data_patch_create(OpenAPI_snssai_t *snssai,
                                           ...
                                           sm_policy_dnn_data) {
	OpenAPI_sm_policy_snssai_data_patch_t *
	        sm_policy_snssai_data_patch_local_var =
	...
	sm_policy_snssai_data_patch_local_var->sm_policy_dnn_data =
		sm_policy_dnn_data;
...
void OpenAPI_sm_policy_snssai_data_patch_free(
	...
	if(sm_policy_snssai_data_patch->sm_policy_dnn_data) {
		OpenAPI_list_for_each(
			sm_policy_snssai_data_patch->sm_policy_dnn_data, node) {
			OpenAPI_map_t *localKeyValue =
				(OpenAPI_map_t *) node->data;
			ogs_free(localKeyValue->key);
			OpenAPI_sm_policy_dnn_data_patch_free(
				localKeyValue->value);
			OpenAPI_map_free(localKeyValue);
		}
		OpenAPI_list_free(
			sm_policy_snssai_data_patch->sm_policy_dnn_data);
...
cJSON *OpenAPI_sm_policy_snssai_data_patch_convertToJSON(
	...
	if(sm_policy_snssai_data_patch == NULL) {
		ogs_error(
			"OpenAPI_sm_policy_snssai_data_patch_convertToJSON() failed [SmPolicySnssaiDataPatch]");
		return NULL;
	}
	...
	if(!sm_policy_snssai_data_patch->snssai) {
		ogs_error(
			"OpenAPI_sm_policy_snssai_data_patch_convertToJSON() failed [snssai]");
		return NULL;
	}
	cJSON *snssai_local_JSON =
		OpenAPI_snssai_convertToJSON(
			sm_policy_snssai_data_patch->snssai);
	...
	if(sm_policy_snssai_data_patch->sm_policy_dnn_data) {
		...
		if(sm_policy_dnn_data == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_snssai_data_patch_convertToJSON() failed [sm_policy_dnn_data]");
			goto end;
		}
		...
		if(sm_policy_snssai_data_patch->sm_policy_dnn_data) {
			OpenAPI_list_for_each(
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_sm_policy_dnn_data_patch_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_sm_policy_snssai_data_patch_convertToJSON() failed [inner]");
					goto end;
				}
...
OpenAPI_sm_policy_snssai_data_patch_parseFromJSON(
	...
	OpenAPI_sm_policy_snssai_data_patch_t *
	        sm_policy_snssai_data_patch_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *snssai = NULL;
	...
	snssai =
		cJSON_GetObjectItemCaseSensitive(
			sm_policy_snssai_data_patchJSON, "snssai");
	if(!snssai) {
		ogs_error(
			"OpenAPI_sm_policy_snssai_data_patch_parseFromJSON() failed [snssai]");
		goto end;
	}
	...
	sm_policy_dnn_data =
		cJSON_GetObjectItemCaseSensitive(
			sm_policy_snssai_data_patchJSON, "smPolicyDnnData");
	if(sm_policy_dnn_data) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_snssai_data_patch_parseFromJSON() failed [sm_policy_dnn_data]");
			goto end;
		}
		if(cJSON_IsObject(sm_policy_dnn_data)) {
			...
			cJSON_ArrayForEach(sm_policy_dnn_data_local_map,
				...
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_sm_policy_snssai_data_patch_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	sm_policy_snssai_data_patch_local_var =
		OpenAPI_sm_policy_snssai_data_patch_create(
...
OpenAPI_sm_policy_snssai_data_patch_t *OpenAPI_sm_policy_snssai_data_patch_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_sm_policy_snssai_data_patch_convertToJSON() failed");
		return NULL;
	}
