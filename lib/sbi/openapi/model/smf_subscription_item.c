...
OpenAPI_smf_subscription_item_t *OpenAPI_smf_subscription_item_create(
	...
	OpenAPI_smf_subscription_item_t *smf_subscription_item_local_var =
		ogs_malloc(sizeof(OpenAPI_smf_subscription_item_t));
...
cJSON *OpenAPI_smf_subscription_item_convertToJSON(
	...
	if(smf_subscription_item == NULL) {
		ogs_error(
			"OpenAPI_smf_subscription_item_convertToJSON() failed [SmfSubscriptionItem]");
		return NULL;
	}
	...
	if(!smf_subscription_item->smf_instance_id) {
		ogs_error(
			"OpenAPI_smf_subscription_item_convertToJSON() failed [smf_instance_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "smfInstanceId",
	                           smf_subscription_item->smf_instance_id) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_smf_subscription_item_convertToJSON() failed [smf_instance_id]");
	...
	if(!smf_subscription_item->subscription_id) {
		ogs_error(
			"OpenAPI_smf_subscription_item_convertToJSON() failed [subscription_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "subscriptionId",
	                           smf_subscription_item->subscription_id) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_smf_subscription_item_convertToJSON() failed [subscription_id]");
	...
	if(smf_subscription_item->context_info) {
		...
		if(context_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_smf_subscription_item_convertToJSON() failed [context_info]");
			goto end;
		}
...
OpenAPI_smf_subscription_item_t *OpenAPI_smf_subscription_item_parseFromJSON(
	...
	smf_instance_id =
		cJSON_GetObjectItemCaseSensitive(smf_subscription_itemJSON,
		                                 "smfInstanceId");
	if(!smf_instance_id) {
		ogs_error(
			"OpenAPI_smf_subscription_item_parseFromJSON() failed [smf_instance_id]");
		goto end;
	}
	...
	if(!subscription_id) {
		ogs_error(
			"OpenAPI_smf_subscription_item_parseFromJSON() failed [subscription_id]");
		goto end;
	}
...
OpenAPI_smf_subscription_item_t *OpenAPI_smf_subscription_item_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_smf_subscription_item_convertToJSON() failed");
		return NULL;
	}
