...
OpenAPI_shared_data_id_range_t *OpenAPI_shared_data_id_range_create(
	char *pattern) {
	OpenAPI_shared_data_id_range_t *shared_data_id_range_local_var =
		ogs_malloc(sizeof(OpenAPI_shared_data_id_range_t));
...
cJSON *OpenAPI_shared_data_id_range_convertToJSON(
	...
	if(shared_data_id_range == NULL) {
		ogs_error(
			"OpenAPI_shared_data_id_range_convertToJSON() failed [SharedDataIdRange]");
		return NULL;
	}
	...
	if(shared_data_id_range->pattern) {
		if(cJSON_AddStringToObject(item, "pattern",
		                           shared_data_id_range->pattern) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_shared_data_id_range_convertToJSON() failed [pattern]");
			goto end;
		}
	}
...
OpenAPI_shared_data_id_range_t *OpenAPI_shared_data_id_range_parseFromJSON(
	...
	OpenAPI_lnode_t *node = NULL;
	cJSON *pattern = NULL;
	pattern = cJSON_GetObjectItemCaseSensitive(shared_data_id_rangeJSON,
	                                           "pattern");
	if(pattern) {
		...
		{
			ogs_error(
				"OpenAPI_shared_data_id_range_parseFromJSON() failed [pattern]");
			goto end;
		}
	}
...
OpenAPI_shared_data_id_range_t *OpenAPI_shared_data_id_range_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_shared_data_id_range_convertToJSON() failed");
		return NULL;
	}
