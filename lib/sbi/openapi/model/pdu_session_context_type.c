...
char * OpenAPI_pdu_session_context_type_ToString(
	...
	const char *pdu_session_context_typeArray[] = { "NULL",
		                                        "AF_COORDINATION_INFO" };
	size_t sizeofArray = sizeof(pdu_session_context_typeArray) /
	                     sizeof(pdu_session_context_typeArray[0]);
	if(pdu_session_context_type < sizeofArray) {
		return (char *) pdu_session_context_typeArray[
...
OpenAPI_pdu_session_context_type_e OpenAPI_pdu_session_context_type_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(pdu_session_context_type,
		          pdu_session_context_typeArray[stringToReturn]) == 0) {
