...
OpenAPI_ue_mobility_t *OpenAPI_ue_mobility_create(char *ts,
                                                  ...
                                                  *
                                                  ...
                                                  OpenAPI_list_t	*
                                                  loc_infos) {
	OpenAPI_ue_mobility_t *ue_mobility_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_mobility_t));
...
cJSON *OpenAPI_ue_mobility_convertToJSON(OpenAPI_ue_mobility_t *ue_mobility) {
	...
	if(ue_mobility == NULL) {
		ogs_error(
			"OpenAPI_ue_mobility_convertToJSON() failed [UeMobility]");
		return NULL;
	}
	...
	if(ue_mobility->ts) {
		...
		{
			ogs_error(
				"OpenAPI_ue_mobility_convertToJSON() failed [ts]");
			goto end;
		}
	}
	...
	if(ue_mobility->recurring_time) {
		...
		if(recurring_time_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_mobility_convertToJSON() failed [recurring_time]");
			goto end;
		}
	...
	if(ue_mobility->is_duration) {
		...
		{
			ogs_error(
				"OpenAPI_ue_mobility_convertToJSON() failed [duration]");
			goto end;
		}
	}
	...
	if(ue_mobility->is_duration_variance) {
		...
		{
			ogs_error(
				"OpenAPI_ue_mobility_convertToJSON() failed [duration_variance]");
			goto end;
		}
	}
	...
	if(ue_mobility->loc_infos) {
		...
		if(loc_infosList == NULL) {
			ogs_error(
				"OpenAPI_ue_mobility_convertToJSON() failed [loc_infos]");
			goto end;
		}
		OpenAPI_list_for_each(ue_mobility->loc_infos, node) {
			cJSON *itemLocal =
...
OpenAPI_ue_mobility_t *OpenAPI_ue_mobility_parseFromJSON(
	...
	if(ts) {
		...
		{
			ogs_error(
				"OpenAPI_ue_mobility_parseFromJSON() failed [ts]");
			goto end;
		}
	}
	...
	duration = cJSON_GetObjectItemCaseSensitive(ue_mobilityJSON,
	                                            "duration");
	if(duration) {
		if(!cJSON_IsNumber(duration)) {
			ogs_error(
				"OpenAPI_ue_mobility_parseFromJSON() failed [duration]");
			goto end;
		}
	}
	...
	if(duration_variance) {
		if(!cJSON_IsNumber(duration_variance)) {
			ogs_error(
				"OpenAPI_ue_mobility_parseFromJSON() failed [duration_variance]");
			goto end;
		}
	}

	loc_infos = cJSON_GetObjectItemCaseSensitive(ue_mobilityJSON,
	                                             "locInfos");
	if(loc_infos) {
		cJSON *loc_infos_local = NULL;
		if(!cJSON_IsArray(loc_infos)) {
			ogs_error(
				"OpenAPI_ue_mobility_parseFromJSON() failed [loc_infos]");
			goto end;
		}
		...
		cJSON_ArrayForEach(loc_infos_local, loc_infos) {
			...
			OpenAPI_location_info_t *loc_infosItem =
