...
void OpenAPI_nssai_free(OpenAPI_nssai_t *nssai) {
	...
	if(nssai->additional_snssai_data) {
		OpenAPI_list_for_each(nssai->additional_snssai_data, node) {
			...
			OpenAPI_additional_snssai_data_free(
				localKeyValue->value);
...
cJSON *OpenAPI_nssai_convertToJSON(OpenAPI_nssai_t *nssai) {
	...
	if(nssai->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_nssai_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(!nssai->default_single_nssais) {
		ogs_error(
			"OpenAPI_nssai_convertToJSON() failed [default_single_nssais]");
		return NULL;
	}
	...
	if(nssai->single_nssais) {
		...
		if(single_nssaisList == NULL) {
			ogs_error(
				"OpenAPI_nssai_convertToJSON() failed [single_nssais]");
			goto end;
		}
		OpenAPI_list_for_each(nssai->single_nssais, node) {
			cJSON *itemLocal =
	...
	if(nssai->provisioning_time) {
		...
		{
			ogs_error(
				"OpenAPI_nssai_convertToJSON() failed [provisioning_time]");
			goto end;
		}
	}
	...
	if(nssai->additional_snssai_data) {
		...
		if(additional_snssai_data == NULL) {
			ogs_error(
				"OpenAPI_nssai_convertToJSON() failed [additional_snssai_data]");
			goto end;
		}
		...
		if(nssai->additional_snssai_data) {
			OpenAPI_list_for_each(nssai->additional_snssai_data,
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_additional_snssai_data_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_nssai_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(nssai->is_suppress_nssrg_ind) {
		if(cJSON_AddBoolToObject(item, "suppressNssrgInd",
		                         nssai->suppress_nssrg_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_nssai_convertToJSON() failed [suppress_nssrg_ind]");
			goto end;
		}
	}
...
OpenAPI_nssai_t *OpenAPI_nssai_parseFromJSON(cJSON *nssaiJSON) {
	...
	supported_features = cJSON_GetObjectItemCaseSensitive(nssaiJSON,
	                                                      "supportedFeatures");
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_nssai_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}

	default_single_nssais = cJSON_GetObjectItemCaseSensitive(nssaiJSON,
	                                                         "defaultSingleNssais");
	if(!default_single_nssais) {
		ogs_error(
			"OpenAPI_nssai_parseFromJSON() failed [default_single_nssais]");
		goto end;
	}
	...
	single_nssais = cJSON_GetObjectItemCaseSensitive(nssaiJSON,
	                                                 "singleNssais");
	if(single_nssais) {
		cJSON *single_nssais_local = NULL;
		if(!cJSON_IsArray(single_nssais)) {
			ogs_error(
				"OpenAPI_nssai_parseFromJSON() failed [single_nssais]");
			goto end;
		}
	...
	provisioning_time = cJSON_GetObjectItemCaseSensitive(nssaiJSON,
	                                                     "provisioningTime");
	if(provisioning_time) {
		...
		{
			ogs_error(
				"OpenAPI_nssai_parseFromJSON() failed [provisioning_time]");
			goto end;
		}
	}

	additional_snssai_data = cJSON_GetObjectItemCaseSensitive(nssaiJSON,
	                                                          "additionalSnssaiData");
	if(additional_snssai_data) {
		...
		{
			ogs_error(
				"OpenAPI_nssai_parseFromJSON() failed [additional_snssai_data]");
			goto end;
		}
		if(cJSON_IsObject(additional_snssai_data)) {
			...
			cJSON_ArrayForEach(additional_snssai_data_local_map,
				...
				if(cJSON_IsObject(localMapObject)) {
					localMapKeyPair = OpenAPI_map_create(
						...
						OpenAPI_additional_snssai_data_parseFromJSON
						        (localMapObject));
					...
					ogs_error(
						"OpenAPI_nssai_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	suppress_nssrg_ind = cJSON_GetObjectItemCaseSensitive(nssaiJSON,
	                                                      "suppressNssrgInd");
	if(suppress_nssrg_ind) {
		if(!cJSON_IsBool(suppress_nssrg_ind)) {
			ogs_error(
				"OpenAPI_nssai_parseFromJSON() failed [suppress_nssrg_ind]");
			goto end;
		}
	}
...
OpenAPI_nssai_t *OpenAPI_nssai_copy(OpenAPI_nssai_t	*dst,
                                    OpenAPI_nssai_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
