...
OpenAPI_ee_subscription_t *OpenAPI_ee_subscription_create(
	...
	OpenAPI_ee_subscription_t *ee_subscription_local_var =
		ogs_malloc(sizeof(OpenAPI_ee_subscription_t));
	ogs_assert(ee_subscription_local_var);

	ee_subscription_local_var->callback_reference = callback_reference;
	ee_subscription_local_var->monitoring_configurations =
	...
	ee_subscription_local_var->notify_correlation_id =
	...
	ee_subscription_local_var->data_restoration_callback_uri =
...
void OpenAPI_ee_subscription_free(OpenAPI_ee_subscription_t *ee_subscription) {
	...
	if(ee_subscription->monitoring_configurations) {
		OpenAPI_list_for_each(
			ee_subscription->monitoring_configurations, node) {
			OpenAPI_map_t *localKeyValue =
				(OpenAPI_map_t *) node->data;
			ogs_free(localKeyValue->key);
			OpenAPI_monitoring_configuration_free(
				localKeyValue->value);
	...
	if(ee_subscription->reporting_options) {
		OpenAPI_reporting_options_free(
			ee_subscription->reporting_options);
	...
	if(ee_subscription->exclude_gpsi_list) {
		OpenAPI_list_for_each(ee_subscription->exclude_gpsi_list,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(ee_subscription->include_gpsi_list) {
		OpenAPI_list_for_each(ee_subscription->include_gpsi_list,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_ee_subscription_convertToJSON(
	OpenAPI_ee_subscription_t *ee_subscription) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(ee_subscription == NULL) {
		ogs_error(
			"OpenAPI_ee_subscription_convertToJSON() failed [EeSubscription]");
		return NULL;
	}
	...
	if(!ee_subscription->callback_reference) {
		ogs_error(
			"OpenAPI_ee_subscription_convertToJSON() failed [callback_reference]");
		return NULL;
	}
	...
	if(!ee_subscription->monitoring_configurations) {
		ogs_error(
			"OpenAPI_ee_subscription_convertToJSON() failed [monitoring_configurations]");
		return NULL;
	}
	...
	if(ee_subscription->monitoring_configurations) {
		OpenAPI_list_for_each(
			...
			cJSON *itemLocal = localKeyValue->value ?
			                   OpenAPI_monitoring_configuration_convertToJSON
			                           (localKeyValue->value) :
			                   cJSON_CreateNull();
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_ee_subscription_convertToJSON() failed [inner]");
				goto end;
			}
	...
	if(ee_subscription->reporting_options) {
		cJSON *reporting_options_local_JSON =
			OpenAPI_reporting_options_convertToJSON(
				ee_subscription->reporting_options);
		if(reporting_options_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ee_subscription_convertToJSON() failed [reporting_options]");
			goto end;
		}
	...
	if(ee_subscription->supported_features) {
		if(cJSON_AddStringToObject(item, "supportedFeatures",
		                           ee_subscription->supported_features)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_ee_subscription_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(ee_subscription->subscription_id) {
		if(cJSON_AddStringToObject(item, "subscriptionId",
		                           ee_subscription->subscription_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_ee_subscription_convertToJSON() failed [subscription_id]");
			goto end;
		}
	}
	...
	if(ee_subscription->context_info) {
		cJSON *context_info_local_JSON =
			OpenAPI_context_info_convertToJSON(
				ee_subscription->context_info);
		if(context_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ee_subscription_convertToJSON() failed [context_info]");
			goto end;
		}
	...
	if(ee_subscription->is_epc_applied_ind) {
		if(cJSON_AddBoolToObject(item, "epcAppliedInd",
		                         ee_subscription->epc_applied_ind) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_ee_subscription_convertToJSON() failed [epc_applied_ind]");
			goto end;
		}
	}
	...
	if(ee_subscription->scef_diam_host) {
		...
		{
			ogs_error(
				"OpenAPI_ee_subscription_convertToJSON() failed [scef_diam_host]");
			goto end;
		}
	}
	...
	if(ee_subscription->scef_diam_realm) {
		if(cJSON_AddStringToObject(item, "scefDiamRealm",
		                           ee_subscription->scef_diam_realm) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_ee_subscription_convertToJSON() failed [scef_diam_realm]");
			goto end;
		}
	}
	...
	if(ee_subscription->notify_correlation_id) {
		...
		{
			ogs_error(
				"OpenAPI_ee_subscription_convertToJSON() failed [notify_correlation_id]");
			goto end;
		}
	}
	...
	if(ee_subscription->second_callback_ref) {
		if(cJSON_AddStringToObject(item, "secondCallbackRef",
		                           ee_subscription->second_callback_ref)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_ee_subscription_convertToJSON() failed [second_callback_ref]");
			goto end;
		}
	}
	...
	if(ee_subscription->gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_ee_subscription_convertToJSON() failed [gpsi]");
			goto end;
		}
	}
	...
	if(ee_subscription->exclude_gpsi_list) {
		...
		if(exclude_gpsi_listList == NULL) {
			ogs_error(
				"OpenAPI_ee_subscription_convertToJSON() failed [exclude_gpsi_list]");
			goto end;
		}
		OpenAPI_list_for_each(ee_subscription->exclude_gpsi_list,
		                      node) {
	...
	if(ee_subscription->include_gpsi_list) {
		...
		if(include_gpsi_listList == NULL) {
			ogs_error(
				"OpenAPI_ee_subscription_convertToJSON() failed [include_gpsi_list]");
			goto end;
		}
		OpenAPI_list_for_each(ee_subscription->include_gpsi_list,
		                      node) {
	...
	if(ee_subscription->data_restoration_callback_uri) {
		...
		{
			ogs_error(
				"OpenAPI_ee_subscription_convertToJSON() failed [data_restoration_callback_uri]");
			goto end;
		}
	}
...
OpenAPI_ee_subscription_t *OpenAPI_ee_subscription_parseFromJSON(
	cJSON *ee_subscriptionJSON) {
	...
	callback_reference =
		cJSON_GetObjectItemCaseSensitive(ee_subscriptionJSON,
		                                 "callbackReference");
	if(!callback_reference) {
		ogs_error(
			"OpenAPI_ee_subscription_parseFromJSON() failed [callback_reference]");
		goto end;
	}
	...
	if(!monitoring_configurations) {
		ogs_error(
			"OpenAPI_ee_subscription_parseFromJSON() failed [monitoring_configurations]");
		goto end;
	}
	...
	if(cJSON_IsObject(monitoring_configurations)) {
		...
		cJSON_ArrayForEach(monitoring_configurations_local_map,
			...
			if(cJSON_IsObject(localMapObject)) {
				...
				ogs_error(
					"OpenAPI_ee_subscription_parseFromJSON() failed [inner]");
				goto end;
			}
	...
	if(reporting_options) {
		...
		if(!reporting_options_local_nonprim) {
			ogs_error(
				"OpenAPI_reporting_options_parseFromJSON failed [reporting_options]");
			goto end;
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_ee_subscription_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}

	subscription_id = cJSON_GetObjectItemCaseSensitive(ee_subscriptionJSON,
	                                                   "subscriptionId");
	if(subscription_id) {
		...
		{
			ogs_error(
				"OpenAPI_ee_subscription_parseFromJSON() failed [subscription_id]");
			goto end;
		}
	}

	context_info = cJSON_GetObjectItemCaseSensitive(ee_subscriptionJSON,
	                                                "contextInfo");
	if(context_info) {
	...
	epc_applied_ind = cJSON_GetObjectItemCaseSensitive(ee_subscriptionJSON,
	                                                   "epcAppliedInd");
	if(epc_applied_ind) {
		if(!cJSON_IsBool(epc_applied_ind)) {
			ogs_error(
				"OpenAPI_ee_subscription_parseFromJSON() failed [epc_applied_ind]");
			goto end;
		}
	}

	scef_diam_host = cJSON_GetObjectItemCaseSensitive(ee_subscriptionJSON,
	                                                  "scefDiamHost");
	if(scef_diam_host) {
		...
		{
			ogs_error(
				"OpenAPI_ee_subscription_parseFromJSON() failed [scef_diam_host]");
			goto end;
		}
	}

	scef_diam_realm = cJSON_GetObjectItemCaseSensitive(ee_subscriptionJSON,
	                                                   "scefDiamRealm");
	if(scef_diam_realm) {
		...
		{
			ogs_error(
				"OpenAPI_ee_subscription_parseFromJSON() failed [scef_diam_realm]");
			goto end;
		}
	}
	...
	if(notify_correlation_id) {
		...
		{
			ogs_error(
				"OpenAPI_ee_subscription_parseFromJSON() failed [notify_correlation_id]");
			goto end;
		}
	}
	...
	if(second_callback_ref) {
		...
		{
			ogs_error(
				"OpenAPI_ee_subscription_parseFromJSON() failed [second_callback_ref]");
			goto end;
		}
	}
	...
	if(gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_ee_subscription_parseFromJSON() failed [gpsi]");
			goto end;
		}
	}
	...
	if(exclude_gpsi_list) {
		...
		if(!cJSON_IsArray(exclude_gpsi_list)) {
			ogs_error(
				"OpenAPI_ee_subscription_parseFromJSON() failed [exclude_gpsi_list]");
			goto end;
		}
	...
	if(include_gpsi_list) {
		...
		if(!cJSON_IsArray(include_gpsi_list)) {
			ogs_error(
				"OpenAPI_ee_subscription_parseFromJSON() failed [include_gpsi_list]");
			goto end;
		}
	...
	if(data_restoration_callback_uri) {
		...
		{
			ogs_error(
				"OpenAPI_ee_subscription_parseFromJSON() failed [data_restoration_callback_uri]");
			goto end;
		}
	}
...
OpenAPI_ee_subscription_t *OpenAPI_ee_subscription_copy(
	OpenAPI_ee_subscription_t *dst, OpenAPI_ee_subscription_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
