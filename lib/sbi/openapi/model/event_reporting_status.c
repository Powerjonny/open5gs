...
OpenAPI_event_reporting_status_t *OpenAPI_event_reporting_status_create(
	...
	OpenAPI_event_reporting_status_t *event_reporting_status_local_var =
		ogs_malloc(sizeof(OpenAPI_event_reporting_status_t));
...
void OpenAPI_event_reporting_status_free(
...
cJSON *OpenAPI_event_reporting_status_convertToJSON(
	OpenAPI_event_reporting_status_t *event_reporting_status) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(event_reporting_status == NULL) {
		ogs_error(
			"OpenAPI_event_reporting_status_convertToJSON() failed [EventReportingStatus]");
		return NULL;
	}
	...
	if(event_reporting_status->is_event_report_counter) {
		...
		{
			ogs_error(
				"OpenAPI_event_reporting_status_convertToJSON() failed [event_report_counter]");
			goto end;
		}
	}
	...
	if(event_reporting_status->is_event_report_duration) {
		...
		{
			ogs_error(
				"OpenAPI_event_reporting_status_convertToJSON() failed [event_report_duration]");
			goto end;
		}
	}
...
OpenAPI_event_reporting_status_t *OpenAPI_event_reporting_status_parseFromJSON(
	...
	event_report_counter =
		cJSON_GetObjectItemCaseSensitive(event_reporting_statusJSON,
		                                 "eventReportCounter");
	if(event_report_counter) {
		if(!cJSON_IsNumber(event_report_counter)) {
			ogs_error(
				"OpenAPI_event_reporting_status_parseFromJSON() failed [event_report_counter]");
			goto end;
		}
	}
	...
	if(event_report_duration) {
		if(!cJSON_IsNumber(event_report_duration)) {
			ogs_error(
				"OpenAPI_event_reporting_status_parseFromJSON() failed [event_report_duration]");
			goto end;
		}
	}

	event_reporting_status_local_var =
		OpenAPI_event_reporting_status_create(
			event_report_counter ? true : false,
			event_report_counter ?
			event_report_counter->valuedouble : 0,
			event_report_duration ? true : false,
			event_report_duration ?
			event_report_duration->valuedouble : 0
			);
...
OpenAPI_event_reporting_status_t *OpenAPI_event_reporting_status_copy(
	OpenAPI_event_reporting_status_t	*dst,
	OpenAPI_event_reporting_status_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_event_reporting_status_convertToJSON() failed");
		return NULL;
	}
