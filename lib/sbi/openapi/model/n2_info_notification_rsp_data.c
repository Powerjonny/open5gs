...
OpenAPI_n2_info_notification_rsp_data_create(
...
void OpenAPI_n2_info_notification_rsp_data_free(
	OpenAPI_n2_info_notification_rsp_data_t *n2_info_notification_rsp_data)
{
...
cJSON *OpenAPI_n2_info_notification_rsp_data_convertToJSON(
	OpenAPI_n2_info_notification_rsp_data_t *n2_info_notification_rsp_data)
{
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(n2_info_notification_rsp_data == NULL) {
		ogs_error(
			"OpenAPI_n2_info_notification_rsp_data_convertToJSON() failed [N2InfoNotificationRspData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(n2_info_notification_rsp_data->sec_rat_data_usage_list) {
		cJSON *sec_rat_data_usage_listList =
			cJSON_AddArrayToObject(item, "secRatDataUsageList");
		if(sec_rat_data_usage_listList == NULL) {
			ogs_error(
				"OpenAPI_n2_info_notification_rsp_data_convertToJSON() failed [sec_rat_data_usage_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			n2_info_notification_rsp_data->sec_rat_data_usage_list,
...
OpenAPI_n2_info_notification_rsp_data_parseFromJSON(
	...
	if(sec_rat_data_usage_list) {
		...
		cJSON_ArrayForEach(sec_rat_data_usage_list_local,
			...
			OpenAPI_n2_sm_information_t *sec_rat_data_usage_listItem
			...
			OpenAPI_list_add(sec_rat_data_usage_listList,
	...
	n2_info_notification_rsp_data_local_var =
		OpenAPI_n2_info_notification_rsp_data_create(
			sec_rat_data_usage_list ? sec_rat_data_usage_listList :
			NULL
			);
...
OpenAPI_n2_info_notification_rsp_data_t *
OpenAPI_n2_info_notification_rsp_data_copy(
	OpenAPI_n2_info_notification_rsp_data_t *dst,
	OpenAPI_n2_info_notification_rsp_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_n2_info_notification_rsp_data_convertToJSON() failed");
		return NULL;
	}
