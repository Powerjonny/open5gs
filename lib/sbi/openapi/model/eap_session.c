...
OpenAPI_eap_session_t *OpenAPI_eap_session_create(bool is_eap_payload_null,
                                                  char *eap_payload,
                                                  char *k_seaf,
                                                  OpenAPI_list_t *_links,
                                                  OpenAPI_auth_result_e
                                                  auth_result, char *supi,
                                                  char *supported_features,
                                                  OpenAPI_list_t *pvs_info,
                                                  char *msk) {
	OpenAPI_eap_session_t *eap_session_local_var =
		ogs_malloc(sizeof(OpenAPI_eap_session_t));
...
cJSON *OpenAPI_eap_session_convertToJSON(OpenAPI_eap_session_t *eap_session) {
	...
	if(eap_session == NULL) {
		ogs_error(
			"OpenAPI_eap_session_convertToJSON() failed [EapSession]");
		return NULL;
	}
	...
	if(!eap_session->eap_payload) {
		ogs_error(
			"OpenAPI_eap_session_convertToJSON() failed [eap_payload]");
		return NULL;
	}
	...
	if(eap_session->k_seaf) {
		if(cJSON_AddStringToObject(item, "kSeaf",
		                           eap_session->k_seaf) == NULL)
		{
			ogs_error(
				"OpenAPI_eap_session_convertToJSON() failed [k_seaf]");
			goto end;
		}
	}
	...
	if(eap_session->_links) {
		...
		if(_links == NULL) {
			ogs_error(
				"OpenAPI_eap_session_convertToJSON() failed [_links]");
			goto end;
		}
		...
		if(eap_session->_links) {
			OpenAPI_list_for_each(eap_session->_links, node) {
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_eap_session_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(eap_session->auth_result != OpenAPI_auth_result_NULL) {
		if(cJSON_AddStringToObject(item, "authResult",
						   ...
						   eap_session->auth_result)) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_eap_session_convertToJSON() failed [auth_result]");
			goto end;
		}
	}
	...
	if(eap_session->supi) {
		...
		{
			ogs_error(
				"OpenAPI_eap_session_convertToJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(eap_session->supported_features) {
		if(cJSON_AddStringToObject(item, "supportedFeatures",
		                           eap_session->supported_features) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_eap_session_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(eap_session->pvs_info) {
		...
		if(pvs_infoList == NULL) {
			ogs_error(
				"OpenAPI_eap_session_convertToJSON() failed [pvs_info]");
			goto end;
		}
		OpenAPI_list_for_each(eap_session->pvs_info, node) {
			cJSON *itemLocal =
	...
	if(eap_session->msk) {
		if(cJSON_AddStringToObject(item, "msk",
		                           eap_session->msk) == NULL)
		{
			ogs_error(
				"OpenAPI_eap_session_convertToJSON() failed [msk]");
			goto end;
		}
	}
...
OpenAPI_eap_session_t *OpenAPI_eap_session_parseFromJSON(
	cJSON *eap_sessionJSON) {
	...
	cJSON *msk = NULL;
	eap_payload = cJSON_GetObjectItemCaseSensitive(eap_sessionJSON,
	                                               "eapPayload");
	if(!eap_payload) {
		ogs_error(
			"OpenAPI_eap_session_parseFromJSON() failed [eap_payload]");
		goto end;
	}
	...
	if(k_seaf) {
		...
		{
			ogs_error(
				"OpenAPI_eap_session_parseFromJSON() failed [k_seaf]");
			goto end;
		}
	}
	...
	if(_links) {
		...
		{
			ogs_error(
				"OpenAPI_eap_session_parseFromJSON() failed [_links]");
			goto end;
		}
		if(cJSON_IsObject(_links)) {
			...
			cJSON_ArrayForEach(_links_local_map, _links) {
				...
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_eap_session_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	auth_result = cJSON_GetObjectItemCaseSensitive(eap_sessionJSON,
	                                               "authResult");
	if(auth_result) {
		if(!cJSON_IsString(auth_result)) {
			ogs_error(
				"OpenAPI_eap_session_parseFromJSON() failed [auth_result]");
			goto end;
		}
		auth_resultVariable =
	...
	if(supi) {
		...
		{
			ogs_error(
				"OpenAPI_eap_session_parseFromJSON() failed [supi]");
			goto end;
		}
	}

	supported_features = cJSON_GetObjectItemCaseSensitive(eap_sessionJSON,
	                                                      "supportedFeatures");
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_eap_session_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(pvs_info) {
		...
		if(!cJSON_IsArray(pvs_info)) {
			ogs_error(
				"OpenAPI_eap_session_parseFromJSON() failed [pvs_info]");
			goto end;
		}
	...
	if(msk) {
		if(!cJSON_IsString(msk) &&
		   !cJSON_IsNull(msk))
		{
			ogs_error(
				"OpenAPI_eap_session_parseFromJSON() failed [msk]");
			goto end;
		}
	}
	...
	eap_session_local_var = OpenAPI_eap_session_create(
		...
		k_seaf &&
		!cJSON_IsNull(k_seaf) ? ogs_strdup(k_seaf->valuestring) : NULL,
		...
		msk &&
		!cJSON_IsNull(msk) ? ogs_strdup(msk->valuestring) : NULL
		);
...
OpenAPI_eap_session_t *OpenAPI_eap_session_copy(OpenAPI_eap_session_t	*dst,
                                                OpenAPI_eap_session_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
