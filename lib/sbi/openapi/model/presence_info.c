...
OpenAPI_presence_info_t *OpenAPI_presence_info_create(char *pra_id,
                                                      ...
                                                      OpenAPI_list_t *
                                                      globale_nb_id_list) {
	OpenAPI_presence_info_t *presence_info_local_var =
...
void OpenAPI_presence_info_free(OpenAPI_presence_info_t *presence_info) {
	...
	if(presence_info->global_ran_node_id_list) {
		OpenAPI_list_for_each(presence_info->global_ran_node_id_list,
		                      node) {
...
cJSON *OpenAPI_presence_info_convertToJSON(
	...
	if(presence_info == NULL) {
		ogs_error(
			"OpenAPI_presence_info_convertToJSON() failed [PresenceInfo]");
		return NULL;
	}
	...
	if(presence_info->pra_id) {
		...
		{
			ogs_error(
				"OpenAPI_presence_info_convertToJSON() failed [pra_id]");
			goto end;
		}
	}
	...
	if(presence_info->additional_pra_id) {
		...
		{
			ogs_error(
				"OpenAPI_presence_info_convertToJSON() failed [additional_pra_id]");
			goto end;
		}
	}
	...
	if(presence_info->presence_state != OpenAPI_presence_state_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_presence_info_convertToJSON() failed [presence_state]");
			goto end;
		}
	}
	...
	if(presence_info->tracking_area_list) {
		...
		if(tracking_area_listList == NULL) {
			ogs_error(
				"OpenAPI_presence_info_convertToJSON() failed [tracking_area_list]");
			goto end;
		}
		OpenAPI_list_for_each(presence_info->tracking_area_list, node) {
			cJSON *itemLocal =
	...
	if(presence_info->ecgi_list) {
		...
		if(ecgi_listList == NULL) {
			ogs_error(
				"OpenAPI_presence_info_convertToJSON() failed [ecgi_list]");
			goto end;
		}
		OpenAPI_list_for_each(presence_info->ecgi_list, node) {
			cJSON *itemLocal =
	...
	if(presence_info->ncgi_list) {
		...
		if(ncgi_listList == NULL) {
			ogs_error(
				"OpenAPI_presence_info_convertToJSON() failed [ncgi_list]");
			goto end;
		}
		OpenAPI_list_for_each(presence_info->ncgi_list, node) {
			cJSON *itemLocal =
	...
	if(presence_info->global_ran_node_id_list) {
		...
		if(global_ran_node_id_listList == NULL) {
			ogs_error(
				"OpenAPI_presence_info_convertToJSON() failed [global_ran_node_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(presence_info->global_ran_node_id_list,
		                      node) {
	...
	if(presence_info->globale_nb_id_list) {
		...
		if(globale_nb_id_listList == NULL) {
			ogs_error(
				"OpenAPI_presence_info_convertToJSON() failed [globale_nb_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(presence_info->globale_nb_id_list, node) {
			cJSON *itemLocal =
...
OpenAPI_presence_info_t *OpenAPI_presence_info_parseFromJSON(
	...
	if(pra_id) {
		...
		{
			ogs_error(
				"OpenAPI_presence_info_parseFromJSON() failed [pra_id]");
			goto end;
		}
	}
	...
	if(additional_pra_id) {
		...
		{
			ogs_error(
				"OpenAPI_presence_info_parseFromJSON() failed [additional_pra_id]");
			goto end;
		}
	}
	...
	if(presence_state) {
		if(!cJSON_IsString(presence_state)) {
			ogs_error(
				"OpenAPI_presence_info_parseFromJSON() failed [presence_state]");
			goto end;
		}
	...
	if(tracking_area_list) {
		...
		if(!cJSON_IsArray(tracking_area_list)) {
			ogs_error(
				"OpenAPI_presence_info_parseFromJSON() failed [tracking_area_list]");
			goto end;
		}
	...
	if(ecgi_list) {
		...
		if(!cJSON_IsArray(ecgi_list)) {
			ogs_error(
				"OpenAPI_presence_info_parseFromJSON() failed [ecgi_list]");
			goto end;
		}
	...
	if(ncgi_list) {
		...
		if(!cJSON_IsArray(ncgi_list)) {
			ogs_error(
				"OpenAPI_presence_info_parseFromJSON() failed [ncgi_list]");
			goto end;
		}
	...
	global_ran_node_id_list =
		cJSON_GetObjectItemCaseSensitive(presence_infoJSON,
		                                 "globalRanNodeIdList");
	if(global_ran_node_id_list) {
		cJSON *global_ran_node_id_list_local = NULL;
		if(!cJSON_IsArray(global_ran_node_id_list)) {
			ogs_error(
				"OpenAPI_presence_info_parseFromJSON() failed [global_ran_node_id_list]");
			goto end;
		}
	...
	if(globale_nb_id_list) {
		...
		if(!cJSON_IsArray(globale_nb_id_list)) {
			ogs_error(
				"OpenAPI_presence_info_parseFromJSON() failed [globale_nb_id_list]");
			goto end;
		}
