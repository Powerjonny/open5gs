...
OpenAPI_subscribed_default_qos_t *OpenAPI_subscribed_default_qos_create(
	int _5qi, OpenAPI_arp_t *arp, bool is_priority_level,
	int priority_level) {
	OpenAPI_subscribed_default_qos_t *subscribed_default_qos_local_var =
		ogs_malloc(sizeof(OpenAPI_subscribed_default_qos_t));
...
cJSON *OpenAPI_subscribed_default_qos_convertToJSON(
	OpenAPI_subscribed_default_qos_t *subscribed_default_qos) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(subscribed_default_qos == NULL) {
		ogs_error(
			"OpenAPI_subscribed_default_qos_convertToJSON() failed [SubscribedDefaultQos]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(cJSON_AddNumberToObject(item, "5qi",
	                           subscribed_default_qos->_5qi) == NULL)
	{
		ogs_error(
			"OpenAPI_subscribed_default_qos_convertToJSON() failed [_5qi]");
		goto end;
	}

	if(!subscribed_default_qos->arp) {
		ogs_error(
			"OpenAPI_subscribed_default_qos_convertToJSON() failed [arp]");
		return NULL;
	}
	cJSON *arp_local_JSON =
		OpenAPI_arp_convertToJSON(subscribed_default_qos->arp);
	...
	if(subscribed_default_qos->is_priority_level) {
		if(cJSON_AddNumberToObject(item, "priorityLevel",
		                           subscribed_default_qos->
		                           priority_level) == NULL)
		{
			ogs_error(
				"OpenAPI_subscribed_default_qos_convertToJSON() failed [priority_level]");
			goto end;
		}
	}
...
OpenAPI_subscribed_default_qos_t *OpenAPI_subscribed_default_qos_parseFromJSON(
	cJSON *subscribed_default_qosJSON) {
	OpenAPI_subscribed_default_qos_t *subscribed_default_qos_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *_5qi = NULL;
	...
	_5qi = cJSON_GetObjectItemCaseSensitive(subscribed_default_qosJSON,
	                                        "5qi");
	if(!_5qi) {
		ogs_error(
			"OpenAPI_subscribed_default_qos_parseFromJSON() failed [_5qi]");
		goto end;
	}
	...
	arp = cJSON_GetObjectItemCaseSensitive(subscribed_default_qosJSON,
	                                       "arp");
	if(!arp) {
		ogs_error(
			"OpenAPI_subscribed_default_qos_parseFromJSON() failed [arp]");
		goto end;
	}
	...
	priority_level =
		cJSON_GetObjectItemCaseSensitive(subscribed_default_qosJSON,
		                                 "priorityLevel");
	if(priority_level) {
		if(!cJSON_IsNumber(priority_level)) {
			ogs_error(
				"OpenAPI_subscribed_default_qos_parseFromJSON() failed [priority_level]");
			goto end;
		}
	}

	subscribed_default_qos_local_var =
		OpenAPI_subscribed_default_qos_create(
...
OpenAPI_subscribed_default_qos_t *OpenAPI_subscribed_default_qos_copy(
	OpenAPI_subscribed_default_qos_t	*dst,
	OpenAPI_subscribed_default_qos_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_subscribed_default_qos_convertToJSON() failed");
		return NULL;
	}
