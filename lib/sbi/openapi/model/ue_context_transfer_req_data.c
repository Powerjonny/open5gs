...
OpenAPI_ue_context_transfer_req_data_create(OpenAPI_transfer_reason_e reason,
	...
	OpenAPI_ue_context_transfer_req_data_t *
		...
		ogs_malloc(sizeof(OpenAPI_ue_context_transfer_req_data_t));
...
void OpenAPI_ue_context_transfer_req_data_free(
	OpenAPI_ue_context_transfer_req_data_t *ue_context_transfer_req_data) {
...
cJSON *OpenAPI_ue_context_transfer_req_data_convertToJSON(
	OpenAPI_ue_context_transfer_req_data_t *ue_context_transfer_req_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(ue_context_transfer_req_data == NULL) {
		ogs_error(
			"OpenAPI_ue_context_transfer_req_data_convertToJSON() failed [UeContextTransferReqData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(ue_context_transfer_req_data->reason ==
	   OpenAPI_transfer_reason_NULL)
	{
		ogs_error(
			"OpenAPI_ue_context_transfer_req_data_convertToJSON() failed [reason]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "reason",
	                           OpenAPI_transfer_reason_ToString(
	...
	{
		ogs_error(
			"OpenAPI_ue_context_transfer_req_data_convertToJSON() failed [access_type]");
		return NULL;
	}
	...
	if(ue_context_transfer_req_data->plmn_id) {
		...
		if(plmn_id_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_transfer_req_data_convertToJSON() failed [plmn_id]");
			goto end;
		}
	...
	if(ue_context_transfer_req_data->reg_request) {
		...
		if(reg_request_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_transfer_req_data_convertToJSON() failed [reg_request]");
			goto end;
		}
	...
	if(ue_context_transfer_req_data->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_transfer_req_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
...
OpenAPI_ue_context_transfer_req_data_t *
OpenAPI_ue_context_transfer_req_data_parseFromJSON(
	cJSON *ue_context_transfer_req_dataJSON) {
	OpenAPI_ue_context_transfer_req_data_t *
	        ue_context_transfer_req_data_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *reason = NULL;
	...
	if(!reason) {
		ogs_error(
			"OpenAPI_ue_context_transfer_req_data_parseFromJSON() failed [reason]");
		goto end;
	}
	...
	reasonVariable =
		OpenAPI_transfer_reason_FromString(reason->valuestring);
	...
	if(!access_type) {
		ogs_error(
			"OpenAPI_ue_context_transfer_req_data_parseFromJSON() failed [access_type]");
		goto end;
	}
	...
	if(reg_request) {
		reg_request_local_nonprim =
			OpenAPI_n1_message_container_parseFromJSON(reg_request);
		if(!reg_request_local_nonprim) {
			ogs_error(
				"OpenAPI_n1_message_container_parseFromJSON failed [reg_request]");
			goto end;
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_transfer_req_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}

	ue_context_transfer_req_data_local_var =
		OpenAPI_ue_context_transfer_req_data_create(
			reasonVariable,
			access_typeVariable,
...
OpenAPI_ue_context_transfer_req_data_t *
OpenAPI_ue_context_transfer_req_data_copy(
	OpenAPI_ue_context_transfer_req_data_t	*dst,
	OpenAPI_ue_context_transfer_req_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ue_context_transfer_req_data_convertToJSON() failed");
		return NULL;
	}
