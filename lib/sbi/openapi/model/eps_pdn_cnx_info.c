...
{
	OpenAPI_eps_pdn_cnx_info_t *eps_pdn_cnx_info_local_var =
		ogs_malloc(sizeof(OpenAPI_eps_pdn_cnx_info_t));
...
void OpenAPI_eps_pdn_cnx_info_free(
...
cJSON *OpenAPI_eps_pdn_cnx_info_convertToJSON(
	OpenAPI_eps_pdn_cnx_info_t *eps_pdn_cnx_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(eps_pdn_cnx_info == NULL) {
		ogs_error(
			"OpenAPI_eps_pdn_cnx_info_convertToJSON() failed [EpsPdnCnxInfo]");
		return NULL;
	}
	...
	if(!eps_pdn_cnx_info->pgw_s8c_fteid) {
		ogs_error(
			"OpenAPI_eps_pdn_cnx_info_convertToJSON() failed [pgw_s8c_fteid]");
		return NULL;
	}
	...
	if(eps_pdn_cnx_info->pgw_node_name) {
		if(cJSON_AddStringToObject(item, "pgwNodeName",
		                           eps_pdn_cnx_info->pgw_node_name) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_eps_pdn_cnx_info_convertToJSON() failed [pgw_node_name]");
			goto end;
		}
	}

	if(eps_pdn_cnx_info->is_linked_bearer_id) {
		if(cJSON_AddNumberToObject(item, "linkedBearerId",
		                           eps_pdn_cnx_info->linked_bearer_id)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_eps_pdn_cnx_info_convertToJSON() failed [linked_bearer_id]");
			goto end;
		}
	}
...
OpenAPI_eps_pdn_cnx_info_t *OpenAPI_eps_pdn_cnx_info_parseFromJSON(
	cJSON *eps_pdn_cnx_infoJSON) {
	...
	pgw_s8c_fteid = cJSON_GetObjectItemCaseSensitive(eps_pdn_cnx_infoJSON,
	                                                 "pgwS8cFteid");
	if(!pgw_s8c_fteid) {
		ogs_error(
			"OpenAPI_eps_pdn_cnx_info_parseFromJSON() failed [pgw_s8c_fteid]");
		goto end;
	}
	...
	pgw_node_name = cJSON_GetObjectItemCaseSensitive(eps_pdn_cnx_infoJSON,
	                                                 "pgwNodeName");
	if(pgw_node_name) {
		if(!cJSON_IsString(pgw_node_name) &&
		   !cJSON_IsNull(pgw_node_name))
		{
			ogs_error(
				"OpenAPI_eps_pdn_cnx_info_parseFromJSON() failed [pgw_node_name]");
			goto end;
		}
	}

	linked_bearer_id =
		cJSON_GetObjectItemCaseSensitive(eps_pdn_cnx_infoJSON,
		                                 "linkedBearerId");
	if(linked_bearer_id) {
		if(!cJSON_IsNumber(linked_bearer_id)) {
			ogs_error(
				"OpenAPI_eps_pdn_cnx_info_parseFromJSON() failed [linked_bearer_id]");
			goto end;
		}
	}
	...
	eps_pdn_cnx_info_local_var = OpenAPI_eps_pdn_cnx_info_create(
		...
		pgw_node_name &&
		!cJSON_IsNull(pgw_node_name) ?
		ogs_strdup(pgw_node_name->valuestring) : NULL,
...
OpenAPI_eps_pdn_cnx_info_t *OpenAPI_eps_pdn_cnx_info_copy(
	OpenAPI_eps_pdn_cnx_info_t *dst, OpenAPI_eps_pdn_cnx_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
