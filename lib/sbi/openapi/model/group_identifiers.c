...
OpenAPI_group_identifiers_t *OpenAPI_group_identifiers_create(
	...
	OpenAPI_group_identifiers_t *group_identifiers_local_var =
		ogs_malloc(sizeof(OpenAPI_group_identifiers_t));
...
cJSON *OpenAPI_group_identifiers_convertToJSON(
	...
	if(group_identifiers == NULL) {
		ogs_error(
			"OpenAPI_group_identifiers_convertToJSON() failed [GroupIdentifiers]");
		return NULL;
	}
	...
	if(group_identifiers->ext_group_id) {
		if(cJSON_AddStringToObject(item, "extGroupId",
		                           group_identifiers->ext_group_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_group_identifiers_convertToJSON() failed [ext_group_id]");
			goto end;
		}
	}
	...
	if(group_identifiers->int_group_id) {
		if(cJSON_AddStringToObject(item, "intGroupId",
		                           group_identifiers->int_group_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_group_identifiers_convertToJSON() failed [int_group_id]");
			goto end;
		}
	}

	if(group_identifiers->ue_id_list) {
		cJSON *ue_id_listList = cJSON_AddArrayToObject(item,
		                                               "ueIdList");
		if(ue_id_listList == NULL) {
			ogs_error(
				"OpenAPI_group_identifiers_convertToJSON() failed [ue_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(group_identifiers->ue_id_list, node) {
			cJSON *itemLocal =
				OpenAPI_ue_id_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_group_identifiers_convertToJSON() failed [ue_id_list]");
				goto end;
...
OpenAPI_group_identifiers_t *OpenAPI_group_identifiers_parseFromJSON(
	...
	ext_group_id = cJSON_GetObjectItemCaseSensitive(group_identifiersJSON,
	                                                "extGroupId");
	if(ext_group_id) {
		...
		{
			ogs_error(
				"OpenAPI_group_identifiers_parseFromJSON() failed [ext_group_id]");
			goto end;
		}
	}

	int_group_id = cJSON_GetObjectItemCaseSensitive(group_identifiersJSON,
	                                                "intGroupId");
	if(int_group_id) {
		...
		{
			ogs_error(
				"OpenAPI_group_identifiers_parseFromJSON() failed [int_group_id]");
			goto end;
		}
	}

	ue_id_list = cJSON_GetObjectItemCaseSensitive(group_identifiersJSON,
	                                              "ueIdList");
	if(ue_id_list) {
		cJSON *ue_id_list_local = NULL;
		if(!cJSON_IsArray(ue_id_list)) {
			ogs_error(
				"OpenAPI_group_identifiers_parseFromJSON() failed [ue_id_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ue_id_list_local, ue_id_list) {
			...
			OpenAPI_ue_id_t *ue_id_listItem =
				OpenAPI_ue_id_parseFromJSON(ue_id_list_local);
			if(!ue_id_listItem) {
	...
	group_identifiers_local_var = OpenAPI_group_identifiers_create(
		...
		int_group_id &&
		!cJSON_IsNull(int_group_id) ?
		ogs_strdup(int_group_id->valuestring) : NULL,
