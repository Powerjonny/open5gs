...
char * OpenAPI_periodicity_any_of_ToString(
	...
	const char *periodicity_any_ofArray[] = { "NULL", "YEARLY", "MONTHLY",
		                                  "WEEKLY", "DAILY", "HOURLY" };
	size_t sizeofArray = sizeof(periodicity_any_ofArray) /
	                     sizeof(periodicity_any_ofArray[0]);
...
OpenAPI_periodicity_any_of_e OpenAPI_periodicity_any_of_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(periodicity_any_of,
		          periodicity_any_ofArray[stringToReturn]) == 0) {
