...
OpenAPI_wireline_area_1_t *OpenAPI_wireline_area_1_create(
	OpenAPI_list_t *global_line_ids, OpenAPI_list_t *hfc_n_ids,
	char *area_code_b, char *area_code_c) {
	OpenAPI_wireline_area_1_t *wireline_area_1_local_var =
		ogs_malloc(sizeof(OpenAPI_wireline_area_1_t));
...
cJSON *OpenAPI_wireline_area_1_convertToJSON(
	OpenAPI_wireline_area_1_t *wireline_area_1) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(wireline_area_1 == NULL) {
		ogs_error(
			"OpenAPI_wireline_area_1_convertToJSON() failed [WirelineArea_1]");
		return NULL;
	}
	...
	if(wireline_area_1->global_line_ids) {
		...
		if(global_line_idsList == NULL) {
			ogs_error(
				"OpenAPI_wireline_area_1_convertToJSON() failed [global_line_ids]");
			goto end;
		}
		OpenAPI_list_for_each(wireline_area_1->global_line_ids, node) {
	...
	if(wireline_area_1->hfc_n_ids) {
		...
		if(hfc_n_idsList == NULL) {
			ogs_error(
				"OpenAPI_wireline_area_1_convertToJSON() failed [hfc_n_ids]");
			goto end;
		}
		OpenAPI_list_for_each(wireline_area_1->hfc_n_ids, node) {
	...
	if(wireline_area_1->area_code_b) {
		...
		{
			ogs_error(
				"OpenAPI_wireline_area_1_convertToJSON() failed [area_code_b]");
			goto end;
		}
	}
	...
	if(wireline_area_1->area_code_c) {
		if(cJSON_AddStringToObject(item, "areaCodeC",
		                           wireline_area_1->area_code_c) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_wireline_area_1_convertToJSON() failed [area_code_c]");
			goto end;
		}
	}
...
OpenAPI_wireline_area_1_t *OpenAPI_wireline_area_1_parseFromJSON(
	cJSON *wireline_area_1JSON) {
	...
	global_line_ids = cJSON_GetObjectItemCaseSensitive(wireline_area_1JSON,
	                                                   "globalLineIds");
	if(global_line_ids) {
		cJSON *global_line_ids_local = NULL;
		if(!cJSON_IsArray(global_line_ids)) {
			ogs_error(
				"OpenAPI_wireline_area_1_parseFromJSON() failed [global_line_ids]");
			goto end;
		}
		...
		cJSON_ArrayForEach(global_line_ids_local, global_line_ids) {
			...
			OpenAPI_list_add(global_line_idsList,
			                 ogs_strdup(
						 global_line_ids_local->
						 valuestring));
		}
	}

	hfc_n_ids = cJSON_GetObjectItemCaseSensitive(wireline_area_1JSON,
	                                             "hfcNIds");
	if(hfc_n_ids) {
		cJSON *hfc_n_ids_local = NULL;
		if(!cJSON_IsArray(hfc_n_ids)) {
			ogs_error(
				"OpenAPI_wireline_area_1_parseFromJSON() failed [hfc_n_ids]");
			goto end;
		}
		...
		cJSON_ArrayForEach(hfc_n_ids_local, hfc_n_ids) {
			...
			OpenAPI_list_add(hfc_n_idsList,
			                 ogs_strdup(
						 hfc_n_ids_local->valuestring));
		}
	}

	area_code_b = cJSON_GetObjectItemCaseSensitive(wireline_area_1JSON,
	                                               "areaCodeB");
	if(area_code_b) {
		...
		{
			ogs_error(
				"OpenAPI_wireline_area_1_parseFromJSON() failed [area_code_b]");
			goto end;
		}
	}

	area_code_c = cJSON_GetObjectItemCaseSensitive(wireline_area_1JSON,
	                                               "areaCodeC");
	if(area_code_c) {
		...
		{
			ogs_error(
				"OpenAPI_wireline_area_1_parseFromJSON() failed [area_code_c]");
			goto end;
		}
	}
	...
	wireline_area_1_local_var = OpenAPI_wireline_area_1_create(
		...
		area_code_b &&
		!cJSON_IsNull(area_code_b) ?
		ogs_strdup(area_code_b->valuestring) : NULL,
		area_code_c &&
		!cJSON_IsNull(area_code_c) ?
		ogs_strdup(area_code_c->valuestring) : NULL
		);
...
OpenAPI_wireline_area_1_t *OpenAPI_wireline_area_1_copy(
	OpenAPI_wireline_area_1_t *dst, OpenAPI_wireline_area_1_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
