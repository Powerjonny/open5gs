...
OpenAPI_wlan_per_ts_performance_info_create(char *ts_start, int ts_duration,
                                            bool is_rssi, int rssi, bool is_rtt,
                                            int rtt,
                                            OpenAPI_traffic_information_t *
                                            traffic_info, bool is_number_of_ues,
                                            int number_of_ues,
                                            bool is_confidence,
                                            int confidence) {
	OpenAPI_wlan_per_ts_performance_info_t *
	        wlan_per_ts_performance_info_local_var =
		ogs_malloc(sizeof(OpenAPI_wlan_per_ts_performance_info_t));
...
void OpenAPI_wlan_per_ts_performance_info_free(
	OpenAPI_wlan_per_ts_performance_info_t *wlan_per_ts_performance_info) {
	...
	if(wlan_per_ts_performance_info->traffic_info) {
		OpenAPI_traffic_information_free(
...
cJSON *OpenAPI_wlan_per_ts_performance_info_convertToJSON(
	OpenAPI_wlan_per_ts_performance_info_t *wlan_per_ts_performance_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(wlan_per_ts_performance_info == NULL) {
		ogs_error(
			"OpenAPI_wlan_per_ts_performance_info_convertToJSON() failed [WlanPerTsPerformanceInfo]");
		return NULL;
	}
	...
	if(!wlan_per_ts_performance_info->ts_start) {
		ogs_error(
			"OpenAPI_wlan_per_ts_performance_info_convertToJSON() failed [ts_start]");
		return NULL;
	}
	...
	{
		ogs_error(
			"OpenAPI_wlan_per_ts_performance_info_convertToJSON() failed [ts_duration]");
		goto end;
	}
	...
	if(wlan_per_ts_performance_info->is_rssi) {
		...
		{
			ogs_error(
				"OpenAPI_wlan_per_ts_performance_info_convertToJSON() failed [rssi]");
			goto end;
		}
	}
	...
	if(wlan_per_ts_performance_info->is_rtt) {
		...
		{
			ogs_error(
				"OpenAPI_wlan_per_ts_performance_info_convertToJSON() failed [rtt]");
			goto end;
		}
	}
	...
	if(wlan_per_ts_performance_info->traffic_info) {
		cJSON *traffic_info_local_JSON =
			OpenAPI_traffic_information_convertToJSON(
				wlan_per_ts_performance_info->traffic_info);
		if(traffic_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_wlan_per_ts_performance_info_convertToJSON() failed [traffic_info]");
			goto end;
		}
	...
	if(wlan_per_ts_performance_info->is_number_of_ues) {
		...
		{
			ogs_error(
				"OpenAPI_wlan_per_ts_performance_info_convertToJSON() failed [number_of_ues]");
			goto end;
		}
	}
	...
	if(wlan_per_ts_performance_info->is_confidence) {
		...
		{
			ogs_error(
				"OpenAPI_wlan_per_ts_performance_info_convertToJSON() failed [confidence]");
			goto end;
		}
	}
...
OpenAPI_wlan_per_ts_performance_info_parseFromJSON(
	...
	OpenAPI_wlan_per_ts_performance_info_t *
	        wlan_per_ts_performance_info_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *ts_start = NULL;
	...
	if(!ts_start) {
		ogs_error(
			"OpenAPI_wlan_per_ts_performance_info_parseFromJSON() failed [ts_start]");
		goto end;
	}
	...
	if(!ts_duration) {
		ogs_error(
			"OpenAPI_wlan_per_ts_performance_info_parseFromJSON() failed [ts_duration]");
		goto end;
	}
	...
	if(rssi) {
		if(!cJSON_IsNumber(rssi)) {
			ogs_error(
				"OpenAPI_wlan_per_ts_performance_info_parseFromJSON() failed [rssi]");
			goto end;
		}
	}
	...
	if(rtt) {
		if(!cJSON_IsNumber(rtt)) {
			ogs_error(
				"OpenAPI_wlan_per_ts_performance_info_parseFromJSON() failed [rtt]");
			goto end;
		}
	}
	...
	if(traffic_info) {
		traffic_info_local_nonprim =
			OpenAPI_traffic_information_parseFromJSON(traffic_info);
		if(!traffic_info_local_nonprim) {
			ogs_error(
				"OpenAPI_traffic_information_parseFromJSON failed [traffic_info]");
			goto end;
		}
	}
	...
	if(number_of_ues) {
		if(!cJSON_IsNumber(number_of_ues)) {
			ogs_error(
				"OpenAPI_wlan_per_ts_performance_info_parseFromJSON() failed [number_of_ues]");
			goto end;
		}
	}
	...
	if(confidence) {
		if(!cJSON_IsNumber(confidence)) {
			ogs_error(
				"OpenAPI_wlan_per_ts_performance_info_parseFromJSON() failed [confidence]");
			goto end;
		}
	}

	wlan_per_ts_performance_info_local_var =
		OpenAPI_wlan_per_ts_performance_info_create(
...
OpenAPI_wlan_per_ts_performance_info_copy(
	OpenAPI_wlan_per_ts_performance_info_t	*dst,
	OpenAPI_wlan_per_ts_performance_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_wlan_per_ts_performance_info_convertToJSON() failed");
		return NULL;
	}
