...
OpenAPI_amf_event_mode_t *OpenAPI_amf_event_mode_create(
	...
	OpenAPI_amf_event_mode_t *amf_event_mode_local_var =
		ogs_malloc(sizeof(OpenAPI_amf_event_mode_t));
...
cJSON *OpenAPI_amf_event_mode_convertToJSON(
	...
	if(amf_event_mode == NULL) {
		ogs_error(
			"OpenAPI_amf_event_mode_convertToJSON() failed [AmfEventMode]");
		return NULL;
	}
	...
	if(!amf_event_mode->trigger) {
		ogs_error(
			"OpenAPI_amf_event_mode_convertToJSON() failed [trigger]");
		return NULL;
	}
	cJSON *trigger_local_JSON =
		OpenAPI_amf_event_trigger_convertToJSON(
	...
	if(amf_event_mode->is_max_reports) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_mode_convertToJSON() failed [max_reports]");
			goto end;
		}
	}
	...
	if(amf_event_mode->expiry) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_mode_convertToJSON() failed [expiry]");
			goto end;
		}
	}
	...
	if(amf_event_mode->is_rep_period) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_mode_convertToJSON() failed [rep_period]");
			goto end;
		}
	}
	...
	if(amf_event_mode->is_samp_ratio) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_mode_convertToJSON() failed [samp_ratio]");
			goto end;
		}
	}
	   ...
	   OpenAPI_partitioning_criteria_NULL)
	{
		...
		if(partitioning_criteriaList == NULL) {
			ogs_error(
				"OpenAPI_amf_event_mode_convertToJSON() failed [partitioning_criteria]");
			goto end;
		}
		OpenAPI_list_for_each(amf_event_mode->partitioning_criteria,
		                      node) {
			if(cJSON_AddStringToObject(partitioning_criteriaList,
			                           "",
	...
	if(amf_event_mode->notif_flag != OpenAPI_notification_flag_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_mode_convertToJSON() failed [notif_flag]");
			goto end;
		}
	}
...
OpenAPI_amf_event_mode_t *OpenAPI_amf_event_mode_parseFromJSON(
	...
	trigger = cJSON_GetObjectItemCaseSensitive(amf_event_modeJSON,
	                                           "trigger");
	if(!trigger) {
		ogs_error(
			"OpenAPI_amf_event_mode_parseFromJSON() failed [trigger]");
		goto end;
	}
	trigger_local_nonprim =
		OpenAPI_amf_event_trigger_parseFromJSON(trigger);
	if(!trigger_local_nonprim) {
		ogs_error(
			"OpenAPI_amf_event_trigger_parseFromJSON failed [trigger]");
		goto end;
	}
	...
	if(max_reports) {
		if(!cJSON_IsNumber(max_reports)) {
			ogs_error(
				"OpenAPI_amf_event_mode_parseFromJSON() failed [max_reports]");
			goto end;
		}
	}
	...
	if(expiry) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_mode_parseFromJSON() failed [expiry]");
			goto end;
		}
	}
	...
	if(rep_period) {
		if(!cJSON_IsNumber(rep_period)) {
			ogs_error(
				"OpenAPI_amf_event_mode_parseFromJSON() failed [rep_period]");
			goto end;
		}
	}
	...
	if(samp_ratio) {
		if(!cJSON_IsNumber(samp_ratio)) {
			ogs_error(
				"OpenAPI_amf_event_mode_parseFromJSON() failed [samp_ratio]");
			goto end;
		}
	}

	partitioning_criteria =
		cJSON_GetObjectItemCaseSensitive(amf_event_modeJSON,
		                                 "partitioningCriteria");
	if(partitioning_criteria) {
		cJSON *partitioning_criteria_local = NULL;
		if(!cJSON_IsArray(partitioning_criteria)) {
			ogs_error(
				"OpenAPI_amf_event_mode_parseFromJSON() failed [partitioning_criteria]");
			goto end;
		}
		...
		cJSON_ArrayForEach(partitioning_criteria_local,
			...
			if(!localEnum) {
				ogs_info(
					"Enum value \"%s\" for field \"partitioning_criteria\" is not supported. Ignoring it ...",
					partitioning_criteria_local->
					valuestring);
			} else {
				OpenAPI_list_add(partitioning_criteriaList,
				                 (void *) localEnum);
			}
		}
		if(partitioning_criteriaList->count == 0) {
			ogs_error(
				"OpenAPI_amf_event_mode_parseFromJSON() failed: Expected partitioning_criteriaList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(notif_flag) {
		if(!cJSON_IsString(notif_flag)) {
			ogs_error(
				"OpenAPI_amf_event_mode_parseFromJSON() failed [notif_flag]");
			goto end;
		}
		notif_flagVariable =
