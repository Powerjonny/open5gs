...
OpenAPI_seaf_data_t *OpenAPI_seaf_data_create(OpenAPI_ng_ksi_t *ng_ksi,
	...
	OpenAPI_seaf_data_t *seaf_data_local_var =
		ogs_malloc(sizeof(OpenAPI_seaf_data_t));
...
cJSON *OpenAPI_seaf_data_convertToJSON(OpenAPI_seaf_data_t *seaf_data) {
	...
	if(seaf_data == NULL) {
		ogs_error(
			"OpenAPI_seaf_data_convertToJSON() failed [SeafData]");
		return NULL;
	}
	...
	cJSON *ng_ksi_local_JSON =
		OpenAPI_ng_ksi_convertToJSON(seaf_data->ng_ksi);
	...
	cJSON *key_amf_local_JSON =
		OpenAPI_key_amf_convertToJSON(seaf_data->key_amf);
	...
	if(seaf_data->nh) {
		if(cJSON_AddStringToObject(item, "nh", seaf_data->nh) == NULL) {
			ogs_error(
				"OpenAPI_seaf_data_convertToJSON() failed [nh]");
			goto end;
		}
	}
	...
	if(seaf_data->is_ncc) {
		...
		{
			ogs_error(
				"OpenAPI_seaf_data_convertToJSON() failed [ncc]");
			goto end;
		}
	}
	...
	if(seaf_data->is_key_amf_change_ind) {
		...
		{
			ogs_error(
				"OpenAPI_seaf_data_convertToJSON() failed [key_amf_change_ind]");
			goto end;
		}
	}
	...
	if(seaf_data->is_key_amf_h_derivation_ind) {
		...
		{
			ogs_error(
				"OpenAPI_seaf_data_convertToJSON() failed [key_amf_h_derivation_ind]");
			goto end;
		}
	}
...
OpenAPI_seaf_data_t *OpenAPI_seaf_data_parseFromJSON(cJSON *seaf_dataJSON) {
	...
	if(nh) {
		...
		{
			ogs_error(
				"OpenAPI_seaf_data_parseFromJSON() failed [nh]");
			goto end;
		}
	}
	...
	if(ncc) {
		if(!cJSON_IsNumber(ncc)) {
			ogs_error(
				"OpenAPI_seaf_data_parseFromJSON() failed [ncc]");
			goto end;
		}
	}
	...
	if(key_amf_change_ind) {
		if(!cJSON_IsBool(key_amf_change_ind)) {
			ogs_error(
				"OpenAPI_seaf_data_parseFromJSON() failed [key_amf_change_ind]");
			goto end;
		}
	}

	key_amf_h_derivation_ind =
		cJSON_GetObjectItemCaseSensitive(seaf_dataJSON,
		                                 "keyAmfHDerivationInd");
	if(key_amf_h_derivation_ind) {
		if(!cJSON_IsBool(key_amf_h_derivation_ind)) {
			ogs_error(
				"OpenAPI_seaf_data_parseFromJSON() failed [key_amf_h_derivation_ind]");
			goto end;
		}
	}
	...
	seaf_data_local_var = OpenAPI_seaf_data_create(
		...
		nh &&
		!cJSON_IsNull(nh) ? ogs_strdup(nh->valuestring) : NULL,
		ncc ? true : false,
		...
		key_amf_h_derivation_ind ? key_amf_h_derivation_ind->valueint :
		0
		);
