...
OpenAPI_pcf_binding_t *OpenAPI_pcf_binding_create(char *supi, char *gpsi,
                                                  char *ipv4_addr,
                                                  char *ipv6_prefix,
                                                  OpenAPI_list_t *
                                                  add_ipv6_prefixes,
                                                  ...
                                                  char *pcf_sm_fqdn,
                                                  OpenAPI_list_t *
                                                  pcf_sm_ip_end_points,
                                                  OpenAPI_snssai_t *snssai,
                                                  char *supp_feat, char *pcf_id,
                                                  char *pcf_set_id,
                                                  char *recovery_time,
	...
	OpenAPI_pcf_binding_t *pcf_binding_local_var =
		ogs_malloc(sizeof(OpenAPI_pcf_binding_t));
...
void OpenAPI_pcf_binding_free(OpenAPI_pcf_binding_t *pcf_binding) {
	...
	if(pcf_binding->ipv6_frame_route_list) {
		OpenAPI_list_for_each(pcf_binding->ipv6_frame_route_list,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_pcf_binding_convertToJSON(OpenAPI_pcf_binding_t *pcf_binding) {
	...
	if(pcf_binding == NULL) {
		ogs_error(
			"OpenAPI_pcf_binding_convertToJSON() failed [PcfBinding]");
		return NULL;
	}
	...
	if(pcf_binding->supi) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(pcf_binding->gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [gpsi]");
			goto end;
		}
	}
	...
	if(pcf_binding->ipv4_addr) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [ipv4_addr]");
			goto end;
		}
	}

	if(pcf_binding->ipv6_prefix) {
		if(cJSON_AddStringToObject(item, "ipv6Prefix",
		                           pcf_binding->ipv6_prefix) == NULL)
		{
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [ipv6_prefix]");
			goto end;
		}
	}

	if(pcf_binding->add_ipv6_prefixes) {
		cJSON *add_ipv6_prefixesList = cJSON_AddArrayToObject(item,
		                                                      "addIpv6Prefixes");
		if(add_ipv6_prefixesList == NULL) {
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [add_ipv6_prefixes]");
			goto end;
		}
		OpenAPI_list_for_each(pcf_binding->add_ipv6_prefixes, node) {
			if(cJSON_AddStringToObject(add_ipv6_prefixesList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_pcf_binding_convertToJSON() failed [add_ipv6_prefixes]");
	...
	if(pcf_binding->ip_domain) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [ip_domain]");
			goto end;
		}
	}

	if(pcf_binding->mac_addr48) {
		if(cJSON_AddStringToObject(item, "macAddr48",
		                           pcf_binding->mac_addr48) == NULL)
		{
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [mac_addr48]");
			goto end;
		}
	}

	if(pcf_binding->add_mac_addrs) {
		cJSON *add_mac_addrsList = cJSON_AddArrayToObject(item,
		                                                  "addMacAddrs");
		if(add_mac_addrsList == NULL) {
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [add_mac_addrs]");
			goto end;
		}
		OpenAPI_list_for_each(pcf_binding->add_mac_addrs, node) {
			if(cJSON_AddStringToObject(add_mac_addrsList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_pcf_binding_convertToJSON() failed [add_mac_addrs]");
	...
	if(pcf_binding->pcf_fqdn) {
		if(cJSON_AddStringToObject(item, "pcfFqdn",
		                           pcf_binding->pcf_fqdn) == NULL)
		{
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [pcf_fqdn]");
			goto end;
		}
	}

	if(pcf_binding->pcf_ip_end_points) {
		cJSON *pcf_ip_end_pointsList = cJSON_AddArrayToObject(item,
		                                                      "pcfIpEndPoints");
		if(pcf_ip_end_pointsList == NULL) {
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [pcf_ip_end_points]");
			goto end;
		}
		OpenAPI_list_for_each(pcf_binding->pcf_ip_end_points, node) {
			cJSON *itemLocal =
	...
	if(pcf_binding->pcf_diam_host) {
		if(cJSON_AddStringToObject(item, "pcfDiamHost",
		                           pcf_binding->pcf_diam_host) == NULL)
		{
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [pcf_diam_host]");
			goto end;
		}
	}

	if(pcf_binding->pcf_diam_realm) {
		if(cJSON_AddStringToObject(item, "pcfDiamRealm",
		                           pcf_binding->pcf_diam_realm) == NULL)
		{
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [pcf_diam_realm]");
			goto end;
		}
	}

	if(pcf_binding->pcf_sm_fqdn) {
		if(cJSON_AddStringToObject(item, "pcfSmFqdn",
		                           pcf_binding->pcf_sm_fqdn) == NULL)
		{
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [pcf_sm_fqdn]");
			goto end;
		}
	}

	if(pcf_binding->pcf_sm_ip_end_points) {
		cJSON *pcf_sm_ip_end_pointsList = cJSON_AddArrayToObject(item,
		                                                         "pcfSmIpEndPoints");
		if(pcf_sm_ip_end_pointsList == NULL) {
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [pcf_sm_ip_end_points]");
			goto end;
		}
		OpenAPI_list_for_each(pcf_binding->pcf_sm_ip_end_points, node) {
			cJSON *itemLocal =
			...
			cJSON_AddItemToArray(pcf_sm_ip_end_pointsList,
			                     itemLocal);
		}
	}
	...
	if(!pcf_binding->snssai) {
		ogs_error(
			"OpenAPI_pcf_binding_convertToJSON() failed [snssai]");
		return NULL;
	}
	cJSON *snssai_local_JSON =
		OpenAPI_snssai_convertToJSON(pcf_binding->snssai);
	...
	if(pcf_binding->supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [supp_feat]");
			goto end;
		}
	}
	...
	if(pcf_binding->pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [pcf_id]");
			goto end;
		}
	}
	...
	if(pcf_binding->pcf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [pcf_set_id]");
			goto end;
		}
	}
	...
	if(pcf_binding->recovery_time) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [recovery_time]");
			goto end;
		}
	}

	if(pcf_binding->para_com) {
		cJSON *para_com_local_JSON =
			OpenAPI_parameter_combination_convertToJSON(
				pcf_binding->para_com);
		if(para_com_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [para_com]");
			goto end;
		}
	...
	if(pcf_binding->bind_level != OpenAPI_binding_level_NULL) {
		if(cJSON_AddStringToObject(item, "bindLevel",
		                           OpenAPI_binding_level_ToString(
						   pcf_binding->bind_level)) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [bind_level]");
			goto end;
		}
	}
	...
	if(pcf_binding->ipv4_frame_route_list) {
		...
		if(ipv4_frame_route_listList == NULL) {
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [ipv4_frame_route_list]");
			goto end;
		}
		OpenAPI_list_for_each(pcf_binding->ipv4_frame_route_list,
		                      node) {
	...
	if(pcf_binding->ipv6_frame_route_list) {
		...
		if(ipv6_frame_route_listList == NULL) {
			ogs_error(
				"OpenAPI_pcf_binding_convertToJSON() failed [ipv6_frame_route_list]");
			goto end;
		}
		OpenAPI_list_for_each(pcf_binding->ipv6_frame_route_list,
		                      node) {
...
OpenAPI_pcf_binding_t *OpenAPI_pcf_binding_parseFromJSON(
	cJSON *pcf_bindingJSON) {
	...
	if(supi) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [gpsi]");
			goto end;
		}
	}

	ipv4_addr = cJSON_GetObjectItemCaseSensitive(pcf_bindingJSON,
	                                             "ipv4Addr");
	if(ipv4_addr) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [ipv4_addr]");
			goto end;
		}
	}

	ipv6_prefix = cJSON_GetObjectItemCaseSensitive(pcf_bindingJSON,
	                                               "ipv6Prefix");
	if(ipv6_prefix) {
		if(!cJSON_IsString(ipv6_prefix) &&
		   !cJSON_IsNull(ipv6_prefix))
		{
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [ipv6_prefix]");
			goto end;
		}
	}

	add_ipv6_prefixes = cJSON_GetObjectItemCaseSensitive(pcf_bindingJSON,
	                                                     "addIpv6Prefixes");
	if(add_ipv6_prefixes) {
		cJSON *add_ipv6_prefixes_local = NULL;
		if(!cJSON_IsArray(add_ipv6_prefixes)) {
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [add_ipv6_prefixes]");
			goto end;
		}
		...
		cJSON_ArrayForEach(add_ipv6_prefixes_local, add_ipv6_prefixes) {
			...
			OpenAPI_list_add(add_ipv6_prefixesList,
			                 ogs_strdup(
						 add_ipv6_prefixes_local->
						 valuestring));
		}
	}

	ip_domain = cJSON_GetObjectItemCaseSensitive(pcf_bindingJSON,
	                                             "ipDomain");
	if(ip_domain) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [ip_domain]");
			goto end;
		}
	}

	mac_addr48 = cJSON_GetObjectItemCaseSensitive(pcf_bindingJSON,
	                                              "macAddr48");
	if(mac_addr48) {
		if(!cJSON_IsString(mac_addr48) &&
		   !cJSON_IsNull(mac_addr48))
		{
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [mac_addr48]");
			goto end;
		}
	}

	add_mac_addrs = cJSON_GetObjectItemCaseSensitive(pcf_bindingJSON,
	                                                 "addMacAddrs");
	if(add_mac_addrs) {
		cJSON *add_mac_addrs_local = NULL;
		if(!cJSON_IsArray(add_mac_addrs)) {
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [add_mac_addrs]");
			goto end;
		}
		...
		cJSON_ArrayForEach(add_mac_addrs_local, add_mac_addrs) {
			...
			OpenAPI_list_add(add_mac_addrsList,
			                 ogs_strdup(
						 add_mac_addrs_local->
						 valuestring));
		}
	...
	if(pcf_fqdn) {
		if(!cJSON_IsString(pcf_fqdn) &&
		   !cJSON_IsNull(pcf_fqdn))
		{
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [pcf_fqdn]");
			goto end;
		}
	}

	pcf_ip_end_points = cJSON_GetObjectItemCaseSensitive(pcf_bindingJSON,
	                                                     "pcfIpEndPoints");
	if(pcf_ip_end_points) {
		cJSON *pcf_ip_end_points_local = NULL;
		if(!cJSON_IsArray(pcf_ip_end_points)) {
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [pcf_ip_end_points]");
			goto end;
		}
		...
		cJSON_ArrayForEach(pcf_ip_end_points_local, pcf_ip_end_points) {
			...
			OpenAPI_ip_end_point_t *pcf_ip_end_pointsItem =
				OpenAPI_ip_end_point_parseFromJSON(
					pcf_ip_end_points_local);
			if(!pcf_ip_end_pointsItem) {
				ogs_error("No pcf_ip_end_pointsItem");
				goto end;
			}
			OpenAPI_list_add(pcf_ip_end_pointsList,
			                 pcf_ip_end_pointsItem);
		}
	}

	pcf_diam_host = cJSON_GetObjectItemCaseSensitive(pcf_bindingJSON,
	                                                 "pcfDiamHost");
	if(pcf_diam_host) {
		if(!cJSON_IsString(pcf_diam_host) &&
		   !cJSON_IsNull(pcf_diam_host))
		{
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [pcf_diam_host]");
			goto end;
		}
	}

	pcf_diam_realm = cJSON_GetObjectItemCaseSensitive(pcf_bindingJSON,
	                                                  "pcfDiamRealm");
	if(pcf_diam_realm) {
		if(!cJSON_IsString(pcf_diam_realm) &&
		   !cJSON_IsNull(pcf_diam_realm))
		{
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [pcf_diam_realm]");
			goto end;
		}
	}

	pcf_sm_fqdn = cJSON_GetObjectItemCaseSensitive(pcf_bindingJSON,
	                                               "pcfSmFqdn");
	if(pcf_sm_fqdn) {
		if(!cJSON_IsString(pcf_sm_fqdn) &&
		   !cJSON_IsNull(pcf_sm_fqdn))
		{
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [pcf_sm_fqdn]");
			goto end;
		}
	}

	pcf_sm_ip_end_points = cJSON_GetObjectItemCaseSensitive(pcf_bindingJSON,
	                                                        "pcfSmIpEndPoints");
	if(pcf_sm_ip_end_points) {
		cJSON *pcf_sm_ip_end_points_local = NULL;
		if(!cJSON_IsArray(pcf_sm_ip_end_points)) {
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [pcf_sm_ip_end_points]");
			goto end;
		}

		pcf_sm_ip_end_pointsList = OpenAPI_list_create();

		cJSON_ArrayForEach(pcf_sm_ip_end_points_local,
		                   pcf_sm_ip_end_points) {
			...
			OpenAPI_ip_end_point_t *pcf_sm_ip_end_pointsItem =
				OpenAPI_ip_end_point_parseFromJSON(
					pcf_sm_ip_end_points_local);
			if(!pcf_sm_ip_end_pointsItem) {
				ogs_error("No pcf_sm_ip_end_pointsItem");
				goto end;
			}
			OpenAPI_list_add(pcf_sm_ip_end_pointsList,
	...
	if(!snssai) {
		ogs_error(
			"OpenAPI_pcf_binding_parseFromJSON() failed [snssai]");
		goto end;
	}
	...
	supp_feat = cJSON_GetObjectItemCaseSensitive(pcf_bindingJSON,
	                                             "suppFeat");
	if(supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [supp_feat]");
			goto end;
		}
	}
	...
	if(pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [pcf_id]");
			goto end;
		}
	}

	pcf_set_id = cJSON_GetObjectItemCaseSensitive(pcf_bindingJSON,
	                                              "pcfSetId");
	if(pcf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [pcf_set_id]");
			goto end;
		}
	}

	recovery_time = cJSON_GetObjectItemCaseSensitive(pcf_bindingJSON,
	                                                 "recoveryTime");
	if(recovery_time) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [recovery_time]");
			goto end;
		}
	}
	...
	if(para_com) {
		para_com_local_nonprim =
			OpenAPI_parameter_combination_parseFromJSON(para_com);
		if(!para_com_local_nonprim) {
			ogs_error(
				"OpenAPI_parameter_combination_parseFromJSON failed [para_com]");
			goto end;
		}
	}

	bind_level = cJSON_GetObjectItemCaseSensitive(pcf_bindingJSON,
	                                              "bindLevel");
	if(bind_level) {
		if(!cJSON_IsString(bind_level)) {
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [bind_level]");
			goto end;
		}
		bind_levelVariable =
			OpenAPI_binding_level_FromString(
				bind_level->valuestring);
	}

	ipv4_frame_route_list =
		cJSON_GetObjectItemCaseSensitive(pcf_bindingJSON,
		                                 "ipv4FrameRouteList");
	if(ipv4_frame_route_list) {
		cJSON *ipv4_frame_route_list_local = NULL;
		if(!cJSON_IsArray(ipv4_frame_route_list)) {
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [ipv4_frame_route_list]");
			goto end;
		}
	...
	if(ipv6_frame_route_list) {
		...
		if(!cJSON_IsArray(ipv6_frame_route_list)) {
			ogs_error(
				"OpenAPI_pcf_binding_parseFromJSON() failed [ipv6_frame_route_list]");
			goto end;
		}
	...
	pcf_binding_local_var = OpenAPI_pcf_binding_create(
		...
		ipv4_addr &&
		!cJSON_IsNull(ipv4_addr) ? ogs_strdup(ipv4_addr->valuestring) :
		NULL,
		ipv6_prefix &&
		!cJSON_IsNull(ipv6_prefix) ?
		ogs_strdup(ipv6_prefix->valuestring) : NULL,
		add_ipv6_prefixes ? add_ipv6_prefixesList : NULL,
		ip_domain &&
		!cJSON_IsNull(ip_domain) ? ogs_strdup(ip_domain->valuestring) :
		NULL,
		mac_addr48 &&
		!cJSON_IsNull(mac_addr48) ?
		ogs_strdup(mac_addr48->valuestring) : NULL,
		add_mac_addrs ? add_mac_addrsList : NULL,
		ogs_strdup(dnn->valuestring),
		pcf_fqdn &&
		!cJSON_IsNull(pcf_fqdn) ? ogs_strdup(pcf_fqdn->valuestring) :
		NULL,
		pcf_ip_end_points ? pcf_ip_end_pointsList : NULL,
		pcf_diam_host &&
		!cJSON_IsNull(pcf_diam_host) ?
		ogs_strdup(pcf_diam_host->valuestring) : NULL,
		pcf_diam_realm &&
		!cJSON_IsNull(pcf_diam_realm) ?
		ogs_strdup(pcf_diam_realm->valuestring) : NULL,
		pcf_sm_fqdn &&
		!cJSON_IsNull(pcf_sm_fqdn) ?
		ogs_strdup(pcf_sm_fqdn->valuestring) : NULL,
...
OpenAPI_pcf_binding_t *OpenAPI_pcf_binding_copy(OpenAPI_pcf_binding_t	*dst,
                                                OpenAPI_pcf_binding_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
