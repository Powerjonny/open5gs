...
OpenAPI_media_sub_component_rm_t *OpenAPI_media_sub_component_rm_create(
	...
	OpenAPI_media_sub_component_rm_t *media_sub_component_rm_local_var =
		ogs_malloc(sizeof(OpenAPI_media_sub_component_rm_t));
	ogs_assert(media_sub_component_rm_local_var);

	media_sub_component_rm_local_var->af_sig_protocol = af_sig_protocol;
	media_sub_component_rm_local_var->is_ethf_descs_null =
		is_ethf_descs_null;
...
void OpenAPI_media_sub_component_rm_free(
	...
	if(media_sub_component_rm->ethf_descs) {
		OpenAPI_list_for_each(media_sub_component_rm->ethf_descs,
		                      node) {
...
cJSON *OpenAPI_media_sub_component_rm_convertToJSON(
	...
	if(media_sub_component_rm == NULL) {
		ogs_error(
			"OpenAPI_media_sub_component_rm_convertToJSON() failed [MediaSubComponentRm]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(media_sub_component_rm->af_sig_protocol !=
	   OpenAPI_af_sig_protocol_NULL)
	{
		if(cJSON_AddStringToObject(item, "afSigProtocol",
		                           OpenAPI_af_sig_protocol_ToString(
						   media_sub_component_rm->
						   af_sig_protocol)) == NULL)
		{
			ogs_error(
				"OpenAPI_media_sub_component_rm_convertToJSON() failed [af_sig_protocol]");
			goto end;
		}
	}

	if(media_sub_component_rm->ethf_descs) {
		cJSON *ethf_descsList = cJSON_AddArrayToObject(item,
		                                               "ethfDescs");
		if(ethf_descsList == NULL) {
			ogs_error(
				"OpenAPI_media_sub_component_rm_convertToJSON() failed [ethf_descs]");
			goto end;
		}
		OpenAPI_list_for_each(media_sub_component_rm->ethf_descs,
		                      node) {
	...
	if(cJSON_AddNumberToObject(item, "fNum",
	                           media_sub_component_rm->f_num) == NULL)
	{
		ogs_error(
			"OpenAPI_media_sub_component_rm_convertToJSON() failed [f_num]");
		goto end;
	}
	...
	if(media_sub_component_rm->f_descs) {
		...
		if(f_descsList == NULL) {
			ogs_error(
				"OpenAPI_media_sub_component_rm_convertToJSON() failed [f_descs]");
			goto end;
		}
		OpenAPI_list_for_each(media_sub_component_rm->f_descs, node) {
			if(cJSON_AddStringToObject(f_descsList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_media_sub_component_rm_convertToJSON() failed [f_descs]");
	...
	if(media_sub_component_rm->f_status != OpenAPI_flow_status_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_media_sub_component_rm_convertToJSON() failed [f_status]");
			goto end;
		}
	}
	...
	if(media_sub_component_rm->mar_bw_dl) {
		if(cJSON_AddStringToObject(item, "marBwDl",
		                           media_sub_component_rm->mar_bw_dl) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_media_sub_component_rm_convertToJSON() failed [mar_bw_dl]");
			goto end;
		}
	...
	if(media_sub_component_rm->mar_bw_ul) {
		if(cJSON_AddStringToObject(item, "marBwUl",
		                           media_sub_component_rm->mar_bw_ul) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_media_sub_component_rm_convertToJSON() failed [mar_bw_ul]");
			goto end;
		}
	...
	if(media_sub_component_rm->tos_tr_cl) {
		if(cJSON_AddStringToObject(item, "tosTrCl",
		                           media_sub_component_rm->tos_tr_cl) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_media_sub_component_rm_convertToJSON() failed [tos_tr_cl]");
			goto end;
		}
	...
	if(media_sub_component_rm->flow_usage != OpenAPI_flow_usage_NULL) {
		if(cJSON_AddStringToObject(item, "flowUsage",
		                           OpenAPI_flow_usage_ToString(
						   media_sub_component_rm->
						   flow_usage)) == NULL)
		{
			ogs_error(
				"OpenAPI_media_sub_component_rm_convertToJSON() failed [flow_usage]");
			goto end;
		}
	}
...
OpenAPI_media_sub_component_rm_t *OpenAPI_media_sub_component_rm_parseFromJSON(
	...
	af_sig_protocol =
		cJSON_GetObjectItemCaseSensitive(media_sub_component_rmJSON,
		                                 "afSigProtocol");
	if(af_sig_protocol) {
		if(!cJSON_IsString(af_sig_protocol)) {
			ogs_error(
				"OpenAPI_media_sub_component_rm_parseFromJSON() failed [af_sig_protocol]");
			goto end;
		}
	...
	if(ethf_descs) {
		if(!cJSON_IsNull(ethf_descs)) {
			...
			if(!cJSON_IsArray(ethf_descs)) {
				ogs_error(
					"OpenAPI_media_sub_component_rm_parseFromJSON() failed [ethf_descs]");
				goto end;
			}
			...
			cJSON_ArrayForEach(ethf_descs_local, ethf_descs) {
				...
				OpenAPI_eth_flow_description_t *ethf_descsItem =
				...
				OpenAPI_list_add(ethf_descsList,
				                 ethf_descsItem);
			}
		}
	}

	f_num = cJSON_GetObjectItemCaseSensitive(media_sub_component_rmJSON,
	                                         "fNum");
	if(!f_num) {
		ogs_error(
			"OpenAPI_media_sub_component_rm_parseFromJSON() failed [f_num]");
		goto end;
	}
	...
	f_descs = cJSON_GetObjectItemCaseSensitive(media_sub_component_rmJSON,
	                                           "fDescs");
	if(f_descs) {
		if(!cJSON_IsNull(f_descs)) {
			...
			if(!cJSON_IsArray(f_descs)) {
				ogs_error(
					"OpenAPI_media_sub_component_rm_parseFromJSON() failed [f_descs]");
				goto end;
			}
	...
	f_status = cJSON_GetObjectItemCaseSensitive(media_sub_component_rmJSON,
	                                            "fStatus");
	if(f_status) {
		if(!cJSON_IsString(f_status)) {
			ogs_error(
				"OpenAPI_media_sub_component_rm_parseFromJSON() failed [f_status]");
			goto end;
		}
		f_statusVariable =
	...
	mar_bw_dl = cJSON_GetObjectItemCaseSensitive(media_sub_component_rmJSON,
	                                             "marBwDl");
	if(mar_bw_dl) {
		if(!cJSON_IsNull(mar_bw_dl)) {
			...
			{
				ogs_error(
					"OpenAPI_media_sub_component_rm_parseFromJSON() failed [mar_bw_dl]");
				goto end;
			}
		}
	}

	mar_bw_ul = cJSON_GetObjectItemCaseSensitive(media_sub_component_rmJSON,
	                                             "marBwUl");
	if(mar_bw_ul) {
		if(!cJSON_IsNull(mar_bw_ul)) {
			...
			{
				ogs_error(
					"OpenAPI_media_sub_component_rm_parseFromJSON() failed [mar_bw_ul]");
				goto end;
			}
		}
	}

	tos_tr_cl = cJSON_GetObjectItemCaseSensitive(media_sub_component_rmJSON,
	                                             "tosTrCl");
	if(tos_tr_cl) {
		if(!cJSON_IsNull(tos_tr_cl)) {
			if(!cJSON_IsString(tos_tr_cl) &&
			   !cJSON_IsNull(tos_tr_cl))
			{
				ogs_error(
					"OpenAPI_media_sub_component_rm_parseFromJSON() failed [tos_tr_cl]");
				goto end;
			}
		}
	}
	...
	if(flow_usage) {
		if(!cJSON_IsString(flow_usage)) {
			ogs_error(
				"OpenAPI_media_sub_component_rm_parseFromJSON() failed [flow_usage]");
			goto end;
		}
		flow_usageVariable =
			OpenAPI_flow_usage_FromString(flow_usage->valuestring);
	}

	media_sub_component_rm_local_var =
		OpenAPI_media_sub_component_rm_create(
			af_sig_protocol ? af_sig_protocolVariable : 0,
			ethf_descs &&
			cJSON_IsNull(ethf_descs) ? true : false,
			ethf_descs ? ethf_descsList : NULL,

			f_num->valuedouble,
			f_descs &&
			cJSON_IsNull(f_descs) ? true : false,
			...
			mar_bw_dl &&
			!cJSON_IsNull(mar_bw_dl) ?
			ogs_strdup(mar_bw_dl->valuestring) :
			NULL,
			mar_bw_ul &&
			cJSON_IsNull(mar_bw_ul) ? true : false,
			mar_bw_ul &&
			!cJSON_IsNull(mar_bw_ul) ?
			ogs_strdup(mar_bw_ul->valuestring) :
			NULL,
			tos_tr_cl &&
			cJSON_IsNull(tos_tr_cl) ? true : false,
			tos_tr_cl &&
			!cJSON_IsNull(tos_tr_cl) ?
			ogs_strdup(tos_tr_cl->valuestring) :
			NULL,
...
OpenAPI_media_sub_component_rm_t *OpenAPI_media_sub_component_rm_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_media_sub_component_rm_convertToJSON() failed");
		return NULL;
	}
