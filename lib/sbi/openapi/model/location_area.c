...
OpenAPI_location_area_t *OpenAPI_location_area_create(
	...
	OpenAPI_location_area_t *location_area_local_var =
		ogs_malloc(sizeof(OpenAPI_location_area_t));
...
cJSON *OpenAPI_location_area_convertToJSON(
	...
	if(location_area == NULL) {
		ogs_error(
			"OpenAPI_location_area_convertToJSON() failed [LocationArea]");
		return NULL;
	}
	...
	if(location_area->geographic_areas) {
		...
		if(geographic_areasList == NULL) {
			ogs_error(
				"OpenAPI_location_area_convertToJSON() failed [geographic_areas]");
			goto end;
		}
		OpenAPI_list_for_each(location_area->geographic_areas, node) {
			cJSON *itemLocal =
	...
	if(location_area->civic_addresses) {
		...
		if(civic_addressesList == NULL) {
			ogs_error(
				"OpenAPI_location_area_convertToJSON() failed [civic_addresses]");
			goto end;
		}
		OpenAPI_list_for_each(location_area->civic_addresses, node) {
			cJSON *itemLocal =
	...
	if(location_area->nw_area_info) {
		...
		if(nw_area_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_location_area_convertToJSON() failed [nw_area_info]");
			goto end;
		}
	...
	if(location_area->umt_time) {
		cJSON *umt_time_local_JSON =
			OpenAPI_umt_time_convertToJSON(location_area->umt_time);
		if(umt_time_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_location_area_convertToJSON() failed [umt_time]");
			goto end;
		}
...
OpenAPI_location_area_t *OpenAPI_location_area_parseFromJSON(
	...
	if(geographic_areas) {
		...
		if(!cJSON_IsArray(geographic_areas)) {
			ogs_error(
				"OpenAPI_location_area_parseFromJSON() failed [geographic_areas]");
			goto end;
		}
	...
	if(civic_addresses) {
		...
		if(!cJSON_IsArray(civic_addresses)) {
			ogs_error(
				"OpenAPI_location_area_parseFromJSON() failed [civic_addresses]");
			goto end;
		}
	...
	umt_time = cJSON_GetObjectItemCaseSensitive(location_areaJSON,
	                                            "umtTime");
	if(umt_time) {
		umt_time_local_nonprim =
			OpenAPI_umt_time_parseFromJSON(umt_time);
		if(!umt_time_local_nonprim) {
			ogs_error(
				"OpenAPI_umt_time_parseFromJSON failed [umt_time]");
			goto end;
		}
	}
