...
OpenAPI_udm_info_t *OpenAPI_udm_info_create(char		*group_id,
	...
	OpenAPI_udm_info_t *udm_info_local_var =
		ogs_malloc(sizeof(OpenAPI_udm_info_t));
	...
	udm_info_local_var->external_group_identifiers_ranges =
		external_group_identifiers_ranges;
	udm_info_local_var->routing_indicators = routing_indicators;
	udm_info_local_var->internal_group_identifiers_ranges =
		internal_group_identifiers_ranges;
...
void OpenAPI_udm_info_free(OpenAPI_udm_info_t *udm_info) {
	...
	if(udm_info->external_group_identifiers_ranges) {
		OpenAPI_list_for_each(
			udm_info->external_group_identifiers_ranges, node) {
			OpenAPI_identity_range_free(node->data);
		}
	...
	if(udm_info->internal_group_identifiers_ranges) {
		OpenAPI_list_for_each(
			udm_info->internal_group_identifiers_ranges, node) {
...
cJSON *OpenAPI_udm_info_convertToJSON(OpenAPI_udm_info_t *udm_info) {
	...
	if(udm_info->group_id) {
		...
		{
			ogs_error(
				"OpenAPI_udm_info_convertToJSON() failed [group_id]");
			goto end;
		}
	}
	...
	if(udm_info->supi_ranges) {
		...
		if(supi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_udm_info_convertToJSON() failed [supi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(udm_info->supi_ranges, node) {
			cJSON *itemLocal =
	...
	if(udm_info->gpsi_ranges) {
		...
		if(gpsi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_udm_info_convertToJSON() failed [gpsi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(udm_info->gpsi_ranges, node) {
			cJSON *itemLocal =
	...
	if(udm_info->external_group_identifiers_ranges) {
		...
		if(external_group_identifiers_rangesList == NULL) {
			ogs_error(
				"OpenAPI_udm_info_convertToJSON() failed [external_group_identifiers_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			udm_info->external_group_identifiers_ranges, node) {
	...
	if(udm_info->routing_indicators) {
		...
		if(routing_indicatorsList == NULL) {
			ogs_error(
				"OpenAPI_udm_info_convertToJSON() failed [routing_indicators]");
			goto end;
		}
		OpenAPI_list_for_each(udm_info->routing_indicators, node) {
	...
	if(udm_info->internal_group_identifiers_ranges) {
		...
		if(internal_group_identifiers_rangesList == NULL) {
			ogs_error(
				"OpenAPI_udm_info_convertToJSON() failed [internal_group_identifiers_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			udm_info->internal_group_identifiers_ranges, node) {
	...
	if(udm_info->suci_infos) {
		...
		if(suci_infosList == NULL) {
			ogs_error(
				"OpenAPI_udm_info_convertToJSON() failed [suci_infos]");
			goto end;
		}
		OpenAPI_list_for_each(udm_info->suci_infos, node) {
			cJSON *itemLocal =
...
OpenAPI_udm_info_t *OpenAPI_udm_info_parseFromJSON(cJSON *udm_infoJSON) {
	...
	if(group_id) {
		...
		{
			ogs_error(
				"OpenAPI_udm_info_parseFromJSON() failed [group_id]");
			goto end;
		}
	}

	supi_ranges = cJSON_GetObjectItemCaseSensitive(udm_infoJSON,
	                                               "supiRanges");
	if(supi_ranges) {
		cJSON *supi_ranges_local = NULL;
		if(!cJSON_IsArray(supi_ranges)) {
			ogs_error(
				"OpenAPI_udm_info_parseFromJSON() failed [supi_ranges]");
			goto end;
		}
	...
	gpsi_ranges = cJSON_GetObjectItemCaseSensitive(udm_infoJSON,
	                                               "gpsiRanges");
	if(gpsi_ranges) {
		cJSON *gpsi_ranges_local = NULL;
		if(!cJSON_IsArray(gpsi_ranges)) {
			ogs_error(
				"OpenAPI_udm_info_parseFromJSON() failed [gpsi_ranges]");
			goto end;
		}
	...
	external_group_identifiers_ranges =
		cJSON_GetObjectItemCaseSensitive(udm_infoJSON,
		                                 "externalGroupIdentifiersRanges");
	if(external_group_identifiers_ranges) {
		...
		if(!cJSON_IsArray(external_group_identifiers_ranges)) {
			ogs_error(
				"OpenAPI_udm_info_parseFromJSON() failed [external_group_identifiers_ranges]");
			goto end;
		}
	...
	routing_indicators = cJSON_GetObjectItemCaseSensitive(udm_infoJSON,
	                                                      "routingIndicators");
	if(routing_indicators) {
		cJSON *routing_indicators_local = NULL;
		if(!cJSON_IsArray(routing_indicators)) {
			ogs_error(
				"OpenAPI_udm_info_parseFromJSON() failed [routing_indicators]");
			goto end;
		}
	...
	if(internal_group_identifiers_ranges) {
		...
		if(!cJSON_IsArray(internal_group_identifiers_ranges)) {
			ogs_error(
				"OpenAPI_udm_info_parseFromJSON() failed [internal_group_identifiers_ranges]");
			goto end;
		}
	...
	suci_infos = cJSON_GetObjectItemCaseSensitive(udm_infoJSON,
	                                              "suciInfos");
	if(suci_infos) {
		cJSON *suci_infos_local = NULL;
		if(!cJSON_IsArray(suci_infos)) {
			ogs_error(
				"OpenAPI_udm_info_parseFromJSON() failed [suci_infos]");
			goto end;
		}
