...
OpenAPI_ue_lcs_capability_t *OpenAPI_ue_lcs_capability_create(
	bool is_lpp_support, int lpp_support, bool is_ciot_optimisation,
	int ciot_optimisation) {
	OpenAPI_ue_lcs_capability_t *ue_lcs_capability_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_lcs_capability_t));
...
void OpenAPI_ue_lcs_capability_free(
	OpenAPI_ue_lcs_capability_t *ue_lcs_capability) {
...
cJSON *OpenAPI_ue_lcs_capability_convertToJSON(
	OpenAPI_ue_lcs_capability_t *ue_lcs_capability) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(ue_lcs_capability == NULL) {
		ogs_error(
			"OpenAPI_ue_lcs_capability_convertToJSON() failed [UeLcsCapability]");
		return NULL;
	}
	...
	if(ue_lcs_capability->is_lpp_support) {
		...
		{
			ogs_error(
				"OpenAPI_ue_lcs_capability_convertToJSON() failed [lpp_support]");
			goto end;
		}
	}
	...
	if(ue_lcs_capability->is_ciot_optimisation) {
		...
		{
			ogs_error(
				"OpenAPI_ue_lcs_capability_convertToJSON() failed [ciot_optimisation]");
			goto end;
		}
	}
...
OpenAPI_ue_lcs_capability_t *OpenAPI_ue_lcs_capability_parseFromJSON(
	...
	if(lpp_support) {
		if(!cJSON_IsBool(lpp_support)) {
			ogs_error(
				"OpenAPI_ue_lcs_capability_parseFromJSON() failed [lpp_support]");
			goto end;
		}
	}

	ciot_optimisation =
		cJSON_GetObjectItemCaseSensitive(ue_lcs_capabilityJSON,
		                                 "ciotOptimisation");
	if(ciot_optimisation) {
		if(!cJSON_IsBool(ciot_optimisation)) {
			ogs_error(
				"OpenAPI_ue_lcs_capability_parseFromJSON() failed [ciot_optimisation]");
			goto end;
		}
	}
...
OpenAPI_ue_lcs_capability_t *OpenAPI_ue_lcs_capability_copy(
	OpenAPI_ue_lcs_capability_t *dst, OpenAPI_ue_lcs_capability_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
