	...
	OpenAPI_plmn_id_t *plmn_id)
{
	OpenAPI_eps_iwk_pgw_t *eps_iwk_pgw_local_var =
		ogs_malloc(sizeof(OpenAPI_eps_iwk_pgw_t));
...
cJSON *OpenAPI_eps_iwk_pgw_convertToJSON(OpenAPI_eps_iwk_pgw_t *eps_iwk_pgw) {
	...
	if(eps_iwk_pgw == NULL) {
		ogs_error(
			"OpenAPI_eps_iwk_pgw_convertToJSON() failed [EpsIwkPgw]");
		return NULL;
	}
	...
	if(!eps_iwk_pgw->pgw_fqdn) {
		ogs_error(
			"OpenAPI_eps_iwk_pgw_convertToJSON() failed [pgw_fqdn]");
		return NULL;
	}
	...
	if(!eps_iwk_pgw->smf_instance_id) {
		ogs_error(
			"OpenAPI_eps_iwk_pgw_convertToJSON() failed [smf_instance_id]");
		return NULL;
	}
	...
	if(eps_iwk_pgw->plmn_id) {
		...
		if(plmn_id_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_eps_iwk_pgw_convertToJSON() failed [plmn_id]");
			goto end;
		}
...
OpenAPI_eps_iwk_pgw_t *OpenAPI_eps_iwk_pgw_parseFromJSON(
	cJSON *eps_iwk_pgwJSON) {
	...
	if(!pgw_fqdn) {
		ogs_error(
			"OpenAPI_eps_iwk_pgw_parseFromJSON() failed [pgw_fqdn]");
		goto end;
	}
	...
	if(!smf_instance_id) {
		ogs_error(
			"OpenAPI_eps_iwk_pgw_parseFromJSON() failed [smf_instance_id]");
		goto end;
	}
...
OpenAPI_eps_iwk_pgw_t *OpenAPI_eps_iwk_pgw_copy(OpenAPI_eps_iwk_pgw_t	*dst,
                                                OpenAPI_eps_iwk_pgw_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
