...
char *OpenAPI_condition_typedccf_cond_ToString(
...
OpenAPI_dccf_cond_condition_type_e OpenAPI_condition_typedccf_cond_FromString(
	char *condition_type) {
...
OpenAPI_dccf_cond_t *OpenAPI_dccf_cond_create(
	...
	OpenAPI_dccf_cond_t *dccf_cond_local_var =
		ogs_malloc(sizeof(OpenAPI_dccf_cond_t));
...
cJSON *OpenAPI_dccf_cond_convertToJSON(OpenAPI_dccf_cond_t *dccf_cond) {
	...
	if(dccf_cond == NULL) {
		ogs_error(
			"OpenAPI_dccf_cond_convertToJSON() failed [DccfCond]");
		return NULL;
	}
	...
	if(dccf_cond->condition_type == OpenAPI_dccf_cond_CONDITIONTYPE_NULL) {
		ogs_error(
			"OpenAPI_dccf_cond_convertToJSON() failed [condition_type]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "conditionType",
	                           OpenAPI_condition_typedccf_cond_ToString(
					   dccf_cond->condition_type)) == NULL)
	{
		ogs_error(
			"OpenAPI_dccf_cond_convertToJSON() failed [condition_type]");
		goto end;
	...
	if(dccf_cond->tai_list) {
		...
		if(tai_listList == NULL) {
			ogs_error(
				"OpenAPI_dccf_cond_convertToJSON() failed [tai_list]");
			goto end;
		}
		OpenAPI_list_for_each(dccf_cond->tai_list, node) {
			cJSON *itemLocal =
	...
	if(dccf_cond->tai_range_list) {
		...
		if(tai_range_listList == NULL) {
			ogs_error(
				"OpenAPI_dccf_cond_convertToJSON() failed [tai_range_list]");
			goto end;
		}
		OpenAPI_list_for_each(dccf_cond->tai_range_list, node) {
			cJSON *itemLocal =
	...
	if(dccf_cond->serving_nf_type_list != OpenAPI_nf_type_NULL) {
		...
		if(serving_nf_type_listList == NULL) {
			ogs_error(
				"OpenAPI_dccf_cond_convertToJSON() failed [serving_nf_type_list]");
			goto end;
		}
	...
	if(dccf_cond->serving_nf_set_id_list) {
		...
		if(serving_nf_set_id_listList == NULL) {
			ogs_error(
				"OpenAPI_dccf_cond_convertToJSON() failed [serving_nf_set_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(dccf_cond->serving_nf_set_id_list, node) {
...
OpenAPI_dccf_cond_t *OpenAPI_dccf_cond_parseFromJSON(cJSON *dccf_condJSON) {
	...
	condition_type = cJSON_GetObjectItemCaseSensitive(dccf_condJSON,
	                                                  "conditionType");
	if(!condition_type) {
		ogs_error(
			"OpenAPI_dccf_cond_parseFromJSON() failed [condition_type]");
		goto end;
	}
	...
	condition_typeVariable =
		OpenAPI_condition_typedccf_cond_FromString(
	...
	if(tai_list) {
		...
		if(!cJSON_IsArray(tai_list)) {
			ogs_error(
				"OpenAPI_dccf_cond_parseFromJSON() failed [tai_list]");
			goto end;
		}
	...
	tai_range_list = cJSON_GetObjectItemCaseSensitive(dccf_condJSON,
	                                                  "taiRangeList");
	if(tai_range_list) {
		cJSON *tai_range_list_local = NULL;
		if(!cJSON_IsArray(tai_range_list)) {
			ogs_error(
				"OpenAPI_dccf_cond_parseFromJSON() failed [tai_range_list]");
			goto end;
		}
	...
	serving_nf_type_list = cJSON_GetObjectItemCaseSensitive(dccf_condJSON,
	                                                        "servingNfTypeList");
	if(serving_nf_type_list) {
		cJSON *serving_nf_type_list_local = NULL;
		if(!cJSON_IsArray(serving_nf_type_list)) {
			ogs_error(
				"OpenAPI_dccf_cond_parseFromJSON() failed [serving_nf_type_list]");
			goto end;
		}
		...
		if(serving_nf_type_listList->count == 0) {
			ogs_error(
				"OpenAPI_dccf_cond_parseFromJSON() failed: Expected serving_nf_type_listList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}

	serving_nf_set_id_list = cJSON_GetObjectItemCaseSensitive(dccf_condJSON,
	                                                          "servingNfSetIdList");
	if(serving_nf_set_id_list) {
		cJSON *serving_nf_set_id_list_local = NULL;
		if(!cJSON_IsArray(serving_nf_set_id_list)) {
			ogs_error(
				"OpenAPI_dccf_cond_parseFromJSON() failed [serving_nf_set_id_list]");
			goto end;
		}
...
OpenAPI_dccf_cond_t *OpenAPI_dccf_cond_copy(OpenAPI_dccf_cond_t *dst,
                                            OpenAPI_dccf_cond_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
