...
OpenAPI_temporal_validity_t *OpenAPI_temporal_validity_create(
	...
	char	  *
	stop_time) {
	OpenAPI_temporal_validity_t *temporal_validity_local_var =
		ogs_malloc(sizeof(OpenAPI_temporal_validity_t));
...
cJSON *OpenAPI_temporal_validity_convertToJSON(
	...
	if(temporal_validity == NULL) {
		ogs_error(
			"OpenAPI_temporal_validity_convertToJSON() failed [TemporalValidity]");
		return NULL;
	}
	...
	if(temporal_validity->start_time) {
		if(cJSON_AddStringToObject(item, "startTime",
		                           temporal_validity->start_time) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_temporal_validity_convertToJSON() failed [start_time]");
			goto end;
		}
	}
	...
	if(temporal_validity->stop_time) {
		if(cJSON_AddStringToObject(item, "stopTime",
		                           temporal_validity->stop_time) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_temporal_validity_convertToJSON() failed [stop_time]");
			goto end;
		}
	}
...
OpenAPI_temporal_validity_t *OpenAPI_temporal_validity_parseFromJSON(
	...
	start_time = cJSON_GetObjectItemCaseSensitive(temporal_validityJSON,
	                                              "startTime");
	if(start_time) {
		...
		{
			ogs_error(
				"OpenAPI_temporal_validity_parseFromJSON() failed [start_time]");
			goto end;
		}
	}

	stop_time = cJSON_GetObjectItemCaseSensitive(temporal_validityJSON,
	                                             "stopTime");
	if(stop_time) {
		...
		{
			ogs_error(
				"OpenAPI_temporal_validity_parseFromJSON() failed [stop_time]");
			goto end;
		}
	}
	...
	temporal_validity_local_var = OpenAPI_temporal_validity_create(
		...
		stop_time &&
		!cJSON_IsNull(stop_time) ? ogs_strdup(stop_time->valuestring) :
		NULL
		);
