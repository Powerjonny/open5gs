...
OpenAPI_release_data_t *OpenAPI_release_data_create(OpenAPI_cause_e cause,
                                                    ...
                                                    OpenAPI_n4_information_t *
                                                    n4_info_ext2) {
	OpenAPI_release_data_t *release_data_local_var =
		ogs_malloc(sizeof(OpenAPI_release_data_t));
	...
	release_data_local_var->secondary_rat_usage_report =
		secondary_rat_usage_report;
	release_data_local_var->secondary_rat_usage_info =
		secondary_rat_usage_info;
...
void OpenAPI_release_data_free(OpenAPI_release_data_t *release_data) {
	...
	if(release_data->secondary_rat_usage_report) {
		OpenAPI_list_for_each(release_data->secondary_rat_usage_report,
		                      node) {
	...
	if(release_data->secondary_rat_usage_info) {
		OpenAPI_list_for_each(release_data->secondary_rat_usage_info,
		                      node) {
...
cJSON *OpenAPI_release_data_convertToJSON(
	OpenAPI_release_data_t *release_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(release_data == NULL) {
		ogs_error(
			"OpenAPI_release_data_convertToJSON() failed [ReleaseData]");
		return NULL;
	}
	...
	if(release_data->cause != OpenAPI_cause_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_release_data_convertToJSON() failed [cause]");
			goto end;
		}
	}
	...
	if(release_data->ng_ap_cause) {
		cJSON *ng_ap_cause_local_JSON =
			OpenAPI_ng_ap_cause_convertToJSON(
				release_data->ng_ap_cause);
		if(ng_ap_cause_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_release_data_convertToJSON() failed [ng_ap_cause]");
			goto end;
		}
	...
	if(release_data->is__5g_mm_cause_value) {
		if(cJSON_AddNumberToObject(item, "5gMmCauseValue",
		                           release_data->_5g_mm_cause_value) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_release_data_convertToJSON() failed [_5g_mm_cause_value]");
			goto end;
		}
	}
	...
	if(release_data->ue_location) {
		cJSON *ue_location_local_JSON =
			OpenAPI_user_location_convertToJSON(
				release_data->ue_location);
		if(ue_location_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_release_data_convertToJSON() failed [ue_location]");
			goto end;
		}
	...
	if(release_data->ue_time_zone) {
		...
		{
			ogs_error(
				"OpenAPI_release_data_convertToJSON() failed [ue_time_zone]");
			goto end;
		}
	}
	...
	if(release_data->add_ue_location) {
		cJSON *add_ue_location_local_JSON =
			OpenAPI_user_location_convertToJSON(
				release_data->add_ue_location);
		if(add_ue_location_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_release_data_convertToJSON() failed [add_ue_location]");
			goto end;
		}
	...
	if(release_data->secondary_rat_usage_report) {
		...
		if(secondary_rat_usage_reportList == NULL) {
			ogs_error(
				"OpenAPI_release_data_convertToJSON() failed [secondary_rat_usage_report]");
			goto end;
		}
		OpenAPI_list_for_each(release_data->secondary_rat_usage_report,
		                      node) {
	...
	if(release_data->secondary_rat_usage_info) {
		...
		if(secondary_rat_usage_infoList == NULL) {
			ogs_error(
				"OpenAPI_release_data_convertToJSON() failed [secondary_rat_usage_info]");
			goto end;
		}
		OpenAPI_list_for_each(release_data->secondary_rat_usage_info,
		                      node) {
	...
	if(release_data->n4_info) {
		cJSON *n4_info_local_JSON =
			OpenAPI_n4_information_convertToJSON(
				release_data->n4_info);
		if(n4_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_release_data_convertToJSON() failed [n4_info]");
			goto end;
		}
	...
	if(release_data->n4_info_ext1) {
		cJSON *n4_info_ext1_local_JSON =
			OpenAPI_n4_information_convertToJSON(
				release_data->n4_info_ext1);
		if(n4_info_ext1_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_release_data_convertToJSON() failed [n4_info_ext1]");
			goto end;
		}
	...
	if(release_data->n4_info_ext2) {
		cJSON *n4_info_ext2_local_JSON =
			OpenAPI_n4_information_convertToJSON(
				release_data->n4_info_ext2);
		if(n4_info_ext2_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_release_data_convertToJSON() failed [n4_info_ext2]");
			goto end;
		}
...
OpenAPI_release_data_t *OpenAPI_release_data_parseFromJSON(
	cJSON *release_dataJSON) {
	...
	if(cause) {
		if(!cJSON_IsString(cause)) {
			ogs_error(
				"OpenAPI_release_data_parseFromJSON() failed [cause]");
			goto end;
		}
		causeVariable = OpenAPI_cause_FromString(cause->valuestring);
	}

	ng_ap_cause = cJSON_GetObjectItemCaseSensitive(release_dataJSON,
	                                               "ngApCause");
	if(ng_ap_cause) {
	...
	_5g_mm_cause_value = cJSON_GetObjectItemCaseSensitive(release_dataJSON,
	                                                      "5gMmCauseValue");
	if(_5g_mm_cause_value) {
		if(!cJSON_IsNumber(_5g_mm_cause_value)) {
			ogs_error(
				"OpenAPI_release_data_parseFromJSON() failed [_5g_mm_cause_value]");
			goto end;
		}
	}

	ue_location = cJSON_GetObjectItemCaseSensitive(release_dataJSON,
	                                               "ueLocation");
	if(ue_location) {
	...
	ue_time_zone = cJSON_GetObjectItemCaseSensitive(release_dataJSON,
	                                                "ueTimeZone");
	if(ue_time_zone) {
		...
		{
			ogs_error(
				"OpenAPI_release_data_parseFromJSON() failed [ue_time_zone]");
			goto end;
		}
	}

	add_ue_location = cJSON_GetObjectItemCaseSensitive(release_dataJSON,
	                                                   "addUeLocation");
	if(add_ue_location) {
	...
	secondary_rat_usage_report =
		cJSON_GetObjectItemCaseSensitive(release_dataJSON,
	...
	if(secondary_rat_usage_report) {
		...
		if(!cJSON_IsArray(secondary_rat_usage_report)) {
			ogs_error(
				"OpenAPI_release_data_parseFromJSON() failed [secondary_rat_usage_report]");
			goto end;
		}
	...
	if(secondary_rat_usage_info) {
		...
		if(!cJSON_IsArray(secondary_rat_usage_info)) {
			ogs_error(
				"OpenAPI_release_data_parseFromJSON() failed [secondary_rat_usage_info]");
			goto end;
		}
	...
	n4_info_ext1 = cJSON_GetObjectItemCaseSensitive(release_dataJSON,
	                                                "n4InfoExt1");
	if(n4_info_ext1) {
	...
	n4_info_ext2 = cJSON_GetObjectItemCaseSensitive(release_dataJSON,
	                                                "n4InfoExt2");
	if(n4_info_ext2) {
...
OpenAPI_release_data_t *OpenAPI_release_data_copy(
	OpenAPI_release_data_t	*dst,
