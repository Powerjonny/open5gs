...
OpenAPI_bdt_policy_data_patch_t *OpenAPI_bdt_policy_data_patch_create(
	char *bdt_ref_id) {
	OpenAPI_bdt_policy_data_patch_t *bdt_policy_data_patch_local_var =
		ogs_malloc(sizeof(OpenAPI_bdt_policy_data_patch_t));
...
cJSON *OpenAPI_bdt_policy_data_patch_convertToJSON(
	...
	if(bdt_policy_data_patch == NULL) {
		ogs_error(
			"OpenAPI_bdt_policy_data_patch_convertToJSON() failed [BdtPolicyDataPatch]");
		return NULL;
	}
	...
	if(!bdt_policy_data_patch->bdt_ref_id) {
		ogs_error(
			"OpenAPI_bdt_policy_data_patch_convertToJSON() failed [bdt_ref_id]");
		return NULL;
	}
...
OpenAPI_bdt_policy_data_patch_t *OpenAPI_bdt_policy_data_patch_parseFromJSON(
	...
	bdt_ref_id = cJSON_GetObjectItemCaseSensitive(bdt_policy_data_patchJSON,
	                                              "bdtRefId");
	if(!bdt_ref_id) {
		ogs_error(
			"OpenAPI_bdt_policy_data_patch_parseFromJSON() failed [bdt_ref_id]");
		goto end;
	}
...
OpenAPI_bdt_policy_data_patch_t *OpenAPI_bdt_policy_data_patch_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_bdt_policy_data_patch_convertToJSON() failed");
		return NULL;
	}
