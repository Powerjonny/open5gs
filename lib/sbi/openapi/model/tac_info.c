...
OpenAPI_tac_info_t *OpenAPI_tac_info_create(OpenAPI_list_t *tac_list) {
	OpenAPI_tac_info_t *tac_info_local_var =
		ogs_malloc(sizeof(OpenAPI_tac_info_t));
...
cJSON *OpenAPI_tac_info_convertToJSON(OpenAPI_tac_info_t *tac_info) {
	...
	OpenAPI_list_for_each(tac_info->tac_list, node) {
		...
		{
			ogs_error(
				"OpenAPI_tac_info_convertToJSON() failed [tac_list]");
			goto end;
		}
	}
...
OpenAPI_tac_info_t *OpenAPI_tac_info_parseFromJSON(cJSON *tac_infoJSON) {
	...
	cJSON_ArrayForEach(tac_list_local, tac_list) {
		...
		if(!cJSON_IsString(tac_list_local)) {
			ogs_error(
				"OpenAPI_tac_info_parseFromJSON() failed [tac_list]");
			goto end;
		}
...
OpenAPI_tac_info_t *OpenAPI_tac_info_copy(OpenAPI_tac_info_t	*dst,
                                          OpenAPI_tac_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
