...
OpenAPI_aerial_ue_subscription_info_create(
	...
	OpenAPI_aerial_ue_subscription_info_t *
	        aerial_ue_subscription_info_local_var =
		ogs_malloc(sizeof(OpenAPI_aerial_ue_subscription_info_t));
...
cJSON *OpenAPI_aerial_ue_subscription_info_convertToJSON(
	...
	if(aerial_ue_subscription_info == NULL) {
		ogs_error(
			"OpenAPI_aerial_ue_subscription_info_convertToJSON() failed [AerialUeSubscriptionInfo]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(aerial_ue_subscription_info->aerial_ue_ind ==
	   OpenAPI_aerial_ue_indication_NULL)
	{
		ogs_error(
			"OpenAPI_aerial_ue_subscription_info_convertToJSON() failed [aerial_ue_ind]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "aerialUeInd",
	                           OpenAPI_aerial_ue_indication_ToString(
					   aerial_ue_subscription_info->
					   aerial_ue_ind)) == NULL)
	{
		ogs_error(
			"OpenAPI_aerial_ue_subscription_info_convertToJSON() failed [aerial_ue_ind]");
	...
	if(aerial_ue_subscription_info->_3gpp_uav_id) {
		...
		{
			ogs_error(
				"OpenAPI_aerial_ue_subscription_info_convertToJSON() failed [_3gpp_uav_id]");
			goto end;
		}
	}
...
OpenAPI_aerial_ue_subscription_info_parseFromJSON(
	...
	OpenAPI_aerial_ue_subscription_info_t *
	        aerial_ue_subscription_info_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	aerial_ue_ind =
		cJSON_GetObjectItemCaseSensitive(
			aerial_ue_subscription_infoJSON, "aerialUeInd");
	if(!aerial_ue_ind) {
		ogs_error(
			"OpenAPI_aerial_ue_subscription_info_parseFromJSON() failed [aerial_ue_ind]");
		goto end;
	}
	...
	_3gpp_uav_id =
		cJSON_GetObjectItemCaseSensitive(
			aerial_ue_subscription_infoJSON, "3gppUavId");
	if(_3gpp_uav_id) {
		if(!cJSON_IsString(_3gpp_uav_id) &&
		   !cJSON_IsNull(_3gpp_uav_id))
		{
			ogs_error(
				"OpenAPI_aerial_ue_subscription_info_parseFromJSON() failed [_3gpp_uav_id]");
			goto end;
		}
	}
...
OpenAPI_aerial_ue_subscription_info_t *OpenAPI_aerial_ue_subscription_info_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_aerial_ue_subscription_info_convertToJSON() failed");
		return NULL;
	}
