...
OpenAPI_dn_perf_t *OpenAPI_dn_perf_create(
	...
	OpenAPI_upf_information_t *upf_info, char *dnai,
	OpenAPI_perf_data_t *perf_data,
	OpenAPI_network_area_info_t *spatial_valid_con,
	OpenAPI_time_window_t *temporal_valid_con) {
	OpenAPI_dn_perf_t *dn_perf_local_var =
		ogs_malloc(sizeof(OpenAPI_dn_perf_t));
...
cJSON *OpenAPI_dn_perf_convertToJSON(OpenAPI_dn_perf_t *dn_perf) {
	...
	if(dn_perf->upf_info) {
		...
		if(upf_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_dn_perf_convertToJSON() failed [upf_info]");
			goto end;
		}
	...
	if(dn_perf->dnai) {
		...
		{
			ogs_error(
				"OpenAPI_dn_perf_convertToJSON() failed [dnai]");
			goto end;
		}
	}
	...
	cJSON *perf_data_local_JSON =
		OpenAPI_perf_data_convertToJSON(dn_perf->perf_data);
	...
	if(dn_perf->spatial_valid_con) {
		...
		if(spatial_valid_con_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_dn_perf_convertToJSON() failed [spatial_valid_con]");
			goto end;
		}
	...
	if(dn_perf->temporal_valid_con) {
		cJSON *temporal_valid_con_local_JSON =
			OpenAPI_time_window_convertToJSON(
				dn_perf->temporal_valid_con);
		if(temporal_valid_con_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_dn_perf_convertToJSON() failed [temporal_valid_con]");
			goto end;
		}
...
OpenAPI_dn_perf_t *OpenAPI_dn_perf_parseFromJSON(cJSON *dn_perfJSON) {
	...
	if(app_server_ins_addr) {
		app_server_ins_addr_local_nonprim =
			OpenAPI_addr_fqdn_parseFromJSON(app_server_ins_addr);
		if(!app_server_ins_addr_local_nonprim) {
			ogs_error(
				"OpenAPI_addr_fqdn_parseFromJSON failed [app_server_ins_addr]");
			goto end;
		}
	}
	...
	if(dnai) {
		...
		{
			ogs_error(
				"OpenAPI_dn_perf_parseFromJSON() failed [dnai]");
			goto end;
		}
	}
	...
	if(spatial_valid_con) {
		...
		if(!spatial_valid_con_local_nonprim) {
			ogs_error(
				"OpenAPI_network_area_info_parseFromJSON failed [spatial_valid_con]");
			goto end;
		}
	}
	...
	if(temporal_valid_con) {
		temporal_valid_con_local_nonprim =
			OpenAPI_time_window_parseFromJSON(temporal_valid_con);
		if(!temporal_valid_con_local_nonprim) {
			ogs_error(
				"OpenAPI_time_window_parseFromJSON failed [temporal_valid_con]");
			goto end;
		}
	}
	...
	dn_perf_local_var = OpenAPI_dn_perf_create(
		...
		dnai &&
		!cJSON_IsNull(dnai) ? ogs_strdup(dnai->valuestring) : NULL,
...
OpenAPI_dn_perf_t *OpenAPI_dn_perf_copy(OpenAPI_dn_perf_t	*dst,
                                        OpenAPI_dn_perf_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
