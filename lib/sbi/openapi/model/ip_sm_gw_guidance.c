...
OpenAPI_ip_sm_gw_guidance_t *OpenAPI_ip_sm_gw_guidance_create(
	...
	OpenAPI_ip_sm_gw_guidance_t *ip_sm_gw_guidance_local_var =
		ogs_malloc(sizeof(OpenAPI_ip_sm_gw_guidance_t));
	ogs_assert(ip_sm_gw_guidance_local_var);

	ip_sm_gw_guidance_local_var->min_delivery_time = min_delivery_time;
	ip_sm_gw_guidance_local_var->recomm_delivery_time =
...
cJSON *OpenAPI_ip_sm_gw_guidance_convertToJSON(
	...
	if(ip_sm_gw_guidance == NULL) {
		ogs_error(
			"OpenAPI_ip_sm_gw_guidance_convertToJSON() failed [IpSmGwGuidance]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(cJSON_AddNumberToObject(item, "minDeliveryTime",
	                           ip_sm_gw_guidance->min_delivery_time) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_ip_sm_gw_guidance_convertToJSON() failed [min_delivery_time]");
		goto end;
	}

	if(cJSON_AddNumberToObject(item, "recommDeliveryTime",
	                           ip_sm_gw_guidance->recomm_delivery_time) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_ip_sm_gw_guidance_convertToJSON() failed [recomm_delivery_time]");
		goto end;
	}
...
OpenAPI_ip_sm_gw_guidance_t *OpenAPI_ip_sm_gw_guidance_parseFromJSON(
	...
	min_delivery_time =
		cJSON_GetObjectItemCaseSensitive(ip_sm_gw_guidanceJSON,
		                                 "minDeliveryTime");
	if(!min_delivery_time) {
		ogs_error(
			"OpenAPI_ip_sm_gw_guidance_parseFromJSON() failed [min_delivery_time]");
		goto end;
	}
	...
	if(!recomm_delivery_time) {
		ogs_error(
			"OpenAPI_ip_sm_gw_guidance_parseFromJSON() failed [recomm_delivery_time]");
		goto end;
	}
