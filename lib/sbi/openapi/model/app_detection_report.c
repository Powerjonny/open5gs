...
OpenAPI_app_detection_report_t *OpenAPI_app_detection_report_create(
	...
	OpenAPI_app_detection_report_t *app_detection_report_local_var =
		ogs_malloc(sizeof(OpenAPI_app_detection_report_t));
...
cJSON *OpenAPI_app_detection_report_convertToJSON(
	...
	if(app_detection_report == NULL) {
		ogs_error(
			"OpenAPI_app_detection_report_convertToJSON() failed [AppDetectionReport]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(app_detection_report->ad_notif_type ==
	   OpenAPI_app_detection_notif_type_NULL)
	{
		ogs_error(
			"OpenAPI_app_detection_report_convertToJSON() failed [ad_notif_type]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "adNotifType",
	                           OpenAPI_app_detection_notif_type_ToString(
					   app_detection_report->ad_notif_type))
	   == NULL)
	{
		ogs_error(
			"OpenAPI_app_detection_report_convertToJSON() failed [ad_notif_type]");
	...
	if(!app_detection_report->af_app_id) {
		ogs_error(
			"OpenAPI_app_detection_report_convertToJSON() failed [af_app_id]");
		return NULL;
	}
...
OpenAPI_app_detection_report_t *OpenAPI_app_detection_report_parseFromJSON(
	...
	ad_notif_type =
		cJSON_GetObjectItemCaseSensitive(app_detection_reportJSON,
		                                 "adNotifType");
	if(!ad_notif_type) {
		ogs_error(
			"OpenAPI_app_detection_report_parseFromJSON() failed [ad_notif_type]");
		goto end;
	}
	...
	ad_notif_typeVariable =
		OpenAPI_app_detection_notif_type_FromString(
			ad_notif_type->valuestring);

	af_app_id = cJSON_GetObjectItemCaseSensitive(app_detection_reportJSON,
	                                             "afAppId");
	if(!af_app_id) {
		ogs_error(
			"OpenAPI_app_detection_report_parseFromJSON() failed [af_app_id]");
		goto end;
	}
...
OpenAPI_app_detection_report_t *OpenAPI_app_detection_report_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_app_detection_report_convertToJSON() failed");
		return NULL;
	}
