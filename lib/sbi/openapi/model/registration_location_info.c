...
OpenAPI_registration_location_info_t *OpenAPI_registration_location_info_create(
	char *amf_instance_id, OpenAPI_guami_t *guami,
	OpenAPI_plmn_id_t *plmn_id, OpenAPI_vgmlc_address_t *vgmlc_address,
	...
	registration_location_info_local_var->access_type_list =
		access_type_list;

	return registration_location_info_local_var;
}

void OpenAPI_registration_location_info_free(
	OpenAPI_registration_location_info_t *registration_location_info) {
	...
	if(registration_location_info->vgmlc_address) {
		OpenAPI_vgmlc_address_free(
			registration_location_info->vgmlc_address);
...
cJSON *OpenAPI_registration_location_info_convertToJSON(
	...
	if(registration_location_info == NULL) {
		ogs_error(
			"OpenAPI_registration_location_info_convertToJSON() failed [RegistrationLocationInfo]");
		return NULL;
	}
	...
	if(!registration_location_info->amf_instance_id) {
		ogs_error(
			"OpenAPI_registration_location_info_convertToJSON() failed [amf_instance_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "amfInstanceId",
	                           registration_location_info->amf_instance_id)
	   == NULL)
	{
		ogs_error(
			"OpenAPI_registration_location_info_convertToJSON() failed [amf_instance_id]");
	...
	if(registration_location_info->guami) {
		cJSON *guami_local_JSON =
			OpenAPI_guami_convertToJSON(
				registration_location_info->guami);
		if(guami_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_registration_location_info_convertToJSON() failed [guami]");
			goto end;
		}
	...
	if(registration_location_info->plmn_id) {
		cJSON *plmn_id_local_JSON =
			OpenAPI_plmn_id_convertToJSON(
				registration_location_info->plmn_id);
		if(plmn_id_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_registration_location_info_convertToJSON() failed [plmn_id]");
			goto end;
		}
	...
	if(registration_location_info->vgmlc_address) {
		...
		if(vgmlc_address_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_registration_location_info_convertToJSON() failed [vgmlc_address]");
			goto end;
		}
	...
	{
		ogs_error(
			"OpenAPI_registration_location_info_convertToJSON() failed [access_type_list]");
		return NULL;
	}
	cJSON *access_type_listList = cJSON_AddArrayToObject(item,
	                                                     "accessTypeList");
	...
	OpenAPI_list_for_each(registration_location_info->access_type_list,
	                      node) {
		if(cJSON_AddStringToObject(access_type_listList, "",
...
OpenAPI_registration_location_info_t *
OpenAPI_registration_location_info_parseFromJSON(
	cJSON *registration_location_infoJSON) {
	OpenAPI_registration_location_info_t *
	        registration_location_info_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	amf_instance_id =
		cJSON_GetObjectItemCaseSensitive(registration_location_infoJSON,
		                                 "amfInstanceId");
	if(!amf_instance_id) {
		ogs_error(
			"OpenAPI_registration_location_info_parseFromJSON() failed [amf_instance_id]");
		goto end;
	}
	...
	guami = cJSON_GetObjectItemCaseSensitive(registration_location_infoJSON,
	                                         "guami");
	if(guami) {
	...
	if(!access_type_list) {
		ogs_error(
			"OpenAPI_registration_location_info_parseFromJSON() failed [access_type_list]");
		goto end;
	}
	...
	cJSON_ArrayForEach(access_type_list_local, access_type_list) {
		...
		if(!localEnum) {
			ogs_info(
				"Enum value \"%s\" for field \"access_type_list\" is not supported. Ignoring it ...",
				access_type_list_local->valuestring);
		} else {
			OpenAPI_list_add(access_type_listList,
			                 (void *) localEnum);
		}
	}
	if(access_type_listList->count == 0) {
		ogs_error(
			"OpenAPI_registration_location_info_parseFromJSON() failed: Expected access_type_listList to not be empty (after ignoring unsupported enum values).");
		goto end;
	}
...
OpenAPI_registration_location_info_t *OpenAPI_registration_location_info_copy(
	OpenAPI_registration_location_info_t	*dst,
	OpenAPI_registration_location_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_registration_location_info_convertToJSON() failed");
		return NULL;
	}
