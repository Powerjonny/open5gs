...
OpenAPI_local3d_point_uncertainty_ellipsoid_create(
	OpenAPI_supported_gad_shapes_e shape,
	OpenAPI_local_origin_t *local_origin,
	OpenAPI_relative_cartesian_location_t *point,
	OpenAPI_uncertainty_ellipsoid_t *uncertainty_ellipsoid, int confidence,
	bool is_v_confidence, int v_confidence) {
	OpenAPI_local3d_point_uncertainty_ellipsoid_t *
	        local3d_point_uncertainty_ellipsoid_local_var =
		ogs_malloc(
			sizeof(OpenAPI_local3d_point_uncertainty_ellipsoid_t));
	...
	local3d_point_uncertainty_ellipsoid_local_var->is_v_confidence =
		is_v_confidence;
	local3d_point_uncertainty_ellipsoid_local_var->v_confidence =
		v_confidence;

	return local3d_point_uncertainty_ellipsoid_local_var;
}

void OpenAPI_local3d_point_uncertainty_ellipsoid_free(
	OpenAPI_local3d_point_uncertainty_ellipsoid_t *
	...
	if(local3d_point_uncertainty_ellipsoid->point) {
		OpenAPI_relative_cartesian_location_free(
...
cJSON *OpenAPI_local3d_point_uncertainty_ellipsoid_convertToJSON(
	OpenAPI_local3d_point_uncertainty_ellipsoid_t *
	local3d_point_uncertainty_ellipsoid) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(local3d_point_uncertainty_ellipsoid == NULL) {
		ogs_error(
			"OpenAPI_local3d_point_uncertainty_ellipsoid_convertToJSON() failed [Local3dPointUncertaintyEllipsoid]");
		return NULL;
	}
	...
	{
		ogs_error(
			"OpenAPI_local3d_point_uncertainty_ellipsoid_convertToJSON() failed [shape]");
		return NULL;
	}
	...
	if(!local3d_point_uncertainty_ellipsoid->local_origin) {
		ogs_error(
			"OpenAPI_local3d_point_uncertainty_ellipsoid_convertToJSON() failed [local_origin]");
		return NULL;
	}
	cJSON *local_origin_local_JSON =
		OpenAPI_local_origin_convertToJSON(
			local3d_point_uncertainty_ellipsoid->local_origin);
	...
	if(!local3d_point_uncertainty_ellipsoid->point) {
		ogs_error(
			"OpenAPI_local3d_point_uncertainty_ellipsoid_convertToJSON() failed [point]");
		return NULL;
	}
	cJSON *point_local_JSON =
		OpenAPI_relative_cartesian_location_convertToJSON(
			local3d_point_uncertainty_ellipsoid->point);
	...
	if(!local3d_point_uncertainty_ellipsoid->uncertainty_ellipsoid) {
		ogs_error(
			"OpenAPI_local3d_point_uncertainty_ellipsoid_convertToJSON() failed [uncertainty_ellipsoid]");
		return NULL;
	}
	cJSON *uncertainty_ellipsoid_local_JSON =
		OpenAPI_uncertainty_ellipsoid_convertToJSON(
			local3d_point_uncertainty_ellipsoid->
			uncertainty_ellipsoid);
	...
	{
		ogs_error(
			"OpenAPI_local3d_point_uncertainty_ellipsoid_convertToJSON() failed [confidence]");
		goto end;
	}
	...
	if(local3d_point_uncertainty_ellipsoid->is_v_confidence) {
		...
		{
			ogs_error(
				"OpenAPI_local3d_point_uncertainty_ellipsoid_convertToJSON() failed [v_confidence]");
			goto end;
		}
	}
...
OpenAPI_local3d_point_uncertainty_ellipsoid_t *
OpenAPI_local3d_point_uncertainty_ellipsoid_parseFromJSON(
	...
	OpenAPI_uncertainty_ellipsoid_t *uncertainty_ellipsoid_local_nonprim =
		NULL;
	...
	if(!shape) {
		ogs_error(
			"OpenAPI_local3d_point_uncertainty_ellipsoid_parseFromJSON() failed [shape]");
		goto end;
	}
	...
	shapeVariable =
		OpenAPI_supported_gad_shapes_FromString(shape->valuestring);
	...
	if(!local_origin) {
		ogs_error(
			"OpenAPI_local3d_point_uncertainty_ellipsoid_parseFromJSON() failed [local_origin]");
		goto end;
	}
	local_origin_local_nonprim =
		OpenAPI_local_origin_parseFromJSON(local_origin);
	if(!local_origin_local_nonprim) {
		ogs_error(
			"OpenAPI_local_origin_parseFromJSON failed [local_origin]");
		goto end;
	}
	...
	if(!point) {
		ogs_error(
			"OpenAPI_local3d_point_uncertainty_ellipsoid_parseFromJSON() failed [point]");
		goto end;
	}
	point_local_nonprim =
		OpenAPI_relative_cartesian_location_parseFromJSON(point);
	if(!point_local_nonprim) {
		ogs_error(
			"OpenAPI_relative_cartesian_location_parseFromJSON failed [point]");
		goto end;
	}
	...
	if(!uncertainty_ellipsoid) {
		ogs_error(
			"OpenAPI_local3d_point_uncertainty_ellipsoid_parseFromJSON() failed [uncertainty_ellipsoid]");
		goto end;
	}
	uncertainty_ellipsoid_local_nonprim =
		OpenAPI_uncertainty_ellipsoid_parseFromJSON(
			uncertainty_ellipsoid);
	if(!uncertainty_ellipsoid_local_nonprim) {
		ogs_error(
			"OpenAPI_uncertainty_ellipsoid_parseFromJSON failed [uncertainty_ellipsoid]");
		goto end;
	}
	...
	if(!confidence) {
		ogs_error(
			"OpenAPI_local3d_point_uncertainty_ellipsoid_parseFromJSON() failed [confidence]");
		goto end;
	}
	...
	v_confidence =
		cJSON_GetObjectItemCaseSensitive(
			local3d_point_uncertainty_ellipsoidJSON, "vConfidence");
	if(v_confidence) {
		if(!cJSON_IsNumber(v_confidence)) {
			ogs_error(
				"OpenAPI_local3d_point_uncertainty_ellipsoid_parseFromJSON() failed [v_confidence]");
			goto end;
		}
	}
...
OpenAPI_local3d_point_uncertainty_ellipsoid_copy(
	OpenAPI_local3d_point_uncertainty_ellipsoid_t	*dst,
	OpenAPI_local3d_point_uncertainty_ellipsoid_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_local3d_point_uncertainty_ellipsoid_convertToJSON() failed");
		return NULL;
	}
