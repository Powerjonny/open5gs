...
OpenAPI_ursp_rule_request_t *OpenAPI_ursp_rule_request_create(
	...
	bool is_relat_precedence, int relat_precedence,
	OpenAPI_list_t *route_sel_param_sets) {
	OpenAPI_ursp_rule_request_t *ursp_rule_request_local_var =
		ogs_malloc(sizeof(OpenAPI_ursp_rule_request_t));
	...
	ursp_rule_request_local_var->route_sel_param_sets =
		route_sel_param_sets;

	return ursp_rule_request_local_var;
}

void OpenAPI_ursp_rule_request_free(
	OpenAPI_ursp_rule_request_t *ursp_rule_request) {
	...
	if(ursp_rule_request->traffic_desc) {
		OpenAPI_traffic_descriptor_components_free(
			ursp_rule_request->traffic_desc);
		ursp_rule_request->traffic_desc = NULL;
	}
	if(ursp_rule_request->route_sel_param_sets) {
		OpenAPI_list_for_each(ursp_rule_request->route_sel_param_sets,
		                      node) {
...
cJSON *OpenAPI_ursp_rule_request_convertToJSON(
	OpenAPI_ursp_rule_request_t *ursp_rule_request) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(ursp_rule_request == NULL) {
		ogs_error(
			"OpenAPI_ursp_rule_request_convertToJSON() failed [UrspRuleRequest]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(ursp_rule_request->traffic_desc) {
		cJSON *traffic_desc_local_JSON =
			OpenAPI_traffic_descriptor_components_convertToJSON(
				ursp_rule_request->traffic_desc);
		if(traffic_desc_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ursp_rule_request_convertToJSON() failed [traffic_desc]");
			goto end;
		}
		cJSON_AddItemToObject(item, "trafficDesc",
		                      traffic_desc_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_ursp_rule_request_convertToJSON() failed [traffic_desc]");
			goto end;
	...
	if(ursp_rule_request->is_relat_precedence) {
		if(cJSON_AddNumberToObject(item, "relatPrecedence",
		                           ursp_rule_request->relat_precedence)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_ursp_rule_request_convertToJSON() failed [relat_precedence]");
			goto end;
		}
	}

	if(ursp_rule_request->route_sel_param_sets) {
		cJSON *route_sel_param_setsList = cJSON_AddArrayToObject(item,
		                                                         "routeSelParamSets");
		if(route_sel_param_setsList == NULL) {
			ogs_error(
				"OpenAPI_ursp_rule_request_convertToJSON() failed [route_sel_param_sets]");
			goto end;
		}
		OpenAPI_list_for_each(ursp_rule_request->route_sel_param_sets,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_route_selection_parameter_set_convertToJSON
				        (node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_ursp_rule_request_convertToJSON() failed [route_sel_param_sets]");
			...
			cJSON_AddItemToArray(route_sel_param_setsList,
			                     itemLocal);
		}
	}
...
OpenAPI_ursp_rule_request_t *OpenAPI_ursp_rule_request_parseFromJSON(
	cJSON *ursp_rule_requestJSON) {
	OpenAPI_ursp_rule_request_t *ursp_rule_request_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *traffic_desc = NULL;
	OpenAPI_traffic_descriptor_components_t *traffic_desc_local_nonprim =
		NULL;
	cJSON *relat_precedence = NULL;
	cJSON *route_sel_param_sets = NULL;
	OpenAPI_list_t *route_sel_param_setsList = NULL;
	traffic_desc = cJSON_GetObjectItemCaseSensitive(ursp_rule_requestJSON,
	                                                "trafficDesc");
	if(traffic_desc) {
		traffic_desc_local_nonprim =
			OpenAPI_traffic_descriptor_components_parseFromJSON(
				traffic_desc);
		if(!traffic_desc_local_nonprim) {
			ogs_error(
				"OpenAPI_traffic_descriptor_components_parseFromJSON failed [traffic_desc]");
			goto end;
		}
	}

	relat_precedence =
		cJSON_GetObjectItemCaseSensitive(ursp_rule_requestJSON,
		                                 "relatPrecedence");
	if(relat_precedence) {
		if(!cJSON_IsNumber(relat_precedence)) {
			ogs_error(
				"OpenAPI_ursp_rule_request_parseFromJSON() failed [relat_precedence]");
			goto end;
		}
	}
	...
	if(route_sel_param_sets) {
		...
		if(!cJSON_IsArray(route_sel_param_sets)) {
			ogs_error(
				"OpenAPI_ursp_rule_request_parseFromJSON() failed [route_sel_param_sets]");
			goto end;
		}

		route_sel_param_setsList = OpenAPI_list_create();

		cJSON_ArrayForEach(route_sel_param_sets_local,
		                   route_sel_param_sets) {
...
OpenAPI_ursp_rule_request_t *OpenAPI_ursp_rule_request_copy(
	OpenAPI_ursp_rule_request_t *dst, OpenAPI_ursp_rule_request_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
