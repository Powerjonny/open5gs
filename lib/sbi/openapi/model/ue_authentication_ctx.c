...
OpenAPI_ue_authentication_ctx_t *OpenAPI_ue_authentication_ctx_create(
	...
	OpenAPI_ue_authentication_ctx_t *ue_authentication_ctx_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_authentication_ctx_t));
	...
	ue_authentication_ctx_local_var->serving_network_name =
...
void OpenAPI_ue_authentication_ctx_free(
	...
	if(ue_authentication_ctx->_5g_auth_data) {
		OpenAPI_ue_authentication_ctx_5g_auth_data_free(
...
cJSON *OpenAPI_ue_authentication_ctx_convertToJSON(
	...
	if(ue_authentication_ctx == NULL) {
		ogs_error(
			"OpenAPI_ue_authentication_ctx_convertToJSON() failed [UEAuthenticationCtx]");
		return NULL;
	}
	...
	if(ue_authentication_ctx->auth_type == OpenAPI_auth_type_NULL) {
		ogs_error(
			"OpenAPI_ue_authentication_ctx_convertToJSON() failed [auth_type]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "authType",
	                           OpenAPI_auth_type_ToString(
					   ue_authentication_ctx->auth_type)) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_ue_authentication_ctx_convertToJSON() failed [auth_type]");
	...
	if(!ue_authentication_ctx->_5g_auth_data) {
		ogs_error(
			"OpenAPI_ue_authentication_ctx_convertToJSON() failed [_5g_auth_data]");
		return NULL;
	}
	cJSON *_5g_auth_data_local_JSON =
	...
	if(!ue_authentication_ctx->_links) {
		ogs_error(
			"OpenAPI_ue_authentication_ctx_convertToJSON() failed [_links]");
		return NULL;
	}
	...
	if(ue_authentication_ctx->_links) {
		OpenAPI_list_for_each(ue_authentication_ctx->_links, node) {
			...
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_ue_authentication_ctx_convertToJSON() failed [inner]");
				goto end;
			}
	...
	if(ue_authentication_ctx->serving_network_name) {
		if(cJSON_AddStringToObject(item, "servingNetworkName",
		                           ue_authentication_ctx->
		                           serving_network_name) == NULL)
		{
			ogs_error(
				"OpenAPI_ue_authentication_ctx_convertToJSON() failed [serving_network_name]");
			goto end;
		}
	}
...
OpenAPI_ue_authentication_ctx_t *OpenAPI_ue_authentication_ctx_parseFromJSON(
	...
	auth_type = cJSON_GetObjectItemCaseSensitive(ue_authentication_ctxJSON,
	                                             "authType");
	if(!auth_type) {
		ogs_error(
			"OpenAPI_ue_authentication_ctx_parseFromJSON() failed [auth_type]");
		goto end;
	}
	...
	_5g_auth_data =
		cJSON_GetObjectItemCaseSensitive(ue_authentication_ctxJSON,
		                                 "5gAuthData");
	if(!_5g_auth_data) {
		ogs_error(
			"OpenAPI_ue_authentication_ctx_parseFromJSON() failed [_5g_auth_data]");
		goto end;
	}
	...
	if(!_5g_auth_data_local_nonprim) {
		ogs_error(
			"OpenAPI_ue_authentication_ctx_5g_auth_data_parseFromJSON failed [_5g_auth_data]");
		goto end;
	}

	_links = cJSON_GetObjectItemCaseSensitive(ue_authentication_ctxJSON,
	                                          "_links");
	if(!_links) {
		ogs_error(
			"OpenAPI_ue_authentication_ctx_parseFromJSON() failed [_links]");
		goto end;
	}
	...
	if(cJSON_IsObject(_links)) {
		...
		cJSON_ArrayForEach(_links_local_map, _links) {
			...
			if(cJSON_IsObject(localMapObject)) {
				...
				ogs_error(
					"OpenAPI_ue_authentication_ctx_parseFromJSON() failed [inner]");
				goto end;
			}
	...
	if(serving_network_name) {
		if(!cJSON_IsString(serving_network_name) &&
		   !cJSON_IsNull(serving_network_name))
		{
			ogs_error(
				"OpenAPI_ue_authentication_ctx_parseFromJSON() failed [serving_network_name]");
			goto end;
		}
	}
	...
	ue_authentication_ctx_local_var = OpenAPI_ue_authentication_ctx_create(
		...
		serving_network_name &&
		!cJSON_IsNull(serving_network_name) ?
		ogs_strdup(serving_network_name->valuestring) : NULL
		);
...
OpenAPI_ue_authentication_ctx_t *OpenAPI_ue_authentication_ctx_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ue_authentication_ctx_convertToJSON() failed");
		return NULL;
	}
