...
OpenAPI_af_coordination_info_t *OpenAPI_af_coordination_info_create(
	...
	OpenAPI_af_coordination_info_t *af_coordination_info_local_var =
		ogs_malloc(sizeof(OpenAPI_af_coordination_info_t));
	ogs_assert(af_coordination_info_local_var);

	af_coordination_info_local_var->source_dnai = source_dnai;
	af_coordination_info_local_var->source_ue_ipv4_addr =
		source_ue_ipv4_addr;
	af_coordination_info_local_var->source_ue_ipv6_prefix =
		source_ue_ipv6_prefix;
	af_coordination_info_local_var->notification_info_list =
...
void OpenAPI_af_coordination_info_free(
	...
	if(af_coordination_info->notification_info_list) {
		OpenAPI_list_for_each(
			af_coordination_info->notification_info_list, node) {
...
cJSON *OpenAPI_af_coordination_info_convertToJSON(
	...
	if(af_coordination_info == NULL) {
		ogs_error(
			"OpenAPI_af_coordination_info_convertToJSON() failed [AfCoordinationInfo]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(af_coordination_info->source_dnai) {
		if(cJSON_AddStringToObject(item, "sourceDnai",
		                           af_coordination_info->source_dnai) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_af_coordination_info_convertToJSON() failed [source_dnai]");
			goto end;
		}
	}

	if(af_coordination_info->source_ue_ipv4_addr) {
		if(cJSON_AddStringToObject(item, "sourceUeIpv4Addr",
		                           af_coordination_info->
		                           source_ue_ipv4_addr) == NULL)
		{
			ogs_error(
				"OpenAPI_af_coordination_info_convertToJSON() failed [source_ue_ipv4_addr]");
			goto end;
		}
	}

	if(af_coordination_info->source_ue_ipv6_prefix) {
		if(cJSON_AddStringToObject(item, "sourceUeIpv6Prefix",
		                           af_coordination_info->
		                           source_ue_ipv6_prefix) == NULL)
		{
			ogs_error(
				"OpenAPI_af_coordination_info_convertToJSON() failed [source_ue_ipv6_prefix]");
			goto end;
		}
	}

	if(af_coordination_info->notification_info_list) {
		cJSON *notification_info_listList = cJSON_AddArrayToObject(item,
		                                                           "notificationInfoList");
		if(notification_info_listList == NULL) {
			ogs_error(
				"OpenAPI_af_coordination_info_convertToJSON() failed [notification_info_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			af_coordination_info->notification_info_list, node) {
			cJSON *itemLocal =
				OpenAPI_notification_info_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_af_coordination_info_convertToJSON() failed [notification_info_list]");
			...
			cJSON_AddItemToArray(notification_info_listList,
			                     itemLocal);
		}
	}
...
OpenAPI_af_coordination_info_t *OpenAPI_af_coordination_info_parseFromJSON(
	...
	source_dnai = cJSON_GetObjectItemCaseSensitive(af_coordination_infoJSON,
	                                               "sourceDnai");
	if(source_dnai) {
		if(!cJSON_IsString(source_dnai) &&
		   !cJSON_IsNull(source_dnai))
		{
			ogs_error(
				"OpenAPI_af_coordination_info_parseFromJSON() failed [source_dnai]");
			goto end;
		}
	}

	source_ue_ipv4_addr =
		cJSON_GetObjectItemCaseSensitive(af_coordination_infoJSON,
		                                 "sourceUeIpv4Addr");
	if(source_ue_ipv4_addr) {
		if(!cJSON_IsString(source_ue_ipv4_addr) &&
		   !cJSON_IsNull(source_ue_ipv4_addr))
		{
			ogs_error(
				"OpenAPI_af_coordination_info_parseFromJSON() failed [source_ue_ipv4_addr]");
			goto end;
		}
	}
	...
	if(source_ue_ipv6_prefix) {
		if(!cJSON_IsString(source_ue_ipv6_prefix) &&
		   !cJSON_IsNull(source_ue_ipv6_prefix))
		{
			ogs_error(
				"OpenAPI_af_coordination_info_parseFromJSON() failed [source_ue_ipv6_prefix]");
			goto end;
		}
	}
	...
	if(notification_info_list) {
		...
		if(!cJSON_IsArray(notification_info_list)) {
			ogs_error(
				"OpenAPI_af_coordination_info_parseFromJSON() failed [notification_info_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(notification_info_list_local,
			...
			OpenAPI_notification_info_t *notification_info_listItem
			...
			OpenAPI_list_add(notification_info_listList,
			                 notification_info_listItem);
		}
	}
	...
	af_coordination_info_local_var = OpenAPI_af_coordination_info_create(
		source_dnai &&
		!cJSON_IsNull(source_dnai) ?
		ogs_strdup(source_dnai->valuestring) : NULL,
		source_ue_ipv4_addr &&
		!cJSON_IsNull(source_ue_ipv4_addr) ?
		ogs_strdup(source_ue_ipv4_addr->valuestring) : NULL,
		source_ue_ipv6_prefix &&
		!cJSON_IsNull(source_ue_ipv6_prefix) ?
		ogs_strdup(source_ue_ipv6_prefix->valuestring) : NULL,
...
OpenAPI_af_coordination_info_t *OpenAPI_af_coordination_info_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_af_coordination_info_convertToJSON() failed");
		return NULL;
	}
