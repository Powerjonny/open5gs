...
OpenAPI_nef_info_t *OpenAPI_nef_info_create(char		*nef_id,
...
{
	OpenAPI_nef_info_t *nef_info_local_var =
		ogs_malloc(sizeof(OpenAPI_nef_info_t));
	...
	nef_info_local_var->is_uas_nf_functionality_ind =
		is_uas_nf_functionality_ind;
...
void OpenAPI_nef_info_free(OpenAPI_nef_info_t *nef_info) {
	...
	if(nef_info->external_group_identifiers_ranges) {
		OpenAPI_list_for_each(
			nef_info->external_group_identifiers_ranges, node) {
			OpenAPI_identity_range_free(node->data);
		}
...
cJSON *OpenAPI_nef_info_convertToJSON(OpenAPI_nef_info_t *nef_info) {
	...
	if(nef_info->nef_id) {
		...
		{
			ogs_error(
				"OpenAPI_nef_info_convertToJSON() failed [nef_id]");
			goto end;
		}
	}
	...
	if(nef_info->pfd_data) {
		cJSON *pfd_data_local_JSON =
			OpenAPI_pfd_data_convertToJSON(nef_info->pfd_data);
		if(pfd_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_nef_info_convertToJSON() failed [pfd_data]");
			goto end;
		}
	...
	if(nef_info->af_ee_data) {
		...
		if(af_ee_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_nef_info_convertToJSON() failed [af_ee_data]");
			goto end;
		}
	...
	if(nef_info->gpsi_ranges) {
		...
		if(gpsi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_nef_info_convertToJSON() failed [gpsi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(nef_info->gpsi_ranges, node) {
			cJSON *itemLocal =
	...
	if(nef_info->external_group_identifiers_ranges) {
		...
		if(external_group_identifiers_rangesList == NULL) {
			ogs_error(
				"OpenAPI_nef_info_convertToJSON() failed [external_group_identifiers_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			nef_info->external_group_identifiers_ranges, node) {
	...
	if(nef_info->served_fqdn_list) {
		...
		if(served_fqdn_listList == NULL) {
			ogs_error(
				"OpenAPI_nef_info_convertToJSON() failed [served_fqdn_list]");
			goto end;
		}
		OpenAPI_list_for_each(nef_info->served_fqdn_list, node) {
	...
	if(nef_info->tai_list) {
		...
		if(tai_listList == NULL) {
			ogs_error(
				"OpenAPI_nef_info_convertToJSON() failed [tai_list]");
			goto end;
		}
		OpenAPI_list_for_each(nef_info->tai_list, node) {
			cJSON *itemLocal =
	...
	if(nef_info->tai_range_list) {
		...
		if(tai_range_listList == NULL) {
			ogs_error(
				"OpenAPI_nef_info_convertToJSON() failed [tai_range_list]");
			goto end;
		}
		OpenAPI_list_for_each(nef_info->tai_range_list, node) {
			cJSON *itemLocal =
	...
	if(nef_info->dnai_list) {
		...
		if(dnai_listList == NULL) {
			ogs_error(
				"OpenAPI_nef_info_convertToJSON() failed [dnai_list]");
			goto end;
		}
		OpenAPI_list_for_each(nef_info->dnai_list, node) {
	...
	if(nef_info->un_trust_af_info_list) {
		...
		if(un_trust_af_info_listList == NULL) {
			ogs_error(
				"OpenAPI_nef_info_convertToJSON() failed [un_trust_af_info_list]");
			goto end;
		}
		OpenAPI_list_for_each(nef_info->un_trust_af_info_list, node) {
			cJSON *itemLocal =
	...
	if(nef_info->is_uas_nf_functionality_ind) {
		if(cJSON_AddBoolToObject(item, "uasNfFunctionalityInd",
		                         nef_info->uas_nf_functionality_ind) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_nef_info_convertToJSON() failed [uas_nf_functionality_ind]");
			goto end;
		}
	}
...
OpenAPI_nef_info_t *OpenAPI_nef_info_parseFromJSON(cJSON *nef_infoJSON) {
	...
	if(nef_id) {
		...
		{
			ogs_error(
				"OpenAPI_nef_info_parseFromJSON() failed [nef_id]");
			goto end;
		}
	}
	...
	gpsi_ranges = cJSON_GetObjectItemCaseSensitive(nef_infoJSON,
	                                               "gpsiRanges");
	if(gpsi_ranges) {
		cJSON *gpsi_ranges_local = NULL;
		if(!cJSON_IsArray(gpsi_ranges)) {
			ogs_error(
				"OpenAPI_nef_info_parseFromJSON() failed [gpsi_ranges]");
			goto end;
		}
	...
	if(external_group_identifiers_ranges) {
		...
		if(!cJSON_IsArray(external_group_identifiers_ranges)) {
			ogs_error(
				"OpenAPI_nef_info_parseFromJSON() failed [external_group_identifiers_ranges]");
			goto end;
		}
	...
	served_fqdn_list = cJSON_GetObjectItemCaseSensitive(nef_infoJSON,
	                                                    "servedFqdnList");
	if(served_fqdn_list) {
		cJSON *served_fqdn_list_local = NULL;
		if(!cJSON_IsArray(served_fqdn_list)) {
			ogs_error(
				"OpenAPI_nef_info_parseFromJSON() failed [served_fqdn_list]");
			goto end;
		}
	...
	if(tai_list) {
		...
		if(!cJSON_IsArray(tai_list)) {
			ogs_error(
				"OpenAPI_nef_info_parseFromJSON() failed [tai_list]");
			goto end;
		}
	...
	tai_range_list = cJSON_GetObjectItemCaseSensitive(nef_infoJSON,
	                                                  "taiRangeList");
	if(tai_range_list) {
		cJSON *tai_range_list_local = NULL;
		if(!cJSON_IsArray(tai_range_list)) {
			ogs_error(
				"OpenAPI_nef_info_parseFromJSON() failed [tai_range_list]");
			goto end;
		}
	...
	if(dnai_list) {
		...
		if(!cJSON_IsArray(dnai_list)) {
			ogs_error(
				"OpenAPI_nef_info_parseFromJSON() failed [dnai_list]");
			goto end;
		}
	...
	if(un_trust_af_info_list) {
		...
		if(!cJSON_IsArray(un_trust_af_info_list)) {
			ogs_error(
				"OpenAPI_nef_info_parseFromJSON() failed [un_trust_af_info_list]");
			goto end;
		}
	...
	if(uas_nf_functionality_ind) {
		if(!cJSON_IsBool(uas_nf_functionality_ind)) {
			ogs_error(
				"OpenAPI_nef_info_parseFromJSON() failed [uas_nf_functionality_ind]");
			goto end;
		}
	}
	...
	nef_info_local_var = OpenAPI_nef_info_create(
		nef_id &&
		!cJSON_IsNull(nef_id) ? ogs_strdup(nef_id->valuestring) : NULL,
		...
		uas_nf_functionality_ind ? uas_nf_functionality_ind->valueint :
		0
		);
...
OpenAPI_nef_info_t *OpenAPI_nef_info_copy(OpenAPI_nef_info_t	*dst,
                                          OpenAPI_nef_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
