...
OpenAPI_n3ga_location_t *OpenAPI_n3ga_location_create(
	...
	hfc_node_id, char *gli,
	OpenAPI_line_type_e w5gban_line_type, char *gci) {
	OpenAPI_n3ga_location_t *n3ga_location_local_var =
		ogs_malloc(sizeof(OpenAPI_n3ga_location_t));
...
cJSON *OpenAPI_n3ga_location_convertToJSON(
	...
	if(n3ga_location == NULL) {
		ogs_error(
			"OpenAPI_n3ga_location_convertToJSON() failed [N3gaLocation]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(n3ga_location->n3gpp_tai) {
		cJSON *n3gpp_tai_local_JSON =
			OpenAPI_tai_convertToJSON(n3ga_location->n3gpp_tai);
		if(n3gpp_tai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_n3ga_location_convertToJSON() failed [n3gpp_tai]");
			goto end;
		}
	...
	if(n3ga_location->n3_iwf_id) {
		...
		{
			ogs_error(
				"OpenAPI_n3ga_location_convertToJSON() failed [n3_iwf_id]");
			goto end;
		}
	}
	...
	if(n3ga_location->ue_ipv4_addr) {
		...
		{
			ogs_error(
				"OpenAPI_n3ga_location_convertToJSON() failed [ue_ipv4_addr]");
			goto end;
		}
	}

	if(n3ga_location->ue_ipv6_addr) {
		if(cJSON_AddStringToObject(item, "ueIpv6Addr",
		                           n3ga_location->ue_ipv6_addr) == NULL)
		{
			ogs_error(
				"OpenAPI_n3ga_location_convertToJSON() failed [ue_ipv6_addr]");
			goto end;
		}
	}
	...
	if(n3ga_location->is_port_number) {
		...
		{
			ogs_error(
				"OpenAPI_n3ga_location_convertToJSON() failed [port_number]");
			goto end;
		}
	}

	if(n3ga_location->protocol != OpenAPI_transport_protocol_NULL) {
		if(cJSON_AddStringToObject(item, "protocol",
		                           OpenAPI_transport_protocol_ToString(
						   n3ga_location->protocol)) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_n3ga_location_convertToJSON() failed [protocol]");
			goto end;
		}
	}
	...
	if(n3ga_location->tnap_id) {
		cJSON *tnap_id_local_JSON =
			OpenAPI_tnap_id_convertToJSON(n3ga_location->tnap_id);
		if(tnap_id_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_n3ga_location_convertToJSON() failed [tnap_id]");
			goto end;
		}
	...
	if(n3ga_location->twap_id) {
		cJSON *twap_id_local_JSON =
			OpenAPI_twap_id_convertToJSON(n3ga_location->twap_id);
		if(twap_id_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_n3ga_location_convertToJSON() failed [twap_id]");
			goto end;
		}
	...
	if(n3ga_location->hfc_node_id) {
		cJSON *hfc_node_id_local_JSON =
			OpenAPI_hfc_node_id_convertToJSON(
				n3ga_location->hfc_node_id);
		if(hfc_node_id_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_n3ga_location_convertToJSON() failed [hfc_node_id]");
			goto end;
		}
	...
	if(n3ga_location->gli) {
		...
		{
			ogs_error(
				"OpenAPI_n3ga_location_convertToJSON() failed [gli]");
			goto end;
		}
	}

	if(n3ga_location->w5gban_line_type != OpenAPI_line_type_NULL) {
		if(cJSON_AddStringToObject(item, "w5gbanLineType",
		                           OpenAPI_line_type_ToString(
						   n3ga_location->
						   w5gban_line_type)) == NULL)
		{
			ogs_error(
				"OpenAPI_n3ga_location_convertToJSON() failed [w5gban_line_type]");
			goto end;
		}
	}
	...
	if(n3ga_location->gci) {
		...
		{
			ogs_error(
				"OpenAPI_n3ga_location_convertToJSON() failed [gci]");
			goto end;
		}
	}
...
OpenAPI_n3ga_location_t *OpenAPI_n3ga_location_parseFromJSON(
	cJSON *n3ga_locationJSON) {
	...
	if(n3gpp_tai) {
		...
		if(!n3gpp_tai_local_nonprim) {
			ogs_error(
				"OpenAPI_tai_parseFromJSON failed [n3gpp_tai]");
			goto end;
		}
	}
	...
	if(n3_iwf_id) {
		...
		{
			ogs_error(
				"OpenAPI_n3ga_location_parseFromJSON() failed [n3_iwf_id]");
			goto end;
		}
	}
	...
	if(ue_ipv4_addr) {
		...
		{
			ogs_error(
				"OpenAPI_n3ga_location_parseFromJSON() failed [ue_ipv4_addr]");
			goto end;
		}
	}
	...
	if(ue_ipv6_addr) {
		if(!cJSON_IsString(ue_ipv6_addr) &&
		   !cJSON_IsNull(ue_ipv6_addr))
		{
			ogs_error(
				"OpenAPI_n3ga_location_parseFromJSON() failed [ue_ipv6_addr]");
			goto end;
		}
	}
	...
	if(port_number) {
		if(!cJSON_IsNumber(port_number)) {
			ogs_error(
				"OpenAPI_n3ga_location_parseFromJSON() failed [port_number]");
			goto end;
		}
	}
	...
	if(protocol) {
		if(!cJSON_IsString(protocol)) {
			ogs_error(
				"OpenAPI_n3ga_location_parseFromJSON() failed [protocol]");
			goto end;
		}
		protocolVariable =
			OpenAPI_transport_protocol_FromString(
	...
	if(hfc_node_id) {
		hfc_node_id_local_nonprim =
			OpenAPI_hfc_node_id_parseFromJSON(hfc_node_id);
		if(!hfc_node_id_local_nonprim) {
			ogs_error(
				"OpenAPI_hfc_node_id_parseFromJSON failed [hfc_node_id]");
			goto end;
		}
	}
	...
	if(gli) {
		...
		{
			ogs_error(
				"OpenAPI_n3ga_location_parseFromJSON() failed [gli]");
			goto end;
		}
	}
	...
	if(w5gban_line_type) {
		if(!cJSON_IsString(w5gban_line_type)) {
			ogs_error(
				"OpenAPI_n3ga_location_parseFromJSON() failed [w5gban_line_type]");
			goto end;
		}
		w5gban_line_typeVariable =
			OpenAPI_line_type_FromString(
				w5gban_line_type->valuestring);
	}
	...
	if(gci) {
		...
		{
			ogs_error(
				"OpenAPI_n3ga_location_parseFromJSON() failed [gci]");
			goto end;
		}
	}
	...
	n3ga_location_local_var = OpenAPI_n3ga_location_create(
		...
		n3_iwf_id &&
		!cJSON_IsNull(n3_iwf_id) ? ogs_strdup(n3_iwf_id->valuestring) :
		NULL,
		ue_ipv4_addr &&
		!cJSON_IsNull(ue_ipv4_addr) ?
		...
		ue_ipv6_addr &&
		!cJSON_IsNull(ue_ipv6_addr) ?
		ogs_strdup(ue_ipv6_addr->valuestring) : NULL,
		...
		gli &&
		!cJSON_IsNull(gli) ? ogs_strdup(gli->valuestring) : NULL,
		w5gban_line_type ? w5gban_line_typeVariable : 0,
		gci &&
		!cJSON_IsNull(gci) ? ogs_strdup(gci->valuestring) : NULL
		);
...
OpenAPI_n3ga_location_t *OpenAPI_n3ga_location_copy(
	OpenAPI_n3ga_location_t *dst, OpenAPI_n3ga_location_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
