...
OpenAPI_upf_information_t *OpenAPI_upf_information_create(
	...
	OpenAPI_addr_fqdn_t
	        *upf_addr) {
	OpenAPI_upf_information_t *upf_information_local_var =
		ogs_malloc(sizeof(OpenAPI_upf_information_t));
...
cJSON *OpenAPI_upf_information_convertToJSON(
	OpenAPI_upf_information_t *upf_information) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(upf_information == NULL) {
		ogs_error(
			"OpenAPI_upf_information_convertToJSON() failed [UpfInformation]");
		return NULL;
	}
	...
	if(upf_information->upf_id) {
		...
		{
			ogs_error(
				"OpenAPI_upf_information_convertToJSON() failed [upf_id]");
			goto end;
		}
	}

	if(upf_information->upf_addr) {
		cJSON *upf_addr_local_JSON =
			OpenAPI_addr_fqdn_convertToJSON(
				upf_information->upf_addr);
		if(upf_addr_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_upf_information_convertToJSON() failed [upf_addr]");
			goto end;
		}
...
OpenAPI_upf_information_t *OpenAPI_upf_information_parseFromJSON(
	cJSON *upf_informationJSON) {
	...
	if(upf_id) {
		...
		{
			ogs_error(
				"OpenAPI_upf_information_parseFromJSON() failed [upf_id]");
			goto end;
		}
	}
	...
	upf_addr = cJSON_GetObjectItemCaseSensitive(upf_informationJSON,
	                                            "upfAddr");
	if(upf_addr) {
		upf_addr_local_nonprim =
			OpenAPI_addr_fqdn_parseFromJSON(upf_addr);
		if(!upf_addr_local_nonprim) {
			ogs_error(
				"OpenAPI_addr_fqdn_parseFromJSON failed [upf_addr]");
			goto end;
		}
	}
	...
	upf_information_local_var = OpenAPI_upf_information_create(
		upf_id &&
		!cJSON_IsNull(upf_id) ? ogs_strdup(upf_id->valuestring) : NULL,
...
OpenAPI_upf_information_t *OpenAPI_upf_information_copy(
	OpenAPI_upf_information_t *dst, OpenAPI_upf_information_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
