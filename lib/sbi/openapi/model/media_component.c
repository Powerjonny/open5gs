...
OpenAPI_media_component_t *OpenAPI_media_component_create(char	*af_app_id,
                                                          ...
                                                          char	*mir_bw_dl,
                                                          char	*mir_bw_ul,
                                                          OpenAPI_preemption_capability_e
                                                          preempt_cap,
                                                          ...
                                                          *tscai_input_ul,
                                                          ...
                                                          int tscai_time_dom) {
	OpenAPI_media_component_t *media_component_local_var =
		ogs_malloc(sizeof(OpenAPI_media_component_t));
	...
	media_component_local_var->is_max_packet_loss_rate_dl_null =
		is_max_packet_loss_rate_dl_null;
	media_component_local_var->is_max_packet_loss_rate_dl =
		is_max_packet_loss_rate_dl;
	media_component_local_var->max_packet_loss_rate_dl =
		max_packet_loss_rate_dl;
	media_component_local_var->is_max_packet_loss_rate_ul_null =
		is_max_packet_loss_rate_ul_null;
	media_component_local_var->is_max_packet_loss_rate_ul =
		is_max_packet_loss_rate_ul;
	media_component_local_var->max_packet_loss_rate_ul =
		max_packet_loss_rate_ul;
	...
	media_component_local_var->is_tscai_input_dl_null =
		is_tscai_input_dl_null;
	media_component_local_var->tscai_input_dl = tscai_input_dl;
	media_component_local_var->is_tscai_input_ul_null =
		is_tscai_input_ul_null;
...
void OpenAPI_media_component_free(OpenAPI_media_component_t *media_component) {
	...
	if(media_component->af_rout_req) {
		OpenAPI_af_routing_requirement_free(
	...
	if(media_component->alt_ser_reqs_data) {
		OpenAPI_list_for_each(media_component->alt_ser_reqs_data,
		                      node) {
...
cJSON *OpenAPI_media_component_convertToJSON(
	...
	if(media_component == NULL) {
		ogs_error(
			"OpenAPI_media_component_convertToJSON() failed [MediaComponent]");
		return NULL;
	}
	...
	if(media_component->af_app_id) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [af_app_id]");
			goto end;
		}
	}
	...
	if(media_component->af_rout_req) {
		cJSON *af_rout_req_local_JSON =
			OpenAPI_af_routing_requirement_convertToJSON(
				media_component->af_rout_req);
		if(af_rout_req_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [af_rout_req]");
			goto end;
		}
	...
	if(media_component->qos_reference) {
		if(cJSON_AddStringToObject(item, "qosReference",
		                           media_component->qos_reference) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [qos_reference]");
			goto end;
		}
	}
	...
	if(media_component->is_dis_ue_notif) {
		if(cJSON_AddBoolToObject(item, "disUeNotif",
		                         media_component->dis_ue_notif) == NULL)
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [dis_ue_notif]");
			goto end;
		}
	}
	...
	if(media_component->alt_ser_reqs) {
		...
		if(alt_ser_reqsList == NULL) {
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [alt_ser_reqs]");
			goto end;
		}
		OpenAPI_list_for_each(media_component->alt_ser_reqs, node) {
	...
	if(media_component->alt_ser_reqs_data) {
		...
		if(alt_ser_reqs_dataList == NULL) {
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [alt_ser_reqs_data]");
			goto end;
		}
		OpenAPI_list_for_each(media_component->alt_ser_reqs_data,
		                      node) {
	...
	if(media_component->is_cont_ver) {
		if(cJSON_AddNumberToObject(item, "contVer",
		                           media_component->cont_ver) == NULL)
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [cont_ver]");
			goto end;
		}
	}
	...
	if(media_component->codecs) {
		...
		if(codecsList == NULL) {
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [codecs]");
			goto end;
		}
		OpenAPI_list_for_each(media_component->codecs, node) {
	...
	if(media_component->is_des_max_latency) {
		if(cJSON_AddNumberToObject(item, "desMaxLatency",
		                           media_component->des_max_latency) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [des_max_latency]");
			goto end;
		}
	}
	...
	if(media_component->is_des_max_loss) {
		if(cJSON_AddNumberToObject(item, "desMaxLoss",
		                           media_component->des_max_loss) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [des_max_loss]");
			goto end;
		}
	}
	...
	if(media_component->flus_id) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [flus_id]");
			goto end;
		}
	}
	...
	if(media_component->f_status != OpenAPI_flow_status_NULL) {
		if(cJSON_AddStringToObject(item, "fStatus",
						   ...
						   media_component->f_status))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [f_status]");
			goto end;
		}
	}
	...
	if(media_component->mar_bw_dl) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [mar_bw_dl]");
			goto end;
		}
	}
	...
	if(media_component->mar_bw_ul) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [mar_bw_ul]");
			goto end;
		}
	}
	...
	if(media_component->is_max_packet_loss_rate_dl) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [max_packet_loss_rate_dl]");
			goto end;
		}
	...
	if(media_component->is_max_packet_loss_rate_ul) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [max_packet_loss_rate_ul]");
			goto end;
		}
	...
	if(media_component->max_supp_bw_dl) {
		if(cJSON_AddStringToObject(item, "maxSuppBwDl",
		                           media_component->max_supp_bw_dl) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [max_supp_bw_dl]");
			goto end;
		}
	}
	...
	if(media_component->max_supp_bw_ul) {
		if(cJSON_AddStringToObject(item, "maxSuppBwUl",
		                           media_component->max_supp_bw_ul) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [max_supp_bw_ul]");
			goto end;
		}
	}

	if(cJSON_AddNumberToObject(item, "medCompN",
	                           media_component->med_comp_n) == NULL)
	{
		ogs_error(
			"OpenAPI_media_component_convertToJSON() failed [med_comp_n]");
		goto end;
	}
	...
	if(media_component->med_sub_comps) {
		...
		if(med_sub_comps == NULL) {
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [med_sub_comps]");
			goto end;
		}
		cJSON *localMapObject = med_sub_comps;
		if(media_component->med_sub_comps) {
			OpenAPI_list_for_each(media_component->med_sub_comps,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_media_sub_component_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_media_component_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(media_component->med_type != OpenAPI_media_type_NULL) {
		if(cJSON_AddStringToObject(item, "medType",
						   ...
						   media_component->med_type))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [med_type]");
			goto end;
		}
	}
	...
	if(media_component->min_des_bw_dl) {
		if(cJSON_AddStringToObject(item, "minDesBwDl",
		                           media_component->min_des_bw_dl) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [min_des_bw_dl]");
			goto end;
		}
	}
	...
	if(media_component->min_des_bw_ul) {
		if(cJSON_AddStringToObject(item, "minDesBwUl",
		                           media_component->min_des_bw_ul) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [min_des_bw_ul]");
			goto end;
		}
	}
	...
	if(media_component->mir_bw_dl) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [mir_bw_dl]");
			goto end;
		}
	}
	...
	if(media_component->mir_bw_ul) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [mir_bw_ul]");
			goto end;
		}
	}

	if(media_component->preempt_cap != OpenAPI_preemption_capability_NULL) {
		if(cJSON_AddStringToObject(item, "preemptCap",
		                           OpenAPI_preemption_capability_ToString
		                                   (media_component->preempt_cap))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [preempt_cap]");
			goto end;
		}
	}
	...
	{
		if(cJSON_AddStringToObject(item, "preemptVuln",
		                           OpenAPI_preemption_vulnerability_ToString
		                                   (media_component->
		                                   preempt_vuln)) == NULL)
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [preempt_vuln]");
			goto end;
		}
	}
	   ...
	   OpenAPI_priority_sharing_indicator_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [prio_sharing_ind]");
			goto end;
		}
	}
	...
	if(media_component->res_prio != OpenAPI_reserv_priority_NULL) {
		if(cJSON_AddStringToObject(item, "resPrio",
						   ...
						   media_component->res_prio))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [res_prio]");
			goto end;
		}
	}
	...
	if(media_component->rr_bw) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [rr_bw]");
			goto end;
		}
	}
	...
	if(media_component->rs_bw) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [rs_bw]");
			goto end;
		}
	}
	...
	if(media_component->is_sharing_key_dl) {
		if(cJSON_AddNumberToObject(item, "sharingKeyDl",
		                           media_component->sharing_key_dl) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [sharing_key_dl]");
			goto end;
		}
	}
	...
	if(media_component->is_sharing_key_ul) {
		if(cJSON_AddNumberToObject(item, "sharingKeyUl",
		                           media_component->sharing_key_ul) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [sharing_key_ul]");
			goto end;
		}
	}
	...
	if(media_component->tsn_qos) {
		cJSON *tsn_qos_local_JSON =
			OpenAPI_tsn_qos_container_convertToJSON(
				media_component->tsn_qos);
		if(tsn_qos_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [tsn_qos]");
			goto end;
		}
	...
	if(media_component->tscai_input_dl) {
		...
		if(tscai_input_dl_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [tscai_input_dl]");
			goto end;
		}
	...
	if(media_component->tscai_input_ul) {
		...
		if(tscai_input_ul_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [tscai_input_ul]");
			goto end;
		}
	...
	if(media_component->is_tscai_time_dom) {
		if(cJSON_AddNumberToObject(item, "tscaiTimeDom",
		                           media_component->tscai_time_dom) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_media_component_convertToJSON() failed [tscai_time_dom]");
			goto end;
		}
	}
...
OpenAPI_media_component_t *OpenAPI_media_component_parseFromJSON(
	...
	af_app_id = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                             "afAppId");
	if(af_app_id) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [af_app_id]");
			goto end;
		}
	}

	af_rout_req = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                               "afRoutReq");
	if(af_rout_req) {
		...
		if(!af_rout_req_local_nonprim) {
			ogs_error(
				"OpenAPI_af_routing_requirement_parseFromJSON failed [af_rout_req]");
			goto end;
		}
	}

	qos_reference = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                                 "qosReference");
	if(qos_reference) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [qos_reference]");
			goto end;
		}
	}

	dis_ue_notif = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                                "disUeNotif");
	if(dis_ue_notif) {
		if(!cJSON_IsBool(dis_ue_notif)) {
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [dis_ue_notif]");
			goto end;
		}
	}

	alt_ser_reqs = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                                "altSerReqs");
	if(alt_ser_reqs) {
		cJSON *alt_ser_reqs_local = NULL;
		if(!cJSON_IsArray(alt_ser_reqs)) {
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [alt_ser_reqs]");
			goto end;
		}
		...
		cJSON_ArrayForEach(alt_ser_reqs_local, alt_ser_reqs) {
			...
			OpenAPI_list_add(alt_ser_reqsList,
			                 ogs_strdup(
						 alt_ser_reqs_local->valuestring));
		}
	}

	alt_ser_reqs_data =
		cJSON_GetObjectItemCaseSensitive(media_componentJSON,
		                                 "altSerReqsData");
	if(alt_ser_reqs_data) {
		cJSON *alt_ser_reqs_data_local = NULL;
		if(!cJSON_IsArray(alt_ser_reqs_data)) {
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [alt_ser_reqs_data]");
			goto end;
		}
	...
	cont_ver = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                            "contVer");
	if(cont_ver) {
		if(!cJSON_IsNumber(cont_ver)) {
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [cont_ver]");
			goto end;
		}
	}

	codecs = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                          "codecs");
	if(codecs) {
		cJSON *codecs_local = NULL;
		if(!cJSON_IsArray(codecs)) {
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [codecs]");
			goto end;
		}
	...
	des_max_latency = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                                   "desMaxLatency");
	if(des_max_latency) {
		if(!cJSON_IsNumber(des_max_latency)) {
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [des_max_latency]");
			goto end;
		}
	}

	des_max_loss = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                                "desMaxLoss");
	if(des_max_loss) {
		if(!cJSON_IsNumber(des_max_loss)) {
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [des_max_loss]");
			goto end;
		}
	}

	flus_id = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                           "flusId");
	if(flus_id) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [flus_id]");
			goto end;
		}
	}

	f_status = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                            "fStatus");
	if(f_status) {
		if(!cJSON_IsString(f_status)) {
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [f_status]");
			goto end;
		}
		f_statusVariable =
	...
	mar_bw_dl = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                             "marBwDl");
	if(mar_bw_dl) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [mar_bw_dl]");
			goto end;
		}
	}

	mar_bw_ul = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                             "marBwUl");
	if(mar_bw_ul) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [mar_bw_ul]");
			goto end;
		}
	}
	...
	if(max_packet_loss_rate_dl) {
		if(!cJSON_IsNull(max_packet_loss_rate_dl)) {
			if(!cJSON_IsNumber(max_packet_loss_rate_dl)) {
				ogs_error(
					"OpenAPI_media_component_parseFromJSON() failed [max_packet_loss_rate_dl]");
				goto end;
			}
		}
	}
	...
	if(max_packet_loss_rate_ul) {
		if(!cJSON_IsNull(max_packet_loss_rate_ul)) {
			if(!cJSON_IsNumber(max_packet_loss_rate_ul)) {
				ogs_error(
					"OpenAPI_media_component_parseFromJSON() failed [max_packet_loss_rate_ul]");
				goto end;
			}
		}
	}

	max_supp_bw_dl = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                                  "maxSuppBwDl");
	if(max_supp_bw_dl) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [max_supp_bw_dl]");
			goto end;
		}
	}

	max_supp_bw_ul = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                                  "maxSuppBwUl");
	if(max_supp_bw_ul) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [max_supp_bw_ul]");
			goto end;
		}
	}

	med_comp_n = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                              "medCompN");
	if(!med_comp_n) {
		ogs_error(
			"OpenAPI_media_component_parseFromJSON() failed [med_comp_n]");
		goto end;
	}
	...
	med_sub_comps = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                                 "medSubComps");
	if(med_sub_comps) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [med_sub_comps]");
			goto end;
		}
		if(cJSON_IsObject(med_sub_comps)) {
			...
			cJSON_ArrayForEach(med_sub_comps_local_map,
				...
				if(cJSON_IsObject(localMapObject)) {
					localMapKeyPair = OpenAPI_map_create(
						...
						OpenAPI_media_sub_component_parseFromJSON
						        (localMapObject));
					...
					ogs_error(
						"OpenAPI_media_component_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	med_type = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                            "medType");
	if(med_type) {
		if(!cJSON_IsString(med_type)) {
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [med_type]");
			goto end;
		}
		med_typeVariable =
	...
	min_des_bw_dl = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                                 "minDesBwDl");
	if(min_des_bw_dl) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [min_des_bw_dl]");
			goto end;
		}
	}

	min_des_bw_ul = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                                 "minDesBwUl");
	if(min_des_bw_ul) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [min_des_bw_ul]");
			goto end;
		}
	}

	mir_bw_dl = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                             "mirBwDl");
	if(mir_bw_dl) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [mir_bw_dl]");
			goto end;
		}
	}

	mir_bw_ul = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                             "mirBwUl");
	if(mir_bw_ul) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [mir_bw_ul]");
			goto end;
		}
	}

	preempt_cap = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                               "preemptCap");
	if(preempt_cap) {
		if(!cJSON_IsString(preempt_cap)) {
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [preempt_cap]");
			goto end;
		}
		preempt_capVariable =
			OpenAPI_preemption_capability_FromString(
				preempt_cap->valuestring);
	}

	preempt_vuln = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                                "preemptVuln");
	if(preempt_vuln) {
		if(!cJSON_IsString(preempt_vuln)) {
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [preempt_vuln]");
			goto end;
		}
	...
	prio_sharing_ind = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                                    "prioSharingInd");
	if(prio_sharing_ind) {
		if(!cJSON_IsString(prio_sharing_ind)) {
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [prio_sharing_ind]");
			goto end;
		}
	...
	res_prio = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                            "resPrio");
	if(res_prio) {
		if(!cJSON_IsString(res_prio)) {
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [res_prio]");
			goto end;
		}
		res_prioVariable =
	...
	if(rr_bw) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [rr_bw]");
			goto end;
		}
	}
	...
	if(rs_bw) {
		...
		{
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [rs_bw]");
			goto end;
		}
	}

	sharing_key_dl = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                                  "sharingKeyDl");
	if(sharing_key_dl) {
		if(!cJSON_IsNumber(sharing_key_dl)) {
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [sharing_key_dl]");
			goto end;
		}
	}

	sharing_key_ul = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                                  "sharingKeyUl");
	if(sharing_key_ul) {
		if(!cJSON_IsNumber(sharing_key_ul)) {
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [sharing_key_ul]");
			goto end;
		}
	}

	tsn_qos = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                           "tsnQos");
	if(tsn_qos) {
		tsn_qos_local_nonprim =
			OpenAPI_tsn_qos_container_parseFromJSON(tsn_qos);
		if(!tsn_qos_local_nonprim) {
			ogs_error(
				"OpenAPI_tsn_qos_container_parseFromJSON failed [tsn_qos]");
			goto end;
		}
	}

	tscai_input_dl = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                                  "tscaiInputDl");
	if(tscai_input_dl) {
	...
	tscai_input_ul = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                                  "tscaiInputUl");
	if(tscai_input_ul) {
	...
	tscai_time_dom = cJSON_GetObjectItemCaseSensitive(media_componentJSON,
	                                                  "tscaiTimeDom");
	if(tscai_time_dom) {
		if(!cJSON_IsNumber(tscai_time_dom)) {
			ogs_error(
				"OpenAPI_media_component_parseFromJSON() failed [tscai_time_dom]");
			goto end;
		}
	}
...
OpenAPI_media_component_t *OpenAPI_media_component_copy(
	OpenAPI_media_component_t *dst, OpenAPI_media_component_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
