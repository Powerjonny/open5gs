...
OpenAPI_exception_t *OpenAPI_exception_create(
	...
	excep_trend) {
	OpenAPI_exception_t *exception_local_var =
		ogs_malloc(sizeof(OpenAPI_exception_t));
...
cJSON *OpenAPI_exception_convertToJSON(OpenAPI_exception_t *exception) {
	...
	if(exception == NULL) {
		ogs_error(
			"OpenAPI_exception_convertToJSON() failed [Exception]");
		return NULL;
	}
	...
	if(!exception->excep_id) {
		ogs_error(
			"OpenAPI_exception_convertToJSON() failed [excep_id]");
		return NULL;
	}
	cJSON *excep_id_local_JSON =
		OpenAPI_exception_id_convertToJSON(exception->excep_id);
	...
	if(exception->is_excep_level) {
		...
		{
			ogs_error(
				"OpenAPI_exception_convertToJSON() failed [excep_level]");
			goto end;
		}
	}

	if(exception->excep_trend) {
		cJSON *excep_trend_local_JSON =
			OpenAPI_exception_trend_convertToJSON(
				exception->excep_trend);
		if(excep_trend_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_exception_convertToJSON() failed [excep_trend]");
			goto end;
		}
...
OpenAPI_exception_t *OpenAPI_exception_parseFromJSON(cJSON *exceptionJSON) {
	...
	if(!excep_id) {
		ogs_error(
			"OpenAPI_exception_parseFromJSON() failed [excep_id]");
		goto end;
	}
	excep_id_local_nonprim = OpenAPI_exception_id_parseFromJSON(excep_id);
	if(!excep_id_local_nonprim) {
		ogs_error(
			"OpenAPI_exception_id_parseFromJSON failed [excep_id]");
		goto end;
	}
	...
	if(excep_level) {
		if(!cJSON_IsNumber(excep_level)) {
			ogs_error(
				"OpenAPI_exception_parseFromJSON() failed [excep_level]");
			goto end;
		}
	}
	...
	excep_trend = cJSON_GetObjectItemCaseSensitive(exceptionJSON,
	                                               "excepTrend");
	if(excep_trend) {
		excep_trend_local_nonprim =
			OpenAPI_exception_trend_parseFromJSON(excep_trend);
		if(!excep_trend_local_nonprim) {
			ogs_error(
				"OpenAPI_exception_trend_parseFromJSON failed [excep_trend]");
			goto end;
		}
	}
...
OpenAPI_exception_t *OpenAPI_exception_copy(OpenAPI_exception_t *dst,
                                            OpenAPI_exception_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
