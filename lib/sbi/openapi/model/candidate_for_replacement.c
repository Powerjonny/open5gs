...
OpenAPI_candidate_for_replacement_t *OpenAPI_candidate_for_replacement_create(
	OpenAPI_snssai_t *snssai, bool is_dnns_null, OpenAPI_list_t *dnns) {
	OpenAPI_candidate_for_replacement_t *candidate_for_replacement_local_var
	        = ogs_malloc(sizeof(OpenAPI_candidate_for_replacement_t));
...
void OpenAPI_candidate_for_replacement_free(
	OpenAPI_candidate_for_replacement_t *candidate_for_replacement) {
...
cJSON *OpenAPI_candidate_for_replacement_convertToJSON(
	OpenAPI_candidate_for_replacement_t *candidate_for_replacement) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(candidate_for_replacement == NULL) {
		ogs_error(
			"OpenAPI_candidate_for_replacement_convertToJSON() failed [CandidateForReplacement]");
		return NULL;
	}
	...
	if(!candidate_for_replacement->snssai) {
		ogs_error(
			"OpenAPI_candidate_for_replacement_convertToJSON() failed [snssai]");
		return NULL;
	}
	cJSON *snssai_local_JSON =
		OpenAPI_snssai_convertToJSON(candidate_for_replacement->snssai);
	...
	if(candidate_for_replacement->dnns) {
		...
		if(dnnsList == NULL) {
			ogs_error(
				"OpenAPI_candidate_for_replacement_convertToJSON() failed [dnns]");
			goto end;
		}
		OpenAPI_list_for_each(candidate_for_replacement->dnns, node) {
...
OpenAPI_candidate_for_replacement_parseFromJSON(
	...
	if(!snssai) {
		ogs_error(
			"OpenAPI_candidate_for_replacement_parseFromJSON() failed [snssai]");
		goto end;
	}
	...
	if(dnns) {
		if(!cJSON_IsNull(dnns)) {
			...
			if(!cJSON_IsArray(dnns)) {
				ogs_error(
					"OpenAPI_candidate_for_replacement_parseFromJSON() failed [dnns]");
				goto end;
			}
			...
			cJSON_ArrayForEach(dnns_local, dnns) {
				...
				OpenAPI_list_add(dnnsList,
				                 ogs_strdup(
							 dnns_local->valuestring));
			}
		}
	}

	candidate_for_replacement_local_var =
		OpenAPI_candidate_for_replacement_create(
...
OpenAPI_candidate_for_replacement_t *OpenAPI_candidate_for_replacement_copy(
	OpenAPI_candidate_for_replacement_t	*dst,
	OpenAPI_candidate_for_replacement_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_candidate_for_replacement_convertToJSON() failed");
		return NULL;
	}
