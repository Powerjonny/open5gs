...
OpenAPI_v2x_subscription_data_t *OpenAPI_v2x_subscription_data_create(
	...
	OpenAPI_v2x_subscription_data_t *v2x_subscription_data_local_var =
		ogs_malloc(sizeof(OpenAPI_v2x_subscription_data_t));
	ogs_assert(v2x_subscription_data_local_var);

	v2x_subscription_data_local_var->nr_v2x_services_auth =
		nr_v2x_services_auth;
	v2x_subscription_data_local_var->lte_v2x_services_auth =
...
void OpenAPI_v2x_subscription_data_free(
	...
	if(v2x_subscription_data->nr_v2x_services_auth) {
		OpenAPI_nr_v2x_auth_free(
			v2x_subscription_data->nr_v2x_services_auth);
		v2x_subscription_data->nr_v2x_services_auth = NULL;
	}
	if(v2x_subscription_data->lte_v2x_services_auth) {
		OpenAPI_lte_v2x_auth_free(
			v2x_subscription_data->lte_v2x_services_auth);
...
cJSON *OpenAPI_v2x_subscription_data_convertToJSON(
	...
	if(v2x_subscription_data == NULL) {
		ogs_error(
			"OpenAPI_v2x_subscription_data_convertToJSON() failed [V2xSubscriptionData]");
		return NULL;
	}
	...
	if(v2x_subscription_data->nr_v2x_services_auth) {
		...
		if(nr_v2x_services_auth_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_v2x_subscription_data_convertToJSON() failed [nr_v2x_services_auth]");
			goto end;
		}
	...
	if(v2x_subscription_data->lte_v2x_services_auth) {
		...
		if(lte_v2x_services_auth_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_v2x_subscription_data_convertToJSON() failed [lte_v2x_services_auth]");
			goto end;
		}
	...
	if(v2x_subscription_data->nr_ue_pc5_ambr) {
		if(cJSON_AddStringToObject(item, "nrUePc5Ambr",
		                           v2x_subscription_data->nr_ue_pc5_ambr)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_v2x_subscription_data_convertToJSON() failed [nr_ue_pc5_ambr]");
			goto end;
		}
	}

	if(v2x_subscription_data->lte_pc5_ambr) {
		if(cJSON_AddStringToObject(item, "ltePc5Ambr",
		                           v2x_subscription_data->lte_pc5_ambr)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_v2x_subscription_data_convertToJSON() failed [lte_pc5_ambr]");
			goto end;
		}
	}
...
OpenAPI_v2x_subscription_data_t *OpenAPI_v2x_subscription_data_parseFromJSON(
	...
	nr_v2x_services_auth =
		cJSON_GetObjectItemCaseSensitive(v2x_subscription_dataJSON,
		                                 "nrV2xServicesAuth");
	...
	if(nr_ue_pc5_ambr) {
		...
		{
			ogs_error(
				"OpenAPI_v2x_subscription_data_parseFromJSON() failed [nr_ue_pc5_ambr]");
			goto end;
		}
	}
	...
	if(lte_pc5_ambr) {
		if(!cJSON_IsString(lte_pc5_ambr) &&
		   !cJSON_IsNull(lte_pc5_ambr))
		{
			ogs_error(
				"OpenAPI_v2x_subscription_data_parseFromJSON() failed [lte_pc5_ambr]");
			goto end;
		}
	}
	...
	v2x_subscription_data_local_var = OpenAPI_v2x_subscription_data_create(
		...
		lte_pc5_ambr &&
		!cJSON_IsNull(lte_pc5_ambr) ?
		ogs_strdup(lte_pc5_ambr->valuestring) : NULL
		);
...
OpenAPI_v2x_subscription_data_t *OpenAPI_v2x_subscription_data_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_v2x_subscription_data_convertToJSON() failed");
		return NULL;
	}
