...
OpenAPI_rule_report_t *OpenAPI_rule_report_create(OpenAPI_list_t *pcc_rule_ids,
                                                  OpenAPI_rule_status_e
                                                  rule_status,
	...
	OpenAPI_rule_report_t *rule_report_local_var =
		ogs_malloc(sizeof(OpenAPI_rule_report_t));
...
cJSON *OpenAPI_rule_report_convertToJSON(OpenAPI_rule_report_t *rule_report) {
	...
	if(rule_report == NULL) {
		ogs_error(
			"OpenAPI_rule_report_convertToJSON() failed [RuleReport]");
		return NULL;
	}
	...
	if(!rule_report->pcc_rule_ids) {
		ogs_error(
			"OpenAPI_rule_report_convertToJSON() failed [pcc_rule_ids]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(rule_report->pcc_rule_ids, node) {
		if(cJSON_AddStringToObject(pcc_rule_idsList, "",
		                           (char *) node->data) == NULL)
		{
			ogs_error(
				"OpenAPI_rule_report_convertToJSON() failed [pcc_rule_ids]");
	...
	if(rule_report->rule_status == OpenAPI_rule_status_NULL) {
		ogs_error(
			"OpenAPI_rule_report_convertToJSON() failed [rule_status]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "ruleStatus",
	                           OpenAPI_rule_status_ToString(
					   rule_report->rule_status)) == NULL)
	{
		ogs_error(
			"OpenAPI_rule_report_convertToJSON() failed [rule_status]");
		goto end;
	...
	if(rule_report->cont_vers) {
		...
		if(cont_versList == NULL) {
			ogs_error(
				"OpenAPI_rule_report_convertToJSON() failed [cont_vers]");
			goto end;
		}
		OpenAPI_list_for_each(rule_report->cont_vers, node) {
			if(node->data == NULL) {
			...
			if(cJSON_AddNumberToObject(cont_versList, "",
			                           *(double *) node->data) ==
			   NULL)
			{
				ogs_error(
	...
	if(rule_report->failure_code != OpenAPI_failure_code_NULL) {
		if(cJSON_AddStringToObject(item, "failureCode",
		                           OpenAPI_failure_code_ToString(
						   rule_report->failure_code))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_rule_report_convertToJSON() failed [failure_code]");
			goto end;
		}
	}

	if(rule_report->fin_unit_act) {
		cJSON *fin_unit_act_local_JSON =
			OpenAPI_final_unit_action_convertToJSON(
				rule_report->fin_unit_act);
		if(fin_unit_act_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_rule_report_convertToJSON() failed [fin_unit_act]");
			goto end;
		}
		cJSON_AddItemToObject(item, "finUnitAct",
		                      fin_unit_act_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_rule_report_convertToJSON() failed [fin_unit_act]");
			goto end;
	...
	if(rule_report->ran_nas_rel_causes) {
		...
		if(ran_nas_rel_causesList == NULL) {
			ogs_error(
				"OpenAPI_rule_report_convertToJSON() failed [ran_nas_rel_causes]");
			goto end;
		}
		OpenAPI_list_for_each(rule_report->ran_nas_rel_causes, node) {
			cJSON *itemLocal =
	...
	if(rule_report->alt_qos_param_id) {
		if(cJSON_AddStringToObject(item, "altQosParamId",
		                           rule_report->alt_qos_param_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_rule_report_convertToJSON() failed [alt_qos_param_id]");
			goto end;
		}
	}
...
OpenAPI_rule_report_t *OpenAPI_rule_report_parseFromJSON(
	cJSON *rule_reportJSON) {
	...
	pcc_rule_ids = cJSON_GetObjectItemCaseSensitive(rule_reportJSON,
	                                                "pccRuleIds");
	if(!pcc_rule_ids) {
		ogs_error(
			"OpenAPI_rule_report_parseFromJSON() failed [pcc_rule_ids]");
		goto end;
	}
	...
	cJSON_ArrayForEach(pcc_rule_ids_local, pcc_rule_ids) {
		...
		OpenAPI_list_add(pcc_rule_idsList,
		                 ogs_strdup(pcc_rule_ids_local->valuestring));
	}

	rule_status = cJSON_GetObjectItemCaseSensitive(rule_reportJSON,
	                                               "ruleStatus");
	if(!rule_status) {
		ogs_error(
			"OpenAPI_rule_report_parseFromJSON() failed [rule_status]");
		goto end;
	}
	...
	rule_statusVariable =
		OpenAPI_rule_status_FromString(rule_status->valuestring);

	cont_vers = cJSON_GetObjectItemCaseSensitive(rule_reportJSON,
	                                             "contVers");
	if(cont_vers) {
		cJSON *cont_vers_local = NULL;
		if(!cJSON_IsArray(cont_vers)) {
			ogs_error(
				"OpenAPI_rule_report_parseFromJSON() failed [cont_vers]");
			goto end;
		}
	...
	failure_code = cJSON_GetObjectItemCaseSensitive(rule_reportJSON,
	                                                "failureCode");
	if(failure_code) {
		if(!cJSON_IsString(failure_code)) {
			ogs_error(
				"OpenAPI_rule_report_parseFromJSON() failed [failure_code]");
			goto end;
		}
		failure_codeVariable =
			OpenAPI_failure_code_FromString(
				failure_code->valuestring);
	}

	fin_unit_act = cJSON_GetObjectItemCaseSensitive(rule_reportJSON,
	                                                "finUnitAct");
	if(fin_unit_act) {
		fin_unit_act_local_nonprim =
			OpenAPI_final_unit_action_parseFromJSON(fin_unit_act);
		if(!fin_unit_act_local_nonprim) {
			ogs_error(
				"OpenAPI_final_unit_action_parseFromJSON failed [fin_unit_act]");
			goto end;
		}
	}

	ran_nas_rel_causes = cJSON_GetObjectItemCaseSensitive(rule_reportJSON,
	                                                      "ranNasRelCauses");
	if(ran_nas_rel_causes) {
		cJSON *ran_nas_rel_causes_local = NULL;
		if(!cJSON_IsArray(ran_nas_rel_causes)) {
			ogs_error(
				"OpenAPI_rule_report_parseFromJSON() failed [ran_nas_rel_causes]");
			goto end;
		}
	...
	alt_qos_param_id = cJSON_GetObjectItemCaseSensitive(rule_reportJSON,
	                                                    "altQosParamId");
	if(alt_qos_param_id) {
		if(!cJSON_IsString(alt_qos_param_id) &&
		   !cJSON_IsNull(alt_qos_param_id))
		{
			ogs_error(
				"OpenAPI_rule_report_parseFromJSON() failed [alt_qos_param_id]");
			goto end;
		}
	}
	...
	rule_report_local_var = OpenAPI_rule_report_create(
		...
		alt_qos_param_id &&
		!cJSON_IsNull(alt_qos_param_id) ?
		ogs_strdup(alt_qos_param_id->valuestring) : NULL
		);
...
OpenAPI_rule_report_t *OpenAPI_rule_report_copy(OpenAPI_rule_report_t	*dst,
                                                OpenAPI_rule_report_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
