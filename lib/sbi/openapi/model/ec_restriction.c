...
OpenAPI_ec_restriction_t *OpenAPI_ec_restriction_create(char	*af_instance_id,
                                                        int	reference_id,
                                                        OpenAPI_list_t *
                                                        plmn_ec_infos,
                                                        char *
...
{
	OpenAPI_ec_restriction_t *ec_restriction_local_var =
		ogs_malloc(sizeof(OpenAPI_ec_restriction_t));
	...
	ec_restriction_local_var->mtc_provider_information =
		mtc_provider_information;
...
cJSON *OpenAPI_ec_restriction_convertToJSON(
	...
	if(ec_restriction == NULL) {
		ogs_error(
			"OpenAPI_ec_restriction_convertToJSON() failed [EcRestriction]");
		return NULL;
	}
	...
	if(!ec_restriction->af_instance_id) {
		ogs_error(
			"OpenAPI_ec_restriction_convertToJSON() failed [af_instance_id]");
		return NULL;
	}
	...
	if(cJSON_AddNumberToObject(item, "referenceId",
	                           ec_restriction->reference_id) == NULL)
	{
		ogs_error(
			"OpenAPI_ec_restriction_convertToJSON() failed [reference_id]");
		goto end;
	}

	if(ec_restriction->plmn_ec_infos) {
		cJSON *plmn_ec_infosList = cJSON_AddArrayToObject(item,
		                                                  "plmnEcInfos");
		if(plmn_ec_infosList == NULL) {
			ogs_error(
				"OpenAPI_ec_restriction_convertToJSON() failed [plmn_ec_infos]");
			goto end;
		}
		OpenAPI_list_for_each(ec_restriction->plmn_ec_infos, node) {
			cJSON *itemLocal =
				OpenAPI_plmn_ec_info_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_ec_restriction_convertToJSON() failed [plmn_ec_infos]");
				goto end;
	...
	if(ec_restriction->mtc_provider_information) {
		...
		{
			ogs_error(
				"OpenAPI_ec_restriction_convertToJSON() failed [mtc_provider_information]");
			goto end;
		}
	}
...
OpenAPI_ec_restriction_t *OpenAPI_ec_restriction_parseFromJSON(
	cJSON *ec_restrictionJSON) {
	...
	af_instance_id = cJSON_GetObjectItemCaseSensitive(ec_restrictionJSON,
	                                                  "afInstanceId");
	if(!af_instance_id) {
		ogs_error(
			"OpenAPI_ec_restriction_parseFromJSON() failed [af_instance_id]");
		goto end;
	}
	...
	reference_id = cJSON_GetObjectItemCaseSensitive(ec_restrictionJSON,
	                                                "referenceId");
	if(!reference_id) {
		ogs_error(
			"OpenAPI_ec_restriction_parseFromJSON() failed [reference_id]");
		goto end;
	}
	...
	plmn_ec_infos = cJSON_GetObjectItemCaseSensitive(ec_restrictionJSON,
	                                                 "plmnEcInfos");
	if(plmn_ec_infos) {
		cJSON *plmn_ec_infos_local = NULL;
		if(!cJSON_IsArray(plmn_ec_infos)) {
			ogs_error(
				"OpenAPI_ec_restriction_parseFromJSON() failed [plmn_ec_infos]");
			goto end;
		}
		...
		cJSON_ArrayForEach(plmn_ec_infos_local, plmn_ec_infos) {
			...
			OpenAPI_plmn_ec_info_t *plmn_ec_infosItem =
				OpenAPI_plmn_ec_info_parseFromJSON(
					plmn_ec_infos_local);
	...
	mtc_provider_information =
		cJSON_GetObjectItemCaseSensitive(ec_restrictionJSON,
	...
	if(mtc_provider_information) {
		...
		{
			ogs_error(
				"OpenAPI_ec_restriction_parseFromJSON() failed [mtc_provider_information]");
			goto end;
		}
	}
...
OpenAPI_ec_restriction_t *OpenAPI_ec_restriction_copy(
	OpenAPI_ec_restriction_t *dst, OpenAPI_ec_restriction_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
