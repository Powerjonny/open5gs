...
OpenAPI_mo_exp_data_counter_t *OpenAPI_mo_exp_data_counter_create(int counter,
                                                                  char	*
                                                                  time_stamp) {
	OpenAPI_mo_exp_data_counter_t *mo_exp_data_counter_local_var =
...
void OpenAPI_mo_exp_data_counter_free(
...
cJSON *OpenAPI_mo_exp_data_counter_convertToJSON(
	OpenAPI_mo_exp_data_counter_t *mo_exp_data_counter) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(mo_exp_data_counter == NULL) {
		ogs_error(
			"OpenAPI_mo_exp_data_counter_convertToJSON() failed [MoExpDataCounter]");
		return NULL;
	}
	...
	{
		ogs_error(
			"OpenAPI_mo_exp_data_counter_convertToJSON() failed [counter]");
		goto end;
	}
	...
	if(mo_exp_data_counter->time_stamp) {
		...
		{
			ogs_error(
				"OpenAPI_mo_exp_data_counter_convertToJSON() failed [time_stamp]");
			goto end;
		}
	}
...
OpenAPI_mo_exp_data_counter_t *OpenAPI_mo_exp_data_counter_parseFromJSON(
	...
	if(!counter) {
		ogs_error(
			"OpenAPI_mo_exp_data_counter_parseFromJSON() failed [counter]");
		goto end;
	}
	...
	if(time_stamp) {
		...
		{
			ogs_error(
				"OpenAPI_mo_exp_data_counter_parseFromJSON() failed [time_stamp]");
			goto end;
		}
	}
	...
	mo_exp_data_counter_local_var = OpenAPI_mo_exp_data_counter_create(
		...
		time_stamp &&
		!cJSON_IsNull(time_stamp) ?
		ogs_strdup(time_stamp->valuestring) : NULL
		);
...
OpenAPI_mo_exp_data_counter_t *OpenAPI_mo_exp_data_counter_copy(
	OpenAPI_mo_exp_data_counter_t	*dst,
	OpenAPI_mo_exp_data_counter_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
