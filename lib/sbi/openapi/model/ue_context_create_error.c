...
OpenAPI_ue_context_create_error_t *OpenAPI_ue_context_create_error_create(
	OpenAPI_problem_details_t *error, OpenAPI_ng_ap_cause_t *ngap_cause,
	OpenAPI_n2_info_content_t *target_to_source_failure_data) {
	OpenAPI_ue_context_create_error_t *ue_context_create_error_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_context_create_error_t));
...
void OpenAPI_ue_context_create_error_free(
...
cJSON *OpenAPI_ue_context_create_error_convertToJSON(
	OpenAPI_ue_context_create_error_t *ue_context_create_error) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(ue_context_create_error == NULL) {
		ogs_error(
			"OpenAPI_ue_context_create_error_convertToJSON() failed [UeContextCreateError]");
		return NULL;
	}
	...
	if(!ue_context_create_error->error) {
		ogs_error(
			"OpenAPI_ue_context_create_error_convertToJSON() failed [error]");
		return NULL;
	}
	...
	if(ue_context_create_error->ngap_cause) {
		...
		if(ngap_cause_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_create_error_convertToJSON() failed [ngap_cause]");
			goto end;
		}
	...
	if(ue_context_create_error->target_to_source_failure_data) {
		...
		if(target_to_source_failure_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_create_error_convertToJSON() failed [target_to_source_failure_data]");
			goto end;
		}
...
OpenAPI_ue_context_create_error_t *OpenAPI_ue_context_create_error_parseFromJSON
	...
	OpenAPI_n2_info_content_t *target_to_source_failure_data_local_nonprim =
		NULL;
	error = cJSON_GetObjectItemCaseSensitive(ue_context_create_errorJSON,
	                                         "error");
	if(!error) {
		ogs_error(
			"OpenAPI_ue_context_create_error_parseFromJSON() failed [error]");
		goto end;
	}
	...
	ngap_cause =
		cJSON_GetObjectItemCaseSensitive(ue_context_create_errorJSON,
		                                 "ngapCause");
	if(ngap_cause) {
	...
	if(target_to_source_failure_data) {
		...
		if(!target_to_source_failure_data_local_nonprim) {
			ogs_error(
				"OpenAPI_n2_info_content_parseFromJSON failed [target_to_source_failure_data]");
			goto end;
		}
	}

	ue_context_create_error_local_var =
...
OpenAPI_ue_context_create_error_t *OpenAPI_ue_context_create_error_copy(
	OpenAPI_ue_context_create_error_t	*dst,
	OpenAPI_ue_context_create_error_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ue_context_create_error_convertToJSON() failed");
		return NULL;
	}
