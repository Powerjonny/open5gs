...
OpenAPI_event_report_mode_t *OpenAPI_event_report_mode_create() {
	OpenAPI_event_report_mode_t *event_report_mode_local_var =
		ogs_malloc(sizeof(OpenAPI_event_report_mode_t));
...
cJSON *OpenAPI_event_report_mode_convertToJSON(
	...
	if(event_report_mode == NULL) {
		ogs_error(
			"OpenAPI_event_report_mode_convertToJSON() failed [EventReportMode]");
		return NULL;
	}
