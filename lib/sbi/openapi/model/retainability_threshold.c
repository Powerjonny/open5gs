...
OpenAPI_retainability_threshold_t *OpenAPI_retainability_threshold_create(
	...
	int rel_flow_ratio) {
	OpenAPI_retainability_threshold_t *retainability_threshold_local_var =
		ogs_malloc(sizeof(OpenAPI_retainability_threshold_t));
...
cJSON *OpenAPI_retainability_threshold_convertToJSON(
	...
	if(retainability_threshold == NULL) {
		ogs_error(
			"OpenAPI_retainability_threshold_convertToJSON() failed [RetainabilityThreshold]");
		return NULL;
	}
	...
	if(retainability_threshold->is_rel_flow_num) {
		...
		{
			ogs_error(
				"OpenAPI_retainability_threshold_convertToJSON() failed [rel_flow_num]");
			goto end;
		}
	}

	if(retainability_threshold->rel_time_unit) {
		cJSON *rel_time_unit_local_JSON =
			OpenAPI_time_unit_convertToJSON(
				retainability_threshold->rel_time_unit);
		if(rel_time_unit_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_retainability_threshold_convertToJSON() failed [rel_time_unit]");
			goto end;
		}
	...
	if(retainability_threshold->is_rel_flow_ratio) {
		...
		{
			ogs_error(
				"OpenAPI_retainability_threshold_convertToJSON() failed [rel_flow_ratio]");
			goto end;
		}
	}
...
OpenAPI_retainability_threshold_t *OpenAPI_retainability_threshold_parseFromJSON
	...
	rel_flow_num =
		cJSON_GetObjectItemCaseSensitive(retainability_thresholdJSON,
		                                 "relFlowNum");
	if(rel_flow_num) {
		if(!cJSON_IsNumber(rel_flow_num)) {
			ogs_error(
				"OpenAPI_retainability_threshold_parseFromJSON() failed [rel_flow_num]");
			goto end;
		}
	}
	...
	rel_time_unit =
		cJSON_GetObjectItemCaseSensitive(retainability_thresholdJSON,
		                                 "relTimeUnit");
	if(rel_time_unit) {
		rel_time_unit_local_nonprim =
			OpenAPI_time_unit_parseFromJSON(rel_time_unit);
		if(!rel_time_unit_local_nonprim) {
			ogs_error(
				"OpenAPI_time_unit_parseFromJSON failed [rel_time_unit]");
			goto end;
		}
	}
	...
	if(rel_flow_ratio) {
		if(!cJSON_IsNumber(rel_flow_ratio)) {
			ogs_error(
				"OpenAPI_retainability_threshold_parseFromJSON() failed [rel_flow_ratio]");
			goto end;
		}
	}

	retainability_threshold_local_var =
		OpenAPI_retainability_threshold_create(
...
OpenAPI_retainability_threshold_t *OpenAPI_retainability_threshold_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_retainability_threshold_convertToJSON() failed");
		return NULL;
	}
