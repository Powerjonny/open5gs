...
OpenAPI_auth_event_t *OpenAPI_auth_event_create(char *nf_instance_id,
	...
	OpenAPI_auth_event_t *auth_event_local_var =
		ogs_malloc(sizeof(OpenAPI_auth_event_t));
...
cJSON *OpenAPI_auth_event_convertToJSON(OpenAPI_auth_event_t *auth_event) {
	...
	if(auth_event == NULL) {
		ogs_error(
			"OpenAPI_auth_event_convertToJSON() failed [AuthEvent]");
		return NULL;
	}
	...
	if(!auth_event->nf_instance_id) {
		ogs_error(
			"OpenAPI_auth_event_convertToJSON() failed [nf_instance_id]");
		return NULL;
	}
	...
	if(cJSON_AddBoolToObject(item, "success",
	                         auth_event->success) == NULL)
	{
		ogs_error(
			"OpenAPI_auth_event_convertToJSON() failed [success]");
		goto end;
	}

	if(!auth_event->time_stamp) {
		ogs_error(
			"OpenAPI_auth_event_convertToJSON() failed [time_stamp]");
		return NULL;
	}
	...
	if(auth_event->auth_type == OpenAPI_auth_type_NULL) {
		ogs_error(
			"OpenAPI_auth_event_convertToJSON() failed [auth_type]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "authType",
	                           OpenAPI_auth_type_ToString(
					   auth_event->auth_type)) == NULL)
	{
		ogs_error(
			"OpenAPI_auth_event_convertToJSON() failed [auth_type]");
		goto end;
	...
	if(!auth_event->serving_network_name) {
		ogs_error(
			"OpenAPI_auth_event_convertToJSON() failed [serving_network_name]");
		return NULL;
	}
	...
	if(auth_event->is_auth_removal_ind) {
		if(cJSON_AddBoolToObject(item, "authRemovalInd",
		                         auth_event->auth_removal_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_auth_event_convertToJSON() failed [auth_removal_ind]");
			goto end;
		}
	}
	...
	if(auth_event->nf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_auth_event_convertToJSON() failed [nf_set_id]");
			goto end;
		}
	}
	...
	if(auth_event->reset_ids) {
		...
		if(reset_idsList == NULL) {
			ogs_error(
				"OpenAPI_auth_event_convertToJSON() failed [reset_ids]");
			goto end;
		}
		OpenAPI_list_for_each(auth_event->reset_ids, node) {
...
OpenAPI_auth_event_t *OpenAPI_auth_event_parseFromJSON(cJSON *auth_eventJSON) {
	...
	nf_instance_id = cJSON_GetObjectItemCaseSensitive(auth_eventJSON,
	                                                  "nfInstanceId");
	if(!nf_instance_id) {
		ogs_error(
			"OpenAPI_auth_event_parseFromJSON() failed [nf_instance_id]");
		goto end;
	}
	...
	if(!success) {
		ogs_error(
			"OpenAPI_auth_event_parseFromJSON() failed [success]");
		goto end;
	}
	...
	time_stamp = cJSON_GetObjectItemCaseSensitive(auth_eventJSON,
	                                              "timeStamp");
	if(!time_stamp) {
		ogs_error(
			"OpenAPI_auth_event_parseFromJSON() failed [time_stamp]");
		goto end;
	}
	...
	auth_type = cJSON_GetObjectItemCaseSensitive(auth_eventJSON,
	                                             "authType");
	if(!auth_type) {
		ogs_error(
			"OpenAPI_auth_event_parseFromJSON() failed [auth_type]");
		goto end;
	}
	...
	serving_network_name = cJSON_GetObjectItemCaseSensitive(auth_eventJSON,
	                                                        "servingNetworkName");
	if(!serving_network_name) {
		ogs_error(
			"OpenAPI_auth_event_parseFromJSON() failed [serving_network_name]");
		goto end;
	}
	...
	auth_removal_ind = cJSON_GetObjectItemCaseSensitive(auth_eventJSON,
	                                                    "authRemovalInd");
	if(auth_removal_ind) {
		if(!cJSON_IsBool(auth_removal_ind)) {
			ogs_error(
				"OpenAPI_auth_event_parseFromJSON() failed [auth_removal_ind]");
			goto end;
		}
	}
	...
	if(nf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_auth_event_parseFromJSON() failed [nf_set_id]");
			goto end;
		}
	}

	reset_ids = cJSON_GetObjectItemCaseSensitive(auth_eventJSON,
	                                             "resetIds");
	if(reset_ids) {
		cJSON *reset_ids_local = NULL;
		if(!cJSON_IsArray(reset_ids)) {
			ogs_error(
				"OpenAPI_auth_event_parseFromJSON() failed [reset_ids]");
			goto end;
		}
