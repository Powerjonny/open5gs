...
OpenAPI_network_area_info_t *OpenAPI_network_area_info_create(
	...
	OpenAPI_network_area_info_t *network_area_info_local_var =
		ogs_malloc(sizeof(OpenAPI_network_area_info_t));
...
void OpenAPI_network_area_info_free(
...
cJSON *OpenAPI_network_area_info_convertToJSON(
	...
	if(network_area_info == NULL) {
		ogs_error(
			"OpenAPI_network_area_info_convertToJSON() failed [NetworkAreaInfo]");
		return NULL;
	}
	...
	if(network_area_info->ecgis) {
		...
		if(ecgisList == NULL) {
			ogs_error(
				"OpenAPI_network_area_info_convertToJSON() failed [ecgis]");
			goto end;
		}
		OpenAPI_list_for_each(network_area_info->ecgis, node) {
			cJSON *itemLocal =
	...
	if(network_area_info->ncgis) {
		...
		if(ncgisList == NULL) {
			ogs_error(
				"OpenAPI_network_area_info_convertToJSON() failed [ncgis]");
			goto end;
		}
		OpenAPI_list_for_each(network_area_info->ncgis, node) {
			cJSON *itemLocal =
	...
	if(network_area_info->g_ran_node_ids) {
		...
		if(g_ran_node_idsList == NULL) {
			ogs_error(
				"OpenAPI_network_area_info_convertToJSON() failed [g_ran_node_ids]");
			goto end;
		}
		OpenAPI_list_for_each(network_area_info->g_ran_node_ids, node) {
			cJSON *itemLocal =
	...
	if(network_area_info->tais) {
		...
		if(taisList == NULL) {
			ogs_error(
				"OpenAPI_network_area_info_convertToJSON() failed [tais]");
			goto end;
		}
		OpenAPI_list_for_each(network_area_info->tais, node) {
			cJSON *itemLocal =
...
OpenAPI_network_area_info_t *OpenAPI_network_area_info_parseFromJSON(
	...
	ecgis = cJSON_GetObjectItemCaseSensitive(network_area_infoJSON,
	                                         "ecgis");
	if(ecgis) {
		cJSON *ecgis_local = NULL;
		if(!cJSON_IsArray(ecgis)) {
			ogs_error(
				"OpenAPI_network_area_info_parseFromJSON() failed [ecgis]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ecgis_local, ecgis) {
			...
			OpenAPI_ecgi_t *ecgisItem =
				OpenAPI_ecgi_parseFromJSON(ecgis_local);
			if(!ecgisItem) {
	...
	ncgis = cJSON_GetObjectItemCaseSensitive(network_area_infoJSON,
	                                         "ncgis");
	if(ncgis) {
		cJSON *ncgis_local = NULL;
		if(!cJSON_IsArray(ncgis)) {
			ogs_error(
				"OpenAPI_network_area_info_parseFromJSON() failed [ncgis]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ncgis_local, ncgis) {
			...
			OpenAPI_ncgi_t *ncgisItem =
				OpenAPI_ncgi_parseFromJSON(ncgis_local);
			if(!ncgisItem) {
	...
	if(g_ran_node_ids) {
		...
		if(!cJSON_IsArray(g_ran_node_ids)) {
			ogs_error(
				"OpenAPI_network_area_info_parseFromJSON() failed [g_ran_node_ids]");
			goto end;
		}
		...
		cJSON_ArrayForEach(g_ran_node_ids_local, g_ran_node_ids) {
			...
			OpenAPI_global_ran_node_id_t *g_ran_node_idsItem =
	...
	if(tais) {
		...
		if(!cJSON_IsArray(tais)) {
			ogs_error(
				"OpenAPI_network_area_info_parseFromJSON() failed [tais]");
			goto end;
		}
		...
		cJSON_ArrayForEach(tais_local, tais) {
			...
			OpenAPI_tai_t *taisItem =
				OpenAPI_tai_parseFromJSON(tais_local);
			if(!taisItem) {
...
OpenAPI_network_area_info_t *OpenAPI_network_area_info_copy(
	OpenAPI_network_area_info_t *dst, OpenAPI_network_area_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
