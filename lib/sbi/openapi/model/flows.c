...
cJSON *OpenAPI_flows_convertToJSON(OpenAPI_flows_t *flows) {
	...
	if(flows->cont_vers) {
		...
		if(cont_versList == NULL) {
			ogs_error(
				"OpenAPI_flows_convertToJSON() failed [cont_vers]");
			goto end;
		}
		OpenAPI_list_for_each(flows->cont_vers, node) {
			if(node->data == NULL) {
	...
	if(flows->f_nums) {
		...
		if(f_numsList == NULL) {
			ogs_error(
				"OpenAPI_flows_convertToJSON() failed [f_nums]");
			goto end;
		}
		OpenAPI_list_for_each(flows->f_nums, node) {
			if(node->data == NULL) {
			...
			if(cJSON_AddNumberToObject(f_numsList, "",
			                           *(double *) node->data) ==
			   NULL)
			{
				ogs_error(
...
OpenAPI_flows_t *OpenAPI_flows_parseFromJSON(cJSON *flowsJSON) {
	...
	if(cont_vers) {
		...
		if(!cJSON_IsArray(cont_vers)) {
			ogs_error(
				"OpenAPI_flows_parseFromJSON() failed [cont_vers]");
			goto end;
		}
	...
	if(f_nums) {
		...
		if(!cJSON_IsArray(f_nums)) {
			ogs_error(
				"OpenAPI_flows_parseFromJSON() failed [f_nums]");
			goto end;
		}
...
OpenAPI_flows_t *OpenAPI_flows_copy(OpenAPI_flows_t	*dst,
                                    OpenAPI_flows_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
