...
OpenAPI_reporting_information_t *OpenAPI_reporting_information_create(
	bool is_imm_rep, int imm_rep,
	OpenAPI_notification_method_1_t *notif_method, bool is_max_report_nbr,
	int max_report_nbr, char *mon_dur, bool is_rep_period, int rep_period,
	bool is_samp_ratio, int samp_ratio, OpenAPI_list_t *partition_criteria,
	bool is_grp_rep_time, int grp_rep_time,
	OpenAPI_notification_flag_e notif_flag) {
	OpenAPI_reporting_information_t *reporting_information_local_var =
		ogs_malloc(sizeof(OpenAPI_reporting_information_t));
...
void OpenAPI_reporting_information_free(
	OpenAPI_reporting_information_t *reporting_information) {
	...
	if(reporting_information->notif_method) {
		OpenAPI_notification_method_1_free(
...
cJSON *OpenAPI_reporting_information_convertToJSON(
	OpenAPI_reporting_information_t *reporting_information) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(reporting_information == NULL) {
		ogs_error(
			"OpenAPI_reporting_information_convertToJSON() failed [ReportingInformation]");
		return NULL;
	}
	...
	if(reporting_information->is_imm_rep) {
		...
		{
			ogs_error(
				"OpenAPI_reporting_information_convertToJSON() failed [imm_rep]");
			goto end;
		}
	}
	...
	if(reporting_information->notif_method) {
		cJSON *notif_method_local_JSON =
			OpenAPI_notification_method_1_convertToJSON(
				reporting_information->notif_method);
		if(notif_method_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_reporting_information_convertToJSON() failed [notif_method]");
			goto end;
		}
	...
	if(reporting_information->is_max_report_nbr) {
		...
		{
			ogs_error(
				"OpenAPI_reporting_information_convertToJSON() failed [max_report_nbr]");
			goto end;
		}
	}
	...
	if(reporting_information->mon_dur) {
		...
		{
			ogs_error(
				"OpenAPI_reporting_information_convertToJSON() failed [mon_dur]");
			goto end;
		}
	}
	...
	if(reporting_information->is_rep_period) {
		...
		{
			ogs_error(
				"OpenAPI_reporting_information_convertToJSON() failed [rep_period]");
			goto end;
		}
	}
	...
	if(reporting_information->is_samp_ratio) {
		...
		{
			ogs_error(
				"OpenAPI_reporting_information_convertToJSON() failed [samp_ratio]");
			goto end;
		}
	}

	if(reporting_information->partition_criteria !=
	   OpenAPI_partitioning_criteria_NULL)
	{
		cJSON *partition_criteriaList = cJSON_AddArrayToObject(item,
		                                                       "partitionCriteria");
		if(partition_criteriaList == NULL) {
			ogs_error(
				"OpenAPI_reporting_information_convertToJSON() failed [partition_criteria]");
			goto end;
		}
		OpenAPI_list_for_each(reporting_information->partition_criteria,
		                      node) {
			if(cJSON_AddStringToObject(partition_criteriaList, "",
			                           OpenAPI_partitioning_criteria_ToString
			                                   ((intptr_t) node->
			                                   data)) == NULL)
			{
				ogs_error(
	...
	if(reporting_information->is_grp_rep_time) {
		...
		{
			ogs_error(
				"OpenAPI_reporting_information_convertToJSON() failed [grp_rep_time]");
			goto end;
		}
	}

	if(reporting_information->notif_flag !=
	   OpenAPI_notification_flag_NULL)
	{
		if(cJSON_AddStringToObject(item, "notifFlag",
		                           OpenAPI_notification_flag_ToString(
						   reporting_information->
						   notif_flag)) == NULL)
		{
			ogs_error(
				"OpenAPI_reporting_information_convertToJSON() failed [notif_flag]");
			goto end;
		}
	}
...
OpenAPI_reporting_information_t *OpenAPI_reporting_information_parseFromJSON(
	...
	if(imm_rep) {
		if(!cJSON_IsBool(imm_rep)) {
			ogs_error(
				"OpenAPI_reporting_information_parseFromJSON() failed [imm_rep]");
			goto end;
		}
	}

	notif_method =
		cJSON_GetObjectItemCaseSensitive(reporting_informationJSON,
		                                 "notifMethod");
	if(notif_method) {
		...
		if(!notif_method_local_nonprim) {
			ogs_error(
				"OpenAPI_notification_method_1_parseFromJSON failed [notif_method]");
			goto end;
		}
	}
	...
	if(max_report_nbr) {
		if(!cJSON_IsNumber(max_report_nbr)) {
			ogs_error(
				"OpenAPI_reporting_information_parseFromJSON() failed [max_report_nbr]");
			goto end;
		}
	}
	...
	if(mon_dur) {
		...
		{
			ogs_error(
				"OpenAPI_reporting_information_parseFromJSON() failed [mon_dur]");
			goto end;
		}
	}
	...
	if(rep_period) {
		if(!cJSON_IsNumber(rep_period)) {
			ogs_error(
				"OpenAPI_reporting_information_parseFromJSON() failed [rep_period]");
			goto end;
		}
	}
	...
	if(samp_ratio) {
		if(!cJSON_IsNumber(samp_ratio)) {
			ogs_error(
				"OpenAPI_reporting_information_parseFromJSON() failed [samp_ratio]");
			goto end;
		}
	}
	...
	if(partition_criteria) {
		...
		if(!cJSON_IsArray(partition_criteria)) {
			ogs_error(
				"OpenAPI_reporting_information_parseFromJSON() failed [partition_criteria]");
			goto end;
		}
		...
		cJSON_ArrayForEach(partition_criteria_local,
		                   partition_criteria) {
			OpenAPI_partitioning_criteria_e localEnum =
				OpenAPI_partitioning_criteria_NULL;
			...
			localEnum =
				OpenAPI_partitioning_criteria_FromString(
					partition_criteria_local->valuestring);
			if(!localEnum) {
				ogs_info(
					"Enum value \"%s\" for field \"partition_criteria\" is not supported. Ignoring it ...",
					partition_criteria_local->valuestring);
			} else {
				OpenAPI_list_add(partition_criteriaList,
				                 (void *) localEnum);
			}
		}
		if(partition_criteriaList->count == 0) {
			ogs_error(
				"OpenAPI_reporting_information_parseFromJSON() failed: Expected partition_criteriaList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(grp_rep_time) {
		if(!cJSON_IsNumber(grp_rep_time)) {
			ogs_error(
				"OpenAPI_reporting_information_parseFromJSON() failed [grp_rep_time]");
			goto end;
		}
	}
	...
	if(notif_flag) {
		if(!cJSON_IsString(notif_flag)) {
			ogs_error(
				"OpenAPI_reporting_information_parseFromJSON() failed [notif_flag]");
			goto end;
		}
		notif_flagVariable =
			OpenAPI_notification_flag_FromString(
				notif_flag->valuestring);
	}
	...
	reporting_information_local_var = OpenAPI_reporting_information_create(
		...
		mon_dur &&
		!cJSON_IsNull(mon_dur) ? ogs_strdup(mon_dur->valuestring) :
		NULL,
...
OpenAPI_reporting_information_t *OpenAPI_reporting_information_copy(
	OpenAPI_reporting_information_t *dst,
	OpenAPI_reporting_information_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_reporting_information_convertToJSON() failed");
		return NULL;
	}
