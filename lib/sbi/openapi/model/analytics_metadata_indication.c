...
OpenAPI_analytics_metadata_indication_create(
	...
	OpenAPI_analytics_metadata_indication_t *
		...
		ogs_malloc(sizeof(OpenAPI_analytics_metadata_indication_t));
...
void OpenAPI_analytics_metadata_indication_free(
	OpenAPI_analytics_metadata_indication_t *analytics_metadata_indication)
{
	...
	if(analytics_metadata_indication->aggr_nwdaf_ids) {
		OpenAPI_list_for_each(
			analytics_metadata_indication->aggr_nwdaf_ids, node) {
			ogs_free(node->data);
		}
		OpenAPI_list_free(
			analytics_metadata_indication->aggr_nwdaf_ids);
		analytics_metadata_indication->aggr_nwdaf_ids = NULL;
	}
	ogs_free(analytics_metadata_indication);
}

cJSON *OpenAPI_analytics_metadata_indication_convertToJSON(
	OpenAPI_analytics_metadata_indication_t *analytics_metadata_indication)
{
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(analytics_metadata_indication == NULL) {
		ogs_error(
			"OpenAPI_analytics_metadata_indication_convertToJSON() failed [AnalyticsMetadataIndication]");
		return NULL;
	}
	...
	if(analytics_metadata_indication->data_window) {
		...
		if(data_window_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_analytics_metadata_indication_convertToJSON() failed [data_window]");
			goto end;
		}
	...
	if(analytics_metadata_indication->data_stat_props) {
		...
		if(data_stat_propsList == NULL) {
			ogs_error(
				"OpenAPI_analytics_metadata_indication_convertToJSON() failed [data_stat_props]");
			goto end;
		}
		OpenAPI_list_for_each(
			analytics_metadata_indication->data_stat_props, node) {
	...
	if(analytics_metadata_indication->strategy) {
		...
		if(strategy_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_analytics_metadata_indication_convertToJSON() failed [strategy]");
			goto end;
		}
	...
	if(analytics_metadata_indication->aggr_nwdaf_ids) {
		...
		if(aggr_nwdaf_idsList == NULL) {
			ogs_error(
				"OpenAPI_analytics_metadata_indication_convertToJSON() failed [aggr_nwdaf_ids]");
			goto end;
		}
		OpenAPI_list_for_each(
			analytics_metadata_indication->aggr_nwdaf_ids, node) {
...
OpenAPI_analytics_metadata_indication_parseFromJSON(
	...
	OpenAPI_analytics_metadata_indication_t *
	        analytics_metadata_indication_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(data_stat_props) {
		...
		if(!cJSON_IsArray(data_stat_props)) {
			ogs_error(
				"OpenAPI_analytics_metadata_indication_parseFromJSON() failed [data_stat_props]");
			goto end;
		}
	...
	if(aggr_nwdaf_ids) {
		...
		if(!cJSON_IsArray(aggr_nwdaf_ids)) {
			ogs_error(
				"OpenAPI_analytics_metadata_indication_parseFromJSON() failed [aggr_nwdaf_ids]");
			goto end;
		}
		...
		cJSON_ArrayForEach(aggr_nwdaf_ids_local, aggr_nwdaf_ids) {
			...
			OpenAPI_list_add(aggr_nwdaf_idsList,
			                 ogs_strdup(
						 aggr_nwdaf_ids_local->
						 valuestring));
		}
	}
...
OpenAPI_analytics_metadata_indication_t *
OpenAPI_analytics_metadata_indication_copy(
	OpenAPI_analytics_metadata_indication_t *dst,
	OpenAPI_analytics_metadata_indication_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_analytics_metadata_indication_convertToJSON() failed");
		return NULL;
	}
