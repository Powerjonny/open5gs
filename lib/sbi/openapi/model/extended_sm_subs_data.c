...
OpenAPI_extended_sm_subs_data_t *OpenAPI_extended_sm_subs_data_create(
	...
	OpenAPI_extended_sm_subs_data_t *extended_sm_subs_data_local_var =
		ogs_malloc(sizeof(OpenAPI_extended_sm_subs_data_t));
	ogs_assert(extended_sm_subs_data_local_var);

	extended_sm_subs_data_local_var->shared_sm_subs_data_ids =
		shared_sm_subs_data_ids;
	extended_sm_subs_data_local_var->individual_sm_subs_data =
		individual_sm_subs_data;

	return extended_sm_subs_data_local_var;
}

void OpenAPI_extended_sm_subs_data_free(
	...
	if(extended_sm_subs_data->shared_sm_subs_data_ids) {
		OpenAPI_list_for_each(
			extended_sm_subs_data->shared_sm_subs_data_ids, node) {
			ogs_free(node->data);
		}
		OpenAPI_list_free(
			extended_sm_subs_data->shared_sm_subs_data_ids);
		extended_sm_subs_data->shared_sm_subs_data_ids = NULL;
	}
	if(extended_sm_subs_data->individual_sm_subs_data) {
		OpenAPI_list_for_each(
			extended_sm_subs_data->individual_sm_subs_data, node) {
			OpenAPI_session_management_subscription_data_free(
				node->data);
		}
		OpenAPI_list_free(
			extended_sm_subs_data->individual_sm_subs_data);
		extended_sm_subs_data->individual_sm_subs_data = NULL;
	}
	ogs_free(extended_sm_subs_data);
}

cJSON *OpenAPI_extended_sm_subs_data_convertToJSON(
	OpenAPI_extended_sm_subs_data_t *extended_sm_subs_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(extended_sm_subs_data == NULL) {
		ogs_error(
			"OpenAPI_extended_sm_subs_data_convertToJSON() failed [ExtendedSmSubsData]");
		return NULL;
	}
	...
	if(!extended_sm_subs_data->shared_sm_subs_data_ids) {
		ogs_error(
			"OpenAPI_extended_sm_subs_data_convertToJSON() failed [shared_sm_subs_data_ids]");
		return NULL;
	}
	cJSON *shared_sm_subs_data_idsList = cJSON_AddArrayToObject(item,
	                                                            "sharedSmSubsDataIds");
	...
	OpenAPI_list_for_each(extended_sm_subs_data->shared_sm_subs_data_ids,
	                      node) {
		if(cJSON_AddStringToObject(shared_sm_subs_data_idsList, "",
		                           (char *) node->data) == NULL)
		{
			ogs_error(
				"OpenAPI_extended_sm_subs_data_convertToJSON() failed [shared_sm_subs_data_ids]");
	...
	if(extended_sm_subs_data->individual_sm_subs_data) {
		cJSON *individual_sm_subs_dataList =
			cJSON_AddArrayToObject(item, "individualSmSubsData");
		if(individual_sm_subs_dataList == NULL) {
			ogs_error(
				"OpenAPI_extended_sm_subs_data_convertToJSON() failed [individual_sm_subs_data]");
			goto end;
		}
		OpenAPI_list_for_each(
			extended_sm_subs_data->individual_sm_subs_data, node) {
			...
			cJSON_AddItemToArray(individual_sm_subs_dataList,
			                     itemLocal);
		}
	}
...
OpenAPI_extended_sm_subs_data_t *OpenAPI_extended_sm_subs_data_parseFromJSON(
	cJSON *extended_sm_subs_dataJSON) {
	...
	shared_sm_subs_data_ids =
		cJSON_GetObjectItemCaseSensitive(extended_sm_subs_dataJSON,
		                                 "sharedSmSubsDataIds");
	if(!shared_sm_subs_data_ids) {
		ogs_error(
			"OpenAPI_extended_sm_subs_data_parseFromJSON() failed [shared_sm_subs_data_ids]");
		goto end;
	}
	...
	cJSON_ArrayForEach(shared_sm_subs_data_ids_local,
	                   shared_sm_subs_data_ids) {
		...
		OpenAPI_list_add(shared_sm_subs_data_idsList,
		                 ogs_strdup(
					 shared_sm_subs_data_ids_local->
					 valuestring));
	}
	...
	if(individual_sm_subs_data) {
		...
		if(!cJSON_IsArray(individual_sm_subs_data)) {
			ogs_error(
				"OpenAPI_extended_sm_subs_data_parseFromJSON() failed [individual_sm_subs_data]");
			goto end;
		}

		individual_sm_subs_dataList = OpenAPI_list_create();

		cJSON_ArrayForEach(individual_sm_subs_data_local,
		                   individual_sm_subs_data) {
			...
			OpenAPI_session_management_subscription_data_t *
				...
				OpenAPI_session_management_subscription_data_parseFromJSON
				        (individual_sm_subs_data_local);
			if(!individual_sm_subs_dataItem) {
				ogs_error("No individual_sm_subs_dataItem");
				goto end;
			}
			OpenAPI_list_add(individual_sm_subs_dataList,
			                 individual_sm_subs_dataItem);
		}
	}
...
OpenAPI_extended_sm_subs_data_t *OpenAPI_extended_sm_subs_data_copy(
	OpenAPI_extended_sm_subs_data_t *dst,
	OpenAPI_extended_sm_subs_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_extended_sm_subs_data_convertToJSON() failed");
		return NULL;
	}
