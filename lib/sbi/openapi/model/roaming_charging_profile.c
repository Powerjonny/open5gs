...
OpenAPI_roaming_charging_profile_t *OpenAPI_roaming_charging_profile_create(
	...
	OpenAPI_roaming_charging_profile_t *roaming_charging_profile_local_var =
		ogs_malloc(sizeof(OpenAPI_roaming_charging_profile_t));
	ogs_assert(roaming_charging_profile_local_var);

	roaming_charging_profile_local_var->triggers = triggers;
	roaming_charging_profile_local_var->partial_record_method =
		partial_record_method;

	return roaming_charging_profile_local_var;
}

void OpenAPI_roaming_charging_profile_free(
	...
	if(roaming_charging_profile->triggers) {
		OpenAPI_list_for_each(roaming_charging_profile->triggers,
		                      node) {
	...
	if(roaming_charging_profile->partial_record_method) {
		OpenAPI_partial_record_method_free(
...
cJSON *OpenAPI_roaming_charging_profile_convertToJSON(
	OpenAPI_roaming_charging_profile_t *roaming_charging_profile) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(roaming_charging_profile == NULL) {
		ogs_error(
			"OpenAPI_roaming_charging_profile_convertToJSON() failed [RoamingChargingProfile]");
		return NULL;
	}
	...
	if(roaming_charging_profile->triggers) {
		...
		if(triggersList == NULL) {
			ogs_error(
				"OpenAPI_roaming_charging_profile_convertToJSON() failed [triggers]");
			goto end;
		}
		OpenAPI_list_for_each(roaming_charging_profile->triggers,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_trigger_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_roaming_charging_profile_convertToJSON() failed [triggers]");
				goto end;
	...
	if(roaming_charging_profile->partial_record_method) {
		cJSON *partial_record_method_local_JSON =
			OpenAPI_partial_record_method_convertToJSON(
				roaming_charging_profile->partial_record_method);
		if(partial_record_method_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_roaming_charging_profile_convertToJSON() failed [partial_record_method]");
			goto end;
		}
		cJSON_AddItemToObject(item, "partialRecordMethod",
		                      partial_record_method_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_roaming_charging_profile_convertToJSON() failed [partial_record_method]");
			goto end;
...
OpenAPI_roaming_charging_profile_parseFromJSON(
	cJSON *roaming_charging_profileJSON) {
	OpenAPI_roaming_charging_profile_t *roaming_charging_profile_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *triggers = NULL;
	...
	OpenAPI_partial_record_method_t *partial_record_method_local_nonprim =
		NULL;
	triggers =
		cJSON_GetObjectItemCaseSensitive(roaming_charging_profileJSON,
		                                 "triggers");
	if(triggers) {
		cJSON *triggers_local = NULL;
		if(!cJSON_IsArray(triggers)) {
			ogs_error(
				"OpenAPI_roaming_charging_profile_parseFromJSON() failed [triggers]");
			goto end;
		}
		...
		cJSON_ArrayForEach(triggers_local, triggers) {
			...
			OpenAPI_trigger_t *triggersItem =
				OpenAPI_trigger_parseFromJSON(triggers_local);
			if(!triggersItem) {
	...
	if(partial_record_method) {
		partial_record_method_local_nonprim =
			OpenAPI_partial_record_method_parseFromJSON(
				partial_record_method);
		if(!partial_record_method_local_nonprim) {
			ogs_error(
				"OpenAPI_partial_record_method_parseFromJSON failed [partial_record_method]");
			goto end;
		}
	}

	roaming_charging_profile_local_var =
...
OpenAPI_roaming_charging_profile_t *OpenAPI_roaming_charging_profile_copy(
	OpenAPI_roaming_charging_profile_t	*dst,
	OpenAPI_roaming_charging_profile_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_roaming_charging_profile_convertToJSON() failed");
		return NULL;
	}
