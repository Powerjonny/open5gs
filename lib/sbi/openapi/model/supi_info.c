...
OpenAPI_supi_info_t *OpenAPI_supi_info_create(OpenAPI_list_t *supi_list) {
	OpenAPI_supi_info_t *supi_info_local_var =
		ogs_malloc(sizeof(OpenAPI_supi_info_t));
...
cJSON *OpenAPI_supi_info_convertToJSON(OpenAPI_supi_info_t *supi_info) {
	...
	if(supi_info == NULL) {
		ogs_error(
			"OpenAPI_supi_info_convertToJSON() failed [SupiInfo]");
		return NULL;
	}
	...
	if(!supi_info->supi_list) {
		ogs_error(
			"OpenAPI_supi_info_convertToJSON() failed [supi_list]");
		return NULL;
	}
...
OpenAPI_supi_info_t *OpenAPI_supi_info_parseFromJSON(cJSON *supi_infoJSON) {
	...
	if(!supi_list) {
		ogs_error(
			"OpenAPI_supi_info_parseFromJSON() failed [supi_list]");
		goto end;
	}
	...
	cJSON_ArrayForEach(supi_list_local, supi_list) {
		...
		OpenAPI_list_add(supi_listList,
		                 ogs_strdup(supi_list_local->valuestring));
	}
