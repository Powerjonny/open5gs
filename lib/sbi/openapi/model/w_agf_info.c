...
OpenAPI_w_agf_info_t *OpenAPI_w_agf_info_create(
	...
	OpenAPI_w_agf_info_t *w_agf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_w_agf_info_t));
...
void OpenAPI_w_agf_info_free(OpenAPI_w_agf_info_t *w_agf_info) {
	...
	if(w_agf_info->ipv4_endpoint_addresses) {
		OpenAPI_list_for_each(w_agf_info->ipv4_endpoint_addresses,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(w_agf_info->ipv6_endpoint_addresses) {
		OpenAPI_list_for_each(w_agf_info->ipv6_endpoint_addresses,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_w_agf_info_convertToJSON(OpenAPI_w_agf_info_t *w_agf_info) {
	...
	if(w_agf_info == NULL) {
		ogs_error(
			"OpenAPI_w_agf_info_convertToJSON() failed [WAgfInfo]");
		return NULL;
	}
	...
	if(w_agf_info->ipv4_endpoint_addresses) {
		...
		if(ipv4_endpoint_addressesList == NULL) {
			ogs_error(
				"OpenAPI_w_agf_info_convertToJSON() failed [ipv4_endpoint_addresses]");
			goto end;
		}
		OpenAPI_list_for_each(w_agf_info->ipv4_endpoint_addresses,
		                      node) {
	...
	if(w_agf_info->ipv6_endpoint_addresses) {
		...
		if(ipv6_endpoint_addressesList == NULL) {
			ogs_error(
				"OpenAPI_w_agf_info_convertToJSON() failed [ipv6_endpoint_addresses]");
			goto end;
		}
		OpenAPI_list_for_each(w_agf_info->ipv6_endpoint_addresses,
		                      node) {
	...
	if(w_agf_info->endpoint_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_w_agf_info_convertToJSON() failed [endpoint_fqdn]");
			goto end;
		}
	}
...
OpenAPI_w_agf_info_t *OpenAPI_w_agf_info_parseFromJSON(cJSON *w_agf_infoJSON) {
	...
	ipv4_endpoint_addresses =
		cJSON_GetObjectItemCaseSensitive(w_agf_infoJSON,
		                                 "ipv4EndpointAddresses");
	if(ipv4_endpoint_addresses) {
		cJSON *ipv4_endpoint_addresses_local = NULL;
		if(!cJSON_IsArray(ipv4_endpoint_addresses)) {
			ogs_error(
				"OpenAPI_w_agf_info_parseFromJSON() failed [ipv4_endpoint_addresses]");
			goto end;
		}
	...
	if(ipv6_endpoint_addresses) {
		...
		if(!cJSON_IsArray(ipv6_endpoint_addresses)) {
			ogs_error(
				"OpenAPI_w_agf_info_parseFromJSON() failed [ipv6_endpoint_addresses]");
			goto end;
		}
	...
	endpoint_fqdn = cJSON_GetObjectItemCaseSensitive(w_agf_infoJSON,
	                                                 "endpointFqdn");
	if(endpoint_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_w_agf_info_parseFromJSON() failed [endpoint_fqdn]");
			goto end;
		}
	}
...
OpenAPI_w_agf_info_t *OpenAPI_w_agf_info_copy(OpenAPI_w_agf_info_t	*dst,
                                              OpenAPI_w_agf_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
