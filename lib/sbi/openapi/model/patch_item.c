...
OpenAPI_patch_item_t *OpenAPI_patch_item_create(OpenAPI_patch_operation_e op,
                                                char *path, char *from,
                                                bool is_value_null,
                                                OpenAPI_any_type_t *value) {
	OpenAPI_patch_item_t *patch_item_local_var =
		ogs_malloc(sizeof(OpenAPI_patch_item_t));
...
cJSON *OpenAPI_patch_item_convertToJSON(OpenAPI_patch_item_t *patch_item) {
	...
	if(patch_item == NULL) {
		ogs_error(
			"OpenAPI_patch_item_convertToJSON() failed [PatchItem]");
		return NULL;
	}
	...
	if(cJSON_AddStringToObject(item, "op",
	                           OpenAPI_patch_operation_ToString(
					   patch_item->op)) == NULL)
	{
		ogs_error("OpenAPI_patch_item_convertToJSON() failed [op]");
		goto end;
	...
	if(patch_item->from) {
		...
		{
			ogs_error(
				"OpenAPI_patch_item_convertToJSON() failed [from]");
			goto end;
		}
	}
	...
	if(patch_item->value) {
		cJSON *value_object =
			OpenAPI_any_type_convertToJSON(patch_item->value);
		if(value_object == NULL) {
			ogs_error(
				"OpenAPI_patch_item_convertToJSON() failed [value]");
			goto end;
		}
...
OpenAPI_patch_item_t *OpenAPI_patch_item_parseFromJSON(cJSON *patch_itemJSON) {
	...
	if(from) {
		...
		{
			ogs_error(
				"OpenAPI_patch_item_parseFromJSON() failed [from]");
			goto end;
		}
	}
	...
	if(value) {
		if(!cJSON_IsNull(value)) {
			value_local_object =
				OpenAPI_any_type_parseFromJSON(value);
		}
	}
...
OpenAPI_patch_item_t *OpenAPI_patch_item_copy(OpenAPI_patch_item_t	*dst,
                                              OpenAPI_patch_item_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
