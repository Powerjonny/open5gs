...
OpenAPI_slice_policy_data_t *OpenAPI_slice_policy_data_create(char	*mbr_ul,
                                                              ...
                                                              OpenAPI_list_t
                                                                *reset_ids) {
	OpenAPI_slice_policy_data_t *slice_policy_data_local_var =
		ogs_malloc(sizeof(OpenAPI_slice_policy_data_t));
...
cJSON *OpenAPI_slice_policy_data_convertToJSON(
	...
	if(slice_policy_data == NULL) {
		ogs_error(
			"OpenAPI_slice_policy_data_convertToJSON() failed [SlicePolicyData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(slice_policy_data->mbr_ul) {
		if(cJSON_AddStringToObject(item, "mbrUl",
		                           slice_policy_data->mbr_ul) == NULL)
		{
			ogs_error(
				"OpenAPI_slice_policy_data_convertToJSON() failed [mbr_ul]");
			goto end;
		}
	}

	if(slice_policy_data->mbr_dl) {
		if(cJSON_AddStringToObject(item, "mbrDl",
		                           slice_policy_data->mbr_dl) == NULL)
		{
			ogs_error(
				"OpenAPI_slice_policy_data_convertToJSON() failed [mbr_dl]");
			goto end;
		}
	}

	if(slice_policy_data->remain_mbr_ul) {
		if(cJSON_AddStringToObject(item, "remainMbrUl",
		                           slice_policy_data->remain_mbr_ul) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_slice_policy_data_convertToJSON() failed [remain_mbr_ul]");
			goto end;
		}
	}

	if(slice_policy_data->remain_mbr_dl) {
		if(cJSON_AddStringToObject(item, "remainMbrDl",
		                           slice_policy_data->remain_mbr_dl) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_slice_policy_data_convertToJSON() failed [remain_mbr_dl]");
			goto end;
		}
	}
	...
	if(slice_policy_data->supp_feat) {
		if(cJSON_AddStringToObject(item, "suppFeat",
		                           slice_policy_data->supp_feat) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_slice_policy_data_convertToJSON() failed [supp_feat]");
			goto end;
		}
	}
	...
	if(slice_policy_data->reset_ids) {
		...
		if(reset_idsList == NULL) {
			ogs_error(
				"OpenAPI_slice_policy_data_convertToJSON() failed [reset_ids]");
			goto end;
		}
		OpenAPI_list_for_each(slice_policy_data->reset_ids, node) {
...
OpenAPI_slice_policy_data_t *OpenAPI_slice_policy_data_parseFromJSON(
	...
	mbr_ul = cJSON_GetObjectItemCaseSensitive(slice_policy_dataJSON,
	                                          "mbrUl");
	if(mbr_ul) {
		if(!cJSON_IsString(mbr_ul) &&
		   !cJSON_IsNull(mbr_ul))
		{
			ogs_error(
				"OpenAPI_slice_policy_data_parseFromJSON() failed [mbr_ul]");
			goto end;
		}
	}

	mbr_dl = cJSON_GetObjectItemCaseSensitive(slice_policy_dataJSON,
	                                          "mbrDl");
	if(mbr_dl) {
		if(!cJSON_IsString(mbr_dl) &&
		   !cJSON_IsNull(mbr_dl))
		{
			ogs_error(
				"OpenAPI_slice_policy_data_parseFromJSON() failed [mbr_dl]");
			goto end;
		}
	}

	remain_mbr_ul = cJSON_GetObjectItemCaseSensitive(slice_policy_dataJSON,
	                                                 "remainMbrUl");
	if(remain_mbr_ul) {
		if(!cJSON_IsString(remain_mbr_ul) &&
		   !cJSON_IsNull(remain_mbr_ul))
		{
			ogs_error(
				"OpenAPI_slice_policy_data_parseFromJSON() failed [remain_mbr_ul]");
			goto end;
		}
	}

	remain_mbr_dl = cJSON_GetObjectItemCaseSensitive(slice_policy_dataJSON,
	                                                 "remainMbrDl");
	if(remain_mbr_dl) {
		if(!cJSON_IsString(remain_mbr_dl) &&
		   !cJSON_IsNull(remain_mbr_dl))
		{
			ogs_error(
				"OpenAPI_slice_policy_data_parseFromJSON() failed [remain_mbr_dl]");
			goto end;
		}
	}

	supp_feat = cJSON_GetObjectItemCaseSensitive(slice_policy_dataJSON,
	                                             "suppFeat");
	if(supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_slice_policy_data_parseFromJSON() failed [supp_feat]");
			goto end;
		}
	}

	reset_ids = cJSON_GetObjectItemCaseSensitive(slice_policy_dataJSON,
	                                             "resetIds");
	if(reset_ids) {
		cJSON *reset_ids_local = NULL;
		if(!cJSON_IsArray(reset_ids)) {
			ogs_error(
				"OpenAPI_slice_policy_data_parseFromJSON() failed [reset_ids]");
			goto end;
		}
	...
	slice_policy_data_local_var = OpenAPI_slice_policy_data_create(
		mbr_ul &&
		!cJSON_IsNull(mbr_ul) ? ogs_strdup(mbr_ul->valuestring) : NULL,
		mbr_dl &&
		!cJSON_IsNull(mbr_dl) ? ogs_strdup(mbr_dl->valuestring) : NULL,
		remain_mbr_ul &&
		!cJSON_IsNull(remain_mbr_ul) ?
		ogs_strdup(remain_mbr_ul->valuestring) : NULL,
		remain_mbr_dl &&
		!cJSON_IsNull(remain_mbr_dl) ?
		ogs_strdup(remain_mbr_dl->valuestring) : NULL,
		supp_feat &&
