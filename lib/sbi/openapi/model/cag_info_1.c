...
OpenAPI_cag_info_1_t *OpenAPI_cag_info_1_create(
	OpenAPI_list_t *allowed_cag_list, bool is_cag_only_indicator,
	int cag_only_indicator) {
	OpenAPI_cag_info_1_t *cag_info_1_local_var =
...
cJSON *OpenAPI_cag_info_1_convertToJSON(OpenAPI_cag_info_1_t *cag_info_1) {
	...
	if(cag_info_1 == NULL) {
		ogs_error(
			"OpenAPI_cag_info_1_convertToJSON() failed [CagInfo_1]");
		return NULL;
	}
	...
	if(!cag_info_1->allowed_cag_list) {
		ogs_error(
			"OpenAPI_cag_info_1_convertToJSON() failed [allowed_cag_list]");
		return NULL;
	}
	...
	if(cag_info_1->is_cag_only_indicator) {
		if(cJSON_AddBoolToObject(item, "cagOnlyIndicator",
		                         cag_info_1->cag_only_indicator) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_cag_info_1_convertToJSON() failed [cag_only_indicator]");
			goto end;
		}
	}
...
OpenAPI_cag_info_1_t *OpenAPI_cag_info_1_parseFromJSON(cJSON *cag_info_1JSON) {
	...
	allowed_cag_list = cJSON_GetObjectItemCaseSensitive(cag_info_1JSON,
	                                                    "allowedCagList");
	if(!allowed_cag_list) {
		ogs_error(
	...
	cJSON_ArrayForEach(allowed_cag_list_local, allowed_cag_list) {
		...
		OpenAPI_list_add(allowed_cag_listList,
		                 ogs_strdup(
					 allowed_cag_list_local->valuestring));
	}

	cag_only_indicator = cJSON_GetObjectItemCaseSensitive(cag_info_1JSON,
	                                                      "cagOnlyIndicator");
	if(cag_only_indicator) {
		if(!cJSON_IsBool(cag_only_indicator)) {
			ogs_error(
				"OpenAPI_cag_info_1_parseFromJSON() failed [cag_only_indicator]");
			goto end;
		}
	}
...
OpenAPI_cag_info_1_t *OpenAPI_cag_info_1_copy(OpenAPI_cag_info_1_t	*dst,
                                              OpenAPI_cag_info_1_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
