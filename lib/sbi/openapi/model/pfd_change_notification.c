...
OpenAPI_pfd_change_notification_t *OpenAPI_pfd_change_notification_create(
	char *application_id, bool is_removal_flag, int removal_flag,
	bool is_partial_flag, int partial_flag, OpenAPI_list_t *pfds) {
	OpenAPI_pfd_change_notification_t *pfd_change_notification_local_var =
		ogs_malloc(sizeof(OpenAPI_pfd_change_notification_t));
...
void OpenAPI_pfd_change_notification_free(
	OpenAPI_pfd_change_notification_t *pfd_change_notification) {
...
cJSON *OpenAPI_pfd_change_notification_convertToJSON(
	OpenAPI_pfd_change_notification_t *pfd_change_notification) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(pfd_change_notification == NULL) {
		ogs_error(
			"OpenAPI_pfd_change_notification_convertToJSON() failed [PfdChangeNotification]");
		return NULL;
	}
	...
	if(!pfd_change_notification->application_id) {
		ogs_error(
			"OpenAPI_pfd_change_notification_convertToJSON() failed [application_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "applicationId",
	                           pfd_change_notification->application_id) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_pfd_change_notification_convertToJSON() failed [application_id]");
	...
	if(pfd_change_notification->is_removal_flag) {
		if(cJSON_AddBoolToObject(item, "removalFlag",
		                         pfd_change_notification->removal_flag)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_pfd_change_notification_convertToJSON() failed [removal_flag]");
			goto end;
		}
	}

	if(pfd_change_notification->is_partial_flag) {
		if(cJSON_AddBoolToObject(item, "partialFlag",
		                         pfd_change_notification->partial_flag)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_pfd_change_notification_convertToJSON() failed [partial_flag]");
			goto end;
		}
	}
	...
	if(pfd_change_notification->pfds) {
		...
		if(pfdsList == NULL) {
			ogs_error(
				"OpenAPI_pfd_change_notification_convertToJSON() failed [pfds]");
			goto end;
		}
		OpenAPI_list_for_each(pfd_change_notification->pfds, node) {
			cJSON *itemLocal =
...
OpenAPI_pfd_change_notification_t *OpenAPI_pfd_change_notification_parseFromJSON
        (cJSON *pfd_change_notificationJSON) {
	OpenAPI_pfd_change_notification_t *pfd_change_notification_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	application_id =
		cJSON_GetObjectItemCaseSensitive(pfd_change_notificationJSON,
		                                 "applicationId");
	if(!application_id) {
		ogs_error(
			"OpenAPI_pfd_change_notification_parseFromJSON() failed [application_id]");
		goto end;
	}
	...
	if(removal_flag) {
		if(!cJSON_IsBool(removal_flag)) {
			ogs_error(
				"OpenAPI_pfd_change_notification_parseFromJSON() failed [removal_flag]");
			goto end;
		}
	}
	...
	if(partial_flag) {
		if(!cJSON_IsBool(partial_flag)) {
			ogs_error(
				"OpenAPI_pfd_change_notification_parseFromJSON() failed [partial_flag]");
			goto end;
		}
	}

	pfds = cJSON_GetObjectItemCaseSensitive(pfd_change_notificationJSON,
	                                        "pfds");
	if(pfds) {
		cJSON *pfds_local = NULL;
		if(!cJSON_IsArray(pfds)) {
			ogs_error(
				"OpenAPI_pfd_change_notification_parseFromJSON() failed [pfds]");
			goto end;
		}
...
OpenAPI_pfd_change_notification_t *OpenAPI_pfd_change_notification_copy(
	OpenAPI_pfd_change_notification_t	*dst,
	OpenAPI_pfd_change_notification_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_pfd_change_notification_convertToJSON() failed");
		return NULL;
	}
