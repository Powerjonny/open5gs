...
OpenAPI_context_info_t *OpenAPI_context_info_create(
	OpenAPI_list_t *orig_headers, OpenAPI_list_t *request_headers) {
	OpenAPI_context_info_t *context_info_local_var =
		ogs_malloc(sizeof(OpenAPI_context_info_t));
...
cJSON *OpenAPI_context_info_convertToJSON(
	...
	if(context_info == NULL) {
		ogs_error(
			"OpenAPI_context_info_convertToJSON() failed [ContextInfo]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(context_info->orig_headers) {
		cJSON *orig_headersList = cJSON_AddArrayToObject(item,
		                                                 "origHeaders");
		if(orig_headersList == NULL) {
			ogs_error(
				"OpenAPI_context_info_convertToJSON() failed [orig_headers]");
			goto end;
		}
		OpenAPI_list_for_each(context_info->orig_headers, node) {
			if(cJSON_AddStringToObject(orig_headersList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_context_info_convertToJSON() failed [orig_headers]");
	...
	if(context_info->request_headers) {
		cJSON *request_headersList = cJSON_AddArrayToObject(item,
		                                                    "requestHeaders");
		if(request_headersList == NULL) {
			ogs_error(
				"OpenAPI_context_info_convertToJSON() failed [request_headers]");
			goto end;
		}
		OpenAPI_list_for_each(context_info->request_headers, node) {
			if(cJSON_AddStringToObject(request_headersList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_context_info_convertToJSON() failed [request_headers]");
...
OpenAPI_context_info_t *OpenAPI_context_info_parseFromJSON(
	cJSON *context_infoJSON) {
	...
	orig_headers = cJSON_GetObjectItemCaseSensitive(context_infoJSON,
	                                                "origHeaders");
	if(orig_headers) {
		cJSON *orig_headers_local = NULL;
		if(!cJSON_IsArray(orig_headers)) {
			ogs_error(
				"OpenAPI_context_info_parseFromJSON() failed [orig_headers]");
			goto end;
		}
		...
		cJSON_ArrayForEach(orig_headers_local, orig_headers) {
			...
			OpenAPI_list_add(orig_headersList,
			                 ogs_strdup(
						 orig_headers_local->valuestring));
		}
	}

	request_headers = cJSON_GetObjectItemCaseSensitive(context_infoJSON,
	                                                   "requestHeaders");
	if(request_headers) {
		cJSON *request_headers_local = NULL;
		if(!cJSON_IsArray(request_headers)) {
			ogs_error(
				"OpenAPI_context_info_parseFromJSON() failed [request_headers]");
			goto end;
		}
		...
		cJSON_ArrayForEach(request_headers_local, request_headers) {
			...
			OpenAPI_list_add(request_headersList,
			                 ogs_strdup(
...
OpenAPI_context_info_t *OpenAPI_context_info_copy(
	OpenAPI_context_info_t	*dst,
