...
OpenAPI_app_detection_info_t *OpenAPI_app_detection_info_create(char *app_id,
                                                                char *
                                                                instance_id,
                                                                OpenAPI_list_t *
                                                                sdf_descriptions)
{
	OpenAPI_app_detection_info_t *app_detection_info_local_var =
		ogs_malloc(sizeof(OpenAPI_app_detection_info_t));
...
void OpenAPI_app_detection_info_free(
	OpenAPI_app_detection_info_t *app_detection_info) {
	...
	if(app_detection_info->sdf_descriptions) {
		OpenAPI_list_for_each(app_detection_info->sdf_descriptions,
		                      node) {
...
cJSON *OpenAPI_app_detection_info_convertToJSON(
	OpenAPI_app_detection_info_t *app_detection_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(app_detection_info == NULL) {
		ogs_error(
			"OpenAPI_app_detection_info_convertToJSON() failed [AppDetectionInfo]");
		return NULL;
	}
	...
	if(!app_detection_info->app_id) {
		ogs_error(
			"OpenAPI_app_detection_info_convertToJSON() failed [app_id]");
		return NULL;
	}
	...
	if(app_detection_info->instance_id) {
		if(cJSON_AddStringToObject(item, "instanceId",
		                           app_detection_info->instance_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_app_detection_info_convertToJSON() failed [instance_id]");
			goto end;
		}
	}

	if(app_detection_info->sdf_descriptions) {
		cJSON *sdf_descriptionsList = cJSON_AddArrayToObject(item,
		                                                     "sdfDescriptions");
		if(sdf_descriptionsList == NULL) {
			ogs_error(
				"OpenAPI_app_detection_info_convertToJSON() failed [sdf_descriptions]");
			goto end;
		}
		OpenAPI_list_for_each(app_detection_info->sdf_descriptions,
		                      node) {
...
OpenAPI_app_detection_info_t *OpenAPI_app_detection_info_parseFromJSON(
	cJSON *app_detection_infoJSON) {
	...
	app_id = cJSON_GetObjectItemCaseSensitive(app_detection_infoJSON,
	                                          "appId");
	if(!app_id) {
		ogs_error(
			"OpenAPI_app_detection_info_parseFromJSON() failed [app_id]");
		goto end;
	}
	...
	instance_id = cJSON_GetObjectItemCaseSensitive(app_detection_infoJSON,
	                                               "instanceId");
	if(instance_id) {
		if(!cJSON_IsString(instance_id) &&
		   !cJSON_IsNull(instance_id))
		{
			ogs_error(
				"OpenAPI_app_detection_info_parseFromJSON() failed [instance_id]");
			goto end;
		}
	}

	sdf_descriptions =
		cJSON_GetObjectItemCaseSensitive(app_detection_infoJSON,
		                                 "sdfDescriptions");
	if(sdf_descriptions) {
		cJSON *sdf_descriptions_local = NULL;
		if(!cJSON_IsArray(sdf_descriptions)) {
			ogs_error(
				"OpenAPI_app_detection_info_parseFromJSON() failed [sdf_descriptions]");
			goto end;
		}
		...
		cJSON_ArrayForEach(sdf_descriptions_local, sdf_descriptions) {
			...
			OpenAPI_flow_information_t *sdf_descriptionsItem =
				OpenAPI_flow_information_parseFromJSON(
					sdf_descriptions_local);
			if(!sdf_descriptionsItem) {
				ogs_error("No sdf_descriptionsItem");
				goto end;
			}
			OpenAPI_list_add(sdf_descriptionsList,
			                 sdf_descriptionsItem);
		}
	}
	...
	app_detection_info_local_var = OpenAPI_app_detection_info_create(
		...
		instance_id &&
		!cJSON_IsNull(instance_id) ?
		ogs_strdup(instance_id->valuestring) : NULL,
...
OpenAPI_app_detection_info_t *OpenAPI_app_detection_info_copy(
	OpenAPI_app_detection_info_t *dst, OpenAPI_app_detection_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
