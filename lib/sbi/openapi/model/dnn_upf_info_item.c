...
OpenAPI_dnn_upf_info_item_t *OpenAPI_dnn_upf_info_item_create(char *dnn,
                                                              OpenAPI_list_t *
                                                              dnai_list,
                                                              OpenAPI_list_t *
                                                              pdu_session_types,
...
{
	OpenAPI_dnn_upf_info_item_t *dnn_upf_info_item_local_var =
		ogs_malloc(sizeof(OpenAPI_dnn_upf_info_item_t));
	...
	dnn_upf_info_item_local_var->dnai_nw_instance_list =
		dnai_nw_instance_list;

	return dnn_upf_info_item_local_var;
}

void OpenAPI_dnn_upf_info_item_free(
	OpenAPI_dnn_upf_info_item_t *dnn_upf_info_item) {
	...
	if(dnn_upf_info_item->ipv4_address_ranges) {
		OpenAPI_list_for_each(dnn_upf_info_item->ipv4_address_ranges,
		                      node) {
	...
	if(dnn_upf_info_item->ipv6_prefix_ranges) {
		OpenAPI_list_for_each(dnn_upf_info_item->ipv6_prefix_ranges,
		                      node) {
	...
	if(dnn_upf_info_item->ipv6_index_list) {
		OpenAPI_list_for_each(dnn_upf_info_item->ipv6_index_list,
		                      node) {
			OpenAPI_ip_index_free(node->data);
		}
	...
	if(dnn_upf_info_item->dnai_nw_instance_list) {
		OpenAPI_list_for_each(dnn_upf_info_item->dnai_nw_instance_list,
		                      node) {
...
cJSON *OpenAPI_dnn_upf_info_item_convertToJSON(
	OpenAPI_dnn_upf_info_item_t *dnn_upf_info_item) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(dnn_upf_info_item == NULL) {
		ogs_error(
			"OpenAPI_dnn_upf_info_item_convertToJSON() failed [DnnUpfInfoItem]");
		return NULL;
	}
	...
	if(!dnn_upf_info_item->dnn) {
		ogs_error(
			"OpenAPI_dnn_upf_info_item_convertToJSON() failed [dnn]");
		return NULL;
	}
	...
	if(dnn_upf_info_item->dnai_list) {
		...
		if(dnai_listList == NULL) {
			ogs_error(
				"OpenAPI_dnn_upf_info_item_convertToJSON() failed [dnai_list]");
			goto end;
		}
		OpenAPI_list_for_each(dnn_upf_info_item->dnai_list, node) {
	   ...
	   OpenAPI_pdu_session_type_NULL)
	{
		...
		if(pdu_session_typesList == NULL) {
			ogs_error(
				"OpenAPI_dnn_upf_info_item_convertToJSON() failed [pdu_session_types]");
			goto end;
		}
		OpenAPI_list_for_each(dnn_upf_info_item->pdu_session_types,
		                      node) {
	...
	if(dnn_upf_info_item->ipv4_address_ranges) {
		...
		if(ipv4_address_rangesList == NULL) {
			ogs_error(
				"OpenAPI_dnn_upf_info_item_convertToJSON() failed [ipv4_address_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(dnn_upf_info_item->ipv4_address_ranges,
		                      node) {
	...
	if(dnn_upf_info_item->ipv6_prefix_ranges) {
		...
		if(ipv6_prefix_rangesList == NULL) {
			ogs_error(
				"OpenAPI_dnn_upf_info_item_convertToJSON() failed [ipv6_prefix_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(dnn_upf_info_item->ipv6_prefix_ranges,
		                      node) {
	...
	if(dnn_upf_info_item->ipv4_index_list) {
		cJSON *ipv4_index_listList = cJSON_AddArrayToObject(item,
		                                                    "ipv4IndexList");
		if(ipv4_index_listList == NULL) {
			ogs_error(
				"OpenAPI_dnn_upf_info_item_convertToJSON() failed [ipv4_index_list]");
			goto end;
		}
		OpenAPI_list_for_each(dnn_upf_info_item->ipv4_index_list,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_ip_index_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_dnn_upf_info_item_convertToJSON() failed [ipv4_index_list]");
				goto end;
	...
	if(dnn_upf_info_item->ipv6_index_list) {
		cJSON *ipv6_index_listList = cJSON_AddArrayToObject(item,
		                                                    "ipv6IndexList");
		if(ipv6_index_listList == NULL) {
			ogs_error(
				"OpenAPI_dnn_upf_info_item_convertToJSON() failed [ipv6_index_list]");
			goto end;
		}
		OpenAPI_list_for_each(dnn_upf_info_item->ipv6_index_list,
		                      node) {
	...
	if(dnn_upf_info_item->dnai_nw_instance_list) {
		cJSON *dnai_nw_instance_list = cJSON_AddObjectToObject(item,
		                                                       "dnaiNwInstanceList");
		if(dnai_nw_instance_list == NULL) {
			ogs_error(
				"OpenAPI_dnn_upf_info_item_convertToJSON() failed [dnai_nw_instance_list]");
			goto end;
		}
		cJSON *localMapObject = dnai_nw_instance_list;
		if(dnn_upf_info_item->dnai_nw_instance_list) {
			OpenAPI_list_for_each(
				dnn_upf_info_item->dnai_nw_instance_list,
				node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				{
					ogs_error(
						"OpenAPI_dnn_upf_info_item_convertToJSON() failed [inner]");
					goto end;
				}
			}
		}
...
OpenAPI_dnn_upf_info_item_t *OpenAPI_dnn_upf_info_item_parseFromJSON(
	cJSON *dnn_upf_info_itemJSON) {
	...
	if(!dnn) {
		ogs_error(
			"OpenAPI_dnn_upf_info_item_parseFromJSON() failed [dnn]");
		goto end;
	}
	...
	dnai_list = cJSON_GetObjectItemCaseSensitive(dnn_upf_info_itemJSON,
	                                             "dnaiList");
	if(dnai_list) {
		cJSON *dnai_list_local = NULL;
		if(!cJSON_IsArray(dnai_list)) {
			ogs_error(
				"OpenAPI_dnn_upf_info_item_parseFromJSON() failed [dnai_list]");
			goto end;
		}
	...
	pdu_session_types =
		cJSON_GetObjectItemCaseSensitive(dnn_upf_info_itemJSON,
		                                 "pduSessionTypes");
	if(pdu_session_types) {
		cJSON *pdu_session_types_local = NULL;
		if(!cJSON_IsArray(pdu_session_types)) {
			ogs_error(
				"OpenAPI_dnn_upf_info_item_parseFromJSON() failed [pdu_session_types]");
			goto end;
		}
		...
		if(pdu_session_typesList->count == 0) {
			ogs_error(
				"OpenAPI_dnn_upf_info_item_parseFromJSON() failed: Expected pdu_session_typesList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(ipv4_address_ranges) {
		...
		if(!cJSON_IsArray(ipv4_address_ranges)) {
			ogs_error(
				"OpenAPI_dnn_upf_info_item_parseFromJSON() failed [ipv4_address_ranges]");
			goto end;
		}
	...
	if(ipv6_prefix_ranges) {
		...
		if(!cJSON_IsArray(ipv6_prefix_ranges)) {
			ogs_error(
				"OpenAPI_dnn_upf_info_item_parseFromJSON() failed [ipv6_prefix_ranges]");
			goto end;
		}
	...
	if(ipv4_index_list) {
		...
		if(!cJSON_IsArray(ipv4_index_list)) {
			ogs_error(
				"OpenAPI_dnn_upf_info_item_parseFromJSON() failed [ipv4_index_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ipv4_index_list_local, ipv4_index_list) {
			...
			OpenAPI_ip_index_t *ipv4_index_listItem =
				OpenAPI_ip_index_parseFromJSON(
					ipv4_index_list_local);
			if(!ipv4_index_listItem) {
				ogs_error("No ipv4_index_listItem");
				goto end;
			}
			OpenAPI_list_add(ipv4_index_listList,
			                 ipv4_index_listItem);
		}
	}
	...
	if(ipv6_index_list) {
		...
		if(!cJSON_IsArray(ipv6_index_list)) {
			ogs_error(
				"OpenAPI_dnn_upf_info_item_parseFromJSON() failed [ipv6_index_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ipv6_index_list_local, ipv6_index_list) {
			...
			OpenAPI_ip_index_t *ipv6_index_listItem =
				OpenAPI_ip_index_parseFromJSON(
					ipv6_index_list_local);
			if(!ipv6_index_listItem) {
				ogs_error("No ipv6_index_listItem");
				goto end;
			}
			OpenAPI_list_add(ipv6_index_listList,
			                 ipv6_index_listItem);
		}
	}
	...
	if(dnai_nw_instance_list) {
		...
		if(!cJSON_IsObject(dnai_nw_instance_list) &&
		   !cJSON_IsNull(dnai_nw_instance_list))
		{
			ogs_error(
				"OpenAPI_dnn_upf_info_item_parseFromJSON() failed [dnai_nw_instance_list]");
			goto end;
		}
		if(cJSON_IsObject(dnai_nw_instance_list)) {
			dnai_nw_instance_listList = OpenAPI_list_create();
			OpenAPI_map_t *localMapKeyPair = NULL;
			cJSON_ArrayForEach(dnai_nw_instance_list_local_map,
			                   dnai_nw_instance_list) {
				cJSON *localMapObject =
					dnai_nw_instance_list_local_map;
				double *localDouble = NULL;
				int *localInt = NULL;
				if(!cJSON_IsString(localMapObject)) {
					ogs_error(
						"OpenAPI_dnn_upf_info_item_parseFromJSON() failed [inner]");
					goto end;
				}
				localMapKeyPair =
					OpenAPI_map_create(ogs_strdup(
								   ...
								   localMapObject
								   ->valuestring));
				OpenAPI_list_add(dnai_nw_instance_listList,
				                 localMapKeyPair);
			}
		}
	}
...
OpenAPI_dnn_upf_info_item_t *OpenAPI_dnn_upf_info_item_copy(
	OpenAPI_dnn_upf_info_item_t *dst, OpenAPI_dnn_upf_info_item_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
