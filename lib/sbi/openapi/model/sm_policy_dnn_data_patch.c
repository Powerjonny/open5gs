...
OpenAPI_sm_policy_dnn_data_patch_t *OpenAPI_sm_policy_dnn_data_patch_create(
	...
	OpenAPI_sm_policy_dnn_data_patch_t *sm_policy_dnn_data_patch_local_var =
		ogs_malloc(sizeof(OpenAPI_sm_policy_dnn_data_patch_t));
	ogs_assert(sm_policy_dnn_data_patch_local_var);

	sm_policy_dnn_data_patch_local_var->dnn = dnn;
	sm_policy_dnn_data_patch_local_var->is_bdt_ref_ids_null =
		is_bdt_ref_ids_null;
...
cJSON *OpenAPI_sm_policy_dnn_data_patch_convertToJSON(
	...
	if(sm_policy_dnn_data_patch == NULL) {
		ogs_error(
			"OpenAPI_sm_policy_dnn_data_patch_convertToJSON() failed [SmPolicyDnnDataPatch]");
		return NULL;
	}
	...
	if(!sm_policy_dnn_data_patch->dnn) {
		ogs_error(
			"OpenAPI_sm_policy_dnn_data_patch_convertToJSON() failed [dnn]");
		return NULL;
	}
	...
	if(sm_policy_dnn_data_patch->bdt_ref_ids) {
		...
		if(bdt_ref_ids == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_dnn_data_patch_convertToJSON() failed [bdt_ref_ids]");
			goto end;
		}
		cJSON *localMapObject = bdt_ref_ids;
		if(sm_policy_dnn_data_patch->bdt_ref_ids) {
			OpenAPI_list_for_each(
				sm_policy_dnn_data_patch->bdt_ref_ids, node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				{
					ogs_error(
						"OpenAPI_sm_policy_dnn_data_patch_convertToJSON() failed [inner]");
					goto end;
				}
			}
		}
...
OpenAPI_sm_policy_dnn_data_patch_parseFromJSON(
	...
	dnn = cJSON_GetObjectItemCaseSensitive(sm_policy_dnn_data_patchJSON,
	                                       "dnn");
	if(!dnn) {
		ogs_error(
			"OpenAPI_sm_policy_dnn_data_patch_parseFromJSON() failed [dnn]");
		goto end;
	}
	...
	bdt_ref_ids =
		cJSON_GetObjectItemCaseSensitive(sm_policy_dnn_data_patchJSON,
		                                 "bdtRefIds");
	if(bdt_ref_ids) {
		if(!cJSON_IsNull(bdt_ref_ids)) {
			...
			{
				ogs_error(
					"OpenAPI_sm_policy_dnn_data_patch_parseFromJSON() failed [bdt_ref_ids]");
				goto end;
			}
			if(cJSON_IsObject(bdt_ref_ids)) {
				...
				cJSON_ArrayForEach(bdt_ref_ids_local_map,
					...
					if(!cJSON_IsString(localMapObject)) {
						ogs_error(
							"OpenAPI_sm_policy_dnn_data_patch_parseFromJSON() failed [inner]");
						goto end;
					}
					localMapKeyPair =
	...
	sm_policy_dnn_data_patch_local_var =
...
OpenAPI_sm_policy_dnn_data_patch_t *OpenAPI_sm_policy_dnn_data_patch_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_sm_policy_dnn_data_patch_convertToJSON() failed");
		return NULL;
	}
