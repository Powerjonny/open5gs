...
OpenAPI_ue_slice_mbr_t *OpenAPI_ue_slice_mbr_create(OpenAPI_list_t *slice_mbr,
                                                    ...
                                                    OpenAPI_snssai_t *
                                                    mapped_home_snssai) {
	OpenAPI_ue_slice_mbr_t *ue_slice_mbr_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_slice_mbr_t));
...
cJSON *OpenAPI_ue_slice_mbr_convertToJSON(
	OpenAPI_ue_slice_mbr_t *ue_slice_mbr) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(ue_slice_mbr == NULL) {
		ogs_error(
			"OpenAPI_ue_slice_mbr_convertToJSON() failed [UeSliceMbr]");
		return NULL;
	}
	...
	if(!ue_slice_mbr->slice_mbr) {
		ogs_error(
			"OpenAPI_ue_slice_mbr_convertToJSON() failed [slice_mbr]");
		return NULL;
	}
	...
	if(ue_slice_mbr->slice_mbr) {
		OpenAPI_list_for_each(ue_slice_mbr->slice_mbr, node) {
			...
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_ue_slice_mbr_convertToJSON() failed [inner]");
				goto end;
			}
	...
	if(!ue_slice_mbr->serving_snssai) {
		ogs_error(
			"OpenAPI_ue_slice_mbr_convertToJSON() failed [serving_snssai]");
		return NULL;
	}
	cJSON *serving_snssai_local_JSON =
		OpenAPI_snssai_convertToJSON(ue_slice_mbr->serving_snssai);
	...
	if(ue_slice_mbr->mapped_home_snssai) {
		cJSON *mapped_home_snssai_local_JSON =
			OpenAPI_snssai_convertToJSON(
				ue_slice_mbr->mapped_home_snssai);
		if(mapped_home_snssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_slice_mbr_convertToJSON() failed [mapped_home_snssai]");
			goto end;
		}
...
OpenAPI_ue_slice_mbr_t *OpenAPI_ue_slice_mbr_parseFromJSON(
	...
	slice_mbr = cJSON_GetObjectItemCaseSensitive(ue_slice_mbrJSON,
	                                             "sliceMbr");
	if(!slice_mbr) {
		ogs_error(
			"OpenAPI_ue_slice_mbr_parseFromJSON() failed [slice_mbr]");
		goto end;
	}
	cJSON *slice_mbr_local_map = NULL;
	if(!cJSON_IsObject(slice_mbr) &&
	   !cJSON_IsNull(slice_mbr))
	{
		ogs_error(
			"OpenAPI_ue_slice_mbr_parseFromJSON() failed [slice_mbr]");
		goto end;
	...
	if(cJSON_IsObject(slice_mbr)) {
		...
		cJSON_ArrayForEach(slice_mbr_local_map, slice_mbr) {
			...
			if(cJSON_IsObject(localMapObject)) {
				...
				ogs_error(
					"OpenAPI_ue_slice_mbr_parseFromJSON() failed [inner]");
				goto end;
			}
	...
	serving_snssai = cJSON_GetObjectItemCaseSensitive(ue_slice_mbrJSON,
	                                                  "servingSnssai");
	if(!serving_snssai) {
		ogs_error(
			"OpenAPI_ue_slice_mbr_parseFromJSON() failed [serving_snssai]");
		goto end;
	}
	serving_snssai_local_nonprim =
		OpenAPI_snssai_parseFromJSON(serving_snssai);
	if(!serving_snssai_local_nonprim) {
		ogs_error(
			"OpenAPI_snssai_parseFromJSON failed [serving_snssai]");
		goto end;
	}

	mapped_home_snssai = cJSON_GetObjectItemCaseSensitive(ue_slice_mbrJSON,
	                                                      "mappedHomeSnssai");
