...
OpenAPI_nwdaf_registration_modification_create(
	...
	supported_features) {
	OpenAPI_nwdaf_registration_modification_t *
	        nwdaf_registration_modification_local_var =
		ogs_malloc(sizeof(OpenAPI_nwdaf_registration_modification_t));
	ogs_assert(nwdaf_registration_modification_local_var);

	nwdaf_registration_modification_local_var->nwdaf_instance_id =
		nwdaf_instance_id;
	nwdaf_registration_modification_local_var->nwdaf_set_id = nwdaf_set_id;
	nwdaf_registration_modification_local_var->analytics_ids =
		analytics_ids;
	nwdaf_registration_modification_local_var->supported_features =
		supported_features;
...
void OpenAPI_nwdaf_registration_modification_free(
	...
	if(nwdaf_registration_modification->analytics_ids) {
		OpenAPI_list_for_each(
			nwdaf_registration_modification->analytics_ids, node) {
			OpenAPI_event_id_free(node->data);
		}
		OpenAPI_list_free(
			nwdaf_registration_modification->analytics_ids);
...
cJSON *OpenAPI_nwdaf_registration_modification_convertToJSON(
	...
	if(nwdaf_registration_modification == NULL) {
		ogs_error(
			"OpenAPI_nwdaf_registration_modification_convertToJSON() failed [NwdafRegistrationModification]");
		return NULL;
	}
	...
	if(!nwdaf_registration_modification->nwdaf_instance_id) {
		ogs_error(
			"OpenAPI_nwdaf_registration_modification_convertToJSON() failed [nwdaf_instance_id]");
		return NULL;
	}
	...
	if(nwdaf_registration_modification->nwdaf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_nwdaf_registration_modification_convertToJSON() failed [nwdaf_set_id]");
			goto end;
		}
	}
	...
	if(nwdaf_registration_modification->analytics_ids) {
		...
		if(analytics_idsList == NULL) {
			ogs_error(
				"OpenAPI_nwdaf_registration_modification_convertToJSON() failed [analytics_ids]");
			goto end;
		}
		OpenAPI_list_for_each(
			nwdaf_registration_modification->analytics_ids, node) {
	...
	if(nwdaf_registration_modification->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_nwdaf_registration_modification_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
...
OpenAPI_nwdaf_registration_modification_parseFromJSON(
	...
	OpenAPI_nwdaf_registration_modification_t *
	        nwdaf_registration_modification_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	nwdaf_instance_id =
		cJSON_GetObjectItemCaseSensitive(
			nwdaf_registration_modificationJSON, "nwdafInstanceId");
	if(!nwdaf_instance_id) {
		ogs_error(
			"OpenAPI_nwdaf_registration_modification_parseFromJSON() failed [nwdaf_instance_id]");
		goto end;
	}
	...
	nwdaf_set_id =
		cJSON_GetObjectItemCaseSensitive(
			nwdaf_registration_modificationJSON, "nwdafSetId");
	if(nwdaf_set_id) {
		if(!cJSON_IsString(nwdaf_set_id) &&
		   !cJSON_IsNull(nwdaf_set_id))
		{
			ogs_error(
				"OpenAPI_nwdaf_registration_modification_parseFromJSON() failed [nwdaf_set_id]");
			goto end;
		}
	}
	...
	analytics_ids =
		cJSON_GetObjectItemCaseSensitive(
			nwdaf_registration_modificationJSON, "analyticsIds");
	if(analytics_ids) {
		...
		cJSON_ArrayForEach(analytics_ids_local, analytics_ids) {
			...
			OpenAPI_event_id_t *analytics_idsItem =
				OpenAPI_event_id_parseFromJSON(
					analytics_ids_local);
	...
	supported_features =
		cJSON_GetObjectItemCaseSensitive(
			nwdaf_registration_modificationJSON,
			"supportedFeatures");
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_nwdaf_registration_modification_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
...
OpenAPI_nwdaf_registration_modification_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_nwdaf_registration_modification_convertToJSON() failed");
		return NULL;
	}
