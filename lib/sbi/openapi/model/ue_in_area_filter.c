...
OpenAPI_ue_in_area_filter_t *OpenAPI_ue_in_area_filter_create(
	OpenAPI_ue_type_t *ue_type, bool is_aerial_srv_dnn_ind,
	int aerial_srv_dnn_ind) {
	OpenAPI_ue_in_area_filter_t *ue_in_area_filter_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_in_area_filter_t));
...
void OpenAPI_ue_in_area_filter_free(
...
cJSON *OpenAPI_ue_in_area_filter_convertToJSON(
	OpenAPI_ue_in_area_filter_t *ue_in_area_filter) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(ue_in_area_filter == NULL) {
		ogs_error(
			"OpenAPI_ue_in_area_filter_convertToJSON() failed [UeInAreaFilter]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(ue_in_area_filter->ue_type) {
		cJSON *ue_type_local_JSON =
			OpenAPI_ue_type_convertToJSON(
				ue_in_area_filter->ue_type);
		if(ue_type_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_in_area_filter_convertToJSON() failed [ue_type]");
			goto end;
		}
	...
	if(ue_in_area_filter->is_aerial_srv_dnn_ind) {
		...
		{
			ogs_error(
				"OpenAPI_ue_in_area_filter_convertToJSON() failed [aerial_srv_dnn_ind]");
			goto end;
		}
	}
...
OpenAPI_ue_in_area_filter_t *OpenAPI_ue_in_area_filter_parseFromJSON(
	...
	if(ue_type) {
		...
		if(!ue_type_local_nonprim) {
			ogs_error(
				"OpenAPI_ue_type_parseFromJSON failed [ue_type]");
			goto end;
		}
	}

	aerial_srv_dnn_ind =
		cJSON_GetObjectItemCaseSensitive(ue_in_area_filterJSON,
		                                 "aerialSrvDnnInd");
	if(aerial_srv_dnn_ind) {
		if(!cJSON_IsBool(aerial_srv_dnn_ind)) {
			ogs_error(
				"OpenAPI_ue_in_area_filter_parseFromJSON() failed [aerial_srv_dnn_ind]");
			goto end;
		}
	}
...
OpenAPI_ue_in_area_filter_t *OpenAPI_ue_in_area_filter_copy(
	OpenAPI_ue_in_area_filter_t *dst, OpenAPI_ue_in_area_filter_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
