...
OpenAPI_vn_group_data_t *OpenAPI_vn_group_data_create(
	...
	OpenAPI_vn_group_data_t *vn_group_data_local_var =
		ogs_malloc(sizeof(OpenAPI_vn_group_data_t));
...
void OpenAPI_vn_group_data_free(OpenAPI_vn_group_data_t *vn_group_data) {
	...
	if(vn_group_data->pdu_session_types) {
		OpenAPI_pdu_session_types_free(
			vn_group_data->pdu_session_types);
...
cJSON *OpenAPI_vn_group_data_convertToJSON(
	...
	if(vn_group_data == NULL) {
		ogs_error(
			"OpenAPI_vn_group_data_convertToJSON() failed [VnGroupData]");
		return NULL;
	}
	...
	if(vn_group_data->pdu_session_types) {
		...
		if(pdu_session_types_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_vn_group_data_convertToJSON() failed [pdu_session_types]");
			goto end;
		}
	...
	if(vn_group_data->dnn) {
		...
		{
			ogs_error(
				"OpenAPI_vn_group_data_convertToJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(vn_group_data->single_nssai) {
		cJSON *single_nssai_local_JSON =
			OpenAPI_snssai_convertToJSON(
				vn_group_data->single_nssai);
		if(single_nssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_vn_group_data_convertToJSON() failed [single_nssai]");
			goto end;
		}
	...
	if(vn_group_data->app_descriptors) {
		...
		if(app_descriptorsList == NULL) {
			ogs_error(
				"OpenAPI_vn_group_data_convertToJSON() failed [app_descriptors]");
			goto end;
		}
		OpenAPI_list_for_each(vn_group_data->app_descriptors, node) {
			cJSON *itemLocal =
...
OpenAPI_vn_group_data_t *OpenAPI_vn_group_data_parseFromJSON(
	...
	pdu_session_types = cJSON_GetObjectItemCaseSensitive(vn_group_dataJSON,
	                                                     "pduSessionTypes");
	if(pdu_session_types) {
		pdu_session_types_local_nonprim =
			OpenAPI_pdu_session_types_parseFromJSON(
				pdu_session_types);
	...
	if(dnn) {
		...
		{
			ogs_error(
				"OpenAPI_vn_group_data_parseFromJSON() failed [dnn]");
			goto end;
		}
	}

	single_nssai = cJSON_GetObjectItemCaseSensitive(vn_group_dataJSON,
	                                                "singleNssai");
	if(single_nssai) {
	...
	app_descriptors = cJSON_GetObjectItemCaseSensitive(vn_group_dataJSON,
	                                                   "appDescriptors");
	if(app_descriptors) {
		cJSON *app_descriptors_local = NULL;
		if(!cJSON_IsArray(app_descriptors)) {
			ogs_error(
				"OpenAPI_vn_group_data_parseFromJSON() failed [app_descriptors]");
			goto end;
		}
