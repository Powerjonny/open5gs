...
char * OpenAPI_request_type_ToString(OpenAPI_request_type_e request_type) {
	const char *request_typeArray[] = { "NULL", "INITIAL_REQUEST",
		                            "EXISTING_PDU_SESSION",
		                            "INITIAL_EMERGENCY_REQUEST",
		                            "EXISTING_EMERGENCY_PDU_SESSION" };
	size_t sizeofArray = sizeof(request_typeArray) /
	                     sizeof(request_typeArray[0]);
...
OpenAPI_request_type_e OpenAPI_request_type_FromString(char *request_type) {
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(request_type,
		          request_typeArray[stringToReturn]) == 0) {
