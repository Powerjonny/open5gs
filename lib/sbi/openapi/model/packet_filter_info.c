...
OpenAPI_packet_filter_info_t *OpenAPI_packet_filter_info_create(
	char *pack_filt_id, char *pack_filt_cont, char *tos_traffic_class,
	char *spi, char *flow_label, OpenAPI_flow_direction_e flow_direction) {
	OpenAPI_packet_filter_info_t *packet_filter_info_local_var =
		ogs_malloc(sizeof(OpenAPI_packet_filter_info_t));
...
cJSON *OpenAPI_packet_filter_info_convertToJSON(
	...
	if(packet_filter_info == NULL) {
		ogs_error(
			"OpenAPI_packet_filter_info_convertToJSON() failed [PacketFilterInfo]");
		return NULL;
	}
	...
	if(packet_filter_info->pack_filt_id) {
		if(cJSON_AddStringToObject(item, "packFiltId",
		                           packet_filter_info->pack_filt_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_packet_filter_info_convertToJSON() failed [pack_filt_id]");
			goto end;
		}
	}

	if(packet_filter_info->pack_filt_cont) {
		if(cJSON_AddStringToObject(item, "packFiltCont",
		                           packet_filter_info->pack_filt_cont)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_packet_filter_info_convertToJSON() failed [pack_filt_cont]");
			goto end;
		}
	}
	...
	if(packet_filter_info->tos_traffic_class) {
		if(cJSON_AddStringToObject(item, "tosTrafficClass",
		                           packet_filter_info->tos_traffic_class)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_packet_filter_info_convertToJSON() failed [tos_traffic_class]");
			goto end;
		}
	}
	...
	if(packet_filter_info->spi) {
		...
		{
			ogs_error(
				"OpenAPI_packet_filter_info_convertToJSON() failed [spi]");
			goto end;
		}
	}
	...
	if(packet_filter_info->flow_label) {
		if(cJSON_AddStringToObject(item, "flowLabel",
		                           packet_filter_info->flow_label) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_packet_filter_info_convertToJSON() failed [flow_label]");
			goto end;
		}
	}
	...
	if(packet_filter_info->flow_direction != OpenAPI_flow_direction_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_packet_filter_info_convertToJSON() failed [flow_direction]");
			goto end;
		}
	}
...
OpenAPI_packet_filter_info_t *OpenAPI_packet_filter_info_parseFromJSON(
	cJSON *packet_filter_infoJSON) {
	...
	pack_filt_id = cJSON_GetObjectItemCaseSensitive(packet_filter_infoJSON,
	                                                "packFiltId");
	if(pack_filt_id) {
		...
		{
			ogs_error(
				"OpenAPI_packet_filter_info_parseFromJSON() failed [pack_filt_id]");
			goto end;
		}
	}
	...
	if(pack_filt_cont) {
		if(!cJSON_IsString(pack_filt_cont) &&
		   !cJSON_IsNull(pack_filt_cont))
		{
			ogs_error(
				"OpenAPI_packet_filter_info_parseFromJSON() failed [pack_filt_cont]");
			goto end;
		}
	}
	...
	if(tos_traffic_class) {
		...
		{
			ogs_error(
				"OpenAPI_packet_filter_info_parseFromJSON() failed [tos_traffic_class]");
			goto end;
		}
	}
	...
	if(spi) {
		...
		{
			ogs_error(
				"OpenAPI_packet_filter_info_parseFromJSON() failed [spi]");
			goto end;
		}
	}

	flow_label = cJSON_GetObjectItemCaseSensitive(packet_filter_infoJSON,
	                                              "flowLabel");
	if(flow_label) {
		...
		{
			ogs_error(
				"OpenAPI_packet_filter_info_parseFromJSON() failed [flow_label]");
			goto end;
		}
	}
	...
	if(flow_direction) {
		if(!cJSON_IsString(flow_direction)) {
			ogs_error(
				"OpenAPI_packet_filter_info_parseFromJSON() failed [flow_direction]");
			goto end;
		}
	...
	packet_filter_info_local_var = OpenAPI_packet_filter_info_create(
		...
		pack_filt_cont &&
		!cJSON_IsNull(pack_filt_cont) ?
		ogs_strdup(pack_filt_cont->valuestring) : NULL,
		tos_traffic_class &&
		!cJSON_IsNull(tos_traffic_class) ?
...
OpenAPI_packet_filter_info_t *OpenAPI_packet_filter_info_copy(
	OpenAPI_packet_filter_info_t *dst, OpenAPI_packet_filter_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
