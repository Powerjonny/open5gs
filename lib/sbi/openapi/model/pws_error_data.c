...
OpenAPI_pws_error_data_t *OpenAPI_pws_error_data_create(int namf_cause) {
	OpenAPI_pws_error_data_t *pws_error_data_local_var =
		ogs_malloc(sizeof(OpenAPI_pws_error_data_t));
...
cJSON *OpenAPI_pws_error_data_convertToJSON(
	...
	if(pws_error_data == NULL) {
		ogs_error(
			"OpenAPI_pws_error_data_convertToJSON() failed [PWSErrorData]");
		return NULL;
	}
	...
	{
		ogs_error(
			"OpenAPI_pws_error_data_convertToJSON() failed [namf_cause]");
		goto end;
	}
...
OpenAPI_pws_error_data_t *OpenAPI_pws_error_data_parseFromJSON(
	...
	if(!namf_cause) {
		ogs_error(
			"OpenAPI_pws_error_data_parseFromJSON() failed [namf_cause]");
		goto end;
	}
