...
OpenAPI_model_5_mbs_authorization_info_create(
	OpenAPI_list_t *_5mbs_session_ids) {
	OpenAPI_model_5_mbs_authorization_info_t *
	        model_5_mbs_authorization_info_local_var =
		ogs_malloc(sizeof(OpenAPI_model_5_mbs_authorization_info_t));
	ogs_assert(model_5_mbs_authorization_info_local_var);

	model_5_mbs_authorization_info_local_var->_5mbs_session_ids =
		_5mbs_session_ids;
...
void OpenAPI_model_5_mbs_authorization_info_free(
	OpenAPI_model_5_mbs_authorization_info_t *model_5_mbs_authorization_info)
{
	...
	if(model_5_mbs_authorization_info->_5mbs_session_ids) {
		OpenAPI_list_for_each(
			model_5_mbs_authorization_info->_5mbs_session_ids,
			node) {
...
cJSON *OpenAPI_model_5_mbs_authorization_info_convertToJSON(
	OpenAPI_model_5_mbs_authorization_info_t *model_5_mbs_authorization_info)
{
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(model_5_mbs_authorization_info == NULL) {
		ogs_error(
			"OpenAPI_model_5_mbs_authorization_info_convertToJSON() failed [5MbsAuthorizationInfo]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(model_5_mbs_authorization_info->_5mbs_session_ids) {
		cJSON *_5mbs_session_idsList = cJSON_AddArrayToObject(item,
		                                                      "5mbsSessionIds");
		if(_5mbs_session_idsList == NULL) {
			ogs_error(
				"OpenAPI_model_5_mbs_authorization_info_convertToJSON() failed [_5mbs_session_ids]");
			goto end;
		}
		OpenAPI_list_for_each(
			model_5_mbs_authorization_info->_5mbs_session_ids,
...
OpenAPI_model_5_mbs_authorization_info_parseFromJSON(
	cJSON *model_5_mbs_authorization_infoJSON) {
	OpenAPI_model_5_mbs_authorization_info_t *
	        model_5_mbs_authorization_info_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *_5mbs_session_ids = NULL;
	OpenAPI_list_t *_5mbs_session_idsList = NULL;
	_5mbs_session_ids =
		cJSON_GetObjectItemCaseSensitive(
			model_5_mbs_authorization_infoJSON, "5mbsSessionIds");
	if(_5mbs_session_ids) {
		...
		cJSON_ArrayForEach(_5mbs_session_ids_local, _5mbs_session_ids) {
			...
			OpenAPI_mbs_session_id_t *_5mbs_session_idsItem =
				OpenAPI_mbs_session_id_parseFromJSON(
					_5mbs_session_ids_local);
			if(!_5mbs_session_idsItem) {
				ogs_error("No _5mbs_session_idsItem");
				goto end;
			}
			OpenAPI_list_add(_5mbs_session_idsList,
			                 _5mbs_session_idsItem);
		}
	}
...
OpenAPI_model_5_mbs_authorization_info_t *
OpenAPI_model_5_mbs_authorization_info_copy(
	OpenAPI_model_5_mbs_authorization_info_t	*dst,
	OpenAPI_model_5_mbs_authorization_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_model_5_mbs_authorization_info_convertToJSON() failed");
		return NULL;
	}
