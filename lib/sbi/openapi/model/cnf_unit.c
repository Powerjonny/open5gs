...
OpenAPI_cnf_unit_t *OpenAPI_cnf_unit_create(OpenAPI_list_t *cnf_unit) {
	OpenAPI_cnf_unit_t *cnf_unit_local_var =
		ogs_malloc(sizeof(OpenAPI_cnf_unit_t));
...
cJSON *OpenAPI_cnf_unit_convertToJSON(OpenAPI_cnf_unit_t *cnf_unit) {
	...
	OpenAPI_list_for_each(cnf_unit->cnf_unit, node) {
		...
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_cnf_unit_convertToJSON() failed [cnf_unit]");
			goto end;
		}
...
OpenAPI_cnf_unit_t *OpenAPI_cnf_unit_parseFromJSON(cJSON *cnf_unitJSON) {
	...
	cJSON_ArrayForEach(cnf_unit_local, cnf_unit) {
		if(!cJSON_IsObject(cnf_unit_local)) {
			ogs_error(
				"OpenAPI_cnf_unit_parseFromJSON() failed [cnf_unit]");
			goto end;
		}
		OpenAPI_atom_t *cnf_unitItem =
			OpenAPI_atom_parseFromJSON(cnf_unit_local);
		if(!cnf_unitItem) {
...
OpenAPI_cnf_unit_t *OpenAPI_cnf_unit_copy(OpenAPI_cnf_unit_t	*dst,
                                          OpenAPI_cnf_unit_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
