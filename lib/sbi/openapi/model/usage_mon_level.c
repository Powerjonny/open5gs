...
OpenAPI_usage_mon_level_t *OpenAPI_usage_mon_level_create() {
	OpenAPI_usage_mon_level_t *usage_mon_level_local_var =
		ogs_malloc(sizeof(OpenAPI_usage_mon_level_t));
...
cJSON *OpenAPI_usage_mon_level_convertToJSON(
	...
	if(usage_mon_level == NULL) {
		ogs_error(
			"OpenAPI_usage_mon_level_convertToJSON() failed [UsageMonLevel]");
		return NULL;
	}
