...
OpenAPI_usage_mon_data_limit_t *OpenAPI_usage_mon_data_limit_create(
	...
	OpenAPI_usage_mon_data_limit_t *usage_mon_data_limit_local_var =
		ogs_malloc(sizeof(OpenAPI_usage_mon_data_limit_t));
...
cJSON *OpenAPI_usage_mon_data_limit_convertToJSON(
	...
	if(usage_mon_data_limit == NULL) {
		ogs_error(
			"OpenAPI_usage_mon_data_limit_convertToJSON() failed [UsageMonDataLimit]");
		return NULL;
	}
	...
	if(!usage_mon_data_limit->limit_id) {
		ogs_error(
			"OpenAPI_usage_mon_data_limit_convertToJSON() failed [limit_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "limitId",
	                           usage_mon_data_limit->limit_id) == NULL)
	{
		ogs_error(
			"OpenAPI_usage_mon_data_limit_convertToJSON() failed [limit_id]");
	...
	if(usage_mon_data_limit->scopes) {
		...
		if(scopes == NULL) {
			ogs_error(
				"OpenAPI_usage_mon_data_limit_convertToJSON() failed [scopes]");
			goto end;
		}
		cJSON *localMapObject = scopes;
		if(usage_mon_data_limit->scopes) {
			OpenAPI_list_for_each(usage_mon_data_limit->scopes,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_usage_mon_data_limit_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(usage_mon_data_limit->um_level) {
		cJSON *um_level_local_JSON =
			OpenAPI_usage_mon_level_convertToJSON(
				usage_mon_data_limit->um_level);
		if(um_level_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_usage_mon_data_limit_convertToJSON() failed [um_level]");
			goto end;
		}
	...
	if(usage_mon_data_limit->start_date) {
		if(cJSON_AddStringToObject(item, "startDate",
		                           usage_mon_data_limit->start_date) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_usage_mon_data_limit_convertToJSON() failed [start_date]");
			goto end;
		}
	}

	if(usage_mon_data_limit->end_date) {
		if(cJSON_AddStringToObject(item, "endDate",
		                           usage_mon_data_limit->end_date) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_usage_mon_data_limit_convertToJSON() failed [end_date]");
			goto end;
		}
	}

	if(usage_mon_data_limit->usage_limit) {
		cJSON *usage_limit_local_JSON =
			OpenAPI_usage_threshold_convertToJSON(
				usage_mon_data_limit->usage_limit);
		if(usage_limit_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_usage_mon_data_limit_convertToJSON() failed [usage_limit]");
			goto end;
		}
		cJSON_AddItemToObject(item, "usageLimit",
		                      usage_limit_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_usage_mon_data_limit_convertToJSON() failed [usage_limit]");
			goto end;
	...
	if(usage_mon_data_limit->reset_period) {
		cJSON *reset_period_local_JSON =
			OpenAPI_time_period_convertToJSON(
				usage_mon_data_limit->reset_period);
		if(reset_period_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_usage_mon_data_limit_convertToJSON() failed [reset_period]");
			goto end;
		}
		cJSON_AddItemToObject(item, "resetPeriod",
		                      reset_period_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_usage_mon_data_limit_convertToJSON() failed [reset_period]");
			goto end;
...
OpenAPI_usage_mon_data_limit_t *OpenAPI_usage_mon_data_limit_parseFromJSON(
	...
	limit_id = cJSON_GetObjectItemCaseSensitive(usage_mon_data_limitJSON,
	                                            "limitId");
	if(!limit_id) {
		ogs_error(
			"OpenAPI_usage_mon_data_limit_parseFromJSON() failed [limit_id]");
		goto end;
	}
	...
	scopes = cJSON_GetObjectItemCaseSensitive(usage_mon_data_limitJSON,
	                                          "scopes");
	if(scopes) {
		...
		{
			ogs_error(
				"OpenAPI_usage_mon_data_limit_parseFromJSON() failed [scopes]");
			goto end;
		}
		if(cJSON_IsObject(scopes)) {
			...
			cJSON_ArrayForEach(scopes_local_map, scopes) {
				...
				if(cJSON_IsObject(localMapObject)) {
					localMapKeyPair = OpenAPI_map_create(
						...
						OpenAPI_usage_mon_data_scope_parseFromJSON
						        (localMapObject));
					...
					ogs_error(
						"OpenAPI_usage_mon_data_limit_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	um_level = cJSON_GetObjectItemCaseSensitive(usage_mon_data_limitJSON,
	                                            "umLevel");
	if(um_level) {
		um_level_local_nonprim =
			OpenAPI_usage_mon_level_parseFromJSON(um_level);
		if(!um_level_local_nonprim) {
			ogs_error(
				"OpenAPI_usage_mon_level_parseFromJSON failed [um_level]");
			goto end;
		}
	}

	start_date = cJSON_GetObjectItemCaseSensitive(usage_mon_data_limitJSON,
	                                              "startDate");
	if(start_date) {
		if(!cJSON_IsString(start_date) &&
		   !cJSON_IsNull(start_date))
		{
			ogs_error(
				"OpenAPI_usage_mon_data_limit_parseFromJSON() failed [start_date]");
			goto end;
		}
	}

	end_date = cJSON_GetObjectItemCaseSensitive(usage_mon_data_limitJSON,
	                                            "endDate");
	if(end_date) {
		if(!cJSON_IsString(end_date) &&
		   !cJSON_IsNull(end_date))
		{
			ogs_error(
				"OpenAPI_usage_mon_data_limit_parseFromJSON() failed [end_date]");
			goto end;
		}
	}

	usage_limit = cJSON_GetObjectItemCaseSensitive(usage_mon_data_limitJSON,
	                                               "usageLimit");
	if(usage_limit) {
		usage_limit_local_nonprim =
			OpenAPI_usage_threshold_parseFromJSON(usage_limit);
		if(!usage_limit_local_nonprim) {
			ogs_error(
				"OpenAPI_usage_threshold_parseFromJSON failed [usage_limit]");
			goto end;
		}
	}

	reset_period =
		cJSON_GetObjectItemCaseSensitive(usage_mon_data_limitJSON,
		                                 "resetPeriod");
	if(reset_period) {
		reset_period_local_nonprim =
			OpenAPI_time_period_parseFromJSON(reset_period);
		if(!reset_period_local_nonprim) {
			ogs_error(
				"OpenAPI_time_period_parseFromJSON failed [reset_period]");
			goto end;
		}
	}
	...
	usage_mon_data_limit_local_var = OpenAPI_usage_mon_data_limit_create(
		...
		start_date &&
		!cJSON_IsNull(start_date) ?
		ogs_strdup(start_date->valuestring) : NULL,
		end_date &&
		!cJSON_IsNull(end_date) ? ogs_strdup(end_date->valuestring) :
		NULL,
...
OpenAPI_usage_mon_data_limit_t *OpenAPI_usage_mon_data_limit_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_usage_mon_data_limit_convertToJSON() failed");
		return NULL;
	}
