...
OpenAPI_immediate_report_t *OpenAPI_immediate_report_create(
	...
	OpenAPI_immediate_report_t *immediate_report_local_var =
		ogs_malloc(sizeof(OpenAPI_immediate_report_t));
	...
	immediate_report_local_var->lcs_broadcast_assistance_types_data =
		lcs_broadcast_assistance_types_data;
...
void OpenAPI_immediate_report_free(
	OpenAPI_immediate_report_t *immediate_report) {
	...
	if(immediate_report->lcs_broadcast_assistance_types_data) {
		OpenAPI_lcs_broadcast_assistance_types_data_free(
			immediate_report->lcs_broadcast_assistance_types_data);
...
cJSON *OpenAPI_immediate_report_convertToJSON(
	OpenAPI_immediate_report_t *immediate_report) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(immediate_report == NULL) {
		ogs_error(
			"OpenAPI_immediate_report_convertToJSON() failed [ImmediateReport]");
		return NULL;
	}
	...
	if(immediate_report->am_data) {
		...
		if(am_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_immediate_report_convertToJSON() failed [am_data]");
			goto end;
		}
	...
	if(immediate_report->smf_sel_data) {
		...
		if(smf_sel_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_immediate_report_convertToJSON() failed [smf_sel_data]");
			goto end;
		}
	...
	if(immediate_report->uec_amf_data) {
		cJSON *uec_amf_data_local_JSON =
			OpenAPI_ue_context_in_amf_data_convertToJSON(
				immediate_report->uec_amf_data);
		if(uec_amf_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_immediate_report_convertToJSON() failed [uec_amf_data]");
			goto end;
		}
		cJSON_AddItemToObject(item, "uecAmfData",
		                      uec_amf_data_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_immediate_report_convertToJSON() failed [uec_amf_data]");
			goto end;
	...
	if(immediate_report->uec_smf_data) {
		cJSON *uec_smf_data_local_JSON =
			OpenAPI_ue_context_in_smf_data_convertToJSON(
				immediate_report->uec_smf_data);
		if(uec_smf_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_immediate_report_convertToJSON() failed [uec_smf_data]");
			goto end;
		}
		cJSON_AddItemToObject(item, "uecSmfData",
		                      uec_smf_data_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_immediate_report_convertToJSON() failed [uec_smf_data]");
			goto end;
	...
	if(immediate_report->uec_smsf_data) {
		cJSON *uec_smsf_data_local_JSON =
			OpenAPI_ue_context_in_smsf_data_convertToJSON(
				immediate_report->uec_smsf_data);
		if(uec_smsf_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_immediate_report_convertToJSON() failed [uec_smsf_data]");
			goto end;
		}
		cJSON_AddItemToObject(item, "uecSmsfData",
		                      uec_smsf_data_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_immediate_report_convertToJSON() failed [uec_smsf_data]");
			goto end;
	...
	if(immediate_report->sms_subs_data) {
		...
		if(sms_subs_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_immediate_report_convertToJSON() failed [sms_subs_data]");
			goto end;
		}
	...
	if(immediate_report->sm_data) {
		cJSON *sm_data_local_JSON =
			OpenAPI_sm_subs_data_convertToJSON(
				immediate_report->sm_data);
		if(sm_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_immediate_report_convertToJSON() failed [sm_data]");
			goto end;
		}
	...
	if(immediate_report->trace_data) {
		cJSON *trace_data_local_JSON =
			OpenAPI_trace_data_convertToJSON(
				immediate_report->trace_data);
		if(trace_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_immediate_report_convertToJSON() failed [trace_data]");
			goto end;
		}
	...
	if(immediate_report->sms_mng_data) {
		...
		if(sms_mng_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_immediate_report_convertToJSON() failed [sms_mng_data]");
			goto end;
		}
	...
	if(immediate_report->lcs_privacy_data) {
		...
		if(lcs_privacy_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_immediate_report_convertToJSON() failed [lcs_privacy_data]");
			goto end;
		}
	...
	if(immediate_report->lcs_mo_data) {
		cJSON *lcs_mo_data_local_JSON =
			OpenAPI_lcs_mo_data_convertToJSON(
				immediate_report->lcs_mo_data);
		if(lcs_mo_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_immediate_report_convertToJSON() failed [lcs_mo_data]");
			goto end;
		}
	...
	if(immediate_report->v2x_data) {
		cJSON *v2x_data_local_JSON =
			OpenAPI_v2x_subscription_data_convertToJSON(
				immediate_report->v2x_data);
		if(v2x_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_immediate_report_convertToJSON() failed [v2x_data]");
			goto end;
		}
	...
	if(immediate_report->lcs_broadcast_assistance_types_data) {
		cJSON *lcs_broadcast_assistance_types_data_local_JSON =
			OpenAPI_lcs_broadcast_assistance_types_data_convertToJSON
			        (immediate_report->
			        lcs_broadcast_assistance_types_data);
		if(lcs_broadcast_assistance_types_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_immediate_report_convertToJSON() failed [lcs_broadcast_assistance_types_data]");
			goto end;
		}
		cJSON_AddItemToObject(item, "lcsBroadcastAssistanceTypesData",
		                      lcs_broadcast_assistance_types_data_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_immediate_report_convertToJSON() failed [lcs_broadcast_assistance_types_data]");
			goto end;
	...
	if(immediate_report->prose_data) {
		...
		if(prose_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_immediate_report_convertToJSON() failed [prose_data]");
			goto end;
		}
	...
	if(immediate_report->mbs_data) {
		cJSON *mbs_data_local_JSON =
			OpenAPI_mbs_subscription_data_convertToJSON(
				immediate_report->mbs_data);
		if(mbs_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_immediate_report_convertToJSON() failed [mbs_data]");
			goto end;
		}
	...
	if(immediate_report->uc_data) {
		cJSON *uc_data_local_JSON =
			OpenAPI_uc_subscription_data_convertToJSON(
				immediate_report->uc_data);
		if(uc_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_immediate_report_convertToJSON() failed [uc_data]");
			goto end;
		}
...
OpenAPI_immediate_report_t *OpenAPI_immediate_report_parseFromJSON(
	cJSON *immediate_reportJSON) {
	...
	am_data = cJSON_GetObjectItemCaseSensitive(immediate_reportJSON,
	                                           "amData");
	if(am_data) {
	...
	smf_sel_data = cJSON_GetObjectItemCaseSensitive(immediate_reportJSON,
	                                                "smfSelData");
	if(smf_sel_data) {
	...
	uec_amf_data = cJSON_GetObjectItemCaseSensitive(immediate_reportJSON,
	                                                "uecAmfData");
	if(uec_amf_data) {
		uec_amf_data_local_nonprim =
			OpenAPI_ue_context_in_amf_data_parseFromJSON(
				uec_amf_data);
		if(!uec_amf_data_local_nonprim) {
			ogs_error(
				"OpenAPI_ue_context_in_amf_data_parseFromJSON failed [uec_amf_data]");
			goto end;
		}
	}

	uec_smf_data = cJSON_GetObjectItemCaseSensitive(immediate_reportJSON,
	                                                "uecSmfData");
	if(uec_smf_data) {
		uec_smf_data_local_nonprim =
			OpenAPI_ue_context_in_smf_data_parseFromJSON(
				uec_smf_data);
		if(!uec_smf_data_local_nonprim) {
			ogs_error(
				"OpenAPI_ue_context_in_smf_data_parseFromJSON failed [uec_smf_data]");
			goto end;
		}
	}

	uec_smsf_data = cJSON_GetObjectItemCaseSensitive(immediate_reportJSON,
	                                                 "uecSmsfData");
	if(uec_smsf_data) {
		uec_smsf_data_local_nonprim =
			OpenAPI_ue_context_in_smsf_data_parseFromJSON(
				uec_smsf_data);
		if(!uec_smsf_data_local_nonprim) {
			ogs_error(
				"OpenAPI_ue_context_in_smsf_data_parseFromJSON failed [uec_smsf_data]");
			goto end;
		}
	}

	sms_subs_data = cJSON_GetObjectItemCaseSensitive(immediate_reportJSON,
	                                                 "smsSubsData");
	if(sms_subs_data) {
	...
	sm_data = cJSON_GetObjectItemCaseSensitive(immediate_reportJSON,
	                                           "smData");
	if(sm_data) {
	...
	trace_data = cJSON_GetObjectItemCaseSensitive(immediate_reportJSON,
	                                              "traceData");
	if(trace_data) {
	...
	sms_mng_data = cJSON_GetObjectItemCaseSensitive(immediate_reportJSON,
	                                                "smsMngData");
	if(sms_mng_data) {
	...
	lcs_privacy_data =
		cJSON_GetObjectItemCaseSensitive(immediate_reportJSON,
		                                 "lcsPrivacyData");
	if(lcs_privacy_data) {
	...
	lcs_mo_data = cJSON_GetObjectItemCaseSensitive(immediate_reportJSON,
	                                               "lcsMoData");
	if(lcs_mo_data) {
	...
	v2x_data = cJSON_GetObjectItemCaseSensitive(immediate_reportJSON,
	                                            "v2xData");
	if(v2x_data) {
	...
	if(lcs_broadcast_assistance_types_data) {
		...
		if(!lcs_broadcast_assistance_types_data_local_nonprim) {
			ogs_error(
				"OpenAPI_lcs_broadcast_assistance_types_data_parseFromJSON failed [lcs_broadcast_assistance_types_data]");
			goto end;
		}
	}

	prose_data = cJSON_GetObjectItemCaseSensitive(immediate_reportJSON,
	                                              "proseData");
	if(prose_data) {
	...
	mbs_data = cJSON_GetObjectItemCaseSensitive(immediate_reportJSON,
	                                            "mbsData");
	if(mbs_data) {
		mbs_data_local_nonprim =
			OpenAPI_mbs_subscription_data_parseFromJSON(mbs_data);
		if(!mbs_data_local_nonprim) {
			ogs_error(
				"OpenAPI_mbs_subscription_data_parseFromJSON failed [mbs_data]");
			goto end;
		}
	}

	uc_data = cJSON_GetObjectItemCaseSensitive(immediate_reportJSON,
	                                           "ucData");
	if(uc_data) {
		uc_data_local_nonprim =
			OpenAPI_uc_subscription_data_parseFromJSON(uc_data);
		if(!uc_data_local_nonprim) {
			ogs_error(
				"OpenAPI_uc_subscription_data_parseFromJSON failed [uc_data]");
			goto end;
		}
	}
...
OpenAPI_immediate_report_t *OpenAPI_immediate_report_copy(
	OpenAPI_immediate_report_t *dst, OpenAPI_immediate_report_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
