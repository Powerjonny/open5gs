...
OpenAPI_nssai_1_t *OpenAPI_nssai_1_create(
	...
	int
	suppress_nssrg_ind) {
	OpenAPI_nssai_1_t *nssai_1_local_var =
		ogs_malloc(sizeof(OpenAPI_nssai_1_t));
...
void OpenAPI_nssai_1_free(OpenAPI_nssai_1_t *nssai_1) {
	...
	if(nssai_1->additional_snssai_data) {
		OpenAPI_list_for_each(nssai_1->additional_snssai_data, node) {
			...
			OpenAPI_additional_snssai_data_1_free(
				localKeyValue->value);
...
cJSON *OpenAPI_nssai_1_convertToJSON(OpenAPI_nssai_1_t *nssai_1) {
	...
	if(nssai_1->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_nssai_1_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(!nssai_1->default_single_nssais) {
		ogs_error(
			"OpenAPI_nssai_1_convertToJSON() failed [default_single_nssais]");
		return NULL;
	}
	cJSON *default_single_nssaisList = cJSON_AddArrayToObject(item,
	                                                          "defaultSingleNssais");
	...
	if(nssai_1->single_nssais) {
		cJSON *single_nssaisList = cJSON_AddArrayToObject(item,
		                                                  "singleNssais");
		if(single_nssaisList == NULL) {
			ogs_error(
				"OpenAPI_nssai_1_convertToJSON() failed [single_nssais]");
			goto end;
		}
		OpenAPI_list_for_each(nssai_1->single_nssais, node) {
			cJSON *itemLocal =
	...
	if(nssai_1->provisioning_time) {
		...
		{
			ogs_error(
				"OpenAPI_nssai_1_convertToJSON() failed [provisioning_time]");
			goto end;
		}
	}

	if(nssai_1->additional_snssai_data) {
		cJSON *additional_snssai_data = cJSON_AddObjectToObject(item,
		                                                        "additionalSnssaiData");
		if(additional_snssai_data == NULL) {
			ogs_error(
				"OpenAPI_nssai_1_convertToJSON() failed [additional_snssai_data]");
			goto end;
		}
		cJSON *localMapObject = additional_snssai_data;
		if(nssai_1->additional_snssai_data) {
			OpenAPI_list_for_each(nssai_1->additional_snssai_data,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_additional_snssai_data_1_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_nssai_1_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(nssai_1->is_suppress_nssrg_ind) {
		if(cJSON_AddBoolToObject(item, "suppressNssrgInd",
		                         nssai_1->suppress_nssrg_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_nssai_1_convertToJSON() failed [suppress_nssrg_ind]");
			goto end;
		}
	}
...
OpenAPI_nssai_1_t *OpenAPI_nssai_1_parseFromJSON(cJSON *nssai_1JSON) {
	...
	supported_features = cJSON_GetObjectItemCaseSensitive(nssai_1JSON,
	                                                      "supportedFeatures");
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_nssai_1_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}

	default_single_nssais = cJSON_GetObjectItemCaseSensitive(nssai_1JSON,
	                                                         "defaultSingleNssais");
	if(!default_single_nssais) {
		ogs_error(
			"OpenAPI_nssai_1_parseFromJSON() failed [default_single_nssais]");
		goto end;
	}
	...
	cJSON_ArrayForEach(default_single_nssais_local, default_single_nssais) {
		...
		OpenAPI_snssai_t *default_single_nssaisItem =
			OpenAPI_snssai_parseFromJSON(
				default_single_nssais_local);
		if(!default_single_nssaisItem) {
			ogs_error("No default_single_nssaisItem");
			goto end;
		}
		OpenAPI_list_add(default_single_nssaisList,
		                 default_single_nssaisItem);
	}

	single_nssais = cJSON_GetObjectItemCaseSensitive(nssai_1JSON,
	                                                 "singleNssais");
	if(single_nssais) {
		cJSON *single_nssais_local = NULL;
		if(!cJSON_IsArray(single_nssais)) {
			ogs_error(
				"OpenAPI_nssai_1_parseFromJSON() failed [single_nssais]");
			goto end;
		}
		...
		cJSON_ArrayForEach(single_nssais_local, single_nssais) {
			...
			OpenAPI_snssai_t *single_nssaisItem =
				OpenAPI_snssai_parseFromJSON(
					single_nssais_local);
	...
	provisioning_time = cJSON_GetObjectItemCaseSensitive(nssai_1JSON,
	                                                     "provisioningTime");
	if(provisioning_time) {
		...
		{
			ogs_error(
				"OpenAPI_nssai_1_parseFromJSON() failed [provisioning_time]");
			goto end;
		}
	}

	additional_snssai_data = cJSON_GetObjectItemCaseSensitive(nssai_1JSON,
	                                                          "additionalSnssaiData");
	if(additional_snssai_data) {
		cJSON *additional_snssai_data_local_map = NULL;
		if(!cJSON_IsObject(additional_snssai_data) &&
		   !cJSON_IsNull(additional_snssai_data))
		{
			ogs_error(
				"OpenAPI_nssai_1_parseFromJSON() failed [additional_snssai_data]");
			goto end;
		}
		if(cJSON_IsObject(additional_snssai_data)) {
			...
			cJSON_ArrayForEach(additional_snssai_data_local_map,
				...
				cJSON *localMapObject =
					additional_snssai_data_local_map;
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_nssai_1_parseFromJSON() failed [inner]");
					goto end;
				}
				OpenAPI_list_add(additional_snssai_dataList,
				                 localMapKeyPair);
			}
		}
	}

	suppress_nssrg_ind = cJSON_GetObjectItemCaseSensitive(nssai_1JSON,
	                                                      "suppressNssrgInd");
	if(suppress_nssrg_ind) {
		if(!cJSON_IsBool(suppress_nssrg_ind)) {
			ogs_error(
				"OpenAPI_nssai_1_parseFromJSON() failed [suppress_nssrg_ind]");
			goto end;
		}
	}
	...
	nssai_1_local_var = OpenAPI_nssai_1_create(
		...
		provisioning_time &&
		!cJSON_IsNull(provisioning_time) ?
		ogs_strdup(provisioning_time->valuestring) : NULL,
...
OpenAPI_nssai_1_t *OpenAPI_nssai_1_copy(OpenAPI_nssai_1_t	*dst,
                                        OpenAPI_nssai_1_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
