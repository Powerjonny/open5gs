...
OpenAPI_nwdaf_registration_t *OpenAPI_nwdaf_registration_create(
	...
	OpenAPI_nwdaf_registration_t *nwdaf_registration_local_var =
		ogs_malloc(sizeof(OpenAPI_nwdaf_registration_t));
...
cJSON *OpenAPI_nwdaf_registration_convertToJSON(
	...
	if(nwdaf_registration == NULL) {
		ogs_error(
			"OpenAPI_nwdaf_registration_convertToJSON() failed [NwdafRegistration]");
		return NULL;
	}
	...
	if(!nwdaf_registration->nwdaf_instance_id) {
		ogs_error(
			"OpenAPI_nwdaf_registration_convertToJSON() failed [nwdaf_instance_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "nwdafInstanceId",
	                           nwdaf_registration->nwdaf_instance_id) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_nwdaf_registration_convertToJSON() failed [nwdaf_instance_id]");
	...
	if(nwdaf_registration->nwdaf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_nwdaf_registration_convertToJSON() failed [nwdaf_set_id]");
			goto end;
		}
	}
	...
	if(nwdaf_registration->registration_time) {
		if(cJSON_AddStringToObject(item, "registrationTime",
		                           nwdaf_registration->registration_time)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_nwdaf_registration_convertToJSON() failed [registration_time]");
			goto end;
		}
	}
	...
	if(nwdaf_registration->context_info) {
		cJSON *context_info_local_JSON =
			OpenAPI_context_info_convertToJSON(
				nwdaf_registration->context_info);
		if(context_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_nwdaf_registration_convertToJSON() failed [context_info]");
			goto end;
		}
	...
	if(nwdaf_registration->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_nwdaf_registration_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(nwdaf_registration->reset_ids) {
		...
		if(reset_idsList == NULL) {
			ogs_error(
				"OpenAPI_nwdaf_registration_convertToJSON() failed [reset_ids]");
			goto end;
		}
		OpenAPI_list_for_each(nwdaf_registration->reset_ids, node) {
...
OpenAPI_nwdaf_registration_t *OpenAPI_nwdaf_registration_parseFromJSON(
	...
	nwdaf_instance_id =
		cJSON_GetObjectItemCaseSensitive(nwdaf_registrationJSON,
		                                 "nwdafInstanceId");
	if(!nwdaf_instance_id) {
		ogs_error(
			"OpenAPI_nwdaf_registration_parseFromJSON() failed [nwdaf_instance_id]");
		goto end;
	}
	...
	analytics_ids = cJSON_GetObjectItemCaseSensitive(nwdaf_registrationJSON,
	                                                 "analyticsIds");
	if(!analytics_ids) {
		ogs_error(
			"OpenAPI_nwdaf_registration_parseFromJSON() failed [analytics_ids]");
		goto end;
	}
	...
	nwdaf_set_id = cJSON_GetObjectItemCaseSensitive(nwdaf_registrationJSON,
	                                                "nwdafSetId");
	if(nwdaf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_nwdaf_registration_parseFromJSON() failed [nwdaf_set_id]");
			goto end;
		}
	}
	...
	if(registration_time) {
		...
		{
			ogs_error(
				"OpenAPI_nwdaf_registration_parseFromJSON() failed [registration_time]");
			goto end;
		}
	}

	context_info = cJSON_GetObjectItemCaseSensitive(nwdaf_registrationJSON,
	                                                "contextInfo");
	if(context_info) {
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_nwdaf_registration_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}

	reset_ids = cJSON_GetObjectItemCaseSensitive(nwdaf_registrationJSON,
	                                             "resetIds");
	if(reset_ids) {
		cJSON *reset_ids_local = NULL;
		if(!cJSON_IsArray(reset_ids)) {
			ogs_error(
				"OpenAPI_nwdaf_registration_parseFromJSON() failed [reset_ids]");
			goto end;
		}
