...
OpenAPI_pws_information_t *OpenAPI_pws_information_create(
	int message_identifier, int serial_number,
	OpenAPI_n2_info_content_t *pws_container,
	OpenAPI_list_t *bc_empty_area_list, bool is_send_ran_response,
	int send_ran_response, char *omc_id, char *nf_id) {
	OpenAPI_pws_information_t *pws_information_local_var =
		ogs_malloc(sizeof(OpenAPI_pws_information_t));
...
void OpenAPI_pws_information_free(OpenAPI_pws_information_t *pws_information) {
	...
	if(pws_information->bc_empty_area_list) {
		OpenAPI_list_for_each(pws_information->bc_empty_area_list,
		                      node) {
...
cJSON *OpenAPI_pws_information_convertToJSON(
	OpenAPI_pws_information_t *pws_information) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(pws_information == NULL) {
		ogs_error(
			"OpenAPI_pws_information_convertToJSON() failed [PwsInformation]");
		return NULL;
	}
	...
	{
		ogs_error(
			"OpenAPI_pws_information_convertToJSON() failed [message_identifier]");
		goto end;
	}
	...
	{
		ogs_error(
			"OpenAPI_pws_information_convertToJSON() failed [serial_number]");
		goto end;
	}

	if(!pws_information->pws_container) {
		ogs_error(
			"OpenAPI_pws_information_convertToJSON() failed [pws_container]");
		return NULL;
	}
	cJSON *pws_container_local_JSON =
	...
	if(pws_information->bc_empty_area_list) {
		...
		if(bc_empty_area_listList == NULL) {
			ogs_error(
				"OpenAPI_pws_information_convertToJSON() failed [bc_empty_area_list]");
			goto end;
		}
		OpenAPI_list_for_each(pws_information->bc_empty_area_list,
		                      node) {
	...
	if(pws_information->is_send_ran_response) {
		...
		{
			ogs_error(
				"OpenAPI_pws_information_convertToJSON() failed [send_ran_response]");
			goto end;
		}
	}
	...
	if(pws_information->omc_id) {
		...
		{
			ogs_error(
				"OpenAPI_pws_information_convertToJSON() failed [omc_id]");
			goto end;
		}
	}
	...
	if(pws_information->nf_id) {
		...
		{
			ogs_error(
				"OpenAPI_pws_information_convertToJSON() failed [nf_id]");
			goto end;
		}
	}
...
OpenAPI_pws_information_t *OpenAPI_pws_information_parseFromJSON(
	...
	cJSON *nf_id = NULL;
	message_identifier =
		cJSON_GetObjectItemCaseSensitive(pws_informationJSON,
		                                 "messageIdentifier");
	if(!message_identifier) {
		ogs_error(
			"OpenAPI_pws_information_parseFromJSON() failed [message_identifier]");
		goto end;
	}
	...
	if(!serial_number) {
		ogs_error(
			"OpenAPI_pws_information_parseFromJSON() failed [serial_number]");
		goto end;
	}
	...
	if(!pws_container) {
		ogs_error(
			"OpenAPI_pws_information_parseFromJSON() failed [pws_container]");
		goto end;
	}
	pws_container_local_nonprim =
		OpenAPI_n2_info_content_parseFromJSON(pws_container);
	if(!pws_container_local_nonprim) {
		ogs_error(
			"OpenAPI_n2_info_content_parseFromJSON failed [pws_container]");
		goto end;
	}
	...
	if(bc_empty_area_list) {
		...
		if(!cJSON_IsArray(bc_empty_area_list)) {
			ogs_error(
				"OpenAPI_pws_information_parseFromJSON() failed [bc_empty_area_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(bc_empty_area_list_local,
			...
			OpenAPI_global_ran_node_id_t *bc_empty_area_listItem =
			...
			OpenAPI_list_add(bc_empty_area_listList,
	...
	if(send_ran_response) {
		if(!cJSON_IsBool(send_ran_response)) {
			ogs_error(
				"OpenAPI_pws_information_parseFromJSON() failed [send_ran_response]");
			goto end;
		}
	}
	...
	if(omc_id) {
		...
		{
			ogs_error(
				"OpenAPI_pws_information_parseFromJSON() failed [omc_id]");
			goto end;
		}
	}
	...
	if(nf_id) {
		...
		{
			ogs_error(
				"OpenAPI_pws_information_parseFromJSON() failed [nf_id]");
			goto end;
		}
	}
	...
	pws_information_local_var = OpenAPI_pws_information_create(
		...
		omc_id &&
		!cJSON_IsNull(omc_id) ? ogs_strdup(omc_id->valuestring) : NULL,
		nf_id &&
		!cJSON_IsNull(nf_id) ? ogs_strdup(nf_id->valuestring) : NULL
		);
...
OpenAPI_pws_information_t *OpenAPI_pws_information_copy(
	OpenAPI_pws_information_t *dst, OpenAPI_pws_information_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
