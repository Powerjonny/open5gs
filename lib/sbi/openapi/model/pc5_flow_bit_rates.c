...
OpenAPI_pc5_flow_bit_rates_t *OpenAPI_pc5_flow_bit_rates_create(
	...
	char	*max_fbr)
{
	OpenAPI_pc5_flow_bit_rates_t *pc5_flow_bit_rates_local_var =
		ogs_malloc(sizeof(OpenAPI_pc5_flow_bit_rates_t));
...
cJSON *OpenAPI_pc5_flow_bit_rates_convertToJSON(
	...
	if(pc5_flow_bit_rates == NULL) {
		ogs_error(
			"OpenAPI_pc5_flow_bit_rates_convertToJSON() failed [Pc5FlowBitRates]");
		return NULL;
	}
	...
	if(pc5_flow_bit_rates->gua_fbr) {
		...
		{
			ogs_error(
				"OpenAPI_pc5_flow_bit_rates_convertToJSON() failed [gua_fbr]");
			goto end;
		}
	}
	...
	if(pc5_flow_bit_rates->max_fbr) {
		...
		{
			ogs_error(
				"OpenAPI_pc5_flow_bit_rates_convertToJSON() failed [max_fbr]");
			goto end;
		}
	}
...
OpenAPI_pc5_flow_bit_rates_t *OpenAPI_pc5_flow_bit_rates_parseFromJSON(
	...
	if(gua_fbr) {
		...
		{
			ogs_error(
				"OpenAPI_pc5_flow_bit_rates_parseFromJSON() failed [gua_fbr]");
			goto end;
		}
	}
	...
	if(max_fbr) {
		...
		{
			ogs_error(
				"OpenAPI_pc5_flow_bit_rates_parseFromJSON() failed [max_fbr]");
			goto end;
		}
	}
	...
	pc5_flow_bit_rates_local_var = OpenAPI_pc5_flow_bit_rates_create(
		gua_fbr &&
		!cJSON_IsNull(gua_fbr) ? ogs_strdup(gua_fbr->valuestring) :
		NULL,
		max_fbr &&
		!cJSON_IsNull(max_fbr) ? ogs_strdup(max_fbr->valuestring) : NULL
		);
