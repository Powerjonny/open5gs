...
OpenAPI_server_addressing_info_t *OpenAPI_server_addressing_info_create(
	OpenAPI_list_t *ipv4_addresses, OpenAPI_list_t *ipv6_addresses,
	OpenAPI_list_t *fqdn_list) {
	OpenAPI_server_addressing_info_t *server_addressing_info_local_var =
		ogs_malloc(sizeof(OpenAPI_server_addressing_info_t));
...
void OpenAPI_server_addressing_info_free(
	OpenAPI_server_addressing_info_t *server_addressing_info) {
	...
	if(server_addressing_info->ipv4_addresses) {
		OpenAPI_list_for_each(server_addressing_info->ipv4_addresses,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(server_addressing_info->ipv6_addresses) {
		OpenAPI_list_for_each(server_addressing_info->ipv6_addresses,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_server_addressing_info_convertToJSON(
	OpenAPI_server_addressing_info_t *server_addressing_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(server_addressing_info == NULL) {
		ogs_error(
			"OpenAPI_server_addressing_info_convertToJSON() failed [ServerAddressingInfo]");
		return NULL;
	}
	...
	if(server_addressing_info->ipv4_addresses) {
		...
		if(ipv4_addressesList == NULL) {
			ogs_error(
				"OpenAPI_server_addressing_info_convertToJSON() failed [ipv4_addresses]");
			goto end;
		}
		OpenAPI_list_for_each(server_addressing_info->ipv4_addresses,
		                      node) {
	...
	if(server_addressing_info->ipv6_addresses) {
		...
		if(ipv6_addressesList == NULL) {
			ogs_error(
				"OpenAPI_server_addressing_info_convertToJSON() failed [ipv6_addresses]");
			goto end;
		}
		OpenAPI_list_for_each(server_addressing_info->ipv6_addresses,
		                      node) {
	...
	if(server_addressing_info->fqdn_list) {
		...
		if(fqdn_listList == NULL) {
			ogs_error(
				"OpenAPI_server_addressing_info_convertToJSON() failed [fqdn_list]");
			goto end;
		}
		OpenAPI_list_for_each(server_addressing_info->fqdn_list, node) {
			if(cJSON_AddStringToObject(fqdn_listList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_server_addressing_info_convertToJSON() failed [fqdn_list]");
...
OpenAPI_server_addressing_info_t *OpenAPI_server_addressing_info_parseFromJSON(
	cJSON *server_addressing_infoJSON) {
	OpenAPI_server_addressing_info_t *server_addressing_info_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	ipv4_addresses =
		cJSON_GetObjectItemCaseSensitive(server_addressing_infoJSON,
		                                 "ipv4Addresses");
	if(ipv4_addresses) {
		cJSON *ipv4_addresses_local = NULL;
		if(!cJSON_IsArray(ipv4_addresses)) {
			ogs_error(
				"OpenAPI_server_addressing_info_parseFromJSON() failed [ipv4_addresses]");
			goto end;
		}
	...
	fqdn_list = cJSON_GetObjectItemCaseSensitive(server_addressing_infoJSON,
	                                             "fqdnList");
	if(fqdn_list) {
		cJSON *fqdn_list_local = NULL;
		if(!cJSON_IsArray(fqdn_list)) {
			ogs_error(
				"OpenAPI_server_addressing_info_parseFromJSON() failed [fqdn_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(fqdn_list_local, fqdn_list) {
			...
			OpenAPI_list_add(fqdn_listList,
			                 ogs_strdup(
						 fqdn_list_local->valuestring));
		}
	}

	server_addressing_info_local_var =
		OpenAPI_server_addressing_info_create(
...
OpenAPI_server_addressing_info_t *OpenAPI_server_addressing_info_copy(
	OpenAPI_server_addressing_info_t	*dst,
	OpenAPI_server_addressing_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_server_addressing_info_convertToJSON() failed");
		return NULL;
	}
