...
OpenAPI_flow_info_t *OpenAPI_flow_info_create(int flow_id,
                                              OpenAPI_list_t	*
                                              flow_descriptions) {
	OpenAPI_flow_info_t *flow_info_local_var =
		ogs_malloc(sizeof(OpenAPI_flow_info_t));
...
cJSON *OpenAPI_flow_info_convertToJSON(OpenAPI_flow_info_t *flow_info) {
	...
	if(flow_info == NULL) {
		ogs_error(
			"OpenAPI_flow_info_convertToJSON() failed [FlowInfo]");
		return NULL;
	}
	...
	if(flow_info->flow_descriptions) {
		...
		if(flow_descriptionsList == NULL) {
			ogs_error(
				"OpenAPI_flow_info_convertToJSON() failed [flow_descriptions]");
			goto end;
		}
		OpenAPI_list_for_each(flow_info->flow_descriptions, node) {
...
OpenAPI_flow_info_t *OpenAPI_flow_info_parseFromJSON(cJSON *flow_infoJSON) {
	...
	if(flow_descriptions) {
		...
		if(!cJSON_IsArray(flow_descriptions)) {
			ogs_error(
				"OpenAPI_flow_info_parseFromJSON() failed [flow_descriptions]");
			goto end;
		}
...
OpenAPI_flow_info_t *OpenAPI_flow_info_copy(OpenAPI_flow_info_t *dst,
                                            OpenAPI_flow_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
