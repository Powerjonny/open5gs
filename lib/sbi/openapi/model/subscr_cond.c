...
OpenAPI_subscr_cond_t *OpenAPI_subscr_cond_create(char *nf_instance_id,
                                                  OpenAPI_list_t *
                                                  nf_instance_list,
	...
	OpenAPI_subscr_cond_t *subscr_cond_local_var =
		ogs_malloc(sizeof(OpenAPI_subscr_cond_t));
...
cJSON *OpenAPI_subscr_cond_convertToJSON(OpenAPI_subscr_cond_t *subscr_cond) {
	...
	if(subscr_cond == NULL) {
		ogs_error(
			"OpenAPI_subscr_cond_convertToJSON() failed [SubscrCond]");
		return NULL;
	}
	...
	if(subscr_cond->nf_instance_id) {
		...
		{
			ogs_error(
				"OpenAPI_subscr_cond_convertToJSON() failed [nf_instance_id]");
			goto end;
		}
	}

	if(subscr_cond->nf_instance_list) {
		cJSON *nf_instance_listList = cJSON_AddArrayToObject(item,
		                                                     "nfInstanceList");
		if(nf_instance_listList == NULL) {
			ogs_error(
				"OpenAPI_subscr_cond_convertToJSON() failed [nf_instance_list]");
			goto end;
		}
		OpenAPI_list_for_each(subscr_cond->nf_instance_list, node) {
			if(cJSON_AddStringToObject(nf_instance_listList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_subscr_cond_convertToJSON() failed [nf_instance_list]");
	...
	if(subscr_cond->nf_type != OpenAPI_nf_type_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_subscr_cond_convertToJSON() failed [nf_type]");
			goto end;
		}
	}
	...
	if(subscr_cond->service_name) {
		...
		{
			ogs_error(
				"OpenAPI_subscr_cond_convertToJSON() failed [service_name]");
			goto end;
		}
	}

	if(subscr_cond->service_name_list) {
		cJSON *service_name_listList = cJSON_AddArrayToObject(item,
		                                                      "serviceNameList");
		if(service_name_listList == NULL) {
			ogs_error(
				"OpenAPI_subscr_cond_convertToJSON() failed [service_name_list]");
			goto end;
		}
		OpenAPI_list_for_each(subscr_cond->service_name_list, node) {
			if(cJSON_AddStringToObject(service_name_listList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_subscr_cond_convertToJSON() failed [service_name_list]");
	...
	if(subscr_cond->amf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_subscr_cond_convertToJSON() failed [amf_set_id]");
			goto end;
		}
	}
	...
	if(subscr_cond->amf_region_id) {
		...
		{
			ogs_error(
				"OpenAPI_subscr_cond_convertToJSON() failed [amf_region_id]");
			goto end;
		}
	}

	if(subscr_cond->guami_list) {
		cJSON *guami_listList = cJSON_AddArrayToObject(item,
		                                               "guamiList");
		if(guami_listList == NULL) {
			ogs_error(
				"OpenAPI_subscr_cond_convertToJSON() failed [guami_list]");
			goto end;
		}
		OpenAPI_list_for_each(subscr_cond->guami_list, node) {
			cJSON *itemLocal =
	...
	if(subscr_cond->snssai_list) {
		...
		if(snssai_listList == NULL) {
			ogs_error(
				"OpenAPI_subscr_cond_convertToJSON() failed [snssai_list]");
			goto end;
		}
		OpenAPI_list_for_each(subscr_cond->snssai_list, node) {
			cJSON *itemLocal =
	...
	if(subscr_cond->nsi_list) {
		...
		if(nsi_listList == NULL) {
			ogs_error(
				"OpenAPI_subscr_cond_convertToJSON() failed [nsi_list]");
			goto end;
		}
		OpenAPI_list_for_each(subscr_cond->nsi_list, node) {
	...
	if(subscr_cond->nf_group_id) {
		if(cJSON_AddStringToObject(item, "nfGroupId",
		                           subscr_cond->nf_group_id) == NULL)
		{
			ogs_error(
				"OpenAPI_subscr_cond_convertToJSON() failed [nf_group_id]");
			goto end;
		}
	}
	...
	if(subscr_cond->nf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_subscr_cond_convertToJSON() failed [nf_set_id]");
			goto end;
		}
	}

	if(subscr_cond->nf_service_set_id) {
		if(cJSON_AddStringToObject(item, "nfServiceSetId",
		                           subscr_cond->nf_service_set_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_subscr_cond_convertToJSON() failed [nf_service_set_id]");
			goto end;
		}
	}
	...
	if(subscr_cond->smf_serving_area) {
		...
		if(smf_serving_areaList == NULL) {
			ogs_error(
				"OpenAPI_subscr_cond_convertToJSON() failed [smf_serving_area]");
			goto end;
		}
		OpenAPI_list_for_each(subscr_cond->smf_serving_area, node) {
	...
	if(subscr_cond->tai_list) {
		...
		if(tai_listList == NULL) {
			ogs_error(
				"OpenAPI_subscr_cond_convertToJSON() failed [tai_list]");
			goto end;
		}
		OpenAPI_list_for_each(subscr_cond->tai_list, node) {
			cJSON *itemLocal =
...
OpenAPI_subscr_cond_t *OpenAPI_subscr_cond_parseFromJSON(
	cJSON *subscr_condJSON) {
	...
	nf_instance_id = cJSON_GetObjectItemCaseSensitive(subscr_condJSON,
	                                                  "nfInstanceId");
	if(nf_instance_id) {
		...
		{
			ogs_error(
				"OpenAPI_subscr_cond_parseFromJSON() failed [nf_instance_id]");
			goto end;
		}
	}

	nf_instance_list = cJSON_GetObjectItemCaseSensitive(subscr_condJSON,
	                                                    "nfInstanceList");
	if(nf_instance_list) {
		cJSON *nf_instance_list_local = NULL;
		if(!cJSON_IsArray(nf_instance_list)) {
			ogs_error(
				"OpenAPI_subscr_cond_parseFromJSON() failed [nf_instance_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(nf_instance_list_local, nf_instance_list) {
			...
			OpenAPI_list_add(nf_instance_listList,
			                 ogs_strdup(
						 nf_instance_list_local->
						 valuestring));
		}
	}
	...
	if(nf_type) {
		if(!cJSON_IsString(nf_type)) {
			ogs_error(
				"OpenAPI_subscr_cond_parseFromJSON() failed [nf_type]");
			goto end;
		}
		nf_typeVariable =
	...
	service_name = cJSON_GetObjectItemCaseSensitive(subscr_condJSON,
	                                                "serviceName");
	if(service_name) {
		...
		{
			ogs_error(
				"OpenAPI_subscr_cond_parseFromJSON() failed [service_name]");
			goto end;
		}
	}

	service_name_list = cJSON_GetObjectItemCaseSensitive(subscr_condJSON,
	                                                     "serviceNameList");
	if(service_name_list) {
		cJSON *service_name_list_local = NULL;
		if(!cJSON_IsArray(service_name_list)) {
			ogs_error(
				"OpenAPI_subscr_cond_parseFromJSON() failed [service_name_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(service_name_list_local, service_name_list) {
			...
			OpenAPI_list_add(service_name_listList,
			                 ogs_strdup(
						 service_name_list_local->
						 valuestring));
		}
	}

	amf_set_id = cJSON_GetObjectItemCaseSensitive(subscr_condJSON,
	                                              "amfSetId");
	if(amf_set_id) {
		if(!cJSON_IsString(amf_set_id) &&
		   !cJSON_IsNull(amf_set_id))
		{
			ogs_error(
				"OpenAPI_subscr_cond_parseFromJSON() failed [amf_set_id]");
			goto end;
		}
	}

	amf_region_id = cJSON_GetObjectItemCaseSensitive(subscr_condJSON,
	                                                 "amfRegionId");
	if(amf_region_id) {
		if(!cJSON_IsString(amf_region_id) &&
		   !cJSON_IsNull(amf_region_id))
		{
			ogs_error(
				"OpenAPI_subscr_cond_parseFromJSON() failed [amf_region_id]");
			goto end;
		}
	}

	guami_list = cJSON_GetObjectItemCaseSensitive(subscr_condJSON,
	                                              "guamiList");
	if(guami_list) {
		cJSON *guami_list_local = NULL;
		if(!cJSON_IsArray(guami_list)) {
			ogs_error(
				"OpenAPI_subscr_cond_parseFromJSON() failed [guami_list]");
			goto end;
		}
	...
	snssai_list = cJSON_GetObjectItemCaseSensitive(subscr_condJSON,
	                                               "snssaiList");
	if(snssai_list) {
		cJSON *snssai_list_local = NULL;
		if(!cJSON_IsArray(snssai_list)) {
			ogs_error(
				"OpenAPI_subscr_cond_parseFromJSON() failed [snssai_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(snssai_list_local, snssai_list) {
			...
			OpenAPI_snssai_t *snssai_listItem =
				OpenAPI_snssai_parseFromJSON(snssai_list_local);
			if(!snssai_listItem) {
	...
	if(nsi_list) {
		...
		if(!cJSON_IsArray(nsi_list)) {
			ogs_error(
				"OpenAPI_subscr_cond_parseFromJSON() failed [nsi_list]");
			goto end;
		}
	...
	nf_group_id = cJSON_GetObjectItemCaseSensitive(subscr_condJSON,
	                                               "nfGroupId");
	if(nf_group_id) {
		if(!cJSON_IsString(nf_group_id) &&
		   !cJSON_IsNull(nf_group_id))
		{
			ogs_error(
				"OpenAPI_subscr_cond_parseFromJSON() failed [nf_group_id]");
			goto end;
		}
	}

	nf_set_id = cJSON_GetObjectItemCaseSensitive(subscr_condJSON,
	                                             "nfSetId");
	if(nf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_subscr_cond_parseFromJSON() failed [nf_set_id]");
			goto end;
		}
	}

	nf_service_set_id = cJSON_GetObjectItemCaseSensitive(subscr_condJSON,
	                                                     "nfServiceSetId");
	if(nf_service_set_id) {
		if(!cJSON_IsString(nf_service_set_id) &&
		   !cJSON_IsNull(nf_service_set_id))
		{
			ogs_error(
				"OpenAPI_subscr_cond_parseFromJSON() failed [nf_service_set_id]");
			goto end;
		}
	}

	smf_serving_area = cJSON_GetObjectItemCaseSensitive(subscr_condJSON,
	                                                    "smfServingArea");
	if(smf_serving_area) {
		cJSON *smf_serving_area_local = NULL;
		if(!cJSON_IsArray(smf_serving_area)) {
			ogs_error(
				"OpenAPI_subscr_cond_parseFromJSON() failed [smf_serving_area]");
			goto end;
		}
	...
	if(tai_list) {
		...
		if(!cJSON_IsArray(tai_list)) {
			ogs_error(
				"OpenAPI_subscr_cond_parseFromJSON() failed [tai_list]");
			goto end;
		}
	...
	subscr_cond_local_var = OpenAPI_subscr_cond_create(
		nf_instance_id &&
		!cJSON_IsNull(nf_instance_id) ?
		ogs_strdup(nf_instance_id->valuestring) : NULL,
		...
		amf_set_id &&
		!cJSON_IsNull(amf_set_id) ?
		ogs_strdup(amf_set_id->valuestring) : NULL,
		amf_region_id &&
		!cJSON_IsNull(amf_region_id) ?
		ogs_strdup(amf_region_id->valuestring) : NULL,
		guami_list ? guami_listList : NULL,
		snssai_list ? snssai_listList : NULL,
		nsi_list ? nsi_listList : NULL,
		nf_group_id &&
		!cJSON_IsNull(nf_group_id) ?
		ogs_strdup(nf_group_id->valuestring) : NULL,
		nf_set_id &&
		!cJSON_IsNull(nf_set_id) ? ogs_strdup(nf_set_id->valuestring) :
		NULL,
		nf_service_set_id &&
		!cJSON_IsNull(nf_service_set_id) ?
		ogs_strdup(nf_service_set_id->valuestring) : NULL,
...
OpenAPI_subscr_cond_t *OpenAPI_subscr_cond_copy(OpenAPI_subscr_cond_t	*dst,
                                                OpenAPI_subscr_cond_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
