...
OpenAPI_target_area_t *OpenAPI_target_area_create(OpenAPI_list_t *ta_list,
                                                  OpenAPI_list_t *tai_range_list,
                                                  bool is_any_ta, int any_ta) {
	OpenAPI_target_area_t *target_area_local_var =
		ogs_malloc(sizeof(OpenAPI_target_area_t));
...
cJSON *OpenAPI_target_area_convertToJSON(OpenAPI_target_area_t *target_area) {
	...
	if(target_area == NULL) {
		ogs_error(
			"OpenAPI_target_area_convertToJSON() failed [TargetArea]");
		return NULL;
	}
	...
	if(target_area->ta_list) {
		...
		if(ta_listList == NULL) {
			ogs_error(
				"OpenAPI_target_area_convertToJSON() failed [ta_list]");
			goto end;
		}
		OpenAPI_list_for_each(target_area->ta_list, node) {
			cJSON *itemLocal =
	...
	if(target_area->tai_range_list) {
		...
		if(tai_range_listList == NULL) {
			ogs_error(
				"OpenAPI_target_area_convertToJSON() failed [tai_range_list]");
			goto end;
		}
		OpenAPI_list_for_each(target_area->tai_range_list, node) {
			cJSON *itemLocal =
	...
	if(target_area->is_any_ta) {
		...
		{
			ogs_error(
				"OpenAPI_target_area_convertToJSON() failed [any_ta]");
			goto end;
		}
	}
...
OpenAPI_target_area_t *OpenAPI_target_area_parseFromJSON(
	cJSON *target_areaJSON) {
	...
	if(ta_list) {
		...
		if(!cJSON_IsArray(ta_list)) {
			ogs_error(
				"OpenAPI_target_area_parseFromJSON() failed [ta_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ta_list_local, ta_list) {
			...
			OpenAPI_tai_t *ta_listItem =
				OpenAPI_tai_parseFromJSON(ta_list_local);
			if(!ta_listItem) {
	...
	if(tai_range_list) {
		...
		if(!cJSON_IsArray(tai_range_list)) {
			ogs_error(
				"OpenAPI_target_area_parseFromJSON() failed [tai_range_list]");
			goto end;
		}
	...
	if(any_ta) {
		if(!cJSON_IsBool(any_ta)) {
			ogs_error(
				"OpenAPI_target_area_parseFromJSON() failed [any_ta]");
			goto end;
		}
	}
...
OpenAPI_target_area_t *OpenAPI_target_area_copy(OpenAPI_target_area_t	*dst,
                                                OpenAPI_target_area_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
