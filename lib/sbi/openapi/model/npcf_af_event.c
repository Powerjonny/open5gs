...
char * OpenAPI_npcf_af_event_ToString(OpenAPI_npcf_af_event_e npcf_af_event) {
	const char *npcf_af_eventArray[] = { "NULL", "ACCESS_TYPE_CHANGE",
		                             "ANI_REPORT", "APP_DETECTION",
		                             "CHARGING_CORRELATION",
		                             "EPS_FALLBACK",
		                             "FAILED_QOS_UPDATE",
		                             "FAILED_RESOURCES_ALLOCATION",
		                             "OUT_OF_CREDIT",
		                             "PDU_SESSION_STATUS", "PLMN_CHG",
		                             "QOS_MONITORING", "QOS_NOTIF",
		                             "RAN_NAS_CAUSE",
		                             ...
		                             "UP_PATH_CHG_FAILURE",
		                             "USAGE_REPORT" };
	size_t sizeofArray = sizeof(npcf_af_eventArray) /
	                     sizeof(npcf_af_eventArray[0]);
...
OpenAPI_npcf_af_event_e OpenAPI_npcf_af_event_FromString(char *npcf_af_event) {
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(npcf_af_event,
		          npcf_af_eventArray[stringToReturn]) == 0) {
