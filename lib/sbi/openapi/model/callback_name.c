...
OpenAPI_callback_name_t *OpenAPI_callback_name_create(char *callback_type) {
	OpenAPI_callback_name_t *callback_name_local_var =
		ogs_malloc(sizeof(OpenAPI_callback_name_t));
...
cJSON *OpenAPI_callback_name_convertToJSON(
	...
	if(callback_name == NULL) {
		ogs_error(
			"OpenAPI_callback_name_convertToJSON() failed [CallbackName]");
		return NULL;
	}
	...
	if(!callback_name->callback_type) {
		ogs_error(
			"OpenAPI_callback_name_convertToJSON() failed [callback_type]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "callbackType",
	                           callback_name->callback_type) == NULL)
	{
		ogs_error(
			"OpenAPI_callback_name_convertToJSON() failed [callback_type]");
...
OpenAPI_callback_name_t *OpenAPI_callback_name_parseFromJSON(
	...
	callback_type = cJSON_GetObjectItemCaseSensitive(callback_nameJSON,
	                                                 "callbackType");
	if(!callback_type) {
		ogs_error(
			"OpenAPI_callback_name_parseFromJSON() failed [callback_type]");
		goto end;
	}
...
OpenAPI_callback_name_t *OpenAPI_callback_name_copy(
	OpenAPI_callback_name_t *dst, OpenAPI_callback_name_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
