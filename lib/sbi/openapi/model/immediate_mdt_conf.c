...
OpenAPI_immediate_mdt_conf_t *OpenAPI_immediate_mdt_conf_create(
	OpenAPI_job_type_e job_type, OpenAPI_list_t *measurement_lte_list,
	...
	int event_threshold_rsrp, bool is_event_threshold_rsrq,
	int event_threshold_rsrq, bool is_event_threshold_rsrp_nr,
	int event_threshold_rsrp_nr, bool is_event_threshold_rsrq_nr,
	...
	OpenAPI_immediate_mdt_conf_t *immediate_mdt_conf_local_var =
		ogs_malloc(sizeof(OpenAPI_immediate_mdt_conf_t));
...
void OpenAPI_immediate_mdt_conf_free(
	...
	if(immediate_mdt_conf->mdt_allowed_plmn_id_list) {
		OpenAPI_list_for_each(
			immediate_mdt_conf->mdt_allowed_plmn_id_list, node) {
...
cJSON *OpenAPI_immediate_mdt_conf_convertToJSON(
	OpenAPI_immediate_mdt_conf_t *immediate_mdt_conf) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(immediate_mdt_conf == NULL) {
		ogs_error(
			"OpenAPI_immediate_mdt_conf_convertToJSON() failed [ImmediateMdtConf]");
		return NULL;
	}
	...
	if(immediate_mdt_conf->job_type == OpenAPI_job_type_NULL) {
		ogs_error(
			"OpenAPI_immediate_mdt_conf_convertToJSON() failed [job_type]");
		return NULL;
	}
	   ...
	   OpenAPI_measurement_lte_for_mdt_NULL)
	{
		...
		if(measurement_lte_listList == NULL) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_convertToJSON() failed [measurement_lte_list]");
			goto end;
		}
		OpenAPI_list_for_each(immediate_mdt_conf->measurement_lte_list,
		                      node) {
	   ...
	   OpenAPI_measurement_nr_for_mdt_NULL)
	{
		...
		if(measurement_nr_listList == NULL) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_convertToJSON() failed [measurement_nr_list]");
			goto end;
		}
		OpenAPI_list_for_each(immediate_mdt_conf->measurement_nr_list,
		                      node) {
	   ...
	   OpenAPI_reporting_trigger_NULL)
	{
		...
		if(reporting_trigger_listList == NULL) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_convertToJSON() failed [reporting_trigger_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			immediate_mdt_conf->reporting_trigger_list, node) {
	   ...
	   OpenAPI_report_interval_mdt_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_immediate_mdt_conf_convertToJSON() failed [report_interval]");
			goto end;
		}
	}
	   ...
	   OpenAPI_report_interval_nr_mdt_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_immediate_mdt_conf_convertToJSON() failed [report_interval_nr]");
			goto end;
		}
	}
	   ...
	   OpenAPI_report_amount_mdt_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_immediate_mdt_conf_convertToJSON() failed [report_amount]");
			goto end;
		}
	}
	...
	if(immediate_mdt_conf->is_event_threshold_rsrp) {
		...
		{
			ogs_error(
				"OpenAPI_immediate_mdt_conf_convertToJSON() failed [event_threshold_rsrp]");
			goto end;
		}
	}
	...
	if(immediate_mdt_conf->is_event_threshold_rsrq) {
		...
		{
			ogs_error(
				"OpenAPI_immediate_mdt_conf_convertToJSON() failed [event_threshold_rsrq]");
			goto end;
		}
	}
	...
	if(immediate_mdt_conf->is_event_threshold_rsrp_nr) {
		...
		{
			ogs_error(
				"OpenAPI_immediate_mdt_conf_convertToJSON() failed [event_threshold_rsrp_nr]");
			goto end;
		}
	}
	...
	if(immediate_mdt_conf->is_event_threshold_rsrq_nr) {
		...
		{
			ogs_error(
				"OpenAPI_immediate_mdt_conf_convertToJSON() failed [event_threshold_rsrq_nr]");
			goto end;
		}
	}
	   ...
	   OpenAPI_collection_period_rmm_lte_mdt_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_immediate_mdt_conf_convertToJSON() failed [collection_period_rmm_lte]");
			goto end;
		}
	}
	   ...
	   OpenAPI_collection_period_rmm_nr_mdt_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_immediate_mdt_conf_convertToJSON() failed [collection_period_rmm_nr]");
			goto end;
		}
	}
	   ...
	   OpenAPI_measurement_period_lte_mdt_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_immediate_mdt_conf_convertToJSON() failed [measurement_period_lte]");
			goto end;
		}
	}
	...
	if(immediate_mdt_conf->area_scope) {
		...
		if(area_scope_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_convertToJSON() failed [area_scope]");
			goto end;
		}
	   ...
	   OpenAPI_positioning_method_mdt_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_immediate_mdt_conf_convertToJSON() failed [positioning_method]");
			goto end;
		}
	}
	   ...
	   OpenAPI_positioning_method_mdt_NULL)
	{
		...
		if(add_positioning_method_listList == NULL) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_convertToJSON() failed [add_positioning_method_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			immediate_mdt_conf->add_positioning_method_list, node) {
	...
	if(immediate_mdt_conf->mdt_allowed_plmn_id_list) {
		...
		if(mdt_allowed_plmn_id_listList == NULL) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_convertToJSON() failed [mdt_allowed_plmn_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			immediate_mdt_conf->mdt_allowed_plmn_id_list, node) {
	   ...
	   OpenAPI_sensor_measurement_NULL)
	{
		...
		if(sensor_measurement_listList == NULL) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_convertToJSON() failed [sensor_measurement_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			immediate_mdt_conf->sensor_measurement_list, node) {
...
OpenAPI_immediate_mdt_conf_t *OpenAPI_immediate_mdt_conf_parseFromJSON(
	...
	if(!job_type) {
		ogs_error(
			"OpenAPI_immediate_mdt_conf_parseFromJSON() failed [job_type]");
		goto end;
	}
	...
	measurement_lte_list =
		cJSON_GetObjectItemCaseSensitive(immediate_mdt_confJSON,
		                                 "measurementLteList");
	if(measurement_lte_list) {
		cJSON *measurement_lte_list_local = NULL;
		if(!cJSON_IsArray(measurement_lte_list)) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed [measurement_lte_list]");
			goto end;
		}
		...
		if(measurement_lte_listList->count == 0) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed: Expected measurement_lte_listList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(measurement_nr_list) {
		...
		if(!cJSON_IsArray(measurement_nr_list)) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed [measurement_nr_list]");
			goto end;
		}
		...
		if(measurement_nr_listList->count == 0) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed: Expected measurement_nr_listList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(reporting_trigger_list) {
		...
		if(!cJSON_IsArray(reporting_trigger_list)) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed [reporting_trigger_list]");
			goto end;
		}
		...
		if(reporting_trigger_listList->count == 0) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed: Expected reporting_trigger_listList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(report_interval) {
		if(!cJSON_IsString(report_interval)) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed [report_interval]");
			goto end;
		}
	...
	if(report_interval_nr) {
		if(!cJSON_IsString(report_interval_nr)) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed [report_interval_nr]");
			goto end;
		}
	...
	if(report_amount) {
		if(!cJSON_IsString(report_amount)) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed [report_amount]");
			goto end;
		}
	...
	if(event_threshold_rsrp) {
		if(!cJSON_IsNumber(event_threshold_rsrp)) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed [event_threshold_rsrp]");
			goto end;
		}
	}
	...
	if(event_threshold_rsrq) {
		if(!cJSON_IsNumber(event_threshold_rsrq)) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed [event_threshold_rsrq]");
			goto end;
		}
	}
	...
	if(event_threshold_rsrp_nr) {
		if(!cJSON_IsNumber(event_threshold_rsrp_nr)) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed [event_threshold_rsrp_nr]");
			goto end;
		}
	}
	...
	if(event_threshold_rsrq_nr) {
		if(!cJSON_IsNumber(event_threshold_rsrq_nr)) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed [event_threshold_rsrq_nr]");
			goto end;
		}
	}
	...
	if(collection_period_rmm_lte) {
		if(!cJSON_IsString(collection_period_rmm_lte)) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed [collection_period_rmm_lte]");
			goto end;
		}
	...
	if(collection_period_rmm_nr) {
		if(!cJSON_IsString(collection_period_rmm_nr)) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed [collection_period_rmm_nr]");
			goto end;
		}
	...
	if(measurement_period_lte) {
		if(!cJSON_IsString(measurement_period_lte)) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed [measurement_period_lte]");
			goto end;
		}
	...
	if(positioning_method) {
		if(!cJSON_IsString(positioning_method)) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed [positioning_method]");
			goto end;
		}
	...
	if(add_positioning_method_list) {
		...
		if(!cJSON_IsArray(add_positioning_method_list)) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed [add_positioning_method_list]");
			goto end;
		}
		...
		if(add_positioning_method_listList->count == 0) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed: Expected add_positioning_method_listList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(mdt_allowed_plmn_id_list) {
		...
		if(!cJSON_IsArray(mdt_allowed_plmn_id_list)) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed [mdt_allowed_plmn_id_list]");
			goto end;
		}
	...
	if(sensor_measurement_list) {
		...
		if(!cJSON_IsArray(sensor_measurement_list)) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed [sensor_measurement_list]");
			goto end;
		}
		...
		if(sensor_measurement_listList->count == 0) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON() failed: Expected sensor_measurement_listList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
...
OpenAPI_immediate_mdt_conf_t *OpenAPI_immediate_mdt_conf_copy(
	OpenAPI_immediate_mdt_conf_t *dst, OpenAPI_immediate_mdt_conf_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
