...
char * OpenAPI_rm_state_any_of_ToString(
	...
	const char *rm_state_any_ofArray[] = { "NULL", "REGISTERED",
		                               "DEREGISTERED" };
	size_t sizeofArray = sizeof(rm_state_any_ofArray) /
	                     sizeof(rm_state_any_ofArray[0]);
...
OpenAPI_rm_state_any_of_e OpenAPI_rm_state_any_of_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(rm_state_any_of,
		          rm_state_any_ofArray[stringToReturn]) == 0) {
