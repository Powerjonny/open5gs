...
OpenAPI_pc5_qo_s_para_t *OpenAPI_pc5_qo_s_para_create(
	OpenAPI_list_t *pc5_qos_flow_list, char *pc5_link_ambr) {
	OpenAPI_pc5_qo_s_para_t *pc5_qo_s_para_local_var =
		ogs_malloc(sizeof(OpenAPI_pc5_qo_s_para_t));
...
cJSON *OpenAPI_pc5_qo_s_para_convertToJSON(
	...
	if(pc5_qo_s_para == NULL) {
		ogs_error(
			"OpenAPI_pc5_qo_s_para_convertToJSON() failed [Pc5QoSPara]");
		return NULL;
	}
	...
	if(!pc5_qo_s_para->pc5_qos_flow_list) {
		ogs_error(
			"OpenAPI_pc5_qo_s_para_convertToJSON() failed [pc5_qos_flow_list]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(pc5_qo_s_para->pc5_qos_flow_list, node) {
		cJSON *itemLocal =
			OpenAPI_pc5_qos_flow_item_convertToJSON(node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_pc5_qo_s_para_convertToJSON() failed [pc5_qos_flow_list]");
			goto end;
	...
	if(pc5_qo_s_para->pc5_link_ambr) {
		...
		{
			ogs_error(
				"OpenAPI_pc5_qo_s_para_convertToJSON() failed [pc5_link_ambr]");
			goto end;
		}
	}
...
OpenAPI_pc5_qo_s_para_t *OpenAPI_pc5_qo_s_para_parseFromJSON(
	cJSON *pc5_qo_s_paraJSON) {
	...
	if(!pc5_qos_flow_list) {
		ogs_error(
			"OpenAPI_pc5_qo_s_para_parseFromJSON() failed [pc5_qos_flow_list]");
		goto end;
	}
	...
	cJSON_ArrayForEach(pc5_qos_flow_list_local, pc5_qos_flow_list) {
		...
		OpenAPI_pc5_qos_flow_item_t *pc5_qos_flow_listItem =
			OpenAPI_pc5_qos_flow_item_parseFromJSON(
				pc5_qos_flow_list_local);
	...
	if(pc5_link_ambr) {
		...
		{
			ogs_error(
				"OpenAPI_pc5_qo_s_para_parseFromJSON() failed [pc5_link_ambr]");
			goto end;
		}
	}
	...
	pc5_qo_s_para_local_var = OpenAPI_pc5_qo_s_para_create(
		...
		pc5_link_ambr &&
		!cJSON_IsNull(pc5_link_ambr) ?
		ogs_strdup(pc5_link_ambr->valuestring) : NULL
		);
...
OpenAPI_pc5_qo_s_para_t *OpenAPI_pc5_qo_s_para_copy(
	OpenAPI_pc5_qo_s_para_t *dst, OpenAPI_pc5_qo_s_para_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
