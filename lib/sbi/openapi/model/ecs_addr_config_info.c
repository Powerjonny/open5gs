...
OpenAPI_ecs_addr_config_info_t *OpenAPI_ecs_addr_config_info_create(
	...
	OpenAPI_ecs_addr_config_info_t *ecs_addr_config_info_local_var =
		ogs_malloc(sizeof(OpenAPI_ecs_addr_config_info_t));
	ogs_assert(ecs_addr_config_info_local_var);

	ecs_addr_config_info_local_var->ecs_server_addr = ecs_server_addr;
	ecs_addr_config_info_local_var->spatial_validity_cond =
...
cJSON *OpenAPI_ecs_addr_config_info_convertToJSON(
	...
	if(ecs_addr_config_info == NULL) {
		ogs_error(
			"OpenAPI_ecs_addr_config_info_convertToJSON() failed [EcsAddrConfigInfo]");
		return NULL;
	}
	...
	if(ecs_addr_config_info->ecs_server_addr) {
		...
		if(ecs_server_addr_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ecs_addr_config_info_convertToJSON() failed [ecs_server_addr]");
			goto end;
		}
	...
	if(ecs_addr_config_info->spatial_validity_cond) {
		cJSON *spatial_validity_cond_local_JSON =
			OpenAPI_spatial_validity_cond_convertToJSON(
				ecs_addr_config_info->spatial_validity_cond);
		if(spatial_validity_cond_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ecs_addr_config_info_convertToJSON() failed [spatial_validity_cond]");
			goto end;
		}
...
OpenAPI_ecs_addr_config_info_t *OpenAPI_ecs_addr_config_info_parseFromJSON(
	cJSON *ecs_addr_config_infoJSON) {
	...
	OpenAPI_spatial_validity_cond_t *spatial_validity_cond_local_nonprim =
		NULL;
	ecs_server_addr =
		cJSON_GetObjectItemCaseSensitive(ecs_addr_config_infoJSON,
		                                 "ecsServerAddr");
	if(ecs_server_addr) {
	...
	if(spatial_validity_cond) {
		spatial_validity_cond_local_nonprim =
			OpenAPI_spatial_validity_cond_parseFromJSON(
				spatial_validity_cond);
		if(!spatial_validity_cond_local_nonprim) {
			ogs_error(
				"OpenAPI_spatial_validity_cond_parseFromJSON failed [spatial_validity_cond]");
			goto end;
		}
	}
	...
	ecs_addr_config_info_local_var = OpenAPI_ecs_addr_config_info_create(
		...
		spatial_validity_cond ? spatial_validity_cond_local_nonprim :
		NULL
		);
...
OpenAPI_ecs_addr_config_info_t *OpenAPI_ecs_addr_config_info_copy(
	OpenAPI_ecs_addr_config_info_t	*dst,
	OpenAPI_ecs_addr_config_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ecs_addr_config_info_convertToJSON() failed");
		return NULL;
	}
