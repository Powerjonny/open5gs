...
OpenAPI_dispersion_requirement_t *OpenAPI_dispersion_requirement_create(
	...
	OpenAPI_dispersion_requirement_t *dispersion_requirement_local_var =
		ogs_malloc(sizeof(OpenAPI_dispersion_requirement_t));
...
void OpenAPI_dispersion_requirement_free(
	...
	if(dispersion_requirement->rank_criters) {
		OpenAPI_list_for_each(dispersion_requirement->rank_criters,
		                      node) {
	...
	if(dispersion_requirement->disp_order_criter) {
		OpenAPI_dispersion_ordering_criterion_free(
...
cJSON *OpenAPI_dispersion_requirement_convertToJSON(
	...
	if(dispersion_requirement == NULL) {
		ogs_error(
			"OpenAPI_dispersion_requirement_convertToJSON() failed [DispersionRequirement]");
		return NULL;
	}
	...
	if(!dispersion_requirement->disper_type) {
		ogs_error(
			"OpenAPI_dispersion_requirement_convertToJSON() failed [disper_type]");
		return NULL;
	}
	cJSON *disper_type_local_JSON =
		OpenAPI_dispersion_type_convertToJSON(
			dispersion_requirement->disper_type);
	...
	if(dispersion_requirement->class_criters) {
		...
		if(class_critersList == NULL) {
			ogs_error(
				"OpenAPI_dispersion_requirement_convertToJSON() failed [class_criters]");
			goto end;
		}
		OpenAPI_list_for_each(dispersion_requirement->class_criters,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_class_criterion_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_dispersion_requirement_convertToJSON() failed [class_criters]");
	...
	if(dispersion_requirement->rank_criters) {
		...
		if(rank_critersList == NULL) {
			ogs_error(
				"OpenAPI_dispersion_requirement_convertToJSON() failed [rank_criters]");
			goto end;
		}
		OpenAPI_list_for_each(dispersion_requirement->rank_criters,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_ranking_criterion_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_dispersion_requirement_convertToJSON() failed [rank_criters]");
	...
	if(dispersion_requirement->disp_order_criter) {
		cJSON *disp_order_criter_local_JSON =
			OpenAPI_dispersion_ordering_criterion_convertToJSON(
				dispersion_requirement->disp_order_criter);
		if(disp_order_criter_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_dispersion_requirement_convertToJSON() failed [disp_order_criter]");
			goto end;
		}
...
OpenAPI_dispersion_requirement_t *OpenAPI_dispersion_requirement_parseFromJSON(
	...
	OpenAPI_dispersion_ordering_criterion_t *disp_order_criter_local_nonprim
	        = NULL;
	cJSON *order = NULL;
	OpenAPI_matching_direction_t *order_local_nonprim = NULL;
	disper_type =
		cJSON_GetObjectItemCaseSensitive(dispersion_requirementJSON,
		                                 "disperType");
	if(!disper_type) {
		ogs_error(
			"OpenAPI_dispersion_requirement_parseFromJSON() failed [disper_type]");
		goto end;
	}
	disper_type_local_nonprim =
		OpenAPI_dispersion_type_parseFromJSON(disper_type);
	if(!disper_type_local_nonprim) {
		ogs_error(
			"OpenAPI_dispersion_type_parseFromJSON failed [disper_type]");
		goto end;
	}
	...
	if(class_criters) {
		...
		if(!cJSON_IsArray(class_criters)) {
			ogs_error(
				"OpenAPI_dispersion_requirement_parseFromJSON() failed [class_criters]");
			goto end;
		}
		...
		cJSON_ArrayForEach(class_criters_local, class_criters) {
			...
			OpenAPI_class_criterion_t *class_critersItem =
	...
	if(rank_criters) {
		...
		if(!cJSON_IsArray(rank_criters)) {
			ogs_error(
				"OpenAPI_dispersion_requirement_parseFromJSON() failed [rank_criters]");
			goto end;
		}
		...
		cJSON_ArrayForEach(rank_criters_local, rank_criters) {
			...
			OpenAPI_ranking_criterion_t *rank_critersItem =
				OpenAPI_ranking_criterion_parseFromJSON(
					rank_criters_local);
			if(!rank_critersItem) {
	...
	if(disp_order_criter) {
		...
		if(!disp_order_criter_local_nonprim) {
			ogs_error(
				"OpenAPI_dispersion_ordering_criterion_parseFromJSON failed [disp_order_criter]");
			goto end;
		}
	}
	...
	if(order) {
		order_local_nonprim =
			OpenAPI_matching_direction_parseFromJSON(order);
		if(!order_local_nonprim) {
			ogs_error(
				"OpenAPI_matching_direction_parseFromJSON failed [order]");
			goto end;
		}
	}

	dispersion_requirement_local_var =
		OpenAPI_dispersion_requirement_create(
			disper_type_local_nonprim,
			class_criters ? class_critersList : NULL,
			rank_criters ? rank_critersList : NULL,
			disp_order_criter ? disp_order_criter_local_nonprim :
			NULL,
...
OpenAPI_dispersion_requirement_t *OpenAPI_dispersion_requirement_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_dispersion_requirement_convertToJSON() failed");
		return NULL;
	}
