...
OpenAPI_expected_ue_behavior_t *OpenAPI_expected_ue_behavior_create(
	OpenAPI_list_t *exp_move_trajectory, char *validity_time) {
	OpenAPI_expected_ue_behavior_t *expected_ue_behavior_local_var =
		ogs_malloc(sizeof(OpenAPI_expected_ue_behavior_t));
...
void OpenAPI_expected_ue_behavior_free(
	OpenAPI_expected_ue_behavior_t *expected_ue_behavior) {
	...
	if(expected_ue_behavior->exp_move_trajectory) {
		OpenAPI_list_for_each(expected_ue_behavior->exp_move_trajectory,
		                      node) {
...
cJSON *OpenAPI_expected_ue_behavior_convertToJSON(
	OpenAPI_expected_ue_behavior_t *expected_ue_behavior) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(expected_ue_behavior == NULL) {
		ogs_error(
			"OpenAPI_expected_ue_behavior_convertToJSON() failed [ExpectedUeBehavior]");
		return NULL;
	}
	...
	if(!expected_ue_behavior->exp_move_trajectory) {
		ogs_error(
			"OpenAPI_expected_ue_behavior_convertToJSON() failed [exp_move_trajectory]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(expected_ue_behavior->exp_move_trajectory, node) {
		cJSON *itemLocal =
			OpenAPI_user_location_convertToJSON(node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_expected_ue_behavior_convertToJSON() failed [exp_move_trajectory]");
			goto end;
	...
	if(!expected_ue_behavior->validity_time) {
		ogs_error(
			"OpenAPI_expected_ue_behavior_convertToJSON() failed [validity_time]");
		return NULL;
	}
...
OpenAPI_expected_ue_behavior_t *OpenAPI_expected_ue_behavior_parseFromJSON(
	...
	exp_move_trajectory =
		cJSON_GetObjectItemCaseSensitive(expected_ue_behaviorJSON,
		                                 "expMoveTrajectory");
	if(!exp_move_trajectory) {
		ogs_error(
			"OpenAPI_expected_ue_behavior_parseFromJSON() failed [exp_move_trajectory]");
		goto end;
	}
	...
	cJSON_ArrayForEach(exp_move_trajectory_local, exp_move_trajectory) {
		...
		OpenAPI_user_location_t *exp_move_trajectoryItem =
			OpenAPI_user_location_parseFromJSON(
				exp_move_trajectory_local);
		if(!exp_move_trajectoryItem) {
			ogs_error("No exp_move_trajectoryItem");
			goto end;
		}
		OpenAPI_list_add(exp_move_trajectoryList,
	...
	if(!validity_time) {
		ogs_error(
			"OpenAPI_expected_ue_behavior_parseFromJSON() failed [validity_time]");
		goto end;
	}
...
OpenAPI_expected_ue_behavior_t *OpenAPI_expected_ue_behavior_copy(
	OpenAPI_expected_ue_behavior_t	*dst,
	OpenAPI_expected_ue_behavior_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_expected_ue_behavior_convertToJSON() failed");
		return NULL;
	}
