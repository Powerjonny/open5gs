...
OpenAPI_location_filter_t *OpenAPI_location_filter_create() {
	OpenAPI_location_filter_t *location_filter_local_var =
		ogs_malloc(sizeof(OpenAPI_location_filter_t));
...
cJSON *OpenAPI_location_filter_convertToJSON(
	...
	if(location_filter == NULL) {
		ogs_error(
			"OpenAPI_location_filter_convertToJSON() failed [LocationFilter]");
		return NULL;
	}
