...
OpenAPI_ue_context_release_t *OpenAPI_ue_context_release_create(char *supi,
                                                                bool
                                                                ...
                                                                *ngap_cause) {
	OpenAPI_ue_context_release_t *ue_context_release_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_context_release_t));
...
void OpenAPI_ue_context_release_free(
	OpenAPI_ue_context_release_t *ue_context_release) {
...
cJSON *OpenAPI_ue_context_release_convertToJSON(
	OpenAPI_ue_context_release_t *ue_context_release) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(ue_context_release == NULL) {
		ogs_error(
			"OpenAPI_ue_context_release_convertToJSON() failed [UEContextRelease]");
		return NULL;
	}
	...
	if(ue_context_release->supi) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_release_convertToJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(ue_context_release->is_unauthenticated_supi) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_release_convertToJSON() failed [unauthenticated_supi]");
			goto end;
		}
	}
	...
	if(!ue_context_release->ngap_cause) {
		ogs_error(
			"OpenAPI_ue_context_release_convertToJSON() failed [ngap_cause]");
		return NULL;
	}
...
OpenAPI_ue_context_release_t *OpenAPI_ue_context_release_parseFromJSON(
	...
	if(supi) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_release_parseFromJSON() failed [supi]");
			goto end;
		}
	}

	unauthenticated_supi =
		cJSON_GetObjectItemCaseSensitive(ue_context_releaseJSON,
		                                 "unauthenticatedSupi");
	if(unauthenticated_supi) {
		if(!cJSON_IsBool(unauthenticated_supi)) {
			ogs_error(
				"OpenAPI_ue_context_release_parseFromJSON() failed [unauthenticated_supi]");
			goto end;
		}
	}
	...
	if(!ngap_cause) {
		ogs_error(
			"OpenAPI_ue_context_release_parseFromJSON() failed [ngap_cause]");
		goto end;
	}
...
OpenAPI_ue_context_release_t *OpenAPI_ue_context_release_copy(
	OpenAPI_ue_context_release_t *dst, OpenAPI_ue_context_release_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
