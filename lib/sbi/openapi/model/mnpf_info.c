...
OpenAPI_mnpf_info_t *OpenAPI_mnpf_info_create(OpenAPI_list_t *msisdn_ranges) {
	OpenAPI_mnpf_info_t *mnpf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_mnpf_info_t));
...
cJSON *OpenAPI_mnpf_info_convertToJSON(OpenAPI_mnpf_info_t *mnpf_info) {
	...
	if(mnpf_info == NULL) {
		ogs_error(
			"OpenAPI_mnpf_info_convertToJSON() failed [MnpfInfo]");
		return NULL;
	}
	...
	if(!mnpf_info->msisdn_ranges) {
		ogs_error(
			"OpenAPI_mnpf_info_convertToJSON() failed [msisdn_ranges]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(mnpf_info->msisdn_ranges, node) {
		cJSON *itemLocal =
			OpenAPI_identity_range_convertToJSON(node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_mnpf_info_convertToJSON() failed [msisdn_ranges]");
			goto end;
...
OpenAPI_mnpf_info_t *OpenAPI_mnpf_info_parseFromJSON(cJSON *mnpf_infoJSON) {
	...
	msisdn_ranges = cJSON_GetObjectItemCaseSensitive(mnpf_infoJSON,
	                                                 "msisdnRanges");
	if(!msisdn_ranges) {
		ogs_error(
			"OpenAPI_mnpf_info_parseFromJSON() failed [msisdn_ranges]");
		goto end;
	}
