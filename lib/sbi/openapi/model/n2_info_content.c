...
OpenAPI_n2_info_content_t *OpenAPI_n2_info_content_create(
	bool is_ngap_message_type, int ngap_message_type,
	OpenAPI_ngap_ie_type_e ngap_ie_type,
	OpenAPI_ref_to_binary_data_t *ngap_data) {
	OpenAPI_n2_info_content_t *n2_info_content_local_var =
		ogs_malloc(sizeof(OpenAPI_n2_info_content_t));
...
cJSON *OpenAPI_n2_info_content_convertToJSON(
	...
	if(n2_info_content == NULL) {
		ogs_error(
			"OpenAPI_n2_info_content_convertToJSON() failed [N2InfoContent]");
		return NULL;
	}
	...
	if(n2_info_content->is_ngap_message_type) {
		...
		{
			ogs_error(
				"OpenAPI_n2_info_content_convertToJSON() failed [ngap_message_type]");
			goto end;
		}
	}
	...
	if(n2_info_content->ngap_ie_type != OpenAPI_ngap_ie_type_NULL) {
		if(cJSON_AddStringToObject(item, "ngapIeType",
		                           OpenAPI_ngap_ie_type_ToString(
						   n2_info_content->ngap_ie_type))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_n2_info_content_convertToJSON() failed [ngap_ie_type]");
			goto end;
		}
	}
	...
	if(!n2_info_content->ngap_data) {
		ogs_error(
			"OpenAPI_n2_info_content_convertToJSON() failed [ngap_data]");
		return NULL;
	}
	cJSON *ngap_data_local_JSON =
...
OpenAPI_n2_info_content_t *OpenAPI_n2_info_content_parseFromJSON(
	cJSON *n2_info_contentJSON) {
	...
	ngap_message_type =
		cJSON_GetObjectItemCaseSensitive(n2_info_contentJSON,
		                                 "ngapMessageType");
	if(ngap_message_type) {
		if(!cJSON_IsNumber(ngap_message_type)) {
			ogs_error(
				"OpenAPI_n2_info_content_parseFromJSON() failed [ngap_message_type]");
			goto end;
		}
	}
	...
	if(ngap_ie_type) {
		if(!cJSON_IsString(ngap_ie_type)) {
			ogs_error(
				"OpenAPI_n2_info_content_parseFromJSON() failed [ngap_ie_type]");
			goto end;
		}
		ngap_ie_typeVariable =
			OpenAPI_ngap_ie_type_FromString(
	...
	if(!ngap_data) {
		ogs_error(
			"OpenAPI_n2_info_content_parseFromJSON() failed [ngap_data]");
		goto end;
	}
	ngap_data_local_nonprim =
		OpenAPI_ref_to_binary_data_parseFromJSON(ngap_data);
	if(!ngap_data_local_nonprim) {
		ogs_error(
			"OpenAPI_ref_to_binary_data_parseFromJSON failed [ngap_data]");
		goto end;
	}
...
OpenAPI_n2_info_content_t *OpenAPI_n2_info_content_copy(
	OpenAPI_n2_info_content_t *dst, OpenAPI_n2_info_content_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
