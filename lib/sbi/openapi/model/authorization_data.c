...
OpenAPI_authorization_data_t *OpenAPI_authorization_data_create(
	OpenAPI_set_t *authorization_data, OpenAPI_list_t *allowed_dnn_list,
	OpenAPI_list_t *allowed_snssai_list,
	OpenAPI_list_t *allowed_mtc_providers, char *validity_time) {
	OpenAPI_authorization_data_t *authorization_data_local_var =
		ogs_malloc(sizeof(OpenAPI_authorization_data_t));
	...
	authorization_data_local_var->allowed_mtc_providers =
		allowed_mtc_providers;
	authorization_data_local_var->validity_time = validity_time;

	return authorization_data_local_var;
}

void OpenAPI_authorization_data_free(
	OpenAPI_authorization_data_t *authorization_data) {
	...
	if(authorization_data->allowed_dnn_list) {
		OpenAPI_list_for_each(authorization_data->allowed_dnn_list,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(authorization_data->allowed_snssai_list) {
		OpenAPI_list_for_each(authorization_data->allowed_snssai_list,
		                      node) {
	...
	if(authorization_data->allowed_mtc_providers) {
		OpenAPI_list_for_each(authorization_data->allowed_mtc_providers,
		                      node) {
...
cJSON *OpenAPI_authorization_data_convertToJSON(
	OpenAPI_authorization_data_t *authorization_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(authorization_data == NULL) {
		ogs_error(
			"OpenAPI_authorization_data_convertToJSON() failed [AuthorizationData]");
		return NULL;
	}
	...
	if(!authorization_data->authorization_data) {
		ogs_error(
			"OpenAPI_authorization_data_convertToJSON() failed [authorization_data]");
		return NULL;
	}
	cJSON *authorization_dataList = cJSON_AddArrayToObject(item,
	                                                       "authorizationData");
	...
	OpenAPI_list_for_each(authorization_data->authorization_data, node) {
		cJSON *itemLocal =
			OpenAPI_user_identifier_convertToJSON(node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_authorization_data_convertToJSON() failed [authorization_data]");
			goto end;
	...
	if(authorization_data->allowed_dnn_list) {
		cJSON *allowed_dnn_listList = cJSON_AddArrayToObject(item,
		                                                     "allowedDnnList");
		if(allowed_dnn_listList == NULL) {
			ogs_error(
				"OpenAPI_authorization_data_convertToJSON() failed [allowed_dnn_list]");
			goto end;
		}
		OpenAPI_list_for_each(authorization_data->allowed_dnn_list,
		                      node) {
			if(cJSON_AddStringToObject(allowed_dnn_listList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_authorization_data_convertToJSON() failed [allowed_dnn_list]");
	...
	if(authorization_data->allowed_snssai_list) {
		...
		if(allowed_snssai_listList == NULL) {
			ogs_error(
				"OpenAPI_authorization_data_convertToJSON() failed [allowed_snssai_list]");
			goto end;
		}
		OpenAPI_list_for_each(authorization_data->allowed_snssai_list,
		                      node) {
			...
			cJSON_AddItemToArray(allowed_snssai_listList,
			                     itemLocal);
		}
	}

	if(authorization_data->allowed_mtc_providers) {
		cJSON *allowed_mtc_providersList = cJSON_AddArrayToObject(item,
		                                                          "allowedMtcProviders");
		if(allowed_mtc_providersList == NULL) {
			ogs_error(
				"OpenAPI_authorization_data_convertToJSON() failed [allowed_mtc_providers]");
			goto end;
		}
		OpenAPI_list_for_each(authorization_data->allowed_mtc_providers,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_mtc_provider_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_authorization_data_convertToJSON() failed [allowed_mtc_providers]");
				goto end;
			}
			cJSON_AddItemToArray(allowed_mtc_providersList,
			                     itemLocal);
		}
	}
	...
	if(authorization_data->validity_time) {
		if(cJSON_AddStringToObject(item, "validityTime",
		                           authorization_data->validity_time) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_authorization_data_convertToJSON() failed [validity_time]");
			goto end;
		}
	}
...
OpenAPI_authorization_data_t *OpenAPI_authorization_data_parseFromJSON(
	cJSON *authorization_dataJSON) {
	...
	authorization_data =
		cJSON_GetObjectItemCaseSensitive(authorization_dataJSON,
		                                 "authorizationData");
	if(!authorization_data) {
		ogs_error(
			"OpenAPI_authorization_data_parseFromJSON() failed [authorization_data]");
		goto end;
	}
	...
	cJSON_ArrayForEach(authorization_data_local, authorization_data) {
		...
		OpenAPI_user_identifier_t *authorization_dataItem =
			OpenAPI_user_identifier_parseFromJSON(
				authorization_data_local);
		if(!authorization_dataItem) {
			ogs_error("No authorization_dataItem");
			goto end;
		}
		OpenAPI_list_add(authorization_dataList,
		                 authorization_dataItem);
	}
	...
	if(allowed_dnn_list) {
		...
		if(!cJSON_IsArray(allowed_dnn_list)) {
			ogs_error(
				"OpenAPI_authorization_data_parseFromJSON() failed [allowed_dnn_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(allowed_dnn_list_local, allowed_dnn_list) {
			...
			OpenAPI_list_add(allowed_dnn_listList,
			                 ogs_strdup(
						 allowed_dnn_list_local->
						 valuestring));
		}
	}
	...
	if(allowed_snssai_list) {
		...
		if(!cJSON_IsArray(allowed_snssai_list)) {
			ogs_error(
				"OpenAPI_authorization_data_parseFromJSON() failed [allowed_snssai_list]");
			goto end;
		}

		allowed_snssai_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(allowed_snssai_list_local,
		                   allowed_snssai_list) {
			...
			OpenAPI_snssai_t *allowed_snssai_listItem =
				OpenAPI_snssai_parseFromJSON(
					allowed_snssai_list_local);
			if(!allowed_snssai_listItem) {
				ogs_error("No allowed_snssai_listItem");
				goto end;
			}
			OpenAPI_list_add(allowed_snssai_listList,
			                 allowed_snssai_listItem);
		}
	}
	...
	if(allowed_mtc_providers) {
		...
		if(!cJSON_IsArray(allowed_mtc_providers)) {
			ogs_error(
				"OpenAPI_authorization_data_parseFromJSON() failed [allowed_mtc_providers]");
			goto end;
		}
		...
		cJSON_ArrayForEach(allowed_mtc_providers_local,
			...
			OpenAPI_mtc_provider_t *allowed_mtc_providersItem =
				OpenAPI_mtc_provider_parseFromJSON(
					allowed_mtc_providers_local);
			if(!allowed_mtc_providersItem) {
				ogs_error("No allowed_mtc_providersItem");
				goto end;
			}
			OpenAPI_list_add(allowed_mtc_providersList,
			                 allowed_mtc_providersItem);
		}
	}

	validity_time = cJSON_GetObjectItemCaseSensitive(authorization_dataJSON,
	                                                 "validityTime");
	if(validity_time) {
...
OpenAPI_authorization_data_t *OpenAPI_authorization_data_copy(
	OpenAPI_authorization_data_t *dst, OpenAPI_authorization_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
