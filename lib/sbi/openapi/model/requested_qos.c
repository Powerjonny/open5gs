...
OpenAPI_requested_qos_t *OpenAPI_requested_qos_create(int _5qi, char *gbr_ul,
                                                      char *gbr_dl) {
	OpenAPI_requested_qos_t *requested_qos_local_var =
		ogs_malloc(sizeof(OpenAPI_requested_qos_t));
...
cJSON *OpenAPI_requested_qos_convertToJSON(
	...
	if(requested_qos == NULL) {
		ogs_error(
			"OpenAPI_requested_qos_convertToJSON() failed [RequestedQos]");
		return NULL;
	}
	...
	if(cJSON_AddNumberToObject(item, "5qi", requested_qos->_5qi) == NULL) {
		ogs_error(
			"OpenAPI_requested_qos_convertToJSON() failed [_5qi]");
		goto end;
	}
	...
	if(requested_qos->gbr_ul) {
		...
		{
			ogs_error(
				"OpenAPI_requested_qos_convertToJSON() failed [gbr_ul]");
			goto end;
		}
	}
	...
	if(requested_qos->gbr_dl) {
		...
		{
			ogs_error(
				"OpenAPI_requested_qos_convertToJSON() failed [gbr_dl]");
			goto end;
		}
	}
...
OpenAPI_requested_qos_t *OpenAPI_requested_qos_parseFromJSON(
	...
	if(!_5qi) {
		ogs_error(
			"OpenAPI_requested_qos_parseFromJSON() failed [_5qi]");
		goto end;
	}
	...
	if(gbr_ul) {
		...
		{
			ogs_error(
				"OpenAPI_requested_qos_parseFromJSON() failed [gbr_ul]");
			goto end;
		}
	}
	...
	if(gbr_dl) {
		...
		{
			ogs_error(
				"OpenAPI_requested_qos_parseFromJSON() failed [gbr_dl]");
			goto end;
		}
	}
