...
OpenAPI_arp_1_t *OpenAPI_arp_1_create(bool	is_priority_level_null,
                                      int	priority_level,
...
cJSON *OpenAPI_arp_1_convertToJSON(OpenAPI_arp_1_t *arp_1) {
	...
	if(cJSON_AddNumberToObject(item, "priorityLevel",
	                           arp_1->priority_level) == NULL)
	{
		ogs_error(
			"OpenAPI_arp_1_convertToJSON() failed [priority_level]");
		goto end;
	}
	...
	if(arp_1->preempt_vuln == OpenAPI_preemption_vulnerability_NULL) {
		ogs_error(
			"OpenAPI_arp_1_convertToJSON() failed [preempt_vuln]");
		return NULL;
	}
...
OpenAPI_arp_1_t *OpenAPI_arp_1_parseFromJSON(cJSON *arp_1JSON) {
	...
	priority_level = cJSON_GetObjectItemCaseSensitive(arp_1JSON,
	                                                  "priorityLevel");
	if(!priority_level) {
		ogs_error(
			"OpenAPI_arp_1_parseFromJSON() failed [priority_level]");
		goto end;
	}
	...
	preempt_vuln = cJSON_GetObjectItemCaseSensitive(arp_1JSON,
	                                                "preemptVuln");
	if(!preempt_vuln) {
		ogs_error(
			"OpenAPI_arp_1_parseFromJSON() failed [preempt_vuln]");
		goto end;
	}
...
OpenAPI_arp_1_t *OpenAPI_arp_1_copy(OpenAPI_arp_1_t	*dst,
                                    OpenAPI_arp_1_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
