...
OpenAPI_relocate_ue_context_request_create(
	...
	OpenAPI_relocate_ue_context_request_t *
		...
		ogs_malloc(sizeof(OpenAPI_relocate_ue_context_request_t));
...
void OpenAPI_relocate_ue_context_request_free(
	OpenAPI_relocate_ue_context_request_t *relocate_ue_context_request) {
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext1) {
		ogs_free(
			relocate_ue_context_request->
			binary_data_n2_information_ext1->data);
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext2) {
		ogs_free(
			relocate_ue_context_request->
			binary_data_n2_information_ext2->data);
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext3) {
		ogs_free(
			relocate_ue_context_request->
			binary_data_n2_information_ext3->data);
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext4) {
		ogs_free(
			relocate_ue_context_request->
			binary_data_n2_information_ext4->data);
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext5) {
		ogs_free(
			relocate_ue_context_request->
			binary_data_n2_information_ext5->data);
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext6) {
		ogs_free(
			relocate_ue_context_request->
			binary_data_n2_information_ext6->data);
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext7) {
		ogs_free(
			relocate_ue_context_request->
			binary_data_n2_information_ext7->data);
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext8) {
		ogs_free(
			relocate_ue_context_request->
			binary_data_n2_information_ext8->data);
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext9) {
		ogs_free(
			relocate_ue_context_request->
			binary_data_n2_information_ext9->data);
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext10) {
		ogs_free(
			relocate_ue_context_request->
			binary_data_n2_information_ext10->data);
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext11) {
		ogs_free(
			relocate_ue_context_request->
			binary_data_n2_information_ext11->data);
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext12) {
		ogs_free(
			relocate_ue_context_request->
			binary_data_n2_information_ext12->data);
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext13) {
		ogs_free(
			relocate_ue_context_request->
			binary_data_n2_information_ext13->data);
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext14) {
		ogs_free(
			relocate_ue_context_request->
			binary_data_n2_information_ext14->data);
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext15) {
		ogs_free(
			relocate_ue_context_request->
			binary_data_n2_information_ext15->data);
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext16) {
		ogs_free(
			relocate_ue_context_request->
			binary_data_n2_information_ext16->data);
...
cJSON *OpenAPI_relocate_ue_context_request_convertToJSON(
	OpenAPI_relocate_ue_context_request_t *relocate_ue_context_request) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(relocate_ue_context_request == NULL) {
		ogs_error(
			"OpenAPI_relocate_ue_context_request_convertToJSON() failed [RelocateUEContext_request]");
		return NULL;
	}
	...
	if(relocate_ue_context_request->json_data) {
		cJSON *json_data_local_JSON =
			OpenAPI_ue_context_relocate_data_convertToJSON(
				relocate_ue_context_request->json_data);
		if(json_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_relocate_ue_context_request_convertToJSON() failed [json_data]");
			goto end;
		}
	...
	if(relocate_ue_context_request->binary_data_forward_relocation_request)
	{
		char *encoded_str_binary_data_forward_relocation_request =
			OpenAPI_base64encode(
				...
				binary_data_forward_relocation_request->data,
				relocate_ue_context_request->
				binary_data_forward_relocation_request->len);
		...
		{
			ogs_error(
				"OpenAPI_relocate_ue_context_request_convertToJSON() failed [binary_data_forward_relocation_request]");
			goto end;
		}
	...
	if(relocate_ue_context_request->binary_data_n2_information) {
		char *encoded_str_binary_data_n2_information =
			OpenAPI_base64encode(
				relocate_ue_context_request->
				binary_data_n2_information->data,
				relocate_ue_context_request->
				binary_data_n2_information->len);
		if(cJSON_AddStringToObject(item, "binaryDataN2Information",
		                           encoded_str_binary_data_n2_information)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_relocate_ue_context_request_convertToJSON() failed [binary_data_n2_information]");
			goto end;
		}
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext1) {
		char *encoded_str_binary_data_n2_information_ext1 =
			OpenAPI_base64encode(
				...
				binary_data_n2_information_ext1->data,
				relocate_ue_context_request->
				binary_data_n2_information_ext1->len);
		if(cJSON_AddStringToObject(item, "binaryDataN2InformationExt1",
		                           encoded_str_binary_data_n2_information_ext1)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_relocate_ue_context_request_convertToJSON() failed [binary_data_n2_information_ext1]");
			goto end;
		}
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext2) {
		char *encoded_str_binary_data_n2_information_ext2 =
			OpenAPI_base64encode(
				...
				binary_data_n2_information_ext2->data,
				relocate_ue_context_request->
				binary_data_n2_information_ext2->len);
		if(cJSON_AddStringToObject(item, "binaryDataN2InformationExt2",
		                           encoded_str_binary_data_n2_information_ext2)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_relocate_ue_context_request_convertToJSON() failed [binary_data_n2_information_ext2]");
			goto end;
		}
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext3) {
		char *encoded_str_binary_data_n2_information_ext3 =
			OpenAPI_base64encode(
				...
				binary_data_n2_information_ext3->data,
				relocate_ue_context_request->
				binary_data_n2_information_ext3->len);
		if(cJSON_AddStringToObject(item, "binaryDataN2InformationExt3",
		                           encoded_str_binary_data_n2_information_ext3)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_relocate_ue_context_request_convertToJSON() failed [binary_data_n2_information_ext3]");
			goto end;
		}
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext4) {
		char *encoded_str_binary_data_n2_information_ext4 =
			OpenAPI_base64encode(
				...
				binary_data_n2_information_ext4->data,
				relocate_ue_context_request->
				binary_data_n2_information_ext4->len);
		if(cJSON_AddStringToObject(item, "binaryDataN2InformationExt4",
		                           encoded_str_binary_data_n2_information_ext4)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_relocate_ue_context_request_convertToJSON() failed [binary_data_n2_information_ext4]");
			goto end;
		}
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext5) {
		char *encoded_str_binary_data_n2_information_ext5 =
			OpenAPI_base64encode(
				...
				binary_data_n2_information_ext5->data,
				relocate_ue_context_request->
				binary_data_n2_information_ext5->len);
		if(cJSON_AddStringToObject(item, "binaryDataN2InformationExt5",
		                           encoded_str_binary_data_n2_information_ext5)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_relocate_ue_context_request_convertToJSON() failed [binary_data_n2_information_ext5]");
			goto end;
		}
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext6) {
		char *encoded_str_binary_data_n2_information_ext6 =
			OpenAPI_base64encode(
				...
				binary_data_n2_information_ext6->data,
				relocate_ue_context_request->
				binary_data_n2_information_ext6->len);
		if(cJSON_AddStringToObject(item, "binaryDataN2InformationExt6",
		                           encoded_str_binary_data_n2_information_ext6)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_relocate_ue_context_request_convertToJSON() failed [binary_data_n2_information_ext6]");
			goto end;
		}
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext7) {
		char *encoded_str_binary_data_n2_information_ext7 =
			OpenAPI_base64encode(
				...
				binary_data_n2_information_ext7->data,
				relocate_ue_context_request->
				binary_data_n2_information_ext7->len);
		if(cJSON_AddStringToObject(item, "binaryDataN2InformationExt7",
		                           encoded_str_binary_data_n2_information_ext7)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_relocate_ue_context_request_convertToJSON() failed [binary_data_n2_information_ext7]");
			goto end;
		}
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext8) {
		char *encoded_str_binary_data_n2_information_ext8 =
			OpenAPI_base64encode(
				...
				binary_data_n2_information_ext8->data,
				relocate_ue_context_request->
				binary_data_n2_information_ext8->len);
		if(cJSON_AddStringToObject(item, "binaryDataN2InformationExt8",
		                           encoded_str_binary_data_n2_information_ext8)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_relocate_ue_context_request_convertToJSON() failed [binary_data_n2_information_ext8]");
			goto end;
		}
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext9) {
		char *encoded_str_binary_data_n2_information_ext9 =
			OpenAPI_base64encode(
				...
				binary_data_n2_information_ext9->data,
				relocate_ue_context_request->
				binary_data_n2_information_ext9->len);
		if(cJSON_AddStringToObject(item, "binaryDataN2InformationExt9",
		                           encoded_str_binary_data_n2_information_ext9)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_relocate_ue_context_request_convertToJSON() failed [binary_data_n2_information_ext9]");
			goto end;
		}
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext10) {
		char *encoded_str_binary_data_n2_information_ext10 =
			OpenAPI_base64encode(
				...
				binary_data_n2_information_ext10->data,
				relocate_ue_context_request->
				binary_data_n2_information_ext10->len);
		if(cJSON_AddStringToObject(item, "binaryDataN2InformationExt10",
		                           encoded_str_binary_data_n2_information_ext10)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_relocate_ue_context_request_convertToJSON() failed [binary_data_n2_information_ext10]");
			goto end;
		}
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext11) {
		char *encoded_str_binary_data_n2_information_ext11 =
			OpenAPI_base64encode(
				...
				binary_data_n2_information_ext11->data,
				relocate_ue_context_request->
				binary_data_n2_information_ext11->len);
		if(cJSON_AddStringToObject(item, "binaryDataN2InformationExt11",
		                           encoded_str_binary_data_n2_information_ext11)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_relocate_ue_context_request_convertToJSON() failed [binary_data_n2_information_ext11]");
			goto end;
		}
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext12) {
		char *encoded_str_binary_data_n2_information_ext12 =
			OpenAPI_base64encode(
				...
				binary_data_n2_information_ext12->data,
				relocate_ue_context_request->
				binary_data_n2_information_ext12->len);
		if(cJSON_AddStringToObject(item, "binaryDataN2InformationExt12",
		                           encoded_str_binary_data_n2_information_ext12)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_relocate_ue_context_request_convertToJSON() failed [binary_data_n2_information_ext12]");
			goto end;
		}
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext13) {
		char *encoded_str_binary_data_n2_information_ext13 =
			OpenAPI_base64encode(
				...
				binary_data_n2_information_ext13->data,
				relocate_ue_context_request->
				binary_data_n2_information_ext13->len);
		if(cJSON_AddStringToObject(item, "binaryDataN2InformationExt13",
		                           encoded_str_binary_data_n2_information_ext13)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_relocate_ue_context_request_convertToJSON() failed [binary_data_n2_information_ext13]");
			goto end;
		}
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext14) {
		char *encoded_str_binary_data_n2_information_ext14 =
			OpenAPI_base64encode(
				...
				binary_data_n2_information_ext14->data,
				relocate_ue_context_request->
				binary_data_n2_information_ext14->len);
		if(cJSON_AddStringToObject(item, "binaryDataN2InformationExt14",
		                           encoded_str_binary_data_n2_information_ext14)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_relocate_ue_context_request_convertToJSON() failed [binary_data_n2_information_ext14]");
			goto end;
		}
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext15) {
		char *encoded_str_binary_data_n2_information_ext15 =
			OpenAPI_base64encode(
				...
				binary_data_n2_information_ext15->data,
				relocate_ue_context_request->
				binary_data_n2_information_ext15->len);
		if(cJSON_AddStringToObject(item, "binaryDataN2InformationExt15",
		                           encoded_str_binary_data_n2_information_ext15)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_relocate_ue_context_request_convertToJSON() failed [binary_data_n2_information_ext15]");
			goto end;
		}
	...
	if(relocate_ue_context_request->binary_data_n2_information_ext16) {
		char *encoded_str_binary_data_n2_information_ext16 =
			OpenAPI_base64encode(
				...
				binary_data_n2_information_ext16->data,
				relocate_ue_context_request->
				binary_data_n2_information_ext16->len);
		if(cJSON_AddStringToObject(item, "binaryDataN2InformationExt16",
		                           encoded_str_binary_data_n2_information_ext16)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_relocate_ue_context_request_convertToJSON() failed [binary_data_n2_information_ext16]");
			goto end;
		}
...
OpenAPI_relocate_ue_context_request_parseFromJSON(
	...
	if(json_data) {
		json_data_local_nonprim =
			OpenAPI_ue_context_relocate_data_parseFromJSON(
				json_data);
		if(!json_data_local_nonprim) {
			ogs_error(
				"OpenAPI_ue_context_relocate_data_parseFromJSON failed [json_data]");
			goto end;
		}
	}
	...
	if(binary_data_forward_relocation_request) {
		decoded_str_binary_data_forward_relocation_request =
			ogs_malloc(sizeof(OpenAPI_binary_t));
		ogs_assert(decoded_str_binary_data_forward_relocation_request);
		if(!cJSON_IsString(binary_data_forward_relocation_request)) {
			ogs_error(
				"OpenAPI_relocate_ue_context_request_parseFromJSON() failed [binary_data_forward_relocation_request]");
			goto end;
		}
		decoded_str_binary_data_forward_relocation_request->data =
			OpenAPI_base64decode(
				binary_data_forward_relocation_request->
				valuestring,
				strlen(
					binary_data_forward_relocation_request->
	...
	if(binary_data_n2_information) {
		...
		if(!cJSON_IsString(binary_data_n2_information)) {
			ogs_error(
				"OpenAPI_relocate_ue_context_request_parseFromJSON() failed [binary_data_n2_information]");
			goto end;
		}
		decoded_str_binary_data_n2_information->data =
			OpenAPI_base64decode(
				binary_data_n2_information->valuestring,
				strlen(binary_data_n2_information->valuestring),
	...
	if(binary_data_n2_information_ext1) {
		...
		if(!cJSON_IsString(binary_data_n2_information_ext1)) {
			ogs_error(
				"OpenAPI_relocate_ue_context_request_parseFromJSON() failed [binary_data_n2_information_ext1]");
			goto end;
		}
		decoded_str_binary_data_n2_information_ext1->data =
			OpenAPI_base64decode(
				binary_data_n2_information_ext1->valuestring,
				strlen(
					binary_data_n2_information_ext1->
					valuestring),
				&decoded_str_binary_data_n2_information_ext1->
	...
	if(binary_data_n2_information_ext2) {
		...
		if(!cJSON_IsString(binary_data_n2_information_ext2)) {
			ogs_error(
				"OpenAPI_relocate_ue_context_request_parseFromJSON() failed [binary_data_n2_information_ext2]");
			goto end;
		}
		decoded_str_binary_data_n2_information_ext2->data =
			OpenAPI_base64decode(
				binary_data_n2_information_ext2->valuestring,
				strlen(
					binary_data_n2_information_ext2->
					valuestring),
				&decoded_str_binary_data_n2_information_ext2->
	...
	if(binary_data_n2_information_ext3) {
		...
		if(!cJSON_IsString(binary_data_n2_information_ext3)) {
			ogs_error(
				"OpenAPI_relocate_ue_context_request_parseFromJSON() failed [binary_data_n2_information_ext3]");
			goto end;
		}
		decoded_str_binary_data_n2_information_ext3->data =
			OpenAPI_base64decode(
				binary_data_n2_information_ext3->valuestring,
				strlen(
					binary_data_n2_information_ext3->
					valuestring),
				&decoded_str_binary_data_n2_information_ext3->
	...
	if(binary_data_n2_information_ext4) {
		...
		if(!cJSON_IsString(binary_data_n2_information_ext4)) {
			ogs_error(
				"OpenAPI_relocate_ue_context_request_parseFromJSON() failed [binary_data_n2_information_ext4]");
			goto end;
		}
		decoded_str_binary_data_n2_information_ext4->data =
			OpenAPI_base64decode(
				binary_data_n2_information_ext4->valuestring,
				strlen(
					binary_data_n2_information_ext4->
					valuestring),
				&decoded_str_binary_data_n2_information_ext4->
	...
	if(binary_data_n2_information_ext5) {
		...
		if(!cJSON_IsString(binary_data_n2_information_ext5)) {
			ogs_error(
				"OpenAPI_relocate_ue_context_request_parseFromJSON() failed [binary_data_n2_information_ext5]");
			goto end;
		}
		decoded_str_binary_data_n2_information_ext5->data =
			OpenAPI_base64decode(
				binary_data_n2_information_ext5->valuestring,
				strlen(
					binary_data_n2_information_ext5->
					valuestring),
				&decoded_str_binary_data_n2_information_ext5->
	...
	if(binary_data_n2_information_ext6) {
		...
		if(!cJSON_IsString(binary_data_n2_information_ext6)) {
			ogs_error(
				"OpenAPI_relocate_ue_context_request_parseFromJSON() failed [binary_data_n2_information_ext6]");
			goto end;
		}
		decoded_str_binary_data_n2_information_ext6->data =
			OpenAPI_base64decode(
				binary_data_n2_information_ext6->valuestring,
				strlen(
					binary_data_n2_information_ext6->
					valuestring),
				&decoded_str_binary_data_n2_information_ext6->
	...
	if(binary_data_n2_information_ext7) {
		...
		if(!cJSON_IsString(binary_data_n2_information_ext7)) {
			ogs_error(
				"OpenAPI_relocate_ue_context_request_parseFromJSON() failed [binary_data_n2_information_ext7]");
			goto end;
		}
		decoded_str_binary_data_n2_information_ext7->data =
			OpenAPI_base64decode(
				binary_data_n2_information_ext7->valuestring,
				strlen(
					binary_data_n2_information_ext7->
					valuestring),
				&decoded_str_binary_data_n2_information_ext7->
	...
	if(binary_data_n2_information_ext8) {
		...
		if(!cJSON_IsString(binary_data_n2_information_ext8)) {
			ogs_error(
				"OpenAPI_relocate_ue_context_request_parseFromJSON() failed [binary_data_n2_information_ext8]");
			goto end;
		}
		decoded_str_binary_data_n2_information_ext8->data =
			OpenAPI_base64decode(
				binary_data_n2_information_ext8->valuestring,
				strlen(
					binary_data_n2_information_ext8->
					valuestring),
				&decoded_str_binary_data_n2_information_ext8->
	...
	if(binary_data_n2_information_ext9) {
		...
		if(!cJSON_IsString(binary_data_n2_information_ext9)) {
			ogs_error(
				"OpenAPI_relocate_ue_context_request_parseFromJSON() failed [binary_data_n2_information_ext9]");
			goto end;
		}
		decoded_str_binary_data_n2_information_ext9->data =
			OpenAPI_base64decode(
				binary_data_n2_information_ext9->valuestring,
				strlen(
					binary_data_n2_information_ext9->
					valuestring),
				&decoded_str_binary_data_n2_information_ext9->
	...
	if(binary_data_n2_information_ext10) {
		...
		if(!cJSON_IsString(binary_data_n2_information_ext10)) {
			ogs_error(
				"OpenAPI_relocate_ue_context_request_parseFromJSON() failed [binary_data_n2_information_ext10]");
			goto end;
		}
		decoded_str_binary_data_n2_information_ext10->data =
			OpenAPI_base64decode(
				binary_data_n2_information_ext10->valuestring,
				strlen(
					binary_data_n2_information_ext10->
					valuestring),
				&decoded_str_binary_data_n2_information_ext10->
	...
	if(binary_data_n2_information_ext11) {
		...
		if(!cJSON_IsString(binary_data_n2_information_ext11)) {
			ogs_error(
				"OpenAPI_relocate_ue_context_request_parseFromJSON() failed [binary_data_n2_information_ext11]");
			goto end;
		}
		decoded_str_binary_data_n2_information_ext11->data =
			OpenAPI_base64decode(
				binary_data_n2_information_ext11->valuestring,
				strlen(
					binary_data_n2_information_ext11->
					valuestring),
				&decoded_str_binary_data_n2_information_ext11->
	...
	if(binary_data_n2_information_ext12) {
		...
		if(!cJSON_IsString(binary_data_n2_information_ext12)) {
			ogs_error(
				"OpenAPI_relocate_ue_context_request_parseFromJSON() failed [binary_data_n2_information_ext12]");
			goto end;
		}
		decoded_str_binary_data_n2_information_ext12->data =
			OpenAPI_base64decode(
				binary_data_n2_information_ext12->valuestring,
				strlen(
					binary_data_n2_information_ext12->
					valuestring),
				&decoded_str_binary_data_n2_information_ext12->
	...
	if(binary_data_n2_information_ext13) {
		...
		if(!cJSON_IsString(binary_data_n2_information_ext13)) {
			ogs_error(
				"OpenAPI_relocate_ue_context_request_parseFromJSON() failed [binary_data_n2_information_ext13]");
			goto end;
		}
		decoded_str_binary_data_n2_information_ext13->data =
			OpenAPI_base64decode(
				binary_data_n2_information_ext13->valuestring,
				strlen(
					binary_data_n2_information_ext13->
					valuestring),
				&decoded_str_binary_data_n2_information_ext13->
	...
	if(binary_data_n2_information_ext14) {
		...
		if(!cJSON_IsString(binary_data_n2_information_ext14)) {
			ogs_error(
				"OpenAPI_relocate_ue_context_request_parseFromJSON() failed [binary_data_n2_information_ext14]");
			goto end;
		}
		decoded_str_binary_data_n2_information_ext14->data =
			OpenAPI_base64decode(
				binary_data_n2_information_ext14->valuestring,
				strlen(
					binary_data_n2_information_ext14->
					valuestring),
				&decoded_str_binary_data_n2_information_ext14->
	...
	if(binary_data_n2_information_ext15) {
		...
		if(!cJSON_IsString(binary_data_n2_information_ext15)) {
			ogs_error(
				"OpenAPI_relocate_ue_context_request_parseFromJSON() failed [binary_data_n2_information_ext15]");
			goto end;
		}
		decoded_str_binary_data_n2_information_ext15->data =
			OpenAPI_base64decode(
				binary_data_n2_information_ext15->valuestring,
				strlen(
					binary_data_n2_information_ext15->
					valuestring),
				&decoded_str_binary_data_n2_information_ext15->
	...
	if(binary_data_n2_information_ext16) {
		...
		if(!cJSON_IsString(binary_data_n2_information_ext16)) {
			ogs_error(
				"OpenAPI_relocate_ue_context_request_parseFromJSON() failed [binary_data_n2_information_ext16]");
			goto end;
		}
		decoded_str_binary_data_n2_information_ext16->data =
			OpenAPI_base64decode(
				binary_data_n2_information_ext16->valuestring,
				strlen(
					binary_data_n2_information_ext16->
					valuestring),
				&decoded_str_binary_data_n2_information_ext16->
	...
	relocate_ue_context_request_local_var =
		OpenAPI_relocate_ue_context_request_create(
			...
			NULL,
			binary_data_n2_information ?
			decoded_str_binary_data_n2_information : NULL,
			binary_data_n2_information_ext1 ?
			decoded_str_binary_data_n2_information_ext1 : NULL,
			binary_data_n2_information_ext2 ?
			decoded_str_binary_data_n2_information_ext2 : NULL,
			binary_data_n2_information_ext3 ?
			decoded_str_binary_data_n2_information_ext3 : NULL,
			binary_data_n2_information_ext4 ?
			decoded_str_binary_data_n2_information_ext4 : NULL,
			binary_data_n2_information_ext5 ?
			decoded_str_binary_data_n2_information_ext5 : NULL,
			binary_data_n2_information_ext6 ?
			decoded_str_binary_data_n2_information_ext6 : NULL,
			binary_data_n2_information_ext7 ?
			decoded_str_binary_data_n2_information_ext7 : NULL,
			binary_data_n2_information_ext8 ?
			decoded_str_binary_data_n2_information_ext8 : NULL,
			binary_data_n2_information_ext9 ?
			decoded_str_binary_data_n2_information_ext9 : NULL,
			binary_data_n2_information_ext10 ?
			decoded_str_binary_data_n2_information_ext10 : NULL,
			binary_data_n2_information_ext11 ?
			decoded_str_binary_data_n2_information_ext11 : NULL,
			binary_data_n2_information_ext12 ?
			decoded_str_binary_data_n2_information_ext12 : NULL,
			binary_data_n2_information_ext13 ?
			decoded_str_binary_data_n2_information_ext13 : NULL,
			binary_data_n2_information_ext14 ?
			decoded_str_binary_data_n2_information_ext14 : NULL,
			binary_data_n2_information_ext15 ?
			decoded_str_binary_data_n2_information_ext15 : NULL,
			binary_data_n2_information_ext16 ?
			decoded_str_binary_data_n2_information_ext16 : NULL
			);
...
OpenAPI_relocate_ue_context_request_t *OpenAPI_relocate_ue_context_request_copy(
	OpenAPI_relocate_ue_context_request_t	*dst,
	OpenAPI_relocate_ue_context_request_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_relocate_ue_context_request_convertToJSON() failed");
		return NULL;
	}
