...
OpenAPI_subscribed_snssai_t *OpenAPI_subscribed_snssai_create(
	...
	OpenAPI_subscribed_snssai_t *subscribed_snssai_local_var =
		ogs_malloc(sizeof(OpenAPI_subscribed_snssai_t));
	ogs_assert(subscribed_snssai_local_var);

	subscribed_snssai_local_var->subscribed_snssai = subscribed_snssai;
	subscribed_snssai_local_var->is_default_indication =
		is_default_indication;
	subscribed_snssai_local_var->default_indication = default_indication;
	subscribed_snssai_local_var->subscribed_ns_srg_list =
...
void OpenAPI_subscribed_snssai_free(
	...
	if(subscribed_snssai->subscribed_ns_srg_list) {
		OpenAPI_list_for_each(subscribed_snssai->subscribed_ns_srg_list,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_subscribed_snssai_convertToJSON(
	...
	if(subscribed_snssai == NULL) {
		ogs_error(
			"OpenAPI_subscribed_snssai_convertToJSON() failed [SubscribedSnssai]");
		return NULL;
	}
	...
	if(!subscribed_snssai->subscribed_snssai) {
		ogs_error(
			"OpenAPI_subscribed_snssai_convertToJSON() failed [subscribed_snssai]");
		return NULL;
	}
	cJSON *subscribed_snssai_local_JSON =
	...
	cJSON_AddItemToObject(item, "subscribedSnssai",
	                      subscribed_snssai_local_JSON);
	if(item->child == NULL) {
		ogs_error(
			"OpenAPI_subscribed_snssai_convertToJSON() failed [subscribed_snssai]");
		goto end;
	...
	if(subscribed_snssai->is_default_indication) {
		...
		{
			ogs_error(
				"OpenAPI_subscribed_snssai_convertToJSON() failed [default_indication]");
			goto end;
		}
	}
	...
	if(subscribed_snssai->subscribed_ns_srg_list) {
		...
		if(subscribed_ns_srg_listList == NULL) {
			ogs_error(
				"OpenAPI_subscribed_snssai_convertToJSON() failed [subscribed_ns_srg_list]");
			goto end;
		}
		OpenAPI_list_for_each(subscribed_snssai->subscribed_ns_srg_list,
		                      node) {
...
OpenAPI_subscribed_snssai_t *OpenAPI_subscribed_snssai_parseFromJSON(
	...
	subscribed_snssai =
		cJSON_GetObjectItemCaseSensitive(subscribed_snssaiJSON,
		                                 "subscribedSnssai");
	if(!subscribed_snssai) {
		ogs_error(
			"OpenAPI_subscribed_snssai_parseFromJSON() failed [subscribed_snssai]");
		goto end;
	}
	subscribed_snssai_local_nonprim =
		OpenAPI_snssai_parseFromJSON(subscribed_snssai);
	if(!subscribed_snssai_local_nonprim) {
		ogs_error(
			"OpenAPI_snssai_parseFromJSON failed [subscribed_snssai]");
		goto end;
	}
	...
	if(default_indication) {
		if(!cJSON_IsBool(default_indication)) {
			ogs_error(
				"OpenAPI_subscribed_snssai_parseFromJSON() failed [default_indication]");
			goto end;
		}
	}
	...
	if(subscribed_ns_srg_list) {
		...
		if(!cJSON_IsArray(subscribed_ns_srg_list)) {
			ogs_error(
				"OpenAPI_subscribed_snssai_parseFromJSON() failed [subscribed_ns_srg_list]");
			goto end;
		}
