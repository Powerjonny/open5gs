...
OpenAPI_amf_subscription_info_t *OpenAPI_amf_subscription_info_create(
	char *amf_instance_id, char *subscription_id,
	char *subs_change_notify_correlation_id,
	OpenAPI_context_info_t *context_info) {
	OpenAPI_amf_subscription_info_t *amf_subscription_info_local_var =
		ogs_malloc(sizeof(OpenAPI_amf_subscription_info_t));
	...
	amf_subscription_info_local_var->subs_change_notify_correlation_id =
...
void OpenAPI_amf_subscription_info_free(
	OpenAPI_amf_subscription_info_t *amf_subscription_info) {
	...
	if(amf_subscription_info->subs_change_notify_correlation_id) {
		ogs_free(
			amf_subscription_info->subs_change_notify_correlation_id);
...
cJSON *OpenAPI_amf_subscription_info_convertToJSON(
	OpenAPI_amf_subscription_info_t *amf_subscription_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(amf_subscription_info == NULL) {
		ogs_error(
			"OpenAPI_amf_subscription_info_convertToJSON() failed [AmfSubscriptionInfo]");
		return NULL;
	}
	...
	if(!amf_subscription_info->amf_instance_id) {
		ogs_error(
			"OpenAPI_amf_subscription_info_convertToJSON() failed [amf_instance_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "amfInstanceId",
	                           amf_subscription_info->amf_instance_id) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_amf_subscription_info_convertToJSON() failed [amf_instance_id]");
	...
	if(!amf_subscription_info->subscription_id) {
		ogs_error(
			"OpenAPI_amf_subscription_info_convertToJSON() failed [subscription_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "subscriptionId",
	                           amf_subscription_info->subscription_id) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_amf_subscription_info_convertToJSON() failed [subscription_id]");
	...
	if(amf_subscription_info->subs_change_notify_correlation_id) {
		if(cJSON_AddStringToObject(item,
		                           ...
		                           amf_subscription_info->
		                           subs_change_notify_correlation_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_amf_subscription_info_convertToJSON() failed [subs_change_notify_correlation_id]");
			goto end;
		}
	}
	...
	if(amf_subscription_info->context_info) {
		cJSON *context_info_local_JSON =
			OpenAPI_context_info_convertToJSON(
				amf_subscription_info->context_info);
		if(context_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_amf_subscription_info_convertToJSON() failed [context_info]");
			goto end;
		}
...
OpenAPI_amf_subscription_info_t *OpenAPI_amf_subscription_info_parseFromJSON(
	cJSON *amf_subscription_infoJSON) {
	...
	amf_instance_id =
		cJSON_GetObjectItemCaseSensitive(amf_subscription_infoJSON,
		                                 "amfInstanceId");
	if(!amf_instance_id) {
		ogs_error(
			"OpenAPI_amf_subscription_info_parseFromJSON() failed [amf_instance_id]");
		goto end;
	}
	...
	if(!subscription_id) {
		ogs_error(
			"OpenAPI_amf_subscription_info_parseFromJSON() failed [subscription_id]");
		goto end;
	}
	...
	if(subs_change_notify_correlation_id) {
		...
		{
			ogs_error(
				"OpenAPI_amf_subscription_info_parseFromJSON() failed [subs_change_notify_correlation_id]");
			goto end;
		}
	}
	...
	amf_subscription_info_local_var = OpenAPI_amf_subscription_info_create(
		...
		subs_change_notify_correlation_id &&
		!cJSON_IsNull(subs_change_notify_correlation_id) ?
		ogs_strdup(subs_change_notify_correlation_id->valuestring) :
		NULL,
...
OpenAPI_amf_subscription_info_t *OpenAPI_amf_subscription_info_copy(
	OpenAPI_amf_subscription_info_t *dst,
	OpenAPI_amf_subscription_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_amf_subscription_info_convertToJSON() failed");
		return NULL;
	}
