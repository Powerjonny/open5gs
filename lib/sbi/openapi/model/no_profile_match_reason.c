...
cJSON *OpenAPI_no_profile_match_reason_convertToJSON(
	...
	if(no_profile_match_reason == NULL) {
		ogs_error(
			"OpenAPI_no_profile_match_reason_convertToJSON() failed [NoProfileMatchReason]");
		return NULL;
	}
...
OpenAPI_no_profile_match_reason_t *OpenAPI_no_profile_match_reason_parseFromJSON
	...
	OpenAPI_no_profile_match_reason_t *no_profile_match_reason_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	no_profile_match_reason_local_var =
		OpenAPI_no_profile_match_reason_create(
			);
...
OpenAPI_no_profile_match_reason_t *OpenAPI_no_profile_match_reason_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_no_profile_match_reason_convertToJSON() failed");
		return NULL;
	}
