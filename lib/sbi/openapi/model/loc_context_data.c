...
char *OpenAPI_lcs_upp_exist_indloc_context_data_ToString(
	OpenAPI_loc_context_data_lcs_upp_exist_ind_e lcs_upp_exist_ind) {
...
OpenAPI_loc_context_data_lcs_upp_exist_ind_e
OpenAPI_lcs_upp_exist_indloc_context_data_FromString(char *lcs_upp_exist_ind) {
...
OpenAPI_loc_context_data_t *OpenAPI_loc_context_data_create(char *amf_id,
...
{
	OpenAPI_loc_context_data_t *loc_context_data_local_var =
		ogs_malloc(sizeof(OpenAPI_loc_context_data_t));
...
void OpenAPI_loc_context_data_free(
	OpenAPI_loc_context_data_t *loc_context_data) {
	...
	if(loc_context_data->add_loc_context_list) {
		OpenAPI_list_for_each(loc_context_data->add_loc_context_list,
		                      node) {
...
cJSON *OpenAPI_loc_context_data_convertToJSON(
	OpenAPI_loc_context_data_t *loc_context_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(loc_context_data == NULL) {
		ogs_error(
			"OpenAPI_loc_context_data_convertToJSON() failed [LocContextData]");
		return NULL;
	}
	...
	if(!loc_context_data->amf_id) {
		ogs_error(
			"OpenAPI_loc_context_data_convertToJSON() failed [amf_id]");
		return NULL;
	}
	...
	if(loc_context_data->location_qo_s) {
		...
		if(location_qo_s_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [location_qo_s]");
			goto end;
		}
	   ...
	   OpenAPI_supported_gad_shapes_NULL)
	{
		...
		if(supported_gad_shapesList == NULL) {
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [supported_gad_shapes]");
			goto end;
		}
		OpenAPI_list_for_each(loc_context_data->supported_gad_shapes,
		                      node) {
	...
	if(loc_context_data->supi) {
		...
		{
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(loc_context_data->gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [gpsi]");
			goto end;
		}
	}

	if(loc_context_data->add_loc_context_list) {
		cJSON *add_loc_context_listList = cJSON_AddArrayToObject(item,
		                                                         "addLocContextList");
		if(add_loc_context_listList == NULL) {
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [add_loc_context_list]");
			goto end;
		}
		OpenAPI_list_for_each(loc_context_data->add_loc_context_list,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_add_location_context_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_loc_context_data_convertToJSON() failed [add_loc_context_list]");
			...
			cJSON_AddItemToArray(add_loc_context_listList,
			                     itemLocal);
		}
	}
	...
	if(loc_context_data->ldr_type == OpenAPI_ldr_type_NULL) {
		ogs_error(
			"OpenAPI_loc_context_data_convertToJSON() failed [ldr_type]");
		return NULL;
	}
	...
	if(!loc_context_data->hgmlc_call_back_uri) {
		ogs_error(
			"OpenAPI_loc_context_data_convertToJSON() failed [hgmlc_call_back_uri]");
		return NULL;
	}
	...
	if(!loc_context_data->ldr_reference) {
		ogs_error(
			"OpenAPI_loc_context_data_convertToJSON() failed [ldr_reference]");
		return NULL;
	}
	...
	if(loc_context_data->periodic_event_info) {
		...
		if(periodic_event_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [periodic_event_info]");
			goto end;
		}
	...
	if(loc_context_data->area_event_info) {
		...
		if(area_event_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [area_event_info]");
			goto end;
		}
	...
	if(loc_context_data->motion_event_info) {
		...
		if(motion_event_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [motion_event_info]");
			goto end;
		}
	...
	if(!loc_context_data->event_report_message) {
		ogs_error(
			"OpenAPI_loc_context_data_convertToJSON() failed [event_report_message]");
		return NULL;
	}
	...
	if(loc_context_data->event_reporting_status) {
		...
		if(event_reporting_status_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [event_reporting_status]");
			goto end;
		}
	...
	if(loc_context_data->ue_location_info) {
		...
		if(ue_location_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [ue_location_info]");
			goto end;
		}
	...
	if(loc_context_data->is_c_io_t5_gs_optimisation) {
		...
		{
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [c_io_t5_gs_optimisation]");
			goto end;
		}
	}
	...
	if(loc_context_data->ecgi) {
		cJSON *ecgi_local_JSON =
			OpenAPI_ecgi_convertToJSON(loc_context_data->ecgi);
		if(ecgi_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [ecgi]");
			goto end;
		}
	...
	if(loc_context_data->ncgi) {
		cJSON *ncgi_local_JSON =
			OpenAPI_ncgi_convertToJSON(loc_context_data->ncgi);
		if(ncgi_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [ncgi]");
			goto end;
		}
	...
	if(loc_context_data->guami) {
		cJSON *guami_local_JSON =
			OpenAPI_guami_convertToJSON(loc_context_data->guami);
		if(guami_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [guami]");
			goto end;
		}
	...
	if(loc_context_data->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(loc_context_data->ue_positioning_cap) {
		...
		{
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [ue_positioning_cap]");
			goto end;
		}
	}
	   ...
	   OpenAPI_ue_up_positioning_capabilities_NULL)
	{
		...
		if(ue_up_pos_capsList == NULL) {
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [ue_up_pos_caps]");
			goto end;
		}
	...
	if(loc_context_data->scheduled_loc_time) {
		...
		{
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [scheduled_loc_time]");
			goto end;
		}
	}
	   ...
	   OpenAPI_indoor_outdoor_ind_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [indoor_outdoor_ind]");
			goto end;
		}
	}
	   ...
	   OpenAPI_los_nlos_measure_ind_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [los_nlos_measure_ind]");
			goto end;
		}
	}
	...
	if(loc_context_data->up_cum_evt_rpt_criteria) {
		...
		if(up_cum_evt_rpt_criteria_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [up_cum_evt_rpt_criteria]");
			goto end;
		}
	...
	if(loc_context_data->lcs_upp_exist_ind !=
	   OpenAPI_loc_context_data_LCSUPPEXISTIND_NULL)
	{
		if(cJSON_AddStringToObject(item, "lcsUppExistInd",
		                           OpenAPI_lcs_upp_exist_indloc_context_data_ToString
		                                   (loc_context_data->
		                                   lcs_upp_exist_ind)) == NULL)
		{
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [lcs_upp_exist_ind]");
			goto end;
		}
	}

	if(loc_context_data->src_lmf_routing_info) {
		if(cJSON_AddStringToObject(item, "srcLmfRoutingInfo",
		                           loc_context_data->
		                           src_lmf_routing_info) == NULL)
		{
			ogs_error(
				"OpenAPI_loc_context_data_convertToJSON() failed [src_lmf_routing_info]");
			goto end;
		}
	}
...
OpenAPI_loc_context_data_t *OpenAPI_loc_context_data_parseFromJSON(
	cJSON *loc_context_dataJSON) {
	...
	cJSON *supi = NULL;
	cJSON *gpsi = NULL;
	cJSON *add_loc_context_list = NULL;
	OpenAPI_list_t *add_loc_context_listList = NULL;
	cJSON *ldr_type = NULL;
	...
	OpenAPI_loc_context_data_lcs_upp_exist_ind_e lcs_upp_exist_indVariable =
		0;
	cJSON *src_lmf_routing_info = NULL;
	amf_id = cJSON_GetObjectItemCaseSensitive(loc_context_dataJSON,
	                                          "amfId");
	if(!amf_id) {
		ogs_error(
			"OpenAPI_loc_context_data_parseFromJSON() failed [amf_id]");
		goto end;
	}
	...
	supported_gad_shapes =
		cJSON_GetObjectItemCaseSensitive(loc_context_dataJSON,
		                                 "supportedGADShapes");
	if(supported_gad_shapes) {
		cJSON *supported_gad_shapes_local = NULL;
		if(!cJSON_IsArray(supported_gad_shapes)) {
			ogs_error(
				"OpenAPI_loc_context_data_parseFromJSON() failed [supported_gad_shapes]");
			goto end;
		}
		...
		if(supported_gad_shapesList->count == 0) {
			ogs_error(
				"OpenAPI_loc_context_data_parseFromJSON() failed: Expected supported_gad_shapesList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(supi) {
		...
		{
			ogs_error(
				"OpenAPI_loc_context_data_parseFromJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_loc_context_data_parseFromJSON() failed [gpsi]");
			goto end;
		}
	}
	...
	if(add_loc_context_list) {
		cJSON *add_loc_context_list_local = NULL;
		if(!cJSON_IsArray(add_loc_context_list)) {
			ogs_error(
				"OpenAPI_loc_context_data_parseFromJSON() failed [add_loc_context_list]");
			goto end;
		}

		add_loc_context_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(add_loc_context_list_local,
		                   add_loc_context_list) {
			if(!cJSON_IsObject(add_loc_context_list_local)) {
				ogs_error(
					"OpenAPI_loc_context_data_parseFromJSON() failed [add_loc_context_list]");
				goto end;
			}
			OpenAPI_add_location_context_t *add_loc_context_listItem
			        = OpenAPI_add_location_context_parseFromJSON(
					  add_loc_context_list_local);
			if(!add_loc_context_listItem) {
				ogs_error("No add_loc_context_listItem");
				goto end;
			}
			OpenAPI_list_add(add_loc_context_listList,
			                 add_loc_context_listItem);
		}
	}
	...
	if(!ldr_type) {
		ogs_error(
			"OpenAPI_loc_context_data_parseFromJSON() failed [ldr_type]");
		goto end;
	}
	...
	if(!hgmlc_call_back_uri) {
		ogs_error(
			"OpenAPI_loc_context_data_parseFromJSON() failed [hgmlc_call_back_uri]");
		goto end;
	}
	...
	if(!ldr_reference) {
		ogs_error(
			"OpenAPI_loc_context_data_parseFromJSON() failed [ldr_reference]");
		goto end;
	}
	...
	if(!event_report_message) {
		ogs_error(
			"OpenAPI_loc_context_data_parseFromJSON() failed [event_report_message]");
		goto end;
	}
	...
	if(c_io_t5_gs_optimisation) {
		if(!cJSON_IsBool(c_io_t5_gs_optimisation)) {
			ogs_error(
				"OpenAPI_loc_context_data_parseFromJSON() failed [c_io_t5_gs_optimisation]");
			goto end;
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_loc_context_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(ue_positioning_cap) {
		...
		{
			ogs_error(
				"OpenAPI_loc_context_data_parseFromJSON() failed [ue_positioning_cap]");
			goto end;
		}
	}

	ue_up_pos_caps = cJSON_GetObjectItemCaseSensitive(loc_context_dataJSON,
	                                                  "ueUpPosCaps");
	if(ue_up_pos_caps) {
		cJSON *ue_up_pos_caps_local = NULL;
		if(!cJSON_IsArray(ue_up_pos_caps)) {
			ogs_error(
				"OpenAPI_loc_context_data_parseFromJSON() failed [ue_up_pos_caps]");
			goto end;
		}
		...
		if(ue_up_pos_capsList->count == 0) {
			ogs_error(
				"OpenAPI_loc_context_data_parseFromJSON() failed: Expected ue_up_pos_capsList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(scheduled_loc_time) {
		...
		{
			ogs_error(
				"OpenAPI_loc_context_data_parseFromJSON() failed [scheduled_loc_time]");
			goto end;
		}
	}
	...
	if(indoor_outdoor_ind) {
		if(!cJSON_IsString(indoor_outdoor_ind)) {
			ogs_error(
				"OpenAPI_loc_context_data_parseFromJSON() failed [indoor_outdoor_ind]");
			goto end;
		}
	...
	if(los_nlos_measure_ind) {
		if(!cJSON_IsString(los_nlos_measure_ind)) {
			ogs_error(
				"OpenAPI_loc_context_data_parseFromJSON() failed [los_nlos_measure_ind]");
			goto end;
		}
	...
	if(lcs_upp_exist_ind) {
		if(!cJSON_IsString(lcs_upp_exist_ind)) {
			ogs_error(
				"OpenAPI_loc_context_data_parseFromJSON() failed [lcs_upp_exist_ind]");
			goto end;
		}
		lcs_upp_exist_indVariable =
			OpenAPI_lcs_upp_exist_indloc_context_data_FromString(
	...
	if(src_lmf_routing_info) {
		if(!cJSON_IsString(src_lmf_routing_info) &&
		   !cJSON_IsNull(src_lmf_routing_info))
		{
			ogs_error(
				"OpenAPI_loc_context_data_parseFromJSON() failed [src_lmf_routing_info]");
			goto end;
		}
	}
	...
	loc_context_data_local_var = OpenAPI_loc_context_data_create(
		...
		src_lmf_routing_info &&
		!cJSON_IsNull(src_lmf_routing_info) ?
		ogs_strdup(src_lmf_routing_info->valuestring) : NULL
		);
	...
	if(add_loc_context_listList) {
		OpenAPI_list_for_each(add_loc_context_listList, node) {
			OpenAPI_add_location_context_free(node->data);
		}
...
OpenAPI_loc_context_data_t *OpenAPI_loc_context_data_copy(
	OpenAPI_loc_context_data_t *dst, OpenAPI_loc_context_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
