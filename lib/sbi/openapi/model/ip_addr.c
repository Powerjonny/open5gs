...
OpenAPI_ip_addr_t *OpenAPI_ip_addr_create(char *ipv4_addr, char *ipv6_addr,
                                          char *ipv6_prefix) {
	OpenAPI_ip_addr_t *ip_addr_local_var =
		ogs_malloc(sizeof(OpenAPI_ip_addr_t));
...
cJSON *OpenAPI_ip_addr_convertToJSON(OpenAPI_ip_addr_t *ip_addr) {
	...
	if(ip_addr->ipv4_addr) {
		...
		{
			ogs_error(
				"OpenAPI_ip_addr_convertToJSON() failed [ipv4_addr]");
			goto end;
		}
	}
	...
	if(ip_addr->ipv6_addr) {
		...
		{
			ogs_error(
				"OpenAPI_ip_addr_convertToJSON() failed [ipv6_addr]");
			goto end;
		}
	}
	...
	if(ip_addr->ipv6_prefix) {
		...
		{
			ogs_error(
				"OpenAPI_ip_addr_convertToJSON() failed [ipv6_prefix]");
			goto end;
		}
	}
...
OpenAPI_ip_addr_t *OpenAPI_ip_addr_parseFromJSON(cJSON *ip_addrJSON) {
	...
	if(ipv4_addr) {
		...
		{
			ogs_error(
				"OpenAPI_ip_addr_parseFromJSON() failed [ipv4_addr]");
			goto end;
		}
	}
	...
	if(ipv6_addr) {
		...
		{
			ogs_error(
				"OpenAPI_ip_addr_parseFromJSON() failed [ipv6_addr]");
			goto end;
		}
	}

	ipv6_prefix = cJSON_GetObjectItemCaseSensitive(ip_addrJSON,
	                                               "ipv6Prefix");
	if(ipv6_prefix) {
		...
		{
			ogs_error(
				"OpenAPI_ip_addr_parseFromJSON() failed [ipv6_prefix]");
			goto end;
		}
	}
