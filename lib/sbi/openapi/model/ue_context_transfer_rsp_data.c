...
OpenAPI_ue_context_transfer_rsp_data_create(OpenAPI_ue_context_t *ue_context,
	...
	OpenAPI_ue_context_transfer_rsp_data_t *
		...
		ogs_malloc(sizeof(OpenAPI_ue_context_transfer_rsp_data_t));
...
cJSON *OpenAPI_ue_context_transfer_rsp_data_convertToJSON(
	...
	if(ue_context_transfer_rsp_data == NULL) {
		ogs_error(
			"OpenAPI_ue_context_transfer_rsp_data_convertToJSON() failed [UeContextTransferRspData]");
		return NULL;
	}
	...
	if(!ue_context_transfer_rsp_data->ue_context) {
		ogs_error(
			"OpenAPI_ue_context_transfer_rsp_data_convertToJSON() failed [ue_context]");
		return NULL;
	}
	...
	if(ue_context_transfer_rsp_data->ue_radio_capability) {
		...
		if(ue_radio_capability_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_transfer_rsp_data_convertToJSON() failed [ue_radio_capability]");
			goto end;
		}
	...
	if(ue_context_transfer_rsp_data->ue_radio_capability_for_paging) {
		...
		if(ue_radio_capability_for_paging_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_transfer_rsp_data_convertToJSON() failed [ue_radio_capability_for_paging]");
			goto end;
		}
	...
	if(ue_context_transfer_rsp_data->ue_nbiot_radio_capability) {
		...
		if(ue_nbiot_radio_capability_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_transfer_rsp_data_convertToJSON() failed [ue_nbiot_radio_capability]");
			goto end;
		}
	...
	if(ue_context_transfer_rsp_data->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_transfer_rsp_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
...
OpenAPI_ue_context_transfer_rsp_data_parseFromJSON(
	...
	OpenAPI_ue_context_transfer_rsp_data_t *
	        ue_context_transfer_rsp_data_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	OpenAPI_n2_info_content_t *ue_nbiot_radio_capability_local_nonprim =
		NULL;
	...
	if(!ue_context) {
		ogs_error(
			"OpenAPI_ue_context_transfer_rsp_data_parseFromJSON() failed [ue_context]");
		goto end;
	}
	...
	ue_radio_capability_for_paging =
		cJSON_GetObjectItemCaseSensitive(
			ue_context_transfer_rsp_dataJSON,
	...
	if(ue_nbiot_radio_capability) {
		...
		if(!ue_nbiot_radio_capability_local_nonprim) {
			ogs_error(
				"OpenAPI_n2_info_content_parseFromJSON failed [ue_nbiot_radio_capability]");
			goto end;
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_transfer_rsp_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}

	ue_context_transfer_rsp_data_local_var =
		OpenAPI_ue_context_transfer_rsp_data_create(
...
OpenAPI_ue_context_transfer_rsp_data_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ue_context_transfer_rsp_data_convertToJSON() failed");
		return NULL;
	}
