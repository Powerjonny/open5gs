...
OpenAPI_acceptable_service_info_t *OpenAPI_acceptable_service_info_create(
	OpenAPI_list_t *acc_bw_med_comps, char *mar_bw_ul, char *mar_bw_dl) {
	OpenAPI_acceptable_service_info_t *acceptable_service_info_local_var =
		ogs_malloc(sizeof(OpenAPI_acceptable_service_info_t));
...
void OpenAPI_acceptable_service_info_free(
	OpenAPI_acceptable_service_info_t *acceptable_service_info) {
	...
	if(acceptable_service_info->acc_bw_med_comps) {
		OpenAPI_list_for_each(acceptable_service_info->acc_bw_med_comps,
		                      node) {
...
cJSON *OpenAPI_acceptable_service_info_convertToJSON(
	...
	if(acceptable_service_info == NULL) {
		ogs_error(
			"OpenAPI_acceptable_service_info_convertToJSON() failed [AcceptableServiceInfo]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(acceptable_service_info->acc_bw_med_comps) {
		cJSON *acc_bw_med_comps = cJSON_AddObjectToObject(item,
		                                                  "accBwMedComps");
		if(acc_bw_med_comps == NULL) {
			ogs_error(
				"OpenAPI_acceptable_service_info_convertToJSON() failed [acc_bw_med_comps]");
			goto end;
		}
		cJSON *localMapObject = acc_bw_med_comps;
		if(acceptable_service_info->acc_bw_med_comps) {
			OpenAPI_list_for_each(
				acceptable_service_info->acc_bw_med_comps,
				node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_acceptable_service_info_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(acceptable_service_info->mar_bw_ul) {
		if(cJSON_AddStringToObject(item, "marBwUl",
		                           acceptable_service_info->mar_bw_ul)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_acceptable_service_info_convertToJSON() failed [mar_bw_ul]");
			goto end;
		}
	}
	...
	if(acceptable_service_info->mar_bw_dl) {
		...
		{
			ogs_error(
				"OpenAPI_acceptable_service_info_convertToJSON() failed [mar_bw_dl]");
			goto end;
		}
	}
...
OpenAPI_acceptable_service_info_t *OpenAPI_acceptable_service_info_parseFromJSON
        (cJSON *acceptable_service_infoJSON) {
	OpenAPI_acceptable_service_info_t *acceptable_service_info_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(acc_bw_med_comps) {
		...
		if(!cJSON_IsObject(acc_bw_med_comps) &&
		   !cJSON_IsNull(acc_bw_med_comps))
		{
			ogs_error(
				"OpenAPI_acceptable_service_info_parseFromJSON() failed [acc_bw_med_comps]");
			goto end;
		}
		if(cJSON_IsObject(acc_bw_med_comps)) {
			acc_bw_med_compsList = OpenAPI_list_create();
			OpenAPI_map_t *localMapKeyPair = NULL;
			cJSON_ArrayForEach(acc_bw_med_comps_local_map,
			                   acc_bw_med_comps) {
				cJSON *localMapObject =
					acc_bw_med_comps_local_map;
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_acceptable_service_info_parseFromJSON() failed [inner]");
					goto end;
				}
				OpenAPI_list_add(acc_bw_med_compsList,
				                 localMapKeyPair);
			}
		}
	}
	...
	if(mar_bw_ul) {
		...
		{
			ogs_error(
				"OpenAPI_acceptable_service_info_parseFromJSON() failed [mar_bw_ul]");
			goto end;
		}
	}
	...
	if(mar_bw_dl) {
		...
		{
			ogs_error(
				"OpenAPI_acceptable_service_info_parseFromJSON() failed [mar_bw_dl]");
			goto end;
		}
	}

	acceptable_service_info_local_var =
...
OpenAPI_acceptable_service_info_t *OpenAPI_acceptable_service_info_copy(
	OpenAPI_acceptable_service_info_t	*dst,
	OpenAPI_acceptable_service_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_acceptable_service_info_convertToJSON() failed");
		return NULL;
	}
