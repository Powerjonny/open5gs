...
OpenAPI_spatial_validity_rm_t *OpenAPI_spatial_validity_rm_create(
	...
	OpenAPI_spatial_validity_rm_t *spatial_validity_rm_local_var =
		ogs_malloc(sizeof(OpenAPI_spatial_validity_rm_t));
...
void OpenAPI_spatial_validity_rm_free(
	OpenAPI_spatial_validity_rm_t *spatial_validity_rm) {
...
cJSON *OpenAPI_spatial_validity_rm_convertToJSON(
	OpenAPI_spatial_validity_rm_t *spatial_validity_rm) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(spatial_validity_rm == NULL) {
		ogs_error(
			"OpenAPI_spatial_validity_rm_convertToJSON() failed [SpatialValidityRm]");
		return NULL;
	}
	...
	if(!spatial_validity_rm->presence_info_list) {
		ogs_error(
			"OpenAPI_spatial_validity_rm_convertToJSON() failed [presence_info_list]");
		return NULL;
	}
	...
	if(spatial_validity_rm->presence_info_list) {
		OpenAPI_list_for_each(spatial_validity_rm->presence_info_list,
			...
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_spatial_validity_rm_convertToJSON() failed [inner]");
				goto end;
			}
...
OpenAPI_spatial_validity_rm_t *OpenAPI_spatial_validity_rm_parseFromJSON(
	cJSON *spatial_validity_rmJSON) {
	...
	presence_info_list =
		cJSON_GetObjectItemCaseSensitive(spatial_validity_rmJSON,
		                                 "presenceInfoList");
	if(!presence_info_list) {
		ogs_error(
			"OpenAPI_spatial_validity_rm_parseFromJSON() failed [presence_info_list]");
		goto end;
	}
	...
	if(cJSON_IsObject(presence_info_list)) {
		...
		cJSON_ArrayForEach(presence_info_list_local_map,
			...
			if(cJSON_IsObject(localMapObject)) {
				...
				ogs_error(
					"OpenAPI_spatial_validity_rm_parseFromJSON() failed [inner]");
				goto end;
			}
...
OpenAPI_spatial_validity_rm_t *OpenAPI_spatial_validity_rm_copy(
	OpenAPI_spatial_validity_rm_t	*dst,
	OpenAPI_spatial_validity_rm_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
