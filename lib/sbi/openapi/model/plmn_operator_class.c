...
OpenAPI_plmn_operator_class_t *OpenAPI_plmn_operator_class_create(
	...
	OpenAPI_plmn_operator_class_t *plmn_operator_class_local_var =
		ogs_malloc(sizeof(OpenAPI_plmn_operator_class_t));
...
void OpenAPI_plmn_operator_class_free(
	OpenAPI_plmn_operator_class_t *plmn_operator_class) {
	...
	if(plmn_operator_class->lcs_client_ids) {
		OpenAPI_list_for_each(plmn_operator_class->lcs_client_ids,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_plmn_operator_class_convertToJSON(
	OpenAPI_plmn_operator_class_t *plmn_operator_class) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(plmn_operator_class == NULL) {
		ogs_error(
			"OpenAPI_plmn_operator_class_convertToJSON() failed [PlmnOperatorClass]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(plmn_operator_class->lcs_client_class ==
	   OpenAPI_lcs_client_class_NULL)
	{
		ogs_error(
			"OpenAPI_plmn_operator_class_convertToJSON() failed [lcs_client_class]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "lcsClientClass",
	                           OpenAPI_lcs_client_class_ToString(
					   plmn_operator_class->lcs_client_class))
	   == NULL)
	{
		ogs_error(
			"OpenAPI_plmn_operator_class_convertToJSON() failed [lcs_client_class]");
	...
	if(!plmn_operator_class->lcs_client_ids) {
		ogs_error(
			"OpenAPI_plmn_operator_class_convertToJSON() failed [lcs_client_ids]");
		return NULL;
	}
	cJSON *lcs_client_idsList = cJSON_AddArrayToObject(item,
	                                                   "lcsClientIds");
	...
	OpenAPI_list_for_each(plmn_operator_class->lcs_client_ids, node) {
		if(cJSON_AddStringToObject(lcs_client_idsList, "",
		                           (char *) node->data) == NULL)
		{
			ogs_error(
				"OpenAPI_plmn_operator_class_convertToJSON() failed [lcs_client_ids]");
...
OpenAPI_plmn_operator_class_t *OpenAPI_plmn_operator_class_parseFromJSON(
	cJSON *plmn_operator_classJSON) {
	...
	lcs_client_class =
		cJSON_GetObjectItemCaseSensitive(plmn_operator_classJSON,
		                                 "lcsClientClass");
	if(!lcs_client_class) {
		ogs_error(
			"OpenAPI_plmn_operator_class_parseFromJSON() failed [lcs_client_class]");
		goto end;
	}
	...
	lcs_client_classVariable =
		OpenAPI_lcs_client_class_FromString(
			lcs_client_class->valuestring);
	...
	if(!lcs_client_ids) {
		ogs_error(
			"OpenAPI_plmn_operator_class_parseFromJSON() failed [lcs_client_ids]");
		goto end;
	}
	...
	cJSON_ArrayForEach(lcs_client_ids_local, lcs_client_ids) {
		...
		OpenAPI_list_add(lcs_client_idsList,
		                 ogs_strdup(lcs_client_ids_local->valuestring));
	}
...
OpenAPI_plmn_operator_class_t *OpenAPI_plmn_operator_class_copy(
	OpenAPI_plmn_operator_class_t	*dst,
	OpenAPI_plmn_operator_class_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
