...
cJSON *OpenAPI_ssm_1_convertToJSON(OpenAPI_ssm_1_t *ssm_1) {
	...
	if(!ssm_1->source_ip_addr) {
		ogs_error(
			"OpenAPI_ssm_1_convertToJSON() failed [source_ip_addr]");
		return NULL;
	}
	cJSON *source_ip_addr_local_JSON =
		OpenAPI_ip_addr_1_convertToJSON(ssm_1->source_ip_addr);
	...
	if(!ssm_1->dest_ip_addr) {
		ogs_error(
			"OpenAPI_ssm_1_convertToJSON() failed [dest_ip_addr]");
		return NULL;
	}
	cJSON *dest_ip_addr_local_JSON =
		OpenAPI_ip_addr_1_convertToJSON(ssm_1->dest_ip_addr);
...
OpenAPI_ssm_1_t *OpenAPI_ssm_1_parseFromJSON(cJSON *ssm_1JSON) {
	...
	source_ip_addr = cJSON_GetObjectItemCaseSensitive(ssm_1JSON,
	                                                  "sourceIpAddr");
	if(!source_ip_addr) {
		ogs_error(
			"OpenAPI_ssm_1_parseFromJSON() failed [source_ip_addr]");
		goto end;
	}
	source_ip_addr_local_nonprim =
		OpenAPI_ip_addr_1_parseFromJSON(source_ip_addr);
	if(!source_ip_addr_local_nonprim) {
		ogs_error(
			"OpenAPI_ip_addr_1_parseFromJSON failed [source_ip_addr]");
		goto end;
	}

	dest_ip_addr = cJSON_GetObjectItemCaseSensitive(ssm_1JSON,
	                                                "destIpAddr");
	if(!dest_ip_addr) {
		ogs_error(
			"OpenAPI_ssm_1_parseFromJSON() failed [dest_ip_addr]");
		goto end;
	}
	dest_ip_addr_local_nonprim =
		OpenAPI_ip_addr_1_parseFromJSON(dest_ip_addr);
	if(!dest_ip_addr_local_nonprim) {
		ogs_error(
			"OpenAPI_ip_addr_1_parseFromJSON failed [dest_ip_addr]");
		goto end;
	}
...
OpenAPI_ssm_1_t *OpenAPI_ssm_1_copy(OpenAPI_ssm_1_t	*dst,
                                    OpenAPI_ssm_1_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
