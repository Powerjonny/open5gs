...
OpenAPI_protection_policy_t *OpenAPI_protection_policy_create(
	...
	OpenAPI_protection_policy_t *protection_policy_local_var =
		ogs_malloc(sizeof(OpenAPI_protection_policy_t));
	ogs_assert(protection_policy_local_var);

	protection_policy_local_var->api_ie_mapping_list = api_ie_mapping_list;
	protection_policy_local_var->data_type_enc_policy =
...
void OpenAPI_protection_policy_free(
	...
	if(protection_policy->api_ie_mapping_list) {
		OpenAPI_list_for_each(protection_policy->api_ie_mapping_list,
		                      node) {
...
cJSON *OpenAPI_protection_policy_convertToJSON(
	...
	if(protection_policy == NULL) {
		ogs_error(
			"OpenAPI_protection_policy_convertToJSON() failed [ProtectionPolicy]");
		return NULL;
	}
	...
	if(!protection_policy->api_ie_mapping_list) {
		ogs_error(
			"OpenAPI_protection_policy_convertToJSON() failed [api_ie_mapping_list]");
		return NULL;
	}
	cJSON *api_ie_mapping_listList = cJSON_AddArrayToObject(item,
	                                                        "apiIeMappingList");
	...
	OpenAPI_list_for_each(protection_policy->api_ie_mapping_list, node) {
		cJSON *itemLocal =
			OpenAPI_api_ie_mapping_convertToJSON(node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_protection_policy_convertToJSON() failed [api_ie_mapping_list]");
			goto end;
	...
	if(protection_policy->data_type_enc_policy != OpenAPI_ie_type_NULL) {
		cJSON *data_type_enc_policyList = cJSON_AddArrayToObject(item,
		                                                         "dataTypeEncPolicy");
		if(data_type_enc_policyList == NULL) {
			ogs_error(
				"OpenAPI_protection_policy_convertToJSON() failed [data_type_enc_policy]");
			goto end;
		}
		OpenAPI_list_for_each(protection_policy->data_type_enc_policy,
		                      node) {
			if(cJSON_AddStringToObject(data_type_enc_policyList, "",
...
OpenAPI_protection_policy_t *OpenAPI_protection_policy_parseFromJSON(
	...
	api_ie_mapping_list =
		cJSON_GetObjectItemCaseSensitive(protection_policyJSON,
		                                 "apiIeMappingList");
	if(!api_ie_mapping_list) {
		ogs_error(
			"OpenAPI_protection_policy_parseFromJSON() failed [api_ie_mapping_list]");
		goto end;
	}
	...
	cJSON_ArrayForEach(api_ie_mapping_list_local, api_ie_mapping_list) {
		...
		OpenAPI_api_ie_mapping_t *api_ie_mapping_listItem =
			OpenAPI_api_ie_mapping_parseFromJSON(
				api_ie_mapping_list_local);
		if(!api_ie_mapping_listItem) {
			ogs_error("No api_ie_mapping_listItem");
			goto end;
		}
		OpenAPI_list_add(api_ie_mapping_listList,
	...
	if(data_type_enc_policy) {
		...
		if(!cJSON_IsArray(data_type_enc_policy)) {
			ogs_error(
				"OpenAPI_protection_policy_parseFromJSON() failed [data_type_enc_policy]");
			goto end;
		}

		data_type_enc_policyList = OpenAPI_list_create();

		cJSON_ArrayForEach(data_type_enc_policy_local,
		                   data_type_enc_policy) {
			...
			localEnum =
				OpenAPI_ie_type_FromString(
					data_type_enc_policy_local->valuestring);
			if(!localEnum) {
				ogs_info(
					"Enum value \"%s\" for field \"data_type_enc_policy\" is not supported. Ignoring it ...",
					data_type_enc_policy_local->valuestring);
			} else {
				OpenAPI_list_add(data_type_enc_policyList,
				                 (void *) localEnum);
			}
		}
		if(data_type_enc_policyList->count == 0) {
			ogs_error(
				"OpenAPI_protection_policy_parseFromJSON() failed: Expected data_type_enc_policyList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
