...
OpenAPI_matching_direction_t *OpenAPI_matching_direction_create() {
	OpenAPI_matching_direction_t *matching_direction_local_var =
		ogs_malloc(sizeof(OpenAPI_matching_direction_t));
...
cJSON *OpenAPI_matching_direction_convertToJSON(
	...
	if(matching_direction == NULL) {
		ogs_error(
			"OpenAPI_matching_direction_convertToJSON() failed [MatchingDirection]");
		return NULL;
	}
