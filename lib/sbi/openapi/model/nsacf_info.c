...
OpenAPI_nsacf_info_t *OpenAPI_nsacf_info_create(
	...
	OpenAPI_nsacf_info_t *nsacf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_nsacf_info_t));
...
cJSON *OpenAPI_nsacf_info_convertToJSON(OpenAPI_nsacf_info_t *nsacf_info) {
	...
	if(nsacf_info == NULL) {
		ogs_error(
			"OpenAPI_nsacf_info_convertToJSON() failed [NsacfInfo]");
		return NULL;
	}
	...
	if(!nsacf_info->nsacf_capability) {
		ogs_error(
			"OpenAPI_nsacf_info_convertToJSON() failed [nsacf_capability]");
		return NULL;
	}
	cJSON *nsacf_capability_local_JSON =
		OpenAPI_nsacf_capability_convertToJSON(
			nsacf_info->nsacf_capability);
	if(nsacf_capability_local_JSON == NULL) {
		ogs_error(
			"OpenAPI_nsacf_info_convertToJSON() failed [nsacf_capability]");
		goto end;
	}
	cJSON_AddItemToObject(item, "nsacfCapability",
	                      nsacf_capability_local_JSON);
	if(item->child == NULL) {
		ogs_error(
			"OpenAPI_nsacf_info_convertToJSON() failed [nsacf_capability]");
		goto end;
	...
	if(nsacf_info->tai_list) {
		...
		if(tai_listList == NULL) {
			ogs_error(
				"OpenAPI_nsacf_info_convertToJSON() failed [tai_list]");
			goto end;
		}
		OpenAPI_list_for_each(nsacf_info->tai_list, node) {
			cJSON *itemLocal =
	...
	if(nsacf_info->tai_range_list) {
		...
		if(tai_range_listList == NULL) {
			ogs_error(
				"OpenAPI_nsacf_info_convertToJSON() failed [tai_range_list]");
			goto end;
		}
		OpenAPI_list_for_each(nsacf_info->tai_range_list, node) {
			cJSON *itemLocal =
...
OpenAPI_nsacf_info_t *OpenAPI_nsacf_info_parseFromJSON(cJSON *nsacf_infoJSON) {
	...
	nsacf_capability = cJSON_GetObjectItemCaseSensitive(nsacf_infoJSON,
	                                                    "nsacfCapability");
	if(!nsacf_capability) {
		ogs_error(
			"OpenAPI_nsacf_info_parseFromJSON() failed [nsacf_capability]");
		goto end;
	}
	nsacf_capability_local_nonprim =
		OpenAPI_nsacf_capability_parseFromJSON(nsacf_capability);
	if(!nsacf_capability_local_nonprim) {
		ogs_error(
			"OpenAPI_nsacf_capability_parseFromJSON failed [nsacf_capability]");
		goto end;
	}
	...
	if(tai_list) {
		...
		if(!cJSON_IsArray(tai_list)) {
			ogs_error(
				"OpenAPI_nsacf_info_parseFromJSON() failed [tai_list]");
			goto end;
		}
	...
	tai_range_list = cJSON_GetObjectItemCaseSensitive(nsacf_infoJSON,
	                                                  "taiRangeList");
	if(tai_range_list) {
		cJSON *tai_range_list_local = NULL;
		if(!cJSON_IsArray(tai_range_list)) {
			ogs_error(
				"OpenAPI_nsacf_info_parseFromJSON() failed [tai_range_list]");
			goto end;
		}
