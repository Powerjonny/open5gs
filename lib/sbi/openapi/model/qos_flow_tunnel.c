...
OpenAPI_qos_flow_tunnel_t *OpenAPI_qos_flow_tunnel_create(
	OpenAPI_list_t *qfi_list, OpenAPI_tunnel_info_t *tunnel_info) {
	OpenAPI_qos_flow_tunnel_t *qos_flow_tunnel_local_var =
		ogs_malloc(sizeof(OpenAPI_qos_flow_tunnel_t));
...
cJSON *OpenAPI_qos_flow_tunnel_convertToJSON(
	OpenAPI_qos_flow_tunnel_t *qos_flow_tunnel) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(qos_flow_tunnel == NULL) {
		ogs_error(
			"OpenAPI_qos_flow_tunnel_convertToJSON() failed [QosFlowTunnel]");
		return NULL;
	}
	...
	if(!qos_flow_tunnel->qfi_list) {
		ogs_error(
			"OpenAPI_qos_flow_tunnel_convertToJSON() failed [qfi_list]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(qos_flow_tunnel->qfi_list, node) {
		...
		if(cJSON_AddNumberToObject(qfi_listList, "",
	...
	if(!qos_flow_tunnel->tunnel_info) {
		ogs_error(
			"OpenAPI_qos_flow_tunnel_convertToJSON() failed [tunnel_info]");
		return NULL;
	}
	cJSON *tunnel_info_local_JSON =
		OpenAPI_tunnel_info_convertToJSON(qos_flow_tunnel->tunnel_info);
...
OpenAPI_qos_flow_tunnel_t *OpenAPI_qos_flow_tunnel_parseFromJSON(
	cJSON *qos_flow_tunnelJSON) {
	...
	qfi_list = cJSON_GetObjectItemCaseSensitive(qos_flow_tunnelJSON,
	                                            "qfiList");
	if(!qfi_list) {
		ogs_error(
			"OpenAPI_qos_flow_tunnel_parseFromJSON() failed [qfi_list]");
		goto end;
	}
	...
	tunnel_info = cJSON_GetObjectItemCaseSensitive(qos_flow_tunnelJSON,
	                                               "tunnelInfo");
	if(!tunnel_info) {
		ogs_error(
			"OpenAPI_qos_flow_tunnel_parseFromJSON() failed [tunnel_info]");
		goto end;
	}
	tunnel_info_local_nonprim =
		OpenAPI_tunnel_info_parseFromJSON(tunnel_info);
	if(!tunnel_info_local_nonprim) {
		ogs_error(
			"OpenAPI_tunnel_info_parseFromJSON failed [tunnel_info]");
		goto end;
	}
...
OpenAPI_qos_flow_tunnel_t *OpenAPI_qos_flow_tunnel_copy(
	OpenAPI_qos_flow_tunnel_t *dst, OpenAPI_qos_flow_tunnel_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
