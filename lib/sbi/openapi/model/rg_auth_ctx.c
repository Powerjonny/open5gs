...
OpenAPI_rg_auth_ctx_t *OpenAPI_rg_auth_ctx_create(
	OpenAPI_auth_result_e auth_result, char *supi, bool is_auth_ind,
	int auth_ind) {
	OpenAPI_rg_auth_ctx_t *rg_auth_ctx_local_var =
		ogs_malloc(sizeof(OpenAPI_rg_auth_ctx_t));
...
cJSON *OpenAPI_rg_auth_ctx_convertToJSON(OpenAPI_rg_auth_ctx_t *rg_auth_ctx) {
	...
	if(rg_auth_ctx == NULL) {
		ogs_error(
			"OpenAPI_rg_auth_ctx_convertToJSON() failed [RgAuthCtx]");
		return NULL;
	}
	...
	if(rg_auth_ctx->auth_result == OpenAPI_auth_result_NULL) {
		ogs_error(
			"OpenAPI_rg_auth_ctx_convertToJSON() failed [auth_result]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "authResult",
	                           OpenAPI_auth_result_ToString(
					   rg_auth_ctx->auth_result)) == NULL)
	{
		ogs_error(
			"OpenAPI_rg_auth_ctx_convertToJSON() failed [auth_result]");
		goto end;
	...
	if(rg_auth_ctx->supi) {
		...
		{
			ogs_error(
				"OpenAPI_rg_auth_ctx_convertToJSON() failed [supi]");
			goto end;
		}
	}

	if(rg_auth_ctx->is_auth_ind) {
		if(cJSON_AddBoolToObject(item, "authInd",
		                         rg_auth_ctx->auth_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_rg_auth_ctx_convertToJSON() failed [auth_ind]");
			goto end;
		}
	}
...
OpenAPI_rg_auth_ctx_t *OpenAPI_rg_auth_ctx_parseFromJSON(
	...
	cJSON *supi = NULL;
	cJSON *auth_ind = NULL;
	auth_result = cJSON_GetObjectItemCaseSensitive(rg_auth_ctxJSON,
	                                               "authResult");
	if(!auth_result) {
		ogs_error(
			"OpenAPI_rg_auth_ctx_parseFromJSON() failed [auth_result]");
		goto end;
	}
	...
	if(supi) {
		...
		{
			ogs_error(
				"OpenAPI_rg_auth_ctx_parseFromJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(auth_ind) {
		if(!cJSON_IsBool(auth_ind)) {
			ogs_error(
				"OpenAPI_rg_auth_ctx_parseFromJSON() failed [auth_ind]");
			goto end;
		}
	}
...
OpenAPI_rg_auth_ctx_t *OpenAPI_rg_auth_ctx_copy(OpenAPI_rg_auth_ctx_t	*dst,
                                                OpenAPI_rg_auth_ctx_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
