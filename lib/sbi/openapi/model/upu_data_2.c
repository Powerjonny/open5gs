...
OpenAPI_upu_data_2_t *OpenAPI_upu_data_2_create(char *sec_packet,
                                                OpenAPI_list_t *
                                                default_conf_nssai,
                                                char *routing_id) {
	OpenAPI_upu_data_2_t *upu_data_2_local_var =
		ogs_malloc(sizeof(OpenAPI_upu_data_2_t));
...
cJSON *OpenAPI_upu_data_2_convertToJSON(OpenAPI_upu_data_2_t *upu_data_2) {
	...
	if(upu_data_2 == NULL) {
		ogs_error(
			"OpenAPI_upu_data_2_convertToJSON() failed [UpuData_2]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(upu_data_2->sec_packet) {
		if(cJSON_AddStringToObject(item, "secPacket",
		                           upu_data_2->sec_packet) == NULL)
		{
			ogs_error(
				"OpenAPI_upu_data_2_convertToJSON() failed [sec_packet]");
			goto end;
		}
	}

	if(upu_data_2->default_conf_nssai) {
		cJSON *default_conf_nssaiList = cJSON_AddArrayToObject(item,
		                                                       "defaultConfNssai");
		if(default_conf_nssaiList == NULL) {
			ogs_error(
				"OpenAPI_upu_data_2_convertToJSON() failed [default_conf_nssai]");
			goto end;
		}
		OpenAPI_list_for_each(upu_data_2->default_conf_nssai, node) {
			cJSON *itemLocal =
	...
	if(upu_data_2->routing_id) {
		...
		{
			ogs_error(
				"OpenAPI_upu_data_2_convertToJSON() failed [routing_id]");
			goto end;
		}
	}
...
OpenAPI_upu_data_2_t *OpenAPI_upu_data_2_parseFromJSON(cJSON *upu_data_2JSON) {
	...
	sec_packet = cJSON_GetObjectItemCaseSensitive(upu_data_2JSON,
	                                              "secPacket");
	if(sec_packet) {
		if(!cJSON_IsString(sec_packet) &&
		   !cJSON_IsNull(sec_packet))
		{
			ogs_error(
				"OpenAPI_upu_data_2_parseFromJSON() failed [sec_packet]");
			goto end;
		}
	}

	default_conf_nssai = cJSON_GetObjectItemCaseSensitive(upu_data_2JSON,
	                                                      "defaultConfNssai");
	if(default_conf_nssai) {
		cJSON *default_conf_nssai_local = NULL;
		if(!cJSON_IsArray(default_conf_nssai)) {
			ogs_error(
				"OpenAPI_upu_data_2_parseFromJSON() failed [default_conf_nssai]");
			goto end;
		}

		default_conf_nssaiList = OpenAPI_list_create();

		cJSON_ArrayForEach(default_conf_nssai_local,
		                   default_conf_nssai) {
			...
			OpenAPI_snssai_t *default_conf_nssaiItem =
				OpenAPI_snssai_parseFromJSON(
					default_conf_nssai_local);
			if(!default_conf_nssaiItem) {
				ogs_error("No default_conf_nssaiItem");
				goto end;
			}
			OpenAPI_list_add(default_conf_nssaiList,
			                 default_conf_nssaiItem);
		}
	}

	routing_id = cJSON_GetObjectItemCaseSensitive(upu_data_2JSON,
	                                              "routingId");
	if(routing_id) {
		...
		{
			ogs_error(
				"OpenAPI_upu_data_2_parseFromJSON() failed [routing_id]");
			goto end;
		}
	}
	...
	upu_data_2_local_var = OpenAPI_upu_data_2_create(
		sec_packet &&
		!cJSON_IsNull(sec_packet) ?
		ogs_strdup(sec_packet->valuestring) : NULL,
...
OpenAPI_upu_data_2_t *OpenAPI_upu_data_2_copy(OpenAPI_upu_data_2_t	*dst,
                                              OpenAPI_upu_data_2_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
