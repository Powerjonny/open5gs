...
OpenAPI_pdu_session_management_data_create(
	OpenAPI_pdu_session_status_e pdu_session_status,
	char *pdu_session_status_ts, char *dnai, char *dnai_ts,
	OpenAPI_list_t *n6_traffic_routing_info,
	char *n6_traffic_routing_info_ts, char *ipv4_addr,
	OpenAPI_list_t *ipv6_prefix, OpenAPI_list_t *ipv6_addrs,
	OpenAPI_pdu_session_type_e pdu_sess_type, char *ip_addr_ts, char *dnn,
	bool is_pdu_session_id, int pdu_session_id, char *supp_feat,
	OpenAPI_list_t *reset_ids) {
	OpenAPI_pdu_session_management_data_t *
	        pdu_session_management_data_local_var =
		ogs_malloc(sizeof(OpenAPI_pdu_session_management_data_t));
	ogs_assert(pdu_session_management_data_local_var);

	pdu_session_management_data_local_var->pdu_session_status =
		pdu_session_status;
	pdu_session_management_data_local_var->pdu_session_status_ts =
		pdu_session_status_ts;
	pdu_session_management_data_local_var->dnai = dnai;
	pdu_session_management_data_local_var->dnai_ts = dnai_ts;
	pdu_session_management_data_local_var->n6_traffic_routing_info =
		n6_traffic_routing_info;
	pdu_session_management_data_local_var->n6_traffic_routing_info_ts =
	...
	pdu_session_management_data_local_var->is_pdu_session_id =
		is_pdu_session_id;
...
void OpenAPI_pdu_session_management_data_free(
	OpenAPI_pdu_session_management_data_t *pdu_session_management_data) {
	...
	if(pdu_session_management_data->n6_traffic_routing_info) {
		OpenAPI_list_for_each(
			pdu_session_management_data->n6_traffic_routing_info,
			node) {
	...
	if(pdu_session_management_data->n6_traffic_routing_info_ts) {
		ogs_free(
			pdu_session_management_data->n6_traffic_routing_info_ts);
	...
	if(pdu_session_management_data->ipv6_addrs) {
		OpenAPI_list_for_each(pdu_session_management_data->ipv6_addrs,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(pdu_session_management_data->reset_ids) {
		OpenAPI_list_for_each(pdu_session_management_data->reset_ids,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_pdu_session_management_data_convertToJSON(
	OpenAPI_pdu_session_management_data_t *pdu_session_management_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(pdu_session_management_data == NULL) {
		ogs_error(
			"OpenAPI_pdu_session_management_data_convertToJSON() failed [PduSessionManagementData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(pdu_session_management_data->pdu_session_status !=
	   OpenAPI_pdu_session_status_NULL)
	{
		if(cJSON_AddStringToObject(item, "pduSessionStatus",
		                           OpenAPI_pdu_session_status_ToString(
						   pdu_session_management_data->
						   pdu_session_status)) == NULL)
		{
			ogs_error(
				"OpenAPI_pdu_session_management_data_convertToJSON() failed [pdu_session_status]");
			goto end;
		}
	}

	if(pdu_session_management_data->pdu_session_status_ts) {
		if(cJSON_AddStringToObject(item, "pduSessionStatusTs",
		                           pdu_session_management_data->
		                           pdu_session_status_ts) == NULL)
		{
			ogs_error(
				"OpenAPI_pdu_session_management_data_convertToJSON() failed [pdu_session_status_ts]");
			goto end;
		}
	}
	...
	if(pdu_session_management_data->dnai) {
		if(cJSON_AddStringToObject(item, "dnai",
		                           pdu_session_management_data->dnai) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pdu_session_management_data_convertToJSON() failed [dnai]");
			goto end;
		}
	}

	if(pdu_session_management_data->dnai_ts) {
		if(cJSON_AddStringToObject(item, "dnaiTs",
		                           pdu_session_management_data->dnai_ts)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_pdu_session_management_data_convertToJSON() failed [dnai_ts]");
			goto end;
		}
	}

	if(pdu_session_management_data->n6_traffic_routing_info) {
		cJSON *n6_traffic_routing_infoList =
			cJSON_AddArrayToObject(item, "n6TrafficRoutingInfo");
		if(n6_traffic_routing_infoList == NULL) {
			ogs_error(
				"OpenAPI_pdu_session_management_data_convertToJSON() failed [n6_traffic_routing_info]");
			goto end;
		}
		OpenAPI_list_for_each(
			pdu_session_management_data->n6_traffic_routing_info,
			...
			cJSON_AddItemToArray(n6_traffic_routing_infoList,
			                     itemLocal);
		}
	}

	if(pdu_session_management_data->n6_traffic_routing_info_ts) {
		if(cJSON_AddStringToObject(item, "n6TrafficRoutingInfoTs",
		                           pdu_session_management_data->
		                           n6_traffic_routing_info_ts) == NULL)
		{
			ogs_error(
				"OpenAPI_pdu_session_management_data_convertToJSON() failed [n6_traffic_routing_info_ts]");
			goto end;
		}
	}
	...
	if(pdu_session_management_data->ipv4_addr) {
		...
		{
			ogs_error(
				"OpenAPI_pdu_session_management_data_convertToJSON() failed [ipv4_addr]");
			goto end;
		}
	}

	if(pdu_session_management_data->ipv6_prefix) {
		cJSON *ipv6_prefixList = cJSON_AddArrayToObject(item,
		                                                "ipv6Prefix");
		if(ipv6_prefixList == NULL) {
			ogs_error(
				"OpenAPI_pdu_session_management_data_convertToJSON() failed [ipv6_prefix]");
			goto end;
		}
		OpenAPI_list_for_each(pdu_session_management_data->ipv6_prefix,
		                      node) {
			if(cJSON_AddStringToObject(ipv6_prefixList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_pdu_session_management_data_convertToJSON() failed [ipv6_prefix]");
	...
	if(pdu_session_management_data->ipv6_addrs) {
		...
		if(ipv6_addrsList == NULL) {
			ogs_error(
				"OpenAPI_pdu_session_management_data_convertToJSON() failed [ipv6_addrs]");
			goto end;
		}
		OpenAPI_list_for_each(pdu_session_management_data->ipv6_addrs,
		                      node) {
	...
	{
		if(cJSON_AddStringToObject(item, "pduSessType",
		                           OpenAPI_pdu_session_type_ToString(
						   pdu_session_management_data->
						   pdu_sess_type)) == NULL)
		{
			ogs_error(
				"OpenAPI_pdu_session_management_data_convertToJSON() failed [pdu_sess_type]");
			goto end;
		}
	}

	if(pdu_session_management_data->ip_addr_ts) {
		if(cJSON_AddStringToObject(item, "ipAddrTs",
		                           pdu_session_management_data->
		                           ip_addr_ts) == NULL)
		{
			ogs_error(
				"OpenAPI_pdu_session_management_data_convertToJSON() failed [ip_addr_ts]");
			goto end;
		}
	}
	...
	if(pdu_session_management_data->dnn) {
		if(cJSON_AddStringToObject(item, "dnn",
		                           pdu_session_management_data->dnn) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pdu_session_management_data_convertToJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(pdu_session_management_data->is_pdu_session_id) {
		...
		{
			ogs_error(
				"OpenAPI_pdu_session_management_data_convertToJSON() failed [pdu_session_id]");
			goto end;
		}
	}
	...
	if(pdu_session_management_data->supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_pdu_session_management_data_convertToJSON() failed [supp_feat]");
			goto end;
		}
	}
	...
	if(pdu_session_management_data->reset_ids) {
		...
		if(reset_idsList == NULL) {
			ogs_error(
				"OpenAPI_pdu_session_management_data_convertToJSON() failed [reset_ids]");
			goto end;
		}
		OpenAPI_list_for_each(pdu_session_management_data->reset_ids,
		                      node) {
...
OpenAPI_pdu_session_management_data_t *
OpenAPI_pdu_session_management_data_parseFromJSON(
	cJSON *pdu_session_management_dataJSON) {
	OpenAPI_pdu_session_management_data_t *
	        pdu_session_management_data_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	pdu_session_status =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_management_dataJSON, "pduSessionStatus");
	if(pdu_session_status) {
		if(!cJSON_IsString(pdu_session_status)) {
			ogs_error(
				"OpenAPI_pdu_session_management_data_parseFromJSON() failed [pdu_session_status]");
			goto end;
		}
		pdu_session_statusVariable =
			OpenAPI_pdu_session_status_FromString(
				pdu_session_status->valuestring);
	}

	pdu_session_status_ts =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_management_dataJSON, "pduSessionStatusTs");
	if(pdu_session_status_ts) {
		if(!cJSON_IsString(pdu_session_status_ts) &&
		   !cJSON_IsNull(pdu_session_status_ts))
		{
			ogs_error(
				"OpenAPI_pdu_session_management_data_parseFromJSON() failed [pdu_session_status_ts]");
			goto end;
		}
	}

	dnai = cJSON_GetObjectItemCaseSensitive(pdu_session_management_dataJSON,
	                                        "dnai");
	if(dnai) {
		...
		{
			ogs_error(
				"OpenAPI_pdu_session_management_data_parseFromJSON() failed [dnai]");
			goto end;
		}
	}
	...
	dnai_ts =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_management_dataJSON, "dnaiTs");
	if(dnai_ts) {
		if(!cJSON_IsString(dnai_ts) &&
		   !cJSON_IsNull(dnai_ts))
		{
			ogs_error(
				"OpenAPI_pdu_session_management_data_parseFromJSON() failed [dnai_ts]");
			goto end;
		}
	}
	...
	n6_traffic_routing_info =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_management_dataJSON,
	...
	if(n6_traffic_routing_info) {
		...
		if(!cJSON_IsArray(n6_traffic_routing_info)) {
			ogs_error(
				"OpenAPI_pdu_session_management_data_parseFromJSON() failed [n6_traffic_routing_info]");
			goto end;
		}
		...
		cJSON_ArrayForEach(n6_traffic_routing_info_local,
			...
			OpenAPI_route_to_location_t *n6_traffic_routing_infoItem
			        = OpenAPI_route_to_location_parseFromJSON(
					  n6_traffic_routing_info_local);
			if(!n6_traffic_routing_infoItem) {
				ogs_error("No n6_traffic_routing_infoItem");
				goto end;
			}
			OpenAPI_list_add(n6_traffic_routing_infoList,
			                 n6_traffic_routing_infoItem);
		}
	}
	...
	if(n6_traffic_routing_info_ts) {
		...
		{
			ogs_error(
				"OpenAPI_pdu_session_management_data_parseFromJSON() failed [n6_traffic_routing_info_ts]");
			goto end;
		}
	}
	...
	ipv4_addr =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_management_dataJSON, "ipv4Addr");
	if(ipv4_addr) {
		if(!cJSON_IsString(ipv4_addr) &&
		   !cJSON_IsNull(ipv4_addr))
		{
			ogs_error(
				"OpenAPI_pdu_session_management_data_parseFromJSON() failed [ipv4_addr]");
			goto end;
		}
	}
	...
	ipv6_prefix =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_management_dataJSON, "ipv6Prefix");
	if(ipv6_prefix) {
		cJSON *ipv6_prefix_local = NULL;
		if(!cJSON_IsArray(ipv6_prefix)) {
			ogs_error(
				"OpenAPI_pdu_session_management_data_parseFromJSON() failed [ipv6_prefix]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ipv6_prefix_local, ipv6_prefix) {
			...
			OpenAPI_list_add(ipv6_prefixList,
			                 ogs_strdup(
	...
	ipv6_addrs =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_management_dataJSON, "ipv6Addrs");
	if(ipv6_addrs) {
		cJSON *ipv6_addrs_local = NULL;
		if(!cJSON_IsArray(ipv6_addrs)) {
			ogs_error(
				"OpenAPI_pdu_session_management_data_parseFromJSON() failed [ipv6_addrs]");
			goto end;
		}
	...
	pdu_sess_type =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_management_dataJSON, "pduSessType");
	if(pdu_sess_type) {
		if(!cJSON_IsString(pdu_sess_type)) {
			ogs_error(
				"OpenAPI_pdu_session_management_data_parseFromJSON() failed [pdu_sess_type]");
			goto end;
		}
		pdu_sess_typeVariable =
			OpenAPI_pdu_session_type_FromString(
				pdu_sess_type->valuestring);
	}

	ip_addr_ts =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_management_dataJSON, "ipAddrTs");
	if(ip_addr_ts) {
		if(!cJSON_IsString(ip_addr_ts) &&
		   !cJSON_IsNull(ip_addr_ts))
		{
			ogs_error(
				"OpenAPI_pdu_session_management_data_parseFromJSON() failed [ip_addr_ts]");
			goto end;
		}
	}

	dnn = cJSON_GetObjectItemCaseSensitive(pdu_session_management_dataJSON,
	                                       "dnn");
	if(dnn) {
		...
		{
			ogs_error(
				"OpenAPI_pdu_session_management_data_parseFromJSON() failed [dnn]");
			goto end;
		}
	}
	...
	pdu_session_id =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_management_dataJSON, "pduSessionId");
	if(pdu_session_id) {
		if(!cJSON_IsNumber(pdu_session_id)) {
			ogs_error(
				"OpenAPI_pdu_session_management_data_parseFromJSON() failed [pdu_session_id]");
			goto end;
		}
	}
	...
	supp_feat =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_management_dataJSON, "suppFeat");
	if(supp_feat) {
		if(!cJSON_IsString(supp_feat) &&
		   !cJSON_IsNull(supp_feat))
		{
			ogs_error(
				"OpenAPI_pdu_session_management_data_parseFromJSON() failed [supp_feat]");
			goto end;
		}
	}
	...
	reset_ids =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_management_dataJSON, "resetIds");
	if(reset_ids) {
		cJSON *reset_ids_local = NULL;
		if(!cJSON_IsArray(reset_ids)) {
			ogs_error(
				"OpenAPI_pdu_session_management_data_parseFromJSON() failed [reset_ids]");
			goto end;
		}
	...
	pdu_session_management_data_local_var =
		OpenAPI_pdu_session_management_data_create(
			...
			pdu_session_status_ts &&
			!cJSON_IsNull(pdu_session_status_ts) ?
			ogs_strdup(pdu_session_status_ts->valuestring) : NULL,
			dnai &&
			!cJSON_IsNull(dnai) ? ogs_strdup(dnai->valuestring) :
			NULL,
			dnai_ts &&
			!cJSON_IsNull(dnai_ts) ?
			ogs_strdup(dnai_ts->valuestring) :
			NULL,
			n6_traffic_routing_info ? n6_traffic_routing_infoList :
			NULL,
			n6_traffic_routing_info_ts &&
			!cJSON_IsNull(n6_traffic_routing_info_ts) ?
			ogs_strdup(n6_traffic_routing_info_ts->valuestring) :
			NULL,
			ipv4_addr &&
			!cJSON_IsNull(ipv4_addr) ?
			ogs_strdup(ipv4_addr->valuestring) :
			NULL,
			...
			ip_addr_ts &&
			!cJSON_IsNull(ip_addr_ts) ?
			ogs_strdup(ip_addr_ts->valuestring) : NULL,
			dnn &&
...
OpenAPI_pdu_session_management_data_t *OpenAPI_pdu_session_management_data_copy(
	OpenAPI_pdu_session_management_data_t	*dst,
	OpenAPI_pdu_session_management_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_pdu_session_management_data_convertToJSON() failed");
		return NULL;
	}
