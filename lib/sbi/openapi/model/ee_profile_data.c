...
OpenAPI_ee_profile_data_t *OpenAPI_ee_profile_data_create(
	OpenAPI_list_t *restricted_event_types, char *supported_features,
	OpenAPI_list_t *allowed_mtc_provider, bool is_iwk_epc_restricted,
	int iwk_epc_restricted, char *imsi, char *hss_group_id) {
	OpenAPI_ee_profile_data_t *ee_profile_data_local_var =
		ogs_malloc(sizeof(OpenAPI_ee_profile_data_t));
	ogs_assert(ee_profile_data_local_var);

	ee_profile_data_local_var->restricted_event_types =
	...
	ee_profile_data_local_var->is_iwk_epc_restricted =
		is_iwk_epc_restricted;
...
void OpenAPI_ee_profile_data_free(OpenAPI_ee_profile_data_t *ee_profile_data) {
	...
	if(ee_profile_data->restricted_event_types) {
		OpenAPI_list_for_each(ee_profile_data->restricted_event_types,
		                      node) {
	...
	if(ee_profile_data->allowed_mtc_provider) {
		OpenAPI_list_for_each(ee_profile_data->allowed_mtc_provider,
		                      node) {
...
cJSON *OpenAPI_ee_profile_data_convertToJSON(
	...
	if(ee_profile_data == NULL) {
		ogs_error(
			"OpenAPI_ee_profile_data_convertToJSON() failed [EeProfileData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(ee_profile_data->restricted_event_types) {
		cJSON *restricted_event_typesList = cJSON_AddArrayToObject(item,
		                                                           "restrictedEventTypes");
		if(restricted_event_typesList == NULL) {
			ogs_error(
				"OpenAPI_ee_profile_data_convertToJSON() failed [restricted_event_types]");
			goto end;
		}
		OpenAPI_list_for_each(ee_profile_data->restricted_event_types,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_event_type_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_ee_profile_data_convertToJSON() failed [restricted_event_types]");
				goto end;
			}
			cJSON_AddItemToArray(restricted_event_typesList,
			                     itemLocal);
		}
	}
	...
	if(ee_profile_data->supported_features) {
		if(cJSON_AddStringToObject(item, "supportedFeatures",
		                           ee_profile_data->supported_features)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_ee_profile_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}

	if(ee_profile_data->allowed_mtc_provider) {
		cJSON *allowed_mtc_provider = cJSON_AddObjectToObject(item,
		                                                      "allowedMtcProvider");
		if(allowed_mtc_provider == NULL) {
			ogs_error(
				"OpenAPI_ee_profile_data_convertToJSON() failed [allowed_mtc_provider]");
			goto end;
		}
		cJSON *localMapObject = allowed_mtc_provider;
		if(ee_profile_data->allowed_mtc_provider) {
			OpenAPI_list_for_each(
				ee_profile_data->allowed_mtc_provider, node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_mtc_provider_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_ee_profile_data_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(ee_profile_data->is_iwk_epc_restricted) {
		if(cJSON_AddBoolToObject(item, "iwkEpcRestricted",
		                         ee_profile_data->iwk_epc_restricted) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_ee_profile_data_convertToJSON() failed [iwk_epc_restricted]");
			goto end;
		}
	}

	if(ee_profile_data->imsi) {
		if(cJSON_AddStringToObject(item, "imsi",
		                           ee_profile_data->imsi) == NULL)
		{
			ogs_error(
				"OpenAPI_ee_profile_data_convertToJSON() failed [imsi]");
			goto end;
		}
	}
	...
	if(ee_profile_data->hss_group_id) {
		if(cJSON_AddStringToObject(item, "hssGroupId",
		                           ee_profile_data->hss_group_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_ee_profile_data_convertToJSON() failed [hss_group_id]");
			goto end;
		}
	}
...
OpenAPI_ee_profile_data_t *OpenAPI_ee_profile_data_parseFromJSON(
	...
	restricted_event_types =
		cJSON_GetObjectItemCaseSensitive(ee_profile_dataJSON,
		                                 "restrictedEventTypes");
	if(restricted_event_types) {
		cJSON *restricted_event_types_local = NULL;
		if(!cJSON_IsArray(restricted_event_types)) {
			ogs_error(
				"OpenAPI_ee_profile_data_parseFromJSON() failed [restricted_event_types]");
			goto end;
		}

		restricted_event_typesList = OpenAPI_list_create();

		cJSON_ArrayForEach(restricted_event_types_local,
		                   restricted_event_types) {
			...
			OpenAPI_event_type_t *restricted_event_typesItem =
				OpenAPI_event_type_parseFromJSON(
					restricted_event_types_local);
			if(!restricted_event_typesItem) {
				ogs_error("No restricted_event_typesItem");
				goto end;
			}
			OpenAPI_list_add(restricted_event_typesList,
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_ee_profile_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(allowed_mtc_provider) {
		...
		if(!cJSON_IsObject(allowed_mtc_provider) &&
		   !cJSON_IsNull(allowed_mtc_provider))
		{
			ogs_error(
				"OpenAPI_ee_profile_data_parseFromJSON() failed [allowed_mtc_provider]");
			goto end;
		}
		if(cJSON_IsObject(allowed_mtc_provider)) {
			...
			cJSON_ArrayForEach(allowed_mtc_provider_local_map,
				...
				cJSON *localMapObject =
					allowed_mtc_provider_local_map;
				...
				if(cJSON_IsObject(localMapObject)) {
					localMapKeyPair = OpenAPI_map_create(
						...
						OpenAPI_mtc_provider_parseFromJSON
						        (localMapObject));
					...
					ogs_error(
						"OpenAPI_ee_profile_data_parseFromJSON() failed [inner]");
					goto end;
				}
				OpenAPI_list_add(allowed_mtc_providerList,
				                 localMapKeyPair);
			}
		}
	}
	...
	if(iwk_epc_restricted) {
		if(!cJSON_IsBool(iwk_epc_restricted)) {
			ogs_error(
				"OpenAPI_ee_profile_data_parseFromJSON() failed [iwk_epc_restricted]");
			goto end;
		}
	}
	...
	if(imsi) {
		if(!cJSON_IsString(imsi) &&
		   !cJSON_IsNull(imsi))
		{
			ogs_error(
				"OpenAPI_ee_profile_data_parseFromJSON() failed [imsi]");
			goto end;
		}
	}

	hss_group_id = cJSON_GetObjectItemCaseSensitive(ee_profile_dataJSON,
	                                                "hssGroupId");
	if(hss_group_id) {
		...
		{
			ogs_error(
				"OpenAPI_ee_profile_data_parseFromJSON() failed [hss_group_id]");
			goto end;
		}
	}
	...
	ee_profile_data_local_var = OpenAPI_ee_profile_data_create(
		...
		imsi &&
		!cJSON_IsNull(imsi) ? ogs_strdup(imsi->valuestring) : NULL,
		hss_group_id &&
		!cJSON_IsNull(hss_group_id) ?
...
OpenAPI_ee_profile_data_t *OpenAPI_ee_profile_data_copy(
	OpenAPI_ee_profile_data_t *dst, OpenAPI_ee_profile_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
