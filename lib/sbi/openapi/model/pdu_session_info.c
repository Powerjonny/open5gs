...
OpenAPI_pdu_session_info_t *OpenAPI_pdu_session_info_create(
	...
	OpenAPI_pdu_session_info_t *pdu_session_info_local_var =
		ogs_malloc(sizeof(OpenAPI_pdu_session_info_t));
...
cJSON *OpenAPI_pdu_session_info_convertToJSON(
	...
	if(pdu_session_info == NULL) {
		ogs_error(
			"OpenAPI_pdu_session_info_convertToJSON() failed [PduSessionInfo]");
		return NULL;
	}
	...
	if(!pdu_session_info->snssai) {
		ogs_error(
			"OpenAPI_pdu_session_info_convertToJSON() failed [snssai]");
		return NULL;
	}
	cJSON *snssai_local_JSON =
		OpenAPI_snssai_convertToJSON(pdu_session_info->snssai);
	...
	if(!pdu_session_info->dnn) {
		ogs_error(
			"OpenAPI_pdu_session_info_convertToJSON() failed [dnn]");
		return NULL;
	}
...
OpenAPI_pdu_session_info_t *OpenAPI_pdu_session_info_parseFromJSON(
	...
	cJSON *dnn = NULL;
	snssai = cJSON_GetObjectItemCaseSensitive(pdu_session_infoJSON,
	                                          "snssai");
	if(!snssai) {
		ogs_error(
			"OpenAPI_pdu_session_info_parseFromJSON() failed [snssai]");
		goto end;
	}
	...
	if(!dnn) {
		ogs_error(
			"OpenAPI_pdu_session_info_parseFromJSON() failed [dnn]");
		goto end;
	}
