...
OpenAPI_pdu_session_event_notification_create(
	OpenAPI_af_event_notification_t *ev_notif, char *supi, char *ue_ipv4,
	char *ue_ipv6, char *ue_mac, OpenAPI_npcf_pdu_session_status_e status,
	OpenAPI_pcf_addressing_info_t *pcf_info, char *dnn,
	OpenAPI_snssai_t *snssai, char *gpsi) {
	OpenAPI_pdu_session_event_notification_t *
	        pdu_session_event_notification_local_var =
		ogs_malloc(sizeof(OpenAPI_pdu_session_event_notification_t));
...
void OpenAPI_pdu_session_event_notification_free(
	OpenAPI_pdu_session_event_notification_t *pdu_session_event_notification)
{
	...
	if(pdu_session_event_notification->ev_notif) {
		OpenAPI_af_event_notification_free(
			pdu_session_event_notification->ev_notif);
	...
	if(pdu_session_event_notification->pcf_info) {
		OpenAPI_pcf_addressing_info_free(
			pdu_session_event_notification->pcf_info);
...
cJSON *OpenAPI_pdu_session_event_notification_convertToJSON(
	OpenAPI_pdu_session_event_notification_t *pdu_session_event_notification)
{
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(pdu_session_event_notification == NULL) {
		ogs_error(
			"OpenAPI_pdu_session_event_notification_convertToJSON() failed [PduSessionEventNotification]");
		return NULL;
	}
	...
	if(!pdu_session_event_notification->ev_notif) {
		ogs_error(
			"OpenAPI_pdu_session_event_notification_convertToJSON() failed [ev_notif]");
		return NULL;
	}
	cJSON *ev_notif_local_JSON =
	...
	if(pdu_session_event_notification->supi) {
		if(cJSON_AddStringToObject(item, "supi",
		                           pdu_session_event_notification->supi)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_pdu_session_event_notification_convertToJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(pdu_session_event_notification->ue_ipv4) {
		if(cJSON_AddStringToObject(item, "ueIpv4",
		                           pdu_session_event_notification->
		                           ue_ipv4) == NULL)
		{
			ogs_error(
				"OpenAPI_pdu_session_event_notification_convertToJSON() failed [ue_ipv4]");
			goto end;
		}
	}
	...
	if(pdu_session_event_notification->ue_ipv6) {
		...
		{
			ogs_error(
				"OpenAPI_pdu_session_event_notification_convertToJSON() failed [ue_ipv6]");
			goto end;
		}
	}
	...
	if(pdu_session_event_notification->ue_mac) {
		...
		{
			ogs_error(
				"OpenAPI_pdu_session_event_notification_convertToJSON() failed [ue_mac]");
			goto end;
		}
	}

	if(pdu_session_event_notification->status !=
	   OpenAPI_npcf_pdu_session_status_NULL)
	{
		if(cJSON_AddStringToObject(item, "status",
		                           OpenAPI_npcf_pdu_session_status_ToString
					   (
						   pdu_session_event_notification
						   ->status)) == NULL)
		{
			ogs_error(
				"OpenAPI_pdu_session_event_notification_convertToJSON() failed [status]");
			goto end;
		}
	}
	...
	if(pdu_session_event_notification->pcf_info) {
		cJSON *pcf_info_local_JSON =
			OpenAPI_pcf_addressing_info_convertToJSON(
				pdu_session_event_notification->pcf_info);
		if(pcf_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_pdu_session_event_notification_convertToJSON() failed [pcf_info]");
			goto end;
		}
	...
	if(pdu_session_event_notification->dnn) {
		if(cJSON_AddStringToObject(item, "dnn",
		                           pdu_session_event_notification->dnn)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_pdu_session_event_notification_convertToJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(pdu_session_event_notification->snssai) {
		cJSON *snssai_local_JSON =
			OpenAPI_snssai_convertToJSON(
				pdu_session_event_notification->snssai);
		if(snssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_pdu_session_event_notification_convertToJSON() failed [snssai]");
			goto end;
		}
	...
	if(pdu_session_event_notification->gpsi) {
		if(cJSON_AddStringToObject(item, "gpsi",
		                           pdu_session_event_notification->gpsi)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_pdu_session_event_notification_convertToJSON() failed [gpsi]");
			goto end;
		}
	}
...
OpenAPI_pdu_session_event_notification_parseFromJSON(
	cJSON *pdu_session_event_notificationJSON) {
	OpenAPI_pdu_session_event_notification_t *
	        pdu_session_event_notification_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *ev_notif = NULL;
	...
	ev_notif =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_event_notificationJSON, "evNotif");
	if(!ev_notif) {
		ogs_error(
			"OpenAPI_pdu_session_event_notification_parseFromJSON() failed [ev_notif]");
		goto end;
	}
	ev_notif_local_nonprim =
		OpenAPI_af_event_notification_parseFromJSON(ev_notif);
	if(!ev_notif_local_nonprim) {
		ogs_error(
			"OpenAPI_af_event_notification_parseFromJSON failed [ev_notif]");
		goto end;
	}

	supi =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_event_notificationJSON, "supi");
	if(supi) {
		if(!cJSON_IsString(supi) &&
		   !cJSON_IsNull(supi))
		{
			ogs_error(
				"OpenAPI_pdu_session_event_notification_parseFromJSON() failed [supi]");
			goto end;
		}
	}
	...
	ue_ipv4 =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_event_notificationJSON, "ueIpv4");
	if(ue_ipv4) {
		if(!cJSON_IsString(ue_ipv4) &&
		   !cJSON_IsNull(ue_ipv4))
		{
			ogs_error(
				"OpenAPI_pdu_session_event_notification_parseFromJSON() failed [ue_ipv4]");
			goto end;
		}
	}
	...
	ue_ipv6 =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_event_notificationJSON, "ueIpv6");
	if(ue_ipv6) {
		if(!cJSON_IsString(ue_ipv6) &&
		   !cJSON_IsNull(ue_ipv6))
		{
			ogs_error(
				"OpenAPI_pdu_session_event_notification_parseFromJSON() failed [ue_ipv6]");
			goto end;
		}
	}
	...
	ue_mac =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_event_notificationJSON, "ueMac");
	if(ue_mac) {
		if(!cJSON_IsString(ue_mac) &&
		   !cJSON_IsNull(ue_mac))
		{
			ogs_error(
				"OpenAPI_pdu_session_event_notification_parseFromJSON() failed [ue_mac]");
			goto end;
		}
	}
	...
	status =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_event_notificationJSON, "status");
	if(status) {
		if(!cJSON_IsString(status)) {
			ogs_error(
				"OpenAPI_pdu_session_event_notification_parseFromJSON() failed [status]");
			goto end;
		}
		statusVariable =
	...
	pcf_info =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_event_notificationJSON, "pcfInfo");
	if(pcf_info) {
		pcf_info_local_nonprim =
			OpenAPI_pcf_addressing_info_parseFromJSON(pcf_info);
		if(!pcf_info_local_nonprim) {
			ogs_error(
				"OpenAPI_pcf_addressing_info_parseFromJSON failed [pcf_info]");
			goto end;
		}
	}
	...
	dnn =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_event_notificationJSON, "dnn");
	if(dnn) {
		if(!cJSON_IsString(dnn) &&
		   !cJSON_IsNull(dnn))
		{
			ogs_error(
				"OpenAPI_pdu_session_event_notification_parseFromJSON() failed [dnn]");
			goto end;
		}
	}
	...
	snssai =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_event_notificationJSON, "snssai");
	if(snssai) {
	...
	gpsi =
		cJSON_GetObjectItemCaseSensitive(
			pdu_session_event_notificationJSON, "gpsi");
	if(gpsi) {
		if(!cJSON_IsString(gpsi) &&
		   !cJSON_IsNull(gpsi))
		{
			ogs_error(
				"OpenAPI_pdu_session_event_notification_parseFromJSON() failed [gpsi]");
			goto end;
		}
	}
...
OpenAPI_pdu_session_event_notification_t *
OpenAPI_pdu_session_event_notification_copy(
	OpenAPI_pdu_session_event_notification_t	*dst,
	OpenAPI_pdu_session_event_notification_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_pdu_session_event_notification_convertToJSON() failed");
		return NULL;
	}
