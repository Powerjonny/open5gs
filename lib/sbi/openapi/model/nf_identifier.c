...
OpenAPI_nf_identifier_t *OpenAPI_nf_identifier_create(OpenAPI_nf_type_e nf_type,
                                                      char		*
                                                      nf_instance_id) {
	OpenAPI_nf_identifier_t *nf_identifier_local_var =
		ogs_malloc(sizeof(OpenAPI_nf_identifier_t));
...
cJSON *OpenAPI_nf_identifier_convertToJSON(
	...
	if(nf_identifier == NULL) {
		ogs_error(
			"OpenAPI_nf_identifier_convertToJSON() failed [NfIdentifier]");
		return NULL;
	}
	...
	if(nf_identifier->nf_type == OpenAPI_nf_type_NULL) {
		ogs_error(
			"OpenAPI_nf_identifier_convertToJSON() failed [nf_type]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "nfType",
	                           OpenAPI_nf_type_ToString(
					   nf_identifier->nf_type)) == NULL)
	{
		ogs_error(
			"OpenAPI_nf_identifier_convertToJSON() failed [nf_type]");
		goto end;
	...
	if(nf_identifier->nf_instance_id) {
		if(cJSON_AddStringToObject(item, "nfInstanceId",
		                           nf_identifier->nf_instance_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_nf_identifier_convertToJSON() failed [nf_instance_id]");
			goto end;
		}
	}
...
OpenAPI_nf_identifier_t *OpenAPI_nf_identifier_parseFromJSON(
	...
	if(!nf_type) {
		ogs_error(
			"OpenAPI_nf_identifier_parseFromJSON() failed [nf_type]");
		goto end;
	}
	...
	nf_instance_id = cJSON_GetObjectItemCaseSensitive(nf_identifierJSON,
	                                                  "nfInstanceId");
	if(nf_instance_id) {
		...
		{
			ogs_error(
				"OpenAPI_nf_identifier_parseFromJSON() failed [nf_instance_id]");
			goto end;
		}
	}
