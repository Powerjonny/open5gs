...
OpenAPI_lcs_mo_data_t *OpenAPI_lcs_mo_data_create(
	...
	OpenAPI_lcs_mo_data_t *lcs_mo_data_local_var =
		ogs_malloc(sizeof(OpenAPI_lcs_mo_data_t));
	ogs_assert(lcs_mo_data_local_var);

	lcs_mo_data_local_var->allowed_service_classes =
		allowed_service_classes;
	lcs_mo_data_local_var->mo_assistance_data_types =
		mo_assistance_data_types;
...
void OpenAPI_lcs_mo_data_free(OpenAPI_lcs_mo_data_t *lcs_mo_data) {
	...
	if(lcs_mo_data->mo_assistance_data_types) {
		OpenAPI_lcs_broadcast_assistance_types_data_free(
			lcs_mo_data->mo_assistance_data_types);
...
cJSON *OpenAPI_lcs_mo_data_convertToJSON(OpenAPI_lcs_mo_data_t *lcs_mo_data) {
	...
	if(lcs_mo_data == NULL) {
		ogs_error(
			"OpenAPI_lcs_mo_data_convertToJSON() failed [LcsMoData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(lcs_mo_data->allowed_service_classes ==
	   OpenAPI_lcs_mo_service_class_NULL)
	{
		ogs_error(
			"OpenAPI_lcs_mo_data_convertToJSON() failed [allowed_service_classes]");
		return NULL;
	}
	cJSON *allowed_service_classesList = cJSON_AddArrayToObject(item,
	...
	OpenAPI_list_for_each(lcs_mo_data->allowed_service_classes, node) {
		if(cJSON_AddStringToObject(allowed_service_classesList, "",
		                           OpenAPI_lcs_mo_service_class_ToString
		                                   ((intptr_t) node->data)) ==
		   NULL)
		{
			ogs_error(
	...
	if(lcs_mo_data->mo_assistance_data_types) {
		...
		if(mo_assistance_data_types_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_lcs_mo_data_convertToJSON() failed [mo_assistance_data_types]");
			goto end;
		}
...
OpenAPI_lcs_mo_data_t *OpenAPI_lcs_mo_data_parseFromJSON(
	cJSON *lcs_mo_dataJSON) {
	...
	allowed_service_classes =
		cJSON_GetObjectItemCaseSensitive(lcs_mo_dataJSON,
		                                 "allowedServiceClasses");
	if(!allowed_service_classes) {
		ogs_error(
			"OpenAPI_lcs_mo_data_parseFromJSON() failed [allowed_service_classes]");
		goto end;
	}
	...
	cJSON_ArrayForEach(allowed_service_classes_local,
	                   allowed_service_classes) {
		OpenAPI_lcs_mo_service_class_e localEnum =
			OpenAPI_lcs_mo_service_class_NULL;
		...
		localEnum =
			OpenAPI_lcs_mo_service_class_FromString(
				allowed_service_classes_local->valuestring);
		if(!localEnum) {
			ogs_info(
				"Enum value \"%s\" for field \"allowed_service_classes\" is not supported. Ignoring it ...",
				allowed_service_classes_local->valuestring);
		} else {
			OpenAPI_list_add(allowed_service_classesList,
			                 (void *) localEnum);
		}
	}
	if(allowed_service_classesList->count == 0) {
		ogs_error(
			"OpenAPI_lcs_mo_data_parseFromJSON() failed: Expected allowed_service_classesList to not be empty (after ignoring unsupported enum values).");
		goto end;
	}
...
OpenAPI_lcs_mo_data_t *OpenAPI_lcs_mo_data_copy(OpenAPI_lcs_mo_data_t	*dst,
                                                OpenAPI_lcs_mo_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
