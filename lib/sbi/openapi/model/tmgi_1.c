...
OpenAPI_tmgi_1_t *OpenAPI_tmgi_1_create(char			*mbs_service_id,
                                        OpenAPI_plmn_id_1_t	*plmn_id) {
	OpenAPI_tmgi_1_t *tmgi_1_local_var =
		ogs_malloc(sizeof(OpenAPI_tmgi_1_t));
...
cJSON *OpenAPI_tmgi_1_convertToJSON(OpenAPI_tmgi_1_t *tmgi_1) {
	...
	if(cJSON_AddStringToObject(item, "mbsServiceId",
	                           tmgi_1->mbs_service_id) == NULL)
	{
		ogs_error(
			"OpenAPI_tmgi_1_convertToJSON() failed [mbs_service_id]");
	...
	cJSON *plmn_id_local_JSON =
		OpenAPI_plmn_id_1_convertToJSON(tmgi_1->plmn_id);
...
OpenAPI_tmgi_1_t *OpenAPI_tmgi_1_parseFromJSON(cJSON *tmgi_1JSON) {
	...
	mbs_service_id = cJSON_GetObjectItemCaseSensitive(tmgi_1JSON,
	                                                  "mbsServiceId");
	if(!mbs_service_id) {
		ogs_error(
			"OpenAPI_tmgi_1_parseFromJSON() failed [mbs_service_id]");
		goto end;
	}
...
OpenAPI_tmgi_1_t *OpenAPI_tmgi_1_copy(OpenAPI_tmgi_1_t	*dst,
                                      OpenAPI_tmgi_1_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
