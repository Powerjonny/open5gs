...
OpenAPI_analytics_subscription_t *OpenAPI_analytics_subscription_create(
	char *nwdaf_id, char *nwdaf_set_id,
	OpenAPI_list_t *nwdaf_subscription_list) {
	OpenAPI_analytics_subscription_t *analytics_subscription_local_var =
...
void OpenAPI_analytics_subscription_free(
	OpenAPI_analytics_subscription_t *analytics_subscription) {
	...
	if(analytics_subscription->nwdaf_subscription_list) {
		OpenAPI_list_for_each(
			analytics_subscription->nwdaf_subscription_list, node) {
			OpenAPI_nwdaf_subscription_free(node->data);
		}
		OpenAPI_list_free(
			analytics_subscription->nwdaf_subscription_list);
		analytics_subscription->nwdaf_subscription_list = NULL;
	}
	ogs_free(analytics_subscription);
}

cJSON *OpenAPI_analytics_subscription_convertToJSON(
	OpenAPI_analytics_subscription_t *analytics_subscription) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(analytics_subscription == NULL) {
		ogs_error(
			"OpenAPI_analytics_subscription_convertToJSON() failed [AnalyticsSubscription]");
		return NULL;
	}
	...
	if(analytics_subscription->nwdaf_id) {
		...
		{
			ogs_error(
				"OpenAPI_analytics_subscription_convertToJSON() failed [nwdaf_id]");
			goto end;
		}
	}
	...
	if(analytics_subscription->nwdaf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_analytics_subscription_convertToJSON() failed [nwdaf_set_id]");
			goto end;
		}
	}
	...
	if(!analytics_subscription->nwdaf_subscription_list) {
		ogs_error(
			"OpenAPI_analytics_subscription_convertToJSON() failed [nwdaf_subscription_list]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(analytics_subscription->nwdaf_subscription_list,
	                      node) {
		cJSON *itemLocal =
			OpenAPI_nwdaf_subscription_convertToJSON(node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_analytics_subscription_convertToJSON() failed [nwdaf_subscription_list]");
			goto end;
...
OpenAPI_analytics_subscription_t *OpenAPI_analytics_subscription_parseFromJSON(
	...
	nwdaf_set_id =
		cJSON_GetObjectItemCaseSensitive(analytics_subscriptionJSON,
		                                 "nwdafSetId");
	if(nwdaf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_analytics_subscription_parseFromJSON() failed [nwdaf_set_id]");
			goto end;
		}
	}
	...
	if(!nwdaf_subscription_list) {
		ogs_error(
			"OpenAPI_analytics_subscription_parseFromJSON() failed [nwdaf_subscription_list]");
		goto end;
	}
	...
	cJSON_ArrayForEach(nwdaf_subscription_list_local,
		...
		OpenAPI_nwdaf_subscription_t *nwdaf_subscription_listItem =
		...
		OpenAPI_list_add(nwdaf_subscription_listList,
		                 nwdaf_subscription_listItem);
	}

	analytics_subscription_local_var =
		OpenAPI_analytics_subscription_create(
			nwdaf_id &&
			!cJSON_IsNull(nwdaf_id) ?
			ogs_strdup(nwdaf_id->valuestring) :
			NULL,
			nwdaf_set_id &&
			!cJSON_IsNull(nwdaf_set_id) ?
...
OpenAPI_analytics_subscription_t *OpenAPI_analytics_subscription_copy(
	OpenAPI_analytics_subscription_t	*dst,
	OpenAPI_analytics_subscription_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_analytics_subscription_convertToJSON() failed");
		return NULL;
	}
