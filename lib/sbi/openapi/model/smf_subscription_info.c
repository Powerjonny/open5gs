...
OpenAPI_smf_subscription_info_t *OpenAPI_smf_subscription_info_create(
	...
	OpenAPI_smf_subscription_info_t *smf_subscription_info_local_var =
		ogs_malloc(sizeof(OpenAPI_smf_subscription_info_t));
	ogs_assert(smf_subscription_info_local_var);

	smf_subscription_info_local_var->smf_subscription_list =
		smf_subscription_list;

	return smf_subscription_info_local_var;
}

void OpenAPI_smf_subscription_info_free(
	...
	if(smf_subscription_info->smf_subscription_list) {
		OpenAPI_list_for_each(
			smf_subscription_info->smf_subscription_list, node) {
...
cJSON *OpenAPI_smf_subscription_info_convertToJSON(
	OpenAPI_smf_subscription_info_t *smf_subscription_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(smf_subscription_info == NULL) {
		ogs_error(
			"OpenAPI_smf_subscription_info_convertToJSON() failed [SmfSubscriptionInfo]");
		return NULL;
	}
	...
	if(!smf_subscription_info->smf_subscription_list) {
		ogs_error(
			"OpenAPI_smf_subscription_info_convertToJSON() failed [smf_subscription_list]");
		return NULL;
	}
	cJSON *smf_subscription_listList = cJSON_AddArrayToObject(item,
	                                                          "smfSubscriptionList");
	...
	OpenAPI_list_for_each(smf_subscription_info->smf_subscription_list,
	                      node) {
		cJSON *itemLocal =
			OpenAPI_smf_subscription_item_convertToJSON(node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_smf_subscription_info_convertToJSON() failed [smf_subscription_list]");
			goto end;
...
OpenAPI_smf_subscription_info_t *OpenAPI_smf_subscription_info_parseFromJSON(
	cJSON *smf_subscription_infoJSON) {
	...
	smf_subscription_list =
		cJSON_GetObjectItemCaseSensitive(smf_subscription_infoJSON,
		                                 "smfSubscriptionList");
	if(!smf_subscription_list) {
		ogs_error(
			"OpenAPI_smf_subscription_info_parseFromJSON() failed [smf_subscription_list]");
		goto end;
	}
	...
	cJSON_ArrayForEach(smf_subscription_list_local, smf_subscription_list) {
		...
		OpenAPI_smf_subscription_item_t *smf_subscription_listItem =
			OpenAPI_smf_subscription_item_parseFromJSON(
				smf_subscription_list_local);
		if(!smf_subscription_listItem) {
			ogs_error("No smf_subscription_listItem");
			goto end;
		}
		OpenAPI_list_add(smf_subscription_listList,
		                 smf_subscription_listItem);
	}
...
OpenAPI_smf_subscription_info_t *OpenAPI_smf_subscription_info_copy(
	OpenAPI_smf_subscription_info_t *dst,
	OpenAPI_smf_subscription_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_smf_subscription_info_convertToJSON() failed");
		return NULL;
	}
