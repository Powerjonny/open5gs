...
OpenAPI_abnormal_behaviour_t *OpenAPI_abnormal_behaviour_create(
	...
	OpenAPI_abnormal_behaviour_t *abnormal_behaviour_local_var =
		ogs_malloc(sizeof(OpenAPI_abnormal_behaviour_t));
...
cJSON *OpenAPI_abnormal_behaviour_convertToJSON(
	...
	if(abnormal_behaviour == NULL) {
		ogs_error(
			"OpenAPI_abnormal_behaviour_convertToJSON() failed [AbnormalBehaviour]");
		return NULL;
	}
	...
	if(abnormal_behaviour->supis) {
		...
		if(supisList == NULL) {
			ogs_error(
				"OpenAPI_abnormal_behaviour_convertToJSON() failed [supis]");
			goto end;
		}
		OpenAPI_list_for_each(abnormal_behaviour->supis, node) {
	...
	if(!abnormal_behaviour->excep) {
		ogs_error(
			"OpenAPI_abnormal_behaviour_convertToJSON() failed [excep]");
		return NULL;
	}
	cJSON *excep_local_JSON =
		OpenAPI_exception_convertToJSON(abnormal_behaviour->excep);
	...
	if(abnormal_behaviour->dnn) {
		...
		{
			ogs_error(
				"OpenAPI_abnormal_behaviour_convertToJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(abnormal_behaviour->snssai) {
		...
		if(snssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_abnormal_behaviour_convertToJSON() failed [snssai]");
			goto end;
		}
	...
	if(abnormal_behaviour->is_ratio) {
		...
		{
			ogs_error(
				"OpenAPI_abnormal_behaviour_convertToJSON() failed [ratio]");
			goto end;
		}
	}
	...
	if(abnormal_behaviour->is_confidence) {
		...
		{
			ogs_error(
				"OpenAPI_abnormal_behaviour_convertToJSON() failed [confidence]");
			goto end;
		}
	}
	...
	if(abnormal_behaviour->addt_meas_info) {
		cJSON *addt_meas_info_local_JSON =
			OpenAPI_additional_measurement_convertToJSON(
				abnormal_behaviour->addt_meas_info);
		if(addt_meas_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_abnormal_behaviour_convertToJSON() failed [addt_meas_info]");
			goto end;
		}
...
OpenAPI_abnormal_behaviour_t *OpenAPI_abnormal_behaviour_parseFromJSON(
	...
	supis = cJSON_GetObjectItemCaseSensitive(abnormal_behaviourJSON,
	                                         "supis");
	if(supis) {
		cJSON *supis_local = NULL;
		if(!cJSON_IsArray(supis)) {
			ogs_error(
				"OpenAPI_abnormal_behaviour_parseFromJSON() failed [supis]");
			goto end;
		}
	...
	excep = cJSON_GetObjectItemCaseSensitive(abnormal_behaviourJSON,
	                                         "excep");
	if(!excep) {
		ogs_error(
			"OpenAPI_abnormal_behaviour_parseFromJSON() failed [excep]");
		goto end;
	}
	...
	if(dnn) {
		...
		{
			ogs_error(
				"OpenAPI_abnormal_behaviour_parseFromJSON() failed [dnn]");
			goto end;
		}
	}
	...
	ratio = cJSON_GetObjectItemCaseSensitive(abnormal_behaviourJSON,
	                                         "ratio");
	if(ratio) {
		if(!cJSON_IsNumber(ratio)) {
			ogs_error(
				"OpenAPI_abnormal_behaviour_parseFromJSON() failed [ratio]");
			goto end;
		}
	}
	...
	if(confidence) {
		if(!cJSON_IsNumber(confidence)) {
			ogs_error(
				"OpenAPI_abnormal_behaviour_parseFromJSON() failed [confidence]");
			goto end;
		}
	}
	...
	if(addt_meas_info) {
		...
		if(!addt_meas_info_local_nonprim) {
			ogs_error(
				"OpenAPI_additional_measurement_parseFromJSON failed [addt_meas_info]");
			goto end;
		}
	}
