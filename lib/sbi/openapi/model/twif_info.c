...
OpenAPI_twif_info_t *OpenAPI_twif_info_create(
	...
	OpenAPI_twif_info_t *twif_info_local_var =
		ogs_malloc(sizeof(OpenAPI_twif_info_t));
...
void OpenAPI_twif_info_free(OpenAPI_twif_info_t *twif_info) {
	...
	if(twif_info->ipv4_endpoint_addresses) {
		OpenAPI_list_for_each(twif_info->ipv4_endpoint_addresses,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(twif_info->ipv6_endpoint_addresses) {
		OpenAPI_list_for_each(twif_info->ipv6_endpoint_addresses,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_twif_info_convertToJSON(OpenAPI_twif_info_t *twif_info) {
	...
	if(twif_info == NULL) {
		ogs_error(
			"OpenAPI_twif_info_convertToJSON() failed [TwifInfo]");
		return NULL;
	}
	...
	if(twif_info->ipv4_endpoint_addresses) {
		...
		if(ipv4_endpoint_addressesList == NULL) {
			ogs_error(
				"OpenAPI_twif_info_convertToJSON() failed [ipv4_endpoint_addresses]");
			goto end;
		}
		OpenAPI_list_for_each(twif_info->ipv4_endpoint_addresses,
		                      node) {
	...
	if(twif_info->ipv6_endpoint_addresses) {
		...
		if(ipv6_endpoint_addressesList == NULL) {
			ogs_error(
				"OpenAPI_twif_info_convertToJSON() failed [ipv6_endpoint_addresses]");
			goto end;
		}
		OpenAPI_list_for_each(twif_info->ipv6_endpoint_addresses,
		                      node) {
	...
	if(twif_info->endpoint_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_twif_info_convertToJSON() failed [endpoint_fqdn]");
			goto end;
		}
	}
...
OpenAPI_twif_info_t *OpenAPI_twif_info_parseFromJSON(cJSON *twif_infoJSON) {
	...
	if(ipv6_endpoint_addresses) {
		...
		if(!cJSON_IsArray(ipv6_endpoint_addresses)) {
			ogs_error(
				"OpenAPI_twif_info_parseFromJSON() failed [ipv6_endpoint_addresses]");
			goto end;
		}
	...
	endpoint_fqdn = cJSON_GetObjectItemCaseSensitive(twif_infoJSON,
	                                                 "endpointFqdn");
	if(endpoint_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_twif_info_parseFromJSON() failed [endpoint_fqdn]");
			goto end;
		}
	}
...
OpenAPI_twif_info_t *OpenAPI_twif_info_copy(OpenAPI_twif_info_t *dst,
                                            OpenAPI_twif_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
