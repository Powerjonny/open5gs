...
OpenAPI_qos_data_t *OpenAPI_qos_data_create(char *qos_id, bool is__5qi,
                                            int _5qi, bool is_maxbr_ul_null,
                                            char *maxbr_ul,
                                            bool is_maxbr_dl_null,
                                            char *maxbr_dl, bool is_gbr_ul_null,
                                            char *gbr_ul, bool is_gbr_dl_null,
                                            char *gbr_dl, OpenAPI_arp_t *arp,
                                            bool is_qnc, int qnc,
	...
	OpenAPI_qos_data_t *qos_data_local_var =
		ogs_malloc(sizeof(OpenAPI_qos_data_t));
	...
	qos_data_local_var->is_max_data_burst_vol_null =
		is_max_data_burst_vol_null;
	...
	qos_data_local_var->is_max_packet_loss_rate_dl_null =
		is_max_packet_loss_rate_dl_null;
	qos_data_local_var->is_max_packet_loss_rate_dl =
		is_max_packet_loss_rate_dl;
	qos_data_local_var->max_packet_loss_rate_dl = max_packet_loss_rate_dl;
	qos_data_local_var->is_max_packet_loss_rate_ul_null =
		is_max_packet_loss_rate_ul_null;
	qos_data_local_var->is_max_packet_loss_rate_ul =
		is_max_packet_loss_rate_ul;
	qos_data_local_var->max_packet_loss_rate_ul = max_packet_loss_rate_ul;
	qos_data_local_var->is_def_qos_flow_indication =
		is_def_qos_flow_indication;
	qos_data_local_var->def_qos_flow_indication = def_qos_flow_indication;
	qos_data_local_var->is_ext_max_data_burst_vol_null =
		is_ext_max_data_burst_vol_null;
	qos_data_local_var->is_ext_max_data_burst_vol =
		is_ext_max_data_burst_vol;
...
cJSON *OpenAPI_qos_data_convertToJSON(OpenAPI_qos_data_t *qos_data) {
	...
	if(qos_data->is__5qi) {
		...
		{
			ogs_error(
				"OpenAPI_qos_data_convertToJSON() failed [_5qi]");
			goto end;
		}
	}

	if(qos_data->maxbr_ul) {
		if(cJSON_AddStringToObject(item, "maxbrUl",
		                           qos_data->maxbr_ul) == NULL)
		{
			ogs_error(
				"OpenAPI_qos_data_convertToJSON() failed [maxbr_ul]");
			goto end;
		}
	...
	if(qos_data->maxbr_dl) {
		if(cJSON_AddStringToObject(item, "maxbrDl",
		                           qos_data->maxbr_dl) == NULL)
		{
			ogs_error(
				"OpenAPI_qos_data_convertToJSON() failed [maxbr_dl]");
			goto end;
		}
	...
	if(qos_data->gbr_ul) {
		...
		{
			ogs_error(
				"OpenAPI_qos_data_convertToJSON() failed [gbr_ul]");
			goto end;
		}
	...
	if(qos_data->gbr_dl) {
		...
		{
			ogs_error(
				"OpenAPI_qos_data_convertToJSON() failed [gbr_dl]");
			goto end;
		}
	...
	if(qos_data->arp) {
		cJSON *arp_local_JSON =
			OpenAPI_arp_convertToJSON(qos_data->arp);
		if(arp_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_qos_data_convertToJSON() failed [arp]");
			goto end;
		}
	...
	if(qos_data->is_qnc) {
		if(cJSON_AddBoolToObject(item, "qnc", qos_data->qnc) == NULL) {
			ogs_error(
				"OpenAPI_qos_data_convertToJSON() failed [qnc]");
			goto end;
		}
	}
	...
	if(qos_data->is_priority_level) {
		if(cJSON_AddNumberToObject(item, "priorityLevel",
		                           qos_data->priority_level) == NULL)
		{
			ogs_error(
				"OpenAPI_qos_data_convertToJSON() failed [priority_level]");
			goto end;
		}
	...
	if(qos_data->is_aver_window) {
		if(cJSON_AddNumberToObject(item, "averWindow",
		                           qos_data->aver_window) == NULL)
		{
			ogs_error(
				"OpenAPI_qos_data_convertToJSON() failed [aver_window]");
			goto end;
		}
	...
	if(qos_data->is_max_data_burst_vol) {
		if(cJSON_AddNumberToObject(item, "maxDataBurstVol",
		                           qos_data->max_data_burst_vol) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_qos_data_convertToJSON() failed [max_data_burst_vol]");
			goto end;
		}
	...
	if(qos_data->is_reflective_qos) {
		if(cJSON_AddBoolToObject(item, "reflectiveQos",
		                         qos_data->reflective_qos) == NULL)
		{
			ogs_error(
				"OpenAPI_qos_data_convertToJSON() failed [reflective_qos]");
			goto end;
		}
	}

	if(qos_data->sharing_key_dl) {
		if(cJSON_AddStringToObject(item, "sharingKeyDl",
		                           qos_data->sharing_key_dl) == NULL)
		{
			ogs_error(
				"OpenAPI_qos_data_convertToJSON() failed [sharing_key_dl]");
			goto end;
		}
	}

	if(qos_data->sharing_key_ul) {
		if(cJSON_AddStringToObject(item, "sharingKeyUl",
		                           qos_data->sharing_key_ul) == NULL)
		{
			ogs_error(
				"OpenAPI_qos_data_convertToJSON() failed [sharing_key_ul]");
			goto end;
		}
	}
	...
	if(qos_data->is_max_packet_loss_rate_dl) {
		if(cJSON_AddNumberToObject(item, "maxPacketLossRateDl",
		                           qos_data->max_packet_loss_rate_dl) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_qos_data_convertToJSON() failed [max_packet_loss_rate_dl]");
			goto end;
		}
	...
	if(qos_data->is_max_packet_loss_rate_ul) {
		if(cJSON_AddNumberToObject(item, "maxPacketLossRateUl",
		                           qos_data->max_packet_loss_rate_ul) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_qos_data_convertToJSON() failed [max_packet_loss_rate_ul]");
			goto end;
		}
	...
	if(qos_data->is_def_qos_flow_indication) {
		if(cJSON_AddBoolToObject(item, "defQosFlowIndication",
		                         qos_data->def_qos_flow_indication) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_qos_data_convertToJSON() failed [def_qos_flow_indication]");
			goto end;
		}
	}
	...
	if(qos_data->is_ext_max_data_burst_vol) {
		if(cJSON_AddNumberToObject(item, "extMaxDataBurstVol",
		                           qos_data->ext_max_data_burst_vol) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_qos_data_convertToJSON() failed [ext_max_data_burst_vol]");
			goto end;
		}
	...
	if(qos_data->is_packet_delay_budget) {
		if(cJSON_AddNumberToObject(item, "packetDelayBudget",
		                           qos_data->packet_delay_budget) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_qos_data_convertToJSON() failed [packet_delay_budget]");
			goto end;
		}
	}

	if(qos_data->packet_error_rate) {
		if(cJSON_AddStringToObject(item, "packetErrorRate",
		                           qos_data->packet_error_rate) == NULL)
		{
			ogs_error(
				"OpenAPI_qos_data_convertToJSON() failed [packet_error_rate]");
			goto end;
		}
	}
...
OpenAPI_qos_data_t *OpenAPI_qos_data_parseFromJSON(cJSON *qos_dataJSON) {
	...
	if(_5qi) {
		if(!cJSON_IsNumber(_5qi)) {
			ogs_error(
				"OpenAPI_qos_data_parseFromJSON() failed [_5qi]");
			goto end;
		}
	}
	...
	if(maxbr_ul) {
		if(!cJSON_IsNull(maxbr_ul)) {
			if(!cJSON_IsString(maxbr_ul) &&
			   !cJSON_IsNull(maxbr_ul))
			{
				ogs_error(
					"OpenAPI_qos_data_parseFromJSON() failed [maxbr_ul]");
				goto end;
			}
		}
	}
	...
	if(maxbr_dl) {
		if(!cJSON_IsNull(maxbr_dl)) {
			if(!cJSON_IsString(maxbr_dl) &&
			   !cJSON_IsNull(maxbr_dl))
			{
				ogs_error(
					"OpenAPI_qos_data_parseFromJSON() failed [maxbr_dl]");
				goto end;
			}
		}
	}
	...
	if(gbr_ul) {
		if(!cJSON_IsNull(gbr_ul)) {
			...
			{
				ogs_error(
					"OpenAPI_qos_data_parseFromJSON() failed [gbr_ul]");
				goto end;
			}
		}
	}
	...
	if(gbr_dl) {
		if(!cJSON_IsNull(gbr_dl)) {
			...
			{
				ogs_error(
					"OpenAPI_qos_data_parseFromJSON() failed [gbr_dl]");
				goto end;
			}
		}
	}
	...
	if(qnc) {
		if(!cJSON_IsBool(qnc)) {
			ogs_error(
				"OpenAPI_qos_data_parseFromJSON() failed [qnc]");
			goto end;
		}
	}

	priority_level = cJSON_GetObjectItemCaseSensitive(qos_dataJSON,
	                                                  "priorityLevel");
	if(priority_level) {
		if(!cJSON_IsNull(priority_level)) {
			if(!cJSON_IsNumber(priority_level)) {
				ogs_error(
					"OpenAPI_qos_data_parseFromJSON() failed [priority_level]");
				goto end;
			}
		}
	}

	aver_window = cJSON_GetObjectItemCaseSensitive(qos_dataJSON,
	                                               "averWindow");
	if(aver_window) {
	...
	max_data_burst_vol = cJSON_GetObjectItemCaseSensitive(qos_dataJSON,
	                                                      "maxDataBurstVol");
	if(max_data_burst_vol) {
		if(!cJSON_IsNull(max_data_burst_vol)) {
			if(!cJSON_IsNumber(max_data_burst_vol)) {
				ogs_error(
					"OpenAPI_qos_data_parseFromJSON() failed [max_data_burst_vol]");
				goto end;
			}
		}
	}

	reflective_qos = cJSON_GetObjectItemCaseSensitive(qos_dataJSON,
	                                                  "reflectiveQos");
	if(reflective_qos) {
		if(!cJSON_IsBool(reflective_qos)) {
			ogs_error(
				"OpenAPI_qos_data_parseFromJSON() failed [reflective_qos]");
			goto end;
		}
	}

	sharing_key_dl = cJSON_GetObjectItemCaseSensitive(qos_dataJSON,
	                                                  "sharingKeyDl");
	if(sharing_key_dl) {
		...
		{
			ogs_error(
				"OpenAPI_qos_data_parseFromJSON() failed [sharing_key_dl]");
			goto end;
		}
	}

	sharing_key_ul = cJSON_GetObjectItemCaseSensitive(qos_dataJSON,
	                                                  "sharingKeyUl");
	if(sharing_key_ul) {
		if(!cJSON_IsString(sharing_key_ul) &&
		   !cJSON_IsNull(sharing_key_ul))
		{
			ogs_error(
				"OpenAPI_qos_data_parseFromJSON() failed [sharing_key_ul]");
			goto end;
		}
	}

	max_packet_loss_rate_dl = cJSON_GetObjectItemCaseSensitive(qos_dataJSON,
	                                                           "maxPacketLossRateDl");
	if(max_packet_loss_rate_dl) {
		if(!cJSON_IsNull(max_packet_loss_rate_dl)) {
			if(!cJSON_IsNumber(max_packet_loss_rate_dl)) {
				ogs_error(
					"OpenAPI_qos_data_parseFromJSON() failed [max_packet_loss_rate_dl]");
				goto end;
			}
		}
	}

	max_packet_loss_rate_ul = cJSON_GetObjectItemCaseSensitive(qos_dataJSON,
	                                                           "maxPacketLossRateUl");
	if(max_packet_loss_rate_ul) {
		if(!cJSON_IsNull(max_packet_loss_rate_ul)) {
			if(!cJSON_IsNumber(max_packet_loss_rate_ul)) {
				ogs_error(
					"OpenAPI_qos_data_parseFromJSON() failed [max_packet_loss_rate_ul]");
				goto end;
			}
		}
	}

	def_qos_flow_indication = cJSON_GetObjectItemCaseSensitive(qos_dataJSON,
	                                                           "defQosFlowIndication");
	if(def_qos_flow_indication) {
		if(!cJSON_IsBool(def_qos_flow_indication)) {
			ogs_error(
				"OpenAPI_qos_data_parseFromJSON() failed [def_qos_flow_indication]");
			goto end;
		}
	}

	ext_max_data_burst_vol = cJSON_GetObjectItemCaseSensitive(qos_dataJSON,
	                                                          "extMaxDataBurstVol");
	if(ext_max_data_burst_vol) {
		if(!cJSON_IsNull(ext_max_data_burst_vol)) {
			if(!cJSON_IsNumber(ext_max_data_burst_vol)) {
				ogs_error(
					"OpenAPI_qos_data_parseFromJSON() failed [ext_max_data_burst_vol]");
				goto end;
			}
		}
	}

	packet_delay_budget = cJSON_GetObjectItemCaseSensitive(qos_dataJSON,
	                                                       "packetDelayBudget");
	if(packet_delay_budget) {
		if(!cJSON_IsNumber(packet_delay_budget)) {
			ogs_error(
				"OpenAPI_qos_data_parseFromJSON() failed [packet_delay_budget]");
			goto end;
		}
	}

	packet_error_rate = cJSON_GetObjectItemCaseSensitive(qos_dataJSON,
	                                                     "packetErrorRate");
	if(packet_error_rate) {
		if(!cJSON_IsString(packet_error_rate) &&
		   !cJSON_IsNull(packet_error_rate))
		{
			ogs_error(
				"OpenAPI_qos_data_parseFromJSON() failed [packet_error_rate]");
			goto end;
		}
	}
	...
	qos_data_local_var = OpenAPI_qos_data_create(
		...
		maxbr_ul &&
		cJSON_IsNull(maxbr_ul) ? true : false,
		maxbr_ul &&
		!cJSON_IsNull(maxbr_ul) ? ogs_strdup(maxbr_ul->valuestring) :
		NULL,
		maxbr_dl &&
		cJSON_IsNull(maxbr_dl) ? true : false,
		maxbr_dl &&
		!cJSON_IsNull(maxbr_dl) ? ogs_strdup(maxbr_dl->valuestring) :
		NULL,
		gbr_ul &&
		cJSON_IsNull(gbr_ul) ? true : false,
		gbr_ul &&
		!cJSON_IsNull(gbr_ul) ? ogs_strdup(gbr_ul->valuestring) : NULL,
		gbr_dl &&
		cJSON_IsNull(gbr_dl) ? true : false,
		gbr_dl &&
		...
		aver_window &&
		cJSON_IsNull(aver_window) ? true : false,
		aver_window ? true : false,
		aver_window ? aver_window->valuedouble : 0,
		max_data_burst_vol &&
		cJSON_IsNull(max_data_burst_vol) ? true : false,
		...
		sharing_key_dl &&
		!cJSON_IsNull(sharing_key_dl) ?
		ogs_strdup(sharing_key_dl->valuestring) : NULL,
		sharing_key_ul &&
		!cJSON_IsNull(sharing_key_ul) ?
		ogs_strdup(sharing_key_ul->valuestring) : NULL,
		...
		cJSON_IsNull(ext_max_data_burst_vol) ? true : false,
		ext_max_data_burst_vol ? true : false,
		ext_max_data_burst_vol ? ext_max_data_burst_vol->valuedouble :
		0,
		packet_delay_budget ? true : false,
		packet_delay_budget ? packet_delay_budget->valuedouble : 0,
		packet_error_rate &&
		!cJSON_IsNull(packet_error_rate) ?
		ogs_strdup(packet_error_rate->valuestring) : NULL
		);
...
OpenAPI_qos_data_t *OpenAPI_qos_data_copy(OpenAPI_qos_data_t	*dst,
                                          OpenAPI_qos_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
