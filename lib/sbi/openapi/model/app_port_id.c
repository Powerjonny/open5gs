...
OpenAPI_app_port_id_t *OpenAPI_app_port_id_create(bool	is_destination_port,
                                                  ...
                                                  int	originator_port) {
	OpenAPI_app_port_id_t *app_port_id_local_var =
		ogs_malloc(sizeof(OpenAPI_app_port_id_t));
...
cJSON *OpenAPI_app_port_id_convertToJSON(OpenAPI_app_port_id_t *app_port_id) {
	...
	if(app_port_id == NULL) {
		ogs_error(
			"OpenAPI_app_port_id_convertToJSON() failed [AppPortId]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(app_port_id->is_destination_port) {
		if(cJSON_AddNumberToObject(item, "destinationPort",
		                           app_port_id->destination_port) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_app_port_id_convertToJSON() failed [destination_port]");
			goto end;
		}
	}

	if(app_port_id->is_originator_port) {
		if(cJSON_AddNumberToObject(item, "originatorPort",
		                           app_port_id->originator_port) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_app_port_id_convertToJSON() failed [originator_port]");
			goto end;
		}
	}
...
OpenAPI_app_port_id_t *OpenAPI_app_port_id_parseFromJSON(
	...
	destination_port = cJSON_GetObjectItemCaseSensitive(app_port_idJSON,
	                                                    "destinationPort");
	if(destination_port) {
		if(!cJSON_IsNumber(destination_port)) {
			ogs_error(
				"OpenAPI_app_port_id_parseFromJSON() failed [destination_port]");
			goto end;
		}
	}

	originator_port = cJSON_GetObjectItemCaseSensitive(app_port_idJSON,
	                                                   "originatorPort");
	if(originator_port) {
		if(!cJSON_IsNumber(originator_port)) {
			ogs_error(
				"OpenAPI_app_port_id_parseFromJSON() failed [originator_port]");
			goto end;
		}
	}
