...
cJSON *OpenAPI_lpi_convertToJSON(OpenAPI_lpi_t *lpi) {
	...
	if(lpi->location_privacy_ind == OpenAPI_location_privacy_ind_NULL) {
		ogs_error(
			"OpenAPI_lpi_convertToJSON() failed [location_privacy_ind]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "locationPrivacyInd",
	                           OpenAPI_location_privacy_ind_ToString(
					   lpi->location_privacy_ind)) == NULL)
	{
		ogs_error(
			"OpenAPI_lpi_convertToJSON() failed [location_privacy_ind]");
		goto end;
	...
	if(lpi->valid_time_period) {
		cJSON *valid_time_period_local_JSON =
			OpenAPI_valid_time_period_convertToJSON(
				lpi->valid_time_period);
		if(valid_time_period_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_lpi_convertToJSON() failed [valid_time_period]");
			goto end;
		}
...
OpenAPI_lpi_t *OpenAPI_lpi_parseFromJSON(cJSON *lpiJSON) {
	...
	location_privacy_ind = cJSON_GetObjectItemCaseSensitive(lpiJSON,
	                                                        "locationPrivacyInd");
	if(!location_privacy_ind) {
		ogs_error(
			"OpenAPI_lpi_parseFromJSON() failed [location_privacy_ind]");
		goto end;
	}
	...
	location_privacy_indVariable =
		OpenAPI_location_privacy_ind_FromString(
			location_privacy_ind->valuestring);

	valid_time_period = cJSON_GetObjectItemCaseSensitive(lpiJSON,
	                                                     "validTimePeriod");
