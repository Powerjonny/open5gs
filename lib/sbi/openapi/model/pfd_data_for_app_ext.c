...
OpenAPI_pfd_data_for_app_ext_t *OpenAPI_pfd_data_for_app_ext_create(
	char *application_id, OpenAPI_list_t *pfds, char *caching_time,
	char *supp_feat, OpenAPI_list_t *reset_ids, bool is_allowed_delay,
	int allowed_delay) {
	OpenAPI_pfd_data_for_app_ext_t *pfd_data_for_app_ext_local_var =
		ogs_malloc(sizeof(OpenAPI_pfd_data_for_app_ext_t));
...
void OpenAPI_pfd_data_for_app_ext_free(
	OpenAPI_pfd_data_for_app_ext_t *pfd_data_for_app_ext) {
...
cJSON *OpenAPI_pfd_data_for_app_ext_convertToJSON(
	OpenAPI_pfd_data_for_app_ext_t *pfd_data_for_app_ext) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(pfd_data_for_app_ext == NULL) {
		ogs_error(
			"OpenAPI_pfd_data_for_app_ext_convertToJSON() failed [PfdDataForAppExt]");
		return NULL;
	}
	...
	if(!pfd_data_for_app_ext->application_id) {
		ogs_error(
			"OpenAPI_pfd_data_for_app_ext_convertToJSON() failed [application_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "applicationId",
	                           pfd_data_for_app_ext->application_id) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_pfd_data_for_app_ext_convertToJSON() failed [application_id]");
	...
	if(!pfd_data_for_app_ext->pfds) {
		ogs_error(
			"OpenAPI_pfd_data_for_app_ext_convertToJSON() failed [pfds]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(pfd_data_for_app_ext->pfds, node) {
		cJSON *itemLocal =
			OpenAPI_pfd_content_convertToJSON(node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_pfd_data_for_app_ext_convertToJSON() failed [pfds]");
			goto end;
	...
	if(pfd_data_for_app_ext->caching_time) {
		if(cJSON_AddStringToObject(item, "cachingTime",
		                           pfd_data_for_app_ext->caching_time)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_pfd_data_for_app_ext_convertToJSON() failed [caching_time]");
			goto end;
		}
	}
	...
	if(pfd_data_for_app_ext->supp_feat) {
		if(cJSON_AddStringToObject(item, "suppFeat",
		                           pfd_data_for_app_ext->supp_feat) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pfd_data_for_app_ext_convertToJSON() failed [supp_feat]");
			goto end;
		}
	}
	...
	if(pfd_data_for_app_ext->reset_ids) {
		...
		if(reset_idsList == NULL) {
			ogs_error(
				"OpenAPI_pfd_data_for_app_ext_convertToJSON() failed [reset_ids]");
			goto end;
		}
		OpenAPI_list_for_each(pfd_data_for_app_ext->reset_ids, node) {
	...
	if(pfd_data_for_app_ext->is_allowed_delay) {
		if(cJSON_AddNumberToObject(item, "allowedDelay",
		                           pfd_data_for_app_ext->allowed_delay)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_pfd_data_for_app_ext_convertToJSON() failed [allowed_delay]");
			goto end;
		}
	}
...
OpenAPI_pfd_data_for_app_ext_t *OpenAPI_pfd_data_for_app_ext_parseFromJSON(
	...
	application_id =
		cJSON_GetObjectItemCaseSensitive(pfd_data_for_app_extJSON,
		                                 "applicationId");
	if(!application_id) {
		ogs_error(
			"OpenAPI_pfd_data_for_app_ext_parseFromJSON() failed [application_id]");
		goto end;
	}
	...
	pfds = cJSON_GetObjectItemCaseSensitive(pfd_data_for_app_extJSON,
	                                        "pfds");
	if(!pfds) {
		ogs_error(
			"OpenAPI_pfd_data_for_app_ext_parseFromJSON() failed [pfds]");
		goto end;
	}
	...
	cJSON_ArrayForEach(pfds_local, pfds) {
		...
		OpenAPI_pfd_content_t *pfdsItem =
			OpenAPI_pfd_content_parseFromJSON(pfds_local);
		if(!pfdsItem) {
	...
	if(caching_time) {
		if(!cJSON_IsString(caching_time) &&
		   !cJSON_IsNull(caching_time))
		{
			ogs_error(
				"OpenAPI_pfd_data_for_app_ext_parseFromJSON() failed [caching_time]");
			goto end;
		}
	}

	supp_feat = cJSON_GetObjectItemCaseSensitive(pfd_data_for_app_extJSON,
	                                             "suppFeat");
	if(supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_pfd_data_for_app_ext_parseFromJSON() failed [supp_feat]");
			goto end;
		}
	}

	reset_ids = cJSON_GetObjectItemCaseSensitive(pfd_data_for_app_extJSON,
	                                             "resetIds");
	if(reset_ids) {
		cJSON *reset_ids_local = NULL;
		if(!cJSON_IsArray(reset_ids)) {
			ogs_error(
				"OpenAPI_pfd_data_for_app_ext_parseFromJSON() failed [reset_ids]");
			goto end;
		}
	...
	if(allowed_delay) {
		if(!cJSON_IsNumber(allowed_delay)) {
			ogs_error(
				"OpenAPI_pfd_data_for_app_ext_parseFromJSON() failed [allowed_delay]");
			goto end;
		}
	}
	...
	pfd_data_for_app_ext_local_var = OpenAPI_pfd_data_for_app_ext_create(
		...
		caching_time &&
		!cJSON_IsNull(caching_time) ?
		ogs_strdup(caching_time->valuestring) : NULL,
		supp_feat &&
...
OpenAPI_pfd_data_for_app_ext_t *OpenAPI_pfd_data_for_app_ext_copy(
	OpenAPI_pfd_data_for_app_ext_t	*dst,
	OpenAPI_pfd_data_for_app_ext_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_pfd_data_for_app_ext_convertToJSON() failed");
		return NULL;
	}
