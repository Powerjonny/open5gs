...
{
	const char *preemption_control_informationArray[] = { "NULL",
		                                              "MOST_RECENT",
		                                              "LEAST_RECENT",
		                                              "HIGHEST_BW" };
	size_t sizeofArray = sizeof(preemption_control_informationArray) /
	                     sizeof(preemption_control_informationArray[0]);
	if(preemption_control_information < sizeofArray) {
		return (char *) preemption_control_informationArray[
...
OpenAPI_preemption_control_information_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(preemption_control_information,
		          preemption_control_informationArray[stringToReturn])
		   == 0) {
