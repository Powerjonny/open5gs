...
OpenAPI_am_policy_data_t *OpenAPI_am_policy_data_create(
	OpenAPI_list_t *pra_infos, OpenAPI_list_t *subsc_cats) {
	OpenAPI_am_policy_data_t *am_policy_data_local_var =
		ogs_malloc(sizeof(OpenAPI_am_policy_data_t));
...
cJSON *OpenAPI_am_policy_data_convertToJSON(
	...
	if(am_policy_data == NULL) {
		ogs_error(
			"OpenAPI_am_policy_data_convertToJSON() failed [AmPolicyData]");
		return NULL;
	}
	...
	if(am_policy_data->pra_infos) {
		...
		if(pra_infos == NULL) {
			ogs_error(
				"OpenAPI_am_policy_data_convertToJSON() failed [pra_infos]");
			goto end;
		}
		...
		if(am_policy_data->pra_infos) {
			OpenAPI_list_for_each(am_policy_data->pra_infos, node) {
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_am_policy_data_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(am_policy_data->subsc_cats) {
		...
		if(subsc_catsList == NULL) {
			ogs_error(
				"OpenAPI_am_policy_data_convertToJSON() failed [subsc_cats]");
			goto end;
		}
		OpenAPI_list_for_each(am_policy_data->subsc_cats, node) {
...
OpenAPI_am_policy_data_t *OpenAPI_am_policy_data_parseFromJSON(
	...
	pra_infos = cJSON_GetObjectItemCaseSensitive(am_policy_dataJSON,
	                                             "praInfos");
	if(pra_infos) {
		...
		{
			ogs_error(
				"OpenAPI_am_policy_data_parseFromJSON() failed [pra_infos]");
			goto end;
		}
		if(cJSON_IsObject(pra_infos)) {
			...
			cJSON_ArrayForEach(pra_infos_local_map, pra_infos) {
				...
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_am_policy_data_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	subsc_cats = cJSON_GetObjectItemCaseSensitive(am_policy_dataJSON,
	                                              "subscCats");
	if(subsc_cats) {
		cJSON *subsc_cats_local = NULL;
		if(!cJSON_IsArray(subsc_cats)) {
			ogs_error(
				"OpenAPI_am_policy_data_parseFromJSON() failed [subsc_cats]");
			goto end;
		}
...
OpenAPI_am_policy_data_t *OpenAPI_am_policy_data_copy(
	OpenAPI_am_policy_data_t *dst, OpenAPI_am_policy_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
