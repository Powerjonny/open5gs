...
OpenAPI_mb_smf_info_t *OpenAPI_mb_smf_info_create(
	...
	OpenAPI_mb_smf_info_t *mb_smf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_mb_smf_info_t));
...
cJSON *OpenAPI_mb_smf_info_convertToJSON(OpenAPI_mb_smf_info_t *mb_smf_info) {
	...
	if(mb_smf_info == NULL) {
		ogs_error(
			"OpenAPI_mb_smf_info_convertToJSON() failed [MbSmfInfo]");
		return NULL;
	}
	...
	if(mb_smf_info->s_nssai_info_list) {
		...
		if(s_nssai_info_list == NULL) {
			ogs_error(
				"OpenAPI_mb_smf_info_convertToJSON() failed [s_nssai_info_list]");
			goto end;
		}
		cJSON *localMapObject = s_nssai_info_list;
		if(mb_smf_info->s_nssai_info_list) {
			OpenAPI_list_for_each(mb_smf_info->s_nssai_info_list,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_mb_smf_info_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(mb_smf_info->tmgi_range_list) {
		...
		if(tmgi_range_list == NULL) {
			ogs_error(
				"OpenAPI_mb_smf_info_convertToJSON() failed [tmgi_range_list]");
			goto end;
		}
		cJSON *localMapObject = tmgi_range_list;
		if(mb_smf_info->tmgi_range_list) {
			OpenAPI_list_for_each(mb_smf_info->tmgi_range_list,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				if(localKeyValue == NULL) {
	...
	if(mb_smf_info->tai_list) {
		...
		if(tai_listList == NULL) {
			ogs_error(
				"OpenAPI_mb_smf_info_convertToJSON() failed [tai_list]");
			goto end;
		}
		OpenAPI_list_for_each(mb_smf_info->tai_list, node) {
			cJSON *itemLocal =
	...
	if(mb_smf_info->tai_range_list) {
		...
		if(tai_range_listList == NULL) {
			ogs_error(
				"OpenAPI_mb_smf_info_convertToJSON() failed [tai_range_list]");
			goto end;
		}
		OpenAPI_list_for_each(mb_smf_info->tai_range_list, node) {
			cJSON *itemLocal =
	...
	if(mb_smf_info->mbs_session_list) {
		...
		if(mbs_session_list == NULL) {
			ogs_error(
				"OpenAPI_mb_smf_info_convertToJSON() failed [mbs_session_list]");
			goto end;
		}
		cJSON *localMapObject = mbs_session_list;
		if(mb_smf_info->mbs_session_list) {
			OpenAPI_list_for_each(mb_smf_info->mbs_session_list,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				if(localKeyValue == NULL) {
...
OpenAPI_mb_smf_info_t *OpenAPI_mb_smf_info_parseFromJSON(
	...
	s_nssai_info_list = cJSON_GetObjectItemCaseSensitive(mb_smf_infoJSON,
	                                                     "sNssaiInfoList");
	if(s_nssai_info_list) {
		...
		{
			ogs_error(
				"OpenAPI_mb_smf_info_parseFromJSON() failed [s_nssai_info_list]");
			goto end;
		}
		if(cJSON_IsObject(s_nssai_info_list)) {
			...
			cJSON_ArrayForEach(s_nssai_info_list_local_map,
				...
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_mb_smf_info_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	tmgi_range_list = cJSON_GetObjectItemCaseSensitive(mb_smf_infoJSON,
	                                                   "tmgiRangeList");
	if(tmgi_range_list) {
		...
		{
			ogs_error(
				"OpenAPI_mb_smf_info_parseFromJSON() failed [tmgi_range_list]");
			goto end;
		}
		if(cJSON_IsObject(tmgi_range_list)) {
	...
	if(tai_list) {
		...
		if(!cJSON_IsArray(tai_list)) {
			ogs_error(
				"OpenAPI_mb_smf_info_parseFromJSON() failed [tai_list]");
			goto end;
		}
	...
	tai_range_list = cJSON_GetObjectItemCaseSensitive(mb_smf_infoJSON,
	                                                  "taiRangeList");
	if(tai_range_list) {
		cJSON *tai_range_list_local = NULL;
		if(!cJSON_IsArray(tai_range_list)) {
			ogs_error(
				"OpenAPI_mb_smf_info_parseFromJSON() failed [tai_range_list]");
			goto end;
		}
	...
	mbs_session_list = cJSON_GetObjectItemCaseSensitive(mb_smf_infoJSON,
	                                                    "mbsSessionList");
	if(mbs_session_list) {
		...
		{
			ogs_error(
				"OpenAPI_mb_smf_info_parseFromJSON() failed [mbs_session_list]");
			goto end;
		}
		if(cJSON_IsObject(mbs_session_list)) {
