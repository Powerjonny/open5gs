...
OpenAPI_policy_data_subset_t *OpenAPI_policy_data_subset_create() {
	OpenAPI_policy_data_subset_t *policy_data_subset_local_var =
		ogs_malloc(sizeof(OpenAPI_policy_data_subset_t));
...
cJSON *OpenAPI_policy_data_subset_convertToJSON(
	...
	if(policy_data_subset == NULL) {
		ogs_error(
			"OpenAPI_policy_data_subset_convertToJSON() failed [PolicyDataSubset]");
		return NULL;
	}
