...
OpenAPI_spatial_validity_cond_1_t *OpenAPI_spatial_validity_cond_1_create(
	...
	OpenAPI_spatial_validity_cond_1_t *spatial_validity_cond_1_local_var =
		ogs_malloc(sizeof(OpenAPI_spatial_validity_cond_1_t));
	ogs_assert(spatial_validity_cond_1_local_var);

	spatial_validity_cond_1_local_var->tracking_area_list =
		tracking_area_list;
	spatial_validity_cond_1_local_var->countries = countries;
	spatial_validity_cond_1_local_var->geographical_service_area =
		geographical_service_area;

	return spatial_validity_cond_1_local_var;
}

void OpenAPI_spatial_validity_cond_1_free(
	OpenAPI_spatial_validity_cond_1_t *spatial_validity_cond_1) {
	...
	if(spatial_validity_cond_1->tracking_area_list) {
		OpenAPI_list_for_each(
			spatial_validity_cond_1->tracking_area_list, node) {
	...
	if(spatial_validity_cond_1->countries) {
		OpenAPI_list_for_each(spatial_validity_cond_1->countries,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(spatial_validity_cond_1->geographical_service_area) {
		OpenAPI_geo_service_area_1_free(
			spatial_validity_cond_1->geographical_service_area);
		spatial_validity_cond_1->geographical_service_area = NULL;
	}
	ogs_free(spatial_validity_cond_1);
}

cJSON *OpenAPI_spatial_validity_cond_1_convertToJSON(
	OpenAPI_spatial_validity_cond_1_t *spatial_validity_cond_1) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(spatial_validity_cond_1 == NULL) {
		ogs_error(
			"OpenAPI_spatial_validity_cond_1_convertToJSON() failed [SpatialValidityCond_1]");
		return NULL;
	}
	...
	if(spatial_validity_cond_1->tracking_area_list) {
		...
		if(tracking_area_listList == NULL) {
			ogs_error(
				"OpenAPI_spatial_validity_cond_1_convertToJSON() failed [tracking_area_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			spatial_validity_cond_1->tracking_area_list, node) {
	...
	if(spatial_validity_cond_1->countries) {
		cJSON *countriesList = cJSON_AddArrayToObject(item,
		                                              "countries");
		if(countriesList == NULL) {
			ogs_error(
				"OpenAPI_spatial_validity_cond_1_convertToJSON() failed [countries]");
			goto end;
		}
		OpenAPI_list_for_each(spatial_validity_cond_1->countries,
		                      node) {
			if(cJSON_AddStringToObject(countriesList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_spatial_validity_cond_1_convertToJSON() failed [countries]");
	...
	if(spatial_validity_cond_1->geographical_service_area) {
		cJSON *geographical_service_area_local_JSON =
			OpenAPI_geo_service_area_1_convertToJSON(
				spatial_validity_cond_1->
				geographical_service_area);
		if(geographical_service_area_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_spatial_validity_cond_1_convertToJSON() failed [geographical_service_area]");
			goto end;
		}
		cJSON_AddItemToObject(item, "geographicalServiceArea",
		                      geographical_service_area_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_spatial_validity_cond_1_convertToJSON() failed [geographical_service_area]");
			goto end;
...
OpenAPI_spatial_validity_cond_1_t *OpenAPI_spatial_validity_cond_1_parseFromJSON
        (cJSON *spatial_validity_cond_1JSON) {
	OpenAPI_spatial_validity_cond_1_t *spatial_validity_cond_1_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	OpenAPI_geo_service_area_1_t *geographical_service_area_local_nonprim =
		NULL;
	tracking_area_list =
		cJSON_GetObjectItemCaseSensitive(spatial_validity_cond_1JSON,
		                                 "trackingAreaList");
	if(tracking_area_list) {
		cJSON *tracking_area_list_local = NULL;
		if(!cJSON_IsArray(tracking_area_list)) {
			ogs_error(
				"OpenAPI_spatial_validity_cond_1_parseFromJSON() failed [tracking_area_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(tracking_area_list_local,
			...
			OpenAPI_tai_1_t *tracking_area_listItem =
	...
	if(countries) {
		...
		if(!cJSON_IsArray(countries)) {
			ogs_error(
				"OpenAPI_spatial_validity_cond_1_parseFromJSON() failed [countries]");
			goto end;
		}
		...
		cJSON_ArrayForEach(countries_local, countries) {
			...
			OpenAPI_list_add(countriesList,
			                 ogs_strdup(
						 countries_local->valuestring));
		}
	}
	...
	if(geographical_service_area) {
		geographical_service_area_local_nonprim =
			OpenAPI_geo_service_area_1_parseFromJSON(
				geographical_service_area);
		if(!geographical_service_area_local_nonprim) {
			ogs_error(
				"OpenAPI_geo_service_area_1_parseFromJSON failed [geographical_service_area]");
			goto end;
		}
	}

	spatial_validity_cond_1_local_var =
...
OpenAPI_spatial_validity_cond_1_t *OpenAPI_spatial_validity_cond_1_copy(
	OpenAPI_spatial_validity_cond_1_t	*dst,
	OpenAPI_spatial_validity_cond_1_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_spatial_validity_cond_1_convertToJSON() failed");
		return NULL;
	}
