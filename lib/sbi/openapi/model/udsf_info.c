...
OpenAPI_udsf_info_t *OpenAPI_udsf_info_create(char		*group_id,
                                              OpenAPI_list_t	*supi_ranges,
                                              OpenAPI_list_t	*
                                              storage_id_ranges) {
	OpenAPI_udsf_info_t *udsf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_udsf_info_t));
...
cJSON *OpenAPI_udsf_info_convertToJSON(OpenAPI_udsf_info_t *udsf_info) {
	...
	if(udsf_info == NULL) {
		ogs_error(
			"OpenAPI_udsf_info_convertToJSON() failed [UdsfInfo]");
		return NULL;
	}
	...
	if(udsf_info->group_id) {
		...
		{
			ogs_error(
				"OpenAPI_udsf_info_convertToJSON() failed [group_id]");
			goto end;
		}
	}
	...
	if(udsf_info->supi_ranges) {
		...
		if(supi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_udsf_info_convertToJSON() failed [supi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(udsf_info->supi_ranges, node) {
			cJSON *itemLocal =
	...
	if(udsf_info->storage_id_ranges) {
		...
		if(storage_id_ranges == NULL) {
			ogs_error(
				"OpenAPI_udsf_info_convertToJSON() failed [storage_id_ranges]");
			goto end;
		}
		cJSON *localMapObject = storage_id_ranges;
		if(udsf_info->storage_id_ranges) {
			OpenAPI_list_for_each(udsf_info->storage_id_ranges,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_udsf_info_convertToJSON() failed [inner]");
					goto end;
				}
...
OpenAPI_udsf_info_t *OpenAPI_udsf_info_parseFromJSON(cJSON *udsf_infoJSON) {
	...
	if(group_id) {
		...
		{
			ogs_error(
				"OpenAPI_udsf_info_parseFromJSON() failed [group_id]");
			goto end;
		}
	}

	supi_ranges = cJSON_GetObjectItemCaseSensitive(udsf_infoJSON,
	                                               "supiRanges");
	if(supi_ranges) {
		cJSON *supi_ranges_local = NULL;
		if(!cJSON_IsArray(supi_ranges)) {
			ogs_error(
				"OpenAPI_udsf_info_parseFromJSON() failed [supi_ranges]");
			goto end;
		}
	...
	storage_id_ranges = cJSON_GetObjectItemCaseSensitive(udsf_infoJSON,
	                                                     "storageIdRanges");
	if(storage_id_ranges) {
		...
		{
			ogs_error(
				"OpenAPI_udsf_info_parseFromJSON() failed [storage_id_ranges]");
			goto end;
		}
		if(cJSON_IsObject(storage_id_ranges)) {
			...
			cJSON_ArrayForEach(storage_id_ranges_local_map,
				...
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_udsf_info_parseFromJSON() failed [inner]");
					goto end;
				}
...
OpenAPI_udsf_info_t *OpenAPI_udsf_info_copy(OpenAPI_udsf_info_t *dst,
                                            OpenAPI_udsf_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
