...
OpenAPI_registration_data_sets_t *OpenAPI_registration_data_sets_create(
	...
	OpenAPI_registration_data_sets_t *registration_data_sets_local_var =
	...
	registration_data_sets_local_var->nwdaf_registration =
		nwdaf_registration;

	return registration_data_sets_local_var;
}

void OpenAPI_registration_data_sets_free(
	OpenAPI_registration_data_sets_t *registration_data_sets) {
...
cJSON *OpenAPI_registration_data_sets_convertToJSON(
	...
	if(registration_data_sets == NULL) {
		ogs_error(
			"OpenAPI_registration_data_sets_convertToJSON() failed [RegistrationDataSets]");
		return NULL;
	}
	...
	if(registration_data_sets->amf3_gpp) {
		...
		if(amf3_gpp_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_registration_data_sets_convertToJSON() failed [amf3_gpp]");
			goto end;
		}
	...
	if(registration_data_sets->amf_non3_gpp) {
		...
		if(amf_non3_gpp_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_registration_data_sets_convertToJSON() failed [amf_non3_gpp]");
			goto end;
		}
	...
	if(registration_data_sets->smf_registration) {
		cJSON *smf_registration_local_JSON =
			OpenAPI_smf_registration_info_convertToJSON(
				registration_data_sets->smf_registration);
		if(smf_registration_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_registration_data_sets_convertToJSON() failed [smf_registration]");
			goto end;
		}
		cJSON_AddItemToObject(item, "smfRegistration",
		                      smf_registration_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_registration_data_sets_convertToJSON() failed [smf_registration]");
			goto end;
	...
	if(registration_data_sets->smsf3_gpp) {
		cJSON *smsf3_gpp_local_JSON =
			OpenAPI_smsf_registration_convertToJSON(
				registration_data_sets->smsf3_gpp);
		if(smsf3_gpp_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_registration_data_sets_convertToJSON() failed [smsf3_gpp]");
			goto end;
		}
	...
	if(registration_data_sets->smsf_non3_gpp) {
		cJSON *smsf_non3_gpp_local_JSON =
			OpenAPI_smsf_registration_convertToJSON(
				registration_data_sets->smsf_non3_gpp);
		if(smsf_non3_gpp_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_registration_data_sets_convertToJSON() failed [smsf_non3_gpp]");
			goto end;
		}
		cJSON_AddItemToObject(item, "smsfNon3Gpp",
		                      smsf_non3_gpp_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_registration_data_sets_convertToJSON() failed [smsf_non3_gpp]");
			goto end;
	...
	if(registration_data_sets->ip_sm_gw) {
		...
		if(ip_sm_gw_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_registration_data_sets_convertToJSON() failed [ip_sm_gw]");
			goto end;
		}
	...
	if(registration_data_sets->nwdaf_registration) {
		cJSON *nwdaf_registration_local_JSON =
			OpenAPI_nwdaf_registration_info_convertToJSON(
				registration_data_sets->nwdaf_registration);
		if(nwdaf_registration_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_registration_data_sets_convertToJSON() failed [nwdaf_registration]");
			goto end;
		}
		cJSON_AddItemToObject(item, "nwdafRegistration",
		                      nwdaf_registration_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_registration_data_sets_convertToJSON() failed [nwdaf_registration]");
			goto end;
...
OpenAPI_registration_data_sets_t *OpenAPI_registration_data_sets_parseFromJSON(
	cJSON *registration_data_setsJSON) {
	OpenAPI_registration_data_sets_t *registration_data_sets_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *amf3_gpp = NULL;
	...
	OpenAPI_nwdaf_registration_info_t *nwdaf_registration_local_nonprim =
		NULL;
	amf3_gpp = cJSON_GetObjectItemCaseSensitive(registration_data_setsJSON,
	                                            "amf3Gpp");
	if(amf3_gpp) {
	...
	amf_non3_gpp =
		cJSON_GetObjectItemCaseSensitive(registration_data_setsJSON,
		                                 "amfNon3Gpp");
	if(amf_non3_gpp) {
	...
	smf_registration =
		cJSON_GetObjectItemCaseSensitive(registration_data_setsJSON,
		                                 "smfRegistration");
	if(smf_registration) {
		smf_registration_local_nonprim =
			OpenAPI_smf_registration_info_parseFromJSON(
				smf_registration);
		if(!smf_registration_local_nonprim) {
			ogs_error(
				"OpenAPI_smf_registration_info_parseFromJSON failed [smf_registration]");
			goto end;
		}
	}

	smsf3_gpp = cJSON_GetObjectItemCaseSensitive(registration_data_setsJSON,
	                                             "smsf3Gpp");
	if(smsf3_gpp) {
		smsf3_gpp_local_nonprim =
			OpenAPI_smsf_registration_parseFromJSON(smsf3_gpp);
		if(!smsf3_gpp_local_nonprim) {
			ogs_error(
				"OpenAPI_smsf_registration_parseFromJSON failed [smsf3_gpp]");
			goto end;
		}
	}
	...
	smsf_non3_gpp =
		cJSON_GetObjectItemCaseSensitive(registration_data_setsJSON,
		                                 "smsfNon3Gpp");
	if(smsf_non3_gpp) {
		smsf_non3_gpp_local_nonprim =
			OpenAPI_smsf_registration_parseFromJSON(smsf_non3_gpp);
		if(!smsf_non3_gpp_local_nonprim) {
			ogs_error(
				"OpenAPI_smsf_registration_parseFromJSON failed [smsf_non3_gpp]");
			goto end;
		}
	}

	ip_sm_gw = cJSON_GetObjectItemCaseSensitive(registration_data_setsJSON,
	                                            "ipSmGw");
	if(ip_sm_gw) {
	...
	if(nwdaf_registration) {
		nwdaf_registration_local_nonprim =
			OpenAPI_nwdaf_registration_info_parseFromJSON(
				nwdaf_registration);
		if(!nwdaf_registration_local_nonprim) {
			ogs_error(
				"OpenAPI_nwdaf_registration_info_parseFromJSON failed [nwdaf_registration]");
			goto end;
		}
	}

	registration_data_sets_local_var =
		OpenAPI_registration_data_sets_create(
			amf3_gpp ? amf3_gpp_local_nonprim : NULL,
			amf_non3_gpp ? amf_non3_gpp_local_nonprim : NULL,
			smf_registration ? smf_registration_local_nonprim :
			NULL,
			smsf3_gpp ? smsf3_gpp_local_nonprim : NULL,
			smsf_non3_gpp ? smsf_non3_gpp_local_nonprim : NULL,
			ip_sm_gw ? ip_sm_gw_local_nonprim : NULL,
			nwdaf_registration ? nwdaf_registration_local_nonprim :
			NULL
			);
...
OpenAPI_registration_data_sets_t *OpenAPI_registration_data_sets_copy(
	OpenAPI_registration_data_sets_t	*dst,
	OpenAPI_registration_data_sets_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_registration_data_sets_convertToJSON() failed");
		return NULL;
	}
