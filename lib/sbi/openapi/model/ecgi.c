...
cJSON *OpenAPI_ecgi_convertToJSON(OpenAPI_ecgi_t *ecgi) {
	...
	cJSON *plmn_id_local_JSON =
		OpenAPI_plmn_id_convertToJSON(ecgi->plmn_id);
	...
	if(!ecgi->eutra_cell_id) {
		ogs_error(
			"OpenAPI_ecgi_convertToJSON() failed [eutra_cell_id]");
		return NULL;
	}
...
OpenAPI_ecgi_t *OpenAPI_ecgi_parseFromJSON(cJSON *ecgiJSON) {
	...
	eutra_cell_id = cJSON_GetObjectItemCaseSensitive(ecgiJSON,
	                                                 "eutraCellId");
	if(!eutra_cell_id) {
		ogs_error(
			"OpenAPI_ecgi_parseFromJSON() failed [eutra_cell_id]");
		goto end;
	}
