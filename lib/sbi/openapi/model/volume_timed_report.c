...
OpenAPI_volume_timed_report_t *OpenAPI_volume_timed_report_create(
	...
	long uplink_volume) {
	OpenAPI_volume_timed_report_t *volume_timed_report_local_var =
		ogs_malloc(sizeof(OpenAPI_volume_timed_report_t));
...
cJSON *OpenAPI_volume_timed_report_convertToJSON(
	...
	if(volume_timed_report == NULL) {
		ogs_error(
			"OpenAPI_volume_timed_report_convertToJSON() failed [VolumeTimedReport]");
		return NULL;
	}
	...
	if(!volume_timed_report->start_time_stamp) {
		ogs_error(
			"OpenAPI_volume_timed_report_convertToJSON() failed [start_time_stamp]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "startTimeStamp",
	                           volume_timed_report->start_time_stamp) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_volume_timed_report_convertToJSON() failed [start_time_stamp]");
	...
	if(!volume_timed_report->end_time_stamp) {
		ogs_error(
			"OpenAPI_volume_timed_report_convertToJSON() failed [end_time_stamp]");
		return NULL;
	}
	...
	if(cJSON_AddNumberToObject(item, "downlinkVolume",
	                           volume_timed_report->downlink_volume) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_volume_timed_report_convertToJSON() failed [downlink_volume]");
		goto end;
	}

	if(cJSON_AddNumberToObject(item, "uplinkVolume",
	                           volume_timed_report->uplink_volume) == NULL)
	{
		ogs_error(
			"OpenAPI_volume_timed_report_convertToJSON() failed [uplink_volume]");
		goto end;
	}
...
OpenAPI_volume_timed_report_t *OpenAPI_volume_timed_report_parseFromJSON(
	...
	start_time_stamp =
		cJSON_GetObjectItemCaseSensitive(volume_timed_reportJSON,
		                                 "startTimeStamp");
	if(!start_time_stamp) {
		ogs_error(
			"OpenAPI_volume_timed_report_parseFromJSON() failed [start_time_stamp]");
		goto end;
	}
	...
	if(!end_time_stamp) {
		ogs_error(
			"OpenAPI_volume_timed_report_parseFromJSON() failed [end_time_stamp]");
		goto end;
	}
	...
	if(!downlink_volume) {
		ogs_error(
			"OpenAPI_volume_timed_report_parseFromJSON() failed [downlink_volume]");
		goto end;
	}
	...
	if(!uplink_volume) {
		ogs_error(
			"OpenAPI_volume_timed_report_parseFromJSON() failed [uplink_volume]");
		goto end;
	}
...
OpenAPI_volume_timed_report_t *OpenAPI_volume_timed_report_copy(
	OpenAPI_volume_timed_report_t	*dst,
	OpenAPI_volume_timed_report_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
