...
OpenAPI_identity_range_t *OpenAPI_identity_range_create(char *start, char *end,
                                                        char *pattern) {
	OpenAPI_identity_range_t *identity_range_local_var =
		ogs_malloc(sizeof(OpenAPI_identity_range_t));
...
cJSON *OpenAPI_identity_range_convertToJSON(
	OpenAPI_identity_range_t *identity_range) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(identity_range == NULL) {
		ogs_error(
			"OpenAPI_identity_range_convertToJSON() failed [IdentityRange]");
		return NULL;
	}
	...
	if(identity_range->start) {
		...
		{
			ogs_error(
				"OpenAPI_identity_range_convertToJSON() failed [start]");
			goto end;
		}
	}
	...
	if(identity_range->end) {
		...
		{
			ogs_error(
				"OpenAPI_identity_range_convertToJSON() failed [end]");
			goto end;
		}
	}
	...
	if(identity_range->pattern) {
		...
		{
			ogs_error(
				"OpenAPI_identity_range_convertToJSON() failed [pattern]");
			goto end;
		}
	}
...
OpenAPI_identity_range_t *OpenAPI_identity_range_parseFromJSON(
	cJSON *identity_rangeJSON) {
	...
	if(start) {
		...
		{
			ogs_error(
				"OpenAPI_identity_range_parseFromJSON() failed [start]");
			goto end;
		}
	}
	...
	if(end) {
		...
		{
			ogs_error(
				"OpenAPI_identity_range_parseFromJSON() failed [end]");
			goto end;
		}
	}

	pattern = cJSON_GetObjectItemCaseSensitive(identity_rangeJSON,
	                                           "pattern");
	if(pattern) {
		...
		{
			ogs_error(
				"OpenAPI_identity_range_parseFromJSON() failed [pattern]");
			goto end;
		}
	}
	...
	identity_range_local_var = OpenAPI_identity_range_create(
		...
		!cJSON_IsNull(end) ? ogs_strdup(end->valuestring) : NULL,
		pattern &&
		!cJSON_IsNull(pattern) ? ogs_strdup(pattern->valuestring) : NULL
		);
...
OpenAPI_identity_range_t *OpenAPI_identity_range_copy(
	OpenAPI_identity_range_t *dst, OpenAPI_identity_range_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
