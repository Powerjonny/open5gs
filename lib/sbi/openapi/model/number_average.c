...
OpenAPI_number_average_t *OpenAPI_number_average_create(float	number,
                                                        ...
                                                        bool	is_skewness,
                                                        float	skewness) {
	OpenAPI_number_average_t *number_average_local_var =
		ogs_malloc(sizeof(OpenAPI_number_average_t));
...
cJSON *OpenAPI_number_average_convertToJSON(
	...
	if(number_average == NULL) {
		ogs_error(
			"OpenAPI_number_average_convertToJSON() failed [NumberAverage]");
		return NULL;
	}
	...
	{
		ogs_error(
			"OpenAPI_number_average_convertToJSON() failed [number]");
		goto end;
	}
	...
	{
		ogs_error(
			"OpenAPI_number_average_convertToJSON() failed [variance]");
		goto end;
	}
	...
	if(number_average->is_skewness) {
		...
		{
			ogs_error(
				"OpenAPI_number_average_convertToJSON() failed [skewness]");
			goto end;
		}
	}
...
OpenAPI_number_average_t *OpenAPI_number_average_parseFromJSON(
	...
	if(!number) {
		ogs_error(
			"OpenAPI_number_average_parseFromJSON() failed [number]");
		goto end;
	}
	...
	if(!variance) {
		ogs_error(
			"OpenAPI_number_average_parseFromJSON() failed [variance]");
		goto end;
	}
	...
	if(skewness) {
		if(!cJSON_IsNumber(skewness)) {
			ogs_error(
				"OpenAPI_number_average_parseFromJSON() failed [skewness]");
			goto end;
		}
	}
