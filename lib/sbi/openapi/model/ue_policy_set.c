...
OpenAPI_ue_policy_set_t *OpenAPI_ue_policy_set_create(
	...
	OpenAPI_list_t *
	allowed_route_sel_descs,
	bool is_andsp_ind,
	int andsp_ind, char *pei, OpenAPI_list_t *os_ids, char *supp_feat,
	OpenAPI_list_t *reset_ids) {
	OpenAPI_ue_policy_set_t *ue_policy_set_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_policy_set_t));
	...
	ue_policy_set_local_var->allowed_route_sel_descs =
		allowed_route_sel_descs;
...
void OpenAPI_ue_policy_set_free(OpenAPI_ue_policy_set_t *ue_policy_set) {
	...
	if(ue_policy_set->allowed_route_sel_descs) {
		OpenAPI_list_for_each(ue_policy_set->allowed_route_sel_descs,
		                      node) {
			OpenAPI_map_t *localKeyValue =
				(OpenAPI_map_t *) node->data;
			ogs_free(localKeyValue->key);
			OpenAPI_plmn_route_selection_descriptor_free(
				localKeyValue->value);
			OpenAPI_map_free(localKeyValue);
		}
...
cJSON *OpenAPI_ue_policy_set_convertToJSON(
	...
	if(ue_policy_set == NULL) {
		ogs_error(
			"OpenAPI_ue_policy_set_convertToJSON() failed [UePolicySet]");
		return NULL;
	}
	...
	if(ue_policy_set->pra_infos) {
		...
		if(pra_infos == NULL) {
			ogs_error(
				"OpenAPI_ue_policy_set_convertToJSON() failed [pra_infos]");
			goto end;
		}
		...
		if(ue_policy_set->pra_infos) {
			OpenAPI_list_for_each(ue_policy_set->pra_infos, node) {
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_ue_policy_set_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(ue_policy_set->subsc_cats) {
		...
		if(subsc_catsList == NULL) {
			ogs_error(
				"OpenAPI_ue_policy_set_convertToJSON() failed [subsc_cats]");
			goto end;
		}
		OpenAPI_list_for_each(ue_policy_set->subsc_cats, node) {
	...
	if(ue_policy_set->ue_policy_sections) {
		cJSON *ue_policy_sections = cJSON_AddObjectToObject(item,
		                                                    "uePolicySections");
		if(ue_policy_sections == NULL) {
			ogs_error(
				"OpenAPI_ue_policy_set_convertToJSON() failed [ue_policy_sections]");
			goto end;
		}
		cJSON *localMapObject = ue_policy_sections;
		if(ue_policy_set->ue_policy_sections) {
			OpenAPI_list_for_each(ue_policy_set->ue_policy_sections,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				if(localKeyValue == NULL) {
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_ue_policy_section_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
	...
	if(ue_policy_set->upsis) {
		...
		if(upsisList == NULL) {
			ogs_error(
				"OpenAPI_ue_policy_set_convertToJSON() failed [upsis]");
			goto end;
		}
		OpenAPI_list_for_each(ue_policy_set->upsis, node) {
			if(cJSON_AddStringToObject(upsisList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_ue_policy_set_convertToJSON() failed [upsis]");
	...
	if(ue_policy_set->allowed_route_sel_descs) {
		cJSON *allowed_route_sel_descs = cJSON_AddObjectToObject(item,
		                                                         "allowedRouteSelDescs");
		if(allowed_route_sel_descs == NULL) {
			ogs_error(
				"OpenAPI_ue_policy_set_convertToJSON() failed [allowed_route_sel_descs]");
			goto end;
		}
		cJSON *localMapObject = allowed_route_sel_descs;
		if(ue_policy_set->allowed_route_sel_descs) {
			OpenAPI_list_for_each(
				ue_policy_set->allowed_route_sel_descs, node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				if(localKeyValue == NULL) {
					ogs_error(
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_plmn_route_selection_descriptor_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
	...
	if(ue_policy_set->is_andsp_ind) {
		if(cJSON_AddBoolToObject(item, "andspInd",
		                         ue_policy_set->andsp_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_ue_policy_set_convertToJSON() failed [andsp_ind]");
			goto end;
		}
	}
	...
	if(ue_policy_set->pei) {
		...
		{
			ogs_error(
				"OpenAPI_ue_policy_set_convertToJSON() failed [pei]");
			goto end;
		}
	}
	...
	if(ue_policy_set->os_ids) {
		...
		if(os_idsList == NULL) {
			ogs_error(
				"OpenAPI_ue_policy_set_convertToJSON() failed [os_ids]");
			goto end;
		}
		OpenAPI_list_for_each(ue_policy_set->os_ids, node) {
			if(cJSON_AddStringToObject(os_idsList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_ue_policy_set_convertToJSON() failed [os_ids]");
	...
	if(ue_policy_set->supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_ue_policy_set_convertToJSON() failed [supp_feat]");
			goto end;
		}
	}
	...
	if(ue_policy_set->reset_ids) {
		...
		if(reset_idsList == NULL) {
			ogs_error(
				"OpenAPI_ue_policy_set_convertToJSON() failed [reset_ids]");
			goto end;
		}
		OpenAPI_list_for_each(ue_policy_set->reset_ids, node) {
...
OpenAPI_ue_policy_set_t *OpenAPI_ue_policy_set_parseFromJSON(
	cJSON *ue_policy_setJSON) {
	...
	pra_infos = cJSON_GetObjectItemCaseSensitive(ue_policy_setJSON,
	                                             "praInfos");
	if(pra_infos) {
		...
		{
			ogs_error(
				"OpenAPI_ue_policy_set_parseFromJSON() failed [pra_infos]");
			goto end;
		}
		if(cJSON_IsObject(pra_infos)) {
			...
			cJSON_ArrayForEach(pra_infos_local_map, pra_infos) {
				...
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_ue_policy_set_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	subsc_cats = cJSON_GetObjectItemCaseSensitive(ue_policy_setJSON,
	                                              "subscCats");
	if(subsc_cats) {
		cJSON *subsc_cats_local = NULL;
		if(!cJSON_IsArray(subsc_cats)) {
			ogs_error(
				"OpenAPI_ue_policy_set_parseFromJSON() failed [subsc_cats]");
			goto end;
		}
	...
	ue_policy_sections = cJSON_GetObjectItemCaseSensitive(ue_policy_setJSON,
	                                                      "uePolicySections");
	if(ue_policy_sections) {
		cJSON *ue_policy_sections_local_map = NULL;
		if(!cJSON_IsObject(ue_policy_sections) &&
		   !cJSON_IsNull(ue_policy_sections))
		{
			ogs_error(
				"OpenAPI_ue_policy_set_parseFromJSON() failed [ue_policy_sections]");
			goto end;
		}
		if(cJSON_IsObject(ue_policy_sections)) {
			ue_policy_sectionsList = OpenAPI_list_create();
			OpenAPI_map_t *localMapKeyPair = NULL;
			cJSON_ArrayForEach(ue_policy_sections_local_map,
			                   ue_policy_sections) {
				cJSON *localMapObject =
					ue_policy_sections_local_map;
				if(cJSON_IsObject(localMapObject)) {
					localMapKeyPair = OpenAPI_map_create(
						ogs_strdup(
							localMapObject->string),
						OpenAPI_ue_policy_section_parseFromJSON
						        (localMapObject));
				} else if(cJSON_IsNull(localMapObject)) {
					localMapKeyPair =
						OpenAPI_map_create(ogs_strdup(
	...
	if(upsis) {
		...
		if(!cJSON_IsArray(upsis)) {
			ogs_error(
				"OpenAPI_ue_policy_set_parseFromJSON() failed [upsis]");
			goto end;
		}
		...
		cJSON_ArrayForEach(upsis_local, upsis) {
			...
			OpenAPI_list_add(upsisList,
			                 ogs_strdup(upsis_local->valuestring));
		}
	}

	allowed_route_sel_descs =
		cJSON_GetObjectItemCaseSensitive(ue_policy_setJSON,
		                                 "allowedRouteSelDescs");
	if(allowed_route_sel_descs) {
		cJSON *allowed_route_sel_descs_local_map = NULL;
		if(!cJSON_IsObject(allowed_route_sel_descs) &&
		   !cJSON_IsNull(allowed_route_sel_descs))
		{
			ogs_error(
		...
		if(cJSON_IsObject(allowed_route_sel_descs)) {
			...
			cJSON_ArrayForEach(allowed_route_sel_descs_local_map,
			                   allowed_route_sel_descs) {
				cJSON *localMapObject =
					allowed_route_sel_descs_local_map;
				if(cJSON_IsObject(localMapObject)) {
					localMapKeyPair = OpenAPI_map_create(
						ogs_strdup(
							localMapObject->string),
				...
				OpenAPI_list_add(allowed_route_sel_descsList,
				                 localMapKeyPair);
			}
		}
	}

	andsp_ind = cJSON_GetObjectItemCaseSensitive(ue_policy_setJSON,
	                                             "andspInd");
	if(andsp_ind) {
		if(!cJSON_IsBool(andsp_ind)) {
			ogs_error(
				"OpenAPI_ue_policy_set_parseFromJSON() failed [andsp_ind]");
			goto end;
		}
	}
	...
	if(pei) {
		...
		{
			ogs_error(
				"OpenAPI_ue_policy_set_parseFromJSON() failed [pei]");
			goto end;
		}
	}
	...
	if(os_ids) {
		...
		if(!cJSON_IsArray(os_ids)) {
			ogs_error(
				"OpenAPI_ue_policy_set_parseFromJSON() failed [os_ids]");
			goto end;
		}
		...
		cJSON_ArrayForEach(os_ids_local, os_ids) {
			...
			OpenAPI_list_add(os_idsList,
			                 ogs_strdup(os_ids_local->valuestring));
		}
	}

	supp_feat = cJSON_GetObjectItemCaseSensitive(ue_policy_setJSON,
	                                             "suppFeat");
	if(supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_ue_policy_set_parseFromJSON() failed [supp_feat]");
			goto end;
		}
	}

	reset_ids = cJSON_GetObjectItemCaseSensitive(ue_policy_setJSON,
	                                             "resetIds");
	if(reset_ids) {
		cJSON *reset_ids_local = NULL;
		if(!cJSON_IsArray(reset_ids)) {
			ogs_error(
				"OpenAPI_ue_policy_set_parseFromJSON() failed [reset_ids]");
			goto end;
		}
...
OpenAPI_ue_policy_set_t *OpenAPI_ue_policy_set_copy(
	OpenAPI_ue_policy_set_t *dst, OpenAPI_ue_policy_set_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
