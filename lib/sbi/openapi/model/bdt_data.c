...
OpenAPI_bdt_data_t *OpenAPI_bdt_data_create(char *asp_id,
                                            OpenAPI_transfer_policy_t *
                                            trans_policy, char *bdt_ref_id,
                                            OpenAPI_network_area_info_2_t *
                                            nw_area_info, bool is_num_of_ues,
                                            int num_of_ues,
                                            ...
                                            bdtp_status, char *supp_feat,
                                            OpenAPI_list_t *reset_ids) {
	OpenAPI_bdt_data_t *bdt_data_local_var =
		ogs_malloc(sizeof(OpenAPI_bdt_data_t));
...
cJSON *OpenAPI_bdt_data_convertToJSON(OpenAPI_bdt_data_t *bdt_data) {
	...
	if(!bdt_data->trans_policy) {
		ogs_error(
			"OpenAPI_bdt_data_convertToJSON() failed [trans_policy]");
		return NULL;
	}
	cJSON *trans_policy_local_JSON =
		OpenAPI_transfer_policy_convertToJSON(bdt_data->trans_policy);
	...
	if(bdt_data->bdt_ref_id) {
		...
		{
			ogs_error(
				"OpenAPI_bdt_data_convertToJSON() failed [bdt_ref_id]");
			goto end;
		}
	}
	...
	if(bdt_data->nw_area_info) {
		cJSON *nw_area_info_local_JSON =
			OpenAPI_network_area_info_2_convertToJSON(
				bdt_data->nw_area_info);
		if(nw_area_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_bdt_data_convertToJSON() failed [nw_area_info]");
			goto end;
		}
	...
	if(bdt_data->is_num_of_ues) {
		if(cJSON_AddNumberToObject(item, "numOfUes",
		                           bdt_data->num_of_ues) == NULL)
		{
			ogs_error(
				"OpenAPI_bdt_data_convertToJSON() failed [num_of_ues]");
			goto end;
		}
	}

	if(bdt_data->vol_per_ue) {
		cJSON *vol_per_ue_local_JSON =
			OpenAPI_usage_threshold_convertToJSON(
				bdt_data->vol_per_ue);
		if(vol_per_ue_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_bdt_data_convertToJSON() failed [vol_per_ue]");
			goto end;
		}
	...
	if(bdt_data->dnn) {
		...
		{
			ogs_error(
				"OpenAPI_bdt_data_convertToJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(bdt_data->snssai) {
		cJSON *snssai_local_JSON =
			OpenAPI_snssai_convertToJSON(bdt_data->snssai);
		if(snssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_bdt_data_convertToJSON() failed [snssai]");
			goto end;
		}
	...
	if(bdt_data->traffic_des) {
		if(cJSON_AddStringToObject(item, "trafficDes",
		                           bdt_data->traffic_des) == NULL)
		{
			ogs_error(
				"OpenAPI_bdt_data_convertToJSON() failed [traffic_des]");
			goto end;
		}
	}

	if(bdt_data->bdtp_status) {
		cJSON *bdtp_status_local_JSON =
			OpenAPI_bdt_policy_status_convertToJSON(
				bdt_data->bdtp_status);
		if(bdtp_status_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_bdt_data_convertToJSON() failed [bdtp_status]");
			goto end;
		}
		cJSON_AddItemToObject(item, "bdtpStatus",
		                      bdtp_status_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_bdt_data_convertToJSON() failed [bdtp_status]");
			goto end;
	...
	if(bdt_data->supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_bdt_data_convertToJSON() failed [supp_feat]");
			goto end;
		}
	}
	...
	if(bdt_data->reset_ids) {
		...
		if(reset_idsList == NULL) {
			ogs_error(
				"OpenAPI_bdt_data_convertToJSON() failed [reset_ids]");
			goto end;
		}
		OpenAPI_list_for_each(bdt_data->reset_ids, node) {
...
OpenAPI_bdt_data_t *OpenAPI_bdt_data_parseFromJSON(cJSON *bdt_dataJSON) {
	...
	trans_policy = cJSON_GetObjectItemCaseSensitive(bdt_dataJSON,
	                                                "transPolicy");
	if(!trans_policy) {
		ogs_error(
			"OpenAPI_bdt_data_parseFromJSON() failed [trans_policy]");
		goto end;
	}
	trans_policy_local_nonprim =
		OpenAPI_transfer_policy_parseFromJSON(trans_policy);
	if(!trans_policy_local_nonprim) {
		ogs_error(
			"OpenAPI_transfer_policy_parseFromJSON failed [trans_policy]");
		goto end;
	}
	...
	if(bdt_ref_id) {
		...
		{
			ogs_error(
				"OpenAPI_bdt_data_parseFromJSON() failed [bdt_ref_id]");
			goto end;
		}
	}

	nw_area_info = cJSON_GetObjectItemCaseSensitive(bdt_dataJSON,
	                                                "nwAreaInfo");
	if(nw_area_info) {
	...
	if(num_of_ues) {
		if(!cJSON_IsNumber(num_of_ues)) {
			ogs_error(
				"OpenAPI_bdt_data_parseFromJSON() failed [num_of_ues]");
			goto end;
		}
	}
	...
	if(vol_per_ue) {
		vol_per_ue_local_nonprim =
			OpenAPI_usage_threshold_parseFromJSON(vol_per_ue);
		if(!vol_per_ue_local_nonprim) {
			ogs_error(
				"OpenAPI_usage_threshold_parseFromJSON failed [vol_per_ue]");
			goto end;
		}
	}
	...
	if(dnn) {
		...
		{
			ogs_error(
				"OpenAPI_bdt_data_parseFromJSON() failed [dnn]");
			goto end;
		}
	}
	...
	traffic_des = cJSON_GetObjectItemCaseSensitive(bdt_dataJSON,
	                                               "trafficDes");
	if(traffic_des) {
		if(!cJSON_IsString(traffic_des) &&
		   !cJSON_IsNull(traffic_des))
		{
			ogs_error(
				"OpenAPI_bdt_data_parseFromJSON() failed [traffic_des]");
			goto end;
		}
	}

	bdtp_status = cJSON_GetObjectItemCaseSensitive(bdt_dataJSON,
	                                               "bdtpStatus");
	if(bdtp_status) {
		bdtp_status_local_nonprim =
			OpenAPI_bdt_policy_status_parseFromJSON(bdtp_status);
		if(!bdtp_status_local_nonprim) {
			ogs_error(
				"OpenAPI_bdt_policy_status_parseFromJSON failed [bdtp_status]");
			goto end;
		}
	}
	...
	if(supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_bdt_data_parseFromJSON() failed [supp_feat]");
			goto end;
		}
	}
	...
	if(reset_ids) {
		...
		if(!cJSON_IsArray(reset_ids)) {
			ogs_error(
				"OpenAPI_bdt_data_parseFromJSON() failed [reset_ids]");
			goto end;
		}
	...
	bdt_data_local_var = OpenAPI_bdt_data_create(
		...
		traffic_des &&
		!cJSON_IsNull(traffic_des) ?
		ogs_strdup(traffic_des->valuestring) : NULL,
...
OpenAPI_bdt_data_t *OpenAPI_bdt_data_copy(OpenAPI_bdt_data_t	*dst,
                                          OpenAPI_bdt_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
