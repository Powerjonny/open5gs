...
OpenAPI_ue_context_in_smf_data_sub_filter_create(OpenAPI_list_t *dnn_list,
                                                 OpenAPI_list_t *snssai_list,
                                                 bool		is_emergency_ind,
                                                 int		emergency_ind) {
	OpenAPI_ue_context_in_smf_data_sub_filter_t *
	        ue_context_in_smf_data_sub_filter_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_context_in_smf_data_sub_filter_t));
	...
	ue_context_in_smf_data_sub_filter_local_var->is_emergency_ind =
		is_emergency_ind;
	ue_context_in_smf_data_sub_filter_local_var->emergency_ind =
		emergency_ind;

	return ue_context_in_smf_data_sub_filter_local_var;
}

void OpenAPI_ue_context_in_smf_data_sub_filter_free(
	OpenAPI_ue_context_in_smf_data_sub_filter_t *
	ue_context_in_smf_data_sub_filter) {
	...
	if(ue_context_in_smf_data_sub_filter->dnn_list) {
		OpenAPI_list_for_each(
			ue_context_in_smf_data_sub_filter->dnn_list, node) {
			ogs_free(node->data);
		}
	...
	if(ue_context_in_smf_data_sub_filter->snssai_list) {
		OpenAPI_list_for_each(
			ue_context_in_smf_data_sub_filter->snssai_list, node) {
...
cJSON *OpenAPI_ue_context_in_smf_data_sub_filter_convertToJSON(
	OpenAPI_ue_context_in_smf_data_sub_filter_t *
	ue_context_in_smf_data_sub_filter) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(ue_context_in_smf_data_sub_filter == NULL) {
		ogs_error(
			"OpenAPI_ue_context_in_smf_data_sub_filter_convertToJSON() failed [UeContextInSmfDataSubFilter]");
		return NULL;
	}
	...
	if(ue_context_in_smf_data_sub_filter->dnn_list) {
		...
		if(dnn_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_in_smf_data_sub_filter_convertToJSON() failed [dnn_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			ue_context_in_smf_data_sub_filter->dnn_list, node) {
	...
	if(ue_context_in_smf_data_sub_filter->snssai_list) {
		...
		if(snssai_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_in_smf_data_sub_filter_convertToJSON() failed [snssai_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			ue_context_in_smf_data_sub_filter->snssai_list, node) {
	...
	if(ue_context_in_smf_data_sub_filter->is_emergency_ind) {
		if(cJSON_AddBoolToObject(item, "emergencyInd",
		                         ue_context_in_smf_data_sub_filter->
		                         emergency_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_ue_context_in_smf_data_sub_filter_convertToJSON() failed [emergency_ind]");
			goto end;
		}
	}
...
OpenAPI_ue_context_in_smf_data_sub_filter_parseFromJSON(
	cJSON *ue_context_in_smf_data_sub_filterJSON) {
	OpenAPI_ue_context_in_smf_data_sub_filter_t *
	        ue_context_in_smf_data_sub_filter_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *dnn_list = NULL;
	...
	dnn_list =
		cJSON_GetObjectItemCaseSensitive(
			ue_context_in_smf_data_sub_filterJSON, "dnnList");
	if(dnn_list) {
		cJSON *dnn_list_local = NULL;
		if(!cJSON_IsArray(dnn_list)) {
			ogs_error(
				"OpenAPI_ue_context_in_smf_data_sub_filter_parseFromJSON() failed [dnn_list]");
			goto end;
		}
	...
	snssai_list =
		cJSON_GetObjectItemCaseSensitive(
			ue_context_in_smf_data_sub_filterJSON, "snssaiList");
	if(snssai_list) {
		cJSON *snssai_list_local = NULL;
		if(!cJSON_IsArray(snssai_list)) {
			ogs_error(
				"OpenAPI_ue_context_in_smf_data_sub_filter_parseFromJSON() failed [snssai_list]");
			goto end;
		}
	...
	emergency_ind =
		cJSON_GetObjectItemCaseSensitive(
			ue_context_in_smf_data_sub_filterJSON, "emergencyInd");
	if(emergency_ind) {
		if(!cJSON_IsBool(emergency_ind)) {
			ogs_error(
				"OpenAPI_ue_context_in_smf_data_sub_filter_parseFromJSON() failed [emergency_ind]");
			goto end;
		}
	}

	ue_context_in_smf_data_sub_filter_local_var =
		OpenAPI_ue_context_in_smf_data_sub_filter_create(
...
OpenAPI_ue_context_in_smf_data_sub_filter_t *
OpenAPI_ue_context_in_smf_data_sub_filter_copy(
	OpenAPI_ue_context_in_smf_data_sub_filter_t	*dst,
	OpenAPI_ue_context_in_smf_data_sub_filter_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ue_context_in_smf_data_sub_filter_convertToJSON() failed");
		return NULL;
	}
