...
OpenAPI_sm_context_t *OpenAPI_sm_context_create(int pdu_session_id, char *dnn,
                                                char *selected_dnn,
                                                OpenAPI_snssai_t *s_nssai,
                                                OpenAPI_snssai_t *hplmn_snssai,
                                                OpenAPI_pdu_session_type_e
                                                pdu_session_type, char *gpsi,
                                                char *h_smf_uri, char *smf_uri,
                                                ...
                                                int anchor_smf_oauth2_required)
{
	OpenAPI_sm_context_t *sm_context_local_var =
		ogs_malloc(sizeof(OpenAPI_sm_context_t));
	...
	sm_context_local_var->pdu_session_smf_service_set_id =
		pdu_session_smf_service_set_id;
	sm_context_local_var->pdu_session_smf_binding = pdu_session_smf_binding;
	sm_context_local_var->is_enable_pause_charging =
		is_enable_pause_charging;
	...
	sm_context_local_var->max_integrity_protected_data_rate =
		max_integrity_protected_data_rate;
	sm_context_local_var->max_integrity_protected_data_rate_dl =
		max_integrity_protected_data_rate_dl;
	...
	sm_context_local_var->h_smf_service_instance_id =
	...
	sm_context_local_var->roaming_charging_profile =
		roaming_charging_profile;
	sm_context_local_var->is_nef_ext_buf_support_ind =
		is_nef_ext_buf_support_ind;
	...
	sm_context_local_var->redundant_pdu_session_info =
		redundant_pdu_session_info;
	...
	sm_context_local_var->is_anchor_smf_oauth2_required =
		is_anchor_smf_oauth2_required;
	sm_context_local_var->anchor_smf_oauth2_required =
...
void OpenAPI_sm_context_free(OpenAPI_sm_context_t *sm_context) {
	...
	if(sm_context->roaming_charging_profile) {
		OpenAPI_roaming_charging_profile_free(
			sm_context->roaming_charging_profile);
	...
	if(sm_context->add_red_ran_tunnel_info) {
		OpenAPI_list_for_each(sm_context->add_red_ran_tunnel_info,
		                      node) {
			OpenAPI_qos_flow_tunnel_free(node->data);
		}
...
cJSON *OpenAPI_sm_context_convertToJSON(OpenAPI_sm_context_t *sm_context) {
	...
	if(sm_context == NULL) {
		ogs_error(
			"OpenAPI_sm_context_convertToJSON() failed [SmContext]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(cJSON_AddNumberToObject(item, "pduSessionId",
	                           sm_context->pdu_session_id) == NULL)
	{
		ogs_error(
			"OpenAPI_sm_context_convertToJSON() failed [pdu_session_id]");
		goto end;
	}
	...
	if(sm_context->selected_dnn) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [selected_dnn]");
			goto end;
		}
	}
	...
	if(!sm_context->s_nssai) {
		ogs_error(
			"OpenAPI_sm_context_convertToJSON() failed [s_nssai]");
		return NULL;
	}
	cJSON *s_nssai_local_JSON =
		OpenAPI_snssai_convertToJSON(sm_context->s_nssai);
	...
	if(sm_context->hplmn_snssai) {
		cJSON *hplmn_snssai_local_JSON =
			OpenAPI_snssai_convertToJSON(sm_context->hplmn_snssai);
		if(hplmn_snssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [hplmn_snssai]");
			goto end;
		}
	...
	if(sm_context->pdu_session_type == OpenAPI_pdu_session_type_NULL) {
		ogs_error(
			"OpenAPI_sm_context_convertToJSON() failed [pdu_session_type]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "pduSessionType",
	...
	if(sm_context->gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [gpsi]");
			goto end;
		}
	}
	...
	if(sm_context->h_smf_uri) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [h_smf_uri]");
			goto end;
		}
	}
	...
	if(sm_context->smf_uri) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [smf_uri]");
			goto end;
		}
	}

	if(sm_context->pdu_session_ref) {
		if(cJSON_AddStringToObject(item, "pduSessionRef",
		                           sm_context->pdu_session_ref) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [pdu_session_ref]");
			goto end;
		}
	}
	...
	if(sm_context->inter_plmn_api_root) {
		if(cJSON_AddStringToObject(item, "interPlmnApiRoot",
		                           sm_context->inter_plmn_api_root) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [inter_plmn_api_root]");
			goto end;
		}
	}

	if(sm_context->intra_plmn_api_root) {
		if(cJSON_AddStringToObject(item, "intraPlmnApiRoot",
		                           sm_context->intra_plmn_api_root) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [intra_plmn_api_root]");
			goto end;
		}
	}
	...
	if(sm_context->pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [pcf_id]");
			goto end;
		}
	}
	...
	if(sm_context->pcf_group_id) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [pcf_group_id]");
			goto end;
		}
	}
	...
	if(sm_context->pcf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [pcf_set_id]");
			goto end;
		}
	}
	...
	if(sm_context->sel_mode != OpenAPI_dnn_selection_mode_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [sel_mode]");
			goto end;
		}
	}
	...
	if(sm_context->udm_group_id) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [udm_group_id]");
			goto end;
		}
	}
	...
	if(sm_context->routing_indicator) {
		if(cJSON_AddStringToObject(item, "routingIndicator",
		                           sm_context->routing_indicator) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [routing_indicator]");
			goto end;
		}
	}
	...
	if(sm_context->is_h_nw_pub_key_id) {
		if(cJSON_AddNumberToObject(item, "hNwPubKeyId",
		                           sm_context->h_nw_pub_key_id) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [h_nw_pub_key_id]");
			goto end;
		}
	}
	...
	if(!sm_context->session_ambr) {
		ogs_error(
			"OpenAPI_sm_context_convertToJSON() failed [session_ambr]");
		return NULL;
	}
	cJSON *session_ambr_local_JSON =
		OpenAPI_ambr_convertToJSON(sm_context->session_ambr);
	...
	if(!sm_context->qos_flows_list) {
		ogs_error(
			"OpenAPI_sm_context_convertToJSON() failed [qos_flows_list]");
		return NULL;
	}
	cJSON *qos_flows_listList = cJSON_AddArrayToObject(item,
	                                                   "qosFlowsList");
	...
	OpenAPI_list_for_each(sm_context->qos_flows_list, node) {
		cJSON *itemLocal =
			OpenAPI_qos_flow_setup_item_convertToJSON(node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [qos_flows_list]");
			goto end;
	...
	if(sm_context->h_smf_instance_id) {
		if(cJSON_AddStringToObject(item, "hSmfInstanceId",
		                           sm_context->h_smf_instance_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [h_smf_instance_id]");
			goto end;
		}
	}
	...
	if(sm_context->smf_instance_id) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [smf_instance_id]");
			goto end;
		}
	}

	if(sm_context->pdu_session_smf_set_id) {
		if(cJSON_AddStringToObject(item, "pduSessionSmfSetId",
		                           sm_context->pdu_session_smf_set_id)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [pdu_session_smf_set_id]");
			goto end;
		}
	}

	if(sm_context->pdu_session_smf_service_set_id) {
		if(cJSON_AddStringToObject(item, "pduSessionSmfServiceSetId",
		                           sm_context->
		                           pdu_session_smf_service_set_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [pdu_session_smf_service_set_id]");
			goto end;
		}
	}
	...
	{
		if(cJSON_AddStringToObject(item, "pduSessionSmfBinding",
		...
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [pdu_session_smf_binding]");
			goto end;
		}
	}

	if(sm_context->is_enable_pause_charging) {
		if(cJSON_AddBoolToObject(item, "enablePauseCharging",
		                         sm_context->enable_pause_charging) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [enable_pause_charging]");
			goto end;
		}
	}

	if(sm_context->ue_ipv4_address) {
		if(cJSON_AddStringToObject(item, "ueIpv4Address",
		                           sm_context->ue_ipv4_address) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [ue_ipv4_address]");
			goto end;
		}
	}

	if(sm_context->ue_ipv6_prefix) {
		if(cJSON_AddStringToObject(item, "ueIpv6Prefix",
		                           sm_context->ue_ipv6_prefix) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [ue_ipv6_prefix]");
			goto end;
		}
	}

	if(sm_context->eps_pdn_cnx_info) {
		cJSON *eps_pdn_cnx_info_local_JSON =
			OpenAPI_eps_pdn_cnx_info_convertToJSON(
				sm_context->eps_pdn_cnx_info);
		if(eps_pdn_cnx_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [eps_pdn_cnx_info]");
			goto end;
		}
		cJSON_AddItemToObject(item, "epsPdnCnxInfo",
		                      eps_pdn_cnx_info_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [eps_pdn_cnx_info]");
			goto end;
	...
	if(sm_context->eps_bearer_info) {
		cJSON *eps_bearer_infoList = cJSON_AddArrayToObject(item,
		                                                    "epsBearerInfo");
		if(eps_bearer_infoList == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [eps_bearer_info]");
			goto end;
		}
		OpenAPI_list_for_each(sm_context->eps_bearer_info, node) {
			cJSON *itemLocal =
				OpenAPI_eps_bearer_info_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_sm_context_convertToJSON() failed [eps_bearer_info]");
	   ...
	   OpenAPI_max_integrity_protected_data_rate_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [max_integrity_protected_data_rate]");
			goto end;
		}
	}

	if(sm_context->max_integrity_protected_data_rate_dl !=
	   OpenAPI_max_integrity_protected_data_rate_NULL)
	{
		if(cJSON_AddStringToObject(item,
		...
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [max_integrity_protected_data_rate_dl]");
			goto end;
		}
	}

	if(sm_context->is_always_on_granted) {
		if(cJSON_AddBoolToObject(item, "alwaysOnGranted",
		                         sm_context->always_on_granted) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [always_on_granted]");
			goto end;
		}
	}
	...
	if(sm_context->up_security) {
		cJSON *up_security_local_JSON =
			OpenAPI_up_security_convertToJSON(
				sm_context->up_security);
		if(up_security_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [up_security]");
			goto end;
		}
		cJSON_AddItemToObject(item, "upSecurity",
		                      up_security_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [up_security]");
			goto end;
	...
	if(sm_context->h_smf_service_instance_id) {
		if(cJSON_AddStringToObject(item, "hSmfServiceInstanceId",
		                           sm_context->h_smf_service_instance_id)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [h_smf_service_instance_id]");
			goto end;
		}
	}
	...
	if(sm_context->smf_service_instance_id) {
		if(cJSON_AddStringToObject(item, "smfServiceInstanceId",
		                           sm_context->smf_service_instance_id)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [smf_service_instance_id]");
			goto end;
		}
	}
	...
	if(sm_context->recovery_time) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [recovery_time]");
			goto end;
		}
	}

	if(sm_context->is_forwarding_ind) {
		if(cJSON_AddBoolToObject(item, "forwardingInd",
		                         sm_context->forwarding_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [forwarding_ind]");
			goto end;
		}
	}

	if(sm_context->psa_tunnel_info) {
		cJSON *psa_tunnel_info_local_JSON =
			OpenAPI_tunnel_info_convertToJSON(
				sm_context->psa_tunnel_info);
		if(psa_tunnel_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [psa_tunnel_info]");
			goto end;
		}
		cJSON_AddItemToObject(item, "psaTunnelInfo",
		                      psa_tunnel_info_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [psa_tunnel_info]");
			goto end;
	...
	if(sm_context->charging_id) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [charging_id]");
			goto end;
		}
	}

	if(sm_context->charging_info) {
		cJSON *charging_info_local_JSON =
			OpenAPI_charging_information_convertToJSON(
				sm_context->charging_info);
		if(charging_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [charging_info]");
			goto end;
		}
		cJSON_AddItemToObject(item, "chargingInfo",
		                      charging_info_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [charging_info]");
			goto end;
	...
	if(sm_context->roaming_charging_profile) {
		...
		if(roaming_charging_profile_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [roaming_charging_profile]");
			goto end;
		}
	...
	if(sm_context->is_nef_ext_buf_support_ind) {
		if(cJSON_AddBoolToObject(item, "nefExtBufSupportInd",
		                         sm_context->nef_ext_buf_support_ind) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [nef_ext_buf_support_ind]");
			goto end;
		}
	}

	if(sm_context->is_ipv6_index) {
		if(cJSON_AddNumberToObject(item, "ipv6Index",
		                           sm_context->ipv6_index) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [ipv6_index]");
			goto end;
		}
	}

	if(sm_context->dn_aaa_address) {
		cJSON *dn_aaa_address_local_JSON =
			OpenAPI_ip_address_convertToJSON(
				sm_context->dn_aaa_address);
		if(dn_aaa_address_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [dn_aaa_address]");
			goto end;
		}
		cJSON_AddItemToObject(item, "dnAaaAddress",
		                      dn_aaa_address_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [dn_aaa_address]");
			goto end;
	...
	if(sm_context->redundant_pdu_session_info) {
		...
		if(redundant_pdu_session_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [redundant_pdu_session_info]");
			goto end;
		}
	...
	if(sm_context->ran_tunnel_info) {
		cJSON *ran_tunnel_info_local_JSON =
			OpenAPI_qos_flow_tunnel_convertToJSON(
				sm_context->ran_tunnel_info);
		if(ran_tunnel_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [ran_tunnel_info]");
			goto end;
		}
		cJSON_AddItemToObject(item, "ranTunnelInfo",
		                      ran_tunnel_info_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [ran_tunnel_info]");
			goto end;
	...
	if(sm_context->add_ran_tunnel_info) {
		cJSON *add_ran_tunnel_infoList = cJSON_AddArrayToObject(item,
		                                                        "addRanTunnelInfo");
		if(add_ran_tunnel_infoList == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [add_ran_tunnel_info]");
			goto end;
		}
		OpenAPI_list_for_each(sm_context->add_ran_tunnel_info, node) {
			cJSON *itemLocal =
			...
			cJSON_AddItemToArray(add_ran_tunnel_infoList,
			                     itemLocal);
		}
	}

	if(sm_context->red_ran_tunnel_info) {
		cJSON *red_ran_tunnel_info_local_JSON =
			OpenAPI_qos_flow_tunnel_convertToJSON(
				sm_context->red_ran_tunnel_info);
		if(red_ran_tunnel_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [red_ran_tunnel_info]");
			goto end;
		}
		cJSON_AddItemToObject(item, "redRanTunnelInfo",
		                      red_ran_tunnel_info_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [red_ran_tunnel_info]");
			goto end;
	...
	if(sm_context->add_red_ran_tunnel_info) {
		cJSON *add_red_ran_tunnel_infoList =
			cJSON_AddArrayToObject(item, "addRedRanTunnelInfo");
		if(add_red_ran_tunnel_infoList == NULL) {
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [add_red_ran_tunnel_info]");
			goto end;
		}
		OpenAPI_list_for_each(sm_context->add_red_ran_tunnel_info,
		                      node) {
			...
			cJSON_AddItemToArray(add_red_ran_tunnel_infoList,
			                     itemLocal);
		}
	}

	if(sm_context->is_nspu_support_ind) {
		if(cJSON_AddBoolToObject(item, "nspuSupportInd",
		                         sm_context->nspu_support_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [nspu_support_ind]");
			goto end;
		}
	}
	...
	if(sm_context->smf_binding_info) {
		if(cJSON_AddStringToObject(item, "smfBindingInfo",
		                           sm_context->smf_binding_info) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [smf_binding_info]");
			goto end;
		}
	}

	if(sm_context->satellite_backhaul_cat !=
	   OpenAPI_satellite_backhaul_category_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [satellite_backhaul_cat]");
			goto end;
		}
	}
	...
	if(sm_context->ssc_mode) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [ssc_mode]");
			goto end;
		}
	}

	if(sm_context->is_dlset_support_ind) {
		if(cJSON_AddBoolToObject(item, "dlsetSupportInd",
		                         sm_context->dlset_support_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [dlset_support_ind]");
			goto end;
		}
	}

	if(sm_context->is_n9fsc_support_ind) {
		if(cJSON_AddBoolToObject(item, "n9fscSupportInd",
		                         sm_context->n9fsc_support_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [n9fsc_support_ind]");
			goto end;
		}
	}
	...
	if(sm_context->is_disaster_roaming_ind) {
		if(cJSON_AddBoolToObject(item, "disasterRoamingInd",
		                         sm_context->disaster_roaming_ind) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [disaster_roaming_ind]");
			goto end;
		}
	}
	...
	if(sm_context->is_anchor_smf_oauth2_required) {
		if(cJSON_AddBoolToObject(item, "anchorSmfOauth2Required",
		                         sm_context->anchor_smf_oauth2_required)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_convertToJSON() failed [anchor_smf_oauth2_required]");
			goto end;
		}
	}
...
OpenAPI_sm_context_t *OpenAPI_sm_context_parseFromJSON(cJSON *sm_contextJSON) {
	...
	OpenAPI_max_integrity_protected_data_rate_e
	        max_integrity_protected_data_rateVariable = 0;
	...
	pdu_session_id = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                  "pduSessionId");
	if(!pdu_session_id) {
		ogs_error(
			"OpenAPI_sm_context_parseFromJSON() failed [pdu_session_id]");
		goto end;
	}
	...
	selected_dnn = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                "selectedDnn");
	if(selected_dnn) {
	...
	if(!s_nssai) {
		ogs_error(
			"OpenAPI_sm_context_parseFromJSON() failed [s_nssai]");
		goto end;
	}
	...
	hplmn_snssai = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                "hplmnSnssai");
	if(hplmn_snssai) {
	...
	pdu_session_type = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                    "pduSessionType");
	if(!pdu_session_type) {
		ogs_error(
			"OpenAPI_sm_context_parseFromJSON() failed [pdu_session_type]");
		goto end;
	}
	...
	if(gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [gpsi]");
			goto end;
		}
	}
	...
	if(h_smf_uri) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [h_smf_uri]");
			goto end;
		}
	}
	...
	if(smf_uri) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [smf_uri]");
			goto end;
		}
	}

	pdu_session_ref = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                   "pduSessionRef");
	if(pdu_session_ref) {
		if(!cJSON_IsString(pdu_session_ref) &&
		   !cJSON_IsNull(pdu_session_ref))
		{
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [pdu_session_ref]");
			goto end;
		}
	}

	inter_plmn_api_root = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                       "interPlmnApiRoot");
	if(inter_plmn_api_root) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [inter_plmn_api_root]");
			goto end;
		}
	}

	intra_plmn_api_root = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                       "intraPlmnApiRoot");
	if(intra_plmn_api_root) {
		if(!cJSON_IsString(intra_plmn_api_root) &&
		   !cJSON_IsNull(intra_plmn_api_root))
		{
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [intra_plmn_api_root]");
			goto end;
		}
	}
	...
	if(pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [pcf_id]");
			goto end;
		}
	}

	pcf_group_id = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                "pcfGroupId");
	if(pcf_group_id) {
	...
	pcf_set_id = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                              "pcfSetId");
	if(pcf_set_id) {
	...
	if(sel_mode) {
		if(!cJSON_IsString(sel_mode)) {
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [sel_mode]");
			goto end;
		}
		sel_modeVariable =
	...
	udm_group_id = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                "udmGroupId");
	if(udm_group_id) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [udm_group_id]");
			goto end;
		}
	}

	routing_indicator = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                     "routingIndicator");
	if(routing_indicator) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [routing_indicator]");
			goto end;
		}
	}

	h_nw_pub_key_id = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                   "hNwPubKeyId");
	if(h_nw_pub_key_id) {
		if(!cJSON_IsNumber(h_nw_pub_key_id)) {
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [h_nw_pub_key_id]");
			goto end;
		}
	}

	session_ambr = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                "sessionAmbr");
	if(!session_ambr) {
		ogs_error(
			"OpenAPI_sm_context_parseFromJSON() failed [session_ambr]");
		goto end;
	}
	...
	qos_flows_list = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                  "qosFlowsList");
	if(!qos_flows_list) {
		ogs_error(
			"OpenAPI_sm_context_parseFromJSON() failed [qos_flows_list]");
		goto end;
	}
	...
	cJSON_ArrayForEach(qos_flows_list_local, qos_flows_list) {
		...
		OpenAPI_qos_flow_setup_item_t *qos_flows_listItem =
			OpenAPI_qos_flow_setup_item_parseFromJSON(
				qos_flows_list_local);
	...
	h_smf_instance_id = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                     "hSmfInstanceId");
	if(h_smf_instance_id) {
		if(!cJSON_IsString(h_smf_instance_id) &&
		   !cJSON_IsNull(h_smf_instance_id))
		{
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [h_smf_instance_id]");
			goto end;
		}
	}

	smf_instance_id = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                   "smfInstanceId");
	if(smf_instance_id) {
		if(!cJSON_IsString(smf_instance_id) &&
		   !cJSON_IsNull(smf_instance_id))
		{
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [smf_instance_id]");
			goto end;
		}
	}

	pdu_session_smf_set_id =
		cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
		                                 "pduSessionSmfSetId");
	if(pdu_session_smf_set_id) {
		if(!cJSON_IsString(pdu_session_smf_set_id) &&
		   !cJSON_IsNull(pdu_session_smf_set_id))
		{
			ogs_error(
	...
	if(pdu_session_smf_binding) {
		if(!cJSON_IsString(pdu_session_smf_binding)) {
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [pdu_session_smf_binding]");
			goto end;
		}
		pdu_session_smf_bindingVariable =
			OpenAPI_sbi_binding_level_FromString(
				pdu_session_smf_binding->valuestring);
	}

	enable_pause_charging = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                         "enablePauseCharging");
	if(enable_pause_charging) {
		if(!cJSON_IsBool(enable_pause_charging)) {
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [enable_pause_charging]");
			goto end;
		}
	}

	ue_ipv4_address = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                   "ueIpv4Address");
	if(ue_ipv4_address) {
		if(!cJSON_IsString(ue_ipv4_address) &&
		   !cJSON_IsNull(ue_ipv4_address))
		{
			ogs_error(
	...
	ue_ipv6_prefix = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                  "ueIpv6Prefix");
	if(ue_ipv6_prefix) {
		if(!cJSON_IsString(ue_ipv6_prefix) &&
		   !cJSON_IsNull(ue_ipv6_prefix))
		{
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [ue_ipv6_prefix]");
			goto end;
		}
	}

	eps_pdn_cnx_info = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                    "epsPdnCnxInfo");
	if(eps_pdn_cnx_info) {
		eps_pdn_cnx_info_local_nonprim =
			OpenAPI_eps_pdn_cnx_info_parseFromJSON(
				eps_pdn_cnx_info);
		if(!eps_pdn_cnx_info_local_nonprim) {
			ogs_error(
				"OpenAPI_eps_pdn_cnx_info_parseFromJSON failed [eps_pdn_cnx_info]");
			goto end;
		}
	}

	eps_bearer_info = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                   "epsBearerInfo");
	if(eps_bearer_info) {
		cJSON *eps_bearer_info_local = NULL;
		if(!cJSON_IsArray(eps_bearer_info)) {
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [eps_bearer_info]");
			goto end;
		}
		...
		cJSON_ArrayForEach(eps_bearer_info_local, eps_bearer_info) {
			...
			OpenAPI_eps_bearer_info_t *eps_bearer_infoItem =
				OpenAPI_eps_bearer_info_parseFromJSON(
					eps_bearer_info_local);
	...
	if(max_integrity_protected_data_rate) {
		if(!cJSON_IsString(max_integrity_protected_data_rate)) {
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [max_integrity_protected_data_rate]");
			goto end;
		}
		max_integrity_protected_data_rateVariable =
			OpenAPI_max_integrity_protected_data_rate_FromString(
				max_integrity_protected_data_rate->valuestring);
	}
	...
	if(max_integrity_protected_data_rate_dl) {
		if(!cJSON_IsString(max_integrity_protected_data_rate_dl)) {
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [max_integrity_protected_data_rate_dl]");
			goto end;
		}
	...
	always_on_granted = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                     "alwaysOnGranted");
	if(always_on_granted) {
		if(!cJSON_IsBool(always_on_granted)) {
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [always_on_granted]");
			goto end;
		}
	}

	up_security = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                               "upSecurity");
	if(up_security) {
		up_security_local_nonprim =
			OpenAPI_up_security_parseFromJSON(up_security);
		if(!up_security_local_nonprim) {
			ogs_error(
				"OpenAPI_up_security_parseFromJSON failed [up_security]");
			goto end;
		}
	}
	...
	if(h_smf_service_instance_id) {
		if(!cJSON_IsString(h_smf_service_instance_id) &&
		   !cJSON_IsNull(h_smf_service_instance_id))
		{
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [h_smf_service_instance_id]");
			goto end;
		}
	}
	...
	if(smf_service_instance_id) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [smf_service_instance_id]");
			goto end;
		}
	}

	recovery_time = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                 "recoveryTime");
	if(recovery_time) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [recovery_time]");
			goto end;
		}
	}

	forwarding_ind = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                  "forwardingInd");
	if(forwarding_ind) {
		if(!cJSON_IsBool(forwarding_ind)) {
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [forwarding_ind]");
			goto end;
		}
	}

	psa_tunnel_info = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                   "psaTunnelInfo");
	if(psa_tunnel_info) {
		psa_tunnel_info_local_nonprim =
			OpenAPI_tunnel_info_parseFromJSON(psa_tunnel_info);
		if(!psa_tunnel_info_local_nonprim) {
			ogs_error(
				"OpenAPI_tunnel_info_parseFromJSON failed [psa_tunnel_info]");
			goto end;
		}
	}

	charging_id = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                               "chargingId");
	if(charging_id) {
	...
	charging_info = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                 "chargingInfo");
	if(charging_info) {
		charging_info_local_nonprim =
			OpenAPI_charging_information_parseFromJSON(
				charging_info);
		if(!charging_info_local_nonprim) {
			ogs_error(
				"OpenAPI_charging_information_parseFromJSON failed [charging_info]");
			goto end;
		}
	}
	...
	ipv6_index = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                              "ipv6Index");
	if(ipv6_index) {
		if(!cJSON_IsNumber(ipv6_index)) {
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [ipv6_index]");
			goto end;
		}
	}

	dn_aaa_address = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                  "dnAaaAddress");
	if(dn_aaa_address) {
		dn_aaa_address_local_nonprim =
			OpenAPI_ip_address_parseFromJSON(dn_aaa_address);
		if(!dn_aaa_address_local_nonprim) {
			ogs_error(
				"OpenAPI_ip_address_parseFromJSON failed [dn_aaa_address]");
			goto end;
		}
	}
	...
	ran_tunnel_info = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                   "ranTunnelInfo");
	if(ran_tunnel_info) {
		ran_tunnel_info_local_nonprim =
			OpenAPI_qos_flow_tunnel_parseFromJSON(ran_tunnel_info);
		if(!ran_tunnel_info_local_nonprim) {
			ogs_error(
				"OpenAPI_qos_flow_tunnel_parseFromJSON failed [ran_tunnel_info]");
			goto end;
		}
	}

	add_ran_tunnel_info = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                       "addRanTunnelInfo");
	if(add_ran_tunnel_info) {
		cJSON *add_ran_tunnel_info_local = NULL;
		if(!cJSON_IsArray(add_ran_tunnel_info)) {
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [add_ran_tunnel_info]");
			goto end;
		}

		add_ran_tunnel_infoList = OpenAPI_list_create();

		cJSON_ArrayForEach(add_ran_tunnel_info_local,
		                   add_ran_tunnel_info) {
			...
			OpenAPI_qos_flow_tunnel_t *add_ran_tunnel_infoItem =
				OpenAPI_qos_flow_tunnel_parseFromJSON(
					add_ran_tunnel_info_local);
			if(!add_ran_tunnel_infoItem) {
				ogs_error("No add_ran_tunnel_infoItem");
				goto end;
			}
			OpenAPI_list_add(add_ran_tunnel_infoList,
			                 add_ran_tunnel_infoItem);
		}
	}

	red_ran_tunnel_info = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                       "redRanTunnelInfo");
	if(red_ran_tunnel_info) {
		red_ran_tunnel_info_local_nonprim =
			OpenAPI_qos_flow_tunnel_parseFromJSON(
				red_ran_tunnel_info);
		if(!red_ran_tunnel_info_local_nonprim) {
			ogs_error(
				"OpenAPI_qos_flow_tunnel_parseFromJSON failed [red_ran_tunnel_info]");
			goto end;
		}
	}
	...
	if(add_red_ran_tunnel_info) {
		...
		if(!cJSON_IsArray(add_red_ran_tunnel_info)) {
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [add_red_ran_tunnel_info]");
			goto end;
		}

		add_red_ran_tunnel_infoList = OpenAPI_list_create();

		cJSON_ArrayForEach(add_red_ran_tunnel_info_local,
		                   add_red_ran_tunnel_info) {
			...
			OpenAPI_qos_flow_tunnel_t *add_red_ran_tunnel_infoItem =
				OpenAPI_qos_flow_tunnel_parseFromJSON(
					add_red_ran_tunnel_info_local);
			if(!add_red_ran_tunnel_infoItem) {
				ogs_error("No add_red_ran_tunnel_infoItem");
				goto end;
			}
			OpenAPI_list_add(add_red_ran_tunnel_infoList,
			                 add_red_ran_tunnel_infoItem);
		}
	}

	nspu_support_ind = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                    "nspuSupportInd");
	if(nspu_support_ind) {
		if(!cJSON_IsBool(nspu_support_ind)) {
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [nspu_support_ind]");
			goto end;
		}
	}

	smf_binding_info = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                    "smfBindingInfo");
	if(smf_binding_info) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [smf_binding_info]");
			goto end;
		}
	}
	...
	if(satellite_backhaul_cat) {
		if(!cJSON_IsString(satellite_backhaul_cat)) {
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [satellite_backhaul_cat]");
			goto end;
		}
	...
	if(ssc_mode) {
		if(!cJSON_IsString(ssc_mode) &&
		   !cJSON_IsNull(ssc_mode))
		{
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [ssc_mode]");
			goto end;
		}
	}

	dlset_support_ind = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                     "dlsetSupportInd");
	if(dlset_support_ind) {
		if(!cJSON_IsBool(dlset_support_ind)) {
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [dlset_support_ind]");
			goto end;
		}
	}

	n9fsc_support_ind = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                     "n9fscSupportInd");
	if(n9fsc_support_ind) {
		if(!cJSON_IsBool(n9fsc_support_ind)) {
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [n9fsc_support_ind]");
			goto end;
		}
	}

	disaster_roaming_ind = cJSON_GetObjectItemCaseSensitive(sm_contextJSON,
	                                                        "disasterRoamingInd");
	if(disaster_roaming_ind) {
		if(!cJSON_IsBool(disaster_roaming_ind)) {
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [disaster_roaming_ind]");
			goto end;
		}
	}
	...
	if(anchor_smf_oauth2_required) {
		if(!cJSON_IsBool(anchor_smf_oauth2_required)) {
			ogs_error(
				"OpenAPI_sm_context_parseFromJSON() failed [anchor_smf_oauth2_required]");
			goto end;
		}
	}
	...
	sm_context_local_var = OpenAPI_sm_context_create(
		...
		h_smf_uri &&
		!cJSON_IsNull(h_smf_uri) ? ogs_strdup(h_smf_uri->valuestring) :
		NULL,
		smf_uri &&
		!cJSON_IsNull(smf_uri) ? ogs_strdup(smf_uri->valuestring) :
		NULL,
		pdu_session_ref &&
		!cJSON_IsNull(pdu_session_ref) ?
		ogs_strdup(pdu_session_ref->valuestring) : NULL,
		inter_plmn_api_root &&
		!cJSON_IsNull(inter_plmn_api_root) ?
		ogs_strdup(inter_plmn_api_root->valuestring) : NULL,
		intra_plmn_api_root &&
		!cJSON_IsNull(intra_plmn_api_root) ?
		ogs_strdup(intra_plmn_api_root->valuestring) : NULL,
		pcf_id &&
		...
		h_smf_instance_id &&
		!cJSON_IsNull(h_smf_instance_id) ?
		ogs_strdup(h_smf_instance_id->valuestring) : NULL,
		smf_instance_id &&
		!cJSON_IsNull(smf_instance_id) ?
		ogs_strdup(smf_instance_id->valuestring) : NULL,
		pdu_session_smf_set_id &&
		!cJSON_IsNull(pdu_session_smf_set_id) ?
		ogs_strdup(pdu_session_smf_set_id->valuestring) : NULL,
		pdu_session_smf_service_set_id &&
		!cJSON_IsNull(pdu_session_smf_service_set_id) ?
		ogs_strdup(pdu_session_smf_service_set_id->valuestring) : NULL,
		...
		ue_ipv4_address &&
		!cJSON_IsNull(ue_ipv4_address) ?
		ogs_strdup(ue_ipv4_address->valuestring) : NULL,
		ue_ipv6_prefix &&
		!cJSON_IsNull(ue_ipv6_prefix) ?
		ogs_strdup(ue_ipv6_prefix->valuestring) : NULL,
		...
		h_smf_service_instance_id &&
		!cJSON_IsNull(h_smf_service_instance_id) ?
		ogs_strdup(h_smf_service_instance_id->valuestring) : NULL,
		smf_service_instance_id &&
		!cJSON_IsNull(smf_service_instance_id) ?
		ogs_strdup(smf_service_instance_id->valuestring) : NULL,
		recovery_time &&
		!cJSON_IsNull(recovery_time) ?
		...
		ssc_mode &&
		!cJSON_IsNull(ssc_mode) ? ogs_strdup(ssc_mode->valuestring) :
		NULL,
...
OpenAPI_sm_context_t *OpenAPI_sm_context_copy(OpenAPI_sm_context_t	*dst,
                                              OpenAPI_sm_context_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
