...
OpenAPI_network_area_info_1_t *OpenAPI_network_area_info_1_create(
	...
	OpenAPI_network_area_info_1_t *network_area_info_1_local_var =
		ogs_malloc(sizeof(OpenAPI_network_area_info_1_t));
...
void OpenAPI_network_area_info_1_free(
	...
	if(network_area_info_1->g_ran_node_ids) {
		OpenAPI_list_for_each(network_area_info_1->g_ran_node_ids,
		                      node) {
...
cJSON *OpenAPI_network_area_info_1_convertToJSON(
	...
	if(network_area_info_1 == NULL) {
		ogs_error(
			"OpenAPI_network_area_info_1_convertToJSON() failed [NetworkAreaInfo_1]");
		return NULL;
	}
	...
	if(network_area_info_1->ecgis) {
		...
		if(ecgisList == NULL) {
			ogs_error(
				"OpenAPI_network_area_info_1_convertToJSON() failed [ecgis]");
			goto end;
		}
		OpenAPI_list_for_each(network_area_info_1->ecgis, node) {
			cJSON *itemLocal =
	...
	if(network_area_info_1->ncgis) {
		...
		if(ncgisList == NULL) {
			ogs_error(
				"OpenAPI_network_area_info_1_convertToJSON() failed [ncgis]");
			goto end;
		}
		OpenAPI_list_for_each(network_area_info_1->ncgis, node) {
			cJSON *itemLocal =
	...
	if(network_area_info_1->g_ran_node_ids) {
		...
		if(g_ran_node_idsList == NULL) {
			ogs_error(
				"OpenAPI_network_area_info_1_convertToJSON() failed [g_ran_node_ids]");
			goto end;
		}
		OpenAPI_list_for_each(network_area_info_1->g_ran_node_ids,
		                      node) {
	...
	if(network_area_info_1->tais) {
		...
		if(taisList == NULL) {
			ogs_error(
				"OpenAPI_network_area_info_1_convertToJSON() failed [tais]");
			goto end;
		}
		OpenAPI_list_for_each(network_area_info_1->tais, node) {
			cJSON *itemLocal =
...
OpenAPI_network_area_info_1_t *OpenAPI_network_area_info_1_parseFromJSON(
	...
	if(ecgis) {
		...
		if(!cJSON_IsArray(ecgis)) {
			ogs_error(
				"OpenAPI_network_area_info_1_parseFromJSON() failed [ecgis]");
			goto end;
		}
	...
	if(ncgis) {
		...
		if(!cJSON_IsArray(ncgis)) {
			ogs_error(
				"OpenAPI_network_area_info_1_parseFromJSON() failed [ncgis]");
			goto end;
		}
	...
	if(g_ran_node_ids) {
		...
		if(!cJSON_IsArray(g_ran_node_ids)) {
			ogs_error(
				"OpenAPI_network_area_info_1_parseFromJSON() failed [g_ran_node_ids]");
			goto end;
		}
	...
	tais = cJSON_GetObjectItemCaseSensitive(network_area_info_1JSON,
	                                        "tais");
	if(tais) {
		cJSON *tais_local = NULL;
		if(!cJSON_IsArray(tais)) {
			ogs_error(
				"OpenAPI_network_area_info_1_parseFromJSON() failed [tais]");
			goto end;
		}
...
OpenAPI_network_area_info_1_t *OpenAPI_network_area_info_1_copy(
	OpenAPI_network_area_info_1_t	*dst,
	OpenAPI_network_area_info_1_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
