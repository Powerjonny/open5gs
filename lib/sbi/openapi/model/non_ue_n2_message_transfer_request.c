...
OpenAPI_non_ue_n2_message_transfer_request_create(
	...
	OpenAPI_non_ue_n2_message_transfer_request_t *
		...
		ogs_malloc(
			sizeof(OpenAPI_non_ue_n2_message_transfer_request_t));
...
cJSON *OpenAPI_non_ue_n2_message_transfer_request_convertToJSON(
	...
	if(non_ue_n2_message_transfer_request == NULL) {
		ogs_error(
			"OpenAPI_non_ue_n2_message_transfer_request_convertToJSON() failed [NonUeN2MessageTransfer_request]");
		return NULL;
	}
	...
	if(non_ue_n2_message_transfer_request->json_data) {
		cJSON *json_data_local_JSON =
			OpenAPI_n2_information_transfer_req_data_convertToJSON(
				non_ue_n2_message_transfer_request->json_data);
		if(json_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_non_ue_n2_message_transfer_request_convertToJSON() failed [json_data]");
			goto end;
		}
	...
	if(non_ue_n2_message_transfer_request->binary_data_n2_information) {
		...
		{
			ogs_error(
				"OpenAPI_non_ue_n2_message_transfer_request_convertToJSON() failed [binary_data_n2_information]");
			goto end;
		}
...
OpenAPI_non_ue_n2_message_transfer_request_parseFromJSON(
	...
	OpenAPI_n2_information_transfer_req_data_t *json_data_local_nonprim =
		NULL;
	...
	if(json_data) {
		...
		if(!json_data_local_nonprim) {
			ogs_error(
				"OpenAPI_n2_information_transfer_req_data_parseFromJSON failed [json_data]");
			goto end;
		}
	}
	...
	if(binary_data_n2_information) {
		...
		if(!cJSON_IsString(binary_data_n2_information)) {
			ogs_error(
				"OpenAPI_non_ue_n2_message_transfer_request_parseFromJSON() failed [binary_data_n2_information]");
			goto end;
		}
...
OpenAPI_non_ue_n2_message_transfer_request_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_non_ue_n2_message_transfer_request_convertToJSON() failed");
		return NULL;
	}
