...
OpenAPI_snssai_dnn_pair_t *OpenAPI_snssai_dnn_pair_create(
	...
	OpenAPI_snssai_t
	        *snssai) {
	OpenAPI_snssai_dnn_pair_t *snssai_dnn_pair_local_var =
		ogs_malloc(sizeof(OpenAPI_snssai_dnn_pair_t));
...
cJSON *OpenAPI_snssai_dnn_pair_convertToJSON(
	...
	if(snssai_dnn_pair == NULL) {
		ogs_error(
			"OpenAPI_snssai_dnn_pair_convertToJSON() failed [SnssaiDnnPair]");
		return NULL;
	}
	...
	if(!snssai_dnn_pair->dnn) {
		ogs_error(
			"OpenAPI_snssai_dnn_pair_convertToJSON() failed [dnn]");
		return NULL;
	}
	...
	if(!snssai_dnn_pair->snssai) {
		ogs_error(
			"OpenAPI_snssai_dnn_pair_convertToJSON() failed [snssai]");
		return NULL;
	}
	cJSON *snssai_local_JSON =
		OpenAPI_snssai_convertToJSON(snssai_dnn_pair->snssai);
...
OpenAPI_snssai_dnn_pair_t *OpenAPI_snssai_dnn_pair_parseFromJSON(
	...
	if(!dnn) {
		ogs_error(
			"OpenAPI_snssai_dnn_pair_parseFromJSON() failed [dnn]");
		goto end;
	}
	...
	snssai = cJSON_GetObjectItemCaseSensitive(snssai_dnn_pairJSON,
	                                          "snssai");
	if(!snssai) {
		ogs_error(
			"OpenAPI_snssai_dnn_pair_parseFromJSON() failed [snssai]");
		goto end;
	}
