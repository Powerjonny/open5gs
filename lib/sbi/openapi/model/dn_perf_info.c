...
OpenAPI_dn_perf_info_t *OpenAPI_dn_perf_info_create(char *app_id, char *dnn,
                                                    OpenAPI_snssai_t *snssai,
                                                    OpenAPI_list_t *dn_perf,
                                                    bool is_confidence,
                                                    int confidence) {
	OpenAPI_dn_perf_info_t *dn_perf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_dn_perf_info_t));
...
cJSON *OpenAPI_dn_perf_info_convertToJSON(
	OpenAPI_dn_perf_info_t *dn_perf_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(dn_perf_info == NULL) {
		ogs_error(
			"OpenAPI_dn_perf_info_convertToJSON() failed [DnPerfInfo]");
		return NULL;
	}
	...
	if(dn_perf_info->app_id) {
		...
		{
			ogs_error(
				"OpenAPI_dn_perf_info_convertToJSON() failed [app_id]");
			goto end;
		}
	}
	...
	if(dn_perf_info->dnn) {
		...
		{
			ogs_error(
				"OpenAPI_dn_perf_info_convertToJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(dn_perf_info->snssai) {
		cJSON *snssai_local_JSON =
			OpenAPI_snssai_convertToJSON(dn_perf_info->snssai);
		if(snssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_dn_perf_info_convertToJSON() failed [snssai]");
			goto end;
		}
	...
	if(!dn_perf_info->dn_perf) {
		ogs_error(
			"OpenAPI_dn_perf_info_convertToJSON() failed [dn_perf]");
		return NULL;
	}
	...
	if(dn_perf_info->is_confidence) {
		...
		{
			ogs_error(
				"OpenAPI_dn_perf_info_convertToJSON() failed [confidence]");
			goto end;
		}
	}
...
OpenAPI_dn_perf_info_t *OpenAPI_dn_perf_info_parseFromJSON(
	cJSON *dn_perf_infoJSON) {
	...
	if(app_id) {
		...
		{
			ogs_error(
				"OpenAPI_dn_perf_info_parseFromJSON() failed [app_id]");
			goto end;
		}
	}
	...
	if(dnn) {
		...
		{
			ogs_error(
				"OpenAPI_dn_perf_info_parseFromJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(!dn_perf) {
		ogs_error(
			"OpenAPI_dn_perf_info_parseFromJSON() failed [dn_perf]");
		goto end;
	}
	...
	cJSON_ArrayForEach(dn_perf_local, dn_perf) {
		...
		OpenAPI_dn_perf_t *dn_perfItem =
			OpenAPI_dn_perf_parseFromJSON(dn_perf_local);
		if(!dn_perfItem) {
	...
	if(confidence) {
		if(!cJSON_IsNumber(confidence)) {
			ogs_error(
				"OpenAPI_dn_perf_info_parseFromJSON() failed [confidence]");
			goto end;
		}
	}
...
OpenAPI_dn_perf_info_t *OpenAPI_dn_perf_info_copy(
	OpenAPI_dn_perf_info_t	*dst,
