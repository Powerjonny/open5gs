...
OpenAPI_transfer_mt_data_add_info_t *OpenAPI_transfer_mt_data_add_info_create(
	...
	OpenAPI_transfer_mt_data_add_info_t *transfer_mt_data_add_info_local_var
	        = ogs_malloc(sizeof(OpenAPI_transfer_mt_data_add_info_t));
	ogs_assert(transfer_mt_data_add_info_local_var);

	transfer_mt_data_add_info_local_var->is_max_waiting_time =
		is_max_waiting_time;
	transfer_mt_data_add_info_local_var->max_waiting_time =
		max_waiting_time;
...
cJSON *OpenAPI_transfer_mt_data_add_info_convertToJSON(
	...
	if(transfer_mt_data_add_info == NULL) {
		ogs_error(
			"OpenAPI_transfer_mt_data_add_info_convertToJSON() failed [TransferMtDataAddInfo]");
		return NULL;
	}
	...
	if(transfer_mt_data_add_info->is_max_waiting_time) {
		if(cJSON_AddNumberToObject(item, "maxWaitingTime",
		                           transfer_mt_data_add_info->
		                           max_waiting_time) == NULL)
		{
			ogs_error(
				"OpenAPI_transfer_mt_data_add_info_convertToJSON() failed [max_waiting_time]");
			goto end;
		}
	}
...
OpenAPI_transfer_mt_data_add_info_parseFromJSON(
	...
	max_waiting_time =
		cJSON_GetObjectItemCaseSensitive(transfer_mt_data_add_infoJSON,
		                                 "maxWaitingTime");
	if(max_waiting_time) {
		if(!cJSON_IsNumber(max_waiting_time)) {
			ogs_error(
				"OpenAPI_transfer_mt_data_add_info_parseFromJSON() failed [max_waiting_time]");
			goto end;
		}
	}

	transfer_mt_data_add_info_local_var =
		OpenAPI_transfer_mt_data_add_info_create(
...
OpenAPI_transfer_mt_data_add_info_t *OpenAPI_transfer_mt_data_add_info_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_transfer_mt_data_add_info_convertToJSON() failed");
		return NULL;
	}
