...
OpenAPI_spatial_validity_t *OpenAPI_spatial_validity_create(
	...
	OpenAPI_spatial_validity_t *spatial_validity_local_var =
		ogs_malloc(sizeof(OpenAPI_spatial_validity_t));
...
void OpenAPI_spatial_validity_free(
	OpenAPI_spatial_validity_t *spatial_validity) {
	...
	if(spatial_validity->presence_info_list) {
		OpenAPI_list_for_each(spatial_validity->presence_info_list,
		                      node) {
...
cJSON *OpenAPI_spatial_validity_convertToJSON(
	OpenAPI_spatial_validity_t *spatial_validity) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(spatial_validity == NULL) {
		ogs_error(
			"OpenAPI_spatial_validity_convertToJSON() failed [SpatialValidity]");
		return NULL;
	}
	...
	if(!spatial_validity->presence_info_list) {
		ogs_error(
			"OpenAPI_spatial_validity_convertToJSON() failed [presence_info_list]");
		return NULL;
	}
	...
	if(spatial_validity->presence_info_list) {
		OpenAPI_list_for_each(spatial_validity->presence_info_list,
			...
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_spatial_validity_convertToJSON() failed [inner]");
				goto end;
			}
...
OpenAPI_spatial_validity_t *OpenAPI_spatial_validity_parseFromJSON(
	cJSON *spatial_validityJSON) {
	...
	presence_info_list =
		cJSON_GetObjectItemCaseSensitive(spatial_validityJSON,
		                                 "presenceInfoList");
	if(!presence_info_list) {
		ogs_error(
			"OpenAPI_spatial_validity_parseFromJSON() failed [presence_info_list]");
		goto end;
	}
	...
	if(cJSON_IsObject(presence_info_list)) {
		...
		cJSON_ArrayForEach(presence_info_list_local_map,
			...
			if(cJSON_IsObject(localMapObject)) {
				...
				ogs_error(
					"OpenAPI_spatial_validity_parseFromJSON() failed [inner]");
				goto end;
			}
...
OpenAPI_spatial_validity_t *OpenAPI_spatial_validity_copy(
	OpenAPI_spatial_validity_t *dst, OpenAPI_spatial_validity_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
