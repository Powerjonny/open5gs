...
OpenAPI_qos_flow_item_t *OpenAPI_qos_flow_item_create(int		qfi,
                                                      ...
                                                      ng_ap_cause) {
	OpenAPI_qos_flow_item_t *qos_flow_item_local_var =
		ogs_malloc(sizeof(OpenAPI_qos_flow_item_t));
	ogs_assert(qos_flow_item_local_var);

	qos_flow_item_local_var->qfi = qfi;
	qos_flow_item_local_var->cause = cause;
	qos_flow_item_local_var->is_current_qos_profile_index =
		is_current_qos_profile_index;
	qos_flow_item_local_var->current_qos_profile_index =
		current_qos_profile_index;
	qos_flow_item_local_var->is_null_qo_s_profile_index =
		is_null_qo_s_profile_index;
	qos_flow_item_local_var->null_qo_s_profile_index =
		null_qo_s_profile_index;
...
cJSON *OpenAPI_qos_flow_item_convertToJSON(
	...
	if(qos_flow_item == NULL) {
		ogs_error(
			"OpenAPI_qos_flow_item_convertToJSON() failed [QosFlowItem]");
		return NULL;
	}
	...
	if(qos_flow_item->cause != OpenAPI_cause_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_qos_flow_item_convertToJSON() failed [cause]");
			goto end;
		}
	}
	...
	if(qos_flow_item->is_current_qos_profile_index) {
		...
		{
			ogs_error(
				"OpenAPI_qos_flow_item_convertToJSON() failed [current_qos_profile_index]");
			goto end;
		}
	}
	...
	if(qos_flow_item->is_null_qo_s_profile_index) {
		if(cJSON_AddBoolToObject(item, "nullQoSProfileIndex",
		                         qos_flow_item->null_qo_s_profile_index)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_qos_flow_item_convertToJSON() failed [null_qo_s_profile_index]");
			goto end;
		}
	}
	...
	if(qos_flow_item->ng_ap_cause) {
		...
		if(ng_ap_cause_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_qos_flow_item_convertToJSON() failed [ng_ap_cause]");
			goto end;
		}
...
OpenAPI_qos_flow_item_t *OpenAPI_qos_flow_item_parseFromJSON(
	...
	if(cause) {
		if(!cJSON_IsString(cause)) {
			ogs_error(
				"OpenAPI_qos_flow_item_parseFromJSON() failed [cause]");
			goto end;
		}
	...
	current_qos_profile_index =
		cJSON_GetObjectItemCaseSensitive(qos_flow_itemJSON,
		                                 "currentQosProfileIndex");
	if(current_qos_profile_index) {
		if(!cJSON_IsNumber(current_qos_profile_index)) {
			ogs_error(
				"OpenAPI_qos_flow_item_parseFromJSON() failed [current_qos_profile_index]");
			goto end;
		}
	}
	...
	if(null_qo_s_profile_index) {
		if(!cJSON_IsBool(null_qo_s_profile_index)) {
			ogs_error(
				"OpenAPI_qos_flow_item_parseFromJSON() failed [null_qo_s_profile_index]");
			goto end;
		}
	}

	ng_ap_cause = cJSON_GetObjectItemCaseSensitive(qos_flow_itemJSON,
	                                               "ngApCause");
	if(ng_ap_cause) {
