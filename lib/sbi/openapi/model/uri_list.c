...
OpenAPI_uri_list_t *OpenAPI_uri_list_create(OpenAPI_list_t *_links,
                                            bool
                                            is_total_item_count,
                                            int total_item_count)
{
	OpenAPI_uri_list_t *uri_list_local_var =
		ogs_malloc(sizeof(OpenAPI_uri_list_t));
...
cJSON *OpenAPI_uri_list_convertToJSON(OpenAPI_uri_list_t *uri_list) {
	...
	if(uri_list->_links) {
		...
		if(_links == NULL) {
			ogs_error(
				"OpenAPI_uri_list_convertToJSON() failed [_links]");
			goto end;
		}
		...
		if(uri_list->_links) {
			OpenAPI_list_for_each(uri_list->_links, node) {
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_uri_list_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(uri_list->is_total_item_count) {
		if(cJSON_AddNumberToObject(item, "totalItemCount",
		                           uri_list->total_item_count) == NULL)
		{
			ogs_error(
				"OpenAPI_uri_list_convertToJSON() failed [total_item_count]");
			goto end;
		}
	}
...
OpenAPI_uri_list_t *OpenAPI_uri_list_parseFromJSON(cJSON *uri_listJSON) {
	...
	if(_links) {
		...
		{
			ogs_error(
				"OpenAPI_uri_list_parseFromJSON() failed [_links]");
			goto end;
		}
		if(cJSON_IsObject(_links)) {
			...
			cJSON_ArrayForEach(_links_local_map, _links) {
				...
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_uri_list_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	total_item_count = cJSON_GetObjectItemCaseSensitive(uri_listJSON,
	                                                    "totalItemCount");
	if(total_item_count) {
		if(!cJSON_IsNumber(total_item_count)) {
			ogs_error(
				"OpenAPI_uri_list_parseFromJSON() failed [total_item_count]");
			goto end;
		}
	}
...
OpenAPI_uri_list_t *OpenAPI_uri_list_copy(OpenAPI_uri_list_t	*dst,
                                          OpenAPI_uri_list_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
