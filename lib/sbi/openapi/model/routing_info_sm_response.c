...
OpenAPI_routing_info_sm_response_t *OpenAPI_routing_info_sm_response_create(
	...
	OpenAPI_routing_info_sm_response_t *routing_info_sm_response_local_var =
		ogs_malloc(sizeof(OpenAPI_routing_info_sm_response_t));
...
void OpenAPI_routing_info_sm_response_free(
	...
	if(routing_info_sm_response->smsf_non3_gpp) {
		OpenAPI_smsf_registration_free(
			routing_info_sm_response->smsf_non3_gpp);
	...
	if(routing_info_sm_response->sms_router) {
		OpenAPI_sms_router_info_free(
			routing_info_sm_response->sms_router);
...
cJSON *OpenAPI_routing_info_sm_response_convertToJSON(
	...
	if(routing_info_sm_response == NULL) {
		ogs_error(
			"OpenAPI_routing_info_sm_response_convertToJSON() failed [RoutingInfoSmResponse]");
		return NULL;
	}
	...
	if(routing_info_sm_response->supi) {
		if(cJSON_AddStringToObject(item, "supi",
		                           routing_info_sm_response->supi) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_routing_info_sm_response_convertToJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(routing_info_sm_response->smsf3_gpp) {
		...
		if(smsf3_gpp_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_routing_info_sm_response_convertToJSON() failed [smsf3_gpp]");
			goto end;
		}
	...
	if(routing_info_sm_response->smsf_non3_gpp) {
		...
		if(smsf_non3_gpp_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_routing_info_sm_response_convertToJSON() failed [smsf_non3_gpp]");
			goto end;
		}
	...
	if(routing_info_sm_response->ip_sm_gw) {
		cJSON *ip_sm_gw_local_JSON =
			OpenAPI_ip_sm_gw_info_convertToJSON(
				routing_info_sm_response->ip_sm_gw);
		if(ip_sm_gw_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_routing_info_sm_response_convertToJSON() failed [ip_sm_gw]");
			goto end;
		}
	...
	if(routing_info_sm_response->sms_router) {
		cJSON *sms_router_local_JSON =
			OpenAPI_sms_router_info_convertToJSON(
				routing_info_sm_response->sms_router);
		if(sms_router_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_routing_info_sm_response_convertToJSON() failed [sms_router]");
			goto end;
		}
...
OpenAPI_routing_info_sm_response_parseFromJSON(
	...
	supi = cJSON_GetObjectItemCaseSensitive(routing_info_sm_responseJSON,
	                                        "supi");
	if(supi) {
		...
		{
			ogs_error(
				"OpenAPI_routing_info_sm_response_parseFromJSON() failed [supi]");
			goto end;
		}
	}

	smsf3_gpp =
		cJSON_GetObjectItemCaseSensitive(routing_info_sm_responseJSON,
		                                 "smsf3Gpp");
	if(smsf3_gpp) {
	...
	if(ip_sm_gw) {
		ip_sm_gw_local_nonprim =
			OpenAPI_ip_sm_gw_info_parseFromJSON(ip_sm_gw);
		if(!ip_sm_gw_local_nonprim) {
			ogs_error(
				"OpenAPI_ip_sm_gw_info_parseFromJSON failed [ip_sm_gw]");
			goto end;
		}
	}
	...
	sms_router =
		cJSON_GetObjectItemCaseSensitive(routing_info_sm_responseJSON,
		                                 "smsRouter");
	if(sms_router) {
		sms_router_local_nonprim =
			OpenAPI_sms_router_info_parseFromJSON(sms_router);
		if(!sms_router_local_nonprim) {
			ogs_error(
				"OpenAPI_sms_router_info_parseFromJSON failed [sms_router]");
			goto end;
		}
	}

	routing_info_sm_response_local_var =
...
OpenAPI_routing_info_sm_response_t *OpenAPI_routing_info_sm_response_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_routing_info_sm_response_convertToJSON() failed");
		return NULL;
	}
