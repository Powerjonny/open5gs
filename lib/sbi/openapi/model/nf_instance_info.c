...
OpenAPI_nf_instance_info_t *OpenAPI_nf_instance_info_create(
	char *nrf_disc_api_uri, OpenAPI_preferred_search_t *preferred_search,
	OpenAPI_list_t *nrf_altered_priorities) {
	OpenAPI_nf_instance_info_t *nf_instance_info_local_var =
		ogs_malloc(sizeof(OpenAPI_nf_instance_info_t));
	...
	nf_instance_info_local_var->nrf_altered_priorities =
		nrf_altered_priorities;

	return nf_instance_info_local_var;
}

void OpenAPI_nf_instance_info_free(
	OpenAPI_nf_instance_info_t *nf_instance_info) {
	...
	if(nf_instance_info->preferred_search) {
		OpenAPI_preferred_search_free(
			nf_instance_info->preferred_search);
		nf_instance_info->preferred_search = NULL;
	}
	if(nf_instance_info->nrf_altered_priorities) {
		OpenAPI_list_for_each(nf_instance_info->nrf_altered_priorities,
		                      node) {
...
cJSON *OpenAPI_nf_instance_info_convertToJSON(
	OpenAPI_nf_instance_info_t *nf_instance_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(nf_instance_info == NULL) {
		ogs_error(
			"OpenAPI_nf_instance_info_convertToJSON() failed [NfInstanceInfo]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(nf_instance_info->nrf_disc_api_uri) {
		if(cJSON_AddStringToObject(item, "nrfDiscApiUri",
		                           nf_instance_info->nrf_disc_api_uri)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_nf_instance_info_convertToJSON() failed [nrf_disc_api_uri]");
			goto end;
		}
	}
	...
	if(nf_instance_info->preferred_search) {
		...
		if(preferred_search_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_nf_instance_info_convertToJSON() failed [preferred_search]");
			goto end;
		}
	...
	if(nf_instance_info->nrf_altered_priorities) {
		cJSON *nrf_altered_priorities = cJSON_AddObjectToObject(item,
		                                                        "nrfAlteredPriorities");
		if(nrf_altered_priorities == NULL) {
			ogs_error(
				"OpenAPI_nf_instance_info_convertToJSON() failed [nrf_altered_priorities]");
			goto end;
		}
		cJSON *localMapObject = nrf_altered_priorities;
		if(nf_instance_info->nrf_altered_priorities) {
			OpenAPI_list_for_each(
				nf_instance_info->nrf_altered_priorities,
				node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				if(localKeyValue->value == NULL) {
					ogs_error(
						"OpenAPI_nf_instance_info_convertToJSON() failed [inner]");
					goto end;
				}
...
OpenAPI_nf_instance_info_t *OpenAPI_nf_instance_info_parseFromJSON(
	cJSON *nf_instance_infoJSON) {
	...
	nrf_disc_api_uri =
		cJSON_GetObjectItemCaseSensitive(nf_instance_infoJSON,
		                                 "nrfDiscApiUri");
	if(nrf_disc_api_uri) {
		if(!cJSON_IsString(nrf_disc_api_uri) &&
		   !cJSON_IsNull(nrf_disc_api_uri))
		{
			ogs_error(
				"OpenAPI_nf_instance_info_parseFromJSON() failed [nrf_disc_api_uri]");
			goto end;
		}
	}
	...
	if(nrf_altered_priorities) {
		...
		if(!cJSON_IsObject(nrf_altered_priorities) &&
		   !cJSON_IsNull(nrf_altered_priorities))
		{
			ogs_error(
				"OpenAPI_nf_instance_info_parseFromJSON() failed [nrf_altered_priorities]");
			goto end;
		}
		if(cJSON_IsObject(nrf_altered_priorities)) {
			nrf_altered_prioritiesList = OpenAPI_list_create();
			OpenAPI_map_t *localMapKeyPair = NULL;
			cJSON_ArrayForEach(nrf_altered_priorities_local_map,
			                   nrf_altered_priorities) {
				cJSON *localMapObject =
					nrf_altered_priorities_local_map;
				double *localDouble = NULL;
				int *localInt = NULL;
				if(!cJSON_IsNumber(localMapObject)) {
					ogs_error(
						"OpenAPI_nf_instance_info_parseFromJSON() failed [inner]");
					goto end;
				}
				...
				*localDouble = localMapObject->valuedouble;
				localMapKeyPair =
					OpenAPI_map_create(ogs_strdup(
								   ...
								   ->string),
					                   localDouble);
				OpenAPI_list_add(nrf_altered_prioritiesList,
				                 localMapKeyPair);
			}
		}
	}
	...
	nf_instance_info_local_var = OpenAPI_nf_instance_info_create(
		nrf_disc_api_uri &&
		!cJSON_IsNull(nrf_disc_api_uri) ?
		ogs_strdup(nrf_disc_api_uri->valuestring) : NULL,
...
OpenAPI_nf_instance_info_t *OpenAPI_nf_instance_info_copy(
	OpenAPI_nf_instance_info_t *dst, OpenAPI_nf_instance_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
