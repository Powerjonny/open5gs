...
OpenAPI_prose_subscription_data_t *OpenAPI_prose_subscription_data_create(
	...
	OpenAPI_prose_subscription_data_t *prose_subscription_data_local_var =
		ogs_malloc(sizeof(OpenAPI_prose_subscription_data_t));
	ogs_assert(prose_subscription_data_local_var);

	prose_subscription_data_local_var->prose_service_auth =
		prose_service_auth;
	prose_subscription_data_local_var->nr_ue_pc5_ambr = nr_ue_pc5_ambr;
	prose_subscription_data_local_var->prose_allowed_plmn =
		prose_allowed_plmn;
...
void OpenAPI_prose_subscription_data_free(
	...
	if(prose_subscription_data->prose_service_auth) {
		OpenAPI_prose_service_auth_free(
			prose_subscription_data->prose_service_auth);
	...
	if(prose_subscription_data->prose_allowed_plmn) {
		OpenAPI_list_for_each(
			prose_subscription_data->prose_allowed_plmn, node) {
...
cJSON *OpenAPI_prose_subscription_data_convertToJSON(
	...
	if(prose_subscription_data == NULL) {
		ogs_error(
			"OpenAPI_prose_subscription_data_convertToJSON() failed [ProseSubscriptionData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(prose_subscription_data->prose_service_auth) {
		cJSON *prose_service_auth_local_JSON =
			OpenAPI_prose_service_auth_convertToJSON(
				prose_subscription_data->prose_service_auth);
		if(prose_service_auth_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_prose_subscription_data_convertToJSON() failed [prose_service_auth]");
			goto end;
		}
		cJSON_AddItemToObject(item, "proseServiceAuth",
		                      prose_service_auth_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_prose_subscription_data_convertToJSON() failed [prose_service_auth]");
			goto end;
	...
	if(prose_subscription_data->nr_ue_pc5_ambr) {
		...
		{
			ogs_error(
				"OpenAPI_prose_subscription_data_convertToJSON() failed [nr_ue_pc5_ambr]");
			goto end;
		}
	}

	if(prose_subscription_data->prose_allowed_plmn) {
		cJSON *prose_allowed_plmnList = cJSON_AddArrayToObject(item,
		                                                       "proseAllowedPlmn");
		if(prose_allowed_plmnList == NULL) {
			ogs_error(
				"OpenAPI_prose_subscription_data_convertToJSON() failed [prose_allowed_plmn]");
			goto end;
		}
		OpenAPI_list_for_each(
			prose_subscription_data->prose_allowed_plmn, node) {
			cJSON *itemLocal =
				OpenAPI_pro_se_allowed_plmn_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_prose_subscription_data_convertToJSON() failed [prose_allowed_plmn]");
...
OpenAPI_prose_subscription_data_t *OpenAPI_prose_subscription_data_parseFromJSON
	...
	prose_service_auth =
		cJSON_GetObjectItemCaseSensitive(prose_subscription_dataJSON,
		                                 "proseServiceAuth");
	if(prose_service_auth) {
		prose_service_auth_local_nonprim =
			OpenAPI_prose_service_auth_parseFromJSON(
				prose_service_auth);
		if(!prose_service_auth_local_nonprim) {
			ogs_error(
				"OpenAPI_prose_service_auth_parseFromJSON failed [prose_service_auth]");
			goto end;
		}
	}
	...
	if(nr_ue_pc5_ambr) {
		...
		{
			ogs_error(
				"OpenAPI_prose_subscription_data_parseFromJSON() failed [nr_ue_pc5_ambr]");
			goto end;
		}
	}
	...
	if(prose_allowed_plmn) {
		...
		cJSON_ArrayForEach(prose_allowed_plmn_local,
			...
			OpenAPI_pro_se_allowed_plmn_t *prose_allowed_plmnItem =
				OpenAPI_pro_se_allowed_plmn_parseFromJSON(
					prose_allowed_plmn_local);
			if(!prose_allowed_plmnItem) {
				ogs_error("No prose_allowed_plmnItem");
				goto end;
			}
			OpenAPI_list_add(prose_allowed_plmnList,
			                 prose_allowed_plmnItem);
		}
	}

	prose_subscription_data_local_var =
		OpenAPI_prose_subscription_data_create(
			prose_service_auth ? prose_service_auth_local_nonprim :
			NULL,
			nr_ue_pc5_ambr &&
			!cJSON_IsNull(nr_ue_pc5_ambr) ?
...
OpenAPI_prose_subscription_data_t *OpenAPI_prose_subscription_data_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_prose_subscription_data_convertToJSON() failed");
		return NULL;
	}
