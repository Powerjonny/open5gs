...
OpenAPI_nnwdaf_events_subscription_t *OpenAPI_nnwdaf_events_subscription_create(
	...
	OpenAPI_reporting_information_t *evt_req, char *notification_uri,
	char *notif_corr_id, char *supported_features,
	OpenAPI_list_t *event_notifications, OpenAPI_list_t *fail_event_reports,
	...
	OpenAPI_nnwdaf_events_subscription_t *
		...
		ogs_malloc(sizeof(OpenAPI_nnwdaf_events_subscription_t));
...
void OpenAPI_nnwdaf_events_subscription_free(
	OpenAPI_nnwdaf_events_subscription_t *nnwdaf_events_subscription) {
	...
	if(nnwdaf_events_subscription->event_subscriptions) {
		OpenAPI_list_for_each(
			nnwdaf_events_subscription->event_subscriptions, node) {
			OpenAPI_event_subscription_free(node->data);
		}
		OpenAPI_list_free(
			nnwdaf_events_subscription->event_subscriptions);
		nnwdaf_events_subscription->event_subscriptions = NULL;
	}
	if(nnwdaf_events_subscription->evt_req) {
		OpenAPI_reporting_information_free(
	...
	if(nnwdaf_events_subscription->event_notifications) {
		OpenAPI_list_for_each(
			nnwdaf_events_subscription->event_notifications, node) {
			OpenAPI_event_notification_free(node->data);
		}
		OpenAPI_list_free(
			nnwdaf_events_subscription->event_notifications);
		nnwdaf_events_subscription->event_notifications = NULL;
	}
	if(nnwdaf_events_subscription->fail_event_reports) {
		OpenAPI_list_for_each(
			nnwdaf_events_subscription->fail_event_reports, node) {
	...
	if(nnwdaf_events_subscription->cons_nf_info) {
		OpenAPI_consumer_nf_information_free(
...
cJSON *OpenAPI_nnwdaf_events_subscription_convertToJSON(
	OpenAPI_nnwdaf_events_subscription_t *nnwdaf_events_subscription) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(nnwdaf_events_subscription == NULL) {
		ogs_error(
			"OpenAPI_nnwdaf_events_subscription_convertToJSON() failed [NnwdafEventsSubscription]");
		return NULL;
	}
	...
	if(!nnwdaf_events_subscription->event_subscriptions) {
		ogs_error(
			"OpenAPI_nnwdaf_events_subscription_convertToJSON() failed [event_subscriptions]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(nnwdaf_events_subscription->event_subscriptions,
	                      node) {
		cJSON *itemLocal =
			OpenAPI_event_subscription_convertToJSON(node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_nnwdaf_events_subscription_convertToJSON() failed [event_subscriptions]");
			goto end;
	...
	if(nnwdaf_events_subscription->evt_req) {
		cJSON *evt_req_local_JSON =
			OpenAPI_reporting_information_convertToJSON(
				nnwdaf_events_subscription->evt_req);
		if(evt_req_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_nnwdaf_events_subscription_convertToJSON() failed [evt_req]");
			goto end;
		}
	...
	if(nnwdaf_events_subscription->notification_uri) {
		...
		{
			ogs_error(
				"OpenAPI_nnwdaf_events_subscription_convertToJSON() failed [notification_uri]");
			goto end;
		}
	}
	...
	if(nnwdaf_events_subscription->notif_corr_id) {
		...
		{
			ogs_error(
				"OpenAPI_nnwdaf_events_subscription_convertToJSON() failed [notif_corr_id]");
			goto end;
		}
	}
	...
	if(nnwdaf_events_subscription->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_nnwdaf_events_subscription_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(nnwdaf_events_subscription->event_notifications) {
		...
		if(event_notificationsList == NULL) {
			ogs_error(
				"OpenAPI_nnwdaf_events_subscription_convertToJSON() failed [event_notifications]");
			goto end;
		}
		OpenAPI_list_for_each(
			nnwdaf_events_subscription->event_notifications, node) {
			cJSON *itemLocal =
				OpenAPI_event_notification_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_nnwdaf_events_subscription_convertToJSON() failed [event_notifications]");
	...
	if(nnwdaf_events_subscription->fail_event_reports) {
		...
		if(fail_event_reportsList == NULL) {
			ogs_error(
				"OpenAPI_nnwdaf_events_subscription_convertToJSON() failed [fail_event_reports]");
			goto end;
		}
		OpenAPI_list_for_each(
			nnwdaf_events_subscription->fail_event_reports, node) {
			cJSON *itemLocal =
				OpenAPI_failure_event_info_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_nnwdaf_events_subscription_convertToJSON() failed [fail_event_reports]");
	...
	if(nnwdaf_events_subscription->prev_sub) {
		cJSON *prev_sub_local_JSON =
			OpenAPI_prev_sub_info_convertToJSON(
				nnwdaf_events_subscription->prev_sub);
		if(prev_sub_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_nnwdaf_events_subscription_convertToJSON() failed [prev_sub]");
			goto end;
		}
	...
	if(nnwdaf_events_subscription->cons_nf_info) {
		cJSON *cons_nf_info_local_JSON =
			OpenAPI_consumer_nf_information_convertToJSON(
				nnwdaf_events_subscription->cons_nf_info);
		if(cons_nf_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_nnwdaf_events_subscription_convertToJSON() failed [cons_nf_info]");
			goto end;
		}
...
OpenAPI_nnwdaf_events_subscription_t *
OpenAPI_nnwdaf_events_subscription_parseFromJSON(
	cJSON *nnwdaf_events_subscriptionJSON) {
	OpenAPI_nnwdaf_events_subscription_t *
	        nnwdaf_events_subscription_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	event_subscriptions =
		cJSON_GetObjectItemCaseSensitive(nnwdaf_events_subscriptionJSON,
		                                 "eventSubscriptions");
	if(!event_subscriptions) {
		ogs_error(
			"OpenAPI_nnwdaf_events_subscription_parseFromJSON() failed [event_subscriptions]");
		goto end;
	}
	...
	cJSON_ArrayForEach(event_subscriptions_local, event_subscriptions) {
		...
		OpenAPI_event_subscription_t *event_subscriptionsItem =
		...
		OpenAPI_list_add(event_subscriptionsList,
		                 event_subscriptionsItem);
	}
	...
	evt_req =
		cJSON_GetObjectItemCaseSensitive(nnwdaf_events_subscriptionJSON,
		                                 "evtReq");
	if(evt_req) {
		evt_req_local_nonprim =
			OpenAPI_reporting_information_parseFromJSON(evt_req);
		if(!evt_req_local_nonprim) {
			ogs_error(
				"OpenAPI_reporting_information_parseFromJSON failed [evt_req]");
			goto end;
		}
	}
	...
	if(notification_uri) {
		...
		{
			ogs_error(
				"OpenAPI_nnwdaf_events_subscription_parseFromJSON() failed [notification_uri]");
			goto end;
		}
	}
	...
	if(notif_corr_id) {
		...
		{
			ogs_error(
				"OpenAPI_nnwdaf_events_subscription_parseFromJSON() failed [notif_corr_id]");
			goto end;
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_nnwdaf_events_subscription_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(event_notifications) {
		...
		if(!cJSON_IsArray(event_notifications)) {
			ogs_error(
				"OpenAPI_nnwdaf_events_subscription_parseFromJSON() failed [event_notifications]");
			goto end;
		}
		...
		cJSON_ArrayForEach(event_notifications_local,
			...
			OpenAPI_event_notification_t *event_notificationsItem =
			...
			OpenAPI_list_add(event_notificationsList,
	...
	if(fail_event_reports) {
		...
		if(!cJSON_IsArray(fail_event_reports)) {
			ogs_error(
				"OpenAPI_nnwdaf_events_subscription_parseFromJSON() failed [fail_event_reports]");
			goto end;
		}
		...
		cJSON_ArrayForEach(fail_event_reports_local,
			...
			OpenAPI_failure_event_info_t *fail_event_reportsItem =
				OpenAPI_failure_event_info_parseFromJSON(
					fail_event_reports_local);
			if(!fail_event_reportsItem) {
				ogs_error("No fail_event_reportsItem");
				goto end;
			}
			OpenAPI_list_add(fail_event_reportsList,
	...
	prev_sub =
		cJSON_GetObjectItemCaseSensitive(nnwdaf_events_subscriptionJSON,
		                                 "prevSub");
	if(prev_sub) {
		prev_sub_local_nonprim =
			OpenAPI_prev_sub_info_parseFromJSON(prev_sub);
		if(!prev_sub_local_nonprim) {
			ogs_error(
				"OpenAPI_prev_sub_info_parseFromJSON failed [prev_sub]");
			goto end;
		}
	}
	...
	if(cons_nf_info) {
		cons_nf_info_local_nonprim =
			OpenAPI_consumer_nf_information_parseFromJSON(
				cons_nf_info);
		if(!cons_nf_info_local_nonprim) {
			ogs_error(
				"OpenAPI_consumer_nf_information_parseFromJSON failed [cons_nf_info]");
			goto end;
		}
	}
...
OpenAPI_nnwdaf_events_subscription_t *OpenAPI_nnwdaf_events_subscription_copy(
	OpenAPI_nnwdaf_events_subscription_t	*dst,
	OpenAPI_nnwdaf_events_subscription_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_nnwdaf_events_subscription_convertToJSON() failed");
		return NULL;
	}
