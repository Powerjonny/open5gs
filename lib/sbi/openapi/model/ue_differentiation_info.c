...
OpenAPI_ue_differentiation_info_t *OpenAPI_ue_differentiation_info_create(
	...
	OpenAPI_battery_indication_t *battery_ind, char *validity_time) {
	OpenAPI_ue_differentiation_info_t *ue_differentiation_info_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_differentiation_info_t));
...
void OpenAPI_ue_differentiation_info_free(
...
cJSON *OpenAPI_ue_differentiation_info_convertToJSON(
	OpenAPI_ue_differentiation_info_t *ue_differentiation_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(ue_differentiation_info == NULL) {
		ogs_error(
			"OpenAPI_ue_differentiation_info_convertToJSON() failed [UeDifferentiationInfo]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(ue_differentiation_info->periodic_com_ind !=
	   OpenAPI_periodic_communication_indicator_NULL)
	{
		if(cJSON_AddStringToObject(item, "periodicComInd",
		                           OpenAPI_periodic_communication_indicator_ToString
		                                   (ue_differentiation_info->
		                                   periodic_com_ind)) == NULL)
		{
			ogs_error(
				"OpenAPI_ue_differentiation_info_convertToJSON() failed [periodic_com_ind]");
			goto end;
		}
	}
	...
	if(ue_differentiation_info->is_periodic_time) {
		...
		{
			ogs_error(
				"OpenAPI_ue_differentiation_info_convertToJSON() failed [periodic_time]");
			goto end;
		}
	}
	...
	if(ue_differentiation_info->scheduled_com_time) {
		...
		if(scheduled_com_time_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_differentiation_info_convertToJSON() failed [scheduled_com_time]");
			goto end;
		}
	...
	{
		if(cJSON_AddStringToObject(item, "stationaryInd",
		                           OpenAPI_stationary_indication_ToString
		                                   (ue_differentiation_info->
		                                   stationary_ind)) == NULL)
		{
			ogs_error(
				"OpenAPI_ue_differentiation_info_convertToJSON() failed [stationary_ind]");
			goto end;
		}
	}
	   ...
	   OpenAPI_traffic_profile_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_ue_differentiation_info_convertToJSON() failed [traffic_profile]");
			goto end;
		}
	}
	...
	if(ue_differentiation_info->battery_ind) {
		...
		if(battery_ind_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_differentiation_info_convertToJSON() failed [battery_ind]");
			goto end;
		}
	...
	if(ue_differentiation_info->validity_time) {
		...
		{
			ogs_error(
				"OpenAPI_ue_differentiation_info_convertToJSON() failed [validity_time]");
			goto end;
		}
	}
...
OpenAPI_ue_differentiation_info_t *OpenAPI_ue_differentiation_info_parseFromJSON
	...
	OpenAPI_scheduled_communication_time_t *scheduled_com_time_local_nonprim
	        = NULL;
	...
	periodic_com_ind =
		cJSON_GetObjectItemCaseSensitive(ue_differentiation_infoJSON,
		                                 "periodicComInd");
	if(periodic_com_ind) {
		if(!cJSON_IsString(periodic_com_ind)) {
			ogs_error(
				"OpenAPI_ue_differentiation_info_parseFromJSON() failed [periodic_com_ind]");
			goto end;
		}
	...
	if(periodic_time) {
		if(!cJSON_IsNumber(periodic_time)) {
			ogs_error(
				"OpenAPI_ue_differentiation_info_parseFromJSON() failed [periodic_time]");
			goto end;
		}
	}
	...
	if(scheduled_com_time) {
		...
		if(!scheduled_com_time_local_nonprim) {
			ogs_error(
				"OpenAPI_scheduled_communication_time_parseFromJSON failed [scheduled_com_time]");
			goto end;
		}
	}
	...
	if(stationary_ind) {
		if(!cJSON_IsString(stationary_ind)) {
			ogs_error(
				"OpenAPI_ue_differentiation_info_parseFromJSON() failed [stationary_ind]");
			goto end;
		}
		stationary_indVariable =
			OpenAPI_stationary_indication_FromString(
				stationary_ind->valuestring);
	}
	...
	if(traffic_profile) {
		if(!cJSON_IsString(traffic_profile)) {
			ogs_error(
				"OpenAPI_ue_differentiation_info_parseFromJSON() failed [traffic_profile]");
			goto end;
		}
	...
	battery_ind =
		cJSON_GetObjectItemCaseSensitive(ue_differentiation_infoJSON,
		                                 "batteryInd");
	if(battery_ind) {
		battery_ind_local_nonprim =
			OpenAPI_battery_indication_parseFromJSON(battery_ind);
		if(!battery_ind_local_nonprim) {
			ogs_error(
				"OpenAPI_battery_indication_parseFromJSON failed [battery_ind]");
			goto end;
		}
	}
	...
	if(validity_time) {
		...
		{
			ogs_error(
				"OpenAPI_ue_differentiation_info_parseFromJSON() failed [validity_time]");
			goto end;
		}
	}

	ue_differentiation_info_local_var =
		OpenAPI_ue_differentiation_info_create(
			periodic_com_ind ? periodic_com_indVariable : 0,
			periodic_time ? true : false,
			periodic_time ? periodic_time->valuedouble : 0,
			scheduled_com_time ? scheduled_com_time_local_nonprim :
			NULL,
...
OpenAPI_ue_differentiation_info_t *OpenAPI_ue_differentiation_info_copy(
	OpenAPI_ue_differentiation_info_t	*dst,
	OpenAPI_ue_differentiation_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ue_differentiation_info_convertToJSON() failed");
		return NULL;
	}
