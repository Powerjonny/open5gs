...
OpenAPI_ue_camping_rep_t *OpenAPI_ue_camping_rep_create(
	...
	OpenAPI_ue_camping_rep_t *ue_camping_rep_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_camping_rep_t));
...
cJSON *OpenAPI_ue_camping_rep_convertToJSON(
	...
	if(ue_camping_rep == NULL) {
		ogs_error(
			"OpenAPI_ue_camping_rep_convertToJSON() failed [UeCampingRep]");
		return NULL;
	}
	...
	if(ue_camping_rep->access_type != OpenAPI_access_type_NULL) {
		if(cJSON_AddStringToObject(item, "accessType",
						   ...
						   ue_camping_rep->access_type))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_ue_camping_rep_convertToJSON() failed [access_type]");
			goto end;
		}
	}
	...
	if(ue_camping_rep->rat_type != OpenAPI_rat_type_NULL) {
		if(cJSON_AddStringToObject(item, "ratType",
						   ...
						   ue_camping_rep->rat_type)) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_ue_camping_rep_convertToJSON() failed [rat_type]");
			goto end;
		}
	}
	...
	if(ue_camping_rep->serv_nf_id) {
		cJSON *serv_nf_id_local_JSON =
			OpenAPI_serving_nf_identity_convertToJSON(
				ue_camping_rep->serv_nf_id);
		if(serv_nf_id_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_camping_rep_convertToJSON() failed [serv_nf_id]");
			goto end;
		}
	...
	if(ue_camping_rep->serving_network) {
		cJSON *serving_network_local_JSON =
			OpenAPI_plmn_id_nid_convertToJSON(
				ue_camping_rep->serving_network);
		if(serving_network_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_camping_rep_convertToJSON() failed [serving_network]");
			goto end;
		}
	...
	if(ue_camping_rep->user_location_info) {
		cJSON *user_location_info_local_JSON =
			OpenAPI_user_location_convertToJSON(
				ue_camping_rep->user_location_info);
		if(user_location_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_camping_rep_convertToJSON() failed [user_location_info]");
			goto end;
		}
	...
	if(ue_camping_rep->ue_time_zone) {
		if(cJSON_AddStringToObject(item, "ueTimeZone",
		                           ue_camping_rep->ue_time_zone) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_ue_camping_rep_convertToJSON() failed [ue_time_zone]");
			goto end;
		}
	}
	...
	{
		if(cJSON_AddStringToObject(item, "netLocAccSupp",
		                           OpenAPI_net_loc_access_support_ToString
		                                   (ue_camping_rep->
		                                   net_loc_acc_supp)) == NULL)
		{
			ogs_error(
				"OpenAPI_ue_camping_rep_convertToJSON() failed [net_loc_acc_supp]");
			goto end;
		}
	}

	if(ue_camping_rep->sat_backhaul_category !=
	   OpenAPI_satellite_backhaul_category_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_ue_camping_rep_convertToJSON() failed [sat_backhaul_category]");
			goto end;
		}
	}
...
OpenAPI_ue_camping_rep_t *OpenAPI_ue_camping_rep_parseFromJSON(
	...
	access_type = cJSON_GetObjectItemCaseSensitive(ue_camping_repJSON,
	                                               "accessType");
	if(access_type) {
		if(!cJSON_IsString(access_type)) {
			ogs_error(
				"OpenAPI_ue_camping_rep_parseFromJSON() failed [access_type]");
			goto end;
		}
		access_typeVariable =
	...
	rat_type = cJSON_GetObjectItemCaseSensitive(ue_camping_repJSON,
	                                            "ratType");
	if(rat_type) {
		if(!cJSON_IsString(rat_type)) {
			ogs_error(
				"OpenAPI_ue_camping_rep_parseFromJSON() failed [rat_type]");
			goto end;
		}
		rat_typeVariable =
	...
	serv_nf_id = cJSON_GetObjectItemCaseSensitive(ue_camping_repJSON,
	                                              "servNfId");
	if(serv_nf_id) {
	...
	serving_network = cJSON_GetObjectItemCaseSensitive(ue_camping_repJSON,
	                                                   "servingNetwork");
	if(serving_network) {
	...
	user_location_info =
		cJSON_GetObjectItemCaseSensitive(ue_camping_repJSON,
		                                 "userLocationInfo");
	...
	ue_time_zone = cJSON_GetObjectItemCaseSensitive(ue_camping_repJSON,
	                                                "ueTimeZone");
	if(ue_time_zone) {
		...
		{
			ogs_error(
				"OpenAPI_ue_camping_rep_parseFromJSON() failed [ue_time_zone]");
			goto end;
		}
	}

	net_loc_acc_supp = cJSON_GetObjectItemCaseSensitive(ue_camping_repJSON,
	                                                    "netLocAccSupp");
	if(net_loc_acc_supp) {
		if(!cJSON_IsString(net_loc_acc_supp)) {
			ogs_error(
				"OpenAPI_ue_camping_rep_parseFromJSON() failed [net_loc_acc_supp]");
			goto end;
		}
		net_loc_acc_suppVariable =
			OpenAPI_net_loc_access_support_FromString(
				net_loc_acc_supp->valuestring);
	}
	...
	if(sat_backhaul_category) {
		if(!cJSON_IsString(sat_backhaul_category)) {
			ogs_error(
				"OpenAPI_ue_camping_rep_parseFromJSON() failed [sat_backhaul_category]");
			goto end;
		}
...
OpenAPI_ue_camping_rep_t *OpenAPI_ue_camping_rep_copy(
	OpenAPI_ue_camping_rep_t *dst, OpenAPI_ue_camping_rep_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
