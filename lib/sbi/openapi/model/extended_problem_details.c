...
OpenAPI_extended_problem_details_t *OpenAPI_extended_problem_details_create(
	...
	OpenAPI_extended_problem_details_t *extended_problem_details_local_var =
		ogs_malloc(sizeof(OpenAPI_extended_problem_details_t));
	...
	extended_problem_details_local_var->supported_features =
		supported_features;
	extended_problem_details_local_var->access_token_error =
		access_token_error;
	extended_problem_details_local_var->access_token_request =
		access_token_request;
	extended_problem_details_local_var->nrf_id = nrf_id;
	extended_problem_details_local_var->acceptable_serv_info =
		acceptable_serv_info;

	return extended_problem_details_local_var;
}

void OpenAPI_extended_problem_details_free(
	OpenAPI_extended_problem_details_t *extended_problem_details) {
	...
	if(extended_problem_details->invalid_params) {
		OpenAPI_list_for_each(extended_problem_details->invalid_params,
		                      node) {
	...
	if(extended_problem_details->access_token_error) {
		OpenAPI_access_token_err_free(
			extended_problem_details->access_token_error);
	...
	if(extended_problem_details->access_token_request) {
		OpenAPI_access_token_req_free(
			extended_problem_details->access_token_request);
	...
	if(extended_problem_details->acceptable_serv_info) {
		OpenAPI_acceptable_service_info_free(
			extended_problem_details->acceptable_serv_info);
...
cJSON *OpenAPI_extended_problem_details_convertToJSON(
	...
	if(extended_problem_details == NULL) {
		ogs_error(
			"OpenAPI_extended_problem_details_convertToJSON() failed [ExtendedProblemDetails]");
		return NULL;
	}
	...
	if(extended_problem_details->type) {
		if(cJSON_AddStringToObject(item, "type",
		                           extended_problem_details->type) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_extended_problem_details_convertToJSON() failed [type]");
			goto end;
		}
	}
	...
	if(extended_problem_details->title) {
		if(cJSON_AddStringToObject(item, "title",
		                           extended_problem_details->title) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_extended_problem_details_convertToJSON() failed [title]");
			goto end;
		}
	}
	...
	if(extended_problem_details->is_status) {
		if(cJSON_AddNumberToObject(item, "status",
		                           extended_problem_details->status) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_extended_problem_details_convertToJSON() failed [status]");
			goto end;
		}
	}
	...
	if(extended_problem_details->detail) {
		if(cJSON_AddStringToObject(item, "detail",
		                           extended_problem_details->detail) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_extended_problem_details_convertToJSON() failed [detail]");
			goto end;
		}
	}
	...
	if(extended_problem_details->instance) {
		if(cJSON_AddStringToObject(item, "instance",
		                           extended_problem_details->instance)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_extended_problem_details_convertToJSON() failed [instance]");
			goto end;
		}
	}
	...
	if(extended_problem_details->cause) {
		if(cJSON_AddStringToObject(item, "cause",
		                           extended_problem_details->cause) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_extended_problem_details_convertToJSON() failed [cause]");
			goto end;
		}
	}
	...
	if(extended_problem_details->invalid_params) {
		...
		if(invalid_paramsList == NULL) {
			ogs_error(
				"OpenAPI_extended_problem_details_convertToJSON() failed [invalid_params]");
			goto end;
		}
		OpenAPI_list_for_each(extended_problem_details->invalid_params,
		                      node) {
	...
	if(extended_problem_details->supported_features) {
		if(cJSON_AddStringToObject(item, "supportedFeatures",
		                           extended_problem_details->
		                           supported_features) == NULL)
		{
			ogs_error(
				"OpenAPI_extended_problem_details_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(extended_problem_details->access_token_error) {
		...
		if(access_token_error_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_extended_problem_details_convertToJSON() failed [access_token_error]");
			goto end;
		}
	...
	if(extended_problem_details->access_token_request) {
		...
		if(access_token_request_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_extended_problem_details_convertToJSON() failed [access_token_request]");
			goto end;
		}
	...
	if(extended_problem_details->nrf_id) {
		if(cJSON_AddStringToObject(item, "nrfId",
		                           extended_problem_details->nrf_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_extended_problem_details_convertToJSON() failed [nrf_id]");
			goto end;
		}
	}

	if(extended_problem_details->acceptable_serv_info) {
		cJSON *acceptable_serv_info_local_JSON =
			OpenAPI_acceptable_service_info_convertToJSON(
				extended_problem_details->acceptable_serv_info);
		if(acceptable_serv_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_extended_problem_details_convertToJSON() failed [acceptable_serv_info]");
			goto end;
		}
		cJSON_AddItemToObject(item, "acceptableServInfo",
		                      acceptable_serv_info_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_extended_problem_details_convertToJSON() failed [acceptable_serv_info]");
			goto end;
...
OpenAPI_extended_problem_details_parseFromJSON(
	cJSON *extended_problem_detailsJSON) {
	OpenAPI_extended_problem_details_t *extended_problem_details_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *type = NULL;
	...
	OpenAPI_acceptable_service_info_t *acceptable_serv_info_local_nonprim =
		NULL;
	type = cJSON_GetObjectItemCaseSensitive(extended_problem_detailsJSON,
	                                        "type");
	if(type) {
		...
		{
			ogs_error(
				"OpenAPI_extended_problem_details_parseFromJSON() failed [type]");
			goto end;
		}
	}

	title = cJSON_GetObjectItemCaseSensitive(extended_problem_detailsJSON,
	                                         "title");
	if(title) {
		...
		{
			ogs_error(
				"OpenAPI_extended_problem_details_parseFromJSON() failed [title]");
			goto end;
		}
	}

	status = cJSON_GetObjectItemCaseSensitive(extended_problem_detailsJSON,
	                                          "status");
	if(status) {
		if(!cJSON_IsNumber(status)) {
			ogs_error(
				"OpenAPI_extended_problem_details_parseFromJSON() failed [status]");
			goto end;
		}
	}

	detail = cJSON_GetObjectItemCaseSensitive(extended_problem_detailsJSON,
	                                          "detail");
	if(detail) {
		...
		{
			ogs_error(
				"OpenAPI_extended_problem_details_parseFromJSON() failed [detail]");
			goto end;
		}
	}
	...
	if(instance) {
		...
		{
			ogs_error(
				"OpenAPI_extended_problem_details_parseFromJSON() failed [instance]");
			goto end;
		}
	}

	cause = cJSON_GetObjectItemCaseSensitive(extended_problem_detailsJSON,
	                                         "cause");
	if(cause) {
		...
		{
			ogs_error(
				"OpenAPI_extended_problem_details_parseFromJSON() failed [cause]");
			goto end;
		}
	}
	...
	if(invalid_params) {
		...
		if(!cJSON_IsArray(invalid_params)) {
			ogs_error(
				"OpenAPI_extended_problem_details_parseFromJSON() failed [invalid_params]");
			goto end;
		}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_extended_problem_details_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	nrf_id = cJSON_GetObjectItemCaseSensitive(extended_problem_detailsJSON,
	                                          "nrfId");
	if(nrf_id) {
		...
		{
			ogs_error(
				"OpenAPI_extended_problem_details_parseFromJSON() failed [nrf_id]");
			goto end;
		}
	}
	...
	if(acceptable_serv_info) {
		acceptable_serv_info_local_nonprim =
			OpenAPI_acceptable_service_info_parseFromJSON(
				acceptable_serv_info);
		if(!acceptable_serv_info_local_nonprim) {
			ogs_error(
				"OpenAPI_acceptable_service_info_parseFromJSON failed [acceptable_serv_info]");
			goto end;
		}
	}

	extended_problem_details_local_var =
...
OpenAPI_extended_problem_details_t *OpenAPI_extended_problem_details_copy(
	OpenAPI_extended_problem_details_t	*dst,
	OpenAPI_extended_problem_details_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_extended_problem_details_convertToJSON() failed");
		return NULL;
	}
