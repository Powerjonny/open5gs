...
OpenAPI_am_influ_data_t *OpenAPI_am_influ_data_create(OpenAPI_list_t *app_ids,
                                                      OpenAPI_list_t *
                                                      dnn_snssai_infos,
                                                      char		*
                                                                inter_group_id,
                                                      ...
                                                      OpenAPI_list_t	*
                                                      reset_ids) {
	OpenAPI_am_influ_data_t *am_influ_data_local_var =
		ogs_malloc(sizeof(OpenAPI_am_influ_data_t));
...
cJSON *OpenAPI_am_influ_data_convertToJSON(
	...
	if(am_influ_data == NULL) {
		ogs_error(
			"OpenAPI_am_influ_data_convertToJSON() failed [AmInfluData]");
		return NULL;
	}
	...
	if(am_influ_data->app_ids) {
		...
		if(app_idsList == NULL) {
			ogs_error(
				"OpenAPI_am_influ_data_convertToJSON() failed [app_ids]");
			goto end;
		}
		OpenAPI_list_for_each(am_influ_data->app_ids, node) {
	...
	if(am_influ_data->dnn_snssai_infos) {
		cJSON *dnn_snssai_infosList = cJSON_AddArrayToObject(item,
		                                                     "dnnSnssaiInfos");
		if(dnn_snssai_infosList == NULL) {
			ogs_error(
				"OpenAPI_am_influ_data_convertToJSON() failed [dnn_snssai_infos]");
			goto end;
		}
		OpenAPI_list_for_each(am_influ_data->dnn_snssai_infos, node) {
			cJSON *itemLocal =
				OpenAPI_dnn_snssai_information_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_am_influ_data_convertToJSON() failed [dnn_snssai_infos]");
	...
	if(am_influ_data->inter_group_id) {
		if(cJSON_AddStringToObject(item, "interGroupId",
		                           am_influ_data->inter_group_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_am_influ_data_convertToJSON() failed [inter_group_id]");
			goto end;
		}
	}
	...
	if(am_influ_data->supi) {
		...
		{
			ogs_error(
				"OpenAPI_am_influ_data_convertToJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(am_influ_data->is_any_ue_ind) {
		if(cJSON_AddBoolToObject(item, "anyUeInd",
		                         am_influ_data->any_ue_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_am_influ_data_convertToJSON() failed [any_ue_ind]");
			goto end;
		}
	}

	if(am_influ_data->is_policy_duration) {
		if(cJSON_AddNumberToObject(item, "policyDuration",
		                           am_influ_data->policy_duration) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_am_influ_data_convertToJSON() failed [policy_duration]");
			goto end;
		}
	}
	...
	if(am_influ_data->ev_subs) {
		...
		if(ev_subsList == NULL) {
			ogs_error(
				"OpenAPI_am_influ_data_convertToJSON() failed [ev_subs]");
			goto end;
		}
		OpenAPI_list_for_each(am_influ_data->ev_subs, node) {
			cJSON *itemLocal =
				OpenAPI_am_influ_event_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_am_influ_data_convertToJSON() failed [ev_subs]");
	...
	if(am_influ_data->notif_uri) {
		...
		{
			ogs_error(
				"OpenAPI_am_influ_data_convertToJSON() failed [notif_uri]");
			goto end;
		}
	}
	...
	if(am_influ_data->notif_corr_id) {
		if(cJSON_AddStringToObject(item, "notifCorrId",
		                           am_influ_data->notif_corr_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_am_influ_data_convertToJSON() failed [notif_corr_id]");
			goto end;
		}
	}
	...
	if(am_influ_data->headers) {
		...
		if(headersList == NULL) {
			ogs_error(
				"OpenAPI_am_influ_data_convertToJSON() failed [headers]");
			goto end;
		}
		OpenAPI_list_for_each(am_influ_data->headers, node) {
	...
	if(am_influ_data->is_thru_req) {
		if(cJSON_AddBoolToObject(item, "thruReq",
		                         am_influ_data->thru_req) == NULL)
		{
			ogs_error(
				"OpenAPI_am_influ_data_convertToJSON() failed [thru_req]");
			goto end;
		}
	}
	...
	if(am_influ_data->cov_req) {
		...
		if(cov_reqList == NULL) {
			ogs_error(
				"OpenAPI_am_influ_data_convertToJSON() failed [cov_req]");
			goto end;
		}
		OpenAPI_list_for_each(am_influ_data->cov_req, node) {
			cJSON *itemLocal =
	...
	if(am_influ_data->supported_features) {
		if(cJSON_AddStringToObject(item, "supportedFeatures",
		                           am_influ_data->supported_features) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_am_influ_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(am_influ_data->res_uri) {
		...
		{
			ogs_error(
				"OpenAPI_am_influ_data_convertToJSON() failed [res_uri]");
			goto end;
		}
	}
	...
	if(am_influ_data->reset_ids) {
		...
		if(reset_idsList == NULL) {
			ogs_error(
				"OpenAPI_am_influ_data_convertToJSON() failed [reset_ids]");
			goto end;
		}
		OpenAPI_list_for_each(am_influ_data->reset_ids, node) {
...
OpenAPI_am_influ_data_t *OpenAPI_am_influ_data_parseFromJSON(
	cJSON *am_influ_dataJSON) {
	...
	if(app_ids) {
		...
		if(!cJSON_IsArray(app_ids)) {
			ogs_error(
				"OpenAPI_am_influ_data_parseFromJSON() failed [app_ids]");
			goto end;
		}
	...
	dnn_snssai_infos = cJSON_GetObjectItemCaseSensitive(am_influ_dataJSON,
	                                                    "dnnSnssaiInfos");
	if(dnn_snssai_infos) {
		cJSON *dnn_snssai_infos_local = NULL;
		if(!cJSON_IsArray(dnn_snssai_infos)) {
			ogs_error(
				"OpenAPI_am_influ_data_parseFromJSON() failed [dnn_snssai_infos]");
			goto end;
		}
		...
		cJSON_ArrayForEach(dnn_snssai_infos_local, dnn_snssai_infos) {
			...
			OpenAPI_dnn_snssai_information_t *dnn_snssai_infosItem =
				OpenAPI_dnn_snssai_information_parseFromJSON(
					dnn_snssai_infos_local);
			if(!dnn_snssai_infosItem) {
				ogs_error("No dnn_snssai_infosItem");
				goto end;
			}
			OpenAPI_list_add(dnn_snssai_infosList,
			                 dnn_snssai_infosItem);
		}
	}

	inter_group_id = cJSON_GetObjectItemCaseSensitive(am_influ_dataJSON,
	                                                  "interGroupId");
	if(inter_group_id) {
		...
		{
			ogs_error(
				"OpenAPI_am_influ_data_parseFromJSON() failed [inter_group_id]");
			goto end;
		}
	}
	...
	if(supi) {
		...
		{
			ogs_error(
				"OpenAPI_am_influ_data_parseFromJSON() failed [supi]");
			goto end;
		}
	}

	any_ue_ind = cJSON_GetObjectItemCaseSensitive(am_influ_dataJSON,
	                                              "anyUeInd");
	if(any_ue_ind) {
		if(!cJSON_IsBool(any_ue_ind)) {
			ogs_error(
				"OpenAPI_am_influ_data_parseFromJSON() failed [any_ue_ind]");
			goto end;
		}
	}

	policy_duration = cJSON_GetObjectItemCaseSensitive(am_influ_dataJSON,
	                                                   "policyDuration");
	if(policy_duration) {
		if(!cJSON_IsNumber(policy_duration)) {
			ogs_error(
				"OpenAPI_am_influ_data_parseFromJSON() failed [policy_duration]");
			goto end;
		}
	}
	...
	if(ev_subs) {
		...
		if(!cJSON_IsArray(ev_subs)) {
			ogs_error(
				"OpenAPI_am_influ_data_parseFromJSON() failed [ev_subs]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ev_subs_local, ev_subs) {
			...
			OpenAPI_am_influ_event_t *ev_subsItem =
				OpenAPI_am_influ_event_parseFromJSON(
					ev_subs_local);
			if(!ev_subsItem) {
	...
	notif_uri = cJSON_GetObjectItemCaseSensitive(am_influ_dataJSON,
	                                             "notifUri");
	if(notif_uri) {
		...
		{
			ogs_error(
				"OpenAPI_am_influ_data_parseFromJSON() failed [notif_uri]");
			goto end;
		}
	}

	notif_corr_id = cJSON_GetObjectItemCaseSensitive(am_influ_dataJSON,
	                                                 "notifCorrId");
	if(notif_corr_id) {
		...
		{
			ogs_error(
				"OpenAPI_am_influ_data_parseFromJSON() failed [notif_corr_id]");
			goto end;
		}
	}

	headers = cJSON_GetObjectItemCaseSensitive(am_influ_dataJSON,
	                                           "headers");
	if(headers) {
		cJSON *headers_local = NULL;
		if(!cJSON_IsArray(headers)) {
			ogs_error(
				"OpenAPI_am_influ_data_parseFromJSON() failed [headers]");
			goto end;
		}
	...
	thru_req = cJSON_GetObjectItemCaseSensitive(am_influ_dataJSON,
	                                            "thruReq");
	if(thru_req) {
		if(!cJSON_IsBool(thru_req)) {
			ogs_error(
				"OpenAPI_am_influ_data_parseFromJSON() failed [thru_req]");
			goto end;
		}
	}
	...
	if(cov_req) {
		...
		if(!cJSON_IsArray(cov_req)) {
			ogs_error(
				"OpenAPI_am_influ_data_parseFromJSON() failed [cov_req]");
			goto end;
		}
		...
		cJSON_ArrayForEach(cov_req_local, cov_req) {
			...
			OpenAPI_service_area_coverage_info_t *cov_reqItem =
	...
	supported_features = cJSON_GetObjectItemCaseSensitive(am_influ_dataJSON,
	                                                      "supportedFeatures");
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_am_influ_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(res_uri) {
		...
		{
			ogs_error(
				"OpenAPI_am_influ_data_parseFromJSON() failed [res_uri]");
			goto end;
		}
	}

	reset_ids = cJSON_GetObjectItemCaseSensitive(am_influ_dataJSON,
	                                             "resetIds");
	if(reset_ids) {
		cJSON *reset_ids_local = NULL;
		if(!cJSON_IsArray(reset_ids)) {
			ogs_error(
				"OpenAPI_am_influ_data_parseFromJSON() failed [reset_ids]");
			goto end;
		}
	...
	am_influ_data_local_var = OpenAPI_am_influ_data_create(
		...
		notif_corr_id &&
		!cJSON_IsNull(notif_corr_id) ?
		ogs_strdup(notif_corr_id->valuestring) : NULL,
...
OpenAPI_am_influ_data_t *OpenAPI_am_influ_data_copy(
	OpenAPI_am_influ_data_t *dst, OpenAPI_am_influ_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
