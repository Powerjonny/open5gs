...
OpenAPI_snssai_smf_info_item_t *OpenAPI_snssai_smf_info_item_create(
	OpenAPI_ext_snssai_t *s_nssai, OpenAPI_list_t *dnn_smf_info_list) {
	OpenAPI_snssai_smf_info_item_t *snssai_smf_info_item_local_var =
		ogs_malloc(sizeof(OpenAPI_snssai_smf_info_item_t));
...
void OpenAPI_snssai_smf_info_item_free(
	OpenAPI_snssai_smf_info_item_t *snssai_smf_info_item) {
	...
	if(snssai_smf_info_item->dnn_smf_info_list) {
		OpenAPI_list_for_each(snssai_smf_info_item->dnn_smf_info_list,
		                      node) {
...
cJSON *OpenAPI_snssai_smf_info_item_convertToJSON(
	OpenAPI_snssai_smf_info_item_t *snssai_smf_info_item) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(snssai_smf_info_item == NULL) {
		ogs_error(
			"OpenAPI_snssai_smf_info_item_convertToJSON() failed [SnssaiSmfInfoItem]");
		return NULL;
	}
	...
	if(!snssai_smf_info_item->s_nssai) {
		ogs_error(
			"OpenAPI_snssai_smf_info_item_convertToJSON() failed [s_nssai]");
		return NULL;
	}
	cJSON *s_nssai_local_JSON =
		OpenAPI_ext_snssai_convertToJSON(snssai_smf_info_item->s_nssai);
	...
	if(!snssai_smf_info_item->dnn_smf_info_list) {
		ogs_error(
			"OpenAPI_snssai_smf_info_item_convertToJSON() failed [dnn_smf_info_list]");
		return NULL;
	}
	cJSON *dnn_smf_info_listList = cJSON_AddArrayToObject(item,
	                                                      "dnnSmfInfoList");
	...
	OpenAPI_list_for_each(snssai_smf_info_item->dnn_smf_info_list, node) {
		cJSON *itemLocal =
			OpenAPI_dnn_smf_info_item_convertToJSON(node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_snssai_smf_info_item_convertToJSON() failed [dnn_smf_info_list]");
			goto end;
...
OpenAPI_snssai_smf_info_item_t *OpenAPI_snssai_smf_info_item_parseFromJSON(
	cJSON *snssai_smf_info_itemJSON) {
	...
	s_nssai = cJSON_GetObjectItemCaseSensitive(snssai_smf_info_itemJSON,
	                                           "sNssai");
	if(!s_nssai) {
		ogs_error(
			"OpenAPI_snssai_smf_info_item_parseFromJSON() failed [s_nssai]");
		goto end;
	}
	...
	dnn_smf_info_list =
		cJSON_GetObjectItemCaseSensitive(snssai_smf_info_itemJSON,
		                                 "dnnSmfInfoList");
	if(!dnn_smf_info_list) {
		ogs_error(
			"OpenAPI_snssai_smf_info_item_parseFromJSON() failed [dnn_smf_info_list]");
		goto end;
	}
	...
	cJSON_ArrayForEach(dnn_smf_info_list_local, dnn_smf_info_list) {
		...
		OpenAPI_dnn_smf_info_item_t *dnn_smf_info_listItem =
			OpenAPI_dnn_smf_info_item_parseFromJSON(
				dnn_smf_info_list_local);
...
OpenAPI_snssai_smf_info_item_t *OpenAPI_snssai_smf_info_item_copy(
	OpenAPI_snssai_smf_info_item_t	*dst,
	OpenAPI_snssai_smf_info_item_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_snssai_smf_info_item_convertToJSON() failed");
		return NULL;
	}
