...
OpenAPI_transfer_policy_t *OpenAPI_transfer_policy_create(
	...
	int	trans_policy_id)
{
	OpenAPI_transfer_policy_t *transfer_policy_local_var =
		ogs_malloc(sizeof(OpenAPI_transfer_policy_t));
...
cJSON *OpenAPI_transfer_policy_convertToJSON(
	OpenAPI_transfer_policy_t *transfer_policy) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(transfer_policy == NULL) {
		ogs_error(
			"OpenAPI_transfer_policy_convertToJSON() failed [TransferPolicy]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(transfer_policy->max_bit_rate_dl) {
		if(cJSON_AddStringToObject(item, "maxBitRateDl",
		                           transfer_policy->max_bit_rate_dl) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_transfer_policy_convertToJSON() failed [max_bit_rate_dl]");
			goto end;
		}
	}

	if(transfer_policy->max_bit_rate_ul) {
		if(cJSON_AddStringToObject(item, "maxBitRateUl",
		                           transfer_policy->max_bit_rate_ul) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_transfer_policy_convertToJSON() failed [max_bit_rate_ul]");
			goto end;
		}
	}

	if(cJSON_AddNumberToObject(item, "ratingGroup",
	                           transfer_policy->rating_group) == NULL)
	{
		ogs_error(
			"OpenAPI_transfer_policy_convertToJSON() failed [rating_group]");
		goto end;
	}

	if(!transfer_policy->rec_time_int) {
		ogs_error(
			"OpenAPI_transfer_policy_convertToJSON() failed [rec_time_int]");
		return NULL;
	}
	cJSON *rec_time_int_local_JSON =
		OpenAPI_time_window_convertToJSON(
			transfer_policy->rec_time_int);
	...
	if(cJSON_AddNumberToObject(item, "transPolicyId",
	                           transfer_policy->trans_policy_id) == NULL)
	{
		ogs_error(
			"OpenAPI_transfer_policy_convertToJSON() failed [trans_policy_id]");
		goto end;
	}
...
OpenAPI_transfer_policy_t *OpenAPI_transfer_policy_parseFromJSON(
	cJSON *transfer_policyJSON) {
	...
	max_bit_rate_dl = cJSON_GetObjectItemCaseSensitive(transfer_policyJSON,
	                                                   "maxBitRateDl");
	if(max_bit_rate_dl) {
		if(!cJSON_IsString(max_bit_rate_dl) &&
		   !cJSON_IsNull(max_bit_rate_dl))
		{
			ogs_error(
				"OpenAPI_transfer_policy_parseFromJSON() failed [max_bit_rate_dl]");
			goto end;
		}
	}

	max_bit_rate_ul = cJSON_GetObjectItemCaseSensitive(transfer_policyJSON,
	                                                   "maxBitRateUl");
	if(max_bit_rate_ul) {
		if(!cJSON_IsString(max_bit_rate_ul) &&
		   !cJSON_IsNull(max_bit_rate_ul))
		{
			ogs_error(
				"OpenAPI_transfer_policy_parseFromJSON() failed [max_bit_rate_ul]");
			goto end;
		}
	}
	...
	if(!rating_group) {
		ogs_error(
			"OpenAPI_transfer_policy_parseFromJSON() failed [rating_group]");
		goto end;
	}
	...
	rec_time_int = cJSON_GetObjectItemCaseSensitive(transfer_policyJSON,
	                                                "recTimeInt");
	if(!rec_time_int) {
		ogs_error(
			"OpenAPI_transfer_policy_parseFromJSON() failed [rec_time_int]");
		goto end;
	}
	rec_time_int_local_nonprim =
		OpenAPI_time_window_parseFromJSON(rec_time_int);
	if(!rec_time_int_local_nonprim) {
		ogs_error(
			"OpenAPI_time_window_parseFromJSON failed [rec_time_int]");
		goto end;
	}

	trans_policy_id = cJSON_GetObjectItemCaseSensitive(transfer_policyJSON,
	                                                   "transPolicyId");
	if(!trans_policy_id) {
		ogs_error(
			"OpenAPI_transfer_policy_parseFromJSON() failed [trans_policy_id]");
		goto end;
	}
	...
	transfer_policy_local_var = OpenAPI_transfer_policy_create(
		max_bit_rate_dl &&
		!cJSON_IsNull(max_bit_rate_dl) ?
		ogs_strdup(max_bit_rate_dl->valuestring) : NULL,
		max_bit_rate_ul &&
		!cJSON_IsNull(max_bit_rate_ul) ?
		ogs_strdup(max_bit_rate_ul->valuestring) : NULL,
...
OpenAPI_transfer_policy_t *OpenAPI_transfer_policy_copy(
	OpenAPI_transfer_policy_t *dst, OpenAPI_transfer_policy_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
