...
OpenAPI_ptw_parameters_1_t *OpenAPI_ptw_parameters_1_create(
	...
	OpenAPI_ptw_parameters_1_t *ptw_parameters_1_local_var =
		ogs_malloc(sizeof(OpenAPI_ptw_parameters_1_t));
...
cJSON *OpenAPI_ptw_parameters_1_convertToJSON(
	...
	if(ptw_parameters_1 == NULL) {
		ogs_error(
			"OpenAPI_ptw_parameters_1_convertToJSON() failed [PtwParameters_1]");
		return NULL;
	}
	...
	if(ptw_parameters_1->operation_mode == OpenAPI_operation_mode_NULL) {
		ogs_error(
			"OpenAPI_ptw_parameters_1_convertToJSON() failed [operation_mode]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "operationMode",
	                           OpenAPI_operation_mode_ToString(
					   ptw_parameters_1->operation_mode)) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_ptw_parameters_1_convertToJSON() failed [operation_mode]");
	...
	if(!ptw_parameters_1->ptw_value) {
		ogs_error(
			"OpenAPI_ptw_parameters_1_convertToJSON() failed [ptw_value]");
		return NULL;
	}
...
OpenAPI_ptw_parameters_1_t *OpenAPI_ptw_parameters_1_parseFromJSON(
	...
	operation_mode = cJSON_GetObjectItemCaseSensitive(ptw_parameters_1JSON,
	                                                  "operationMode");
	if(!operation_mode) {
		ogs_error(
			"OpenAPI_ptw_parameters_1_parseFromJSON() failed [operation_mode]");
		goto end;
	}
	...
	ptw_value = cJSON_GetObjectItemCaseSensitive(ptw_parameters_1JSON,
	                                             "ptwValue");
	if(!ptw_value) {
		ogs_error(
			"OpenAPI_ptw_parameters_1_parseFromJSON() failed [ptw_value]");
		goto end;
	}
