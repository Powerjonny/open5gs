...
OpenAPI_uc_subscription_data_t *OpenAPI_uc_subscription_data_create(
	...
	OpenAPI_uc_subscription_data_t *uc_subscription_data_local_var =
		ogs_malloc(sizeof(OpenAPI_uc_subscription_data_t));
	ogs_assert(uc_subscription_data_local_var);

	uc_subscription_data_local_var->user_consent_per_purpose_list =
		user_consent_per_purpose_list;

	return uc_subscription_data_local_var;
}

void OpenAPI_uc_subscription_data_free(
	OpenAPI_uc_subscription_data_t *uc_subscription_data) {
	...
	if(uc_subscription_data->user_consent_per_purpose_list) {
		OpenAPI_list_for_each(
			uc_subscription_data->user_consent_per_purpose_list,
			node) {
...
cJSON *OpenAPI_uc_subscription_data_convertToJSON(
	OpenAPI_uc_subscription_data_t *uc_subscription_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(uc_subscription_data == NULL) {
		ogs_error(
			"OpenAPI_uc_subscription_data_convertToJSON() failed [UcSubscriptionData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(uc_subscription_data->user_consent_per_purpose_list !=
	   OpenAPI_user_consent_NULL)
	{
		...
		if(user_consent_per_purpose_list == NULL) {
			ogs_error(
				"OpenAPI_uc_subscription_data_convertToJSON() failed [user_consent_per_purpose_list]");
			goto end;
		}
		cJSON *localMapObject = user_consent_per_purpose_list;
		if(uc_subscription_data->user_consent_per_purpose_list) {
			OpenAPI_list_for_each(
				uc_subscription_data->
				user_consent_per_purpose_list, node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
...
OpenAPI_uc_subscription_data_t *OpenAPI_uc_subscription_data_parseFromJSON(
	cJSON *uc_subscription_dataJSON) {
	...
	user_consent_per_purpose_list =
		cJSON_GetObjectItemCaseSensitive(uc_subscription_dataJSON,
		                                 "userConsentPerPurposeList");
	if(user_consent_per_purpose_list) {
		...
		if(!cJSON_IsObject(user_consent_per_purpose_list) &&
		   !cJSON_IsNull(user_consent_per_purpose_list))
		{
			ogs_error(
				"OpenAPI_uc_subscription_data_parseFromJSON() failed [user_consent_per_purpose_list]");
			goto end;
		}
		if(cJSON_IsObject(user_consent_per_purpose_list)) {
			user_consent_per_purpose_listList =
				OpenAPI_list_create();
			OpenAPI_map_t *localMapKeyPair = NULL;
			cJSON_ArrayForEach(
				user_consent_per_purpose_list_local_map,
				user_consent_per_purpose_list) {
				...
				localMapKeyPair =
					OpenAPI_map_create(ogs_strdup(
								   ...
								   ->string),
					                   (void *)
					                   OpenAPI_user_consent_FromString
							   (
								   localMapObject
								   ->string));
				OpenAPI_list_add(
...
OpenAPI_uc_subscription_data_t *OpenAPI_uc_subscription_data_copy(
	OpenAPI_uc_subscription_data_t	*dst,
	OpenAPI_uc_subscription_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_uc_subscription_data_convertToJSON() failed");
		return NULL;
	}
