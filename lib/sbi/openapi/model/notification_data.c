...
OpenAPI_notification_data_t *OpenAPI_notification_data_create(
	OpenAPI_notification_event_type_e event, char *nf_instance_uri,
	OpenAPI_nf_profile_t *nf_profile, OpenAPI_list_t *profile_changes,
	OpenAPI_condition_event_type_e condition_event,
	OpenAPI_subscription_context_t *subscription_context) {
	OpenAPI_notification_data_t *notification_data_local_var =
		ogs_malloc(sizeof(OpenAPI_notification_data_t));
	...
	notification_data_local_var->subscription_context =
		subscription_context;

	return notification_data_local_var;
}

void OpenAPI_notification_data_free(
	OpenAPI_notification_data_t *notification_data) {
	...
	if(notification_data->profile_changes) {
		OpenAPI_list_for_each(notification_data->profile_changes,
		                      node) {
	...
	if(notification_data->subscription_context) {
		OpenAPI_subscription_context_free(
			notification_data->subscription_context);
		notification_data->subscription_context = NULL;
	}
	ogs_free(notification_data);
}

cJSON *OpenAPI_notification_data_convertToJSON(
	OpenAPI_notification_data_t *notification_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(notification_data == NULL) {
		ogs_error(
			"OpenAPI_notification_data_convertToJSON() failed [NotificationData]");
		return NULL;
	}
	...
	if(notification_data->event == OpenAPI_notification_event_type_NULL) {
		ogs_error(
			"OpenAPI_notification_data_convertToJSON() failed [event]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "event",
	                           OpenAPI_notification_event_type_ToString(
					   notification_data->event)) == NULL)
	{
		ogs_error(
			"OpenAPI_notification_data_convertToJSON() failed [event]");
		goto end;
	...
	if(!notification_data->nf_instance_uri) {
		ogs_error(
			"OpenAPI_notification_data_convertToJSON() failed [nf_instance_uri]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "nfInstanceUri",
	                           notification_data->nf_instance_uri) == NULL)
	{
		ogs_error(
			"OpenAPI_notification_data_convertToJSON() failed [nf_instance_uri]");
	...
	if(notification_data->nf_profile) {
		cJSON *nf_profile_local_JSON =
			OpenAPI_nf_profile_convertToJSON(
				notification_data->nf_profile);
		if(nf_profile_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_notification_data_convertToJSON() failed [nf_profile]");
			goto end;
		}
	...
	if(notification_data->profile_changes) {
		cJSON *profile_changesList = cJSON_AddArrayToObject(item,
		                                                    "profileChanges");
		if(profile_changesList == NULL) {
			ogs_error(
				"OpenAPI_notification_data_convertToJSON() failed [profile_changes]");
			goto end;
		}
		OpenAPI_list_for_each(notification_data->profile_changes,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_change_item_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_notification_data_convertToJSON() failed [profile_changes]");
				goto end;
	...
	if(notification_data->condition_event !=
	   OpenAPI_condition_event_type_NULL)
	{
		if(cJSON_AddStringToObject(item, "conditionEvent",
		                           OpenAPI_condition_event_type_ToString
		                                   (notification_data->
		                                   condition_event)) == NULL)
		{
			ogs_error(
				"OpenAPI_notification_data_convertToJSON() failed [condition_event]");
			goto end;
		}
	}

	if(notification_data->subscription_context) {
		cJSON *subscription_context_local_JSON =
			OpenAPI_subscription_context_convertToJSON(
				notification_data->subscription_context);
		if(subscription_context_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_notification_data_convertToJSON() failed [subscription_context]");
			goto end;
		}
		cJSON_AddItemToObject(item, "subscriptionContext",
		                      subscription_context_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_notification_data_convertToJSON() failed [subscription_context]");
			goto end;
...
OpenAPI_notification_data_t *OpenAPI_notification_data_parseFromJSON(
	cJSON *notification_dataJSON) {
	...
	OpenAPI_subscription_context_t *subscription_context_local_nonprim =
		NULL;
	event = cJSON_GetObjectItemCaseSensitive(notification_dataJSON,
	                                         "event");
	if(!event) {
		ogs_error(
			"OpenAPI_notification_data_parseFromJSON() failed [event]");
		goto end;
	}
	...
	eventVariable =
		OpenAPI_notification_event_type_FromString(event->valuestring);

	nf_instance_uri =
		cJSON_GetObjectItemCaseSensitive(notification_dataJSON,
		                                 "nfInstanceUri");
	if(!nf_instance_uri) {
		ogs_error(
			"OpenAPI_notification_data_parseFromJSON() failed [nf_instance_uri]");
		goto end;
	}
	...
	nf_profile = cJSON_GetObjectItemCaseSensitive(notification_dataJSON,
	                                              "nfProfile");
	if(nf_profile) {
		nf_profile_local_nonprim =
			OpenAPI_nf_profile_parseFromJSON(nf_profile);
		if(!nf_profile_local_nonprim) {
			ogs_error(
				"OpenAPI_nf_profile_parseFromJSON failed [nf_profile]");
			goto end;
		}
	}
	...
	if(profile_changes) {
		...
		if(!cJSON_IsArray(profile_changes)) {
			ogs_error(
				"OpenAPI_notification_data_parseFromJSON() failed [profile_changes]");
			goto end;
		}
		...
		cJSON_ArrayForEach(profile_changes_local, profile_changes) {
			...
			OpenAPI_change_item_t *profile_changesItem =
				OpenAPI_change_item_parseFromJSON(
					profile_changes_local);
			if(!profile_changesItem) {
				ogs_error("No profile_changesItem");
				goto end;
			}
			OpenAPI_list_add(profile_changesList,
			                 profile_changesItem);
		}
	}
	...
	if(condition_event) {
		if(!cJSON_IsString(condition_event)) {
			ogs_error(
				"OpenAPI_notification_data_parseFromJSON() failed [condition_event]");
			goto end;
		}
		condition_eventVariable =
			OpenAPI_condition_event_type_FromString(
				condition_event->valuestring);
	}
	...
	if(subscription_context) {
		subscription_context_local_nonprim =
			OpenAPI_subscription_context_parseFromJSON(
				subscription_context);
		if(!subscription_context_local_nonprim) {
			ogs_error(
				"OpenAPI_subscription_context_parseFromJSON failed [subscription_context]");
			goto end;
		}
	}
...
OpenAPI_notification_data_t *OpenAPI_notification_data_copy(
	OpenAPI_notification_data_t *dst, OpenAPI_notification_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
