...
char * OpenAPI_flow_direction_ToString(
	OpenAPI_flow_direction_e flow_direction) {
	const char *flow_directionArray[] = { "NULL", "DOWNLINK", "UPLINK",
		                              "BIDIRECTIONAL", "UNSPECIFIED" };
	size_t sizeofArray = sizeof(flow_directionArray) /
	                     sizeof(flow_directionArray[0]);
...
OpenAPI_flow_direction_e OpenAPI_flow_direction_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(flow_direction,
		          flow_directionArray[stringToReturn]) == 0) {
