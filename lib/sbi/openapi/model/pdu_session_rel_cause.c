...
char * OpenAPI_pdu_session_rel_cause_ToString(
	...
	const char *pdu_session_rel_causeArray[] = { "NULL", "PS_TO_CS_HO",
		                                     "RULE_ERROR" };
	size_t sizeofArray = sizeof(pdu_session_rel_causeArray) /
	                     sizeof(pdu_session_rel_causeArray[0]);
...
OpenAPI_pdu_session_rel_cause_e OpenAPI_pdu_session_rel_cause_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(pdu_session_rel_cause,
		          pdu_session_rel_causeArray[stringToReturn]) == 0) {
