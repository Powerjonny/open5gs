...
OpenAPI_threshold_value_t *OpenAPI_threshold_value_create(
	...
	OpenAPI_threshold_value_t *threshold_value_local_var =
		ogs_malloc(sizeof(OpenAPI_threshold_value_t));
...
cJSON *OpenAPI_threshold_value_convertToJSON(
	...
	if(threshold_value == NULL) {
		ogs_error(
			"OpenAPI_threshold_value_convertToJSON() failed [ThresholdValue]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(threshold_value->is_rtt_thres) {
		if(cJSON_AddNumberToObject(item, "rttThres",
		                           threshold_value->rtt_thres) == NULL)
		{
			ogs_error(
				"OpenAPI_threshold_value_convertToJSON() failed [rtt_thres]");
			goto end;
		}
	...
	if(threshold_value->is_plr_thres) {
		if(cJSON_AddNumberToObject(item, "plrThres",
		                           threshold_value->plr_thres) == NULL)
		{
			ogs_error(
				"OpenAPI_threshold_value_convertToJSON() failed [plr_thres]");
			goto end;
		}
...
OpenAPI_threshold_value_t *OpenAPI_threshold_value_parseFromJSON(
	...
	rtt_thres = cJSON_GetObjectItemCaseSensitive(threshold_valueJSON,
	                                             "rttThres");
	if(rtt_thres) {
		if(!cJSON_IsNull(rtt_thres)) {
			if(!cJSON_IsNumber(rtt_thres)) {
				ogs_error(
					"OpenAPI_threshold_value_parseFromJSON() failed [rtt_thres]");
				goto end;
			}
		}
	}

	plr_thres = cJSON_GetObjectItemCaseSensitive(threshold_valueJSON,
	                                             "plrThres");
	if(plr_thres) {
		if(!cJSON_IsNull(plr_thres)) {
			if(!cJSON_IsNumber(plr_thres)) {
				ogs_error(
					"OpenAPI_threshold_value_parseFromJSON() failed [plr_thres]");
				goto end;
			}
		}
	}
	...
	threshold_value_local_var = OpenAPI_threshold_value_create(
		rtt_thres &&
		cJSON_IsNull(rtt_thres) ? true : false,
		rtt_thres ? true : false,
		rtt_thres ? rtt_thres->valuedouble : 0,
		plr_thres &&
		cJSON_IsNull(plr_thres) ? true : false,
