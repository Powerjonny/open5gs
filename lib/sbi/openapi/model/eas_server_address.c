...
OpenAPI_eas_server_address_t *OpenAPI_eas_server_address_create(
	...
	OpenAPI_eas_server_address_t *eas_server_address_local_var =
		ogs_malloc(sizeof(OpenAPI_eas_server_address_t));
...
cJSON *OpenAPI_eas_server_address_convertToJSON(
	...
	if(eas_server_address == NULL) {
		ogs_error(
			"OpenAPI_eas_server_address_convertToJSON() failed [EasServerAddress]");
		return NULL;
	}
	...
	if(!eas_server_address->ip) {
		ogs_error(
			"OpenAPI_eas_server_address_convertToJSON() failed [ip]");
		return NULL;
	}
	cJSON *ip_local_JSON =
		OpenAPI_ip_addr_convertToJSON(eas_server_address->ip);
	...
	if(cJSON_AddNumberToObject(item, "port",
	                           eas_server_address->port) == NULL)
	{
		ogs_error(
			"OpenAPI_eas_server_address_convertToJSON() failed [port]");
		goto end;
	}
...
OpenAPI_eas_server_address_t *OpenAPI_eas_server_address_parseFromJSON(
	...
	if(!ip) {
		ogs_error(
			"OpenAPI_eas_server_address_parseFromJSON() failed [ip]");
		goto end;
	}
	...
	if(!port) {
		ogs_error(
			"OpenAPI_eas_server_address_parseFromJSON() failed [port]");
		goto end;
	}
