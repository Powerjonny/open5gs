...
OpenAPI_invalid_param_t *OpenAPI_invalid_param_create(char	*param,
                                                      char	*reason) {
	OpenAPI_invalid_param_t *invalid_param_local_var =
		ogs_malloc(sizeof(OpenAPI_invalid_param_t));
...
cJSON *OpenAPI_invalid_param_convertToJSON(
	...
	if(invalid_param == NULL) {
		ogs_error(
			"OpenAPI_invalid_param_convertToJSON() failed [InvalidParam]");
		return NULL;
	}
	...
	if(!invalid_param->param) {
		ogs_error(
			"OpenAPI_invalid_param_convertToJSON() failed [param]");
		return NULL;
	}
	...
	if(invalid_param->reason) {
		...
		{
			ogs_error(
				"OpenAPI_invalid_param_convertToJSON() failed [reason]");
			goto end;
		}
	}
...
OpenAPI_invalid_param_t *OpenAPI_invalid_param_parseFromJSON(
	...
	if(!param) {
		ogs_error(
			"OpenAPI_invalid_param_parseFromJSON() failed [param]");
		goto end;
	}
	...
	if(reason) {
		...
		{
			ogs_error(
				"OpenAPI_invalid_param_parseFromJSON() failed [reason]");
			goto end;
		}
	}
	...
	invalid_param_local_var = OpenAPI_invalid_param_create(
		...
		reason &&
		!cJSON_IsNull(reason) ? ogs_strdup(reason->valuestring) : NULL
		);
