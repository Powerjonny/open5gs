...
OpenAPI_indirect_data_forwarding_tunnel_info_create(char *ipv4_addr,
                                                    char *ipv6_addr,
                                                    char *gtp_teid,
                                                    bool is_drb_id, int drb_id,
                                                    bool is_additional_tnl_nb,
                                                    int additional_tnl_nb) {
	OpenAPI_indirect_data_forwarding_tunnel_info_t *
	        indirect_data_forwarding_tunnel_info_local_var =
		ogs_malloc(
			sizeof(OpenAPI_indirect_data_forwarding_tunnel_info_t));
	...
	indirect_data_forwarding_tunnel_info_local_var->is_additional_tnl_nb =
		is_additional_tnl_nb;
	indirect_data_forwarding_tunnel_info_local_var->additional_tnl_nb =
		additional_tnl_nb;

	return indirect_data_forwarding_tunnel_info_local_var;
}

void OpenAPI_indirect_data_forwarding_tunnel_info_free(
	OpenAPI_indirect_data_forwarding_tunnel_info_t *
	indirect_data_forwarding_tunnel_info) {
...
cJSON *OpenAPI_indirect_data_forwarding_tunnel_info_convertToJSON(
	OpenAPI_indirect_data_forwarding_tunnel_info_t *
	indirect_data_forwarding_tunnel_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(indirect_data_forwarding_tunnel_info == NULL) {
		ogs_error(
			"OpenAPI_indirect_data_forwarding_tunnel_info_convertToJSON() failed [IndirectDataForwardingTunnelInfo]");
		return NULL;
	}
	...
	if(indirect_data_forwarding_tunnel_info->ipv4_addr) {
		...
		{
			ogs_error(
				"OpenAPI_indirect_data_forwarding_tunnel_info_convertToJSON() failed [ipv4_addr]");
			goto end;
		}
	}

	if(indirect_data_forwarding_tunnel_info->ipv6_addr) {
		if(cJSON_AddStringToObject(item, "ipv6Addr",
		                           indirect_data_forwarding_tunnel_info
		                           ->ipv6_addr) == NULL)
		{
			ogs_error(
				"OpenAPI_indirect_data_forwarding_tunnel_info_convertToJSON() failed [ipv6_addr]");
			goto end;
		}
	}
	...
	if(!indirect_data_forwarding_tunnel_info->gtp_teid) {
		ogs_error(
			"OpenAPI_indirect_data_forwarding_tunnel_info_convertToJSON() failed [gtp_teid]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "gtpTeid",
	                           indirect_data_forwarding_tunnel_info->
	                           gtp_teid) == NULL)
	{
		ogs_error(
	...
	if(indirect_data_forwarding_tunnel_info->is_drb_id) {
		if(cJSON_AddNumberToObject(item, "drbId",
		                           indirect_data_forwarding_tunnel_info
		                           ->drb_id) == NULL)
		{
			ogs_error(
				"OpenAPI_indirect_data_forwarding_tunnel_info_convertToJSON() failed [drb_id]");
			goto end;
		}
	}

	if(indirect_data_forwarding_tunnel_info->is_additional_tnl_nb) {
		if(cJSON_AddNumberToObject(item, "additionalTnlNb",
		                           indirect_data_forwarding_tunnel_info
		                           ->additional_tnl_nb) == NULL)
		{
			ogs_error(
				"OpenAPI_indirect_data_forwarding_tunnel_info_convertToJSON() failed [additional_tnl_nb]");
			goto end;
		}
	}
...
OpenAPI_indirect_data_forwarding_tunnel_info_parseFromJSON(
	cJSON *indirect_data_forwarding_tunnel_infoJSON) {
	OpenAPI_indirect_data_forwarding_tunnel_info_t *
	        indirect_data_forwarding_tunnel_info_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	ipv4_addr =
		cJSON_GetObjectItemCaseSensitive(
			indirect_data_forwarding_tunnel_infoJSON, "ipv4Addr");
	if(ipv4_addr) {
		if(!cJSON_IsString(ipv4_addr) &&
		   !cJSON_IsNull(ipv4_addr))
		{
			ogs_error(
				"OpenAPI_indirect_data_forwarding_tunnel_info_parseFromJSON() failed [ipv4_addr]");
			goto end;
		}
	}
	...
	ipv6_addr =
		cJSON_GetObjectItemCaseSensitive(
			indirect_data_forwarding_tunnel_infoJSON, "ipv6Addr");
	if(ipv6_addr) {
		if(!cJSON_IsString(ipv6_addr) &&
		   !cJSON_IsNull(ipv6_addr))
		{
			ogs_error(
				"OpenAPI_indirect_data_forwarding_tunnel_info_parseFromJSON() failed [ipv6_addr]");
			goto end;
		}
	}
	...
	gtp_teid =
		cJSON_GetObjectItemCaseSensitive(
			indirect_data_forwarding_tunnel_infoJSON, "gtpTeid");
	if(!gtp_teid) {
		ogs_error(
			"OpenAPI_indirect_data_forwarding_tunnel_info_parseFromJSON() failed [gtp_teid]");
		goto end;
	}
	...
	drb_id =
		cJSON_GetObjectItemCaseSensitive(
			indirect_data_forwarding_tunnel_infoJSON, "drbId");
	if(drb_id) {
		if(!cJSON_IsNumber(drb_id)) {
			ogs_error(
				"OpenAPI_indirect_data_forwarding_tunnel_info_parseFromJSON() failed [drb_id]");
			goto end;
		}
	}
	...
	if(additional_tnl_nb) {
		if(!cJSON_IsNumber(additional_tnl_nb)) {
			ogs_error(
				"OpenAPI_indirect_data_forwarding_tunnel_info_parseFromJSON() failed [additional_tnl_nb]");
			goto end;
		}
	}
	...
	indirect_data_forwarding_tunnel_info_local_var =
		OpenAPI_indirect_data_forwarding_tunnel_info_create(
			...
			ipv6_addr &&
			!cJSON_IsNull(ipv6_addr) ?
			ogs_strdup(ipv6_addr->valuestring) :
			NULL,
...
OpenAPI_indirect_data_forwarding_tunnel_info_t *
OpenAPI_indirect_data_forwarding_tunnel_info_copy(
	OpenAPI_indirect_data_forwarding_tunnel_info_t	*dst,
	OpenAPI_indirect_data_forwarding_tunnel_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_indirect_data_forwarding_tunnel_info_convertToJSON() failed");
		return NULL;
	}
