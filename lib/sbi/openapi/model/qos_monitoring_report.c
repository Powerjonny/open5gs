...
OpenAPI_qos_monitoring_report_t *OpenAPI_qos_monitoring_report_create(
	OpenAPI_list_t *flows, OpenAPI_list_t *ul_delays,
	OpenAPI_list_t *dl_delays, OpenAPI_list_t *rt_delays) {
	OpenAPI_qos_monitoring_report_t *qos_monitoring_report_local_var =
		ogs_malloc(sizeof(OpenAPI_qos_monitoring_report_t));
...
void OpenAPI_qos_monitoring_report_free(
	OpenAPI_qos_monitoring_report_t *qos_monitoring_report) {
...
cJSON *OpenAPI_qos_monitoring_report_convertToJSON(
	OpenAPI_qos_monitoring_report_t *qos_monitoring_report) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(qos_monitoring_report == NULL) {
		ogs_error(
			"OpenAPI_qos_monitoring_report_convertToJSON() failed [QosMonitoringReport]");
		return NULL;
	}
	...
	if(qos_monitoring_report->flows) {
		...
		if(flowsList == NULL) {
			ogs_error(
				"OpenAPI_qos_monitoring_report_convertToJSON() failed [flows]");
			goto end;
		}
		OpenAPI_list_for_each(qos_monitoring_report->flows, node) {
			cJSON *itemLocal =
	...
	if(qos_monitoring_report->ul_delays) {
		...
		if(ul_delaysList == NULL) {
			ogs_error(
				"OpenAPI_qos_monitoring_report_convertToJSON() failed [ul_delays]");
			goto end;
		}
		OpenAPI_list_for_each(qos_monitoring_report->ul_delays, node) {
			if(node->data == NULL) {
			...
			if(cJSON_AddNumberToObject(ul_delaysList, "",
			                           *(double *) node->data) ==
			   NULL)
			{
				ogs_error(
	...
	if(qos_monitoring_report->dl_delays) {
		...
		if(dl_delaysList == NULL) {
			ogs_error(
				"OpenAPI_qos_monitoring_report_convertToJSON() failed [dl_delays]");
			goto end;
		}
		OpenAPI_list_for_each(qos_monitoring_report->dl_delays, node) {
			if(node->data == NULL) {
			...
			if(cJSON_AddNumberToObject(dl_delaysList, "",
			                           *(double *) node->data) ==
			   NULL)
			{
				ogs_error(
	...
	if(qos_monitoring_report->rt_delays) {
		...
		if(rt_delaysList == NULL) {
			ogs_error(
				"OpenAPI_qos_monitoring_report_convertToJSON() failed [rt_delays]");
			goto end;
		}
		OpenAPI_list_for_each(qos_monitoring_report->rt_delays, node) {
			if(node->data == NULL) {
			...
			if(cJSON_AddNumberToObject(rt_delaysList, "",
			                           *(double *) node->data) ==
			   NULL)
			{
				ogs_error(
...
OpenAPI_qos_monitoring_report_t *OpenAPI_qos_monitoring_report_parseFromJSON(
	cJSON *qos_monitoring_reportJSON) {
	...
	flows = cJSON_GetObjectItemCaseSensitive(qos_monitoring_reportJSON,
	                                         "flows");
	if(flows) {
		cJSON *flows_local = NULL;
		if(!cJSON_IsArray(flows)) {
			ogs_error(
				"OpenAPI_qos_monitoring_report_parseFromJSON() failed [flows]");
			goto end;
		}
	...
	ul_delays = cJSON_GetObjectItemCaseSensitive(qos_monitoring_reportJSON,
	                                             "ulDelays");
	if(ul_delays) {
		cJSON *ul_delays_local = NULL;
		if(!cJSON_IsArray(ul_delays)) {
			ogs_error(
				"OpenAPI_qos_monitoring_report_parseFromJSON() failed [ul_delays]");
			goto end;
		}
	...
	dl_delays = cJSON_GetObjectItemCaseSensitive(qos_monitoring_reportJSON,
	                                             "dlDelays");
	if(dl_delays) {
		cJSON *dl_delays_local = NULL;
		if(!cJSON_IsArray(dl_delays)) {
			ogs_error(
				"OpenAPI_qos_monitoring_report_parseFromJSON() failed [dl_delays]");
			goto end;
		}
	...
	rt_delays = cJSON_GetObjectItemCaseSensitive(qos_monitoring_reportJSON,
	                                             "rtDelays");
	if(rt_delays) {
		cJSON *rt_delays_local = NULL;
		if(!cJSON_IsArray(rt_delays)) {
			ogs_error(
				"OpenAPI_qos_monitoring_report_parseFromJSON() failed [rt_delays]");
			goto end;
		}
...
OpenAPI_qos_monitoring_report_t *OpenAPI_qos_monitoring_report_copy(
	OpenAPI_qos_monitoring_report_t *dst,
	OpenAPI_qos_monitoring_report_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_qos_monitoring_report_convertToJSON() failed");
		return NULL;
	}
