...
OpenAPI_session_management_subscription_data_create(
	...
	OpenAPI_session_management_subscription_data_t *
		...
		ogs_malloc(
			sizeof(OpenAPI_session_management_subscription_data_t));
	ogs_assert(session_management_subscription_data_local_var);

	session_management_subscription_data_local_var->single_nssai =
		single_nssai;
	session_management_subscription_data_local_var->dnn_configurations =
		dnn_configurations;
	session_management_subscription_data_local_var->internal_group_ids =
		internal_group_ids;
	...
	session_management_subscription_data_local_var->odb_packet_services =
		odb_packet_services;
	session_management_subscription_data_local_var->is_trace_data_null =
		is_trace_data_null;
	session_management_subscription_data_local_var->trace_data = trace_data;
	session_management_subscription_data_local_var->shared_trace_data_id =
	...
	session_management_subscription_data_local_var->supported_features =
		supported_features;

	return session_management_subscription_data_local_var;
}

void OpenAPI_session_management_subscription_data_free(
	OpenAPI_session_management_subscription_data_t *
	...
	if(session_management_subscription_data->internal_group_ids) {
		OpenAPI_list_for_each(
			session_management_subscription_data->internal_group_ids,
			node) {
			ogs_free(node->data);
		}
		OpenAPI_list_free(
	...
	if(session_management_subscription_data->shared_vn_group_data_ids) {
		...
		OpenAPI_list_free(
			session_management_subscription_data->
			shared_vn_group_data_ids);
		session_management_subscription_data->shared_vn_group_data_ids =
			NULL;
	}
	...
	if(session_management_subscription_data->trace_data) {
		OpenAPI_trace_data_free(
			session_management_subscription_data->trace_data);
	...
	if(session_management_subscription_data->supported_features) {
		ogs_free(
			session_management_subscription_data->supported_features);
...
cJSON *OpenAPI_session_management_subscription_data_convertToJSON(
	...
	if(session_management_subscription_data == NULL) {
		ogs_error(
			"OpenAPI_session_management_subscription_data_convertToJSON() failed [SessionManagementSubscriptionData]");
		return NULL;
	}
	...
	if(!session_management_subscription_data->single_nssai) {
		ogs_error(
			"OpenAPI_session_management_subscription_data_convertToJSON() failed [single_nssai]");
		return NULL;
	}
	...
	if(session_management_subscription_data->dnn_configurations) {
		...
		if(dnn_configurations == NULL) {
			ogs_error(
				"OpenAPI_session_management_subscription_data_convertToJSON() failed [dnn_configurations]");
			goto end;
		}
		...
		if(session_management_subscription_data->dnn_configurations) {
			OpenAPI_list_for_each(
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_dnn_configuration_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_session_management_subscription_data_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(session_management_subscription_data->internal_group_ids) {
		...
		if(internal_group_idsList == NULL) {
			ogs_error(
				"OpenAPI_session_management_subscription_data_convertToJSON() failed [internal_group_ids]");
			goto end;
		}
		OpenAPI_list_for_each(
			session_management_subscription_data->internal_group_ids,
	...
	if(session_management_subscription_data->shared_vn_group_data_ids) {
		...
		if(shared_vn_group_data_ids == NULL) {
			ogs_error(
				"OpenAPI_session_management_subscription_data_convertToJSON() failed [shared_vn_group_data_ids]");
			goto end;
		}
		cJSON *localMapObject = shared_vn_group_data_ids;
		if(session_management_subscription_data->
		   shared_vn_group_data_ids)
		{
			OpenAPI_list_for_each(
				session_management_subscription_data->
	...
	if(session_management_subscription_data->shared_dnn_configurations_id) {
		...
		{
			ogs_error(
				"OpenAPI_session_management_subscription_data_convertToJSON() failed [shared_dnn_configurations_id]");
			goto end;
		}
	}
	   ...
	   OpenAPI_odb_packet_services_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_session_management_subscription_data_convertToJSON() failed [odb_packet_services]");
			goto end;
		}
	}
	...
	if(session_management_subscription_data->trace_data) {
		...
		if(trace_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_session_management_subscription_data_convertToJSON() failed [trace_data]");
			goto end;
		}
	...
	if(session_management_subscription_data->shared_trace_data_id) {
		...
		{
			ogs_error(
				"OpenAPI_session_management_subscription_data_convertToJSON() failed [shared_trace_data_id]");
			goto end;
		}
	}
	...
	if(session_management_subscription_data->expected_ue_behaviours_list) {
		...
		if(expected_ue_behaviours_list == NULL) {
			ogs_error(
				"OpenAPI_session_management_subscription_data_convertToJSON() failed [expected_ue_behaviours_list]");
			goto end;
		}
	...
	if(session_management_subscription_data->suggested_packet_num_dl_list) {
		...
		if(suggested_packet_num_dl_list == NULL) {
			ogs_error(
				"OpenAPI_session_management_subscription_data_convertToJSON() failed [suggested_packet_num_dl_list]");
			goto end;
		}
		   ...
		   suggested_packet_num_dl_list)
		{
			OpenAPI_list_for_each(
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_suggested_packet_num_dl_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
	...
	if(session_management_subscription_data->_3gpp_charging_characteristics)
	{
		...
		{
			ogs_error(
				"OpenAPI_session_management_subscription_data_convertToJSON() failed [_3gpp_charging_characteristics]");
			goto end;
		}
	}
	...
	if(session_management_subscription_data->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_session_management_subscription_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
...
OpenAPI_session_management_subscription_data_parseFromJSON(
	...
	OpenAPI_session_management_subscription_data_t *
	        session_management_subscription_data_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	single_nssai =
		cJSON_GetObjectItemCaseSensitive(
			session_management_subscription_dataJSON,
			"singleNssai");
	if(!single_nssai) {
		ogs_error(
			"OpenAPI_session_management_subscription_data_parseFromJSON() failed [single_nssai]");
		goto end;
	}
	...
	if(dnn_configurations) {
		...
		{
			ogs_error(
				"OpenAPI_session_management_subscription_data_parseFromJSON() failed [dnn_configurations]");
			goto end;
		}
		if(cJSON_IsObject(dnn_configurations)) {
			...
			cJSON_ArrayForEach(dnn_configurations_local_map,
				...
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_session_management_subscription_data_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	if(internal_group_ids) {
		...
		if(!cJSON_IsArray(internal_group_ids)) {
			ogs_error(
				"OpenAPI_session_management_subscription_data_parseFromJSON() failed [internal_group_ids]");
			goto end;
		}
	...
	if(shared_vn_group_data_ids) {
		...
		{
			ogs_error(
				"OpenAPI_session_management_subscription_data_parseFromJSON() failed [shared_vn_group_data_ids]");
			goto end;
		}
		if(cJSON_IsObject(shared_vn_group_data_ids)) {
	...
	if(shared_dnn_configurations_id) {
		...
		{
			ogs_error(
				"OpenAPI_session_management_subscription_data_parseFromJSON() failed [shared_dnn_configurations_id]");
			goto end;
		}
	}
	...
	if(odb_packet_services) {
		if(!cJSON_IsString(odb_packet_services)) {
			ogs_error(
				"OpenAPI_session_management_subscription_data_parseFromJSON() failed [odb_packet_services]");
			goto end;
		}
	...
	trace_data =
		cJSON_GetObjectItemCaseSensitive(
			session_management_subscription_dataJSON, "traceData");
	if(trace_data) {
	...
	if(shared_trace_data_id) {
		...
		{
			ogs_error(
				"OpenAPI_session_management_subscription_data_parseFromJSON() failed [shared_trace_data_id]");
			goto end;
		}
	}
	...
	if(expected_ue_behaviours_list) {
		...
		{
			ogs_error(
				"OpenAPI_session_management_subscription_data_parseFromJSON() failed [expected_ue_behaviours_list]");
			goto end;
		}
		if(cJSON_IsObject(expected_ue_behaviours_list)) {
	...
	if(suggested_packet_num_dl_list) {
		...
		{
			ogs_error(
				"OpenAPI_session_management_subscription_data_parseFromJSON() failed [suggested_packet_num_dl_list]");
			goto end;
		}
		if(cJSON_IsObject(suggested_packet_num_dl_list)) {
	...
	if(_3gpp_charging_characteristics) {
		...
		{
			ogs_error(
				"OpenAPI_session_management_subscription_data_parseFromJSON() failed [_3gpp_charging_characteristics]");
			goto end;
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_session_management_subscription_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
...
OpenAPI_session_management_subscription_data_copy(
	OpenAPI_session_management_subscription_data_t	*dst,
	OpenAPI_session_management_subscription_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_session_management_subscription_data_convertToJSON() failed");
		return NULL;
	}
