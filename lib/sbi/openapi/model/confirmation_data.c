...
OpenAPI_confirmation_data_t *OpenAPI_confirmation_data_create(
	...
	OpenAPI_confirmation_data_t *confirmation_data_local_var =
		ogs_malloc(sizeof(OpenAPI_confirmation_data_t));
...
cJSON *OpenAPI_confirmation_data_convertToJSON(
	...
	if(confirmation_data == NULL) {
		ogs_error(
			"OpenAPI_confirmation_data_convertToJSON() failed [ConfirmationData]");
		return NULL;
	}
	...
	if(!confirmation_data->res_star) {
		ogs_error(
			"OpenAPI_confirmation_data_convertToJSON() failed [res_star]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "resStar",
	                           confirmation_data->res_star) == NULL)
	{
		ogs_error(
			"OpenAPI_confirmation_data_convertToJSON() failed [res_star]");
	...
	if(confirmation_data->supported_features) {
		if(cJSON_AddStringToObject(item, "supportedFeatures",
		                           confirmation_data->supported_features)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_confirmation_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
...
OpenAPI_confirmation_data_t *OpenAPI_confirmation_data_parseFromJSON(
	...
	res_star = cJSON_GetObjectItemCaseSensitive(confirmation_dataJSON,
	                                            "resStar");
	if(!res_star) {
		ogs_error(
			"OpenAPI_confirmation_data_parseFromJSON() failed [res_star]");
		goto end;
	}
	...
	supported_features =
		cJSON_GetObjectItemCaseSensitive(confirmation_dataJSON,
		                                 "supportedFeatures");
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_confirmation_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	confirmation_data_local_var = OpenAPI_confirmation_data_create(
		res_star &&
		cJSON_IsNull(res_star) ? true : false,
