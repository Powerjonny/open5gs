...
OpenAPI_pgw_info_t *OpenAPI_pgw_info_create(char *dnn, char *pgw_fqdn,
                                            OpenAPI_ip_address_t *pgw_ip_addr,
                                            OpenAPI_plmn_id_t *plmn_id,
                                            bool is_epdg_ind, int epdg_ind,
                                            char *pcf_id,
                                            char *registration_time) {
	OpenAPI_pgw_info_t *pgw_info_local_var =
		ogs_malloc(sizeof(OpenAPI_pgw_info_t));
...
cJSON *OpenAPI_pgw_info_convertToJSON(OpenAPI_pgw_info_t *pgw_info) {
	...
	if(pgw_info->pgw_ip_addr) {
		cJSON *pgw_ip_addr_local_JSON =
			OpenAPI_ip_address_convertToJSON(pgw_info->pgw_ip_addr);
		if(pgw_ip_addr_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_pgw_info_convertToJSON() failed [pgw_ip_addr]");
			goto end;
		}
	...
	if(pgw_info->plmn_id) {
		cJSON *plmn_id_local_JSON =
			OpenAPI_plmn_id_convertToJSON(pgw_info->plmn_id);
		if(plmn_id_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_pgw_info_convertToJSON() failed [plmn_id]");
			goto end;
		}
	...
	if(pgw_info->is_epdg_ind) {
		if(cJSON_AddBoolToObject(item, "epdgInd",
		                         pgw_info->epdg_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_pgw_info_convertToJSON() failed [epdg_ind]");
			goto end;
		}
	}
	...
	if(pgw_info->pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_pgw_info_convertToJSON() failed [pcf_id]");
			goto end;
		}
	}
	...
	if(pgw_info->registration_time) {
		...
		{
			ogs_error(
				"OpenAPI_pgw_info_convertToJSON() failed [registration_time]");
			goto end;
		}
	}
...
OpenAPI_pgw_info_t *OpenAPI_pgw_info_parseFromJSON(cJSON *pgw_infoJSON) {
	...
	pgw_ip_addr = cJSON_GetObjectItemCaseSensitive(pgw_infoJSON,
	                                               "pgwIpAddr");
	if(pgw_ip_addr) {
	...
	if(epdg_ind) {
		if(!cJSON_IsBool(epdg_ind)) {
			ogs_error(
				"OpenAPI_pgw_info_parseFromJSON() failed [epdg_ind]");
			goto end;
		}
	}
	...
	if(pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_pgw_info_parseFromJSON() failed [pcf_id]");
			goto end;
		}
	}

	registration_time = cJSON_GetObjectItemCaseSensitive(pgw_infoJSON,
	                                                     "registrationTime");
	if(registration_time) {
		...
		{
			ogs_error(
				"OpenAPI_pgw_info_parseFromJSON() failed [registration_time]");
			goto end;
		}
	}
...
OpenAPI_pgw_info_t *OpenAPI_pgw_info_copy(OpenAPI_pgw_info_t	*dst,
                                          OpenAPI_pgw_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
