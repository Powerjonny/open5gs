...
OpenAPI_ee_group_profile_data_t *OpenAPI_ee_group_profile_data_create(
	...
	char *hss_group_id) {
	OpenAPI_ee_group_profile_data_t *ee_group_profile_data_local_var =
		ogs_malloc(sizeof(OpenAPI_ee_group_profile_data_t));
	ogs_assert(ee_group_profile_data_local_var);

	ee_group_profile_data_local_var->restricted_event_types =
		restricted_event_types;
	ee_group_profile_data_local_var->allowed_mtc_provider =
		allowed_mtc_provider;
	ee_group_profile_data_local_var->supported_features =
		supported_features;
	ee_group_profile_data_local_var->is_iwk_epc_restricted =
		is_iwk_epc_restricted;
	ee_group_profile_data_local_var->iwk_epc_restricted =
		iwk_epc_restricted;
...
void OpenAPI_ee_group_profile_data_free(
	...
	if(ee_group_profile_data->restricted_event_types) {
		OpenAPI_list_for_each(
			ee_group_profile_data->restricted_event_types, node) {
			OpenAPI_event_type_free(node->data);
		}
		OpenAPI_list_free(
			ee_group_profile_data->restricted_event_types);
		ee_group_profile_data->restricted_event_types = NULL;
	}
	if(ee_group_profile_data->allowed_mtc_provider) {
		OpenAPI_list_for_each(
			ee_group_profile_data->allowed_mtc_provider, node) {
...
cJSON *OpenAPI_ee_group_profile_data_convertToJSON(
	...
	if(ee_group_profile_data == NULL) {
		ogs_error(
			"OpenAPI_ee_group_profile_data_convertToJSON() failed [EeGroupProfileData]");
		return NULL;
	}
	...
	if(ee_group_profile_data->restricted_event_types) {
		...
		if(restricted_event_typesList == NULL) {
			ogs_error(
				"OpenAPI_ee_group_profile_data_convertToJSON() failed [restricted_event_types]");
			goto end;
		}
		OpenAPI_list_for_each(
			ee_group_profile_data->restricted_event_types, node) {
	...
	if(ee_group_profile_data->allowed_mtc_provider) {
		...
		if(allowed_mtc_provider == NULL) {
			ogs_error(
				"OpenAPI_ee_group_profile_data_convertToJSON() failed [allowed_mtc_provider]");
			goto end;
		}
		...
		if(ee_group_profile_data->allowed_mtc_provider) {
			OpenAPI_list_for_each(
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_ee_group_profile_data_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(ee_group_profile_data->supported_features) {
		if(cJSON_AddStringToObject(item, "supportedFeatures",
		                           ee_group_profile_data->
		                           supported_features) == NULL)
		{
			ogs_error(
				"OpenAPI_ee_group_profile_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(ee_group_profile_data->is_iwk_epc_restricted) {
		if(cJSON_AddBoolToObject(item, "iwkEpcRestricted",
		                         ee_group_profile_data->
		                         iwk_epc_restricted) == NULL)
		{
			ogs_error(
				"OpenAPI_ee_group_profile_data_convertToJSON() failed [iwk_epc_restricted]");
			goto end;
		}
	}
	...
	if(ee_group_profile_data->ext_group_id) {
		if(cJSON_AddStringToObject(item, "extGroupId",
		                           ee_group_profile_data->ext_group_id)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_ee_group_profile_data_convertToJSON() failed [ext_group_id]");
			goto end;
		}
	}
	...
	if(ee_group_profile_data->hss_group_id) {
		...
		{
			ogs_error(
				"OpenAPI_ee_group_profile_data_convertToJSON() failed [hss_group_id]");
			goto end;
		}
	}
...
OpenAPI_ee_group_profile_data_t *OpenAPI_ee_group_profile_data_parseFromJSON(
	...
	restricted_event_types =
		cJSON_GetObjectItemCaseSensitive(ee_group_profile_dataJSON,
		                                 "restrictedEventTypes");
	if(restricted_event_types) {
		cJSON *restricted_event_types_local = NULL;
		if(!cJSON_IsArray(restricted_event_types)) {
			ogs_error(
				"OpenAPI_ee_group_profile_data_parseFromJSON() failed [restricted_event_types]");
			goto end;
		}
	...
	if(allowed_mtc_provider) {
		...
		{
			ogs_error(
				"OpenAPI_ee_group_profile_data_parseFromJSON() failed [allowed_mtc_provider]");
			goto end;
		}
		if(cJSON_IsObject(allowed_mtc_provider)) {
			...
			cJSON_ArrayForEach(allowed_mtc_provider_local_map,
				...
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_ee_group_profile_data_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_ee_group_profile_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(iwk_epc_restricted) {
		if(!cJSON_IsBool(iwk_epc_restricted)) {
			ogs_error(
				"OpenAPI_ee_group_profile_data_parseFromJSON() failed [iwk_epc_restricted]");
			goto end;
		}
	}
	...
	if(ext_group_id) {
		...
		{
			ogs_error(
				"OpenAPI_ee_group_profile_data_parseFromJSON() failed [ext_group_id]");
			goto end;
		}
	}
	...
	if(hss_group_id) {
		...
		{
			ogs_error(
				"OpenAPI_ee_group_profile_data_parseFromJSON() failed [hss_group_id]");
			goto end;
		}
	}
	...
	ee_group_profile_data_local_var = OpenAPI_ee_group_profile_data_create(
		...
		ext_group_id &&
		!cJSON_IsNull(ext_group_id) ?
		ogs_strdup(ext_group_id->valuestring) : NULL,
		hss_group_id &&
		!cJSON_IsNull(hss_group_id) ?
...
OpenAPI_ee_group_profile_data_t *OpenAPI_ee_group_profile_data_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ee_group_profile_data_convertToJSON() failed");
		return NULL;
	}
