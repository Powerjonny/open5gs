...
OpenAPI_pcscf_restoration_request_data_create(char *dnn, char *ip_domain,
	...
	OpenAPI_pcscf_restoration_request_data_t *
		...
		ogs_malloc(sizeof(OpenAPI_pcscf_restoration_request_data_t));
	...
	OpenAPI_pcscf_restoration_request_data_t *pcscf_restoration_request_data)
{
	...
	if(pcscf_restoration_request_data == NULL) {
		ogs_error(
			"OpenAPI_pcscf_restoration_request_data_convertToJSON() failed [PcscfRestorationRequestData]");
		return NULL;
	}
	...
	if(pcscf_restoration_request_data->dnn) {
		...
		{
			ogs_error(
				"OpenAPI_pcscf_restoration_request_data_convertToJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(pcscf_restoration_request_data->ip_domain) {
		...
		{
			ogs_error(
				"OpenAPI_pcscf_restoration_request_data_convertToJSON() failed [ip_domain]");
			goto end;
		}
	}
	...
	if(pcscf_restoration_request_data->slice_info) {
		cJSON *slice_info_local_JSON =
			OpenAPI_snssai_convertToJSON(
				pcscf_restoration_request_data->slice_info);
		if(slice_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_pcscf_restoration_request_data_convertToJSON() failed [slice_info]");
			goto end;
		}
	...
	if(pcscf_restoration_request_data->supi) {
		if(cJSON_AddStringToObject(item, "supi",
		                           pcscf_restoration_request_data->supi)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_pcscf_restoration_request_data_convertToJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(pcscf_restoration_request_data->ue_ipv4) {
		...
		{
			ogs_error(
				"OpenAPI_pcscf_restoration_request_data_convertToJSON() failed [ue_ipv4]");
			goto end;
		}
	}
	...
	if(pcscf_restoration_request_data->ue_ipv6) {
		...
		{
			ogs_error(
				"OpenAPI_pcscf_restoration_request_data_convertToJSON() failed [ue_ipv6]");
			goto end;
		}
	}
...
OpenAPI_pcscf_restoration_request_data_parseFromJSON(
	...
	OpenAPI_pcscf_restoration_request_data_t *
	        pcscf_restoration_request_data_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *dnn = NULL;
	...
	dnn =
		cJSON_GetObjectItemCaseSensitive(
			pcscf_restoration_request_dataJSON, "dnn");
	if(dnn) {
		if(!cJSON_IsString(dnn) &&
		   !cJSON_IsNull(dnn))
		{
			ogs_error(
				"OpenAPI_pcscf_restoration_request_data_parseFromJSON() failed [dnn]");
			goto end;
		}
	}
	...
	ip_domain =
		cJSON_GetObjectItemCaseSensitive(
			pcscf_restoration_request_dataJSON, "ipDomain");
	if(ip_domain) {
		if(!cJSON_IsString(ip_domain) &&
		   !cJSON_IsNull(ip_domain))
		{
			ogs_error(
				"OpenAPI_pcscf_restoration_request_data_parseFromJSON() failed [ip_domain]");
			goto end;
		}
	}
	...
	slice_info =
		cJSON_GetObjectItemCaseSensitive(
			pcscf_restoration_request_dataJSON, "sliceInfo");
	if(slice_info) {
	...
	supi =
		cJSON_GetObjectItemCaseSensitive(
			pcscf_restoration_request_dataJSON, "supi");
	if(supi) {
		if(!cJSON_IsString(supi) &&
		   !cJSON_IsNull(supi))
		{
			ogs_error(
				"OpenAPI_pcscf_restoration_request_data_parseFromJSON() failed [supi]");
			goto end;
		}
	}
	...
	ue_ipv4 =
		cJSON_GetObjectItemCaseSensitive(
			pcscf_restoration_request_dataJSON, "ueIpv4");
	if(ue_ipv4) {
		if(!cJSON_IsString(ue_ipv4) &&
		   !cJSON_IsNull(ue_ipv4))
		{
			ogs_error(
				"OpenAPI_pcscf_restoration_request_data_parseFromJSON() failed [ue_ipv4]");
			goto end;
		}
	}
	...
	ue_ipv6 =
		cJSON_GetObjectItemCaseSensitive(
			pcscf_restoration_request_dataJSON, "ueIpv6");
	if(ue_ipv6) {
		if(!cJSON_IsString(ue_ipv6) &&
		   !cJSON_IsNull(ue_ipv6))
		{
			ogs_error(
				"OpenAPI_pcscf_restoration_request_data_parseFromJSON() failed [ue_ipv6]");
			goto end;
		}
	}
	...
	pcscf_restoration_request_data_local_var =
		OpenAPI_pcscf_restoration_request_data_create(
			...
			ue_ipv6 &&
			!cJSON_IsNull(ue_ipv6) ?
			ogs_strdup(ue_ipv6->valuestring) : NULL
			);
...
OpenAPI_pcscf_restoration_request_data_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_pcscf_restoration_request_data_convertToJSON() failed");
		return NULL;
	}
