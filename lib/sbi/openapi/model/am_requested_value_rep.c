...
OpenAPI_am_requested_value_rep_t *OpenAPI_am_requested_value_rep_create(
	...
	OpenAPI_list_t *allowed_snssais, OpenAPI_list_t *n3g_allowed_snssais) {
	OpenAPI_am_requested_value_rep_t *am_requested_value_rep_local_var =
		ogs_malloc(sizeof(OpenAPI_am_requested_value_rep_t));
	...
	am_requested_value_rep_local_var->n3g_allowed_snssais =
		n3g_allowed_snssais;

	return am_requested_value_rep_local_var;
}

void OpenAPI_am_requested_value_rep_free(
	OpenAPI_am_requested_value_rep_t *am_requested_value_rep) {
	...
	if(am_requested_value_rep->pra_statuses) {
		OpenAPI_list_for_each(am_requested_value_rep->pra_statuses,
		                      node) {
	...
	if(am_requested_value_rep->allowed_snssais) {
		OpenAPI_list_for_each(am_requested_value_rep->allowed_snssais,
		                      node) {
	...
	if(am_requested_value_rep->n3g_allowed_snssais) {
		OpenAPI_list_for_each(
			am_requested_value_rep->n3g_allowed_snssais, node) {
			OpenAPI_snssai_free(node->data);
		}
...
cJSON *OpenAPI_am_requested_value_rep_convertToJSON(
	OpenAPI_am_requested_value_rep_t *am_requested_value_rep) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(am_requested_value_rep == NULL) {
		ogs_error(
			"OpenAPI_am_requested_value_rep_convertToJSON() failed [AmRequestedValueRep]");
		return NULL;
	}
	...
	if(am_requested_value_rep->user_loc) {
		cJSON *user_loc_local_JSON =
			OpenAPI_user_location_convertToJSON(
				am_requested_value_rep->user_loc);
		if(user_loc_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_am_requested_value_rep_convertToJSON() failed [user_loc]");
			goto end;
		}
	...
	if(am_requested_value_rep->pra_statuses) {
		...
		if(pra_statuses == NULL) {
			ogs_error(
				"OpenAPI_am_requested_value_rep_convertToJSON() failed [pra_statuses]");
			goto end;
		}
		cJSON *localMapObject = pra_statuses;
		if(am_requested_value_rep->pra_statuses) {
			OpenAPI_list_for_each(
				am_requested_value_rep->pra_statuses, node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_am_requested_value_rep_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(am_requested_value_rep->access_types != OpenAPI_access_type_NULL) {
		...
		if(access_typesList == NULL) {
			ogs_error(
				"OpenAPI_am_requested_value_rep_convertToJSON() failed [access_types]");
			goto end;
		}
		OpenAPI_list_for_each(am_requested_value_rep->access_types,
		                      node) {
	...
	if(am_requested_value_rep->rat_types != OpenAPI_rat_type_NULL) {
		...
		if(rat_typesList == NULL) {
			ogs_error(
				"OpenAPI_am_requested_value_rep_convertToJSON() failed [rat_types]");
			goto end;
		}
	...
	if(am_requested_value_rep->allowed_snssais) {
		...
		if(allowed_snssaisList == NULL) {
			ogs_error(
				"OpenAPI_am_requested_value_rep_convertToJSON() failed [allowed_snssais]");
			goto end;
		}
		OpenAPI_list_for_each(am_requested_value_rep->allowed_snssais,
		                      node) {
	...
	if(am_requested_value_rep->n3g_allowed_snssais) {
		...
		if(n3g_allowed_snssaisList == NULL) {
			ogs_error(
				"OpenAPI_am_requested_value_rep_convertToJSON() failed [n3g_allowed_snssais]");
			goto end;
		}
		OpenAPI_list_for_each(
			am_requested_value_rep->n3g_allowed_snssais, node) {
...
OpenAPI_am_requested_value_rep_t *OpenAPI_am_requested_value_rep_parseFromJSON(
	cJSON *am_requested_value_repJSON) {
	OpenAPI_am_requested_value_rep_t *am_requested_value_rep_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *user_loc = NULL;
	...
	user_loc = cJSON_GetObjectItemCaseSensitive(am_requested_value_repJSON,
	                                            "userLoc");
	if(user_loc) {
	...
	pra_statuses =
		cJSON_GetObjectItemCaseSensitive(am_requested_value_repJSON,
		                                 "praStatuses");
	if(pra_statuses) {
		...
		{
			ogs_error(
				"OpenAPI_am_requested_value_rep_parseFromJSON() failed [pra_statuses]");
			goto end;
		}
		if(cJSON_IsObject(pra_statuses)) {
			...
			cJSON_ArrayForEach(pra_statuses_local_map,
				...
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_am_requested_value_rep_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	if(access_types) {
		...
		if(!cJSON_IsArray(access_types)) {
			ogs_error(
				"OpenAPI_am_requested_value_rep_parseFromJSON() failed [access_types]");
			goto end;
		}
		...
		if(access_typesList->count == 0) {
			ogs_error(
				"OpenAPI_am_requested_value_rep_parseFromJSON() failed: Expected access_typesList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}

	rat_types = cJSON_GetObjectItemCaseSensitive(am_requested_value_repJSON,
	                                             "ratTypes");
	if(rat_types) {
		cJSON *rat_types_local = NULL;
		if(!cJSON_IsArray(rat_types)) {
			ogs_error(
				"OpenAPI_am_requested_value_rep_parseFromJSON() failed [rat_types]");
			goto end;
		}
		...
		if(rat_typesList->count == 0) {
			ogs_error(
				"OpenAPI_am_requested_value_rep_parseFromJSON() failed: Expected rat_typesList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(allowed_snssais) {
		...
		if(!cJSON_IsArray(allowed_snssais)) {
			ogs_error(
				"OpenAPI_am_requested_value_rep_parseFromJSON() failed [allowed_snssais]");
			goto end;
		}
	...
	if(n3g_allowed_snssais) {
		...
		if(!cJSON_IsArray(n3g_allowed_snssais)) {
			ogs_error(
				"OpenAPI_am_requested_value_rep_parseFromJSON() failed [n3g_allowed_snssais]");
			goto end;
		}
	...
	am_requested_value_rep_local_var =
		OpenAPI_am_requested_value_rep_create(
...
OpenAPI_am_requested_value_rep_t *OpenAPI_am_requested_value_rep_copy(
	OpenAPI_am_requested_value_rep_t	*dst,
	OpenAPI_am_requested_value_rep_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_am_requested_value_rep_convertToJSON() failed");
		return NULL;
	}
