...
OpenAPI_nrf_info_served_pcf_info_value_create(char		*group_id,
	...
	OpenAPI_nrf_info_served_pcf_info_value_t *
		...
		ogs_malloc(sizeof(OpenAPI_nrf_info_served_pcf_info_value_t));
	...
	nrf_info_served_pcf_info_value_local_var->is_v2x_support_ind =
		is_v2x_support_ind;
	nrf_info_served_pcf_info_value_local_var->v2x_support_ind =
		v2x_support_ind;
	nrf_info_served_pcf_info_value_local_var->is_prose_support_ind =
		is_prose_support_ind;
	nrf_info_served_pcf_info_value_local_var->prose_support_ind =
		prose_support_ind;
	nrf_info_served_pcf_info_value_local_var->prose_capability =
		prose_capability;
	nrf_info_served_pcf_info_value_local_var->v2x_capability =
		v2x_capability;

	return nrf_info_served_pcf_info_value_local_var;
}

void OpenAPI_nrf_info_served_pcf_info_value_free(
	OpenAPI_nrf_info_served_pcf_info_value_t *nrf_info_served_pcf_info_value)
{
	...
	if(nrf_info_served_pcf_info_value->dnn_list) {
		OpenAPI_list_for_each(nrf_info_served_pcf_info_value->dnn_list,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(nrf_info_served_pcf_info_value->supi_ranges) {
		OpenAPI_list_for_each(
			nrf_info_served_pcf_info_value->supi_ranges, node) {
	...
	if(nrf_info_served_pcf_info_value->gpsi_ranges) {
		OpenAPI_list_for_each(
			nrf_info_served_pcf_info_value->gpsi_ranges, node) {
	...
	if(nrf_info_served_pcf_info_value->prose_capability) {
		OpenAPI_pro_se_capability_free(
			nrf_info_served_pcf_info_value->prose_capability);
		nrf_info_served_pcf_info_value->prose_capability = NULL;
	}
	if(nrf_info_served_pcf_info_value->v2x_capability) {
		OpenAPI_v2x_capability_free(
			nrf_info_served_pcf_info_value->v2x_capability);
		nrf_info_served_pcf_info_value->v2x_capability = NULL;
	}
	ogs_free(nrf_info_served_pcf_info_value);
}

cJSON *OpenAPI_nrf_info_served_pcf_info_value_convertToJSON(
	OpenAPI_nrf_info_served_pcf_info_value_t *nrf_info_served_pcf_info_value)
{
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(nrf_info_served_pcf_info_value == NULL) {
		ogs_error(
			"OpenAPI_nrf_info_served_pcf_info_value_convertToJSON() failed [NrfInfo_servedPcfInfo_value]");
		return NULL;
	}
	...
	if(nrf_info_served_pcf_info_value->group_id) {
		if(cJSON_AddStringToObject(item, "groupId",
		                           nrf_info_served_pcf_info_value->
		                           group_id) == NULL)
		{
			ogs_error(
				"OpenAPI_nrf_info_served_pcf_info_value_convertToJSON() failed [group_id]");
			goto end;
		}
	}
	...
	if(nrf_info_served_pcf_info_value->dnn_list) {
		...
		if(dnn_listList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_pcf_info_value_convertToJSON() failed [dnn_list]");
			goto end;
		}
		OpenAPI_list_for_each(nrf_info_served_pcf_info_value->dnn_list,
		                      node) {
	...
	if(nrf_info_served_pcf_info_value->supi_ranges) {
		cJSON *supi_rangesList = cJSON_AddArrayToObject(item,
		                                                "supiRanges");
		if(supi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_pcf_info_value_convertToJSON() failed [supi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			nrf_info_served_pcf_info_value->supi_ranges, node) {
			cJSON *itemLocal =
				OpenAPI_supi_range_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_nrf_info_served_pcf_info_value_convertToJSON() failed [supi_ranges]");
				goto end;
	...
	if(nrf_info_served_pcf_info_value->gpsi_ranges) {
		...
		if(gpsi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_pcf_info_value_convertToJSON() failed [gpsi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			nrf_info_served_pcf_info_value->gpsi_ranges, node) {
	...
	if(nrf_info_served_pcf_info_value->rx_diam_host) {
		if(cJSON_AddStringToObject(item, "rxDiamHost",
		                           nrf_info_served_pcf_info_value->
		                           rx_diam_host) == NULL)
		{
			ogs_error(
				"OpenAPI_nrf_info_served_pcf_info_value_convertToJSON() failed [rx_diam_host]");
			goto end;
		}
	}

	if(nrf_info_served_pcf_info_value->rx_diam_realm) {
		if(cJSON_AddStringToObject(item, "rxDiamRealm",
		                           nrf_info_served_pcf_info_value->
		                           rx_diam_realm) == NULL)
		{
			ogs_error(
				"OpenAPI_nrf_info_served_pcf_info_value_convertToJSON() failed [rx_diam_realm]");
			goto end;
		}
	}

	if(nrf_info_served_pcf_info_value->is_v2x_support_ind) {
		if(cJSON_AddBoolToObject(item, "v2xSupportInd",
		                         nrf_info_served_pcf_info_value->
		                         v2x_support_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_nrf_info_served_pcf_info_value_convertToJSON() failed [v2x_support_ind]");
			goto end;
		}
	}

	if(nrf_info_served_pcf_info_value->is_prose_support_ind) {
		if(cJSON_AddBoolToObject(item, "proseSupportInd",
		                         nrf_info_served_pcf_info_value->
		                         prose_support_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_nrf_info_served_pcf_info_value_convertToJSON() failed [prose_support_ind]");
			goto end;
		}
	}

	if(nrf_info_served_pcf_info_value->prose_capability) {
		cJSON *prose_capability_local_JSON =
			OpenAPI_pro_se_capability_convertToJSON(
				nrf_info_served_pcf_info_value->prose_capability);
		if(prose_capability_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_pcf_info_value_convertToJSON() failed [prose_capability]");
			goto end;
		}
		cJSON_AddItemToObject(item, "proseCapability",
		                      prose_capability_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_pcf_info_value_convertToJSON() failed [prose_capability]");
			goto end;
	...
	if(nrf_info_served_pcf_info_value->v2x_capability) {
		cJSON *v2x_capability_local_JSON =
			OpenAPI_v2x_capability_convertToJSON(
				nrf_info_served_pcf_info_value->v2x_capability);
		if(v2x_capability_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_pcf_info_value_convertToJSON() failed [v2x_capability]");
			goto end;
		}
		cJSON_AddItemToObject(item, "v2xCapability",
		                      v2x_capability_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_pcf_info_value_convertToJSON() failed [v2x_capability]");
			goto end;
...
OpenAPI_nrf_info_served_pcf_info_value_parseFromJSON(
	cJSON *nrf_info_served_pcf_info_valueJSON) {
	OpenAPI_nrf_info_served_pcf_info_value_t *
	        nrf_info_served_pcf_info_value_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *group_id = NULL;
	cJSON *dnn_list = NULL;
	...
	group_id =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_pcf_info_valueJSON, "groupId");
	if(group_id) {
		if(!cJSON_IsString(group_id) &&
		   !cJSON_IsNull(group_id))
		{
			ogs_error(
				"OpenAPI_nrf_info_served_pcf_info_value_parseFromJSON() failed [group_id]");
			goto end;
		}
	}
	...
	dnn_list =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_pcf_info_valueJSON, "dnnList");
	if(dnn_list) {
		cJSON *dnn_list_local = NULL;
		if(!cJSON_IsArray(dnn_list)) {
			ogs_error(
				"OpenAPI_nrf_info_served_pcf_info_value_parseFromJSON() failed [dnn_list]");
			goto end;
		}
	...
	supi_ranges =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_pcf_info_valueJSON, "supiRanges");
	if(supi_ranges) {
		cJSON *supi_ranges_local = NULL;
		if(!cJSON_IsArray(supi_ranges)) {
			ogs_error(
				"OpenAPI_nrf_info_served_pcf_info_value_parseFromJSON() failed [supi_ranges]");
			goto end;
		}
		...
		cJSON_ArrayForEach(supi_ranges_local, supi_ranges) {
			...
			OpenAPI_supi_range_t *supi_rangesItem =
				OpenAPI_supi_range_parseFromJSON(
					supi_ranges_local);
			if(!supi_rangesItem) {
	...
	gpsi_ranges =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_pcf_info_valueJSON, "gpsiRanges");
	if(gpsi_ranges) {
		cJSON *gpsi_ranges_local = NULL;
		if(!cJSON_IsArray(gpsi_ranges)) {
			ogs_error(
				"OpenAPI_nrf_info_served_pcf_info_value_parseFromJSON() failed [gpsi_ranges]");
			goto end;
		}
	...
	rx_diam_host =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_pcf_info_valueJSON, "rxDiamHost");
	if(rx_diam_host) {
		if(!cJSON_IsString(rx_diam_host) &&
		   !cJSON_IsNull(rx_diam_host))
		{
			ogs_error(
				"OpenAPI_nrf_info_served_pcf_info_value_parseFromJSON() failed [rx_diam_host]");
			goto end;
		}
	}
	...
	rx_diam_realm =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_pcf_info_valueJSON, "rxDiamRealm");
	if(rx_diam_realm) {
		if(!cJSON_IsString(rx_diam_realm) &&
		   !cJSON_IsNull(rx_diam_realm))
		{
			ogs_error(
				"OpenAPI_nrf_info_served_pcf_info_value_parseFromJSON() failed [rx_diam_realm]");
			goto end;
		}
	}
	...
	v2x_support_ind =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_pcf_info_valueJSON, "v2xSupportInd");
	if(v2x_support_ind) {
		if(!cJSON_IsBool(v2x_support_ind)) {
			ogs_error(
				"OpenAPI_nrf_info_served_pcf_info_value_parseFromJSON() failed [v2x_support_ind]");
			goto end;
		}
	}
	...
	prose_support_ind =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_pcf_info_valueJSON, "proseSupportInd");
	if(prose_support_ind) {
		if(!cJSON_IsBool(prose_support_ind)) {
			ogs_error(
				"OpenAPI_nrf_info_served_pcf_info_value_parseFromJSON() failed [prose_support_ind]");
			goto end;
		}
	}
	...
	prose_capability =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_pcf_info_valueJSON, "proseCapability");
	if(prose_capability) {
		prose_capability_local_nonprim =
			OpenAPI_pro_se_capability_parseFromJSON(
				prose_capability);
		if(!prose_capability_local_nonprim) {
			ogs_error(
				"OpenAPI_pro_se_capability_parseFromJSON failed [prose_capability]");
			goto end;
		}
	}
	...
	if(v2x_capability) {
		v2x_capability_local_nonprim =
			OpenAPI_v2x_capability_parseFromJSON(v2x_capability);
		if(!v2x_capability_local_nonprim) {
			ogs_error(
				"OpenAPI_v2x_capability_parseFromJSON failed [v2x_capability]");
			goto end;
		}
	}

	nrf_info_served_pcf_info_value_local_var =
		OpenAPI_nrf_info_served_pcf_info_value_create(
			group_id &&
			!cJSON_IsNull(group_id) ?
			...
			rx_diam_host &&
			!cJSON_IsNull(rx_diam_host) ?
			ogs_strdup(rx_diam_host->valuestring) : NULL,
			rx_diam_realm &&
			!cJSON_IsNull(rx_diam_realm) ?
			ogs_strdup(rx_diam_realm->valuestring) : NULL,
			...
			prose_capability ? prose_capability_local_nonprim :
			NULL,
...
OpenAPI_nrf_info_served_pcf_info_value_t *
OpenAPI_nrf_info_served_pcf_info_value_copy(
	OpenAPI_nrf_info_served_pcf_info_value_t	*dst,
	OpenAPI_nrf_info_served_pcf_info_value_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_nrf_info_served_pcf_info_value_convertToJSON() failed");
		return NULL;
	}
