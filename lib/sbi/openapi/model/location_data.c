...
char *OpenAPI_remote_ue_indlocation_data_ToString(
	OpenAPI_location_data_remote_ue_ind_e remote_ue_ind) {
...
OpenAPI_location_data_remote_ue_ind_e
OpenAPI_remote_ue_indlocation_data_FromString(char *remote_ue_ind) {
...
char *OpenAPI_nrppa_periodic_indlocation_data_ToString(
	OpenAPI_location_data_nrppa_periodic_ind_e nrppa_periodic_ind) {
...
OpenAPI_location_data_nrppa_periodic_ind_e
OpenAPI_nrppa_periodic_indlocation_data_FromString(char *nrppa_periodic_ind) {
	int stringToReturn = 0;
	const char *nrppa_periodic_indArray[] = { "NULL", "true" };
	size_t sizeofArray = sizeof(nrppa_periodic_indArray) /
	                     sizeof(nrppa_periodic_indArray[0]);
...
OpenAPI_location_data_t *OpenAPI_location_data_create(
	...
	OpenAPI_ncgi_t *ncgi,
	OpenAPI_location_data_remote_ue_ind_e remote_ue_ind, bool is_altitude,
	double altitude, bool is_barometric_pressure, int barometric_pressure,
	char *serving_lmf_identification, char *ue_positioning_cap,
	OpenAPI_ue_area_indication_t *ue_area_ind, char *supported_features,
	OpenAPI_minor_location_qo_s_t *achieved_qos, bool is_direct_report_ind,
	int direct_report_ind, OpenAPI_indoor_outdoor_ind_e indoor_outdoor_ind,
	...
	OpenAPI_location_data_nrppa_periodic_ind_e nrppa_periodic_ind) {
	OpenAPI_location_data_t *location_data_local_var =
		ogs_malloc(sizeof(OpenAPI_location_data_t));
...
void OpenAPI_location_data_free(OpenAPI_location_data_t *location_data) {
	...
	if(location_data->positioning_data_list) {
		OpenAPI_list_for_each(location_data->positioning_data_list,
		                      node) {
	...
	if(location_data->gnss_positioning_data_list) {
		OpenAPI_list_for_each(location_data->gnss_positioning_data_list,
		                      node) {
	...
	if(location_data->ranging_sl_capability) {
		ogs_free(location_data->ranging_sl_capability);
	...
	if(location_data->integrity_result) {
		...
		location_data->integrity_result = NULL;
	}
	ogs_free(location_data);
}

cJSON *OpenAPI_location_data_convertToJSON(
	OpenAPI_location_data_t *location_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(location_data == NULL) {
		ogs_error(
			"OpenAPI_location_data_convertToJSON() failed [LocationData]");
		return NULL;
	}
	...
	if(!location_data->location_estimate) {
		ogs_error(
			"OpenAPI_location_data_convertToJSON() failed [location_estimate]");
		return NULL;
	}
	   ...
	   OpenAPI_accuracy_fulfilment_indicator_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [accuracy_fulfilment_indicator]");
			goto end;
		}
	}
	...
	if(location_data->is_age_of_location_estimate) {
		...
		{
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [age_of_location_estimate]");
			goto end;
		}
	}
	...
	if(location_data->timestamp_of_location_estimate) {
		...
		{
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [timestamp_of_location_estimate]");
			goto end;
		}
	}
	...
	if(location_data->velocity_estimate) {
		...
		if(velocity_estimate_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [velocity_estimate]");
			goto end;
		}
	...
	if(location_data->civic_address) {
		...
		if(civic_address_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [civic_address]");
			goto end;
		}
	...
	if(location_data->local_location_estimate) {
		...
		if(local_location_estimate_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [local_location_estimate]");
			goto end;
		}
	...
	if(location_data->positioning_data_list) {
		...
		if(positioning_data_listList == NULL) {
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [positioning_data_list]");
			goto end;
		}
		OpenAPI_list_for_each(location_data->positioning_data_list,
		                      node) {
	...
	if(location_data->gnss_positioning_data_list) {
		...
		if(gnss_positioning_data_listList == NULL) {
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [gnss_positioning_data_list]");
			goto end;
		}
		OpenAPI_list_for_each(location_data->gnss_positioning_data_list,
		                      node) {
	...
	if(location_data->ecgi) {
		cJSON *ecgi_local_JSON =
			OpenAPI_ecgi_convertToJSON(location_data->ecgi);
		if(ecgi_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [ecgi]");
			goto end;
		}
	...
	if(location_data->ncgi) {
		cJSON *ncgi_local_JSON =
			OpenAPI_ncgi_convertToJSON(location_data->ncgi);
		if(ncgi_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [ncgi]");
			goto end;
		}
	...
	if(location_data->remote_ue_ind !=
	   OpenAPI_location_data_REMOTEUEIND_NULL)
	{
		if(cJSON_AddStringToObject(item, "remoteUeInd",
		                           OpenAPI_remote_ue_indlocation_data_ToString
		                                   (location_data->remote_ue_ind))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [remote_ue_ind]");
			goto end;
		}
	}
	...
	if(location_data->is_altitude) {
		...
		{
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [altitude]");
			goto end;
		}
	}
	...
	if(location_data->is_barometric_pressure) {
		...
		{
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [barometric_pressure]");
			goto end;
		}
	}
	...
	if(location_data->serving_lmf_identification) {
		...
		{
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [serving_lmf_identification]");
			goto end;
		}
	}
	...
	if(location_data->ue_positioning_cap) {
		...
		{
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [ue_positioning_cap]");
			goto end;
		}
	}
	...
	if(location_data->ue_area_ind) {
		...
		if(ue_area_ind_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [ue_area_ind]");
			goto end;
		}
	...
	if(location_data->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(location_data->achieved_qos) {
		...
		if(achieved_qos_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [achieved_qos]");
			goto end;
		}
	...
	if(location_data->is_direct_report_ind) {
		...
		{
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [direct_report_ind]");
			goto end;
		}
	}
	   ...
	   OpenAPI_indoor_outdoor_ind_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [indoor_outdoor_ind]");
			goto end;
		}
	}
	...
	if(location_data->accepted_periodic_event_info) {
		...
		if(accepted_periodic_event_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [accepted_periodic_event_info]");
			goto end;
		}
	...
	if(location_data->ha_gnss_metrics) {
		...
		if(ha_gnss_metrics_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [ha_gnss_metrics]");
			goto end;
		}
	   ...
	   OpenAPI_los_nlos_measure_ind_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [los_nlos_measure_ind]");
			goto end;
		}
	}
	...
	if(location_data->related_applicationlayer_id) {
		...
		{
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [related_applicationlayer_id]");
			goto end;
		}
	}
	...
	if(location_data->distance_direction) {
		...
		if(distance_direction_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [distance_direction]");
			goto end;
		}
	...
	if(location_data->_2d_relative_location) {
		...
		if(_2d_relative_location_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [_2d_relative_location]");
			goto end;
		}
	...
	if(location_data->_3d_relative_location) {
		...
		if(_3d_relative_location_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [_3d_relative_location]");
			goto end;
		}
	...
	if(location_data->relative_velocity) {
		...
		if(relative_velocity_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [relative_velocity]");
			goto end;
		}
	...
	if(location_data->ranging_sl_capability) {
		...
		{
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [ranging_sl_capability]");
			goto end;
		}
	}
	...
	if(location_data->integrity_result) {
		cJSON *integrity_result_local_JSON =
			OpenAPI_integrity_result_convertToJSON(
				location_data->integrity_result);
		if(integrity_result_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [integrity_result]");
			goto end;
		}
	...
	if(location_data->nrppa_periodic_ind !=
	   OpenAPI_location_data_NRPPAPERIODICIND_NULL)
	{
		if(cJSON_AddStringToObject(item, "nrppaPeriodicInd",
		                           OpenAPI_nrppa_periodic_indlocation_data_ToString
		                                   (location_data->
		                                   nrppa_periodic_ind)) == NULL)
		{
			ogs_error(
				"OpenAPI_location_data_convertToJSON() failed [nrppa_periodic_ind]");
			goto end;
		}
	}
...
OpenAPI_location_data_t *OpenAPI_location_data_parseFromJSON(
	cJSON *location_dataJSON) {
	...
	OpenAPI_location_data_remote_ue_ind_e remote_ue_indVariable = 0;
	cJSON *altitude = NULL;
	...
	OpenAPI_location_data_nrppa_periodic_ind_e nrppa_periodic_indVariable =
		0;
	location_estimate = cJSON_GetObjectItemCaseSensitive(location_dataJSON,
	                                                     "locationEstimate");
	if(!location_estimate) {
		ogs_error(
			"OpenAPI_location_data_parseFromJSON() failed [location_estimate]");
		goto end;
	}
	...
	accuracy_fulfilment_indicator =
		cJSON_GetObjectItemCaseSensitive(location_dataJSON,
		                                 "accuracyFulfilmentIndicator");
	if(accuracy_fulfilment_indicator) {
		if(!cJSON_IsString(accuracy_fulfilment_indicator)) {
			ogs_error(
				"OpenAPI_location_data_parseFromJSON() failed [accuracy_fulfilment_indicator]");
			goto end;
		}
	...
	if(age_of_location_estimate) {
		if(!cJSON_IsNumber(age_of_location_estimate)) {
			ogs_error(
				"OpenAPI_location_data_parseFromJSON() failed [age_of_location_estimate]");
			goto end;
		}
	}
	...
	if(timestamp_of_location_estimate) {
		...
		{
			ogs_error(
				"OpenAPI_location_data_parseFromJSON() failed [timestamp_of_location_estimate]");
			goto end;
		}
	}
	...
	if(positioning_data_list) {
		...
		if(!cJSON_IsArray(positioning_data_list)) {
			ogs_error(
				"OpenAPI_location_data_parseFromJSON() failed [positioning_data_list]");
			goto end;
		}
	...
	if(gnss_positioning_data_list) {
		...
		if(!cJSON_IsArray(gnss_positioning_data_list)) {
			ogs_error(
				"OpenAPI_location_data_parseFromJSON() failed [gnss_positioning_data_list]");
			goto end;
		}
	...
	remote_ue_ind = cJSON_GetObjectItemCaseSensitive(location_dataJSON,
	                                                 "remoteUeInd");
	if(remote_ue_ind) {
		if(!cJSON_IsString(remote_ue_ind)) {
			ogs_error(
				"OpenAPI_location_data_parseFromJSON() failed [remote_ue_ind]");
			goto end;
		}
		remote_ue_indVariable =
			OpenAPI_remote_ue_indlocation_data_FromString(
	...
	if(altitude) {
		if(!cJSON_IsNumber(altitude)) {
			ogs_error(
				"OpenAPI_location_data_parseFromJSON() failed [altitude]");
			goto end;
		}
	}
	...
	if(barometric_pressure) {
		if(!cJSON_IsNumber(barometric_pressure)) {
			ogs_error(
				"OpenAPI_location_data_parseFromJSON() failed [barometric_pressure]");
			goto end;
		}
	}
	...
	if(serving_lmf_identification) {
		...
		{
			ogs_error(
				"OpenAPI_location_data_parseFromJSON() failed [serving_lmf_identification]");
			goto end;
		}
	}
	...
	if(ue_positioning_cap) {
		...
		{
			ogs_error(
				"OpenAPI_location_data_parseFromJSON() failed [ue_positioning_cap]");
			goto end;
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_location_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(direct_report_ind) {
		if(!cJSON_IsBool(direct_report_ind)) {
			ogs_error(
				"OpenAPI_location_data_parseFromJSON() failed [direct_report_ind]");
			goto end;
		}
	}
	...
	if(indoor_outdoor_ind) {
		if(!cJSON_IsString(indoor_outdoor_ind)) {
			ogs_error(
				"OpenAPI_location_data_parseFromJSON() failed [indoor_outdoor_ind]");
			goto end;
		}
	...
	if(los_nlos_measure_ind) {
		if(!cJSON_IsString(los_nlos_measure_ind)) {
			ogs_error(
				"OpenAPI_location_data_parseFromJSON() failed [los_nlos_measure_ind]");
			goto end;
		}
	...
	if(related_applicationlayer_id) {
		...
		{
			ogs_error(
				"OpenAPI_location_data_parseFromJSON() failed [related_applicationlayer_id]");
			goto end;
		}
	}
	...
	if(ranging_sl_capability) {
		...
		{
			ogs_error(
				"OpenAPI_location_data_parseFromJSON() failed [ranging_sl_capability]");
			goto end;
		}
	}

	integrity_result = cJSON_GetObjectItemCaseSensitive(location_dataJSON,
	                                                    "integrityResult");
	if(integrity_result) {
	...
	nrppa_periodic_ind = cJSON_GetObjectItemCaseSensitive(location_dataJSON,
	                                                      "nrppaPeriodicInd");
	if(nrppa_periodic_ind) {
		if(!cJSON_IsString(nrppa_periodic_ind)) {
			ogs_error(
				"OpenAPI_location_data_parseFromJSON() failed [nrppa_periodic_ind]");
			goto end;
		}
		nrppa_periodic_indVariable =
			OpenAPI_nrppa_periodic_indlocation_data_FromString(
...
OpenAPI_location_data_t *OpenAPI_location_data_copy(
	OpenAPI_location_data_t *dst, OpenAPI_location_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
