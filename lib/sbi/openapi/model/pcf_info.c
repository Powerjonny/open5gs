...
OpenAPI_pcf_info_t *OpenAPI_pcf_info_create(char		*group_id,
                                            ...
                                            v2x_capability) {
	OpenAPI_pcf_info_t *pcf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_pcf_info_t));
...
cJSON *OpenAPI_pcf_info_convertToJSON(OpenAPI_pcf_info_t *pcf_info) {
	...
	if(pcf_info->group_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_info_convertToJSON() failed [group_id]");
			goto end;
		}
	}
	...
	if(pcf_info->dnn_list) {
		...
		if(dnn_listList == NULL) {
			ogs_error(
				"OpenAPI_pcf_info_convertToJSON() failed [dnn_list]");
			goto end;
		}
		OpenAPI_list_for_each(pcf_info->dnn_list, node) {
	...
	if(pcf_info->supi_ranges) {
		...
		if(supi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_pcf_info_convertToJSON() failed [supi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(pcf_info->supi_ranges, node) {
			cJSON *itemLocal =
	...
	if(pcf_info->gpsi_ranges) {
		...
		if(gpsi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_pcf_info_convertToJSON() failed [gpsi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(pcf_info->gpsi_ranges, node) {
			cJSON *itemLocal =
	...
	if(pcf_info->rx_diam_host) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_info_convertToJSON() failed [rx_diam_host]");
			goto end;
		}
	}
	...
	if(pcf_info->rx_diam_realm) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_info_convertToJSON() failed [rx_diam_realm]");
			goto end;
		}
	}
	...
	if(pcf_info->is_v2x_support_ind) {
		if(cJSON_AddBoolToObject(item, "v2xSupportInd",
		                         pcf_info->v2x_support_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_pcf_info_convertToJSON() failed [v2x_support_ind]");
			goto end;
		}
	}
	...
	if(pcf_info->is_prose_support_ind) {
		if(cJSON_AddBoolToObject(item, "proseSupportInd",
		                         pcf_info->prose_support_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_pcf_info_convertToJSON() failed [prose_support_ind]");
			goto end;
		}
	}
	...
	if(pcf_info->prose_capability) {
		cJSON *prose_capability_local_JSON =
			OpenAPI_pro_se_capability_convertToJSON(
				pcf_info->prose_capability);
		if(prose_capability_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_pcf_info_convertToJSON() failed [prose_capability]");
			goto end;
		}
	...
	if(pcf_info->v2x_capability) {
		cJSON *v2x_capability_local_JSON =
			OpenAPI_v2x_capability_convertToJSON(
				pcf_info->v2x_capability);
		if(v2x_capability_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_pcf_info_convertToJSON() failed [v2x_capability]");
			goto end;
		}
...
OpenAPI_pcf_info_t *OpenAPI_pcf_info_parseFromJSON(cJSON *pcf_infoJSON) {
	...
	if(group_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_info_parseFromJSON() failed [group_id]");
			goto end;
		}
	}
	...
	if(dnn_list) {
		...
		if(!cJSON_IsArray(dnn_list)) {
			ogs_error(
				"OpenAPI_pcf_info_parseFromJSON() failed [dnn_list]");
			goto end;
		}
	...
	supi_ranges = cJSON_GetObjectItemCaseSensitive(pcf_infoJSON,
	                                               "supiRanges");
	if(supi_ranges) {
		cJSON *supi_ranges_local = NULL;
		if(!cJSON_IsArray(supi_ranges)) {
			ogs_error(
				"OpenAPI_pcf_info_parseFromJSON() failed [supi_ranges]");
			goto end;
		}
	...
	gpsi_ranges = cJSON_GetObjectItemCaseSensitive(pcf_infoJSON,
	                                               "gpsiRanges");
	if(gpsi_ranges) {
		cJSON *gpsi_ranges_local = NULL;
		if(!cJSON_IsArray(gpsi_ranges)) {
			ogs_error(
				"OpenAPI_pcf_info_parseFromJSON() failed [gpsi_ranges]");
			goto end;
		}
	...
	rx_diam_host = cJSON_GetObjectItemCaseSensitive(pcf_infoJSON,
	                                                "rxDiamHost");
	if(rx_diam_host) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_info_parseFromJSON() failed [rx_diam_host]");
			goto end;
		}
	}

	rx_diam_realm = cJSON_GetObjectItemCaseSensitive(pcf_infoJSON,
	                                                 "rxDiamRealm");
	if(rx_diam_realm) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_info_parseFromJSON() failed [rx_diam_realm]");
			goto end;
		}
	}

	v2x_support_ind = cJSON_GetObjectItemCaseSensitive(pcf_infoJSON,
	                                                   "v2xSupportInd");
	if(v2x_support_ind) {
		if(!cJSON_IsBool(v2x_support_ind)) {
			ogs_error(
				"OpenAPI_pcf_info_parseFromJSON() failed [v2x_support_ind]");
			goto end;
		}
	}

	prose_support_ind = cJSON_GetObjectItemCaseSensitive(pcf_infoJSON,
	                                                     "proseSupportInd");
	if(prose_support_ind) {
		if(!cJSON_IsBool(prose_support_ind)) {
			ogs_error(
				"OpenAPI_pcf_info_parseFromJSON() failed [prose_support_ind]");
			goto end;
		}
	}

	prose_capability = cJSON_GetObjectItemCaseSensitive(pcf_infoJSON,
	                                                    "proseCapability");
	if(prose_capability) {
	...
	v2x_capability = cJSON_GetObjectItemCaseSensitive(pcf_infoJSON,
	                                                  "v2xCapability");
	if(v2x_capability) {
...
OpenAPI_pcf_info_t *OpenAPI_pcf_info_copy(OpenAPI_pcf_info_t	*dst,
                                          OpenAPI_pcf_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
