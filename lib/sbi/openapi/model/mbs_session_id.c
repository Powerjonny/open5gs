...
OpenAPI_mbs_session_id_t *OpenAPI_mbs_session_id_create(OpenAPI_tmgi_t	*tmgi,
                                                        OpenAPI_ssm_t	*ssm,
                                                        char		*nid) {
	OpenAPI_mbs_session_id_t *mbs_session_id_local_var =
		ogs_malloc(sizeof(OpenAPI_mbs_session_id_t));
...
cJSON *OpenAPI_mbs_session_id_convertToJSON(
	...
	if(mbs_session_id == NULL) {
		ogs_error(
			"OpenAPI_mbs_session_id_convertToJSON() failed [MbsSessionId]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(mbs_session_id->tmgi) {
		cJSON *tmgi_local_JSON =
			OpenAPI_tmgi_convertToJSON(mbs_session_id->tmgi);
		if(tmgi_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_mbs_session_id_convertToJSON() failed [tmgi]");
			goto end;
		}
	...
	if(mbs_session_id->ssm) {
		cJSON *ssm_local_JSON =
			OpenAPI_ssm_convertToJSON(mbs_session_id->ssm);
		if(ssm_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_mbs_session_id_convertToJSON() failed [ssm]");
			goto end;
		}
	...
	if(mbs_session_id->nid) {
		...
		{
			ogs_error(
				"OpenAPI_mbs_session_id_convertToJSON() failed [nid]");
			goto end;
		}
	}
...
OpenAPI_mbs_session_id_t *OpenAPI_mbs_session_id_parseFromJSON(
	cJSON *mbs_session_idJSON) {
	...
	if(nid) {
		...
		{
			ogs_error(
				"OpenAPI_mbs_session_id_parseFromJSON() failed [nid]");
			goto end;
		}
	}
...
OpenAPI_mbs_session_id_t *OpenAPI_mbs_session_id_copy(
	OpenAPI_mbs_session_id_t *dst, OpenAPI_mbs_session_id_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
