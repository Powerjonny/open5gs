...
OpenAPI_complex_query_t *OpenAPI_complex_query_create(
	...
	OpenAPI_list_t	  *
	dnf_units) {
	OpenAPI_complex_query_t *complex_query_local_var =
		ogs_malloc(sizeof(OpenAPI_complex_query_t));
...
cJSON *OpenAPI_complex_query_convertToJSON(
	OpenAPI_complex_query_t *complex_query) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(complex_query == NULL) {
		ogs_error(
			"OpenAPI_complex_query_convertToJSON() failed [ComplexQuery]");
		return NULL;
	}
	...
	if(!complex_query->cnf_units) {
		ogs_error(
			"OpenAPI_complex_query_convertToJSON() failed [cnf_units]");
		return NULL;
	}
	...
	if(!complex_query->dnf_units) {
		ogs_error(
			"OpenAPI_complex_query_convertToJSON() failed [dnf_units]");
		return NULL;
	}
...
OpenAPI_complex_query_t *OpenAPI_complex_query_parseFromJSON(
	cJSON *complex_queryJSON) {
	...
	cnf_units = cJSON_GetObjectItemCaseSensitive(complex_queryJSON,
	                                             "cnfUnits");
	if(!cnf_units) {
		ogs_error(
			"OpenAPI_complex_query_parseFromJSON() failed [cnf_units]");
		goto end;
	}
	...
	cJSON_ArrayForEach(cnf_units_local, cnf_units) {
		...
		OpenAPI_cnf_unit_t *cnf_unitsItem =
			OpenAPI_cnf_unit_parseFromJSON(cnf_units_local);
		if(!cnf_unitsItem) {
	...
	dnf_units = cJSON_GetObjectItemCaseSensitive(complex_queryJSON,
	                                             "dnfUnits");
	if(!dnf_units) {
		ogs_error(
	...
	cJSON_ArrayForEach(dnf_units_local, dnf_units) {
		...
		OpenAPI_dnf_unit_t *dnf_unitsItem =
			OpenAPI_dnf_unit_parseFromJSON(dnf_units_local);
		if(!dnf_unitsItem) {
...
OpenAPI_complex_query_t *OpenAPI_complex_query_copy(
	OpenAPI_complex_query_t *dst, OpenAPI_complex_query_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
