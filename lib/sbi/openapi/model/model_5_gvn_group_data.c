...
OpenAPI_model_5_gvn_group_data_t *OpenAPI_model_5_gvn_group_data_create(
	...
	OpenAPI_model_5_gvn_group_data_t *model_5_gvn_group_data_local_var =
		ogs_malloc(sizeof(OpenAPI_model_5_gvn_group_data_t));
	...
	model_5_gvn_group_data_local_var->is_dn_aaa_ip_address_allocation =
		is_dn_aaa_ip_address_allocation;
	model_5_gvn_group_data_local_var->dn_aaa_ip_address_allocation =
		dn_aaa_ip_address_allocation;
	model_5_gvn_group_data_local_var->dn_aaa_address = dn_aaa_address;
	model_5_gvn_group_data_local_var->additional_dn_aaa_addresses =
...
void OpenAPI_model_5_gvn_group_data_free(
	...
	if(model_5_gvn_group_data->dn_aaa_address) {
		OpenAPI_ip_address_1_free(
			model_5_gvn_group_data->dn_aaa_address);
...
cJSON *OpenAPI_model_5_gvn_group_data_convertToJSON(
	...
	if(model_5_gvn_group_data == NULL) {
		ogs_error(
			"OpenAPI_model_5_gvn_group_data_convertToJSON() failed [5GVnGroupData]");
		return NULL;
	}
	...
	if(!model_5_gvn_group_data->dnn) {
		ogs_error(
			"OpenAPI_model_5_gvn_group_data_convertToJSON() failed [dnn]");
		return NULL;
	}
	...
	if(!model_5_gvn_group_data->s_nssai) {
		ogs_error(
			"OpenAPI_model_5_gvn_group_data_convertToJSON() failed [s_nssai]");
		return NULL;
	}
	cJSON *s_nssai_local_JSON =
		OpenAPI_snssai_convertToJSON(model_5_gvn_group_data->s_nssai);
	   ...
	   OpenAPI_pdu_session_type_NULL)
	{
		...
		if(pdu_session_typesList == NULL) {
			ogs_error(
				"OpenAPI_model_5_gvn_group_data_convertToJSON() failed [pdu_session_types]");
			goto end;
		}
		OpenAPI_list_for_each(model_5_gvn_group_data->pdu_session_types,
		                      node) {
	...
	if(model_5_gvn_group_data->app_descriptors) {
		cJSON *app_descriptorsList = cJSON_AddArrayToObject(item,
		                                                    "appDescriptors");
		if(app_descriptorsList == NULL) {
			ogs_error(
				"OpenAPI_model_5_gvn_group_data_convertToJSON() failed [app_descriptors]");
			goto end;
		}
		OpenAPI_list_for_each(model_5_gvn_group_data->app_descriptors,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_app_descriptor_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_model_5_gvn_group_data_convertToJSON() failed [app_descriptors]");
	...
	if(model_5_gvn_group_data->is_secondary_auth) {
		if(cJSON_AddBoolToObject(item, "secondaryAuth",
		                         model_5_gvn_group_data->secondary_auth)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_model_5_gvn_group_data_convertToJSON() failed [secondary_auth]");
			goto end;
		}
	}
	...
	if(model_5_gvn_group_data->is_dn_aaa_ip_address_allocation) {
		...
		{
			ogs_error(
				"OpenAPI_model_5_gvn_group_data_convertToJSON() failed [dn_aaa_ip_address_allocation]");
			goto end;
		}
	}
	...
	if(model_5_gvn_group_data->dn_aaa_address) {
		...
		if(dn_aaa_address_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_model_5_gvn_group_data_convertToJSON() failed [dn_aaa_address]");
			goto end;
		}
	...
	if(model_5_gvn_group_data->additional_dn_aaa_addresses) {
		...
		if(additional_dn_aaa_addressesList == NULL) {
			ogs_error(
				"OpenAPI_model_5_gvn_group_data_convertToJSON() failed [additional_dn_aaa_addresses]");
			goto end;
		}
		OpenAPI_list_for_each(
			model_5_gvn_group_data->additional_dn_aaa_addresses,
	...
	if(model_5_gvn_group_data->dn_aaa_fqdn) {
		if(cJSON_AddStringToObject(item, "dnAaaFqdn",
		                           model_5_gvn_group_data->dn_aaa_fqdn)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_model_5_gvn_group_data_convertToJSON() failed [dn_aaa_fqdn]");
			goto end;
		}
	}
...
OpenAPI_model_5_gvn_group_data_t *OpenAPI_model_5_gvn_group_data_parseFromJSON(
	...
	dnn = cJSON_GetObjectItemCaseSensitive(model_5_gvn_group_dataJSON,
	                                       "dnn");
	if(!dnn) {
		ogs_error(
			"OpenAPI_model_5_gvn_group_data_parseFromJSON() failed [dnn]");
		goto end;
	}
	...
	s_nssai = cJSON_GetObjectItemCaseSensitive(model_5_gvn_group_dataJSON,
	                                           "sNssai");
	if(!s_nssai) {
		ogs_error(
			"OpenAPI_model_5_gvn_group_data_parseFromJSON() failed [s_nssai]");
		goto end;
	}
	...
	pdu_session_types =
		cJSON_GetObjectItemCaseSensitive(model_5_gvn_group_dataJSON,
		                                 "pduSessionTypes");
	if(pdu_session_types) {
		cJSON *pdu_session_types_local = NULL;
		if(!cJSON_IsArray(pdu_session_types)) {
			ogs_error(
				"OpenAPI_model_5_gvn_group_data_parseFromJSON() failed [pdu_session_types]");
			goto end;
		}
		...
		if(pdu_session_typesList->count == 0) {
			ogs_error(
				"OpenAPI_model_5_gvn_group_data_parseFromJSON() failed: Expected pdu_session_typesList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(app_descriptors) {
		...
		if(!cJSON_IsArray(app_descriptors)) {
			ogs_error(
				"OpenAPI_model_5_gvn_group_data_parseFromJSON() failed [app_descriptors]");
			goto end;
		}
		...
		cJSON_ArrayForEach(app_descriptors_local, app_descriptors) {
			...
			OpenAPI_app_descriptor_t *app_descriptorsItem =
				OpenAPI_app_descriptor_parseFromJSON(
					app_descriptors_local);
			if(!app_descriptorsItem) {
				ogs_error("No app_descriptorsItem");
				goto end;
			}
			OpenAPI_list_add(app_descriptorsList,
			                 app_descriptorsItem);
		}
	}
	...
	if(secondary_auth) {
		if(!cJSON_IsBool(secondary_auth)) {
			ogs_error(
				"OpenAPI_model_5_gvn_group_data_parseFromJSON() failed [secondary_auth]");
			goto end;
		}
	}
	...
	if(dn_aaa_ip_address_allocation) {
		if(!cJSON_IsBool(dn_aaa_ip_address_allocation)) {
			ogs_error(
				"OpenAPI_model_5_gvn_group_data_parseFromJSON() failed [dn_aaa_ip_address_allocation]");
			goto end;
		}
	}
	...
	if(dn_aaa_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_model_5_gvn_group_data_parseFromJSON() failed [dn_aaa_fqdn]");
			goto end;
		}
	}

	model_5_gvn_group_data_local_var =
...
OpenAPI_model_5_gvn_group_data_t *OpenAPI_model_5_gvn_group_data_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_model_5_gvn_group_data_convertToJSON() failed");
		return NULL;
	}
