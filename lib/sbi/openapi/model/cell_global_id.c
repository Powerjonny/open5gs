...
OpenAPI_cell_global_id_t *OpenAPI_cell_global_id_create(
	...
	OpenAPI_cell_global_id_t *cell_global_id_local_var =
		ogs_malloc(sizeof(OpenAPI_cell_global_id_t));
...
cJSON *OpenAPI_cell_global_id_convertToJSON(
	...
	if(cell_global_id == NULL) {
		ogs_error(
			"OpenAPI_cell_global_id_convertToJSON() failed [CellGlobalId]");
		return NULL;
	}
	...
	if(!cell_global_id->plmn_id) {
		ogs_error(
			"OpenAPI_cell_global_id_convertToJSON() failed [plmn_id]");
		return NULL;
	}
	cJSON *plmn_id_local_JSON =
		OpenAPI_plmn_id_convertToJSON(cell_global_id->plmn_id);
	...
	if(!cell_global_id->lac) {
		ogs_error(
			"OpenAPI_cell_global_id_convertToJSON() failed [lac]");
		return NULL;
	}
	...
	if(!cell_global_id->cell_id) {
		ogs_error(
			"OpenAPI_cell_global_id_convertToJSON() failed [cell_id]");
		return NULL;
	}
...
OpenAPI_cell_global_id_t *OpenAPI_cell_global_id_parseFromJSON(
	...
	cJSON *lac = NULL;
	cJSON *cell_id = NULL;
	plmn_id = cJSON_GetObjectItemCaseSensitive(cell_global_idJSON,
	                                           "plmnId");
	if(!plmn_id) {
		ogs_error(
			"OpenAPI_cell_global_id_parseFromJSON() failed [plmn_id]");
		goto end;
	}
	...
	if(!lac) {
		ogs_error(
			"OpenAPI_cell_global_id_parseFromJSON() failed [lac]");
		goto end;
	}
	...
	cell_id = cJSON_GetObjectItemCaseSensitive(cell_global_idJSON,
	                                           "cellId");
	if(!cell_id) {
		ogs_error(
			"OpenAPI_cell_global_id_parseFromJSON() failed [cell_id]");
		goto end;
	}
