...
OpenAPI_pdu_session_types_1_t *OpenAPI_pdu_session_types_1_create(
	...
	OpenAPI_pdu_session_types_1_t *pdu_session_types_1_local_var =
		ogs_malloc(sizeof(OpenAPI_pdu_session_types_1_t));
	ogs_assert(pdu_session_types_1_local_var);

	pdu_session_types_1_local_var->default_session_type =
		default_session_type;
	pdu_session_types_1_local_var->allowed_session_types =
...
void OpenAPI_pdu_session_types_1_free(
	OpenAPI_pdu_session_types_1_t *pdu_session_types_1) {
...
cJSON *OpenAPI_pdu_session_types_1_convertToJSON(
	...
	if(pdu_session_types_1 == NULL) {
		ogs_error(
			"OpenAPI_pdu_session_types_1_convertToJSON() failed [PduSessionTypes_1]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(pdu_session_types_1->default_session_type !=
	   OpenAPI_pdu_session_type_NULL)
	{
		if(cJSON_AddStringToObject(item, "defaultSessionType",
		...
		{
			ogs_error(
				"OpenAPI_pdu_session_types_1_convertToJSON() failed [default_session_type]");
			goto end;
		}
	}
	...
	if(pdu_session_types_1->allowed_session_types !=
	   OpenAPI_pdu_session_type_NULL)
	{
		cJSON *allowed_session_typesList = cJSON_AddArrayToObject(item,
		                                                          "allowedSessionTypes");
		if(allowed_session_typesList == NULL) {
			ogs_error(
				"OpenAPI_pdu_session_types_1_convertToJSON() failed [allowed_session_types]");
			goto end;
		}
		OpenAPI_list_for_each(
			pdu_session_types_1->allowed_session_types, node) {
			if(cJSON_AddStringToObject(allowed_session_typesList,
			                           "",
			                           OpenAPI_pdu_session_type_ToString
			                                   ((intptr_t) node->
			                                   data)) == NULL)
...
OpenAPI_pdu_session_types_1_t *OpenAPI_pdu_session_types_1_parseFromJSON(
	cJSON *pdu_session_types_1JSON) {
	...
	default_session_type =
		cJSON_GetObjectItemCaseSensitive(pdu_session_types_1JSON,
		                                 "defaultSessionType");
	if(default_session_type) {
		if(!cJSON_IsString(default_session_type)) {
			ogs_error(
				"OpenAPI_pdu_session_types_1_parseFromJSON() failed [default_session_type]");
			goto end;
		}
		default_session_typeVariable =
			OpenAPI_pdu_session_type_FromString(
				default_session_type->valuestring);
	}
	...
	if(allowed_session_types) {
		...
		if(!cJSON_IsArray(allowed_session_types)) {
			ogs_error(
				"OpenAPI_pdu_session_types_1_parseFromJSON() failed [allowed_session_types]");
			goto end;
		}

		allowed_session_typesList = OpenAPI_list_create();

		cJSON_ArrayForEach(allowed_session_types_local,
		                   allowed_session_types) {
			...
			if(!localEnum) {
				ogs_info(
					"Enum value \"%s\" for field \"allowed_session_types\" is not supported. Ignoring it ...",
					allowed_session_types_local->
					valuestring);
			} else {
				OpenAPI_list_add(allowed_session_typesList,
				                 (void *) localEnum);
			}
		}
		if(allowed_session_typesList->count == 0) {
			ogs_error(
				"OpenAPI_pdu_session_types_1_parseFromJSON() failed: Expected allowed_session_typesList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
...
OpenAPI_pdu_session_types_1_t *OpenAPI_pdu_session_types_1_copy(
	OpenAPI_pdu_session_types_1_t	*dst,
	OpenAPI_pdu_session_types_1_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
