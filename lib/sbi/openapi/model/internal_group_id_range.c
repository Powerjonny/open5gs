...
OpenAPI_internal_group_id_range_t *OpenAPI_internal_group_id_range_create(
	...
	OpenAPI_internal_group_id_range_t *internal_group_id_range_local_var =
		ogs_malloc(sizeof(OpenAPI_internal_group_id_range_t));
...
cJSON *OpenAPI_internal_group_id_range_convertToJSON(
	...
	if(internal_group_id_range == NULL) {
		ogs_error(
			"OpenAPI_internal_group_id_range_convertToJSON() failed [InternalGroupIdRange]");
		return NULL;
	}
	...
	if(internal_group_id_range->start) {
		if(cJSON_AddStringToObject(item, "start",
		                           internal_group_id_range->start) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_internal_group_id_range_convertToJSON() failed [start]");
			goto end;
		}
	}
	...
	if(internal_group_id_range->end) {
		if(cJSON_AddStringToObject(item, "end",
		                           internal_group_id_range->end) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_internal_group_id_range_convertToJSON() failed [end]");
			goto end;
		}
	}
	...
	if(internal_group_id_range->pattern) {
		if(cJSON_AddStringToObject(item, "pattern",
		                           internal_group_id_range->pattern) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_internal_group_id_range_convertToJSON() failed [pattern]");
			goto end;
		}
	}
...
OpenAPI_internal_group_id_range_t *OpenAPI_internal_group_id_range_parseFromJSON
	...
	cJSON *end = NULL;
	cJSON *pattern = NULL;
	start = cJSON_GetObjectItemCaseSensitive(internal_group_id_rangeJSON,
	                                         "start");
	if(start) {
		...
		{
			ogs_error(
				"OpenAPI_internal_group_id_range_parseFromJSON() failed [start]");
			goto end;
		}
	}

	end = cJSON_GetObjectItemCaseSensitive(internal_group_id_rangeJSON,
	                                       "end");
	if(end) {
		...
		{
			ogs_error(
				"OpenAPI_internal_group_id_range_parseFromJSON() failed [end]");
			goto end;
		}
	}

	pattern = cJSON_GetObjectItemCaseSensitive(internal_group_id_rangeJSON,
	                                           "pattern");
	if(pattern) {
		...
		{
			ogs_error(
				"OpenAPI_internal_group_id_range_parseFromJSON() failed [pattern]");
			goto end;
		}
	}

	internal_group_id_range_local_var =
		OpenAPI_internal_group_id_range_create(
			start &&
			!cJSON_IsNull(start) ? ogs_strdup(start->valuestring) :
			NULL,
			end &&
			!cJSON_IsNull(end) ? ogs_strdup(end->valuestring) :
			NULL,
			pattern &&
			!cJSON_IsNull(pattern) ?
			ogs_strdup(pattern->valuestring) : NULL
			);
...
OpenAPI_internal_group_id_range_t *OpenAPI_internal_group_id_range_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_internal_group_id_range_convertToJSON() failed");
		return NULL;
	}
