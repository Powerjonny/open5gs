...
OpenAPI_allowed_nssai_t *OpenAPI_allowed_nssai_create(
	...
	OpenAPI_allowed_nssai_t *allowed_nssai_local_var =
		ogs_malloc(sizeof(OpenAPI_allowed_nssai_t));
...
void OpenAPI_allowed_nssai_free(OpenAPI_allowed_nssai_t *allowed_nssai) {
	...
	if(allowed_nssai->allowed_snssai_list) {
		OpenAPI_list_for_each(allowed_nssai->allowed_snssai_list,
		                      node) {
...
cJSON *OpenAPI_allowed_nssai_convertToJSON(
	...
	if(allowed_nssai == NULL) {
		ogs_error(
			"OpenAPI_allowed_nssai_convertToJSON() failed [AllowedNssai]");
		return NULL;
	}
	...
	if(!allowed_nssai->allowed_snssai_list) {
		ogs_error(
			"OpenAPI_allowed_nssai_convertToJSON() failed [allowed_snssai_list]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(allowed_nssai->allowed_snssai_list, node) {
		cJSON *itemLocal =
			OpenAPI_allowed_snssai_convertToJSON(node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_allowed_nssai_convertToJSON() failed [allowed_snssai_list]");
			goto end;
	...
	if(allowed_nssai->access_type == OpenAPI_access_type_NULL) {
		ogs_error(
			"OpenAPI_allowed_nssai_convertToJSON() failed [access_type]");
		return NULL;
	}
...
OpenAPI_allowed_nssai_t *OpenAPI_allowed_nssai_parseFromJSON(
	cJSON *allowed_nssaiJSON) {
	...
	allowed_snssai_list =
		cJSON_GetObjectItemCaseSensitive(allowed_nssaiJSON,
		                                 "allowedSnssaiList");
	if(!allowed_snssai_list) {
		ogs_error(
			"OpenAPI_allowed_nssai_parseFromJSON() failed [allowed_snssai_list]");
		goto end;
	}
	...
	cJSON_ArrayForEach(allowed_snssai_list_local, allowed_snssai_list) {
		...
		OpenAPI_allowed_snssai_t *allowed_snssai_listItem =
	...
	if(!access_type) {
		ogs_error(
			"OpenAPI_allowed_nssai_parseFromJSON() failed [access_type]");
		goto end;
	}
...
OpenAPI_allowed_nssai_t *OpenAPI_allowed_nssai_copy(
	OpenAPI_allowed_nssai_t *dst, OpenAPI_allowed_nssai_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
