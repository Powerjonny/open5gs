...
char * OpenAPI_subscribed_event_any_of_ToString(
	...
	const char *subscribed_event_any_ofArray[] = { "NULL",
		                                       "UP_PATH_CHANGE" };
	size_t sizeofArray = sizeof(subscribed_event_any_ofArray) /
	                     sizeof(subscribed_event_any_ofArray[0]);
	if(subscribed_event_any_of < sizeofArray) {
		return (char *) subscribed_event_any_ofArray[
			subscribed_event_any_of];
	} else {
		return (char *) "Unknown";
	}
}
...
OpenAPI_subscribed_event_any_of_e OpenAPI_subscribed_event_any_of_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(subscribed_event_any_of,
		          subscribed_event_any_ofArray[stringToReturn]) == 0) {
