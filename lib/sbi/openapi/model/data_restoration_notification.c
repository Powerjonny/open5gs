...
OpenAPI_data_restoration_notification_create(
	...
	data_restoration_notification_local_var->last_replication_time =
	...
	OpenAPI_data_restoration_notification_t *data_restoration_notification)
{
	...
	if(data_restoration_notification->supi_ranges) {
		OpenAPI_list_for_each(
			data_restoration_notification->supi_ranges, node) {
	...
	if(data_restoration_notification->gpsi_ranges) {
		OpenAPI_list_for_each(
			data_restoration_notification->gpsi_ranges, node) {
	...
	if(data_restoration_notification->reset_ids) {
		OpenAPI_list_for_each(data_restoration_notification->reset_ids,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(data_restoration_notification->s_nssai_list) {
		OpenAPI_list_for_each(
			data_restoration_notification->s_nssai_list, node) {
	...
	if(data_restoration_notification->dnn_list) {
		OpenAPI_list_for_each(data_restoration_notification->dnn_list,
		                      node) {
			ogs_free(node->data);
		}
	...
	OpenAPI_data_restoration_notification_t *data_restoration_notification)
{
	...
	if(data_restoration_notification == NULL) {
		ogs_error(
			"OpenAPI_data_restoration_notification_convertToJSON() failed [DataRestorationNotification]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(data_restoration_notification->last_replication_time) {
		if(cJSON_AddStringToObject(item, "lastReplicationTime",
		                           data_restoration_notification->
		                           last_replication_time) == NULL)
		{
			ogs_error(
				"OpenAPI_data_restoration_notification_convertToJSON() failed [last_replication_time]");
			goto end;
		}
	}
	...
	if(data_restoration_notification->recovery_time) {
		...
		{
			ogs_error(
				"OpenAPI_data_restoration_notification_convertToJSON() failed [recovery_time]");
			goto end;
		}
	}
	...
	if(data_restoration_notification->plmn_id) {
		cJSON *plmn_id_local_JSON =
			OpenAPI_plmn_id_convertToJSON(
				data_restoration_notification->plmn_id);
		if(plmn_id_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_data_restoration_notification_convertToJSON() failed [plmn_id]");
			goto end;
		}
	...
	if(data_restoration_notification->supi_ranges) {
		...
		if(supi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_data_restoration_notification_convertToJSON() failed [supi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			data_restoration_notification->supi_ranges, node) {
	...
	if(data_restoration_notification->gpsi_ranges) {
		...
		if(gpsi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_data_restoration_notification_convertToJSON() failed [gpsi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			data_restoration_notification->gpsi_ranges, node) {
	...
	if(data_restoration_notification->reset_ids) {
		...
		if(reset_idsList == NULL) {
			ogs_error(
				"OpenAPI_data_restoration_notification_convertToJSON() failed [reset_ids]");
			goto end;
		}
		OpenAPI_list_for_each(data_restoration_notification->reset_ids,
		                      node) {
	...
	if(data_restoration_notification->s_nssai_list) {
		cJSON *s_nssai_listList = cJSON_AddArrayToObject(item,
		                                                 "sNssaiList");
		if(s_nssai_listList == NULL) {
			ogs_error(
				"OpenAPI_data_restoration_notification_convertToJSON() failed [s_nssai_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			data_restoration_notification->s_nssai_list, node) {
	...
	if(data_restoration_notification->dnn_list) {
		...
		if(dnn_listList == NULL) {
			ogs_error(
				"OpenAPI_data_restoration_notification_convertToJSON() failed [dnn_list]");
			goto end;
		}
		OpenAPI_list_for_each(data_restoration_notification->dnn_list,
		                      node) {
	...
	if(data_restoration_notification->udm_group_id) {
		...
		{
			ogs_error(
				"OpenAPI_data_restoration_notification_convertToJSON() failed [udm_group_id]");
			goto end;
		}
	}
...
OpenAPI_data_restoration_notification_parseFromJSON(
	...
	OpenAPI_data_restoration_notification_t *
	        data_restoration_notification_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	last_replication_time =
		cJSON_GetObjectItemCaseSensitive(
			data_restoration_notificationJSON,
			"lastReplicationTime");
	if(last_replication_time) {
		if(!cJSON_IsString(last_replication_time) &&
		   !cJSON_IsNull(last_replication_time))
		{
			ogs_error(
				"OpenAPI_data_restoration_notification_parseFromJSON() failed [last_replication_time]");
			goto end;
		}
	}
	...
	recovery_time =
		cJSON_GetObjectItemCaseSensitive(
			data_restoration_notificationJSON, "recoveryTime");
	if(recovery_time) {
		if(!cJSON_IsString(recovery_time) &&
		   !cJSON_IsNull(recovery_time))
		{
			ogs_error(
				"OpenAPI_data_restoration_notification_parseFromJSON() failed [recovery_time]");
			goto end;
		}
	}
	...
	plmn_id =
		cJSON_GetObjectItemCaseSensitive(
			data_restoration_notificationJSON, "plmnId");
	if(plmn_id) {
	...
	supi_ranges =
		cJSON_GetObjectItemCaseSensitive(
			data_restoration_notificationJSON, "supiRanges");
	if(supi_ranges) {
		cJSON *supi_ranges_local = NULL;
		if(!cJSON_IsArray(supi_ranges)) {
			ogs_error(
				"OpenAPI_data_restoration_notification_parseFromJSON() failed [supi_ranges]");
			goto end;
		}
	...
	gpsi_ranges =
		cJSON_GetObjectItemCaseSensitive(
			data_restoration_notificationJSON, "gpsiRanges");
	if(gpsi_ranges) {
		cJSON *gpsi_ranges_local = NULL;
		if(!cJSON_IsArray(gpsi_ranges)) {
			ogs_error(
				"OpenAPI_data_restoration_notification_parseFromJSON() failed [gpsi_ranges]");
			goto end;
		}
	...
	reset_ids =
		cJSON_GetObjectItemCaseSensitive(
			data_restoration_notificationJSON, "resetIds");
	if(reset_ids) {
		cJSON *reset_ids_local = NULL;
		if(!cJSON_IsArray(reset_ids)) {
			ogs_error(
				"OpenAPI_data_restoration_notification_parseFromJSON() failed [reset_ids]");
			goto end;
		}
	...
	s_nssai_list =
		cJSON_GetObjectItemCaseSensitive(
			data_restoration_notificationJSON, "sNssaiList");
	if(s_nssai_list) {
		cJSON *s_nssai_list_local = NULL;
		if(!cJSON_IsArray(s_nssai_list)) {
			ogs_error(
				"OpenAPI_data_restoration_notification_parseFromJSON() failed [s_nssai_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(s_nssai_list_local, s_nssai_list) {
			...
			OpenAPI_snssai_t *s_nssai_listItem =
	...
	dnn_list =
		cJSON_GetObjectItemCaseSensitive(
			data_restoration_notificationJSON, "dnnList");
	if(dnn_list) {
		cJSON *dnn_list_local = NULL;
		if(!cJSON_IsArray(dnn_list)) {
			ogs_error(
				"OpenAPI_data_restoration_notification_parseFromJSON() failed [dnn_list]");
			goto end;
		}
	...
	udm_group_id =
		cJSON_GetObjectItemCaseSensitive(
			data_restoration_notificationJSON, "udmGroupId");
	if(udm_group_id) {
		if(!cJSON_IsString(udm_group_id) &&
		   !cJSON_IsNull(udm_group_id))
		{
			ogs_error(
				"OpenAPI_data_restoration_notification_parseFromJSON() failed [udm_group_id]");
			goto end;
		}
	}
	...
	data_restoration_notification_local_var =
		OpenAPI_data_restoration_notification_create(
			last_replication_time &&
			!cJSON_IsNull(last_replication_time) ?
			ogs_strdup(last_replication_time->valuestring) : NULL,
			recovery_time &&
			!cJSON_IsNull(recovery_time) ?
...
OpenAPI_data_restoration_notification_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_data_restoration_notification_convertToJSON() failed");
		return NULL;
	}
