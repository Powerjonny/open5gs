...
OpenAPI_report_item_t *OpenAPI_report_item_create(char *path, char *reason) {
	OpenAPI_report_item_t *report_item_local_var =
		ogs_malloc(sizeof(OpenAPI_report_item_t));
...
cJSON *OpenAPI_report_item_convertToJSON(OpenAPI_report_item_t *report_item) {
	...
	if(report_item == NULL) {
		ogs_error(
			"OpenAPI_report_item_convertToJSON() failed [ReportItem]");
		return NULL;
	}
	...
	if(report_item->reason) {
		...
		{
			ogs_error(
				"OpenAPI_report_item_convertToJSON() failed [reason]");
			goto end;
		}
	}
...
OpenAPI_report_item_t *OpenAPI_report_item_parseFromJSON(
	...
	if(reason) {
		...
		{
			ogs_error(
				"OpenAPI_report_item_parseFromJSON() failed [reason]");
			goto end;
		}
	}
