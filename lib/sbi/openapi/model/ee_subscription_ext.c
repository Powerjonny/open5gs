...
OpenAPI_ee_subscription_ext_t *OpenAPI_ee_subscription_ext_create(
	char *callback_reference, OpenAPI_list_t *monitoring_configurations,
	OpenAPI_reporting_options_1_t *reporting_options,
	char *supported_features, char *subscription_id,
	OpenAPI_context_info_t *context_info, bool is_epc_applied_ind,
	int epc_applied_ind, char *scef_diam_host, char *scef_diam_realm,
	char *notify_correlation_id, char *second_callback_ref, char *gpsi,
	OpenAPI_list_t *exclude_gpsi_list, OpenAPI_list_t *include_gpsi_list,
	...
	OpenAPI_ee_subscription_ext_t *ee_subscription_ext_local_var =
		ogs_malloc(sizeof(OpenAPI_ee_subscription_ext_t));
	ogs_assert(ee_subscription_ext_local_var);

	ee_subscription_ext_local_var->callback_reference = callback_reference;
	ee_subscription_ext_local_var->monitoring_configurations =
	...
	ee_subscription_ext_local_var->notify_correlation_id =
		notify_correlation_id;
	ee_subscription_ext_local_var->second_callback_ref =
		second_callback_ref;
	...
	ee_subscription_ext_local_var->data_restoration_callback_uri =
		data_restoration_callback_uri;
	ee_subscription_ext_local_var->amf_subscription_info_list =
		amf_subscription_info_list;
	ee_subscription_ext_local_var->smf_subscription_info =
		smf_subscription_info;
	ee_subscription_ext_local_var->hss_subscription_info =
...
void OpenAPI_ee_subscription_ext_free(
	OpenAPI_ee_subscription_ext_t *ee_subscription_ext) {
	...
	if(ee_subscription_ext->monitoring_configurations) {
		OpenAPI_list_for_each(
			ee_subscription_ext->monitoring_configurations, node) {
			OpenAPI_map_t *localKeyValue =
				(OpenAPI_map_t *) node->data;
			ogs_free(localKeyValue->key);
			OpenAPI_monitoring_configuration_1_free(
				localKeyValue->value);
			OpenAPI_map_free(localKeyValue);
		}
		OpenAPI_list_free(
			ee_subscription_ext->monitoring_configurations);
		ee_subscription_ext->monitoring_configurations = NULL;
	}
	if(ee_subscription_ext->reporting_options) {
		OpenAPI_reporting_options_1_free(
			ee_subscription_ext->reporting_options);
	...
	if(ee_subscription_ext->exclude_gpsi_list) {
		OpenAPI_list_for_each(ee_subscription_ext->exclude_gpsi_list,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(ee_subscription_ext->include_gpsi_list) {
		OpenAPI_list_for_each(ee_subscription_ext->include_gpsi_list,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(ee_subscription_ext->amf_subscription_info_list) {
		OpenAPI_list_for_each(
			ee_subscription_ext->amf_subscription_info_list, node) {
			OpenAPI_amf_subscription_info_free(node->data);
		}
		OpenAPI_list_free(
			ee_subscription_ext->amf_subscription_info_list);
		ee_subscription_ext->amf_subscription_info_list = NULL;
	}
	if(ee_subscription_ext->smf_subscription_info) {
		OpenAPI_smf_subscription_info_free(
			ee_subscription_ext->smf_subscription_info);
		ee_subscription_ext->smf_subscription_info = NULL;
	}
	if(ee_subscription_ext->hss_subscription_info) {
		OpenAPI_hss_subscription_info_free(
			ee_subscription_ext->hss_subscription_info);
...
cJSON *OpenAPI_ee_subscription_ext_convertToJSON(
	...
	if(ee_subscription_ext == NULL) {
		ogs_error(
			"OpenAPI_ee_subscription_ext_convertToJSON() failed [EeSubscriptionExt]");
		return NULL;
	}
	...
	if(!ee_subscription_ext->callback_reference) {
		ogs_error(
			"OpenAPI_ee_subscription_ext_convertToJSON() failed [callback_reference]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "callbackReference",
	                           ee_subscription_ext->callback_reference) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_ee_subscription_ext_convertToJSON() failed [callback_reference]");
	...
	if(!ee_subscription_ext->monitoring_configurations) {
		ogs_error(
			"OpenAPI_ee_subscription_ext_convertToJSON() failed [monitoring_configurations]");
		return NULL;
	}
	cJSON *monitoring_configurations = cJSON_AddObjectToObject(item,
	...
	if(ee_subscription_ext->monitoring_configurations) {
		OpenAPI_list_for_each(
			...
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_ee_subscription_ext_convertToJSON() failed [inner]");
				goto end;
			}
	...
	if(ee_subscription_ext->reporting_options) {
		cJSON *reporting_options_local_JSON =
			OpenAPI_reporting_options_1_convertToJSON(
				ee_subscription_ext->reporting_options);
		if(reporting_options_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ee_subscription_ext_convertToJSON() failed [reporting_options]");
			goto end;
		}
		cJSON_AddItemToObject(item, "reportingOptions",
		                      reporting_options_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_ee_subscription_ext_convertToJSON() failed [reporting_options]");
			goto end;
	...
	if(ee_subscription_ext->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_ee_subscription_ext_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(ee_subscription_ext->subscription_id) {
		if(cJSON_AddStringToObject(item, "subscriptionId",
		                           ee_subscription_ext->subscription_id)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_ee_subscription_ext_convertToJSON() failed [subscription_id]");
			goto end;
		}
	}
	...
	if(ee_subscription_ext->context_info) {
		cJSON *context_info_local_JSON =
			OpenAPI_context_info_convertToJSON(
				ee_subscription_ext->context_info);
		if(context_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ee_subscription_ext_convertToJSON() failed [context_info]");
			goto end;
		}
	...
	if(ee_subscription_ext->is_epc_applied_ind) {
		if(cJSON_AddBoolToObject(item, "epcAppliedInd",
		                         ee_subscription_ext->epc_applied_ind)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_ee_subscription_ext_convertToJSON() failed [epc_applied_ind]");
			goto end;
		}
	}

	if(ee_subscription_ext->scef_diam_host) {
		if(cJSON_AddStringToObject(item, "scefDiamHost",
		                           ee_subscription_ext->scef_diam_host)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_ee_subscription_ext_convertToJSON() failed [scef_diam_host]");
			goto end;
		}
	}

	if(ee_subscription_ext->scef_diam_realm) {
		if(cJSON_AddStringToObject(item, "scefDiamRealm",
		                           ee_subscription_ext->scef_diam_realm)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_ee_subscription_ext_convertToJSON() failed [scef_diam_realm]");
			goto end;
		}
	}
	...
	if(ee_subscription_ext->notify_correlation_id) {
		...
		{
			ogs_error(
				"OpenAPI_ee_subscription_ext_convertToJSON() failed [notify_correlation_id]");
			goto end;
		}
	}

	if(ee_subscription_ext->second_callback_ref) {
		if(cJSON_AddStringToObject(item, "secondCallbackRef",
		                           ee_subscription_ext->
		                           second_callback_ref) == NULL)
		{
			ogs_error(
				"OpenAPI_ee_subscription_ext_convertToJSON() failed [second_callback_ref]");
			goto end;
		}
	}
	...
	if(ee_subscription_ext->gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_ee_subscription_ext_convertToJSON() failed [gpsi]");
			goto end;
		}
	}
	...
	if(ee_subscription_ext->exclude_gpsi_list) {
		...
		if(exclude_gpsi_listList == NULL) {
			ogs_error(
				"OpenAPI_ee_subscription_ext_convertToJSON() failed [exclude_gpsi_list]");
			goto end;
		}
		OpenAPI_list_for_each(ee_subscription_ext->exclude_gpsi_list,
		                      node) {
	...
	if(ee_subscription_ext->include_gpsi_list) {
		...
		if(include_gpsi_listList == NULL) {
			ogs_error(
				"OpenAPI_ee_subscription_ext_convertToJSON() failed [include_gpsi_list]");
			goto end;
		}
		OpenAPI_list_for_each(ee_subscription_ext->include_gpsi_list,
		                      node) {
	...
	if(ee_subscription_ext->data_restoration_callback_uri) {
		...
		{
			ogs_error(
				"OpenAPI_ee_subscription_ext_convertToJSON() failed [data_restoration_callback_uri]");
			goto end;
		}
	}

	if(ee_subscription_ext->amf_subscription_info_list) {
		cJSON *amf_subscription_info_listList =
			cJSON_AddArrayToObject(item, "amfSubscriptionInfoList");
		if(amf_subscription_info_listList == NULL) {
			ogs_error(
				"OpenAPI_ee_subscription_ext_convertToJSON() failed [amf_subscription_info_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			ee_subscription_ext->amf_subscription_info_list, node) {
			cJSON *itemLocal =
				OpenAPI_amf_subscription_info_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_ee_subscription_ext_convertToJSON() failed [amf_subscription_info_list]");
			...
			cJSON_AddItemToArray(amf_subscription_info_listList,
			                     itemLocal);
		}
	}

	if(ee_subscription_ext->smf_subscription_info) {
		cJSON *smf_subscription_info_local_JSON =
			OpenAPI_smf_subscription_info_convertToJSON(
				ee_subscription_ext->smf_subscription_info);
		if(smf_subscription_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ee_subscription_ext_convertToJSON() failed [smf_subscription_info]");
			goto end;
		}
		cJSON_AddItemToObject(item, "smfSubscriptionInfo",
		                      smf_subscription_info_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_ee_subscription_ext_convertToJSON() failed [smf_subscription_info]");
			goto end;
	...
	if(ee_subscription_ext->hss_subscription_info) {
		cJSON *hss_subscription_info_local_JSON =
			OpenAPI_hss_subscription_info_convertToJSON(
				ee_subscription_ext->hss_subscription_info);
		if(hss_subscription_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ee_subscription_ext_convertToJSON() failed [hss_subscription_info]");
			goto end;
		}
		cJSON_AddItemToObject(item, "hssSubscriptionInfo",
		                      hss_subscription_info_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_ee_subscription_ext_convertToJSON() failed [hss_subscription_info]");
			goto end;
...
OpenAPI_ee_subscription_ext_t *OpenAPI_ee_subscription_ext_parseFromJSON(
	cJSON *ee_subscription_extJSON) {
	...
	OpenAPI_smf_subscription_info_t *smf_subscription_info_local_nonprim =
		NULL;
	cJSON *hss_subscription_info = NULL;
	OpenAPI_hss_subscription_info_t *hss_subscription_info_local_nonprim =
		NULL;
	callback_reference =
		cJSON_GetObjectItemCaseSensitive(ee_subscription_extJSON,
		                                 "callbackReference");
	if(!callback_reference) {
		ogs_error(
			"OpenAPI_ee_subscription_ext_parseFromJSON() failed [callback_reference]");
		goto end;
	}
	...
	if(!monitoring_configurations) {
		ogs_error(
			"OpenAPI_ee_subscription_ext_parseFromJSON() failed [monitoring_configurations]");
		goto end;
	}
	cJSON *monitoring_configurations_local_map = NULL;
	if(!cJSON_IsObject(monitoring_configurations) &&
	   !cJSON_IsNull(monitoring_configurations))
	{
		ogs_error(
			"OpenAPI_ee_subscription_ext_parseFromJSON() failed [monitoring_configurations]");
		goto end;
	...
	if(cJSON_IsObject(monitoring_configurations)) {
		...
		cJSON_ArrayForEach(monitoring_configurations_local_map,
			...
			cJSON *localMapObject =
				monitoring_configurations_local_map;
			if(cJSON_IsObject(localMapObject)) {
				localMapKeyPair = OpenAPI_map_create(
					ogs_strdup(localMapObject->string),
				...
				ogs_error(
					"OpenAPI_ee_subscription_ext_parseFromJSON() failed [inner]");
				goto end;
			}
			OpenAPI_list_add(monitoring_configurationsList,
			                 localMapKeyPair);
		}
	}
	...
	if(reporting_options) {
		reporting_options_local_nonprim =
			OpenAPI_reporting_options_1_parseFromJSON(
				reporting_options);
		if(!reporting_options_local_nonprim) {
			ogs_error(
				"OpenAPI_reporting_options_1_parseFromJSON failed [reporting_options]");
			goto end;
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_ee_subscription_ext_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(subscription_id) {
		...
		{
			ogs_error(
				"OpenAPI_ee_subscription_ext_parseFromJSON() failed [subscription_id]");
			goto end;
		}
	}

	context_info = cJSON_GetObjectItemCaseSensitive(ee_subscription_extJSON,
	                                                "contextInfo");
	if(context_info) {
	...
	if(epc_applied_ind) {
		if(!cJSON_IsBool(epc_applied_ind)) {
			ogs_error(
				"OpenAPI_ee_subscription_ext_parseFromJSON() failed [epc_applied_ind]");
			goto end;
		}
	}
	...
	if(scef_diam_host) {
		if(!cJSON_IsString(scef_diam_host) &&
		   !cJSON_IsNull(scef_diam_host))
		{
			ogs_error(
				"OpenAPI_ee_subscription_ext_parseFromJSON() failed [scef_diam_host]");
			goto end;
		}
	}
	...
	if(scef_diam_realm) {
		if(!cJSON_IsString(scef_diam_realm) &&
		   !cJSON_IsNull(scef_diam_realm))
		{
			ogs_error(
				"OpenAPI_ee_subscription_ext_parseFromJSON() failed [scef_diam_realm]");
			goto end;
		}
	}
	...
	if(notify_correlation_id) {
		if(!cJSON_IsString(notify_correlation_id) &&
		   !cJSON_IsNull(notify_correlation_id))
		{
			ogs_error(
				"OpenAPI_ee_subscription_ext_parseFromJSON() failed [notify_correlation_id]");
			goto end;
		}
	}
	...
	if(second_callback_ref) {
		if(!cJSON_IsString(second_callback_ref) &&
		   !cJSON_IsNull(second_callback_ref))
		{
			ogs_error(
				"OpenAPI_ee_subscription_ext_parseFromJSON() failed [second_callback_ref]");
			goto end;
		}
	}

	gpsi = cJSON_GetObjectItemCaseSensitive(ee_subscription_extJSON,
	                                        "gpsi");
	if(gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_ee_subscription_ext_parseFromJSON() failed [gpsi]");
			goto end;
		}
	}
	...
	if(exclude_gpsi_list) {
		...
		if(!cJSON_IsArray(exclude_gpsi_list)) {
			ogs_error(
				"OpenAPI_ee_subscription_ext_parseFromJSON() failed [exclude_gpsi_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(exclude_gpsi_list_local, exclude_gpsi_list) {
			...
			OpenAPI_list_add(exclude_gpsi_listList,
			                 ogs_strdup(
						 exclude_gpsi_list_local->
						 valuestring));
		}
	}
	...
	if(include_gpsi_list) {
		...
		if(!cJSON_IsArray(include_gpsi_list)) {
			ogs_error(
				"OpenAPI_ee_subscription_ext_parseFromJSON() failed [include_gpsi_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(include_gpsi_list_local, include_gpsi_list) {
			...
			OpenAPI_list_add(include_gpsi_listList,
			                 ogs_strdup(
						 include_gpsi_list_local->
						 valuestring));
		}
	}
	...
	if(data_restoration_callback_uri) {
		...
		{
			ogs_error(
				"OpenAPI_ee_subscription_ext_parseFromJSON() failed [data_restoration_callback_uri]");
			goto end;
		}
	}
	...
	if(amf_subscription_info_list) {
		...
		if(!cJSON_IsArray(amf_subscription_info_list)) {
			ogs_error(
				"OpenAPI_ee_subscription_ext_parseFromJSON() failed [amf_subscription_info_list]");
			goto end;
		}

		amf_subscription_info_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(amf_subscription_info_list_local,
		                   amf_subscription_info_list) {
			...
			OpenAPI_amf_subscription_info_t *
				...
				OpenAPI_amf_subscription_info_parseFromJSON(
					amf_subscription_info_list_local);
			if(!amf_subscription_info_listItem) {
				ogs_error("No amf_subscription_info_listItem");
				goto end;
			}
			OpenAPI_list_add(amf_subscription_info_listList,
			                 amf_subscription_info_listItem);
		}
	}
	...
	if(smf_subscription_info) {
		smf_subscription_info_local_nonprim =
			OpenAPI_smf_subscription_info_parseFromJSON(
				smf_subscription_info);
		if(!smf_subscription_info_local_nonprim) {
			ogs_error(
				"OpenAPI_smf_subscription_info_parseFromJSON failed [smf_subscription_info]");
			goto end;
		}
	}
	...
	if(hss_subscription_info) {
		hss_subscription_info_local_nonprim =
			OpenAPI_hss_subscription_info_parseFromJSON(
				hss_subscription_info);
		if(!hss_subscription_info_local_nonprim) {
			ogs_error(
				"OpenAPI_hss_subscription_info_parseFromJSON failed [hss_subscription_info]");
			goto end;
		}
	}
	...
	ee_subscription_ext_local_var = OpenAPI_ee_subscription_ext_create(
		...
		scef_diam_host &&
		!cJSON_IsNull(scef_diam_host) ?
		ogs_strdup(scef_diam_host->valuestring) : NULL,
		scef_diam_realm &&
		!cJSON_IsNull(scef_diam_realm) ?
		ogs_strdup(scef_diam_realm->valuestring) : NULL,
		notify_correlation_id &&
		!cJSON_IsNull(notify_correlation_id) ?
		ogs_strdup(notify_correlation_id->valuestring) : NULL,
		second_callback_ref &&
		!cJSON_IsNull(second_callback_ref) ?
		ogs_strdup(second_callback_ref->valuestring) : NULL,
		gpsi &&
		...
		data_restoration_callback_uri &&
		!cJSON_IsNull(data_restoration_callback_uri) ?
		ogs_strdup(data_restoration_callback_uri->valuestring) : NULL,
		amf_subscription_info_list ? amf_subscription_info_listList :
		NULL,
		smf_subscription_info ? smf_subscription_info_local_nonprim :
		NULL,
		hss_subscription_info ? hss_subscription_info_local_nonprim :
		NULL
		);
...
OpenAPI_ee_subscription_ext_t *OpenAPI_ee_subscription_ext_copy(
	OpenAPI_ee_subscription_ext_t	*dst,
	OpenAPI_ee_subscription_ext_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
