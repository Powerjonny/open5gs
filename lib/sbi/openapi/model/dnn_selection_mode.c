...
char * OpenAPI_dnn_selection_mode_ToString(
	...
	const char *dnn_selection_modeArray[] = { "NULL", "VERIFIED",
		                                  "UE_DNN_NOT_VERIFIED",
		                                  "NW_DNN_NOT_VERIFIED" };
	size_t sizeofArray = sizeof(dnn_selection_modeArray) /
	                     sizeof(dnn_selection_modeArray[0]);
...
OpenAPI_dnn_selection_mode_e OpenAPI_dnn_selection_mode_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(dnn_selection_mode,
		          dnn_selection_modeArray[stringToReturn]) == 0) {
