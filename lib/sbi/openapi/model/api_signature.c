...
OpenAPI_api_signature_t *OpenAPI_api_signature_create(char *callback_type) {
	OpenAPI_api_signature_t *api_signature_local_var =
		ogs_malloc(sizeof(OpenAPI_api_signature_t));
...
cJSON *OpenAPI_api_signature_convertToJSON(
	...
	if(api_signature == NULL) {
		ogs_error(
			"OpenAPI_api_signature_convertToJSON() failed [ApiSignature]");
		return NULL;
	}
	...
	if(!api_signature->callback_type) {
		ogs_error(
			"OpenAPI_api_signature_convertToJSON() failed [callback_type]");
		return NULL;
	}
...
OpenAPI_api_signature_t *OpenAPI_api_signature_parseFromJSON(
	...
	callback_type = cJSON_GetObjectItemCaseSensitive(api_signatureJSON,
	                                                 "callbackType");
	if(!callback_type) {
		ogs_error(
			"OpenAPI_api_signature_parseFromJSON() failed [callback_type]");
		goto end;
	}
