...
OpenAPI_subscription_data_sets_t *OpenAPI_subscription_data_sets_create(
	...
	OpenAPI_subscription_data_sets_t *subscription_data_sets_local_var =
		ogs_malloc(sizeof(OpenAPI_subscription_data_sets_t));
	...
	subscription_data_sets_local_var->is_trace_data_null =
		is_trace_data_null;
	...
	subscription_data_sets_local_var->lcs_broadcast_assistance_types_data =
		lcs_broadcast_assistance_types_data;
...
void OpenAPI_subscription_data_sets_free(
	...
	if(subscription_data_sets->v2x_data) {
		OpenAPI_v2x_subscription_data_free(
			subscription_data_sets->v2x_data);
...
cJSON *OpenAPI_subscription_data_sets_convertToJSON(
	...
	if(subscription_data_sets == NULL) {
		ogs_error(
			"OpenAPI_subscription_data_sets_convertToJSON() failed [SubscriptionDataSets]");
		return NULL;
	}
	...
	if(subscription_data_sets->am_data) {
		...
		if(am_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_subscription_data_sets_convertToJSON() failed [am_data]");
			goto end;
		}
	...
	if(subscription_data_sets->smf_sel_data) {
		...
		if(smf_sel_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_subscription_data_sets_convertToJSON() failed [smf_sel_data]");
			goto end;
		}
	...
	if(subscription_data_sets->uec_amf_data) {
		...
		if(uec_amf_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_subscription_data_sets_convertToJSON() failed [uec_amf_data]");
			goto end;
		}
	...
	if(subscription_data_sets->uec_smf_data) {
		...
		if(uec_smf_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_subscription_data_sets_convertToJSON() failed [uec_smf_data]");
			goto end;
		}
	...
	if(subscription_data_sets->uec_smsf_data) {
		...
		if(uec_smsf_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_subscription_data_sets_convertToJSON() failed [uec_smsf_data]");
			goto end;
		}
	...
	if(subscription_data_sets->sms_subs_data) {
		...
		if(sms_subs_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_subscription_data_sets_convertToJSON() failed [sms_subs_data]");
			goto end;
		}
	...
	if(subscription_data_sets->sm_data) {
		cJSON *sm_data_local_JSON =
			OpenAPI_sm_subs_data_convertToJSON(
				subscription_data_sets->sm_data);
		if(sm_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_subscription_data_sets_convertToJSON() failed [sm_data]");
			goto end;
		}
	...
	if(subscription_data_sets->trace_data) {
		cJSON *trace_data_local_JSON =
			OpenAPI_trace_data_convertToJSON(
				subscription_data_sets->trace_data);
		if(trace_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_subscription_data_sets_convertToJSON() failed [trace_data]");
			goto end;
		}
	...
	if(subscription_data_sets->sms_mng_data) {
		...
		if(sms_mng_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_subscription_data_sets_convertToJSON() failed [sms_mng_data]");
			goto end;
		}
	...
	if(subscription_data_sets->lcs_privacy_data) {
		...
		if(lcs_privacy_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_subscription_data_sets_convertToJSON() failed [lcs_privacy_data]");
			goto end;
		}
	...
	if(subscription_data_sets->lcs_mo_data) {
		cJSON *lcs_mo_data_local_JSON =
			OpenAPI_lcs_mo_data_convertToJSON(
				subscription_data_sets->lcs_mo_data);
		if(lcs_mo_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_subscription_data_sets_convertToJSON() failed [lcs_mo_data]");
			goto end;
		}
	...
	if(subscription_data_sets->v2x_data) {
		...
		if(v2x_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_subscription_data_sets_convertToJSON() failed [v2x_data]");
			goto end;
		}
	...
	if(subscription_data_sets->lcs_broadcast_assistance_types_data) {
		...
		if(lcs_broadcast_assistance_types_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_subscription_data_sets_convertToJSON() failed [lcs_broadcast_assistance_types_data]");
			goto end;
		}
	...
	if(subscription_data_sets->prose_data) {
		...
		if(prose_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_subscription_data_sets_convertToJSON() failed [prose_data]");
			goto end;
		}
	...
	if(subscription_data_sets->mbs_data) {
		...
		if(mbs_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_subscription_data_sets_convertToJSON() failed [mbs_data]");
			goto end;
		}
	...
	if(subscription_data_sets->uc_data) {
		...
		if(uc_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_subscription_data_sets_convertToJSON() failed [uc_data]");
			goto end;
		}
...
OpenAPI_subscription_data_sets_t *OpenAPI_subscription_data_sets_parseFromJSON(
	cJSON *subscription_data_setsJSON) {
	OpenAPI_subscription_data_sets_t *subscription_data_sets_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *am_data = NULL;
	...
	am_data = cJSON_GetObjectItemCaseSensitive(subscription_data_setsJSON,
	                                           "amData");
	if(am_data) {
	...
	smf_sel_data =
		cJSON_GetObjectItemCaseSensitive(subscription_data_setsJSON,
		                                 "smfSelData");
	if(smf_sel_data) {
	...
	sm_data = cJSON_GetObjectItemCaseSensitive(subscription_data_setsJSON,
	                                           "smData");
	if(sm_data) {
	...
	v2x_data = cJSON_GetObjectItemCaseSensitive(subscription_data_setsJSON,
	                                            "v2xData");
	if(v2x_data) {
	...
	mbs_data = cJSON_GetObjectItemCaseSensitive(subscription_data_setsJSON,
	                                            "mbsData");
	if(mbs_data) {
	...
	uc_data = cJSON_GetObjectItemCaseSensitive(subscription_data_setsJSON,
	                                           "ucData");
	if(uc_data) {
	...
	subscription_data_sets_local_var =
		OpenAPI_subscription_data_sets_create(
			...
			lcs_privacy_data ? lcs_privacy_data_local_nonprim :
			NULL,
...
OpenAPI_subscription_data_sets_t *OpenAPI_subscription_data_sets_copy(
	OpenAPI_subscription_data_sets_t	*dst,
	OpenAPI_subscription_data_sets_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_subscription_data_sets_convertToJSON() failed");
		return NULL;
	}
