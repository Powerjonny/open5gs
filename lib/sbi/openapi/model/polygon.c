...
OpenAPI_polygon_t *OpenAPI_polygon_create(
	...
	OpenAPI_list_t
	                                *point_list) {
	OpenAPI_polygon_t *polygon_local_var =
		ogs_malloc(sizeof(OpenAPI_polygon_t));
...
cJSON *OpenAPI_polygon_convertToJSON(OpenAPI_polygon_t *polygon) {
	...
	if(!polygon->point_list) {
		ogs_error(
			"OpenAPI_polygon_convertToJSON() failed [point_list]");
		return NULL;
	}
...
OpenAPI_polygon_t *OpenAPI_polygon_parseFromJSON(cJSON *polygonJSON) {
	...
	if(!point_list) {
		ogs_error(
			"OpenAPI_polygon_parseFromJSON() failed [point_list]");
		goto end;
	}
