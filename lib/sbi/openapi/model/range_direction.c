...
OpenAPI_range_direction_t *OpenAPI_range_direction_create(
	...
	double	distance,
	bool
	is_azimuth_direction,
	int azimuth_direction,
	bool is_elevation_direction, int elevation_direction) {
	OpenAPI_range_direction_t *range_direction_local_var =
		ogs_malloc(sizeof(OpenAPI_range_direction_t));
...
cJSON *OpenAPI_range_direction_convertToJSON(
	OpenAPI_range_direction_t *range_direction) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(range_direction == NULL) {
		ogs_error(
			"OpenAPI_range_direction_convertToJSON() failed [RangeDirection]");
		return NULL;
	}
	...
	if(range_direction->is_distance) {
		...
		{
			ogs_error(
				"OpenAPI_range_direction_convertToJSON() failed [distance]");
			goto end;
		}
	}
	...
	if(range_direction->is_azimuth_direction) {
		...
		{
			ogs_error(
				"OpenAPI_range_direction_convertToJSON() failed [azimuth_direction]");
			goto end;
		}
	}
	...
	if(range_direction->is_elevation_direction) {
		...
		{
			ogs_error(
				"OpenAPI_range_direction_convertToJSON() failed [elevation_direction]");
			goto end;
		}
	}
...
OpenAPI_range_direction_t *OpenAPI_range_direction_parseFromJSON(
	cJSON *range_directionJSON) {
	...
	if(distance) {
		if(!cJSON_IsNumber(distance)) {
			ogs_error(
				"OpenAPI_range_direction_parseFromJSON() failed [distance]");
			goto end;
		}
	}

	azimuth_direction =
		cJSON_GetObjectItemCaseSensitive(range_directionJSON,
		                                 "azimuthDirection");
	if(azimuth_direction) {
		if(!cJSON_IsNumber(azimuth_direction)) {
			ogs_error(
				"OpenAPI_range_direction_parseFromJSON() failed [azimuth_direction]");
			goto end;
		}
	}
	...
	if(elevation_direction) {
		if(!cJSON_IsNumber(elevation_direction)) {
			ogs_error(
				"OpenAPI_range_direction_parseFromJSON() failed [elevation_direction]");
			goto end;
		}
	}
...
OpenAPI_range_direction_t *OpenAPI_range_direction_copy(
	OpenAPI_range_direction_t *dst, OpenAPI_range_direction_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
