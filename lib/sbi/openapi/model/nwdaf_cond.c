...
char *OpenAPI_condition_typenwdaf_cond_ToString(
	OpenAPI_nwdaf_cond_condition_type_e condition_type) {
	const char *condition_typeArray[] = { "NULL", "NWDAF_COND" };
	size_t sizeofArray = sizeof(condition_typeArray) /
	                     sizeof(condition_typeArray[0]);
...
OpenAPI_nwdaf_cond_condition_type_e OpenAPI_condition_typenwdaf_cond_FromString(
	char *condition_type) {
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(condition_type,
		          condition_typeArray[stringToReturn]) == 0)
		{
...
OpenAPI_nwdaf_cond_t *OpenAPI_nwdaf_cond_create(
	OpenAPI_nwdaf_cond_condition_type_e condition_type,
	OpenAPI_list_t *analytics_ids, OpenAPI_list_t *snssai_list,
	...
	OpenAPI_nwdaf_cond_t *nwdaf_cond_local_var =
		ogs_malloc(sizeof(OpenAPI_nwdaf_cond_t));
...
cJSON *OpenAPI_nwdaf_cond_convertToJSON(OpenAPI_nwdaf_cond_t *nwdaf_cond) {
	...
	if(nwdaf_cond == NULL) {
		ogs_error(
			"OpenAPI_nwdaf_cond_convertToJSON() failed [NwdafCond]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(nwdaf_cond->condition_type ==
	   OpenAPI_nwdaf_cond_CONDITIONTYPE_NULL)
	{
		ogs_error(
			"OpenAPI_nwdaf_cond_convertToJSON() failed [condition_type]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "conditionType",
	                           OpenAPI_condition_typenwdaf_cond_ToString(
					   nwdaf_cond->condition_type)) == NULL)
	{
		ogs_error(
			"OpenAPI_nwdaf_cond_convertToJSON() failed [condition_type]");
		goto end;
	...
	if(nwdaf_cond->analytics_ids) {
		cJSON *analytics_idsList = cJSON_AddArrayToObject(item,
		                                                  "analyticsIds");
		if(analytics_idsList == NULL) {
			ogs_error(
				"OpenAPI_nwdaf_cond_convertToJSON() failed [analytics_ids]");
			goto end;
		}
		OpenAPI_list_for_each(nwdaf_cond->analytics_ids, node) {
			if(cJSON_AddStringToObject(analytics_idsList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_nwdaf_cond_convertToJSON() failed [analytics_ids]");
	...
	if(nwdaf_cond->snssai_list) {
		...
		if(snssai_listList == NULL) {
			ogs_error(
				"OpenAPI_nwdaf_cond_convertToJSON() failed [snssai_list]");
			goto end;
		}
		OpenAPI_list_for_each(nwdaf_cond->snssai_list, node) {
			cJSON *itemLocal =
	...
	if(nwdaf_cond->tai_list) {
		...
		if(tai_listList == NULL) {
			ogs_error(
				"OpenAPI_nwdaf_cond_convertToJSON() failed [tai_list]");
			goto end;
		}
		OpenAPI_list_for_each(nwdaf_cond->tai_list, node) {
			cJSON *itemLocal =
	...
	if(nwdaf_cond->tai_range_list) {
		...
		if(tai_range_listList == NULL) {
			ogs_error(
				"OpenAPI_nwdaf_cond_convertToJSON() failed [tai_range_list]");
			goto end;
		}
		OpenAPI_list_for_each(nwdaf_cond->tai_range_list, node) {
			cJSON *itemLocal =
	...
	if(nwdaf_cond->serving_nf_type_list != OpenAPI_nf_type_NULL) {
		...
		if(serving_nf_type_listList == NULL) {
			ogs_error(
				"OpenAPI_nwdaf_cond_convertToJSON() failed [serving_nf_type_list]");
			goto end;
		}
	...
	if(nwdaf_cond->serving_nf_set_id_list) {
		...
		if(serving_nf_set_id_listList == NULL) {
			ogs_error(
				"OpenAPI_nwdaf_cond_convertToJSON() failed [serving_nf_set_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(nwdaf_cond->serving_nf_set_id_list,
		                      node) {
	...
	if(nwdaf_cond->ml_analytics_list) {
		...
		if(ml_analytics_listList == NULL) {
			ogs_error(
				"OpenAPI_nwdaf_cond_convertToJSON() failed [ml_analytics_list]");
			goto end;
		}
		OpenAPI_list_for_each(nwdaf_cond->ml_analytics_list, node) {
			cJSON *itemLocal =
...
OpenAPI_nwdaf_cond_t *OpenAPI_nwdaf_cond_parseFromJSON(cJSON *nwdaf_condJSON) {
	...
	condition_type = cJSON_GetObjectItemCaseSensitive(nwdaf_condJSON,
	                                                  "conditionType");
	if(!condition_type) {
		ogs_error(
			"OpenAPI_nwdaf_cond_parseFromJSON() failed [condition_type]");
		goto end;
	}
	...
	condition_typeVariable =
		OpenAPI_condition_typenwdaf_cond_FromString(
			condition_type->valuestring);

	analytics_ids = cJSON_GetObjectItemCaseSensitive(nwdaf_condJSON,
	                                                 "analyticsIds");
	if(analytics_ids) {
		...
		cJSON_ArrayForEach(analytics_ids_local, analytics_ids) {
			...
			OpenAPI_list_add(analytics_idsList,
			                 ogs_strdup(
						 analytics_ids_local->
						 valuestring));
		}
	...
	snssai_list = cJSON_GetObjectItemCaseSensitive(nwdaf_condJSON,
	                                               "snssaiList");
	if(snssai_list) {
		cJSON *snssai_list_local = NULL;
		if(!cJSON_IsArray(snssai_list)) {
			ogs_error(
				"OpenAPI_nwdaf_cond_parseFromJSON() failed [snssai_list]");
			goto end;
		}
	...
	if(tai_list) {
		...
		if(!cJSON_IsArray(tai_list)) {
			ogs_error(
				"OpenAPI_nwdaf_cond_parseFromJSON() failed [tai_list]");
			goto end;
		}
	...
	tai_range_list = cJSON_GetObjectItemCaseSensitive(nwdaf_condJSON,
	                                                  "taiRangeList");
	if(tai_range_list) {
		cJSON *tai_range_list_local = NULL;
		if(!cJSON_IsArray(tai_range_list)) {
			ogs_error(
				"OpenAPI_nwdaf_cond_parseFromJSON() failed [tai_range_list]");
			goto end;
		}
	...
	serving_nf_type_list = cJSON_GetObjectItemCaseSensitive(nwdaf_condJSON,
	                                                        "servingNfTypeList");
	if(serving_nf_type_list) {
		cJSON *serving_nf_type_list_local = NULL;
		if(!cJSON_IsArray(serving_nf_type_list)) {
			ogs_error(
				"OpenAPI_nwdaf_cond_parseFromJSON() failed [serving_nf_type_list]");
			goto end;
		}
		...
		if(serving_nf_type_listList->count == 0) {
			ogs_error(
				"OpenAPI_nwdaf_cond_parseFromJSON() failed: Expected serving_nf_type_listList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}

	serving_nf_set_id_list =
		cJSON_GetObjectItemCaseSensitive(nwdaf_condJSON,
		                                 "servingNfSetIdList");
	if(serving_nf_set_id_list) {
		cJSON *serving_nf_set_id_list_local = NULL;
		if(!cJSON_IsArray(serving_nf_set_id_list)) {
			ogs_error(
				"OpenAPI_nwdaf_cond_parseFromJSON() failed [serving_nf_set_id_list]");
			goto end;
		}
	...
	ml_analytics_list = cJSON_GetObjectItemCaseSensitive(nwdaf_condJSON,
	                                                     "mlAnalyticsList");
	if(ml_analytics_list) {
		cJSON *ml_analytics_list_local = NULL;
		if(!cJSON_IsArray(ml_analytics_list)) {
			ogs_error(
				"OpenAPI_nwdaf_cond_parseFromJSON() failed [ml_analytics_list]");
			goto end;
		}
...
OpenAPI_nwdaf_cond_t *OpenAPI_nwdaf_cond_copy(OpenAPI_nwdaf_cond_t	*dst,
                                              OpenAPI_nwdaf_cond_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
