...
OpenAPI_cag_ack_data_t *OpenAPI_cag_ack_data_create(
	...
	ue_update_status) {
	OpenAPI_cag_ack_data_t *cag_ack_data_local_var =
		ogs_malloc(sizeof(OpenAPI_cag_ack_data_t));
...
cJSON *OpenAPI_cag_ack_data_convertToJSON(
	OpenAPI_cag_ack_data_t *cag_ack_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(cag_ack_data == NULL) {
		ogs_error(
			"OpenAPI_cag_ack_data_convertToJSON() failed [CagAckData]");
		return NULL;
	}
	...
	if(!cag_ack_data->provisioning_time) {
		ogs_error(
			"OpenAPI_cag_ack_data_convertToJSON() failed [provisioning_time]");
		return NULL;
	}
	...
	if(cag_ack_data->ue_update_status == OpenAPI_ue_update_status_NULL) {
		ogs_error(
			"OpenAPI_cag_ack_data_convertToJSON() failed [ue_update_status]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "ueUpdateStatus",
	                           OpenAPI_ue_update_status_ToString(
					   cag_ack_data->ue_update_status)) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_cag_ack_data_convertToJSON() failed [ue_update_status]");
...
OpenAPI_cag_ack_data_t *OpenAPI_cag_ack_data_parseFromJSON(
	cJSON *cag_ack_dataJSON) {
	...
	provisioning_time = cJSON_GetObjectItemCaseSensitive(cag_ack_dataJSON,
	                                                     "provisioningTime");
	if(!provisioning_time) {
		ogs_error(
			"OpenAPI_cag_ack_data_parseFromJSON() failed [provisioning_time]");
		goto end;
	}
	...
	ue_update_status = cJSON_GetObjectItemCaseSensitive(cag_ack_dataJSON,
	                                                    "ueUpdateStatus");
	if(!ue_update_status) {
		ogs_error(
			"OpenAPI_cag_ack_data_parseFromJSON() failed [ue_update_status]");
		goto end;
	}
...
OpenAPI_cag_ack_data_t *OpenAPI_cag_ack_data_copy(
	OpenAPI_cag_ack_data_t	*dst,
