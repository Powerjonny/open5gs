...
OpenAPI_tmgi_range_t *OpenAPI_tmgi_range_create(
	...
	char			*nid) {
	OpenAPI_tmgi_range_t *tmgi_range_local_var =
		ogs_malloc(sizeof(OpenAPI_tmgi_range_t));
...
cJSON *OpenAPI_tmgi_range_convertToJSON(OpenAPI_tmgi_range_t *tmgi_range) {
	...
	if(tmgi_range == NULL) {
		ogs_error(
			"OpenAPI_tmgi_range_convertToJSON() failed [TmgiRange]");
		return NULL;
	}
	...
	if(!tmgi_range->mbs_service_id_start) {
		ogs_error(
			"OpenAPI_tmgi_range_convertToJSON() failed [mbs_service_id_start]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "mbsServiceIdStart",
	                           tmgi_range->mbs_service_id_start) == NULL)
	{
		ogs_error(
			"OpenAPI_tmgi_range_convertToJSON() failed [mbs_service_id_start]");
	...
	if(!tmgi_range->mbs_service_id_end) {
		ogs_error(
			"OpenAPI_tmgi_range_convertToJSON() failed [mbs_service_id_end]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "mbsServiceIdEnd",
	                           tmgi_range->mbs_service_id_end) == NULL)
	{
		ogs_error(
			"OpenAPI_tmgi_range_convertToJSON() failed [mbs_service_id_end]");
	...
	if(!tmgi_range->plmn_id) {
		ogs_error(
			"OpenAPI_tmgi_range_convertToJSON() failed [plmn_id]");
		return NULL;
	}
	...
	if(tmgi_range->nid) {
		...
		{
			ogs_error(
				"OpenAPI_tmgi_range_convertToJSON() failed [nid]");
			goto end;
		}
	}
...
OpenAPI_tmgi_range_t *OpenAPI_tmgi_range_parseFromJSON(cJSON *tmgi_rangeJSON) {
	...
	cJSON *nid = NULL;
	mbs_service_id_start = cJSON_GetObjectItemCaseSensitive(tmgi_rangeJSON,
	                                                        "mbsServiceIdStart");
	if(!mbs_service_id_start) {
		ogs_error(
			"OpenAPI_tmgi_range_parseFromJSON() failed [mbs_service_id_start]");
		goto end;
	}
	...
	mbs_service_id_end = cJSON_GetObjectItemCaseSensitive(tmgi_rangeJSON,
	                                                      "mbsServiceIdEnd");
	if(!mbs_service_id_end) {
		ogs_error(
			"OpenAPI_tmgi_range_parseFromJSON() failed [mbs_service_id_end]");
		goto end;
	}
	...
	if(!plmn_id) {
		ogs_error(
			"OpenAPI_tmgi_range_parseFromJSON() failed [plmn_id]");
		goto end;
	}
	...
	if(nid) {
		...
		{
			ogs_error(
				"OpenAPI_tmgi_range_parseFromJSON() failed [nid]");
			goto end;
		}
	}
...
OpenAPI_tmgi_range_t *OpenAPI_tmgi_range_copy(OpenAPI_tmgi_range_t	*dst,
                                              OpenAPI_tmgi_range_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
