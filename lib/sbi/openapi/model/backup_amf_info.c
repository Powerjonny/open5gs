...
OpenAPI_backup_amf_info_t *OpenAPI_backup_amf_info_create(
	...
	OpenAPI_backup_amf_info_t *backup_amf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_backup_amf_info_t));
...
cJSON *OpenAPI_backup_amf_info_convertToJSON(
	OpenAPI_backup_amf_info_t *backup_amf_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(backup_amf_info == NULL) {
		ogs_error(
			"OpenAPI_backup_amf_info_convertToJSON() failed [BackupAmfInfo]");
		return NULL;
	}
	...
	if(!backup_amf_info->backup_amf) {
		ogs_error(
			"OpenAPI_backup_amf_info_convertToJSON() failed [backup_amf]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "backupAmf",
	                           backup_amf_info->backup_amf) == NULL)
	{
		ogs_error(
			"OpenAPI_backup_amf_info_convertToJSON() failed [backup_amf]");
	...
	if(backup_amf_info->guami_list) {
		...
		if(guami_listList == NULL) {
			ogs_error(
				"OpenAPI_backup_amf_info_convertToJSON() failed [guami_list]");
			goto end;
		}
		OpenAPI_list_for_each(backup_amf_info->guami_list, node) {
			cJSON *itemLocal =
...
OpenAPI_backup_amf_info_t *OpenAPI_backup_amf_info_parseFromJSON(
	...
	backup_amf = cJSON_GetObjectItemCaseSensitive(backup_amf_infoJSON,
	                                              "backupAmf");
	if(!backup_amf) {
		ogs_error(
			"OpenAPI_backup_amf_info_parseFromJSON() failed [backup_amf]");
		goto end;
	}
	...
	guami_list = cJSON_GetObjectItemCaseSensitive(backup_amf_infoJSON,
	                                              "guamiList");
	if(guami_list) {
		cJSON *guami_list_local = NULL;
		if(!cJSON_IsArray(guami_list)) {
			ogs_error(
				"OpenAPI_backup_amf_info_parseFromJSON() failed [guami_list]");
			goto end;
		}
...
OpenAPI_backup_amf_info_t *OpenAPI_backup_amf_info_copy(
	OpenAPI_backup_amf_info_t *dst, OpenAPI_backup_amf_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
