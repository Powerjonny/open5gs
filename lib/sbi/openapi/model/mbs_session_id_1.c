...
OpenAPI_mbs_session_id_1_t *OpenAPI_mbs_session_id_1_create(
	...
	OpenAPI_mbs_session_id_1_t *mbs_session_id_1_local_var =
		ogs_malloc(sizeof(OpenAPI_mbs_session_id_1_t));
...
cJSON *OpenAPI_mbs_session_id_1_convertToJSON(
	...
	if(mbs_session_id_1 == NULL) {
		ogs_error(
			"OpenAPI_mbs_session_id_1_convertToJSON() failed [MbsSessionId_1]");
		return NULL;
	}
	...
	if(mbs_session_id_1->tmgi) {
		cJSON *tmgi_local_JSON =
			OpenAPI_tmgi_1_convertToJSON(mbs_session_id_1->tmgi);
		if(tmgi_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_mbs_session_id_1_convertToJSON() failed [tmgi]");
			goto end;
		}
	...
	if(mbs_session_id_1->ssm) {
		cJSON *ssm_local_JSON =
			OpenAPI_ssm_1_convertToJSON(mbs_session_id_1->ssm);
		if(ssm_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_mbs_session_id_1_convertToJSON() failed [ssm]");
			goto end;
		}
	...
	if(mbs_session_id_1->nid) {
		...
		{
			ogs_error(
				"OpenAPI_mbs_session_id_1_convertToJSON() failed [nid]");
			goto end;
		}
	}
...
OpenAPI_mbs_session_id_1_t *OpenAPI_mbs_session_id_1_parseFromJSON(
	...
	if(nid) {
		...
		{
			ogs_error(
				"OpenAPI_mbs_session_id_1_parseFromJSON() failed [nid]");
			goto end;
		}
	}
