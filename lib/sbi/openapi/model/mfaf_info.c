...
OpenAPI_mfaf_info_t *OpenAPI_mfaf_info_create(
	...
	OpenAPI_mfaf_info_t *mfaf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_mfaf_info_t));
...
cJSON *OpenAPI_mfaf_info_convertToJSON(OpenAPI_mfaf_info_t *mfaf_info) {
	...
	if(mfaf_info == NULL) {
		ogs_error(
			"OpenAPI_mfaf_info_convertToJSON() failed [MfafInfo]");
		return NULL;
	}
	...
	if(mfaf_info->serving_nf_type_list != OpenAPI_nf_type_NULL) {
		...
		if(serving_nf_type_listList == NULL) {
			ogs_error(
				"OpenAPI_mfaf_info_convertToJSON() failed [serving_nf_type_list]");
			goto end;
		}
	...
	if(mfaf_info->serving_nf_set_id_list) {
		...
		if(serving_nf_set_id_listList == NULL) {
			ogs_error(
				"OpenAPI_mfaf_info_convertToJSON() failed [serving_nf_set_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(mfaf_info->serving_nf_set_id_list, node) {
	...
	if(mfaf_info->tai_list) {
		...
		if(tai_listList == NULL) {
			ogs_error(
				"OpenAPI_mfaf_info_convertToJSON() failed [tai_list]");
			goto end;
		}
		OpenAPI_list_for_each(mfaf_info->tai_list, node) {
			cJSON *itemLocal =
	...
	if(mfaf_info->tai_range_list) {
		...
		if(tai_range_listList == NULL) {
			ogs_error(
				"OpenAPI_mfaf_info_convertToJSON() failed [tai_range_list]");
			goto end;
		}
		OpenAPI_list_for_each(mfaf_info->tai_range_list, node) {
			cJSON *itemLocal =
...
OpenAPI_mfaf_info_t *OpenAPI_mfaf_info_parseFromJSON(cJSON *mfaf_infoJSON) {
	...
	serving_nf_type_list = cJSON_GetObjectItemCaseSensitive(mfaf_infoJSON,
	                                                        "servingNfTypeList");
	if(serving_nf_type_list) {
		cJSON *serving_nf_type_list_local = NULL;
		if(!cJSON_IsArray(serving_nf_type_list)) {
			ogs_error(
				"OpenAPI_mfaf_info_parseFromJSON() failed [serving_nf_type_list]");
			goto end;
		}
		...
		if(serving_nf_type_listList->count == 0) {
			ogs_error(
				"OpenAPI_mfaf_info_parseFromJSON() failed: Expected serving_nf_type_listList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}

	serving_nf_set_id_list = cJSON_GetObjectItemCaseSensitive(mfaf_infoJSON,
	                                                          "servingNfSetIdList");
	if(serving_nf_set_id_list) {
		cJSON *serving_nf_set_id_list_local = NULL;
		if(!cJSON_IsArray(serving_nf_set_id_list)) {
			ogs_error(
				"OpenAPI_mfaf_info_parseFromJSON() failed [serving_nf_set_id_list]");
			goto end;
		}
	...
	if(tai_list) {
		...
		if(!cJSON_IsArray(tai_list)) {
			ogs_error(
				"OpenAPI_mfaf_info_parseFromJSON() failed [tai_list]");
			goto end;
		}
	...
	tai_range_list = cJSON_GetObjectItemCaseSensitive(mfaf_infoJSON,
	                                                  "taiRangeList");
	if(tai_range_list) {
		cJSON *tai_range_list_local = NULL;
		if(!cJSON_IsArray(tai_range_list)) {
			ogs_error(
				"OpenAPI_mfaf_info_parseFromJSON() failed [tai_range_list]");
			goto end;
		}
