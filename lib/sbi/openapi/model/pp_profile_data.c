...
OpenAPI_pp_profile_data_t *OpenAPI_pp_profile_data_create(
	...
	OpenAPI_pp_profile_data_t *pp_profile_data_local_var =
		ogs_malloc(sizeof(OpenAPI_pp_profile_data_t));
	ogs_assert(pp_profile_data_local_var);

	pp_profile_data_local_var->allowed_mtc_providers =
...
void OpenAPI_pp_profile_data_free(OpenAPI_pp_profile_data_t *pp_profile_data) {
	...
	if(pp_profile_data->allowed_mtc_providers) {
		OpenAPI_list_for_each(pp_profile_data->allowed_mtc_providers,
		                      node) {
...
cJSON *OpenAPI_pp_profile_data_convertToJSON(
	...
	if(pp_profile_data == NULL) {
		ogs_error(
			"OpenAPI_pp_profile_data_convertToJSON() failed [PpProfileData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(pp_profile_data->allowed_mtc_providers) {
		cJSON *allowed_mtc_providers = cJSON_AddObjectToObject(item,
		                                                       "allowedMtcProviders");
		if(allowed_mtc_providers == NULL) {
			ogs_error(
				"OpenAPI_pp_profile_data_convertToJSON() failed [allowed_mtc_providers]");
			goto end;
		}
		cJSON *localMapObject = allowed_mtc_providers;
		if(pp_profile_data->allowed_mtc_providers) {
			OpenAPI_list_for_each(
				pp_profile_data->allowed_mtc_providers, node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_allowed_mtc_provider_info_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_pp_profile_data_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(pp_profile_data->supported_features) {
		if(cJSON_AddStringToObject(item, "supportedFeatures",
		                           pp_profile_data->supported_features)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_pp_profile_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
...
OpenAPI_pp_profile_data_t *OpenAPI_pp_profile_data_parseFromJSON(
	cJSON *pp_profile_dataJSON) {
	...
	allowed_mtc_providers =
		cJSON_GetObjectItemCaseSensitive(pp_profile_dataJSON,
		                                 "allowedMtcProviders");
	if(allowed_mtc_providers) {
		cJSON *allowed_mtc_providers_local_map = NULL;
		if(!cJSON_IsObject(allowed_mtc_providers) &&
		   !cJSON_IsNull(allowed_mtc_providers))
		{
			ogs_error(
				"OpenAPI_pp_profile_data_parseFromJSON() failed [allowed_mtc_providers]");
			goto end;
		}
		if(cJSON_IsObject(allowed_mtc_providers)) {
			...
			cJSON_ArrayForEach(allowed_mtc_providers_local_map,
				...
				cJSON *localMapObject =
					allowed_mtc_providers_local_map;
				...
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_pp_profile_data_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_pp_profile_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
...
OpenAPI_pp_profile_data_t *OpenAPI_pp_profile_data_copy(
	OpenAPI_pp_profile_data_t *dst, OpenAPI_pp_profile_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
