...
OpenAPI_slice_mbr_t *OpenAPI_slice_mbr_create(char *uplink, char *downlink) {
	OpenAPI_slice_mbr_t *slice_mbr_local_var =
		ogs_malloc(sizeof(OpenAPI_slice_mbr_t));
...
cJSON *OpenAPI_slice_mbr_convertToJSON(OpenAPI_slice_mbr_t *slice_mbr) {
	...
	if(slice_mbr == NULL) {
		ogs_error(
			"OpenAPI_slice_mbr_convertToJSON() failed [SliceMbr]");
		return NULL;
	}
	...
	if(!slice_mbr->downlink) {
		ogs_error(
			"OpenAPI_slice_mbr_convertToJSON() failed [downlink]");
		return NULL;
	}
...
OpenAPI_slice_mbr_t *OpenAPI_slice_mbr_parseFromJSON(cJSON *slice_mbrJSON) {
	...
	if(!downlink) {
		ogs_error(
			"OpenAPI_slice_mbr_parseFromJSON() failed [downlink]");
		goto end;
	}
...
OpenAPI_slice_mbr_t *OpenAPI_slice_mbr_copy(OpenAPI_slice_mbr_t *dst,
                                            OpenAPI_slice_mbr_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
