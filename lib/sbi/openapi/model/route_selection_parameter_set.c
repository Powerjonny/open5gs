...
OpenAPI_route_selection_parameter_set_create(char *dnn,
	...
	OpenAPI_route_selection_parameter_set_t *
	        route_selection_parameter_set_local_var =
		ogs_malloc(sizeof(OpenAPI_route_selection_parameter_set_t));
	...
	route_selection_parameter_set_local_var->spatial_validity_areas =
		spatial_validity_areas;
	route_selection_parameter_set_local_var->spatial_validity_tais =
		spatial_validity_tais;
	...
	OpenAPI_route_selection_parameter_set_t *route_selection_parameter_set)
{
	...
	if(route_selection_parameter_set->spatial_validity_areas) {
		OpenAPI_list_for_each(
			route_selection_parameter_set->spatial_validity_areas,
			node) {
	...
	if(route_selection_parameter_set->spatial_validity_tais) {
		OpenAPI_list_for_each(
			route_selection_parameter_set->spatial_validity_tais,
			node) {
	...
	OpenAPI_route_selection_parameter_set_t *route_selection_parameter_set)
{
	...
	if(route_selection_parameter_set == NULL) {
		ogs_error(
			"OpenAPI_route_selection_parameter_set_convertToJSON() failed [RouteSelectionParameterSet]");
		return NULL;
	}
	...
	if(route_selection_parameter_set->dnn) {
		if(cJSON_AddStringToObject(item, "dnn",
		                           route_selection_parameter_set->dnn)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_route_selection_parameter_set_convertToJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(route_selection_parameter_set->snssai) {
		cJSON *snssai_local_JSON =
			OpenAPI_snssai_convertToJSON(
				route_selection_parameter_set->snssai);
		if(snssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_route_selection_parameter_set_convertToJSON() failed [snssai]");
			goto end;
		}
	...
	if(route_selection_parameter_set->is_precedence) {
		if(cJSON_AddNumberToObject(item, "precedence",
		                           route_selection_parameter_set->
		                           precedence) == NULL)
		{
			ogs_error(
				"OpenAPI_route_selection_parameter_set_convertToJSON() failed [precedence]");
			goto end;
		}
	}

	if(route_selection_parameter_set->spatial_validity_areas) {
		cJSON *spatial_validity_areasList = cJSON_AddArrayToObject(item,
		                                                           "spatialValidityAreas");
		if(spatial_validity_areasList == NULL) {
			ogs_error(
				"OpenAPI_route_selection_parameter_set_convertToJSON() failed [spatial_validity_areas]");
			goto end;
		}
		OpenAPI_list_for_each(
			route_selection_parameter_set->spatial_validity_areas,
			node) {
			cJSON *itemLocal =
				OpenAPI_geographical_area_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_route_selection_parameter_set_convertToJSON() failed [spatial_validity_areas]");
			...
			cJSON_AddItemToArray(spatial_validity_areasList,
			                     itemLocal);
		}
	}

	if(route_selection_parameter_set->spatial_validity_tais) {
		cJSON *spatial_validity_taisList = cJSON_AddArrayToObject(item,
		                                                          "spatialValidityTais");
		if(spatial_validity_taisList == NULL) {
			ogs_error(
				"OpenAPI_route_selection_parameter_set_convertToJSON() failed [spatial_validity_tais]");
			goto end;
		}
		OpenAPI_list_for_each(
			route_selection_parameter_set->spatial_validity_tais,
			node) {
			cJSON *itemLocal =
				OpenAPI_tai_1_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_route_selection_parameter_set_convertToJSON() failed [spatial_validity_tais]");
				goto end;
			}
			cJSON_AddItemToArray(spatial_validity_taisList,
			                     itemLocal);
		}
	}
...
OpenAPI_route_selection_parameter_set_parseFromJSON(
	...
	OpenAPI_route_selection_parameter_set_t *
	        route_selection_parameter_set_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *dnn = NULL;
	cJSON *snssai = NULL;
	...
	dnn =
		cJSON_GetObjectItemCaseSensitive(
			route_selection_parameter_setJSON, "dnn");
	if(dnn) {
		if(!cJSON_IsString(dnn) &&
		   !cJSON_IsNull(dnn))
		{
			ogs_error(
				"OpenAPI_route_selection_parameter_set_parseFromJSON() failed [dnn]");
			goto end;
		}
	}
	...
	snssai =
		cJSON_GetObjectItemCaseSensitive(
			route_selection_parameter_setJSON, "snssai");
	if(snssai) {
	...
	precedence =
		cJSON_GetObjectItemCaseSensitive(
			route_selection_parameter_setJSON, "precedence");
	if(precedence) {
		if(!cJSON_IsNumber(precedence)) {
			ogs_error(
				"OpenAPI_route_selection_parameter_set_parseFromJSON() failed [precedence]");
			goto end;
		}
	}
	...
	spatial_validity_areas =
		cJSON_GetObjectItemCaseSensitive(
			route_selection_parameter_setJSON,
			"spatialValidityAreas");
	if(spatial_validity_areas) {
		cJSON *spatial_validity_areas_local = NULL;
		if(!cJSON_IsArray(spatial_validity_areas)) {
			ogs_error(
				"OpenAPI_route_selection_parameter_set_parseFromJSON() failed [spatial_validity_areas]");
			goto end;
		}

		spatial_validity_areasList = OpenAPI_list_create();

		cJSON_ArrayForEach(spatial_validity_areas_local,
		                   spatial_validity_areas) {
			...
			OpenAPI_geographical_area_t *spatial_validity_areasItem
			        = OpenAPI_geographical_area_parseFromJSON(
					  spatial_validity_areas_local);
			if(!spatial_validity_areasItem) {
				ogs_error("No spatial_validity_areasItem");
				goto end;
			}
			OpenAPI_list_add(spatial_validity_areasList,
	...
	if(spatial_validity_tais) {
		...
		if(!cJSON_IsArray(spatial_validity_tais)) {
			ogs_error(
				"OpenAPI_route_selection_parameter_set_parseFromJSON() failed [spatial_validity_tais]");
			goto end;
		}

		spatial_validity_taisList = OpenAPI_list_create();

		cJSON_ArrayForEach(spatial_validity_tais_local,
		                   spatial_validity_tais) {
			...
			OpenAPI_tai_1_t *spatial_validity_taisItem =
				OpenAPI_tai_1_parseFromJSON(
					spatial_validity_tais_local);
			if(!spatial_validity_taisItem) {
				ogs_error("No spatial_validity_taisItem");
				goto end;
			}
			OpenAPI_list_add(spatial_validity_taisList,
			                 spatial_validity_taisItem);
		}
	}
	...
	route_selection_parameter_set_local_var =
		OpenAPI_route_selection_parameter_set_create(
			...
			spatial_validity_areas ? spatial_validity_areasList :
			NULL,
...
OpenAPI_route_selection_parameter_set_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_route_selection_parameter_set_convertToJSON() failed");
		return NULL;
	}
