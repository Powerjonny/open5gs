...
OpenAPI_pcf_for_ue_binding_t *OpenAPI_pcf_for_ue_binding_create(char	*supi,
                                                                char	*gpsi,
                                                                char *
                                                                pcf_for_ue_fqdn,
                                                                OpenAPI_list_t *
                                                                pcf_for_ue_ip_end_points,
                                                                char
                                                                *pcf_id,
                                                                char
                                                                *pcf_set_id,
                                                                ...
                                                                *supp_feat)
{
	OpenAPI_pcf_for_ue_binding_t *pcf_for_ue_binding_local_var =
		ogs_malloc(sizeof(OpenAPI_pcf_for_ue_binding_t));
	...
	pcf_for_ue_binding_local_var->pcf_for_ue_ip_end_points =
		pcf_for_ue_ip_end_points;
...
void OpenAPI_pcf_for_ue_binding_free(
	OpenAPI_pcf_for_ue_binding_t *pcf_for_ue_binding) {
	...
	if(pcf_for_ue_binding->pcf_for_ue_ip_end_points) {
		OpenAPI_list_for_each(
			pcf_for_ue_binding->pcf_for_ue_ip_end_points, node) {
...
cJSON *OpenAPI_pcf_for_ue_binding_convertToJSON(
	OpenAPI_pcf_for_ue_binding_t *pcf_for_ue_binding) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(pcf_for_ue_binding == NULL) {
		ogs_error(
			"OpenAPI_pcf_for_ue_binding_convertToJSON() failed [PcfForUeBinding]");
		return NULL;
	}
	...
	if(!pcf_for_ue_binding->supi) {
		ogs_error(
			"OpenAPI_pcf_for_ue_binding_convertToJSON() failed [supi]");
		return NULL;
	}
	...
	if(pcf_for_ue_binding->gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_for_ue_binding_convertToJSON() failed [gpsi]");
			goto end;
		}
	}

	if(pcf_for_ue_binding->pcf_for_ue_fqdn) {
		if(cJSON_AddStringToObject(item, "pcfForUeFqdn",
		                           pcf_for_ue_binding->pcf_for_ue_fqdn)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_pcf_for_ue_binding_convertToJSON() failed [pcf_for_ue_fqdn]");
			goto end;
		}
	}

	if(pcf_for_ue_binding->pcf_for_ue_ip_end_points) {
		cJSON *pcf_for_ue_ip_end_pointsList =
			cJSON_AddArrayToObject(item, "pcfForUeIpEndPoints");
		if(pcf_for_ue_ip_end_pointsList == NULL) {
			ogs_error(
				"OpenAPI_pcf_for_ue_binding_convertToJSON() failed [pcf_for_ue_ip_end_points]");
			goto end;
		}
		OpenAPI_list_for_each(
			pcf_for_ue_binding->pcf_for_ue_ip_end_points, node) {
			...
			cJSON_AddItemToArray(pcf_for_ue_ip_end_pointsList,
			                     itemLocal);
		}
	}
	...
	if(pcf_for_ue_binding->pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_for_ue_binding_convertToJSON() failed [pcf_id]");
			goto end;
		}
	}
	...
	if(pcf_for_ue_binding->pcf_set_id) {
		if(cJSON_AddStringToObject(item, "pcfSetId",
		                           pcf_for_ue_binding->pcf_set_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pcf_for_ue_binding_convertToJSON() failed [pcf_set_id]");
			goto end;
		}
	}
	...
	if(pcf_for_ue_binding->bind_level != OpenAPI_binding_level_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_for_ue_binding_convertToJSON() failed [bind_level]");
			goto end;
		}
	}
	...
	if(pcf_for_ue_binding->supp_feat) {
		if(cJSON_AddStringToObject(item, "suppFeat",
		                           pcf_for_ue_binding->supp_feat) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pcf_for_ue_binding_convertToJSON() failed [supp_feat]");
			goto end;
		}
	}
...
OpenAPI_pcf_for_ue_binding_t *OpenAPI_pcf_for_ue_binding_parseFromJSON(
	cJSON *pcf_for_ue_bindingJSON) {
	...
	if(!supi) {
		ogs_error(
			"OpenAPI_pcf_for_ue_binding_parseFromJSON() failed [supi]");
		goto end;
	}
	...
	if(gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_for_ue_binding_parseFromJSON() failed [gpsi]");
			goto end;
		}
	}

	pcf_for_ue_fqdn =
		cJSON_GetObjectItemCaseSensitive(pcf_for_ue_bindingJSON,
		                                 "pcfForUeFqdn");
	if(pcf_for_ue_fqdn) {
		if(!cJSON_IsString(pcf_for_ue_fqdn) &&
		   !cJSON_IsNull(pcf_for_ue_fqdn))
		{
			ogs_error(
	...
	if(pcf_for_ue_ip_end_points) {
		...
		if(!cJSON_IsArray(pcf_for_ue_ip_end_points)) {
			ogs_error(
				"OpenAPI_pcf_for_ue_binding_parseFromJSON() failed [pcf_for_ue_ip_end_points]");
			goto end;
		}

		pcf_for_ue_ip_end_pointsList = OpenAPI_list_create();

		cJSON_ArrayForEach(pcf_for_ue_ip_end_points_local,
		                   pcf_for_ue_ip_end_points) {
			...
			OpenAPI_ip_end_point_t *pcf_for_ue_ip_end_pointsItem =
				OpenAPI_ip_end_point_parseFromJSON(
					pcf_for_ue_ip_end_points_local);
			if(!pcf_for_ue_ip_end_pointsItem) {
				ogs_error("No pcf_for_ue_ip_end_pointsItem");
				goto end;
			}
			OpenAPI_list_add(pcf_for_ue_ip_end_pointsList,
			                 pcf_for_ue_ip_end_pointsItem);
		}
	}

	pcf_id = cJSON_GetObjectItemCaseSensitive(pcf_for_ue_bindingJSON,
	                                          "pcfId");
	if(pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_for_ue_binding_parseFromJSON() failed [pcf_id]");
			goto end;
		}
	}

	pcf_set_id = cJSON_GetObjectItemCaseSensitive(pcf_for_ue_bindingJSON,
	                                              "pcfSetId");
	if(pcf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_for_ue_binding_parseFromJSON() failed [pcf_set_id]");
			goto end;
		}
	}

	bind_level = cJSON_GetObjectItemCaseSensitive(pcf_for_ue_bindingJSON,
	                                              "bindLevel");
	if(bind_level) {
		if(!cJSON_IsString(bind_level)) {
			ogs_error(
				"OpenAPI_pcf_for_ue_binding_parseFromJSON() failed [bind_level]");
			goto end;
		}
		bind_levelVariable =
	...
	supp_feat = cJSON_GetObjectItemCaseSensitive(pcf_for_ue_bindingJSON,
	                                             "suppFeat");
	if(supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_for_ue_binding_parseFromJSON() failed [supp_feat]");
			goto end;
		}
	}
	...
	pcf_for_ue_binding_local_var = OpenAPI_pcf_for_ue_binding_create(
		...
		pcf_for_ue_fqdn &&
		!cJSON_IsNull(pcf_for_ue_fqdn) ?
		ogs_strdup(pcf_for_ue_fqdn->valuestring) : NULL,
...
OpenAPI_pcf_for_ue_binding_t *OpenAPI_pcf_for_ue_binding_copy(
	OpenAPI_pcf_for_ue_binding_t *dst, OpenAPI_pcf_for_ue_binding_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
