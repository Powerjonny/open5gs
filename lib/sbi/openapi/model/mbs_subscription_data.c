...
OpenAPI_mbs_subscription_data_t *OpenAPI_mbs_subscription_data_create(
	...
	OpenAPI_mbs_subscription_data_t *mbs_subscription_data_local_var =
		ogs_malloc(sizeof(OpenAPI_mbs_subscription_data_t));
	...
	mbs_subscription_data_local_var->mbs_session_id_list =
		mbs_session_id_list;

	return mbs_subscription_data_local_var;
}

void OpenAPI_mbs_subscription_data_free(
	OpenAPI_mbs_subscription_data_t *mbs_subscription_data) {
	...
	if(mbs_subscription_data->mbs_session_id_list) {
		OpenAPI_list_for_each(
			mbs_subscription_data->mbs_session_id_list, node) {
...
cJSON *OpenAPI_mbs_subscription_data_convertToJSON(
	...
	if(mbs_subscription_data == NULL) {
		ogs_error(
			"OpenAPI_mbs_subscription_data_convertToJSON() failed [MbsSubscriptionData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(mbs_subscription_data->is_mbs_allowed) {
		if(cJSON_AddBoolToObject(item, "mbsAllowed",
		                         mbs_subscription_data->mbs_allowed) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_mbs_subscription_data_convertToJSON() failed [mbs_allowed]");
			goto end;
		}
	}

	if(mbs_subscription_data->mbs_session_id_list) {
		cJSON *mbs_session_id_listList = cJSON_AddArrayToObject(item,
		                                                        "mbsSessionIdList");
		if(mbs_session_id_listList == NULL) {
			ogs_error(
				"OpenAPI_mbs_subscription_data_convertToJSON() failed [mbs_session_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			mbs_subscription_data->mbs_session_id_list, node) {
			...
			cJSON_AddItemToArray(mbs_session_id_listList,
			                     itemLocal);
		}
	}
...
OpenAPI_mbs_subscription_data_t *OpenAPI_mbs_subscription_data_parseFromJSON(
	...
	mbs_allowed =
		cJSON_GetObjectItemCaseSensitive(mbs_subscription_dataJSON,
		                                 "mbsAllowed");
	if(mbs_allowed) {
		if(!cJSON_IsBool(mbs_allowed)) {
			ogs_error(
				"OpenAPI_mbs_subscription_data_parseFromJSON() failed [mbs_allowed]");
			goto end;
		}
	}
	...
	if(mbs_session_id_list) {
		...
		if(!cJSON_IsArray(mbs_session_id_list)) {
			ogs_error(
				"OpenAPI_mbs_subscription_data_parseFromJSON() failed [mbs_session_id_list]");
			goto end;
		}

		mbs_session_id_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(mbs_session_id_list_local,
		                   mbs_session_id_list) {
			...
			OpenAPI_mbs_session_id_t *mbs_session_id_listItem =
				OpenAPI_mbs_session_id_parseFromJSON(
					mbs_session_id_list_local);
			if(!mbs_session_id_listItem) {
				ogs_error("No mbs_session_id_listItem");
				goto end;
			}
			OpenAPI_list_add(mbs_session_id_listList,
			                 mbs_session_id_listItem);
		}
	}
...
OpenAPI_mbs_subscription_data_t *OpenAPI_mbs_subscription_data_copy(
	OpenAPI_mbs_subscription_data_t *dst,
	OpenAPI_mbs_subscription_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_mbs_subscription_data_convertToJSON() failed");
		return NULL;
	}
