...
OpenAPI_app_descriptor_1_t *OpenAPI_app_descriptor_1_create(
	...
	OpenAPI_list_t
	        *app_ids) {
	OpenAPI_app_descriptor_1_t *app_descriptor_1_local_var =
		ogs_malloc(sizeof(OpenAPI_app_descriptor_1_t));
...
void OpenAPI_app_descriptor_1_free(
	OpenAPI_app_descriptor_1_t *app_descriptor_1) {
...
cJSON *OpenAPI_app_descriptor_1_convertToJSON(
	OpenAPI_app_descriptor_1_t *app_descriptor_1) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(app_descriptor_1 == NULL) {
		ogs_error(
			"OpenAPI_app_descriptor_1_convertToJSON() failed [AppDescriptor_1]");
		return NULL;
	}
	...
	if(!app_descriptor_1->os_id) {
		ogs_error(
			"OpenAPI_app_descriptor_1_convertToJSON() failed [os_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "osId",
	                           app_descriptor_1->os_id) == NULL)
	{
		ogs_error(
			"OpenAPI_app_descriptor_1_convertToJSON() failed [os_id]");
	...
	if(!app_descriptor_1->app_ids) {
		ogs_error(
			"OpenAPI_app_descriptor_1_convertToJSON() failed [app_ids]");
		return NULL;
	}
	...
	if(app_descriptor_1->app_ids) {
		OpenAPI_list_for_each(app_descriptor_1->app_ids, node) {
			...
			if(cJSON_AddStringToObject(localMapObject,
			                           localKeyValue->key,
			                           (char *) localKeyValue->value)
			   == NULL)
			{
				ogs_error(
					"OpenAPI_app_descriptor_1_convertToJSON() failed [inner]");
				goto end;
			}
		}
	}
...
OpenAPI_app_descriptor_1_t *OpenAPI_app_descriptor_1_parseFromJSON(
	cJSON *app_descriptor_1JSON) {
	...
	if(!os_id) {
		ogs_error(
			"OpenAPI_app_descriptor_1_parseFromJSON() failed [os_id]");
		goto end;
	}
	...
	app_ids = cJSON_GetObjectItemCaseSensitive(app_descriptor_1JSON,
	                                           "appIds");
	if(!app_ids) {
		ogs_error(
			"OpenAPI_app_descriptor_1_parseFromJSON() failed [app_ids]");
		goto end;
	}
	cJSON *app_ids_local_map = NULL;
	if(!cJSON_IsObject(app_ids) &&
	   !cJSON_IsNull(app_ids))
	{
		ogs_error(
			"OpenAPI_app_descriptor_1_parseFromJSON() failed [app_ids]");
		goto end;
	...
	if(cJSON_IsObject(app_ids)) {
		...
		cJSON_ArrayForEach(app_ids_local_map, app_ids) {
			...
			if(!cJSON_IsString(localMapObject)) {
				ogs_error(
					"OpenAPI_app_descriptor_1_parseFromJSON() failed [inner]");
				goto end;
			}
			localMapKeyPair =
...
OpenAPI_app_descriptor_1_t *OpenAPI_app_descriptor_1_copy(
	OpenAPI_app_descriptor_1_t *dst, OpenAPI_app_descriptor_1_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
