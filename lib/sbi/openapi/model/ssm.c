...
OpenAPI_ssm_t *OpenAPI_ssm_create(OpenAPI_ip_addr_t	*source_ip_addr,
...
cJSON *OpenAPI_ssm_convertToJSON(OpenAPI_ssm_t *ssm) {
	...
	if(!ssm->source_ip_addr) {
		ogs_error(
			"OpenAPI_ssm_convertToJSON() failed [source_ip_addr]");
		return NULL;
	}
	...
	cJSON *dest_ip_addr_local_JSON =
		OpenAPI_ip_addr_convertToJSON(ssm->dest_ip_addr);
...
OpenAPI_ssm_t *OpenAPI_ssm_parseFromJSON(cJSON *ssmJSON) {
	...
	source_ip_addr = cJSON_GetObjectItemCaseSensitive(ssmJSON,
	                                                  "sourceIpAddr");
	if(!source_ip_addr) {
		ogs_error(
			"OpenAPI_ssm_parseFromJSON() failed [source_ip_addr]");
		goto end;
	}
	source_ip_addr_local_nonprim =
		OpenAPI_ip_addr_parseFromJSON(source_ip_addr);
	if(!source_ip_addr_local_nonprim) {
		ogs_error(
			"OpenAPI_ip_addr_parseFromJSON failed [source_ip_addr]");
		goto end;
	}
	...
	dest_ip_addr_local_nonprim =
		OpenAPI_ip_addr_parseFromJSON(dest_ip_addr);
	if(!dest_ip_addr_local_nonprim) {
		ogs_error(
			"OpenAPI_ip_addr_parseFromJSON failed [dest_ip_addr]");
		goto end;
	}
