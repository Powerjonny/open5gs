...
OpenAPI_local_area_t *OpenAPI_local_area_create(
	...
	OpenAPI_local_area_t *local_area_local_var =
		ogs_malloc(sizeof(OpenAPI_local_area_t));
	...
	local_area_local_var->is_v_confidence = is_v_confidence;
...
cJSON *OpenAPI_local_area_convertToJSON(OpenAPI_local_area_t *local_area) {
	...
	if(local_area == NULL) {
		ogs_error(
			"OpenAPI_local_area_convertToJSON() failed [LocalArea]");
		return NULL;
	}
	...
	if(!local_area->local_origin) {
		ogs_error(
			"OpenAPI_local_area_convertToJSON() failed [local_origin]");
		return NULL;
	}
	cJSON *local_origin_local_JSON =
		OpenAPI_local_origin_convertToJSON(local_area->local_origin);
	...
	if(!local_area->uncertainty_ellipse) {
		ogs_error(
			"OpenAPI_local_area_convertToJSON() failed [uncertainty_ellipse]");
		return NULL;
	}
	...
	{
		ogs_error(
			"OpenAPI_local_area_convertToJSON() failed [confidence]");
		goto end;
	}

	if(!local_area->uncertainty_ellipsoid) {
		ogs_error(
			"OpenAPI_local_area_convertToJSON() failed [uncertainty_ellipsoid]");
		return NULL;
	}
	...
	if(local_area->is_v_confidence) {
		if(cJSON_AddNumberToObject(item, "vConfidence",
		                           local_area->v_confidence) == NULL)
		{
			ogs_error(
				"OpenAPI_local_area_convertToJSON() failed [v_confidence]");
			goto end;
		}
	}
...
OpenAPI_local_area_t *OpenAPI_local_area_parseFromJSON(cJSON *local_areaJSON) {
	...
	if(!local_origin) {
		ogs_error(
			"OpenAPI_local_area_parseFromJSON() failed [local_origin]");
		goto end;
	}
	...
	if(!uncertainty_ellipse) {
		ogs_error(
			"OpenAPI_local_area_parseFromJSON() failed [uncertainty_ellipse]");
		goto end;
	}
	...
	if(!confidence) {
		ogs_error(
			"OpenAPI_local_area_parseFromJSON() failed [confidence]");
		goto end;
	}
	...
	if(!uncertainty_ellipsoid) {
		ogs_error(
			"OpenAPI_local_area_parseFromJSON() failed [uncertainty_ellipsoid]");
		goto end;
	}
	...
	v_confidence = cJSON_GetObjectItemCaseSensitive(local_areaJSON,
	                                                "vConfidence");
	if(v_confidence) {
		if(!cJSON_IsNumber(v_confidence)) {
			ogs_error(
				"OpenAPI_local_area_parseFromJSON() failed [v_confidence]");
			goto end;
		}
	}
...
OpenAPI_local_area_t *OpenAPI_local_area_copy(OpenAPI_local_area_t	*dst,
                                              OpenAPI_local_area_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
