...
OpenAPI_no_profile_match_info_t *OpenAPI_no_profile_match_info_create(
	...
	OpenAPI_no_profile_match_info_t *no_profile_match_info_local_var =
		ogs_malloc(sizeof(OpenAPI_no_profile_match_info_t));
...
void OpenAPI_no_profile_match_info_free(
	...
	if(no_profile_match_info->reason) {
		OpenAPI_no_profile_match_reason_free(
...
cJSON *OpenAPI_no_profile_match_info_convertToJSON(
	OpenAPI_no_profile_match_info_t *no_profile_match_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(no_profile_match_info == NULL) {
		ogs_error(
			"OpenAPI_no_profile_match_info_convertToJSON() failed [NoProfileMatchInfo]");
		return NULL;
	}
	...
	if(!no_profile_match_info->reason) {
		ogs_error(
			"OpenAPI_no_profile_match_info_convertToJSON() failed [reason]");
		return NULL;
	}
	cJSON *reason_local_JSON =
		OpenAPI_no_profile_match_reason_convertToJSON(
			no_profile_match_info->reason);
	...
	if(no_profile_match_info->query_param_combination_list) {
		...
		if(query_param_combination_listList == NULL) {
			ogs_error(
				"OpenAPI_no_profile_match_info_convertToJSON() failed [query_param_combination_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			no_profile_match_info->query_param_combination_list,
			node) {
			cJSON *itemLocal =
				OpenAPI_query_param_combination_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_no_profile_match_info_convertToJSON() failed [query_param_combination_list]");
...
OpenAPI_no_profile_match_info_t *OpenAPI_no_profile_match_info_parseFromJSON(
	...
	if(!reason) {
		ogs_error(
			"OpenAPI_no_profile_match_info_parseFromJSON() failed [reason]");
		goto end;
	}
	reason_local_nonprim =
		OpenAPI_no_profile_match_reason_parseFromJSON(reason);
	if(!reason_local_nonprim) {
		ogs_error(
			"OpenAPI_no_profile_match_reason_parseFromJSON failed [reason]");
		goto end;
	}

	query_param_combination_list =
		cJSON_GetObjectItemCaseSensitive(no_profile_match_infoJSON,
		                                 "queryParamCombinationList");
	if(query_param_combination_list) {
		...
		if(!cJSON_IsArray(query_param_combination_list)) {
			ogs_error(
				"OpenAPI_no_profile_match_info_parseFromJSON() failed [query_param_combination_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(query_param_combination_list_local,
			...
			if(!query_param_combination_listItem) {
				ogs_error(
					"No query_param_combination_listItem");
				goto end;
			}
			OpenAPI_list_add(query_param_combination_listList,
			                 query_param_combination_listItem);
		}
	}
...
OpenAPI_no_profile_match_info_t *OpenAPI_no_profile_match_info_copy(
	OpenAPI_no_profile_match_info_t *dst,
	OpenAPI_no_profile_match_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_no_profile_match_info_convertToJSON() failed");
		return NULL;
	}
