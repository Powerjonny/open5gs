...
OpenAPI_slice_mbr_1_t *OpenAPI_slice_mbr_1_create(char	*uplink,
                                                  char	*downlink) {
	OpenAPI_slice_mbr_1_t *slice_mbr_1_local_var =
		ogs_malloc(sizeof(OpenAPI_slice_mbr_1_t));
...
cJSON *OpenAPI_slice_mbr_1_convertToJSON(OpenAPI_slice_mbr_1_t *slice_mbr_1) {
	...
	if(slice_mbr_1 == NULL) {
		ogs_error(
			"OpenAPI_slice_mbr_1_convertToJSON() failed [SliceMbr_1]");
		return NULL;
	}
	...
	if(!slice_mbr_1->uplink) {
		ogs_error(
			"OpenAPI_slice_mbr_1_convertToJSON() failed [uplink]");
		return NULL;
	}
	...
	if(!slice_mbr_1->downlink) {
		ogs_error(
			"OpenAPI_slice_mbr_1_convertToJSON() failed [downlink]");
		return NULL;
	}
...
OpenAPI_slice_mbr_1_t *OpenAPI_slice_mbr_1_parseFromJSON(
	cJSON *slice_mbr_1JSON) {
	...
	if(!uplink) {
		ogs_error(
			"OpenAPI_slice_mbr_1_parseFromJSON() failed [uplink]");
		goto end;
	}
	...
	downlink = cJSON_GetObjectItemCaseSensitive(slice_mbr_1JSON,
	                                            "downlink");
	if(!downlink) {
		ogs_error(
			"OpenAPI_slice_mbr_1_parseFromJSON() failed [downlink]");
		goto end;
	}
...
OpenAPI_slice_mbr_1_t *OpenAPI_slice_mbr_1_copy(OpenAPI_slice_mbr_1_t	*dst,
                                                OpenAPI_slice_mbr_1_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
