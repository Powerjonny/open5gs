...
OpenAPI_policy_association_t *OpenAPI_policy_association_create(
	OpenAPI_policy_association_request_t *request, OpenAPI_list_t *triggers,
	OpenAPI_service_area_restriction_t *serv_area_res,
	OpenAPI_wireline_service_area_restriction_t *wl_serv_area_res,
	bool is_rfsp, int rfsp, bool is_target_rfsp, int target_rfsp,
	bool is_smf_sel_info_null, OpenAPI_smf_selection_data_t *smf_sel_info,
	OpenAPI_ambr_t *ue_ambr, OpenAPI_list_t *ue_slice_mbrs,
	OpenAPI_list_t *pras, char *supp_feat, bool is_pcf_ue_info_null,
	OpenAPI_pcf_ue_callback_info_t *pcf_ue_info, bool is_match_pdus_null,
	OpenAPI_list_t *match_pdus, bool is_as_time_dis_param_null,
	OpenAPI_as_time_distribution_param_t *as_time_dis_param) {
	OpenAPI_policy_association_t *policy_association_local_var =
		ogs_malloc(sizeof(OpenAPI_policy_association_t));
	...
	policy_association_local_var->is_smf_sel_info_null =
	...
	policy_association_local_var->is_as_time_dis_param_null =
...
void OpenAPI_policy_association_free(
	...
	if(policy_association->serv_area_res) {
		OpenAPI_service_area_restriction_free(
			policy_association->serv_area_res);
	...
	if(policy_association->pcf_ue_info) {
		OpenAPI_pcf_ue_callback_info_free(
			policy_association->pcf_ue_info);
	...
	if(policy_association->as_time_dis_param) {
		OpenAPI_as_time_distribution_param_free(
			policy_association->as_time_dis_param);
		policy_association->as_time_dis_param = NULL;
	}
	ogs_free(policy_association);
}

cJSON *OpenAPI_policy_association_convertToJSON(
	OpenAPI_policy_association_t *policy_association) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(policy_association == NULL) {
		ogs_error(
			"OpenAPI_policy_association_convertToJSON() failed [PolicyAssociation]");
		return NULL;
	}
	...
	if(policy_association->request) {
		cJSON *request_local_JSON =
			OpenAPI_policy_association_request_convertToJSON(
				policy_association->request);
		if(request_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_policy_association_convertToJSON() failed [request]");
			goto end;
		}
	...
	if(policy_association->triggers != OpenAPI_request_trigger_NULL) {
		...
		if(triggersList == NULL) {
			ogs_error(
				"OpenAPI_policy_association_convertToJSON() failed [triggers]");
			goto end;
		}
	...
	if(policy_association->serv_area_res) {
		...
		if(serv_area_res_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_policy_association_convertToJSON() failed [serv_area_res]");
			goto end;
		}
	...
	if(policy_association->wl_serv_area_res) {
		...
		if(wl_serv_area_res_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_policy_association_convertToJSON() failed [wl_serv_area_res]");
			goto end;
		}
	...
	if(policy_association->is_rfsp) {
		if(cJSON_AddNumberToObject(item, "rfsp",
		                           policy_association->rfsp) == NULL)
		{
			ogs_error(
				"OpenAPI_policy_association_convertToJSON() failed [rfsp]");
			goto end;
		}
	}

	if(policy_association->is_target_rfsp) {
		if(cJSON_AddNumberToObject(item, "targetRfsp",
		                           policy_association->target_rfsp) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_policy_association_convertToJSON() failed [target_rfsp]");
			goto end;
		}
	}
	...
	if(policy_association->smf_sel_info) {
		...
		if(smf_sel_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_policy_association_convertToJSON() failed [smf_sel_info]");
			goto end;
		}
	...
	if(policy_association->ue_ambr) {
		cJSON *ue_ambr_local_JSON =
			OpenAPI_ambr_convertToJSON(policy_association->ue_ambr);
		if(ue_ambr_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_policy_association_convertToJSON() failed [ue_ambr]");
			goto end;
		}
	...
	if(policy_association->ue_slice_mbrs) {
		...
		if(ue_slice_mbrsList == NULL) {
			ogs_error(
				"OpenAPI_policy_association_convertToJSON() failed [ue_slice_mbrs]");
			goto end;
		}
		OpenAPI_list_for_each(policy_association->ue_slice_mbrs, node) {
			cJSON *itemLocal =
	...
	if(policy_association->pras) {
		...
		if(pras == NULL) {
			ogs_error(
				"OpenAPI_policy_association_convertToJSON() failed [pras]");
			goto end;
		}
		...
		if(policy_association->pras) {
			OpenAPI_list_for_each(policy_association->pras, node) {
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_policy_association_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(!policy_association->supp_feat) {
		ogs_error(
			"OpenAPI_policy_association_convertToJSON() failed [supp_feat]");
		return NULL;
	}
	...
	if(policy_association->pcf_ue_info) {
		...
		if(pcf_ue_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_policy_association_convertToJSON() failed [pcf_ue_info]");
			goto end;
		}
	...
	if(policy_association->match_pdus) {
		...
		if(match_pdusList == NULL) {
			ogs_error(
				"OpenAPI_policy_association_convertToJSON() failed [match_pdus]");
			goto end;
		}
		OpenAPI_list_for_each(policy_association->match_pdus, node) {
			cJSON *itemLocal =
	...
	if(policy_association->as_time_dis_param) {
		cJSON *as_time_dis_param_local_JSON =
			OpenAPI_as_time_distribution_param_convertToJSON(
				policy_association->as_time_dis_param);
		if(as_time_dis_param_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_policy_association_convertToJSON() failed [as_time_dis_param]");
			goto end;
		}
...
OpenAPI_policy_association_t *OpenAPI_policy_association_parseFromJSON(
	cJSON *policy_associationJSON) {
	...
	OpenAPI_as_time_distribution_param_t *as_time_dis_param_local_nonprim =
		NULL;
	request = cJSON_GetObjectItemCaseSensitive(policy_associationJSON,
	                                           "request");
	if(request) {
		...
		if(!request_local_nonprim) {
			ogs_error(
				"OpenAPI_policy_association_request_parseFromJSON failed [request]");
			goto end;
		}
	}

	triggers = cJSON_GetObjectItemCaseSensitive(policy_associationJSON,
	                                            "triggers");
	if(triggers) {
		cJSON *triggers_local = NULL;
		if(!cJSON_IsArray(triggers)) {
			ogs_error(
				"OpenAPI_policy_association_parseFromJSON() failed [triggers]");
			goto end;
		}
		...
		if(triggersList->count == 0) {
			ogs_error(
				"OpenAPI_policy_association_parseFromJSON() failed: Expected triggersList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}

	serv_area_res = cJSON_GetObjectItemCaseSensitive(policy_associationJSON,
	                                                 "servAreaRes");
	if(serv_area_res) {
	...
	wl_serv_area_res =
		cJSON_GetObjectItemCaseSensitive(policy_associationJSON,
		                                 "wlServAreaRes");
	if(wl_serv_area_res) {
	...
	if(rfsp) {
		if(!cJSON_IsNumber(rfsp)) {
			ogs_error(
				"OpenAPI_policy_association_parseFromJSON() failed [rfsp]");
			goto end;
		}
	}

	target_rfsp = cJSON_GetObjectItemCaseSensitive(policy_associationJSON,
	                                               "targetRfsp");
	if(target_rfsp) {
		if(!cJSON_IsNumber(target_rfsp)) {
			ogs_error(
				"OpenAPI_policy_association_parseFromJSON() failed [target_rfsp]");
			goto end;
		}
	}

	smf_sel_info = cJSON_GetObjectItemCaseSensitive(policy_associationJSON,
	                                                "smfSelInfo");
	if(smf_sel_info) {
	...
	ue_ambr = cJSON_GetObjectItemCaseSensitive(policy_associationJSON,
	                                           "ueAmbr");
	if(ue_ambr) {
	...
	ue_slice_mbrs = cJSON_GetObjectItemCaseSensitive(policy_associationJSON,
	                                                 "ueSliceMbrs");
	if(ue_slice_mbrs) {
		cJSON *ue_slice_mbrs_local = NULL;
		if(!cJSON_IsArray(ue_slice_mbrs)) {
			ogs_error(
				"OpenAPI_policy_association_parseFromJSON() failed [ue_slice_mbrs]");
			goto end;
		}
		...
		if(cJSON_IsObject(pras)) {
			...
			cJSON_ArrayForEach(pras_local_map, pras) {
				...
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_policy_association_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	supp_feat = cJSON_GetObjectItemCaseSensitive(policy_associationJSON,
	                                             "suppFeat");
	if(!supp_feat) {
		ogs_error(
			"OpenAPI_policy_association_parseFromJSON() failed [supp_feat]");
		goto end;
	}
	...
	pcf_ue_info = cJSON_GetObjectItemCaseSensitive(policy_associationJSON,
	                                               "pcfUeInfo");
	if(pcf_ue_info) {
	...
	match_pdus = cJSON_GetObjectItemCaseSensitive(policy_associationJSON,
	                                              "matchPdus");
	if(match_pdus) {
		if(!cJSON_IsNull(match_pdus)) {
			...
			if(!cJSON_IsArray(match_pdus)) {
				ogs_error(
					"OpenAPI_policy_association_parseFromJSON() failed [match_pdus]");
				goto end;
			}
	...
	if(as_time_dis_param) {
		if(!cJSON_IsNull(as_time_dis_param)) {
			as_time_dis_param_local_nonprim =
				OpenAPI_as_time_distribution_param_parseFromJSON
				        (as_time_dis_param);
			if(!as_time_dis_param_local_nonprim) {
				ogs_error(
					"OpenAPI_as_time_distribution_param_parseFromJSON failed [as_time_dis_param]");
				goto end;
			}
		}
	}
	...
	policy_association_local_var = OpenAPI_policy_association_create(
		...
		as_time_dis_param &&
		cJSON_IsNull(as_time_dis_param) ? true : false,
...
OpenAPI_policy_association_t *OpenAPI_policy_association_copy(
	OpenAPI_policy_association_t *dst, OpenAPI_policy_association_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
