...
{
	OpenAPI_notify_item_t *notify_item_local_var =
		ogs_malloc(sizeof(OpenAPI_notify_item_t));
...
cJSON *OpenAPI_notify_item_convertToJSON(OpenAPI_notify_item_t *notify_item) {
	...
	if(notify_item == NULL) {
		ogs_error(
			"OpenAPI_notify_item_convertToJSON() failed [NotifyItem]");
		return NULL;
	}
	...
	if(!notify_item->resource_id) {
		ogs_error(
			"OpenAPI_notify_item_convertToJSON() failed [resource_id]");
		return NULL;
	}
	...
	if(!notify_item->changes) {
		ogs_error(
			"OpenAPI_notify_item_convertToJSON() failed [changes]");
		return NULL;
	}
...
OpenAPI_notify_item_t *OpenAPI_notify_item_parseFromJSON(
	...
	resource_id = cJSON_GetObjectItemCaseSensitive(notify_itemJSON,
	                                               "resourceId");
	if(!resource_id) {
		ogs_error(
			"OpenAPI_notify_item_parseFromJSON() failed [resource_id]");
		goto end;
	}
	...
	if(!changes) {
		ogs_error(
			"OpenAPI_notify_item_parseFromJSON() failed [changes]");
		goto end;
	}
	...
	cJSON_ArrayForEach(changes_local, changes) {
		...
		OpenAPI_change_item_t *changesItem =
			OpenAPI_change_item_parseFromJSON(changes_local);
		if(!changesItem) {
