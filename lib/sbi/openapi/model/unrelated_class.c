...
{
	OpenAPI_unrelated_class_t *unrelated_class_local_var =
		ogs_malloc(sizeof(OpenAPI_unrelated_class_t));
	ogs_assert(unrelated_class_local_var);

	unrelated_class_local_var->default_unrelated_class =
		default_unrelated_class;
	unrelated_class_local_var->external_unrelated_class =
		external_unrelated_class;
	unrelated_class_local_var->service_type_unrelated_classes =
...
cJSON *OpenAPI_unrelated_class_convertToJSON(
	...
	if(unrelated_class == NULL) {
		ogs_error(
			"OpenAPI_unrelated_class_convertToJSON() failed [UnrelatedClass]");
		return NULL;
	}
	...
	if(!unrelated_class->default_unrelated_class) {
		ogs_error(
			"OpenAPI_unrelated_class_convertToJSON() failed [default_unrelated_class]");
		return NULL;
	}
	cJSON *default_unrelated_class_local_JSON =
		OpenAPI_default_unrelated_class_convertToJSON(
			unrelated_class->default_unrelated_class);
	...
	cJSON_AddItemToObject(item, "defaultUnrelatedClass",
	                      default_unrelated_class_local_JSON);
	if(item->child == NULL) {
		ogs_error(
			"OpenAPI_unrelated_class_convertToJSON() failed [default_unrelated_class]");
		goto end;
	...
	if(unrelated_class->external_unrelated_class) {
		cJSON *external_unrelated_class_local_JSON =
			OpenAPI_external_unrelated_class_convertToJSON(
				unrelated_class->external_unrelated_class);
		if(external_unrelated_class_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_unrelated_class_convertToJSON() failed [external_unrelated_class]");
			goto end;
		}
		cJSON_AddItemToObject(item, "externalUnrelatedClass",
		                      external_unrelated_class_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_unrelated_class_convertToJSON() failed [external_unrelated_class]");
			goto end;
	...
	if(unrelated_class->service_type_unrelated_classes) {
		cJSON *service_type_unrelated_classesList =
			cJSON_AddArrayToObject(item,
			                       "serviceTypeUnrelatedClasses");
		if(service_type_unrelated_classesList == NULL) {
			ogs_error(
				"OpenAPI_unrelated_class_convertToJSON() failed [service_type_unrelated_classes]");
			goto end;
		}
		OpenAPI_list_for_each(
			unrelated_class->service_type_unrelated_classes, node) {
			cJSON *itemLocal =
				OpenAPI_service_type_unrelated_class_convertToJSON
				        (node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_unrelated_class_convertToJSON() failed [service_type_unrelated_classes]");
			...
			cJSON_AddItemToArray(service_type_unrelated_classesList,
			                     itemLocal);
		}
	}
...
OpenAPI_unrelated_class_t *OpenAPI_unrelated_class_parseFromJSON(
	cJSON *unrelated_classJSON) {
	OpenAPI_unrelated_class_t *unrelated_class_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *default_unrelated_class = NULL;
	OpenAPI_default_unrelated_class_t *default_unrelated_class_local_nonprim
	        = NULL;
	...
	if(!default_unrelated_class) {
		ogs_error(
			"OpenAPI_unrelated_class_parseFromJSON() failed [default_unrelated_class]");
		goto end;
	}
	...
	if(!default_unrelated_class_local_nonprim) {
		ogs_error(
			"OpenAPI_default_unrelated_class_parseFromJSON failed [default_unrelated_class]");
		goto end;
	}
	...
	if(external_unrelated_class) {
		external_unrelated_class_local_nonprim =
			OpenAPI_external_unrelated_class_parseFromJSON(
				external_unrelated_class);
		if(!external_unrelated_class_local_nonprim) {
			ogs_error(
				"OpenAPI_external_unrelated_class_parseFromJSON failed [external_unrelated_class]");
			goto end;
		}
	}
	...
	if(service_type_unrelated_classes) {
		...
		if(!cJSON_IsArray(service_type_unrelated_classes)) {
			ogs_error(
				"OpenAPI_unrelated_class_parseFromJSON() failed [service_type_unrelated_classes]");
			goto end;
		}

		service_type_unrelated_classesList = OpenAPI_list_create();

		cJSON_ArrayForEach(service_type_unrelated_classes_local,
		                   service_type_unrelated_classes) {
			if(!cJSON_IsObject(
			...
			OpenAPI_service_type_unrelated_class_t *
			        service_type_unrelated_classesItem =
				OpenAPI_service_type_unrelated_class_parseFromJSON
				        (service_type_unrelated_classes_local);
			if(!service_type_unrelated_classesItem) {
				ogs_error(
					"No service_type_unrelated_classesItem");
				goto end;
			}
			OpenAPI_list_add(service_type_unrelated_classesList,
			                 service_type_unrelated_classesItem);
		}
	}
	...
	if(service_type_unrelated_classesList) {
		OpenAPI_list_for_each(service_type_unrelated_classesList,
		                      node) {
			OpenAPI_service_type_unrelated_class_free(node->data);
		}
...
OpenAPI_unrelated_class_t *OpenAPI_unrelated_class_copy(
	OpenAPI_unrelated_class_t *dst, OpenAPI_unrelated_class_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
