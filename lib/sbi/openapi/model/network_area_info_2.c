...
OpenAPI_network_area_info_2_t *OpenAPI_network_area_info_2_create(
	...
	OpenAPI_network_area_info_2_t *network_area_info_2_local_var =
		ogs_malloc(sizeof(OpenAPI_network_area_info_2_t));
...
void OpenAPI_network_area_info_2_free(
	OpenAPI_network_area_info_2_t *network_area_info_2) {
	...
	if(network_area_info_2->g_ran_node_ids) {
		OpenAPI_list_for_each(network_area_info_2->g_ran_node_ids,
		                      node) {
...
cJSON *OpenAPI_network_area_info_2_convertToJSON(
	OpenAPI_network_area_info_2_t *network_area_info_2) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(network_area_info_2 == NULL) {
		ogs_error(
			"OpenAPI_network_area_info_2_convertToJSON() failed [NetworkAreaInfo_2]");
		return NULL;
	}
	...
	if(network_area_info_2->ecgis) {
		...
		if(ecgisList == NULL) {
			ogs_error(
				"OpenAPI_network_area_info_2_convertToJSON() failed [ecgis]");
			goto end;
		}
		OpenAPI_list_for_each(network_area_info_2->ecgis, node) {
			cJSON *itemLocal =
				OpenAPI_ecgi_1_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_network_area_info_2_convertToJSON() failed [ecgis]");
				goto end;
	...
	if(network_area_info_2->ncgis) {
		...
		if(ncgisList == NULL) {
			ogs_error(
				"OpenAPI_network_area_info_2_convertToJSON() failed [ncgis]");
			goto end;
		}
		OpenAPI_list_for_each(network_area_info_2->ncgis, node) {
			cJSON *itemLocal =
				OpenAPI_ncgi_1_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_network_area_info_2_convertToJSON() failed [ncgis]");
				goto end;
	...
	if(network_area_info_2->g_ran_node_ids) {
		...
		if(g_ran_node_idsList == NULL) {
			ogs_error(
				"OpenAPI_network_area_info_2_convertToJSON() failed [g_ran_node_ids]");
			goto end;
		}
		OpenAPI_list_for_each(network_area_info_2->g_ran_node_ids,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_global_ran_node_id_1_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_network_area_info_2_convertToJSON() failed [g_ran_node_ids]");
	...
	if(network_area_info_2->tais) {
		...
		if(taisList == NULL) {
			ogs_error(
				"OpenAPI_network_area_info_2_convertToJSON() failed [tais]");
			goto end;
		}
		OpenAPI_list_for_each(network_area_info_2->tais, node) {
			cJSON *itemLocal =
...
OpenAPI_network_area_info_2_t *OpenAPI_network_area_info_2_parseFromJSON(
	...
	ecgis = cJSON_GetObjectItemCaseSensitive(network_area_info_2JSON,
	                                         "ecgis");
	if(ecgis) {
		cJSON *ecgis_local = NULL;
		if(!cJSON_IsArray(ecgis)) {
			ogs_error(
				"OpenAPI_network_area_info_2_parseFromJSON() failed [ecgis]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ecgis_local, ecgis) {
			...
			OpenAPI_ecgi_1_t *ecgisItem =
				OpenAPI_ecgi_1_parseFromJSON(ecgis_local);
			if(!ecgisItem) {
	...
	ncgis = cJSON_GetObjectItemCaseSensitive(network_area_info_2JSON,
	                                         "ncgis");
	if(ncgis) {
		cJSON *ncgis_local = NULL;
		if(!cJSON_IsArray(ncgis)) {
			ogs_error(
				"OpenAPI_network_area_info_2_parseFromJSON() failed [ncgis]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ncgis_local, ncgis) {
			...
			OpenAPI_ncgi_1_t *ncgisItem =
				OpenAPI_ncgi_1_parseFromJSON(ncgis_local);
			if(!ncgisItem) {
	...
	g_ran_node_ids =
		cJSON_GetObjectItemCaseSensitive(network_area_info_2JSON,
		                                 "gRanNodeIds");
	if(g_ran_node_ids) {
		cJSON *g_ran_node_ids_local = NULL;
		if(!cJSON_IsArray(g_ran_node_ids)) {
			ogs_error(
				"OpenAPI_network_area_info_2_parseFromJSON() failed [g_ran_node_ids]");
			goto end;
		}
		...
		cJSON_ArrayForEach(g_ran_node_ids_local, g_ran_node_ids) {
			...
			OpenAPI_global_ran_node_id_1_t *g_ran_node_idsItem =
	...
	tais = cJSON_GetObjectItemCaseSensitive(network_area_info_2JSON,
	                                        "tais");
	if(tais) {
		cJSON *tais_local = NULL;
		if(!cJSON_IsArray(tais)) {
			ogs_error(
				"OpenAPI_network_area_info_2_parseFromJSON() failed [tais]");
			goto end;
		}
		...
		cJSON_ArrayForEach(tais_local, tais) {
			...
			OpenAPI_tai_1_t *taisItem =
				OpenAPI_tai_1_parseFromJSON(tais_local);
			if(!taisItem) {
...
OpenAPI_network_area_info_2_t *OpenAPI_network_area_info_2_copy(
	OpenAPI_network_area_info_2_t	*dst,
	OpenAPI_network_area_info_2_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
