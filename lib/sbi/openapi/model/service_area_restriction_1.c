...
OpenAPI_service_area_restriction_1_t *OpenAPI_service_area_restriction_1_create(
	...
	OpenAPI_service_area_restriction_1_t *
	        service_area_restriction_1_local_var =
		ogs_malloc(sizeof(OpenAPI_service_area_restriction_1_t));
	ogs_assert(service_area_restriction_1_local_var);

	service_area_restriction_1_local_var->restriction_type =
		restriction_type;
	service_area_restriction_1_local_var->areas = areas;
	service_area_restriction_1_local_var->is_max_num_of_tas =
		is_max_num_of_tas;
...
void OpenAPI_service_area_restriction_1_free(
...
cJSON *OpenAPI_service_area_restriction_1_convertToJSON(
	OpenAPI_service_area_restriction_1_t *service_area_restriction_1) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(service_area_restriction_1 == NULL) {
		ogs_error(
			"OpenAPI_service_area_restriction_1_convertToJSON() failed [ServiceAreaRestriction_1]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(service_area_restriction_1->restriction_type !=
	   OpenAPI_restriction_type_NULL)
	{
		if(cJSON_AddStringToObject(item, "restrictionType",
		                           OpenAPI_restriction_type_ToString(
						   service_area_restriction_1->
						   restriction_type)) == NULL)
		{
			ogs_error(
				"OpenAPI_service_area_restriction_1_convertToJSON() failed [restriction_type]");
			goto end;
		}
	}
	...
	if(service_area_restriction_1->areas) {
		...
		if(areasList == NULL) {
			ogs_error(
				"OpenAPI_service_area_restriction_1_convertToJSON() failed [areas]");
			goto end;
		}
		OpenAPI_list_for_each(service_area_restriction_1->areas, node) {
			cJSON *itemLocal =
	...
	if(service_area_restriction_1->is_max_num_of_tas) {
		...
		{
			ogs_error(
				"OpenAPI_service_area_restriction_1_convertToJSON() failed [max_num_of_tas]");
			goto end;
		}
	}
	...
	if(service_area_restriction_1->is_max_num_of_tas_for_not_allowed_areas)
	{
		...
		{
			ogs_error(
				"OpenAPI_service_area_restriction_1_convertToJSON() failed [max_num_of_tas_for_not_allowed_areas]");
			goto end;
		}
	}
...
OpenAPI_service_area_restriction_1_parseFromJSON(
	cJSON *service_area_restriction_1JSON) {
	OpenAPI_service_area_restriction_1_t *
	        service_area_restriction_1_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(restriction_type) {
		if(!cJSON_IsString(restriction_type)) {
			ogs_error(
				"OpenAPI_service_area_restriction_1_parseFromJSON() failed [restriction_type]");
			goto end;
		}
		restriction_typeVariable =
			OpenAPI_restriction_type_FromString(
				restriction_type->valuestring);
	}

	areas = cJSON_GetObjectItemCaseSensitive(service_area_restriction_1JSON,
	                                         "areas");
	if(areas) {
		cJSON *areas_local = NULL;
		if(!cJSON_IsArray(areas)) {
			ogs_error(
				"OpenAPI_service_area_restriction_1_parseFromJSON() failed [areas]");
			goto end;
		}
		...
		cJSON_ArrayForEach(areas_local, areas) {
			...
			OpenAPI_area_1_t *areasItem =
				OpenAPI_area_1_parseFromJSON(areas_local);
			if(!areasItem) {
	...
	if(max_num_of_tas) {
		if(!cJSON_IsNumber(max_num_of_tas)) {
			ogs_error(
				"OpenAPI_service_area_restriction_1_parseFromJSON() failed [max_num_of_tas]");
			goto end;
		}
	}
	...
	if(max_num_of_tas_for_not_allowed_areas) {
		if(!cJSON_IsNumber(max_num_of_tas_for_not_allowed_areas)) {
			ogs_error(
				"OpenAPI_service_area_restriction_1_parseFromJSON() failed [max_num_of_tas_for_not_allowed_areas]");
			goto end;
		}
	}

	service_area_restriction_1_local_var =
		OpenAPI_service_area_restriction_1_create(
			...
			max_num_of_tas_for_not_allowed_areas ?
			max_num_of_tas_for_not_allowed_areas->valuedouble : 0
			);
...
OpenAPI_service_area_restriction_1_t *OpenAPI_service_area_restriction_1_copy(
	OpenAPI_service_area_restriction_1_t	*dst,
	OpenAPI_service_area_restriction_1_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_service_area_restriction_1_convertToJSON() failed");
		return NULL;
	}
