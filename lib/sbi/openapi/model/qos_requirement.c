...
OpenAPI_qos_requirement_t *OpenAPI_qos_requirement_create(bool is__5qi,
                                                          int _5qi,
                                                          char *gfbr_ul,
                                                          char *gfbr_dl,
                                                          OpenAPI_qos_resource_type_e
                                                          res_type, bool is_pdb,
                                                          int pdb, char *per) {
	OpenAPI_qos_requirement_t *qos_requirement_local_var =
		ogs_malloc(sizeof(OpenAPI_qos_requirement_t));
...
cJSON *OpenAPI_qos_requirement_convertToJSON(
	OpenAPI_qos_requirement_t *qos_requirement) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(qos_requirement == NULL) {
		ogs_error(
			"OpenAPI_qos_requirement_convertToJSON() failed [QosRequirement]");
		return NULL;
	}
	...
	if(qos_requirement->is__5qi) {
		...
		{
			ogs_error(
				"OpenAPI_qos_requirement_convertToJSON() failed [_5qi]");
			goto end;
		}
	}
	...
	if(qos_requirement->gfbr_ul) {
		...
		{
			ogs_error(
				"OpenAPI_qos_requirement_convertToJSON() failed [gfbr_ul]");
			goto end;
		}
	}
	...
	if(qos_requirement->gfbr_dl) {
		...
		{
			ogs_error(
				"OpenAPI_qos_requirement_convertToJSON() failed [gfbr_dl]");
			goto end;
		}
	}

	if(qos_requirement->res_type != OpenAPI_qos_resource_type_NULL) {
		if(cJSON_AddStringToObject(item, "resType",
		                           OpenAPI_qos_resource_type_ToString(
						   qos_requirement->res_type))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_qos_requirement_convertToJSON() failed [res_type]");
			goto end;
		}
	}
	...
	if(qos_requirement->is_pdb) {
		...
		{
			ogs_error(
				"OpenAPI_qos_requirement_convertToJSON() failed [pdb]");
			goto end;
		}
	}
	...
	if(qos_requirement->per) {
		...
		{
			ogs_error(
				"OpenAPI_qos_requirement_convertToJSON() failed [per]");
			goto end;
		}
	}
...
OpenAPI_qos_requirement_t *OpenAPI_qos_requirement_parseFromJSON(
	...
	if(_5qi) {
		if(!cJSON_IsNumber(_5qi)) {
			ogs_error(
				"OpenAPI_qos_requirement_parseFromJSON() failed [_5qi]");
			goto end;
		}
	}

	gfbr_ul = cJSON_GetObjectItemCaseSensitive(qos_requirementJSON,
	                                           "gfbrUl");
	if(gfbr_ul) {
		...
		{
			ogs_error(
				"OpenAPI_qos_requirement_parseFromJSON() failed [gfbr_ul]");
			goto end;
		}
	}

	gfbr_dl = cJSON_GetObjectItemCaseSensitive(qos_requirementJSON,
	                                           "gfbrDl");
	if(gfbr_dl) {
		...
		{
			ogs_error(
				"OpenAPI_qos_requirement_parseFromJSON() failed [gfbr_dl]");
			goto end;
		}
	}
	...
	if(res_type) {
		if(!cJSON_IsString(res_type)) {
			ogs_error(
				"OpenAPI_qos_requirement_parseFromJSON() failed [res_type]");
			goto end;
		}
		res_typeVariable =
			OpenAPI_qos_resource_type_FromString(
				res_type->valuestring);
	}
	...
	if(pdb) {
		if(!cJSON_IsNumber(pdb)) {
			ogs_error(
				"OpenAPI_qos_requirement_parseFromJSON() failed [pdb]");
			goto end;
		}
	}
	...
	if(per) {
		...
		{
			ogs_error(
				"OpenAPI_qos_requirement_parseFromJSON() failed [per]");
			goto end;
		}
	}
	...
	qos_requirement_local_var = OpenAPI_qos_requirement_create(
		...
		gfbr_ul &&
		!cJSON_IsNull(gfbr_ul) ? ogs_strdup(gfbr_ul->valuestring) :
		NULL,
		gfbr_dl &&
		!cJSON_IsNull(gfbr_dl) ? ogs_strdup(gfbr_dl->valuestring) :
		NULL,
		res_type ? res_typeVariable : 0,
		pdb ? true : false,
		pdb ? pdb->valuedouble : 0,
		per &&
		!cJSON_IsNull(per) ? ogs_strdup(per->valuestring) : NULL
		);
...
OpenAPI_qos_requirement_t *OpenAPI_qos_requirement_copy(
	OpenAPI_qos_requirement_t *dst, OpenAPI_qos_requirement_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
