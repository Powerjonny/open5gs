...
OpenAPI_pfd_content_t *OpenAPI_pfd_content_create(char *pfd_id,
                                                  OpenAPI_list_t *
                                                  flow_descriptions,
                                                  OpenAPI_list_t
                                                        *urls,
                                                  ...
                                                  OpenAPI_domain_name_protocol_t
                                                        *dn_protocol) {
	OpenAPI_pfd_content_t *pfd_content_local_var =
		ogs_malloc(sizeof(OpenAPI_pfd_content_t));
...
cJSON *OpenAPI_pfd_content_convertToJSON(OpenAPI_pfd_content_t *pfd_content) {
	...
	if(pfd_content == NULL) {
		ogs_error(
			"OpenAPI_pfd_content_convertToJSON() failed [PfdContent]");
		return NULL;
	}
	...
	if(pfd_content->pfd_id) {
		...
		{
			ogs_error(
				"OpenAPI_pfd_content_convertToJSON() failed [pfd_id]");
			goto end;
		}
	}
	...
	if(pfd_content->flow_descriptions) {
		...
		if(flow_descriptionsList == NULL) {
			ogs_error(
				"OpenAPI_pfd_content_convertToJSON() failed [flow_descriptions]");
			goto end;
		}
		OpenAPI_list_for_each(pfd_content->flow_descriptions, node) {
	...
	if(pfd_content->urls) {
		...
		if(urlsList == NULL) {
			ogs_error(
				"OpenAPI_pfd_content_convertToJSON() failed [urls]");
			goto end;
		}
		OpenAPI_list_for_each(pfd_content->urls, node) {
	...
	if(pfd_content->domain_names) {
		...
		if(domain_namesList == NULL) {
			ogs_error(
				"OpenAPI_pfd_content_convertToJSON() failed [domain_names]");
			goto end;
		}
		OpenAPI_list_for_each(pfd_content->domain_names, node) {
	...
	if(pfd_content->dn_protocol) {
		cJSON *dn_protocol_local_JSON =
			OpenAPI_domain_name_protocol_convertToJSON(
				pfd_content->dn_protocol);
		if(dn_protocol_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_pfd_content_convertToJSON() failed [dn_protocol]");
			goto end;
		}
...
OpenAPI_pfd_content_t *OpenAPI_pfd_content_parseFromJSON(
	cJSON *pfd_contentJSON) {
	...
	if(pfd_id) {
		...
		{
			ogs_error(
				"OpenAPI_pfd_content_parseFromJSON() failed [pfd_id]");
			goto end;
		}
	}

	flow_descriptions = cJSON_GetObjectItemCaseSensitive(pfd_contentJSON,
	                                                     "flowDescriptions");
	if(flow_descriptions) {
		cJSON *flow_descriptions_local = NULL;
		if(!cJSON_IsArray(flow_descriptions)) {
			ogs_error(
				"OpenAPI_pfd_content_parseFromJSON() failed [flow_descriptions]");
			goto end;
		}
		...
		cJSON_ArrayForEach(flow_descriptions_local, flow_descriptions) {
			...
			OpenAPI_list_add(flow_descriptionsList,
			                 ogs_strdup(
						 flow_descriptions_local->
						 valuestring));
		}
	}
	...
	if(urls) {
		...
		if(!cJSON_IsArray(urls)) {
			ogs_error(
				"OpenAPI_pfd_content_parseFromJSON() failed [urls]");
			goto end;
		}
		...
		cJSON_ArrayForEach(urls_local, urls) {
			...
			OpenAPI_list_add(urlsList,
			                 ogs_strdup(urls_local->valuestring));
		}
	}

	domain_names = cJSON_GetObjectItemCaseSensitive(pfd_contentJSON,
	                                                "domainNames");
	if(domain_names) {
		cJSON *domain_names_local = NULL;
		if(!cJSON_IsArray(domain_names)) {
			ogs_error(
				"OpenAPI_pfd_content_parseFromJSON() failed [domain_names]");
			goto end;
		}
		...
		cJSON_ArrayForEach(domain_names_local, domain_names) {
			...
			OpenAPI_list_add(domain_namesList,
			                 ogs_strdup(
						 domain_names_local->valuestring));
		}
	}

	dn_protocol = cJSON_GetObjectItemCaseSensitive(pfd_contentJSON,
	                                               "dnProtocol");
	if(dn_protocol) {
		dn_protocol_local_nonprim =
			OpenAPI_domain_name_protocol_parseFromJSON(dn_protocol);
		if(!dn_protocol_local_nonprim) {
			ogs_error(
				"OpenAPI_domain_name_protocol_parseFromJSON failed [dn_protocol]");
			goto end;
		}
	}
	...
	pfd_content_local_var = OpenAPI_pfd_content_create(
		pfd_id &&
		!cJSON_IsNull(pfd_id) ? ogs_strdup(pfd_id->valuestring) : NULL,
...
OpenAPI_pfd_content_t *OpenAPI_pfd_content_copy(OpenAPI_pfd_content_t	*dst,
                                                OpenAPI_pfd_content_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
