...
OpenAPI_un_trust_af_info_t *OpenAPI_un_trust_af_info_create(char *af_id,
                                                            OpenAPI_list_t *
                                                            s_nssai_info_list,
                                                            bool
                                                                is_mapping_ind,
                                                            int
                                                                mapping_ind) {
	OpenAPI_un_trust_af_info_t *un_trust_af_info_local_var =
		ogs_malloc(sizeof(OpenAPI_un_trust_af_info_t));
...
void OpenAPI_un_trust_af_info_free(
	OpenAPI_un_trust_af_info_t *un_trust_af_info) {
	...
	if(un_trust_af_info->s_nssai_info_list) {
		OpenAPI_list_for_each(un_trust_af_info->s_nssai_info_list,
		                      node) {
...
cJSON *OpenAPI_un_trust_af_info_convertToJSON(
	...
	if(un_trust_af_info == NULL) {
		ogs_error(
			"OpenAPI_un_trust_af_info_convertToJSON() failed [UnTrustAfInfo]");
		return NULL;
	}
	...
	if(!un_trust_af_info->af_id) {
		ogs_error(
			"OpenAPI_un_trust_af_info_convertToJSON() failed [af_id]");
		return NULL;
	}
	...
	if(un_trust_af_info->s_nssai_info_list) {
		...
		if(s_nssai_info_listList == NULL) {
			ogs_error(
				"OpenAPI_un_trust_af_info_convertToJSON() failed [s_nssai_info_list]");
			goto end;
		}
		OpenAPI_list_for_each(un_trust_af_info->s_nssai_info_list,
		                      node) {
	...
	if(un_trust_af_info->is_mapping_ind) {
		if(cJSON_AddBoolToObject(item, "mappingInd",
		                         un_trust_af_info->mapping_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_un_trust_af_info_convertToJSON() failed [mapping_ind]");
			goto end;
		}
	}
...
OpenAPI_un_trust_af_info_t *OpenAPI_un_trust_af_info_parseFromJSON(
	cJSON *un_trust_af_infoJSON) {
	...
	if(!af_id) {
		ogs_error(
			"OpenAPI_un_trust_af_info_parseFromJSON() failed [af_id]");
		goto end;
	}
	...
	s_nssai_info_list =
		cJSON_GetObjectItemCaseSensitive(un_trust_af_infoJSON,
		                                 "sNssaiInfoList");
	if(s_nssai_info_list) {
		cJSON *s_nssai_info_list_local = NULL;
		if(!cJSON_IsArray(s_nssai_info_list)) {
			ogs_error(
				"OpenAPI_un_trust_af_info_parseFromJSON() failed [s_nssai_info_list]");
			goto end;
		}
	...
	mapping_ind = cJSON_GetObjectItemCaseSensitive(un_trust_af_infoJSON,
	                                               "mappingInd");
	if(mapping_ind) {
		if(!cJSON_IsBool(mapping_ind)) {
			ogs_error(
				"OpenAPI_un_trust_af_info_parseFromJSON() failed [mapping_ind]");
			goto end;
		}
	}
...
OpenAPI_un_trust_af_info_t *OpenAPI_un_trust_af_info_copy(
	OpenAPI_un_trust_af_info_t *dst, OpenAPI_un_trust_af_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
