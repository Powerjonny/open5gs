...
OpenAPI_updated_item_t *OpenAPI_updated_item_create(char *item,
                                                    bool
                                                    is_value_null,
                                                    OpenAPI_any_type_t *value)
{
	OpenAPI_updated_item_t *updated_item_local_var =
		ogs_malloc(sizeof(OpenAPI_updated_item_t));
...
cJSON *OpenAPI_updated_item_convertToJSON(
	OpenAPI_updated_item_t *updated_item) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(updated_item == NULL) {
		ogs_error(
			"OpenAPI_updated_item_convertToJSON() failed [UpdatedItem]");
		return NULL;
	}
	...
	if(!updated_item->value) {
		ogs_error(
			"OpenAPI_updated_item_convertToJSON() failed [value]");
		return NULL;
	}
	cJSON *value_object =
		OpenAPI_any_type_convertToJSON(updated_item->value);
...
OpenAPI_updated_item_t *OpenAPI_updated_item_parseFromJSON(
	cJSON *updated_itemJSON) {
	...
	if(!value) {
		ogs_error(
			"OpenAPI_updated_item_parseFromJSON() failed [value]");
		goto end;
	}
...
OpenAPI_updated_item_t *OpenAPI_updated_item_copy(
	OpenAPI_updated_item_t	*dst,
