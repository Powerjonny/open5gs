...
OpenAPI_default_unrelated_class_t *OpenAPI_default_unrelated_class_create(
	...
	OpenAPI_default_unrelated_class_t *default_unrelated_class_local_var =
		ogs_malloc(sizeof(OpenAPI_default_unrelated_class_t));
	ogs_assert(default_unrelated_class_local_var);

	default_unrelated_class_local_var->allowed_geographic_area =
		allowed_geographic_area;
	default_unrelated_class_local_var->privacy_check_related_action =
		privacy_check_related_action;
	default_unrelated_class_local_var->code_word_ind = code_word_ind;
	default_unrelated_class_local_var->valid_time_period =
		valid_time_period;
...
void OpenAPI_default_unrelated_class_free(
	...
	if(default_unrelated_class->allowed_geographic_area) {
		OpenAPI_list_for_each(
			default_unrelated_class->allowed_geographic_area,
			node) {
	...
	if(default_unrelated_class->valid_time_period) {
		OpenAPI_valid_time_period_free(
			default_unrelated_class->valid_time_period);
		default_unrelated_class->valid_time_period = NULL;
	}
	if(default_unrelated_class->code_word_list) {
		OpenAPI_list_for_each(default_unrelated_class->code_word_list,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_default_unrelated_class_convertToJSON(
	...
	if(default_unrelated_class == NULL) {
		ogs_error(
			"OpenAPI_default_unrelated_class_convertToJSON() failed [DefaultUnrelatedClass]");
		return NULL;
	}
	...
	if(default_unrelated_class->allowed_geographic_area) {
		...
		if(allowed_geographic_areaList == NULL) {
			ogs_error(
				"OpenAPI_default_unrelated_class_convertToJSON() failed [allowed_geographic_area]");
			goto end;
		}
		OpenAPI_list_for_each(
			default_unrelated_class->allowed_geographic_area,
	   ...
	   OpenAPI_privacy_check_related_action_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_default_unrelated_class_convertToJSON() failed [privacy_check_related_action]");
			goto end;
		}
	}
	   ...
	   OpenAPI_code_word_ind_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_default_unrelated_class_convertToJSON() failed [code_word_ind]");
			goto end;
		}
	}
	...
	if(default_unrelated_class->valid_time_period) {
		...
		if(valid_time_period_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_default_unrelated_class_convertToJSON() failed [valid_time_period]");
			goto end;
		}
	...
	if(default_unrelated_class->code_word_list) {
		...
		if(code_word_listList == NULL) {
			ogs_error(
				"OpenAPI_default_unrelated_class_convertToJSON() failed [code_word_list]");
			goto end;
		}
		OpenAPI_list_for_each(default_unrelated_class->code_word_list,
		                      node) {
...
OpenAPI_default_unrelated_class_t *OpenAPI_default_unrelated_class_parseFromJSON
        (cJSON *default_unrelated_classJSON) {
	OpenAPI_default_unrelated_class_t *default_unrelated_class_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	allowed_geographic_area =
		cJSON_GetObjectItemCaseSensitive(default_unrelated_classJSON,
	...
	if(allowed_geographic_area) {
		...
		if(!cJSON_IsArray(allowed_geographic_area)) {
			ogs_error(
				"OpenAPI_default_unrelated_class_parseFromJSON() failed [allowed_geographic_area]");
			goto end;
		}
	...
	if(privacy_check_related_action) {
		if(!cJSON_IsString(privacy_check_related_action)) {
			ogs_error(
				"OpenAPI_default_unrelated_class_parseFromJSON() failed [privacy_check_related_action]");
			goto end;
		}
	...
	if(code_word_ind) {
		if(!cJSON_IsString(code_word_ind)) {
			ogs_error(
				"OpenAPI_default_unrelated_class_parseFromJSON() failed [code_word_ind]");
			goto end;
		}
	...
	if(code_word_list) {
		...
		if(!cJSON_IsArray(code_word_list)) {
			ogs_error(
				"OpenAPI_default_unrelated_class_parseFromJSON() failed [code_word_list]");
			goto end;
		}
	...
	default_unrelated_class_local_var =
		OpenAPI_default_unrelated_class_create(
...
OpenAPI_default_unrelated_class_t *OpenAPI_default_unrelated_class_copy(
	OpenAPI_default_unrelated_class_t	*dst,
	OpenAPI_default_unrelated_class_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_default_unrelated_class_convertToJSON() failed");
		return NULL;
	}
