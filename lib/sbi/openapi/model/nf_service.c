...
OpenAPI_nf_service_t *OpenAPI_nf_service_create(char *service_instance_id,
                                                char *service_name,
                                                OpenAPI_list_t *versions,
                                                OpenAPI_uri_scheme_e scheme,
                                                OpenAPI_nf_service_status_e
                                                nf_service_status, char *fqdn,
                                                ...
                                                bool is_priority, int priority,
                                                bool is_capacity, int capacity,
                                                bool is_load,
                                                int load, char *load_time_stamp,
                                                char *recovery_time,
                                                ...
                                                per_plmn_oauth2_req_list) {
	OpenAPI_nf_service_t *nf_service_local_var =
		ogs_malloc(sizeof(OpenAPI_nf_service_t));
	...
	nf_service_local_var->default_notification_subscriptions =
		default_notification_subscriptions;
	...
	nf_service_local_var->allowed_operations_per_nf_type =
		allowed_operations_per_nf_type;
	nf_service_local_var->allowed_operations_per_nf_instance =
		allowed_operations_per_nf_instance;
...
void OpenAPI_nf_service_free(OpenAPI_nf_service_t *nf_service) {
	...
	if(nf_service->default_notification_subscriptions) {
		OpenAPI_list_for_each(
			nf_service->default_notification_subscriptions, node) {
	...
	if(nf_service->allowed_operations_per_nf_type) {
		OpenAPI_list_for_each(
			nf_service->allowed_operations_per_nf_type, node) {
	...
	if(nf_service->allowed_operations_per_nf_instance) {
		OpenAPI_list_for_each(
			nf_service->allowed_operations_per_nf_instance, node) {
			OpenAPI_map_t *localKeyValue =
				(OpenAPI_map_t *) node->data;
			ogs_free(localKeyValue->key);
			ogs_free(localKeyValue->value);
		...
		OpenAPI_list_free(
			nf_service->allowed_operations_per_nf_instance);
	...
	if(nf_service->nf_service_set_id_list) {
		OpenAPI_list_for_each(nf_service->nf_service_set_id_list,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(nf_service->supported_vendor_specific_features) {
		OpenAPI_list_for_each(
			nf_service->supported_vendor_specific_features, node) {
			OpenAPI_map_t *localKeyValue =
				(OpenAPI_map_t *) node->data;
			ogs_free(localKeyValue->key);
			ogs_free(localKeyValue->value);
		...
		OpenAPI_list_free(
			nf_service->supported_vendor_specific_features);
...
cJSON *OpenAPI_nf_service_convertToJSON(OpenAPI_nf_service_t *nf_service) {
	...
	if(nf_service == NULL) {
		ogs_error(
			"OpenAPI_nf_service_convertToJSON() failed [NFService]");
		return NULL;
	}
	...
	if(!nf_service->service_instance_id) {
		ogs_error(
			"OpenAPI_nf_service_convertToJSON() failed [service_instance_id]");
		return NULL;
	}
	...
	if(!nf_service->service_name) {
		ogs_error(
			"OpenAPI_nf_service_convertToJSON() failed [service_name]");
		return NULL;
	}
	...
	if(!nf_service->versions) {
		ogs_error(
			"OpenAPI_nf_service_convertToJSON() failed [versions]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(nf_service->versions, node) {
		cJSON *itemLocal =
			OpenAPI_nf_service_version_convertToJSON(node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [versions]");
			goto end;
	...
	if(cJSON_AddStringToObject(item, "scheme",
	                           OpenAPI_uri_scheme_ToString(
					   nf_service->scheme)) == NULL)
	{
		ogs_error("OpenAPI_nf_service_convertToJSON() failed [scheme]");
		goto end;
	}
	...
	if(nf_service->nf_service_status == OpenAPI_nf_service_status_NULL) {
		ogs_error(
			"OpenAPI_nf_service_convertToJSON() failed [nf_service_status]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "nfServiceStatus",
	                           OpenAPI_nf_service_status_ToString(
	...
	if(nf_service->fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [fqdn]");
			goto end;
		}
	}
	...
	if(nf_service->inter_plmn_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [inter_plmn_fqdn]");
			goto end;
		}
	}

	if(nf_service->ip_end_points) {
		cJSON *ip_end_pointsList = cJSON_AddArrayToObject(item,
		                                                  "ipEndPoints");
		if(ip_end_pointsList == NULL) {
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [ip_end_points]");
			goto end;
		}
		OpenAPI_list_for_each(nf_service->ip_end_points, node) {
			cJSON *itemLocal =
				OpenAPI_ip_end_point_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_nf_service_convertToJSON() failed [ip_end_points]");
				goto end;
	...
	if(nf_service->api_prefix) {
		if(cJSON_AddStringToObject(item, "apiPrefix",
		                           nf_service->api_prefix) == NULL)
		{
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [api_prefix]");
			goto end;
		}
	}
	...
	if(nf_service->default_notification_subscriptions) {
		...
		if(default_notification_subscriptionsList == NULL) {
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [default_notification_subscriptions]");
			goto end;
		}
		OpenAPI_list_for_each(
			nf_service->default_notification_subscriptions, node) {
	...
	if(nf_service->allowed_plmns) {
		...
		if(allowed_plmnsList == NULL) {
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [allowed_plmns]");
			goto end;
		}
		OpenAPI_list_for_each(nf_service->allowed_plmns, node) {
			cJSON *itemLocal =
	...
	if(nf_service->allowed_snpns) {
		...
		if(allowed_snpnsList == NULL) {
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [allowed_snpns]");
			goto end;
		}
		OpenAPI_list_for_each(nf_service->allowed_snpns, node) {
			cJSON *itemLocal =
	...
	if(nf_service->allowed_nf_types != OpenAPI_nf_type_NULL) {
		...
		if(allowed_nf_typesList == NULL) {
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [allowed_nf_types]");
			goto end;
		}
	...
	if(nf_service->allowed_nf_domains) {
		...
		if(allowed_nf_domainsList == NULL) {
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [allowed_nf_domains]");
			goto end;
		}
		OpenAPI_list_for_each(nf_service->allowed_nf_domains, node) {
	...
	if(nf_service->allowed_nssais) {
		...
		if(allowed_nssaisList == NULL) {
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [allowed_nssais]");
			goto end;
		}
		OpenAPI_list_for_each(nf_service->allowed_nssais, node) {
			cJSON *itemLocal =
	...
	if(nf_service->allowed_operations_per_nf_type) {
		cJSON *allowed_operations_per_nf_type =
			cJSON_AddObjectToObject(item,
			                        "allowedOperationsPerNfType");
		if(allowed_operations_per_nf_type == NULL) {
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [allowed_operations_per_nf_type]");
			goto end;
		}
		cJSON *localMapObject = allowed_operations_per_nf_type;
		if(nf_service->allowed_operations_per_nf_type) {
			OpenAPI_list_for_each(
				nf_service->allowed_operations_per_nf_type,
				node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
	...
	if(nf_service->allowed_operations_per_nf_instance) {
		cJSON *allowed_operations_per_nf_instance =
			cJSON_AddObjectToObject(item,
			                        "allowedOperationsPerNfInstance");
		if(allowed_operations_per_nf_instance == NULL) {
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [allowed_operations_per_nf_instance]");
			goto end;
		}
	...
	if(nf_service->is_priority) {
		if(cJSON_AddNumberToObject(item, "priority",
		                           nf_service->priority) == NULL)
		{
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [priority]");
			goto end;
		}
	}
	...
	if(nf_service->is_capacity) {
		if(cJSON_AddNumberToObject(item, "capacity",
		                           nf_service->capacity) == NULL)
		{
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [capacity]");
			goto end;
		}
	}
	...
	if(nf_service->is_load) {
		if(cJSON_AddNumberToObject(item, "load",
		                           nf_service->load) == NULL)
		{
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [load]");
			goto end;
		}
	}
	...
	if(nf_service->load_time_stamp) {
		...
		{
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [load_time_stamp]");
			goto end;
		}
	}
	...
	if(nf_service->recovery_time) {
		...
		{
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [recovery_time]");
			goto end;
		}
	}
	...
	if(nf_service->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [supported_features]");
			goto end;
		}
	}

	if(nf_service->nf_service_set_id_list) {
		cJSON *nf_service_set_id_listList = cJSON_AddArrayToObject(item,
		                                                           "nfServiceSetIdList");
		if(nf_service_set_id_listList == NULL) {
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [nf_service_set_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(nf_service->nf_service_set_id_list,
		                      node) {
			if(cJSON_AddStringToObject(nf_service_set_id_listList,
			                           "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
	...
	if(nf_service->s_nssais) {
		...
		if(s_nssaisList == NULL) {
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [s_nssais]");
			goto end;
		}
		OpenAPI_list_for_each(nf_service->s_nssais, node) {
			cJSON *itemLocal =
	...
	if(nf_service->per_plmn_snssai_list) {
		...
		if(per_plmn_snssai_listList == NULL) {
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [per_plmn_snssai_list]");
			goto end;
		}
		OpenAPI_list_for_each(nf_service->per_plmn_snssai_list, node) {
			cJSON *itemLocal =
	...
	if(nf_service->vendor_id) {
		...
		{
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [vendor_id]");
			goto end;
		}
	}
	...
	if(nf_service->supported_vendor_specific_features) {
		...
		if(supported_vendor_specific_features == NULL) {
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [supported_vendor_specific_features]");
			goto end;
		}
		...
		if(nf_service->supported_vendor_specific_features) {
			OpenAPI_list_for_each(
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_nf_service_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(nf_service->is_oauth2_required) {
		if(cJSON_AddBoolToObject(item, "oauth2Required",
		                         nf_service->oauth2_required) == NULL)
		{
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [oauth2_required]");
			goto end;
		}
	}

	if(nf_service->per_plmn_oauth2_req_list) {
		cJSON *per_plmn_oauth2_req_list_local_JSON =
			OpenAPI_plmn_oauth2_convertToJSON(
				nf_service->per_plmn_oauth2_req_list);
		if(per_plmn_oauth2_req_list_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [per_plmn_oauth2_req_list]");
			goto end;
		}
		cJSON_AddItemToObject(item, "perPlmnOauth2ReqList",
		                      per_plmn_oauth2_req_list_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_nf_service_convertToJSON() failed [per_plmn_oauth2_req_list]");
			goto end;
...
OpenAPI_nf_service_t *OpenAPI_nf_service_parseFromJSON(cJSON *nf_serviceJSON) {
	...
	service_instance_id = cJSON_GetObjectItemCaseSensitive(nf_serviceJSON,
	                                                       "serviceInstanceId");
	if(!service_instance_id) {
		ogs_error(
			"OpenAPI_nf_service_parseFromJSON() failed [service_instance_id]");
		goto end;
	}
	...
	service_name = cJSON_GetObjectItemCaseSensitive(nf_serviceJSON,
	                                                "serviceName");
	if(!service_name) {
		ogs_error(
			"OpenAPI_nf_service_parseFromJSON() failed [service_name]");
		goto end;
	}
	...
	if(!versions) {
		ogs_error(
			"OpenAPI_nf_service_parseFromJSON() failed [versions]");
		goto end;
	}
	...
	cJSON_ArrayForEach(versions_local, versions) {
		...
		OpenAPI_nf_service_version_t *versionsItem =
			OpenAPI_nf_service_version_parseFromJSON(
				versions_local);
		if(!versionsItem) {
	...
	nf_service_status = cJSON_GetObjectItemCaseSensitive(nf_serviceJSON,
	                                                     "nfServiceStatus");
	if(!nf_service_status) {
		ogs_error(
			"OpenAPI_nf_service_parseFromJSON() failed [nf_service_status]");
		goto end;
	}
	...
	nf_service_statusVariable =
		OpenAPI_nf_service_status_FromString(
			nf_service_status->valuestring);
	...
	if(fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [fqdn]");
			goto end;
		}
	}

	inter_plmn_fqdn = cJSON_GetObjectItemCaseSensitive(nf_serviceJSON,
	                                                   "interPlmnFqdn");
	if(inter_plmn_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [inter_plmn_fqdn]");
			goto end;
		}
	}

	ip_end_points = cJSON_GetObjectItemCaseSensitive(nf_serviceJSON,
	                                                 "ipEndPoints");
	if(ip_end_points) {
		cJSON *ip_end_points_local = NULL;
		if(!cJSON_IsArray(ip_end_points)) {
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [ip_end_points]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ip_end_points_local, ip_end_points) {
			...
			OpenAPI_ip_end_point_t *ip_end_pointsItem =
	...
	api_prefix = cJSON_GetObjectItemCaseSensitive(nf_serviceJSON,
	                                              "apiPrefix");
	if(api_prefix) {
		if(!cJSON_IsString(api_prefix) &&
		   !cJSON_IsNull(api_prefix))
		{
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [api_prefix]");
			goto end;
		}
	}

	default_notification_subscriptions =
		cJSON_GetObjectItemCaseSensitive(nf_serviceJSON,
		                                 "defaultNotificationSubscriptions");
	if(default_notification_subscriptions) {
		...
		if(!cJSON_IsArray(default_notification_subscriptions)) {
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [default_notification_subscriptions]");
			goto end;
		}
	...
	allowed_plmns = cJSON_GetObjectItemCaseSensitive(nf_serviceJSON,
	                                                 "allowedPlmns");
	if(allowed_plmns) {
		cJSON *allowed_plmns_local = NULL;
		if(!cJSON_IsArray(allowed_plmns)) {
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [allowed_plmns]");
			goto end;
		}
	...
	allowed_snpns = cJSON_GetObjectItemCaseSensitive(nf_serviceJSON,
	                                                 "allowedSnpns");
	if(allowed_snpns) {
		cJSON *allowed_snpns_local = NULL;
		if(!cJSON_IsArray(allowed_snpns)) {
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [allowed_snpns]");
			goto end;
		}
	...
	allowed_nf_types = cJSON_GetObjectItemCaseSensitive(nf_serviceJSON,
	                                                    "allowedNfTypes");
	if(allowed_nf_types) {
		cJSON *allowed_nf_types_local = NULL;
		if(!cJSON_IsArray(allowed_nf_types)) {
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [allowed_nf_types]");
			goto end;
		}
		...
		if(allowed_nf_typesList->count == 0) {
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed: Expected allowed_nf_typesList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}

	allowed_nf_domains = cJSON_GetObjectItemCaseSensitive(nf_serviceJSON,
	                                                      "allowedNfDomains");
	if(allowed_nf_domains) {
		cJSON *allowed_nf_domains_local = NULL;
		if(!cJSON_IsArray(allowed_nf_domains)) {
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [allowed_nf_domains]");
			goto end;
		}
	...
	allowed_nssais = cJSON_GetObjectItemCaseSensitive(nf_serviceJSON,
	                                                  "allowedNssais");
	if(allowed_nssais) {
		cJSON *allowed_nssais_local = NULL;
		if(!cJSON_IsArray(allowed_nssais)) {
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [allowed_nssais]");
			goto end;
		}
	...
	if(allowed_operations_per_nf_type) {
		...
		if(!cJSON_IsObject(allowed_operations_per_nf_type) &&
		   !cJSON_IsNull(allowed_operations_per_nf_type))
		{
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [allowed_operations_per_nf_type]");
			goto end;
		}
		if(cJSON_IsObject(allowed_operations_per_nf_type)) {
			allowed_operations_per_nf_typeList =
				OpenAPI_list_create();
			OpenAPI_map_t *localMapKeyPair = NULL;
			cJSON_ArrayForEach(
				allowed_operations_per_nf_type_local_map,
				allowed_operations_per_nf_type) {
	...
	if(allowed_operations_per_nf_instance) {
		...
		if(!cJSON_IsObject(allowed_operations_per_nf_instance) &&
		   !cJSON_IsNull(allowed_operations_per_nf_instance))
		{
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [allowed_operations_per_nf_instance]");
			goto end;
		}
		if(cJSON_IsObject(allowed_operations_per_nf_instance)) {
			allowed_operations_per_nf_instanceList =
				OpenAPI_list_create();
			OpenAPI_map_t *localMapKeyPair = NULL;
			cJSON_ArrayForEach(
				allowed_operations_per_nf_instance_local_map,
				allowed_operations_per_nf_instance) {
				cJSON *localMapObject =
					allowed_operations_per_nf_instance_local_map;
				double *localDouble = NULL;
	...
	if(priority) {
		if(!cJSON_IsNumber(priority)) {
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [priority]");
			goto end;
		}
	}
	...
	if(capacity) {
		if(!cJSON_IsNumber(capacity)) {
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [capacity]");
			goto end;
		}
	}
	...
	if(load) {
		if(!cJSON_IsNumber(load)) {
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [load]");
			goto end;
		}
	}

	load_time_stamp = cJSON_GetObjectItemCaseSensitive(nf_serviceJSON,
	                                                   "loadTimeStamp");
	if(load_time_stamp) {
		...
		{
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [load_time_stamp]");
			goto end;
		}
	}

	recovery_time = cJSON_GetObjectItemCaseSensitive(nf_serviceJSON,
	                                                 "recoveryTime");
	if(recovery_time) {
		...
		{
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [recovery_time]");
			goto end;
		}
	}

	supported_features = cJSON_GetObjectItemCaseSensitive(nf_serviceJSON,
	                                                      "supportedFeatures");
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(nf_service_set_id_list) {
		...
		if(!cJSON_IsArray(nf_service_set_id_list)) {
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [nf_service_set_id_list]");
			goto end;
		}

		nf_service_set_id_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(nf_service_set_id_list_local,
		                   nf_service_set_id_list) {
			double *localDouble = NULL;
			int *localInt = NULL;
			...
			OpenAPI_list_add(nf_service_set_id_listList,
			                 ogs_strdup(
						 nf_service_set_id_list_local->
						 valuestring));
		}
	}
	...
	if(s_nssais) {
		...
		if(!cJSON_IsArray(s_nssais)) {
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [s_nssais]");
			goto end;
		}
	...
	per_plmn_snssai_list = cJSON_GetObjectItemCaseSensitive(nf_serviceJSON,
	                                                        "perPlmnSnssaiList");
	if(per_plmn_snssai_list) {
		cJSON *per_plmn_snssai_list_local = NULL;
		if(!cJSON_IsArray(per_plmn_snssai_list)) {
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [per_plmn_snssai_list]");
			goto end;
		}
	...
	vendor_id = cJSON_GetObjectItemCaseSensitive(nf_serviceJSON,
	                                             "vendorId");
	if(vendor_id) {
		...
		{
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [vendor_id]");
			goto end;
		}
	}
	...
	if(supported_vendor_specific_features) {
		...
		{
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [supported_vendor_specific_features]");
			goto end;
		}
		if(cJSON_IsObject(supported_vendor_specific_features)) {
			...
			cJSON_ArrayForEach(
				...
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_nf_service_parseFromJSON() failed [inner]");
					goto end;
				}
				OpenAPI_list_add(
					supported_vendor_specific_featuresList,
	...
	oauth2_required = cJSON_GetObjectItemCaseSensitive(nf_serviceJSON,
	                                                   "oauth2Required");
	if(oauth2_required) {
		if(!cJSON_IsBool(oauth2_required)) {
			ogs_error(
				"OpenAPI_nf_service_parseFromJSON() failed [oauth2_required]");
			goto end;
		}
	}
	...
	if(per_plmn_oauth2_req_list) {
		per_plmn_oauth2_req_list_local_nonprim =
			OpenAPI_plmn_oauth2_parseFromJSON(
				per_plmn_oauth2_req_list);
		if(!per_plmn_oauth2_req_list_local_nonprim) {
			ogs_error(
				"OpenAPI_plmn_oauth2_parseFromJSON failed [per_plmn_oauth2_req_list]");
			goto end;
		}
	}
	...
	nf_service_local_var = OpenAPI_nf_service_create(
		...
		api_prefix &&
		!cJSON_IsNull(api_prefix) ?
		ogs_strdup(api_prefix->valuestring) : NULL,
		default_notification_subscriptions ?
	...
	if(allowed_operations_per_nf_typeList) {
		OpenAPI_list_for_each(allowed_operations_per_nf_typeList,
		                      node) {
			OpenAPI_map_t *localKeyValue =
				(OpenAPI_map_t *) node->data;
			ogs_free(localKeyValue->key);
	...
	if(allowed_operations_per_nf_instanceList) {
		OpenAPI_list_for_each(allowed_operations_per_nf_instanceList,
		                      node) {
			OpenAPI_map_t *localKeyValue =
				(OpenAPI_map_t *) node->data;
			ogs_free(localKeyValue->key);
...
OpenAPI_nf_service_t *OpenAPI_nf_service_copy(OpenAPI_nf_service_t	*dst,
                                              OpenAPI_nf_service_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
