...
OpenAPI_n1_n2_message_transfer_rsp_data_create(
	...
	OpenAPI_n1_n2_message_transfer_rsp_data_t *
		...
		ogs_malloc(sizeof(OpenAPI_n1_n2_message_transfer_rsp_data_t));
...
cJSON *OpenAPI_n1_n2_message_transfer_rsp_data_convertToJSON(
	...
	if(n1_n2_message_transfer_rsp_data == NULL) {
		ogs_error(
			"OpenAPI_n1_n2_message_transfer_rsp_data_convertToJSON() failed [N1N2MessageTransferRspData]");
		return NULL;
	}
	...
	{
		ogs_error(
			"OpenAPI_n1_n2_message_transfer_rsp_data_convertToJSON() failed [cause]");
		return NULL;
	}
	...
	if(n1_n2_message_transfer_rsp_data->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_rsp_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
...
OpenAPI_n1_n2_message_transfer_rsp_data_parseFromJSON(
	...
	if(!cause) {
		ogs_error(
			"OpenAPI_n1_n2_message_transfer_rsp_data_parseFromJSON() failed [cause]");
		goto end;
	}
	...
	supported_features =
		cJSON_GetObjectItemCaseSensitive(
			n1_n2_message_transfer_rsp_dataJSON,
			"supportedFeatures");
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_rsp_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
...
OpenAPI_n1_n2_message_transfer_rsp_data_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_n1_n2_message_transfer_rsp_data_convertToJSON() failed");
		return NULL;
	}
