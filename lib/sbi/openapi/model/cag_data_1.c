...
OpenAPI_cag_data_1_t *OpenAPI_cag_data_1_create(OpenAPI_list_t *cag_infos,
                                                char		*
                                                provisioning_time) {
	OpenAPI_cag_data_1_t *cag_data_1_local_var =
		ogs_malloc(sizeof(OpenAPI_cag_data_1_t));
...
cJSON *OpenAPI_cag_data_1_convertToJSON(OpenAPI_cag_data_1_t *cag_data_1) {
	...
	if(cag_data_1 == NULL) {
		ogs_error(
			"OpenAPI_cag_data_1_convertToJSON() failed [CagData_1]");
		return NULL;
	}
	...
	if(!cag_data_1->cag_infos) {
		ogs_error(
			"OpenAPI_cag_data_1_convertToJSON() failed [cag_infos]");
		return NULL;
	}
	...
	if(cag_data_1->cag_infos) {
		OpenAPI_list_for_each(cag_data_1->cag_infos, node) {
			...
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_cag_data_1_convertToJSON() failed [inner]");
				goto end;
			}
	...
	if(cag_data_1->provisioning_time) {
		if(cJSON_AddStringToObject(item, "provisioningTime",
		                           cag_data_1->provisioning_time) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_cag_data_1_convertToJSON() failed [provisioning_time]");
			goto end;
		}
	}
...
OpenAPI_cag_data_1_t *OpenAPI_cag_data_1_parseFromJSON(cJSON *cag_data_1JSON) {
	...
	cag_infos = cJSON_GetObjectItemCaseSensitive(cag_data_1JSON,
	                                             "cagInfos");
	if(!cag_infos) {
		ogs_error(
			"OpenAPI_cag_data_1_parseFromJSON() failed [cag_infos]");
		goto end;
	}
		...
		cJSON_ArrayForEach(cag_infos_local_map, cag_infos) {
			...
			if(cJSON_IsObject(localMapObject)) {
				localMapKeyPair = OpenAPI_map_create(
					...
					OpenAPI_cag_info_1_parseFromJSON(
						localMapObject));
				...
				ogs_error(
					"OpenAPI_cag_data_1_parseFromJSON() failed [inner]");
				goto end;
			}
	...
	provisioning_time = cJSON_GetObjectItemCaseSensitive(cag_data_1JSON,
	                                                     "provisioningTime");
	if(provisioning_time) {
		...
		{
			ogs_error(
				"OpenAPI_cag_data_1_parseFromJSON() failed [provisioning_time]");
			goto end;
		}
	}
