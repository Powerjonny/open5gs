...
OpenAPI_usage_mon_data_scope_t *OpenAPI_usage_mon_data_scope_create(
	OpenAPI_snssai_t *snssai, OpenAPI_list_t *dnn) {
	OpenAPI_usage_mon_data_scope_t *usage_mon_data_scope_local_var =
		ogs_malloc(sizeof(OpenAPI_usage_mon_data_scope_t));
...
void OpenAPI_usage_mon_data_scope_free(
	OpenAPI_usage_mon_data_scope_t *usage_mon_data_scope) {
...
cJSON *OpenAPI_usage_mon_data_scope_convertToJSON(
	...
	if(usage_mon_data_scope == NULL) {
		ogs_error(
			"OpenAPI_usage_mon_data_scope_convertToJSON() failed [UsageMonDataScope]");
		return NULL;
	}
	...
	if(!usage_mon_data_scope->snssai) {
		ogs_error(
			"OpenAPI_usage_mon_data_scope_convertToJSON() failed [snssai]");
		return NULL;
	}
	cJSON *snssai_local_JSON =
		OpenAPI_snssai_convertToJSON(usage_mon_data_scope->snssai);
	...
	if(usage_mon_data_scope->dnn) {
		...
		if(dnnList == NULL) {
			ogs_error(
				"OpenAPI_usage_mon_data_scope_convertToJSON() failed [dnn]");
			goto end;
		}
		OpenAPI_list_for_each(usage_mon_data_scope->dnn, node) {
			if(cJSON_AddStringToObject(dnnList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_usage_mon_data_scope_convertToJSON() failed [dnn]");
...
OpenAPI_usage_mon_data_scope_t *OpenAPI_usage_mon_data_scope_parseFromJSON(
	cJSON *usage_mon_data_scopeJSON) {
	...
	snssai = cJSON_GetObjectItemCaseSensitive(usage_mon_data_scopeJSON,
	                                          "snssai");
	if(!snssai) {
		ogs_error(
			"OpenAPI_usage_mon_data_scope_parseFromJSON() failed [snssai]");
		goto end;
	}
	...
	if(dnn) {
		...
		if(!cJSON_IsArray(dnn)) {
			ogs_error(
				"OpenAPI_usage_mon_data_scope_parseFromJSON() failed [dnn]");
			goto end;
		}
		...
		cJSON_ArrayForEach(dnn_local, dnn) {
			...
			OpenAPI_list_add(dnnList,
			                 ogs_strdup(dnn_local->valuestring));
		}
	}
...
OpenAPI_usage_mon_data_scope_t *OpenAPI_usage_mon_data_scope_copy(
	OpenAPI_usage_mon_data_scope_t	*dst,
	OpenAPI_usage_mon_data_scope_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_usage_mon_data_scope_convertToJSON() failed");
		return NULL;
	}
