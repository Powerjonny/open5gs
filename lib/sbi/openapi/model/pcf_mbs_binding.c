...
OpenAPI_pcf_mbs_binding_t *OpenAPI_pcf_mbs_binding_create(
	...
	char *supp_feat) {
	OpenAPI_pcf_mbs_binding_t *pcf_mbs_binding_local_var =
		ogs_malloc(sizeof(OpenAPI_pcf_mbs_binding_t));
...
void OpenAPI_pcf_mbs_binding_free(OpenAPI_pcf_mbs_binding_t *pcf_mbs_binding) {
	...
	if(pcf_mbs_binding->pcf_ip_end_points) {
		OpenAPI_list_for_each(pcf_mbs_binding->pcf_ip_end_points,
		                      node) {
...
cJSON *OpenAPI_pcf_mbs_binding_convertToJSON(
	...
	if(pcf_mbs_binding == NULL) {
		ogs_error(
			"OpenAPI_pcf_mbs_binding_convertToJSON() failed [PcfMbsBinding]");
		return NULL;
	}
	...
	if(!pcf_mbs_binding->mbs_session_id) {
		ogs_error(
			"OpenAPI_pcf_mbs_binding_convertToJSON() failed [mbs_session_id]");
		return NULL;
	}
	cJSON *mbs_session_id_local_JSON =
		OpenAPI_mbs_session_id_convertToJSON(
			pcf_mbs_binding->mbs_session_id);
	...
	if(pcf_mbs_binding->pcf_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_mbs_binding_convertToJSON() failed [pcf_fqdn]");
			goto end;
		}
	}
	...
	if(pcf_mbs_binding->pcf_ip_end_points) {
		...
		if(pcf_ip_end_pointsList == NULL) {
			ogs_error(
				"OpenAPI_pcf_mbs_binding_convertToJSON() failed [pcf_ip_end_points]");
			goto end;
		}
		OpenAPI_list_for_each(pcf_mbs_binding->pcf_ip_end_points,
		                      node) {
	...
	if(pcf_mbs_binding->pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_mbs_binding_convertToJSON() failed [pcf_id]");
			goto end;
		}
	}
	...
	if(pcf_mbs_binding->pcf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_mbs_binding_convertToJSON() failed [pcf_set_id]");
			goto end;
		}
	}
	...
	if(pcf_mbs_binding->bind_level != OpenAPI_binding_level_NULL) {
		if(cJSON_AddStringToObject(item, "bindLevel",
						   ...
						   pcf_mbs_binding->bind_level))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_pcf_mbs_binding_convertToJSON() failed [bind_level]");
			goto end;
		}
	}
	...
	if(pcf_mbs_binding->recovery_time) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_mbs_binding_convertToJSON() failed [recovery_time]");
			goto end;
		}
	}
	...
	if(pcf_mbs_binding->supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_mbs_binding_convertToJSON() failed [supp_feat]");
			goto end;
		}
	}
...
OpenAPI_pcf_mbs_binding_t *OpenAPI_pcf_mbs_binding_parseFromJSON(
	...
	mbs_session_id = cJSON_GetObjectItemCaseSensitive(pcf_mbs_bindingJSON,
	                                                  "mbsSessionId");
	if(!mbs_session_id) {
		ogs_error(
			"OpenAPI_pcf_mbs_binding_parseFromJSON() failed [mbs_session_id]");
		goto end;
	}
	mbs_session_id_local_nonprim =
		OpenAPI_mbs_session_id_parseFromJSON(mbs_session_id);
	if(!mbs_session_id_local_nonprim) {
		ogs_error(
			"OpenAPI_mbs_session_id_parseFromJSON failed [mbs_session_id]");
		goto end;
	}

	pcf_fqdn = cJSON_GetObjectItemCaseSensitive(pcf_mbs_bindingJSON,
	                                            "pcfFqdn");
	if(pcf_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_mbs_binding_parseFromJSON() failed [pcf_fqdn]");
			goto end;
		}
	}

	pcf_ip_end_points =
		cJSON_GetObjectItemCaseSensitive(pcf_mbs_bindingJSON,
		                                 "pcfIpEndPoints");
	if(pcf_ip_end_points) {
		cJSON *pcf_ip_end_points_local = NULL;
		if(!cJSON_IsArray(pcf_ip_end_points)) {
			ogs_error(
				"OpenAPI_pcf_mbs_binding_parseFromJSON() failed [pcf_ip_end_points]");
			goto end;
		}
	...
	if(pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_mbs_binding_parseFromJSON() failed [pcf_id]");
			goto end;
		}
	}

	pcf_set_id = cJSON_GetObjectItemCaseSensitive(pcf_mbs_bindingJSON,
	                                              "pcfSetId");
	if(pcf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_mbs_binding_parseFromJSON() failed [pcf_set_id]");
			goto end;
		}
	}

	bind_level = cJSON_GetObjectItemCaseSensitive(pcf_mbs_bindingJSON,
	                                              "bindLevel");
	if(bind_level) {
		if(!cJSON_IsString(bind_level)) {
			ogs_error(
				"OpenAPI_pcf_mbs_binding_parseFromJSON() failed [bind_level]");
			goto end;
		}
		bind_levelVariable =
	...
	recovery_time = cJSON_GetObjectItemCaseSensitive(pcf_mbs_bindingJSON,
	                                                 "recoveryTime");
	if(recovery_time) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_mbs_binding_parseFromJSON() failed [recovery_time]");
			goto end;
		}
	}

	supp_feat = cJSON_GetObjectItemCaseSensitive(pcf_mbs_bindingJSON,
	                                             "suppFeat");
	if(supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_mbs_binding_parseFromJSON() failed [supp_feat]");
			goto end;
		}
	}
