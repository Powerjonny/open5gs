...
OpenAPI_smsf_registration_t *OpenAPI_smsf_registration_create(
	char *smsf_instance_id, char *smsf_set_id, char *supported_features,
	OpenAPI_plmn_id_t *plmn_id, char *smsf_map_address,
	OpenAPI_network_node_diameter_address_t *smsf_diameter_address,
	char *registration_time, OpenAPI_context_info_t *context_info,
	char *data_restoration_callback_uri, OpenAPI_list_t *reset_ids,
	bool is_smsf_sbi_sup_ind, int smsf_sbi_sup_ind, bool is_udr_restart_ind,
	int udr_restart_ind, char *last_synchronization_time) {
	OpenAPI_smsf_registration_t *smsf_registration_local_var =
		ogs_malloc(sizeof(OpenAPI_smsf_registration_t));
	...
	smsf_registration_local_var->smsf_diameter_address =
		smsf_diameter_address;
	smsf_registration_local_var->registration_time = registration_time;
	smsf_registration_local_var->context_info = context_info;
	smsf_registration_local_var->data_restoration_callback_uri =
	...
	smsf_registration_local_var->last_synchronization_time =
		last_synchronization_time;

	return smsf_registration_local_var;
}

void OpenAPI_smsf_registration_free(
	...
	if(smsf_registration->smsf_diameter_address) {
		OpenAPI_network_node_diameter_address_free(
			smsf_registration->smsf_diameter_address);
...
cJSON *OpenAPI_smsf_registration_convertToJSON(
	...
	if(smsf_registration == NULL) {
		ogs_error(
			"OpenAPI_smsf_registration_convertToJSON() failed [SmsfRegistration]");
		return NULL;
	}
	...
	if(!smsf_registration->smsf_instance_id) {
		ogs_error(
			"OpenAPI_smsf_registration_convertToJSON() failed [smsf_instance_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "smsfInstanceId",
	                           smsf_registration->smsf_instance_id) == NULL)
	{
		ogs_error(
			"OpenAPI_smsf_registration_convertToJSON() failed [smsf_instance_id]");
	...
	if(smsf_registration->smsf_set_id) {
		if(cJSON_AddStringToObject(item, "smsfSetId",
		                           smsf_registration->smsf_set_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_smsf_registration_convertToJSON() failed [smsf_set_id]");
			goto end;
		}
	}
	...
	if(smsf_registration->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_smsf_registration_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(!smsf_registration->plmn_id) {
		ogs_error(
			"OpenAPI_smsf_registration_convertToJSON() failed [plmn_id]");
		return NULL;
	}
	...
	if(smsf_registration->smsf_map_address) {
		if(cJSON_AddStringToObject(item, "smsfMAPAddress",
		                           smsf_registration->smsf_map_address)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_smsf_registration_convertToJSON() failed [smsf_map_address]");
			goto end;
		}
	}

	if(smsf_registration->smsf_diameter_address) {
		cJSON *smsf_diameter_address_local_JSON =
			OpenAPI_network_node_diameter_address_convertToJSON(
				smsf_registration->smsf_diameter_address);
		if(smsf_diameter_address_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_smsf_registration_convertToJSON() failed [smsf_diameter_address]");
			goto end;
		}
		cJSON_AddItemToObject(item, "smsfDiameterAddress",
		                      smsf_diameter_address_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_smsf_registration_convertToJSON() failed [smsf_diameter_address]");
			goto end;
	...
	if(smsf_registration->registration_time) {
		if(cJSON_AddStringToObject(item, "registrationTime",
		                           smsf_registration->registration_time)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_smsf_registration_convertToJSON() failed [registration_time]");
			goto end;
		}
	}
	...
	if(smsf_registration->context_info) {
		cJSON *context_info_local_JSON =
			OpenAPI_context_info_convertToJSON(
				smsf_registration->context_info);
		if(context_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_smsf_registration_convertToJSON() failed [context_info]");
			goto end;
		}
	...
	if(smsf_registration->data_restoration_callback_uri) {
		...
		{
			ogs_error(
				"OpenAPI_smsf_registration_convertToJSON() failed [data_restoration_callback_uri]");
			goto end;
		}
	}
	...
	if(smsf_registration->reset_ids) {
		...
		if(reset_idsList == NULL) {
			ogs_error(
				"OpenAPI_smsf_registration_convertToJSON() failed [reset_ids]");
			goto end;
		}
		OpenAPI_list_for_each(smsf_registration->reset_ids, node) {
	...
	if(smsf_registration->is_smsf_sbi_sup_ind) {
		if(cJSON_AddBoolToObject(item, "smsfSbiSupInd",
		                         smsf_registration->smsf_sbi_sup_ind) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_smsf_registration_convertToJSON() failed [smsf_sbi_sup_ind]");
			goto end;
		}
	}
	...
	if(smsf_registration->is_udr_restart_ind) {
		if(cJSON_AddBoolToObject(item, "udrRestartInd",
		                         smsf_registration->udr_restart_ind) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_smsf_registration_convertToJSON() failed [udr_restart_ind]");
			goto end;
		}
	}
	...
	if(smsf_registration->last_synchronization_time) {
		...
		{
			ogs_error(
				"OpenAPI_smsf_registration_convertToJSON() failed [last_synchronization_time]");
			goto end;
		}
	}
...
OpenAPI_smsf_registration_t *OpenAPI_smsf_registration_parseFromJSON(
	cJSON *smsf_registrationJSON) {
	...
	if(!smsf_instance_id) {
		ogs_error(
			"OpenAPI_smsf_registration_parseFromJSON() failed [smsf_instance_id]");
		goto end;
	}
	...
	smsf_set_id = cJSON_GetObjectItemCaseSensitive(smsf_registrationJSON,
	                                               "smsfSetId");
	if(smsf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_smsf_registration_parseFromJSON() failed [smsf_set_id]");
			goto end;
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_smsf_registration_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}

	plmn_id = cJSON_GetObjectItemCaseSensitive(smsf_registrationJSON,
	                                           "plmnId");
	if(!plmn_id) {
		ogs_error(
			"OpenAPI_smsf_registration_parseFromJSON() failed [plmn_id]");
		goto end;
	}
	...
	if(smsf_map_address) {
		if(!cJSON_IsString(smsf_map_address) &&
		   !cJSON_IsNull(smsf_map_address))
		{
			ogs_error(
				"OpenAPI_smsf_registration_parseFromJSON() failed [smsf_map_address]");
			goto end;
		}
	}
	...
	if(smsf_diameter_address) {
		smsf_diameter_address_local_nonprim =
			OpenAPI_network_node_diameter_address_parseFromJSON(
				smsf_diameter_address);
		if(!smsf_diameter_address_local_nonprim) {
			ogs_error(
				"OpenAPI_network_node_diameter_address_parseFromJSON failed [smsf_diameter_address]");
			goto end;
		}
	}
	...
	if(registration_time) {
		...
		{
			ogs_error(
				"OpenAPI_smsf_registration_parseFromJSON() failed [registration_time]");
			goto end;
		}
	}

	context_info = cJSON_GetObjectItemCaseSensitive(smsf_registrationJSON,
	                                                "contextInfo");
	if(context_info) {
	...
	if(data_restoration_callback_uri) {
		...
		{
			ogs_error(
				"OpenAPI_smsf_registration_parseFromJSON() failed [data_restoration_callback_uri]");
			goto end;
		}
	}

	reset_ids = cJSON_GetObjectItemCaseSensitive(smsf_registrationJSON,
	                                             "resetIds");
	if(reset_ids) {
		cJSON *reset_ids_local = NULL;
		if(!cJSON_IsArray(reset_ids)) {
			ogs_error(
				"OpenAPI_smsf_registration_parseFromJSON() failed [reset_ids]");
			goto end;
		}
	...
	if(smsf_sbi_sup_ind) {
		if(!cJSON_IsBool(smsf_sbi_sup_ind)) {
			ogs_error(
				"OpenAPI_smsf_registration_parseFromJSON() failed [smsf_sbi_sup_ind]");
			goto end;
		}
	}
	...
	if(udr_restart_ind) {
		if(!cJSON_IsBool(udr_restart_ind)) {
			ogs_error(
				"OpenAPI_smsf_registration_parseFromJSON() failed [udr_restart_ind]");
			goto end;
		}
	}
	...
	if(last_synchronization_time) {
		...
		{
			ogs_error(
				"OpenAPI_smsf_registration_parseFromJSON() failed [last_synchronization_time]");
			goto end;
		}
	}
	...
	smsf_registration_local_var = OpenAPI_smsf_registration_create(
		...
		smsf_map_address &&
		!cJSON_IsNull(smsf_map_address) ?
		ogs_strdup(smsf_map_address->valuestring) : NULL,
		smsf_diameter_address ? smsf_diameter_address_local_nonprim :
		NULL,
		registration_time &&
		!cJSON_IsNull(registration_time) ?
...
OpenAPI_smsf_registration_t *OpenAPI_smsf_registration_copy(
	OpenAPI_smsf_registration_t *dst, OpenAPI_smsf_registration_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
