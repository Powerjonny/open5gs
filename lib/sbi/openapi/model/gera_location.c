...
OpenAPI_gera_location_t *OpenAPI_gera_location_create(char *location_number,
...
{
	OpenAPI_gera_location_t *gera_location_local_var =
		ogs_malloc(sizeof(OpenAPI_gera_location_t));
...
cJSON *OpenAPI_gera_location_convertToJSON(
	...
	if(gera_location == NULL) {
		ogs_error(
			"OpenAPI_gera_location_convertToJSON() failed [GeraLocation]");
		return NULL;
	}
	...
	if(gera_location->location_number) {
		...
		{
			ogs_error(
				"OpenAPI_gera_location_convertToJSON() failed [location_number]");
			goto end;
		}
	}

	if(gera_location->cgi) {
		cJSON *cgi_local_JSON =
			OpenAPI_cell_global_id_convertToJSON(
				gera_location->cgi);
		if(cgi_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_gera_location_convertToJSON() failed [cgi]");
			goto end;
		}
	...
	if(gera_location->rai) {
		cJSON *rai_local_JSON =
			OpenAPI_routing_area_id_convertToJSON(
				gera_location->rai);
		if(rai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_gera_location_convertToJSON() failed [rai]");
			goto end;
		}
	...
	if(gera_location->sai) {
		cJSON *sai_local_JSON =
			OpenAPI_service_area_id_convertToJSON(
				gera_location->sai);
		if(sai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_gera_location_convertToJSON() failed [sai]");
			goto end;
		}
	...
	if(gera_location->lai) {
		cJSON *lai_local_JSON =
			OpenAPI_location_area_id_convertToJSON(
				gera_location->lai);
		if(lai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_gera_location_convertToJSON() failed [lai]");
			goto end;
		}
	...
	if(gera_location->vlr_number) {
		...
		{
			ogs_error(
				"OpenAPI_gera_location_convertToJSON() failed [vlr_number]");
			goto end;
		}
	}
	...
	if(gera_location->msc_number) {
		...
		{
			ogs_error(
				"OpenAPI_gera_location_convertToJSON() failed [msc_number]");
			goto end;
		}
	}
	...
	if(gera_location->is_age_of_location_information) {
		...
		{
			ogs_error(
				"OpenAPI_gera_location_convertToJSON() failed [age_of_location_information]");
			goto end;
		}
	}
	...
	if(gera_location->ue_location_timestamp) {
		...
		{
			ogs_error(
				"OpenAPI_gera_location_convertToJSON() failed [ue_location_timestamp]");
			goto end;
		}
	}
	...
	if(gera_location->geographical_information) {
		...
		{
			ogs_error(
				"OpenAPI_gera_location_convertToJSON() failed [geographical_information]");
			goto end;
		}
	}
	...
	if(gera_location->geodetic_information) {
		...
		{
			ogs_error(
				"OpenAPI_gera_location_convertToJSON() failed [geodetic_information]");
			goto end;
		}
	}
...
OpenAPI_gera_location_t *OpenAPI_gera_location_parseFromJSON(
	...
	if(location_number) {
		...
		{
			ogs_error(
				"OpenAPI_gera_location_parseFromJSON() failed [location_number]");
			goto end;
		}
	}
	...
	if(cgi) {
		...
		if(!cgi_local_nonprim) {
			ogs_error(
				"OpenAPI_cell_global_id_parseFromJSON failed [cgi]");
			goto end;
		}
	}
	...
	if(rai) {
		...
		if(!rai_local_nonprim) {
			ogs_error(
				"OpenAPI_routing_area_id_parseFromJSON failed [rai]");
			goto end;
		}
	}
	...
	if(sai) {
		...
		if(!sai_local_nonprim) {
			ogs_error(
				"OpenAPI_service_area_id_parseFromJSON failed [sai]");
			goto end;
		}
	}
	...
	if(lai) {
		...
		if(!lai_local_nonprim) {
			ogs_error(
				"OpenAPI_location_area_id_parseFromJSON failed [lai]");
			goto end;
		}
	}
	...
	if(vlr_number) {
		...
		{
			ogs_error(
				"OpenAPI_gera_location_parseFromJSON() failed [vlr_number]");
			goto end;
		}
	}
	...
	if(msc_number) {
		...
		{
			ogs_error(
				"OpenAPI_gera_location_parseFromJSON() failed [msc_number]");
			goto end;
		}
	}

	age_of_location_information =
		cJSON_GetObjectItemCaseSensitive(gera_locationJSON,
		                                 "ageOfLocationInformation");
	if(age_of_location_information) {
		if(!cJSON_IsNumber(age_of_location_information)) {
			ogs_error(
				"OpenAPI_gera_location_parseFromJSON() failed [age_of_location_information]");
			goto end;
		}
	}
	...
	if(ue_location_timestamp) {
		...
		{
			ogs_error(
				"OpenAPI_gera_location_parseFromJSON() failed [ue_location_timestamp]");
			goto end;
		}
	}
	...
	if(geographical_information) {
		...
		{
			ogs_error(
				"OpenAPI_gera_location_parseFromJSON() failed [geographical_information]");
			goto end;
		}
	}
	...
	if(geodetic_information) {
		...
		{
			ogs_error(
				"OpenAPI_gera_location_parseFromJSON() failed [geodetic_information]");
			goto end;
		}
	}
	...
	gera_location_local_var = OpenAPI_gera_location_create(
		location_number &&
		!cJSON_IsNull(location_number) ?
		ogs_strdup(location_number->valuestring) : NULL,
		...
		vlr_number &&
		!cJSON_IsNull(vlr_number) ?
		ogs_strdup(vlr_number->valuestring) : NULL,
		msc_number &&
		!cJSON_IsNull(msc_number) ?
		ogs_strdup(msc_number->valuestring) : NULL,
		age_of_location_information ? true : false,
		age_of_location_information ?
		age_of_location_information->valuedouble : 0,
		ue_location_timestamp &&
		!cJSON_IsNull(ue_location_timestamp) ?
		ogs_strdup(ue_location_timestamp->valuestring) : NULL,
		geographical_information &&
		!cJSON_IsNull(geographical_information) ?
		ogs_strdup(geographical_information->valuestring) : NULL,
		geodetic_information &&
		!cJSON_IsNull(geodetic_information) ?
		ogs_strdup(geodetic_information->valuestring) : NULL
		);
...
OpenAPI_gera_location_t *OpenAPI_gera_location_copy(
	OpenAPI_gera_location_t *dst, OpenAPI_gera_location_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
