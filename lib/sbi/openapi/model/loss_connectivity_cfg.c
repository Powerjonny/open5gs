...
OpenAPI_loss_connectivity_cfg_t *OpenAPI_loss_connectivity_cfg_create(
	...
	OpenAPI_loss_connectivity_cfg_t *loss_connectivity_cfg_local_var =
		ogs_malloc(sizeof(OpenAPI_loss_connectivity_cfg_t));
	ogs_assert(loss_connectivity_cfg_local_var);

	loss_connectivity_cfg_local_var->is_max_detection_time =
		is_max_detection_time;
	loss_connectivity_cfg_local_var->max_detection_time =
		max_detection_time;
...
cJSON *OpenAPI_loss_connectivity_cfg_convertToJSON(
	...
	if(loss_connectivity_cfg == NULL) {
		ogs_error(
			"OpenAPI_loss_connectivity_cfg_convertToJSON() failed [LossConnectivityCfg]");
		return NULL;
	}
	...
	if(loss_connectivity_cfg->is_max_detection_time) {
		...
		{
			ogs_error(
				"OpenAPI_loss_connectivity_cfg_convertToJSON() failed [max_detection_time]");
			goto end;
		}
	}
...
OpenAPI_loss_connectivity_cfg_t *OpenAPI_loss_connectivity_cfg_parseFromJSON(
	...
	max_detection_time =
		cJSON_GetObjectItemCaseSensitive(loss_connectivity_cfgJSON,
		                                 "maxDetectionTime");
	if(max_detection_time) {
		if(!cJSON_IsNumber(max_detection_time)) {
			ogs_error(
				"OpenAPI_loss_connectivity_cfg_parseFromJSON() failed [max_detection_time]");
			goto end;
		}
	}
...
OpenAPI_loss_connectivity_cfg_t *OpenAPI_loss_connectivity_cfg_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_loss_connectivity_cfg_convertToJSON() failed");
		return NULL;
	}
