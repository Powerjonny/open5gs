...
char * OpenAPI_qos_flow_usage_ToString(
	OpenAPI_qos_flow_usage_e qos_flow_usage) {
	const char *qos_flow_usageArray[] = { "NULL", "GENERAL", "IMS_SIG" };
	size_t sizeofArray = sizeof(qos_flow_usageArray) /
	                     sizeof(qos_flow_usageArray[0]);
...
OpenAPI_qos_flow_usage_e OpenAPI_qos_flow_usage_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(qos_flow_usage,
		          qos_flow_usageArray[stringToReturn]) == 0)
		{
