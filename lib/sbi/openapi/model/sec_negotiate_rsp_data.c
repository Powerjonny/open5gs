...
OpenAPI_sec_negotiate_rsp_data_t *OpenAPI_sec_negotiate_rsp_data_create(
	char *sender, OpenAPI_security_capability_e selected_sec_capability,
	bool is__3_gpp_sbi_target_api_root_supported,
	int _3_gpp_sbi_target_api_root_supported, OpenAPI_list_t *plmn_id_list,
	OpenAPI_list_t *snpn_id_list, OpenAPI_list_t *allowed_usage_purpose,
	OpenAPI_list_t *rejected_usage_purpose, char *supported_features) {
	OpenAPI_sec_negotiate_rsp_data_t *sec_negotiate_rsp_data_local_var =
		ogs_malloc(sizeof(OpenAPI_sec_negotiate_rsp_data_t));
	ogs_assert(sec_negotiate_rsp_data_local_var);

	sec_negotiate_rsp_data_local_var->sender = sender;
	sec_negotiate_rsp_data_local_var->selected_sec_capability =
		selected_sec_capability;
	sec_negotiate_rsp_data_local_var->
	is__3_gpp_sbi_target_api_root_supported =
		is__3_gpp_sbi_target_api_root_supported;
	sec_negotiate_rsp_data_local_var->_3_gpp_sbi_target_api_root_supported =
	...
	sec_negotiate_rsp_data_local_var->allowed_usage_purpose =
		allowed_usage_purpose;
	sec_negotiate_rsp_data_local_var->rejected_usage_purpose =
		rejected_usage_purpose;
	sec_negotiate_rsp_data_local_var->supported_features =
		supported_features;

	return sec_negotiate_rsp_data_local_var;
}

void OpenAPI_sec_negotiate_rsp_data_free(
	OpenAPI_sec_negotiate_rsp_data_t *sec_negotiate_rsp_data) {
	...
	if(sec_negotiate_rsp_data->plmn_id_list) {
		OpenAPI_list_for_each(sec_negotiate_rsp_data->plmn_id_list,
		                      node) {
	...
	if(sec_negotiate_rsp_data->snpn_id_list) {
		OpenAPI_list_for_each(sec_negotiate_rsp_data->snpn_id_list,
		                      node) {
	...
	if(sec_negotiate_rsp_data->allowed_usage_purpose) {
		OpenAPI_list_for_each(
			sec_negotiate_rsp_data->allowed_usage_purpose, node) {
			OpenAPI_intended_n32_purpose_free(node->data);
		}
		OpenAPI_list_free(
			sec_negotiate_rsp_data->allowed_usage_purpose);
		sec_negotiate_rsp_data->allowed_usage_purpose = NULL;
	}
	if(sec_negotiate_rsp_data->rejected_usage_purpose) {
		OpenAPI_list_for_each(
			sec_negotiate_rsp_data->rejected_usage_purpose, node) {
			OpenAPI_intended_n32_purpose_free(node->data);
		}
		OpenAPI_list_free(
...
cJSON *OpenAPI_sec_negotiate_rsp_data_convertToJSON(
	OpenAPI_sec_negotiate_rsp_data_t *sec_negotiate_rsp_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(sec_negotiate_rsp_data == NULL) {
		ogs_error(
			"OpenAPI_sec_negotiate_rsp_data_convertToJSON() failed [SecNegotiateRspData]");
		return NULL;
	}
	...
	if(!sec_negotiate_rsp_data->sender) {
		ogs_error(
			"OpenAPI_sec_negotiate_rsp_data_convertToJSON() failed [sender]");
		return NULL;
	}
	...
	if(sec_negotiate_rsp_data->selected_sec_capability ==
	   OpenAPI_security_capability_NULL)
	{
		ogs_error(
			"OpenAPI_sec_negotiate_rsp_data_convertToJSON() failed [selected_sec_capability]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "selectedSecCapability",
	...
	if(sec_negotiate_rsp_data->is__3_gpp_sbi_target_api_root_supported) {
		...
		{
			ogs_error(
				"OpenAPI_sec_negotiate_rsp_data_convertToJSON() failed [_3_gpp_sbi_target_api_root_supported]");
			goto end;
		}
	}
	...
	if(sec_negotiate_rsp_data->plmn_id_list) {
		...
		if(plmn_id_listList == NULL) {
			ogs_error(
				"OpenAPI_sec_negotiate_rsp_data_convertToJSON() failed [plmn_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(sec_negotiate_rsp_data->plmn_id_list,
		                      node) {
	...
	if(sec_negotiate_rsp_data->snpn_id_list) {
		...
		if(snpn_id_listList == NULL) {
			ogs_error(
				"OpenAPI_sec_negotiate_rsp_data_convertToJSON() failed [snpn_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(sec_negotiate_rsp_data->snpn_id_list,
		                      node) {
	...
	if(sec_negotiate_rsp_data->allowed_usage_purpose) {
		cJSON *allowed_usage_purposeList = cJSON_AddArrayToObject(item,
		                                                          "allowedUsagePurpose");
		if(allowed_usage_purposeList == NULL) {
			ogs_error(
				"OpenAPI_sec_negotiate_rsp_data_convertToJSON() failed [allowed_usage_purpose]");
			goto end;
		}
		OpenAPI_list_for_each(
			sec_negotiate_rsp_data->allowed_usage_purpose, node) {
			...
			cJSON_AddItemToArray(allowed_usage_purposeList,
			                     itemLocal);
		}
	}

	if(sec_negotiate_rsp_data->rejected_usage_purpose) {
		cJSON *rejected_usage_purposeList = cJSON_AddArrayToObject(item,
		                                                           "rejectedUsagePurpose");
		if(rejected_usage_purposeList == NULL) {
			ogs_error(
				"OpenAPI_sec_negotiate_rsp_data_convertToJSON() failed [rejected_usage_purpose]");
			goto end;
		}
		OpenAPI_list_for_each(
			sec_negotiate_rsp_data->rejected_usage_purpose, node) {
			...
			cJSON_AddItemToArray(rejected_usage_purposeList,
			                     itemLocal);
		}
	}
	...
	if(sec_negotiate_rsp_data->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_sec_negotiate_rsp_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
...
OpenAPI_sec_negotiate_rsp_data_t *OpenAPI_sec_negotiate_rsp_data_parseFromJSON(
	cJSON *sec_negotiate_rsp_dataJSON) {
	OpenAPI_sec_negotiate_rsp_data_t *sec_negotiate_rsp_data_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *sender = NULL;
	...
	sender = cJSON_GetObjectItemCaseSensitive(sec_negotiate_rsp_dataJSON,
	                                          "sender");
	if(!sender) {
		ogs_error(
			"OpenAPI_sec_negotiate_rsp_data_parseFromJSON() failed [sender]");
		goto end;
	}
	...
	selected_sec_capability =
		cJSON_GetObjectItemCaseSensitive(sec_negotiate_rsp_dataJSON,
		                                 "selectedSecCapability");
	if(!selected_sec_capability) {
		ogs_error(
			"OpenAPI_sec_negotiate_rsp_data_parseFromJSON() failed [selected_sec_capability]");
		goto end;
	}
	...
	selected_sec_capabilityVariable =
		OpenAPI_security_capability_FromString(
			selected_sec_capability->valuestring);
	...
	if(_3_gpp_sbi_target_api_root_supported) {
		if(!cJSON_IsBool(_3_gpp_sbi_target_api_root_supported)) {
			ogs_error(
				"OpenAPI_sec_negotiate_rsp_data_parseFromJSON() failed [_3_gpp_sbi_target_api_root_supported]");
			goto end;
		}
	}
	...
	if(plmn_id_list) {
		...
		if(!cJSON_IsArray(plmn_id_list)) {
			ogs_error(
				"OpenAPI_sec_negotiate_rsp_data_parseFromJSON() failed [plmn_id_list]");
			goto end;
		}
	...
	if(snpn_id_list) {
		...
		if(!cJSON_IsArray(snpn_id_list)) {
			ogs_error(
				"OpenAPI_sec_negotiate_rsp_data_parseFromJSON() failed [snpn_id_list]");
			goto end;
		}
	...
	if(allowed_usage_purpose) {
		...
		if(!cJSON_IsArray(allowed_usage_purpose)) {
			ogs_error(
				"OpenAPI_sec_negotiate_rsp_data_parseFromJSON() failed [allowed_usage_purpose]");
			goto end;
		}

		allowed_usage_purposeList = OpenAPI_list_create();

		cJSON_ArrayForEach(allowed_usage_purpose_local,
		                   allowed_usage_purpose) {
			...
			OpenAPI_list_add(allowed_usage_purposeList,
			                 allowed_usage_purposeItem);
		}
	}
	...
	if(rejected_usage_purpose) {
		...
		cJSON_ArrayForEach(rejected_usage_purpose_local,
		                   rejected_usage_purpose) {
			...
			OpenAPI_intended_n32_purpose_t *
			        rejected_usage_purposeItem =
				OpenAPI_intended_n32_purpose_parseFromJSON(
					rejected_usage_purpose_local);
			if(!rejected_usage_purposeItem) {
				ogs_error("No rejected_usage_purposeItem");
				goto end;
			}
			OpenAPI_list_add(rejected_usage_purposeList,
			                 rejected_usage_purposeItem);
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_sec_negotiate_rsp_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}

	sec_negotiate_rsp_data_local_var =
		OpenAPI_sec_negotiate_rsp_data_create(
			...
			allowed_usage_purpose ? allowed_usage_purposeList :
			NULL,
			rejected_usage_purpose ? rejected_usage_purposeList :
			NULL,
			supported_features &&
			!cJSON_IsNull(supported_features) ?
...
OpenAPI_sec_negotiate_rsp_data_t *OpenAPI_sec_negotiate_rsp_data_copy(
	OpenAPI_sec_negotiate_rsp_data_t	*dst,
	OpenAPI_sec_negotiate_rsp_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_sec_negotiate_rsp_data_convertToJSON() failed");
		return NULL;
	}
