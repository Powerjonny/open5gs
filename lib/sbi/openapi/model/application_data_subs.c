...
OpenAPI_application_data_subs_t *OpenAPI_application_data_subs_create(
	char *notification_uri, OpenAPI_list_t *data_filters, char *expiry,
	bool is_imm_rep, int imm_rep, OpenAPI_list_t *am_influ_entries,
	char *supported_features, OpenAPI_list_t *reset_ids) {
	OpenAPI_application_data_subs_t *application_data_subs_local_var =
		ogs_malloc(sizeof(OpenAPI_application_data_subs_t));
	...
	application_data_subs_local_var->supported_features =
		supported_features;
...
void OpenAPI_application_data_subs_free(
	OpenAPI_application_data_subs_t *application_data_subs) {
	...
	if(application_data_subs->data_filters) {
		OpenAPI_list_for_each(application_data_subs->data_filters,
		                      node) {
	...
	if(application_data_subs->am_influ_entries) {
		OpenAPI_list_for_each(application_data_subs->am_influ_entries,
		                      node) {
...
cJSON *OpenAPI_application_data_subs_convertToJSON(
	...
	if(application_data_subs == NULL) {
		ogs_error(
			"OpenAPI_application_data_subs_convertToJSON() failed [ApplicationDataSubs]");
		return NULL;
	}
	...
	if(!application_data_subs->notification_uri) {
		ogs_error(
			"OpenAPI_application_data_subs_convertToJSON() failed [notification_uri]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "notificationUri",
	                           application_data_subs->notification_uri) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_application_data_subs_convertToJSON() failed [notification_uri]");
	...
	if(application_data_subs->data_filters) {
		cJSON *data_filtersList = cJSON_AddArrayToObject(item,
		                                                 "dataFilters");
		if(data_filtersList == NULL) {
			ogs_error(
				"OpenAPI_application_data_subs_convertToJSON() failed [data_filters]");
			goto end;
		}
		OpenAPI_list_for_each(application_data_subs->data_filters,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_data_filter_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_application_data_subs_convertToJSON() failed [data_filters]");
				goto end;
	...
	if(application_data_subs->expiry) {
		if(cJSON_AddStringToObject(item, "expiry",
		                           application_data_subs->expiry) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_application_data_subs_convertToJSON() failed [expiry]");
			goto end;
		}
	}
	...
	if(application_data_subs->is_imm_rep) {
		if(cJSON_AddBoolToObject(item, "immRep",
		                         application_data_subs->imm_rep) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_application_data_subs_convertToJSON() failed [imm_rep]");
			goto end;
		}
	}

	if(application_data_subs->am_influ_entries) {
		cJSON *am_influ_entriesList = cJSON_AddArrayToObject(item,
		                                                     "amInfluEntries");
		if(am_influ_entriesList == NULL) {
			ogs_error(
				"OpenAPI_application_data_subs_convertToJSON() failed [am_influ_entries]");
			goto end;
		}
		OpenAPI_list_for_each(application_data_subs->am_influ_entries,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_am_influ_data_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_application_data_subs_convertToJSON() failed [am_influ_entries]");
				goto end;
	...
	if(application_data_subs->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_application_data_subs_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(application_data_subs->reset_ids) {
		...
		if(reset_idsList == NULL) {
			ogs_error(
				"OpenAPI_application_data_subs_convertToJSON() failed [reset_ids]");
			goto end;
		}
		OpenAPI_list_for_each(application_data_subs->reset_ids, node) {
...
OpenAPI_application_data_subs_t *OpenAPI_application_data_subs_parseFromJSON(
	cJSON *application_data_subsJSON) {
	...
	notification_uri =
		cJSON_GetObjectItemCaseSensitive(application_data_subsJSON,
		                                 "notificationUri");
	if(!notification_uri) {
		ogs_error(
			"OpenAPI_application_data_subs_parseFromJSON() failed [notification_uri]");
		goto end;
	}
	...
	if(data_filters) {
		...
		if(!cJSON_IsArray(data_filters)) {
			ogs_error(
				"OpenAPI_application_data_subs_parseFromJSON() failed [data_filters]");
			goto end;
		}
		...
		cJSON_ArrayForEach(data_filters_local, data_filters) {
			...
			OpenAPI_data_filter_t *data_filtersItem =
				OpenAPI_data_filter_parseFromJSON(
					data_filters_local);
			if(!data_filtersItem) {
	...
	expiry = cJSON_GetObjectItemCaseSensitive(application_data_subsJSON,
	                                          "expiry");
	if(expiry) {
		...
		{
			ogs_error(
				"OpenAPI_application_data_subs_parseFromJSON() failed [expiry]");
			goto end;
		}
	}

	imm_rep = cJSON_GetObjectItemCaseSensitive(application_data_subsJSON,
	                                           "immRep");
	if(imm_rep) {
		if(!cJSON_IsBool(imm_rep)) {
			ogs_error(
				"OpenAPI_application_data_subs_parseFromJSON() failed [imm_rep]");
			goto end;
		}
	}
	...
	if(am_influ_entries) {
		...
		if(!cJSON_IsArray(am_influ_entries)) {
			ogs_error(
				"OpenAPI_application_data_subs_parseFromJSON() failed [am_influ_entries]");
			goto end;
		}
		...
		cJSON_ArrayForEach(am_influ_entries_local, am_influ_entries) {
			...
			OpenAPI_am_influ_data_t *am_influ_entriesItem =
				OpenAPI_am_influ_data_parseFromJSON(
					am_influ_entries_local);
			if(!am_influ_entriesItem) {
				ogs_error("No am_influ_entriesItem");
				goto end;
			}
			OpenAPI_list_add(am_influ_entriesList,
			                 am_influ_entriesItem);
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_application_data_subs_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}

	reset_ids = cJSON_GetObjectItemCaseSensitive(application_data_subsJSON,
	                                             "resetIds");
	if(reset_ids) {
		cJSON *reset_ids_local = NULL;
		if(!cJSON_IsArray(reset_ids)) {
			ogs_error(
				"OpenAPI_application_data_subs_parseFromJSON() failed [reset_ids]");
			goto end;
		}
...
OpenAPI_application_data_subs_t *OpenAPI_application_data_subs_copy(
	OpenAPI_application_data_subs_t *dst,
	OpenAPI_application_data_subs_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_application_data_subs_convertToJSON() failed");
		return NULL;
	}
