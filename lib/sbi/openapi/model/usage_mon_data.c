...
OpenAPI_usage_mon_data_t *OpenAPI_usage_mon_data_create(
	...
	*
	...
	OpenAPI_list_t *
	reset_ids) {
	OpenAPI_usage_mon_data_t *usage_mon_data_local_var =
		ogs_malloc(sizeof(OpenAPI_usage_mon_data_t));
...
cJSON *OpenAPI_usage_mon_data_convertToJSON(
	...
	if(usage_mon_data == NULL) {
		ogs_error(
			"OpenAPI_usage_mon_data_convertToJSON() failed [UsageMonData]");
		return NULL;
	}
	...
	if(!usage_mon_data->limit_id) {
		ogs_error(
			"OpenAPI_usage_mon_data_convertToJSON() failed [limit_id]");
		return NULL;
	}
	...
	if(usage_mon_data->scopes) {
		...
		if(scopes == NULL) {
			ogs_error(
				"OpenAPI_usage_mon_data_convertToJSON() failed [scopes]");
			goto end;
		}
		...
		if(usage_mon_data->scopes) {
			OpenAPI_list_for_each(usage_mon_data->scopes, node) {
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_usage_mon_data_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(usage_mon_data->um_level) {
		cJSON *um_level_local_JSON =
			OpenAPI_usage_mon_level_convertToJSON(
				usage_mon_data->um_level);
		if(um_level_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_usage_mon_data_convertToJSON() failed [um_level]");
			goto end;
		}
	...
	if(usage_mon_data->allowed_usage) {
		cJSON *allowed_usage_local_JSON =
			OpenAPI_usage_threshold_convertToJSON(
				usage_mon_data->allowed_usage);
		if(allowed_usage_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_usage_mon_data_convertToJSON() failed [allowed_usage]");
			goto end;
		}
		cJSON_AddItemToObject(item, "allowedUsage",
	...
	if(usage_mon_data->reset_time) {
		if(cJSON_AddStringToObject(item, "resetTime",
		                           usage_mon_data->reset_time) == NULL)
		{
			ogs_error(
				"OpenAPI_usage_mon_data_convertToJSON() failed [reset_time]");
			goto end;
		}
	}
	...
	if(usage_mon_data->supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_usage_mon_data_convertToJSON() failed [supp_feat]");
			goto end;
		}
	}
	...
	if(usage_mon_data->reset_ids) {
		...
		if(reset_idsList == NULL) {
			ogs_error(
				"OpenAPI_usage_mon_data_convertToJSON() failed [reset_ids]");
			goto end;
		}
		OpenAPI_list_for_each(usage_mon_data->reset_ids, node) {
...
OpenAPI_usage_mon_data_t *OpenAPI_usage_mon_data_parseFromJSON(
	...
	limit_id = cJSON_GetObjectItemCaseSensitive(usage_mon_dataJSON,
	                                            "limitId");
	if(!limit_id) {
		ogs_error(
			"OpenAPI_usage_mon_data_parseFromJSON() failed [limit_id]");
		goto end;
	}
	...
	if(scopes) {
		...
		{
			ogs_error(
				"OpenAPI_usage_mon_data_parseFromJSON() failed [scopes]");
			goto end;
		}
		if(cJSON_IsObject(scopes)) {
			...
			cJSON_ArrayForEach(scopes_local_map, scopes) {
				...
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_usage_mon_data_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	um_level = cJSON_GetObjectItemCaseSensitive(usage_mon_dataJSON,
	                                            "umLevel");
	if(um_level) {
	...
	allowed_usage = cJSON_GetObjectItemCaseSensitive(usage_mon_dataJSON,
	                                                 "allowedUsage");
	if(allowed_usage) {
		allowed_usage_local_nonprim =
			OpenAPI_usage_threshold_parseFromJSON(allowed_usage);
		if(!allowed_usage_local_nonprim) {
			ogs_error(
				"OpenAPI_usage_threshold_parseFromJSON failed [allowed_usage]");
			goto end;
		}
	}

	reset_time = cJSON_GetObjectItemCaseSensitive(usage_mon_dataJSON,
	                                              "resetTime");
	if(reset_time) {
		if(!cJSON_IsString(reset_time) &&
		   !cJSON_IsNull(reset_time))
		{
			ogs_error(
				"OpenAPI_usage_mon_data_parseFromJSON() failed [reset_time]");
			goto end;
		}
	}

	supp_feat = cJSON_GetObjectItemCaseSensitive(usage_mon_dataJSON,
	                                             "suppFeat");
	if(supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_usage_mon_data_parseFromJSON() failed [supp_feat]");
			goto end;
		}
	}

	reset_ids = cJSON_GetObjectItemCaseSensitive(usage_mon_dataJSON,
	                                             "resetIds");
	if(reset_ids) {
		cJSON *reset_ids_local = NULL;
		if(!cJSON_IsArray(reset_ids)) {
			ogs_error(
				"OpenAPI_usage_mon_data_parseFromJSON() failed [reset_ids]");
			goto end;
		}
	...
	usage_mon_data_local_var = OpenAPI_usage_mon_data_create(
		...
		reset_time &&
		!cJSON_IsNull(reset_time) ?
		ogs_strdup(reset_time->valuestring) : NULL,
		supp_feat &&
