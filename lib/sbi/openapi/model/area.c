...
OpenAPI_area_t *OpenAPI_area_create(OpenAPI_list_t *tacs, char *area_code) {
...
cJSON *OpenAPI_area_convertToJSON(OpenAPI_area_t *area) {
	...
	if(area->tacs) {
		...
		OpenAPI_list_for_each(area->tacs, node) {
			...
			{
				ogs_error(
					"OpenAPI_area_convertToJSON() failed [tacs]");
				goto end;
			}
		}
	}
	...
	if(area->area_code) {
		...
		{
			ogs_error(
				"OpenAPI_area_convertToJSON() failed [area_code]");
			goto end;
		}
	}
...
OpenAPI_area_t *OpenAPI_area_parseFromJSON(cJSON *areaJSON) {
	...
	if(tacs) {
		...
		cJSON_ArrayForEach(tacs_local, tacs) {
			...
			if(!cJSON_IsString(tacs_local)) {
				ogs_error(
					"OpenAPI_area_parseFromJSON() failed [tacs]");
				goto end;
			}
	...
	if(area_code) {
		...
		{
			ogs_error(
				"OpenAPI_area_parseFromJSON() failed [area_code]");
			goto end;
		}
	}
