...
OpenAPI_event_subscription_t *OpenAPI_event_subscription_create(
	bool is_any_slice, int any_slice, OpenAPI_list_t *app_ids,
	OpenAPI_list_t *dnns, OpenAPI_list_t *dnais,
	...
	OpenAPI_list_t *nf_load_lvl_thds, OpenAPI_list_t *nf_instance_ids,
	OpenAPI_list_t *nf_set_ids, OpenAPI_list_t *nf_types,
	OpenAPI_network_area_info_t *network_area,
	OpenAPI_list_t *visited_areas, bool is_max_top_app_ul_nbr,
	int max_top_app_ul_nbr, bool is_max_top_app_dl_nbr,
	int max_top_app_dl_nbr, OpenAPI_list_t *nsi_id_infos,
	OpenAPI_list_t *nsi_level_thrds, OpenAPI_qos_requirement_t *qos_requ,
	OpenAPI_list_t *qos_flow_ret_thds, OpenAPI_list_t *ran_ue_throu_thds,
	bool is_repetition_period, int repetition_period,
	OpenAPI_list_t *snssaia, OpenAPI_target_ue_information_t *tgt_ue,
	OpenAPI_list_t *cong_thresholds, OpenAPI_list_t *nw_perf_requs,
	OpenAPI_list_t *bw_requs, OpenAPI_list_t *excep_requs,
	OpenAPI_expected_analytics_type_t *expt_ana_type,
	OpenAPI_expected_ue_behaviour_data_t *expt_ue_behav,
	OpenAPI_list_t *rat_freqs, OpenAPI_list_t *list_of_ana_subsets,
	OpenAPI_list_t *disper_reqs, OpenAPI_list_t *red_trans_reqs,
	OpenAPI_list_t *wlan_reqs, OpenAPI_upf_information_t *upf_info,
	OpenAPI_list_t *app_server_addrs, OpenAPI_list_t *dn_perf_reqs) {
	OpenAPI_event_subscription_t *event_subscription_local_var =
		ogs_malloc(sizeof(OpenAPI_event_subscription_t));
...
void OpenAPI_event_subscription_free(
	OpenAPI_event_subscription_t *event_subscription) {
	...
	if(event_subscription->extra_report_req) {
		OpenAPI_event_reporting_requirement_free(
	...
	if(event_subscription->notification_method) {
		OpenAPI_notification_method_free(
	...
	if(event_subscription->nf_load_lvl_thds) {
		OpenAPI_list_for_each(event_subscription->nf_load_lvl_thds,
		                      node) {
	...
	if(event_subscription->nf_instance_ids) {
		OpenAPI_list_for_each(event_subscription->nf_instance_ids,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(event_subscription->network_area) {
		OpenAPI_network_area_info_free(
	...
	if(event_subscription->nsi_level_thrds) {
		OpenAPI_list_for_each(event_subscription->nsi_level_thrds,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(event_subscription->qos_flow_ret_thds) {
		OpenAPI_list_for_each(event_subscription->qos_flow_ret_thds,
		                      node) {
	...
	if(event_subscription->ran_ue_throu_thds) {
		OpenAPI_list_for_each(event_subscription->ran_ue_throu_thds,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(event_subscription->cong_thresholds) {
		OpenAPI_list_for_each(event_subscription->cong_thresholds,
		                      node) {
			OpenAPI_threshold_level_free(node->data);
		}
	...
	if(event_subscription->expt_ana_type) {
		OpenAPI_expected_analytics_type_free(
	...
	if(event_subscription->list_of_ana_subsets) {
		OpenAPI_list_for_each(event_subscription->list_of_ana_subsets,
		                      node) {
	...
	if(event_subscription->red_trans_reqs) {
		OpenAPI_list_for_each(event_subscription->red_trans_reqs,
		                      node) {
	...
	if(event_subscription->app_server_addrs) {
		OpenAPI_list_for_each(event_subscription->app_server_addrs,
		                      node) {
...
cJSON *OpenAPI_event_subscription_convertToJSON(
	OpenAPI_event_subscription_t *event_subscription) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(event_subscription == NULL) {
		ogs_error(
			"OpenAPI_event_subscription_convertToJSON() failed [EventSubscription]");
		return NULL;
	}
	...
	if(event_subscription->is_any_slice) {
		...
		{
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [any_slice]");
			goto end;
		}
	}
	...
	if(event_subscription->app_ids) {
		...
		if(app_idsList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [app_ids]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->app_ids, node) {
	...
	if(event_subscription->dnns) {
		...
		if(dnnsList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [dnns]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->dnns, node) {
	...
	if(event_subscription->dnais) {
		...
		if(dnaisList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [dnais]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->dnais, node) {
	...
	if(!event_subscription->event) {
		ogs_error(
			"OpenAPI_event_subscription_convertToJSON() failed [event]");
		return NULL;
	}
	cJSON *event_local_JSON =
		OpenAPI_nwdaf_event_convertToJSON(event_subscription->event);
	...
	if(event_subscription->extra_report_req) {
		cJSON *extra_report_req_local_JSON =
			OpenAPI_event_reporting_requirement_convertToJSON(
				event_subscription->extra_report_req);
		if(extra_report_req_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [extra_report_req]");
			goto end;
		}
	...
	if(event_subscription->ladn_dnns) {
		...
		if(ladn_dnnsList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [ladn_dnns]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->ladn_dnns, node) {
	...
	if(event_subscription->is_load_level_threshold) {
		...
		{
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [load_level_threshold]");
			goto end;
		}
	}
	...
	if(event_subscription->notification_method) {
		cJSON *notification_method_local_JSON =
			OpenAPI_notification_method_convertToJSON(
				event_subscription->notification_method);
		if(notification_method_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [notification_method]");
			goto end;
		}
	...
	if(event_subscription->matching_dir) {
		cJSON *matching_dir_local_JSON =
			OpenAPI_matching_direction_convertToJSON(
				event_subscription->matching_dir);
		if(matching_dir_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [matching_dir]");
			goto end;
		}
	...
	if(event_subscription->nf_load_lvl_thds) {
		...
		if(nf_load_lvl_thdsList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [nf_load_lvl_thds]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->nf_load_lvl_thds,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_threshold_level_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_event_subscription_convertToJSON() failed [nf_load_lvl_thds]");
	...
	if(event_subscription->nf_instance_ids) {
		...
		if(nf_instance_idsList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [nf_instance_ids]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->nf_instance_ids,
		                      node) {
	...
	if(event_subscription->nf_set_ids) {
		cJSON *nf_set_idsList = cJSON_AddArrayToObject(item,
		                                               "nfSetIds");
		if(nf_set_idsList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [nf_set_ids]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->nf_set_ids, node) {
	...
	if(event_subscription->nf_types != OpenAPI_nf_type_NULL) {
		...
		if(nf_typesList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [nf_types]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->nf_types, node) {
			if(cJSON_AddStringToObject(nf_typesList, "",
	...
	if(event_subscription->network_area) {
		cJSON *network_area_local_JSON =
			OpenAPI_network_area_info_convertToJSON(
				event_subscription->network_area);
		if(network_area_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [network_area]");
			goto end;
		}
	...
	if(event_subscription->visited_areas) {
		...
		if(visited_areasList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [visited_areas]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->visited_areas, node) {
			cJSON *itemLocal =
	...
	if(event_subscription->is_max_top_app_ul_nbr) {
		...
		{
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [max_top_app_ul_nbr]");
			goto end;
		}
	}
	...
	if(event_subscription->is_max_top_app_dl_nbr) {
		...
		{
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [max_top_app_dl_nbr]");
			goto end;
		}
	}
	...
	if(event_subscription->nsi_id_infos) {
		...
		if(nsi_id_infosList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [nsi_id_infos]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->nsi_id_infos, node) {
			cJSON *itemLocal =
				OpenAPI_nsi_id_info_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_event_subscription_convertToJSON() failed [nsi_id_infos]");
				goto end;
	...
	if(event_subscription->nsi_level_thrds) {
		...
		if(nsi_level_thrdsList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [nsi_level_thrds]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->nsi_level_thrds,
		                      node) {
	...
	if(event_subscription->qos_requ) {
		cJSON *qos_requ_local_JSON =
			OpenAPI_qos_requirement_convertToJSON(
				event_subscription->qos_requ);
		if(qos_requ_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [qos_requ]");
			goto end;
		}
	...
	if(event_subscription->qos_flow_ret_thds) {
		...
		if(qos_flow_ret_thdsList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [qos_flow_ret_thds]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->qos_flow_ret_thds,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_retainability_threshold_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_event_subscription_convertToJSON() failed [qos_flow_ret_thds]");
	...
	if(event_subscription->ran_ue_throu_thds) {
		...
		if(ran_ue_throu_thdsList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [ran_ue_throu_thds]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->ran_ue_throu_thds,
		                      node) {
	...
	if(event_subscription->is_repetition_period) {
		...
		{
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [repetition_period]");
			goto end;
		}
	}
	...
	if(event_subscription->snssaia) {
		...
		if(snssaiaList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [snssaia]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->snssaia, node) {
			cJSON *itemLocal =
				OpenAPI_snssai_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_event_subscription_convertToJSON() failed [snssaia]");
				goto end;
	...
	if(event_subscription->tgt_ue) {
		cJSON *tgt_ue_local_JSON =
			OpenAPI_target_ue_information_convertToJSON(
				event_subscription->tgt_ue);
		if(tgt_ue_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [tgt_ue]");
			goto end;
		}
	...
	if(event_subscription->cong_thresholds) {
		...
		if(cong_thresholdsList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [cong_thresholds]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->cong_thresholds,
		                      node) {
	...
	if(event_subscription->nw_perf_requs) {
		...
		if(nw_perf_requsList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [nw_perf_requs]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->nw_perf_requs, node) {
			cJSON *itemLocal =
				OpenAPI_network_perf_requirement_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_event_subscription_convertToJSON() failed [nw_perf_requs]");
	...
	if(event_subscription->bw_requs) {
		...
		if(bw_requsList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [bw_requs]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->bw_requs, node) {
			cJSON *itemLocal =
				OpenAPI_bw_requirement_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_event_subscription_convertToJSON() failed [bw_requs]");
	...
	if(event_subscription->excep_requs) {
		...
		if(excep_requsList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [excep_requs]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->excep_requs, node) {
			cJSON *itemLocal =
				OpenAPI_exception_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_event_subscription_convertToJSON() failed [excep_requs]");
				goto end;
	...
	if(event_subscription->expt_ana_type) {
		cJSON *expt_ana_type_local_JSON =
			OpenAPI_expected_analytics_type_convertToJSON(
				event_subscription->expt_ana_type);
		if(expt_ana_type_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [expt_ana_type]");
			goto end;
		}
	...
	if(event_subscription->expt_ue_behav) {
		...
		if(expt_ue_behav_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [expt_ue_behav]");
			goto end;
		}
	...
	if(event_subscription->rat_freqs) {
		...
		if(rat_freqsList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [rat_freqs]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->rat_freqs, node) {
			cJSON *itemLocal =
				OpenAPI_rat_freq_information_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_event_subscription_convertToJSON() failed [rat_freqs]");
	...
	if(event_subscription->list_of_ana_subsets) {
		...
		if(list_of_ana_subsetsList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [list_of_ana_subsets]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->list_of_ana_subsets,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_analytics_subset_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_event_subscription_convertToJSON() failed [list_of_ana_subsets]");
	...
	if(event_subscription->disper_reqs) {
		...
		if(disper_reqsList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [disper_reqs]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->disper_reqs, node) {
			cJSON *itemLocal =
				OpenAPI_dispersion_requirement_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_event_subscription_convertToJSON() failed [disper_reqs]");
	...
	if(event_subscription->red_trans_reqs) {
		...
		if(red_trans_reqsList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [red_trans_reqs]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->red_trans_reqs,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_redundant_transmission_exp_req_convertToJSON
				        (node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_event_subscription_convertToJSON() failed [red_trans_reqs]");
	...
	if(event_subscription->wlan_reqs) {
		...
		if(wlan_reqsList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [wlan_reqs]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->wlan_reqs, node) {
			cJSON *itemLocal =
				OpenAPI_wlan_performance_req_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_event_subscription_convertToJSON() failed [wlan_reqs]");
	...
	if(event_subscription->upf_info) {
		cJSON *upf_info_local_JSON =
			OpenAPI_upf_information_convertToJSON(
				event_subscription->upf_info);
		if(upf_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [upf_info]");
			goto end;
		}
	...
	if(event_subscription->app_server_addrs) {
		...
		if(app_server_addrsList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [app_server_addrs]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->app_server_addrs,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_addr_fqdn_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_event_subscription_convertToJSON() failed [app_server_addrs]");
				goto end;
	...
	if(event_subscription->dn_perf_reqs) {
		...
		if(dn_perf_reqsList == NULL) {
			ogs_error(
				"OpenAPI_event_subscription_convertToJSON() failed [dn_perf_reqs]");
			goto end;
		}
		OpenAPI_list_for_each(event_subscription->dn_perf_reqs, node) {
			cJSON *itemLocal =
				OpenAPI_dn_performance_req_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_event_subscription_convertToJSON() failed [dn_perf_reqs]");
...
OpenAPI_event_subscription_t *OpenAPI_event_subscription_parseFromJSON(
	...
	OpenAPI_event_reporting_requirement_t *extra_report_req_local_nonprim =
		NULL;
	...
	OpenAPI_expected_ue_behaviour_data_t *expt_ue_behav_local_nonprim =
		NULL;
	...
	if(any_slice) {
		if(!cJSON_IsBool(any_slice)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [any_slice]");
			goto end;
		}
	}
	...
	if(app_ids) {
		...
		if(!cJSON_IsArray(app_ids)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [app_ids]");
			goto end;
		}
		...
		cJSON_ArrayForEach(app_ids_local, app_ids) {
			...
			OpenAPI_list_add(app_idsList,
			                 ogs_strdup(
						 app_ids_local->valuestring));
		}
	}
	...
	if(dnns) {
		...
		if(!cJSON_IsArray(dnns)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [dnns]");
			goto end;
		}
		...
		cJSON_ArrayForEach(dnns_local, dnns) {
			...
			OpenAPI_list_add(dnnsList,
			                 ogs_strdup(dnns_local->valuestring));
		}
	}

	dnais = cJSON_GetObjectItemCaseSensitive(event_subscriptionJSON,
	                                         "dnais");
	if(dnais) {
		cJSON *dnais_local = NULL;
		if(!cJSON_IsArray(dnais)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [dnais]");
			goto end;
		}
		...
		cJSON_ArrayForEach(dnais_local, dnais) {
			...
			OpenAPI_list_add(dnaisList,
			                 ogs_strdup(dnais_local->valuestring));
		}
	}

	event = cJSON_GetObjectItemCaseSensitive(event_subscriptionJSON,
	                                         "event");
	if(!event) {
		ogs_error(
			"OpenAPI_event_subscription_parseFromJSON() failed [event]");
		goto end;
	}
	...
	if(extra_report_req) {
		extra_report_req_local_nonprim =
			OpenAPI_event_reporting_requirement_parseFromJSON(
				extra_report_req);
		if(!extra_report_req_local_nonprim) {
			ogs_error(
				"OpenAPI_event_reporting_requirement_parseFromJSON failed [extra_report_req]");
			goto end;
		}
	}
	...
	if(ladn_dnns) {
		...
		if(!cJSON_IsArray(ladn_dnns)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [ladn_dnns]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ladn_dnns_local, ladn_dnns) {
			...
			OpenAPI_list_add(ladn_dnnsList,
			                 ogs_strdup(
						 ladn_dnns_local->valuestring));
		}
	}
	...
	if(load_level_threshold) {
		if(!cJSON_IsNumber(load_level_threshold)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [load_level_threshold]");
			goto end;
		}
	}
	...
	if(notification_method) {
		notification_method_local_nonprim =
			OpenAPI_notification_method_parseFromJSON(
				notification_method);
		if(!notification_method_local_nonprim) {
			ogs_error(
				"OpenAPI_notification_method_parseFromJSON failed [notification_method]");
			goto end;
		}
	}
	...
	if(matching_dir) {
		matching_dir_local_nonprim =
			OpenAPI_matching_direction_parseFromJSON(matching_dir);
		if(!matching_dir_local_nonprim) {
			ogs_error(
				"OpenAPI_matching_direction_parseFromJSON failed [matching_dir]");
			goto end;
		}
	}
	...
	if(nf_load_lvl_thds) {
		...
		if(!cJSON_IsArray(nf_load_lvl_thds)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [nf_load_lvl_thds]");
			goto end;
		}
		...
		cJSON_ArrayForEach(nf_load_lvl_thds_local, nf_load_lvl_thds) {
			...
			OpenAPI_threshold_level_t *nf_load_lvl_thdsItem =
				OpenAPI_threshold_level_parseFromJSON(
					nf_load_lvl_thds_local);
			if(!nf_load_lvl_thdsItem) {
				ogs_error("No nf_load_lvl_thdsItem");
				goto end;
			}
			OpenAPI_list_add(nf_load_lvl_thdsList,
			                 nf_load_lvl_thdsItem);
		}
	}
	...
	if(nf_instance_ids) {
		...
		if(!cJSON_IsArray(nf_instance_ids)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [nf_instance_ids]");
			goto end;
		}
		...
		cJSON_ArrayForEach(nf_instance_ids_local, nf_instance_ids) {
			...
			OpenAPI_list_add(nf_instance_idsList,
			                 ogs_strdup(
						 nf_instance_ids_local->
						 valuestring));
		}
	}
	...
	if(nf_set_ids) {
		...
		if(!cJSON_IsArray(nf_set_ids)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [nf_set_ids]");
			goto end;
		}
		...
		cJSON_ArrayForEach(nf_set_ids_local, nf_set_ids) {
			...
			OpenAPI_list_add(nf_set_idsList,
			                 ogs_strdup(
						 nf_set_ids_local->valuestring));
		}
	}
	...
	if(nf_types) {
		...
		if(!cJSON_IsArray(nf_types)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [nf_types]");
			goto end;
		}
		...
		cJSON_ArrayForEach(nf_types_local, nf_types) {
			...
			if(!localEnum) {
				ogs_info(
					"Enum value \"%s\" for field \"nf_types\" is not supported. Ignoring it ...",
					nf_types_local->valuestring);
			} else {
				OpenAPI_list_add(nf_typesList,
				                 (void *) localEnum);
			}
		}
		if(nf_typesList->count == 0) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed: Expected nf_typesList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(network_area) {
		network_area_local_nonprim =
			OpenAPI_network_area_info_parseFromJSON(network_area);
		if(!network_area_local_nonprim) {
			ogs_error(
				"OpenAPI_network_area_info_parseFromJSON failed [network_area]");
			goto end;
		}
	}
	...
	if(visited_areas) {
		...
		if(!cJSON_IsArray(visited_areas)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [visited_areas]");
			goto end;
		}
		...
		cJSON_ArrayForEach(visited_areas_local, visited_areas) {
			...
			OpenAPI_network_area_info_t *visited_areasItem =
	...
	if(max_top_app_ul_nbr) {
		if(!cJSON_IsNumber(max_top_app_ul_nbr)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [max_top_app_ul_nbr]");
			goto end;
		}
	}
	...
	if(max_top_app_dl_nbr) {
		if(!cJSON_IsNumber(max_top_app_dl_nbr)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [max_top_app_dl_nbr]");
			goto end;
		}
	}
	...
	if(nsi_id_infos) {
		...
		if(!cJSON_IsArray(nsi_id_infos)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [nsi_id_infos]");
			goto end;
		}
		...
		cJSON_ArrayForEach(nsi_id_infos_local, nsi_id_infos) {
			...
			OpenAPI_nsi_id_info_t *nsi_id_infosItem =
				OpenAPI_nsi_id_info_parseFromJSON(
					nsi_id_infos_local);
			if(!nsi_id_infosItem) {
	...
	if(nsi_level_thrds) {
		...
		if(!cJSON_IsArray(nsi_level_thrds)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [nsi_level_thrds]");
			goto end;
		}
	...
	if(qos_requ) {
		qos_requ_local_nonprim =
			OpenAPI_qos_requirement_parseFromJSON(qos_requ);
		if(!qos_requ_local_nonprim) {
			ogs_error(
				"OpenAPI_qos_requirement_parseFromJSON failed [qos_requ]");
			goto end;
		}
	}
	...
	if(qos_flow_ret_thds) {
		...
		if(!cJSON_IsArray(qos_flow_ret_thds)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [qos_flow_ret_thds]");
			goto end;
		}
		...
		cJSON_ArrayForEach(qos_flow_ret_thds_local, qos_flow_ret_thds) {
			...
			OpenAPI_retainability_threshold_t *qos_flow_ret_thdsItem
			        = OpenAPI_retainability_threshold_parseFromJSON(
					  qos_flow_ret_thds_local);
			if(!qos_flow_ret_thdsItem) {
				ogs_error("No qos_flow_ret_thdsItem");
				goto end;
			}
			OpenAPI_list_add(qos_flow_ret_thdsList,
			                 qos_flow_ret_thdsItem);
		}
	}
	...
	if(ran_ue_throu_thds) {
		...
		if(!cJSON_IsArray(ran_ue_throu_thds)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [ran_ue_throu_thds]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ran_ue_throu_thds_local, ran_ue_throu_thds) {
			...
			OpenAPI_list_add(ran_ue_throu_thdsList,
			                 ogs_strdup(
	...
	if(repetition_period) {
		if(!cJSON_IsNumber(repetition_period)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [repetition_period]");
			goto end;
		}
	}
	...
	if(snssaia) {
		...
		if(!cJSON_IsArray(snssaia)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [snssaia]");
			goto end;
		}
		...
		cJSON_ArrayForEach(snssaia_local, snssaia) {
			...
			OpenAPI_snssai_t *snssaiaItem =
				OpenAPI_snssai_parseFromJSON(snssaia_local);
			if(!snssaiaItem) {
	...
	tgt_ue = cJSON_GetObjectItemCaseSensitive(event_subscriptionJSON,
	                                          "tgtUe");
	if(tgt_ue) {
		tgt_ue_local_nonprim =
			OpenAPI_target_ue_information_parseFromJSON(tgt_ue);
		if(!tgt_ue_local_nonprim) {
			ogs_error(
				"OpenAPI_target_ue_information_parseFromJSON failed [tgt_ue]");
			goto end;
		}
	}
	...
	if(cong_thresholds) {
		...
		if(!cJSON_IsArray(cong_thresholds)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [cong_thresholds]");
			goto end;
		}
		...
		cJSON_ArrayForEach(cong_thresholds_local, cong_thresholds) {
			...
			OpenAPI_threshold_level_t *cong_thresholdsItem =
				OpenAPI_threshold_level_parseFromJSON(
			...
			OpenAPI_list_add(cong_thresholdsList,
			                 cong_thresholdsItem);
		}
	}
	...
	if(nw_perf_requs) {
		...
		if(!cJSON_IsArray(nw_perf_requs)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [nw_perf_requs]");
			goto end;
		}
		...
		cJSON_ArrayForEach(nw_perf_requs_local, nw_perf_requs) {
			...
			OpenAPI_network_perf_requirement_t *nw_perf_requsItem =
				OpenAPI_network_perf_requirement_parseFromJSON(
					nw_perf_requs_local);
	...
	if(bw_requs) {
		...
		if(!cJSON_IsArray(bw_requs)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [bw_requs]");
			goto end;
		}
		...
		cJSON_ArrayForEach(bw_requs_local, bw_requs) {
			...
			OpenAPI_bw_requirement_t *bw_requsItem =
				OpenAPI_bw_requirement_parseFromJSON(
					bw_requs_local);
			if(!bw_requsItem) {
	...
	if(excep_requs) {
		...
		if(!cJSON_IsArray(excep_requs)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [excep_requs]");
			goto end;
		}
		...
		cJSON_ArrayForEach(excep_requs_local, excep_requs) {
			...
			OpenAPI_exception_t *excep_requsItem =
				OpenAPI_exception_parseFromJSON(
					excep_requs_local);
			if(!excep_requsItem) {
	...
	if(expt_ana_type) {
		expt_ana_type_local_nonprim =
			OpenAPI_expected_analytics_type_parseFromJSON(
				expt_ana_type);
		if(!expt_ana_type_local_nonprim) {
			ogs_error(
				"OpenAPI_expected_analytics_type_parseFromJSON failed [expt_ana_type]");
			goto end;
		}
	}
	...
	if(expt_ue_behav) {
		...
		if(!expt_ue_behav_local_nonprim) {
			ogs_error(
				"OpenAPI_expected_ue_behaviour_data_parseFromJSON failed [expt_ue_behav]");
			goto end;
		}
	}
	...
	if(rat_freqs) {
		...
		if(!cJSON_IsArray(rat_freqs)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [rat_freqs]");
			goto end;
		}
		...
		cJSON_ArrayForEach(rat_freqs_local, rat_freqs) {
			...
			OpenAPI_rat_freq_information_t *rat_freqsItem =
				OpenAPI_rat_freq_information_parseFromJSON(
					rat_freqs_local);
			if(!rat_freqsItem) {
	...
	if(list_of_ana_subsets) {
		...
		if(!cJSON_IsArray(list_of_ana_subsets)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [list_of_ana_subsets]");
			goto end;
		}
		...
		cJSON_ArrayForEach(list_of_ana_subsets_local,
			...
			OpenAPI_analytics_subset_t *list_of_ana_subsetsItem =
				OpenAPI_analytics_subset_parseFromJSON(
					list_of_ana_subsets_local);
			if(!list_of_ana_subsetsItem) {
				ogs_error("No list_of_ana_subsetsItem");
				goto end;
			}
			OpenAPI_list_add(list_of_ana_subsetsList,
	...
	if(disper_reqs) {
		...
		if(!cJSON_IsArray(disper_reqs)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [disper_reqs]");
			goto end;
		}
		...
		cJSON_ArrayForEach(disper_reqs_local, disper_reqs) {
			...
			OpenAPI_dispersion_requirement_t *disper_reqsItem =
				OpenAPI_dispersion_requirement_parseFromJSON(
					disper_reqs_local);
			if(!disper_reqsItem) {
	...
	if(red_trans_reqs) {
		...
		if(!cJSON_IsArray(red_trans_reqs)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [red_trans_reqs]");
			goto end;
		}
		...
		cJSON_ArrayForEach(red_trans_reqs_local, red_trans_reqs) {
			...
			OpenAPI_list_add(red_trans_reqsList,
			                 red_trans_reqsItem);
		}
	}
	...
	if(wlan_reqs) {
		...
		if(!cJSON_IsArray(wlan_reqs)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [wlan_reqs]");
			goto end;
		}
		...
		cJSON_ArrayForEach(wlan_reqs_local, wlan_reqs) {
			...
			OpenAPI_wlan_performance_req_t *wlan_reqsItem =
				OpenAPI_wlan_performance_req_parseFromJSON(
					wlan_reqs_local);
			if(!wlan_reqsItem) {
	...
	if(upf_info) {
		upf_info_local_nonprim =
			OpenAPI_upf_information_parseFromJSON(upf_info);
		if(!upf_info_local_nonprim) {
			ogs_error(
				"OpenAPI_upf_information_parseFromJSON failed [upf_info]");
			goto end;
		}
	}
	...
	if(app_server_addrs) {
		...
		if(!cJSON_IsArray(app_server_addrs)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [app_server_addrs]");
			goto end;
		}
		...
		cJSON_ArrayForEach(app_server_addrs_local, app_server_addrs) {
			...
			OpenAPI_addr_fqdn_t *app_server_addrsItem =
				OpenAPI_addr_fqdn_parseFromJSON(
					app_server_addrs_local);
			if(!app_server_addrsItem) {
				ogs_error("No app_server_addrsItem");
				goto end;
			}
			OpenAPI_list_add(app_server_addrsList,
			                 app_server_addrsItem);
		}
	}
	...
	if(dn_perf_reqs) {
		...
		if(!cJSON_IsArray(dn_perf_reqs)) {
			ogs_error(
				"OpenAPI_event_subscription_parseFromJSON() failed [dn_perf_reqs]");
			goto end;
		}
		...
		cJSON_ArrayForEach(dn_perf_reqs_local, dn_perf_reqs) {
			...
			OpenAPI_dn_performance_req_t *dn_perf_reqsItem =
				OpenAPI_dn_performance_req_parseFromJSON(
					dn_perf_reqs_local);
			if(!dn_perf_reqsItem) {
...
OpenAPI_event_subscription_t *OpenAPI_event_subscription_copy(
	OpenAPI_event_subscription_t *dst, OpenAPI_event_subscription_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
