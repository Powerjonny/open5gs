...
OpenAPI_user_location_t *OpenAPI_user_location_create(
	...
	OpenAPI_user_location_t *user_location_local_var =
		ogs_malloc(sizeof(OpenAPI_user_location_t));
...
cJSON *OpenAPI_user_location_convertToJSON(
	...
	if(user_location == NULL) {
		ogs_error(
			"OpenAPI_user_location_convertToJSON() failed [UserLocation]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(user_location->eutra_location) {
		cJSON *eutra_location_local_JSON =
			OpenAPI_eutra_location_convertToJSON(
				user_location->eutra_location);
		if(eutra_location_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_user_location_convertToJSON() failed [eutra_location]");
			goto end;
		}
	...
	if(user_location->nr_location) {
		cJSON *nr_location_local_JSON =
			OpenAPI_nr_location_convertToJSON(
				user_location->nr_location);
		if(nr_location_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_user_location_convertToJSON() failed [nr_location]");
			goto end;
		}
	...
	if(user_location->n3ga_location) {
		cJSON *n3ga_location_local_JSON =
			OpenAPI_n3ga_location_convertToJSON(
				user_location->n3ga_location);
		if(n3ga_location_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_user_location_convertToJSON() failed [n3ga_location]");
			goto end;
		}
	...
	if(user_location->utra_location) {
		cJSON *utra_location_local_JSON =
			OpenAPI_utra_location_convertToJSON(
				user_location->utra_location);
		if(utra_location_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_user_location_convertToJSON() failed [utra_location]");
			goto end;
		}
	...
	if(user_location->gera_location) {
		cJSON *gera_location_local_JSON =
			OpenAPI_gera_location_convertToJSON(
				user_location->gera_location);
		if(gera_location_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_user_location_convertToJSON() failed [gera_location]");
			goto end;
		}
...
OpenAPI_user_location_t *OpenAPI_user_location_parseFromJSON(
	cJSON *user_locationJSON) {
	...
	eutra_location = cJSON_GetObjectItemCaseSensitive(user_locationJSON,
	                                                  "eutraLocation");
	if(eutra_location) {
		eutra_location_local_nonprim =
			OpenAPI_eutra_location_parseFromJSON(eutra_location);
		if(!eutra_location_local_nonprim) {
			ogs_error(
				"OpenAPI_eutra_location_parseFromJSON failed [eutra_location]");
			goto end;
		}
	}
	...
	if(nr_location) {
		nr_location_local_nonprim =
			OpenAPI_nr_location_parseFromJSON(nr_location);
		if(!nr_location_local_nonprim) {
			ogs_error(
				"OpenAPI_nr_location_parseFromJSON failed [nr_location]");
			goto end;
		}
	}
	...
	if(n3ga_location) {
		n3ga_location_local_nonprim =
			OpenAPI_n3ga_location_parseFromJSON(n3ga_location);
	...
	if(utra_location) {
		...
		if(!utra_location_local_nonprim) {
			ogs_error(
				"OpenAPI_utra_location_parseFromJSON failed [utra_location]");
			goto end;
		}
	}
	...
	gera_location = cJSON_GetObjectItemCaseSensitive(user_locationJSON,
	                                                 "geraLocation");
	if(gera_location) {
		gera_location_local_nonprim =
			OpenAPI_gera_location_parseFromJSON(gera_location);
		if(!gera_location_local_nonprim) {
			ogs_error(
				"OpenAPI_gera_location_parseFromJSON failed [gera_location]");
			goto end;
		}
	}
...
OpenAPI_user_location_t *OpenAPI_user_location_copy(
	OpenAPI_user_location_t *dst, OpenAPI_user_location_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
