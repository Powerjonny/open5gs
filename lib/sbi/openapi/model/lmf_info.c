...
char *OpenAPI_aiml_pos_indlmf_info_ToString(
	OpenAPI_lmf_info_aiml_pos_ind_e aiml_pos_ind) {
...
OpenAPI_lmf_info_aiml_pos_ind_e OpenAPI_aiml_pos_indlmf_info_FromString(
	char *aiml_pos_ind) {
...
OpenAPI_lmf_info_t *OpenAPI_lmf_info_create(
	...
	OpenAPI_lmf_info_t *lmf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_lmf_info_t));
	...
	lmf_info_local_var->is_rangingslpos_support_ind =
...
cJSON *OpenAPI_lmf_info_convertToJSON(OpenAPI_lmf_info_t *lmf_info) {
	   ...
	   OpenAPI_external_client_type_NULL)
	{
		...
		if(serving_client_typesList == NULL) {
			ogs_error(
				"OpenAPI_lmf_info_convertToJSON() failed [serving_client_types]");
			goto end;
		}
	...
	if(lmf_info->lmf_id) {
		...
		{
			ogs_error(
				"OpenAPI_lmf_info_convertToJSON() failed [lmf_id]");
			goto end;
		}
	}
	...
	if(lmf_info->serving_access_types != OpenAPI_access_type_NULL) {
		...
		if(serving_access_typesList == NULL) {
			ogs_error(
				"OpenAPI_lmf_info_convertToJSON() failed [serving_access_types]");
			goto end;
		}
	...
	if(lmf_info->serving_an_node_types != OpenAPI_an_node_type_NULL) {
		...
		if(serving_an_node_typesList == NULL) {
			ogs_error(
				"OpenAPI_lmf_info_convertToJSON() failed [serving_an_node_types]");
			goto end;
		}
	...
	if(lmf_info->serving_rat_types != OpenAPI_rat_type_NULL) {
		...
		if(serving_rat_typesList == NULL) {
			ogs_error(
				"OpenAPI_lmf_info_convertToJSON() failed [serving_rat_types]");
			goto end;
		}
	...
	if(lmf_info->tai_list) {
		...
		if(tai_listList == NULL) {
			ogs_error(
				"OpenAPI_lmf_info_convertToJSON() failed [tai_list]");
			goto end;
		}
		OpenAPI_list_for_each(lmf_info->tai_list, node) {
			cJSON *itemLocal =
	...
	if(lmf_info->tai_range_list) {
		...
		if(tai_range_listList == NULL) {
			ogs_error(
				"OpenAPI_lmf_info_convertToJSON() failed [tai_range_list]");
			goto end;
		}
		OpenAPI_list_for_each(lmf_info->tai_range_list, node) {
			cJSON *itemLocal =
	   ...
	   OpenAPI_supported_gad_shapes_NULL)
	{
		...
		if(supported_gad_shapesList == NULL) {
			ogs_error(
				"OpenAPI_lmf_info_convertToJSON() failed [supported_gad_shapes]");
			goto end;
		}
	...
	if(lmf_info->pru_existence_info) {
		cJSON *pru_existence_info_local_JSON =
			OpenAPI_pru_existence_info_convertToJSON(
				lmf_info->pru_existence_info);
		if(pru_existence_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_lmf_info_convertToJSON() failed [pru_existence_info]");
			goto end;
		}
	...
	if(lmf_info->is_pru_support_ind) {
		if(cJSON_AddBoolToObject(item, "pruSupportInd",
		                         lmf_info->pru_support_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_lmf_info_convertToJSON() failed [pru_support_ind]");
			goto end;
		}
	}
	...
	if(lmf_info->is_rangingslpos_support_ind) {
		if(cJSON_AddBoolToObject(item, "rangingslposSupportInd",
		                         lmf_info->rangingslpos_support_ind) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_lmf_info_convertToJSON() failed [rangingslpos_support_ind]");
			goto end;
		}
	}
	...
	if(lmf_info->is_up_positioning_ind) {
		if(cJSON_AddBoolToObject(item, "upPositioningInd",
		                         lmf_info->up_positioning_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_lmf_info_convertToJSON() failed [up_positioning_ind]");
			goto end;
		}
	}

	if(lmf_info->aiml_pos_ind != OpenAPI_lmf_info_AIMLPOSIND_NULL) {
		if(cJSON_AddStringToObject(item, "aimlPosInd",
		                           OpenAPI_aiml_pos_indlmf_info_ToString
		                                   (lmf_info->aiml_pos_ind)) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_lmf_info_convertToJSON() failed [aiml_pos_ind]");
			goto end;
		}
	}
...
OpenAPI_lmf_info_t *OpenAPI_lmf_info_parseFromJSON(cJSON *lmf_infoJSON) {
	...
	OpenAPI_lmf_info_aiml_pos_ind_e aiml_pos_indVariable = 0;
	serving_client_types = cJSON_GetObjectItemCaseSensitive(lmf_infoJSON,
	                                                        "servingClientTypes");
	if(serving_client_types) {
		cJSON *serving_client_types_local = NULL;
		if(!cJSON_IsArray(serving_client_types)) {
			ogs_error(
				"OpenAPI_lmf_info_parseFromJSON() failed [serving_client_types]");
			goto end;
		}
		...
		if(serving_client_typesList->count == 0) {
			ogs_error(
				"OpenAPI_lmf_info_parseFromJSON() failed: Expected serving_client_typesList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(lmf_id) {
		...
		{
			ogs_error(
				"OpenAPI_lmf_info_parseFromJSON() failed [lmf_id]");
			goto end;
		}
	}

	serving_access_types = cJSON_GetObjectItemCaseSensitive(lmf_infoJSON,
	                                                        "servingAccessTypes");
	if(serving_access_types) {
		cJSON *serving_access_types_local = NULL;
		if(!cJSON_IsArray(serving_access_types)) {
			ogs_error(
				"OpenAPI_lmf_info_parseFromJSON() failed [serving_access_types]");
			goto end;
		}
		...
		if(serving_access_typesList->count == 0) {
			ogs_error(
				"OpenAPI_lmf_info_parseFromJSON() failed: Expected serving_access_typesList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}

	serving_an_node_types = cJSON_GetObjectItemCaseSensitive(lmf_infoJSON,
	                                                         "servingAnNodeTypes");
	if(serving_an_node_types) {
		cJSON *serving_an_node_types_local = NULL;
		if(!cJSON_IsArray(serving_an_node_types)) {
			ogs_error(
				"OpenAPI_lmf_info_parseFromJSON() failed [serving_an_node_types]");
			goto end;
		}
		...
		if(serving_an_node_typesList->count == 0) {
			ogs_error(
				"OpenAPI_lmf_info_parseFromJSON() failed: Expected serving_an_node_typesList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}

	serving_rat_types = cJSON_GetObjectItemCaseSensitive(lmf_infoJSON,
	                                                     "servingRatTypes");
	if(serving_rat_types) {
		cJSON *serving_rat_types_local = NULL;
		if(!cJSON_IsArray(serving_rat_types)) {
			ogs_error(
				"OpenAPI_lmf_info_parseFromJSON() failed [serving_rat_types]");
			goto end;
		}
		...
		if(serving_rat_typesList->count == 0) {
			ogs_error(
				"OpenAPI_lmf_info_parseFromJSON() failed: Expected serving_rat_typesList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(tai_list) {
		...
		if(!cJSON_IsArray(tai_list)) {
			ogs_error(
				"OpenAPI_lmf_info_parseFromJSON() failed [tai_list]");
			goto end;
		}
	...
	tai_range_list = cJSON_GetObjectItemCaseSensitive(lmf_infoJSON,
	                                                  "taiRangeList");
	if(tai_range_list) {
		cJSON *tai_range_list_local = NULL;
		if(!cJSON_IsArray(tai_range_list)) {
			ogs_error(
				"OpenAPI_lmf_info_parseFromJSON() failed [tai_range_list]");
			goto end;
		}
	...
	supported_gad_shapes = cJSON_GetObjectItemCaseSensitive(lmf_infoJSON,
	                                                        "supportedGADShapes");
	if(supported_gad_shapes) {
		cJSON *supported_gad_shapes_local = NULL;
		if(!cJSON_IsArray(supported_gad_shapes)) {
			ogs_error(
				"OpenAPI_lmf_info_parseFromJSON() failed [supported_gad_shapes]");
			goto end;
		}
		...
		if(supported_gad_shapesList->count == 0) {
			ogs_error(
				"OpenAPI_lmf_info_parseFromJSON() failed: Expected supported_gad_shapesList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}

	pru_existence_info = cJSON_GetObjectItemCaseSensitive(lmf_infoJSON,
	                                                      "pruExistenceInfo");
	...
	pru_support_ind = cJSON_GetObjectItemCaseSensitive(lmf_infoJSON,
	                                                   "pruSupportInd");
	if(pru_support_ind) {
		if(!cJSON_IsBool(pru_support_ind)) {
			ogs_error(
				"OpenAPI_lmf_info_parseFromJSON() failed [pru_support_ind]");
			goto end;
		}
	}

	rangingslpos_support_ind =
		cJSON_GetObjectItemCaseSensitive(lmf_infoJSON,
		                                 "rangingslposSupportInd");
	if(rangingslpos_support_ind) {
		if(!cJSON_IsBool(rangingslpos_support_ind)) {
			ogs_error(
				"OpenAPI_lmf_info_parseFromJSON() failed [rangingslpos_support_ind]");
			goto end;
		}
	}

	up_positioning_ind = cJSON_GetObjectItemCaseSensitive(lmf_infoJSON,
	                                                      "upPositioningInd");
	if(up_positioning_ind) {
		if(!cJSON_IsBool(up_positioning_ind)) {
			ogs_error(
				"OpenAPI_lmf_info_parseFromJSON() failed [up_positioning_ind]");
			goto end;
		}
	}

	aiml_pos_ind = cJSON_GetObjectItemCaseSensitive(lmf_infoJSON,
	                                                "aimlPosInd");
	if(aiml_pos_ind) {
		if(!cJSON_IsString(aiml_pos_ind)) {
			ogs_error(
				"OpenAPI_lmf_info_parseFromJSON() failed [aiml_pos_ind]");
			goto end;
		}
		aiml_pos_indVariable =
			OpenAPI_aiml_pos_indlmf_info_FromString(
	...
	lmf_info_local_var = OpenAPI_lmf_info_create(
		...
		lmf_id &&
		!cJSON_IsNull(lmf_id) ? ogs_strdup(lmf_id->valuestring) : NULL,
		...
		rangingslpos_support_ind ? rangingslpos_support_ind->valueint :
		0,
...
OpenAPI_lmf_info_t *OpenAPI_lmf_info_copy(OpenAPI_lmf_info_t	*dst,
                                          OpenAPI_lmf_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
