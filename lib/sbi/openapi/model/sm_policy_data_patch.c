...
OpenAPI_sm_policy_data_patch_t *OpenAPI_sm_policy_data_patch_create(
	bool is_um_data_null, OpenAPI_list_t *um_data,
	OpenAPI_list_t *sm_policy_snssai_data) {
	OpenAPI_sm_policy_data_patch_t *sm_policy_data_patch_local_var =
	...
	sm_policy_data_patch_local_var->sm_policy_snssai_data =
...
void OpenAPI_sm_policy_data_patch_free(
	OpenAPI_sm_policy_data_patch_t *sm_policy_data_patch) {
	...
	if(sm_policy_data_patch->sm_policy_snssai_data) {
		OpenAPI_list_for_each(
			sm_policy_data_patch->sm_policy_snssai_data, node) {
			OpenAPI_map_t *localKeyValue =
				(OpenAPI_map_t *) node->data;
			ogs_free(localKeyValue->key);
			OpenAPI_sm_policy_snssai_data_patch_free(
				localKeyValue->value);
			OpenAPI_map_free(localKeyValue);
		}
...
cJSON *OpenAPI_sm_policy_data_patch_convertToJSON(
	OpenAPI_sm_policy_data_patch_t *sm_policy_data_patch) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(sm_policy_data_patch == NULL) {
		ogs_error(
			"OpenAPI_sm_policy_data_patch_convertToJSON() failed [SmPolicyDataPatch]");
		return NULL;
	}
	...
	if(sm_policy_data_patch->um_data) {
		...
		if(um_data == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_data_patch_convertToJSON() failed [um_data]");
			goto end;
		}
		cJSON *localMapObject = um_data;
		if(sm_policy_data_patch->um_data) {
			OpenAPI_list_for_each(sm_policy_data_patch->um_data,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_sm_policy_data_patch_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(sm_policy_data_patch->sm_policy_snssai_data) {
		...
		if(sm_policy_snssai_data == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_data_patch_convertToJSON() failed [sm_policy_snssai_data]");
			goto end;
		}
		cJSON *localMapObject = sm_policy_snssai_data;
		if(sm_policy_data_patch->sm_policy_snssai_data) {
			OpenAPI_list_for_each(
				sm_policy_data_patch->sm_policy_snssai_data,
				node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				if(localKeyValue == NULL) {
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_sm_policy_snssai_data_patch_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
...
OpenAPI_sm_policy_data_patch_t *OpenAPI_sm_policy_data_patch_parseFromJSON(
	cJSON *sm_policy_data_patchJSON) {
	...
	um_data = cJSON_GetObjectItemCaseSensitive(sm_policy_data_patchJSON,
	                                           "umData");
	if(um_data) {
		if(!cJSON_IsNull(um_data)) {
			...
			{
				ogs_error(
					"OpenAPI_sm_policy_data_patch_parseFromJSON() failed [um_data]");
				goto end;
			}
			if(cJSON_IsObject(um_data)) {
				...
				cJSON_ArrayForEach(um_data_local_map, um_data) {
							  ...
							  localMapObject))
					{
						...
						ogs_error(
							"OpenAPI_sm_policy_data_patch_parseFromJSON() failed [inner]");
						goto end;
					}
					OpenAPI_list_add(um_dataList,
					                 localMapKeyPair);
				}
			}
		}
	...
	sm_policy_snssai_data =
		cJSON_GetObjectItemCaseSensitive(sm_policy_data_patchJSON,
		                                 "smPolicySnssaiData");
	if(sm_policy_snssai_data) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_data_patch_parseFromJSON() failed [sm_policy_snssai_data]");
			goto end;
		}
		if(cJSON_IsObject(sm_policy_snssai_data)) {
			...
			cJSON_ArrayForEach(sm_policy_snssai_data_local_map,
			                   sm_policy_snssai_data) {
				cJSON *localMapObject =
					sm_policy_snssai_data_local_map;
				if(cJSON_IsObject(localMapObject)) {
	...
	sm_policy_data_patch_local_var = OpenAPI_sm_policy_data_patch_create(
		um_data &&
		cJSON_IsNull(um_data) ? true : false,
...
OpenAPI_sm_policy_data_patch_t *OpenAPI_sm_policy_data_patch_copy(
	OpenAPI_sm_policy_data_patch_t	*dst,
	OpenAPI_sm_policy_data_patch_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_sm_policy_data_patch_convertToJSON() failed");
		return NULL;
	}
