...
{
	OpenAPI_pp_active_time_t *pp_active_time_local_var =
		ogs_malloc(sizeof(OpenAPI_pp_active_time_t));
	...
	pp_active_time_local_var->mtc_provider_information =
		mtc_provider_information;
...
cJSON *OpenAPI_pp_active_time_convertToJSON(
	...
	if(pp_active_time == NULL) {
		ogs_error(
			"OpenAPI_pp_active_time_convertToJSON() failed [PpActiveTime]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(cJSON_AddNumberToObject(item, "activeTime",
	                           pp_active_time->active_time) == NULL)
	{
		ogs_error(
			"OpenAPI_pp_active_time_convertToJSON() failed [active_time]");
		goto end;
	}

	if(!pp_active_time->af_instance_id) {
		ogs_error(
			"OpenAPI_pp_active_time_convertToJSON() failed [af_instance_id]");
		return NULL;
	}
	...
	if(cJSON_AddNumberToObject(item, "referenceId",
	                           pp_active_time->reference_id) == NULL)
	{
		ogs_error(
			"OpenAPI_pp_active_time_convertToJSON() failed [reference_id]");
		goto end;
	}

	if(pp_active_time->validity_time) {
		if(cJSON_AddStringToObject(item, "validityTime",
		                           pp_active_time->validity_time) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pp_active_time_convertToJSON() failed [validity_time]");
			goto end;
		}
	}
	...
	if(pp_active_time->mtc_provider_information) {
		...
		{
			ogs_error(
				"OpenAPI_pp_active_time_convertToJSON() failed [mtc_provider_information]");
			goto end;
		}
	}
...
OpenAPI_pp_active_time_t *OpenAPI_pp_active_time_parseFromJSON(
	...
	active_time = cJSON_GetObjectItemCaseSensitive(pp_active_timeJSON,
	                                               "activeTime");
	if(!active_time) {
		ogs_error(
			"OpenAPI_pp_active_time_parseFromJSON() failed [active_time]");
		goto end;
	}
	...
	af_instance_id = cJSON_GetObjectItemCaseSensitive(pp_active_timeJSON,
	                                                  "afInstanceId");
	if(!af_instance_id) {
		ogs_error(
			"OpenAPI_pp_active_time_parseFromJSON() failed [af_instance_id]");
		goto end;
	}
	...
	reference_id = cJSON_GetObjectItemCaseSensitive(pp_active_timeJSON,
	                                                "referenceId");
	if(!reference_id) {
		ogs_error(
			"OpenAPI_pp_active_time_parseFromJSON() failed [reference_id]");
		goto end;
	}
	...
	validity_time = cJSON_GetObjectItemCaseSensitive(pp_active_timeJSON,
	                                                 "validityTime");
	if(validity_time) {
		...
		{
			ogs_error(
				"OpenAPI_pp_active_time_parseFromJSON() failed [validity_time]");
			goto end;
		}
	}

	mtc_provider_information =
		cJSON_GetObjectItemCaseSensitive(pp_active_timeJSON,
	...
	if(mtc_provider_information) {
		...
		{
			ogs_error(
				"OpenAPI_pp_active_time_parseFromJSON() failed [mtc_provider_information]");
			goto end;
		}
	}
