...
OpenAPI_parameter_combination_t *OpenAPI_parameter_combination_create(
	...
	OpenAPI_parameter_combination_t *parameter_combination_local_var =
		ogs_malloc(sizeof(OpenAPI_parameter_combination_t));
...
cJSON *OpenAPI_parameter_combination_convertToJSON(
	...
	if(parameter_combination == NULL) {
		ogs_error(
			"OpenAPI_parameter_combination_convertToJSON() failed [ParameterCombination]");
		return NULL;
	}
	...
	if(parameter_combination->supi) {
		...
		{
			ogs_error(
				"OpenAPI_parameter_combination_convertToJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(parameter_combination->dnn) {
		...
		{
			ogs_error(
				"OpenAPI_parameter_combination_convertToJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(parameter_combination->snssai) {
		cJSON *snssai_local_JSON =
			OpenAPI_snssai_convertToJSON(
				parameter_combination->snssai);
		if(snssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_parameter_combination_convertToJSON() failed [snssai]");
			goto end;
		}
...
OpenAPI_parameter_combination_t *OpenAPI_parameter_combination_parseFromJSON(
	...
	supi = cJSON_GetObjectItemCaseSensitive(parameter_combinationJSON,
	                                        "supi");
	if(supi) {
		...
		{
			ogs_error(
				"OpenAPI_parameter_combination_parseFromJSON() failed [supi]");
			goto end;
		}
	}

	dnn = cJSON_GetObjectItemCaseSensitive(parameter_combinationJSON,
	                                       "dnn");
	if(dnn) {
		...
		{
			ogs_error(
				"OpenAPI_parameter_combination_parseFromJSON() failed [dnn]");
			goto end;
		}
	}

	snssai = cJSON_GetObjectItemCaseSensitive(parameter_combinationJSON,
	                                          "snssai");
	if(snssai) {
...
OpenAPI_parameter_combination_t *OpenAPI_parameter_combination_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_parameter_combination_convertToJSON() failed");
		return NULL;
	}
