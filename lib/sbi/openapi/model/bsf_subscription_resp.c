...
OpenAPI_bsf_subscription_resp_t *OpenAPI_bsf_subscription_resp_create(
	OpenAPI_list_t *events, char *notif_uri, char *notif_corre_id,
	char *supi, char *gpsi, OpenAPI_snssai_dnn_pair_t *snssai_dnn_pairs,
	OpenAPI_list_t *add_snssai_dnn_pairs, char *supp_feat, char *pcf_id,
	char *pcf_set_id, OpenAPI_binding_level_e bind_level,
	OpenAPI_list_t *event_notifs) {
	OpenAPI_bsf_subscription_resp_t *bsf_subscription_resp_local_var =
		ogs_malloc(sizeof(OpenAPI_bsf_subscription_resp_t));
	...
	bsf_subscription_resp_local_var->add_snssai_dnn_pairs =
...
void OpenAPI_bsf_subscription_resp_free(
	OpenAPI_bsf_subscription_resp_t *bsf_subscription_resp) {
	...
	if(bsf_subscription_resp->snssai_dnn_pairs) {
		OpenAPI_snssai_dnn_pair_free(
			bsf_subscription_resp->snssai_dnn_pairs);
		bsf_subscription_resp->snssai_dnn_pairs = NULL;
	}
	if(bsf_subscription_resp->add_snssai_dnn_pairs) {
		OpenAPI_list_for_each(
			bsf_subscription_resp->add_snssai_dnn_pairs, node) {
	...
	if(bsf_subscription_resp->event_notifs) {
		OpenAPI_list_for_each(bsf_subscription_resp->event_notifs,
		                      node) {
...
cJSON *OpenAPI_bsf_subscription_resp_convertToJSON(
	OpenAPI_bsf_subscription_resp_t *bsf_subscription_resp) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(bsf_subscription_resp == NULL) {
		ogs_error(
			"OpenAPI_bsf_subscription_resp_convertToJSON() failed [BsfSubscriptionResp]");
		return NULL;
	}
	...
	if(bsf_subscription_resp->events == OpenAPI_bsf_event_NULL) {
		ogs_error(
			"OpenAPI_bsf_subscription_resp_convertToJSON() failed [events]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(bsf_subscription_resp->events, node) {
		if(cJSON_AddStringToObject(eventsList, "",
		                           OpenAPI_bsf_event_ToString(
						   (intptr_t) node->data)) ==
		   NULL)
		{
			ogs_error(
	...
	if(!bsf_subscription_resp->notif_uri) {
		ogs_error(
			"OpenAPI_bsf_subscription_resp_convertToJSON() failed [notif_uri]");
		return NULL;
	}
	...
	if(!bsf_subscription_resp->notif_corre_id) {
		ogs_error(
			"OpenAPI_bsf_subscription_resp_convertToJSON() failed [notif_corre_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "notifCorreId",
	                           bsf_subscription_resp->notif_corre_id) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_bsf_subscription_resp_convertToJSON() failed [notif_corre_id]");
	...
	if(!bsf_subscription_resp->supi) {
		ogs_error(
			"OpenAPI_bsf_subscription_resp_convertToJSON() failed [supi]");
		return NULL;
	}
	...
	if(bsf_subscription_resp->gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_bsf_subscription_resp_convertToJSON() failed [gpsi]");
			goto end;
		}
	}

	if(bsf_subscription_resp->snssai_dnn_pairs) {
		cJSON *snssai_dnn_pairs_local_JSON =
			OpenAPI_snssai_dnn_pair_convertToJSON(
				bsf_subscription_resp->snssai_dnn_pairs);
		if(snssai_dnn_pairs_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_bsf_subscription_resp_convertToJSON() failed [snssai_dnn_pairs]");
			goto end;
		}
		cJSON_AddItemToObject(item, "snssaiDnnPairs",
		                      snssai_dnn_pairs_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_bsf_subscription_resp_convertToJSON() failed [snssai_dnn_pairs]");
			goto end;
	...
	if(bsf_subscription_resp->add_snssai_dnn_pairs) {
		cJSON *add_snssai_dnn_pairsList = cJSON_AddArrayToObject(item,
		                                                         "addSnssaiDnnPairs");
		if(add_snssai_dnn_pairsList == NULL) {
			ogs_error(
				"OpenAPI_bsf_subscription_resp_convertToJSON() failed [add_snssai_dnn_pairs]");
			goto end;
		}
		OpenAPI_list_for_each(
			bsf_subscription_resp->add_snssai_dnn_pairs, node) {
			...
			cJSON_AddItemToArray(add_snssai_dnn_pairsList,
			                     itemLocal);
		}
	}
	...
	if(bsf_subscription_resp->supp_feat) {
		if(cJSON_AddStringToObject(item, "suppFeat",
		                           bsf_subscription_resp->supp_feat) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_bsf_subscription_resp_convertToJSON() failed [supp_feat]");
			goto end;
		}
	}
	...
	if(bsf_subscription_resp->pcf_id) {
		if(cJSON_AddStringToObject(item, "pcfId",
		                           bsf_subscription_resp->pcf_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_bsf_subscription_resp_convertToJSON() failed [pcf_id]");
			goto end;
		}
	}
	...
	if(bsf_subscription_resp->pcf_set_id) {
		if(cJSON_AddStringToObject(item, "pcfSetId",
		                           bsf_subscription_resp->pcf_set_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_bsf_subscription_resp_convertToJSON() failed [pcf_set_id]");
			goto end;
		}
	}
	...
	if(bsf_subscription_resp->bind_level != OpenAPI_binding_level_NULL) {
		if(cJSON_AddStringToObject(item, "bindLevel",
						   ...
						   bsf_subscription_resp->
						   bind_level)) == NULL)
		{
			ogs_error(
				"OpenAPI_bsf_subscription_resp_convertToJSON() failed [bind_level]");
			goto end;
		}
	}
	...
	if(!bsf_subscription_resp->event_notifs) {
		ogs_error(
			"OpenAPI_bsf_subscription_resp_convertToJSON() failed [event_notifs]");
		return NULL;
	}
...
OpenAPI_bsf_subscription_resp_t *OpenAPI_bsf_subscription_resp_parseFromJSON(
	cJSON *bsf_subscription_respJSON) {
	...
	events = cJSON_GetObjectItemCaseSensitive(bsf_subscription_respJSON,
	                                          "events");
	if(!events) {
		ogs_error(
			"OpenAPI_bsf_subscription_resp_parseFromJSON() failed [events]");
		goto end;
	}
	...
	cJSON_ArrayForEach(events_local, events) {
		...
		localEnum =
			OpenAPI_bsf_event_FromString(events_local->valuestring);
		if(!localEnum) {
			ogs_info(
				"Enum value \"%s\" for field \"events\" is not supported. Ignoring it ...",
	...
	if(eventsList->count == 0) {
		ogs_error(
			"OpenAPI_bsf_subscription_resp_parseFromJSON() failed: Expected eventsList to not be empty (after ignoring unsupported enum values).");
		goto end;
	}

	notif_uri = cJSON_GetObjectItemCaseSensitive(bsf_subscription_respJSON,
	                                             "notifUri");
	if(!notif_uri) {
		ogs_error(
			"OpenAPI_bsf_subscription_resp_parseFromJSON() failed [notif_uri]");
		goto end;
	}
	...
	notif_corre_id =
		cJSON_GetObjectItemCaseSensitive(bsf_subscription_respJSON,
		                                 "notifCorreId");
	if(!notif_corre_id) {
		ogs_error(
			"OpenAPI_bsf_subscription_resp_parseFromJSON() failed [notif_corre_id]");
		goto end;
	}
	...
	supi = cJSON_GetObjectItemCaseSensitive(bsf_subscription_respJSON,
	                                        "supi");
	if(!supi) {
		ogs_error(
			"OpenAPI_bsf_subscription_resp_parseFromJSON() failed [supi]");
		goto end;
	}
	...
	gpsi = cJSON_GetObjectItemCaseSensitive(bsf_subscription_respJSON,
	                                        "gpsi");
	if(gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_bsf_subscription_resp_parseFromJSON() failed [gpsi]");
			goto end;
		}
	}
	...
	snssai_dnn_pairs =
		cJSON_GetObjectItemCaseSensitive(bsf_subscription_respJSON,
		                                 "snssaiDnnPairs");
	if(snssai_dnn_pairs) {
		snssai_dnn_pairs_local_nonprim =
			OpenAPI_snssai_dnn_pair_parseFromJSON(snssai_dnn_pairs);
		if(!snssai_dnn_pairs_local_nonprim) {
			ogs_error(
				"OpenAPI_snssai_dnn_pair_parseFromJSON failed [snssai_dnn_pairs]");
			goto end;
		}
	}
	...
	if(add_snssai_dnn_pairs) {
		...
		if(!cJSON_IsArray(add_snssai_dnn_pairs)) {
			ogs_error(
				"OpenAPI_bsf_subscription_resp_parseFromJSON() failed [add_snssai_dnn_pairs]");
			goto end;
		}

		add_snssai_dnn_pairsList = OpenAPI_list_create();

		cJSON_ArrayForEach(add_snssai_dnn_pairs_local,
		                   add_snssai_dnn_pairs) {
			...
			OpenAPI_snssai_dnn_pair_t *add_snssai_dnn_pairsItem =
				OpenAPI_snssai_dnn_pair_parseFromJSON(
					add_snssai_dnn_pairs_local);
			if(!add_snssai_dnn_pairsItem) {
				ogs_error("No add_snssai_dnn_pairsItem");
				goto end;
			}
			OpenAPI_list_add(add_snssai_dnn_pairsList,
			                 add_snssai_dnn_pairsItem);
		}
	}

	supp_feat = cJSON_GetObjectItemCaseSensitive(bsf_subscription_respJSON,
	                                             "suppFeat");
	if(supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_bsf_subscription_resp_parseFromJSON() failed [supp_feat]");
			goto end;
		}
	}

	pcf_id = cJSON_GetObjectItemCaseSensitive(bsf_subscription_respJSON,
	                                          "pcfId");
	if(pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_bsf_subscription_resp_parseFromJSON() failed [pcf_id]");
			goto end;
		}
	}

	pcf_set_id = cJSON_GetObjectItemCaseSensitive(bsf_subscription_respJSON,
	                                              "pcfSetId");
	if(pcf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_bsf_subscription_resp_parseFromJSON() failed [pcf_set_id]");
			goto end;
		}
	}

	bind_level = cJSON_GetObjectItemCaseSensitive(bsf_subscription_respJSON,
	                                              "bindLevel");
	if(bind_level) {
		if(!cJSON_IsString(bind_level)) {
			ogs_error(
				"OpenAPI_bsf_subscription_resp_parseFromJSON() failed [bind_level]");
			goto end;
		}
		bind_levelVariable =
	...
	if(!event_notifs) {
		ogs_error(
			"OpenAPI_bsf_subscription_resp_parseFromJSON() failed [event_notifs]");
		goto end;
	}
	...
	cJSON_ArrayForEach(event_notifs_local, event_notifs) {
		...
		OpenAPI_bsf_event_notification_t *event_notifsItem =
			OpenAPI_bsf_event_notification_parseFromJSON(
				event_notifs_local);
		if(!event_notifsItem) {
...
OpenAPI_bsf_subscription_resp_t *OpenAPI_bsf_subscription_resp_copy(
	OpenAPI_bsf_subscription_resp_t *dst,
	OpenAPI_bsf_subscription_resp_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_bsf_subscription_resp_convertToJSON() failed");
		return NULL;
	}
