...
OpenAPI_acc_net_ch_id_t *OpenAPI_acc_net_ch_id_create(
	...
	char *acc_net_charg_id, OpenAPI_list_t *ref_pcc_rule_ids,
	bool is_session_ch_scope, int session_ch_scope) {
	OpenAPI_acc_net_ch_id_t *acc_net_ch_id_local_var =
		ogs_malloc(sizeof(OpenAPI_acc_net_ch_id_t));
	ogs_assert(acc_net_ch_id_local_var);

	acc_net_ch_id_local_var->is_acc_net_cha_id_value =
...
cJSON *OpenAPI_acc_net_ch_id_convertToJSON(
	...
	if(acc_net_ch_id == NULL) {
		ogs_error(
			"OpenAPI_acc_net_ch_id_convertToJSON() failed [AccNetChId]");
		return NULL;
	}
	...
	if(acc_net_ch_id->is_acc_net_cha_id_value) {
		if(cJSON_AddNumberToObject(item, "accNetChaIdValue",
		                           acc_net_ch_id->acc_net_cha_id_value)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_acc_net_ch_id_convertToJSON() failed [acc_net_cha_id_value]");
			goto end;
		}
	}

	if(acc_net_ch_id->acc_net_charg_id) {
		if(cJSON_AddStringToObject(item, "accNetChargId",
		                           acc_net_ch_id->acc_net_charg_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_acc_net_ch_id_convertToJSON() failed [acc_net_charg_id]");
			goto end;
		}
	}

	if(acc_net_ch_id->ref_pcc_rule_ids) {
		cJSON *ref_pcc_rule_idsList = cJSON_AddArrayToObject(item,
		                                                     "refPccRuleIds");
		if(ref_pcc_rule_idsList == NULL) {
			ogs_error(
				"OpenAPI_acc_net_ch_id_convertToJSON() failed [ref_pcc_rule_ids]");
			goto end;
		}
		OpenAPI_list_for_each(acc_net_ch_id->ref_pcc_rule_ids, node) {
			if(cJSON_AddStringToObject(ref_pcc_rule_idsList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_acc_net_ch_id_convertToJSON() failed [ref_pcc_rule_ids]");
	...
	if(acc_net_ch_id->is_session_ch_scope) {
		if(cJSON_AddBoolToObject(item, "sessionChScope",
		                         acc_net_ch_id->session_ch_scope) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_acc_net_ch_id_convertToJSON() failed [session_ch_scope]");
			goto end;
		}
	}
...
OpenAPI_acc_net_ch_id_t *OpenAPI_acc_net_ch_id_parseFromJSON(
	cJSON *acc_net_ch_idJSON) {
	...
	acc_net_cha_id_value =
		cJSON_GetObjectItemCaseSensitive(acc_net_ch_idJSON,
		                                 "accNetChaIdValue");
	if(acc_net_cha_id_value) {
		if(!cJSON_IsNumber(acc_net_cha_id_value)) {
			ogs_error(
				"OpenAPI_acc_net_ch_id_parseFromJSON() failed [acc_net_cha_id_value]");
			goto end;
		}
	}

	acc_net_charg_id = cJSON_GetObjectItemCaseSensitive(acc_net_ch_idJSON,
	                                                    "accNetChargId");
	if(acc_net_charg_id) {
		if(!cJSON_IsString(acc_net_charg_id) &&
		   !cJSON_IsNull(acc_net_charg_id))
		{
			ogs_error(
				"OpenAPI_acc_net_ch_id_parseFromJSON() failed [acc_net_charg_id]");
			goto end;
		}
	}

	ref_pcc_rule_ids = cJSON_GetObjectItemCaseSensitive(acc_net_ch_idJSON,
	                                                    "refPccRuleIds");
	if(ref_pcc_rule_ids) {
		cJSON *ref_pcc_rule_ids_local = NULL;
		if(!cJSON_IsArray(ref_pcc_rule_ids)) {
			ogs_error(
				"OpenAPI_acc_net_ch_id_parseFromJSON() failed [ref_pcc_rule_ids]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ref_pcc_rule_ids_local, ref_pcc_rule_ids) {
			...
			OpenAPI_list_add(ref_pcc_rule_idsList,
			                 ogs_strdup(
						 ref_pcc_rule_ids_local->
						 valuestring));
		}
	}

	session_ch_scope = cJSON_GetObjectItemCaseSensitive(acc_net_ch_idJSON,
	                                                    "sessionChScope");
	if(session_ch_scope) {
		if(!cJSON_IsBool(session_ch_scope)) {
			ogs_error(
				"OpenAPI_acc_net_ch_id_parseFromJSON() failed [session_ch_scope]");
			goto end;
		}
	}
	...
	acc_net_ch_id_local_var = OpenAPI_acc_net_ch_id_create(
		...
		acc_net_charg_id &&
		!cJSON_IsNull(acc_net_charg_id) ?
		ogs_strdup(acc_net_charg_id->valuestring) : NULL,
...
OpenAPI_acc_net_ch_id_t *OpenAPI_acc_net_ch_id_copy(
	OpenAPI_acc_net_ch_id_t *dst, OpenAPI_acc_net_ch_id_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
