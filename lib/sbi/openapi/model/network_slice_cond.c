...
OpenAPI_network_slice_cond_t *OpenAPI_network_slice_cond_create(
	OpenAPI_list_t *snssai_list, OpenAPI_list_t *nsi_list) {
	OpenAPI_network_slice_cond_t *network_slice_cond_local_var =
		ogs_malloc(sizeof(OpenAPI_network_slice_cond_t));
...
void OpenAPI_network_slice_cond_free(
...
cJSON *OpenAPI_network_slice_cond_convertToJSON(
	OpenAPI_network_slice_cond_t *network_slice_cond) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(network_slice_cond == NULL) {
		ogs_error(
			"OpenAPI_network_slice_cond_convertToJSON() failed [NetworkSliceCond]");
		return NULL;
	}
	...
	if(!network_slice_cond->snssai_list) {
		ogs_error(
			"OpenAPI_network_slice_cond_convertToJSON() failed [snssai_list]");
		return NULL;
	}
	...
	if(network_slice_cond->nsi_list) {
		...
		if(nsi_listList == NULL) {
			ogs_error(
				"OpenAPI_network_slice_cond_convertToJSON() failed [nsi_list]");
			goto end;
		}
		OpenAPI_list_for_each(network_slice_cond->nsi_list, node) {
...
OpenAPI_network_slice_cond_t *OpenAPI_network_slice_cond_parseFromJSON(
	...
	snssai_list = cJSON_GetObjectItemCaseSensitive(network_slice_condJSON,
	                                               "snssaiList");
	if(!snssai_list) {
		ogs_error(
			"OpenAPI_network_slice_cond_parseFromJSON() failed [snssai_list]");
		goto end;
	}
	...
	nsi_list = cJSON_GetObjectItemCaseSensitive(network_slice_condJSON,
	                                            "nsiList");
	if(nsi_list) {
		cJSON *nsi_list_local = NULL;
		if(!cJSON_IsArray(nsi_list)) {
			ogs_error(
				"OpenAPI_network_slice_cond_parseFromJSON() failed [nsi_list]");
			goto end;
		}
...
OpenAPI_network_slice_cond_t *OpenAPI_network_slice_cond_copy(
	OpenAPI_network_slice_cond_t *dst, OpenAPI_network_slice_cond_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
