...
OpenAPI_eth_flow_description_t *OpenAPI_eth_flow_description_create(
	char *dest_mac_addr, char *eth_type, char *f_desc,
	OpenAPI_flow_direction_e f_dir, char *source_mac_addr,
	OpenAPI_list_t *vlan_tags, char *src_mac_addr_end,
	char *dest_mac_addr_end) {
	OpenAPI_eth_flow_description_t *eth_flow_description_local_var =
		ogs_malloc(sizeof(OpenAPI_eth_flow_description_t));
...
void OpenAPI_eth_flow_description_free(
...
cJSON *OpenAPI_eth_flow_description_convertToJSON(
	OpenAPI_eth_flow_description_t *eth_flow_description) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(eth_flow_description == NULL) {
		ogs_error(
			"OpenAPI_eth_flow_description_convertToJSON() failed [EthFlowDescription]");
		return NULL;
	}
	...
	if(eth_flow_description->dest_mac_addr) {
		...
		{
			ogs_error(
				"OpenAPI_eth_flow_description_convertToJSON() failed [dest_mac_addr]");
			goto end;
		}
	}
	...
	if(!eth_flow_description->eth_type) {
		ogs_error(
			"OpenAPI_eth_flow_description_convertToJSON() failed [eth_type]");
		return NULL;
	}
	...
	if(eth_flow_description->f_desc) {
		...
		{
			ogs_error(
				"OpenAPI_eth_flow_description_convertToJSON() failed [f_desc]");
			goto end;
		}
	}

	if(eth_flow_description->f_dir != OpenAPI_flow_direction_NULL) {
		if(cJSON_AddStringToObject(item, "fDir",
		                           OpenAPI_flow_direction_ToString(
						   eth_flow_description->f_dir))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_eth_flow_description_convertToJSON() failed [f_dir]");
			goto end;
		}
	}
	...
	if(eth_flow_description->source_mac_addr) {
		...
		{
			ogs_error(
				"OpenAPI_eth_flow_description_convertToJSON() failed [source_mac_addr]");
			goto end;
		}
	}
	...
	if(eth_flow_description->vlan_tags) {
		...
		if(vlan_tagsList == NULL) {
			ogs_error(
				"OpenAPI_eth_flow_description_convertToJSON() failed [vlan_tags]");
			goto end;
		}
		OpenAPI_list_for_each(eth_flow_description->vlan_tags, node) {
	...
	if(eth_flow_description->src_mac_addr_end) {
		...
		{
			ogs_error(
				"OpenAPI_eth_flow_description_convertToJSON() failed [src_mac_addr_end]");
			goto end;
		}
	}
	...
	if(eth_flow_description->dest_mac_addr_end) {
		...
		{
			ogs_error(
				"OpenAPI_eth_flow_description_convertToJSON() failed [dest_mac_addr_end]");
			goto end;
		}
	}
...
OpenAPI_eth_flow_description_t *OpenAPI_eth_flow_description_parseFromJSON(
	...
	dest_mac_addr =
		cJSON_GetObjectItemCaseSensitive(eth_flow_descriptionJSON,
		                                 "destMacAddr");
	if(dest_mac_addr) {
		...
		{
			ogs_error(
				"OpenAPI_eth_flow_description_parseFromJSON() failed [dest_mac_addr]");
			goto end;
		}
	}
	...
	if(!eth_type) {
		ogs_error(
			"OpenAPI_eth_flow_description_parseFromJSON() failed [eth_type]");
		goto end;
	}
	...
	if(f_desc) {
		...
		{
			ogs_error(
				"OpenAPI_eth_flow_description_parseFromJSON() failed [f_desc]");
			goto end;
		}
	}
	...
	if(f_dir) {
		if(!cJSON_IsString(f_dir)) {
			ogs_error(
				"OpenAPI_eth_flow_description_parseFromJSON() failed [f_dir]");
			goto end;
		}
		f_dirVariable =
			OpenAPI_flow_direction_FromString(f_dir->valuestring);
	}
	...
	if(source_mac_addr) {
		...
		{
			ogs_error(
				"OpenAPI_eth_flow_description_parseFromJSON() failed [source_mac_addr]");
			goto end;
		}
	}
	...
	if(vlan_tags) {
		...
		if(!cJSON_IsArray(vlan_tags)) {
			ogs_error(
				"OpenAPI_eth_flow_description_parseFromJSON() failed [vlan_tags]");
			goto end;
		}
		...
		cJSON_ArrayForEach(vlan_tags_local, vlan_tags) {
			...
			OpenAPI_list_add(vlan_tagsList,
			                 ogs_strdup(
						 vlan_tags_local->valuestring));
		}
	}
	...
	if(src_mac_addr_end) {
		...
		{
			ogs_error(
				"OpenAPI_eth_flow_description_parseFromJSON() failed [src_mac_addr_end]");
			goto end;
		}
	}
	...
	if(dest_mac_addr_end) {
		...
		{
			ogs_error(
				"OpenAPI_eth_flow_description_parseFromJSON() failed [dest_mac_addr_end]");
			goto end;
		}
	}
	...
	eth_flow_description_local_var = OpenAPI_eth_flow_description_create(
		dest_mac_addr &&
		!cJSON_IsNull(dest_mac_addr) ?
		ogs_strdup(dest_mac_addr->valuestring) : NULL,
		ogs_strdup(eth_type->valuestring),
		f_desc &&
		!cJSON_IsNull(f_desc) ? ogs_strdup(f_desc->valuestring) : NULL,
		f_dir ? f_dirVariable : 0,
		source_mac_addr &&
		!cJSON_IsNull(source_mac_addr) ?
		ogs_strdup(source_mac_addr->valuestring) : NULL,
		vlan_tags ? vlan_tagsList : NULL,
		src_mac_addr_end &&
		!cJSON_IsNull(src_mac_addr_end) ?
		ogs_strdup(src_mac_addr_end->valuestring) : NULL,
		dest_mac_addr_end &&
		!cJSON_IsNull(dest_mac_addr_end) ?
		ogs_strdup(dest_mac_addr_end->valuestring) : NULL
		);
...
OpenAPI_eth_flow_description_t *OpenAPI_eth_flow_description_copy(
	OpenAPI_eth_flow_description_t	*dst,
	OpenAPI_eth_flow_description_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_eth_flow_description_convertToJSON() failed");
		return NULL;
	}
