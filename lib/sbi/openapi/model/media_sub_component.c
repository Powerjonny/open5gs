...
OpenAPI_media_sub_component_t *OpenAPI_media_sub_component_create(
	...
	OpenAPI_media_sub_component_t *media_sub_component_local_var =
		ogs_malloc(sizeof(OpenAPI_media_sub_component_t));
...
cJSON *OpenAPI_media_sub_component_convertToJSON(
	...
	if(media_sub_component == NULL) {
		ogs_error(
			"OpenAPI_media_sub_component_convertToJSON() failed [MediaSubComponent]");
		return NULL;
	}
	   ...
	   OpenAPI_af_sig_protocol_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_media_sub_component_convertToJSON() failed [af_sig_protocol]");
			goto end;
		}
	}
	...
	if(media_sub_component->ethf_descs) {
		...
		if(ethf_descsList == NULL) {
			ogs_error(
				"OpenAPI_media_sub_component_convertToJSON() failed [ethf_descs]");
			goto end;
		}
		OpenAPI_list_for_each(media_sub_component->ethf_descs, node) {
			cJSON *itemLocal =
	...
	if(cJSON_AddNumberToObject(item, "fNum",
	                           media_sub_component->f_num) == NULL)
	{
		ogs_error(
			"OpenAPI_media_sub_component_convertToJSON() failed [f_num]");
		goto end;
	}
	...
	if(media_sub_component->f_descs) {
		...
		if(f_descsList == NULL) {
			ogs_error(
				"OpenAPI_media_sub_component_convertToJSON() failed [f_descs]");
			goto end;
		}
		OpenAPI_list_for_each(media_sub_component->f_descs, node) {
	...
	if(media_sub_component->f_status != OpenAPI_flow_status_NULL) {
		if(cJSON_AddStringToObject(item, "fStatus",
						   ...
						   media_sub_component->f_status))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_media_sub_component_convertToJSON() failed [f_status]");
			goto end;
		}
	}
	...
	if(media_sub_component->mar_bw_dl) {
		if(cJSON_AddStringToObject(item, "marBwDl",
		                           media_sub_component->mar_bw_dl) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_media_sub_component_convertToJSON() failed [mar_bw_dl]");
			goto end;
		}
	}
	...
	if(media_sub_component->mar_bw_ul) {
		if(cJSON_AddStringToObject(item, "marBwUl",
		                           media_sub_component->mar_bw_ul) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_media_sub_component_convertToJSON() failed [mar_bw_ul]");
			goto end;
		}
	}
	...
	if(media_sub_component->tos_tr_cl) {
		if(cJSON_AddStringToObject(item, "tosTrCl",
		                           media_sub_component->tos_tr_cl) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_media_sub_component_convertToJSON() failed [tos_tr_cl]");
			goto end;
		}
	}
	...
	if(media_sub_component->flow_usage != OpenAPI_flow_usage_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_media_sub_component_convertToJSON() failed [flow_usage]");
			goto end;
		}
	}
...
OpenAPI_media_sub_component_t *OpenAPI_media_sub_component_parseFromJSON(
	...
	af_sig_protocol =
		cJSON_GetObjectItemCaseSensitive(media_sub_componentJSON,
		                                 "afSigProtocol");
	if(af_sig_protocol) {
		if(!cJSON_IsString(af_sig_protocol)) {
			ogs_error(
				"OpenAPI_media_sub_component_parseFromJSON() failed [af_sig_protocol]");
			goto end;
		}
	...
	ethf_descs = cJSON_GetObjectItemCaseSensitive(media_sub_componentJSON,
	                                              "ethfDescs");
	if(ethf_descs) {
		cJSON *ethf_descs_local = NULL;
		if(!cJSON_IsArray(ethf_descs)) {
			ogs_error(
				"OpenAPI_media_sub_component_parseFromJSON() failed [ethf_descs]");
			goto end;
		}
	...
	f_num = cJSON_GetObjectItemCaseSensitive(media_sub_componentJSON,
	                                         "fNum");
	if(!f_num) {
		ogs_error(
			"OpenAPI_media_sub_component_parseFromJSON() failed [f_num]");
		goto end;
	}
	...
	f_descs = cJSON_GetObjectItemCaseSensitive(media_sub_componentJSON,
	                                           "fDescs");
	if(f_descs) {
		cJSON *f_descs_local = NULL;
		if(!cJSON_IsArray(f_descs)) {
			ogs_error(
				"OpenAPI_media_sub_component_parseFromJSON() failed [f_descs]");
			goto end;
		}
		...
		cJSON_ArrayForEach(f_descs_local, f_descs) {
			...
			OpenAPI_list_add(f_descsList,
			                 ogs_strdup(
						 f_descs_local->valuestring));
		}
	}

	f_status = cJSON_GetObjectItemCaseSensitive(media_sub_componentJSON,
	                                            "fStatus");
	if(f_status) {
		if(!cJSON_IsString(f_status)) {
			ogs_error(
				"OpenAPI_media_sub_component_parseFromJSON() failed [f_status]");
			goto end;
		}
		f_statusVariable =
	...
	mar_bw_dl = cJSON_GetObjectItemCaseSensitive(media_sub_componentJSON,
	                                             "marBwDl");
	if(mar_bw_dl) {
		...
		{
			ogs_error(
				"OpenAPI_media_sub_component_parseFromJSON() failed [mar_bw_dl]");
			goto end;
		}
	}

	mar_bw_ul = cJSON_GetObjectItemCaseSensitive(media_sub_componentJSON,
	                                             "marBwUl");
	if(mar_bw_ul) {
		...
		{
			ogs_error(
				"OpenAPI_media_sub_component_parseFromJSON() failed [mar_bw_ul]");
			goto end;
		}
	}

	tos_tr_cl = cJSON_GetObjectItemCaseSensitive(media_sub_componentJSON,
	                                             "tosTrCl");
	if(tos_tr_cl) {
		...
		{
			ogs_error(
				"OpenAPI_media_sub_component_parseFromJSON() failed [tos_tr_cl]");
			goto end;
		}
	}

	flow_usage = cJSON_GetObjectItemCaseSensitive(media_sub_componentJSON,
	                                              "flowUsage");
	if(flow_usage) {
		if(!cJSON_IsString(flow_usage)) {
			ogs_error(
				"OpenAPI_media_sub_component_parseFromJSON() failed [flow_usage]");
			goto end;
		}
		flow_usageVariable =
	...
	media_sub_component_local_var = OpenAPI_media_sub_component_create(
		...
		NULL,
		tos_tr_cl &&
		!cJSON_IsNull(tos_tr_cl) ? ogs_strdup(tos_tr_cl->valuestring) :
		NULL,
...
OpenAPI_media_sub_component_t *OpenAPI_media_sub_component_copy(
	OpenAPI_media_sub_component_t	*dst,
	OpenAPI_media_sub_component_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
