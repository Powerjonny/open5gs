...
OpenAPI_sor_data_t *OpenAPI_sor_data_create(char *provisioning_time,
                                            OpenAPI_ue_update_status_e
                                            ue_update_status,
                                            char	*sor_xmac_iue,
                                            char	*sor_mac_iue,
                                            bool
                                            is_me_support_of_sor_cmci,
                                            int me_support_of_sor_cmci)
{
	OpenAPI_sor_data_t *sor_data_local_var =
		ogs_malloc(sizeof(OpenAPI_sor_data_t));
	...
	sor_data_local_var->is_me_support_of_sor_cmci =
		is_me_support_of_sor_cmci;
...
cJSON *OpenAPI_sor_data_convertToJSON(OpenAPI_sor_data_t *sor_data) {
	...
	if(!sor_data->provisioning_time) {
		ogs_error(
			"OpenAPI_sor_data_convertToJSON() failed [provisioning_time]");
		return NULL;
	}
	...
	if(sor_data->ue_update_status == OpenAPI_ue_update_status_NULL) {
		ogs_error(
			"OpenAPI_sor_data_convertToJSON() failed [ue_update_status]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "ueUpdateStatus",
	                           OpenAPI_ue_update_status_ToString(
					   sor_data->ue_update_status)) == NULL)
	{
		ogs_error(
			"OpenAPI_sor_data_convertToJSON() failed [ue_update_status]");
		goto end;
	...
	if(sor_data->sor_xmac_iue) {
		if(cJSON_AddStringToObject(item, "sorXmacIue",
		                           sor_data->sor_xmac_iue) == NULL)
		{
			ogs_error(
				"OpenAPI_sor_data_convertToJSON() failed [sor_xmac_iue]");
			goto end;
		}
	}
	...
	if(sor_data->sor_mac_iue) {
		...
		{
			ogs_error(
				"OpenAPI_sor_data_convertToJSON() failed [sor_mac_iue]");
			goto end;
		}
	}

	if(sor_data->is_me_support_of_sor_cmci) {
		if(cJSON_AddBoolToObject(item, "meSupportOfSorCmci",
		                         sor_data->me_support_of_sor_cmci) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sor_data_convertToJSON() failed [me_support_of_sor_cmci]");
			goto end;
		}
	}
...
OpenAPI_sor_data_t *OpenAPI_sor_data_parseFromJSON(cJSON *sor_dataJSON) {
	...
	provisioning_time = cJSON_GetObjectItemCaseSensitive(sor_dataJSON,
	                                                     "provisioningTime");
	if(!provisioning_time) {
		ogs_error(
			"OpenAPI_sor_data_parseFromJSON() failed [provisioning_time]");
		goto end;
	}
	...
	ue_update_status = cJSON_GetObjectItemCaseSensitive(sor_dataJSON,
	                                                    "ueUpdateStatus");
	if(!ue_update_status) {
		ogs_error(
			"OpenAPI_sor_data_parseFromJSON() failed [ue_update_status]");
		goto end;
	}
	...
	ue_update_statusVariable =
		OpenAPI_ue_update_status_FromString(
			ue_update_status->valuestring);

	sor_xmac_iue = cJSON_GetObjectItemCaseSensitive(sor_dataJSON,
	                                                "sorXmacIue");
	if(sor_xmac_iue) {
		if(!cJSON_IsString(sor_xmac_iue) &&
		   !cJSON_IsNull(sor_xmac_iue))
		{
			ogs_error(
				"OpenAPI_sor_data_parseFromJSON() failed [sor_xmac_iue]");
			goto end;
		}
	}

	sor_mac_iue = cJSON_GetObjectItemCaseSensitive(sor_dataJSON,
	                                               "sorMacIue");
	if(sor_mac_iue) {
		...
		{
			ogs_error(
				"OpenAPI_sor_data_parseFromJSON() failed [sor_mac_iue]");
			goto end;
		}
	}

	me_support_of_sor_cmci = cJSON_GetObjectItemCaseSensitive(sor_dataJSON,
	                                                          "meSupportOfSorCmci");
	if(me_support_of_sor_cmci) {
		if(!cJSON_IsBool(me_support_of_sor_cmci)) {
			ogs_error(
				"OpenAPI_sor_data_parseFromJSON() failed [me_support_of_sor_cmci]");
			goto end;
		}
	}
	...
	sor_data_local_var = OpenAPI_sor_data_create(
		...
		sor_xmac_iue &&
		!cJSON_IsNull(sor_xmac_iue) ?
		ogs_strdup(sor_xmac_iue->valuestring) : NULL,
		sor_mac_iue &&
		!cJSON_IsNull(sor_mac_iue) ?
...
OpenAPI_sor_data_t *OpenAPI_sor_data_copy(OpenAPI_sor_data_t	*dst,
                                          OpenAPI_sor_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
