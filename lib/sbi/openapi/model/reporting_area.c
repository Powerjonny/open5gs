...
OpenAPI_reporting_area_t *OpenAPI_reporting_area_create(
	OpenAPI_reporting_area_type_e area_type, OpenAPI_tai_t *tai,
	OpenAPI_ecgi_t *ecgi, OpenAPI_ncgi_t *ncgi) {
	OpenAPI_reporting_area_t *reporting_area_local_var =
		ogs_malloc(sizeof(OpenAPI_reporting_area_t));
...
cJSON *OpenAPI_reporting_area_convertToJSON(
	OpenAPI_reporting_area_t *reporting_area) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(reporting_area == NULL) {
		ogs_error(
			"OpenAPI_reporting_area_convertToJSON() failed [ReportingArea]");
		return NULL;
	}
	...
	if(reporting_area->area_type == OpenAPI_reporting_area_type_NULL) {
		ogs_error(
			"OpenAPI_reporting_area_convertToJSON() failed [area_type]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "areaType",
	                           OpenAPI_reporting_area_type_ToString(
	...
	if(reporting_area->tai) {
		cJSON *tai_local_JSON =
			OpenAPI_tai_convertToJSON(reporting_area->tai);
		if(tai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_reporting_area_convertToJSON() failed [tai]");
			goto end;
		}
	...
	if(reporting_area->ecgi) {
		cJSON *ecgi_local_JSON =
			OpenAPI_ecgi_convertToJSON(reporting_area->ecgi);
		if(ecgi_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_reporting_area_convertToJSON() failed [ecgi]");
			goto end;
		}
	...
	if(reporting_area->ncgi) {
		cJSON *ncgi_local_JSON =
			OpenAPI_ncgi_convertToJSON(reporting_area->ncgi);
		if(ncgi_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_reporting_area_convertToJSON() failed [ncgi]");
			goto end;
		}
...
OpenAPI_reporting_area_t *OpenAPI_reporting_area_parseFromJSON(
	cJSON *reporting_areaJSON) {
	...
	if(!area_type) {
		ogs_error(
			"OpenAPI_reporting_area_parseFromJSON() failed [area_type]");
		goto end;
	}
	...
	area_typeVariable =
		OpenAPI_reporting_area_type_FromString(area_type->valuestring);
...
OpenAPI_reporting_area_t *OpenAPI_reporting_area_copy(
	OpenAPI_reporting_area_t *dst, OpenAPI_reporting_area_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
