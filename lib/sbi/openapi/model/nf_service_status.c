...
char * OpenAPI_nf_service_status_ToString(
	...
	const char *nf_service_statusArray[] = { "NULL", "REGISTERED",
		                                 "SUSPENDED",
		                                 "UNDISCOVERABLE" };
	size_t sizeofArray = sizeof(nf_service_statusArray) /
	                     sizeof(nf_service_statusArray[0]);
...
OpenAPI_nf_service_status_e OpenAPI_nf_service_status_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(nf_service_status,
		          nf_service_statusArray[stringToReturn]) == 0) {
