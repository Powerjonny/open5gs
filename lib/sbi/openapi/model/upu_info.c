...
OpenAPI_upu_info_t *OpenAPI_upu_info_create(OpenAPI_list_t	*upu_data_list,
                                            bool		is_upu_reg_ind,
                                            int			upu_reg_ind,
                                            bool		is_upu_ack_ind,
                                            int			upu_ack_ind,
	...
	OpenAPI_upu_info_t *upu_info_local_var =
		ogs_malloc(sizeof(OpenAPI_upu_info_t));
	...
	upu_info_local_var->upu_transparent_container =
		upu_transparent_container;
...
cJSON *OpenAPI_upu_info_convertToJSON(OpenAPI_upu_info_t *upu_info) {
	...
	if(upu_info->upu_data_list) {
		...
		if(upu_data_listList == NULL) {
			ogs_error(
				"OpenAPI_upu_info_convertToJSON() failed [upu_data_list]");
			goto end;
		}
		OpenAPI_list_for_each(upu_info->upu_data_list, node) {
			cJSON *itemLocal =
				OpenAPI_upu_data_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_upu_info_convertToJSON() failed [upu_data_list]");
				goto end;
	...
	if(upu_info->is_upu_reg_ind) {
		if(cJSON_AddBoolToObject(item, "upuRegInd",
		                         upu_info->upu_reg_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_upu_info_convertToJSON() failed [upu_reg_ind]");
			goto end;
		}
	}
	...
	if(upu_info->is_upu_ack_ind) {
		if(cJSON_AddBoolToObject(item, "upuAckInd",
		                         upu_info->upu_ack_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_upu_info_convertToJSON() failed [upu_ack_ind]");
			goto end;
		}
	}
	...
	if(upu_info->upu_mac_iausf) {
		...
		{
			ogs_error(
				"OpenAPI_upu_info_convertToJSON() failed [upu_mac_iausf]");
			goto end;
		}
	}
	...
	if(upu_info->counter_upu) {
		...
		{
			ogs_error(
				"OpenAPI_upu_info_convertToJSON() failed [counter_upu]");
			goto end;
		}
	}
	...
	if(!upu_info->provisioning_time) {
		ogs_error(
			"OpenAPI_upu_info_convertToJSON() failed [provisioning_time]");
		return NULL;
	}
	...
	if(upu_info->upu_transparent_container) {
		if(cJSON_AddStringToObject(item, "upuTransparentContainer",
		                           upu_info->upu_transparent_container)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_upu_info_convertToJSON() failed [upu_transparent_container]");
			goto end;
		}
	}
...
OpenAPI_upu_info_t *OpenAPI_upu_info_parseFromJSON(cJSON *upu_infoJSON) {
	...
	upu_data_list = cJSON_GetObjectItemCaseSensitive(upu_infoJSON,
	                                                 "upuDataList");
	if(upu_data_list) {
		cJSON *upu_data_list_local = NULL;
		if(!cJSON_IsArray(upu_data_list)) {
			ogs_error(
				"OpenAPI_upu_info_parseFromJSON() failed [upu_data_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(upu_data_list_local, upu_data_list) {
			...
			OpenAPI_upu_data_t *upu_data_listItem =
				OpenAPI_upu_data_parseFromJSON(
					upu_data_list_local);
	...
	upu_reg_ind = cJSON_GetObjectItemCaseSensitive(upu_infoJSON,
	                                               "upuRegInd");
	if(upu_reg_ind) {
		if(!cJSON_IsBool(upu_reg_ind)) {
			ogs_error(
				"OpenAPI_upu_info_parseFromJSON() failed [upu_reg_ind]");
			goto end;
		}
	}

	upu_ack_ind = cJSON_GetObjectItemCaseSensitive(upu_infoJSON,
	                                               "upuAckInd");
	if(upu_ack_ind) {
		if(!cJSON_IsBool(upu_ack_ind)) {
			ogs_error(
				"OpenAPI_upu_info_parseFromJSON() failed [upu_ack_ind]");
			goto end;
		}
	}

	upu_mac_iausf = cJSON_GetObjectItemCaseSensitive(upu_infoJSON,
	                                                 "upuMacIausf");
	if(upu_mac_iausf) {
		...
		{
			ogs_error(
				"OpenAPI_upu_info_parseFromJSON() failed [upu_mac_iausf]");
			goto end;
		}
	}

	counter_upu = cJSON_GetObjectItemCaseSensitive(upu_infoJSON,
	                                               "counterUpu");
	if(counter_upu) {
		...
		{
			ogs_error(
				"OpenAPI_upu_info_parseFromJSON() failed [counter_upu]");
			goto end;
		}
	}

	provisioning_time = cJSON_GetObjectItemCaseSensitive(upu_infoJSON,
	                                                     "provisioningTime");
	if(!provisioning_time) {
		ogs_error(
			"OpenAPI_upu_info_parseFromJSON() failed [provisioning_time]");
		goto end;
	}
	...
	upu_transparent_container =
		cJSON_GetObjectItemCaseSensitive(upu_infoJSON,
	...
	if(upu_transparent_container) {
		...
		{
			ogs_error(
				"OpenAPI_upu_info_parseFromJSON() failed [upu_transparent_container]");
			goto end;
		}
	}
...
OpenAPI_upu_info_t *OpenAPI_upu_info_copy(OpenAPI_upu_info_t	*dst,
                                          OpenAPI_upu_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
