...
OpenAPI_ue_id_t *OpenAPI_ue_id_create(char *supi, OpenAPI_list_t *gpsi_list) {
...
cJSON *OpenAPI_ue_id_convertToJSON(OpenAPI_ue_id_t *ue_id) {
	...
	if(ue_id->gpsi_list) {
		...
		if(gpsi_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_id_convertToJSON() failed [gpsi_list]");
			goto end;
		}
		OpenAPI_list_for_each(ue_id->gpsi_list, node) {
...
OpenAPI_ue_id_t *OpenAPI_ue_id_parseFromJSON(cJSON *ue_idJSON) {
	...
	if(gpsi_list) {
		...
		if(!cJSON_IsArray(gpsi_list)) {
			ogs_error(
				"OpenAPI_ue_id_parseFromJSON() failed [gpsi_list]");
			goto end;
		}
...
OpenAPI_ue_id_t *OpenAPI_ue_id_copy(OpenAPI_ue_id_t	*dst,
                                    OpenAPI_ue_id_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
