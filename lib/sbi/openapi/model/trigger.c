...
OpenAPI_trigger_t *OpenAPI_trigger_create(OpenAPI_trigger_type_t *trigger_type,
                                          OpenAPI_trigger_category_t *
                                          trigger_category, bool is_time_limit,
                                          int time_limit, bool is_volume_limit,
                                          int volume_limit,
                                          bool is_volume_limit64,
                                          int volume_limit64,
                                          bool is_event_limit, int event_limit,
                                          bool is_max_number_ofccc,
                                          int max_number_ofccc,
                                          char *tariff_time_change) {
	OpenAPI_trigger_t *trigger_local_var =
		ogs_malloc(sizeof(OpenAPI_trigger_t));
...
cJSON *OpenAPI_trigger_convertToJSON(OpenAPI_trigger_t *trigger) {
	...
	if(!trigger->trigger_type) {
		ogs_error(
			"OpenAPI_trigger_convertToJSON() failed [trigger_type]");
		return NULL;
	}
	cJSON *trigger_type_local_JSON =
		OpenAPI_trigger_type_convertToJSON(trigger->trigger_type);
	...
	if(!trigger->trigger_category) {
		ogs_error(
			"OpenAPI_trigger_convertToJSON() failed [trigger_category]");
		return NULL;
	}
	cJSON *trigger_category_local_JSON =
	...
	cJSON_AddItemToObject(item, "triggerCategory",
	                      trigger_category_local_JSON);
	if(item->child == NULL) {
		ogs_error(
			"OpenAPI_trigger_convertToJSON() failed [trigger_category]");
		goto end;
	...
	if(trigger->is_time_limit) {
		if(cJSON_AddNumberToObject(item, "timeLimit",
		                           trigger->time_limit) == NULL)
		{
			ogs_error(
				"OpenAPI_trigger_convertToJSON() failed [time_limit]");
			goto end;
		}
	}

	if(trigger->is_volume_limit) {
		if(cJSON_AddNumberToObject(item, "volumeLimit",
		                           trigger->volume_limit) == NULL)
		{
			ogs_error(
				"OpenAPI_trigger_convertToJSON() failed [volume_limit]");
			goto end;
		}
	}

	if(trigger->is_volume_limit64) {
		if(cJSON_AddNumberToObject(item, "volumeLimit64",
		                           trigger->volume_limit64) == NULL)
		{
			ogs_error(
				"OpenAPI_trigger_convertToJSON() failed [volume_limit64]");
			goto end;
		}
	}

	if(trigger->is_event_limit) {
		if(cJSON_AddNumberToObject(item, "eventLimit",
		                           trigger->event_limit) == NULL)
		{
			ogs_error(
				"OpenAPI_trigger_convertToJSON() failed [event_limit]");
			goto end;
		}
	}

	if(trigger->is_max_number_ofccc) {
		if(cJSON_AddNumberToObject(item, "maxNumberOfccc",
		                           trigger->max_number_ofccc) == NULL)
		{
			ogs_error(
				"OpenAPI_trigger_convertToJSON() failed [max_number_ofccc]");
			goto end;
		}
	}

	if(trigger->tariff_time_change) {
		if(cJSON_AddStringToObject(item, "tariffTimeChange",
		                           trigger->tariff_time_change) == NULL)
		{
			ogs_error(
				"OpenAPI_trigger_convertToJSON() failed [tariff_time_change]");
			goto end;
		}
	}
...
OpenAPI_trigger_t *OpenAPI_trigger_parseFromJSON(cJSON *triggerJSON) {
	...
	trigger_type = cJSON_GetObjectItemCaseSensitive(triggerJSON,
	                                                "triggerType");
	if(!trigger_type) {
		ogs_error(
			"OpenAPI_trigger_parseFromJSON() failed [trigger_type]");
		goto end;
	}
	trigger_type_local_nonprim =
		OpenAPI_trigger_type_parseFromJSON(trigger_type);
	if(!trigger_type_local_nonprim) {
		ogs_error(
			"OpenAPI_trigger_type_parseFromJSON failed [trigger_type]");
		goto end;
	}

	trigger_category = cJSON_GetObjectItemCaseSensitive(triggerJSON,
	                                                    "triggerCategory");
	if(!trigger_category) {
		ogs_error(
			"OpenAPI_trigger_parseFromJSON() failed [trigger_category]");
		goto end;
	}
	trigger_category_local_nonprim =
		OpenAPI_trigger_category_parseFromJSON(trigger_category);
	if(!trigger_category_local_nonprim) {
		ogs_error(
			"OpenAPI_trigger_category_parseFromJSON failed [trigger_category]");
		goto end;
	}
	...
	if(time_limit) {
		if(!cJSON_IsNumber(time_limit)) {
			ogs_error(
				"OpenAPI_trigger_parseFromJSON() failed [time_limit]");
			goto end;
		}
	}

	volume_limit = cJSON_GetObjectItemCaseSensitive(triggerJSON,
	                                                "volumeLimit");
	if(volume_limit) {
		if(!cJSON_IsNumber(volume_limit)) {
			ogs_error(
				"OpenAPI_trigger_parseFromJSON() failed [volume_limit]");
			goto end;
		}
	}

	volume_limit64 = cJSON_GetObjectItemCaseSensitive(triggerJSON,
	                                                  "volumeLimit64");
	if(volume_limit64) {
		if(!cJSON_IsNumber(volume_limit64)) {
			ogs_error(
				"OpenAPI_trigger_parseFromJSON() failed [volume_limit64]");
			goto end;
		}
	}

	event_limit = cJSON_GetObjectItemCaseSensitive(triggerJSON,
	                                               "eventLimit");
	if(event_limit) {
		if(!cJSON_IsNumber(event_limit)) {
			ogs_error(
				"OpenAPI_trigger_parseFromJSON() failed [event_limit]");
			goto end;
		}
	}

	max_number_ofccc = cJSON_GetObjectItemCaseSensitive(triggerJSON,
	                                                    "maxNumberOfccc");
	if(max_number_ofccc) {
		if(!cJSON_IsNumber(max_number_ofccc)) {
			ogs_error(
				"OpenAPI_trigger_parseFromJSON() failed [max_number_ofccc]");
			goto end;
		}
	}

	tariff_time_change = cJSON_GetObjectItemCaseSensitive(triggerJSON,
	                                                      "tariffTimeChange");
	if(tariff_time_change) {
		if(!cJSON_IsString(tariff_time_change) &&
		   !cJSON_IsNull(tariff_time_change))
		{
			ogs_error(
				"OpenAPI_trigger_parseFromJSON() failed [tariff_time_change]");
			goto end;
		}
	}
	...
	trigger_local_var = OpenAPI_trigger_create(
		...
		tariff_time_change &&
		!cJSON_IsNull(tariff_time_change) ?
		ogs_strdup(tariff_time_change->valuestring) : NULL
		);
...
OpenAPI_trigger_t *OpenAPI_trigger_copy(OpenAPI_trigger_t	*dst,
                                        OpenAPI_trigger_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
