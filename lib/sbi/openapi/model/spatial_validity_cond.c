...
OpenAPI_spatial_validity_cond_t *OpenAPI_spatial_validity_cond_create(
	...
	OpenAPI_spatial_validity_cond_t *spatial_validity_cond_local_var =
		ogs_malloc(sizeof(OpenAPI_spatial_validity_cond_t));
	ogs_assert(spatial_validity_cond_local_var);

	spatial_validity_cond_local_var->tracking_area_list =
		tracking_area_list;
	spatial_validity_cond_local_var->countries = countries;
	spatial_validity_cond_local_var->geographical_service_area =
...
void OpenAPI_spatial_validity_cond_free(
	...
	if(spatial_validity_cond->tracking_area_list) {
		OpenAPI_list_for_each(spatial_validity_cond->tracking_area_list,
		                      node) {
...
cJSON *OpenAPI_spatial_validity_cond_convertToJSON(
	...
	if(spatial_validity_cond == NULL) {
		ogs_error(
			"OpenAPI_spatial_validity_cond_convertToJSON() failed [SpatialValidityCond]");
		return NULL;
	}
	...
	if(spatial_validity_cond->tracking_area_list) {
		...
		if(tracking_area_listList == NULL) {
			ogs_error(
				"OpenAPI_spatial_validity_cond_convertToJSON() failed [tracking_area_list]");
			goto end;
		}
		OpenAPI_list_for_each(spatial_validity_cond->tracking_area_list,
		                      node) {
	...
	if(spatial_validity_cond->countries) {
		...
		if(countriesList == NULL) {
			ogs_error(
				"OpenAPI_spatial_validity_cond_convertToJSON() failed [countries]");
			goto end;
		}
		OpenAPI_list_for_each(spatial_validity_cond->countries, node) {
	...
	if(spatial_validity_cond->geographical_service_area) {
		cJSON *geographical_service_area_local_JSON =
			OpenAPI_geo_service_area_convertToJSON(
				spatial_validity_cond->geographical_service_area);
		if(geographical_service_area_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_spatial_validity_cond_convertToJSON() failed [geographical_service_area]");
			goto end;
		}
...
OpenAPI_spatial_validity_cond_t *OpenAPI_spatial_validity_cond_parseFromJSON(
	...
	OpenAPI_geo_service_area_t *geographical_service_area_local_nonprim =
		NULL;
	tracking_area_list =
		cJSON_GetObjectItemCaseSensitive(spatial_validity_condJSON,
		                                 "trackingAreaList");
	if(tracking_area_list) {
		cJSON *tracking_area_list_local = NULL;
		if(!cJSON_IsArray(tracking_area_list)) {
			ogs_error(
				"OpenAPI_spatial_validity_cond_parseFromJSON() failed [tracking_area_list]");
			goto end;
		}
	...
	countries = cJSON_GetObjectItemCaseSensitive(spatial_validity_condJSON,
	                                             "countries");
	if(countries) {
		cJSON *countries_local = NULL;
		if(!cJSON_IsArray(countries)) {
			ogs_error(
				"OpenAPI_spatial_validity_cond_parseFromJSON() failed [countries]");
			goto end;
		}
	...
	if(geographical_service_area) {
		geographical_service_area_local_nonprim =
			OpenAPI_geo_service_area_parseFromJSON(
				geographical_service_area);
		if(!geographical_service_area_local_nonprim) {
			ogs_error(
				"OpenAPI_geo_service_area_parseFromJSON failed [geographical_service_area]");
			goto end;
		}
	}
...
OpenAPI_spatial_validity_cond_t *OpenAPI_spatial_validity_cond_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_spatial_validity_cond_convertToJSON() failed");
		return NULL;
	}
