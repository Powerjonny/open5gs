...
OpenAPI_nrf_info_served_udr_info_value_create(char		*group_id,
                                              ...
                                              supported_data_sets,
                                              OpenAPI_list_t *
                                              shared_data_id_ranges) {
	OpenAPI_nrf_info_served_udr_info_value_t *
	        nrf_info_served_udr_info_value_local_var =
		ogs_malloc(sizeof(OpenAPI_nrf_info_served_udr_info_value_t));
	...
	nrf_info_served_udr_info_value_local_var->supported_data_sets =
		supported_data_sets;
	nrf_info_served_udr_info_value_local_var->shared_data_id_ranges =
		shared_data_id_ranges;

	return nrf_info_served_udr_info_value_local_var;
}

void OpenAPI_nrf_info_served_udr_info_value_free(
	OpenAPI_nrf_info_served_udr_info_value_t *nrf_info_served_udr_info_value)
{
	...
	if(nrf_info_served_udr_info_value->supi_ranges) {
		OpenAPI_list_for_each(
			nrf_info_served_udr_info_value->supi_ranges, node) {
	...
	if(nrf_info_served_udr_info_value->gpsi_ranges) {
		OpenAPI_list_for_each(
			nrf_info_served_udr_info_value->gpsi_ranges, node) {
	...
	if(nrf_info_served_udr_info_value->external_group_identifiers_ranges) {
		OpenAPI_list_for_each(
			nrf_info_served_udr_info_value->
...
cJSON *OpenAPI_nrf_info_served_udr_info_value_convertToJSON(
	OpenAPI_nrf_info_served_udr_info_value_t *nrf_info_served_udr_info_value)
{
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(nrf_info_served_udr_info_value == NULL) {
		ogs_error(
			"OpenAPI_nrf_info_served_udr_info_value_convertToJSON() failed [NrfInfo_servedUdrInfo_value]");
		return NULL;
	}
	...
	if(nrf_info_served_udr_info_value->group_id) {
		...
		{
			ogs_error(
				"OpenAPI_nrf_info_served_udr_info_value_convertToJSON() failed [group_id]");
			goto end;
		}
	}
	...
	if(nrf_info_served_udr_info_value->supi_ranges) {
		...
		if(supi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_udr_info_value_convertToJSON() failed [supi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			nrf_info_served_udr_info_value->supi_ranges, node) {
	...
	if(nrf_info_served_udr_info_value->gpsi_ranges) {
		...
		if(gpsi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_udr_info_value_convertToJSON() failed [gpsi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			nrf_info_served_udr_info_value->gpsi_ranges, node) {
	...
	if(nrf_info_served_udr_info_value->external_group_identifiers_ranges) {
		...
		if(external_group_identifiers_rangesList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_udr_info_value_convertToJSON() failed [external_group_identifiers_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			nrf_info_served_udr_info_value->
	...
	if(nrf_info_served_udr_info_value->supported_data_sets !=
	   OpenAPI_data_set_id_NULL)
	{
		cJSON *supported_data_setsList = cJSON_AddArrayToObject(item,
		                                                        "supportedDataSets");
		if(supported_data_setsList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_udr_info_value_convertToJSON() failed [supported_data_sets]");
			goto end;
		}
		OpenAPI_list_for_each(
			nrf_info_served_udr_info_value->supported_data_sets,
			node) {
			if(cJSON_AddStringToObject(supported_data_setsList, "",
			                           OpenAPI_data_set_id_ToString(
							   (intptr_t) node->data))
			   == NULL)
			{
				ogs_error(
	...
	if(nrf_info_served_udr_info_value->shared_data_id_ranges) {
		cJSON *shared_data_id_rangesList = cJSON_AddArrayToObject(item,
		                                                          "sharedDataIdRanges");
		if(shared_data_id_rangesList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_udr_info_value_convertToJSON() failed [shared_data_id_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			nrf_info_served_udr_info_value->shared_data_id_ranges,
			node) {
			cJSON *itemLocal =
				OpenAPI_shared_data_id_range_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_nrf_info_served_udr_info_value_convertToJSON() failed [shared_data_id_ranges]");
			...
			cJSON_AddItemToArray(shared_data_id_rangesList,
			                     itemLocal);
		}
	}
...
OpenAPI_nrf_info_served_udr_info_value_parseFromJSON(
	cJSON *nrf_info_served_udr_info_valueJSON) {
	OpenAPI_nrf_info_served_udr_info_value_t *
	        nrf_info_served_udr_info_value_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *group_id = NULL;
	...
	group_id =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_udr_info_valueJSON, "groupId");
	if(group_id) {
		if(!cJSON_IsString(group_id) &&
		   !cJSON_IsNull(group_id))
		{
			ogs_error(
				"OpenAPI_nrf_info_served_udr_info_value_parseFromJSON() failed [group_id]");
			goto end;
		}
	}
	...
	supi_ranges =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_udr_info_valueJSON, "supiRanges");
	if(supi_ranges) {
		cJSON *supi_ranges_local = NULL;
		if(!cJSON_IsArray(supi_ranges)) {
			ogs_error(
				"OpenAPI_nrf_info_served_udr_info_value_parseFromJSON() failed [supi_ranges]");
			goto end;
		}
	...
	gpsi_ranges =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_udr_info_valueJSON, "gpsiRanges");
	if(gpsi_ranges) {
		cJSON *gpsi_ranges_local = NULL;
		if(!cJSON_IsArray(gpsi_ranges)) {
			ogs_error(
				"OpenAPI_nrf_info_served_udr_info_value_parseFromJSON() failed [gpsi_ranges]");
			goto end;
		}
	...
	external_group_identifiers_ranges =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_udr_info_valueJSON,
			"externalGroupIdentifiersRanges");
	if(external_group_identifiers_ranges) {
		...
		if(!cJSON_IsArray(external_group_identifiers_ranges)) {
			ogs_error(
				"OpenAPI_nrf_info_served_udr_info_value_parseFromJSON() failed [external_group_identifiers_ranges]");
			goto end;
		}
	...
	if(supported_data_sets) {
		...
		if(!cJSON_IsArray(supported_data_sets)) {
			ogs_error(
				"OpenAPI_nrf_info_served_udr_info_value_parseFromJSON() failed [supported_data_sets]");
			goto end;
		}

		supported_data_setsList = OpenAPI_list_create();

		cJSON_ArrayForEach(supported_data_sets_local,
		                   supported_data_sets) {
			OpenAPI_data_set_id_e localEnum =
				OpenAPI_data_set_id_NULL;
			...
			localEnum =
				OpenAPI_data_set_id_FromString(
					supported_data_sets_local->valuestring);
			if(!localEnum) {
				ogs_info(
					"Enum value \"%s\" for field \"supported_data_sets\" is not supported. Ignoring it ...",
					supported_data_sets_local->valuestring);
			} else {
				OpenAPI_list_add(supported_data_setsList,
				                 (void *) localEnum);
			}
		}
		if(supported_data_setsList->count == 0) {
			ogs_error(
				"OpenAPI_nrf_info_served_udr_info_value_parseFromJSON() failed: Expected supported_data_setsList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(shared_data_id_ranges) {
		...
		if(!cJSON_IsArray(shared_data_id_ranges)) {
			ogs_error(
				"OpenAPI_nrf_info_served_udr_info_value_parseFromJSON() failed [shared_data_id_ranges]");
			goto end;
		}

		shared_data_id_rangesList = OpenAPI_list_create();

		cJSON_ArrayForEach(shared_data_id_ranges_local,
		                   shared_data_id_ranges) {
			...
			OpenAPI_shared_data_id_range_t *
			        shared_data_id_rangesItem =
				OpenAPI_shared_data_id_range_parseFromJSON(
					shared_data_id_ranges_local);
			if(!shared_data_id_rangesItem) {
				ogs_error("No shared_data_id_rangesItem");
				goto end;
			}
			OpenAPI_list_add(shared_data_id_rangesList,
			                 shared_data_id_rangesItem);
		}
	}

	nrf_info_served_udr_info_value_local_var =
		OpenAPI_nrf_info_served_udr_info_value_create(
			group_id &&
			!cJSON_IsNull(group_id) ?
...
OpenAPI_nrf_info_served_udr_info_value_t *
OpenAPI_nrf_info_served_udr_info_value_copy(
	OpenAPI_nrf_info_served_udr_info_value_t	*dst,
	OpenAPI_nrf_info_served_udr_info_value_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_nrf_info_served_udr_info_value_convertToJSON() failed");
		return NULL;
	}
