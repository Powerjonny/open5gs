...
OpenAPI_n32f_error_info_t *OpenAPI_n32f_error_info_create(char *n32f_message_id,
                                                          ...
                                                          OpenAPI_list_t *
                                                          error_details_list) {
	OpenAPI_n32f_error_info_t *n32f_error_info_local_var =
		ogs_malloc(sizeof(OpenAPI_n32f_error_info_t));
	...
	n32f_error_info_local_var->failed_modification_list =
		failed_modification_list;
...
void OpenAPI_n32f_error_info_free(OpenAPI_n32f_error_info_t *n32f_error_info) {
	...
	if(n32f_error_info->error_details_list) {
		OpenAPI_list_for_each(n32f_error_info->error_details_list,
		                      node) {
...
cJSON *OpenAPI_n32f_error_info_convertToJSON(
	OpenAPI_n32f_error_info_t *n32f_error_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(n32f_error_info == NULL) {
		ogs_error(
			"OpenAPI_n32f_error_info_convertToJSON() failed [N32fErrorInfo]");
		return NULL;
	}
	...
	if(!n32f_error_info->n32f_message_id) {
		ogs_error(
			"OpenAPI_n32f_error_info_convertToJSON() failed [n32f_message_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "n32fMessageId",
	                           n32f_error_info->n32f_message_id) == NULL)
	{
		ogs_error(
			"OpenAPI_n32f_error_info_convertToJSON() failed [n32f_message_id]");
	...
	if(n32f_error_info->n32f_error_type == OpenAPI_n32f_error_type_NULL) {
		ogs_error(
			"OpenAPI_n32f_error_info_convertToJSON() failed [n32f_error_type]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "n32fErrorType",
	                           OpenAPI_n32f_error_type_ToString(
					   n32f_error_info->n32f_error_type)) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_n32f_error_info_convertToJSON() failed [n32f_error_type]");
	...
	if(n32f_error_info->n32f_context_id) {
		if(cJSON_AddStringToObject(item, "n32fContextId",
		                           n32f_error_info->n32f_context_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_n32f_error_info_convertToJSON() failed [n32f_context_id]");
			goto end;
		}
	}

	if(n32f_error_info->failed_modification_list) {
		cJSON *failed_modification_listList =
			cJSON_AddArrayToObject(item, "failedModificationList");
		if(failed_modification_listList == NULL) {
			ogs_error(
				"OpenAPI_n32f_error_info_convertToJSON() failed [failed_modification_list]");
			goto end;
		}
		OpenAPI_list_for_each(n32f_error_info->failed_modification_list,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_failed_modification_info_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_n32f_error_info_convertToJSON() failed [failed_modification_list]");
			...
			cJSON_AddItemToArray(failed_modification_listList,
			                     itemLocal);
		}
	}

	if(n32f_error_info->error_details_list) {
		cJSON *error_details_listList = cJSON_AddArrayToObject(item,
		                                                       "errorDetailsList");
		if(error_details_listList == NULL) {
			ogs_error(
				"OpenAPI_n32f_error_info_convertToJSON() failed [error_details_list]");
			goto end;
		}
		OpenAPI_list_for_each(n32f_error_info->error_details_list,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_n32f_error_detail_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_n32f_error_info_convertToJSON() failed [error_details_list]");
...
OpenAPI_n32f_error_info_t *OpenAPI_n32f_error_info_parseFromJSON(
	cJSON *n32f_error_infoJSON) {
	...
	n32f_message_id = cJSON_GetObjectItemCaseSensitive(n32f_error_infoJSON,
	                                                   "n32fMessageId");
	if(!n32f_message_id) {
		ogs_error(
			"OpenAPI_n32f_error_info_parseFromJSON() failed [n32f_message_id]");
		goto end;
	}
	...
	n32f_error_type = cJSON_GetObjectItemCaseSensitive(n32f_error_infoJSON,
	                                                   "n32fErrorType");
	if(!n32f_error_type) {
		ogs_error(
			"OpenAPI_n32f_error_info_parseFromJSON() failed [n32f_error_type]");
		goto end;
	}
	...
	n32f_error_typeVariable =
		OpenAPI_n32f_error_type_FromString(
			n32f_error_type->valuestring);

	n32f_context_id = cJSON_GetObjectItemCaseSensitive(n32f_error_infoJSON,
	                                                   "n32fContextId");
	if(n32f_context_id) {
		if(!cJSON_IsString(n32f_context_id) &&
		   !cJSON_IsNull(n32f_context_id))
		{
			ogs_error(
				"OpenAPI_n32f_error_info_parseFromJSON() failed [n32f_context_id]");
			goto end;
		}
	}
	...
	if(failed_modification_list) {
		...
		if(!cJSON_IsArray(failed_modification_list)) {
			ogs_error(
				"OpenAPI_n32f_error_info_parseFromJSON() failed [failed_modification_list]");
			goto end;
		}

		failed_modification_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(failed_modification_list_local,
		                   failed_modification_list) {
			...
			OpenAPI_failed_modification_info_t *
			        failed_modification_listItem =
				OpenAPI_failed_modification_info_parseFromJSON(
					failed_modification_list_local);
			if(!failed_modification_listItem) {
				ogs_error("No failed_modification_listItem");
				goto end;
			}
			OpenAPI_list_add(failed_modification_listList,
			                 failed_modification_listItem);
		}
	}
	...
	if(error_details_list) {
		...
		if(!cJSON_IsArray(error_details_list)) {
			ogs_error(
				"OpenAPI_n32f_error_info_parseFromJSON() failed [error_details_list]");
			goto end;
		}

		error_details_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(error_details_list_local,
		                   error_details_list) {
			...
			OpenAPI_n32f_error_detail_t *error_details_listItem =
				OpenAPI_n32f_error_detail_parseFromJSON(
					error_details_list_local);
			if(!error_details_listItem) {
				ogs_error("No error_details_listItem");
				goto end;
			}
			OpenAPI_list_add(error_details_listList,
			                 error_details_listItem);
		}
	}
	...
	n32f_error_info_local_var = OpenAPI_n32f_error_info_create(
		...
		n32f_context_id &&
		!cJSON_IsNull(n32f_context_id) ?
		ogs_strdup(n32f_context_id->valuestring) : NULL,
...
OpenAPI_n32f_error_info_t *OpenAPI_n32f_error_info_copy(
	OpenAPI_n32f_error_info_t *dst, OpenAPI_n32f_error_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
