...
OpenAPI_policy_update_t *OpenAPI_policy_update_create(char *resource_uri,
                                                      ...
                                                      *as_time_dis_param) {
	OpenAPI_policy_update_t *policy_update_local_var =
		ogs_malloc(sizeof(OpenAPI_policy_update_t));
	...
	policy_update_local_var->is_as_time_dis_param_null =
		is_as_time_dis_param_null;
...
void OpenAPI_policy_update_free(OpenAPI_policy_update_t *policy_update) {
	...
	if(policy_update->as_time_dis_param) {
		OpenAPI_as_time_distribution_param_free(
			policy_update->as_time_dis_param);
		policy_update->as_time_dis_param = NULL;
	}
	ogs_free(policy_update);
}

cJSON *OpenAPI_policy_update_convertToJSON(
	OpenAPI_policy_update_t *policy_update) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(policy_update == NULL) {
		ogs_error(
			"OpenAPI_policy_update_convertToJSON() failed [PolicyUpdate]");
		return NULL;
	}
	...
	if(!policy_update->resource_uri) {
		ogs_error(
			"OpenAPI_policy_update_convertToJSON() failed [resource_uri]");
		return NULL;
	}
	...
	if(policy_update->triggers != OpenAPI_request_trigger_NULL) {
		...
		if(triggersList == NULL) {
			ogs_error(
				"OpenAPI_policy_update_convertToJSON() failed [triggers]");
			goto end;
		}
	...
	if(policy_update->serv_area_res) {
		...
		if(serv_area_res_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_policy_update_convertToJSON() failed [serv_area_res]");
			goto end;
		}
	...
	if(policy_update->wl_serv_area_res) {
		...
		if(wl_serv_area_res_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_policy_update_convertToJSON() failed [wl_serv_area_res]");
			goto end;
		}
	...
	if(policy_update->is_rfsp) {
		if(cJSON_AddNumberToObject(item, "rfsp",
		                           policy_update->rfsp) == NULL)
		{
			ogs_error(
				"OpenAPI_policy_update_convertToJSON() failed [rfsp]");
			goto end;
		}
	}
	...
	if(policy_update->is_target_rfsp) {
		if(cJSON_AddNumberToObject(item, "targetRfsp",
		                           policy_update->target_rfsp) == NULL)
		{
			ogs_error(
				"OpenAPI_policy_update_convertToJSON() failed [target_rfsp]");
			goto end;
		}
	}
	...
	if(policy_update->smf_sel_info) {
		cJSON *smf_sel_info_local_JSON =
			OpenAPI_smf_selection_data_convertToJSON(
				policy_update->smf_sel_info);
		if(smf_sel_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_policy_update_convertToJSON() failed [smf_sel_info]");
			goto end;
		}
	...
	if(policy_update->ue_ambr) {
		cJSON *ue_ambr_local_JSON =
			OpenAPI_ambr_convertToJSON(policy_update->ue_ambr);
		if(ue_ambr_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_policy_update_convertToJSON() failed [ue_ambr]");
			goto end;
		}
	...
	if(policy_update->ue_slice_mbrs) {
		...
		if(ue_slice_mbrsList == NULL) {
			ogs_error(
				"OpenAPI_policy_update_convertToJSON() failed [ue_slice_mbrs]");
			goto end;
		}
		OpenAPI_list_for_each(policy_update->ue_slice_mbrs, node) {
			cJSON *itemLocal =
	...
	if(policy_update->pras) {
		...
		if(pras == NULL) {
			ogs_error(
				"OpenAPI_policy_update_convertToJSON() failed [pras]");
			goto end;
		}
		...
		if(policy_update->pras) {
			OpenAPI_list_for_each(policy_update->pras, node) {
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_policy_update_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(policy_update->pcf_ue_info) {
		cJSON *pcf_ue_info_local_JSON =
			OpenAPI_pcf_ue_callback_info_convertToJSON(
				policy_update->pcf_ue_info);
		if(pcf_ue_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_policy_update_convertToJSON() failed [pcf_ue_info]");
			goto end;
		}
	...
	if(policy_update->match_pdus) {
		...
		if(match_pdusList == NULL) {
			ogs_error(
				"OpenAPI_policy_update_convertToJSON() failed [match_pdus]");
			goto end;
		}
		OpenAPI_list_for_each(policy_update->match_pdus, node) {
			cJSON *itemLocal =
	...
	if(policy_update->as_time_dis_param) {
		...
		if(as_time_dis_param_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_policy_update_convertToJSON() failed [as_time_dis_param]");
			goto end;
		}
...
OpenAPI_policy_update_t *OpenAPI_policy_update_parseFromJSON(
	cJSON *policy_updateJSON) {
	...
	OpenAPI_as_time_distribution_param_t *as_time_dis_param_local_nonprim =
		NULL;
	resource_uri = cJSON_GetObjectItemCaseSensitive(policy_updateJSON,
	                                                "resourceUri");
	if(!resource_uri) {
		ogs_error(
			"OpenAPI_policy_update_parseFromJSON() failed [resource_uri]");
		goto end;
	}
	...
	triggers = cJSON_GetObjectItemCaseSensitive(policy_updateJSON,
	                                            "triggers");
	if(triggers) {
		if(!cJSON_IsNull(triggers)) {
			...
			if(!cJSON_IsArray(triggers)) {
				ogs_error(
					"OpenAPI_policy_update_parseFromJSON() failed [triggers]");
				goto end;
			}
			...
			if(triggersList->count == 0) {
				ogs_error(
					"OpenAPI_policy_update_parseFromJSON() failed: Expected triggersList to not be empty (after ignoring unsupported enum values).");
				goto end;
			}
		}
	}

	serv_area_res = cJSON_GetObjectItemCaseSensitive(policy_updateJSON,
	                                                 "servAreaRes");
	if(serv_area_res) {
	...
	wl_serv_area_res = cJSON_GetObjectItemCaseSensitive(policy_updateJSON,
	                                                    "wlServAreaRes");
	if(wl_serv_area_res) {
	...
	if(rfsp) {
		if(!cJSON_IsNumber(rfsp)) {
			ogs_error(
				"OpenAPI_policy_update_parseFromJSON() failed [rfsp]");
			goto end;
		}
	}

	target_rfsp = cJSON_GetObjectItemCaseSensitive(policy_updateJSON,
	                                               "targetRfsp");
	if(target_rfsp) {
		if(!cJSON_IsNumber(target_rfsp)) {
			ogs_error(
				"OpenAPI_policy_update_parseFromJSON() failed [target_rfsp]");
			goto end;
		}
	}

	smf_sel_info = cJSON_GetObjectItemCaseSensitive(policy_updateJSON,
	                                                "smfSelInfo");
	if(smf_sel_info) {
	...
	ue_slice_mbrs = cJSON_GetObjectItemCaseSensitive(policy_updateJSON,
	                                                 "ueSliceMbrs");
	if(ue_slice_mbrs) {
		cJSON *ue_slice_mbrs_local = NULL;
		if(!cJSON_IsArray(ue_slice_mbrs)) {
			ogs_error(
				"OpenAPI_policy_update_parseFromJSON() failed [ue_slice_mbrs]");
			goto end;
		}
			...
			if(cJSON_IsObject(pras)) {
				...
				cJSON_ArrayForEach(pras_local_map, pras) {
							  ...
							  localMapObject))
					{
						...
						ogs_error(
							"OpenAPI_policy_update_parseFromJSON() failed [inner]");
						goto end;
					}
	...
	pcf_ue_info = cJSON_GetObjectItemCaseSensitive(policy_updateJSON,
	                                               "pcfUeInfo");
	if(pcf_ue_info) {
	...
	match_pdus = cJSON_GetObjectItemCaseSensitive(policy_updateJSON,
	                                              "matchPdus");
	if(match_pdus) {
		if(!cJSON_IsNull(match_pdus)) {
			...
			if(!cJSON_IsArray(match_pdus)) {
				ogs_error(
					"OpenAPI_policy_update_parseFromJSON() failed [match_pdus]");
				goto end;
			}
	...
	as_time_dis_param = cJSON_GetObjectItemCaseSensitive(policy_updateJSON,
	                                                     "asTimeDisParam");
...
OpenAPI_policy_update_t *OpenAPI_policy_update_copy(
	OpenAPI_policy_update_t *dst, OpenAPI_policy_update_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
