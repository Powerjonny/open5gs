...
OpenAPI_nrf_info_served_sepp_info_list_value_create(
	char *sepp_prefix,
	OpenAPI_list_t	    *
	sepp_ports,
	OpenAPI_list_t *
	...
	OpenAPI_nrf_info_served_sepp_info_list_value_t *
		...
		ogs_malloc(
			sizeof(OpenAPI_nrf_info_served_sepp_info_list_value_t));
	ogs_assert(nrf_info_served_sepp_info_list_value_local_var);

	nrf_info_served_sepp_info_list_value_local_var->sepp_prefix =
		sepp_prefix;
	nrf_info_served_sepp_info_list_value_local_var->sepp_ports = sepp_ports;
	nrf_info_served_sepp_info_list_value_local_var->remote_plmn_list =
		remote_plmn_list;
	nrf_info_served_sepp_info_list_value_local_var->remote_snpn_list =
		remote_snpn_list;

	return nrf_info_served_sepp_info_list_value_local_var;
}

void OpenAPI_nrf_info_served_sepp_info_list_value_free(
	OpenAPI_nrf_info_served_sepp_info_list_value_t *
	...
	if(nrf_info_served_sepp_info_list_value->sepp_ports) {
		OpenAPI_list_for_each(
			nrf_info_served_sepp_info_list_value->sepp_ports,
			node) {
	...
	if(nrf_info_served_sepp_info_list_value->remote_plmn_list) {
		OpenAPI_list_for_each(
			nrf_info_served_sepp_info_list_value->remote_plmn_list,
			node) {
	...
	if(nrf_info_served_sepp_info_list_value->remote_snpn_list) {
		OpenAPI_list_for_each(
			nrf_info_served_sepp_info_list_value->remote_snpn_list,
			node) {
...
cJSON *OpenAPI_nrf_info_served_sepp_info_list_value_convertToJSON(
	OpenAPI_nrf_info_served_sepp_info_list_value_t *
	nrf_info_served_sepp_info_list_value) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(nrf_info_served_sepp_info_list_value == NULL) {
		ogs_error(
			"OpenAPI_nrf_info_served_sepp_info_list_value_convertToJSON() failed [NrfInfo_servedSeppInfoList_value]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(nrf_info_served_sepp_info_list_value->sepp_prefix) {
		if(cJSON_AddStringToObject(item, "seppPrefix",
		                           nrf_info_served_sepp_info_list_value
		                           ->sepp_prefix) == NULL)
		{
			ogs_error(
				"OpenAPI_nrf_info_served_sepp_info_list_value_convertToJSON() failed [sepp_prefix]");
			goto end;
		}
	}
	...
	if(nrf_info_served_sepp_info_list_value->sepp_ports) {
		...
		if(sepp_ports == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_sepp_info_list_value_convertToJSON() failed [sepp_ports]");
			goto end;
		}
		...
		if(nrf_info_served_sepp_info_list_value->sepp_ports) {
			OpenAPI_list_for_each(
				...
				if(localKeyValue->value == NULL) {
					ogs_error(
						"OpenAPI_nrf_info_served_sepp_info_list_value_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(nrf_info_served_sepp_info_list_value->remote_plmn_list) {
		...
		if(remote_plmn_listList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_sepp_info_list_value_convertToJSON() failed [remote_plmn_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			nrf_info_served_sepp_info_list_value->remote_plmn_list,
	...
	if(nrf_info_served_sepp_info_list_value->remote_snpn_list) {
		...
		if(remote_snpn_listList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_sepp_info_list_value_convertToJSON() failed [remote_snpn_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			nrf_info_served_sepp_info_list_value->remote_snpn_list,
...
OpenAPI_nrf_info_served_sepp_info_list_value_parseFromJSON(
	cJSON *nrf_info_served_sepp_info_list_valueJSON) {
	OpenAPI_nrf_info_served_sepp_info_list_value_t *
	        nrf_info_served_sepp_info_list_value_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	sepp_prefix =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_sepp_info_list_valueJSON, "seppPrefix");
	if(sepp_prefix) {
		if(!cJSON_IsString(sepp_prefix) &&
		   !cJSON_IsNull(sepp_prefix))
		{
			ogs_error(
				"OpenAPI_nrf_info_served_sepp_info_list_value_parseFromJSON() failed [sepp_prefix]");
			goto end;
		}
	}
	...
	sepp_ports =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_sepp_info_list_valueJSON, "seppPorts");
	if(sepp_ports) {
		cJSON *sepp_ports_local_map = NULL;
		if(!cJSON_IsObject(sepp_ports) &&
		   !cJSON_IsNull(sepp_ports))
		{
			ogs_error(
				"OpenAPI_nrf_info_served_sepp_info_list_value_parseFromJSON() failed [sepp_ports]");
			goto end;
		}
		if(cJSON_IsObject(sepp_ports)) {
			...
			cJSON_ArrayForEach(sepp_ports_local_map, sepp_ports) {
				...
				if(!cJSON_IsNumber(localMapObject)) {
					ogs_error(
						"OpenAPI_nrf_info_served_sepp_info_list_value_parseFromJSON() failed [inner]");
					goto end;
				}
				...
				*localDouble = localMapObject->valuedouble;
				localMapKeyPair =
					OpenAPI_map_create(ogs_strdup(
								   ...
								   ->string),
					                   localDouble);
				OpenAPI_list_add(sepp_portsList,
				                 localMapKeyPair);
			}
		}
	}
	...
	remote_plmn_list =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_sepp_info_list_valueJSON,
			"remotePlmnList");
	if(remote_plmn_list) {
		cJSON *remote_plmn_list_local = NULL;
		if(!cJSON_IsArray(remote_plmn_list)) {
			ogs_error(
				"OpenAPI_nrf_info_served_sepp_info_list_value_parseFromJSON() failed [remote_plmn_list]");
			goto end;
		}
	...
	if(remote_snpn_list) {
		...
		if(!cJSON_IsArray(remote_snpn_list)) {
			ogs_error(
				"OpenAPI_nrf_info_served_sepp_info_list_value_parseFromJSON() failed [remote_snpn_list]");
			goto end;
		}
	...
	nrf_info_served_sepp_info_list_value_local_var =
		OpenAPI_nrf_info_served_sepp_info_list_value_create(
			sepp_prefix &&
			!cJSON_IsNull(sepp_prefix) ?
			ogs_strdup(sepp_prefix->valuestring) : NULL,
...
OpenAPI_nrf_info_served_sepp_info_list_value_t *
OpenAPI_nrf_info_served_sepp_info_list_value_copy(
	OpenAPI_nrf_info_served_sepp_info_list_value_t	*dst,
	OpenAPI_nrf_info_served_sepp_info_list_value_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_nrf_info_served_sepp_info_list_value_convertToJSON() failed");
		return NULL;
	}
