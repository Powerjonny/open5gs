...
OpenAPI_wlan_performance_info_t *OpenAPI_wlan_performance_info_create(
	...
	OpenAPI_wlan_performance_info_t *wlan_performance_info_local_var =
		ogs_malloc(sizeof(OpenAPI_wlan_performance_info_t));
...
void OpenAPI_wlan_performance_info_free(
	...
	if(wlan_performance_info->wlan_per_ssid_infos) {
		OpenAPI_list_for_each(
			wlan_performance_info->wlan_per_ssid_infos, node) {
...
cJSON *OpenAPI_wlan_performance_info_convertToJSON(
	...
	if(wlan_performance_info == NULL) {
		ogs_error(
			"OpenAPI_wlan_performance_info_convertToJSON() failed [WlanPerformanceInfo]");
		return NULL;
	}
	...
	if(wlan_performance_info->network_area) {
		...
		if(network_area_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_wlan_performance_info_convertToJSON() failed [network_area]");
			goto end;
		}
	...
	if(!wlan_performance_info->wlan_per_ssid_infos) {
		ogs_error(
			"OpenAPI_wlan_performance_info_convertToJSON() failed [wlan_per_ssid_infos]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(wlan_performance_info->wlan_per_ssid_infos,
	                      node) {
		cJSON *itemLocal =
			OpenAPI_wlan_per_ss_id_performance_info_convertToJSON(
				node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_wlan_performance_info_convertToJSON() failed [wlan_per_ssid_infos]");
...
OpenAPI_wlan_performance_info_t *OpenAPI_wlan_performance_info_parseFromJSON(
	...
	network_area =
		cJSON_GetObjectItemCaseSensitive(wlan_performance_infoJSON,
		                                 "networkArea");
	if(network_area) {
	...
	if(!wlan_per_ssid_infos) {
		ogs_error(
			"OpenAPI_wlan_performance_info_parseFromJSON() failed [wlan_per_ssid_infos]");
		goto end;
	}
	...
	cJSON_ArrayForEach(wlan_per_ssid_infos_local, wlan_per_ssid_infos) {
		...
		OpenAPI_list_add(wlan_per_ssid_infosList,
		                 wlan_per_ssid_infosItem);
	}
...
OpenAPI_wlan_performance_info_t *OpenAPI_wlan_performance_info_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_wlan_performance_info_convertToJSON() failed");
		return NULL;
	}
