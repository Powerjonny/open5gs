...
OpenAPI_requested_rule_data_t *OpenAPI_requested_rule_data_create(
	OpenAPI_list_t *ref_pcc_rule_ids, OpenAPI_list_t *req_data) {
	OpenAPI_requested_rule_data_t *requested_rule_data_local_var =
		ogs_malloc(sizeof(OpenAPI_requested_rule_data_t));
...
void OpenAPI_requested_rule_data_free(
	OpenAPI_requested_rule_data_t *requested_rule_data) {
	...
	if(requested_rule_data->ref_pcc_rule_ids) {
		OpenAPI_list_for_each(requested_rule_data->ref_pcc_rule_ids,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_requested_rule_data_convertToJSON(
	OpenAPI_requested_rule_data_t *requested_rule_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(requested_rule_data == NULL) {
		ogs_error(
			"OpenAPI_requested_rule_data_convertToJSON() failed [RequestedRuleData]");
		return NULL;
	}
	...
	if(!requested_rule_data->ref_pcc_rule_ids) {
		ogs_error(
			"OpenAPI_requested_rule_data_convertToJSON() failed [ref_pcc_rule_ids]");
		return NULL;
	}
	...
	if(requested_rule_data->req_data ==
	   OpenAPI_requested_rule_data_type_NULL)
	{
		ogs_error(
			"OpenAPI_requested_rule_data_convertToJSON() failed [req_data]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(requested_rule_data->req_data, node) {
		if(cJSON_AddStringToObject(req_dataList, "",
		                           OpenAPI_requested_rule_data_type_ToString
		                                   ((intptr_t) node->data)) ==
		   NULL)
		{
			ogs_error(
...
OpenAPI_requested_rule_data_t *OpenAPI_requested_rule_data_parseFromJSON(
	cJSON *requested_rule_dataJSON) {
	...
	ref_pcc_rule_ids =
		cJSON_GetObjectItemCaseSensitive(requested_rule_dataJSON,
		                                 "refPccRuleIds");
	if(!ref_pcc_rule_ids) {
		ogs_error(
			"OpenAPI_requested_rule_data_parseFromJSON() failed [ref_pcc_rule_ids]");
		goto end;
	}
	...
	cJSON_ArrayForEach(ref_pcc_rule_ids_local, ref_pcc_rule_ids) {
		...
		OpenAPI_list_add(ref_pcc_rule_idsList,
		                 ogs_strdup(
					 ref_pcc_rule_ids_local->valuestring));
	}

	req_data = cJSON_GetObjectItemCaseSensitive(requested_rule_dataJSON,
	                                            "reqData");
	if(!req_data) {
		ogs_error(
			"OpenAPI_requested_rule_data_parseFromJSON() failed [req_data]");
		goto end;
	}
	...
	cJSON_ArrayForEach(req_data_local, req_data) {
		OpenAPI_requested_rule_data_type_e localEnum =
			OpenAPI_requested_rule_data_type_NULL;
		...
		localEnum =
			OpenAPI_requested_rule_data_type_FromString(
				req_data_local->valuestring);
		if(!localEnum) {
			ogs_info(
				"Enum value \"%s\" for field \"req_data\" is not supported. Ignoring it ...",
				req_data_local->valuestring);
		} else {
	...
	if(req_dataList->count == 0) {
		ogs_error(
			"OpenAPI_requested_rule_data_parseFromJSON() failed: Expected req_dataList to not be empty (after ignoring unsupported enum values).");
		goto end;
	}
...
OpenAPI_requested_rule_data_t *OpenAPI_requested_rule_data_copy(
	OpenAPI_requested_rule_data_t	*dst,
	OpenAPI_requested_rule_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
