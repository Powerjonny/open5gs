...
OpenAPI_area_scope_t *OpenAPI_area_scope_create(
	...
	OpenAPI_area_scope_t *area_scope_local_var =
		ogs_malloc(sizeof(OpenAPI_area_scope_t));
...
cJSON *OpenAPI_area_scope_convertToJSON(OpenAPI_area_scope_t *area_scope) {
	...
	if(area_scope == NULL) {
		ogs_error(
			"OpenAPI_area_scope_convertToJSON() failed [AreaScope]");
		return NULL;
	}
	...
	if(area_scope->eutra_cell_id_list) {
		...
		if(eutra_cell_id_listList == NULL) {
			ogs_error(
				"OpenAPI_area_scope_convertToJSON() failed [eutra_cell_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(area_scope->eutra_cell_id_list, node) {
	...
	if(area_scope->nr_cell_id_list) {
		...
		if(nr_cell_id_listList == NULL) {
			ogs_error(
				"OpenAPI_area_scope_convertToJSON() failed [nr_cell_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(area_scope->nr_cell_id_list, node) {
	...
	if(area_scope->tac_list) {
		...
		if(tac_listList == NULL) {
			ogs_error(
				"OpenAPI_area_scope_convertToJSON() failed [tac_list]");
			goto end;
		}
		OpenAPI_list_for_each(area_scope->tac_list, node) {
	...
	if(area_scope->tac_info_per_plmn) {
		...
		if(tac_info_per_plmn == NULL) {
			ogs_error(
				"OpenAPI_area_scope_convertToJSON() failed [tac_info_per_plmn]");
			goto end;
		}
		cJSON *localMapObject = tac_info_per_plmn;
		if(area_scope->tac_info_per_plmn) {
			OpenAPI_list_for_each(area_scope->tac_info_per_plmn,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_tac_info_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_area_scope_convertToJSON() failed [inner]");
					goto end;
				}
...
OpenAPI_area_scope_t *OpenAPI_area_scope_parseFromJSON(cJSON *area_scopeJSON) {
	...
	if(eutra_cell_id_list) {
		...
		if(!cJSON_IsArray(eutra_cell_id_list)) {
			ogs_error(
				"OpenAPI_area_scope_parseFromJSON() failed [eutra_cell_id_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(eutra_cell_id_list_local,
			...
			OpenAPI_list_add(eutra_cell_id_listList,
			                 ogs_strdup(
						 eutra_cell_id_list_local->
						 valuestring));
		}
	}
	...
	if(nr_cell_id_list) {
		...
		if(!cJSON_IsArray(nr_cell_id_list)) {
			ogs_error(
				"OpenAPI_area_scope_parseFromJSON() failed [nr_cell_id_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(nr_cell_id_list_local, nr_cell_id_list) {
			...
			OpenAPI_list_add(nr_cell_id_listList,
			                 ogs_strdup(
						 nr_cell_id_list_local->
						 valuestring));
		}
	}
	...
	if(tac_list) {
		...
		if(!cJSON_IsArray(tac_list)) {
			ogs_error(
				"OpenAPI_area_scope_parseFromJSON() failed [tac_list]");
			goto end;
		}
	...
	if(tac_info_per_plmn) {
		...
		{
			ogs_error(
				"OpenAPI_area_scope_parseFromJSON() failed [tac_info_per_plmn]");
			goto end;
		}
		if(cJSON_IsObject(tac_info_per_plmn)) {
			...
			cJSON_ArrayForEach(tac_info_per_plmn_local_map,
				...
				if(cJSON_IsObject(localMapObject)) {
					localMapKeyPair = OpenAPI_map_create(
						...
						OpenAPI_tac_info_parseFromJSON(
							localMapObject));
					...
					ogs_error(
						"OpenAPI_area_scope_parseFromJSON() failed [inner]");
					goto end;
				}
				OpenAPI_list_add(tac_info_per_plmnList,
				                 localMapKeyPair);
			}
		}
	}
