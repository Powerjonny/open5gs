...
char * OpenAPI_code_word_ind_ToString(OpenAPI_code_word_ind_e code_word_ind) {
	const char *code_word_indArray[] = { "NULL", "CODEWORD_CHECK_IN_UE",
		                             "CODEWORD_CHECK_IN_GMLC" };
	size_t sizeofArray = sizeof(code_word_indArray) /
	                     sizeof(code_word_indArray[0]);
...
OpenAPI_code_word_ind_e OpenAPI_code_word_ind_FromString(char *code_word_ind) {
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(code_word_ind,
		          code_word_indArray[stringToReturn]) == 0) {
