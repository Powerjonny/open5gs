...
char * OpenAPI_event_for_mdt_ToString(OpenAPI_event_for_mdt_e event_for_mdt) {
	const char *event_for_mdtArray[] = { "NULL", "OUT_OF_COVERAG",
		                             "A2_EVENT" };
	size_t sizeofArray = sizeof(event_for_mdtArray) /
	                     sizeof(event_for_mdtArray[0]);
...
OpenAPI_event_for_mdt_e OpenAPI_event_for_mdt_FromString(char *event_for_mdt) {
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(event_for_mdt,
		          event_for_mdtArray[stringToReturn]) == 0) {
