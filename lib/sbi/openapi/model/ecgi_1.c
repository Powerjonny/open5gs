...
OpenAPI_ecgi_1_t *OpenAPI_ecgi_1_create(OpenAPI_plmn_id_1_t *plmn_id,
                                        char *eutra_cell_id, char *nid) {
	OpenAPI_ecgi_1_t *ecgi_1_local_var =
		ogs_malloc(sizeof(OpenAPI_ecgi_1_t));
...
cJSON *OpenAPI_ecgi_1_convertToJSON(OpenAPI_ecgi_1_t *ecgi_1) {
	...
	cJSON *plmn_id_local_JSON =
		OpenAPI_plmn_id_1_convertToJSON(ecgi_1->plmn_id);
	...
	if(!ecgi_1->eutra_cell_id) {
		ogs_error(
			"OpenAPI_ecgi_1_convertToJSON() failed [eutra_cell_id]");
		return NULL;
	}
	...
	if(ecgi_1->nid) {
		if(cJSON_AddStringToObject(item, "nid", ecgi_1->nid) == NULL) {
			ogs_error(
				"OpenAPI_ecgi_1_convertToJSON() failed [nid]");
			goto end;
		}
	}
...
OpenAPI_ecgi_1_t *OpenAPI_ecgi_1_parseFromJSON(cJSON *ecgi_1JSON) {
	...
	eutra_cell_id = cJSON_GetObjectItemCaseSensitive(ecgi_1JSON,
	                                                 "eutraCellId");
	if(!eutra_cell_id) {
		ogs_error(
			"OpenAPI_ecgi_1_parseFromJSON() failed [eutra_cell_id]");
		goto end;
	}
	...
	if(nid) {
		...
		{
			ogs_error(
				"OpenAPI_ecgi_1_parseFromJSON() failed [nid]");
			goto end;
		}
	}
...
OpenAPI_ecgi_1_t *OpenAPI_ecgi_1_copy(OpenAPI_ecgi_1_t	*dst,
                                      OpenAPI_ecgi_1_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
