...
OpenAPI_datalink_reporting_configuration_create(
	...
	ddd_status_list) {
	OpenAPI_datalink_reporting_configuration_t *
	        datalink_reporting_configuration_local_var =
		ogs_malloc(sizeof(OpenAPI_datalink_reporting_configuration_t));
	ogs_assert(datalink_reporting_configuration_local_var);

	datalink_reporting_configuration_local_var->ddd_traffic_des =
		ddd_traffic_des;
	datalink_reporting_configuration_local_var->dnn = dnn;
	datalink_reporting_configuration_local_var->slice = slice;
	datalink_reporting_configuration_local_var->ddd_status_list =
		ddd_status_list;

	return datalink_reporting_configuration_local_var;
}

void OpenAPI_datalink_reporting_configuration_free(
	OpenAPI_datalink_reporting_configuration_t *
	...
	if(datalink_reporting_configuration->ddd_traffic_des) {
		OpenAPI_list_for_each(
			datalink_reporting_configuration->ddd_traffic_des,
			node) {
...
cJSON *OpenAPI_datalink_reporting_configuration_convertToJSON(
	...
	if(datalink_reporting_configuration == NULL) {
		ogs_error(
			"OpenAPI_datalink_reporting_configuration_convertToJSON() failed [DatalinkReportingConfiguration]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(datalink_reporting_configuration->ddd_traffic_des) {
		cJSON *ddd_traffic_desList = cJSON_AddArrayToObject(item,
		                                                    "dddTrafficDes");
		if(ddd_traffic_desList == NULL) {
			ogs_error(
				"OpenAPI_datalink_reporting_configuration_convertToJSON() failed [ddd_traffic_des]");
			goto end;
		}
		OpenAPI_list_for_each(
			datalink_reporting_configuration->ddd_traffic_des,
	...
	if(datalink_reporting_configuration->dnn) {
		if(cJSON_AddStringToObject(item, "dnn",
		                           datalink_reporting_configuration->dnn)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_datalink_reporting_configuration_convertToJSON() failed [dnn]");
			goto end;
		}
	}

	if(datalink_reporting_configuration->slice) {
		cJSON *slice_local_JSON =
			OpenAPI_snssai_convertToJSON(
				datalink_reporting_configuration->slice);
		if(slice_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_datalink_reporting_configuration_convertToJSON() failed [slice]");
			goto end;
		}
	...
	{
		cJSON *ddd_status_listList = cJSON_AddArrayToObject(item,
		                                                    "dddStatusList");
		if(ddd_status_listList == NULL) {
			ogs_error(
				"OpenAPI_datalink_reporting_configuration_convertToJSON() failed [ddd_status_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			datalink_reporting_configuration->ddd_status_list,
			node) {
			if(cJSON_AddStringToObject(ddd_status_listList, "",
...
OpenAPI_datalink_reporting_configuration_parseFromJSON(
	cJSON *datalink_reporting_configurationJSON) {
	OpenAPI_datalink_reporting_configuration_t *
	        datalink_reporting_configuration_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	ddd_traffic_des =
		cJSON_GetObjectItemCaseSensitive(
			datalink_reporting_configurationJSON, "dddTrafficDes");
	if(ddd_traffic_des) {
		cJSON *ddd_traffic_des_local = NULL;
		if(!cJSON_IsArray(ddd_traffic_des)) {
			ogs_error(
				"OpenAPI_datalink_reporting_configuration_parseFromJSON() failed [ddd_traffic_des]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ddd_traffic_des_local, ddd_traffic_des) {
			...
			OpenAPI_ddd_traffic_descriptor_t *ddd_traffic_desItem =
				OpenAPI_ddd_traffic_descriptor_parseFromJSON(
					ddd_traffic_des_local);
			if(!ddd_traffic_desItem) {
				ogs_error("No ddd_traffic_desItem");
				goto end;
			}
			OpenAPI_list_add(ddd_traffic_desList,
			                 ddd_traffic_desItem);
		}
	}
	...
	dnn =
		cJSON_GetObjectItemCaseSensitive(
			datalink_reporting_configurationJSON, "dnn");
	if(dnn) {
		if(!cJSON_IsString(dnn) &&
		   !cJSON_IsNull(dnn))
		{
			ogs_error(
				"OpenAPI_datalink_reporting_configuration_parseFromJSON() failed [dnn]");
			goto end;
		}
	}
	...
	slice =
		cJSON_GetObjectItemCaseSensitive(
			datalink_reporting_configurationJSON, "slice");
	if(slice) {
		slice_local_nonprim = OpenAPI_snssai_parseFromJSON(slice);
		if(!slice_local_nonprim) {
			ogs_error(
				"OpenAPI_snssai_parseFromJSON failed [slice]");
			goto end;
		}
	}
	...
	ddd_status_list =
		cJSON_GetObjectItemCaseSensitive(
			datalink_reporting_configurationJSON, "dddStatusList");
	if(ddd_status_list) {
		cJSON *ddd_status_list_local = NULL;
		if(!cJSON_IsArray(ddd_status_list)) {
			ogs_error(
				"OpenAPI_datalink_reporting_configuration_parseFromJSON() failed [ddd_status_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ddd_status_list_local, ddd_status_list) {
			...
			if(!localEnum) {
				ogs_info(
					"Enum value \"%s\" for field \"ddd_status_list\" is not supported. Ignoring it ...",
					ddd_status_list_local->valuestring);
			} else {
				OpenAPI_list_add(ddd_status_listList,
				                 (void *) localEnum);
			}
		}
		if(ddd_status_listList->count == 0) {
			ogs_error(
				"OpenAPI_datalink_reporting_configuration_parseFromJSON() failed: Expected ddd_status_listList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
...
OpenAPI_datalink_reporting_configuration_t *
OpenAPI_datalink_reporting_configuration_copy(
	OpenAPI_datalink_reporting_configuration_t	*dst,
	OpenAPI_datalink_reporting_configuration_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_datalink_reporting_configuration_convertToJSON() failed");
		return NULL;
	}
