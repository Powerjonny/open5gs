...
OpenAPI_service_parameter_data_t *OpenAPI_service_parameter_data_create(
	char *app_id, char *dnn, OpenAPI_snssai_t *snssai, char *inter_group_id,
	char *supi, char *ue_ipv4, char *ue_ipv6, char *ue_mac,
	bool is_any_ue_ind, int any_ue_ind, char *param_over_pc5,
	char *param_over_uu, char *param_for_pro_se_dd,
	char *param_for_pro_se_dc, char *param_for_pro_se_u2_n_rel_ue,
	char *param_for_pro_se_rem_ue, OpenAPI_list_t *ursp_guidance,
	OpenAPI_list_t *delivery_events, char *polic_deliv_notif_corre_id,
	char *polic_deliv_notif_uri, char *supp_feat, char *res_uri,
	OpenAPI_list_t *headers, OpenAPI_list_t *reset_ids) {
	OpenAPI_service_parameter_data_t *service_parameter_data_local_var =
		ogs_malloc(sizeof(OpenAPI_service_parameter_data_t));
	...
	service_parameter_data_local_var->param_for_pro_se_dd =
		param_for_pro_se_dd;
	service_parameter_data_local_var->param_for_pro_se_dc =
		param_for_pro_se_dc;
	service_parameter_data_local_var->param_for_pro_se_u2_n_rel_ue =
		param_for_pro_se_u2_n_rel_ue;
	service_parameter_data_local_var->param_for_pro_se_rem_ue =
		param_for_pro_se_rem_ue;
	...
	service_parameter_data_local_var->polic_deliv_notif_corre_id =
		polic_deliv_notif_corre_id;
	service_parameter_data_local_var->polic_deliv_notif_uri =
		polic_deliv_notif_uri;
...
void OpenAPI_service_parameter_data_free(
	OpenAPI_service_parameter_data_t *service_parameter_data) {
	...
	if(service_parameter_data->ursp_guidance) {
		OpenAPI_list_for_each(service_parameter_data->ursp_guidance,
		                      node) {
	...
	if(service_parameter_data->delivery_events) {
		OpenAPI_list_for_each(service_parameter_data->delivery_events,
		                      node) {
...
cJSON *OpenAPI_service_parameter_data_convertToJSON(
	OpenAPI_service_parameter_data_t *service_parameter_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(service_parameter_data == NULL) {
		ogs_error(
			"OpenAPI_service_parameter_data_convertToJSON() failed [ServiceParameterData]");
		return NULL;
	}
	...
	if(service_parameter_data->app_id) {
		if(cJSON_AddStringToObject(item, "appId",
		                           service_parameter_data->app_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [app_id]");
			goto end;
		}
	}
	...
	if(service_parameter_data->dnn) {
		...
		{
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(service_parameter_data->snssai) {
		cJSON *snssai_local_JSON =
			OpenAPI_snssai_convertToJSON(
				service_parameter_data->snssai);
		if(snssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [snssai]");
			goto end;
		}
	...
	if(service_parameter_data->inter_group_id) {
		...
		{
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [inter_group_id]");
			goto end;
		}
	}
	...
	if(service_parameter_data->supi) {
		if(cJSON_AddStringToObject(item, "supi",
		                           service_parameter_data->supi) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(service_parameter_data->ue_ipv4) {
		if(cJSON_AddStringToObject(item, "ueIpv4",
		                           service_parameter_data->ue_ipv4) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [ue_ipv4]");
			goto end;
		}
	}
	...
	if(service_parameter_data->ue_ipv6) {
		if(cJSON_AddStringToObject(item, "ueIpv6",
		                           service_parameter_data->ue_ipv6) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [ue_ipv6]");
			goto end;
		}
	}
	...
	if(service_parameter_data->ue_mac) {
		if(cJSON_AddStringToObject(item, "ueMac",
		                           service_parameter_data->ue_mac) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [ue_mac]");
			goto end;
		}
	}

	if(service_parameter_data->is_any_ue_ind) {
		if(cJSON_AddBoolToObject(item, "anyUeInd",
		                         service_parameter_data->any_ue_ind) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [any_ue_ind]");
			goto end;
		}
	}

	if(service_parameter_data->param_over_pc5) {
		if(cJSON_AddStringToObject(item, "paramOverPc5",
		                           service_parameter_data->
		                           param_over_pc5) == NULL)
		{
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [param_over_pc5]");
			goto end;
		}
	}

	if(service_parameter_data->param_over_uu) {
		if(cJSON_AddStringToObject(item, "paramOverUu",
		                           service_parameter_data->param_over_uu)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [param_over_uu]");
			goto end;
		}
	}

	if(service_parameter_data->param_for_pro_se_dd) {
		if(cJSON_AddStringToObject(item, "paramForProSeDd",
		                           service_parameter_data->
		                           param_for_pro_se_dd) == NULL)
		{
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [param_for_pro_se_dd]");
			goto end;
		}
	}

	if(service_parameter_data->param_for_pro_se_dc) {
		if(cJSON_AddStringToObject(item, "paramForProSeDc",
		                           service_parameter_data->
		                           param_for_pro_se_dc) == NULL)
		{
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [param_for_pro_se_dc]");
			goto end;
		}
	}

	if(service_parameter_data->param_for_pro_se_u2_n_rel_ue) {
		if(cJSON_AddStringToObject(item, "paramForProSeU2NRelUe",
		                           service_parameter_data->
		                           param_for_pro_se_u2_n_rel_ue) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [param_for_pro_se_u2_n_rel_ue]");
			goto end;
		}
	}

	if(service_parameter_data->param_for_pro_se_rem_ue) {
		if(cJSON_AddStringToObject(item, "paramForProSeRemUe",
		                           service_parameter_data->
		                           param_for_pro_se_rem_ue) == NULL)
		{
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [param_for_pro_se_rem_ue]");
			goto end;
		}
	}
	...
	if(service_parameter_data->ursp_guidance) {
		...
		if(ursp_guidanceList == NULL) {
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [ursp_guidance]");
			goto end;
		}
		OpenAPI_list_for_each(service_parameter_data->ursp_guidance,
		                      node) {
	...
	if(service_parameter_data->delivery_events) {
		...
		if(delivery_eventsList == NULL) {
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [delivery_events]");
			goto end;
		}
		OpenAPI_list_for_each(service_parameter_data->delivery_events,
		                      node) {
	...
	if(service_parameter_data->polic_deliv_notif_corre_id) {
		if(cJSON_AddStringToObject(item, "policDelivNotifCorreId",
		                           service_parameter_data->
		                           polic_deliv_notif_corre_id) == NULL)
		{
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [polic_deliv_notif_corre_id]");
			goto end;
		}
	}

	if(service_parameter_data->polic_deliv_notif_uri) {
		if(cJSON_AddStringToObject(item, "policDelivNotifUri",
		                           service_parameter_data->
		                           polic_deliv_notif_uri) == NULL)
		{
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [polic_deliv_notif_uri]");
			goto end;
		}
	}
	...
	if(service_parameter_data->supp_feat) {
		if(cJSON_AddStringToObject(item, "suppFeat",
		                           service_parameter_data->supp_feat) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [supp_feat]");
			goto end;
		}
	}
	...
	if(service_parameter_data->res_uri) {
		if(cJSON_AddStringToObject(item, "resUri",
		                           service_parameter_data->res_uri) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [res_uri]");
			goto end;
		}
	}
	...
	if(service_parameter_data->headers) {
		...
		if(headersList == NULL) {
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [headers]");
			goto end;
		}
		OpenAPI_list_for_each(service_parameter_data->headers, node) {
	...
	if(service_parameter_data->reset_ids) {
		...
		if(reset_idsList == NULL) {
			ogs_error(
				"OpenAPI_service_parameter_data_convertToJSON() failed [reset_ids]");
			goto end;
		}
		OpenAPI_list_for_each(service_parameter_data->reset_ids, node) {
...
OpenAPI_service_parameter_data_t *OpenAPI_service_parameter_data_parseFromJSON(
	cJSON *service_parameter_dataJSON) {
	OpenAPI_service_parameter_data_t *service_parameter_data_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *app_id = NULL;
	...
	app_id = cJSON_GetObjectItemCaseSensitive(service_parameter_dataJSON,
	                                          "appId");
	if(app_id) {
		...
		{
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [app_id]");
			goto end;
		}
	}

	dnn = cJSON_GetObjectItemCaseSensitive(service_parameter_dataJSON,
	                                       "dnn");
	if(dnn) {
		...
		{
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [dnn]");
			goto end;
		}
	}

	snssai = cJSON_GetObjectItemCaseSensitive(service_parameter_dataJSON,
	                                          "snssai");
	if(snssai) {
	...
	inter_group_id =
		cJSON_GetObjectItemCaseSensitive(service_parameter_dataJSON,
		                                 "interGroupId");
	if(inter_group_id) {
		...
		{
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [inter_group_id]");
			goto end;
		}
	}

	supi = cJSON_GetObjectItemCaseSensitive(service_parameter_dataJSON,
	                                        "supi");
	if(supi) {
		...
		{
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [supi]");
			goto end;
		}
	}

	ue_ipv4 = cJSON_GetObjectItemCaseSensitive(service_parameter_dataJSON,
	                                           "ueIpv4");
	if(ue_ipv4) {
		...
		{
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [ue_ipv4]");
			goto end;
		}
	}

	ue_ipv6 = cJSON_GetObjectItemCaseSensitive(service_parameter_dataJSON,
	                                           "ueIpv6");
	if(ue_ipv6) {
		...
		{
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [ue_ipv6]");
			goto end;
		}
	}

	ue_mac = cJSON_GetObjectItemCaseSensitive(service_parameter_dataJSON,
	                                          "ueMac");
	if(ue_mac) {
		...
		{
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [ue_mac]");
			goto end;
		}
	}
	...
	if(any_ue_ind) {
		if(!cJSON_IsBool(any_ue_ind)) {
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [any_ue_ind]");
			goto end;
		}
	}
	...
	if(param_over_pc5) {
		if(!cJSON_IsString(param_over_pc5) &&
		   !cJSON_IsNull(param_over_pc5))
		{
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [param_over_pc5]");
			goto end;
		}
	}
	...
	if(param_over_uu) {
		if(!cJSON_IsString(param_over_uu) &&
		   !cJSON_IsNull(param_over_uu))
		{
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [param_over_uu]");
			goto end;
		}
	}
	...
	if(param_for_pro_se_dd) {
		if(!cJSON_IsString(param_for_pro_se_dd) &&
		   !cJSON_IsNull(param_for_pro_se_dd))
		{
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [param_for_pro_se_dd]");
			goto end;
		}
	}
	...
	if(param_for_pro_se_dc) {
		if(!cJSON_IsString(param_for_pro_se_dc) &&
		   !cJSON_IsNull(param_for_pro_se_dc))
		{
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [param_for_pro_se_dc]");
			goto end;
		}
	}
	...
	if(param_for_pro_se_u2_n_rel_ue) {
		if(!cJSON_IsString(param_for_pro_se_u2_n_rel_ue) &&
		   !cJSON_IsNull(param_for_pro_se_u2_n_rel_ue))
		{
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [param_for_pro_se_u2_n_rel_ue]");
			goto end;
		}
	}
	...
	if(param_for_pro_se_rem_ue) {
		if(!cJSON_IsString(param_for_pro_se_rem_ue) &&
		   !cJSON_IsNull(param_for_pro_se_rem_ue))
		{
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [param_for_pro_se_rem_ue]");
			goto end;
		}
	}
	...
	if(ursp_guidance) {
		...
		if(!cJSON_IsArray(ursp_guidance)) {
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [ursp_guidance]");
			goto end;
		}
	...
	if(delivery_events) {
		...
		if(!cJSON_IsArray(delivery_events)) {
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [delivery_events]");
			goto end;
		}
	...
	if(polic_deliv_notif_corre_id) {
		if(!cJSON_IsString(polic_deliv_notif_corre_id) &&
	...
	if(polic_deliv_notif_uri) {
		if(!cJSON_IsString(polic_deliv_notif_uri) &&
		   !cJSON_IsNull(polic_deliv_notif_uri))
		{
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [polic_deliv_notif_uri]");
			goto end;
		}
	}

	supp_feat = cJSON_GetObjectItemCaseSensitive(service_parameter_dataJSON,
	                                             "suppFeat");
	if(supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [supp_feat]");
			goto end;
		}
	}

	res_uri = cJSON_GetObjectItemCaseSensitive(service_parameter_dataJSON,
	                                           "resUri");
	if(res_uri) {
		...
		{
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [res_uri]");
			goto end;
		}
	}

	headers = cJSON_GetObjectItemCaseSensitive(service_parameter_dataJSON,
	                                           "headers");
	if(headers) {
		cJSON *headers_local = NULL;
		if(!cJSON_IsArray(headers)) {
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [headers]");
			goto end;
		}
	...
	reset_ids = cJSON_GetObjectItemCaseSensitive(service_parameter_dataJSON,
	                                             "resetIds");
	if(reset_ids) {
		cJSON *reset_ids_local = NULL;
		if(!cJSON_IsArray(reset_ids)) {
			ogs_error(
				"OpenAPI_service_parameter_data_parseFromJSON() failed [reset_ids]");
			goto end;
		}
	...
	service_parameter_data_local_var =
		OpenAPI_service_parameter_data_create(
			app_id &&
			!cJSON_IsNull(app_id) ?
			ogs_strdup(app_id->valuestring) : NULL,
			dnn &&
			...
			param_over_pc5 &&
			!cJSON_IsNull(param_over_pc5) ?
			ogs_strdup(param_over_pc5->valuestring) : NULL,
			param_over_uu &&
			!cJSON_IsNull(param_over_uu) ?
			ogs_strdup(param_over_uu->valuestring) : NULL,
			param_for_pro_se_dd &&
			!cJSON_IsNull(param_for_pro_se_dd) ?
			ogs_strdup(param_for_pro_se_dd->valuestring) : NULL,
			param_for_pro_se_dc &&
			!cJSON_IsNull(param_for_pro_se_dc) ?
			ogs_strdup(param_for_pro_se_dc->valuestring) : NULL,
			param_for_pro_se_u2_n_rel_ue &&
			!cJSON_IsNull(param_for_pro_se_u2_n_rel_ue) ?
			ogs_strdup(param_for_pro_se_u2_n_rel_ue->valuestring) :
			NULL,
			param_for_pro_se_rem_ue &&
			!cJSON_IsNull(param_for_pro_se_rem_ue) ?
			ogs_strdup(param_for_pro_se_rem_ue->valuestring) : NULL,
			ursp_guidance ? ursp_guidanceList : NULL,
			delivery_events ? delivery_eventsList : NULL,
			polic_deliv_notif_corre_id &&
			!cJSON_IsNull(polic_deliv_notif_corre_id) ?
			ogs_strdup(polic_deliv_notif_corre_id->valuestring) :
			NULL,
			polic_deliv_notif_uri &&
			!cJSON_IsNull(polic_deliv_notif_uri) ?
			ogs_strdup(polic_deliv_notif_uri->valuestring) : NULL,
			supp_feat &&
			!cJSON_IsNull(supp_feat) ?
...
OpenAPI_service_parameter_data_t *OpenAPI_service_parameter_data_copy(
	OpenAPI_service_parameter_data_t	*dst,
	OpenAPI_service_parameter_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_service_parameter_data_convertToJSON() failed");
		return NULL;
	}
