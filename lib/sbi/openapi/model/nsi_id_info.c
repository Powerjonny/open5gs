...
OpenAPI_nsi_id_info_t *OpenAPI_nsi_id_info_create(OpenAPI_snssai_t	*snssai,
                                                  OpenAPI_list_t	*nsi_ids)
{
	OpenAPI_nsi_id_info_t *nsi_id_info_local_var =
		ogs_malloc(sizeof(OpenAPI_nsi_id_info_t));
...
cJSON *OpenAPI_nsi_id_info_convertToJSON(OpenAPI_nsi_id_info_t *nsi_id_info) {
	...
	if(nsi_id_info == NULL) {
		ogs_error(
			"OpenAPI_nsi_id_info_convertToJSON() failed [NsiIdInfo]");
		return NULL;
	}
	...
	if(!nsi_id_info->snssai) {
		ogs_error(
			"OpenAPI_nsi_id_info_convertToJSON() failed [snssai]");
		return NULL;
	}
	cJSON *snssai_local_JSON =
		OpenAPI_snssai_convertToJSON(nsi_id_info->snssai);
	...
	if(nsi_id_info->nsi_ids) {
		...
		if(nsi_idsList == NULL) {
			ogs_error(
				"OpenAPI_nsi_id_info_convertToJSON() failed [nsi_ids]");
			goto end;
		}
		OpenAPI_list_for_each(nsi_id_info->nsi_ids, node) {
...
OpenAPI_nsi_id_info_t *OpenAPI_nsi_id_info_parseFromJSON(
	cJSON *nsi_id_infoJSON) {
	...
	if(!snssai) {
		ogs_error(
			"OpenAPI_nsi_id_info_parseFromJSON() failed [snssai]");
		goto end;
	}
	...
	if(nsi_ids) {
		...
		if(!cJSON_IsArray(nsi_ids)) {
			ogs_error(
				"OpenAPI_nsi_id_info_parseFromJSON() failed [nsi_ids]");
			goto end;
		}
		...
		cJSON_ArrayForEach(nsi_ids_local, nsi_ids) {
			...
			OpenAPI_list_add(nsi_idsList,
			                 ogs_strdup(
						 nsi_ids_local->valuestring));
		}
	}
...
OpenAPI_nsi_id_info_t *OpenAPI_nsi_id_info_copy(OpenAPI_nsi_id_info_t	*dst,
                                                OpenAPI_nsi_id_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
