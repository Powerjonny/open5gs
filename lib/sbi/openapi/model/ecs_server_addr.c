...
OpenAPI_ecs_server_addr_t *OpenAPI_ecs_server_addr_create(
	...
	OpenAPI_ecs_server_addr_t *ecs_server_addr_local_var =
		ogs_malloc(sizeof(OpenAPI_ecs_server_addr_t));
...
void OpenAPI_ecs_server_addr_free(OpenAPI_ecs_server_addr_t *ecs_server_addr) {
	...
	if(ecs_server_addr->ecs_ip_address_list) {
		OpenAPI_list_for_each(ecs_server_addr->ecs_ip_address_list,
		                      node) {
...
cJSON *OpenAPI_ecs_server_addr_convertToJSON(
	...
	if(ecs_server_addr == NULL) {
		ogs_error(
			"OpenAPI_ecs_server_addr_convertToJSON() failed [EcsServerAddr]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(ecs_server_addr->ecs_fqdn_list) {
		cJSON *ecs_fqdn_listList = cJSON_AddArrayToObject(item,
		                                                  "ecsFqdnList");
		if(ecs_fqdn_listList == NULL) {
			ogs_error(
				"OpenAPI_ecs_server_addr_convertToJSON() failed [ecs_fqdn_list]");
			goto end;
		}
		OpenAPI_list_for_each(ecs_server_addr->ecs_fqdn_list, node) {
			if(cJSON_AddStringToObject(ecs_fqdn_listList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_ecs_server_addr_convertToJSON() failed [ecs_fqdn_list]");
	...
	if(ecs_server_addr->ecs_ip_address_list) {
		cJSON *ecs_ip_address_listList = cJSON_AddArrayToObject(item,
		                                                        "ecsIpAddressList");
		if(ecs_ip_address_listList == NULL) {
			ogs_error(
				"OpenAPI_ecs_server_addr_convertToJSON() failed [ecs_ip_address_list]");
			goto end;
		}
		OpenAPI_list_for_each(ecs_server_addr->ecs_ip_address_list,
		                      node) {
			...
			cJSON_AddItemToArray(ecs_ip_address_listList,
			                     itemLocal);
		}
	}

	if(ecs_server_addr->ecs_uri_list) {
		cJSON *ecs_uri_listList = cJSON_AddArrayToObject(item,
		                                                 "ecsUriList");
		if(ecs_uri_listList == NULL) {
			ogs_error(
				"OpenAPI_ecs_server_addr_convertToJSON() failed [ecs_uri_list]");
			goto end;
		}
		OpenAPI_list_for_each(ecs_server_addr->ecs_uri_list, node) {
			if(cJSON_AddStringToObject(ecs_uri_listList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_ecs_server_addr_convertToJSON() failed [ecs_uri_list]");
	...
	if(ecs_server_addr->ecs_provider_id) {
		if(cJSON_AddStringToObject(item, "ecsProviderId",
		                           ecs_server_addr->ecs_provider_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_ecs_server_addr_convertToJSON() failed [ecs_provider_id]");
			goto end;
		}
	}
...
OpenAPI_ecs_server_addr_t *OpenAPI_ecs_server_addr_parseFromJSON(
	...
	ecs_fqdn_list = cJSON_GetObjectItemCaseSensitive(ecs_server_addrJSON,
	                                                 "ecsFqdnList");
	if(ecs_fqdn_list) {
		cJSON *ecs_fqdn_list_local = NULL;
		if(!cJSON_IsArray(ecs_fqdn_list)) {
			ogs_error(
				"OpenAPI_ecs_server_addr_parseFromJSON() failed [ecs_fqdn_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ecs_fqdn_list_local, ecs_fqdn_list) {
			...
			OpenAPI_list_add(ecs_fqdn_listList,
			                 ogs_strdup(
						 ecs_fqdn_list_local->
						 valuestring));
		}
	...
	ecs_ip_address_list =
		cJSON_GetObjectItemCaseSensitive(ecs_server_addrJSON,
		                                 "ecsIpAddressList");
	if(ecs_ip_address_list) {
		cJSON *ecs_ip_address_list_local = NULL;
		if(!cJSON_IsArray(ecs_ip_address_list)) {
			ogs_error(
				"OpenAPI_ecs_server_addr_parseFromJSON() failed [ecs_ip_address_list]");
			goto end;
		}

		ecs_ip_address_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(ecs_ip_address_list_local,
		                   ecs_ip_address_list) {
			...
			OpenAPI_ip_addr_t *ecs_ip_address_listItem =
				OpenAPI_ip_addr_parseFromJSON(
					ecs_ip_address_list_local);
			if(!ecs_ip_address_listItem) {
				ogs_error("No ecs_ip_address_listItem");
				goto end;
			}
			OpenAPI_list_add(ecs_ip_address_listList,
			                 ecs_ip_address_listItem);
		}
	}

	ecs_uri_list = cJSON_GetObjectItemCaseSensitive(ecs_server_addrJSON,
	                                                "ecsUriList");
	if(ecs_uri_list) {
		cJSON *ecs_uri_list_local = NULL;
		if(!cJSON_IsArray(ecs_uri_list)) {
			ogs_error(
				"OpenAPI_ecs_server_addr_parseFromJSON() failed [ecs_uri_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ecs_uri_list_local, ecs_uri_list) {
			...
			OpenAPI_list_add(ecs_uri_listList,
			                 ogs_strdup(
						 ecs_uri_list_local->valuestring));
		}
	}

	ecs_provider_id = cJSON_GetObjectItemCaseSensitive(ecs_server_addrJSON,
	                                                   "ecsProviderId");
	if(ecs_provider_id) {
		if(!cJSON_IsString(ecs_provider_id) &&
		   !cJSON_IsNull(ecs_provider_id))
		{
			ogs_error(
				"OpenAPI_ecs_server_addr_parseFromJSON() failed [ecs_provider_id]");
			goto end;
		}
	}
	...
	ecs_server_addr_local_var = OpenAPI_ecs_server_addr_create(
		...
		ecs_provider_id &&
		!cJSON_IsNull(ecs_provider_id) ?
		ogs_strdup(ecs_provider_id->valuestring) : NULL
		);
