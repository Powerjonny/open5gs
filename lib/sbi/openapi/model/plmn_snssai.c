...
OpenAPI_plmn_snssai_t *OpenAPI_plmn_snssai_create(
	...
	s_nssai_list,
	char *nid) {
	OpenAPI_plmn_snssai_t *plmn_snssai_local_var =
		ogs_malloc(sizeof(OpenAPI_plmn_snssai_t));
...
cJSON *OpenAPI_plmn_snssai_convertToJSON(OpenAPI_plmn_snssai_t *plmn_snssai) {
	...
	if(plmn_snssai == NULL) {
		ogs_error(
			"OpenAPI_plmn_snssai_convertToJSON() failed [PlmnSnssai]");
		return NULL;
	}
	...
	if(!plmn_snssai->plmn_id) {
		ogs_error(
			"OpenAPI_plmn_snssai_convertToJSON() failed [plmn_id]");
		return NULL;
	}
	cJSON *plmn_id_local_JSON =
		OpenAPI_plmn_id_convertToJSON(plmn_snssai->plmn_id);
	...
	if(!plmn_snssai->s_nssai_list) {
		ogs_error(
			"OpenAPI_plmn_snssai_convertToJSON() failed [s_nssai_list]");
		return NULL;
	}
	...
	if(plmn_snssai->nid) {
		...
		{
			ogs_error(
				"OpenAPI_plmn_snssai_convertToJSON() failed [nid]");
			goto end;
		}
	}
...
OpenAPI_plmn_snssai_t *OpenAPI_plmn_snssai_parseFromJSON(
	...
	if(!plmn_id) {
		ogs_error(
			"OpenAPI_plmn_snssai_parseFromJSON() failed [plmn_id]");
		goto end;
	}
	...
	s_nssai_list = cJSON_GetObjectItemCaseSensitive(plmn_snssaiJSON,
	                                                "sNssaiList");
	if(!s_nssai_list) {
		ogs_error(
			"OpenAPI_plmn_snssai_parseFromJSON() failed [s_nssai_list]");
		goto end;
	}
	...
	cJSON_ArrayForEach(s_nssai_list_local, s_nssai_list) {
		...
		OpenAPI_ext_snssai_t *s_nssai_listItem =
			OpenAPI_ext_snssai_parseFromJSON(s_nssai_list_local);
		if(!s_nssai_listItem) {
	...
	if(nid) {
		...
		{
			ogs_error(
				"OpenAPI_plmn_snssai_parseFromJSON() failed [nid]");
			goto end;
		}
	}
