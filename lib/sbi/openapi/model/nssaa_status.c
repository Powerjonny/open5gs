...
OpenAPI_nssaa_status_t *OpenAPI_nssaa_status_create(
	...
	status) {
	OpenAPI_nssaa_status_t *nssaa_status_local_var =
		ogs_malloc(sizeof(OpenAPI_nssaa_status_t));
...
cJSON *OpenAPI_nssaa_status_convertToJSON(
	OpenAPI_nssaa_status_t *nssaa_status) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(nssaa_status == NULL) {
		ogs_error(
			"OpenAPI_nssaa_status_convertToJSON() failed [NssaaStatus]");
		return NULL;
	}
	...
	if(!nssaa_status->snssai) {
		ogs_error(
			"OpenAPI_nssaa_status_convertToJSON() failed [snssai]");
		return NULL;
	}
	cJSON *snssai_local_JSON =
		OpenAPI_snssai_convertToJSON(nssaa_status->snssai);
	...
	if(nssaa_status->status == OpenAPI_auth_status_NULL) {
		ogs_error(
			"OpenAPI_nssaa_status_convertToJSON() failed [status]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "status",
	                           OpenAPI_auth_status_ToString(
...
OpenAPI_nssaa_status_t *OpenAPI_nssaa_status_parseFromJSON(
	...
	if(!snssai) {
		ogs_error(
			"OpenAPI_nssaa_status_parseFromJSON() failed [snssai]");
		goto end;
	}
	...
	if(!status) {
		ogs_error(
			"OpenAPI_nssaa_status_parseFromJSON() failed [status]");
		goto end;
	}
