...
OpenAPI_authorized_network_slice_info_create(
	...
	OpenAPI_list_t *
	rejected_nssai_in_ta, OpenAPI_nsi_information_t *nsi_information,
	char *supported_features, char *nrf_amf_set,
	char *nrf_amf_set_nf_mgt_uri, char *nrf_amf_set_access_token_uri,
	OpenAPI_list_t *nrf_oauth2_required, char *target_amf_service_set,
	OpenAPI_list_t *target_nssai, OpenAPI_list_t *nsag_infos) {
	OpenAPI_authorized_network_slice_info_t *
	        authorized_network_slice_info_local_var =
		ogs_malloc(sizeof(OpenAPI_authorized_network_slice_info_t));
	ogs_assert(authorized_network_slice_info_local_var);

	authorized_network_slice_info_local_var->allowed_nssai_list =
		allowed_nssai_list;
	authorized_network_slice_info_local_var->configured_nssai =
		configured_nssai;
	authorized_network_slice_info_local_var->target_amf_set =
		target_amf_set;
	authorized_network_slice_info_local_var->candidate_amf_list =
		candidate_amf_list;
	authorized_network_slice_info_local_var->rejected_nssai_in_plmn =
		rejected_nssai_in_plmn;
	authorized_network_slice_info_local_var->rejected_nssai_in_ta =
		rejected_nssai_in_ta;
	authorized_network_slice_info_local_var->nsi_information =
		nsi_information;
	authorized_network_slice_info_local_var->supported_features =
		supported_features;
	authorized_network_slice_info_local_var->nrf_amf_set = nrf_amf_set;
	authorized_network_slice_info_local_var->nrf_amf_set_nf_mgt_uri =
		nrf_amf_set_nf_mgt_uri;
	authorized_network_slice_info_local_var->nrf_amf_set_access_token_uri =
		nrf_amf_set_access_token_uri;
	authorized_network_slice_info_local_var->nrf_oauth2_required =
		nrf_oauth2_required;
	authorized_network_slice_info_local_var->target_amf_service_set =
		target_amf_service_set;
...
void OpenAPI_authorized_network_slice_info_free(
	OpenAPI_authorized_network_slice_info_t *authorized_network_slice_info)
{
	...
	if(authorized_network_slice_info->configured_nssai) {
		OpenAPI_list_for_each(
			authorized_network_slice_info->configured_nssai, node) {
	...
	if(authorized_network_slice_info->target_nssai) {
		OpenAPI_list_for_each(
			authorized_network_slice_info->target_nssai, node) {
			OpenAPI_snssai_free(node->data);
		}
	...
	if(authorized_network_slice_info->nsag_infos) {
		OpenAPI_list_for_each(authorized_network_slice_info->nsag_infos,
		                      node) {
...
cJSON *OpenAPI_authorized_network_slice_info_convertToJSON(
	OpenAPI_authorized_network_slice_info_t *authorized_network_slice_info)
{
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(authorized_network_slice_info == NULL) {
		ogs_error(
			"OpenAPI_authorized_network_slice_info_convertToJSON() failed [AuthorizedNetworkSliceInfo]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(authorized_network_slice_info->allowed_nssai_list) {
		cJSON *allowed_nssai_listList = cJSON_AddArrayToObject(item,
		                                                       "allowedNssaiList");
		if(allowed_nssai_listList == NULL) {
			ogs_error(
				"OpenAPI_authorized_network_slice_info_convertToJSON() failed [allowed_nssai_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			authorized_network_slice_info->allowed_nssai_list,
			node) {
			cJSON *itemLocal =
				OpenAPI_allowed_nssai_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_authorized_network_slice_info_convertToJSON() failed [allowed_nssai_list]");
				goto end;
	...
	if(authorized_network_slice_info->configured_nssai) {
		...
		if(configured_nssaiList == NULL) {
			ogs_error(
				"OpenAPI_authorized_network_slice_info_convertToJSON() failed [configured_nssai]");
			goto end;
		}
		OpenAPI_list_for_each(
			authorized_network_slice_info->configured_nssai, node) {
	...
	if(authorized_network_slice_info->target_amf_set) {
		if(cJSON_AddStringToObject(item, "targetAmfSet",
		                           authorized_network_slice_info->
		                           target_amf_set) == NULL)
		{
			ogs_error(
				"OpenAPI_authorized_network_slice_info_convertToJSON() failed [target_amf_set]");
			goto end;
		}
	}

	if(authorized_network_slice_info->candidate_amf_list) {
		cJSON *candidate_amf_listList = cJSON_AddArrayToObject(item,
		                                                       "candidateAmfList");
		if(candidate_amf_listList == NULL) {
			ogs_error(
				"OpenAPI_authorized_network_slice_info_convertToJSON() failed [candidate_amf_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			authorized_network_slice_info->candidate_amf_list,
			node) {
			if(cJSON_AddStringToObject(candidate_amf_listList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_authorized_network_slice_info_convertToJSON() failed [candidate_amf_list]");
	...
	if(authorized_network_slice_info->rejected_nssai_in_plmn) {
		...
		if(rejected_nssai_in_plmnList == NULL) {
			ogs_error(
				"OpenAPI_authorized_network_slice_info_convertToJSON() failed [rejected_nssai_in_plmn]");
			goto end;
		}
		OpenAPI_list_for_each(
			authorized_network_slice_info->rejected_nssai_in_plmn,
	...
	if(authorized_network_slice_info->rejected_nssai_in_ta) {
		...
		if(rejected_nssai_in_taList == NULL) {
			ogs_error(
				"OpenAPI_authorized_network_slice_info_convertToJSON() failed [rejected_nssai_in_ta]");
			goto end;
		}
		OpenAPI_list_for_each(
			authorized_network_slice_info->rejected_nssai_in_ta,
	...
	if(authorized_network_slice_info->nsi_information) {
		cJSON *nsi_information_local_JSON =
			OpenAPI_nsi_information_convertToJSON(
				authorized_network_slice_info->nsi_information);
		if(nsi_information_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_authorized_network_slice_info_convertToJSON() failed [nsi_information]");
			goto end;
		}
		cJSON_AddItemToObject(item, "nsiInformation",
		                      nsi_information_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_authorized_network_slice_info_convertToJSON() failed [nsi_information]");
			goto end;
	...
	if(authorized_network_slice_info->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_authorized_network_slice_info_convertToJSON() failed [supported_features]");
			goto end;
		}
	}

	if(authorized_network_slice_info->nrf_amf_set) {
		if(cJSON_AddStringToObject(item, "nrfAmfSet",
		                           authorized_network_slice_info->
		                           nrf_amf_set) == NULL)
		{
			ogs_error(
				"OpenAPI_authorized_network_slice_info_convertToJSON() failed [nrf_amf_set]");
			goto end;
		}
	}

	if(authorized_network_slice_info->nrf_amf_set_nf_mgt_uri) {
		if(cJSON_AddStringToObject(item, "nrfAmfSetNfMgtUri",
		                           authorized_network_slice_info->
		                           nrf_amf_set_nf_mgt_uri) == NULL)
		{
			ogs_error(
				"OpenAPI_authorized_network_slice_info_convertToJSON() failed [nrf_amf_set_nf_mgt_uri]");
			goto end;
		}
	}

	if(authorized_network_slice_info->nrf_amf_set_access_token_uri) {
		if(cJSON_AddStringToObject(item, "nrfAmfSetAccessTokenUri",
		                           authorized_network_slice_info->
		                           nrf_amf_set_access_token_uri) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_authorized_network_slice_info_convertToJSON() failed [nrf_amf_set_access_token_uri]");
			goto end;
		}
	}
	...
	if(authorized_network_slice_info->nrf_oauth2_required) {
		...
		if(nrf_oauth2_required == NULL) {
			ogs_error(
				"OpenAPI_authorized_network_slice_info_convertToJSON() failed [nrf_oauth2_required]");
			goto end;
		}
		...
		if(authorized_network_slice_info->nrf_oauth2_required) {
			OpenAPI_list_for_each(
				...
				{
					ogs_error(
						"OpenAPI_authorized_network_slice_info_convertToJSON() failed [inner]");
					goto end;
				}
			}
		}
	...
	if(authorized_network_slice_info->target_amf_service_set) {
		if(cJSON_AddStringToObject(item, "targetAmfServiceSet",
		                           authorized_network_slice_info->
		                           target_amf_service_set) == NULL)
		{
			ogs_error(
				"OpenAPI_authorized_network_slice_info_convertToJSON() failed [target_amf_service_set]");
			goto end;
		}
	}

	if(authorized_network_slice_info->target_nssai) {
		cJSON *target_nssaiList = cJSON_AddArrayToObject(item,
		                                                 "targetNssai");
		if(target_nssaiList == NULL) {
			ogs_error(
				"OpenAPI_authorized_network_slice_info_convertToJSON() failed [target_nssai]");
			goto end;
		}
		OpenAPI_list_for_each(
			authorized_network_slice_info->target_nssai, node) {
	...
	if(authorized_network_slice_info->nsag_infos) {
		cJSON *nsag_infosList = cJSON_AddArrayToObject(item,
		                                               "nsagInfos");
		if(nsag_infosList == NULL) {
			ogs_error(
				"OpenAPI_authorized_network_slice_info_convertToJSON() failed [nsag_infos]");
			goto end;
		}
		OpenAPI_list_for_each(authorized_network_slice_info->nsag_infos,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_nsag_info_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_authorized_network_slice_info_convertToJSON() failed [nsag_infos]");
				goto end;
...
OpenAPI_authorized_network_slice_info_parseFromJSON(
	...
	OpenAPI_authorized_network_slice_info_t *
	        authorized_network_slice_info_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	allowed_nssai_list =
		cJSON_GetObjectItemCaseSensitive(
			authorized_network_slice_infoJSON, "allowedNssaiList");
	if(allowed_nssai_list) {
		cJSON *allowed_nssai_list_local = NULL;
		if(!cJSON_IsArray(allowed_nssai_list)) {
			ogs_error(
				"OpenAPI_authorized_network_slice_info_parseFromJSON() failed [allowed_nssai_list]");
			goto end;
		}

		allowed_nssai_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(allowed_nssai_list_local,
		                   allowed_nssai_list) {
			...
			OpenAPI_allowed_nssai_t *allowed_nssai_listItem =
				OpenAPI_allowed_nssai_parseFromJSON(
					allowed_nssai_list_local);
			if(!allowed_nssai_listItem) {
				ogs_error("No allowed_nssai_listItem");
				goto end;
			}
			OpenAPI_list_add(allowed_nssai_listList,
			                 allowed_nssai_listItem);
		}
	}

	configured_nssai =
		cJSON_GetObjectItemCaseSensitive(
			authorized_network_slice_infoJSON, "configuredNssai");
	if(configured_nssai) {
		cJSON *configured_nssai_local = NULL;
		if(!cJSON_IsArray(configured_nssai)) {
			ogs_error(
				"OpenAPI_authorized_network_slice_info_parseFromJSON() failed [configured_nssai]");
			goto end;
		}
		...
		cJSON_ArrayForEach(configured_nssai_local, configured_nssai) {
			...
			OpenAPI_configured_snssai_t *configured_nssaiItem =
			...
			OpenAPI_list_add(configured_nssaiList,
			                 configured_nssaiItem);
		}
	}

	target_amf_set =
		cJSON_GetObjectItemCaseSensitive(
			authorized_network_slice_infoJSON, "targetAmfSet");
	if(target_amf_set) {
		if(!cJSON_IsString(target_amf_set) &&
		   !cJSON_IsNull(target_amf_set))
		{
			ogs_error(
				"OpenAPI_authorized_network_slice_info_parseFromJSON() failed [target_amf_set]");
			goto end;
		}
	}
	...
	candidate_amf_list =
		cJSON_GetObjectItemCaseSensitive(
			authorized_network_slice_infoJSON, "candidateAmfList");
	if(candidate_amf_list) {
		cJSON *candidate_amf_list_local = NULL;
		if(!cJSON_IsArray(candidate_amf_list)) {
			ogs_error(
				"OpenAPI_authorized_network_slice_info_parseFromJSON() failed [candidate_amf_list]");
			goto end;
		}

		candidate_amf_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(candidate_amf_list_local,
		                   candidate_amf_list) {
			...
			OpenAPI_list_add(candidate_amf_listList,
			                 ogs_strdup(
						 candidate_amf_list_local->
						 valuestring));
		}
	}
	...
	rejected_nssai_in_plmn =
		cJSON_GetObjectItemCaseSensitive(
			authorized_network_slice_infoJSON,
			"rejectedNssaiInPlmn");
	if(rejected_nssai_in_plmn) {
		cJSON *rejected_nssai_in_plmn_local = NULL;
		if(!cJSON_IsArray(rejected_nssai_in_plmn)) {
			ogs_error(
				"OpenAPI_authorized_network_slice_info_parseFromJSON() failed [rejected_nssai_in_plmn]");
			goto end;
		}
		...
		cJSON_ArrayForEach(rejected_nssai_in_plmn_local,
			...
			OpenAPI_list_add(rejected_nssai_in_plmnList,
	...
	rejected_nssai_in_ta =
		cJSON_GetObjectItemCaseSensitive(
			authorized_network_slice_infoJSON, "rejectedNssaiInTa");
	if(rejected_nssai_in_ta) {
		cJSON *rejected_nssai_in_ta_local = NULL;
		if(!cJSON_IsArray(rejected_nssai_in_ta)) {
			ogs_error(
				"OpenAPI_authorized_network_slice_info_parseFromJSON() failed [rejected_nssai_in_ta]");
			goto end;
		}
		...
		cJSON_ArrayForEach(rejected_nssai_in_ta_local,
			...
			OpenAPI_snssai_t *rejected_nssai_in_taItem =
				OpenAPI_snssai_parseFromJSON(
					rejected_nssai_in_ta_local);
			if(!rejected_nssai_in_taItem) {
				ogs_error("No rejected_nssai_in_taItem");
				goto end;
			}
			OpenAPI_list_add(rejected_nssai_in_taList,
	...
	nsi_information =
		cJSON_GetObjectItemCaseSensitive(
			authorized_network_slice_infoJSON, "nsiInformation");
	if(nsi_information) {
		nsi_information_local_nonprim =
			OpenAPI_nsi_information_parseFromJSON(nsi_information);
		if(!nsi_information_local_nonprim) {
			ogs_error(
				"OpenAPI_nsi_information_parseFromJSON failed [nsi_information]");
			goto end;
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_authorized_network_slice_info_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	nrf_amf_set =
		cJSON_GetObjectItemCaseSensitive(
			authorized_network_slice_infoJSON, "nrfAmfSet");
	if(nrf_amf_set) {
		if(!cJSON_IsString(nrf_amf_set) &&
		   !cJSON_IsNull(nrf_amf_set))
		{
			ogs_error(
				"OpenAPI_authorized_network_slice_info_parseFromJSON() failed [nrf_amf_set]");
			goto end;
		}
	}
	...
	nrf_amf_set_nf_mgt_uri =
		cJSON_GetObjectItemCaseSensitive(
			authorized_network_slice_infoJSON, "nrfAmfSetNfMgtUri");
	if(nrf_amf_set_nf_mgt_uri) {
		if(!cJSON_IsString(nrf_amf_set_nf_mgt_uri) &&
		   !cJSON_IsNull(nrf_amf_set_nf_mgt_uri))
		{
			ogs_error(
				"OpenAPI_authorized_network_slice_info_parseFromJSON() failed [nrf_amf_set_nf_mgt_uri]");
			goto end;
		}
	}
	...
	if(nrf_amf_set_access_token_uri) {
		if(!cJSON_IsString(nrf_amf_set_access_token_uri) &&
		   !cJSON_IsNull(nrf_amf_set_access_token_uri))
		{
			ogs_error(
				"OpenAPI_authorized_network_slice_info_parseFromJSON() failed [nrf_amf_set_access_token_uri]");
			goto end;
		}
	}
	...
	nrf_oauth2_required =
		cJSON_GetObjectItemCaseSensitive(
			authorized_network_slice_infoJSON, "nrfOauth2Required");
	if(nrf_oauth2_required) {
		...
		{
			ogs_error(
				"OpenAPI_authorized_network_slice_info_parseFromJSON() failed [nrf_oauth2_required]");
			goto end;
		}
		if(cJSON_IsObject(nrf_oauth2_required)) {
			...
			cJSON_ArrayForEach(nrf_oauth2_required_local_map,
				...
				if(!cJSON_IsBool(localMapObject)) {
					ogs_error(
						"OpenAPI_authorized_network_slice_info_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	if(target_amf_service_set) {
		if(!cJSON_IsString(target_amf_service_set) &&
		   !cJSON_IsNull(target_amf_service_set))
		{
			ogs_error(
				"OpenAPI_authorized_network_slice_info_parseFromJSON() failed [target_amf_service_set]");
			goto end;
		}
	}
	...
	target_nssai =
		cJSON_GetObjectItemCaseSensitive(
			authorized_network_slice_infoJSON, "targetNssai");
	if(target_nssai) {
		cJSON *target_nssai_local = NULL;
		if(!cJSON_IsArray(target_nssai)) {
			ogs_error(
				"OpenAPI_authorized_network_slice_info_parseFromJSON() failed [target_nssai]");
			goto end;
		}
		...
		cJSON_ArrayForEach(target_nssai_local, target_nssai) {
			...
			OpenAPI_snssai_t *target_nssaiItem =
				OpenAPI_snssai_parseFromJSON(
					target_nssai_local);
			if(!target_nssaiItem) {
	...
	nsag_infos =
		cJSON_GetObjectItemCaseSensitive(
			authorized_network_slice_infoJSON, "nsagInfos");
	if(nsag_infos) {
		cJSON *nsag_infos_local = NULL;
		if(!cJSON_IsArray(nsag_infos)) {
			ogs_error(
				"OpenAPI_authorized_network_slice_info_parseFromJSON() failed [nsag_infos]");
			goto end;
		}
		...
		cJSON_ArrayForEach(nsag_infos_local, nsag_infos) {
			...
			OpenAPI_nsag_info_t *nsag_infosItem =
				OpenAPI_nsag_info_parseFromJSON(
					nsag_infos_local);
			if(!nsag_infosItem) {
	...
	authorized_network_slice_info_local_var =
		OpenAPI_authorized_network_slice_info_create(
			...
			target_amf_set &&
			!cJSON_IsNull(target_amf_set) ?
			ogs_strdup(target_amf_set->valuestring) : NULL,
			candidate_amf_list ? candidate_amf_listList : NULL,
			rejected_nssai_in_plmn ? rejected_nssai_in_plmnList :
			NULL,
			...
			nrf_amf_set &&
			!cJSON_IsNull(nrf_amf_set) ?
			ogs_strdup(nrf_amf_set->valuestring) : NULL,
			nrf_amf_set_nf_mgt_uri &&
			!cJSON_IsNull(nrf_amf_set_nf_mgt_uri) ?
			ogs_strdup(nrf_amf_set_nf_mgt_uri->valuestring) : NULL,
			nrf_amf_set_access_token_uri &&
			!cJSON_IsNull(nrf_amf_set_access_token_uri) ?
			...
			target_amf_service_set &&
			!cJSON_IsNull(target_amf_service_set) ?
			ogs_strdup(target_amf_service_set->valuestring) : NULL,
...
OpenAPI_authorized_network_slice_info_t *
OpenAPI_authorized_network_slice_info_copy(
	OpenAPI_authorized_network_slice_info_t *dst,
	OpenAPI_authorized_network_slice_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_authorized_network_slice_info_convertToJSON() failed");
		return NULL;
	}
