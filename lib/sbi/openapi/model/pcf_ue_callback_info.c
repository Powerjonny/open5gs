...
OpenAPI_pcf_ue_callback_info_t *OpenAPI_pcf_ue_callback_info_create(
	...
	OpenAPI_pcf_ue_callback_info_t *pcf_ue_callback_info_local_var =
		ogs_malloc(sizeof(OpenAPI_pcf_ue_callback_info_t));
...
cJSON *OpenAPI_pcf_ue_callback_info_convertToJSON(
	...
	if(pcf_ue_callback_info == NULL) {
		ogs_error(
			"OpenAPI_pcf_ue_callback_info_convertToJSON() failed [PcfUeCallbackInfo]");
		return NULL;
	}
	...
	if(!pcf_ue_callback_info->callback_uri) {
		ogs_error(
			"OpenAPI_pcf_ue_callback_info_convertToJSON() failed [callback_uri]");
		return NULL;
	}
	...
	if(pcf_ue_callback_info->binding_info) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_ue_callback_info_convertToJSON() failed [binding_info]");
			goto end;
		}
	}
...
OpenAPI_pcf_ue_callback_info_t *OpenAPI_pcf_ue_callback_info_parseFromJSON(
	...
	callback_uri =
		cJSON_GetObjectItemCaseSensitive(pcf_ue_callback_infoJSON,
		                                 "callbackUri");
	if(!callback_uri) {
		ogs_error(
			"OpenAPI_pcf_ue_callback_info_parseFromJSON() failed [callback_uri]");
		goto end;
	}
	...
	if(binding_info) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_ue_callback_info_parseFromJSON() failed [binding_info]");
			goto end;
		}
	}
...
OpenAPI_pcf_ue_callback_info_t *OpenAPI_pcf_ue_callback_info_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_pcf_ue_callback_info_convertToJSON() failed");
		return NULL;
	}
