...
OpenAPI_analytics_metadata_info_t *OpenAPI_analytics_metadata_info_create(
	bool is_num_samples, int num_samples,
	OpenAPI_time_window_t *data_window, OpenAPI_list_t *data_stat_props,
	OpenAPI_output_strategy_t *strategy, OpenAPI_accuracy_t *accuracy) {
	OpenAPI_analytics_metadata_info_t *analytics_metadata_info_local_var =
		ogs_malloc(sizeof(OpenAPI_analytics_metadata_info_t));
...
void OpenAPI_analytics_metadata_info_free(
	...
	if(analytics_metadata_info->data_stat_props) {
		OpenAPI_list_for_each(analytics_metadata_info->data_stat_props,
		                      node) {
...
cJSON *OpenAPI_analytics_metadata_info_convertToJSON(
	OpenAPI_analytics_metadata_info_t *analytics_metadata_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(analytics_metadata_info == NULL) {
		ogs_error(
			"OpenAPI_analytics_metadata_info_convertToJSON() failed [AnalyticsMetadataInfo]");
		return NULL;
	}
	...
	if(analytics_metadata_info->is_num_samples) {
		...
		{
			ogs_error(
				"OpenAPI_analytics_metadata_info_convertToJSON() failed [num_samples]");
			goto end;
		}
	}

	if(analytics_metadata_info->data_window) {
		cJSON *data_window_local_JSON =
			OpenAPI_time_window_convertToJSON(
				analytics_metadata_info->data_window);
		if(data_window_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_analytics_metadata_info_convertToJSON() failed [data_window]");
			goto end;
		}
	...
	if(analytics_metadata_info->data_stat_props) {
		...
		if(data_stat_propsList == NULL) {
			ogs_error(
				"OpenAPI_analytics_metadata_info_convertToJSON() failed [data_stat_props]");
			goto end;
		}
		OpenAPI_list_for_each(analytics_metadata_info->data_stat_props,
		                      node) {
	...
	if(analytics_metadata_info->strategy) {
		cJSON *strategy_local_JSON =
			OpenAPI_output_strategy_convertToJSON(
				analytics_metadata_info->strategy);
		if(strategy_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_analytics_metadata_info_convertToJSON() failed [strategy]");
			goto end;
		}
	...
	if(analytics_metadata_info->accuracy) {
		...
		if(accuracy_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_analytics_metadata_info_convertToJSON() failed [accuracy]");
			goto end;
		}
...
OpenAPI_analytics_metadata_info_t *OpenAPI_analytics_metadata_info_parseFromJSON
	...
	num_samples =
		cJSON_GetObjectItemCaseSensitive(analytics_metadata_infoJSON,
		                                 "numSamples");
	if(num_samples) {
		if(!cJSON_IsNumber(num_samples)) {
			ogs_error(
				"OpenAPI_analytics_metadata_info_parseFromJSON() failed [num_samples]");
			goto end;
		}
	}
	...
	data_window =
		cJSON_GetObjectItemCaseSensitive(analytics_metadata_infoJSON,
		                                 "dataWindow");
	if(data_window) {
		data_window_local_nonprim =
			OpenAPI_time_window_parseFromJSON(data_window);
		if(!data_window_local_nonprim) {
			ogs_error(
				"OpenAPI_time_window_parseFromJSON failed [data_window]");
			goto end;
		}
	}
	...
	if(data_stat_props) {
		...
		if(!cJSON_IsArray(data_stat_props)) {
			ogs_error(
				"OpenAPI_analytics_metadata_info_parseFromJSON() failed [data_stat_props]");
			goto end;
		}
		...
		cJSON_ArrayForEach(data_stat_props_local, data_stat_props) {
			...
			OpenAPI_dataset_statistical_property_t *
			        data_stat_propsItem =
				OpenAPI_dataset_statistical_property_parseFromJSON
				        (data_stat_props_local);
			if(!data_stat_propsItem) {
				ogs_error("No data_stat_propsItem");
				goto end;
			}
			OpenAPI_list_add(data_stat_propsList,
			                 data_stat_propsItem);
		}
	}
	...
	if(strategy) {
		strategy_local_nonprim =
			OpenAPI_output_strategy_parseFromJSON(strategy);
		if(!strategy_local_nonprim) {
			ogs_error(
				"OpenAPI_output_strategy_parseFromJSON failed [strategy]");
			goto end;
		}
	}
	...
	analytics_metadata_info_local_var =
...
OpenAPI_analytics_metadata_info_t *OpenAPI_analytics_metadata_info_copy(
	OpenAPI_analytics_metadata_info_t	*dst,
	OpenAPI_analytics_metadata_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_analytics_metadata_info_convertToJSON() failed");
		return NULL;
	}
