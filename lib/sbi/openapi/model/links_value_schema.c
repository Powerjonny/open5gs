...
OpenAPI_links_value_schema_t *OpenAPI_links_value_schema_create(char *href) {
	OpenAPI_links_value_schema_t *links_value_schema_local_var =
		ogs_malloc(sizeof(OpenAPI_links_value_schema_t));
...
cJSON *OpenAPI_links_value_schema_convertToJSON(
	...
	if(links_value_schema == NULL) {
		ogs_error(
			"OpenAPI_links_value_schema_convertToJSON() failed [LinksValueSchema]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(links_value_schema->href) {
		if(cJSON_AddStringToObject(item, "href",
		                           links_value_schema->href) == NULL)
		{
			ogs_error(
				"OpenAPI_links_value_schema_convertToJSON() failed [href]");
			goto end;
		}
	}
...
OpenAPI_links_value_schema_t *OpenAPI_links_value_schema_parseFromJSON(
	...
	if(href) {
		...
		{
			ogs_error(
				"OpenAPI_links_value_schema_parseFromJSON() failed [href]");
			goto end;
		}
	}
	...
	links_value_schema_local_var = OpenAPI_links_value_schema_create(
		href &&
		!cJSON_IsNull(href) ? ogs_strdup(href->valuestring) : NULL
		);
