...
OpenAPI_status_info_t *OpenAPI_status_info_create(
	OpenAPI_resource_status_e resource_status, OpenAPI_cause_e cause,
	OpenAPI_cn_assisted_ran_para_t *cn_assisted_ran_para,
	OpenAPI_access_type_e an_type) {
	OpenAPI_status_info_t *status_info_local_var =
		ogs_malloc(sizeof(OpenAPI_status_info_t));
...
void OpenAPI_status_info_free(OpenAPI_status_info_t *status_info) {
	...
	if(status_info->cn_assisted_ran_para) {
		OpenAPI_cn_assisted_ran_para_free(
			status_info->cn_assisted_ran_para);
...
cJSON *OpenAPI_status_info_convertToJSON(OpenAPI_status_info_t *status_info) {
	...
	if(status_info == NULL) {
		ogs_error(
			"OpenAPI_status_info_convertToJSON() failed [StatusInfo]");
		return NULL;
	}
	...
	if(status_info->resource_status == OpenAPI_resource_status_NULL) {
		ogs_error(
			"OpenAPI_status_info_convertToJSON() failed [resource_status]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "resourceStatus",
	                           OpenAPI_resource_status_ToString(
	...
	if(status_info->cause != OpenAPI_cause_NULL) {
		if(cJSON_AddStringToObject(item, "cause",
						   ...
						   status_info->cause)) == NULL)
		{
			ogs_error(
				"OpenAPI_status_info_convertToJSON() failed [cause]");
			goto end;
		}
	}
	...
	if(status_info->cn_assisted_ran_para) {
		...
		if(cn_assisted_ran_para_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_status_info_convertToJSON() failed [cn_assisted_ran_para]");
			goto end;
		}
	...
	if(status_info->an_type != OpenAPI_access_type_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_status_info_convertToJSON() failed [an_type]");
			goto end;
		}
	}
...
OpenAPI_status_info_t *OpenAPI_status_info_parseFromJSON(
	cJSON *status_infoJSON) {
	...
	resource_status = cJSON_GetObjectItemCaseSensitive(status_infoJSON,
	                                                   "resourceStatus");
	if(!resource_status) {
		ogs_error(
			"OpenAPI_status_info_parseFromJSON() failed [resource_status]");
		goto end;
	}
	...
	resource_statusVariable =
		OpenAPI_resource_status_FromString(
			resource_status->valuestring);

	cause = cJSON_GetObjectItemCaseSensitive(status_infoJSON, "cause");
	if(cause) {
		if(!cJSON_IsString(cause)) {
			ogs_error(
				"OpenAPI_status_info_parseFromJSON() failed [cause]");
			goto end;
		}
		causeVariable = OpenAPI_cause_FromString(cause->valuestring);
	}

	cn_assisted_ran_para = cJSON_GetObjectItemCaseSensitive(status_infoJSON,
	                                                        "cnAssistedRanPara");
	...
	if(an_type) {
		if(!cJSON_IsString(an_type)) {
			ogs_error(
				"OpenAPI_status_info_parseFromJSON() failed [an_type]");
			goto end;
		}
		an_typeVariable =
...
OpenAPI_status_info_t *OpenAPI_status_info_copy(OpenAPI_status_info_t	*dst,
                                                OpenAPI_status_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
