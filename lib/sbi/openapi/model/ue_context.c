...
OpenAPI_ue_context_t *OpenAPI_ue_context_create(char *supi,
...
{
	OpenAPI_ue_context_t *ue_context_local_var =
		ogs_malloc(sizeof(OpenAPI_ue_context_t));
...
void OpenAPI_ue_context_free(OpenAPI_ue_context_t *ue_context) {
	...
	if(ue_context->event_subscription_list) {
		OpenAPI_list_for_each(ue_context->event_subscription_list,
		                      node) {
	...
	if(ue_context->eps_interworking_info) {
		OpenAPI_eps_interworking_info_free(
	...
	if(ue_context->small_data_rate_status_infos) {
		OpenAPI_list_for_each(ue_context->small_data_rate_status_infos,
		                      node) {
	...
	if(ue_context->analytics_subscription_list) {
		OpenAPI_list_for_each(ue_context->analytics_subscription_list,
		                      node) {
	...
	if(ue_context->sm_policy_notify_pdu_list) {
		OpenAPI_list_for_each(ue_context->sm_policy_notify_pdu_list,
		                      node) {
...
cJSON *OpenAPI_ue_context_convertToJSON(OpenAPI_ue_context_t *ue_context) {
	...
	if(ue_context == NULL) {
		ogs_error(
			"OpenAPI_ue_context_convertToJSON() failed [UeContext]");
		return NULL;
	}
	...
	if(ue_context->supi) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(ue_context->is_supi_unauth_ind) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [supi_unauth_ind]");
			goto end;
		}
	}
	...
	if(ue_context->gpsi_list) {
		...
		if(gpsi_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [gpsi_list]");
			goto end;
		}
		OpenAPI_list_for_each(ue_context->gpsi_list, node) {
	...
	if(ue_context->pei) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [pei]");
			goto end;
		}
	}
	...
	if(ue_context->udm_group_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [udm_group_id]");
			goto end;
		}
	}
	...
	if(ue_context->ausf_group_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [ausf_group_id]");
			goto end;
		}
	}
	...
	if(ue_context->pcf_group_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [pcf_group_id]");
			goto end;
		}
	}
	...
	if(ue_context->routing_indicator) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [routing_indicator]");
			goto end;
		}
	}
	...
	if(ue_context->is_h_nw_pub_key_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [h_nw_pub_key_id]");
			goto end;
		}
	}

	if(ue_context->group_list) {
		cJSON *group_listList = cJSON_AddArrayToObject(item,
		                                               "groupList");
		if(group_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [group_list]");
			goto end;
		}
		OpenAPI_list_for_each(ue_context->group_list, node) {
	...
	if(ue_context->drx_parameter) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [drx_parameter]");
			goto end;
		}
	}
	...
	if(ue_context->is_sub_rfsp) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [sub_rfsp]");
			goto end;
		}
	}
	...
	if(ue_context->is_used_rfsp) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [used_rfsp]");
			goto end;
		}
	}

	if(ue_context->sub_ue_ambr) {
		cJSON *sub_ue_ambr_local_JSON =
			OpenAPI_ambr_convertToJSON(ue_context->sub_ue_ambr);
		if(sub_ue_ambr_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [sub_ue_ambr]");
			goto end;
		}
	...
	if(ue_context->sub_ue_slice_mbr_list) {
		...
		if(sub_ue_slice_mbr_list == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [sub_ue_slice_mbr_list]");
			goto end;
		}
		cJSON *localMapObject = sub_ue_slice_mbr_list;
		if(ue_context->sub_ue_slice_mbr_list) {
			OpenAPI_list_for_each(ue_context->sub_ue_slice_mbr_list,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_slice_mbr_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_ue_context_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(ue_context->smsf_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [smsf_id]");
			goto end;
		}
	}

	if(ue_context->seaf_data) {
		cJSON *seaf_data_local_JSON =
			OpenAPI_seaf_data_convertToJSON(ue_context->seaf_data);
		if(seaf_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [seaf_data]");
			goto end;
		}
	...
	if(ue_context->_5g_mm_capability) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [_5g_mm_capability]");
			goto end;
		}
	}
	...
	if(ue_context->pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [pcf_id]");
			goto end;
		}
	}
	...
	if(ue_context->pcf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [pcf_set_id]");
			goto end;
		}
	}
	...
	if(ue_context->pcf_amp_service_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [pcf_amp_service_set_id]");
			goto end;
		}
	}
	...
	if(ue_context->pcf_uep_service_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [pcf_uep_service_set_id]");
			goto end;
		}
	}
	...
	if(ue_context->pcf_binding != OpenAPI_sbi_binding_level_NULL) {
		if(cJSON_AddStringToObject(item, "pcfBinding",
						   ...
						   ue_context->pcf_binding)) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [pcf_binding]");
			goto end;
		}
	}
	...
	if(ue_context->pcf_am_policy_uri) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [pcf_am_policy_uri]");
			goto end;
		}
	}

	if(ue_context->am_policy_req_trigger_list !=
	   OpenAPI_policy_req_trigger_NULL)
	{
		cJSON *am_policy_req_trigger_listList =
			cJSON_AddArrayToObject(item, "amPolicyReqTriggerList");
		if(am_policy_req_trigger_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [am_policy_req_trigger_list]");
			goto end;
		}
		OpenAPI_list_for_each(ue_context->am_policy_req_trigger_list,
		                      node) {
			if(cJSON_AddStringToObject(
				   am_policy_req_trigger_listList, "",
				   OpenAPI_policy_req_trigger_ToString(
	...
	if(ue_context->pcf_ue_policy_uri) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [pcf_ue_policy_uri]");
			goto end;
		}
	}
	...
	if(ue_context->ue_policy_req_trigger_list !=
	   OpenAPI_policy_req_trigger_NULL)
	{
		cJSON *ue_policy_req_trigger_listList =
			cJSON_AddArrayToObject(item, "uePolicyReqTriggerList");
		if(ue_policy_req_trigger_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [ue_policy_req_trigger_list]");
			goto end;
		}
		OpenAPI_list_for_each(ue_context->ue_policy_req_trigger_list,
		                      node) {
			if(cJSON_AddStringToObject(
	...
	if(ue_context->hpcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [hpcf_id]");
			goto end;
		}
	}
	...
	if(ue_context->hpcf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [hpcf_set_id]");
			goto end;
		}
	}
	...
	if(ue_context->restricted_rat_list != OpenAPI_rat_type_NULL) {
		...
		if(restricted_rat_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [restricted_rat_list]");
			goto end;
		}
		OpenAPI_list_for_each(ue_context->restricted_rat_list, node) {
			if(cJSON_AddStringToObject(restricted_rat_listList, "",
	...
	if(ue_context->forbidden_area_list) {
		...
		if(forbidden_area_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [forbidden_area_list]");
			goto end;
		}
		OpenAPI_list_for_each(ue_context->forbidden_area_list, node) {
			cJSON *itemLocal =
	...
	if(ue_context->service_area_restriction) {
		...
		if(service_area_restriction_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [service_area_restriction]");
			goto end;
		}
	   ...
	   OpenAPI_core_network_type_NULL)
	{
		...
		if(restricted_core_nw_type_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [restricted_core_nw_type_list]");
			goto end;
		}
		OpenAPI_list_for_each(ue_context->restricted_core_nw_type_list,
		                      node) {
			if(cJSON_AddStringToObject(
				   restricted_core_nw_type_listList, "",
	...
	if(ue_context->event_subscription_list) {
		cJSON *event_subscription_listList =
			cJSON_AddArrayToObject(item, "eventSubscriptionList");
		if(event_subscription_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [event_subscription_list]");
			goto end;
		}
		OpenAPI_list_for_each(ue_context->event_subscription_list,
		                      node) {
	...
	if(ue_context->mm_context_list) {
		...
		if(mm_context_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [mm_context_list]");
			goto end;
		}
		OpenAPI_list_for_each(ue_context->mm_context_list, node) {
			cJSON *itemLocal =
				OpenAPI_mm_context_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_ue_context_convertToJSON() failed [mm_context_list]");
				goto end;
	...
	if(ue_context->session_context_list) {
		...
		if(session_context_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [session_context_list]");
			goto end;
		}
		OpenAPI_list_for_each(ue_context->session_context_list, node) {
			cJSON *itemLocal =
				OpenAPI_pdu_session_context_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_ue_context_convertToJSON() failed [session_context_list]");
	...
	if(ue_context->eps_interworking_info) {
		cJSON *eps_interworking_info_local_JSON =
			OpenAPI_eps_interworking_info_convertToJSON(
				ue_context->eps_interworking_info);
		if(eps_interworking_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [eps_interworking_info]");
			goto end;
		}
	...
	if(ue_context->trace_data) {
		...
		if(trace_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [trace_data]");
			goto end;
		}
	...
	if(ue_context->service_gap_expiry_time) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [service_gap_expiry_time]");
			goto end;
		}
	}
	...
	if(ue_context->stn_sr) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [stn_sr]");
			goto end;
		}
	}
	...
	if(ue_context->c_msisdn) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [c_msisdn]");
			goto end;
		}
	}
	...
	if(ue_context->ms_classmark2) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [ms_classmark2]");
			goto end;
		}
	}
	...
	if(ue_context->supported_codec_list) {
		...
		if(supported_codec_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [supported_codec_list]");
			goto end;
		}
		OpenAPI_list_for_each(ue_context->supported_codec_list, node) {
	...
	if(ue_context->small_data_rate_status_infos) {
		...
		if(small_data_rate_status_infosList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [small_data_rate_status_infos]");
			goto end;
		}
		OpenAPI_list_for_each(ue_context->small_data_rate_status_infos,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_small_data_rate_status_info_convertToJSON
				        (node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_ue_context_convertToJSON() failed [small_data_rate_status_infos]");
	...
	if(ue_context->restricted_primary_rat_list != OpenAPI_rat_type_NULL) {
		cJSON *restricted_primary_rat_listList =
			cJSON_AddArrayToObject(item,
			                       "restrictedPrimaryRatList");
		if(restricted_primary_rat_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [restricted_primary_rat_list]");
			goto end;
		}
		OpenAPI_list_for_each(ue_context->restricted_primary_rat_list,
		                      node) {
			if(cJSON_AddStringToObject(
	...
	if(ue_context->restricted_secondary_rat_list != OpenAPI_rat_type_NULL) {
		...
		if(restricted_secondary_rat_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [restricted_secondary_rat_list]");
			goto end;
		}
		OpenAPI_list_for_each(ue_context->restricted_secondary_rat_list,
		                      node) {
			if(cJSON_AddStringToObject(
				   restricted_secondary_rat_listList, "",
				   OpenAPI_rat_type_ToString(
					   (intptr_t) node->data)) == NULL)
			{
				ogs_error(
	...
	if(ue_context->v2x_context) {
		cJSON *v2x_context_local_JSON =
			OpenAPI_v2x_context_convertToJSON(
				ue_context->v2x_context);
		if(v2x_context_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [v2x_context]");
			goto end;
		}
	...
	if(ue_context->is_lte_cat_m_ind) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [lte_cat_m_ind]");
			goto end;
		}
	}
	...
	if(ue_context->is_red_cap_ind) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [red_cap_ind]");
			goto end;
		}
	}
	...
	if(ue_context->mo_exp_data_counter) {
		...
		if(mo_exp_data_counter_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [mo_exp_data_counter]");
			goto end;
		}
	...
	if(ue_context->cag_data) {
		cJSON *cag_data_local_JSON =
			OpenAPI_cag_data_convertToJSON(ue_context->cag_data);
		if(cag_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [cag_data]");
			goto end;
		}
	...
	if(ue_context->is_management_mdt_ind) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [management_mdt_ind]");
			goto end;
		}
	}
	...
	if(ue_context->immediate_mdt_conf) {
		cJSON *immediate_mdt_conf_local_JSON =
			OpenAPI_immediate_mdt_conf_convertToJSON(
				ue_context->immediate_mdt_conf);
		if(immediate_mdt_conf_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [immediate_mdt_conf]");
			goto end;
		}
	...
	if(ue_context->ec_restriction_data_wb) {
		...
		if(ec_restriction_data_wb_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [ec_restriction_data_wb]");
			goto end;
		}
	...
	if(ue_context->is_ec_restriction_data_nb) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [ec_restriction_data_nb]");
			goto end;
		}
	}
	...
	if(ue_context->is_iab_operation_allowed) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [iab_operation_allowed]");
			goto end;
		}
	}

	if(ue_context->prose_context) {
		cJSON *prose_context_local_JSON =
			OpenAPI_prose_context_convertToJSON(
				ue_context->prose_context);
		if(prose_context_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [prose_context]");
			goto end;
		}
	...
	if(ue_context->analytics_subscription_list) {
		cJSON *analytics_subscription_listList =
			cJSON_AddArrayToObject(item,
			                       "analyticsSubscriptionList");
		if(analytics_subscription_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [analytics_subscription_list]");
			goto end;
		}
		OpenAPI_list_for_each(ue_context->analytics_subscription_list,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_analytics_subscription_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_ue_context_convertToJSON() failed [analytics_subscription_list]");
	...
	if(ue_context->pcf_amp_binding_info) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [pcf_amp_binding_info]");
			goto end;
		}
	}
	...
	if(ue_context->pcf_uep_binding_info) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [pcf_uep_binding_info]");
			goto end;
		}
	}
	...
	if(ue_context->used_service_area_restriction) {
		...
		if(used_service_area_restriction_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [used_service_area_restriction]");
			goto end;
		}
	...
	if(ue_context->pra_in_am_policy) {
		...
		if(pra_in_am_policy == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [pra_in_am_policy]");
			goto end;
		}
		cJSON *localMapObject = pra_in_am_policy;
		if(ue_context->pra_in_am_policy) {
			OpenAPI_list_for_each(ue_context->pra_in_am_policy,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				if(localKeyValue == NULL) {
	...
	if(ue_context->pra_in_ue_policy) {
		...
		if(pra_in_ue_policy == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [pra_in_ue_policy]");
			goto end;
		}
		cJSON *localMapObject = pra_in_ue_policy;
		if(ue_context->pra_in_ue_policy) {
			OpenAPI_list_for_each(ue_context->pra_in_ue_policy,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				if(localKeyValue == NULL) {
	...
	if(ue_context->updp_subscription_data) {
		cJSON *updp_subscription_data_local_JSON =
			OpenAPI_updp_subscription_data_convertToJSON(
				ue_context->updp_subscription_data);
		if(updp_subscription_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [updp_subscription_data]");
			goto end;
		}
	...
	if(ue_context->sm_policy_notify_pdu_list) {
		cJSON *sm_policy_notify_pdu_listList =
			cJSON_AddArrayToObject(item, "smPolicyNotifyPduList");
		if(sm_policy_notify_pdu_listList == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [sm_policy_notify_pdu_list]");
			goto end;
		}
		OpenAPI_list_for_each(ue_context->sm_policy_notify_pdu_list,
		                      node) {
	...
	if(ue_context->pcf_ue_callback_info) {
		...
		if(pcf_ue_callback_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [pcf_ue_callback_info]");
			goto end;
		}
	...
	if(ue_context->ue_positioning_cap) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [ue_positioning_cap]");
			goto end;
		}
	}
	...
	if(ue_context->is_asti_distribution_indication) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [asti_distribution_indication]");
			goto end;
		}
	}
	...
	if(ue_context->is_ts_error_budget) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [ts_error_budget]");
			goto end;
		}
	}
	...
	if(ue_context->is_snpn_onboard_ind) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [snpn_onboard_ind]");
			goto end;
		}
	}
	...
	if(ue_context->smf_sel_info) {
		cJSON *smf_sel_info_local_JSON =
			OpenAPI_smf_selection_data_convertToJSON(
				ue_context->smf_sel_info);
		if(smf_sel_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [smf_sel_info]");
			goto end;
		}
	...
	if(ue_context->pcf_ue_slice_mbr_list) {
		...
		if(pcf_ue_slice_mbr_list == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [pcf_ue_slice_mbr_list]");
			goto end;
		}
		cJSON *localMapObject = pcf_ue_slice_mbr_list;
		if(ue_context->pcf_ue_slice_mbr_list) {
			OpenAPI_list_for_each(ue_context->pcf_ue_slice_mbr_list,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				if(localKeyValue == NULL) {
	...
	if(ue_context->smsf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [smsf_set_id]");
			goto end;
		}
	}
	...
	if(ue_context->smsf_service_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [smsf_service_set_id]");
			goto end;
		}
	}
	...
	if(ue_context->smsf_binding_info) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [smsf_binding_info]");
			goto end;
		}
	}
	...
	if(ue_context->is_disaster_roaming_ind) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [disaster_roaming_ind]");
			goto end;
		}
	}

	if(ue_context->disaster_plmn) {
		cJSON *disaster_plmn_local_JSON =
			OpenAPI_plmn_id_convertToJSON(
				ue_context->disaster_plmn);
		if(disaster_plmn_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ue_context_convertToJSON() failed [disaster_plmn]");
			goto end;
		}
...
OpenAPI_ue_context_t *OpenAPI_ue_context_parseFromJSON(cJSON *ue_contextJSON) {
	...
	OpenAPI_updp_subscription_data_t *updp_subscription_data_local_nonprim =
		NULL;
	...
	if(supi) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(supi_unauth_ind) {
		if(!cJSON_IsBool(supi_unauth_ind)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [supi_unauth_ind]");
			goto end;
		}
	}

	gpsi_list = cJSON_GetObjectItemCaseSensitive(ue_contextJSON,
	                                             "gpsiList");
	if(gpsi_list) {
		cJSON *gpsi_list_local = NULL;
		if(!cJSON_IsArray(gpsi_list)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [gpsi_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(gpsi_list_local, gpsi_list) {
			...
			OpenAPI_list_add(gpsi_listList,
			                 ogs_strdup(
						 gpsi_list_local->valuestring));
		}
	}
	...
	if(pei) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [pei]");
			goto end;
		}
	}
	...
	if(udm_group_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [udm_group_id]");
			goto end;
		}
	}
	...
	if(ausf_group_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [ausf_group_id]");
			goto end;
		}
	}
	...
	if(pcf_group_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [pcf_group_id]");
			goto end;
		}
	}
	...
	if(routing_indicator) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [routing_indicator]");
			goto end;
		}
	}
	...
	if(h_nw_pub_key_id) {
		if(!cJSON_IsNumber(h_nw_pub_key_id)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [h_nw_pub_key_id]");
			goto end;
		}
	}
	...
	if(group_list) {
		...
		if(!cJSON_IsArray(group_list)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [group_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(group_list_local, group_list) {
			...
			OpenAPI_list_add(group_listList,
			                 ogs_strdup(
						 group_list_local->valuestring));
		}
	}
	...
	if(drx_parameter) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [drx_parameter]");
			goto end;
		}
	}
	...
	if(sub_rfsp) {
		if(!cJSON_IsNumber(sub_rfsp)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [sub_rfsp]");
			goto end;
		}
	}

	used_rfsp = cJSON_GetObjectItemCaseSensitive(ue_contextJSON,
	                                             "usedRfsp");
	if(used_rfsp) {
		if(!cJSON_IsNumber(used_rfsp)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [used_rfsp]");
			goto end;
		}
	}
	...
	if(sub_ue_ambr) {
		sub_ue_ambr_local_nonprim =
			OpenAPI_ambr_parseFromJSON(sub_ue_ambr);
		if(!sub_ue_ambr_local_nonprim) {
			ogs_error(
				"OpenAPI_ambr_parseFromJSON failed [sub_ue_ambr]");
			goto end;
		}
	}
	...
	if(sub_ue_slice_mbr_list) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [sub_ue_slice_mbr_list]");
			goto end;
		}
		if(cJSON_IsObject(sub_ue_slice_mbr_list)) {
			...
			cJSON_ArrayForEach(sub_ue_slice_mbr_list_local_map,
				...
				if(cJSON_IsObject(localMapObject)) {
					localMapKeyPair = OpenAPI_map_create(
						...
						OpenAPI_slice_mbr_parseFromJSON(
							localMapObject));
					...
					ogs_error(
						"OpenAPI_ue_context_parseFromJSON() failed [inner]");
					goto end;
				}
				OpenAPI_list_add(sub_ue_slice_mbr_listList,
				                 localMapKeyPair);
			}
		}
	}
	...
	if(smsf_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [smsf_id]");
			goto end;
		}
	}

	seaf_data = cJSON_GetObjectItemCaseSensitive(ue_contextJSON,
	                                             "seafData");
	if(seaf_data) {
		seaf_data_local_nonprim =
			OpenAPI_seaf_data_parseFromJSON(seaf_data);
		if(!seaf_data_local_nonprim) {
			ogs_error(
				"OpenAPI_seaf_data_parseFromJSON failed [seaf_data]");
			goto end;
		}
	}
	...
	if(_5g_mm_capability) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [_5g_mm_capability]");
			goto end;
		}
	}
	...
	if(pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [pcf_id]");
			goto end;
		}
	}

	pcf_set_id = cJSON_GetObjectItemCaseSensitive(ue_contextJSON,
	                                              "pcfSetId");
	if(pcf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [pcf_set_id]");
			goto end;
		}
	}

	pcf_amp_service_set_id =
		cJSON_GetObjectItemCaseSensitive(ue_contextJSON,
		                                 "pcfAmpServiceSetId");
	if(pcf_amp_service_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [pcf_amp_service_set_id]");
			goto end;
		}
	}
	...
	if(pcf_uep_service_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [pcf_uep_service_set_id]");
			goto end;
		}
	}
	...
	if(pcf_binding) {
		if(!cJSON_IsString(pcf_binding)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [pcf_binding]");
			goto end;
		}
		pcf_bindingVariable =
	...
	if(pcf_am_policy_uri) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [pcf_am_policy_uri]");
			goto end;
		}
	}
	...
	if(am_policy_req_trigger_list) {
		...
		if(!cJSON_IsArray(am_policy_req_trigger_list)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [am_policy_req_trigger_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(am_policy_req_trigger_list_local,
		                   am_policy_req_trigger_list) {
			OpenAPI_policy_req_trigger_e localEnum =
				OpenAPI_policy_req_trigger_NULL;
			...
			if(!localEnum) {
				...
				OpenAPI_list_add(am_policy_req_trigger_listList,
				                 (void *) localEnum);
			}
		}
		if(am_policy_req_trigger_listList->count == 0) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed: Expected am_policy_req_trigger_listList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(pcf_ue_policy_uri) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [pcf_ue_policy_uri]");
			goto end;
		}
	}
	...
	if(ue_policy_req_trigger_list) {
		...
		if(!cJSON_IsArray(ue_policy_req_trigger_list)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [ue_policy_req_trigger_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ue_policy_req_trigger_list_local,
			...
			localEnum =
				OpenAPI_policy_req_trigger_FromString(
					ue_policy_req_trigger_list_local->
					valuestring);
			if(!localEnum) {
				ogs_info(
				...
				OpenAPI_list_add(ue_policy_req_trigger_listList,
				                 (void *) localEnum);
			}
		}
		if(ue_policy_req_trigger_listList->count == 0) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed: Expected ue_policy_req_trigger_listList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(hpcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [hpcf_id]");
			goto end;
		}
	}
	...
	if(hpcf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [hpcf_set_id]");
			goto end;
		}
	}
	...
	if(restricted_rat_list) {
		...
		if(!cJSON_IsArray(restricted_rat_list)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [restricted_rat_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(restricted_rat_list_local,
			...
			if(!localEnum) {
				ogs_info(
					"Enum value \"%s\" for field \"restricted_rat_list\" is not supported. Ignoring it ...",
					restricted_rat_list_local->valuestring);
			} else {
				OpenAPI_list_add(restricted_rat_listList,
				                 (void *) localEnum);
			}
		}
		if(restricted_rat_listList->count == 0) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed: Expected restricted_rat_listList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(forbidden_area_list) {
		...
		if(!cJSON_IsArray(forbidden_area_list)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [forbidden_area_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(forbidden_area_list_local,
			...
			OpenAPI_area_t *forbidden_area_listItem =
	...
	if(restricted_core_nw_type_list) {
		...
		if(!cJSON_IsArray(restricted_core_nw_type_list)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [restricted_core_nw_type_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(restricted_core_nw_type_list_local,
			...
			if(!localEnum) {
				ogs_info(
					"Enum value \"%s\" for field \"restricted_core_nw_type_list\" is not supported. Ignoring it ...",
					restricted_core_nw_type_list_local->
					valuestring);
			} else {
				OpenAPI_list_add(
		...
		if(restricted_core_nw_type_listList->count == 0) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed: Expected restricted_core_nw_type_listList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(event_subscription_list) {
		...
		if(!cJSON_IsArray(event_subscription_list)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [event_subscription_list]");
			goto end;
		}
	...
	if(mm_context_list) {
		...
		if(!cJSON_IsArray(mm_context_list)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [mm_context_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(mm_context_list_local, mm_context_list) {
			...
			OpenAPI_mm_context_t *mm_context_listItem =
				OpenAPI_mm_context_parseFromJSON(
					mm_context_list_local);
	...
	if(session_context_list) {
		...
		if(!cJSON_IsArray(session_context_list)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [session_context_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(session_context_list_local,
			...
			OpenAPI_pdu_session_context_t *session_context_listItem
			...
			OpenAPI_list_add(session_context_listList,
	...
	if(eps_interworking_info) {
		...
		if(!eps_interworking_info_local_nonprim) {
			ogs_error(
				"OpenAPI_eps_interworking_info_parseFromJSON failed [eps_interworking_info]");
			goto end;
		}
	}
	...
	if(service_gap_expiry_time) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [service_gap_expiry_time]");
			goto end;
		}
	}
	...
	if(stn_sr) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [stn_sr]");
			goto end;
		}
	}
	...
	if(c_msisdn) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [c_msisdn]");
			goto end;
		}
	}
	...
	if(ms_classmark2) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [ms_classmark2]");
			goto end;
		}
	}
	...
	if(supported_codec_list) {
		...
		if(!cJSON_IsArray(supported_codec_list)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [supported_codec_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(supported_codec_list_local,
			...
			OpenAPI_list_add(supported_codec_listList,
			                 ogs_strdup(
						 supported_codec_list_local->
						 valuestring));
		}
	}
	...
	if(small_data_rate_status_infos) {
		...
		if(!cJSON_IsArray(small_data_rate_status_infos)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [small_data_rate_status_infos]");
			goto end;
		}
		...
		cJSON_ArrayForEach(small_data_rate_status_infos_local,
			...
			OpenAPI_small_data_rate_status_info_t *
				...
				OpenAPI_small_data_rate_status_info_parseFromJSON
				        (small_data_rate_status_infos_local);
			if(!small_data_rate_status_infosItem) {
				ogs_error(
					"No small_data_rate_status_infosItem");
				goto end;
			}
	...
	if(restricted_primary_rat_list) {
		...
		if(!cJSON_IsArray(restricted_primary_rat_list)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [restricted_primary_rat_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(restricted_primary_rat_list_local,
			...
			if(!localEnum) {
				ogs_info(
					"Enum value \"%s\" for field \"restricted_primary_rat_list\" is not supported. Ignoring it ...",
					restricted_primary_rat_list_local->
					valuestring);
			} else {
				OpenAPI_list_add(
		...
		if(restricted_primary_rat_listList->count == 0) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed: Expected restricted_primary_rat_listList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	if(restricted_secondary_rat_list) {
		...
		if(!cJSON_IsArray(restricted_secondary_rat_list)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [restricted_secondary_rat_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(restricted_secondary_rat_list_local,
			...
			if(!localEnum) {
				ogs_info(
					"Enum value \"%s\" for field \"restricted_secondary_rat_list\" is not supported. Ignoring it ...",
					restricted_secondary_rat_list_local->
					valuestring);
			} else {
				OpenAPI_list_add(
		...
		if(restricted_secondary_rat_listList->count == 0) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed: Expected restricted_secondary_rat_listList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
	...
	v2x_context = cJSON_GetObjectItemCaseSensitive(ue_contextJSON,
	                                               "v2xContext");
	if(v2x_context) {
		v2x_context_local_nonprim =
			OpenAPI_v2x_context_parseFromJSON(v2x_context);
		if(!v2x_context_local_nonprim) {
			ogs_error(
				"OpenAPI_v2x_context_parseFromJSON failed [v2x_context]");
			goto end;
		}
	}
	...
	if(lte_cat_m_ind) {
		if(!cJSON_IsBool(lte_cat_m_ind)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [lte_cat_m_ind]");
			goto end;
		}
	}
	...
	if(red_cap_ind) {
		if(!cJSON_IsBool(red_cap_ind)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [red_cap_ind]");
			goto end;
		}
	}
	...
	if(management_mdt_ind) {
		if(!cJSON_IsBool(management_mdt_ind)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [management_mdt_ind]");
			goto end;
		}
	}
	...
	if(immediate_mdt_conf) {
		...
		if(!immediate_mdt_conf_local_nonprim) {
			ogs_error(
				"OpenAPI_immediate_mdt_conf_parseFromJSON failed [immediate_mdt_conf]");
			goto end;
		}
	}
	...
	if(ec_restriction_data_nb) {
		if(!cJSON_IsBool(ec_restriction_data_nb)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [ec_restriction_data_nb]");
			goto end;
		}
	}
	...
	if(iab_operation_allowed) {
		if(!cJSON_IsBool(iab_operation_allowed)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [iab_operation_allowed]");
			goto end;
		}
	}
	...
	prose_context = cJSON_GetObjectItemCaseSensitive(ue_contextJSON,
	                                                 "proseContext");
	if(prose_context) {
		prose_context_local_nonprim =
			OpenAPI_prose_context_parseFromJSON(prose_context);
		if(!prose_context_local_nonprim) {
			ogs_error(
				"OpenAPI_prose_context_parseFromJSON failed [prose_context]");
			goto end;
		}
	}
	...
	if(analytics_subscription_list) {
		...
		if(!cJSON_IsArray(analytics_subscription_list)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [analytics_subscription_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(analytics_subscription_list_local,
			...
			OpenAPI_list_add(analytics_subscription_listList,
			                 analytics_subscription_listItem);
		}
	}
	...
	if(pcf_amp_binding_info) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [pcf_amp_binding_info]");
			goto end;
		}
	}
	...
	if(pcf_uep_binding_info) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [pcf_uep_binding_info]");
			goto end;
		}
	}
	...
	if(used_service_area_restriction) {
		...
		if(!used_service_area_restriction_local_nonprim) {
			ogs_error(
				"OpenAPI_service_area_restriction_parseFromJSON failed [used_service_area_restriction]");
			goto end;
		}
	}
	...
	if(pra_in_am_policy) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [pra_in_am_policy]");
			goto end;
		}
		if(cJSON_IsObject(pra_in_am_policy)) {
			...
			cJSON_ArrayForEach(pra_in_am_policy_local_map,
				...
				OpenAPI_list_add(pra_in_am_policyList,
				                 localMapKeyPair);
			}
		}
	}
	...
	if(pra_in_ue_policy) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [pra_in_ue_policy]");
			goto end;
		}
		if(cJSON_IsObject(pra_in_ue_policy)) {
			...
			cJSON_ArrayForEach(pra_in_ue_policy_local_map,
				...
				OpenAPI_list_add(pra_in_ue_policyList,
				                 localMapKeyPair);
			}
		}
	}
	...
	if(updp_subscription_data) {
		...
		if(!updp_subscription_data_local_nonprim) {
			ogs_error(
				"OpenAPI_updp_subscription_data_parseFromJSON failed [updp_subscription_data]");
			goto end;
		}
	}
	...
	if(sm_policy_notify_pdu_list) {
		...
		if(!cJSON_IsArray(sm_policy_notify_pdu_list)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [sm_policy_notify_pdu_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(sm_policy_notify_pdu_list_local,
			...
			OpenAPI_list_add(sm_policy_notify_pdu_listList,
			                 sm_policy_notify_pdu_listItem);
		}
	}
	...
	if(ue_positioning_cap) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [ue_positioning_cap]");
			goto end;
		}
	}
	...
	if(asti_distribution_indication) {
		if(!cJSON_IsBool(asti_distribution_indication)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [asti_distribution_indication]");
			goto end;
		}
	}
	...
	if(ts_error_budget) {
		if(!cJSON_IsNumber(ts_error_budget)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [ts_error_budget]");
			goto end;
		}
	}
	...
	if(snpn_onboard_ind) {
		if(!cJSON_IsBool(snpn_onboard_ind)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [snpn_onboard_ind]");
			goto end;
		}
	}
	...
	if(smf_sel_info) {
		if(!cJSON_IsNull(smf_sel_info)) {
			smf_sel_info_local_nonprim =
				OpenAPI_smf_selection_data_parseFromJSON(
					smf_sel_info);
			if(!smf_sel_info_local_nonprim) {
				ogs_error(
					"OpenAPI_smf_selection_data_parseFromJSON failed [smf_sel_info]");
				goto end;
			}
		}
	}
	...
	if(pcf_ue_slice_mbr_list) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [pcf_ue_slice_mbr_list]");
			goto end;
		}
		if(cJSON_IsObject(pcf_ue_slice_mbr_list)) {
			...
			cJSON_ArrayForEach(pcf_ue_slice_mbr_list_local_map,
				...
				OpenAPI_list_add(pcf_ue_slice_mbr_listList,
				                 localMapKeyPair);
			}
		}
	}
	...
	if(smsf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [smsf_set_id]");
			goto end;
		}
	}
	...
	if(smsf_service_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [smsf_service_set_id]");
			goto end;
		}
	}
	...
	if(smsf_binding_info) {
		...
		{
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [smsf_binding_info]");
			goto end;
		}
	}
	...
	if(disaster_roaming_ind) {
		if(!cJSON_IsBool(disaster_roaming_ind)) {
			ogs_error(
				"OpenAPI_ue_context_parseFromJSON() failed [disaster_roaming_ind]");
			goto end;
		}
	}
	...
	if(disaster_plmn) {
		disaster_plmn_local_nonprim =
			OpenAPI_plmn_id_parseFromJSON(disaster_plmn);
		if(!disaster_plmn_local_nonprim) {
			ogs_error(
				"OpenAPI_plmn_id_parseFromJSON failed [disaster_plmn]");
			goto end;
		}
	}
	...
	ue_context_local_var = OpenAPI_ue_context_create(
		...
		ausf_group_id &&
		!cJSON_IsNull(ausf_group_id) ?
		ogs_strdup(ausf_group_id->valuestring) : NULL,
		pcf_group_id &&
		!cJSON_IsNull(pcf_group_id) ?
		...
		drx_parameter &&
		!cJSON_IsNull(drx_parameter) ?
		ogs_strdup(drx_parameter->valuestring) : NULL,
		sub_rfsp ? true : false,
		...
		smsf_id &&
		!cJSON_IsNull(smsf_id) ? ogs_strdup(smsf_id->valuestring) :
		NULL,
		seaf_data ? seaf_data_local_nonprim : NULL,
		_5g_mm_capability &&
		!cJSON_IsNull(_5g_mm_capability) ?
		ogs_strdup(_5g_mm_capability->valuestring) : NULL,
		pcf_id &&
		!cJSON_IsNull(pcf_id) ? ogs_strdup(pcf_id->valuestring) : NULL,
		pcf_set_id &&
		!cJSON_IsNull(pcf_set_id) ?
		ogs_strdup(pcf_set_id->valuestring) : NULL,
		pcf_amp_service_set_id &&
		!cJSON_IsNull(pcf_amp_service_set_id) ?
		ogs_strdup(pcf_amp_service_set_id->valuestring) : NULL,
		pcf_uep_service_set_id &&
		!cJSON_IsNull(pcf_uep_service_set_id) ?
		ogs_strdup(pcf_uep_service_set_id->valuestring) : NULL,
		pcf_binding ? pcf_bindingVariable : 0,
		pcf_am_policy_uri &&
		!cJSON_IsNull(pcf_am_policy_uri) ?
		ogs_strdup(pcf_am_policy_uri->valuestring) : NULL,
		am_policy_req_trigger_list ? am_policy_req_trigger_listList :
		NULL,
		pcf_ue_policy_uri &&
		!cJSON_IsNull(pcf_ue_policy_uri) ?
		ogs_strdup(pcf_ue_policy_uri->valuestring) : NULL,
		ue_policy_req_trigger_list ? ue_policy_req_trigger_listList :
		NULL,
		hpcf_id &&
		!cJSON_IsNull(hpcf_id) ? ogs_strdup(hpcf_id->valuestring) :
		NULL,
		hpcf_set_id &&
		!cJSON_IsNull(hpcf_set_id) ?
		ogs_strdup(hpcf_set_id->valuestring) : NULL,
		...
		eps_interworking_info ? eps_interworking_info_local_nonprim :
		NULL,
		trace_data &&
		...
		service_gap_expiry_time &&
		!cJSON_IsNull(service_gap_expiry_time) ?
		ogs_strdup(service_gap_expiry_time->valuestring) : NULL,
		stn_sr &&
		!cJSON_IsNull(stn_sr) ? ogs_strdup(stn_sr->valuestring) : NULL,
		c_msisdn &&
		!cJSON_IsNull(c_msisdn) ? ogs_strdup(c_msisdn->valuestring) :
		NULL,
		ms_classmark2 &&
		!cJSON_IsNull(ms_classmark2) ?
		ogs_strdup(ms_classmark2->valuestring) : NULL,
		supported_codec_list ? supported_codec_listList : NULL,
		small_data_rate_status_infos ?
		small_data_rate_status_infosList : NULL,
		restricted_primary_rat_list ? restricted_primary_rat_listList :
		NULL,
		restricted_secondary_rat_list ?
		...
		ec_restriction_data_wb ? ec_restriction_data_wb_local_nonprim :
		NULL,
		...
		analytics_subscription_list ? analytics_subscription_listList :
		NULL,
		pcf_amp_binding_info &&
		!cJSON_IsNull(pcf_amp_binding_info) ?
		ogs_strdup(pcf_amp_binding_info->valuestring) : NULL,
		pcf_uep_binding_info &&
		!cJSON_IsNull(pcf_uep_binding_info) ?
		ogs_strdup(pcf_uep_binding_info->valuestring) : NULL,
		used_service_area_restriction ?
		used_service_area_restriction_local_nonprim : NULL,
		pra_in_am_policy ? pra_in_am_policyList : NULL,
		pra_in_ue_policy ? pra_in_ue_policyList : NULL,
		updp_subscription_data ? updp_subscription_data_local_nonprim :
		NULL,
		sm_policy_notify_pdu_list ? sm_policy_notify_pdu_listList :
		NULL,
		pcf_ue_callback_info &&
		...
		asti_distribution_indication ?
		asti_distribution_indication->valueint : 0,
		...
		smsf_set_id &&
		!cJSON_IsNull(smsf_set_id) ?
		ogs_strdup(smsf_set_id->valuestring) : NULL,
		smsf_service_set_id &&
		!cJSON_IsNull(smsf_service_set_id) ?
		ogs_strdup(smsf_service_set_id->valuestring) : NULL,
		smsf_binding_info &&
		!cJSON_IsNull(smsf_binding_info) ?
		ogs_strdup(smsf_binding_info->valuestring) : NULL,
	...
	if(immediate_mdt_conf_local_nonprim) {
		OpenAPI_immediate_mdt_conf_free(
			immediate_mdt_conf_local_nonprim);
...
OpenAPI_ue_context_t *OpenAPI_ue_context_copy(OpenAPI_ue_context_t	*dst,
                                              OpenAPI_ue_context_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
