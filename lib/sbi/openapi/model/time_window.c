...
OpenAPI_time_window_t *OpenAPI_time_window_create(char	*start_time,
                                                  char	*stop_time) {
	OpenAPI_time_window_t *time_window_local_var =
		ogs_malloc(sizeof(OpenAPI_time_window_t));
...
cJSON *OpenAPI_time_window_convertToJSON(OpenAPI_time_window_t *time_window) {
	...
	if(time_window == NULL) {
		ogs_error(
			"OpenAPI_time_window_convertToJSON() failed [TimeWindow]");
		return NULL;
	}
	...
	if(!time_window->start_time) {
		ogs_error(
			"OpenAPI_time_window_convertToJSON() failed [start_time]");
		return NULL;
	}
	...
	if(!time_window->stop_time) {
		ogs_error(
			"OpenAPI_time_window_convertToJSON() failed [stop_time]");
		return NULL;
	}
...
OpenAPI_time_window_t *OpenAPI_time_window_parseFromJSON(
	cJSON *time_windowJSON) {
	...
	if(!start_time) {
		ogs_error(
			"OpenAPI_time_window_parseFromJSON() failed [start_time]");
		goto end;
	}
	...
	stop_time = cJSON_GetObjectItemCaseSensitive(time_windowJSON,
	                                             "stopTime");
	if(!stop_time) {
		ogs_error(
			"OpenAPI_time_window_parseFromJSON() failed [stop_time]");
		goto end;
	}
...
OpenAPI_time_window_t *OpenAPI_time_window_copy(OpenAPI_time_window_t	*dst,
                                                OpenAPI_time_window_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
