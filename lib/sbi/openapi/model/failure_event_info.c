...
OpenAPI_failure_event_info_t *OpenAPI_failure_event_info_create(
	...
	OpenAPI_failure_event_info_t *failure_event_info_local_var =
		ogs_malloc(sizeof(OpenAPI_failure_event_info_t));
...
void OpenAPI_failure_event_info_free(
	OpenAPI_failure_event_info_t *failure_event_info) {
...
cJSON *OpenAPI_failure_event_info_convertToJSON(
	OpenAPI_failure_event_info_t *failure_event_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(failure_event_info == NULL) {
		ogs_error(
			"OpenAPI_failure_event_info_convertToJSON() failed [FailureEventInfo]");
		return NULL;
	}
	...
	if(!failure_event_info->event) {
		ogs_error(
			"OpenAPI_failure_event_info_convertToJSON() failed [event]");
		return NULL;
	}
	cJSON *event_local_JSON =
		OpenAPI_nwdaf_event_convertToJSON(failure_event_info->event);
	...
	if(!failure_event_info->failure_code) {
		ogs_error(
			"OpenAPI_failure_event_info_convertToJSON() failed [failure_code]");
		return NULL;
	}
	cJSON *failure_code_local_JSON =
...
OpenAPI_failure_event_info_t *OpenAPI_failure_event_info_parseFromJSON(
	...
	event = cJSON_GetObjectItemCaseSensitive(failure_event_infoJSON,
	                                         "event");
	if(!event) {
		ogs_error(
			"OpenAPI_failure_event_info_parseFromJSON() failed [event]");
		goto end;
	}
	...
	if(!failure_code) {
		ogs_error(
			"OpenAPI_failure_event_info_parseFromJSON() failed [failure_code]");
		goto end;
	}
	failure_code_local_nonprim =
		OpenAPI_nwdaf_failure_code_parseFromJSON(failure_code);
	if(!failure_code_local_nonprim) {
		ogs_error(
			"OpenAPI_nwdaf_failure_code_parseFromJSON failed [failure_code]");
		goto end;
	}
...
OpenAPI_failure_event_info_t *OpenAPI_failure_event_info_copy(
	OpenAPI_failure_event_info_t *dst, OpenAPI_failure_event_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
