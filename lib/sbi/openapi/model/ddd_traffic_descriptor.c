...
OpenAPI_ddd_traffic_descriptor_t *OpenAPI_ddd_traffic_descriptor_create(
	...
	char *mac_addr) {
	OpenAPI_ddd_traffic_descriptor_t *ddd_traffic_descriptor_local_var =
		ogs_malloc(sizeof(OpenAPI_ddd_traffic_descriptor_t));
...
void OpenAPI_ddd_traffic_descriptor_free(
...
cJSON *OpenAPI_ddd_traffic_descriptor_convertToJSON(
	...
	if(ddd_traffic_descriptor == NULL) {
		ogs_error(
			"OpenAPI_ddd_traffic_descriptor_convertToJSON() failed [DddTrafficDescriptor]");
		return NULL;
	}
	...
	if(ddd_traffic_descriptor->ipv4_addr) {
		...
		{
			ogs_error(
				"OpenAPI_ddd_traffic_descriptor_convertToJSON() failed [ipv4_addr]");
			goto end;
		}
	}
	...
	if(ddd_traffic_descriptor->ipv6_addr) {
		...
		{
			ogs_error(
				"OpenAPI_ddd_traffic_descriptor_convertToJSON() failed [ipv6_addr]");
			goto end;
		}
	}
	...
	if(ddd_traffic_descriptor->is_port_number) {
		...
		{
			ogs_error(
				"OpenAPI_ddd_traffic_descriptor_convertToJSON() failed [port_number]");
			goto end;
		}
	}
	...
	if(ddd_traffic_descriptor->mac_addr) {
		...
		{
			ogs_error(
				"OpenAPI_ddd_traffic_descriptor_convertToJSON() failed [mac_addr]");
			goto end;
		}
	}
...
OpenAPI_ddd_traffic_descriptor_t *OpenAPI_ddd_traffic_descriptor_parseFromJSON(
	...
	if(ipv4_addr) {
		...
		{
			ogs_error(
				"OpenAPI_ddd_traffic_descriptor_parseFromJSON() failed [ipv4_addr]");
			goto end;
		}
	}
	...
	if(ipv6_addr) {
		...
		{
			ogs_error(
				"OpenAPI_ddd_traffic_descriptor_parseFromJSON() failed [ipv6_addr]");
			goto end;
		}
	}

	port_number =
		cJSON_GetObjectItemCaseSensitive(ddd_traffic_descriptorJSON,
		                                 "portNumber");
	if(port_number) {
		if(!cJSON_IsNumber(port_number)) {
			ogs_error(
				"OpenAPI_ddd_traffic_descriptor_parseFromJSON() failed [port_number]");
			goto end;
		}
	}
	...
	if(mac_addr) {
		...
		{
			ogs_error(
				"OpenAPI_ddd_traffic_descriptor_parseFromJSON() failed [mac_addr]");
			goto end;
		}
	}

	ddd_traffic_descriptor_local_var =
		OpenAPI_ddd_traffic_descriptor_create(
			ipv4_addr &&
			!cJSON_IsNull(ipv4_addr) ?
...
OpenAPI_ddd_traffic_descriptor_t *OpenAPI_ddd_traffic_descriptor_copy(
	OpenAPI_ddd_traffic_descriptor_t	*dst,
	OpenAPI_ddd_traffic_descriptor_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ddd_traffic_descriptor_convertToJSON() failed");
		return NULL;
	}
