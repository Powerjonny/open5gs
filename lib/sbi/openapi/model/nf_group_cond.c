...
char *OpenAPI_nf_typenf_group_cond_ToString(
...
OpenAPI_nf_group_cond_nf_type_e OpenAPI_nf_typenf_group_cond_FromString(
	char *nf_type) {
...
OpenAPI_nf_group_cond_t *OpenAPI_nf_group_cond_create(
	OpenAPI_nf_group_cond_nf_type_e nf_type, char *nf_group_id) {
	OpenAPI_nf_group_cond_t *nf_group_cond_local_var =
		ogs_malloc(sizeof(OpenAPI_nf_group_cond_t));
...
cJSON *OpenAPI_nf_group_cond_convertToJSON(
	OpenAPI_nf_group_cond_t *nf_group_cond) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(nf_group_cond == NULL) {
		ogs_error(
			"OpenAPI_nf_group_cond_convertToJSON() failed [NfGroupCond]");
		return NULL;
	}
	...
	if(nf_group_cond->nf_type == OpenAPI_nf_group_cond_NFTYPE_NULL) {
		ogs_error(
			"OpenAPI_nf_group_cond_convertToJSON() failed [nf_type]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "nfType",
	                           OpenAPI_nf_typenf_group_cond_ToString(
					   nf_group_cond->nf_type)) == NULL) {
		ogs_error(
			"OpenAPI_nf_group_cond_convertToJSON() failed [nf_type]");
		goto end;
	}
	...
	if(!nf_group_cond->nf_group_id) {
		ogs_error(
			"OpenAPI_nf_group_cond_convertToJSON() failed [nf_group_id]");
		return NULL;
	}
...
OpenAPI_nf_group_cond_t *OpenAPI_nf_group_cond_parseFromJSON(
	cJSON *nf_group_condJSON) {
	...
	if(!nf_type) {
		ogs_error(
			"OpenAPI_nf_group_cond_parseFromJSON() failed [nf_type]");
		goto end;
	}
	...
	nf_typeVariable =
		OpenAPI_nf_typenf_group_cond_FromString(nf_type->valuestring);

	nf_group_id = cJSON_GetObjectItemCaseSensitive(nf_group_condJSON,
	                                               "nfGroupId");
	if(!nf_group_id) {
		ogs_error(
			"OpenAPI_nf_group_cond_parseFromJSON() failed [nf_group_id]");
		goto end;
	}
...
OpenAPI_nf_group_cond_t *OpenAPI_nf_group_cond_copy(
	OpenAPI_nf_group_cond_t *dst, OpenAPI_nf_group_cond_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
