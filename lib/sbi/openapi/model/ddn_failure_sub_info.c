...
OpenAPI_ddn_failure_sub_info_t *OpenAPI_ddn_failure_sub_info_create(
	...
	OpenAPI_ddn_failure_sub_info_t *ddn_failure_sub_info_local_var =
		ogs_malloc(sizeof(OpenAPI_ddn_failure_sub_info_t));
	ogs_assert(ddn_failure_sub_info_local_var);

	ddn_failure_sub_info_local_var->notify_correlation_id =
		notify_correlation_id;
	ddn_failure_sub_info_local_var->ddd_traffic_descriptor_list =
		ddd_traffic_descriptor_list;

	return ddn_failure_sub_info_local_var;
}

void OpenAPI_ddn_failure_sub_info_free(
	OpenAPI_ddn_failure_sub_info_t *ddn_failure_sub_info) {
	...
	if(ddn_failure_sub_info->ddd_traffic_descriptor_list) {
		OpenAPI_list_for_each(
			ddn_failure_sub_info->ddd_traffic_descriptor_list,
			node) {
...
cJSON *OpenAPI_ddn_failure_sub_info_convertToJSON(
	OpenAPI_ddn_failure_sub_info_t *ddn_failure_sub_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(ddn_failure_sub_info == NULL) {
		ogs_error(
			"OpenAPI_ddn_failure_sub_info_convertToJSON() failed [DdnFailureSubInfo]");
		return NULL;
	}
	...
	if(!ddn_failure_sub_info->notify_correlation_id) {
		ogs_error(
			"OpenAPI_ddn_failure_sub_info_convertToJSON() failed [notify_correlation_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "notifyCorrelationId",
	                           ddn_failure_sub_info->notify_correlation_id)
	   == NULL)
	{
		ogs_error(
			"OpenAPI_ddn_failure_sub_info_convertToJSON() failed [notify_correlation_id]");
	...
	if(ddn_failure_sub_info->ddd_traffic_descriptor_list) {
		...
		if(ddd_traffic_descriptor_listList == NULL) {
			ogs_error(
				"OpenAPI_ddn_failure_sub_info_convertToJSON() failed [ddd_traffic_descriptor_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			ddn_failure_sub_info->ddd_traffic_descriptor_list,
...
OpenAPI_ddn_failure_sub_info_t *OpenAPI_ddn_failure_sub_info_parseFromJSON(
	cJSON *ddn_failure_sub_infoJSON) {
	...
	notify_correlation_id =
		cJSON_GetObjectItemCaseSensitive(ddn_failure_sub_infoJSON,
		                                 "notifyCorrelationId");
	if(!notify_correlation_id) {
		ogs_error(
			"OpenAPI_ddn_failure_sub_info_parseFromJSON() failed [notify_correlation_id]");
		goto end;
	}
	...
	if(ddd_traffic_descriptor_list) {
		...
		if(!cJSON_IsArray(ddd_traffic_descriptor_list)) {
			ogs_error(
				"OpenAPI_ddn_failure_sub_info_parseFromJSON() failed [ddd_traffic_descriptor_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ddd_traffic_descriptor_list_local,
			...
			OpenAPI_list_add(ddd_traffic_descriptor_listList,
			                 ddd_traffic_descriptor_listItem);
		}
	}
	...
	ddn_failure_sub_info_local_var = OpenAPI_ddn_failure_sub_info_create(
		...
		ddd_traffic_descriptor_list ? ddd_traffic_descriptor_listList :
		NULL
		);
...
OpenAPI_ddn_failure_sub_info_t *OpenAPI_ddn_failure_sub_info_copy(
	OpenAPI_ddn_failure_sub_info_t	*dst,
	OpenAPI_ddn_failure_sub_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ddn_failure_sub_info_convertToJSON() failed");
		return NULL;
	}
