...
OpenAPI_sgsn_address_t *OpenAPI_sgsn_address_create(char	*sgsn_ipv4_addr,
                                                    char	*sgsn_ipv6_addr)
{
	OpenAPI_sgsn_address_t *sgsn_address_local_var =
		ogs_malloc(sizeof(OpenAPI_sgsn_address_t));
...
cJSON *OpenAPI_sgsn_address_convertToJSON(
	OpenAPI_sgsn_address_t *sgsn_address) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(sgsn_address == NULL) {
		ogs_error(
			"OpenAPI_sgsn_address_convertToJSON() failed [SgsnAddress]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(sgsn_address->sgsn_ipv4_addr) {
		if(cJSON_AddStringToObject(item, "sgsnIpv4Addr",
		                           sgsn_address->sgsn_ipv4_addr) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sgsn_address_convertToJSON() failed [sgsn_ipv4_addr]");
			goto end;
		}
	}

	if(sgsn_address->sgsn_ipv6_addr) {
		if(cJSON_AddStringToObject(item, "sgsnIpv6Addr",
		                           sgsn_address->sgsn_ipv6_addr) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sgsn_address_convertToJSON() failed [sgsn_ipv6_addr]");
			goto end;
		}
	}
...
OpenAPI_sgsn_address_t *OpenAPI_sgsn_address_parseFromJSON(
	...
	sgsn_ipv4_addr = cJSON_GetObjectItemCaseSensitive(sgsn_addressJSON,
	                                                  "sgsnIpv4Addr");
	if(sgsn_ipv4_addr) {
		if(!cJSON_IsString(sgsn_ipv4_addr) &&
		   !cJSON_IsNull(sgsn_ipv4_addr))
		{
			ogs_error(
				"OpenAPI_sgsn_address_parseFromJSON() failed [sgsn_ipv4_addr]");
			goto end;
		}
	}

	sgsn_ipv6_addr = cJSON_GetObjectItemCaseSensitive(sgsn_addressJSON,
	                                                  "sgsnIpv6Addr");
	if(sgsn_ipv6_addr) {
		if(!cJSON_IsString(sgsn_ipv6_addr) &&
		   !cJSON_IsNull(sgsn_ipv6_addr))
		{
			ogs_error(
				"OpenAPI_sgsn_address_parseFromJSON() failed [sgsn_ipv6_addr]");
			goto end;
		}
	}
	...
	sgsn_address_local_var = OpenAPI_sgsn_address_create(
		sgsn_ipv4_addr &&
		!cJSON_IsNull(sgsn_ipv4_addr) ?
		ogs_strdup(sgsn_ipv4_addr->valuestring) : NULL,
		sgsn_ipv6_addr &&
		!cJSON_IsNull(sgsn_ipv6_addr) ?
		ogs_strdup(sgsn_ipv6_addr->valuestring) : NULL
		);
