...
char *OpenAPI_wildcard_sdext_snssai_ToString(
...
OpenAPI_ext_snssai_wildcard_sd_e OpenAPI_wildcard_sdext_snssai_FromString(
	char *wildcard_sd) {
...
OpenAPI_ext_snssai_t *OpenAPI_ext_snssai_create(int sst, char *sd,
                                                OpenAPI_list_t *sd_ranges,
                                                OpenAPI_ext_snssai_wildcard_sd_e
                                                wildcard_sd) {
	OpenAPI_ext_snssai_t *ext_snssai_local_var =
		ogs_malloc(sizeof(OpenAPI_ext_snssai_t));
...
cJSON *OpenAPI_ext_snssai_convertToJSON(OpenAPI_ext_snssai_t *ext_snssai) {
	...
	if(ext_snssai == NULL) {
		ogs_error(
			"OpenAPI_ext_snssai_convertToJSON() failed [ExtSnssai]");
		return NULL;
	}
	...
	if(ext_snssai->sd) {
		...
		{
			ogs_error(
				"OpenAPI_ext_snssai_convertToJSON() failed [sd]");
			goto end;
		}
	}
	...
	if(ext_snssai->sd_ranges) {
		...
		if(sd_rangesList == NULL) {
			ogs_error(
				"OpenAPI_ext_snssai_convertToJSON() failed [sd_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(ext_snssai->sd_ranges, node) {
			cJSON *itemLocal =
	...
	if(ext_snssai->wildcard_sd != OpenAPI_ext_snssai_WILDCARDSD_NULL) {
		if(cJSON_AddStringToObject(item, "wildcardSd",
		                           OpenAPI_wildcard_sdext_snssai_ToString
		                                   (ext_snssai->wildcard_sd)) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_ext_snssai_convertToJSON() failed [wildcard_sd]");
			goto end;
		}
	}
...
OpenAPI_ext_snssai_t *OpenAPI_ext_snssai_parseFromJSON(cJSON *ext_snssaiJSON) {
	...
	if(sd) {
		...
		{
			ogs_error(
				"OpenAPI_ext_snssai_parseFromJSON() failed [sd]");
			goto end;
		}
	}

	sd_ranges = cJSON_GetObjectItemCaseSensitive(ext_snssaiJSON,
	                                             "sdRanges");
	if(sd_ranges) {
		cJSON *sd_ranges_local = NULL;
		if(!cJSON_IsArray(sd_ranges)) {
			ogs_error(
				"OpenAPI_ext_snssai_parseFromJSON() failed [sd_ranges]");
			goto end;
		}
	...
	if(wildcard_sd) {
		if(!cJSON_IsString(wildcard_sd)) {
			ogs_error(
				"OpenAPI_ext_snssai_parseFromJSON() failed [wildcard_sd]");
			goto end;
		}
		wildcard_sdVariable =
			OpenAPI_wildcard_sdext_snssai_FromString(
	...
	ext_snssai_local_var = OpenAPI_ext_snssai_create(
		...
		sst->valuedouble,
		sd &&
		!cJSON_IsNull(sd) ? ogs_strdup(sd->valuestring) : NULL,
...
OpenAPI_ext_snssai_t *OpenAPI_ext_snssai_copy(OpenAPI_ext_snssai_t	*dst,
                                              OpenAPI_ext_snssai_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
