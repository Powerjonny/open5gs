...
OpenAPI_nrf_info_served_hss_info_list_value_value_create(char *group_id,
                                                         OpenAPI_list_t *
                                                         imsi_ranges,
                                                         OpenAPI_list_t *
                                                         ...
                                                         *hss_diameter_address)
{
	...
	nrf_info_served_hss_info_list_value_value_local_var->group_id =
		group_id;
	nrf_info_served_hss_info_list_value_value_local_var->imsi_ranges =
		imsi_ranges;
	nrf_info_served_hss_info_list_value_value_local_var->
	ims_private_identity_ranges = ims_private_identity_ranges;
	nrf_info_served_hss_info_list_value_value_local_var->
	ims_public_identity_ranges = ims_public_identity_ranges;
	nrf_info_served_hss_info_list_value_value_local_var->msisdn_ranges =
		msisdn_ranges;
	nrf_info_served_hss_info_list_value_value_local_var->
	external_group_identifiers_ranges = external_group_identifiers_ranges;
	nrf_info_served_hss_info_list_value_value_local_var->
	hss_diameter_address = hss_diameter_address;

	return nrf_info_served_hss_info_list_value_value_local_var;
}

void OpenAPI_nrf_info_served_hss_info_list_value_value_free(
	OpenAPI_nrf_info_served_hss_info_list_value_value_t *
	nrf_info_served_hss_info_list_value_value) {
	...
	if(nrf_info_served_hss_info_list_value_value->imsi_ranges) {
		OpenAPI_list_for_each(
			nrf_info_served_hss_info_list_value_value->imsi_ranges,
			node) {
	...
	if(nrf_info_served_hss_info_list_value_value->
	   ims_private_identity_ranges)
	{
		OpenAPI_list_for_each(
			nrf_info_served_hss_info_list_value_value->
			ims_private_identity_ranges, node) {
	...
	if(nrf_info_served_hss_info_list_value_value->ims_public_identity_ranges)
	{
		...
		OpenAPI_list_free(
			...
			ims_public_identity_ranges);
		nrf_info_served_hss_info_list_value_value->
		ims_public_identity_ranges = NULL;
	}
	if(nrf_info_served_hss_info_list_value_value->msisdn_ranges) {
		OpenAPI_list_for_each(
			nrf_info_served_hss_info_list_value_value->msisdn_ranges,
			node) {
			OpenAPI_identity_range_free(node->data);
		}
		OpenAPI_list_free(
...
cJSON *OpenAPI_nrf_info_served_hss_info_list_value_value_convertToJSON(
	...
	if(nrf_info_served_hss_info_list_value_value == NULL) {
		ogs_error(
			"OpenAPI_nrf_info_served_hss_info_list_value_value_convertToJSON() failed [NrfInfo_servedHssInfoList_value_value]");
		return NULL;
	}
	...
	if(nrf_info_served_hss_info_list_value_value->group_id) {
		...
		{
			ogs_error(
				"OpenAPI_nrf_info_served_hss_info_list_value_value_convertToJSON() failed [group_id]");
			goto end;
		}
	}

	if(nrf_info_served_hss_info_list_value_value->imsi_ranges) {
		cJSON *imsi_rangesList = cJSON_AddArrayToObject(item,
		                                                "imsiRanges");
		if(imsi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_hss_info_list_value_value_convertToJSON() failed [imsi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			nrf_info_served_hss_info_list_value_value->imsi_ranges,
			node) {
			cJSON *itemLocal =
				OpenAPI_imsi_range_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_nrf_info_served_hss_info_list_value_value_convertToJSON() failed [imsi_ranges]");
				goto end;
	   ...
	   ims_private_identity_ranges)
	{
		...
		if(ims_private_identity_rangesList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_hss_info_list_value_value_convertToJSON() failed [ims_private_identity_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			nrf_info_served_hss_info_list_value_value->
			...
			cJSON_AddItemToArray(ims_private_identity_rangesList,
			                     itemLocal);
		}
	}
	...
	if(nrf_info_served_hss_info_list_value_value->ims_public_identity_ranges)
	{
		cJSON *ims_public_identity_rangesList =
			cJSON_AddArrayToObject(item, "imsPublicIdentityRanges");
		if(ims_public_identity_rangesList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_hss_info_list_value_value_convertToJSON() failed [ims_public_identity_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			nrf_info_served_hss_info_list_value_value->
			...
			cJSON_AddItemToArray(ims_public_identity_rangesList,
			                     itemLocal);
		}
	}

	if(nrf_info_served_hss_info_list_value_value->msisdn_ranges) {
		cJSON *msisdn_rangesList = cJSON_AddArrayToObject(item,
		                                                  "msisdnRanges");
		if(msisdn_rangesList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_hss_info_list_value_value_convertToJSON() failed [msisdn_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			nrf_info_served_hss_info_list_value_value->msisdn_ranges,
	   ...
	   external_group_identifiers_ranges)
	{
		...
		if(external_group_identifiers_rangesList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_hss_info_list_value_value_convertToJSON() failed [external_group_identifiers_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			nrf_info_served_hss_info_list_value_value->
	...
	if(nrf_info_served_hss_info_list_value_value->hss_diameter_address) {
		cJSON *hss_diameter_address_local_JSON =
			OpenAPI_network_node_diameter_address_convertToJSON(
				nrf_info_served_hss_info_list_value_value->
				hss_diameter_address);
		if(hss_diameter_address_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_hss_info_list_value_value_convertToJSON() failed [hss_diameter_address]");
			goto end;
		}
		cJSON_AddItemToObject(item, "hssDiameterAddress",
		                      hss_diameter_address_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_hss_info_list_value_value_convertToJSON() failed [hss_diameter_address]");
			goto end;
...
OpenAPI_nrf_info_served_hss_info_list_value_value_parseFromJSON(
	cJSON *nrf_info_served_hss_info_list_value_valueJSON) {
	OpenAPI_nrf_info_served_hss_info_list_value_value_t *
	        nrf_info_served_hss_info_list_value_value_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *group_id = NULL;
	...
	group_id =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_hss_info_list_value_valueJSON,
			"groupId");
	if(group_id) {
		...
		{
			ogs_error(
				"OpenAPI_nrf_info_served_hss_info_list_value_value_parseFromJSON() failed [group_id]");
			goto end;
		}
	}
	...
	if(imsi_ranges) {
		...
		if(!cJSON_IsArray(imsi_ranges)) {
			ogs_error(
				"OpenAPI_nrf_info_served_hss_info_list_value_value_parseFromJSON() failed [imsi_ranges]");
			goto end;
		}
		...
		cJSON_ArrayForEach(imsi_ranges_local, imsi_ranges) {
			...
			OpenAPI_imsi_range_t *imsi_rangesItem =
				OpenAPI_imsi_range_parseFromJSON(
					imsi_ranges_local);
			if(!imsi_rangesItem) {
	...
	if(ims_private_identity_ranges) {
		...
		if(!cJSON_IsArray(ims_private_identity_ranges)) {
			ogs_error(
				"OpenAPI_nrf_info_served_hss_info_list_value_value_parseFromJSON() failed [ims_private_identity_ranges]");
			goto end;
		}
	...
	if(ims_public_identity_ranges) {
		...
		if(!cJSON_IsArray(ims_public_identity_ranges)) {
			ogs_error(
				"OpenAPI_nrf_info_served_hss_info_list_value_value_parseFromJSON() failed [ims_public_identity_ranges]");
			goto end;
		}

		ims_public_identity_rangesList = OpenAPI_list_create();

		cJSON_ArrayForEach(ims_public_identity_ranges_local,
		                   ims_public_identity_ranges) {
			...
			OpenAPI_identity_range_t *ims_public_identity_rangesItem
			        = OpenAPI_identity_range_parseFromJSON(
					  ims_public_identity_ranges_local);
			if(!ims_public_identity_rangesItem) {
				ogs_error("No ims_public_identity_rangesItem");
				goto end;
			}
			OpenAPI_list_add(ims_public_identity_rangesList,
			                 ims_public_identity_rangesItem);
		}
	}
	...
	if(msisdn_ranges) {
		...
		if(!cJSON_IsArray(msisdn_ranges)) {
			ogs_error(
				"OpenAPI_nrf_info_served_hss_info_list_value_value_parseFromJSON() failed [msisdn_ranges]");
			goto end;
		}
		...
		cJSON_ArrayForEach(msisdn_ranges_local, msisdn_ranges) {
			...
			OpenAPI_identity_range_t *msisdn_rangesItem =
				OpenAPI_identity_range_parseFromJSON(
					msisdn_ranges_local);
	...
	if(external_group_identifiers_ranges) {
		...
		if(!cJSON_IsArray(external_group_identifiers_ranges)) {
			ogs_error(
				"OpenAPI_nrf_info_served_hss_info_list_value_value_parseFromJSON() failed [external_group_identifiers_ranges]");
			goto end;
		}
	...
	if(hss_diameter_address) {
		hss_diameter_address_local_nonprim =
			OpenAPI_network_node_diameter_address_parseFromJSON(
				hss_diameter_address);
		if(!hss_diameter_address_local_nonprim) {
			ogs_error(
				"OpenAPI_network_node_diameter_address_parseFromJSON failed [hss_diameter_address]");
			goto end;
		}
	}
	...
	nrf_info_served_hss_info_list_value_value_local_var =
		OpenAPI_nrf_info_served_hss_info_list_value_value_create(
			...
			ims_private_identity_ranges ?
			ims_private_identity_rangesList :
			NULL,
			ims_public_identity_ranges ?
...
OpenAPI_nrf_info_served_hss_info_list_value_value_copy(
	OpenAPI_nrf_info_served_hss_info_list_value_value_t	*dst,
	OpenAPI_nrf_info_served_hss_info_list_value_value_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_nrf_info_served_hss_info_list_value_value_convertToJSON() failed");
		return NULL;
	}
