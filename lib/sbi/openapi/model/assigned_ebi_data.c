...
OpenAPI_assigned_ebi_data_t *OpenAPI_assigned_ebi_data_create(
	int pdu_session_id, OpenAPI_list_t *assigned_ebi_list,
	OpenAPI_list_t *failed_arp_list, OpenAPI_list_t *released_ebi_list,
	OpenAPI_list_t *modified_ebi_list) {
	OpenAPI_assigned_ebi_data_t *assigned_ebi_data_local_var =
		ogs_malloc(sizeof(OpenAPI_assigned_ebi_data_t));
...
void OpenAPI_assigned_ebi_data_free(
	OpenAPI_assigned_ebi_data_t *assigned_ebi_data) {
	...
	if(assigned_ebi_data->assigned_ebi_list) {
		OpenAPI_list_for_each(assigned_ebi_data->assigned_ebi_list,
		                      node) {
	...
	if(assigned_ebi_data->failed_arp_list) {
		OpenAPI_list_for_each(assigned_ebi_data->failed_arp_list,
		                      node) {
	...
	if(assigned_ebi_data->released_ebi_list) {
		OpenAPI_list_for_each(assigned_ebi_data->released_ebi_list,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_assigned_ebi_data_convertToJSON(
	...
	if(assigned_ebi_data == NULL) {
		ogs_error(
			"OpenAPI_assigned_ebi_data_convertToJSON() failed [AssignedEbiData]");
		return NULL;
	}
	...
	{
		ogs_error(
			"OpenAPI_assigned_ebi_data_convertToJSON() failed [pdu_session_id]");
		goto end;
	}

	if(!assigned_ebi_data->assigned_ebi_list) {
		ogs_error(
			"OpenAPI_assigned_ebi_data_convertToJSON() failed [assigned_ebi_list]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(assigned_ebi_data->assigned_ebi_list, node) {
		cJSON *itemLocal =
			OpenAPI_ebi_arp_mapping_convertToJSON(node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_assigned_ebi_data_convertToJSON() failed [assigned_ebi_list]");
			goto end;
	...
	if(assigned_ebi_data->failed_arp_list) {
		...
		if(failed_arp_listList == NULL) {
			ogs_error(
				"OpenAPI_assigned_ebi_data_convertToJSON() failed [failed_arp_list]");
			goto end;
		}
		OpenAPI_list_for_each(assigned_ebi_data->failed_arp_list,
		                      node) {
	...
	if(assigned_ebi_data->released_ebi_list) {
		...
		if(released_ebi_listList == NULL) {
			ogs_error(
				"OpenAPI_assigned_ebi_data_convertToJSON() failed [released_ebi_list]");
			goto end;
		}
		OpenAPI_list_for_each(assigned_ebi_data->released_ebi_list,
		                      node) {
	...
	if(assigned_ebi_data->modified_ebi_list) {
		...
		if(modified_ebi_listList == NULL) {
			ogs_error(
				"OpenAPI_assigned_ebi_data_convertToJSON() failed [modified_ebi_list]");
			goto end;
		}
		OpenAPI_list_for_each(assigned_ebi_data->modified_ebi_list,
		                      node) {
...
OpenAPI_assigned_ebi_data_t *OpenAPI_assigned_ebi_data_parseFromJSON(
	...
	if(!pdu_session_id) {
		ogs_error(
			"OpenAPI_assigned_ebi_data_parseFromJSON() failed [pdu_session_id]");
		goto end;
	}
	...
	assigned_ebi_list =
		cJSON_GetObjectItemCaseSensitive(assigned_ebi_dataJSON,
		                                 "assignedEbiList");
	if(!assigned_ebi_list) {
		ogs_error(
			"OpenAPI_assigned_ebi_data_parseFromJSON() failed [assigned_ebi_list]");
		goto end;
	}
	...
	if(failed_arp_list) {
		...
		if(!cJSON_IsArray(failed_arp_list)) {
			ogs_error(
				"OpenAPI_assigned_ebi_data_parseFromJSON() failed [failed_arp_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(failed_arp_list_local, failed_arp_list) {
			...
			OpenAPI_arp_t *failed_arp_listItem =
			...
			OpenAPI_list_add(failed_arp_listList,
			                 failed_arp_listItem);
		}
	}
	...
	if(released_ebi_list) {
		...
		if(!cJSON_IsArray(released_ebi_list)) {
			ogs_error(
				"OpenAPI_assigned_ebi_data_parseFromJSON() failed [released_ebi_list]");
			goto end;
		}
	...
	if(modified_ebi_list) {
		...
		if(!cJSON_IsArray(modified_ebi_list)) {
			ogs_error(
				"OpenAPI_assigned_ebi_data_parseFromJSON() failed [modified_ebi_list]");
			goto end;
		}
...
OpenAPI_assigned_ebi_data_t *OpenAPI_assigned_ebi_data_copy(
	OpenAPI_assigned_ebi_data_t *dst, OpenAPI_assigned_ebi_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
