...
OpenAPI_ddn_failure_subs_t *OpenAPI_ddn_failure_subs_create(
	...
	OpenAPI_ddn_failure_subs_t *ddn_failure_subs_local_var =
		ogs_malloc(sizeof(OpenAPI_ddn_failure_subs_t));
	ogs_assert(ddn_failure_subs_local_var);

	ddn_failure_subs_local_var->is_ddn_failure_subs_ind =
		is_ddn_failure_subs_ind;
	ddn_failure_subs_local_var->ddn_failure_subs_ind = ddn_failure_subs_ind;
	ddn_failure_subs_local_var->ddn_failure_subs_info_list =
...
void OpenAPI_ddn_failure_subs_free(
	...
	if(ddn_failure_subs->ddn_failure_subs_info_list) {
		OpenAPI_list_for_each(
			ddn_failure_subs->ddn_failure_subs_info_list, node) {
...
cJSON *OpenAPI_ddn_failure_subs_convertToJSON(
	...
	if(ddn_failure_subs == NULL) {
		ogs_error(
			"OpenAPI_ddn_failure_subs_convertToJSON() failed [DdnFailureSubs]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(ddn_failure_subs->is_ddn_failure_subs_ind) {
		if(cJSON_AddBoolToObject(item, "ddnFailureSubsInd",
		                         ddn_failure_subs->ddn_failure_subs_ind)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_ddn_failure_subs_convertToJSON() failed [ddn_failure_subs_ind]");
			goto end;
		}
	}

	if(ddn_failure_subs->ddn_failure_subs_info_list) {
		cJSON *ddn_failure_subs_info_listList =
			cJSON_AddArrayToObject(item, "ddnFailureSubsInfoList");
		if(ddn_failure_subs_info_listList == NULL) {
			ogs_error(
				"OpenAPI_ddn_failure_subs_convertToJSON() failed [ddn_failure_subs_info_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			ddn_failure_subs->ddn_failure_subs_info_list, node) {
			cJSON *itemLocal =
				OpenAPI_ddn_failure_sub_info_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_ddn_failure_subs_convertToJSON() failed [ddn_failure_subs_info_list]");
			...
			cJSON_AddItemToArray(ddn_failure_subs_info_listList,
			                     itemLocal);
		}
	}
...
OpenAPI_ddn_failure_subs_t *OpenAPI_ddn_failure_subs_parseFromJSON(
	...
	ddn_failure_subs_ind =
		cJSON_GetObjectItemCaseSensitive(ddn_failure_subsJSON,
		                                 "ddnFailureSubsInd");
	if(ddn_failure_subs_ind) {
		if(!cJSON_IsBool(ddn_failure_subs_ind)) {
			ogs_error(
				"OpenAPI_ddn_failure_subs_parseFromJSON() failed [ddn_failure_subs_ind]");
			goto end;
		}
	}
	...
	if(ddn_failure_subs_info_list) {
		...
		if(!cJSON_IsArray(ddn_failure_subs_info_list)) {
			ogs_error(
				"OpenAPI_ddn_failure_subs_parseFromJSON() failed [ddn_failure_subs_info_list]");
			goto end;
		}

		ddn_failure_subs_info_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(ddn_failure_subs_info_list_local,
		                   ddn_failure_subs_info_list) {
			...
			OpenAPI_ddn_failure_sub_info_t *
			        ddn_failure_subs_info_listItem =
				OpenAPI_ddn_failure_sub_info_parseFromJSON(
					ddn_failure_subs_info_list_local);
			if(!ddn_failure_subs_info_listItem) {
				ogs_error("No ddn_failure_subs_info_listItem");
				goto end;
			}
			OpenAPI_list_add(ddn_failure_subs_info_listList,
			                 ddn_failure_subs_info_listItem);
		}
	}
	...
	ddn_failure_subs_local_var = OpenAPI_ddn_failure_subs_create(
		...
		ddn_failure_subs_info_list ? ddn_failure_subs_info_listList :
		NULL
		);
