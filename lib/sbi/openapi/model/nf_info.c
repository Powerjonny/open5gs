...
OpenAPI_nf_info_t *OpenAPI_nf_info_create(OpenAPI_nf_type_e nf_type) {
	OpenAPI_nf_info_t *nf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_nf_info_t));
...
cJSON *OpenAPI_nf_info_convertToJSON(OpenAPI_nf_info_t *nf_info) {
	...
	if(nf_info->nf_type != OpenAPI_nf_type_NULL) {
		if(cJSON_AddStringToObject(item, "nfType",
						   ...
						   nf_info->nf_type)) == NULL)
		{
			ogs_error(
				"OpenAPI_nf_info_convertToJSON() failed [nf_type]");
			goto end;
		}
	}
...
OpenAPI_nf_info_t *OpenAPI_nf_info_parseFromJSON(cJSON *nf_infoJSON) {
	...
	if(nf_type) {
		if(!cJSON_IsString(nf_type)) {
			ogs_error(
				"OpenAPI_nf_info_parseFromJSON() failed [nf_type]");
			goto end;
		}
		nf_typeVariable =
...
OpenAPI_nf_info_t *OpenAPI_nf_info_copy(OpenAPI_nf_info_t	*dst,
                                        OpenAPI_nf_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
