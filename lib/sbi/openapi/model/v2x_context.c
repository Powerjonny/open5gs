...
OpenAPI_v2x_context_t *OpenAPI_v2x_context_create(
	...
	char *nr_ue_sidelink_ambr, char *lte_ue_sidelink_ambr,
	OpenAPI_pc5_qo_s_para_t *pc5_qo_s_para) {
	OpenAPI_v2x_context_t *v2x_context_local_var =
		ogs_malloc(sizeof(OpenAPI_v2x_context_t));
...
cJSON *OpenAPI_v2x_context_convertToJSON(OpenAPI_v2x_context_t *v2x_context) {
	...
	if(v2x_context == NULL) {
		ogs_error(
			"OpenAPI_v2x_context_convertToJSON() failed [V2xContext]");
		return NULL;
	}
	...
	if(v2x_context->nr_v2x_services_auth) {
		cJSON *nr_v2x_services_auth_local_JSON =
			OpenAPI_nr_v2x_auth_convertToJSON(
				v2x_context->nr_v2x_services_auth);
		if(nr_v2x_services_auth_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_v2x_context_convertToJSON() failed [nr_v2x_services_auth]");
			goto end;
		}
	...
	if(v2x_context->lte_v2x_services_auth) {
		cJSON *lte_v2x_services_auth_local_JSON =
			OpenAPI_lte_v2x_auth_convertToJSON(
				v2x_context->lte_v2x_services_auth);
		if(lte_v2x_services_auth_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_v2x_context_convertToJSON() failed [lte_v2x_services_auth]");
			goto end;
		}
	...
	if(v2x_context->nr_ue_sidelink_ambr) {
		...
		{
			ogs_error(
				"OpenAPI_v2x_context_convertToJSON() failed [nr_ue_sidelink_ambr]");
			goto end;
		}
	}
	...
	if(v2x_context->lte_ue_sidelink_ambr) {
		...
		{
			ogs_error(
				"OpenAPI_v2x_context_convertToJSON() failed [lte_ue_sidelink_ambr]");
			goto end;
		}
	}
	...
	if(v2x_context->pc5_qo_s_para) {
		...
		if(pc5_qo_s_para_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_v2x_context_convertToJSON() failed [pc5_qo_s_para]");
			goto end;
		}
...
OpenAPI_v2x_context_t *OpenAPI_v2x_context_parseFromJSON(
	cJSON *v2x_contextJSON) {
	...
	if(nr_v2x_services_auth) {
		nr_v2x_services_auth_local_nonprim =
			OpenAPI_nr_v2x_auth_parseFromJSON(nr_v2x_services_auth);
		if(!nr_v2x_services_auth_local_nonprim) {
			ogs_error(
				"OpenAPI_nr_v2x_auth_parseFromJSON failed [nr_v2x_services_auth]");
			goto end;
		}
	}

	lte_v2x_services_auth =
		cJSON_GetObjectItemCaseSensitive(v2x_contextJSON,
		                                 "lteV2xServicesAuth");
	if(lte_v2x_services_auth) {
		lte_v2x_services_auth_local_nonprim =
			OpenAPI_lte_v2x_auth_parseFromJSON(
				lte_v2x_services_auth);
		if(!lte_v2x_services_auth_local_nonprim) {
			ogs_error(
				"OpenAPI_lte_v2x_auth_parseFromJSON failed [lte_v2x_services_auth]");
			goto end;
		}
	}
	...
	if(nr_ue_sidelink_ambr) {
		...
		{
			ogs_error(
				"OpenAPI_v2x_context_parseFromJSON() failed [nr_ue_sidelink_ambr]");
			goto end;
		}
	}
	...
	if(lte_ue_sidelink_ambr) {
		...
		{
			ogs_error(
				"OpenAPI_v2x_context_parseFromJSON() failed [lte_ue_sidelink_ambr]");
			goto end;
		}
	}
	...
	v2x_context_local_var = OpenAPI_v2x_context_create(
		nr_v2x_services_auth ? nr_v2x_services_auth_local_nonprim :
		NULL,
		lte_v2x_services_auth ? lte_v2x_services_auth_local_nonprim :
		NULL,
		nr_ue_sidelink_ambr &&
		!cJSON_IsNull(nr_ue_sidelink_ambr) ?
		ogs_strdup(nr_ue_sidelink_ambr->valuestring) : NULL,
		lte_ue_sidelink_ambr &&
		!cJSON_IsNull(lte_ue_sidelink_ambr) ?
		ogs_strdup(lte_ue_sidelink_ambr->valuestring) : NULL,
...
OpenAPI_v2x_context_t *OpenAPI_v2x_context_copy(OpenAPI_v2x_context_t	*dst,
                                                OpenAPI_v2x_context_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
