...
OpenAPI_nrf_info_served_udm_info_value_create(char		*group_id,
	...
	nrf_info_served_udm_info_value_local_var->routing_indicators =
		routing_indicators;
	nrf_info_served_udm_info_value_local_var->
	internal_group_identifiers_ranges = internal_group_identifiers_ranges;
...
void OpenAPI_nrf_info_served_udm_info_value_free(
	OpenAPI_nrf_info_served_udm_info_value_t *nrf_info_served_udm_info_value)
{
	...
	if(nrf_info_served_udm_info_value->supi_ranges) {
		OpenAPI_list_for_each(
			nrf_info_served_udm_info_value->supi_ranges, node) {
	...
	if(nrf_info_served_udm_info_value->gpsi_ranges) {
		OpenAPI_list_for_each(
			nrf_info_served_udm_info_value->gpsi_ranges, node) {
	...
	if(nrf_info_served_udm_info_value->internal_group_identifiers_ranges) {
		OpenAPI_list_for_each(
			nrf_info_served_udm_info_value->
			internal_group_identifiers_ranges, node) {
		...
		OpenAPI_list_free(
			...
			internal_group_identifiers_ranges);
		nrf_info_served_udm_info_value->
		internal_group_identifiers_ranges = NULL;
	}
	if(nrf_info_served_udm_info_value->suci_infos) {
		OpenAPI_list_for_each(
			nrf_info_served_udm_info_value->suci_infos, node) {
...
cJSON *OpenAPI_nrf_info_served_udm_info_value_convertToJSON(
	OpenAPI_nrf_info_served_udm_info_value_t *nrf_info_served_udm_info_value)
{
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(nrf_info_served_udm_info_value == NULL) {
		ogs_error(
			"OpenAPI_nrf_info_served_udm_info_value_convertToJSON() failed [NrfInfo_servedUdmInfo_value]");
		return NULL;
	}
	...
	if(nrf_info_served_udm_info_value->group_id) {
		...
		{
			ogs_error(
				"OpenAPI_nrf_info_served_udm_info_value_convertToJSON() failed [group_id]");
			goto end;
		}
	}
	...
	if(nrf_info_served_udm_info_value->supi_ranges) {
		...
		if(supi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_udm_info_value_convertToJSON() failed [supi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			nrf_info_served_udm_info_value->supi_ranges, node) {
	...
	if(nrf_info_served_udm_info_value->gpsi_ranges) {
		...
		if(gpsi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_udm_info_value_convertToJSON() failed [gpsi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			nrf_info_served_udm_info_value->gpsi_ranges, node) {
	...
	if(nrf_info_served_udm_info_value->external_group_identifiers_ranges) {
		...
		if(external_group_identifiers_rangesList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_udm_info_value_convertToJSON() failed [external_group_identifiers_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			nrf_info_served_udm_info_value->
	...
	if(nrf_info_served_udm_info_value->routing_indicators) {
		cJSON *routing_indicatorsList = cJSON_AddArrayToObject(item,
		                                                       "routingIndicators");
		if(routing_indicatorsList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_udm_info_value_convertToJSON() failed [routing_indicators]");
			goto end;
		}
		OpenAPI_list_for_each(
			nrf_info_served_udm_info_value->routing_indicators,
			node) {
			if(cJSON_AddStringToObject(routing_indicatorsList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_nrf_info_served_udm_info_value_convertToJSON() failed [routing_indicators]");
	...
	if(nrf_info_served_udm_info_value->internal_group_identifiers_ranges) {
		cJSON *internal_group_identifiers_rangesList =
			cJSON_AddArrayToObject(item,
			                       "internalGroupIdentifiersRanges");
		if(internal_group_identifiers_rangesList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_udm_info_value_convertToJSON() failed [internal_group_identifiers_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			nrf_info_served_udm_info_value->
			internal_group_identifiers_ranges, node) {
			cJSON *itemLocal =
				OpenAPI_internal_group_id_range_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_nrf_info_served_udm_info_value_convertToJSON() failed [internal_group_identifiers_ranges]");
	...
	if(nrf_info_served_udm_info_value->suci_infos) {
		cJSON *suci_infosList = cJSON_AddArrayToObject(item,
		                                               "suciInfos");
		if(suci_infosList == NULL) {
			ogs_error(
				"OpenAPI_nrf_info_served_udm_info_value_convertToJSON() failed [suci_infos]");
			goto end;
		}
		OpenAPI_list_for_each(
			nrf_info_served_udm_info_value->suci_infos, node) {
			cJSON *itemLocal =
				OpenAPI_suci_info_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_nrf_info_served_udm_info_value_convertToJSON() failed [suci_infos]");
				goto end;
...
OpenAPI_nrf_info_served_udm_info_value_parseFromJSON(
	cJSON *nrf_info_served_udm_info_valueJSON) {
	OpenAPI_nrf_info_served_udm_info_value_t *
	        nrf_info_served_udm_info_value_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *group_id = NULL;
	...
	group_id =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_udm_info_valueJSON, "groupId");
	if(group_id) {
		if(!cJSON_IsString(group_id) &&
		   !cJSON_IsNull(group_id))
		{
			ogs_error(
				"OpenAPI_nrf_info_served_udm_info_value_parseFromJSON() failed [group_id]");
			goto end;
		}
	}
	...
	supi_ranges =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_udm_info_valueJSON, "supiRanges");
	if(supi_ranges) {
		cJSON *supi_ranges_local = NULL;
		if(!cJSON_IsArray(supi_ranges)) {
			ogs_error(
				"OpenAPI_nrf_info_served_udm_info_value_parseFromJSON() failed [supi_ranges]");
			goto end;
		}
	...
	gpsi_ranges =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_udm_info_valueJSON, "gpsiRanges");
	if(gpsi_ranges) {
		cJSON *gpsi_ranges_local = NULL;
		if(!cJSON_IsArray(gpsi_ranges)) {
			ogs_error(
				"OpenAPI_nrf_info_served_udm_info_value_parseFromJSON() failed [gpsi_ranges]");
			goto end;
		}
	...
	if(external_group_identifiers_ranges) {
		...
		if(!cJSON_IsArray(external_group_identifiers_ranges)) {
			ogs_error(
				"OpenAPI_nrf_info_served_udm_info_value_parseFromJSON() failed [external_group_identifiers_ranges]");
			goto end;
		}
	...
	if(routing_indicators) {
		...
		if(!cJSON_IsArray(routing_indicators)) {
			ogs_error(
				"OpenAPI_nrf_info_served_udm_info_value_parseFromJSON() failed [routing_indicators]");
			goto end;
		}

		routing_indicatorsList = OpenAPI_list_create();

		cJSON_ArrayForEach(routing_indicators_local,
		                   routing_indicators) {
			...
			OpenAPI_list_add(routing_indicatorsList,
			                 ogs_strdup(
						 routing_indicators_local->
						 valuestring));
		}
	}
	...
	if(internal_group_identifiers_ranges) {
		...
		if(!cJSON_IsArray(internal_group_identifiers_ranges)) {
			ogs_error(
				"OpenAPI_nrf_info_served_udm_info_value_parseFromJSON() failed [internal_group_identifiers_ranges]");
			goto end;
		}

		internal_group_identifiers_rangesList = OpenAPI_list_create();

		cJSON_ArrayForEach(internal_group_identifiers_ranges_local,
		                   internal_group_identifiers_ranges) {
			if(!cJSON_IsObject(
				   internal_group_identifiers_ranges_local))
			{
				ogs_error(
					"OpenAPI_nrf_info_served_udm_info_value_parseFromJSON() failed [internal_group_identifiers_ranges]");
				goto end;
			...
			if(!internal_group_identifiers_rangesItem) {
				ogs_error(
					"No internal_group_identifiers_rangesItem");
				goto end;
			}
			OpenAPI_list_add(internal_group_identifiers_rangesList,
			                 internal_group_identifiers_rangesItem);
		}
	}
	...
	suci_infos =
		cJSON_GetObjectItemCaseSensitive(
			nrf_info_served_udm_info_valueJSON, "suciInfos");
	if(suci_infos) {
		cJSON *suci_infos_local = NULL;
		if(!cJSON_IsArray(suci_infos)) {
			ogs_error(
				"OpenAPI_nrf_info_served_udm_info_value_parseFromJSON() failed [suci_infos]");
			goto end;
		}
		...
		cJSON_ArrayForEach(suci_infos_local, suci_infos) {
			...
			OpenAPI_suci_info_t *suci_infosItem =
				OpenAPI_suci_info_parseFromJSON(
					suci_infos_local);
			if(!suci_infosItem) {
	...
	nrf_info_served_udm_info_value_local_var =
		OpenAPI_nrf_info_served_udm_info_value_create(
			group_id &&
			!cJSON_IsNull(group_id) ?
	...
	if(internal_group_identifiers_rangesList) {
		OpenAPI_list_for_each(internal_group_identifiers_rangesList,
		                      node) {
			OpenAPI_internal_group_id_range_free(node->data);
		}
...
OpenAPI_nrf_info_served_udm_info_value_t *
OpenAPI_nrf_info_served_udm_info_value_copy(
	OpenAPI_nrf_info_served_udm_info_value_t	*dst,
	OpenAPI_nrf_info_served_udm_info_value_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_nrf_info_served_udm_info_value_convertToJSON() failed");
		return NULL;
	}
