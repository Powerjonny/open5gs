...
OpenAPI_lte_v2x_auth_t *OpenAPI_lte_v2x_auth_create(
	...
	OpenAPI_lte_v2x_auth_t *lte_v2x_auth_local_var =
		ogs_malloc(sizeof(OpenAPI_lte_v2x_auth_t));
...
cJSON *OpenAPI_lte_v2x_auth_convertToJSON(
	OpenAPI_lte_v2x_auth_t *lte_v2x_auth) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(lte_v2x_auth == NULL) {
		ogs_error(
			"OpenAPI_lte_v2x_auth_convertToJSON() failed [LteV2xAuth]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(lte_v2x_auth->vehicle_ue_auth != OpenAPI_ue_auth_NULL) {
		if(cJSON_AddStringToObject(item, "vehicleUeAuth",
		                           OpenAPI_ue_auth_ToString(
						   lte_v2x_auth->vehicle_ue_auth))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_lte_v2x_auth_convertToJSON() failed [vehicle_ue_auth]");
			goto end;
		}
	}

	if(lte_v2x_auth->pedestrian_ue_auth != OpenAPI_ue_auth_NULL) {
		if(cJSON_AddStringToObject(item, "pedestrianUeAuth",
		                           OpenAPI_ue_auth_ToString(
						   lte_v2x_auth->
						   pedestrian_ue_auth)) == NULL)
		{
			ogs_error(
				"OpenAPI_lte_v2x_auth_convertToJSON() failed [pedestrian_ue_auth]");
			goto end;
		}
	}
...
OpenAPI_lte_v2x_auth_t *OpenAPI_lte_v2x_auth_parseFromJSON(
	...
	if(vehicle_ue_auth) {
		if(!cJSON_IsString(vehicle_ue_auth)) {
			ogs_error(
				"OpenAPI_lte_v2x_auth_parseFromJSON() failed [vehicle_ue_auth]");
			goto end;
		}
		vehicle_ue_authVariable =
			OpenAPI_ue_auth_FromString(
				vehicle_ue_auth->valuestring);
	}
	...
	if(pedestrian_ue_auth) {
		if(!cJSON_IsString(pedestrian_ue_auth)) {
			ogs_error(
				"OpenAPI_lte_v2x_auth_parseFromJSON() failed [pedestrian_ue_auth]");
			goto end;
		}
		pedestrian_ue_authVariable =
			OpenAPI_ue_auth_FromString(
				pedestrian_ue_auth->valuestring);
	}
