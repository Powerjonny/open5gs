...
OpenAPI_nudm_amf_info_t *OpenAPI_nudm_amf_info_create(
	...
	access_type) {
	OpenAPI_nudm_amf_info_t *nudm_amf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_nudm_amf_info_t));
...
cJSON *OpenAPI_nudm_amf_info_convertToJSON(
	OpenAPI_nudm_amf_info_t *nudm_amf_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(nudm_amf_info == NULL) {
		ogs_error(
			"OpenAPI_nudm_amf_info_convertToJSON() failed [NudmAmfInfo]");
		return NULL;
	}
	...
	if(!nudm_amf_info->amf_instance_id) {
		ogs_error(
			"OpenAPI_nudm_amf_info_convertToJSON() failed [amf_instance_id]");
		return NULL;
	}
	...
	if(!nudm_amf_info->guami) {
		ogs_error(
			"OpenAPI_nudm_amf_info_convertToJSON() failed [guami]");
		return NULL;
	}
	cJSON *guami_local_JSON =
		OpenAPI_guami_convertToJSON(nudm_amf_info->guami);
	...
	if(nudm_amf_info->access_type != OpenAPI_access_type_NULL) {
		if(cJSON_AddStringToObject(item, "accessType",
						   ...
						   nudm_amf_info->access_type))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_nudm_amf_info_convertToJSON() failed [access_type]");
			goto end;
		}
	}
...
OpenAPI_nudm_amf_info_t *OpenAPI_nudm_amf_info_parseFromJSON(
	cJSON *nudm_amf_infoJSON) {
	...
	amf_instance_id = cJSON_GetObjectItemCaseSensitive(nudm_amf_infoJSON,
	                                                   "amfInstanceId");
	if(!amf_instance_id) {
		ogs_error(
			"OpenAPI_nudm_amf_info_parseFromJSON() failed [amf_instance_id]");
		goto end;
	}
	...
	if(!guami) {
		ogs_error(
			"OpenAPI_nudm_amf_info_parseFromJSON() failed [guami]");
		goto end;
	}
	...
	access_type = cJSON_GetObjectItemCaseSensitive(nudm_amf_infoJSON,
	                                               "accessType");
	if(access_type) {
		if(!cJSON_IsString(access_type)) {
			ogs_error(
				"OpenAPI_nudm_amf_info_parseFromJSON() failed [access_type]");
			goto end;
		}
		access_typeVariable =
...
OpenAPI_nudm_amf_info_t *OpenAPI_nudm_amf_info_copy(
	OpenAPI_nudm_amf_info_t *dst, OpenAPI_nudm_amf_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
