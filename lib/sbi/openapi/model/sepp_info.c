...
OpenAPI_sepp_info_t *OpenAPI_sepp_info_create(char		*sepp_prefix,
                                              OpenAPI_list_t	*sepp_ports,
                                              OpenAPI_list_t	*
                                              remote_plmn_list,
                                              OpenAPI_list_t	*
                                              remote_snpn_list) {
	OpenAPI_sepp_info_t *sepp_info_local_var =
		ogs_malloc(sizeof(OpenAPI_sepp_info_t));
...
cJSON *OpenAPI_sepp_info_convertToJSON(OpenAPI_sepp_info_t *sepp_info) {
	...
	if(sepp_info == NULL) {
		ogs_error(
			"OpenAPI_sepp_info_convertToJSON() failed [SeppInfo]");
		return NULL;
	}
	...
	if(sepp_info->sepp_prefix) {
		...
		{
			ogs_error(
				"OpenAPI_sepp_info_convertToJSON() failed [sepp_prefix]");
			goto end;
		}
	}
	...
	if(sepp_info->sepp_ports) {
		...
		if(sepp_ports == NULL) {
			ogs_error(
				"OpenAPI_sepp_info_convertToJSON() failed [sepp_ports]");
			goto end;
		}
		...
		if(sepp_info->sepp_ports) {
			OpenAPI_list_for_each(sepp_info->sepp_ports, node) {
				...
				if(localKeyValue->value == NULL) {
					ogs_error(
						"OpenAPI_sepp_info_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(sepp_info->remote_plmn_list) {
		...
		if(remote_plmn_listList == NULL) {
			ogs_error(
				"OpenAPI_sepp_info_convertToJSON() failed [remote_plmn_list]");
			goto end;
		}
		OpenAPI_list_for_each(sepp_info->remote_plmn_list, node) {
			cJSON *itemLocal =
	...
	if(sepp_info->remote_snpn_list) {
		...
		if(remote_snpn_listList == NULL) {
			ogs_error(
				"OpenAPI_sepp_info_convertToJSON() failed [remote_snpn_list]");
			goto end;
		}
		OpenAPI_list_for_each(sepp_info->remote_snpn_list, node) {
			cJSON *itemLocal =
...
OpenAPI_sepp_info_t *OpenAPI_sepp_info_parseFromJSON(cJSON *sepp_infoJSON) {
	...
	sepp_prefix = cJSON_GetObjectItemCaseSensitive(sepp_infoJSON,
	                                               "seppPrefix");
	if(sepp_prefix) {
		...
		{
			ogs_error(
				"OpenAPI_sepp_info_parseFromJSON() failed [sepp_prefix]");
			goto end;
		}
	}

	sepp_ports = cJSON_GetObjectItemCaseSensitive(sepp_infoJSON,
	                                              "seppPorts");
	if(sepp_ports) {
		...
		{
			ogs_error(
				"OpenAPI_sepp_info_parseFromJSON() failed [sepp_ports]");
			goto end;
		}
		if(cJSON_IsObject(sepp_ports)) {
			...
			cJSON_ArrayForEach(sepp_ports_local_map, sepp_ports) {
				...
				if(!cJSON_IsNumber(localMapObject)) {
					ogs_error(
						"OpenAPI_sepp_info_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	remote_plmn_list = cJSON_GetObjectItemCaseSensitive(sepp_infoJSON,
	                                                    "remotePlmnList");
	if(remote_plmn_list) {
		cJSON *remote_plmn_list_local = NULL;
		if(!cJSON_IsArray(remote_plmn_list)) {
			ogs_error(
				"OpenAPI_sepp_info_parseFromJSON() failed [remote_plmn_list]");
			goto end;
		}
	...
	remote_snpn_list = cJSON_GetObjectItemCaseSensitive(sepp_infoJSON,
	                                                    "remoteSnpnList");
	if(remote_snpn_list) {
		cJSON *remote_snpn_list_local = NULL;
		if(!cJSON_IsArray(remote_snpn_list)) {
			ogs_error(
				"OpenAPI_sepp_info_parseFromJSON() failed [remote_snpn_list]");
			goto end;
		}
...
OpenAPI_sepp_info_t *OpenAPI_sepp_info_copy(OpenAPI_sepp_info_t *dst,
                                            OpenAPI_sepp_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
