...
{
	OpenAPI_tunnel_info_t *tunnel_info_local_var =
		ogs_malloc(sizeof(OpenAPI_tunnel_info_t));
...
cJSON *OpenAPI_tunnel_info_convertToJSON(OpenAPI_tunnel_info_t *tunnel_info) {
	...
	if(tunnel_info == NULL) {
		ogs_error(
			"OpenAPI_tunnel_info_convertToJSON() failed [TunnelInfo]");
		return NULL;
	}
	...
	if(tunnel_info->ipv4_addr) {
		...
		{
			ogs_error(
				"OpenAPI_tunnel_info_convertToJSON() failed [ipv4_addr]");
			goto end;
		}
	}
	...
	if(tunnel_info->ipv6_addr) {
		...
		{
			ogs_error(
				"OpenAPI_tunnel_info_convertToJSON() failed [ipv6_addr]");
			goto end;
		}
	}
	...
	if(!tunnel_info->gtp_teid) {
		ogs_error(
			"OpenAPI_tunnel_info_convertToJSON() failed [gtp_teid]");
		return NULL;
	}
	...
	if(tunnel_info->an_type != OpenAPI_access_type_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_tunnel_info_convertToJSON() failed [an_type]");
			goto end;
		}
	}
...
OpenAPI_tunnel_info_t *OpenAPI_tunnel_info_parseFromJSON(
	...
	ipv4_addr = cJSON_GetObjectItemCaseSensitive(tunnel_infoJSON,
	                                             "ipv4Addr");
	if(ipv4_addr) {
		...
		{
			ogs_error(
				"OpenAPI_tunnel_info_parseFromJSON() failed [ipv4_addr]");
			goto end;
		}
	}

	ipv6_addr = cJSON_GetObjectItemCaseSensitive(tunnel_infoJSON,
	                                             "ipv6Addr");
	if(ipv6_addr) {
		...
		{
			ogs_error(
				"OpenAPI_tunnel_info_parseFromJSON() failed [ipv6_addr]");
			goto end;
		}
	}
	...
	if(!gtp_teid) {
		ogs_error(
			"OpenAPI_tunnel_info_parseFromJSON() failed [gtp_teid]");
		goto end;
	}
	...
	if(an_type) {
		if(!cJSON_IsString(an_type)) {
			ogs_error(
				"OpenAPI_tunnel_info_parseFromJSON() failed [an_type]");
			goto end;
		}
		an_typeVariable =
