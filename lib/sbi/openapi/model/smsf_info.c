...
OpenAPI_smsf_info_t *OpenAPI_smsf_info_create(
	...
	char			*smsf_set_id) {
	OpenAPI_smsf_info_t *smsf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_smsf_info_t));
...
cJSON *OpenAPI_smsf_info_convertToJSON(OpenAPI_smsf_info_t *smsf_info) {
	...
	if(smsf_info == NULL) {
		ogs_error(
			"OpenAPI_smsf_info_convertToJSON() failed [SmsfInfo]");
		return NULL;
	}
	...
	if(!smsf_info->smsf_instance_id) {
		ogs_error(
			"OpenAPI_smsf_info_convertToJSON() failed [smsf_instance_id]");
		return NULL;
	}
	...
	cJSON *plmn_id_local_JSON =
		OpenAPI_plmn_id_convertToJSON(smsf_info->plmn_id);
	...
	if(smsf_info->smsf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_smsf_info_convertToJSON() failed [smsf_set_id]");
			goto end;
		}
	}
...
OpenAPI_smsf_info_t *OpenAPI_smsf_info_parseFromJSON(cJSON *smsf_infoJSON) {
	...
	smsf_instance_id = cJSON_GetObjectItemCaseSensitive(smsf_infoJSON,
	                                                    "smsfInstanceId");
	if(!smsf_instance_id) {
		ogs_error(
			"OpenAPI_smsf_info_parseFromJSON() failed [smsf_instance_id]");
		goto end;
	}
	...
	smsf_set_id = cJSON_GetObjectItemCaseSensitive(smsf_infoJSON,
	                                               "smsfSetId");
	if(smsf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_smsf_info_parseFromJSON() failed [smsf_set_id]");
			goto end;
		}
	}
