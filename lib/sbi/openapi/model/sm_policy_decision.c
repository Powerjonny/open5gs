...
OpenAPI_sm_policy_decision_t *OpenAPI_sm_policy_decision_create(
	OpenAPI_list_t *sess_rules, bool is_pcc_rules_null,
	OpenAPI_list_t *pcc_rules, bool is_pcscf_rest_indication,
	int pcscf_rest_indication, OpenAPI_list_t *qos_decs,
	bool is_chg_decs_null, OpenAPI_list_t *chg_decs,
	OpenAPI_charging_information_t *charging_info,
	OpenAPI_list_t *traff_cont_decs, bool is_um_decs_null,
	OpenAPI_list_t *um_decs, OpenAPI_list_t *qos_chars,
	bool is_qos_mon_decs_null, OpenAPI_list_t *qos_mon_decs,
	bool is_reflective_qo_s_timer, int reflective_qo_s_timer,
	bool is_conds_null, OpenAPI_list_t *conds, char *revalidation_time,
	bool is_offline, int offline, bool is_online, int online,
	bool is_offline_ch_only, int offline_ch_only,
	...
	bool is_pra_infos_null, OpenAPI_list_t *pra_infos, bool is_ipv4_index,
	int ipv4_index, bool is_ipv6_index, int ipv6_index,
	...
	OpenAPI_list_t *tsn_port_man_cont_nwtts, bool is_red_sess_indication,
	int red_sess_indication) {
	OpenAPI_sm_policy_decision_t *sm_policy_decision_local_var =
		ogs_malloc(sizeof(OpenAPI_sm_policy_decision_t));
	...
	sm_policy_decision_local_var->is_pcscf_rest_indication =
		is_pcscf_rest_indication;
	sm_policy_decision_local_var->pcscf_rest_indication =
		pcscf_rest_indication;
	...
	sm_policy_decision_local_var->is_qos_mon_decs_null =
		is_qos_mon_decs_null;
	sm_policy_decision_local_var->qos_mon_decs = qos_mon_decs;
	sm_policy_decision_local_var->is_reflective_qo_s_timer =
		is_reflective_qo_s_timer;
	sm_policy_decision_local_var->reflective_qo_s_timer =
		reflective_qo_s_timer;
	...
	sm_policy_decision_local_var->is_policy_ctrl_req_triggers_null =
		is_policy_ctrl_req_triggers_null;
	sm_policy_decision_local_var->policy_ctrl_req_triggers =
	...
	sm_policy_decision_local_var->tsn_port_man_cont_dstt =
		tsn_port_man_cont_dstt;
	sm_policy_decision_local_var->tsn_port_man_cont_nwtts =
		tsn_port_man_cont_nwtts;
	sm_policy_decision_local_var->is_red_sess_indication =
		is_red_sess_indication;
	sm_policy_decision_local_var->red_sess_indication = red_sess_indication;

	return sm_policy_decision_local_var;
}

void OpenAPI_sm_policy_decision_free(
	...
	if(sm_policy_decision->charging_info) {
		OpenAPI_charging_information_free(
			sm_policy_decision->charging_info);
		sm_policy_decision->charging_info = NULL;
	}
	if(sm_policy_decision->traff_cont_decs) {
		OpenAPI_list_for_each(sm_policy_decision->traff_cont_decs,
		                      node) {
			OpenAPI_map_t *localKeyValue =
				(OpenAPI_map_t *) node->data;
			ogs_free(localKeyValue->key);
	...
	if(sm_policy_decision->um_decs) {
		OpenAPI_list_for_each(sm_policy_decision->um_decs, node) {
			OpenAPI_map_t *localKeyValue =
				(OpenAPI_map_t *) node->data;
			ogs_free(localKeyValue->key);
			OpenAPI_usage_monitoring_data_free(
				localKeyValue->value);
			OpenAPI_map_free(localKeyValue);
		}
	...
	if(sm_policy_decision->last_req_rule_data) {
		OpenAPI_list_for_each(sm_policy_decision->last_req_rule_data,
		                      node) {
	...
	if(sm_policy_decision->last_req_usage_data) {
		OpenAPI_requested_usage_data_free(
			sm_policy_decision->last_req_usage_data);
	...
	if(sm_policy_decision->tsn_port_man_cont_nwtts) {
		OpenAPI_list_for_each(
			sm_policy_decision->tsn_port_man_cont_nwtts, node) {
...
cJSON *OpenAPI_sm_policy_decision_convertToJSON(
	OpenAPI_sm_policy_decision_t *sm_policy_decision) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(sm_policy_decision == NULL) {
		ogs_error(
			"OpenAPI_sm_policy_decision_convertToJSON() failed [SmPolicyDecision]");
		return NULL;
	}
	...
	if(sm_policy_decision->sess_rules) {
		...
		if(sess_rules == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [sess_rules]");
			goto end;
		}
		cJSON *localMapObject = sess_rules;
		if(sm_policy_decision->sess_rules) {
			OpenAPI_list_for_each(sm_policy_decision->sess_rules,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_session_rule_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_sm_policy_decision_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(sm_policy_decision->pcc_rules) {
		...
		if(pcc_rules == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [pcc_rules]");
			goto end;
		}
		cJSON *localMapObject = pcc_rules;
		if(sm_policy_decision->pcc_rules) {
			OpenAPI_list_for_each(sm_policy_decision->pcc_rules,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				if(localKeyValue == NULL) {
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_pcc_rule_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
	...
	if(sm_policy_decision->is_pcscf_rest_indication) {
		if(cJSON_AddBoolToObject(item, "pcscfRestIndication",
		                         sm_policy_decision->
		                         pcscf_rest_indication) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [pcscf_rest_indication]");
			goto end;
		}
	}
	...
	if(sm_policy_decision->qos_decs) {
		...
		if(qos_decs == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [qos_decs]");
			goto end;
		}
		cJSON *localMapObject = qos_decs;
		if(sm_policy_decision->qos_decs) {
			OpenAPI_list_for_each(sm_policy_decision->qos_decs,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				if(localKeyValue == NULL) {
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_qos_data_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
	...
	if(sm_policy_decision->chg_decs) {
		...
		if(chg_decs == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [chg_decs]");
			goto end;
		}
		cJSON *localMapObject = chg_decs;
		if(sm_policy_decision->chg_decs) {
			OpenAPI_list_for_each(sm_policy_decision->chg_decs,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				if(localKeyValue == NULL) {
	...
	if(sm_policy_decision->charging_info) {
		...
		if(charging_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [charging_info]");
			goto end;
		}
	...
	if(sm_policy_decision->traff_cont_decs) {
		cJSON *traff_cont_decs = cJSON_AddObjectToObject(item,
		                                                 "traffContDecs");
		if(traff_cont_decs == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [traff_cont_decs]");
			goto end;
		}
		cJSON *localMapObject = traff_cont_decs;
		if(sm_policy_decision->traff_cont_decs) {
			OpenAPI_list_for_each(
				sm_policy_decision->traff_cont_decs, node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				if(localKeyValue == NULL) {
					ogs_error(
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_traffic_control_data_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
	...
	if(sm_policy_decision->um_decs) {
		...
		if(um_decs == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [um_decs]");
			goto end;
		}
		...
		if(sm_policy_decision->um_decs) {
			OpenAPI_list_for_each(sm_policy_decision->um_decs,
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_usage_monitoring_data_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
	...
	if(sm_policy_decision->qos_chars) {
		...
		if(qos_chars == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [qos_chars]");
			goto end;
		}
		cJSON *localMapObject = qos_chars;
		if(sm_policy_decision->qos_chars) {
			OpenAPI_list_for_each(sm_policy_decision->qos_chars,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				if(localKeyValue == NULL) {
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_qos_characteristics_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
	...
	if(sm_policy_decision->qos_mon_decs) {
		cJSON *qos_mon_decs = cJSON_AddObjectToObject(item,
		                                              "qosMonDecs");
		if(qos_mon_decs == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [qos_mon_decs]");
			goto end;
		}
		cJSON *localMapObject = qos_mon_decs;
		if(sm_policy_decision->qos_mon_decs) {
			OpenAPI_list_for_each(sm_policy_decision->qos_mon_decs,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				if(localKeyValue == NULL) {
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_qos_monitoring_data_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
	...
	if(sm_policy_decision->is_reflective_qo_s_timer) {
		if(cJSON_AddNumberToObject(item, "reflectiveQoSTimer",
		                           sm_policy_decision->
		                           reflective_qo_s_timer) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [reflective_qo_s_timer]");
			goto end;
		}
	}
	...
	if(sm_policy_decision->conds) {
		...
		if(conds == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [conds]");
			goto end;
		}
		...
		if(sm_policy_decision->conds) {
			OpenAPI_list_for_each(sm_policy_decision->conds, node) {
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_condition_data_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
	...
	if(sm_policy_decision->revalidation_time) {
		if(cJSON_AddStringToObject(item, "revalidationTime",
		                           sm_policy_decision->revalidation_time)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [revalidation_time]");
			goto end;
		}
	}
	...
	if(sm_policy_decision->is_offline) {
		if(cJSON_AddBoolToObject(item, "offline",
		                         sm_policy_decision->offline) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [offline]");
			goto end;
		}
	}
	...
	if(sm_policy_decision->is_online) {
		if(cJSON_AddBoolToObject(item, "online",
		                         sm_policy_decision->online) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [online]");
			goto end;
		}
	}

	if(sm_policy_decision->is_offline_ch_only) {
		if(cJSON_AddBoolToObject(item, "offlineChOnly",
		                         sm_policy_decision->offline_ch_only) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [offline_ch_only]");
			goto end;
		}
	}
	...
	{
		cJSON *policy_ctrl_req_triggersList =
			cJSON_AddArrayToObject(item, "policyCtrlReqTriggers");
		if(policy_ctrl_req_triggersList == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [policy_ctrl_req_triggers]");
			goto end;
		}
		OpenAPI_list_for_each(
			sm_policy_decision->policy_ctrl_req_triggers, node) {
			if(cJSON_AddStringToObject(policy_ctrl_req_triggersList,
			                           "",
	...
	if(sm_policy_decision->last_req_rule_data) {
		cJSON *last_req_rule_dataList = cJSON_AddArrayToObject(item,
		                                                       "lastReqRuleData");
		if(last_req_rule_dataList == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [last_req_rule_data]");
			goto end;
		}
		OpenAPI_list_for_each(sm_policy_decision->last_req_rule_data,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_requested_rule_data_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_sm_policy_decision_convertToJSON() failed [last_req_rule_data]");
	...
	if(sm_policy_decision->last_req_usage_data) {
		cJSON *last_req_usage_data_local_JSON =
			OpenAPI_requested_usage_data_convertToJSON(
				sm_policy_decision->last_req_usage_data);
		if(last_req_usage_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [last_req_usage_data]");
			goto end;
		}
		cJSON_AddItemToObject(item, "lastReqUsageData",
		                      last_req_usage_data_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [last_req_usage_data]");
			goto end;
	...
	if(sm_policy_decision->pra_infos) {
		...
		if(pra_infos == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [pra_infos]");
			goto end;
		}
		cJSON *localMapObject = pra_infos;
		if(sm_policy_decision->pra_infos) {
			OpenAPI_list_for_each(sm_policy_decision->pra_infos,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				if(localKeyValue == NULL) {
	...
	if(sm_policy_decision->is_ipv4_index) {
		if(cJSON_AddNumberToObject(item, "ipv4Index",
		                           sm_policy_decision->ipv4_index) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [ipv4_index]");
			goto end;
		}
	}
	...
	if(sm_policy_decision->is_ipv6_index) {
		if(cJSON_AddNumberToObject(item, "ipv6Index",
		                           sm_policy_decision->ipv6_index) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [ipv6_index]");
			goto end;
		}
	}
	...
	if(sm_policy_decision->qos_flow_usage != OpenAPI_qos_flow_usage_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [qos_flow_usage]");
			goto end;
		}
	}
	...
	{
		if(cJSON_AddStringToObject(item, "relCause",
		                           OpenAPI_sm_policy_association_release_cause_ToString
		                                   (sm_policy_decision->
		                                   rel_cause)) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [rel_cause]");
			goto end;
		}
	}
	...
	if(sm_policy_decision->supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [supp_feat]");
			goto end;
		}
	}
	...
	if(sm_policy_decision->tsn_bridge_man_cont) {
		...
		if(tsn_bridge_man_cont_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [tsn_bridge_man_cont]");
			goto end;
		}
	...
	if(sm_policy_decision->tsn_port_man_cont_dstt) {
		...
		if(tsn_port_man_cont_dstt_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [tsn_port_man_cont_dstt]");
			goto end;
		}
	...
	if(sm_policy_decision->tsn_port_man_cont_nwtts) {
		...
		if(tsn_port_man_cont_nwttsList == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [tsn_port_man_cont_nwtts]");
			goto end;
		}
		OpenAPI_list_for_each(
			sm_policy_decision->tsn_port_man_cont_nwtts, node) {
	...
	if(sm_policy_decision->is_red_sess_indication) {
		if(cJSON_AddBoolToObject(item, "redSessIndication",
		                         sm_policy_decision->red_sess_indication)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_decision_convertToJSON() failed [red_sess_indication]");
			goto end;
		}
	}
...
OpenAPI_sm_policy_decision_t *OpenAPI_sm_policy_decision_parseFromJSON(
	...
	OpenAPI_requested_usage_data_t *last_req_usage_data_local_nonprim =
		NULL;
	...
	sess_rules = cJSON_GetObjectItemCaseSensitive(sm_policy_decisionJSON,
	                                              "sessRules");
	if(sess_rules) {
		cJSON *sess_rules_local_map = NULL;
		if(!cJSON_IsObject(sess_rules) &&
		   !cJSON_IsNull(sess_rules))
		{
			ogs_error(
				"OpenAPI_sm_policy_decision_parseFromJSON() failed [sess_rules]");
			goto end;
		}
		if(cJSON_IsObject(sess_rules)) {
			...
			cJSON_ArrayForEach(sess_rules_local_map, sess_rules) {
				...
				if(cJSON_IsObject(localMapObject)) {
					localMapKeyPair = OpenAPI_map_create(
						...
						OpenAPI_session_rule_parseFromJSON
						        (localMapObject));
					...
					ogs_error(
						"OpenAPI_sm_policy_decision_parseFromJSON() failed [inner]");
					goto end;
				}
				OpenAPI_list_add(sess_rulesList,
				                 localMapKeyPair);
			}
		}
	}

	pcc_rules = cJSON_GetObjectItemCaseSensitive(sm_policy_decisionJSON,
	                                             "pccRules");
	if(pcc_rules) {
		if(!cJSON_IsNull(pcc_rules)) {
			cJSON *pcc_rules_local_map = NULL;
			if(!cJSON_IsObject(pcc_rules) &&
			   !cJSON_IsNull(pcc_rules))
			{
				ogs_error(
					"OpenAPI_sm_policy_decision_parseFromJSON() failed [pcc_rules]");
				goto end;
			}
			if(cJSON_IsObject(pcc_rules)) {
				pcc_rulesList = OpenAPI_list_create();
				OpenAPI_map_t *localMapKeyPair = NULL;
				cJSON_ArrayForEach(pcc_rules_local_map,
				                   pcc_rules) {
					...
					OpenAPI_list_add(pcc_rulesList,
					                 localMapKeyPair);
				}
			}
		}
	...
	if(pcscf_rest_indication) {
		if(!cJSON_IsBool(pcscf_rest_indication)) {
			ogs_error(
				"OpenAPI_sm_policy_decision_parseFromJSON() failed [pcscf_rest_indication]");
			goto end;
		}
	}

	qos_decs = cJSON_GetObjectItemCaseSensitive(sm_policy_decisionJSON,
	                                            "qosDecs");
	if(qos_decs) {
		cJSON *qos_decs_local_map = NULL;
		if(!cJSON_IsObject(qos_decs) &&
		   !cJSON_IsNull(qos_decs))
		{
			ogs_error(
				"OpenAPI_sm_policy_decision_parseFromJSON() failed [qos_decs]");
			goto end;
		}
		if(cJSON_IsObject(qos_decs)) {
			...
			cJSON_ArrayForEach(qos_decs_local_map, qos_decs) {
				...
				if(cJSON_IsObject(localMapObject)) {
					localMapKeyPair = OpenAPI_map_create(
						ogs_strdup(
							localMapObject->string),
						OpenAPI_qos_data_parseFromJSON(
							localMapObject));
				} else if(cJSON_IsNull(localMapObject)) {
					localMapKeyPair =
						OpenAPI_map_create(ogs_strdup(
	...
	if(chg_decs) {
		if(!cJSON_IsNull(chg_decs)) {
			...
			if(!cJSON_IsObject(chg_decs) &&
			   !cJSON_IsNull(chg_decs))
			{
				ogs_error(
					"OpenAPI_sm_policy_decision_parseFromJSON() failed [chg_decs]");
				goto end;
			}
			if(cJSON_IsObject(chg_decs)) {
				chg_decsList = OpenAPI_list_create();
				OpenAPI_map_t *localMapKeyPair = NULL;
				cJSON_ArrayForEach(chg_decs_local_map,
				                   chg_decs) {
					cJSON *localMapObject =
						chg_decs_local_map;
					if(cJSON_IsObject(localMapObject)) {
	...
	charging_info = cJSON_GetObjectItemCaseSensitive(sm_policy_decisionJSON,
	                                                 "chargingInfo");
	if(charging_info) {
	...
	if(traff_cont_decs) {
		...
		if(!cJSON_IsObject(traff_cont_decs) &&
		   !cJSON_IsNull(traff_cont_decs))
		{
			ogs_error(
				"OpenAPI_sm_policy_decision_parseFromJSON() failed [traff_cont_decs]");
			goto end;
		}
		if(cJSON_IsObject(traff_cont_decs)) {
			traff_cont_decsList = OpenAPI_list_create();
			OpenAPI_map_t *localMapKeyPair = NULL;
			cJSON_ArrayForEach(traff_cont_decs_local_map,
			                   traff_cont_decs) {
				cJSON *localMapObject =
					traff_cont_decs_local_map;
				if(cJSON_IsObject(localMapObject)) {
					localMapKeyPair = OpenAPI_map_create(
						ogs_strdup(
							localMapObject->string),
						OpenAPI_traffic_control_data_parseFromJSON
						        (localMapObject));
				} else if(cJSON_IsNull(localMapObject)) {
					localMapKeyPair =
						OpenAPI_map_create(ogs_strdup(
				...
				OpenAPI_list_add(traff_cont_decsList,
				                 localMapKeyPair);
			}
		}
	}

	um_decs = cJSON_GetObjectItemCaseSensitive(sm_policy_decisionJSON,
	                                           "umDecs");
	if(um_decs) {
		if(!cJSON_IsNull(um_decs)) {
			cJSON *um_decs_local_map = NULL;
			if(!cJSON_IsObject(um_decs) &&
			   !cJSON_IsNull(um_decs))
			{
				ogs_error(
					"OpenAPI_sm_policy_decision_parseFromJSON() failed [um_decs]");
				goto end;
			}
			if(cJSON_IsObject(um_decs)) {
	...
	qos_chars = cJSON_GetObjectItemCaseSensitive(sm_policy_decisionJSON,
	                                             "qosChars");
	if(qos_chars) {
		cJSON *qos_chars_local_map = NULL;
		if(!cJSON_IsObject(qos_chars) &&
		   !cJSON_IsNull(qos_chars))
		{
			ogs_error(
				"OpenAPI_sm_policy_decision_parseFromJSON() failed [qos_chars]");
			goto end;
		}
		if(cJSON_IsObject(qos_chars)) {
			...
			cJSON_ArrayForEach(qos_chars_local_map, qos_chars) {
				...
				if(cJSON_IsObject(localMapObject)) {
					localMapKeyPair = OpenAPI_map_create(
						ogs_strdup(
							localMapObject->string),
						OpenAPI_qos_characteristics_parseFromJSON
						        (localMapObject));
				} else if(cJSON_IsNull(localMapObject)) {
					localMapKeyPair =
						OpenAPI_map_create(ogs_strdup(
				...
				OpenAPI_list_add(qos_charsList,
				                 localMapKeyPair);
			}
		}
	}

	qos_mon_decs = cJSON_GetObjectItemCaseSensitive(sm_policy_decisionJSON,
	                                                "qosMonDecs");
	if(qos_mon_decs) {
		if(!cJSON_IsNull(qos_mon_decs)) {
			cJSON *qos_mon_decs_local_map = NULL;
			if(!cJSON_IsObject(qos_mon_decs) &&
			   !cJSON_IsNull(qos_mon_decs))
			{
				ogs_error(
			...
			if(cJSON_IsObject(qos_mon_decs)) {
				...
				OpenAPI_map_t *localMapKeyPair = NULL;
				cJSON_ArrayForEach(qos_mon_decs_local_map,
				                   qos_mon_decs) {
					cJSON *localMapObject =
						qos_mon_decs_local_map;
					if(cJSON_IsObject(localMapObject)) {
						localMapKeyPair =
							OpenAPI_map_create(
								ogs_strdup(
					...
					OpenAPI_list_add(qos_mon_decsList,
					                 localMapKeyPair);
				}
			}
		}
	...
	if(reflective_qo_s_timer) {
		if(!cJSON_IsNumber(reflective_qo_s_timer)) {
			ogs_error(
				"OpenAPI_sm_policy_decision_parseFromJSON() failed [reflective_qo_s_timer]");
			goto end;
		}
	}

	conds = cJSON_GetObjectItemCaseSensitive(sm_policy_decisionJSON,
	                                         "conds");
	if(conds) {
		if(!cJSON_IsNull(conds)) {
			...
			{
				ogs_error(
					"OpenAPI_sm_policy_decision_parseFromJSON() failed [conds]");
				goto end;
			}
			if(cJSON_IsObject(conds)) {
						...
						localMapKeyPair =
							OpenAPI_map_create(
								ogs_strdup(
									localMapObject
									->string),
								OpenAPI_condition_data_parseFromJSON
								(
									localMapObject));
					} else if(cJSON_IsNull(
							  localMapObject))
	...
	if(revalidation_time) {
		if(!cJSON_IsString(revalidation_time) &&
		   !cJSON_IsNull(revalidation_time))
		{
			ogs_error(
				"OpenAPI_sm_policy_decision_parseFromJSON() failed [revalidation_time]");
			goto end;
		}
	}

	offline = cJSON_GetObjectItemCaseSensitive(sm_policy_decisionJSON,
	                                           "offline");
	if(offline) {
		if(!cJSON_IsBool(offline)) {
			ogs_error(
				"OpenAPI_sm_policy_decision_parseFromJSON() failed [offline]");
			goto end;
		}
	}

	online = cJSON_GetObjectItemCaseSensitive(sm_policy_decisionJSON,
	                                          "online");
	if(online) {
		if(!cJSON_IsBool(online)) {
			ogs_error(
				"OpenAPI_sm_policy_decision_parseFromJSON() failed [online]");
			goto end;
		}
	}
	...
	if(offline_ch_only) {
		if(!cJSON_IsBool(offline_ch_only)) {
			ogs_error(
				"OpenAPI_sm_policy_decision_parseFromJSON() failed [offline_ch_only]");
			goto end;
		}
	}
	...
	if(policy_ctrl_req_triggers) {
		if(!cJSON_IsNull(policy_ctrl_req_triggers)) {
			...
			if(!cJSON_IsArray(policy_ctrl_req_triggers)) {
				ogs_error(
					"OpenAPI_sm_policy_decision_parseFromJSON() failed [policy_ctrl_req_triggers]");
				goto end;
			}

			policy_ctrl_req_triggersList = OpenAPI_list_create();

			cJSON_ArrayForEach(policy_ctrl_req_triggers_local,
			                   policy_ctrl_req_triggers) {
				OpenAPI_policy_control_request_trigger_e
				        localEnum =
					OpenAPI_policy_control_request_trigger_NULL;
				if(!cJSON_IsString(
					   policy_ctrl_req_triggers_local))
				{
					ogs_error(
						"OpenAPI_sm_policy_decision_parseFromJSON() failed [policy_ctrl_req_triggers]");
					goto end;
				...
				if(!localEnum) {
					ogs_info(
						"Enum value \"%s\" for field \"policy_ctrl_req_triggers\" is not supported. Ignoring it ...",
						policy_ctrl_req_triggers_local
						->valuestring);
				} else {
					OpenAPI_list_add(
						policy_ctrl_req_triggersList,
						(void *) localEnum);
				}
			}
			if(policy_ctrl_req_triggersList->count == 0) {
				ogs_error(
					"OpenAPI_sm_policy_decision_parseFromJSON() failed: Expected policy_ctrl_req_triggersList to not be empty (after ignoring unsupported enum values).");
				goto end;
			}
		}
	}
	...
	if(last_req_rule_data) {
		...
		if(!cJSON_IsArray(last_req_rule_data)) {
			ogs_error(
				"OpenAPI_sm_policy_decision_parseFromJSON() failed [last_req_rule_data]");
			goto end;
		}

		last_req_rule_dataList = OpenAPI_list_create();

		cJSON_ArrayForEach(last_req_rule_data_local,
		                   last_req_rule_data) {
			...
			OpenAPI_requested_rule_data_t *last_req_rule_dataItem =
				OpenAPI_requested_rule_data_parseFromJSON(
					last_req_rule_data_local);
			if(!last_req_rule_dataItem) {
				ogs_error("No last_req_rule_dataItem");
				goto end;
			}
			OpenAPI_list_add(last_req_rule_dataList,
			                 last_req_rule_dataItem);
		}
	}
	...
	if(last_req_usage_data) {
		last_req_usage_data_local_nonprim =
			OpenAPI_requested_usage_data_parseFromJSON(
				last_req_usage_data);
		if(!last_req_usage_data_local_nonprim) {
			ogs_error(
				"OpenAPI_requested_usage_data_parseFromJSON failed [last_req_usage_data]");
			goto end;
		}
	}

	pra_infos = cJSON_GetObjectItemCaseSensitive(sm_policy_decisionJSON,
	                                             "praInfos");
	if(pra_infos) {
		if(!cJSON_IsNull(pra_infos)) {
			cJSON *pra_infos_local_map = NULL;
			if(!cJSON_IsObject(pra_infos) &&
			   !cJSON_IsNull(pra_infos))
			{
				ogs_error(
					"OpenAPI_sm_policy_decision_parseFromJSON() failed [pra_infos]");
				goto end;
			}
			if(cJSON_IsObject(pra_infos)) {
				pra_infosList = OpenAPI_list_create();
				OpenAPI_map_t *localMapKeyPair = NULL;
				cJSON_ArrayForEach(pra_infos_local_map,
				                   pra_infos) {
					cJSON *localMapObject =
						pra_infos_local_map;
					if(cJSON_IsObject(localMapObject)) {
					...
					OpenAPI_list_add(pra_infosList,
					                 localMapKeyPair);
				}
			}
		}
	...
	ipv4_index = cJSON_GetObjectItemCaseSensitive(sm_policy_decisionJSON,
	                                              "ipv4Index");
	if(ipv4_index) {
		if(!cJSON_IsNumber(ipv4_index)) {
			ogs_error(
				"OpenAPI_sm_policy_decision_parseFromJSON() failed [ipv4_index]");
			goto end;
		}
	}

	ipv6_index = cJSON_GetObjectItemCaseSensitive(sm_policy_decisionJSON,
	                                              "ipv6Index");
	if(ipv6_index) {
		if(!cJSON_IsNumber(ipv6_index)) {
			ogs_error(
				"OpenAPI_sm_policy_decision_parseFromJSON() failed [ipv6_index]");
			goto end;
		}
	}
	...
	if(qos_flow_usage) {
		if(!cJSON_IsString(qos_flow_usage)) {
			ogs_error(
				"OpenAPI_sm_policy_decision_parseFromJSON() failed [qos_flow_usage]");
			goto end;
		}
	...
	rel_cause = cJSON_GetObjectItemCaseSensitive(sm_policy_decisionJSON,
	                                             "relCause");
	if(rel_cause) {
		if(!cJSON_IsString(rel_cause)) {
			ogs_error(
				"OpenAPI_sm_policy_decision_parseFromJSON() failed [rel_cause]");
			goto end;
		}
		rel_causeVariable =
			OpenAPI_sm_policy_association_release_cause_FromString(
				rel_cause->valuestring);
	}

	supp_feat = cJSON_GetObjectItemCaseSensitive(sm_policy_decisionJSON,
	                                             "suppFeat");
	if(supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_decision_parseFromJSON() failed [supp_feat]");
			goto end;
		}
	}
	...
	if(tsn_port_man_cont_nwtts) {
		...
		if(!cJSON_IsArray(tsn_port_man_cont_nwtts)) {
			ogs_error(
				"OpenAPI_sm_policy_decision_parseFromJSON() failed [tsn_port_man_cont_nwtts]");
			goto end;
		}
	...
	if(red_sess_indication) {
		if(!cJSON_IsBool(red_sess_indication)) {
			ogs_error(
				"OpenAPI_sm_policy_decision_parseFromJSON() failed [red_sess_indication]");
			goto end;
		}
	}
	...
	sm_policy_decision_local_var = OpenAPI_sm_policy_decision_create(
		...
		pcc_rules &&
		cJSON_IsNull(pcc_rules) ? true : false,
		...
		conds &&
		cJSON_IsNull(conds) ? true : false,
		conds ? condsList : NULL,
		revalidation_time &&
		!cJSON_IsNull(revalidation_time) ?
		ogs_strdup(revalidation_time->valuestring) : NULL,
		offline ? true : false,
		...
		policy_ctrl_req_triggers &&
		cJSON_IsNull(policy_ctrl_req_triggers) ? true : false,
		...
		pra_infos &&
		cJSON_IsNull(pra_infos) ? true : false,
...
OpenAPI_sm_policy_decision_t *OpenAPI_sm_policy_decision_copy(
	OpenAPI_sm_policy_decision_t *dst, OpenAPI_sm_policy_decision_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
