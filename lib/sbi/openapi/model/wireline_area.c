...
OpenAPI_wireline_area_t *OpenAPI_wireline_area_create(
	...
	OpenAPI_wireline_area_t *wireline_area_local_var =
		ogs_malloc(sizeof(OpenAPI_wireline_area_t));
...
cJSON *OpenAPI_wireline_area_convertToJSON(
	...
	if(wireline_area == NULL) {
		ogs_error(
			"OpenAPI_wireline_area_convertToJSON() failed [WirelineArea]");
		return NULL;
	}
	...
	if(wireline_area->global_line_ids) {
		...
		if(global_line_idsList == NULL) {
			ogs_error(
				"OpenAPI_wireline_area_convertToJSON() failed [global_line_ids]");
			goto end;
		}
		OpenAPI_list_for_each(wireline_area->global_line_ids, node) {
	...
	if(wireline_area->hfc_n_ids) {
		...
		if(hfc_n_idsList == NULL) {
			ogs_error(
				"OpenAPI_wireline_area_convertToJSON() failed [hfc_n_ids]");
			goto end;
		}
		OpenAPI_list_for_each(wireline_area->hfc_n_ids, node) {
	...
	if(wireline_area->area_code_b) {
		...
		{
			ogs_error(
				"OpenAPI_wireline_area_convertToJSON() failed [area_code_b]");
			goto end;
		}
	}
	...
	if(wireline_area->area_code_c) {
		...
		{
			ogs_error(
				"OpenAPI_wireline_area_convertToJSON() failed [area_code_c]");
			goto end;
		}
	}
...
OpenAPI_wireline_area_t *OpenAPI_wireline_area_parseFromJSON(
	cJSON *wireline_areaJSON) {
	...
	if(global_line_ids) {
		...
		if(!cJSON_IsArray(global_line_ids)) {
			ogs_error(
				"OpenAPI_wireline_area_parseFromJSON() failed [global_line_ids]");
			goto end;
		}
	...
	if(hfc_n_ids) {
		...
		if(!cJSON_IsArray(hfc_n_ids)) {
			ogs_error(
				"OpenAPI_wireline_area_parseFromJSON() failed [hfc_n_ids]");
			goto end;
		}
	...
	if(area_code_b) {
		...
		{
			ogs_error(
				"OpenAPI_wireline_area_parseFromJSON() failed [area_code_b]");
			goto end;
		}
	}
	...
	if(area_code_c) {
		...
		{
			ogs_error(
				"OpenAPI_wireline_area_parseFromJSON() failed [area_code_c]");
			goto end;
		}
	}
...
OpenAPI_wireline_area_t *OpenAPI_wireline_area_copy(
	OpenAPI_wireline_area_t *dst, OpenAPI_wireline_area_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
