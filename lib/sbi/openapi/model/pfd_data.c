...
OpenAPI_pfd_data_t *OpenAPI_pfd_data_create(OpenAPI_list_t	*app_ids,
                                            OpenAPI_list_t	*af_ids) {
	OpenAPI_pfd_data_t *pfd_data_local_var =
		ogs_malloc(sizeof(OpenAPI_pfd_data_t));
...
cJSON *OpenAPI_pfd_data_convertToJSON(OpenAPI_pfd_data_t *pfd_data) {
	...
	if(pfd_data->app_ids) {
		...
		if(app_idsList == NULL) {
			ogs_error(
				"OpenAPI_pfd_data_convertToJSON() failed [app_ids]");
			goto end;
		}
		OpenAPI_list_for_each(pfd_data->app_ids, node) {
	...
	if(pfd_data->af_ids) {
		...
		if(af_idsList == NULL) {
			ogs_error(
				"OpenAPI_pfd_data_convertToJSON() failed [af_ids]");
			goto end;
		}
		OpenAPI_list_for_each(pfd_data->af_ids, node) {
...
OpenAPI_pfd_data_t *OpenAPI_pfd_data_parseFromJSON(cJSON *pfd_dataJSON) {
	...
	if(app_ids) {
		...
		if(!cJSON_IsArray(app_ids)) {
			ogs_error(
				"OpenAPI_pfd_data_parseFromJSON() failed [app_ids]");
			goto end;
		}
	...
	if(af_ids) {
		...
		if(!cJSON_IsArray(af_ids)) {
			ogs_error(
				"OpenAPI_pfd_data_parseFromJSON() failed [af_ids]");
			goto end;
		}
...
OpenAPI_pfd_data_t *OpenAPI_pfd_data_copy(OpenAPI_pfd_data_t	*dst,
                                          OpenAPI_pfd_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
