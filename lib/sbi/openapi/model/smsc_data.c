...
OpenAPI_smsc_data_t *OpenAPI_smsc_data_create(
	...
	OpenAPI_smsc_data_t *smsc_data_local_var =
		ogs_malloc(sizeof(OpenAPI_smsc_data_t));
...
void OpenAPI_smsc_data_free(OpenAPI_smsc_data_t *smsc_data) {
	...
	if(smsc_data->smsc_diameter_address) {
		OpenAPI_network_node_diameter_address_1_free(
			smsc_data->smsc_diameter_address);
...
cJSON *OpenAPI_smsc_data_convertToJSON(OpenAPI_smsc_data_t *smsc_data) {
	...
	if(smsc_data == NULL) {
		ogs_error(
			"OpenAPI_smsc_data_convertToJSON() failed [SmscData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(smsc_data->smsc_map_address) {
		if(cJSON_AddStringToObject(item, "smscMapAddress",
		                           smsc_data->smsc_map_address) == NULL)
		{
			ogs_error(
				"OpenAPI_smsc_data_convertToJSON() failed [smsc_map_address]");
			goto end;
		}
	}

	if(smsc_data->smsc_diameter_address) {
		cJSON *smsc_diameter_address_local_JSON =
			OpenAPI_network_node_diameter_address_1_convertToJSON(
				smsc_data->smsc_diameter_address);
		if(smsc_diameter_address_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_smsc_data_convertToJSON() failed [smsc_diameter_address]");
			goto end;
		}
		cJSON_AddItemToObject(item, "smscDiameterAddress",
		                      smsc_diameter_address_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_smsc_data_convertToJSON() failed [smsc_diameter_address]");
			goto end;
...
OpenAPI_smsc_data_t *OpenAPI_smsc_data_parseFromJSON(cJSON *smsc_dataJSON) {
	...
	smsc_map_address = cJSON_GetObjectItemCaseSensitive(smsc_dataJSON,
	                                                    "smscMapAddress");
	if(smsc_map_address) {
		if(!cJSON_IsString(smsc_map_address) &&
		   !cJSON_IsNull(smsc_map_address))
		{
			ogs_error(
				"OpenAPI_smsc_data_parseFromJSON() failed [smsc_map_address]");
			goto end;
		}
	}

	smsc_diameter_address = cJSON_GetObjectItemCaseSensitive(smsc_dataJSON,
	                                                         "smscDiameterAddress");
	if(smsc_diameter_address) {
		smsc_diameter_address_local_nonprim =
			OpenAPI_network_node_diameter_address_1_parseFromJSON(
				smsc_diameter_address);
		if(!smsc_diameter_address_local_nonprim) {
			ogs_error(
				"OpenAPI_network_node_diameter_address_1_parseFromJSON failed [smsc_diameter_address]");
			goto end;
		}
	}
	...
	smsc_data_local_var = OpenAPI_smsc_data_create(
		smsc_map_address &&
		!cJSON_IsNull(smsc_map_address) ?
		ogs_strdup(smsc_map_address->valuestring) : NULL,
		smsc_diameter_address ? smsc_diameter_address_local_nonprim :
		NULL
		);
...
OpenAPI_smsc_data_t *OpenAPI_smsc_data_copy(OpenAPI_smsc_data_t *dst,
                                            OpenAPI_smsc_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
