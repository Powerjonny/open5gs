...
OpenAPI_cancel_relocate_ue_context_request_create(
	...
	OpenAPI_cancel_relocate_ue_context_request_t *
		...
		ogs_malloc(
			sizeof(OpenAPI_cancel_relocate_ue_context_request_t));
...
void OpenAPI_cancel_relocate_ue_context_request_free(
	OpenAPI_cancel_relocate_ue_context_request_t *
	...
	if(cancel_relocate_ue_context_request->json_data) {
		OpenAPI_ue_context_cancel_relocate_data_free(
...
cJSON *OpenAPI_cancel_relocate_ue_context_request_convertToJSON(
	OpenAPI_cancel_relocate_ue_context_request_t *
	cancel_relocate_ue_context_request) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(cancel_relocate_ue_context_request == NULL) {
		ogs_error(
			"OpenAPI_cancel_relocate_ue_context_request_convertToJSON() failed [CancelRelocateUEContext_request]");
		return NULL;
	}
	...
	if(cancel_relocate_ue_context_request->json_data) {
		cJSON *json_data_local_JSON =
			OpenAPI_ue_context_cancel_relocate_data_convertToJSON(
				cancel_relocate_ue_context_request->json_data);
		if(json_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_cancel_relocate_ue_context_request_convertToJSON() failed [json_data]");
			goto end;
		}
	...
	if(cancel_relocate_ue_context_request->binary_data_gtpc_message) {
		char *encoded_str_binary_data_gtpc_message =
			OpenAPI_base64encode(
				cancel_relocate_ue_context_request->
				binary_data_gtpc_message->data,
				cancel_relocate_ue_context_request->
				binary_data_gtpc_message->len);
		if(cJSON_AddStringToObject(item, "binaryDataGtpcMessage",
		                           encoded_str_binary_data_gtpc_message)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_cancel_relocate_ue_context_request_convertToJSON() failed [binary_data_gtpc_message]");
			goto end;
		}
...
OpenAPI_cancel_relocate_ue_context_request_t *
OpenAPI_cancel_relocate_ue_context_request_parseFromJSON(
	cJSON *cancel_relocate_ue_context_requestJSON) {
	OpenAPI_cancel_relocate_ue_context_request_t *
	        cancel_relocate_ue_context_request_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *json_data = NULL;
	OpenAPI_ue_context_cancel_relocate_data_t *json_data_local_nonprim =
		NULL;
	...
	if(json_data) {
		json_data_local_nonprim =
			OpenAPI_ue_context_cancel_relocate_data_parseFromJSON(
				json_data);
		if(!json_data_local_nonprim) {
			ogs_error(
				"OpenAPI_ue_context_cancel_relocate_data_parseFromJSON failed [json_data]");
			goto end;
		}
	}
	...
	if(binary_data_gtpc_message) {
		decoded_str_binary_data_gtpc_message =
		...
		if(!cJSON_IsString(binary_data_gtpc_message)) {
			ogs_error(
				"OpenAPI_cancel_relocate_ue_context_request_parseFromJSON() failed [binary_data_gtpc_message]");
			goto end;
		}
		decoded_str_binary_data_gtpc_message->data =
			OpenAPI_base64decode(
				binary_data_gtpc_message->valuestring,
				strlen(binary_data_gtpc_message->valuestring),
	...
	cancel_relocate_ue_context_request_local_var =
		OpenAPI_cancel_relocate_ue_context_request_create(
			...
			binary_data_gtpc_message ?
			decoded_str_binary_data_gtpc_message : NULL
			);
...
OpenAPI_cancel_relocate_ue_context_request_t *
OpenAPI_cancel_relocate_ue_context_request_copy(
	OpenAPI_cancel_relocate_ue_context_request_t	*dst,
	OpenAPI_cancel_relocate_ue_context_request_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_cancel_relocate_ue_context_request_convertToJSON() failed");
		return NULL;
	}
