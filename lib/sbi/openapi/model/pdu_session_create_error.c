...
OpenAPI_pdu_session_create_error_t *OpenAPI_pdu_session_create_error_create(
	OpenAPI_problem_details_t *error, char *n1sm_cause,
	OpenAPI_ref_to_binary_data_t *n1_sm_info_to_ue, bool is_back_off_timer,
	int back_off_timer, char *recovery_time) {
	OpenAPI_pdu_session_create_error_t *pdu_session_create_error_local_var =
		ogs_malloc(sizeof(OpenAPI_pdu_session_create_error_t));
	...
	pdu_session_create_error_local_var->is_back_off_timer =
		is_back_off_timer;
...
void OpenAPI_pdu_session_create_error_free(
	OpenAPI_pdu_session_create_error_t *pdu_session_create_error) {
	...
	if(pdu_session_create_error->n1_sm_info_to_ue) {
		OpenAPI_ref_to_binary_data_free(
			pdu_session_create_error->n1_sm_info_to_ue);
...
cJSON *OpenAPI_pdu_session_create_error_convertToJSON(
	OpenAPI_pdu_session_create_error_t *pdu_session_create_error) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(pdu_session_create_error == NULL) {
		ogs_error(
			"OpenAPI_pdu_session_create_error_convertToJSON() failed [PduSessionCreateError]");
		return NULL;
	}
	...
	if(!pdu_session_create_error->error) {
		ogs_error(
			"OpenAPI_pdu_session_create_error_convertToJSON() failed [error]");
		return NULL;
	}
	cJSON *error_local_JSON =
		OpenAPI_problem_details_convertToJSON(
			pdu_session_create_error->error);
	...
	if(pdu_session_create_error->n1sm_cause) {
		if(cJSON_AddStringToObject(item, "n1smCause",
		                           pdu_session_create_error->n1sm_cause)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_pdu_session_create_error_convertToJSON() failed [n1sm_cause]");
			goto end;
		}
	}
	...
	if(pdu_session_create_error->n1_sm_info_to_ue) {
		...
		if(n1_sm_info_to_ue_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_pdu_session_create_error_convertToJSON() failed [n1_sm_info_to_ue]");
			goto end;
		}
	...
	if(pdu_session_create_error->is_back_off_timer) {
		...
		{
			ogs_error(
				"OpenAPI_pdu_session_create_error_convertToJSON() failed [back_off_timer]");
			goto end;
		}
	}
	...
	if(pdu_session_create_error->recovery_time) {
		...
		{
			ogs_error(
				"OpenAPI_pdu_session_create_error_convertToJSON() failed [recovery_time]");
			goto end;
		}
	}
...
OpenAPI_pdu_session_create_error_parseFromJSON(
	cJSON *pdu_session_create_errorJSON) {
	OpenAPI_pdu_session_create_error_t *pdu_session_create_error_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *error = NULL;
	...
	error = cJSON_GetObjectItemCaseSensitive(pdu_session_create_errorJSON,
	                                         "error");
	if(!error) {
		ogs_error(
			"OpenAPI_pdu_session_create_error_parseFromJSON() failed [error]");
		goto end;
	}
	...
	if(n1sm_cause) {
		...
		{
			ogs_error(
				"OpenAPI_pdu_session_create_error_parseFromJSON() failed [n1sm_cause]");
			goto end;
		}
	}
	...
	if(back_off_timer) {
		if(!cJSON_IsNumber(back_off_timer)) {
			ogs_error(
				"OpenAPI_pdu_session_create_error_parseFromJSON() failed [back_off_timer]");
			goto end;
		}
	}
	...
	if(recovery_time) {
		...
		{
			ogs_error(
				"OpenAPI_pdu_session_create_error_parseFromJSON() failed [recovery_time]");
			goto end;
		}
	}

	pdu_session_create_error_local_var =
		OpenAPI_pdu_session_create_error_create(
			error_local_nonprim,
			n1sm_cause &&
			!cJSON_IsNull(n1sm_cause) ?
...
OpenAPI_pdu_session_create_error_t *OpenAPI_pdu_session_create_error_copy(
	OpenAPI_pdu_session_create_error_t	*dst,
	OpenAPI_pdu_session_create_error_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_pdu_session_create_error_convertToJSON() failed");
		return NULL;
	}
