...
OpenAPI_snssai_upf_info_item_t *OpenAPI_snssai_upf_info_item_create(
	OpenAPI_ext_snssai_t *s_nssai, OpenAPI_list_t *dnn_upf_info_list,
	bool is_redundant_transport, int redundant_transport) {
	OpenAPI_snssai_upf_info_item_t *snssai_upf_info_item_local_var =
		ogs_malloc(sizeof(OpenAPI_snssai_upf_info_item_t));
	...
	snssai_upf_info_item_local_var->is_redundant_transport =
		is_redundant_transport;
	snssai_upf_info_item_local_var->redundant_transport =
		redundant_transport;

	return snssai_upf_info_item_local_var;
}

void OpenAPI_snssai_upf_info_item_free(
	OpenAPI_snssai_upf_info_item_t *snssai_upf_info_item) {
	...
	if(snssai_upf_info_item->dnn_upf_info_list) {
		OpenAPI_list_for_each(snssai_upf_info_item->dnn_upf_info_list,
		                      node) {
...
cJSON *OpenAPI_snssai_upf_info_item_convertToJSON(
	OpenAPI_snssai_upf_info_item_t *snssai_upf_info_item) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(snssai_upf_info_item == NULL) {
		ogs_error(
			"OpenAPI_snssai_upf_info_item_convertToJSON() failed [SnssaiUpfInfoItem]");
		return NULL;
	}
	...
	if(!snssai_upf_info_item->s_nssai) {
		ogs_error(
			"OpenAPI_snssai_upf_info_item_convertToJSON() failed [s_nssai]");
		return NULL;
	}
	cJSON *s_nssai_local_JSON =
		OpenAPI_ext_snssai_convertToJSON(snssai_upf_info_item->s_nssai);
	...
	if(!snssai_upf_info_item->dnn_upf_info_list) {
		ogs_error(
			"OpenAPI_snssai_upf_info_item_convertToJSON() failed [dnn_upf_info_list]");
		return NULL;
	}
	cJSON *dnn_upf_info_listList = cJSON_AddArrayToObject(item,
	                                                      "dnnUpfInfoList");
	...
	OpenAPI_list_for_each(snssai_upf_info_item->dnn_upf_info_list, node) {
		cJSON *itemLocal =
			OpenAPI_dnn_upf_info_item_convertToJSON(node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_snssai_upf_info_item_convertToJSON() failed [dnn_upf_info_list]");
			goto end;
	...
	if(snssai_upf_info_item->is_redundant_transport) {
		if(cJSON_AddBoolToObject(item, "redundantTransport",
		                         snssai_upf_info_item->
		                         redundant_transport) == NULL)
		{
			ogs_error(
				"OpenAPI_snssai_upf_info_item_convertToJSON() failed [redundant_transport]");
			goto end;
		}
	}
...
OpenAPI_snssai_upf_info_item_t *OpenAPI_snssai_upf_info_item_parseFromJSON(
	cJSON *snssai_upf_info_itemJSON) {
	...
	s_nssai = cJSON_GetObjectItemCaseSensitive(snssai_upf_info_itemJSON,
	                                           "sNssai");
	if(!s_nssai) {
		ogs_error(
			"OpenAPI_snssai_upf_info_item_parseFromJSON() failed [s_nssai]");
		goto end;
	}
	...
	dnn_upf_info_list =
		cJSON_GetObjectItemCaseSensitive(snssai_upf_info_itemJSON,
		                                 "dnnUpfInfoList");
	if(!dnn_upf_info_list) {
		ogs_error(
			"OpenAPI_snssai_upf_info_item_parseFromJSON() failed [dnn_upf_info_list]");
		goto end;
	}
	...
	cJSON_ArrayForEach(dnn_upf_info_list_local, dnn_upf_info_list) {
		...
		OpenAPI_dnn_upf_info_item_t *dnn_upf_info_listItem =
			OpenAPI_dnn_upf_info_item_parseFromJSON(
				dnn_upf_info_list_local);
	...
	if(redundant_transport) {
		if(!cJSON_IsBool(redundant_transport)) {
			ogs_error(
				"OpenAPI_snssai_upf_info_item_parseFromJSON() failed [redundant_transport]");
			goto end;
		}
	}
...
OpenAPI_snssai_upf_info_item_t *OpenAPI_snssai_upf_info_item_copy(
	OpenAPI_snssai_upf_info_item_t	*dst,
	OpenAPI_snssai_upf_info_item_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_snssai_upf_info_item_convertToJSON() failed");
		return NULL;
	}
