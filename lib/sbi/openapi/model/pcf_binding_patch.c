...
OpenAPI_pcf_binding_patch_t *OpenAPI_pcf_binding_patch_create(
	bool is_ipv4_addr_null, char *ipv4_addr, bool is_ip_domain_null,
	char *ip_domain, bool is_ipv6_prefix_null, char *ipv6_prefix,
	bool is_add_ipv6_prefixes_null, OpenAPI_list_t *add_ipv6_prefixes,
	bool is_mac_addr48_null, char *mac_addr48, bool is_add_mac_addrs_null,
	OpenAPI_list_t *add_mac_addrs, char *pcf_id, char *pcf_fqdn,
	OpenAPI_list_t *pcf_ip_end_points, char *pcf_diam_host,
	char *pcf_diam_realm) {
	OpenAPI_pcf_binding_patch_t *pcf_binding_patch_local_var =
		ogs_malloc(sizeof(OpenAPI_pcf_binding_patch_t));
	...
	pcf_binding_patch_local_var->is_add_ipv6_prefixes_null =
		is_add_ipv6_prefixes_null;
	...
	pcf_binding_patch_local_var->is_add_mac_addrs_null =
		is_add_mac_addrs_null;
...
void OpenAPI_pcf_binding_patch_free(
	OpenAPI_pcf_binding_patch_t *pcf_binding_patch) {
	...
	if(pcf_binding_patch->add_ipv6_prefixes) {
		OpenAPI_list_for_each(pcf_binding_patch->add_ipv6_prefixes,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_pcf_binding_patch_convertToJSON(
	OpenAPI_pcf_binding_patch_t *pcf_binding_patch) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(pcf_binding_patch == NULL) {
		ogs_error(
			"OpenAPI_pcf_binding_patch_convertToJSON() failed [PcfBindingPatch]");
		return NULL;
	}
	...
	if(pcf_binding_patch->ipv4_addr) {
		if(cJSON_AddStringToObject(item, "ipv4Addr",
		                           pcf_binding_patch->ipv4_addr) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pcf_binding_patch_convertToJSON() failed [ipv4_addr]");
			goto end;
		}
	...
	if(pcf_binding_patch->ip_domain) {
		if(cJSON_AddStringToObject(item, "ipDomain",
		                           pcf_binding_patch->ip_domain) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pcf_binding_patch_convertToJSON() failed [ip_domain]");
			goto end;
		}
	...
	if(pcf_binding_patch->ipv6_prefix) {
		if(cJSON_AddStringToObject(item, "ipv6Prefix",
		                           pcf_binding_patch->ipv6_prefix) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pcf_binding_patch_convertToJSON() failed [ipv6_prefix]");
			goto end;
		}
	...
	if(pcf_binding_patch->add_ipv6_prefixes) {
		...
		if(add_ipv6_prefixesList == NULL) {
			ogs_error(
				"OpenAPI_pcf_binding_patch_convertToJSON() failed [add_ipv6_prefixes]");
			goto end;
		}
		OpenAPI_list_for_each(pcf_binding_patch->add_ipv6_prefixes,
		                      node) {
	...
	if(pcf_binding_patch->mac_addr48) {
		if(cJSON_AddStringToObject(item, "macAddr48",
		                           pcf_binding_patch->mac_addr48) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pcf_binding_patch_convertToJSON() failed [mac_addr48]");
			goto end;
		}
	...
	if(pcf_binding_patch->add_mac_addrs) {
		...
		if(add_mac_addrsList == NULL) {
			ogs_error(
				"OpenAPI_pcf_binding_patch_convertToJSON() failed [add_mac_addrs]");
			goto end;
		}
		OpenAPI_list_for_each(pcf_binding_patch->add_mac_addrs, node) {
	...
	if(pcf_binding_patch->pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_patch_convertToJSON() failed [pcf_id]");
			goto end;
		}
	}
	...
	if(pcf_binding_patch->pcf_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_patch_convertToJSON() failed [pcf_fqdn]");
			goto end;
		}
	}
	...
	if(pcf_binding_patch->pcf_ip_end_points) {
		...
		if(pcf_ip_end_pointsList == NULL) {
			ogs_error(
				"OpenAPI_pcf_binding_patch_convertToJSON() failed [pcf_ip_end_points]");
			goto end;
		}
		OpenAPI_list_for_each(pcf_binding_patch->pcf_ip_end_points,
		                      node) {
	...
	if(pcf_binding_patch->pcf_diam_host) {
		if(cJSON_AddStringToObject(item, "pcfDiamHost",
		                           pcf_binding_patch->pcf_diam_host) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pcf_binding_patch_convertToJSON() failed [pcf_diam_host]");
			goto end;
		}
	}
	...
	if(pcf_binding_patch->pcf_diam_realm) {
		if(cJSON_AddStringToObject(item, "pcfDiamRealm",
		                           pcf_binding_patch->pcf_diam_realm) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pcf_binding_patch_convertToJSON() failed [pcf_diam_realm]");
			goto end;
		}
	}
...
OpenAPI_pcf_binding_patch_t *OpenAPI_pcf_binding_patch_parseFromJSON(
	...
	ipv4_addr = cJSON_GetObjectItemCaseSensitive(pcf_binding_patchJSON,
	                                             "ipv4Addr");
	if(ipv4_addr) {
		if(!cJSON_IsNull(ipv4_addr)) {
			...
			{
				ogs_error(
					"OpenAPI_pcf_binding_patch_parseFromJSON() failed [ipv4_addr]");
				goto end;
			}
		}
	}

	ip_domain = cJSON_GetObjectItemCaseSensitive(pcf_binding_patchJSON,
	                                             "ipDomain");
	if(ip_domain) {
		if(!cJSON_IsNull(ip_domain)) {
			...
			{
				ogs_error(
					"OpenAPI_pcf_binding_patch_parseFromJSON() failed [ip_domain]");
				goto end;
			}
		}
	}

	ipv6_prefix = cJSON_GetObjectItemCaseSensitive(pcf_binding_patchJSON,
	                                               "ipv6Prefix");
	if(ipv6_prefix) {
		if(!cJSON_IsNull(ipv6_prefix)) {
			...
			{
				ogs_error(
					"OpenAPI_pcf_binding_patch_parseFromJSON() failed [ipv6_prefix]");
				goto end;
			}
		}
	}
	...
	add_ipv6_prefixes =
		cJSON_GetObjectItemCaseSensitive(pcf_binding_patchJSON,
		                                 "addIpv6Prefixes");
	if(add_ipv6_prefixes) {
		if(!cJSON_IsNull(add_ipv6_prefixes)) {
			...
			if(!cJSON_IsArray(add_ipv6_prefixes)) {
				ogs_error(
					"OpenAPI_pcf_binding_patch_parseFromJSON() failed [add_ipv6_prefixes]");
				goto end;
			}

			add_ipv6_prefixesList = OpenAPI_list_create();

			cJSON_ArrayForEach(add_ipv6_prefixes_local,
			                   add_ipv6_prefixes) {
	...
	mac_addr48 = cJSON_GetObjectItemCaseSensitive(pcf_binding_patchJSON,
	                                              "macAddr48");
	if(mac_addr48) {
		if(!cJSON_IsNull(mac_addr48)) {
			...
			{
				ogs_error(
					"OpenAPI_pcf_binding_patch_parseFromJSON() failed [mac_addr48]");
				goto end;
			}
		}
	}

	add_mac_addrs = cJSON_GetObjectItemCaseSensitive(pcf_binding_patchJSON,
	                                                 "addMacAddrs");
	if(add_mac_addrs) {
		if(!cJSON_IsNull(add_mac_addrs)) {
			...
			if(!cJSON_IsArray(add_mac_addrs)) {
				ogs_error(
					"OpenAPI_pcf_binding_patch_parseFromJSON() failed [add_mac_addrs]");
				goto end;
			}
	...
	pcf_id = cJSON_GetObjectItemCaseSensitive(pcf_binding_patchJSON,
	                                          "pcfId");
	if(pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_patch_parseFromJSON() failed [pcf_id]");
			goto end;
		}
	}

	pcf_fqdn = cJSON_GetObjectItemCaseSensitive(pcf_binding_patchJSON,
	                                            "pcfFqdn");
	if(pcf_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_patch_parseFromJSON() failed [pcf_fqdn]");
			goto end;
		}
	}
	...
	if(pcf_ip_end_points) {
		...
		if(!cJSON_IsArray(pcf_ip_end_points)) {
			ogs_error(
				"OpenAPI_pcf_binding_patch_parseFromJSON() failed [pcf_ip_end_points]");
			goto end;
		}
	...
	pcf_diam_host = cJSON_GetObjectItemCaseSensitive(pcf_binding_patchJSON,
	                                                 "pcfDiamHost");
	if(pcf_diam_host) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_patch_parseFromJSON() failed [pcf_diam_host]");
			goto end;
		}
	}

	pcf_diam_realm = cJSON_GetObjectItemCaseSensitive(pcf_binding_patchJSON,
	                                                  "pcfDiamRealm");
	if(pcf_diam_realm) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_binding_patch_parseFromJSON() failed [pcf_diam_realm]");
			goto end;
		}
	}
...
OpenAPI_pcf_binding_patch_t *OpenAPI_pcf_binding_patch_copy(
	OpenAPI_pcf_binding_patch_t *dst, OpenAPI_pcf_binding_patch_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
