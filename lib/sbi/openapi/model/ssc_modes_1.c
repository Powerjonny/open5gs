...
OpenAPI_ssc_modes_1_t *OpenAPI_ssc_modes_1_create(
	...
	OpenAPI_ssc_modes_1_t *ssc_modes_1_local_var =
		ogs_malloc(sizeof(OpenAPI_ssc_modes_1_t));
...
cJSON *OpenAPI_ssc_modes_1_convertToJSON(OpenAPI_ssc_modes_1_t *ssc_modes_1) {
	...
	if(ssc_modes_1 == NULL) {
		ogs_error(
			"OpenAPI_ssc_modes_1_convertToJSON() failed [SscModes_1]");
		return NULL;
	}
	...
	if(ssc_modes_1->default_ssc_mode == OpenAPI_ssc_mode_NULL) {
		ogs_error(
			"OpenAPI_ssc_modes_1_convertToJSON() failed [default_ssc_mode]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "defaultSscMode",
	                           OpenAPI_ssc_mode_ToString(
					   ssc_modes_1->default_ssc_mode)) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_ssc_modes_1_convertToJSON() failed [default_ssc_mode]");
	...
	if(ssc_modes_1->allowed_ssc_modes != OpenAPI_ssc_mode_NULL) {
		cJSON *allowed_ssc_modesList = cJSON_AddArrayToObject(item,
		                                                      "allowedSscModes");
		if(allowed_ssc_modesList == NULL) {
			ogs_error(
				"OpenAPI_ssc_modes_1_convertToJSON() failed [allowed_ssc_modes]");
			goto end;
		}
		OpenAPI_list_for_each(ssc_modes_1->allowed_ssc_modes, node) {
			if(cJSON_AddStringToObject(allowed_ssc_modesList, "",
			                           OpenAPI_ssc_mode_ToString(
							   (intptr_t) node->data))
			   == NULL)
			{
...
OpenAPI_ssc_modes_1_t *OpenAPI_ssc_modes_1_parseFromJSON(
	...
	default_ssc_mode = cJSON_GetObjectItemCaseSensitive(ssc_modes_1JSON,
	                                                    "defaultSscMode");
	if(!default_ssc_mode) {
		ogs_error(
			"OpenAPI_ssc_modes_1_parseFromJSON() failed [default_ssc_mode]");
		goto end;
	}
	...
	default_ssc_modeVariable =
		OpenAPI_ssc_mode_FromString(default_ssc_mode->valuestring);

	allowed_ssc_modes = cJSON_GetObjectItemCaseSensitive(ssc_modes_1JSON,
	                                                     "allowedSscModes");
	if(allowed_ssc_modes) {
		cJSON *allowed_ssc_modes_local = NULL;
		if(!cJSON_IsArray(allowed_ssc_modes)) {
			ogs_error(
				"OpenAPI_ssc_modes_1_parseFromJSON() failed [allowed_ssc_modes]");
			goto end;
		}
		...
		cJSON_ArrayForEach(allowed_ssc_modes_local, allowed_ssc_modes) {
			...
			if(!localEnum) {
				ogs_info(
					"Enum value \"%s\" for field \"allowed_ssc_modes\" is not supported. Ignoring it ...",
					allowed_ssc_modes_local->valuestring);
			} else {
				OpenAPI_list_add(allowed_ssc_modesList,
				                 (void *) localEnum);
			}
		}
		if(allowed_ssc_modesList->count == 0) {
			ogs_error(
				"OpenAPI_ssc_modes_1_parseFromJSON() failed: Expected allowed_ssc_modesList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
