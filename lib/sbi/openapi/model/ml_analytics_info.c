...
OpenAPI_ml_analytics_info_t *OpenAPI_ml_analytics_info_create(
	...
	OpenAPI_ml_analytics_info_t *ml_analytics_info_local_var =
		ogs_malloc(sizeof(OpenAPI_ml_analytics_info_t));
...
void OpenAPI_ml_analytics_info_free(
	...
	if(ml_analytics_info->ml_analytics_ids) {
		OpenAPI_list_for_each(ml_analytics_info->ml_analytics_ids,
		                      node) {
	...
	if(ml_analytics_info->tracking_area_list) {
		OpenAPI_list_for_each(ml_analytics_info->tracking_area_list,
		                      node) {
...
cJSON *OpenAPI_ml_analytics_info_convertToJSON(
	...
	if(ml_analytics_info == NULL) {
		ogs_error(
			"OpenAPI_ml_analytics_info_convertToJSON() failed [MlAnalyticsInfo]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(ml_analytics_info->ml_analytics_ids) {
		cJSON *ml_analytics_idsList = cJSON_AddArrayToObject(item,
		                                                     "mlAnalyticsIds");
		if(ml_analytics_idsList == NULL) {
			ogs_error(
				"OpenAPI_ml_analytics_info_convertToJSON() failed [ml_analytics_ids]");
			goto end;
		}
		OpenAPI_list_for_each(ml_analytics_info->ml_analytics_ids,
		                      node) {
	...
	if(ml_analytics_info->snssai_list) {
		...
		if(snssai_listList == NULL) {
			ogs_error(
				"OpenAPI_ml_analytics_info_convertToJSON() failed [snssai_list]");
			goto end;
		}
		OpenAPI_list_for_each(ml_analytics_info->snssai_list, node) {
			cJSON *itemLocal =
	...
	if(ml_analytics_info->tracking_area_list) {
		...
		if(tracking_area_listList == NULL) {
			ogs_error(
				"OpenAPI_ml_analytics_info_convertToJSON() failed [tracking_area_list]");
			goto end;
		}
		OpenAPI_list_for_each(ml_analytics_info->tracking_area_list,
		                      node) {
...
OpenAPI_ml_analytics_info_t *OpenAPI_ml_analytics_info_parseFromJSON(
	...
	ml_analytics_ids =
		cJSON_GetObjectItemCaseSensitive(ml_analytics_infoJSON,
		                                 "mlAnalyticsIds");
	if(ml_analytics_ids) {
		cJSON *ml_analytics_ids_local = NULL;
		if(!cJSON_IsArray(ml_analytics_ids)) {
			ogs_error(
				"OpenAPI_ml_analytics_info_parseFromJSON() failed [ml_analytics_ids]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ml_analytics_ids_local, ml_analytics_ids) {
			...
			OpenAPI_nwdaf_event_t *ml_analytics_idsItem =
				OpenAPI_nwdaf_event_parseFromJSON(
					ml_analytics_ids_local);
			if(!ml_analytics_idsItem) {
				ogs_error("No ml_analytics_idsItem");
				goto end;
			}
			OpenAPI_list_add(ml_analytics_idsList,
			                 ml_analytics_idsItem);
		}
	}

	snssai_list = cJSON_GetObjectItemCaseSensitive(ml_analytics_infoJSON,
	                                               "snssaiList");
	if(snssai_list) {
		cJSON *snssai_list_local = NULL;
		if(!cJSON_IsArray(snssai_list)) {
			ogs_error(
				"OpenAPI_ml_analytics_info_parseFromJSON() failed [snssai_list]");
			goto end;
		}
	...
	if(tracking_area_list) {
		...
		if(!cJSON_IsArray(tracking_area_list)) {
			ogs_error(
				"OpenAPI_ml_analytics_info_parseFromJSON() failed [tracking_area_list]");
			goto end;
		}
