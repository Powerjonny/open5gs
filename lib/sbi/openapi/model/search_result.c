...
OpenAPI_search_result_t *OpenAPI_search_result_create(
	...
	nf_instances,
	char *search_id,
	bool is_num_nf_inst_complete, int num_nf_inst_complete,
	OpenAPI_preferred_search_t *preferred_search,
	char *nrf_supported_features, OpenAPI_list_t *nf_instance_list,
	bool is_altered_priority_ind, int altered_priority_ind,
	OpenAPI_no_profile_match_info_t *no_profile_match_info) {
	OpenAPI_search_result_t *search_result_local_var =
		ogs_malloc(sizeof(OpenAPI_search_result_t));
	...
	search_result_local_var->is_num_nf_inst_complete =
		is_num_nf_inst_complete;
	...
	search_result_local_var->nrf_supported_features =
		nrf_supported_features;
	search_result_local_var->nf_instance_list = nf_instance_list;
	search_result_local_var->is_altered_priority_ind =
...
void OpenAPI_search_result_free(OpenAPI_search_result_t *search_result) {
	...
	if(search_result->no_profile_match_info) {
		OpenAPI_no_profile_match_info_free(
			search_result->no_profile_match_info);
		search_result->no_profile_match_info = NULL;
	}
	ogs_free(search_result);
}

cJSON *OpenAPI_search_result_convertToJSON(
	OpenAPI_search_result_t *search_result) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(search_result == NULL) {
		ogs_error(
			"OpenAPI_search_result_convertToJSON() failed [SearchResult]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(search_result->is_validity_period) {
		if(cJSON_AddNumberToObject(item, "validityPeriod",
		                           search_result->validity_period) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_search_result_convertToJSON() failed [validity_period]");
			goto end;
		}
	}
	...
	if(!search_result->nf_instances) {
		ogs_error(
			"OpenAPI_search_result_convertToJSON() failed [nf_instances]");
		return NULL;
	}
	...
	if(search_result->search_id) {
		if(cJSON_AddStringToObject(item, "searchId",
		                           search_result->search_id) == NULL)
		{
			ogs_error(
				"OpenAPI_search_result_convertToJSON() failed [search_id]");
			goto end;
		}
	}

	if(search_result->is_num_nf_inst_complete) {
		if(cJSON_AddNumberToObject(item, "numNfInstComplete",
		                           search_result->num_nf_inst_complete)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_search_result_convertToJSON() failed [num_nf_inst_complete]");
			goto end;
		}
	}
	...
	if(search_result->preferred_search) {
		...
		if(preferred_search_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_search_result_convertToJSON() failed [preferred_search]");
			goto end;
		}
		cJSON_AddItemToObject(item, "preferredSearch",
		                      preferred_search_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_search_result_convertToJSON() failed [preferred_search]");
			goto end;
	...
	if(search_result->nrf_supported_features) {
		if(cJSON_AddStringToObject(item, "nrfSupportedFeatures",
		                           search_result->nrf_supported_features)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_search_result_convertToJSON() failed [nrf_supported_features]");
			goto end;
		}
	}

	if(search_result->nf_instance_list) {
		cJSON *nf_instance_list = cJSON_AddObjectToObject(item,
		                                                  "nfInstanceList");
		if(nf_instance_list == NULL) {
			ogs_error(
				"OpenAPI_search_result_convertToJSON() failed [nf_instance_list]");
			goto end;
		}
		...
		if(search_result->nf_instance_list) {
			OpenAPI_list_for_each(search_result->nf_instance_list,
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_nf_instance_info_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_search_result_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(search_result->is_altered_priority_ind) {
		if(cJSON_AddBoolToObject(item, "alteredPriorityInd",
		                         search_result->altered_priority_ind) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_search_result_convertToJSON() failed [altered_priority_ind]");
			goto end;
		}
	}
	...
	if(search_result->no_profile_match_info) {
		cJSON *no_profile_match_info_local_JSON =
			OpenAPI_no_profile_match_info_convertToJSON(
				search_result->no_profile_match_info);
		if(no_profile_match_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_search_result_convertToJSON() failed [no_profile_match_info]");
			goto end;
		}
...
OpenAPI_search_result_t *OpenAPI_search_result_parseFromJSON(
	cJSON *search_resultJSON) {
	...
	OpenAPI_no_profile_match_info_t *no_profile_match_info_local_nonprim =
		NULL;
	validity_period = cJSON_GetObjectItemCaseSensitive(search_resultJSON,
	                                                   "validityPeriod");
	if(validity_period) {
		if(!cJSON_IsNumber(validity_period)) {
			ogs_error(
				"OpenAPI_search_result_parseFromJSON() failed [validity_period]");
			goto end;
		}
	}

	nf_instances = cJSON_GetObjectItemCaseSensitive(search_resultJSON,
	                                                "nfInstances");
	if(!nf_instances) {
		ogs_error(
			"OpenAPI_search_result_parseFromJSON() failed [nf_instances]");
		goto end;
	}
	...
	cJSON_ArrayForEach(nf_instances_local, nf_instances) {
		...
		OpenAPI_nf_profile_t *nf_instancesItem =
			OpenAPI_nf_profile_parseFromJSON(nf_instances_local);
		if(!nf_instancesItem) {
	...
	search_id = cJSON_GetObjectItemCaseSensitive(search_resultJSON,
	                                             "searchId");
	if(search_id) {
		if(!cJSON_IsString(search_id) &&
		   !cJSON_IsNull(search_id))
		{
			ogs_error(
				"OpenAPI_search_result_parseFromJSON() failed [search_id]");
			goto end;
		}
	}

	num_nf_inst_complete =
		cJSON_GetObjectItemCaseSensitive(search_resultJSON,
		                                 "numNfInstComplete");
	if(num_nf_inst_complete) {
		if(!cJSON_IsNumber(num_nf_inst_complete)) {
			ogs_error(
				"OpenAPI_search_result_parseFromJSON() failed [num_nf_inst_complete]");
			goto end;
		}
	}

	preferred_search = cJSON_GetObjectItemCaseSensitive(search_resultJSON,
	                                                    "preferredSearch");
	if(preferred_search) {
		preferred_search_local_nonprim =
			OpenAPI_preferred_search_parseFromJSON(
				preferred_search);
		if(!preferred_search_local_nonprim) {
			ogs_error(
				"OpenAPI_preferred_search_parseFromJSON failed [preferred_search]");
			goto end;
		}
	}
	...
	if(nrf_supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_search_result_parseFromJSON() failed [nrf_supported_features]");
			goto end;
		}
	}

	nf_instance_list = cJSON_GetObjectItemCaseSensitive(search_resultJSON,
	                                                    "nfInstanceList");
	if(nf_instance_list) {
		cJSON *nf_instance_list_local_map = NULL;
		if(!cJSON_IsObject(nf_instance_list) &&
		   !cJSON_IsNull(nf_instance_list))
		{
			ogs_error(
				"OpenAPI_search_result_parseFromJSON() failed [nf_instance_list]");
			goto end;
		}
		if(cJSON_IsObject(nf_instance_list)) {
			nf_instance_listList = OpenAPI_list_create();
			OpenAPI_map_t *localMapKeyPair = NULL;
			cJSON_ArrayForEach(nf_instance_list_local_map,
			                   nf_instance_list) {
				cJSON *localMapObject =
					nf_instance_list_local_map;
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_search_result_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	altered_priority_ind =
		cJSON_GetObjectItemCaseSensitive(search_resultJSON,
		                                 "alteredPriorityInd");
	if(altered_priority_ind) {
		if(!cJSON_IsBool(altered_priority_ind)) {
			ogs_error(
				"OpenAPI_search_result_parseFromJSON() failed [altered_priority_ind]");
			goto end;
		}
	}
	...
	if(no_profile_match_info) {
		no_profile_match_info_local_nonprim =
			OpenAPI_no_profile_match_info_parseFromJSON(
				no_profile_match_info);
		if(!no_profile_match_info_local_nonprim) {
			ogs_error(
				"OpenAPI_no_profile_match_info_parseFromJSON failed [no_profile_match_info]");
			goto end;
		}
	}
	...
	search_result_local_var = OpenAPI_search_result_create(
		...
		nf_instancesList,
		search_id &&
		!cJSON_IsNull(search_id) ? ogs_strdup(search_id->valuestring) :
		NULL,
		...
		no_profile_match_info ? no_profile_match_info_local_nonprim :
		NULL
		);
...
OpenAPI_search_result_t *OpenAPI_search_result_copy(
	OpenAPI_search_result_t *dst, OpenAPI_search_result_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
