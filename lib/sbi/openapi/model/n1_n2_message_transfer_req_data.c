...
OpenAPI_n1_n2_message_transfer_req_data_create(
	...
	bool is_ma_accepted_ind, int ma_accepted_ind, bool is_ext_buf_support,
	int ext_buf_support, OpenAPI_access_type_e target_access, char *nf_id) {
	OpenAPI_n1_n2_message_transfer_req_data_t *
	        n1_n2_message_transfer_req_data_local_var =
		ogs_malloc(sizeof(OpenAPI_n1_n2_message_transfer_req_data_t));
...
void OpenAPI_n1_n2_message_transfer_req_data_free(
	OpenAPI_n1_n2_message_transfer_req_data_t *
	...
	if(n1_n2_message_transfer_req_data->n1_message_container) {
		OpenAPI_n1_message_container_free(
...
cJSON *OpenAPI_n1_n2_message_transfer_req_data_convertToJSON(
	OpenAPI_n1_n2_message_transfer_req_data_t *
	n1_n2_message_transfer_req_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(n1_n2_message_transfer_req_data == NULL) {
		ogs_error(
			"OpenAPI_n1_n2_message_transfer_req_data_convertToJSON() failed [N1N2MessageTransferReqData]");
		return NULL;
	}
	...
	if(n1_n2_message_transfer_req_data->n1_message_container) {
		...
		if(n1_message_container_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_convertToJSON() failed [n1_message_container]");
			goto end;
		}
	...
	if(n1_n2_message_transfer_req_data->n2_info_container) {
		...
		if(n2_info_container_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_convertToJSON() failed [n2_info_container]");
			goto end;
		}
	...
	if(n1_n2_message_transfer_req_data->mt_data) {
		cJSON *mt_data_local_JSON =
			OpenAPI_ref_to_binary_data_convertToJSON(
				n1_n2_message_transfer_req_data->mt_data);
		if(mt_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_convertToJSON() failed [mt_data]");
			goto end;
		}
	...
	if(n1_n2_message_transfer_req_data->is_skip_ind) {
		...
		{
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_convertToJSON() failed [skip_ind]");
			goto end;
		}
	}
	...
	if(n1_n2_message_transfer_req_data->is_last_msg_indication) {
		...
		{
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_convertToJSON() failed [last_msg_indication]");
			goto end;
		}
	}
	...
	if(n1_n2_message_transfer_req_data->is_pdu_session_id) {
		...
		{
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_convertToJSON() failed [pdu_session_id]");
			goto end;
		}
	}
	...
	if(n1_n2_message_transfer_req_data->is_ppi) {
		...
		{
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_convertToJSON() failed [ppi]");
			goto end;
		}
	}
	...
	if(n1_n2_message_transfer_req_data->arp) {
		cJSON *arp_local_JSON =
			OpenAPI_arp_convertToJSON(
				n1_n2_message_transfer_req_data->arp);
		if(arp_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_convertToJSON() failed [arp]");
			goto end;
		}
	...
	if(n1_n2_message_transfer_req_data->is__5qi) {
		...
		{
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_convertToJSON() failed [_5qi]");
			goto end;
		}
	}
	...
	if(n1_n2_message_transfer_req_data->n1n2_failure_txf_notif_uri) {
		if(cJSON_AddStringToObject(item, "n1n2FailureTxfNotifURI",
		                           n1_n2_message_transfer_req_data->
		                           n1n2_failure_txf_notif_uri) == NULL)
		{
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_convertToJSON() failed [n1n2_failure_txf_notif_uri]");
			goto end;
		}
	}
	...
	if(n1_n2_message_transfer_req_data->is_smf_reallocation_ind) {
		if(cJSON_AddBoolToObject(item, "smfReallocationInd",
		                         n1_n2_message_transfer_req_data->
		                         smf_reallocation_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_convertToJSON() failed [smf_reallocation_ind]");
			goto end;
		}
	}
	...
	if(n1_n2_message_transfer_req_data->area_of_validity) {
		cJSON *area_of_validity_local_JSON =
			OpenAPI_area_of_validity_convertToJSON(
				n1_n2_message_transfer_req_data->
				area_of_validity);
		if(area_of_validity_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_convertToJSON() failed [area_of_validity]");
			goto end;
		}
	...
	if(n1_n2_message_transfer_req_data->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}

	if(n1_n2_message_transfer_req_data->old_guami) {
		cJSON *old_guami_local_JSON =
			OpenAPI_guami_convertToJSON(
				n1_n2_message_transfer_req_data->old_guami);
		if(old_guami_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_convertToJSON() failed [old_guami]");
			goto end;
		}
	...
	if(n1_n2_message_transfer_req_data->is_ma_accepted_ind) {
		...
		{
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_convertToJSON() failed [ma_accepted_ind]");
			goto end;
		}
	}
	...
	if(n1_n2_message_transfer_req_data->is_ext_buf_support) {
		...
		{
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_convertToJSON() failed [ext_buf_support]");
			goto end;
		}
	}
	   ...
	   OpenAPI_access_type_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_convertToJSON() failed [target_access]");
			goto end;
		}
	}
	...
	if(n1_n2_message_transfer_req_data->nf_id) {
		...
		{
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_convertToJSON() failed [nf_id]");
			goto end;
		}
	}
...
OpenAPI_n1_n2_message_transfer_req_data_parseFromJSON(
	...
	OpenAPI_n1_message_container_t *n1_message_container_local_nonprim =
		NULL;
	...
	if(n1_message_container) {
		n1_message_container_local_nonprim =
			OpenAPI_n1_message_container_parseFromJSON(
				n1_message_container);
		if(!n1_message_container_local_nonprim) {
			ogs_error(
				"OpenAPI_n1_message_container_parseFromJSON failed [n1_message_container]");
			goto end;
		}
	}
	...
	if(n2_info_container) {
		n2_info_container_local_nonprim =
			OpenAPI_n2_info_container_parseFromJSON(
				n2_info_container);
		if(!n2_info_container_local_nonprim) {
			ogs_error(
				"OpenAPI_n2_info_container_parseFromJSON failed [n2_info_container]");
			goto end;
		}
	}
	...
	if(mt_data) {
		mt_data_local_nonprim =
			OpenAPI_ref_to_binary_data_parseFromJSON(mt_data);
		if(!mt_data_local_nonprim) {
			ogs_error(
				"OpenAPI_ref_to_binary_data_parseFromJSON failed [mt_data]");
			goto end;
		}
	}
	...
	if(skip_ind) {
		if(!cJSON_IsBool(skip_ind)) {
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_parseFromJSON() failed [skip_ind]");
			goto end;
		}
	}
	...
	if(last_msg_indication) {
		if(!cJSON_IsBool(last_msg_indication)) {
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_parseFromJSON() failed [last_msg_indication]");
			goto end;
		}
	}
	...
	if(pdu_session_id) {
		if(!cJSON_IsNumber(pdu_session_id)) {
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_parseFromJSON() failed [pdu_session_id]");
			goto end;
		}
	}
	...
	if(ppi) {
		if(!cJSON_IsNumber(ppi)) {
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_parseFromJSON() failed [ppi]");
			goto end;
		}
	}
	...
	if(_5qi) {
		if(!cJSON_IsNumber(_5qi)) {
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_parseFromJSON() failed [_5qi]");
			goto end;
		}
	}
	...
	if(n1n2_failure_txf_notif_uri) {
		...
		{
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_parseFromJSON() failed [n1n2_failure_txf_notif_uri]");
			goto end;
		}
	}
	...
	if(smf_reallocation_ind) {
		if(!cJSON_IsBool(smf_reallocation_ind)) {
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_parseFromJSON() failed [smf_reallocation_ind]");
			goto end;
		}
	}
	...
	if(area_of_validity) {
		area_of_validity_local_nonprim =
			OpenAPI_area_of_validity_parseFromJSON(
				area_of_validity);
		if(!area_of_validity_local_nonprim) {
			ogs_error(
				"OpenAPI_area_of_validity_parseFromJSON failed [area_of_validity]");
			goto end;
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(old_guami) {
		old_guami_local_nonprim =
			OpenAPI_guami_parseFromJSON(old_guami);
		if(!old_guami_local_nonprim) {
			ogs_error(
				"OpenAPI_guami_parseFromJSON failed [old_guami]");
			goto end;
		}
	}
	...
	if(ma_accepted_ind) {
		if(!cJSON_IsBool(ma_accepted_ind)) {
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_parseFromJSON() failed [ma_accepted_ind]");
			goto end;
		}
	}
	...
	if(ext_buf_support) {
		if(!cJSON_IsBool(ext_buf_support)) {
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_parseFromJSON() failed [ext_buf_support]");
			goto end;
		}
	}
	...
	if(target_access) {
		if(!cJSON_IsString(target_access)) {
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_parseFromJSON() failed [target_access]");
			goto end;
		}
	...
	if(nf_id) {
		...
		{
			ogs_error(
				"OpenAPI_n1_n2_message_transfer_req_data_parseFromJSON() failed [nf_id]");
			goto end;
		}
	}
	...
	n1_n2_message_transfer_req_data_local_var =
		OpenAPI_n1_n2_message_transfer_req_data_create(
			...
			n1_message_container_local_nonprim :
			NULL,
			n2_info_container ? n2_info_container_local_nonprim :
			NULL,
			...
			n1n2_failure_txf_notif_uri &&
			!cJSON_IsNull(n1n2_failure_txf_notif_uri) ?
			ogs_strdup(n1n2_failure_txf_notif_uri->valuestring) :
			NULL,
			smf_reallocation_ind ? true : false,
			smf_reallocation_ind ? smf_reallocation_ind->valueint :
			0,
			area_of_validity ? area_of_validity_local_nonprim :
			NULL,
			supported_features &&
			!cJSON_IsNull(supported_features) ?
			...
			nf_id &&
			!cJSON_IsNull(nf_id) ? ogs_strdup(nf_id->valuestring) :
			NULL
			);
...
OpenAPI_n1_n2_message_transfer_req_data_t *
OpenAPI_n1_n2_message_transfer_req_data_copy(
	OpenAPI_n1_n2_message_transfer_req_data_t	*dst,
	OpenAPI_n1_n2_message_transfer_req_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_n1_n2_message_transfer_req_data_convertToJSON() failed");
		return NULL;
	}
