...
OpenAPI_svc_experience_t *OpenAPI_svc_experience_create(bool is_mos, float mos,
                                                        ...
                                                        bool is_lower_range,
                                                        float lower_range) {
	OpenAPI_svc_experience_t *svc_experience_local_var =
		ogs_malloc(sizeof(OpenAPI_svc_experience_t));
...
cJSON *OpenAPI_svc_experience_convertToJSON(
	...
	if(svc_experience == NULL) {
		ogs_error(
			"OpenAPI_svc_experience_convertToJSON() failed [SvcExperience]");
		return NULL;
	}
	...
	if(svc_experience->is_mos) {
		...
		{
			ogs_error(
				"OpenAPI_svc_experience_convertToJSON() failed [mos]");
			goto end;
		}
	}
	...
	if(svc_experience->is_upper_range) {
		...
		{
			ogs_error(
				"OpenAPI_svc_experience_convertToJSON() failed [upper_range]");
			goto end;
		}
	}
	...
	if(svc_experience->is_lower_range) {
		...
		{
			ogs_error(
				"OpenAPI_svc_experience_convertToJSON() failed [lower_range]");
			goto end;
		}
	}
...
OpenAPI_svc_experience_t *OpenAPI_svc_experience_parseFromJSON(
	...
	if(mos) {
		if(!cJSON_IsNumber(mos)) {
			ogs_error(
				"OpenAPI_svc_experience_parseFromJSON() failed [mos]");
			goto end;
		}
	}
	...
	if(upper_range) {
		if(!cJSON_IsNumber(upper_range)) {
			ogs_error(
				"OpenAPI_svc_experience_parseFromJSON() failed [upper_range]");
			goto end;
		}
	}
	...
	if(lower_range) {
		if(!cJSON_IsNumber(lower_range)) {
			ogs_error(
				"OpenAPI_svc_experience_parseFromJSON() failed [lower_range]");
			goto end;
		}
	}
