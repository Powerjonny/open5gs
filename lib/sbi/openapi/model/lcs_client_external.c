...
OpenAPI_lcs_client_external_t *OpenAPI_lcs_client_external_create(
	...
	OpenAPI_lcs_client_external_t *lcs_client_external_local_var =
		ogs_malloc(sizeof(OpenAPI_lcs_client_external_t));
	ogs_assert(lcs_client_external_local_var);

	lcs_client_external_local_var->allowed_geographic_area =
		allowed_geographic_area;
	lcs_client_external_local_var->privacy_check_related_action =
...
void OpenAPI_lcs_client_external_free(
	OpenAPI_lcs_client_external_t *lcs_client_external) {
	...
	if(lcs_client_external->allowed_geographic_area) {
		OpenAPI_list_for_each(
			lcs_client_external->allowed_geographic_area, node) {
...
cJSON *OpenAPI_lcs_client_external_convertToJSON(
	OpenAPI_lcs_client_external_t *lcs_client_external) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(lcs_client_external == NULL) {
		ogs_error(
			"OpenAPI_lcs_client_external_convertToJSON() failed [LcsClientExternal]");
		return NULL;
	}
	...
	if(lcs_client_external->allowed_geographic_area) {
		...
		if(allowed_geographic_areaList == NULL) {
			ogs_error(
				"OpenAPI_lcs_client_external_convertToJSON() failed [allowed_geographic_area]");
			goto end;
		}
		OpenAPI_list_for_each(
			lcs_client_external->allowed_geographic_area, node) {
	   ...
	   OpenAPI_privacy_check_related_action_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_lcs_client_external_convertToJSON() failed [privacy_check_related_action]");
			goto end;
		}
	}
	...
	if(lcs_client_external->valid_time_period) {
		...
		if(valid_time_period_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_lcs_client_external_convertToJSON() failed [valid_time_period]");
			goto end;
		}
...
OpenAPI_lcs_client_external_t *OpenAPI_lcs_client_external_parseFromJSON(
	cJSON *lcs_client_externalJSON) {
	...
	if(allowed_geographic_area) {
		...
		if(!cJSON_IsArray(allowed_geographic_area)) {
			ogs_error(
				"OpenAPI_lcs_client_external_parseFromJSON() failed [allowed_geographic_area]");
			goto end;
		}
	...
	if(privacy_check_related_action) {
		if(!cJSON_IsString(privacy_check_related_action)) {
			ogs_error(
				"OpenAPI_lcs_client_external_parseFromJSON() failed [privacy_check_related_action]");
			goto end;
		}
...
OpenAPI_lcs_client_external_t *OpenAPI_lcs_client_external_copy(
	OpenAPI_lcs_client_external_t	*dst,
	OpenAPI_lcs_client_external_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
