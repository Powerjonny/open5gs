...
OpenAPI_redundant_transmission_exp_req_create(
	...
	OpenAPI_redundant_transmission_exp_req_t *
		...
		ogs_malloc(sizeof(OpenAPI_redundant_transmission_exp_req_t));
	...
	OpenAPI_redundant_transmission_exp_req_t *redundant_transmission_exp_req)
{
	...
	if(redundant_transmission_exp_req == NULL) {
		ogs_error(
			"OpenAPI_redundant_transmission_exp_req_convertToJSON() failed [RedundantTransmissionExpReq]");
		return NULL;
	}
	...
	if(redundant_transmission_exp_req->red_t_order_criter) {
		...
		if(red_t_order_criter_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_redundant_transmission_exp_req_convertToJSON() failed [red_t_order_criter]");
			goto end;
		}
	...
	if(redundant_transmission_exp_req->order) {
		...
		if(order_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_redundant_transmission_exp_req_convertToJSON() failed [order]");
			goto end;
		}
...
OpenAPI_redundant_transmission_exp_req_parseFromJSON(
	...
	if(red_t_order_criter) {
		...
		if(!red_t_order_criter_local_nonprim) {
			ogs_error(
				"OpenAPI_red_trans_exp_ordering_criterion_parseFromJSON failed [red_t_order_criter]");
			goto end;
		}
	}
	...
	redundant_transmission_exp_req_local_var =
		OpenAPI_redundant_transmission_exp_req_create(
			red_t_order_criter ? red_t_order_criter_local_nonprim :
			NULL,
...
OpenAPI_redundant_transmission_exp_req_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_redundant_transmission_exp_req_convertToJSON() failed");
		return NULL;
	}
