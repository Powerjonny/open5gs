...
OpenAPI_plmn_oauth2_t *OpenAPI_plmn_oauth2_create(
	...
	OpenAPI_plmn_oauth2_t *plmn_oauth2_local_var =
		ogs_malloc(sizeof(OpenAPI_plmn_oauth2_t));
	ogs_assert(plmn_oauth2_local_var);

	plmn_oauth2_local_var->oauth2_required_plmn_id_list =
		oauth2_required_plmn_id_list;
	plmn_oauth2_local_var->oauth2_not_required_plmn_id_list =
		oauth2_not_required_plmn_id_list;
...
void OpenAPI_plmn_oauth2_free(OpenAPI_plmn_oauth2_t *plmn_oauth2) {
	...
	if(plmn_oauth2->oauth2_required_plmn_id_list) {
		OpenAPI_list_for_each(plmn_oauth2->oauth2_required_plmn_id_list,
		                      node) {
	...
	if(plmn_oauth2->oauth2_not_required_plmn_id_list) {
		OpenAPI_list_for_each(
			plmn_oauth2->oauth2_not_required_plmn_id_list, node) {
			OpenAPI_plmn_id_free(node->data);
		}
		OpenAPI_list_free(
			plmn_oauth2->oauth2_not_required_plmn_id_list);
...
cJSON *OpenAPI_plmn_oauth2_convertToJSON(OpenAPI_plmn_oauth2_t *plmn_oauth2) {
	...
	if(plmn_oauth2 == NULL) {
		ogs_error(
			"OpenAPI_plmn_oauth2_convertToJSON() failed [PlmnOauth2]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(plmn_oauth2->oauth2_required_plmn_id_list) {
		cJSON *oauth2_required_plmn_id_listList =
			cJSON_AddArrayToObject(item,
			                       "oauth2RequiredPlmnIdList");
		if(oauth2_required_plmn_id_listList == NULL) {
			ogs_error(
				"OpenAPI_plmn_oauth2_convertToJSON() failed [oauth2_required_plmn_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(plmn_oauth2->oauth2_required_plmn_id_list,
		                      node) {
			...
			cJSON_AddItemToArray(oauth2_required_plmn_id_listList,
			                     itemLocal);
		}
	}

	if(plmn_oauth2->oauth2_not_required_plmn_id_list) {
		cJSON *oauth2_not_required_plmn_id_listList =
			cJSON_AddArrayToObject(item,
			                       "oauth2NotRequiredPlmnIdList");
		if(oauth2_not_required_plmn_id_listList == NULL) {
			ogs_error(
				"OpenAPI_plmn_oauth2_convertToJSON() failed [oauth2_not_required_plmn_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			plmn_oauth2->oauth2_not_required_plmn_id_list, node) {
...
OpenAPI_plmn_oauth2_t *OpenAPI_plmn_oauth2_parseFromJSON(
	...
	if(oauth2_required_plmn_id_list) {
		...
		if(!cJSON_IsArray(oauth2_required_plmn_id_list)) {
			ogs_error(
				"OpenAPI_plmn_oauth2_parseFromJSON() failed [oauth2_required_plmn_id_list]");
			goto end;
		}

		oauth2_required_plmn_id_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(oauth2_required_plmn_id_list_local,
		                   oauth2_required_plmn_id_list) {
			if(!cJSON_IsObject(
				   oauth2_required_plmn_id_list_local))
			{
				ogs_error(
					"OpenAPI_plmn_oauth2_parseFromJSON() failed [oauth2_required_plmn_id_list]");
				goto end;
			...
			if(!oauth2_required_plmn_id_listItem) {
				ogs_error(
					"No oauth2_required_plmn_id_listItem");
				goto end;
			}
			OpenAPI_list_add(oauth2_required_plmn_id_listList,
			                 oauth2_required_plmn_id_listItem);
		}
	}
	...
	if(oauth2_not_required_plmn_id_list) {
		...
		if(!cJSON_IsArray(oauth2_not_required_plmn_id_list)) {
			ogs_error(
				"OpenAPI_plmn_oauth2_parseFromJSON() failed [oauth2_not_required_plmn_id_list]");
			goto end;
		}

		oauth2_not_required_plmn_id_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(oauth2_not_required_plmn_id_list_local,
		                   oauth2_not_required_plmn_id_list) {
			if(!cJSON_IsObject(
				   oauth2_not_required_plmn_id_list_local))
			{
				ogs_error(
					"OpenAPI_plmn_oauth2_parseFromJSON() failed [oauth2_not_required_plmn_id_list]");
				goto end;
			}
			OpenAPI_plmn_id_t *oauth2_not_required_plmn_id_listItem
			        = OpenAPI_plmn_id_parseFromJSON(
					  oauth2_not_required_plmn_id_list_local);
			if(!oauth2_not_required_plmn_id_listItem) {
				ogs_error(
					"No oauth2_not_required_plmn_id_listItem");
				goto end;
			}
			OpenAPI_list_add(oauth2_not_required_plmn_id_listList,
			                 oauth2_not_required_plmn_id_listItem);
		}
	}
	...
	if(oauth2_not_required_plmn_id_listList) {
		OpenAPI_list_for_each(oauth2_not_required_plmn_id_listList,
		                      node) {
			OpenAPI_plmn_id_free(node->data);
		}
