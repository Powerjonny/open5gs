...
OpenAPI_scp_info_t *OpenAPI_scp_info_create(
	...
	OpenAPI_scp_info_t *scp_info_local_var =
		ogs_malloc(sizeof(OpenAPI_scp_info_t));
...
cJSON *OpenAPI_scp_info_convertToJSON(OpenAPI_scp_info_t *scp_info) {
	...
	if(scp_info->scp_domain_info_list) {
		...
		if(scp_domain_info_list == NULL) {
			ogs_error(
				"OpenAPI_scp_info_convertToJSON() failed [scp_domain_info_list]");
			goto end;
		}
		cJSON *localMapObject = scp_domain_info_list;
		if(scp_info->scp_domain_info_list) {
			OpenAPI_list_for_each(scp_info->scp_domain_info_list,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_scp_info_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(scp_info->scp_prefix) {
		...
		{
			ogs_error(
				"OpenAPI_scp_info_convertToJSON() failed [scp_prefix]");
			goto end;
		}
	}
	...
	if(scp_info->scp_ports) {
		...
		if(scp_ports == NULL) {
			ogs_error(
				"OpenAPI_scp_info_convertToJSON() failed [scp_ports]");
			goto end;
		}
	...
	if(scp_info->address_domains) {
		...
		if(address_domainsList == NULL) {
			ogs_error(
				"OpenAPI_scp_info_convertToJSON() failed [address_domains]");
			goto end;
		}
		OpenAPI_list_for_each(scp_info->address_domains, node) {
	...
	if(scp_info->ipv4_addresses) {
		...
		if(ipv4_addressesList == NULL) {
			ogs_error(
				"OpenAPI_scp_info_convertToJSON() failed [ipv4_addresses]");
			goto end;
		}
		OpenAPI_list_for_each(scp_info->ipv4_addresses, node) {
	...
	if(scp_info->ipv6_prefixes) {
		...
		if(ipv6_prefixesList == NULL) {
			ogs_error(
				"OpenAPI_scp_info_convertToJSON() failed [ipv6_prefixes]");
			goto end;
		}
		OpenAPI_list_for_each(scp_info->ipv6_prefixes, node) {
	...
	if(scp_info->ipv4_addr_ranges) {
		...
		if(ipv4_addr_rangesList == NULL) {
			ogs_error(
				"OpenAPI_scp_info_convertToJSON() failed [ipv4_addr_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(scp_info->ipv4_addr_ranges, node) {
			cJSON *itemLocal =
	...
	if(scp_info->ipv6_prefix_ranges) {
		...
		if(ipv6_prefix_rangesList == NULL) {
			ogs_error(
				"OpenAPI_scp_info_convertToJSON() failed [ipv6_prefix_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(scp_info->ipv6_prefix_ranges, node) {
			cJSON *itemLocal =
	...
	if(scp_info->served_nf_set_id_list) {
		...
		if(served_nf_set_id_listList == NULL) {
			ogs_error(
				"OpenAPI_scp_info_convertToJSON() failed [served_nf_set_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(scp_info->served_nf_set_id_list, node) {
	...
	if(scp_info->remote_plmn_list) {
		...
		if(remote_plmn_listList == NULL) {
			ogs_error(
				"OpenAPI_scp_info_convertToJSON() failed [remote_plmn_list]");
			goto end;
		}
		OpenAPI_list_for_each(scp_info->remote_plmn_list, node) {
			cJSON *itemLocal =
	...
	if(scp_info->remote_snpn_list) {
		...
		if(remote_snpn_listList == NULL) {
			ogs_error(
				"OpenAPI_scp_info_convertToJSON() failed [remote_snpn_list]");
			goto end;
		}
		OpenAPI_list_for_each(scp_info->remote_snpn_list, node) {
			cJSON *itemLocal =
	...
	if(scp_info->ip_reachability != OpenAPI_ip_reachability_NULL) {
		if(cJSON_AddStringToObject(item, "ipReachability",
						   ...
						   scp_info->ip_reachability))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_scp_info_convertToJSON() failed [ip_reachability]");
			goto end;
		}
	}
	...
	if(scp_info->scp_capabilities != OpenAPI_scp_capability_NULL) {
		...
		if(scp_capabilitiesList == NULL) {
			ogs_error(
				"OpenAPI_scp_info_convertToJSON() failed [scp_capabilities]");
			goto end;
		}
...
OpenAPI_scp_info_t *OpenAPI_scp_info_parseFromJSON(cJSON *scp_infoJSON) {
	...
	scp_domain_info_list = cJSON_GetObjectItemCaseSensitive(scp_infoJSON,
	                                                        "scpDomainInfoList");
	if(scp_domain_info_list) {
		...
		{
			ogs_error(
				"OpenAPI_scp_info_parseFromJSON() failed [scp_domain_info_list]");
			goto end;
		}
		if(cJSON_IsObject(scp_domain_info_list)) {
			...
			cJSON_ArrayForEach(scp_domain_info_list_local_map,
				...
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_scp_info_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	scp_prefix = cJSON_GetObjectItemCaseSensitive(scp_infoJSON,
	                                              "scpPrefix");
	if(scp_prefix) {
		...
		{
			ogs_error(
				"OpenAPI_scp_info_parseFromJSON() failed [scp_prefix]");
			goto end;
		}
	}
	...
	if(scp_ports) {
		...
		{
			ogs_error(
				"OpenAPI_scp_info_parseFromJSON() failed [scp_ports]");
			goto end;
		}
		if(cJSON_IsObject(scp_ports)) {
	...
	address_domains = cJSON_GetObjectItemCaseSensitive(scp_infoJSON,
	                                                   "addressDomains");
	if(address_domains) {
		cJSON *address_domains_local = NULL;
		if(!cJSON_IsArray(address_domains)) {
			ogs_error(
				"OpenAPI_scp_info_parseFromJSON() failed [address_domains]");
			goto end;
		}
	...
	ipv4_addresses = cJSON_GetObjectItemCaseSensitive(scp_infoJSON,
	                                                  "ipv4Addresses");
	if(ipv4_addresses) {
		cJSON *ipv4_addresses_local = NULL;
		if(!cJSON_IsArray(ipv4_addresses)) {
			ogs_error(
				"OpenAPI_scp_info_parseFromJSON() failed [ipv4_addresses]");
			goto end;
		}
	...
	ipv6_prefixes = cJSON_GetObjectItemCaseSensitive(scp_infoJSON,
	                                                 "ipv6Prefixes");
	if(ipv6_prefixes) {
		cJSON *ipv6_prefixes_local = NULL;
		if(!cJSON_IsArray(ipv6_prefixes)) {
			ogs_error(
				"OpenAPI_scp_info_parseFromJSON() failed [ipv6_prefixes]");
			goto end;
		}
	...
	ipv4_addr_ranges = cJSON_GetObjectItemCaseSensitive(scp_infoJSON,
	                                                    "ipv4AddrRanges");
	if(ipv4_addr_ranges) {
		cJSON *ipv4_addr_ranges_local = NULL;
		if(!cJSON_IsArray(ipv4_addr_ranges)) {
			ogs_error(
				"OpenAPI_scp_info_parseFromJSON() failed [ipv4_addr_ranges]");
			goto end;
		}
	...
	ipv6_prefix_ranges = cJSON_GetObjectItemCaseSensitive(scp_infoJSON,
	                                                      "ipv6PrefixRanges");
	if(ipv6_prefix_ranges) {
		cJSON *ipv6_prefix_ranges_local = NULL;
		if(!cJSON_IsArray(ipv6_prefix_ranges)) {
			ogs_error(
				"OpenAPI_scp_info_parseFromJSON() failed [ipv6_prefix_ranges]");
			goto end;
		}
	...
	served_nf_set_id_list = cJSON_GetObjectItemCaseSensitive(scp_infoJSON,
	                                                         "servedNfSetIdList");
	if(served_nf_set_id_list) {
		cJSON *served_nf_set_id_list_local = NULL;
		if(!cJSON_IsArray(served_nf_set_id_list)) {
			ogs_error(
				"OpenAPI_scp_info_parseFromJSON() failed [served_nf_set_id_list]");
			goto end;
		}
	...
	remote_plmn_list = cJSON_GetObjectItemCaseSensitive(scp_infoJSON,
	                                                    "remotePlmnList");
	if(remote_plmn_list) {
		cJSON *remote_plmn_list_local = NULL;
		if(!cJSON_IsArray(remote_plmn_list)) {
			ogs_error(
				"OpenAPI_scp_info_parseFromJSON() failed [remote_plmn_list]");
			goto end;
		}
	...
	remote_snpn_list = cJSON_GetObjectItemCaseSensitive(scp_infoJSON,
	                                                    "remoteSnpnList");
	if(remote_snpn_list) {
		cJSON *remote_snpn_list_local = NULL;
		if(!cJSON_IsArray(remote_snpn_list)) {
			ogs_error(
				"OpenAPI_scp_info_parseFromJSON() failed [remote_snpn_list]");
			goto end;
		}
	...
	ip_reachability = cJSON_GetObjectItemCaseSensitive(scp_infoJSON,
	                                                   "ipReachability");
	if(ip_reachability) {
		if(!cJSON_IsString(ip_reachability)) {
			ogs_error(
				"OpenAPI_scp_info_parseFromJSON() failed [ip_reachability]");
			goto end;
		}
	...
	scp_capabilities = cJSON_GetObjectItemCaseSensitive(scp_infoJSON,
	                                                    "scpCapabilities");
	if(scp_capabilities) {
		cJSON *scp_capabilities_local = NULL;
		if(!cJSON_IsArray(scp_capabilities)) {
			ogs_error(
				"OpenAPI_scp_info_parseFromJSON() failed [scp_capabilities]");
			goto end;
		}
		...
		if(scp_capabilitiesList->count == 0) {
			ogs_error(
				"OpenAPI_scp_info_parseFromJSON() failed: Expected scp_capabilitiesList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}
...
OpenAPI_scp_info_t *OpenAPI_scp_info_copy(OpenAPI_scp_info_t	*dst,
                                          OpenAPI_scp_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
