...
OpenAPI_v2x_capability_t *OpenAPI_v2x_capability_create(bool	is_lte_v2x,
                                                        ...
                                                        bool	is_nr_v2x,
                                                        int	nr_v2x) {
	OpenAPI_v2x_capability_t *v2x_capability_local_var =
		ogs_malloc(sizeof(OpenAPI_v2x_capability_t));
...
cJSON *OpenAPI_v2x_capability_convertToJSON(
	...
	if(v2x_capability == NULL) {
		ogs_error(
			"OpenAPI_v2x_capability_convertToJSON() failed [V2xCapability]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(v2x_capability->is_lte_v2x) {
		if(cJSON_AddBoolToObject(item, "lteV2x",
		                         v2x_capability->lte_v2x) == NULL)
		{
			ogs_error(
				"OpenAPI_v2x_capability_convertToJSON() failed [lte_v2x]");
			goto end;
		}
	}

	if(v2x_capability->is_nr_v2x) {
		if(cJSON_AddBoolToObject(item, "nrV2x",
		                         v2x_capability->nr_v2x) == NULL)
		{
			ogs_error(
				"OpenAPI_v2x_capability_convertToJSON() failed [nr_v2x]");
			goto end;
		}
	}
...
OpenAPI_v2x_capability_t *OpenAPI_v2x_capability_parseFromJSON(
	...
	cJSON *lte_v2x = NULL;
	cJSON *nr_v2x = NULL;
	lte_v2x = cJSON_GetObjectItemCaseSensitive(v2x_capabilityJSON,
	                                           "lteV2x");
	if(lte_v2x) {
		if(!cJSON_IsBool(lte_v2x)) {
			ogs_error(
				"OpenAPI_v2x_capability_parseFromJSON() failed [lte_v2x]");
			goto end;
		}
	}
	...
	if(nr_v2x) {
		if(!cJSON_IsBool(nr_v2x)) {
			ogs_error(
				"OpenAPI_v2x_capability_parseFromJSON() failed [nr_v2x]");
			goto end;
		}
	}
