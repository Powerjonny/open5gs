...
OpenAPI_slice_info_for_pdu_session_t *OpenAPI_slice_info_for_pdu_session_create(
	...
	OpenAPI_slice_info_for_pdu_session_t *
		...
		ogs_malloc(sizeof(OpenAPI_slice_info_for_pdu_session_t));
	ogs_assert(slice_info_for_pdu_session_local_var);

	slice_info_for_pdu_session_local_var->s_nssai = s_nssai;
	slice_info_for_pdu_session_local_var->roaming_indication =
		roaming_indication;
...
cJSON *OpenAPI_slice_info_for_pdu_session_convertToJSON(
	...
	if(slice_info_for_pdu_session == NULL) {
		ogs_error(
			"OpenAPI_slice_info_for_pdu_session_convertToJSON() failed [SliceInfoForPDUSession]");
		return NULL;
	}
	...
	if(!slice_info_for_pdu_session->s_nssai) {
		ogs_error(
			"OpenAPI_slice_info_for_pdu_session_convertToJSON() failed [s_nssai]");
		return NULL;
	}
	...
	if(slice_info_for_pdu_session->roaming_indication ==
	   OpenAPI_roaming_indication_NULL)
	{
		ogs_error(
			"OpenAPI_slice_info_for_pdu_session_convertToJSON() failed [roaming_indication]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "roamingIndication",
	                           OpenAPI_roaming_indication_ToString(
					   slice_info_for_pdu_session->
					   roaming_indication)) == NULL)
	{
		ogs_error(
			"OpenAPI_slice_info_for_pdu_session_convertToJSON() failed [roaming_indication]");
		goto end;
	...
	if(slice_info_for_pdu_session->home_snssai) {
		...
		if(home_snssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_slice_info_for_pdu_session_convertToJSON() failed [home_snssai]");
			goto end;
		}
		cJSON_AddItemToObject(item, "homeSnssai",
		                      home_snssai_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_slice_info_for_pdu_session_convertToJSON() failed [home_snssai]");
			goto end;
...
OpenAPI_slice_info_for_pdu_session_parseFromJSON(
	...
	OpenAPI_slice_info_for_pdu_session_t *
	        slice_info_for_pdu_session_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *s_nssai = NULL;
	...
	s_nssai =
		cJSON_GetObjectItemCaseSensitive(slice_info_for_pdu_sessionJSON,
		                                 "sNssai");
	if(!s_nssai) {
		ogs_error(
			"OpenAPI_slice_info_for_pdu_session_parseFromJSON() failed [s_nssai]");
		goto end;
	}
	...
	if(!roaming_indication) {
		ogs_error(
			"OpenAPI_slice_info_for_pdu_session_parseFromJSON() failed [roaming_indication]");
		goto end;
	}
	...
	roaming_indicationVariable =
		OpenAPI_roaming_indication_FromString(
			roaming_indication->valuestring);
	...
	if(home_snssai) {
		home_snssai_local_nonprim =
			OpenAPI_snssai_parseFromJSON(home_snssai);
		if(!home_snssai_local_nonprim) {
			ogs_error(
				"OpenAPI_snssai_parseFromJSON failed [home_snssai]");
			goto end;
		}
	}

	slice_info_for_pdu_session_local_var =
		OpenAPI_slice_info_for_pdu_session_create(
...
OpenAPI_slice_info_for_pdu_session_t *OpenAPI_slice_info_for_pdu_session_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_slice_info_for_pdu_session_convertToJSON() failed");
		return NULL;
	}
