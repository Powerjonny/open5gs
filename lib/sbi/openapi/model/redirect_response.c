...
OpenAPI_redirect_response_t *OpenAPI_redirect_response_create(char *cause,
                                                              char	*
                                                              target_scp,
                                                              char	*
                                                              target_sepp) {
	OpenAPI_redirect_response_t *redirect_response_local_var =
		ogs_malloc(sizeof(OpenAPI_redirect_response_t));
...
void OpenAPI_redirect_response_free(
	OpenAPI_redirect_response_t *redirect_response) {
...
cJSON *OpenAPI_redirect_response_convertToJSON(
	OpenAPI_redirect_response_t *redirect_response) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(redirect_response == NULL) {
		ogs_error(
			"OpenAPI_redirect_response_convertToJSON() failed [RedirectResponse]");
		return NULL;
	}
	...
	if(redirect_response->cause) {
		...
		{
			ogs_error(
				"OpenAPI_redirect_response_convertToJSON() failed [cause]");
			goto end;
		}
	}
	...
	if(redirect_response->target_scp) {
		...
		{
			ogs_error(
				"OpenAPI_redirect_response_convertToJSON() failed [target_scp]");
			goto end;
		}
	}
	...
	if(redirect_response->target_sepp) {
		...
		{
			ogs_error(
				"OpenAPI_redirect_response_convertToJSON() failed [target_sepp]");
			goto end;
		}
	}
...
OpenAPI_redirect_response_t *OpenAPI_redirect_response_parseFromJSON(
	...
	cause = cJSON_GetObjectItemCaseSensitive(redirect_responseJSON,
	                                         "cause");
	if(cause) {
		...
		{
			ogs_error(
				"OpenAPI_redirect_response_parseFromJSON() failed [cause]");
			goto end;
		}
	}
	...
	if(target_scp) {
		...
		{
			ogs_error(
				"OpenAPI_redirect_response_parseFromJSON() failed [target_scp]");
			goto end;
		}
	}
	...
	if(target_sepp) {
		...
		{
			ogs_error(
				"OpenAPI_redirect_response_parseFromJSON() failed [target_sepp]");
			goto end;
		}
	}
	...
	redirect_response_local_var = OpenAPI_redirect_response_create(
		...
		target_scp &&
		!cJSON_IsNull(target_scp) ?
		ogs_strdup(target_scp->valuestring) : NULL,
		target_sepp &&
		!cJSON_IsNull(target_sepp) ?
		ogs_strdup(target_sepp->valuestring) : NULL
		);
...
OpenAPI_redirect_response_t *OpenAPI_redirect_response_copy(
	OpenAPI_redirect_response_t *dst, OpenAPI_redirect_response_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
