...
OpenAPI_pws_response_data_t *OpenAPI_pws_response_data_create(
	int ngap_message_type, int serial_number, int message_identifier,
	OpenAPI_list_t *unknown_tai_list) {
	OpenAPI_pws_response_data_t *pws_response_data_local_var =
		ogs_malloc(sizeof(OpenAPI_pws_response_data_t));
...
void OpenAPI_pws_response_data_free(
	OpenAPI_pws_response_data_t *pws_response_data) {
	...
	if(pws_response_data->unknown_tai_list) {
		OpenAPI_list_for_each(pws_response_data->unknown_tai_list,
		                      node) {
...
cJSON *OpenAPI_pws_response_data_convertToJSON(
	OpenAPI_pws_response_data_t *pws_response_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(pws_response_data == NULL) {
		ogs_error(
			"OpenAPI_pws_response_data_convertToJSON() failed [PWSResponseData]");
		return NULL;
	}
	...
	{
		ogs_error(
			"OpenAPI_pws_response_data_convertToJSON() failed [ngap_message_type]");
		goto end;
	}
	...
	{
		ogs_error(
			"OpenAPI_pws_response_data_convertToJSON() failed [serial_number]");
		goto end;
	}
	...
	{
		ogs_error(
			"OpenAPI_pws_response_data_convertToJSON() failed [message_identifier]");
		goto end;
	}
	...
	if(pws_response_data->unknown_tai_list) {
		...
		if(unknown_tai_listList == NULL) {
			ogs_error(
				"OpenAPI_pws_response_data_convertToJSON() failed [unknown_tai_list]");
			goto end;
		}
		OpenAPI_list_for_each(pws_response_data->unknown_tai_list,
		                      node) {
...
OpenAPI_pws_response_data_t *OpenAPI_pws_response_data_parseFromJSON(
	...
	ngap_message_type =
		cJSON_GetObjectItemCaseSensitive(pws_response_dataJSON,
		                                 "ngapMessageType");
	if(!ngap_message_type) {
		ogs_error(
			"OpenAPI_pws_response_data_parseFromJSON() failed [ngap_message_type]");
		goto end;
	}
	...
	if(!serial_number) {
		ogs_error(
			"OpenAPI_pws_response_data_parseFromJSON() failed [serial_number]");
		goto end;
	}
	...
	if(!message_identifier) {
		ogs_error(
			"OpenAPI_pws_response_data_parseFromJSON() failed [message_identifier]");
		goto end;
	}
	...
	if(unknown_tai_list) {
		...
		cJSON_ArrayForEach(unknown_tai_list_local, unknown_tai_list) {
			...
			OpenAPI_tai_t *unknown_tai_listItem =
			...
			OpenAPI_list_add(unknown_tai_listList,
			                 unknown_tai_listItem);
		}
	}
...
OpenAPI_pws_response_data_t *OpenAPI_pws_response_data_copy(
	OpenAPI_pws_response_data_t *dst, OpenAPI_pws_response_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
