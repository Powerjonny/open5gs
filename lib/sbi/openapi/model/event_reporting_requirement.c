...
OpenAPI_event_reporting_requirement_create(OpenAPI_accuracy_t *accuracy,
	...
	OpenAPI_event_reporting_requirement_t *
		...
		ogs_malloc(sizeof(OpenAPI_event_reporting_requirement_t));
...
void OpenAPI_event_reporting_requirement_free(
	OpenAPI_event_reporting_requirement_t *event_reporting_requirement) {
	...
	if(event_reporting_requirement->acc_per_subset) {
		OpenAPI_list_for_each(
			event_reporting_requirement->acc_per_subset, node) {
	...
	if(event_reporting_requirement->ana_meta) {
		OpenAPI_list_for_each(event_reporting_requirement->ana_meta,
		                      node) {
	...
	if(event_reporting_requirement->ana_meta_ind) {
		OpenAPI_analytics_metadata_indication_free(
...
cJSON *OpenAPI_event_reporting_requirement_convertToJSON(
	OpenAPI_event_reporting_requirement_t *event_reporting_requirement) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(event_reporting_requirement == NULL) {
		ogs_error(
			"OpenAPI_event_reporting_requirement_convertToJSON() failed [EventReportingRequirement]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(event_reporting_requirement->accuracy) {
		cJSON *accuracy_local_JSON =
			OpenAPI_accuracy_convertToJSON(
				event_reporting_requirement->accuracy);
		if(accuracy_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_event_reporting_requirement_convertToJSON() failed [accuracy]");
			goto end;
		}
	...
	if(event_reporting_requirement->acc_per_subset) {
		...
		if(acc_per_subsetList == NULL) {
			ogs_error(
				"OpenAPI_event_reporting_requirement_convertToJSON() failed [acc_per_subset]");
			goto end;
		}
		OpenAPI_list_for_each(
			event_reporting_requirement->acc_per_subset, node) {
			cJSON *itemLocal =
				OpenAPI_accuracy_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_event_reporting_requirement_convertToJSON() failed [acc_per_subset]");
				goto end;
	...
	if(event_reporting_requirement->start_ts) {
		...
		{
			ogs_error(
				"OpenAPI_event_reporting_requirement_convertToJSON() failed [start_ts]");
			goto end;
		}
	}
	...
	if(event_reporting_requirement->end_ts) {
		...
		{
			ogs_error(
				"OpenAPI_event_reporting_requirement_convertToJSON() failed [end_ts]");
			goto end;
		}
	}
	...
	if(event_reporting_requirement->is_offset_period) {
		...
		{
			ogs_error(
				"OpenAPI_event_reporting_requirement_convertToJSON() failed [offset_period]");
			goto end;
		}
	}
	...
	if(event_reporting_requirement->is_samp_ratio) {
		...
		{
			ogs_error(
				"OpenAPI_event_reporting_requirement_convertToJSON() failed [samp_ratio]");
			goto end;
		}
	}
	...
	if(event_reporting_requirement->is_max_object_nbr) {
		...
		{
			ogs_error(
				"OpenAPI_event_reporting_requirement_convertToJSON() failed [max_object_nbr]");
			goto end;
		}
	}
	...
	if(event_reporting_requirement->is_max_supi_nbr) {
		...
		{
			ogs_error(
				"OpenAPI_event_reporting_requirement_convertToJSON() failed [max_supi_nbr]");
			goto end;
		}
	}
	...
	if(event_reporting_requirement->time_ana_needed) {
		...
		{
			ogs_error(
				"OpenAPI_event_reporting_requirement_convertToJSON() failed [time_ana_needed]");
			goto end;
		}
	}
	...
	if(event_reporting_requirement->ana_meta) {
		...
		if(ana_metaList == NULL) {
			ogs_error(
				"OpenAPI_event_reporting_requirement_convertToJSON() failed [ana_meta]");
			goto end;
		}
		OpenAPI_list_for_each(event_reporting_requirement->ana_meta,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_analytics_metadata_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_event_reporting_requirement_convertToJSON() failed [ana_meta]");
	...
	if(event_reporting_requirement->ana_meta_ind) {
		cJSON *ana_meta_ind_local_JSON =
			OpenAPI_analytics_metadata_indication_convertToJSON(
				event_reporting_requirement->ana_meta_ind);
		if(ana_meta_ind_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_event_reporting_requirement_convertToJSON() failed [ana_meta_ind]");
			goto end;
		}
...
OpenAPI_event_reporting_requirement_parseFromJSON(
	...
	OpenAPI_event_reporting_requirement_t *
	        event_reporting_requirement_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *accuracy = NULL;
	...
	OpenAPI_analytics_metadata_indication_t *ana_meta_ind_local_nonprim =
		NULL;
	accuracy =
		cJSON_GetObjectItemCaseSensitive(
			event_reporting_requirementJSON, "accuracy");
	if(accuracy) {
		accuracy_local_nonprim =
			OpenAPI_accuracy_parseFromJSON(accuracy);
		if(!accuracy_local_nonprim) {
			ogs_error(
				"OpenAPI_accuracy_parseFromJSON failed [accuracy]");
			goto end;
		}
	}
	...
	if(acc_per_subset) {
		...
		if(!cJSON_IsArray(acc_per_subset)) {
			ogs_error(
				"OpenAPI_event_reporting_requirement_parseFromJSON() failed [acc_per_subset]");
			goto end;
		}
		...
		cJSON_ArrayForEach(acc_per_subset_local, acc_per_subset) {
			...
			OpenAPI_accuracy_t *acc_per_subsetItem =
				OpenAPI_accuracy_parseFromJSON(
					acc_per_subset_local);
			if(!acc_per_subsetItem) {
				ogs_error("No acc_per_subsetItem");
				goto end;
			}
			OpenAPI_list_add(acc_per_subsetList,
			                 acc_per_subsetItem);
		}
	}
	...
	if(start_ts) {
		...
		{
			ogs_error(
				"OpenAPI_event_reporting_requirement_parseFromJSON() failed [start_ts]");
			goto end;
		}
	}
	...
	if(end_ts) {
		...
		{
			ogs_error(
				"OpenAPI_event_reporting_requirement_parseFromJSON() failed [end_ts]");
			goto end;
		}
	}
	...
	if(offset_period) {
		if(!cJSON_IsNumber(offset_period)) {
			ogs_error(
				"OpenAPI_event_reporting_requirement_parseFromJSON() failed [offset_period]");
			goto end;
		}
	}
	...
	if(samp_ratio) {
		if(!cJSON_IsNumber(samp_ratio)) {
			ogs_error(
				"OpenAPI_event_reporting_requirement_parseFromJSON() failed [samp_ratio]");
			goto end;
		}
	}
	...
	if(max_object_nbr) {
		if(!cJSON_IsNumber(max_object_nbr)) {
			ogs_error(
				"OpenAPI_event_reporting_requirement_parseFromJSON() failed [max_object_nbr]");
			goto end;
		}
	}
	...
	if(max_supi_nbr) {
		if(!cJSON_IsNumber(max_supi_nbr)) {
			ogs_error(
				"OpenAPI_event_reporting_requirement_parseFromJSON() failed [max_supi_nbr]");
			goto end;
		}
	}
	...
	if(time_ana_needed) {
		...
		{
			ogs_error(
				"OpenAPI_event_reporting_requirement_parseFromJSON() failed [time_ana_needed]");
			goto end;
		}
	}
	...
	if(ana_meta) {
		...
		if(!cJSON_IsArray(ana_meta)) {
			ogs_error(
				"OpenAPI_event_reporting_requirement_parseFromJSON() failed [ana_meta]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ana_meta_local, ana_meta) {
			...
			OpenAPI_analytics_metadata_t *ana_metaItem =
	...
	if(ana_meta_ind) {
		ana_meta_ind_local_nonprim =
			OpenAPI_analytics_metadata_indication_parseFromJSON(
				ana_meta_ind);
		if(!ana_meta_ind_local_nonprim) {
			ogs_error(
				"OpenAPI_analytics_metadata_indication_parseFromJSON failed [ana_meta_ind]");
			goto end;
		}
	}

	event_reporting_requirement_local_var =
		OpenAPI_event_reporting_requirement_create(
			accuracy ? accuracy_local_nonprim : NULL,
			acc_per_subset ? acc_per_subsetList : NULL,
			start_ts &&
			!cJSON_IsNull(start_ts) ?
			ogs_strdup(start_ts->valuestring) :
			NULL,
			end_ts &&
			!cJSON_IsNull(end_ts) ?
			ogs_strdup(end_ts->valuestring) : NULL,
			...
			time_ana_needed &&
			!cJSON_IsNull(time_ana_needed) ?
			ogs_strdup(time_ana_needed->valuestring) : NULL,
...
OpenAPI_event_reporting_requirement_t *OpenAPI_event_reporting_requirement_copy(
	OpenAPI_event_reporting_requirement_t	*dst,
	OpenAPI_event_reporting_requirement_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_event_reporting_requirement_convertToJSON() failed");
		return NULL;
	}
