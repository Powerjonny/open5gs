...
OpenAPI_configured_snssai_t *OpenAPI_configured_snssai_create(
	...
	OpenAPI_configured_snssai_t *configured_snssai_local_var =
		ogs_malloc(sizeof(OpenAPI_configured_snssai_t));
...
cJSON *OpenAPI_configured_snssai_convertToJSON(
	...
	if(configured_snssai == NULL) {
		ogs_error(
			"OpenAPI_configured_snssai_convertToJSON() failed [ConfiguredSnssai]");
		return NULL;
	}
	...
	if(!configured_snssai->configured_snssai) {
		ogs_error(
			"OpenAPI_configured_snssai_convertToJSON() failed [configured_snssai]");
		return NULL;
	}
	cJSON *configured_snssai_local_JSON =
	...
	if(configured_snssai->mapped_home_snssai) {
		...
		if(mapped_home_snssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_configured_snssai_convertToJSON() failed [mapped_home_snssai]");
			goto end;
		}
...
OpenAPI_configured_snssai_t *OpenAPI_configured_snssai_parseFromJSON(
	...
	configured_snssai =
		cJSON_GetObjectItemCaseSensitive(configured_snssaiJSON,
		                                 "configuredSnssai");
	if(!configured_snssai) {
		ogs_error(
			"OpenAPI_configured_snssai_parseFromJSON() failed [configured_snssai]");
		goto end;
	}
	configured_snssai_local_nonprim =
		OpenAPI_snssai_parseFromJSON(configured_snssai);
	if(!configured_snssai_local_nonprim) {
		ogs_error(
			"OpenAPI_snssai_parseFromJSON failed [configured_snssai]");
		goto end;
	}
