...
OpenAPI_ncgi_1_t *OpenAPI_ncgi_1_create(OpenAPI_plmn_id_1_t *plmn_id,
	...
	OpenAPI_ncgi_1_t *ncgi_1_local_var =
		ogs_malloc(sizeof(OpenAPI_ncgi_1_t));
...
cJSON *OpenAPI_ncgi_1_convertToJSON(OpenAPI_ncgi_1_t *ncgi_1) {
	...
	cJSON *plmn_id_local_JSON =
		OpenAPI_plmn_id_1_convertToJSON(ncgi_1->plmn_id);
	...
	if(ncgi_1->nid) {
		if(cJSON_AddStringToObject(item, "nid", ncgi_1->nid) == NULL) {
			ogs_error(
				"OpenAPI_ncgi_1_convertToJSON() failed [nid]");
			goto end;
		}
	}
...
OpenAPI_ncgi_1_t *OpenAPI_ncgi_1_parseFromJSON(cJSON *ncgi_1JSON) {
	...
	if(nid) {
		...
		{
			ogs_error(
				"OpenAPI_ncgi_1_parseFromJSON() failed [nid]");
			goto end;
		}
	}
