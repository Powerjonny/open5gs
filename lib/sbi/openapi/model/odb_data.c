...
OpenAPI_odb_data_t *OpenAPI_odb_data_create(OpenAPI_roaming_odb_e roaming_odb) {
	OpenAPI_odb_data_t *odb_data_local_var =
		ogs_malloc(sizeof(OpenAPI_odb_data_t));
...
cJSON *OpenAPI_odb_data_convertToJSON(OpenAPI_odb_data_t *odb_data) {
	...
	if(odb_data->roaming_odb != OpenAPI_roaming_odb_NULL) {
		if(cJSON_AddStringToObject(item, "roamingOdb",
		                           OpenAPI_roaming_odb_ToString(
						   odb_data->roaming_odb)) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_odb_data_convertToJSON() failed [roaming_odb]");
			goto end;
		}
	}
...
OpenAPI_odb_data_t *OpenAPI_odb_data_parseFromJSON(cJSON *odb_dataJSON) {
	...
	roaming_odb = cJSON_GetObjectItemCaseSensitive(odb_dataJSON,
	                                               "roamingOdb");
	if(roaming_odb) {
		if(!cJSON_IsString(roaming_odb)) {
			ogs_error(
				"OpenAPI_odb_data_parseFromJSON() failed [roaming_odb]");
			goto end;
		}
		roaming_odbVariable =
			OpenAPI_roaming_odb_FromString(
				roaming_odb->valuestring);
	}
...
OpenAPI_odb_data_t *OpenAPI_odb_data_copy(OpenAPI_odb_data_t	*dst,
                                          OpenAPI_odb_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
