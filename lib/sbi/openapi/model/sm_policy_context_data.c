...
OpenAPI_sm_policy_context_data_t *OpenAPI_sm_policy_context_data_create(
	...
	OpenAPI_acc_net_charging_address_t *charg_entity_addr, char *gpsi,
	char *supi, bool is_invalid_supi, int invalid_supi,
	OpenAPI_list_t *inter_grp_ids, int pdu_session_id,
	OpenAPI_pdu_session_type_e pdu_session_type,
	char *chargingcharacteristics, char *dnn,
	OpenAPI_dnn_selection_mode_e dnn_sel_mode, char *notification_uri,
	OpenAPI_access_type_e access_type, OpenAPI_rat_type_e rat_type,
	...
	char *pei, char *ipv4_address, char *ipv6_address_prefix,
	char *ip_domain, OpenAPI_ambr_t *subs_sess_ambr, char *auth_prof_index,
	OpenAPI_subscribed_default_qos_t *subs_def_qos,
	OpenAPI_vplmn_qos_t *vplmn_qos, bool is_num_of_pack_filter,
	int num_of_pack_filter, bool is_online, int online, bool is_offline,
	int offline, bool is__3gpp_ps_data_off_status,
	int _3gpp_ps_data_off_status, bool is_ref_qos_indication,
	int ref_qos_indication, bool is_trace_req_null,
	OpenAPI_trace_data_t *trace_req, OpenAPI_snssai_t *slice_info,
	OpenAPI_qos_flow_usage_e qos_flow_usage,
	OpenAPI_serving_nf_identity_t *serv_nf_id, char *supp_feat,
	char *smf_id, char *recovery_time,
	...
	OpenAPI_list_t *pvs_info, bool is_onboard_ind, int onboard_ind,
	OpenAPI_list_t *nwdaf_datas) {
	OpenAPI_sm_policy_context_data_t *sm_policy_context_data_local_var =
	...
	sm_policy_context_data_local_var->chargingcharacteristics =
	...
	sm_policy_context_data_local_var->user_location_info =
		user_location_info;
	...
	sm_policy_context_data_local_var->ipv6_address_prefix =
		ipv6_address_prefix;
	...
	sm_policy_context_data_local_var->is_num_of_pack_filter =
		is_num_of_pack_filter;
	sm_policy_context_data_local_var->num_of_pack_filter =
		num_of_pack_filter;
	...
	sm_policy_context_data_local_var->is__3gpp_ps_data_off_status =
		is__3gpp_ps_data_off_status;
	sm_policy_context_data_local_var->_3gpp_ps_data_off_status =
		_3gpp_ps_data_off_status;
	sm_policy_context_data_local_var->is_ref_qos_indication =
		is_ref_qos_indication;
	sm_policy_context_data_local_var->ref_qos_indication =
		ref_qos_indication;
	...
	sm_policy_context_data_local_var->ipv4_frame_route_list =
		ipv4_frame_route_list;
	sm_policy_context_data_local_var->ipv6_frame_route_list =
		ipv6_frame_route_list;
	sm_policy_context_data_local_var->sat_backhaul_category =
		sat_backhaul_category;
	sm_policy_context_data_local_var->is_pcf_ue_info_null =
		is_pcf_ue_info_null;
...
void OpenAPI_sm_policy_context_data_free(
	OpenAPI_sm_policy_context_data_t *sm_policy_context_data) {
	...
	if(sm_policy_context_data->acc_net_ch_id) {
		OpenAPI_acc_net_ch_id_free(
			sm_policy_context_data->acc_net_ch_id);
		sm_policy_context_data->acc_net_ch_id = NULL;
	}
	if(sm_policy_context_data->charg_entity_addr) {
		OpenAPI_acc_net_charging_address_free(
			sm_policy_context_data->charg_entity_addr);
	...
	if(sm_policy_context_data->inter_grp_ids) {
		OpenAPI_list_for_each(sm_policy_context_data->inter_grp_ids,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(sm_policy_context_data->subs_def_qos) {
		OpenAPI_subscribed_default_qos_free(
			sm_policy_context_data->subs_def_qos);
	...
	if(sm_policy_context_data->serv_nf_id) {
		OpenAPI_serving_nf_identity_free(
			sm_policy_context_data->serv_nf_id);
	...
	if(sm_policy_context_data->ipv4_frame_route_list) {
		OpenAPI_list_for_each(
			sm_policy_context_data->ipv4_frame_route_list, node) {
			ogs_free(node->data);
		}
		OpenAPI_list_free(
	...
	if(sm_policy_context_data->ipv6_frame_route_list) {
		OpenAPI_list_for_each(
			sm_policy_context_data->ipv6_frame_route_list, node) {
			ogs_free(node->data);
		}
		OpenAPI_list_free(
			sm_policy_context_data->ipv6_frame_route_list);
		sm_policy_context_data->ipv6_frame_route_list = NULL;
	}
	if(sm_policy_context_data->pcf_ue_info) {
		OpenAPI_pcf_ue_callback_info_free(
			sm_policy_context_data->pcf_ue_info);
	...
	if(sm_policy_context_data->nwdaf_datas) {
		OpenAPI_list_for_each(sm_policy_context_data->nwdaf_datas,
		                      node) {
...
cJSON *OpenAPI_sm_policy_context_data_convertToJSON(
	OpenAPI_sm_policy_context_data_t *sm_policy_context_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(sm_policy_context_data == NULL) {
		ogs_error(
			"OpenAPI_sm_policy_context_data_convertToJSON() failed [SmPolicyContextData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(sm_policy_context_data->acc_net_ch_id) {
		cJSON *acc_net_ch_id_local_JSON =
			OpenAPI_acc_net_ch_id_convertToJSON(
				sm_policy_context_data->acc_net_ch_id);
		if(acc_net_ch_id_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [acc_net_ch_id]");
			goto end;
		}
		cJSON_AddItemToObject(item, "accNetChId",
		                      acc_net_ch_id_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [acc_net_ch_id]");
			goto end;
	...
	if(sm_policy_context_data->charg_entity_addr) {
		cJSON *charg_entity_addr_local_JSON =
			OpenAPI_acc_net_charging_address_convertToJSON(
				sm_policy_context_data->charg_entity_addr);
		if(charg_entity_addr_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [charg_entity_addr]");
			goto end;
		}
		cJSON_AddItemToObject(item, "chargEntityAddr",
		                      charg_entity_addr_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [charg_entity_addr]");
			goto end;
	...
	if(sm_policy_context_data->gpsi) {
		if(cJSON_AddStringToObject(item, "gpsi",
		                           sm_policy_context_data->gpsi) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [gpsi]");
			goto end;
		}
	}
	...
	if(!sm_policy_context_data->supi) {
		ogs_error(
			"OpenAPI_sm_policy_context_data_convertToJSON() failed [supi]");
		return NULL;
	}
	...
	if(sm_policy_context_data->is_invalid_supi) {
		if(cJSON_AddBoolToObject(item, "invalidSupi",
		                         sm_policy_context_data->invalid_supi)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [invalid_supi]");
			goto end;
		}
	}
	...
	if(sm_policy_context_data->inter_grp_ids) {
		...
		if(inter_grp_idsList == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [inter_grp_ids]");
			goto end;
		}
		OpenAPI_list_for_each(sm_policy_context_data->inter_grp_ids,
		                      node) {
	...
	if(cJSON_AddNumberToObject(item, "pduSessionId",
	                           sm_policy_context_data->pdu_session_id) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_sm_policy_context_data_convertToJSON() failed [pdu_session_id]");
		goto end;
	}
	...
	if(sm_policy_context_data->chargingcharacteristics) {
		if(cJSON_AddStringToObject(item, "chargingcharacteristics",
		                           sm_policy_context_data->
		                           chargingcharacteristics) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [chargingcharacteristics]");
			goto end;
		}
	}
	...
	if(!sm_policy_context_data->dnn) {
		ogs_error(
			"OpenAPI_sm_policy_context_data_convertToJSON() failed [dnn]");
		return NULL;
	}
	...
	{
		if(cJSON_AddStringToObject(item, "dnnSelMode",
		                           OpenAPI_dnn_selection_mode_ToString(
						   sm_policy_context_data->
						   dnn_sel_mode)) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [dnn_sel_mode]");
			goto end;
		}
	}
	...
	if(!sm_policy_context_data->notification_uri) {
		ogs_error(
			"OpenAPI_sm_policy_context_data_convertToJSON() failed [notification_uri]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "notificationUri",
	                           sm_policy_context_data->notification_uri) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_sm_policy_context_data_convertToJSON() failed [notification_uri]");
	...
	if(sm_policy_context_data->access_type != OpenAPI_access_type_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [access_type]");
			goto end;
		}
	}
	...
	if(sm_policy_context_data->rat_type != OpenAPI_rat_type_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [rat_type]");
			goto end;
		}
	}
	...
	if(sm_policy_context_data->add_access_info) {
		...
		if(add_access_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [add_access_info]");
			goto end;
		}
	...
	if(sm_policy_context_data->serving_network) {
		...
		if(serving_network_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [serving_network]");
			goto end;
		}
	...
	if(sm_policy_context_data->user_location_info) {
		...
		if(user_location_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [user_location_info]");
			goto end;
		}
	...
	if(sm_policy_context_data->ue_time_zone) {
		if(cJSON_AddStringToObject(item, "ueTimeZone",
		                           sm_policy_context_data->ue_time_zone)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [ue_time_zone]");
			goto end;
		}
	}
	...
	if(sm_policy_context_data->pei) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [pei]");
			goto end;
		}
	}
	...
	if(sm_policy_context_data->ipv4_address) {
		if(cJSON_AddStringToObject(item, "ipv4Address",
		                           sm_policy_context_data->ipv4_address)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [ipv4_address]");
			goto end;
		}
	}
	...
	if(sm_policy_context_data->ipv6_address_prefix) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [ipv6_address_prefix]");
			goto end;
		}
	}
	...
	if(sm_policy_context_data->ip_domain) {
		if(cJSON_AddStringToObject(item, "ipDomain",
		                           sm_policy_context_data->ip_domain) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [ip_domain]");
			goto end;
		}
	}
	...
	if(sm_policy_context_data->subs_sess_ambr) {
		cJSON *subs_sess_ambr_local_JSON =
			OpenAPI_ambr_convertToJSON(
				sm_policy_context_data->subs_sess_ambr);
		if(subs_sess_ambr_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [subs_sess_ambr]");
			goto end;
		}
	...
	if(sm_policy_context_data->auth_prof_index) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [auth_prof_index]");
			goto end;
		}
	}
	...
	if(sm_policy_context_data->subs_def_qos) {
		...
		if(subs_def_qos_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [subs_def_qos]");
			goto end;
		}
	...
	if(sm_policy_context_data->vplmn_qos) {
		cJSON *vplmn_qos_local_JSON =
			OpenAPI_vplmn_qos_convertToJSON(
				sm_policy_context_data->vplmn_qos);
		if(vplmn_qos_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [vplmn_qos]");
			goto end;
		}
	...
	if(sm_policy_context_data->is_num_of_pack_filter) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [num_of_pack_filter]");
			goto end;
		}
	}

	if(sm_policy_context_data->is_online) {
		if(cJSON_AddBoolToObject(item, "online",
		                         sm_policy_context_data->online) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [online]");
			goto end;
		}
	}

	if(sm_policy_context_data->is_offline) {
		if(cJSON_AddBoolToObject(item, "offline",
		                         sm_policy_context_data->offline) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [offline]");
			goto end;
		}
	}
	...
	if(sm_policy_context_data->is__3gpp_ps_data_off_status) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [_3gpp_ps_data_off_status]");
			goto end;
		}
	}
	...
	if(sm_policy_context_data->is_ref_qos_indication) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [ref_qos_indication]");
			goto end;
		}
	}
	...
	if(sm_policy_context_data->trace_req) {
		cJSON *trace_req_local_JSON =
			OpenAPI_trace_data_convertToJSON(
				sm_policy_context_data->trace_req);
		if(trace_req_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [trace_req]");
			goto end;
		}
	...
	if(!sm_policy_context_data->slice_info) {
		ogs_error(
			"OpenAPI_sm_policy_context_data_convertToJSON() failed [slice_info]");
		return NULL;
	}
	cJSON *slice_info_local_JSON =
		OpenAPI_snssai_convertToJSON(
			sm_policy_context_data->slice_info);
	   ...
	   OpenAPI_qos_flow_usage_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [qos_flow_usage]");
			goto end;
		}
	}
	...
	if(sm_policy_context_data->serv_nf_id) {
		...
		if(serv_nf_id_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [serv_nf_id]");
			goto end;
		}
	...
	if(sm_policy_context_data->supp_feat) {
		if(cJSON_AddStringToObject(item, "suppFeat",
		                           sm_policy_context_data->supp_feat) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [supp_feat]");
			goto end;
		}
	}
	...
	if(sm_policy_context_data->smf_id) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [smf_id]");
			goto end;
		}
	}
	...
	if(sm_policy_context_data->recovery_time) {
		if(cJSON_AddStringToObject(item, "recoveryTime",
		                           sm_policy_context_data->recovery_time)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [recovery_time]");
			goto end;
		}
	}
	   ...
	   OpenAPI_ma_pdu_indication_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [ma_pdu_ind]");
			goto end;
		}
	}
	   ...
	   OpenAPI_npcf_atsss_capability_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [atsss_capab]");
			goto end;
		}
	}

	if(sm_policy_context_data->ipv4_frame_route_list) {
		cJSON *ipv4_frame_route_listList = cJSON_AddArrayToObject(item,
		                                                          "ipv4FrameRouteList");
		if(ipv4_frame_route_listList == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [ipv4_frame_route_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			sm_policy_context_data->ipv4_frame_route_list, node) {
			if(cJSON_AddStringToObject(ipv4_frame_route_listList,
			                           "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
	...
	if(sm_policy_context_data->ipv6_frame_route_list) {
		cJSON *ipv6_frame_route_listList = cJSON_AddArrayToObject(item,
		                                                          "ipv6FrameRouteList");
		if(ipv6_frame_route_listList == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [ipv6_frame_route_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			sm_policy_context_data->ipv6_frame_route_list, node) {
			if(cJSON_AddStringToObject(ipv6_frame_route_listList,
			                           "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
	...
	if(sm_policy_context_data->sat_backhaul_category !=
	   OpenAPI_satellite_backhaul_category_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [sat_backhaul_category]");
			goto end;
		}
	}
	...
	if(sm_policy_context_data->pcf_ue_info) {
		...
		if(pcf_ue_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [pcf_ue_info]");
			goto end;
		}
	...
	if(sm_policy_context_data->pvs_info) {
		...
		if(pvs_infoList == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [pvs_info]");
			goto end;
		}
		OpenAPI_list_for_each(sm_policy_context_data->pvs_info, node) {
			cJSON *itemLocal =
	...
	if(sm_policy_context_data->is_onboard_ind) {
		if(cJSON_AddBoolToObject(item, "onboardInd",
		                         sm_policy_context_data->onboard_ind) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [onboard_ind]");
			goto end;
		}
	}
	...
	if(sm_policy_context_data->nwdaf_datas) {
		...
		if(nwdaf_datasList == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_convertToJSON() failed [nwdaf_datas]");
			goto end;
		}
		OpenAPI_list_for_each(sm_policy_context_data->nwdaf_datas,
		                      node) {
...
OpenAPI_sm_policy_context_data_t *OpenAPI_sm_policy_context_data_parseFromJSON(
	cJSON *sm_policy_context_dataJSON) {
	OpenAPI_sm_policy_context_data_t *sm_policy_context_data_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	OpenAPI_acc_net_charging_address_t *charg_entity_addr_local_nonprim =
		NULL;
	cJSON *gpsi = NULL;
	cJSON *supi = NULL;
	...
	acc_net_ch_id =
		cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON,
		                                 "accNetChId");
	if(acc_net_ch_id) {
		acc_net_ch_id_local_nonprim =
			OpenAPI_acc_net_ch_id_parseFromJSON(acc_net_ch_id);
		if(!acc_net_ch_id_local_nonprim) {
			ogs_error(
				"OpenAPI_acc_net_ch_id_parseFromJSON failed [acc_net_ch_id]");
			goto end;
		}
	}
	...
	if(charg_entity_addr) {
		charg_entity_addr_local_nonprim =
			OpenAPI_acc_net_charging_address_parseFromJSON(
				charg_entity_addr);
		if(!charg_entity_addr_local_nonprim) {
			ogs_error(
				"OpenAPI_acc_net_charging_address_parseFromJSON failed [charg_entity_addr]");
			goto end;
		}
	}

	gpsi = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON,
	                                        "gpsi");
	if(gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [gpsi]");
			goto end;
		}
	}

	supi = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON,
	                                        "supi");
	if(!supi) {
		ogs_error(
			"OpenAPI_sm_policy_context_data_parseFromJSON() failed [supi]");
		goto end;
	}
	...
	if(invalid_supi) {
		if(!cJSON_IsBool(invalid_supi)) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [invalid_supi]");
			goto end;
		}
	}
	...
	if(inter_grp_ids) {
		...
		if(!cJSON_IsArray(inter_grp_ids)) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [inter_grp_ids]");
			goto end;
		}
	...
	if(!pdu_session_id) {
		ogs_error(
			"OpenAPI_sm_policy_context_data_parseFromJSON() failed [pdu_session_id]");
		goto end;
	}
	...
	if(!pdu_session_type) {
		ogs_error(
			"OpenAPI_sm_policy_context_data_parseFromJSON() failed [pdu_session_type]");
		goto end;
	}
	...
	if(chargingcharacteristics) {
		if(!cJSON_IsString(chargingcharacteristics) &&
		   !cJSON_IsNull(chargingcharacteristics))
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [chargingcharacteristics]");
			goto end;
		}
	}

	dnn = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON,
	                                       "dnn");
	if(!dnn) {
		ogs_error(
			"OpenAPI_sm_policy_context_data_parseFromJSON() failed [dnn]");
		goto end;
	}
	...
	if(dnn_sel_mode) {
		if(!cJSON_IsString(dnn_sel_mode)) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [dnn_sel_mode]");
			goto end;
		}
		dnn_sel_modeVariable =
			OpenAPI_dnn_selection_mode_FromString(
				dnn_sel_mode->valuestring);
	}
	...
	if(!notification_uri) {
		ogs_error(
			"OpenAPI_sm_policy_context_data_parseFromJSON() failed [notification_uri]");
		goto end;
	}
	...
	if(access_type) {
		if(!cJSON_IsString(access_type)) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [access_type]");
			goto end;
		}
		access_typeVariable =
	...
	rat_type = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON,
	                                            "ratType");
	if(rat_type) {
		if(!cJSON_IsString(rat_type)) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [rat_type]");
			goto end;
		}
		rat_typeVariable =
	...
	if(ue_time_zone) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [ue_time_zone]");
			goto end;
		}
	}

	pei = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON,
	                                       "pei");
	if(pei) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [pei]");
			goto end;
		}
	}
	...
	if(ipv4_address) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [ipv4_address]");
			goto end;
		}
	}
	...
	if(ipv6_address_prefix) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [ipv6_address_prefix]");
			goto end;
		}
	}

	ip_domain = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON,
	                                             "ipDomain");
	if(ip_domain) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [ip_domain]");
			goto end;
		}
	}
	...
	if(auth_prof_index) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [auth_prof_index]");
			goto end;
		}
	}
	...
	vplmn_qos = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON,
	                                             "vplmnQos");
	if(vplmn_qos) {
	...
	if(num_of_pack_filter) {
		if(!cJSON_IsNumber(num_of_pack_filter)) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [num_of_pack_filter]");
			goto end;
		}
	}

	online = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON,
	                                          "online");
	if(online) {
		if(!cJSON_IsBool(online)) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [online]");
			goto end;
		}
	}
	...
	if(offline) {
		if(!cJSON_IsBool(offline)) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [offline]");
			goto end;
		}
	}
	...
	if(_3gpp_ps_data_off_status) {
		if(!cJSON_IsBool(_3gpp_ps_data_off_status)) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [_3gpp_ps_data_off_status]");
			goto end;
		}
	}
	...
	if(ref_qos_indication) {
		if(!cJSON_IsBool(ref_qos_indication)) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [ref_qos_indication]");
			goto end;
		}
	}
	...
	if(!slice_info) {
		ogs_error(
			"OpenAPI_sm_policy_context_data_parseFromJSON() failed [slice_info]");
		goto end;
	}
	...
	if(qos_flow_usage) {
		if(!cJSON_IsString(qos_flow_usage)) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [qos_flow_usage]");
			goto end;
		}
	...
	supp_feat = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON,
	                                             "suppFeat");
	if(supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [supp_feat]");
			goto end;
		}
	}

	smf_id = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON,
	                                          "smfId");
	if(smf_id) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [smf_id]");
			goto end;
		}
	}
	...
	if(recovery_time) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [recovery_time]");
			goto end;
		}
	}
	...
	if(ma_pdu_ind) {
		if(!cJSON_IsString(ma_pdu_ind)) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [ma_pdu_ind]");
			goto end;
		}
		ma_pdu_indVariable =
	...
	if(atsss_capab) {
		if(!cJSON_IsString(atsss_capab)) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [atsss_capab]");
			goto end;
		}
		atsss_capabVariable =
	...
	if(ipv4_frame_route_list) {
		...
		if(!cJSON_IsArray(ipv4_frame_route_list)) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [ipv4_frame_route_list]");
			goto end;
		}

		ipv4_frame_route_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(ipv4_frame_route_list_local,
		                   ipv4_frame_route_list) {
			double *localDouble = NULL;
			int *localInt = NULL;
			...
			OpenAPI_list_add(ipv4_frame_route_listList,
			                 ogs_strdup(
						 ipv4_frame_route_list_local->
						 valuestring));
		}
	}
	...
	if(ipv6_frame_route_list) {
		...
		if(!cJSON_IsArray(ipv6_frame_route_list)) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [ipv6_frame_route_list]");
			goto end;
		}

		ipv6_frame_route_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(ipv6_frame_route_list_local,
		                   ipv6_frame_route_list) {
			double *localDouble = NULL;
			int *localInt = NULL;
			...
			OpenAPI_list_add(ipv6_frame_route_listList,
			                 ogs_strdup(
						 ipv6_frame_route_list_local->
						 valuestring));
		}
	}
	...
	if(sat_backhaul_category) {
		if(!cJSON_IsString(sat_backhaul_category)) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [sat_backhaul_category]");
			goto end;
		}
	...
	pvs_info = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON,
	                                            "pvsInfo");
	if(pvs_info) {
		cJSON *pvs_info_local = NULL;
		if(!cJSON_IsArray(pvs_info)) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [pvs_info]");
			goto end;
		}
	...
	if(onboard_ind) {
		if(!cJSON_IsBool(onboard_ind)) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [onboard_ind]");
			goto end;
		}
	}
	...
	if(nwdaf_datas) {
		...
		if(!cJSON_IsArray(nwdaf_datas)) {
			ogs_error(
				"OpenAPI_sm_policy_context_data_parseFromJSON() failed [nwdaf_datas]");
			goto end;
		}
	...
	sm_policy_context_data_local_var =
		OpenAPI_sm_policy_context_data_create(
			acc_net_ch_id ? acc_net_ch_id_local_nonprim : NULL,
			charg_entity_addr ? charg_entity_addr_local_nonprim :
			NULL,
			gpsi &&
			...
			chargingcharacteristics &&
			!cJSON_IsNull(chargingcharacteristics) ?
			ogs_strdup(chargingcharacteristics->valuestring) : NULL,
			...
			ipv4_frame_route_list ? ipv4_frame_route_listList :
			NULL,
			ipv6_frame_route_list ? ipv6_frame_route_listList :
			NULL,
...
OpenAPI_sm_policy_context_data_t *OpenAPI_sm_policy_context_data_copy(
	OpenAPI_sm_policy_context_data_t	*dst,
	OpenAPI_sm_policy_context_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_sm_policy_context_data_convertToJSON() failed");
		return NULL;
	}
