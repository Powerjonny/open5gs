...
OpenAPI_sm_context_updated_data_t *OpenAPI_sm_context_updated_data_create(
	...
	OpenAPI_list_t *release_ebi_list, OpenAPI_list_t *allocated_ebi_list,
	...
	OpenAPI_list_t *eps_bearer_setup, bool is_data_forwarding,
	int data_forwarding, OpenAPI_list_t *n3_dl_forwarding_tnl_list,
	OpenAPI_list_t *n3_ul_forwarding_tnl_list,
	OpenAPI_tunnel_info_t *n9_ul_forwarding_tunnel, OpenAPI_cause_e cause,
	bool is_ma_accepted_ind, int ma_accepted_ind, char *supported_features,
	char *forwarding_f_teid, OpenAPI_list_t *forwarding_bearer_contexts,
	char *selected_smf_id, char *selected_old_smf_id,
	char *inter_plmn_api_root,
	OpenAPI_anchor_smf_features_t *anchor_smf_features) {
	OpenAPI_sm_context_updated_data_t *sm_context_updated_data_local_var =
	...
	sm_context_updated_data_local_var->allocated_ebi_list =
		allocated_ebi_list;
	sm_context_updated_data_local_var->modified_ebi_list =
		modified_ebi_list;
	...
	sm_context_updated_data_local_var->is_data_forwarding =
		is_data_forwarding;
	sm_context_updated_data_local_var->data_forwarding = data_forwarding;
	sm_context_updated_data_local_var->n3_dl_forwarding_tnl_list =
		n3_dl_forwarding_tnl_list;
	sm_context_updated_data_local_var->n3_ul_forwarding_tnl_list =
		n3_ul_forwarding_tnl_list;
	sm_context_updated_data_local_var->n9_ul_forwarding_tunnel =
		n9_ul_forwarding_tunnel;
	sm_context_updated_data_local_var->cause = cause;
	sm_context_updated_data_local_var->is_ma_accepted_ind =
		is_ma_accepted_ind;
	sm_context_updated_data_local_var->ma_accepted_ind = ma_accepted_ind;
	sm_context_updated_data_local_var->supported_features =
		supported_features;
	sm_context_updated_data_local_var->forwarding_f_teid =
		forwarding_f_teid;
	sm_context_updated_data_local_var->forwarding_bearer_contexts =
		forwarding_bearer_contexts;
	sm_context_updated_data_local_var->selected_smf_id = selected_smf_id;
	sm_context_updated_data_local_var->selected_old_smf_id =
		selected_old_smf_id;
	sm_context_updated_data_local_var->inter_plmn_api_root =
		inter_plmn_api_root;
	sm_context_updated_data_local_var->anchor_smf_features =
		anchor_smf_features;
...
void OpenAPI_sm_context_updated_data_free(
	OpenAPI_sm_context_updated_data_t *sm_context_updated_data) {
	...
	if(sm_context_updated_data->release_ebi_list) {
		OpenAPI_list_for_each(sm_context_updated_data->release_ebi_list,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(sm_context_updated_data->allocated_ebi_list) {
		OpenAPI_list_for_each(
			sm_context_updated_data->allocated_ebi_list, node) {
	...
	if(sm_context_updated_data->modified_ebi_list) {
		OpenAPI_list_for_each(
			sm_context_updated_data->modified_ebi_list, node) {
			OpenAPI_ebi_arp_mapping_free(node->data);
		}
	...
	if(sm_context_updated_data->n1_sm_msg) {
		OpenAPI_ref_to_binary_data_free(
			sm_context_updated_data->n1_sm_msg);
	...
	if(sm_context_updated_data->eps_bearer_setup) {
		OpenAPI_list_for_each(sm_context_updated_data->eps_bearer_setup,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(sm_context_updated_data->n3_dl_forwarding_tnl_list) {
		OpenAPI_list_for_each(
			sm_context_updated_data->n3_dl_forwarding_tnl_list,
			node) {
	...
	if(sm_context_updated_data->n3_ul_forwarding_tnl_list) {
		OpenAPI_list_for_each(
			sm_context_updated_data->n3_ul_forwarding_tnl_list,
			node) {
			OpenAPI_indirect_data_forwarding_tunnel_info_free(
				node->data);
		}
	...
	if(sm_context_updated_data->n9_ul_forwarding_tunnel) {
		OpenAPI_tunnel_info_free(
			sm_context_updated_data->n9_ul_forwarding_tunnel);
	...
	if(sm_context_updated_data->anchor_smf_features) {
		OpenAPI_anchor_smf_features_free(
			sm_context_updated_data->anchor_smf_features);
		sm_context_updated_data->anchor_smf_features = NULL;
	}
	ogs_free(sm_context_updated_data);
}

cJSON *OpenAPI_sm_context_updated_data_convertToJSON(
	OpenAPI_sm_context_updated_data_t *sm_context_updated_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(sm_context_updated_data == NULL) {
		ogs_error(
			"OpenAPI_sm_context_updated_data_convertToJSON() failed [SmContextUpdatedData]");
		return NULL;
	}
	...
	if(sm_context_updated_data->up_cnx_state != OpenAPI_up_cnx_state_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [up_cnx_state]");
			goto end;
		}
	}
	...
	if(sm_context_updated_data->ho_state != OpenAPI_ho_state_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [ho_state]");
			goto end;
		}
	}

	if(sm_context_updated_data->release_ebi_list) {
		cJSON *release_ebi_listList = cJSON_AddArrayToObject(item,
		                                                     "releaseEbiList");
		if(release_ebi_listList == NULL) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [release_ebi_list]");
			goto end;
		}
		OpenAPI_list_for_each(sm_context_updated_data->release_ebi_list,
		                      node) {
			...
			if(cJSON_AddNumberToObject(release_ebi_listList, "",
			                           *(double *) node->data) ==
			   NULL)
			{
				ogs_error(
	...
	if(sm_context_updated_data->allocated_ebi_list) {
		...
		if(allocated_ebi_listList == NULL) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [allocated_ebi_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			sm_context_updated_data->allocated_ebi_list, node) {
	...
	if(sm_context_updated_data->modified_ebi_list) {
		...
		if(modified_ebi_listList == NULL) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [modified_ebi_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			sm_context_updated_data->modified_ebi_list, node) {
	...
	if(sm_context_updated_data->n1_sm_msg) {
		...
		if(n1_sm_msg_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [n1_sm_msg]");
			goto end;
		}
	...
	if(sm_context_updated_data->n2_sm_info) {
		...
		if(n2_sm_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [n2_sm_info]");
			goto end;
		}
	   ...
	   OpenAPI_n2_sm_info_type_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [n2_sm_info_type]");
			goto end;
		}
	}
	...
	if(sm_context_updated_data->eps_bearer_setup) {
		...
		if(eps_bearer_setupList == NULL) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [eps_bearer_setup]");
			goto end;
		}
		OpenAPI_list_for_each(sm_context_updated_data->eps_bearer_setup,
		                      node) {
	...
	if(sm_context_updated_data->is_data_forwarding) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [data_forwarding]");
			goto end;
		}
	}

	if(sm_context_updated_data->n3_dl_forwarding_tnl_list) {
		cJSON *n3_dl_forwarding_tnl_listList =
			cJSON_AddArrayToObject(item, "n3DlForwardingTnlList");
		if(n3_dl_forwarding_tnl_listList == NULL) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [n3_dl_forwarding_tnl_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			sm_context_updated_data->n3_dl_forwarding_tnl_list,
			...
			cJSON_AddItemToArray(n3_dl_forwarding_tnl_listList,
			                     itemLocal);
		}
	}

	if(sm_context_updated_data->n3_ul_forwarding_tnl_list) {
		cJSON *n3_ul_forwarding_tnl_listList =
			cJSON_AddArrayToObject(item, "n3UlForwardingTnlList");
		if(n3_ul_forwarding_tnl_listList == NULL) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [n3_ul_forwarding_tnl_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			sm_context_updated_data->n3_ul_forwarding_tnl_list,
			...
			cJSON_AddItemToArray(n3_ul_forwarding_tnl_listList,
			                     itemLocal);
		}
	}

	if(sm_context_updated_data->n9_ul_forwarding_tunnel) {
		cJSON *n9_ul_forwarding_tunnel_local_JSON =
			OpenAPI_tunnel_info_convertToJSON(
				sm_context_updated_data->n9_ul_forwarding_tunnel);
		if(n9_ul_forwarding_tunnel_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [n9_ul_forwarding_tunnel]");
			goto end;
		}
		cJSON_AddItemToObject(item, "n9UlForwardingTunnel",
		                      n9_ul_forwarding_tunnel_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [n9_ul_forwarding_tunnel]");
			goto end;
	...
	if(sm_context_updated_data->cause != OpenAPI_cause_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [cause]");
			goto end;
		}
	}
	...
	if(sm_context_updated_data->is_ma_accepted_ind) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [ma_accepted_ind]");
			goto end;
		}
	}
	...
	if(sm_context_updated_data->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(sm_context_updated_data->forwarding_f_teid) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [forwarding_f_teid]");
			goto end;
		}
	}
	...
	if(sm_context_updated_data->forwarding_bearer_contexts) {
		...
		if(forwarding_bearer_contextsList == NULL) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [forwarding_bearer_contexts]");
			goto end;
		}
		OpenAPI_list_for_each(
			sm_context_updated_data->forwarding_bearer_contexts,
	...
	if(sm_context_updated_data->selected_smf_id) {
		if(cJSON_AddStringToObject(item, "selectedSmfId",
		                           sm_context_updated_data->
		                           selected_smf_id) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [selected_smf_id]");
			goto end;
		}
	}

	if(sm_context_updated_data->selected_old_smf_id) {
		if(cJSON_AddStringToObject(item, "selectedOldSmfId",
		                           sm_context_updated_data->
		                           selected_old_smf_id) == NULL)
		{
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [selected_old_smf_id]");
			goto end;
		}
	}
	...
	if(sm_context_updated_data->inter_plmn_api_root) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [inter_plmn_api_root]");
			goto end;
		}
	}

	if(sm_context_updated_data->anchor_smf_features) {
		cJSON *anchor_smf_features_local_JSON =
			OpenAPI_anchor_smf_features_convertToJSON(
				sm_context_updated_data->anchor_smf_features);
		if(anchor_smf_features_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [anchor_smf_features]");
			goto end;
		}
		cJSON_AddItemToObject(item, "anchorSmfFeatures",
		                      anchor_smf_features_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_convertToJSON() failed [anchor_smf_features]");
			goto end;
...
OpenAPI_sm_context_updated_data_t *OpenAPI_sm_context_updated_data_parseFromJSON
        (cJSON *sm_context_updated_dataJSON) {
	OpenAPI_sm_context_updated_data_t *sm_context_updated_data_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(up_cnx_state) {
		if(!cJSON_IsString(up_cnx_state)) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_parseFromJSON() failed [up_cnx_state]");
			goto end;
		}
	...
	if(ho_state) {
		if(!cJSON_IsString(ho_state)) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_parseFromJSON() failed [ho_state]");
			goto end;
		}
		ho_stateVariable =
	...
	if(release_ebi_list) {
		...
		if(!cJSON_IsArray(release_ebi_list)) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_parseFromJSON() failed [release_ebi_list]");
			goto end;
		}
	...
	if(allocated_ebi_list) {
		...
		if(!cJSON_IsArray(allocated_ebi_list)) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_parseFromJSON() failed [allocated_ebi_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(allocated_ebi_list_local,
			...
			OpenAPI_ebi_arp_mapping_t *allocated_ebi_listItem =
			...
			OpenAPI_list_add(allocated_ebi_listList,
	...
	if(modified_ebi_list) {
		...
		if(!cJSON_IsArray(modified_ebi_list)) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_parseFromJSON() failed [modified_ebi_list]");
			goto end;
		}
	...
	if(n2_sm_info_type) {
		if(!cJSON_IsString(n2_sm_info_type)) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_parseFromJSON() failed [n2_sm_info_type]");
			goto end;
		}
	...
	if(eps_bearer_setup) {
		...
		if(!cJSON_IsArray(eps_bearer_setup)) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_parseFromJSON() failed [eps_bearer_setup]");
			goto end;
		}
	...
	if(data_forwarding) {
		if(!cJSON_IsBool(data_forwarding)) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_parseFromJSON() failed [data_forwarding]");
			goto end;
		}
	}
	...
	if(n3_dl_forwarding_tnl_list) {
		...
		if(!cJSON_IsArray(n3_dl_forwarding_tnl_list)) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_parseFromJSON() failed [n3_dl_forwarding_tnl_list]");
			goto end;
		}

		n3_dl_forwarding_tnl_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(n3_dl_forwarding_tnl_list_local,
		                   n3_dl_forwarding_tnl_list) {
	...
	if(n3_ul_forwarding_tnl_list) {
		...
		if(!cJSON_IsArray(n3_ul_forwarding_tnl_list)) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_parseFromJSON() failed [n3_ul_forwarding_tnl_list]");
			goto end;
		}

		n3_ul_forwarding_tnl_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(n3_ul_forwarding_tnl_list_local,
		                   n3_ul_forwarding_tnl_list) {
			...
			OpenAPI_list_add(n3_ul_forwarding_tnl_listList,
			                 n3_ul_forwarding_tnl_listItem);
		}
	}
	...
	if(n9_ul_forwarding_tunnel) {
		n9_ul_forwarding_tunnel_local_nonprim =
			OpenAPI_tunnel_info_parseFromJSON(
				n9_ul_forwarding_tunnel);
		if(!n9_ul_forwarding_tunnel_local_nonprim) {
			ogs_error(
				"OpenAPI_tunnel_info_parseFromJSON failed [n9_ul_forwarding_tunnel]");
			goto end;
		}
	}

	cause = cJSON_GetObjectItemCaseSensitive(sm_context_updated_dataJSON,
	                                         "cause");
	if(cause) {
		if(!cJSON_IsString(cause)) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_parseFromJSON() failed [cause]");
			goto end;
		}
	...
	if(ma_accepted_ind) {
		if(!cJSON_IsBool(ma_accepted_ind)) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_parseFromJSON() failed [ma_accepted_ind]");
			goto end;
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_updated_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(forwarding_f_teid) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_updated_data_parseFromJSON() failed [forwarding_f_teid]");
			goto end;
		}
	}
	...
	if(forwarding_bearer_contexts) {
		...
		if(!cJSON_IsArray(forwarding_bearer_contexts)) {
			ogs_error(
				"OpenAPI_sm_context_updated_data_parseFromJSON() failed [forwarding_bearer_contexts]");
			goto end;
		}
	...
	if(selected_smf_id) {
		if(!cJSON_IsString(selected_smf_id) &&
		   !cJSON_IsNull(selected_smf_id))
		{
			ogs_error(
				"OpenAPI_sm_context_updated_data_parseFromJSON() failed [selected_smf_id]");
			goto end;
		}
	}
	...
	if(selected_old_smf_id) {
		if(!cJSON_IsString(selected_old_smf_id) &&
		   !cJSON_IsNull(selected_old_smf_id))
		{
			ogs_error(
				"OpenAPI_sm_context_updated_data_parseFromJSON() failed [selected_old_smf_id]");
			goto end;
		}
	}
	...
	if(inter_plmn_api_root) {
		...
		{
			ogs_error(
				"OpenAPI_sm_context_updated_data_parseFromJSON() failed [inter_plmn_api_root]");
			goto end;
		}
	}
	...
	if(anchor_smf_features) {
		anchor_smf_features_local_nonprim =
			OpenAPI_anchor_smf_features_parseFromJSON(
				anchor_smf_features);
		if(!anchor_smf_features_local_nonprim) {
			ogs_error(
				"OpenAPI_anchor_smf_features_parseFromJSON failed [anchor_smf_features]");
			goto end;
		}
	}
	...
	sm_context_updated_data_local_var =
		OpenAPI_sm_context_updated_data_create(
			...
			selected_smf_id &&
			!cJSON_IsNull(selected_smf_id) ?
			ogs_strdup(selected_smf_id->valuestring) : NULL,
			selected_old_smf_id &&
			!cJSON_IsNull(selected_old_smf_id) ?
			ogs_strdup(selected_old_smf_id->valuestring) : NULL,
			inter_plmn_api_root &&
			!cJSON_IsNull(inter_plmn_api_root) ?
...
OpenAPI_sm_context_updated_data_t *OpenAPI_sm_context_updated_data_copy(
	OpenAPI_sm_context_updated_data_t	*dst,
	OpenAPI_sm_context_updated_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_sm_context_updated_data_convertToJSON() failed");
		return NULL;
	}
