...
OpenAPI_n4_information_t *OpenAPI_n4_information_create(
	...
	OpenAPI_dnai_information_t *n4_dnai_info, char *psa_upf_id,
	char *ul_cl_bp_id, OpenAPI_list_t *n9_ul_pdr_id_list) {
	OpenAPI_n4_information_t *n4_information_local_var =
		ogs_malloc(sizeof(OpenAPI_n4_information_t));
...
void OpenAPI_n4_information_free(OpenAPI_n4_information_t *n4_information) {
	...
	if(n4_information->n4_message_payload) {
		OpenAPI_ref_to_binary_data_free(
			n4_information->n4_message_payload);
...
cJSON *OpenAPI_n4_information_convertToJSON(
	OpenAPI_n4_information_t *n4_information) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(n4_information == NULL) {
		ogs_error(
			"OpenAPI_n4_information_convertToJSON() failed [N4Information]");
		return NULL;
	}
	...
	if(n4_information->n4_message_type == OpenAPI_n4_message_type_NULL) {
		ogs_error(
			"OpenAPI_n4_information_convertToJSON() failed [n4_message_type]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "n4MessageType",
	                           OpenAPI_n4_message_type_ToString(
					   n4_information->n4_message_type)) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_n4_information_convertToJSON() failed [n4_message_type]");
	...
	if(!n4_information->n4_message_payload) {
		ogs_error(
			"OpenAPI_n4_information_convertToJSON() failed [n4_message_payload]");
		return NULL;
	}
	cJSON *n4_message_payload_local_JSON =
	...
	cJSON_AddItemToObject(item, "n4MessagePayload",
	                      n4_message_payload_local_JSON);
	if(item->child == NULL) {
		ogs_error(
			"OpenAPI_n4_information_convertToJSON() failed [n4_message_payload]");
		goto end;
	...
	if(n4_information->n4_dnai_info) {
		cJSON *n4_dnai_info_local_JSON =
			OpenAPI_dnai_information_convertToJSON(
				n4_information->n4_dnai_info);
		if(n4_dnai_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_n4_information_convertToJSON() failed [n4_dnai_info]");
			goto end;
		}
		cJSON_AddItemToObject(item, "n4DnaiInfo",
		                      n4_dnai_info_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_n4_information_convertToJSON() failed [n4_dnai_info]");
			goto end;
	...
	if(n4_information->psa_upf_id) {
		if(cJSON_AddStringToObject(item, "psaUpfId",
		                           n4_information->psa_upf_id) == NULL)
		{
			ogs_error(
				"OpenAPI_n4_information_convertToJSON() failed [psa_upf_id]");
			goto end;
		}
	}

	if(n4_information->ul_cl_bp_id) {
		if(cJSON_AddStringToObject(item, "ulClBpId",
		                           n4_information->ul_cl_bp_id) == NULL)
		{
			ogs_error(
				"OpenAPI_n4_information_convertToJSON() failed [ul_cl_bp_id]");
			goto end;
		}
	}

	if(n4_information->n9_ul_pdr_id_list) {
		cJSON *n9_ul_pdr_id_listList = cJSON_AddArrayToObject(item,
		                                                      "n9UlPdrIdList");
		if(n9_ul_pdr_id_listList == NULL) {
			ogs_error(
				"OpenAPI_n4_information_convertToJSON() failed [n9_ul_pdr_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(n4_information->n9_ul_pdr_id_list, node) {
			if(node->data == NULL) {
			...
			if(cJSON_AddNumberToObject(n9_ul_pdr_id_listList, "",
			                           *(double *) node->data) ==
			   NULL)
			{
				ogs_error(
...
OpenAPI_n4_information_t *OpenAPI_n4_information_parseFromJSON(
	cJSON *n4_informationJSON) {
	...
	n4_message_type = cJSON_GetObjectItemCaseSensitive(n4_informationJSON,
	                                                   "n4MessageType");
	if(!n4_message_type) {
		ogs_error(
			"OpenAPI_n4_information_parseFromJSON() failed [n4_message_type]");
		goto end;
	}
	...
	n4_message_typeVariable =
		OpenAPI_n4_message_type_FromString(
			n4_message_type->valuestring);

	n4_message_payload =
		cJSON_GetObjectItemCaseSensitive(n4_informationJSON,
		                                 "n4MessagePayload");
	if(!n4_message_payload) {
		ogs_error(
			"OpenAPI_n4_information_parseFromJSON() failed [n4_message_payload]");
		goto end;
	}
	n4_message_payload_local_nonprim =
		OpenAPI_ref_to_binary_data_parseFromJSON(n4_message_payload);
	if(!n4_message_payload_local_nonprim) {
		ogs_error(
			"OpenAPI_ref_to_binary_data_parseFromJSON failed [n4_message_payload]");
		goto end;
	}

	n4_dnai_info = cJSON_GetObjectItemCaseSensitive(n4_informationJSON,
	                                                "n4DnaiInfo");
	if(n4_dnai_info) {
		n4_dnai_info_local_nonprim =
			OpenAPI_dnai_information_parseFromJSON(n4_dnai_info);
		if(!n4_dnai_info_local_nonprim) {
			ogs_error(
				"OpenAPI_dnai_information_parseFromJSON failed [n4_dnai_info]");
			goto end;
		}
	}

	psa_upf_id = cJSON_GetObjectItemCaseSensitive(n4_informationJSON,
	                                              "psaUpfId");
	if(psa_upf_id) {
		...
		{
			ogs_error(
				"OpenAPI_n4_information_parseFromJSON() failed [psa_upf_id]");
			goto end;
		}
	}

	ul_cl_bp_id = cJSON_GetObjectItemCaseSensitive(n4_informationJSON,
	                                               "ulClBpId");
	if(ul_cl_bp_id) {
		if(!cJSON_IsString(ul_cl_bp_id) &&
		   !cJSON_IsNull(ul_cl_bp_id))
		{
			ogs_error(
				"OpenAPI_n4_information_parseFromJSON() failed [ul_cl_bp_id]");
			goto end;
		}
	}

	n9_ul_pdr_id_list = cJSON_GetObjectItemCaseSensitive(n4_informationJSON,
	                                                     "n9UlPdrIdList");
	if(n9_ul_pdr_id_list) {
		cJSON *n9_ul_pdr_id_list_local = NULL;
		if(!cJSON_IsArray(n9_ul_pdr_id_list)) {
			ogs_error(
				"OpenAPI_n4_information_parseFromJSON() failed [n9_ul_pdr_id_list]");
			goto end;
		}
	...
	n4_information_local_var = OpenAPI_n4_information_create(
		...
		psa_upf_id &&
		!cJSON_IsNull(psa_upf_id) ?
		ogs_strdup(psa_upf_id->valuestring) : NULL,
		ul_cl_bp_id &&
		!cJSON_IsNull(ul_cl_bp_id) ?
		ogs_strdup(ul_cl_bp_id->valuestring) : NULL,
...
OpenAPI_n4_information_t *OpenAPI_n4_information_copy(
	OpenAPI_n4_information_t *dst, OpenAPI_n4_information_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
