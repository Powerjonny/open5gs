...
OpenAPI_ue_analytics_context_descriptor_create(char		*supi,
	...
	OpenAPI_ue_analytics_context_descriptor_t *
		...
		ogs_malloc(sizeof(OpenAPI_ue_analytics_context_descriptor_t));
...
void OpenAPI_ue_analytics_context_descriptor_free(
	...
	if(ue_analytics_context_descriptor->ana_types) {
		OpenAPI_list_for_each(
			ue_analytics_context_descriptor->ana_types, node) {
...
cJSON *OpenAPI_ue_analytics_context_descriptor_convertToJSON(
	...
	if(ue_analytics_context_descriptor == NULL) {
		ogs_error(
			"OpenAPI_ue_analytics_context_descriptor_convertToJSON() failed [UeAnalyticsContextDescriptor]");
		return NULL;
	}
	...
	if(!ue_analytics_context_descriptor->supi) {
		ogs_error(
			"OpenAPI_ue_analytics_context_descriptor_convertToJSON() failed [supi]");
		return NULL;
	}
	...
	if(!ue_analytics_context_descriptor->ana_types) {
		ogs_error(
			"OpenAPI_ue_analytics_context_descriptor_convertToJSON() failed [ana_types]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(ue_analytics_context_descriptor->ana_types,
	                      node) {
		cJSON *itemLocal =
...
OpenAPI_ue_analytics_context_descriptor_parseFromJSON(
	...
	if(!supi) {
		ogs_error(
			"OpenAPI_ue_analytics_context_descriptor_parseFromJSON() failed [supi]");
		goto end;
	}
	...
	if(!ana_types) {
		ogs_error(
			"OpenAPI_ue_analytics_context_descriptor_parseFromJSON() failed [ana_types]");
		goto end;
	}
	...
	cJSON_ArrayForEach(ana_types_local, ana_types) {
		...
		OpenAPI_nwdaf_event_t *ana_typesItem =
			OpenAPI_nwdaf_event_parseFromJSON(ana_types_local);
		if(!ana_typesItem) {
	...
	ue_analytics_context_descriptor_local_var =
		OpenAPI_ue_analytics_context_descriptor_create(
...
OpenAPI_ue_analytics_context_descriptor_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ue_analytics_context_descriptor_convertToJSON() failed");
		return NULL;
	}
