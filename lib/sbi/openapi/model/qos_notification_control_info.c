...
OpenAPI_qos_notification_control_info_create(
	...
	char *alt_ser_req) {
	OpenAPI_qos_notification_control_info_t *
	        qos_notification_control_info_local_var =
		ogs_malloc(sizeof(OpenAPI_qos_notification_control_info_t));
	...
	OpenAPI_qos_notification_control_info_t *qos_notification_control_info)
{
	...
	if(qos_notification_control_info->flows) {
		OpenAPI_list_for_each(qos_notification_control_info->flows,
		                      node) {
	...
	OpenAPI_qos_notification_control_info_t *qos_notification_control_info)
{
	...
	if(qos_notification_control_info == NULL) {
		ogs_error(
			"OpenAPI_qos_notification_control_info_convertToJSON() failed [QosNotificationControlInfo]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(qos_notification_control_info->notif_type ==
	   OpenAPI_qos_notif_type_NULL)
	{
		ogs_error(
			"OpenAPI_qos_notification_control_info_convertToJSON() failed [notif_type]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "notifType",
	                           OpenAPI_qos_notif_type_ToString(
	...
	if(qos_notification_control_info->flows) {
		...
		if(flowsList == NULL) {
			ogs_error(
				"OpenAPI_qos_notification_control_info_convertToJSON() failed [flows]");
			goto end;
		}
		OpenAPI_list_for_each(qos_notification_control_info->flows,
		                      node) {
	...
	if(qos_notification_control_info->alt_ser_req) {
		...
		{
			ogs_error(
				"OpenAPI_qos_notification_control_info_convertToJSON() failed [alt_ser_req]");
			goto end;
		}
	}
...
OpenAPI_qos_notification_control_info_parseFromJSON(
	...
	OpenAPI_qos_notification_control_info_t *
	        qos_notification_control_info_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	notif_type =
		cJSON_GetObjectItemCaseSensitive(
			qos_notification_control_infoJSON, "notifType");
	if(!notif_type) {
		ogs_error(
			"OpenAPI_qos_notification_control_info_parseFromJSON() failed [notif_type]");
		goto end;
	}
	...
	notif_typeVariable =
		OpenAPI_qos_notif_type_FromString(notif_type->valuestring);

	flows =
		cJSON_GetObjectItemCaseSensitive(
			qos_notification_control_infoJSON, "flows");
	if(flows) {
		cJSON *flows_local = NULL;
		if(!cJSON_IsArray(flows)) {
			ogs_error(
				"OpenAPI_qos_notification_control_info_parseFromJSON() failed [flows]");
			goto end;
		}
	...
	alt_ser_req =
		cJSON_GetObjectItemCaseSensitive(
			qos_notification_control_infoJSON, "altSerReq");
	if(alt_ser_req) {
		if(!cJSON_IsString(alt_ser_req) &&
		   !cJSON_IsNull(alt_ser_req))
		{
			ogs_error(
				"OpenAPI_qos_notification_control_info_parseFromJSON() failed [alt_ser_req]");
			goto end;
		}
	}
...
OpenAPI_qos_notification_control_info_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_qos_notification_control_info_convertToJSON() failed");
		return NULL;
	}
