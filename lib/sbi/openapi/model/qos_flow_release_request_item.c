...
OpenAPI_qos_flow_release_request_item_create(int qfi, char *qos_rules,
	...
	OpenAPI_qos_flow_release_request_item_t *
	        qos_flow_release_request_item_local_var =
		ogs_malloc(sizeof(OpenAPI_qos_flow_release_request_item_t));
	...
	qos_flow_release_request_item_local_var->qos_flow_description =
	...
	OpenAPI_qos_flow_release_request_item_t *qos_flow_release_request_item)
{
	...
	if(qos_flow_release_request_item == NULL) {
		ogs_error(
			"OpenAPI_qos_flow_release_request_item_convertToJSON() failed [QosFlowReleaseRequestItem]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(cJSON_AddNumberToObject(item, "qfi",
	                           qos_flow_release_request_item->qfi) == NULL)
	{
		ogs_error(
			"OpenAPI_qos_flow_release_request_item_convertToJSON() failed [qfi]");
		goto end;
	}

	if(qos_flow_release_request_item->qos_rules) {
		if(cJSON_AddStringToObject(item, "qosRules",
		                           qos_flow_release_request_item->
		                           qos_rules) == NULL)
		{
			ogs_error(
				"OpenAPI_qos_flow_release_request_item_convertToJSON() failed [qos_rules]");
			goto end;
		}
	}
	...
	if(qos_flow_release_request_item->qos_flow_description) {
		...
		{
			ogs_error(
				"OpenAPI_qos_flow_release_request_item_convertToJSON() failed [qos_flow_description]");
			goto end;
		}
	}
...
OpenAPI_qos_flow_release_request_item_parseFromJSON(
	...
	OpenAPI_qos_flow_release_request_item_t *
	        qos_flow_release_request_item_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *qfi = NULL;
	...
	qfi =
		cJSON_GetObjectItemCaseSensitive(
			qos_flow_release_request_itemJSON, "qfi");
	if(!qfi) {
		ogs_error(
			"OpenAPI_qos_flow_release_request_item_parseFromJSON() failed [qfi]");
		goto end;
	}
	...
	qos_rules =
		cJSON_GetObjectItemCaseSensitive(
			qos_flow_release_request_itemJSON, "qosRules");
	if(qos_rules) {
		if(!cJSON_IsString(qos_rules) &&
		   !cJSON_IsNull(qos_rules))
		{
			ogs_error(
				"OpenAPI_qos_flow_release_request_item_parseFromJSON() failed [qos_rules]");
			goto end;
		}
	}
	...
	if(qos_flow_description) {
		...
		{
			ogs_error(
				"OpenAPI_qos_flow_release_request_item_parseFromJSON() failed [qos_flow_description]");
			goto end;
		}
	}
...
OpenAPI_qos_flow_release_request_item_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_qos_flow_release_request_item_convertToJSON() failed");
		return NULL;
	}
