...
OpenAPI_easdf_info_t *OpenAPI_easdf_info_create(
	...
	OpenAPI_easdf_info_t *easdf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_easdf_info_t));
	ogs_assert(easdf_info_local_var);

	easdf_info_local_var->s_nssai_easdf_info_list = s_nssai_easdf_info_list;
	easdf_info_local_var->easdf_n6_ip_address_list =
		easdf_n6_ip_address_list;
...
void OpenAPI_easdf_info_free(OpenAPI_easdf_info_t *easdf_info) {
	...
	if(easdf_info->s_nssai_easdf_info_list) {
		OpenAPI_list_for_each(easdf_info->s_nssai_easdf_info_list,
		                      node) {
	...
	if(easdf_info->easdf_n6_ip_address_list) {
		OpenAPI_list_for_each(easdf_info->easdf_n6_ip_address_list,
		                      node) {
	...
	if(easdf_info->upf_n6_ip_address_list) {
		OpenAPI_list_for_each(easdf_info->upf_n6_ip_address_list,
		                      node) {
			OpenAPI_ip_addr_free(node->data);
		}
...
cJSON *OpenAPI_easdf_info_convertToJSON(OpenAPI_easdf_info_t *easdf_info) {
	...
	if(easdf_info == NULL) {
		ogs_error(
			"OpenAPI_easdf_info_convertToJSON() failed [EasdfInfo]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(easdf_info->s_nssai_easdf_info_list) {
		cJSON *s_nssai_easdf_info_listList =
			cJSON_AddArrayToObject(item, "sNssaiEasdfInfoList");
		if(s_nssai_easdf_info_listList == NULL) {
			ogs_error(
				"OpenAPI_easdf_info_convertToJSON() failed [s_nssai_easdf_info_list]");
			goto end;
		}
		OpenAPI_list_for_each(easdf_info->s_nssai_easdf_info_list,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_snssai_easdf_info_item_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_easdf_info_convertToJSON() failed [s_nssai_easdf_info_list]");
			...
			cJSON_AddItemToArray(s_nssai_easdf_info_listList,
			                     itemLocal);
		}
	}

	if(easdf_info->easdf_n6_ip_address_list) {
		cJSON *easdf_n6_ip_address_listList =
			cJSON_AddArrayToObject(item, "easdfN6IpAddressList");
		if(easdf_n6_ip_address_listList == NULL) {
			ogs_error(
				"OpenAPI_easdf_info_convertToJSON() failed [easdf_n6_ip_address_list]");
			goto end;
		}
		OpenAPI_list_for_each(easdf_info->easdf_n6_ip_address_list,
		                      node) {
			...
			cJSON_AddItemToArray(easdf_n6_ip_address_listList,
			                     itemLocal);
		}
	}

	if(easdf_info->upf_n6_ip_address_list) {
		cJSON *upf_n6_ip_address_listList = cJSON_AddArrayToObject(item,
		                                                           "upfN6IpAddressList");
		if(upf_n6_ip_address_listList == NULL) {
			ogs_error(
				"OpenAPI_easdf_info_convertToJSON() failed [upf_n6_ip_address_list]");
			goto end;
		}
		OpenAPI_list_for_each(easdf_info->upf_n6_ip_address_list,
		                      node) {
			...
			cJSON_AddItemToArray(upf_n6_ip_address_listList,
			                     itemLocal);
		}
	}
...
OpenAPI_easdf_info_t *OpenAPI_easdf_info_parseFromJSON(cJSON *easdf_infoJSON) {
	...
	s_nssai_easdf_info_list =
		cJSON_GetObjectItemCaseSensitive(easdf_infoJSON,
		                                 "sNssaiEasdfInfoList");
	if(s_nssai_easdf_info_list) {
		cJSON *s_nssai_easdf_info_list_local = NULL;
		if(!cJSON_IsArray(s_nssai_easdf_info_list)) {
			ogs_error(
				"OpenAPI_easdf_info_parseFromJSON() failed [s_nssai_easdf_info_list]");
			goto end;
		}

		s_nssai_easdf_info_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(s_nssai_easdf_info_list_local,
		                   s_nssai_easdf_info_list) {
			...
			OpenAPI_snssai_easdf_info_item_t *
			        s_nssai_easdf_info_listItem =
				OpenAPI_snssai_easdf_info_item_parseFromJSON(
					s_nssai_easdf_info_list_local);
			if(!s_nssai_easdf_info_listItem) {
				ogs_error("No s_nssai_easdf_info_listItem");
				goto end;
			}
			OpenAPI_list_add(s_nssai_easdf_info_listList,
			                 s_nssai_easdf_info_listItem);
		}
	}
	...
	if(easdf_n6_ip_address_list) {
		...
		if(!cJSON_IsArray(easdf_n6_ip_address_list)) {
			ogs_error(
				"OpenAPI_easdf_info_parseFromJSON() failed [easdf_n6_ip_address_list]");
			goto end;
		}

		easdf_n6_ip_address_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(easdf_n6_ip_address_list_local,
		                   easdf_n6_ip_address_list) {
			...
			OpenAPI_ip_addr_t *easdf_n6_ip_address_listItem =
				OpenAPI_ip_addr_parseFromJSON(
					easdf_n6_ip_address_list_local);
			if(!easdf_n6_ip_address_listItem) {
				ogs_error("No easdf_n6_ip_address_listItem");
				goto end;
			}
			OpenAPI_list_add(easdf_n6_ip_address_listList,
			                 easdf_n6_ip_address_listItem);
		}
	}

	upf_n6_ip_address_list =
		cJSON_GetObjectItemCaseSensitive(easdf_infoJSON,
		                                 "upfN6IpAddressList");
	if(upf_n6_ip_address_list) {
		...
		if(!cJSON_IsArray(upf_n6_ip_address_list)) {
			ogs_error(
				"OpenAPI_easdf_info_parseFromJSON() failed [upf_n6_ip_address_list]");
			goto end;
		}

		upf_n6_ip_address_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(upf_n6_ip_address_list_local,
		                   upf_n6_ip_address_list) {
			...
			OpenAPI_ip_addr_t *upf_n6_ip_address_listItem =
				OpenAPI_ip_addr_parseFromJSON(
					upf_n6_ip_address_list_local);
			if(!upf_n6_ip_address_listItem) {
				ogs_error("No upf_n6_ip_address_listItem");
				goto end;
			}
			OpenAPI_list_add(upf_n6_ip_address_listList,
