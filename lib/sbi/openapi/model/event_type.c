...
OpenAPI_event_type_t *OpenAPI_event_type_create() {
	OpenAPI_event_type_t *event_type_local_var =
		ogs_malloc(sizeof(OpenAPI_event_type_t));
...
cJSON *OpenAPI_event_type_convertToJSON(OpenAPI_event_type_t *event_type) {
	...
	if(event_type == NULL) {
		ogs_error(
			"OpenAPI_event_type_convertToJSON() failed [EventType]");
		return NULL;
	}
...
OpenAPI_event_type_t *OpenAPI_event_type_copy(OpenAPI_event_type_t	*dst,
                                              OpenAPI_event_type_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
