	...
	OpenAPI_binary_t			*binary_data_n2_sm_information)
{
	OpenAPI_release_sm_context_request_t *
	        release_sm_context_request_local_var =
		ogs_malloc(sizeof(OpenAPI_release_sm_context_request_t));
...
void OpenAPI_release_sm_context_request_free(
	...
	if(release_sm_context_request->json_data) {
		OpenAPI_sm_context_release_data_free(
	...
	if(release_sm_context_request->binary_data_n2_sm_information) {
		...
		release_sm_context_request->binary_data_n2_sm_information =
			NULL;
	}
	ogs_free(release_sm_context_request);
}

cJSON *OpenAPI_release_sm_context_request_convertToJSON(
	OpenAPI_release_sm_context_request_t *release_sm_context_request) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(release_sm_context_request == NULL) {
		ogs_error(
			"OpenAPI_release_sm_context_request_convertToJSON() failed [ReleaseSmContext_request]");
		return NULL;
	}
	...
	if(release_sm_context_request->json_data) {
		cJSON *json_data_local_JSON =
			OpenAPI_sm_context_release_data_convertToJSON(
				release_sm_context_request->json_data);
		if(json_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_release_sm_context_request_convertToJSON() failed [json_data]");
			goto end;
		}
	...
	if(release_sm_context_request->binary_data_n2_sm_information) {
		...
		{
			ogs_error(
				"OpenAPI_release_sm_context_request_convertToJSON() failed [binary_data_n2_sm_information]");
			goto end;
		}
...
OpenAPI_release_sm_context_request_t *
OpenAPI_release_sm_context_request_parseFromJSON(
	cJSON *release_sm_context_requestJSON) {
	OpenAPI_release_sm_context_request_t *
	        release_sm_context_request_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	json_data =
		cJSON_GetObjectItemCaseSensitive(release_sm_context_requestJSON,
		                                 "jsonData");
	if(json_data) {
		json_data_local_nonprim =
			OpenAPI_sm_context_release_data_parseFromJSON(
				json_data);
		if(!json_data_local_nonprim) {
			ogs_error(
				"OpenAPI_sm_context_release_data_parseFromJSON failed [json_data]");
			goto end;
		}
	}
	...
	if(binary_data_n2_sm_information) {
		...
		if(!cJSON_IsString(binary_data_n2_sm_information)) {
			ogs_error(
				"OpenAPI_release_sm_context_request_parseFromJSON() failed [binary_data_n2_sm_information]");
			goto end;
		}
	...
	release_sm_context_request_local_var =
		OpenAPI_release_sm_context_request_create(
...
OpenAPI_release_sm_context_request_t *OpenAPI_release_sm_context_request_copy(
	OpenAPI_release_sm_context_request_t	*dst,
	OpenAPI_release_sm_context_request_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_release_sm_context_request_convertToJSON() failed");
		return NULL;
	}
