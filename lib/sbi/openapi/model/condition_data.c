...
OpenAPI_condition_data_t *OpenAPI_condition_data_create(char *cond_id,
                                                        bool
                                                        is_activation_time_null,
                                                        char *activation_time,
                                                        bool
                                                        is_deactivation_time_null,
                                                        char
                                                        *deactivation_time,
                                                        ...
                                                        OpenAPI_rat_type_e
                                                        rat_type) {
	OpenAPI_condition_data_t *condition_data_local_var =
		ogs_malloc(sizeof(OpenAPI_condition_data_t));
	ogs_assert(condition_data_local_var);

	condition_data_local_var->cond_id = cond_id;
	condition_data_local_var->is_activation_time_null =
		is_activation_time_null;
	condition_data_local_var->activation_time = activation_time;
	condition_data_local_var->is_deactivation_time_null =
		is_deactivation_time_null;
...
cJSON *OpenAPI_condition_data_convertToJSON(
	OpenAPI_condition_data_t *condition_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(condition_data == NULL) {
		ogs_error(
			"OpenAPI_condition_data_convertToJSON() failed [ConditionData]");
		return NULL;
	}
	...
	if(!condition_data->cond_id) {
		ogs_error(
			"OpenAPI_condition_data_convertToJSON() failed [cond_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "condId",
	                           condition_data->cond_id) == NULL)
	{
		ogs_error(
			"OpenAPI_condition_data_convertToJSON() failed [cond_id]");
	...
	if(condition_data->activation_time) {
		if(cJSON_AddStringToObject(item, "activationTime",
		                           condition_data->activation_time) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_condition_data_convertToJSON() failed [activation_time]");
			goto end;
		}
	...
	if(condition_data->deactivation_time) {
		if(cJSON_AddStringToObject(item, "deactivationTime",
		                           condition_data->deactivation_time) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_condition_data_convertToJSON() failed [deactivation_time]");
			goto end;
		}
	...
	if(condition_data->access_type != OpenAPI_access_type_NULL) {
		if(cJSON_AddStringToObject(item, "accessType",
						   ...
						   condition_data->access_type))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_condition_data_convertToJSON() failed [access_type]");
			goto end;
		}
	}
	...
	if(condition_data->rat_type != OpenAPI_rat_type_NULL) {
		if(cJSON_AddStringToObject(item, "ratType",
						   ...
						   condition_data->rat_type)) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_condition_data_convertToJSON() failed [rat_type]");
			goto end;
		}
	}
...
OpenAPI_condition_data_t *OpenAPI_condition_data_parseFromJSON(
	cJSON *condition_dataJSON) {
	...
	cond_id = cJSON_GetObjectItemCaseSensitive(condition_dataJSON,
	                                           "condId");
	if(!cond_id) {
		ogs_error(
			"OpenAPI_condition_data_parseFromJSON() failed [cond_id]");
		goto end;
	}
	...
	activation_time = cJSON_GetObjectItemCaseSensitive(condition_dataJSON,
	                                                   "activationTime");
	if(activation_time) {
		if(!cJSON_IsNull(activation_time)) {
			...
			{
				ogs_error(
					"OpenAPI_condition_data_parseFromJSON() failed [activation_time]");
				goto end;
			}
		}
	}

	deactivation_time = cJSON_GetObjectItemCaseSensitive(condition_dataJSON,
	                                                     "deactivationTime");
	if(deactivation_time) {
		if(!cJSON_IsNull(deactivation_time)) {
			if(!cJSON_IsString(deactivation_time) &&
			   !cJSON_IsNull(deactivation_time))
			{
				ogs_error(
					"OpenAPI_condition_data_parseFromJSON() failed [deactivation_time]");
				goto end;
			}
		}
	}

	access_type = cJSON_GetObjectItemCaseSensitive(condition_dataJSON,
	                                               "accessType");
	if(access_type) {
		if(!cJSON_IsString(access_type)) {
			ogs_error(
				"OpenAPI_condition_data_parseFromJSON() failed [access_type]");
			goto end;
		}
		access_typeVariable =
	...
	rat_type = cJSON_GetObjectItemCaseSensitive(condition_dataJSON,
	                                            "ratType");
	if(rat_type) {
		if(!cJSON_IsString(rat_type)) {
			ogs_error(
				"OpenAPI_condition_data_parseFromJSON() failed [rat_type]");
			goto end;
		}
		rat_typeVariable =
	...
	condition_data_local_var = OpenAPI_condition_data_create(
		...
		activation_time &&
		cJSON_IsNull(activation_time) ? true : false,
		activation_time &&
		!cJSON_IsNull(activation_time) ?
		ogs_strdup(activation_time->valuestring) : NULL,
		deactivation_time &&
		cJSON_IsNull(deactivation_time) ? true : false,
		deactivation_time &&
		!cJSON_IsNull(deactivation_time) ?
		ogs_strdup(deactivation_time->valuestring) : NULL,
...
OpenAPI_condition_data_t *OpenAPI_condition_data_copy(
	OpenAPI_condition_data_t *dst, OpenAPI_condition_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
