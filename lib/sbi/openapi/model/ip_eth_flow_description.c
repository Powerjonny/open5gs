...
OpenAPI_ip_eth_flow_description_t *OpenAPI_ip_eth_flow_description_create(
	...
	OpenAPI_ip_eth_flow_description_t *ip_eth_flow_description_local_var =
		ogs_malloc(sizeof(OpenAPI_ip_eth_flow_description_t));
...
void OpenAPI_ip_eth_flow_description_free(
	OpenAPI_ip_eth_flow_description_t *ip_eth_flow_description) {
...
cJSON *OpenAPI_ip_eth_flow_description_convertToJSON(
	OpenAPI_ip_eth_flow_description_t *ip_eth_flow_description) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(ip_eth_flow_description == NULL) {
		ogs_error(
			"OpenAPI_ip_eth_flow_description_convertToJSON() failed [IpEthFlowDescription]");
		return NULL;
	}
	...
	if(ip_eth_flow_description->ip_traffic_filter) {
		...
		{
			ogs_error(
				"OpenAPI_ip_eth_flow_description_convertToJSON() failed [ip_traffic_filter]");
			goto end;
		}
	}
	...
	if(ip_eth_flow_description->eth_traffic_filter) {
		...
		if(eth_traffic_filter_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ip_eth_flow_description_convertToJSON() failed [eth_traffic_filter]");
			goto end;
		}
...
OpenAPI_ip_eth_flow_description_t *OpenAPI_ip_eth_flow_description_parseFromJSON
	...
	ip_traffic_filter =
		cJSON_GetObjectItemCaseSensitive(ip_eth_flow_descriptionJSON,
		                                 "ipTrafficFilter");
	if(ip_traffic_filter) {
		...
		{
			ogs_error(
				"OpenAPI_ip_eth_flow_description_parseFromJSON() failed [ip_traffic_filter]");
			goto end;
		}
	}
	...
	if(eth_traffic_filter) {
		...
		if(!eth_traffic_filter_local_nonprim) {
			ogs_error(
				"OpenAPI_eth_flow_description_parseFromJSON failed [eth_traffic_filter]");
			goto end;
		}
	}

	ip_eth_flow_description_local_var =
		OpenAPI_ip_eth_flow_description_create(
			ip_traffic_filter &&
			!cJSON_IsNull(ip_traffic_filter) ?
			ogs_strdup(ip_traffic_filter->valuestring) : NULL,
			eth_traffic_filter ? eth_traffic_filter_local_nonprim :
			NULL
			);
...
OpenAPI_ip_eth_flow_description_t *OpenAPI_ip_eth_flow_description_copy(
	OpenAPI_ip_eth_flow_description_t	*dst,
	OpenAPI_ip_eth_flow_description_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ip_eth_flow_description_convertToJSON() failed");
		return NULL;
	}
