...
OpenAPI_notif_condition_t *OpenAPI_notif_condition_create(
	...
	OpenAPI_notif_condition_t *notif_condition_local_var =
	...
	notif_condition_local_var->unmonitored_attributes =
		unmonitored_attributes;
...
void OpenAPI_notif_condition_free(OpenAPI_notif_condition_t *notif_condition) {
	...
	if(notif_condition->monitored_attributes) {
		OpenAPI_list_for_each(notif_condition->monitored_attributes,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(notif_condition->unmonitored_attributes) {
		OpenAPI_list_for_each(notif_condition->unmonitored_attributes,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_notif_condition_convertToJSON(
	...
	if(notif_condition == NULL) {
		ogs_error(
			"OpenAPI_notif_condition_convertToJSON() failed [NotifCondition]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(notif_condition->monitored_attributes) {
		cJSON *monitored_attributesList = cJSON_AddArrayToObject(item,
		                                                         "monitoredAttributes");
		if(monitored_attributesList == NULL) {
			ogs_error(
				"OpenAPI_notif_condition_convertToJSON() failed [monitored_attributes]");
			goto end;
		}
		OpenAPI_list_for_each(notif_condition->monitored_attributes,
		                      node) {
			if(cJSON_AddStringToObject(monitored_attributesList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_notif_condition_convertToJSON() failed [monitored_attributes]");
	...
	if(notif_condition->unmonitored_attributes) {
		cJSON *unmonitored_attributesList = cJSON_AddArrayToObject(item,
		                                                           "unmonitoredAttributes");
		if(unmonitored_attributesList == NULL) {
			ogs_error(
				"OpenAPI_notif_condition_convertToJSON() failed [unmonitored_attributes]");
			goto end;
		}
		OpenAPI_list_for_each(notif_condition->unmonitored_attributes,
		                      node) {
			if(cJSON_AddStringToObject(unmonitored_attributesList,
			                           "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
...
OpenAPI_notif_condition_t *OpenAPI_notif_condition_parseFromJSON(
	...
	if(monitored_attributes) {
		...
		if(!cJSON_IsArray(monitored_attributes)) {
			ogs_error(
				"OpenAPI_notif_condition_parseFromJSON() failed [monitored_attributes]");
			goto end;
		}

		monitored_attributesList = OpenAPI_list_create();

		cJSON_ArrayForEach(monitored_attributes_local,
		                   monitored_attributes) {
			...
			OpenAPI_list_add(monitored_attributesList,
			                 ogs_strdup(
						 monitored_attributes_local->
						 valuestring));
		}
	}
	...
	if(unmonitored_attributes) {
		...
		if(!cJSON_IsArray(unmonitored_attributes)) {
			ogs_error(
				"OpenAPI_notif_condition_parseFromJSON() failed [unmonitored_attributes]");
			goto end;
		}

		unmonitored_attributesList = OpenAPI_list_create();

		cJSON_ArrayForEach(unmonitored_attributes_local,
		                   unmonitored_attributes) {
			double *localDouble = NULL;
			int *localInt = NULL;
			...
			OpenAPI_list_add(unmonitored_attributesList,
			                 ogs_strdup(
						 unmonitored_attributes_local->
						 valuestring));
		}
	}
