...
OpenAPI_nsi_information_t *OpenAPI_nsi_information_create(char	*nrf_id,
                                                          char	*nsi_id,
                                                          char	*nrf_nf_mgt_uri,
                                                          char *
                                                          ...
                                                          OpenAPI_list_t *
                                                          nrf_oauth2_required) {
	OpenAPI_nsi_information_t *nsi_information_local_var =
		ogs_malloc(sizeof(OpenAPI_nsi_information_t));
...
void OpenAPI_nsi_information_free(OpenAPI_nsi_information_t *nsi_information) {
	...
	if(nsi_information->nrf_oauth2_required) {
		OpenAPI_list_for_each(nsi_information->nrf_oauth2_required,
		                      node) {
...
cJSON *OpenAPI_nsi_information_convertToJSON(
	...
	if(nsi_information == NULL) {
		ogs_error(
			"OpenAPI_nsi_information_convertToJSON() failed [NsiInformation]");
		return NULL;
	}
	...
	if(!nsi_information->nrf_id) {
		ogs_error(
			"OpenAPI_nsi_information_convertToJSON() failed [nrf_id]");
		return NULL;
	}
	...
	if(nsi_information->nsi_id) {
		...
		{
			ogs_error(
				"OpenAPI_nsi_information_convertToJSON() failed [nsi_id]");
			goto end;
		}
	}
	...
	if(nsi_information->nrf_nf_mgt_uri) {
		...
		{
			ogs_error(
				"OpenAPI_nsi_information_convertToJSON() failed [nrf_nf_mgt_uri]");
			goto end;
		}
	}
	...
	if(nsi_information->nrf_access_token_uri) {
		...
		{
			ogs_error(
				"OpenAPI_nsi_information_convertToJSON() failed [nrf_access_token_uri]");
			goto end;
		}
	}
	...
	if(nsi_information->nrf_oauth2_required) {
		...
		if(nrf_oauth2_required == NULL) {
			ogs_error(
				"OpenAPI_nsi_information_convertToJSON() failed [nrf_oauth2_required]");
			goto end;
		}
		cJSON *localMapObject = nrf_oauth2_required;
		if(nsi_information->nrf_oauth2_required) {
			OpenAPI_list_for_each(
				nsi_information->nrf_oauth2_required, node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				{
					ogs_error(
						"OpenAPI_nsi_information_convertToJSON() failed [inner]");
					goto end;
				}
			}
		}
...
OpenAPI_nsi_information_t *OpenAPI_nsi_information_parseFromJSON(
	cJSON *nsi_informationJSON) {
	...
	if(!nrf_id) {
		ogs_error(
			"OpenAPI_nsi_information_parseFromJSON() failed [nrf_id]");
		goto end;
	}
	...
	if(nsi_id) {
		...
		{
			ogs_error(
				"OpenAPI_nsi_information_parseFromJSON() failed [nsi_id]");
			goto end;
		}
	}
	...
	if(nrf_nf_mgt_uri) {
		...
		{
			ogs_error(
				"OpenAPI_nsi_information_parseFromJSON() failed [nrf_nf_mgt_uri]");
			goto end;
		}
	}

	nrf_access_token_uri =
		cJSON_GetObjectItemCaseSensitive(nsi_informationJSON,
		                                 "nrfAccessTokenUri");
	if(nrf_access_token_uri) {
		...
		{
			ogs_error(
				"OpenAPI_nsi_information_parseFromJSON() failed [nrf_access_token_uri]");
			goto end;
		}
	}
	...
	if(nrf_oauth2_required) {
		...
		{
			ogs_error(
				"OpenAPI_nsi_information_parseFromJSON() failed [nrf_oauth2_required]");
			goto end;
		}
		if(cJSON_IsObject(nrf_oauth2_required)) {
			...
			cJSON_ArrayForEach(nrf_oauth2_required_local_map,
				...
				if(!cJSON_IsBool(localMapObject)) {
					ogs_error(
						"OpenAPI_nsi_information_parseFromJSON() failed [inner]");
					goto end;
				}
	...
	nsi_information_local_var = OpenAPI_nsi_information_create(
		...
		nrf_nf_mgt_uri &&
		!cJSON_IsNull(nrf_nf_mgt_uri) ?
		ogs_strdup(nrf_nf_mgt_uri->valuestring) : NULL,
		nrf_access_token_uri &&
		!cJSON_IsNull(nrf_access_token_uri) ?
...
OpenAPI_nsi_information_t *OpenAPI_nsi_information_copy(
	OpenAPI_nsi_information_t *dst, OpenAPI_nsi_information_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
