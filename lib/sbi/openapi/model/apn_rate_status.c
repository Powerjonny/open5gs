...
OpenAPI_apn_rate_status_t *OpenAPI_apn_rate_status_create(
	...
	OpenAPI_apn_rate_status_t *apn_rate_status_local_var =
		ogs_malloc(sizeof(OpenAPI_apn_rate_status_t));
	...
	apn_rate_status_local_var->is_remain_ex_reports_ul =
		is_remain_ex_reports_ul;
	apn_rate_status_local_var->remain_ex_reports_ul = remain_ex_reports_ul;
	apn_rate_status_local_var->is_remain_ex_reports_dl =
...
cJSON *OpenAPI_apn_rate_status_convertToJSON(
	...
	if(apn_rate_status == NULL) {
		ogs_error(
			"OpenAPI_apn_rate_status_convertToJSON() failed [ApnRateStatus]");
		return NULL;
	}
	...
	if(apn_rate_status->is_remain_packets_ul) {
		if(cJSON_AddNumberToObject(item, "remainPacketsUl",
		                           apn_rate_status->remain_packets_ul)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_apn_rate_status_convertToJSON() failed [remain_packets_ul]");
			goto end;
		}
	}
	...
	if(apn_rate_status->is_remain_packets_dl) {
		if(cJSON_AddNumberToObject(item, "remainPacketsDl",
		                           apn_rate_status->remain_packets_dl)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_apn_rate_status_convertToJSON() failed [remain_packets_dl]");
			goto end;
		}
	}
	...
	if(apn_rate_status->validity_time) {
		...
		{
			ogs_error(
				"OpenAPI_apn_rate_status_convertToJSON() failed [validity_time]");
			goto end;
		}
	}
	...
	if(apn_rate_status->is_remain_ex_reports_ul) {
		if(cJSON_AddNumberToObject(item, "remainExReportsUl",
		                           apn_rate_status->remain_ex_reports_ul)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_apn_rate_status_convertToJSON() failed [remain_ex_reports_ul]");
			goto end;
		}
	}
	...
	if(apn_rate_status->is_remain_ex_reports_dl) {
		if(cJSON_AddNumberToObject(item, "remainExReportsDl",
		                           apn_rate_status->remain_ex_reports_dl)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_apn_rate_status_convertToJSON() failed [remain_ex_reports_dl]");
			goto end;
		}
	}
...
OpenAPI_apn_rate_status_t *OpenAPI_apn_rate_status_parseFromJSON(
	cJSON *apn_rate_statusJSON) {
	...
	remain_packets_ul =
		cJSON_GetObjectItemCaseSensitive(apn_rate_statusJSON,
		                                 "remainPacketsUl");
	if(remain_packets_ul) {
		if(!cJSON_IsNumber(remain_packets_ul)) {
			ogs_error(
				"OpenAPI_apn_rate_status_parseFromJSON() failed [remain_packets_ul]");
			goto end;
		}
	}
	...
	if(remain_packets_dl) {
		if(!cJSON_IsNumber(remain_packets_dl)) {
			ogs_error(
				"OpenAPI_apn_rate_status_parseFromJSON() failed [remain_packets_dl]");
			goto end;
		}
	}

	validity_time = cJSON_GetObjectItemCaseSensitive(apn_rate_statusJSON,
	                                                 "validityTime");
	if(validity_time) {
		...
		{
			ogs_error(
				"OpenAPI_apn_rate_status_parseFromJSON() failed [validity_time]");
			goto end;
		}
	}
	...
	if(remain_ex_reports_ul) {
		if(!cJSON_IsNumber(remain_ex_reports_ul)) {
			ogs_error(
				"OpenAPI_apn_rate_status_parseFromJSON() failed [remain_ex_reports_ul]");
			goto end;
		}
	}
	...
	if(remain_ex_reports_dl) {
		if(!cJSON_IsNumber(remain_ex_reports_dl)) {
			ogs_error(
				"OpenAPI_apn_rate_status_parseFromJSON() failed [remain_ex_reports_dl]");
			goto end;
		}
	}
...
OpenAPI_apn_rate_status_t *OpenAPI_apn_rate_status_copy(
	OpenAPI_apn_rate_status_t *dst, OpenAPI_apn_rate_status_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
