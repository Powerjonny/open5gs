...
OpenAPI_amf_event_subscription_add_info_create(OpenAPI_list_t *binding_info,
                                               OpenAPI_nf_type_e
                                               subscribing_nf_type,
                                               bool
                                                        is_event_sync_ind,
                                               ...
                                               OpenAPI_list_t *aoi_state_list)
{
	OpenAPI_amf_event_subscription_add_info_t *
		...
		ogs_malloc(sizeof(OpenAPI_amf_event_subscription_add_info_t));
...
void OpenAPI_amf_event_subscription_add_info_free(
	OpenAPI_amf_event_subscription_add_info_t *
	...
	if(amf_event_subscription_add_info->binding_info) {
		OpenAPI_list_for_each(
			amf_event_subscription_add_info->binding_info, node) {
			ogs_free(node->data);
		}
		OpenAPI_list_free(
			amf_event_subscription_add_info->binding_info);
		amf_event_subscription_add_info->binding_info = NULL;
	}
	if(amf_event_subscription_add_info->nf_consumer_info) {
		OpenAPI_list_for_each(
			amf_event_subscription_add_info->nf_consumer_info,
			node) {
			ogs_free(node->data);
		}
		OpenAPI_list_free(
	...
	if(amf_event_subscription_add_info->aoi_state_list) {
		OpenAPI_list_for_each(
			amf_event_subscription_add_info->aoi_state_list, node) {
...
cJSON *OpenAPI_amf_event_subscription_add_info_convertToJSON(
	OpenAPI_amf_event_subscription_add_info_t *
	amf_event_subscription_add_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(amf_event_subscription_add_info == NULL) {
		ogs_error(
			"OpenAPI_amf_event_subscription_add_info_convertToJSON() failed [AmfEventSubscriptionAddInfo]");
		return NULL;
	}
	...
	if(amf_event_subscription_add_info->binding_info) {
		...
		if(binding_infoList == NULL) {
			ogs_error(
				"OpenAPI_amf_event_subscription_add_info_convertToJSON() failed [binding_info]");
			goto end;
		}
		OpenAPI_list_for_each(
			amf_event_subscription_add_info->binding_info, node) {
	   ...
	   OpenAPI_nf_type_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_amf_event_subscription_add_info_convertToJSON() failed [subscribing_nf_type]");
			goto end;
		}
	}
	...
	if(amf_event_subscription_add_info->is_event_sync_ind) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_subscription_add_info_convertToJSON() failed [event_sync_ind]");
			goto end;
		}
	}
	...
	if(amf_event_subscription_add_info->nf_consumer_info) {
		...
		if(nf_consumer_infoList == NULL) {
			ogs_error(
				"OpenAPI_amf_event_subscription_add_info_convertToJSON() failed [nf_consumer_info]");
			goto end;
		}
		OpenAPI_list_for_each(
			amf_event_subscription_add_info->nf_consumer_info,
	...
	if(amf_event_subscription_add_info->aoi_state_list) {
		...
		if(aoi_state_list == NULL) {
			ogs_error(
				"OpenAPI_amf_event_subscription_add_info_convertToJSON() failed [aoi_state_list]");
			goto end;
		}
		...
		if(amf_event_subscription_add_info->aoi_state_list) {
			OpenAPI_list_for_each(
				...
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_amf_event_subscription_add_info_convertToJSON() failed [inner]");
					goto end;
				}
...
OpenAPI_amf_event_subscription_add_info_t *
OpenAPI_amf_event_subscription_add_info_parseFromJSON(
	...
	if(binding_info) {
		...
		if(!cJSON_IsArray(binding_info)) {
			ogs_error(
				"OpenAPI_amf_event_subscription_add_info_parseFromJSON() failed [binding_info]");
			goto end;
		}
	...
	if(subscribing_nf_type) {
		if(!cJSON_IsString(subscribing_nf_type)) {
			ogs_error(
				"OpenAPI_amf_event_subscription_add_info_parseFromJSON() failed [subscribing_nf_type]");
			goto end;
		}
	...
	if(event_sync_ind) {
		if(!cJSON_IsBool(event_sync_ind)) {
			ogs_error(
				"OpenAPI_amf_event_subscription_add_info_parseFromJSON() failed [event_sync_ind]");
			goto end;
		}
	}
	...
	if(nf_consumer_info) {
		...
		if(!cJSON_IsArray(nf_consumer_info)) {
			ogs_error(
				"OpenAPI_amf_event_subscription_add_info_parseFromJSON() failed [nf_consumer_info]");
			goto end;
		}
	...
	if(aoi_state_list) {
		...
		{
			ogs_error(
				"OpenAPI_amf_event_subscription_add_info_parseFromJSON() failed [aoi_state_list]");
			goto end;
		}
		if(cJSON_IsObject(aoi_state_list)) {
			...
			cJSON_ArrayForEach(aoi_state_list_local_map,
				...
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_amf_event_subscription_add_info_parseFromJSON() failed [inner]");
					goto end;
				}
...
OpenAPI_amf_event_subscription_add_info_t *
OpenAPI_amf_event_subscription_add_info_copy(
	OpenAPI_amf_event_subscription_add_info_t	*dst,
	OpenAPI_amf_event_subscription_add_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_amf_event_subscription_add_info_convertToJSON() failed");
		return NULL;
	}
