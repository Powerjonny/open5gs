...
char * OpenAPI_lcs_mo_service_class_ToString(
	...
	const char *lcs_mo_service_classArray[] = { "NULL",
		                                    "BASIC_SELF_LOCATION",
		                                    "AUTONOMOUS_SELF_LOCATION",
		                                    "TRANSFER_TO_THIRD_PARTY" };
	size_t sizeofArray = sizeof(lcs_mo_service_classArray) /
	                     sizeof(lcs_mo_service_classArray[0]);
...
OpenAPI_lcs_mo_service_class_e OpenAPI_lcs_mo_service_class_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(lcs_mo_service_class,
		          lcs_mo_service_classArray[stringToReturn]) == 0) {
