...
OpenAPI_prev_sub_info_t *OpenAPI_prev_sub_info_create(char	*producer_id,
                                                      ...
                                                      OpenAPI_list_t *
                                                      ue_ana_events) {
	OpenAPI_prev_sub_info_t *prev_sub_info_local_var =
		ogs_malloc(sizeof(OpenAPI_prev_sub_info_t));
...
void OpenAPI_prev_sub_info_free(OpenAPI_prev_sub_info_t *prev_sub_info) {
	...
	if(prev_sub_info->ue_ana_events) {
		OpenAPI_list_for_each(prev_sub_info->ue_ana_events, node) {
			OpenAPI_ue_analytics_context_descriptor_free(
				node->data);
		}
...
cJSON *OpenAPI_prev_sub_info_convertToJSON(
	OpenAPI_prev_sub_info_t *prev_sub_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(prev_sub_info == NULL) {
		ogs_error(
			"OpenAPI_prev_sub_info_convertToJSON() failed [PrevSubInfo]");
		return NULL;
	}
	...
	if(prev_sub_info->producer_id) {
		...
		{
			ogs_error(
				"OpenAPI_prev_sub_info_convertToJSON() failed [producer_id]");
			goto end;
		}
	}
	...
	if(prev_sub_info->producer_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_prev_sub_info_convertToJSON() failed [producer_set_id]");
			goto end;
		}
	}
	...
	if(!prev_sub_info->subscription_id) {
		ogs_error(
			"OpenAPI_prev_sub_info_convertToJSON() failed [subscription_id]");
		return NULL;
	}
	...
	if(prev_sub_info->nf_ana_events) {
		...
		if(nf_ana_eventsList == NULL) {
			ogs_error(
				"OpenAPI_prev_sub_info_convertToJSON() failed [nf_ana_events]");
			goto end;
		}
		OpenAPI_list_for_each(prev_sub_info->nf_ana_events, node) {
			cJSON *itemLocal =
	...
	if(prev_sub_info->ue_ana_events) {
		...
		if(ue_ana_eventsList == NULL) {
			ogs_error(
				"OpenAPI_prev_sub_info_convertToJSON() failed [ue_ana_events]");
			goto end;
		}
		OpenAPI_list_for_each(prev_sub_info->ue_ana_events, node) {
			cJSON *itemLocal =
				OpenAPI_ue_analytics_context_descriptor_convertToJSON
				        (node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_prev_sub_info_convertToJSON() failed [ue_ana_events]");
...
OpenAPI_prev_sub_info_t *OpenAPI_prev_sub_info_parseFromJSON(
	cJSON *prev_sub_infoJSON) {
	...
	if(producer_id) {
		...
		{
			ogs_error(
				"OpenAPI_prev_sub_info_parseFromJSON() failed [producer_id]");
			goto end;
		}
	}
	...
	if(producer_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_prev_sub_info_parseFromJSON() failed [producer_set_id]");
			goto end;
		}
	}
	...
	if(!subscription_id) {
		ogs_error(
			"OpenAPI_prev_sub_info_parseFromJSON() failed [subscription_id]");
		goto end;
	}
	...
	if(nf_ana_events) {
		...
		cJSON_ArrayForEach(nf_ana_events_local, nf_ana_events) {
			...
			OpenAPI_nwdaf_event_t *nf_ana_eventsItem =
	...
	if(ue_ana_events) {
		...
		if(!cJSON_IsArray(ue_ana_events)) {
			ogs_error(
				"OpenAPI_prev_sub_info_parseFromJSON() failed [ue_ana_events]");
			goto end;
		}
	...
	prev_sub_info_local_var = OpenAPI_prev_sub_info_create(
		producer_id &&
		!cJSON_IsNull(producer_id) ?
		ogs_strdup(producer_id->valuestring) : NULL,
		producer_set_id &&
		!cJSON_IsNull(producer_set_id) ?
		ogs_strdup(producer_set_id->valuestring) : NULL,
...
OpenAPI_prev_sub_info_t *OpenAPI_prev_sub_info_copy(
	OpenAPI_prev_sub_info_t *dst, OpenAPI_prev_sub_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
