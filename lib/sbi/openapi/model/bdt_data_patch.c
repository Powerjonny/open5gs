...
OpenAPI_bdt_data_patch_t *OpenAPI_bdt_data_patch_create(
	...
	OpenAPI_bdt_data_patch_t *bdt_data_patch_local_var =
		ogs_malloc(sizeof(OpenAPI_bdt_data_patch_t));
...
cJSON *OpenAPI_bdt_data_patch_convertToJSON(
	OpenAPI_bdt_data_patch_t *bdt_data_patch) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(bdt_data_patch == NULL) {
		ogs_error(
			"OpenAPI_bdt_data_patch_convertToJSON() failed [BdtDataPatch]");
		return NULL;
	}
	...
	if(bdt_data_patch->trans_policy) {
		cJSON *trans_policy_local_JSON =
			OpenAPI_transfer_policy_convertToJSON(
				bdt_data_patch->trans_policy);
		if(trans_policy_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_bdt_data_patch_convertToJSON() failed [trans_policy]");
			goto end;
		}
		cJSON_AddItemToObject(item, "transPolicy",
		                      trans_policy_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_bdt_data_patch_convertToJSON() failed [trans_policy]");
			goto end;
	...
	if(bdt_data_patch->bdtp_status) {
		cJSON *bdtp_status_local_JSON =
			OpenAPI_bdt_policy_status_convertToJSON(
				bdt_data_patch->bdtp_status);
		if(bdtp_status_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_bdt_data_patch_convertToJSON() failed [bdtp_status]");
			goto end;
		}
...
OpenAPI_bdt_data_patch_t *OpenAPI_bdt_data_patch_parseFromJSON(
	cJSON *bdt_data_patchJSON) {
	...
	trans_policy = cJSON_GetObjectItemCaseSensitive(bdt_data_patchJSON,
	                                                "transPolicy");
	if(trans_policy) {
	...
	bdtp_status = cJSON_GetObjectItemCaseSensitive(bdt_data_patchJSON,
	                                               "bdtpStatus");
	if(bdtp_status) {
...
OpenAPI_bdt_data_patch_t *OpenAPI_bdt_data_patch_copy(
	OpenAPI_bdt_data_patch_t *dst, OpenAPI_bdt_data_patch_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
