...
OpenAPI_as_time_distribution_param_t *OpenAPI_as_time_distribution_param_create(
	...
	OpenAPI_as_time_distribution_param_t *
		...
		ogs_malloc(sizeof(OpenAPI_as_time_distribution_param_t));
...
cJSON *OpenAPI_as_time_distribution_param_convertToJSON(
	OpenAPI_as_time_distribution_param_t *as_time_distribution_param) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(as_time_distribution_param == NULL) {
		ogs_error(
			"OpenAPI_as_time_distribution_param_convertToJSON() failed [AsTimeDistributionParam]");
		return NULL;
	}
	...
	if(as_time_distribution_param->is_as_time_dist_ind) {
		...
		{
			ogs_error(
				"OpenAPI_as_time_distribution_param_convertToJSON() failed [as_time_dist_ind]");
			goto end;
		}
	}
	...
	if(as_time_distribution_param->is_uu_error_budget) {
		...
		{
			ogs_error(
				"OpenAPI_as_time_distribution_param_convertToJSON() failed [uu_error_budget]");
			goto end;
		}
...
OpenAPI_as_time_distribution_param_parseFromJSON(
	...
	OpenAPI_as_time_distribution_param_t *
	        as_time_distribution_param_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *as_time_dist_ind = NULL;
	cJSON *uu_error_budget = NULL;
	as_time_dist_ind =
		cJSON_GetObjectItemCaseSensitive(as_time_distribution_paramJSON,
		                                 "asTimeDistInd");
	if(as_time_dist_ind) {
		if(!cJSON_IsBool(as_time_dist_ind)) {
			ogs_error(
				"OpenAPI_as_time_distribution_param_parseFromJSON() failed [as_time_dist_ind]");
			goto end;
		}
	}
	...
	if(uu_error_budget) {
		if(!cJSON_IsNull(uu_error_budget)) {
			if(!cJSON_IsNumber(uu_error_budget)) {
				ogs_error(
					"OpenAPI_as_time_distribution_param_parseFromJSON() failed [uu_error_budget]");
				goto end;
			}
		}
	}
	...
	as_time_distribution_param_local_var =
		OpenAPI_as_time_distribution_param_create(
...
OpenAPI_as_time_distribution_param_t *OpenAPI_as_time_distribution_param_copy(
	OpenAPI_as_time_distribution_param_t	*dst,
	OpenAPI_as_time_distribution_param_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_as_time_distribution_param_convertToJSON() failed");
		return NULL;
	}
