...
OpenAPI_smcce_ue_list_t *OpenAPI_smcce_ue_list_create(
	...
	OpenAPI_smcce_ue_list_t *smcce_ue_list_local_var =
		ogs_malloc(sizeof(OpenAPI_smcce_ue_list_t));
...
cJSON *OpenAPI_smcce_ue_list_convertToJSON(
	...
	if(smcce_ue_list == NULL) {
		ogs_error(
			"OpenAPI_smcce_ue_list_convertToJSON() failed [SmcceUeList]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(smcce_ue_list->high_level) {
		cJSON *high_levelList = cJSON_AddArrayToObject(item,
		                                               "highLevel");
		if(high_levelList == NULL) {
			ogs_error(
				"OpenAPI_smcce_ue_list_convertToJSON() failed [high_level]");
			goto end;
		}
		OpenAPI_list_for_each(smcce_ue_list->high_level, node) {
	...
	if(smcce_ue_list->medium_level) {
		...
		if(medium_levelList == NULL) {
			ogs_error(
				"OpenAPI_smcce_ue_list_convertToJSON() failed [medium_level]");
			goto end;
		}
		OpenAPI_list_for_each(smcce_ue_list->medium_level, node) {
	...
	if(smcce_ue_list->low_level) {
		...
		if(low_levelList == NULL) {
			ogs_error(
				"OpenAPI_smcce_ue_list_convertToJSON() failed [low_level]");
			goto end;
		}
		OpenAPI_list_for_each(smcce_ue_list->low_level, node) {
...
OpenAPI_smcce_ue_list_t *OpenAPI_smcce_ue_list_parseFromJSON(
	cJSON *smcce_ue_listJSON) {
	...
	if(high_level) {
		...
		if(!cJSON_IsArray(high_level)) {
			ogs_error(
				"OpenAPI_smcce_ue_list_parseFromJSON() failed [high_level]");
			goto end;
		}
		...
		cJSON_ArrayForEach(high_level_local, high_level) {
			...
			OpenAPI_list_add(high_levelList,
			                 ogs_strdup(
						 high_level_local->valuestring));
		}
	}
	...
	if(medium_level) {
		...
		if(!cJSON_IsArray(medium_level)) {
			ogs_error(
				"OpenAPI_smcce_ue_list_parseFromJSON() failed [medium_level]");
			goto end;
		}
		...
		cJSON_ArrayForEach(medium_level_local, medium_level) {
			...
			OpenAPI_list_add(medium_levelList,
			                 ogs_strdup(
						 medium_level_local->valuestring));
		}
	}
	...
	if(low_level) {
		...
		if(!cJSON_IsArray(low_level)) {
			ogs_error(
				"OpenAPI_smcce_ue_list_parseFromJSON() failed [low_level]");
			goto end;
		}
		...
		cJSON_ArrayForEach(low_level_local, low_level) {
			...
			OpenAPI_list_add(low_levelList,
			                 ogs_strdup(
						 low_level_local->valuestring));
		}
	}
...
OpenAPI_smcce_ue_list_t *OpenAPI_smcce_ue_list_copy(
	OpenAPI_smcce_ue_list_t *dst, OpenAPI_smcce_ue_list_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
