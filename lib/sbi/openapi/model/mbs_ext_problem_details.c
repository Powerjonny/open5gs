...
OpenAPI_mbs_ext_problem_details_t *OpenAPI_mbs_ext_problem_details_create(
	char *type, char *title, bool is_status, int status, char *detail,
	char *instance, char *cause, OpenAPI_list_t *invalid_params,
	char *supported_features,
	OpenAPI_access_token_err_t *access_token_error,
	OpenAPI_access_token_req_t *access_token_request, char *nrf_id,
	char *pcf_fqdn, OpenAPI_list_t *pcf_ip_end_points) {
	OpenAPI_mbs_ext_problem_details_t *mbs_ext_problem_details_local_var =
		ogs_malloc(sizeof(OpenAPI_mbs_ext_problem_details_t));
	...
	mbs_ext_problem_details_local_var->supported_features =
		supported_features;
	mbs_ext_problem_details_local_var->access_token_error =
		access_token_error;
	mbs_ext_problem_details_local_var->access_token_request =
		access_token_request;
	mbs_ext_problem_details_local_var->nrf_id = nrf_id;
	mbs_ext_problem_details_local_var->pcf_fqdn = pcf_fqdn;
	mbs_ext_problem_details_local_var->pcf_ip_end_points =
		pcf_ip_end_points;
...
void OpenAPI_mbs_ext_problem_details_free(
	...
	if(mbs_ext_problem_details->invalid_params) {
		OpenAPI_list_for_each(mbs_ext_problem_details->invalid_params,
		                      node) {
	...
	if(mbs_ext_problem_details->access_token_error) {
		OpenAPI_access_token_err_free(
			mbs_ext_problem_details->access_token_error);
		mbs_ext_problem_details->access_token_error = NULL;
	}
	if(mbs_ext_problem_details->access_token_request) {
		OpenAPI_access_token_req_free(
			mbs_ext_problem_details->access_token_request);
	...
	if(mbs_ext_problem_details->pcf_ip_end_points) {
		OpenAPI_list_for_each(
			mbs_ext_problem_details->pcf_ip_end_points, node) {
...
cJSON *OpenAPI_mbs_ext_problem_details_convertToJSON(
	OpenAPI_mbs_ext_problem_details_t *mbs_ext_problem_details) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(mbs_ext_problem_details == NULL) {
		ogs_error(
			"OpenAPI_mbs_ext_problem_details_convertToJSON() failed [MbsExtProblemDetails]");
		return NULL;
	}
	...
	if(mbs_ext_problem_details->type) {
		if(cJSON_AddStringToObject(item, "type",
		                           mbs_ext_problem_details->type) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_convertToJSON() failed [type]");
			goto end;
		}
	}
	...
	if(mbs_ext_problem_details->title) {
		if(cJSON_AddStringToObject(item, "title",
		                           mbs_ext_problem_details->title) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_convertToJSON() failed [title]");
			goto end;
		}
	}
	...
	if(mbs_ext_problem_details->is_status) {
		if(cJSON_AddNumberToObject(item, "status",
		                           mbs_ext_problem_details->status) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_convertToJSON() failed [status]");
			goto end;
		}
	}
	...
	if(mbs_ext_problem_details->detail) {
		if(cJSON_AddStringToObject(item, "detail",
		                           mbs_ext_problem_details->detail) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_convertToJSON() failed [detail]");
			goto end;
		}
	}
	...
	if(mbs_ext_problem_details->instance) {
		if(cJSON_AddStringToObject(item, "instance",
		                           mbs_ext_problem_details->instance) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_convertToJSON() failed [instance]");
			goto end;
		}
	}
	...
	if(mbs_ext_problem_details->cause) {
		if(cJSON_AddStringToObject(item, "cause",
		                           mbs_ext_problem_details->cause) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_convertToJSON() failed [cause]");
			goto end;
		}
	}
	...
	if(mbs_ext_problem_details->invalid_params) {
		...
		if(invalid_paramsList == NULL) {
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_convertToJSON() failed [invalid_params]");
			goto end;
		}
		OpenAPI_list_for_each(mbs_ext_problem_details->invalid_params,
		                      node) {
	...
	if(mbs_ext_problem_details->supported_features) {
		if(cJSON_AddStringToObject(item, "supportedFeatures",
		                           mbs_ext_problem_details->
		                           supported_features) == NULL)
		{
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(mbs_ext_problem_details->access_token_error) {
		cJSON *access_token_error_local_JSON =
			OpenAPI_access_token_err_convertToJSON(
				mbs_ext_problem_details->access_token_error);
		if(access_token_error_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_convertToJSON() failed [access_token_error]");
			goto end;
		}
	...
	if(mbs_ext_problem_details->access_token_request) {
		...
		if(access_token_request_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_convertToJSON() failed [access_token_request]");
			goto end;
		}
	...
	if(mbs_ext_problem_details->nrf_id) {
		if(cJSON_AddStringToObject(item, "nrfId",
		                           mbs_ext_problem_details->nrf_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_convertToJSON() failed [nrf_id]");
			goto end;
		}
	}
	...
	if(mbs_ext_problem_details->pcf_fqdn) {
		if(cJSON_AddStringToObject(item, "pcfFqdn",
		                           mbs_ext_problem_details->pcf_fqdn) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_convertToJSON() failed [pcf_fqdn]");
			goto end;
		}
	}
	...
	if(mbs_ext_problem_details->pcf_ip_end_points) {
		...
		if(pcf_ip_end_pointsList == NULL) {
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_convertToJSON() failed [pcf_ip_end_points]");
			goto end;
		}
		OpenAPI_list_for_each(
			mbs_ext_problem_details->pcf_ip_end_points, node) {
...
OpenAPI_mbs_ext_problem_details_t *OpenAPI_mbs_ext_problem_details_parseFromJSON
        (cJSON *mbs_ext_problem_detailsJSON) {
	OpenAPI_mbs_ext_problem_details_t *mbs_ext_problem_details_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	cJSON *type = NULL;
	...
	type = cJSON_GetObjectItemCaseSensitive(mbs_ext_problem_detailsJSON,
	                                        "type");
	if(type) {
		...
		{
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_parseFromJSON() failed [type]");
			goto end;
		}
	}

	title = cJSON_GetObjectItemCaseSensitive(mbs_ext_problem_detailsJSON,
	                                         "title");
	if(title) {
		...
		{
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_parseFromJSON() failed [title]");
			goto end;
		}
	}

	status = cJSON_GetObjectItemCaseSensitive(mbs_ext_problem_detailsJSON,
	                                          "status");
	if(status) {
		if(!cJSON_IsNumber(status)) {
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_parseFromJSON() failed [status]");
			goto end;
		}
	}

	detail = cJSON_GetObjectItemCaseSensitive(mbs_ext_problem_detailsJSON,
	                                          "detail");
	if(detail) {
		...
		{
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_parseFromJSON() failed [detail]");
			goto end;
		}
	}

	instance = cJSON_GetObjectItemCaseSensitive(mbs_ext_problem_detailsJSON,
	                                            "instance");
	if(instance) {
		...
		{
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_parseFromJSON() failed [instance]");
			goto end;
		}
	}

	cause = cJSON_GetObjectItemCaseSensitive(mbs_ext_problem_detailsJSON,
	                                         "cause");
	if(cause) {
		...
		{
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_parseFromJSON() failed [cause]");
			goto end;
		}
	}

	invalid_params =
		cJSON_GetObjectItemCaseSensitive(mbs_ext_problem_detailsJSON,
		                                 "invalidParams");
	if(invalid_params) {
		cJSON *invalid_params_local = NULL;
		if(!cJSON_IsArray(invalid_params)) {
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_parseFromJSON() failed [invalid_params]");
			goto end;
		}
		...
		cJSON_ArrayForEach(invalid_params_local, invalid_params) {
			...
			OpenAPI_invalid_param_t *invalid_paramsItem =
				OpenAPI_invalid_param_parseFromJSON(
					invalid_params_local);
			if(!invalid_paramsItem) {
				ogs_error("No invalid_paramsItem");
				goto end;
			}
			OpenAPI_list_add(invalid_paramsList,
			                 invalid_paramsItem);
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(access_token_error) {
		access_token_error_local_nonprim =
			OpenAPI_access_token_err_parseFromJSON(
				access_token_error);
		if(!access_token_error_local_nonprim) {
			ogs_error(
				"OpenAPI_access_token_err_parseFromJSON failed [access_token_error]");
			goto end;
		}
	}
	...
	if(access_token_request) {
		...
		if(!access_token_request_local_nonprim) {
			ogs_error(
				"OpenAPI_access_token_req_parseFromJSON failed [access_token_request]");
			goto end;
		}
	}

	nrf_id = cJSON_GetObjectItemCaseSensitive(mbs_ext_problem_detailsJSON,
	                                          "nrfId");
	if(nrf_id) {
		...
		{
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_parseFromJSON() failed [nrf_id]");
			goto end;
		}
	}

	pcf_fqdn = cJSON_GetObjectItemCaseSensitive(mbs_ext_problem_detailsJSON,
	                                            "pcfFqdn");
	if(pcf_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_parseFromJSON() failed [pcf_fqdn]");
			goto end;
		}
	}
	...
	if(pcf_ip_end_points) {
		...
		if(!cJSON_IsArray(pcf_ip_end_points)) {
			ogs_error(
				"OpenAPI_mbs_ext_problem_details_parseFromJSON() failed [pcf_ip_end_points]");
			goto end;
		}
	...
	mbs_ext_problem_details_local_var =
		OpenAPI_mbs_ext_problem_details_create(
			type &&
			!cJSON_IsNull(type) ? ogs_strdup(type->valuestring) :
			NULL,
			title &&
			!cJSON_IsNull(title) ? ogs_strdup(title->valuestring) :
			NULL,
			status ? true : false,
			status ? status->valuedouble : 0,
			detail &&
			!cJSON_IsNull(detail) ?
			ogs_strdup(detail->valuestring) : NULL,
			instance &&
			!cJSON_IsNull(instance) ?
			ogs_strdup(instance->valuestring) :
			NULL,
			cause &&
			!cJSON_IsNull(cause) ? ogs_strdup(cause->valuestring) :
			NULL,
			invalid_params ? invalid_paramsList : NULL,
			supported_features &&
			!cJSON_IsNull(supported_features) ?
			ogs_strdup(supported_features->valuestring) : NULL,
			access_token_error ? access_token_error_local_nonprim :
			NULL,
			access_token_request ?
			access_token_request_local_nonprim :
			NULL,
			nrf_id &&
			!cJSON_IsNull(nrf_id) ?
			ogs_strdup(nrf_id->valuestring) : NULL,
			pcf_fqdn &&
			!cJSON_IsNull(pcf_fqdn) ?
			ogs_strdup(pcf_fqdn->valuestring) :
			NULL,
...
OpenAPI_mbs_ext_problem_details_t *OpenAPI_mbs_ext_problem_details_copy(
	OpenAPI_mbs_ext_problem_details_t	*dst,
	OpenAPI_mbs_ext_problem_details_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_mbs_ext_problem_details_convertToJSON() failed");
		return NULL;
	}
