...
OpenAPI_mb_upf_info_t *OpenAPI_mb_upf_info_create(
	...
	OpenAPI_list_t *interface_mb_upf_info_list, OpenAPI_list_t *tai_list,
	OpenAPI_list_t *tai_range_list, bool is_priority, int priority,
	char *supported_pfcp_features) {
	OpenAPI_mb_upf_info_t *mb_upf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_mb_upf_info_t));
	ogs_assert(mb_upf_info_local_var);

	mb_upf_info_local_var->s_nssai_mb_upf_info_list =
		s_nssai_mb_upf_info_list;
	mb_upf_info_local_var->mb_smf_serving_area = mb_smf_serving_area;
	mb_upf_info_local_var->interface_mb_upf_info_list =
	...
	mb_upf_info_local_var->supported_pfcp_features =
...
void OpenAPI_mb_upf_info_free(OpenAPI_mb_upf_info_t *mb_upf_info) {
	...
	if(mb_upf_info->interface_mb_upf_info_list) {
		OpenAPI_list_for_each(mb_upf_info->interface_mb_upf_info_list,
		                      node) {
...
cJSON *OpenAPI_mb_upf_info_convertToJSON(OpenAPI_mb_upf_info_t *mb_upf_info) {
	...
	if(mb_upf_info == NULL) {
		ogs_error(
			"OpenAPI_mb_upf_info_convertToJSON() failed [MbUpfInfo]");
		return NULL;
	}
	...
	if(!mb_upf_info->s_nssai_mb_upf_info_list) {
		ogs_error(
			"OpenAPI_mb_upf_info_convertToJSON() failed [s_nssai_mb_upf_info_list]");
		return NULL;
	}
	cJSON *s_nssai_mb_upf_info_listList = cJSON_AddArrayToObject(item,
	                                                             "sNssaiMbUpfInfoList");
	...
	if(mb_upf_info->mb_smf_serving_area) {
		cJSON *mb_smf_serving_areaList = cJSON_AddArrayToObject(item,
		                                                        "mbSmfServingArea");
		if(mb_smf_serving_areaList == NULL) {
			ogs_error(
				"OpenAPI_mb_upf_info_convertToJSON() failed [mb_smf_serving_area]");
			goto end;
		}
		OpenAPI_list_for_each(mb_upf_info->mb_smf_serving_area, node) {
			if(cJSON_AddStringToObject(mb_smf_serving_areaList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_mb_upf_info_convertToJSON() failed [mb_smf_serving_area]");
	...
	if(mb_upf_info->interface_mb_upf_info_list) {
		cJSON *interface_mb_upf_info_listList =
			cJSON_AddArrayToObject(item, "interfaceMbUpfInfoList");
		if(interface_mb_upf_info_listList == NULL) {
			ogs_error(
				"OpenAPI_mb_upf_info_convertToJSON() failed [interface_mb_upf_info_list]");
			goto end;
		}
		OpenAPI_list_for_each(mb_upf_info->interface_mb_upf_info_list,
		                      node) {
			...
			cJSON_AddItemToArray(interface_mb_upf_info_listList,
			                     itemLocal);
		}
	}
	...
	if(mb_upf_info->tai_list) {
		...
		if(tai_listList == NULL) {
			ogs_error(
				"OpenAPI_mb_upf_info_convertToJSON() failed [tai_list]");
			goto end;
		}
		OpenAPI_list_for_each(mb_upf_info->tai_list, node) {
			cJSON *itemLocal =
	...
	if(mb_upf_info->tai_range_list) {
		...
		if(tai_range_listList == NULL) {
			ogs_error(
				"OpenAPI_mb_upf_info_convertToJSON() failed [tai_range_list]");
			goto end;
		}
		OpenAPI_list_for_each(mb_upf_info->tai_range_list, node) {
			cJSON *itemLocal =
	...
	if(mb_upf_info->is_priority) {
		if(cJSON_AddNumberToObject(item, "priority",
		                           mb_upf_info->priority) == NULL)
		{
			ogs_error(
				"OpenAPI_mb_upf_info_convertToJSON() failed [priority]");
			goto end;
		}
	}
	...
	if(mb_upf_info->supported_pfcp_features) {
		if(cJSON_AddStringToObject(item, "supportedPfcpFeatures",
		                           mb_upf_info->supported_pfcp_features)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_mb_upf_info_convertToJSON() failed [supported_pfcp_features]");
			goto end;
		}
	}
...
OpenAPI_mb_upf_info_t *OpenAPI_mb_upf_info_parseFromJSON(
	cJSON *mb_upf_infoJSON) {
	...
	s_nssai_mb_upf_info_list =
		cJSON_GetObjectItemCaseSensitive(mb_upf_infoJSON,
		                                 "sNssaiMbUpfInfoList");
	if(!s_nssai_mb_upf_info_list) {
		ogs_error(
			"OpenAPI_mb_upf_info_parseFromJSON() failed [s_nssai_mb_upf_info_list]");
		goto end;
	}
	...
	cJSON_ArrayForEach(s_nssai_mb_upf_info_list_local,
	                   s_nssai_mb_upf_info_list) {
		...
		OpenAPI_snssai_upf_info_item_t *s_nssai_mb_upf_info_listItem =
			OpenAPI_snssai_upf_info_item_parseFromJSON(
				s_nssai_mb_upf_info_list_local);
		if(!s_nssai_mb_upf_info_listItem) {
			ogs_error("No s_nssai_mb_upf_info_listItem");
			goto end;
		}
		OpenAPI_list_add(s_nssai_mb_upf_info_listList,
		                 s_nssai_mb_upf_info_listItem);
	}

	mb_smf_serving_area = cJSON_GetObjectItemCaseSensitive(mb_upf_infoJSON,
	                                                       "mbSmfServingArea");
	if(mb_smf_serving_area) {
		cJSON *mb_smf_serving_area_local = NULL;
		if(!cJSON_IsArray(mb_smf_serving_area)) {
			ogs_error(
				"OpenAPI_mb_upf_info_parseFromJSON() failed [mb_smf_serving_area]");
			goto end;
		}

		mb_smf_serving_areaList = OpenAPI_list_create();

		cJSON_ArrayForEach(mb_smf_serving_area_local,
		                   mb_smf_serving_area) {
			...
			OpenAPI_list_add(mb_smf_serving_areaList,
			                 ogs_strdup(
						 mb_smf_serving_area_local->
						 valuestring));
		}
	}
	...
	if(interface_mb_upf_info_list) {
		...
		if(!cJSON_IsArray(interface_mb_upf_info_list)) {
			ogs_error(
				"OpenAPI_mb_upf_info_parseFromJSON() failed [interface_mb_upf_info_list]");
			goto end;
		}
	...
	if(tai_list) {
		...
		if(!cJSON_IsArray(tai_list)) {
			ogs_error(
				"OpenAPI_mb_upf_info_parseFromJSON() failed [tai_list]");
			goto end;
		}
	...
	tai_range_list = cJSON_GetObjectItemCaseSensitive(mb_upf_infoJSON,
	                                                  "taiRangeList");
	if(tai_range_list) {
		cJSON *tai_range_list_local = NULL;
		if(!cJSON_IsArray(tai_range_list)) {
			ogs_error(
				"OpenAPI_mb_upf_info_parseFromJSON() failed [tai_range_list]");
			goto end;
		}
	...
	priority = cJSON_GetObjectItemCaseSensitive(mb_upf_infoJSON,
	                                            "priority");
	if(priority) {
		if(!cJSON_IsNumber(priority)) {
			ogs_error(
				"OpenAPI_mb_upf_info_parseFromJSON() failed [priority]");
			goto end;
		}
	}
	...
	if(supported_pfcp_features) {
		...
		{
			ogs_error(
				"OpenAPI_mb_upf_info_parseFromJSON() failed [supported_pfcp_features]");
			goto end;
		}
	}
	...
	mb_upf_info_local_var = OpenAPI_mb_upf_info_create(
		...
		interface_mb_upf_info_list ? interface_mb_upf_info_listList :
		NULL,
...
OpenAPI_mb_upf_info_t *OpenAPI_mb_upf_info_copy(OpenAPI_mb_upf_info_t	*dst,
                                                OpenAPI_mb_upf_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
