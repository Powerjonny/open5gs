...
OpenAPI_bsf_notification_t *OpenAPI_bsf_notification_create(
	char *notif_corre_id, char *pcf_id, char *pcf_set_id,
	OpenAPI_binding_level_e bind_level, OpenAPI_list_t *event_notifs) {
	OpenAPI_bsf_notification_t *bsf_notification_local_var =
...
void OpenAPI_bsf_notification_free(
	OpenAPI_bsf_notification_t *bsf_notification) {
...
cJSON *OpenAPI_bsf_notification_convertToJSON(
	OpenAPI_bsf_notification_t *bsf_notification) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(bsf_notification == NULL) {
		ogs_error(
			"OpenAPI_bsf_notification_convertToJSON() failed [BsfNotification]");
		return NULL;
	}
	...
	if(!bsf_notification->notif_corre_id) {
		ogs_error(
			"OpenAPI_bsf_notification_convertToJSON() failed [notif_corre_id]");
		return NULL;
	}
	...
	if(bsf_notification->pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_bsf_notification_convertToJSON() failed [pcf_id]");
			goto end;
		}
	}
	...
	if(bsf_notification->pcf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_bsf_notification_convertToJSON() failed [pcf_set_id]");
			goto end;
		}
	}
	...
	if(bsf_notification->bind_level != OpenAPI_binding_level_NULL) {
		if(cJSON_AddStringToObject(item, "bindLevel",
						   ...
						   bsf_notification->bind_level))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_bsf_notification_convertToJSON() failed [bind_level]");
			goto end;
		}
	}
	...
	if(!bsf_notification->event_notifs) {
		ogs_error(
			"OpenAPI_bsf_notification_convertToJSON() failed [event_notifs]");
		return NULL;
	}
...
OpenAPI_bsf_notification_t *OpenAPI_bsf_notification_parseFromJSON(
	cJSON *bsf_notificationJSON) {
	...
	notif_corre_id = cJSON_GetObjectItemCaseSensitive(bsf_notificationJSON,
	                                                  "notifCorreId");
	if(!notif_corre_id) {
		ogs_error(
			"OpenAPI_bsf_notification_parseFromJSON() failed [notif_corre_id]");
		goto end;
	}
	...
	pcf_id = cJSON_GetObjectItemCaseSensitive(bsf_notificationJSON,
	                                          "pcfId");
	if(pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_bsf_notification_parseFromJSON() failed [pcf_id]");
			goto end;
		}
	}

	pcf_set_id = cJSON_GetObjectItemCaseSensitive(bsf_notificationJSON,
	                                              "pcfSetId");
	if(pcf_set_id) {
		...
		{
			ogs_error(
				"OpenAPI_bsf_notification_parseFromJSON() failed [pcf_set_id]");
			goto end;
		}
	}

	bind_level = cJSON_GetObjectItemCaseSensitive(bsf_notificationJSON,
	                                              "bindLevel");
	if(bind_level) {
		if(!cJSON_IsString(bind_level)) {
			ogs_error(
				"OpenAPI_bsf_notification_parseFromJSON() failed [bind_level]");
			goto end;
		}
		bind_levelVariable =
	...
	event_notifs = cJSON_GetObjectItemCaseSensitive(bsf_notificationJSON,
	                                                "eventNotifs");
	if(!event_notifs) {
		ogs_error(
			"OpenAPI_bsf_notification_parseFromJSON() failed [event_notifs]");
		goto end;
	}
...
OpenAPI_bsf_notification_t *OpenAPI_bsf_notification_copy(
	OpenAPI_bsf_notification_t *dst, OpenAPI_bsf_notification_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
