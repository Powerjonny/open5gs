...
OpenAPI_sm_policy_data_t *OpenAPI_sm_policy_data_create(
	OpenAPI_list_t *sm_policy_snssai_data, OpenAPI_list_t *um_data_limits,
	OpenAPI_list_t *um_data, char *supp_feat, OpenAPI_list_t *reset_ids) {
	OpenAPI_sm_policy_data_t *sm_policy_data_local_var =
		ogs_malloc(sizeof(OpenAPI_sm_policy_data_t));
...
void OpenAPI_sm_policy_data_free(OpenAPI_sm_policy_data_t *sm_policy_data) {
	...
	if(sm_policy_data->sm_policy_snssai_data) {
		OpenAPI_list_for_each(sm_policy_data->sm_policy_snssai_data,
		                      node) {
			OpenAPI_map_t *localKeyValue =
				(OpenAPI_map_t *) node->data;
			ogs_free(localKeyValue->key);
			OpenAPI_sm_policy_snssai_data_free(
				localKeyValue->value);
...
cJSON *OpenAPI_sm_policy_data_convertToJSON(
	...
	if(sm_policy_data == NULL) {
		ogs_error(
			"OpenAPI_sm_policy_data_convertToJSON() failed [SmPolicyData]");
		return NULL;
	}
	...
	if(!sm_policy_data->sm_policy_snssai_data) {
		ogs_error(
			"OpenAPI_sm_policy_data_convertToJSON() failed [sm_policy_snssai_data]");
		return NULL;
	}
	cJSON *sm_policy_snssai_data = cJSON_AddObjectToObject(item,
	                                                       "smPolicySnssaiData");
	...
	if(sm_policy_data->sm_policy_snssai_data) {
		OpenAPI_list_for_each(sm_policy_data->sm_policy_snssai_data,
			...
			cJSON *itemLocal = localKeyValue->value ?
			                   OpenAPI_sm_policy_snssai_data_convertToJSON
			                           (localKeyValue->value) :
			                   cJSON_CreateNull();
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_sm_policy_data_convertToJSON() failed [inner]");
				goto end;
			}
	...
	if(sm_policy_data->um_data_limits) {
		cJSON *um_data_limits = cJSON_AddObjectToObject(item,
		                                                "umDataLimits");
		if(um_data_limits == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_data_convertToJSON() failed [um_data_limits]");
			goto end;
		}
		cJSON *localMapObject = um_data_limits;
		if(sm_policy_data->um_data_limits) {
			OpenAPI_list_for_each(sm_policy_data->um_data_limits,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				if(localKeyValue == NULL) {
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_usage_mon_data_limit_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
	...
	if(sm_policy_data->um_data) {
		...
		if(um_data == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_data_convertToJSON() failed [um_data]");
			goto end;
		}
	...
	if(sm_policy_data->supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_data_convertToJSON() failed [supp_feat]");
			goto end;
		}
	}
	...
	if(sm_policy_data->reset_ids) {
		...
		if(reset_idsList == NULL) {
			ogs_error(
				"OpenAPI_sm_policy_data_convertToJSON() failed [reset_ids]");
			goto end;
		}
		OpenAPI_list_for_each(sm_policy_data->reset_ids, node) {
...
OpenAPI_sm_policy_data_t *OpenAPI_sm_policy_data_parseFromJSON(
	cJSON *sm_policy_dataJSON) {
	...
	sm_policy_snssai_data =
		cJSON_GetObjectItemCaseSensitive(sm_policy_dataJSON,
		                                 "smPolicySnssaiData");
	if(!sm_policy_snssai_data) {
		ogs_error(
			"OpenAPI_sm_policy_data_parseFromJSON() failed [sm_policy_snssai_data]");
		goto end;
	}
	cJSON *sm_policy_snssai_data_local_map = NULL;
	if(!cJSON_IsObject(sm_policy_snssai_data) &&
	   !cJSON_IsNull(sm_policy_snssai_data))
	{
		ogs_error(
			"OpenAPI_sm_policy_data_parseFromJSON() failed [sm_policy_snssai_data]");
		goto end;
	...
	if(cJSON_IsObject(sm_policy_snssai_data)) {
		...
		cJSON_ArrayForEach(sm_policy_snssai_data_local_map,
		                   sm_policy_snssai_data) {
			...
			if(cJSON_IsObject(localMapObject)) {
				...
				ogs_error(
					"OpenAPI_sm_policy_data_parseFromJSON() failed [inner]");
				goto end;
			}
			OpenAPI_list_add(sm_policy_snssai_dataList,
			                 localMapKeyPair);
		}
	}

	um_data_limits = cJSON_GetObjectItemCaseSensitive(sm_policy_dataJSON,
	                                                  "umDataLimits");
	if(um_data_limits) {
		cJSON *um_data_limits_local_map = NULL;
		if(!cJSON_IsObject(um_data_limits) &&
		   !cJSON_IsNull(um_data_limits))
		{
			ogs_error(
				"OpenAPI_sm_policy_data_parseFromJSON() failed [um_data_limits]");
			goto end;
		}
		if(cJSON_IsObject(um_data_limits)) {
			um_data_limitsList = OpenAPI_list_create();
			OpenAPI_map_t *localMapKeyPair = NULL;
			cJSON_ArrayForEach(um_data_limits_local_map,
			                   um_data_limits) {
				...
				if(cJSON_IsObject(localMapObject)) {
					localMapKeyPair = OpenAPI_map_create(
						...
						OpenAPI_usage_mon_data_limit_parseFromJSON
						        (localMapObject));
				} else if(cJSON_IsNull(localMapObject)) {
					localMapKeyPair =
						OpenAPI_map_create(ogs_strdup(
				...
				OpenAPI_list_add(um_data_limitsList,
				                 localMapKeyPair);
			}
		}
	}

	um_data = cJSON_GetObjectItemCaseSensitive(sm_policy_dataJSON,
	                                           "umData");
	if(um_data) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_data_parseFromJSON() failed [um_data]");
			goto end;
		}
		if(cJSON_IsObject(um_data)) {
	...
	supp_feat = cJSON_GetObjectItemCaseSensitive(sm_policy_dataJSON,
	                                             "suppFeat");
	if(supp_feat) {
		...
		{
			ogs_error(
				"OpenAPI_sm_policy_data_parseFromJSON() failed [supp_feat]");
			goto end;
		}
	}

	reset_ids = cJSON_GetObjectItemCaseSensitive(sm_policy_dataJSON,
	                                             "resetIds");
	if(reset_ids) {
		cJSON *reset_ids_local = NULL;
		if(!cJSON_IsArray(reset_ids)) {
			ogs_error(
				"OpenAPI_sm_policy_data_parseFromJSON() failed [reset_ids]");
			goto end;
		}
...
OpenAPI_sm_policy_data_t *OpenAPI_sm_policy_data_copy(
	OpenAPI_sm_policy_data_t *dst, OpenAPI_sm_policy_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
