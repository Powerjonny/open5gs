...
OpenAPI_external_unrelated_class_t *OpenAPI_external_unrelated_class_create(
	OpenAPI_list_t *lcs_client_externals, OpenAPI_list_t *af_externals,
	OpenAPI_list_t *lcs_client_group_externals) {
	OpenAPI_external_unrelated_class_t *external_unrelated_class_local_var =
		ogs_malloc(sizeof(OpenAPI_external_unrelated_class_t));
	ogs_assert(external_unrelated_class_local_var);

	external_unrelated_class_local_var->lcs_client_externals =
		lcs_client_externals;
	external_unrelated_class_local_var->af_externals = af_externals;
	external_unrelated_class_local_var->lcs_client_group_externals =
...
void OpenAPI_external_unrelated_class_free(
	...
	if(external_unrelated_class->lcs_client_externals) {
		OpenAPI_list_for_each(
			external_unrelated_class->lcs_client_externals, node) {
			OpenAPI_lcs_client_external_free(node->data);
		}
		OpenAPI_list_free(
			external_unrelated_class->lcs_client_externals);
		external_unrelated_class->lcs_client_externals = NULL;
	}
	if(external_unrelated_class->af_externals) {
		OpenAPI_list_for_each(external_unrelated_class->af_externals,
		                      node) {
	...
	if(external_unrelated_class->lcs_client_group_externals) {
		OpenAPI_list_for_each(
			external_unrelated_class->lcs_client_group_externals,
			node) {
...
cJSON *OpenAPI_external_unrelated_class_convertToJSON(
	...
	if(external_unrelated_class == NULL) {
		ogs_error(
			"OpenAPI_external_unrelated_class_convertToJSON() failed [ExternalUnrelatedClass]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(external_unrelated_class->lcs_client_externals) {
		cJSON *lcs_client_externalsList = cJSON_AddArrayToObject(item,
		                                                         "lcsClientExternals");
		if(lcs_client_externalsList == NULL) {
			ogs_error(
				"OpenAPI_external_unrelated_class_convertToJSON() failed [lcs_client_externals]");
			goto end;
		}
		OpenAPI_list_for_each(
			external_unrelated_class->lcs_client_externals, node) {
			cJSON *itemLocal =
				OpenAPI_lcs_client_external_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_external_unrelated_class_convertToJSON() failed [lcs_client_externals]");
			...
			cJSON_AddItemToArray(lcs_client_externalsList,
			                     itemLocal);
		}
	}

	if(external_unrelated_class->af_externals) {
		cJSON *af_externalsList = cJSON_AddArrayToObject(item,
		                                                 "afExternals");
		if(af_externalsList == NULL) {
			ogs_error(
				"OpenAPI_external_unrelated_class_convertToJSON() failed [af_externals]");
			goto end;
		}
		OpenAPI_list_for_each(external_unrelated_class->af_externals,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_af_external_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_external_unrelated_class_convertToJSON() failed [af_externals]");
				goto end;
	...
	if(external_unrelated_class->lcs_client_group_externals) {
		cJSON *lcs_client_group_externalsList =
			cJSON_AddArrayToObject(item, "lcsClientGroupExternals");
		if(lcs_client_group_externalsList == NULL) {
			ogs_error(
				"OpenAPI_external_unrelated_class_convertToJSON() failed [lcs_client_group_externals]");
			goto end;
		}
		OpenAPI_list_for_each(
			external_unrelated_class->lcs_client_group_externals,
			node) {
			cJSON *itemLocal =
				OpenAPI_lcs_client_group_external_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_external_unrelated_class_convertToJSON() failed [lcs_client_group_externals]");
			...
			cJSON_AddItemToArray(lcs_client_group_externalsList,
			                     itemLocal);
		}
	}
...
OpenAPI_external_unrelated_class_parseFromJSON(
	cJSON *external_unrelated_classJSON) {
	OpenAPI_external_unrelated_class_t *external_unrelated_class_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	lcs_client_externals =
		cJSON_GetObjectItemCaseSensitive(external_unrelated_classJSON,
		                                 "lcsClientExternals");
	if(lcs_client_externals) {
		cJSON *lcs_client_externals_local = NULL;
		if(!cJSON_IsArray(lcs_client_externals)) {
			ogs_error(
				"OpenAPI_external_unrelated_class_parseFromJSON() failed [lcs_client_externals]");
			goto end;
		}

		lcs_client_externalsList = OpenAPI_list_create();

		cJSON_ArrayForEach(lcs_client_externals_local,
		                   lcs_client_externals) {
			...
			OpenAPI_lcs_client_external_t *lcs_client_externalsItem
			        = OpenAPI_lcs_client_external_parseFromJSON(
					  lcs_client_externals_local);
	...
	if(af_externals) {
		...
		if(!cJSON_IsArray(af_externals)) {
			ogs_error(
				"OpenAPI_external_unrelated_class_parseFromJSON() failed [af_externals]");
			goto end;
		}
		...
		cJSON_ArrayForEach(af_externals_local, af_externals) {
			...
			OpenAPI_af_external_t *af_externalsItem =
				OpenAPI_af_external_parseFromJSON(
					af_externals_local);
			if(!af_externalsItem) {
	...
	if(lcs_client_group_externals) {
		...
		if(!cJSON_IsArray(lcs_client_group_externals)) {
			ogs_error(
				"OpenAPI_external_unrelated_class_parseFromJSON() failed [lcs_client_group_externals]");
			goto end;
		}

		lcs_client_group_externalsList = OpenAPI_list_create();

		cJSON_ArrayForEach(lcs_client_group_externals_local,
		                   lcs_client_group_externals) {
	...
	external_unrelated_class_local_var =
		OpenAPI_external_unrelated_class_create(
			...
			lcs_client_group_externals ?
			lcs_client_group_externalsList :
			NULL
			);
...
OpenAPI_external_unrelated_class_t *OpenAPI_external_unrelated_class_copy(
	OpenAPI_external_unrelated_class_t	*dst,
	OpenAPI_external_unrelated_class_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_external_unrelated_class_convertToJSON() failed");
		return NULL;
	}
