...
OpenAPI_av_eps_aka_t *OpenAPI_av_eps_aka_create(OpenAPI_hss_av_type_e av_type,
	...
	OpenAPI_av_eps_aka_t *av_eps_aka_local_var =
		ogs_malloc(sizeof(OpenAPI_av_eps_aka_t));
...
cJSON *OpenAPI_av_eps_aka_convertToJSON(OpenAPI_av_eps_aka_t *av_eps_aka) {
	...
	if(av_eps_aka == NULL) {
		ogs_error(
			"OpenAPI_av_eps_aka_convertToJSON() failed [AvEpsAka]");
		return NULL;
	}
	...
	if(av_eps_aka->av_type == OpenAPI_hss_av_type_NULL) {
		ogs_error(
			"OpenAPI_av_eps_aka_convertToJSON() failed [av_type]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "avType",
	                           OpenAPI_hss_av_type_ToString(
					   av_eps_aka->av_type)) == NULL)
	{
		ogs_error(
			"OpenAPI_av_eps_aka_convertToJSON() failed [av_type]");
		goto end;
...
OpenAPI_av_eps_aka_t *OpenAPI_av_eps_aka_parseFromJSON(cJSON *av_eps_akaJSON) {
	...
	if(!av_type) {
		ogs_error(
			"OpenAPI_av_eps_aka_parseFromJSON() failed [av_type]");
		goto end;
	}
