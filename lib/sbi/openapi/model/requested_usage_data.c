...
OpenAPI_requested_usage_data_t *OpenAPI_requested_usage_data_create(
	OpenAPI_list_t *ref_um_ids, bool is_all_um_ids, int all_um_ids) {
	OpenAPI_requested_usage_data_t *requested_usage_data_local_var =
		ogs_malloc(sizeof(OpenAPI_requested_usage_data_t));
...
void OpenAPI_requested_usage_data_free(
	OpenAPI_requested_usage_data_t *requested_usage_data) {
...
cJSON *OpenAPI_requested_usage_data_convertToJSON(
	...
	if(requested_usage_data == NULL) {
		ogs_error(
			"OpenAPI_requested_usage_data_convertToJSON() failed [RequestedUsageData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(requested_usage_data->ref_um_ids) {
		cJSON *ref_um_idsList = cJSON_AddArrayToObject(item,
		                                               "refUmIds");
		if(ref_um_idsList == NULL) {
			ogs_error(
				"OpenAPI_requested_usage_data_convertToJSON() failed [ref_um_ids]");
			goto end;
		}
		OpenAPI_list_for_each(requested_usage_data->ref_um_ids, node) {
			if(cJSON_AddStringToObject(ref_um_idsList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_requested_usage_data_convertToJSON() failed [ref_um_ids]");
	...
	if(requested_usage_data->is_all_um_ids) {
		...
		{
			ogs_error(
				"OpenAPI_requested_usage_data_convertToJSON() failed [all_um_ids]");
			goto end;
		}
	}
...
OpenAPI_requested_usage_data_t *OpenAPI_requested_usage_data_parseFromJSON(
	...
	ref_um_ids = cJSON_GetObjectItemCaseSensitive(requested_usage_dataJSON,
	                                              "refUmIds");
	if(ref_um_ids) {
		cJSON *ref_um_ids_local = NULL;
		if(!cJSON_IsArray(ref_um_ids)) {
			ogs_error(
				"OpenAPI_requested_usage_data_parseFromJSON() failed [ref_um_ids]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ref_um_ids_local, ref_um_ids) {
			...
			OpenAPI_list_add(ref_um_idsList,
			                 ogs_strdup(
						 ref_um_ids_local->valuestring));
		}
	}

	all_um_ids = cJSON_GetObjectItemCaseSensitive(requested_usage_dataJSON,
	                                              "allUmIds");
	if(all_um_ids) {
		if(!cJSON_IsBool(all_um_ids)) {
			ogs_error(
				"OpenAPI_requested_usage_data_parseFromJSON() failed [all_um_ids]");
			goto end;
		}
	}
...
OpenAPI_requested_usage_data_t *OpenAPI_requested_usage_data_copy(
	OpenAPI_requested_usage_data_t	*dst,
	OpenAPI_requested_usage_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_requested_usage_data_convertToJSON() failed");
		return NULL;
	}
