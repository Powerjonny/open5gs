...
OpenAPI_steering_info_t *OpenAPI_steering_info_create(
	...
	OpenAPI_steering_info_t *steering_info_local_var =
		ogs_malloc(sizeof(OpenAPI_steering_info_t));
...
cJSON *OpenAPI_steering_info_convertToJSON(
	...
	if(steering_info == NULL) {
		ogs_error(
			"OpenAPI_steering_info_convertToJSON() failed [SteeringInfo]");
		return NULL;
	}
	...
	if(!steering_info->plmn_id) {
		ogs_error(
			"OpenAPI_steering_info_convertToJSON() failed [plmn_id]");
		return NULL;
	}
	cJSON *plmn_id_local_JSON =
		OpenAPI_plmn_id_convertToJSON(steering_info->plmn_id);
	...
	if(steering_info->access_tech_list) {
		cJSON *access_tech_listList = cJSON_AddArrayToObject(item,
		                                                     "accessTechList");
		if(access_tech_listList == NULL) {
			ogs_error(
				"OpenAPI_steering_info_convertToJSON() failed [access_tech_list]");
			goto end;
		}
		OpenAPI_list_for_each(steering_info->access_tech_list, node) {
			cJSON *itemLocal =
				OpenAPI_access_tech_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_steering_info_convertToJSON() failed [access_tech_list]");
				goto end;
...
OpenAPI_steering_info_t *OpenAPI_steering_info_parseFromJSON(
	...
	if(!plmn_id) {
		ogs_error(
			"OpenAPI_steering_info_parseFromJSON() failed [plmn_id]");
		goto end;
	}
	...
	access_tech_list = cJSON_GetObjectItemCaseSensitive(steering_infoJSON,
	                                                    "accessTechList");
	if(access_tech_list) {
		cJSON *access_tech_list_local = NULL;
		if(!cJSON_IsArray(access_tech_list)) {
			ogs_error(
				"OpenAPI_steering_info_parseFromJSON() failed [access_tech_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(access_tech_list_local, access_tech_list) {
			...
			OpenAPI_access_tech_t *access_tech_listItem =
				OpenAPI_access_tech_parseFromJSON(
					access_tech_list_local);
			if(!access_tech_listItem) {
				ogs_error("No access_tech_listItem");
				goto end;
			}
			OpenAPI_list_add(access_tech_listList,
			                 access_tech_listItem);
		}
	}
