...
OpenAPI_nwdaf_capability_t *OpenAPI_nwdaf_capability_create(
	...
	OpenAPI_nwdaf_capability_t *nwdaf_capability_local_var =
		ogs_malloc(sizeof(OpenAPI_nwdaf_capability_t));
	ogs_assert(nwdaf_capability_local_var);

	nwdaf_capability_local_var->is_analytics_aggregation =
		is_analytics_aggregation;
	nwdaf_capability_local_var->analytics_aggregation =
		analytics_aggregation;
	nwdaf_capability_local_var->is_analytics_metadata_provisioning =
		is_analytics_metadata_provisioning;
	nwdaf_capability_local_var->analytics_metadata_provisioning =
...
cJSON *OpenAPI_nwdaf_capability_convertToJSON(
	...
	if(nwdaf_capability == NULL) {
		ogs_error(
			"OpenAPI_nwdaf_capability_convertToJSON() failed [NwdafCapability]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(nwdaf_capability->is_analytics_aggregation) {
		if(cJSON_AddBoolToObject(item, "analyticsAggregation",
		                         nwdaf_capability->analytics_aggregation)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_nwdaf_capability_convertToJSON() failed [analytics_aggregation]");
			goto end;
		}
	}

	if(nwdaf_capability->is_analytics_metadata_provisioning) {
		if(cJSON_AddBoolToObject(item, "analyticsMetadataProvisioning",
		                         nwdaf_capability->
		                         analytics_metadata_provisioning) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_nwdaf_capability_convertToJSON() failed [analytics_metadata_provisioning]");
			goto end;
		}
	}
...
OpenAPI_nwdaf_capability_t *OpenAPI_nwdaf_capability_parseFromJSON(
	...
	analytics_aggregation =
		cJSON_GetObjectItemCaseSensitive(nwdaf_capabilityJSON,
		                                 "analyticsAggregation");
	if(analytics_aggregation) {
		if(!cJSON_IsBool(analytics_aggregation)) {
			ogs_error(
				"OpenAPI_nwdaf_capability_parseFromJSON() failed [analytics_aggregation]");
			goto end;
		}
	}
	...
	if(analytics_metadata_provisioning) {
		if(!cJSON_IsBool(analytics_metadata_provisioning)) {
			ogs_error(
				"OpenAPI_nwdaf_capability_parseFromJSON() failed [analytics_metadata_provisioning]");
			goto end;
		}
	}
	...
	nwdaf_capability_local_var = OpenAPI_nwdaf_capability_create(
		...
		analytics_metadata_provisioning ?
		analytics_metadata_provisioning->valueint : 0
		);
