...
OpenAPI_downlink_data_notification_control_rm_create(
	...
	OpenAPI_downlink_data_notification_control_rm_t *
		...
		ogs_malloc(
			sizeof(OpenAPI_downlink_data_notification_control_rm_t));
	ogs_assert(downlink_data_notification_control_rm_local_var);

	downlink_data_notification_control_rm_local_var->is_notif_ctrl_inds_null
	        = is_notif_ctrl_inds_null;
	downlink_data_notification_control_rm_local_var->notif_ctrl_inds =
		notif_ctrl_inds;
	downlink_data_notification_control_rm_local_var->is_types_of_notif_null
	        = is_types_of_notif_null;
	downlink_data_notification_control_rm_local_var->types_of_notif =
		types_of_notif;
...
cJSON *OpenAPI_downlink_data_notification_control_rm_convertToJSON(
	...
	if(downlink_data_notification_control_rm == NULL) {
		ogs_error(
			"OpenAPI_downlink_data_notification_control_rm_convertToJSON() failed [DownlinkDataNotificationControlRm]");
		return NULL;
	}
	   ...
	   OpenAPI_notification_control_indication_NULL)
	{
		...
		if(notif_ctrl_indsList == NULL) {
			ogs_error(
				"OpenAPI_downlink_data_notification_control_rm_convertToJSON() failed [notif_ctrl_inds]");
			goto end;
		}
	   ...
	   OpenAPI_dl_data_delivery_status_NULL)
	{
		...
		if(types_of_notifList == NULL) {
			ogs_error(
				"OpenAPI_downlink_data_notification_control_rm_convertToJSON() failed [types_of_notif]");
			goto end;
		}
		OpenAPI_list_for_each(
			downlink_data_notification_control_rm->types_of_notif,
...
OpenAPI_downlink_data_notification_control_rm_parseFromJSON(
	...
	OpenAPI_downlink_data_notification_control_rm_t *
	        downlink_data_notification_control_rm_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	notif_ctrl_inds =
		cJSON_GetObjectItemCaseSensitive(
			downlink_data_notification_control_rmJSON,
			"notifCtrlInds");
	if(notif_ctrl_inds) {
		if(!cJSON_IsNull(notif_ctrl_inds)) {
			...
			if(!cJSON_IsArray(notif_ctrl_inds)) {
				ogs_error(
					"OpenAPI_downlink_data_notification_control_rm_parseFromJSON() failed [notif_ctrl_inds]");
				goto end;
			}

			notif_ctrl_indsList = OpenAPI_list_create();

			cJSON_ArrayForEach(notif_ctrl_inds_local,
			                   notif_ctrl_inds) {
				OpenAPI_notification_control_indication_e
				        localEnum =
					OpenAPI_notification_control_indication_NULL;
			...
			if(notif_ctrl_indsList->count == 0) {
				ogs_error(
					"OpenAPI_downlink_data_notification_control_rm_parseFromJSON() failed: Expected notif_ctrl_indsList to not be empty (after ignoring unsupported enum values).");
				goto end;
			}
		}
	}
	...
	if(types_of_notif) {
		if(!cJSON_IsNull(types_of_notif)) {
			...
			if(!cJSON_IsArray(types_of_notif)) {
				ogs_error(
					"OpenAPI_downlink_data_notification_control_rm_parseFromJSON() failed [types_of_notif]");
				goto end;
			}

			types_of_notifList = OpenAPI_list_create();

			cJSON_ArrayForEach(types_of_notif_local,
			                   types_of_notif) {
			...
			if(types_of_notifList->count == 0) {
				ogs_error(
					"OpenAPI_downlink_data_notification_control_rm_parseFromJSON() failed: Expected types_of_notifList to not be empty (after ignoring unsupported enum values).");
				goto end;
			}
		}
	}
	...
	downlink_data_notification_control_rm_local_var =
		OpenAPI_downlink_data_notification_control_rm_create(
			notif_ctrl_inds &&
			cJSON_IsNull(notif_ctrl_inds) ? true : false,
			notif_ctrl_inds ? notif_ctrl_indsList : NULL,
			types_of_notif &&
			cJSON_IsNull(types_of_notif) ? true : false,
...
OpenAPI_downlink_data_notification_control_rm_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_downlink_data_notification_control_rm_convertToJSON() failed");
		return NULL;
	}
