...
OpenAPI_tngf_info_t *OpenAPI_tngf_info_create(
	...
	OpenAPI_tngf_info_t *tngf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_tngf_info_t));
...
void OpenAPI_tngf_info_free(OpenAPI_tngf_info_t *tngf_info) {
	...
	if(tngf_info->ipv4_endpoint_addresses) {
		OpenAPI_list_for_each(tngf_info->ipv4_endpoint_addresses,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(tngf_info->ipv6_endpoint_addresses) {
		OpenAPI_list_for_each(tngf_info->ipv6_endpoint_addresses,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_tngf_info_convertToJSON(OpenAPI_tngf_info_t *tngf_info) {
	...
	if(tngf_info == NULL) {
		ogs_error(
			"OpenAPI_tngf_info_convertToJSON() failed [TngfInfo]");
		return NULL;
	}
	...
	if(tngf_info->ipv4_endpoint_addresses) {
		...
		if(ipv4_endpoint_addressesList == NULL) {
			ogs_error(
				"OpenAPI_tngf_info_convertToJSON() failed [ipv4_endpoint_addresses]");
			goto end;
		}
		OpenAPI_list_for_each(tngf_info->ipv4_endpoint_addresses,
		                      node) {
	...
	if(tngf_info->ipv6_endpoint_addresses) {
		...
		if(ipv6_endpoint_addressesList == NULL) {
			ogs_error(
				"OpenAPI_tngf_info_convertToJSON() failed [ipv6_endpoint_addresses]");
			goto end;
		}
		OpenAPI_list_for_each(tngf_info->ipv6_endpoint_addresses,
		                      node) {
	...
	if(tngf_info->endpoint_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_tngf_info_convertToJSON() failed [endpoint_fqdn]");
			goto end;
		}
	}
...
OpenAPI_tngf_info_t *OpenAPI_tngf_info_parseFromJSON(cJSON *tngf_infoJSON) {
	...
	if(ipv6_endpoint_addresses) {
		...
		if(!cJSON_IsArray(ipv6_endpoint_addresses)) {
			ogs_error(
				"OpenAPI_tngf_info_parseFromJSON() failed [ipv6_endpoint_addresses]");
			goto end;
		}
	...
	endpoint_fqdn = cJSON_GetObjectItemCaseSensitive(tngf_infoJSON,
	                                                 "endpointFqdn");
	if(endpoint_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_tngf_info_parseFromJSON() failed [endpoint_fqdn]");
			goto end;
		}
	}
...
OpenAPI_tngf_info_t *OpenAPI_tngf_info_copy(OpenAPI_tngf_info_t *dst,
                                            OpenAPI_tngf_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
