...
OpenAPI_policy_data_subscription_t *OpenAPI_policy_data_subscription_create(
	char *notification_uri, char *notif_id,
	OpenAPI_list_t *monitored_resource_uris, OpenAPI_list_t *mon_res_items,
	OpenAPI_list_t *excluded_res_items, char *expiry,
	char *supported_features, OpenAPI_list_t *reset_ids) {
	OpenAPI_policy_data_subscription_t *policy_data_subscription_local_var =
		ogs_malloc(sizeof(OpenAPI_policy_data_subscription_t));
	...
	policy_data_subscription_local_var->monitored_resource_uris =
		monitored_resource_uris;
	policy_data_subscription_local_var->mon_res_items = mon_res_items;
	policy_data_subscription_local_var->excluded_res_items =
		excluded_res_items;
	policy_data_subscription_local_var->expiry = expiry;
	policy_data_subscription_local_var->supported_features =
		supported_features;
...
void OpenAPI_policy_data_subscription_free(
	OpenAPI_policy_data_subscription_t *policy_data_subscription) {
	...
	if(policy_data_subscription->monitored_resource_uris) {
		OpenAPI_list_for_each(
			policy_data_subscription->monitored_resource_uris,
			node) {
			ogs_free(node->data);
		}
		OpenAPI_list_free(
	...
	if(policy_data_subscription->mon_res_items) {
		OpenAPI_list_for_each(policy_data_subscription->mon_res_items,
		                      node) {
	...
	if(policy_data_subscription->excluded_res_items) {
		OpenAPI_list_for_each(
			policy_data_subscription->excluded_res_items, node) {
			OpenAPI_resource_item_free(node->data);
		}
	...
	if(policy_data_subscription->reset_ids) {
		OpenAPI_list_for_each(policy_data_subscription->reset_ids,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_policy_data_subscription_convertToJSON(
	OpenAPI_policy_data_subscription_t *policy_data_subscription) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(policy_data_subscription == NULL) {
		ogs_error(
			"OpenAPI_policy_data_subscription_convertToJSON() failed [PolicyDataSubscription]");
		return NULL;
	}
	...
	if(!policy_data_subscription->notification_uri) {
		ogs_error(
			"OpenAPI_policy_data_subscription_convertToJSON() failed [notification_uri]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "notificationUri",
	                           policy_data_subscription->notification_uri)
	   == NULL)
	{
		ogs_error(
			"OpenAPI_policy_data_subscription_convertToJSON() failed [notification_uri]");
	...
	if(policy_data_subscription->notif_id) {
		if(cJSON_AddStringToObject(item, "notifId",
		                           policy_data_subscription->notif_id)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_policy_data_subscription_convertToJSON() failed [notif_id]");
			goto end;
		}
	}
	...
	if(!policy_data_subscription->monitored_resource_uris) {
		ogs_error(
			"OpenAPI_policy_data_subscription_convertToJSON() failed [monitored_resource_uris]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(policy_data_subscription->monitored_resource_uris,
	                      node) {
	...
	if(policy_data_subscription->mon_res_items) {
		cJSON *mon_res_itemsList = cJSON_AddArrayToObject(item,
		                                                  "monResItems");
		if(mon_res_itemsList == NULL) {
			ogs_error(
				"OpenAPI_policy_data_subscription_convertToJSON() failed [mon_res_items]");
			goto end;
		}
		OpenAPI_list_for_each(policy_data_subscription->mon_res_items,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_resource_item_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_policy_data_subscription_convertToJSON() failed [mon_res_items]");
				goto end;
	...
	if(policy_data_subscription->excluded_res_items) {
		cJSON *excluded_res_itemsList = cJSON_AddArrayToObject(item,
		                                                       "excludedResItems");
		if(excluded_res_itemsList == NULL) {
			ogs_error(
				"OpenAPI_policy_data_subscription_convertToJSON() failed [excluded_res_items]");
			goto end;
		}
		OpenAPI_list_for_each(
			policy_data_subscription->excluded_res_items, node) {
	...
	if(policy_data_subscription->expiry) {
		if(cJSON_AddStringToObject(item, "expiry",
		                           policy_data_subscription->expiry) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_policy_data_subscription_convertToJSON() failed [expiry]");
			goto end;
		}
	}
	...
	if(policy_data_subscription->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_policy_data_subscription_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(policy_data_subscription->reset_ids) {
		...
		if(reset_idsList == NULL) {
			ogs_error(
				"OpenAPI_policy_data_subscription_convertToJSON() failed [reset_ids]");
			goto end;
		}
		OpenAPI_list_for_each(policy_data_subscription->reset_ids,
		                      node) {
...
OpenAPI_policy_data_subscription_parseFromJSON(
	cJSON *policy_data_subscriptionJSON) {
	OpenAPI_policy_data_subscription_t *policy_data_subscription_local_var =
		NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	notification_uri =
		cJSON_GetObjectItemCaseSensitive(policy_data_subscriptionJSON,
		                                 "notificationUri");
	if(!notification_uri) {
		ogs_error(
			"OpenAPI_policy_data_subscription_parseFromJSON() failed [notification_uri]");
		goto end;
	}
	...
	if(notif_id) {
		...
		{
			ogs_error(
				"OpenAPI_policy_data_subscription_parseFromJSON() failed [notif_id]");
			goto end;
		}
	}
	...
	if(!monitored_resource_uris) {
		ogs_error(
			"OpenAPI_policy_data_subscription_parseFromJSON() failed [monitored_resource_uris]");
		goto end;
	}
	...
	if(mon_res_items) {
		...
		cJSON_ArrayForEach(mon_res_items_local, mon_res_items) {
			...
			OpenAPI_resource_item_t *mon_res_itemsItem =
	...
	if(excluded_res_items) {
		...
		if(!cJSON_IsArray(excluded_res_items)) {
			ogs_error(
				"OpenAPI_policy_data_subscription_parseFromJSON() failed [excluded_res_items]");
			goto end;
		}

		excluded_res_itemsList = OpenAPI_list_create();

		cJSON_ArrayForEach(excluded_res_items_local,
		                   excluded_res_items) {
			...
			OpenAPI_resource_item_t *excluded_res_itemsItem =
				OpenAPI_resource_item_parseFromJSON(
					excluded_res_items_local);
			if(!excluded_res_itemsItem) {
				ogs_error("No excluded_res_itemsItem");
				goto end;
			}
			OpenAPI_list_add(excluded_res_itemsList,
			                 excluded_res_itemsItem);
		}
	}

	expiry = cJSON_GetObjectItemCaseSensitive(policy_data_subscriptionJSON,
	                                          "expiry");
	if(expiry) {
		...
		{
			ogs_error(
				"OpenAPI_policy_data_subscription_parseFromJSON() failed [expiry]");
			goto end;
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_policy_data_subscription_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(reset_ids) {
		...
		if(!cJSON_IsArray(reset_ids)) {
			ogs_error(
				"OpenAPI_policy_data_subscription_parseFromJSON() failed [reset_ids]");
			goto end;
		}
...
OpenAPI_policy_data_subscription_t *OpenAPI_policy_data_subscription_copy(
	OpenAPI_policy_data_subscription_t	*dst,
	OpenAPI_policy_data_subscription_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_policy_data_subscription_convertToJSON() failed");
		return NULL;
	}
