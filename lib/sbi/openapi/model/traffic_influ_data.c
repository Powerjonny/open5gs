...
OpenAPI_traffic_influ_data_t *OpenAPI_traffic_influ_data_create(
	char *up_path_chg_notif_corre_id, bool is_app_relo_ind,
	int app_relo_ind, char *af_app_id, char *dnn,
	OpenAPI_list_t *eth_traffic_filters, OpenAPI_snssai_t *snssai,
	char *inter_group_id, char *supi, OpenAPI_list_t *traffic_filters,
	OpenAPI_list_t *traffic_routes, bool is_traff_corre_ind,
	int traff_corre_ind, char *valid_start_time, char *valid_end_time,
	OpenAPI_list_t *temp_validities,
	OpenAPI_network_area_info_2_t *nw_area_info,
	char *up_path_chg_notif_uri, OpenAPI_list_t *headers,
	OpenAPI_list_t *subscribed_events,
	OpenAPI_dnai_change_type_e dnai_chg_type, bool is_af_ack_ind,
	int af_ack_ind, bool is_addr_preser_ind, int addr_preser_ind,
	bool is_max_allowed_up_lat, int max_allowed_up_lat,
	bool is_sim_conn_ind, int sim_conn_ind, bool is_sim_conn_term,
	int sim_conn_term, char *supported_features, char *res_uri,
	OpenAPI_list_t *reset_ids) {
	OpenAPI_traffic_influ_data_t *traffic_influ_data_local_var =
		ogs_malloc(sizeof(OpenAPI_traffic_influ_data_t));
	ogs_assert(traffic_influ_data_local_var);

	traffic_influ_data_local_var->up_path_chg_notif_corre_id =
		up_path_chg_notif_corre_id;
	...
	traffic_influ_data_local_var->up_path_chg_notif_uri =
		up_path_chg_notif_uri;
	...
	traffic_influ_data_local_var->is_max_allowed_up_lat =
...
void OpenAPI_traffic_influ_data_free(
	...
	if(traffic_influ_data->eth_traffic_filters) {
		OpenAPI_list_for_each(traffic_influ_data->eth_traffic_filters,
		                      node) {
	...
	if(traffic_influ_data->traffic_filters) {
		OpenAPI_list_for_each(traffic_influ_data->traffic_filters,
		                      node) {
	...
	if(traffic_influ_data->traffic_routes) {
		OpenAPI_list_for_each(traffic_influ_data->traffic_routes,
		                      node) {
	...
	if(traffic_influ_data->temp_validities) {
		OpenAPI_list_for_each(traffic_influ_data->temp_validities,
		                      node) {
	...
	if(traffic_influ_data->nw_area_info) {
		OpenAPI_network_area_info_2_free(
			traffic_influ_data->nw_area_info);
	...
	if(traffic_influ_data->subscribed_events) {
		OpenAPI_list_for_each(traffic_influ_data->subscribed_events,
		                      node) {
...
cJSON *OpenAPI_traffic_influ_data_convertToJSON(
	OpenAPI_traffic_influ_data_t *traffic_influ_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(traffic_influ_data == NULL) {
		ogs_error(
			"OpenAPI_traffic_influ_data_convertToJSON() failed [TrafficInfluData]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(traffic_influ_data->up_path_chg_notif_corre_id) {
		if(cJSON_AddStringToObject(item, "upPathChgNotifCorreId",
		                           traffic_influ_data->
		                           up_path_chg_notif_corre_id) == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [up_path_chg_notif_corre_id]");
			goto end;
		}
	}

	if(traffic_influ_data->is_app_relo_ind) {
		if(cJSON_AddBoolToObject(item, "appReloInd",
		                         traffic_influ_data->app_relo_ind) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [app_relo_ind]");
			goto end;
		}
	}
	...
	if(traffic_influ_data->af_app_id) {
		if(cJSON_AddStringToObject(item, "afAppId",
		                           traffic_influ_data->af_app_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [af_app_id]");
			goto end;
		}
	}
	...
	if(traffic_influ_data->dnn) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [dnn]");
			goto end;
		}
	}

	if(traffic_influ_data->eth_traffic_filters) {
		cJSON *eth_traffic_filtersList = cJSON_AddArrayToObject(item,
		                                                        "ethTrafficFilters");
		if(eth_traffic_filtersList == NULL) {
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [eth_traffic_filters]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_influ_data->eth_traffic_filters,
		                      node) {
			...
			cJSON_AddItemToArray(eth_traffic_filtersList,
			                     itemLocal);
		}
	}
	...
	if(traffic_influ_data->snssai) {
		cJSON *snssai_local_JSON =
			OpenAPI_snssai_convertToJSON(
				traffic_influ_data->snssai);
		if(snssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [snssai]");
			goto end;
		}
	...
	if(traffic_influ_data->inter_group_id) {
		if(cJSON_AddStringToObject(item, "interGroupId",
		                           traffic_influ_data->inter_group_id)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [inter_group_id]");
			goto end;
		}
	}
	...
	if(traffic_influ_data->supi) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [supi]");
			goto end;
		}
	}

	if(traffic_influ_data->traffic_filters) {
		cJSON *traffic_filtersList = cJSON_AddArrayToObject(item,
		                                                    "trafficFilters");
		if(traffic_filtersList == NULL) {
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [traffic_filters]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_influ_data->traffic_filters,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_flow_info_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_traffic_influ_data_convertToJSON() failed [traffic_filters]");
				goto end;
	...
	if(traffic_influ_data->traffic_routes) {
		cJSON *traffic_routesList = cJSON_AddArrayToObject(item,
		                                                   "trafficRoutes");
		if(traffic_routesList == NULL) {
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [traffic_routes]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_influ_data->traffic_routes,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_route_to_location_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_traffic_influ_data_convertToJSON() failed [traffic_routes]");
	...
	if(traffic_influ_data->is_traff_corre_ind) {
		if(cJSON_AddBoolToObject(item, "traffCorreInd",
		                         traffic_influ_data->traff_corre_ind) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [traff_corre_ind]");
			goto end;
		}
	}

	if(traffic_influ_data->valid_start_time) {
		if(cJSON_AddStringToObject(item, "validStartTime",
		                           traffic_influ_data->valid_start_time)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [valid_start_time]");
			goto end;
		}
	}

	if(traffic_influ_data->valid_end_time) {
		if(cJSON_AddStringToObject(item, "validEndTime",
		                           traffic_influ_data->valid_end_time)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [valid_end_time]");
			goto end;
		}
	}

	if(traffic_influ_data->temp_validities) {
		cJSON *temp_validitiesList = cJSON_AddArrayToObject(item,
		                                                    "tempValidities");
		if(temp_validitiesList == NULL) {
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [temp_validities]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_influ_data->temp_validities,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_temporal_validity_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_traffic_influ_data_convertToJSON() failed [temp_validities]");
	...
	if(traffic_influ_data->nw_area_info) {
		cJSON *nw_area_info_local_JSON =
			OpenAPI_network_area_info_2_convertToJSON(
				traffic_influ_data->nw_area_info);
		if(nw_area_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [nw_area_info]");
			goto end;
		}
	...
	if(traffic_influ_data->up_path_chg_notif_uri) {
		if(cJSON_AddStringToObject(item, "upPathChgNotifUri",
		                           traffic_influ_data->
		                           up_path_chg_notif_uri) == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [up_path_chg_notif_uri]");
			goto end;
		}
	}
	...
	if(traffic_influ_data->headers) {
		...
		if(headersList == NULL) {
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [headers]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_influ_data->headers, node) {
			if(cJSON_AddStringToObject(headersList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_traffic_influ_data_convertToJSON() failed [headers]");
	...
	if(traffic_influ_data->subscribed_events) {
		cJSON *subscribed_eventsList = cJSON_AddArrayToObject(item,
		                                                      "subscribedEvents");
		if(subscribed_eventsList == NULL) {
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [subscribed_events]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_influ_data->subscribed_events,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_subscribed_event_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_traffic_influ_data_convertToJSON() failed [subscribed_events]");
	...
	if(traffic_influ_data->dnai_chg_type != OpenAPI_dnai_change_type_NULL) {
		if(cJSON_AddStringToObject(item, "dnaiChgType",
		                           OpenAPI_dnai_change_type_ToString(
						   traffic_influ_data->
						   dnai_chg_type)) == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [dnai_chg_type]");
			goto end;
		}
	}

	if(traffic_influ_data->is_af_ack_ind) {
		if(cJSON_AddBoolToObject(item, "afAckInd",
		                         traffic_influ_data->af_ack_ind) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [af_ack_ind]");
			goto end;
		}
	}

	if(traffic_influ_data->is_addr_preser_ind) {
		if(cJSON_AddBoolToObject(item, "addrPreserInd",
		                         traffic_influ_data->addr_preser_ind) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [addr_preser_ind]");
			goto end;
		}
	}

	if(traffic_influ_data->is_max_allowed_up_lat) {
		if(cJSON_AddNumberToObject(item, "maxAllowedUpLat",
		                           traffic_influ_data->
		                           max_allowed_up_lat) == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [max_allowed_up_lat]");
			goto end;
		}
	}

	if(traffic_influ_data->is_sim_conn_ind) {
		if(cJSON_AddBoolToObject(item, "simConnInd",
		                         traffic_influ_data->sim_conn_ind) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [sim_conn_ind]");
			goto end;
		}
	}

	if(traffic_influ_data->is_sim_conn_term) {
		if(cJSON_AddNumberToObject(item, "simConnTerm",
		                           traffic_influ_data->sim_conn_term) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [sim_conn_term]");
			goto end;
		}
	}
	...
	if(traffic_influ_data->supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}

	if(traffic_influ_data->res_uri) {
		if(cJSON_AddStringToObject(item, "resUri",
		                           traffic_influ_data->res_uri) == NULL)
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [res_uri]");
			goto end;
		}
	}
	...
	if(traffic_influ_data->reset_ids) {
		...
		if(reset_idsList == NULL) {
			ogs_error(
				"OpenAPI_traffic_influ_data_convertToJSON() failed [reset_ids]");
			goto end;
		}
		OpenAPI_list_for_each(traffic_influ_data->reset_ids, node) {
...
OpenAPI_traffic_influ_data_t *OpenAPI_traffic_influ_data_parseFromJSON(
	cJSON *traffic_influ_dataJSON) {
	...
	up_path_chg_notif_corre_id =
		cJSON_GetObjectItemCaseSensitive(traffic_influ_dataJSON,
		                                 "upPathChgNotifCorreId");
	if(up_path_chg_notif_corre_id) {
		if(!cJSON_IsString(up_path_chg_notif_corre_id) &&
		   !cJSON_IsNull(up_path_chg_notif_corre_id))
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [up_path_chg_notif_corre_id]");
			goto end;
		}
	}

	app_relo_ind = cJSON_GetObjectItemCaseSensitive(traffic_influ_dataJSON,
	                                                "appReloInd");
	if(app_relo_ind) {
		if(!cJSON_IsBool(app_relo_ind)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [app_relo_ind]");
			goto end;
		}
	}

	af_app_id = cJSON_GetObjectItemCaseSensitive(traffic_influ_dataJSON,
	                                             "afAppId");
	if(af_app_id) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [af_app_id]");
			goto end;
		}
	}
	...
	if(dnn) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [dnn]");
			goto end;
		}
	}
	...
	if(eth_traffic_filters) {
		...
		if(!cJSON_IsArray(eth_traffic_filters)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [eth_traffic_filters]");
			goto end;
		}

		eth_traffic_filtersList = OpenAPI_list_create();

		cJSON_ArrayForEach(eth_traffic_filters_local,
		                   eth_traffic_filters) {
			...
			OpenAPI_eth_flow_description_t *eth_traffic_filtersItem
			        = OpenAPI_eth_flow_description_parseFromJSON(
					  eth_traffic_filters_local);
			if(!eth_traffic_filtersItem) {
				ogs_error("No eth_traffic_filtersItem");
				goto end;
			}
			OpenAPI_list_add(eth_traffic_filtersList,
			                 eth_traffic_filtersItem);
		}
	}

	snssai = cJSON_GetObjectItemCaseSensitive(traffic_influ_dataJSON,
	                                          "snssai");
	if(snssai) {
	...
	if(inter_group_id) {
		if(!cJSON_IsString(inter_group_id) &&
		   !cJSON_IsNull(inter_group_id))
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [inter_group_id]");
			goto end;
		}
	}
	...
	if(supi) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(traffic_filters) {
		...
		if(!cJSON_IsArray(traffic_filters)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [traffic_filters]");
			goto end;
		}
		...
		cJSON_ArrayForEach(traffic_filters_local, traffic_filters) {
			...
			OpenAPI_flow_info_t *traffic_filtersItem =
				OpenAPI_flow_info_parseFromJSON(
					traffic_filters_local);
			if(!traffic_filtersItem) {
				ogs_error("No traffic_filtersItem");
				goto end;
			}
			OpenAPI_list_add(traffic_filtersList,
			                 traffic_filtersItem);
		}
	}
	...
	if(traffic_routes) {
		...
		if(!cJSON_IsArray(traffic_routes)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [traffic_routes]");
			goto end;
		}
		...
		cJSON_ArrayForEach(traffic_routes_local, traffic_routes) {
			...
			OpenAPI_route_to_location_t *traffic_routesItem =
				OpenAPI_route_to_location_parseFromJSON(
					traffic_routes_local);
			if(!traffic_routesItem) {
				ogs_error("No traffic_routesItem");
				goto end;
			}
			OpenAPI_list_add(traffic_routesList,
			                 traffic_routesItem);
		}
	}
	...
	if(traff_corre_ind) {
		if(!cJSON_IsBool(traff_corre_ind)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [traff_corre_ind]");
			goto end;
		}
	}
	...
	if(valid_start_time) {
		if(!cJSON_IsString(valid_start_time) &&
		   !cJSON_IsNull(valid_start_time))
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [valid_start_time]");
			goto end;
		}
	}
	...
	if(valid_end_time) {
		if(!cJSON_IsString(valid_end_time) &&
		   !cJSON_IsNull(valid_end_time))
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [valid_end_time]");
			goto end;
		}
	}
	...
	if(temp_validities) {
		...
		if(!cJSON_IsArray(temp_validities)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [temp_validities]");
			goto end;
		}
		...
		cJSON_ArrayForEach(temp_validities_local, temp_validities) {
			...
			OpenAPI_temporal_validity_t *temp_validitiesItem =
				OpenAPI_temporal_validity_parseFromJSON(
					temp_validities_local);
			if(!temp_validitiesItem) {
				ogs_error("No temp_validitiesItem");
				goto end;
			}
			OpenAPI_list_add(temp_validitiesList,
			                 temp_validitiesItem);
		}
	}

	nw_area_info = cJSON_GetObjectItemCaseSensitive(traffic_influ_dataJSON,
	                                                "nwAreaInfo");
	if(nw_area_info) {
		nw_area_info_local_nonprim =
			OpenAPI_network_area_info_2_parseFromJSON(nw_area_info);
		if(!nw_area_info_local_nonprim) {
			ogs_error(
				"OpenAPI_network_area_info_2_parseFromJSON failed [nw_area_info]");
			goto end;
		}
	}
	...
	if(up_path_chg_notif_uri) {
		if(!cJSON_IsString(up_path_chg_notif_uri) &&
		   !cJSON_IsNull(up_path_chg_notif_uri))
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [up_path_chg_notif_uri]");
			goto end;
		}
	}

	headers = cJSON_GetObjectItemCaseSensitive(traffic_influ_dataJSON,
	                                           "headers");
	if(headers) {
		cJSON *headers_local = NULL;
		if(!cJSON_IsArray(headers)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [headers]");
			goto end;
		}
		...
		cJSON_ArrayForEach(headers_local, headers) {
			...
			OpenAPI_list_add(headersList,
			                 ogs_strdup(
						 headers_local->valuestring));
		}
	}
	...
	if(subscribed_events) {
		...
		if(!cJSON_IsArray(subscribed_events)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [subscribed_events]");
			goto end;
		}
		...
		cJSON_ArrayForEach(subscribed_events_local, subscribed_events) {
			...
			OpenAPI_subscribed_event_t *subscribed_eventsItem =
				OpenAPI_subscribed_event_parseFromJSON(
					subscribed_events_local);
			if(!subscribed_eventsItem) {
				ogs_error("No subscribed_eventsItem");
				goto end;
			}
			OpenAPI_list_add(subscribed_eventsList,
			                 subscribed_eventsItem);
		}
	}

	dnai_chg_type = cJSON_GetObjectItemCaseSensitive(traffic_influ_dataJSON,
	                                                 "dnaiChgType");
	if(dnai_chg_type) {
		if(!cJSON_IsString(dnai_chg_type)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [dnai_chg_type]");
			goto end;
		}
		dnai_chg_typeVariable =
			OpenAPI_dnai_change_type_FromString(
				dnai_chg_type->valuestring);
	}

	af_ack_ind = cJSON_GetObjectItemCaseSensitive(traffic_influ_dataJSON,
	                                              "afAckInd");
	if(af_ack_ind) {
		if(!cJSON_IsBool(af_ack_ind)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [af_ack_ind]");
			goto end;
		}
	}
	...
	if(addr_preser_ind) {
		if(!cJSON_IsBool(addr_preser_ind)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [addr_preser_ind]");
			goto end;
		}
	}
	...
	if(max_allowed_up_lat) {
		if(!cJSON_IsNumber(max_allowed_up_lat)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [max_allowed_up_lat]");
			goto end;
		}
	}

	sim_conn_ind = cJSON_GetObjectItemCaseSensitive(traffic_influ_dataJSON,
	                                                "simConnInd");
	if(sim_conn_ind) {
		if(!cJSON_IsBool(sim_conn_ind)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [sim_conn_ind]");
			goto end;
		}
	}

	sim_conn_term = cJSON_GetObjectItemCaseSensitive(traffic_influ_dataJSON,
	                                                 "simConnTerm");
	if(sim_conn_term) {
		if(!cJSON_IsNumber(sim_conn_term)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [sim_conn_term]");
			goto end;
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}

	res_uri = cJSON_GetObjectItemCaseSensitive(traffic_influ_dataJSON,
	                                           "resUri");
	if(res_uri) {
		if(!cJSON_IsString(res_uri) &&
		   !cJSON_IsNull(res_uri))
		{
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [res_uri]");
			goto end;
		}
	}

	reset_ids = cJSON_GetObjectItemCaseSensitive(traffic_influ_dataJSON,
	                                             "resetIds");
	if(reset_ids) {
		cJSON *reset_ids_local = NULL;
		if(!cJSON_IsArray(reset_ids)) {
			ogs_error(
				"OpenAPI_traffic_influ_data_parseFromJSON() failed [reset_ids]");
			goto end;
		}
	...
	traffic_influ_data_local_var = OpenAPI_traffic_influ_data_create(
		up_path_chg_notif_corre_id &&
		!cJSON_IsNull(up_path_chg_notif_corre_id) ?
		ogs_strdup(up_path_chg_notif_corre_id->valuestring) : NULL,
		...
		NULL,
		dnn &&
		!cJSON_IsNull(dnn) ? ogs_strdup(dnn->valuestring) : NULL,
		eth_traffic_filters ? eth_traffic_filtersList : NULL,
		snssai ? snssai_local_nonprim : NULL,
		inter_group_id &&
		!cJSON_IsNull(inter_group_id) ?
		ogs_strdup(inter_group_id->valuestring) : NULL,
		supi &&
		...
		valid_start_time &&
		!cJSON_IsNull(valid_start_time) ?
		ogs_strdup(valid_start_time->valuestring) : NULL,
		valid_end_time &&
		!cJSON_IsNull(valid_end_time) ?
		ogs_strdup(valid_end_time->valuestring) : NULL,
		temp_validities ? temp_validitiesList : NULL,
		nw_area_info ? nw_area_info_local_nonprim : NULL,
		up_path_chg_notif_uri &&
		!cJSON_IsNull(up_path_chg_notif_uri) ?
		ogs_strdup(up_path_chg_notif_uri->valuestring) : NULL,
		...
		res_uri &&
		!cJSON_IsNull(res_uri) ? ogs_strdup(res_uri->valuestring) :
		NULL,
...
OpenAPI_traffic_influ_data_t *OpenAPI_traffic_influ_data_copy(
	OpenAPI_traffic_influ_data_t *dst, OpenAPI_traffic_influ_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
