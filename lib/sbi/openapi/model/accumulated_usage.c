...
OpenAPI_accumulated_usage_t *OpenAPI_accumulated_usage_create(
	...
	is_total_volume,
	long total_volume,
	bool is_downlink_volume, long downlink_volume, bool is_uplink_volume,
	long uplink_volume) {
	OpenAPI_accumulated_usage_t *accumulated_usage_local_var =
		ogs_malloc(sizeof(OpenAPI_accumulated_usage_t));
...
void OpenAPI_accumulated_usage_free(
	OpenAPI_accumulated_usage_t *accumulated_usage) {
...
cJSON *OpenAPI_accumulated_usage_convertToJSON(
	OpenAPI_accumulated_usage_t *accumulated_usage) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(accumulated_usage == NULL) {
		ogs_error(
			"OpenAPI_accumulated_usage_convertToJSON() failed [AccumulatedUsage]");
		return NULL;
	}
	...
	if(accumulated_usage->is_duration) {
		if(cJSON_AddNumberToObject(item, "duration",
		                           accumulated_usage->duration) == NULL)
		{
			ogs_error(
				"OpenAPI_accumulated_usage_convertToJSON() failed [duration]");
			goto end;
		}
	}
	...
	if(accumulated_usage->is_total_volume) {
		if(cJSON_AddNumberToObject(item, "totalVolume",
		                           accumulated_usage->total_volume) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_accumulated_usage_convertToJSON() failed [total_volume]");
			goto end;
		}
	}
	...
	if(accumulated_usage->is_downlink_volume) {
		if(cJSON_AddNumberToObject(item, "downlinkVolume",
		                           accumulated_usage->downlink_volume)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_accumulated_usage_convertToJSON() failed [downlink_volume]");
			goto end;
		}
	}
	...
	if(accumulated_usage->is_uplink_volume) {
		if(cJSON_AddNumberToObject(item, "uplinkVolume",
		                           accumulated_usage->uplink_volume) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_accumulated_usage_convertToJSON() failed [uplink_volume]");
			goto end;
		}
	}
...
OpenAPI_accumulated_usage_t *OpenAPI_accumulated_usage_parseFromJSON(
	cJSON *accumulated_usageJSON) {
	...
	duration = cJSON_GetObjectItemCaseSensitive(accumulated_usageJSON,
	                                            "duration");
	if(duration) {
		if(!cJSON_IsNumber(duration)) {
			ogs_error(
				"OpenAPI_accumulated_usage_parseFromJSON() failed [duration]");
			goto end;
		}
	}

	total_volume = cJSON_GetObjectItemCaseSensitive(accumulated_usageJSON,
	                                                "totalVolume");
	if(total_volume) {
		if(!cJSON_IsNumber(total_volume)) {
			ogs_error(
				"OpenAPI_accumulated_usage_parseFromJSON() failed [total_volume]");
			goto end;
		}
	}

	downlink_volume =
		cJSON_GetObjectItemCaseSensitive(accumulated_usageJSON,
		                                 "downlinkVolume");
	if(downlink_volume) {
		if(!cJSON_IsNumber(downlink_volume)) {
			ogs_error(
				"OpenAPI_accumulated_usage_parseFromJSON() failed [downlink_volume]");
			goto end;
		}
	}

	uplink_volume = cJSON_GetObjectItemCaseSensitive(accumulated_usageJSON,
	                                                 "uplinkVolume");
	if(uplink_volume) {
		if(!cJSON_IsNumber(uplink_volume)) {
			ogs_error(
				"OpenAPI_accumulated_usage_parseFromJSON() failed [uplink_volume]");
			goto end;
		}
	}
...
OpenAPI_accumulated_usage_t *OpenAPI_accumulated_usage_copy(
	OpenAPI_accumulated_usage_t *dst, OpenAPI_accumulated_usage_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
