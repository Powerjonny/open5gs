...
OpenAPI_deregistration_data_t *OpenAPI_deregistration_data_create(
	...
	OpenAPI_access_type_e access_type, bool is_pdu_session_id,
	int pdu_session_id, char *new_smf_instance_id) {
	OpenAPI_deregistration_data_t *deregistration_data_local_var =
		ogs_malloc(sizeof(OpenAPI_deregistration_data_t));
	...
	deregistration_data_local_var->new_smf_instance_id =
		new_smf_instance_id;

	return deregistration_data_local_var;
}

void OpenAPI_deregistration_data_free(
	OpenAPI_deregistration_data_t *deregistration_data) {
...
cJSON *OpenAPI_deregistration_data_convertToJSON(
	OpenAPI_deregistration_data_t *deregistration_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(deregistration_data == NULL) {
		ogs_error(
			"OpenAPI_deregistration_data_convertToJSON() failed [DeregistrationData]");
		return NULL;
	}
	...
	{
		ogs_error(
			"OpenAPI_deregistration_data_convertToJSON() failed [dereg_reason]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "deregReason",
	                           OpenAPI_deregistration_reason_ToString(
					   deregistration_data->dereg_reason))
	   == NULL)
	{
		ogs_error(
			"OpenAPI_deregistration_data_convertToJSON() failed [dereg_reason]");
	...
	if(deregistration_data->access_type != OpenAPI_access_type_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_deregistration_data_convertToJSON() failed [access_type]");
			goto end;
		}
	}
	...
	if(deregistration_data->is_pdu_session_id) {
		if(cJSON_AddNumberToObject(item, "pduSessionId",
		                           deregistration_data->pdu_session_id)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_deregistration_data_convertToJSON() failed [pdu_session_id]");
			goto end;
		}
	}

	if(deregistration_data->new_smf_instance_id) {
		if(cJSON_AddStringToObject(item, "newSmfInstanceId",
		                           deregistration_data->
		                           new_smf_instance_id) == NULL)
		{
			ogs_error(
				"OpenAPI_deregistration_data_convertToJSON() failed [new_smf_instance_id]");
			goto end;
		}
	}
...
OpenAPI_deregistration_data_t *OpenAPI_deregistration_data_parseFromJSON(
	cJSON *deregistration_dataJSON) {
	...
	dereg_reason = cJSON_GetObjectItemCaseSensitive(deregistration_dataJSON,
	                                                "deregReason");
	if(!dereg_reason) {
		ogs_error(
			"OpenAPI_deregistration_data_parseFromJSON() failed [dereg_reason]");
		goto end;
	}
	...
	dereg_reasonVariable =
		OpenAPI_deregistration_reason_FromString(
			dereg_reason->valuestring);

	access_type = cJSON_GetObjectItemCaseSensitive(deregistration_dataJSON,
	                                               "accessType");
	if(access_type) {
		if(!cJSON_IsString(access_type)) {
			ogs_error(
				"OpenAPI_deregistration_data_parseFromJSON() failed [access_type]");
			goto end;
		}
		access_typeVariable =
	...
	if(pdu_session_id) {
		if(!cJSON_IsNumber(pdu_session_id)) {
			ogs_error(
				"OpenAPI_deregistration_data_parseFromJSON() failed [pdu_session_id]");
			goto end;
		}
	}
	...
	if(new_smf_instance_id) {
		if(!cJSON_IsString(new_smf_instance_id) &&
		   !cJSON_IsNull(new_smf_instance_id))
		{
			ogs_error(
				"OpenAPI_deregistration_data_parseFromJSON() failed [new_smf_instance_id]");
			goto end;
		}
	}
	...
	deregistration_data_local_var = OpenAPI_deregistration_data_create(
		...
		new_smf_instance_id &&
		!cJSON_IsNull(new_smf_instance_id) ?
		ogs_strdup(new_smf_instance_id->valuestring) : NULL
		);
...
OpenAPI_deregistration_data_t *OpenAPI_deregistration_data_copy(
	OpenAPI_deregistration_data_t	*dst,
	OpenAPI_deregistration_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
