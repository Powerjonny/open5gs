...
OpenAPI_vsmf_update_data_t *OpenAPI_vsmf_update_data_create(
	...
	OpenAPI_vsmf_update_data_t *vsmf_update_data_local_var =
		ogs_malloc(sizeof(OpenAPI_vsmf_update_data_t));
	...
	vsmf_update_data_local_var->qos_flows_add_mod_request_list =
		qos_flows_add_mod_request_list;
	vsmf_update_data_local_var->qos_flows_rel_request_list =
	...
	vsmf_update_data_local_var->new_smf_pdu_session_uri =
		new_smf_pdu_session_uri;
	...
	vsmf_update_data_local_var->additional_cn_tunnel_info =
	...
	vsmf_update_data_local_var->is_small_data_rate_control_enabled =
		is_small_data_rate_control_enabled;
	vsmf_update_data_local_var->small_data_rate_control_enabled =
	...
	vsmf_update_data_local_var->is_n9_data_forwarding_ind =
		is_n9_data_forwarding_ind;
	vsmf_update_data_local_var->n9_data_forwarding_ind =
		n9_data_forwarding_ind;
	vsmf_update_data_local_var->is_n9_inactivity_timer =
...
void OpenAPI_vsmf_update_data_free(
	...
	if(vsmf_update_data->qos_flows_add_mod_request_list) {
		OpenAPI_list_for_each(
			vsmf_update_data->qos_flows_add_mod_request_list,
			node) {
	...
	if(vsmf_update_data->qos_flows_rel_request_list) {
		OpenAPI_list_for_each(
			vsmf_update_data->qos_flows_rel_request_list, node) {
	...
	if(vsmf_update_data->modified_ebi_list) {
		OpenAPI_list_for_each(vsmf_update_data->modified_ebi_list,
		                      node) {
	...
	if(vsmf_update_data->additional_cn_tunnel_info) {
		OpenAPI_tunnel_info_free(
			vsmf_update_data->additional_cn_tunnel_info);
	...
	if(vsmf_update_data->qos_monitoring_info) {
		OpenAPI_qos_monitoring_info_free(
			vsmf_update_data->qos_monitoring_info);
...
cJSON *OpenAPI_vsmf_update_data_convertToJSON(
	...
	if(vsmf_update_data == NULL) {
		ogs_error(
			"OpenAPI_vsmf_update_data_convertToJSON() failed [VsmfUpdateData]");
		return NULL;
	}
	...
	{
		ogs_error(
			"OpenAPI_vsmf_update_data_convertToJSON() failed [request_indication]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "requestIndication",
	                           OpenAPI_request_indication_ToString(
					   vsmf_update_data->request_indication))
	   == NULL)
	{
		ogs_error(
			"OpenAPI_vsmf_update_data_convertToJSON() failed [request_indication]");
	...
	if(vsmf_update_data->session_ambr) {
		cJSON *session_ambr_local_JSON =
			OpenAPI_ambr_convertToJSON(
				vsmf_update_data->session_ambr);
		if(session_ambr_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [session_ambr]");
			goto end;
		}
	...
	if(vsmf_update_data->qos_flows_add_mod_request_list) {
		cJSON *qos_flows_add_mod_request_listList =
			cJSON_AddArrayToObject(item,
			                       "qosFlowsAddModRequestList");
		if(qos_flows_add_mod_request_listList == NULL) {
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [qos_flows_add_mod_request_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			vsmf_update_data->qos_flows_add_mod_request_list,
			node) {
			cJSON *itemLocal =
				OpenAPI_qos_flow_add_modify_request_item_convertToJSON
				        (node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_vsmf_update_data_convertToJSON() failed [qos_flows_add_mod_request_list]");
			...
			cJSON_AddItemToArray(qos_flows_add_mod_request_listList,
			                     itemLocal);
		}
	}

	if(vsmf_update_data->qos_flows_rel_request_list) {
		cJSON *qos_flows_rel_request_listList =
			cJSON_AddArrayToObject(item, "qosFlowsRelRequestList");
		if(qos_flows_rel_request_listList == NULL) {
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [qos_flows_rel_request_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			vsmf_update_data->qos_flows_rel_request_list, node) {
			cJSON *itemLocal =
				OpenAPI_qos_flow_release_request_item_convertToJSON
				        (node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_vsmf_update_data_convertToJSON() failed [qos_flows_rel_request_list]");
			...
			cJSON_AddItemToArray(qos_flows_rel_request_listList,
			                     itemLocal);
		}
	}
	...
	if(vsmf_update_data->eps_bearer_info) {
		...
		if(eps_bearer_infoList == NULL) {
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [eps_bearer_info]");
			goto end;
		}
		OpenAPI_list_for_each(vsmf_update_data->eps_bearer_info, node) {
			cJSON *itemLocal =
	...
	if(vsmf_update_data->assign_ebi_list) {
		cJSON *assign_ebi_listList = cJSON_AddArrayToObject(item,
		                                                    "assignEbiList");
		if(assign_ebi_listList == NULL) {
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [assign_ebi_list]");
			goto end;
		}
		OpenAPI_list_for_each(vsmf_update_data->assign_ebi_list, node) {
			cJSON *itemLocal =
				OpenAPI_arp_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_vsmf_update_data_convertToJSON() failed [assign_ebi_list]");
				goto end;
	...
	if(vsmf_update_data->revoke_ebi_list) {
		...
		if(revoke_ebi_listList == NULL) {
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [revoke_ebi_list]");
			goto end;
		}
		OpenAPI_list_for_each(vsmf_update_data->revoke_ebi_list, node) {
			if(node->data == NULL) {
	...
	if(vsmf_update_data->modified_ebi_list) {
		...
		if(modified_ebi_listList == NULL) {
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [modified_ebi_list]");
			goto end;
		}
		OpenAPI_list_for_each(vsmf_update_data->modified_ebi_list,
		                      node) {
			cJSON *itemLocal =
				OpenAPI_ebi_arp_mapping_convertToJSON(
					node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_vsmf_update_data_convertToJSON() failed [modified_ebi_list]");
	...
	if(vsmf_update_data->is_pti) {
		if(cJSON_AddNumberToObject(item, "pti",
		                           vsmf_update_data->pti) == NULL)
		{
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [pti]");
			goto end;
		}
	}
	...
	if(vsmf_update_data->n1_sm_info_to_ue) {
		...
		if(n1_sm_info_to_ue_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [n1_sm_info_to_ue]");
			goto end;
		}
	...
	if(vsmf_update_data->is_always_on_granted) {
		if(cJSON_AddBoolToObject(item, "alwaysOnGranted",
		                         vsmf_update_data->always_on_granted) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [always_on_granted]");
			goto end;
		}
	}

	if(vsmf_update_data->hsmf_pdu_session_uri) {
		if(cJSON_AddStringToObject(item, "hsmfPduSessionUri",
		                           vsmf_update_data->
		                           hsmf_pdu_session_uri) == NULL)
		{
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [hsmf_pdu_session_uri]");
			goto end;
		}
	}

	if(vsmf_update_data->new_smf_id) {
		if(cJSON_AddStringToObject(item, "newSmfId",
		                           vsmf_update_data->new_smf_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [new_smf_id]");
			goto end;
		}
	}

	if(vsmf_update_data->new_smf_pdu_session_uri) {
		if(cJSON_AddStringToObject(item, "newSmfPduSessionUri",
		                           vsmf_update_data->
		                           new_smf_pdu_session_uri) == NULL)
		{
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [new_smf_pdu_session_uri]");
			goto end;
		}
	}
	...
	if(vsmf_update_data->supported_features) {
		if(cJSON_AddStringToObject(item, "supportedFeatures",
		                           vsmf_update_data->supported_features)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(vsmf_update_data->cause != OpenAPI_cause_NULL) {
		if(cJSON_AddStringToObject(item, "cause",
						   ...
						   vsmf_update_data->cause)) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [cause]");
			goto end;
		}
	}

	if(vsmf_update_data->n1sm_cause) {
		if(cJSON_AddStringToObject(item, "n1smCause",
		                           vsmf_update_data->n1sm_cause) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [n1sm_cause]");
			goto end;
		}
	}
	...
	if(vsmf_update_data->is_back_off_timer) {
		if(cJSON_AddNumberToObject(item, "backOffTimer",
		                           vsmf_update_data->back_off_timer) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [back_off_timer]");
			goto end;
		}
	}
	   ...
	   OpenAPI_ma_release_indication_NULL)
	{
		...
		{
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [ma_release_ind]");
			goto end;
		}
	}
	...
	if(vsmf_update_data->is_ma_accepted_ind) {
		if(cJSON_AddBoolToObject(item, "maAcceptedInd",
		                         vsmf_update_data->ma_accepted_ind) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [ma_accepted_ind]");
			goto end;
		}
	}
	...
	if(vsmf_update_data->additional_cn_tunnel_info) {
		...
		if(additional_cn_tunnel_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [additional_cn_tunnel_info]");
			goto end;
		}
	...
	if(vsmf_update_data->dnai_list) {
		...
		if(dnai_listList == NULL) {
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [dnai_list]");
			goto end;
		}
		OpenAPI_list_for_each(vsmf_update_data->dnai_list, node) {
	...
	if(vsmf_update_data->n4_info) {
		cJSON *n4_info_local_JSON =
			OpenAPI_n4_information_convertToJSON(
				vsmf_update_data->n4_info);
		if(n4_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [n4_info]");
			goto end;
		}
	...
	if(vsmf_update_data->n4_info_ext1) {
		cJSON *n4_info_ext1_local_JSON =
			OpenAPI_n4_information_convertToJSON(
				vsmf_update_data->n4_info_ext1);
		if(n4_info_ext1_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [n4_info_ext1]");
			goto end;
		}
	...
	if(vsmf_update_data->n4_info_ext2) {
		cJSON *n4_info_ext2_local_JSON =
			OpenAPI_n4_information_convertToJSON(
				vsmf_update_data->n4_info_ext2);
		if(n4_info_ext2_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [n4_info_ext2]");
			goto end;
		}
	...
	if(vsmf_update_data->n4_info_ext3) {
		cJSON *n4_info_ext3_local_JSON =
			OpenAPI_n4_information_convertToJSON(
				vsmf_update_data->n4_info_ext3);
		if(n4_info_ext3_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [n4_info_ext3]");
			goto end;
		}
		cJSON_AddItemToObject(item, "n4InfoExt3",
		                      n4_info_ext3_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [n4_info_ext3]");
			goto end;
	...
	if(vsmf_update_data->is_small_data_rate_control_enabled) {
		...
		{
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [small_data_rate_control_enabled]");
			goto end;
		}
	}

	if(vsmf_update_data->qos_monitoring_info) {
		cJSON *qos_monitoring_info_local_JSON =
			OpenAPI_qos_monitoring_info_convertToJSON(
				vsmf_update_data->qos_monitoring_info);
		if(qos_monitoring_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [qos_monitoring_info]");
			goto end;
		}
		cJSON_AddItemToObject(item, "qosMonitoringInfo",
		                      qos_monitoring_info_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [qos_monitoring_info]");
			goto end;
	...
	if(vsmf_update_data->eps_pdn_cnx_info) {
		...
		if(eps_pdn_cnx_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [eps_pdn_cnx_info]");
			goto end;
		}
	...
	if(vsmf_update_data->is_n9_data_forwarding_ind) {
		if(cJSON_AddBoolToObject(item, "n9DataForwardingInd",
		                         vsmf_update_data->
		                         n9_data_forwarding_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [n9_data_forwarding_ind]");
			goto end;
		}
	}
	...
	if(vsmf_update_data->is_n9_inactivity_timer) {
		if(cJSON_AddNumberToObject(item, "n9InactivityTimer",
		                           vsmf_update_data->n9_inactivity_timer)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_vsmf_update_data_convertToJSON() failed [n9_inactivity_timer]");
			goto end;
		}
	}
...
OpenAPI_vsmf_update_data_t *OpenAPI_vsmf_update_data_parseFromJSON(
	...
	request_indication =
		cJSON_GetObjectItemCaseSensitive(vsmf_update_dataJSON,
		                                 "requestIndication");
	if(!request_indication) {
		ogs_error(
			"OpenAPI_vsmf_update_data_parseFromJSON() failed [request_indication]");
		goto end;
	}
	...
	session_ambr = cJSON_GetObjectItemCaseSensitive(vsmf_update_dataJSON,
	                                                "sessionAmbr");
	if(session_ambr) {
	...
	if(qos_flows_add_mod_request_list) {
		...
		if(!cJSON_IsArray(qos_flows_add_mod_request_list)) {
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [qos_flows_add_mod_request_list]");
			goto end;
		}

		qos_flows_add_mod_request_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(qos_flows_add_mod_request_list_local,
		                   qos_flows_add_mod_request_list) {
			if(!cJSON_IsObject(
				   qos_flows_add_mod_request_list_local))
			{
				ogs_error(
					"OpenAPI_vsmf_update_data_parseFromJSON() failed [qos_flows_add_mod_request_list]");
				goto end;
			...
			OpenAPI_qos_flow_add_modify_request_item_t *
			        qos_flows_add_mod_request_listItem =
				OpenAPI_qos_flow_add_modify_request_item_parseFromJSON
				        (qos_flows_add_mod_request_list_local);
			if(!qos_flows_add_mod_request_listItem) {
				ogs_error(
					"No qos_flows_add_mod_request_listItem");
				goto end;
			}
			OpenAPI_list_add(qos_flows_add_mod_request_listList,
			                 qos_flows_add_mod_request_listItem);
		}
	}
	...
	if(qos_flows_rel_request_list) {
		...
		if(!cJSON_IsArray(qos_flows_rel_request_list)) {
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [qos_flows_rel_request_list]");
			goto end;
		}

		qos_flows_rel_request_listList = OpenAPI_list_create();

		cJSON_ArrayForEach(qos_flows_rel_request_list_local,
		                   qos_flows_rel_request_list) {
			...
			OpenAPI_qos_flow_release_request_item_t *
			        qos_flows_rel_request_listItem =
				OpenAPI_qos_flow_release_request_item_parseFromJSON
				        (qos_flows_rel_request_list_local);
			if(!qos_flows_rel_request_listItem) {
				ogs_error("No qos_flows_rel_request_listItem");
				goto end;
			}
			OpenAPI_list_add(qos_flows_rel_request_listList,
			                 qos_flows_rel_request_listItem);
		}
	}

	eps_bearer_info = cJSON_GetObjectItemCaseSensitive(vsmf_update_dataJSON,
	                                                   "epsBearerInfo");
	if(eps_bearer_info) {
		cJSON *eps_bearer_info_local = NULL;
		if(!cJSON_IsArray(eps_bearer_info)) {
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [eps_bearer_info]");
			goto end;
		}
	...
	assign_ebi_list = cJSON_GetObjectItemCaseSensitive(vsmf_update_dataJSON,
	                                                   "assignEbiList");
	if(assign_ebi_list) {
		cJSON *assign_ebi_list_local = NULL;
		if(!cJSON_IsArray(assign_ebi_list)) {
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [assign_ebi_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(assign_ebi_list_local, assign_ebi_list) {
			...
			OpenAPI_arp_t *assign_ebi_listItem =
				OpenAPI_arp_parseFromJSON(
					assign_ebi_list_local);
			if(!assign_ebi_listItem) {
				ogs_error("No assign_ebi_listItem");
				goto end;
			}
			OpenAPI_list_add(assign_ebi_listList,
			                 assign_ebi_listItem);
		}
	}

	revoke_ebi_list = cJSON_GetObjectItemCaseSensitive(vsmf_update_dataJSON,
	                                                   "revokeEbiList");
	if(revoke_ebi_list) {
		cJSON *revoke_ebi_list_local = NULL;
		if(!cJSON_IsArray(revoke_ebi_list)) {
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [revoke_ebi_list]");
			goto end;
		}
	...
	if(modified_ebi_list) {
		...
		if(!cJSON_IsArray(modified_ebi_list)) {
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [modified_ebi_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(modified_ebi_list_local, modified_ebi_list) {
			...
			OpenAPI_ebi_arp_mapping_t *modified_ebi_listItem =
				OpenAPI_ebi_arp_mapping_parseFromJSON(
					modified_ebi_list_local);
			if(!modified_ebi_listItem) {
				ogs_error("No modified_ebi_listItem");
				goto end;
			}
			OpenAPI_list_add(modified_ebi_listList,
			                 modified_ebi_listItem);
		}
	}
	...
	if(pti) {
		if(!cJSON_IsNumber(pti)) {
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [pti]");
			goto end;
		}
	}
	...
	if(always_on_granted) {
		if(!cJSON_IsBool(always_on_granted)) {
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [always_on_granted]");
			goto end;
		}
	}

	hsmf_pdu_session_uri =
		cJSON_GetObjectItemCaseSensitive(vsmf_update_dataJSON,
		                                 "hsmfPduSessionUri");
	if(hsmf_pdu_session_uri) {
		if(!cJSON_IsString(hsmf_pdu_session_uri) &&
		   !cJSON_IsNull(hsmf_pdu_session_uri))
		{
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [hsmf_pdu_session_uri]");
			goto end;
		}
	}

	new_smf_id = cJSON_GetObjectItemCaseSensitive(vsmf_update_dataJSON,
	                                              "newSmfId");
	if(new_smf_id) {
		if(!cJSON_IsString(new_smf_id) &&
		   !cJSON_IsNull(new_smf_id))
		{
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [new_smf_id]");
			goto end;
		}
	}
	...
	if(new_smf_pdu_session_uri) {
		if(!cJSON_IsString(new_smf_pdu_session_uri) &&
		   !cJSON_IsNull(new_smf_pdu_session_uri))
		{
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [new_smf_pdu_session_uri]");
			goto end;
		}
	}
	...
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(cause) {
		if(!cJSON_IsString(cause)) {
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [cause]");
			goto end;
		}
		causeVariable = OpenAPI_cause_FromString(cause->valuestring);
	}

	n1sm_cause = cJSON_GetObjectItemCaseSensitive(vsmf_update_dataJSON,
	                                              "n1smCause");
	if(n1sm_cause) {
		if(!cJSON_IsString(n1sm_cause) &&
		   !cJSON_IsNull(n1sm_cause))
		{
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [n1sm_cause]");
			goto end;
		}
	}

	back_off_timer = cJSON_GetObjectItemCaseSensitive(vsmf_update_dataJSON,
	                                                  "backOffTimer");
	if(back_off_timer) {
		if(!cJSON_IsNumber(back_off_timer)) {
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [back_off_timer]");
			goto end;
		}
	}

	ma_release_ind = cJSON_GetObjectItemCaseSensitive(vsmf_update_dataJSON,
	                                                  "maReleaseInd");
	if(ma_release_ind) {
		if(!cJSON_IsString(ma_release_ind)) {
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [ma_release_ind]");
			goto end;
		}
	...
	ma_accepted_ind = cJSON_GetObjectItemCaseSensitive(vsmf_update_dataJSON,
	                                                   "maAcceptedInd");
	if(ma_accepted_ind) {
		if(!cJSON_IsBool(ma_accepted_ind)) {
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [ma_accepted_ind]");
			goto end;
		}
	}
	...
	dnai_list = cJSON_GetObjectItemCaseSensitive(vsmf_update_dataJSON,
	                                             "dnaiList");
	if(dnai_list) {
		cJSON *dnai_list_local = NULL;
		if(!cJSON_IsArray(dnai_list)) {
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [dnai_list]");
			goto end;
		}
	...
	n4_info = cJSON_GetObjectItemCaseSensitive(vsmf_update_dataJSON,
	                                           "n4Info");
	if(n4_info) {
	...
	n4_info_ext1 = cJSON_GetObjectItemCaseSensitive(vsmf_update_dataJSON,
	                                                "n4InfoExt1");
	if(n4_info_ext1) {
	...
	n4_info_ext2 = cJSON_GetObjectItemCaseSensitive(vsmf_update_dataJSON,
	                                                "n4InfoExt2");
	if(n4_info_ext2) {
	...
	n4_info_ext3 = cJSON_GetObjectItemCaseSensitive(vsmf_update_dataJSON,
	                                                "n4InfoExt3");
	if(n4_info_ext3) {
		n4_info_ext3_local_nonprim =
			OpenAPI_n4_information_parseFromJSON(n4_info_ext3);
		if(!n4_info_ext3_local_nonprim) {
			ogs_error(
				"OpenAPI_n4_information_parseFromJSON failed [n4_info_ext3]");
			goto end;
		}
	}
	...
	if(small_data_rate_control_enabled) {
		if(!cJSON_IsBool(small_data_rate_control_enabled)) {
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [small_data_rate_control_enabled]");
			goto end;
		}
	}
	...
	if(qos_monitoring_info) {
		qos_monitoring_info_local_nonprim =
			OpenAPI_qos_monitoring_info_parseFromJSON(
				qos_monitoring_info);
		if(!qos_monitoring_info_local_nonprim) {
			ogs_error(
				"OpenAPI_qos_monitoring_info_parseFromJSON failed [qos_monitoring_info]");
			goto end;
		}
	}
	...
	if(n9_data_forwarding_ind) {
		if(!cJSON_IsBool(n9_data_forwarding_ind)) {
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [n9_data_forwarding_ind]");
			goto end;
		}
	}
	...
	if(n9_inactivity_timer) {
		if(!cJSON_IsNumber(n9_inactivity_timer)) {
			ogs_error(
				"OpenAPI_vsmf_update_data_parseFromJSON() failed [n9_inactivity_timer]");
			goto end;
		}
	}
	...
	vsmf_update_data_local_var = OpenAPI_vsmf_update_data_create(
		...
		qos_flows_rel_request_list ? qos_flows_rel_request_listList :
		NULL,
		...
		hsmf_pdu_session_uri &&
		!cJSON_IsNull(hsmf_pdu_session_uri) ?
		ogs_strdup(hsmf_pdu_session_uri->valuestring) : NULL,
		new_smf_id &&
		!cJSON_IsNull(new_smf_id) ?
		ogs_strdup(new_smf_id->valuestring) : NULL,
		new_smf_pdu_session_uri &&
		!cJSON_IsNull(new_smf_pdu_session_uri) ?
		ogs_strdup(new_smf_pdu_session_uri->valuestring) : NULL,
		supported_features &&
		!cJSON_IsNull(supported_features) ?
		...
		n1sm_cause &&
		!cJSON_IsNull(n1sm_cause) ?
		ogs_strdup(n1sm_cause->valuestring) : NULL,
	...
	if(qos_flows_add_mod_request_listList) {
		OpenAPI_list_for_each(qos_flows_add_mod_request_listList,
		                      node) {
			OpenAPI_qos_flow_add_modify_request_item_free(
				node->data);
		}
