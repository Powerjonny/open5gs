...
OpenAPI_chf_info_t *OpenAPI_chf_info_create(
	...
	OpenAPI_chf_info_t *chf_info_local_var =
		ogs_malloc(sizeof(OpenAPI_chf_info_t));
...
cJSON *OpenAPI_chf_info_convertToJSON(OpenAPI_chf_info_t *chf_info) {
	...
	if(chf_info->supi_range_list) {
		...
		if(supi_range_listList == NULL) {
			ogs_error(
				"OpenAPI_chf_info_convertToJSON() failed [supi_range_list]");
			goto end;
		}
		OpenAPI_list_for_each(chf_info->supi_range_list, node) {
			cJSON *itemLocal =
	...
	if(chf_info->gpsi_range_list) {
		...
		if(gpsi_range_listList == NULL) {
			ogs_error(
				"OpenAPI_chf_info_convertToJSON() failed [gpsi_range_list]");
			goto end;
		}
		OpenAPI_list_for_each(chf_info->gpsi_range_list, node) {
			cJSON *itemLocal =
	...
	if(chf_info->plmn_range_list) {
		...
		if(plmn_range_listList == NULL) {
			ogs_error(
				"OpenAPI_chf_info_convertToJSON() failed [plmn_range_list]");
			goto end;
		}
		OpenAPI_list_for_each(chf_info->plmn_range_list, node) {
			cJSON *itemLocal =
	...
	if(chf_info->group_id) {
		...
		{
			ogs_error(
				"OpenAPI_chf_info_convertToJSON() failed [group_id]");
			goto end;
		}
	}
	...
	if(chf_info->primary_chf_instance) {
		if(cJSON_AddStringToObject(item, "primaryChfInstance",
		                           chf_info->primary_chf_instance) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_chf_info_convertToJSON() failed [primary_chf_instance]");
			goto end;
		}
	}
	...
	if(chf_info->secondary_chf_instance) {
		if(cJSON_AddStringToObject(item, "secondaryChfInstance",
		                           chf_info->secondary_chf_instance) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_chf_info_convertToJSON() failed [secondary_chf_instance]");
			goto end;
		}
	}
...
OpenAPI_chf_info_t *OpenAPI_chf_info_parseFromJSON(cJSON *chf_infoJSON) {
	...
	supi_range_list = cJSON_GetObjectItemCaseSensitive(chf_infoJSON,
	                                                   "supiRangeList");
	if(supi_range_list) {
		cJSON *supi_range_list_local = NULL;
		if(!cJSON_IsArray(supi_range_list)) {
			ogs_error(
				"OpenAPI_chf_info_parseFromJSON() failed [supi_range_list]");
			goto end;
		}
	...
	gpsi_range_list = cJSON_GetObjectItemCaseSensitive(chf_infoJSON,
	                                                   "gpsiRangeList");
	if(gpsi_range_list) {
		cJSON *gpsi_range_list_local = NULL;
		if(!cJSON_IsArray(gpsi_range_list)) {
			ogs_error(
				"OpenAPI_chf_info_parseFromJSON() failed [gpsi_range_list]");
			goto end;
		}
	...
	plmn_range_list = cJSON_GetObjectItemCaseSensitive(chf_infoJSON,
	                                                   "plmnRangeList");
	if(plmn_range_list) {
		cJSON *plmn_range_list_local = NULL;
		if(!cJSON_IsArray(plmn_range_list)) {
			ogs_error(
				"OpenAPI_chf_info_parseFromJSON() failed [plmn_range_list]");
			goto end;
		}
	...
	if(group_id) {
		...
		{
			ogs_error(
				"OpenAPI_chf_info_parseFromJSON() failed [group_id]");
			goto end;
		}
	}

	primary_chf_instance = cJSON_GetObjectItemCaseSensitive(chf_infoJSON,
	                                                        "primaryChfInstance");
	if(primary_chf_instance) {
		...
		{
			ogs_error(
				"OpenAPI_chf_info_parseFromJSON() failed [primary_chf_instance]");
			goto end;
		}
	}

	secondary_chf_instance = cJSON_GetObjectItemCaseSensitive(chf_infoJSON,
	...
	if(secondary_chf_instance) {
		...
		{
			ogs_error(
				"OpenAPI_chf_info_parseFromJSON() failed [secondary_chf_instance]");
			goto end;
		}
	}
