...
OpenAPI_ext_amf_event_subscription_t *OpenAPI_ext_amf_event_subscription_create(
	OpenAPI_list_t *event_list, char *event_notify_uri,
	char *notify_correlation_id, char *nf_id, char *subs_change_notify_uri,
	char *subs_change_notify_correlation_id, char *supi, char *group_id,
	OpenAPI_list_t *exclude_supi_list, OpenAPI_list_t *exclude_gpsi_list,
	OpenAPI_list_t *include_supi_list, OpenAPI_list_t *include_gpsi_list,
	char *gpsi, char *pei, bool is_any_ue, int any_ue,
	OpenAPI_amf_event_mode_t *options, OpenAPI_nf_type_e source_nf_type,
	OpenAPI_list_t *binding_info, OpenAPI_nf_type_e subscribing_nf_type,
	bool is_event_sync_ind, int event_sync_ind,
	OpenAPI_list_t *nf_consumer_info, OpenAPI_list_t *aoi_state_list) {
	OpenAPI_ext_amf_event_subscription_t *
	        ext_amf_event_subscription_local_var =
		ogs_malloc(sizeof(OpenAPI_ext_amf_event_subscription_t));
...
void OpenAPI_ext_amf_event_subscription_free(
	OpenAPI_ext_amf_event_subscription_t *ext_amf_event_subscription) {
	...
	if(ext_amf_event_subscription->event_list) {
		OpenAPI_list_for_each(ext_amf_event_subscription->event_list,
		                      node) {
	...
	if(ext_amf_event_subscription->exclude_supi_list) {
		OpenAPI_list_for_each(
			ext_amf_event_subscription->exclude_supi_list, node) {
			ogs_free(node->data);
		}
		OpenAPI_list_free(
			ext_amf_event_subscription->exclude_supi_list);
		ext_amf_event_subscription->exclude_supi_list = NULL;
	}
	if(ext_amf_event_subscription->exclude_gpsi_list) {
		OpenAPI_list_for_each(
			ext_amf_event_subscription->exclude_gpsi_list, node) {
			ogs_free(node->data);
		}
		OpenAPI_list_free(
			ext_amf_event_subscription->exclude_gpsi_list);
		ext_amf_event_subscription->exclude_gpsi_list = NULL;
	}
	if(ext_amf_event_subscription->include_supi_list) {
		OpenAPI_list_for_each(
			ext_amf_event_subscription->include_supi_list, node) {
			ogs_free(node->data);
		}
		OpenAPI_list_free(
			ext_amf_event_subscription->include_supi_list);
		ext_amf_event_subscription->include_supi_list = NULL;
	}
	if(ext_amf_event_subscription->include_gpsi_list) {
		OpenAPI_list_for_each(
			ext_amf_event_subscription->include_gpsi_list, node) {
			ogs_free(node->data);
		}
		OpenAPI_list_free(
			ext_amf_event_subscription->include_gpsi_list);
	...
	if(ext_amf_event_subscription->nf_consumer_info) {
		OpenAPI_list_for_each(
			ext_amf_event_subscription->nf_consumer_info, node) {
			ogs_free(node->data);
		}
	...
	if(ext_amf_event_subscription->aoi_state_list) {
		OpenAPI_list_for_each(
			ext_amf_event_subscription->aoi_state_list, node) {
			OpenAPI_map_t *localKeyValue =
				(OpenAPI_map_t *) node->data;
			ogs_free(localKeyValue->key);
			OpenAPI_area_of_interest_event_state_free(
				localKeyValue->value);
...
cJSON *OpenAPI_ext_amf_event_subscription_convertToJSON(
	...
	if(ext_amf_event_subscription == NULL) {
		ogs_error(
			"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [ExtAmfEventSubscription]");
		return NULL;
	}
	...
	if(!ext_amf_event_subscription->event_list) {
		ogs_error(
			"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [event_list]");
		return NULL;
	}
	...
	if(!ext_amf_event_subscription->event_notify_uri) {
		ogs_error(
			"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [event_notify_uri]");
		return NULL;
	}
	...
	if(!ext_amf_event_subscription->notify_correlation_id) {
		ogs_error(
			"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [notify_correlation_id]");
		return NULL;
	}
	...
	if(!ext_amf_event_subscription->nf_id) {
		ogs_error(
			"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [nf_id]");
		return NULL;
	}
	...
	if(ext_amf_event_subscription->subs_change_notify_uri) {
		...
		{
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [subs_change_notify_uri]");
			goto end;
		}
	}
	...
	if(ext_amf_event_subscription->subs_change_notify_correlation_id) {
		...
		{
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [subs_change_notify_correlation_id]");
			goto end;
		}
	}
	...
	if(ext_amf_event_subscription->supi) {
		...
		{
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(ext_amf_event_subscription->group_id) {
		...
		{
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [group_id]");
			goto end;
		}
	}
	...
	if(ext_amf_event_subscription->exclude_supi_list) {
		...
		if(exclude_supi_listList == NULL) {
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [exclude_supi_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			ext_amf_event_subscription->exclude_supi_list, node) {
	...
	if(ext_amf_event_subscription->exclude_gpsi_list) {
		...
		if(exclude_gpsi_listList == NULL) {
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [exclude_gpsi_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			ext_amf_event_subscription->exclude_gpsi_list, node) {
	...
	if(ext_amf_event_subscription->include_supi_list) {
		...
		if(include_supi_listList == NULL) {
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [include_supi_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			ext_amf_event_subscription->include_supi_list, node) {
	...
	if(ext_amf_event_subscription->include_gpsi_list) {
		...
		if(include_gpsi_listList == NULL) {
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [include_gpsi_list]");
			goto end;
		}
		OpenAPI_list_for_each(
			ext_amf_event_subscription->include_gpsi_list, node) {
	...
	if(ext_amf_event_subscription->gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [gpsi]");
			goto end;
		}
	}
	...
	if(ext_amf_event_subscription->pei) {
		...
		{
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [pei]");
			goto end;
		}
	}
	...
	if(ext_amf_event_subscription->is_any_ue) {
		...
		{
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [any_ue]");
			goto end;
		}
	}

	if(ext_amf_event_subscription->options) {
		cJSON *options_local_JSON =
			OpenAPI_amf_event_mode_convertToJSON(
				ext_amf_event_subscription->options);
		if(options_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [options]");
			goto end;
		}
	...
	if(ext_amf_event_subscription->source_nf_type != OpenAPI_nf_type_NULL) {
		if(cJSON_AddStringToObject(item, "sourceNfType",
		                           OpenAPI_nf_type_ToString(
						   ext_amf_event_subscription->
						   source_nf_type)) == NULL)
		{
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [source_nf_type]");
			goto end;
		}
	}
	...
	if(ext_amf_event_subscription->binding_info) {
		...
		if(binding_infoList == NULL) {
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [binding_info]");
			goto end;
		}
		OpenAPI_list_for_each(ext_amf_event_subscription->binding_info,
		                      node) {
	...
	{
		if(cJSON_AddStringToObject(item, "subscribingNfType",
		                           OpenAPI_nf_type_ToString(
						   ext_amf_event_subscription->
						   subscribing_nf_type)) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [subscribing_nf_type]");
			goto end;
		}
	}
	...
	if(ext_amf_event_subscription->is_event_sync_ind) {
		...
		{
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [event_sync_ind]");
			goto end;
		}
	}
	...
	if(ext_amf_event_subscription->nf_consumer_info) {
		...
		if(nf_consumer_infoList == NULL) {
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [nf_consumer_info]");
			goto end;
		}
		OpenAPI_list_for_each(
			ext_amf_event_subscription->nf_consumer_info, node) {
	...
	if(ext_amf_event_subscription->aoi_state_list) {
		...
		if(aoi_state_list == NULL) {
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [aoi_state_list]");
			goto end;
		}
		...
		if(ext_amf_event_subscription->aoi_state_list) {
			OpenAPI_list_for_each(
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_area_of_interest_event_state_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_ext_amf_event_subscription_convertToJSON() failed [inner]");
					goto end;
				}
...
OpenAPI_ext_amf_event_subscription_parseFromJSON(
	...
	OpenAPI_ext_amf_event_subscription_t *
	        ext_amf_event_subscription_local_var = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(!event_list) {
		ogs_error(
			"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [event_list]");
		goto end;
	}
	...
	cJSON_ArrayForEach(event_list_local, event_list) {
		...
		OpenAPI_amf_event_t *event_listItem =
			OpenAPI_amf_event_parseFromJSON(event_list_local);
		if(!event_listItem) {
	...
	if(!event_notify_uri) {
		ogs_error(
			"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [event_notify_uri]");
		goto end;
	}
	...
	if(!notify_correlation_id) {
		ogs_error(
			"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [notify_correlation_id]");
		goto end;
	}
	...
	if(!nf_id) {
		ogs_error(
			"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [nf_id]");
		goto end;
	}
	...
	if(subs_change_notify_uri) {
		...
		{
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [subs_change_notify_uri]");
			goto end;
		}
	}
	...
	if(subs_change_notify_correlation_id) {
		...
		{
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [subs_change_notify_correlation_id]");
			goto end;
		}
	}
	...
	if(supi) {
		...
		{
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [supi]");
			goto end;
		}
	}
	...
	if(group_id) {
		...
		{
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [group_id]");
			goto end;
		}
	}
	...
	if(exclude_supi_list) {
		...
		if(!cJSON_IsArray(exclude_supi_list)) {
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [exclude_supi_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(exclude_supi_list_local, exclude_supi_list) {
			...
			OpenAPI_list_add(exclude_supi_listList,
			                 ogs_strdup(
						 exclude_supi_list_local->
						 valuestring));
		}
	}
	...
	if(exclude_gpsi_list) {
		...
		if(!cJSON_IsArray(exclude_gpsi_list)) {
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [exclude_gpsi_list]");
			goto end;
		}
	...
	if(include_supi_list) {
		...
		if(!cJSON_IsArray(include_supi_list)) {
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [include_supi_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(include_supi_list_local, include_supi_list) {
			...
			OpenAPI_list_add(include_supi_listList,
			                 ogs_strdup(
						 include_supi_list_local->
						 valuestring));
		}
	}
	...
	if(include_gpsi_list) {
		...
		if(!cJSON_IsArray(include_gpsi_list)) {
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [include_gpsi_list]");
			goto end;
		}
	...
	if(gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [gpsi]");
			goto end;
		}
	}
	...
	if(pei) {
		...
		{
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [pei]");
			goto end;
		}
	}
	...
	if(any_ue) {
		if(!cJSON_IsBool(any_ue)) {
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [any_ue]");
			goto end;
		}
	}
	...
	options =
		cJSON_GetObjectItemCaseSensitive(ext_amf_event_subscriptionJSON,
		                                 "options");
	if(options) {
		options_local_nonprim =
			OpenAPI_amf_event_mode_parseFromJSON(options);
		if(!options_local_nonprim) {
			ogs_error(
				"OpenAPI_amf_event_mode_parseFromJSON failed [options]");
			goto end;
		}
	}
	...
	if(source_nf_type) {
		if(!cJSON_IsString(source_nf_type)) {
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [source_nf_type]");
			goto end;
		}
		source_nf_typeVariable =
			OpenAPI_nf_type_FromString(source_nf_type->valuestring);
	}
	...
	if(binding_info) {
		...
		if(!cJSON_IsArray(binding_info)) {
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [binding_info]");
			goto end;
		}
		...
		cJSON_ArrayForEach(binding_info_local, binding_info) {
			...
			OpenAPI_list_add(binding_infoList,
			                 ogs_strdup(
						 binding_info_local->valuestring));
		}
	}
	...
	if(subscribing_nf_type) {
		if(!cJSON_IsString(subscribing_nf_type)) {
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [subscribing_nf_type]");
			goto end;
		}
		subscribing_nf_typeVariable =
			OpenAPI_nf_type_FromString(
				subscribing_nf_type->valuestring);
	}
	...
	if(event_sync_ind) {
		if(!cJSON_IsBool(event_sync_ind)) {
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [event_sync_ind]");
			goto end;
		}
	}
	...
	if(nf_consumer_info) {
		...
		if(!cJSON_IsArray(nf_consumer_info)) {
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [nf_consumer_info]");
			goto end;
		}
		...
		cJSON_ArrayForEach(nf_consumer_info_local, nf_consumer_info) {
			...
			OpenAPI_list_add(nf_consumer_infoList,
			                 ogs_strdup(
						 nf_consumer_info_local->
						 valuestring));
		}
	}
	...
	if(aoi_state_list) {
		...
		{
			ogs_error(
				"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [aoi_state_list]");
			goto end;
		}
		if(cJSON_IsObject(aoi_state_list)) {
			...
			cJSON_ArrayForEach(aoi_state_list_local_map,
				...
				if(cJSON_IsObject(localMapObject)) {
					...
					ogs_error(
						"OpenAPI_ext_amf_event_subscription_parseFromJSON() failed [inner]");
					goto end;
				}
				OpenAPI_list_add(aoi_state_listList,
				                 localMapKeyPair);
			}
		}
	}
	...
	ext_amf_event_subscription_local_var =
		OpenAPI_ext_amf_event_subscription_create(
			event_listList,
			...
			subs_change_notify_uri &&
			!cJSON_IsNull(subs_change_notify_uri) ?
			ogs_strdup(subs_change_notify_uri->valuestring) : NULL,
			subs_change_notify_correlation_id &&
			!cJSON_IsNull(subs_change_notify_correlation_id) ?
			ogs_strdup(
				subs_change_notify_correlation_id->valuestring)
	: NULL,
			supi &&
			...
			group_id &&
			!cJSON_IsNull(group_id) ?
			ogs_strdup(group_id->valuestring) :
			NULL,
...
OpenAPI_ext_amf_event_subscription_t *OpenAPI_ext_amf_event_subscription_copy(
	OpenAPI_ext_amf_event_subscription_t	*dst,
	OpenAPI_ext_amf_event_subscription_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_ext_amf_event_subscription_convertToJSON() failed");
		return NULL;
	}
