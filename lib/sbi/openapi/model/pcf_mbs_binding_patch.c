...
OpenAPI_pcf_mbs_binding_patch_t *OpenAPI_pcf_mbs_binding_patch_create(
	...
	OpenAPI_pcf_mbs_binding_patch_t *pcf_mbs_binding_patch_local_var =
		ogs_malloc(sizeof(OpenAPI_pcf_mbs_binding_patch_t));
...
void OpenAPI_pcf_mbs_binding_patch_free(
	...
	if(pcf_mbs_binding_patch->pcf_ip_end_points) {
		OpenAPI_list_for_each(pcf_mbs_binding_patch->pcf_ip_end_points,
		                      node) {
...
cJSON *OpenAPI_pcf_mbs_binding_patch_convertToJSON(
	...
	if(pcf_mbs_binding_patch == NULL) {
		ogs_error(
			"OpenAPI_pcf_mbs_binding_patch_convertToJSON() failed [PcfMbsBindingPatch]");
		return NULL;
	}
	...
	if(pcf_mbs_binding_patch->pcf_fqdn) {
		if(cJSON_AddStringToObject(item, "pcfFqdn",
		                           pcf_mbs_binding_patch->pcf_fqdn) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pcf_mbs_binding_patch_convertToJSON() failed [pcf_fqdn]");
			goto end;
		}
	}
	...
	if(pcf_mbs_binding_patch->pcf_ip_end_points) {
		...
		if(pcf_ip_end_pointsList == NULL) {
			ogs_error(
				"OpenAPI_pcf_mbs_binding_patch_convertToJSON() failed [pcf_ip_end_points]");
			goto end;
		}
		OpenAPI_list_for_each(pcf_mbs_binding_patch->pcf_ip_end_points,
		                      node) {
	...
	if(pcf_mbs_binding_patch->pcf_id) {
		if(cJSON_AddStringToObject(item, "pcfId",
		                           pcf_mbs_binding_patch->pcf_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_pcf_mbs_binding_patch_convertToJSON() failed [pcf_id]");
			goto end;
		}
	}
...
OpenAPI_pcf_mbs_binding_patch_t *OpenAPI_pcf_mbs_binding_patch_parseFromJSON(
	...
	cJSON *pcf_id = NULL;
	pcf_fqdn = cJSON_GetObjectItemCaseSensitive(pcf_mbs_binding_patchJSON,
	                                            "pcfFqdn");
	if(pcf_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_mbs_binding_patch_parseFromJSON() failed [pcf_fqdn]");
			goto end;
		}
	}

	pcf_ip_end_points =
		cJSON_GetObjectItemCaseSensitive(pcf_mbs_binding_patchJSON,
		                                 "pcfIpEndPoints");
	if(pcf_ip_end_points) {
		cJSON *pcf_ip_end_points_local = NULL;
		if(!cJSON_IsArray(pcf_ip_end_points)) {
			ogs_error(
				"OpenAPI_pcf_mbs_binding_patch_parseFromJSON() failed [pcf_ip_end_points]");
			goto end;
		}
	...
	pcf_id = cJSON_GetObjectItemCaseSensitive(pcf_mbs_binding_patchJSON,
	                                          "pcfId");
	if(pcf_id) {
		...
		{
			ogs_error(
				"OpenAPI_pcf_mbs_binding_patch_parseFromJSON() failed [pcf_id]");
			goto end;
		}
	}
...
OpenAPI_pcf_mbs_binding_patch_t *OpenAPI_pcf_mbs_binding_patch_copy(
	...
	if(!item) {
		ogs_error(
			"OpenAPI_pcf_mbs_binding_patch_convertToJSON() failed");
		return NULL;
	}
