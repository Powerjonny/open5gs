...
OpenAPI_nnwdaf_nf_status_t *OpenAPI_nnwdaf_nf_status_create(
	bool is_status_registered, int status_registered,
	bool is_status_unregistered, int status_unregistered,
	bool is_status_undiscoverable, int status_undiscoverable) {
	OpenAPI_nnwdaf_nf_status_t *nnwdaf_nf_status_local_var =
		ogs_malloc(sizeof(OpenAPI_nnwdaf_nf_status_t));
	...
	nnwdaf_nf_status_local_var->is_status_unregistered =
		is_status_unregistered;
	nnwdaf_nf_status_local_var->status_unregistered = status_unregistered;
	nnwdaf_nf_status_local_var->is_status_undiscoverable =
		is_status_undiscoverable;
	nnwdaf_nf_status_local_var->status_undiscoverable =
		status_undiscoverable;

	return nnwdaf_nf_status_local_var;
}

void OpenAPI_nnwdaf_nf_status_free(
	OpenAPI_nnwdaf_nf_status_t *nnwdaf_nf_status) {
...
cJSON *OpenAPI_nnwdaf_nf_status_convertToJSON(
	OpenAPI_nnwdaf_nf_status_t *nnwdaf_nf_status) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(nnwdaf_nf_status == NULL) {
		ogs_error(
			"OpenAPI_nnwdaf_nf_status_convertToJSON() failed [NnwdafNfStatus]");
		return NULL;
	}
	...
	if(nnwdaf_nf_status->is_status_registered) {
		if(cJSON_AddNumberToObject(item, "statusRegistered",
		                           nnwdaf_nf_status->status_registered)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_nnwdaf_nf_status_convertToJSON() failed [status_registered]");
			goto end;
		}
	}
	...
	if(nnwdaf_nf_status->is_status_unregistered) {
		if(cJSON_AddNumberToObject(item, "statusUnregistered",
		                           nnwdaf_nf_status->status_unregistered)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_nnwdaf_nf_status_convertToJSON() failed [status_unregistered]");
			goto end;
		}
	}
	...
	if(nnwdaf_nf_status->is_status_undiscoverable) {
		if(cJSON_AddNumberToObject(item, "statusUndiscoverable",
		                           nnwdaf_nf_status->
		                           status_undiscoverable) == NULL)
		{
			ogs_error(
				"OpenAPI_nnwdaf_nf_status_convertToJSON() failed [status_undiscoverable]");
			goto end;
		}
	}
...
OpenAPI_nnwdaf_nf_status_t *OpenAPI_nnwdaf_nf_status_parseFromJSON(
	cJSON *nnwdaf_nf_statusJSON) {
	...
	status_registered =
		cJSON_GetObjectItemCaseSensitive(nnwdaf_nf_statusJSON,
		                                 "statusRegistered");
	if(status_registered) {
		if(!cJSON_IsNumber(status_registered)) {
			ogs_error(
				"OpenAPI_nnwdaf_nf_status_parseFromJSON() failed [status_registered]");
			goto end;
		}
	}
	...
	if(status_unregistered) {
		if(!cJSON_IsNumber(status_unregistered)) {
			ogs_error(
				"OpenAPI_nnwdaf_nf_status_parseFromJSON() failed [status_unregistered]");
			goto end;
		}
	}
	...
	if(status_undiscoverable) {
		if(!cJSON_IsNumber(status_undiscoverable)) {
			ogs_error(
				"OpenAPI_nnwdaf_nf_status_parseFromJSON() failed [status_undiscoverable]");
			goto end;
		}
	}
...
OpenAPI_nnwdaf_nf_status_t *OpenAPI_nnwdaf_nf_status_copy(
	OpenAPI_nnwdaf_nf_status_t *dst, OpenAPI_nnwdaf_nf_status_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
