...
OpenAPI_serving_nf_identity_t *OpenAPI_serving_nf_identity_create(
	char *serv_nf_inst_id, OpenAPI_guami_t *guami,
	OpenAPI_an_gw_address_t *an_gw_addr,
	OpenAPI_sgsn_address_t *sgsn_addr) {
	OpenAPI_serving_nf_identity_t *serving_nf_identity_local_var =
		ogs_malloc(sizeof(OpenAPI_serving_nf_identity_t));
...
void OpenAPI_serving_nf_identity_free(
	OpenAPI_serving_nf_identity_t *serving_nf_identity) {
...
cJSON *OpenAPI_serving_nf_identity_convertToJSON(
	OpenAPI_serving_nf_identity_t *serving_nf_identity) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(serving_nf_identity == NULL) {
		ogs_error(
			"OpenAPI_serving_nf_identity_convertToJSON() failed [ServingNfIdentity]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(serving_nf_identity->serv_nf_inst_id) {
		if(cJSON_AddStringToObject(item, "servNfInstId",
		                           serving_nf_identity->serv_nf_inst_id)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_serving_nf_identity_convertToJSON() failed [serv_nf_inst_id]");
			goto end;
		}
	}
	...
	if(serving_nf_identity->guami) {
		cJSON *guami_local_JSON =
			OpenAPI_guami_convertToJSON(serving_nf_identity->guami);
		if(guami_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_serving_nf_identity_convertToJSON() failed [guami]");
			goto end;
		}
	...
	if(serving_nf_identity->an_gw_addr) {
		cJSON *an_gw_addr_local_JSON =
			OpenAPI_an_gw_address_convertToJSON(
				serving_nf_identity->an_gw_addr);
		if(an_gw_addr_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_serving_nf_identity_convertToJSON() failed [an_gw_addr]");
			goto end;
		}
	...
	if(serving_nf_identity->sgsn_addr) {
		cJSON *sgsn_addr_local_JSON =
			OpenAPI_sgsn_address_convertToJSON(
				serving_nf_identity->sgsn_addr);
		if(sgsn_addr_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_serving_nf_identity_convertToJSON() failed [sgsn_addr]");
			goto end;
		}
...
OpenAPI_serving_nf_identity_t *OpenAPI_serving_nf_identity_parseFromJSON(
	cJSON *serving_nf_identityJSON) {
	...
	serv_nf_inst_id =
		cJSON_GetObjectItemCaseSensitive(serving_nf_identityJSON,
		                                 "servNfInstId");
	if(serv_nf_inst_id) {
		if(!cJSON_IsString(serv_nf_inst_id) &&
		   !cJSON_IsNull(serv_nf_inst_id))
		{
			ogs_error(
				"OpenAPI_serving_nf_identity_parseFromJSON() failed [serv_nf_inst_id]");
			goto end;
		}
	}

	guami = cJSON_GetObjectItemCaseSensitive(serving_nf_identityJSON,
	                                         "guami");
	if(guami) {
	...
	an_gw_addr = cJSON_GetObjectItemCaseSensitive(serving_nf_identityJSON,
	                                              "anGwAddr");
	if(an_gw_addr) {
	...
	sgsn_addr = cJSON_GetObjectItemCaseSensitive(serving_nf_identityJSON,
	                                             "sgsnAddr");
	if(sgsn_addr) {
		sgsn_addr_local_nonprim =
			OpenAPI_sgsn_address_parseFromJSON(sgsn_addr);
		if(!sgsn_addr_local_nonprim) {
			ogs_error(
				"OpenAPI_sgsn_address_parseFromJSON failed [sgsn_addr]");
			goto end;
		}
	}
	...
	serving_nf_identity_local_var = OpenAPI_serving_nf_identity_create(
		serv_nf_inst_id &&
		!cJSON_IsNull(serv_nf_inst_id) ?
		ogs_strdup(serv_nf_inst_id->valuestring) : NULL,
...
OpenAPI_serving_nf_identity_t *OpenAPI_serving_nf_identity_copy(
	OpenAPI_serving_nf_identity_t	*dst,
	OpenAPI_serving_nf_identity_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
