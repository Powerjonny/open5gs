...
OpenAPI_af_event_notification_t *OpenAPI_af_event_notification_create(
	OpenAPI_npcf_af_event_e event, OpenAPI_list_t *flows) {
	OpenAPI_af_event_notification_t *af_event_notification_local_var =
		ogs_malloc(sizeof(OpenAPI_af_event_notification_t));
...
void OpenAPI_af_event_notification_free(
	OpenAPI_af_event_notification_t *af_event_notification) {
...
cJSON *OpenAPI_af_event_notification_convertToJSON(
	OpenAPI_af_event_notification_t *af_event_notification) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(af_event_notification == NULL) {
		ogs_error(
			"OpenAPI_af_event_notification_convertToJSON() failed [AfEventNotification]");
		return NULL;
	}
	...
	if(af_event_notification->event == OpenAPI_npcf_af_event_NULL) {
		ogs_error(
			"OpenAPI_af_event_notification_convertToJSON() failed [event]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "event",
	                           OpenAPI_npcf_af_event_ToString(
					   af_event_notification->event)) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_af_event_notification_convertToJSON() failed [event]");
	...
	if(af_event_notification->flows) {
		...
		if(flowsList == NULL) {
			ogs_error(
				"OpenAPI_af_event_notification_convertToJSON() failed [flows]");
			goto end;
		}
		OpenAPI_list_for_each(af_event_notification->flows, node) {
			cJSON *itemLocal =
...
OpenAPI_af_event_notification_t *OpenAPI_af_event_notification_parseFromJSON(
	cJSON *af_event_notificationJSON) {
	...
	event = cJSON_GetObjectItemCaseSensitive(af_event_notificationJSON,
	                                         "event");
	if(!event) {
		ogs_error(
			"OpenAPI_af_event_notification_parseFromJSON() failed [event]");
		goto end;
	}
	...
	flows = cJSON_GetObjectItemCaseSensitive(af_event_notificationJSON,
	                                         "flows");
	if(flows) {
		cJSON *flows_local = NULL;
		if(!cJSON_IsArray(flows)) {
			ogs_error(
				"OpenAPI_af_event_notification_parseFromJSON() failed [flows]");
			goto end;
		}
...
OpenAPI_af_event_notification_t *OpenAPI_af_event_notification_copy(
	OpenAPI_af_event_notification_t *dst,
	OpenAPI_af_event_notification_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_af_event_notification_convertToJSON() failed");
		return NULL;
	}
