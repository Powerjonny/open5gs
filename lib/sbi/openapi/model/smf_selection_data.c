...
OpenAPI_smf_selection_data_t *OpenAPI_smf_selection_data_create(
	bool is_unsupp_dnn, int unsupp_dnn, bool is_candidates_null,
	OpenAPI_list_t *candidates, OpenAPI_snssai_t *snssai,
	OpenAPI_snssai_t *mapping_snssai, char *dnn) {
	OpenAPI_smf_selection_data_t *smf_selection_data_local_var =
		ogs_malloc(sizeof(OpenAPI_smf_selection_data_t));
...
void OpenAPI_smf_selection_data_free(
	OpenAPI_smf_selection_data_t *smf_selection_data) {
	...
	if(smf_selection_data->candidates) {
		OpenAPI_list_for_each(smf_selection_data->candidates, node) {
			...
			OpenAPI_candidate_for_replacement_free(
				localKeyValue->value);
...
cJSON *OpenAPI_smf_selection_data_convertToJSON(
	OpenAPI_smf_selection_data_t *smf_selection_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(smf_selection_data == NULL) {
		ogs_error(
			"OpenAPI_smf_selection_data_convertToJSON() failed [SmfSelectionData]");
		return NULL;
	}
	...
	if(smf_selection_data->is_unsupp_dnn) {
		...
		{
			ogs_error(
				"OpenAPI_smf_selection_data_convertToJSON() failed [unsupp_dnn]");
			goto end;
		}
	}
	...
	if(smf_selection_data->candidates) {
		...
		if(candidates == NULL) {
			ogs_error(
				"OpenAPI_smf_selection_data_convertToJSON() failed [candidates]");
			goto end;
		}
		cJSON *localMapObject = candidates;
		if(smf_selection_data->candidates) {
			OpenAPI_list_for_each(smf_selection_data->candidates,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				cJSON *itemLocal = localKeyValue->value ?
				                   OpenAPI_candidate_for_replacement_convertToJSON
				                           (localKeyValue->value)
				:
				                   cJSON_CreateNull();
				if(itemLocal == NULL) {
					ogs_error(
						"OpenAPI_smf_selection_data_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(smf_selection_data->snssai) {
		...
		if(snssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_smf_selection_data_convertToJSON() failed [snssai]");
			goto end;
		}
	...
	if(smf_selection_data->mapping_snssai) {
		cJSON *mapping_snssai_local_JSON =
			OpenAPI_snssai_convertToJSON(
				smf_selection_data->mapping_snssai);
		if(mapping_snssai_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_smf_selection_data_convertToJSON() failed [mapping_snssai]");
			goto end;
		}
	...
	if(smf_selection_data->dnn) {
		...
		{
			ogs_error(
				"OpenAPI_smf_selection_data_convertToJSON() failed [dnn]");
			goto end;
		}
	}
...
OpenAPI_smf_selection_data_t *OpenAPI_smf_selection_data_parseFromJSON(
	...
	if(unsupp_dnn) {
		if(!cJSON_IsBool(unsupp_dnn)) {
			ogs_error(
				"OpenAPI_smf_selection_data_parseFromJSON() failed [unsupp_dnn]");
			goto end;
		}
	}
	...
	if(candidates) {
		if(!cJSON_IsNull(candidates)) {
			...
			{
				ogs_error(
					"OpenAPI_smf_selection_data_parseFromJSON() failed [candidates]");
				goto end;
			}
			if(cJSON_IsObject(candidates)) {
				...
				cJSON_ArrayForEach(candidates_local_map,
						...
						localMapKeyPair =
							OpenAPI_map_create(
								ogs_strdup(
									localMapObject
									->string),
								OpenAPI_candidate_for_replacement_parseFromJSON
								(
									localMapObject));
							  ...
							  localMapObject))
					{
						...
						ogs_error(
							"OpenAPI_smf_selection_data_parseFromJSON() failed [inner]");
						goto end;
					}
					OpenAPI_list_add(candidatesList,
					                 localMapKeyPair);
				}
			}
		}
	...
	mapping_snssai =
		cJSON_GetObjectItemCaseSensitive(smf_selection_dataJSON,
		                                 "mappingSnssai");
	if(mapping_snssai) {
		mapping_snssai_local_nonprim =
			OpenAPI_snssai_parseFromJSON(mapping_snssai);
		if(!mapping_snssai_local_nonprim) {
			ogs_error(
				"OpenAPI_snssai_parseFromJSON failed [mapping_snssai]");
			goto end;
		}
	}
	...
	if(dnn) {
		...
		{
			ogs_error(
				"OpenAPI_smf_selection_data_parseFromJSON() failed [dnn]");
			goto end;
		}
	}
...
OpenAPI_smf_selection_data_t *OpenAPI_smf_selection_data_copy(
	OpenAPI_smf_selection_data_t *dst, OpenAPI_smf_selection_data_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
