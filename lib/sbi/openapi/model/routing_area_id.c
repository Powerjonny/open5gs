...
OpenAPI_routing_area_id_t *OpenAPI_routing_area_id_create(
	...
	OpenAPI_routing_area_id_t *routing_area_id_local_var =
		ogs_malloc(sizeof(OpenAPI_routing_area_id_t));
...
cJSON *OpenAPI_routing_area_id_convertToJSON(
	...
	if(routing_area_id == NULL) {
		ogs_error(
			"OpenAPI_routing_area_id_convertToJSON() failed [RoutingAreaId]");
		return NULL;
	}
	...
	if(!routing_area_id->plmn_id) {
		ogs_error(
			"OpenAPI_routing_area_id_convertToJSON() failed [plmn_id]");
		return NULL;
	}
	cJSON *plmn_id_local_JSON =
		OpenAPI_plmn_id_convertToJSON(routing_area_id->plmn_id);
	...
	if(!routing_area_id->lac) {
		ogs_error(
			"OpenAPI_routing_area_id_convertToJSON() failed [lac]");
		return NULL;
	}
	...
	if(!routing_area_id->rac) {
		ogs_error(
			"OpenAPI_routing_area_id_convertToJSON() failed [rac]");
		return NULL;
	}
...
OpenAPI_routing_area_id_t *OpenAPI_routing_area_id_parseFromJSON(
	...
	cJSON *lac = NULL;
	cJSON *rac = NULL;
	plmn_id = cJSON_GetObjectItemCaseSensitive(routing_area_idJSON,
	                                           "plmnId");
	if(!plmn_id) {
		ogs_error(
			"OpenAPI_routing_area_id_parseFromJSON() failed [plmn_id]");
		goto end;
	}
	...
	if(!lac) {
		ogs_error(
			"OpenAPI_routing_area_id_parseFromJSON() failed [lac]");
		goto end;
	}
	...
	if(!rac) {
		ogs_error(
			"OpenAPI_routing_area_id_parseFromJSON() failed [rac]");
		goto end;
	}
