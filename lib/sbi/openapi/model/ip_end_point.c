...
OpenAPI_ip_end_point_t *OpenAPI_ip_end_point_create(char *ipv4_address,
                                                    ...
                                                    int port) {
	OpenAPI_ip_end_point_t *ip_end_point_local_var =
		ogs_malloc(sizeof(OpenAPI_ip_end_point_t));
...
cJSON *OpenAPI_ip_end_point_convertToJSON(
	OpenAPI_ip_end_point_t *ip_end_point) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(ip_end_point == NULL) {
		ogs_error(
			"OpenAPI_ip_end_point_convertToJSON() failed [IpEndPoint]");
		return NULL;
	}
	...
	if(ip_end_point->ipv4_address) {
		...
		{
			ogs_error(
				"OpenAPI_ip_end_point_convertToJSON() failed [ipv4_address]");
			goto end;
		}
	}

	if(ip_end_point->ipv6_address) {
		if(cJSON_AddStringToObject(item, "ipv6Address",
		                           ip_end_point->ipv6_address) == NULL)
		{
			ogs_error(
				"OpenAPI_ip_end_point_convertToJSON() failed [ipv6_address]");
			goto end;
		}
	}

	if(ip_end_point->transport != OpenAPI_transport_protocol_NULL) {
		if(cJSON_AddStringToObject(item, "transport",
		                           OpenAPI_transport_protocol_ToString(
						   ip_end_point->transport)) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_ip_end_point_convertToJSON() failed [transport]");
			goto end;
		}
	}

	if(ip_end_point->is_port) {
		if(cJSON_AddNumberToObject(item, "port",
		                           ip_end_point->port) == NULL)
		{
			ogs_error(
				"OpenAPI_ip_end_point_convertToJSON() failed [port]");
			goto end;
		}
	}
...
OpenAPI_ip_end_point_t *OpenAPI_ip_end_point_parseFromJSON(
	...
	cJSON *port = NULL;
	ipv4_address = cJSON_GetObjectItemCaseSensitive(ip_end_pointJSON,
	                                                "ipv4Address");
	if(ipv4_address) {
		...
		{
			ogs_error(
				"OpenAPI_ip_end_point_parseFromJSON() failed [ipv4_address]");
			goto end;
		}
	}

	ipv6_address = cJSON_GetObjectItemCaseSensitive(ip_end_pointJSON,
	                                                "ipv6Address");
	if(ipv6_address) {
		if(!cJSON_IsString(ipv6_address) &&
		   !cJSON_IsNull(ipv6_address))
		{
			ogs_error(
				"OpenAPI_ip_end_point_parseFromJSON() failed [ipv6_address]");
			goto end;
		}
	}

	transport = cJSON_GetObjectItemCaseSensitive(ip_end_pointJSON,
	                                             "transport");
	if(transport) {
		if(!cJSON_IsString(transport)) {
			ogs_error(
				"OpenAPI_ip_end_point_parseFromJSON() failed [transport]");
			goto end;
		}
		transportVariable =
	...
	if(port) {
		if(!cJSON_IsNumber(port)) {
			ogs_error(
				"OpenAPI_ip_end_point_parseFromJSON() failed [port]");
			goto end;
		}
	}
	...
	ip_end_point_local_var = OpenAPI_ip_end_point_create(
		...
		ipv6_address &&
		!cJSON_IsNull(ipv6_address) ?
		ogs_strdup(ipv6_address->valuestring) : NULL,
