...
OpenAPI_notification_item_t *OpenAPI_notification_item_create(
	...
	OpenAPI_list_t
	        *notif_items) {
	OpenAPI_notification_item_t *notification_item_local_var =
		ogs_malloc(sizeof(OpenAPI_notification_item_t));
...
void OpenAPI_notification_item_free(
	OpenAPI_notification_item_t *notification_item) {
...
cJSON *OpenAPI_notification_item_convertToJSON(
	OpenAPI_notification_item_t *notification_item) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(notification_item == NULL) {
		ogs_error(
			"OpenAPI_notification_item_convertToJSON() failed [NotificationItem]");
		return NULL;
	}
	...
	if(!notification_item->resource_id) {
		ogs_error(
			"OpenAPI_notification_item_convertToJSON() failed [resource_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "resourceId",
	                           notification_item->resource_id) == NULL)
	{
		ogs_error(
			"OpenAPI_notification_item_convertToJSON() failed [resource_id]");
	...
	if(!notification_item->notif_items) {
		ogs_error(
			"OpenAPI_notification_item_convertToJSON() failed [notif_items]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(notification_item->notif_items, node) {
		cJSON *itemLocal =
			OpenAPI_updated_item_convertToJSON(node->data);
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_notification_item_convertToJSON() failed [notif_items]");
			goto end;
...
OpenAPI_notification_item_t *OpenAPI_notification_item_parseFromJSON(
	cJSON *notification_itemJSON) {
	...
	resource_id = cJSON_GetObjectItemCaseSensitive(notification_itemJSON,
	                                               "resourceId");
	if(!resource_id) {
		ogs_error(
			"OpenAPI_notification_item_parseFromJSON() failed [resource_id]");
		goto end;
	}
	...
	notif_items = cJSON_GetObjectItemCaseSensitive(notification_itemJSON,
	                                               "notifItems");
	if(!notif_items) {
		ogs_error(
			"OpenAPI_notification_item_parseFromJSON() failed [notif_items]");
		goto end;
	}
	...
	cJSON_ArrayForEach(notif_items_local, notif_items) {
		...
		OpenAPI_updated_item_t *notif_itemsItem =
			OpenAPI_updated_item_parseFromJSON(notif_items_local);
		if(!notif_itemsItem) {
...
OpenAPI_notification_item_t *OpenAPI_notification_item_copy(
	OpenAPI_notification_item_t *dst, OpenAPI_notification_item_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
