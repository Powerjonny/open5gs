...
OpenAPI_dnf_unit_t *OpenAPI_dnf_unit_create(OpenAPI_list_t *dnf_unit) {
	OpenAPI_dnf_unit_t *dnf_unit_local_var =
		ogs_malloc(sizeof(OpenAPI_dnf_unit_t));
...
cJSON *OpenAPI_dnf_unit_convertToJSON(OpenAPI_dnf_unit_t *dnf_unit) {
	...
	OpenAPI_list_for_each(dnf_unit->dnf_unit, node) {
		...
		if(itemLocal == NULL) {
			ogs_error(
				"OpenAPI_dnf_unit_convertToJSON() failed [dnf_unit]");
			goto end;
		}
...
OpenAPI_dnf_unit_t *OpenAPI_dnf_unit_parseFromJSON(cJSON *dnf_unitJSON) {
	...
	cJSON_ArrayForEach(dnf_unit_local, dnf_unit) {
		if(!cJSON_IsObject(dnf_unit_local)) {
			ogs_error(
				"OpenAPI_dnf_unit_parseFromJSON() failed [dnf_unit]");
			goto end;
		}
		OpenAPI_atom_t *dnf_unitItem =
			OpenAPI_atom_parseFromJSON(dnf_unit_local);
		if(!dnf_unitItem) {
