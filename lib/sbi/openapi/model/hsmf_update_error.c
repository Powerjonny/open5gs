...
OpenAPI_hsmf_update_error_t *OpenAPI_hsmf_update_error_create(
	OpenAPI_problem_details_t *error, bool is_pti, int pti,
	char *n1sm_cause, OpenAPI_ref_to_binary_data_t *n1_sm_info_to_ue,
	bool is_back_off_timer, int back_off_timer, char *recovery_time) {
	OpenAPI_hsmf_update_error_t *hsmf_update_error_local_var =
		ogs_malloc(sizeof(OpenAPI_hsmf_update_error_t));
...
void OpenAPI_hsmf_update_error_free(
	OpenAPI_hsmf_update_error_t *hsmf_update_error) {
...
cJSON *OpenAPI_hsmf_update_error_convertToJSON(
	OpenAPI_hsmf_update_error_t *hsmf_update_error) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(hsmf_update_error == NULL) {
		ogs_error(
			"OpenAPI_hsmf_update_error_convertToJSON() failed [HsmfUpdateError]");
		return NULL;
	}
	...
	if(!hsmf_update_error->error) {
		ogs_error(
			"OpenAPI_hsmf_update_error_convertToJSON() failed [error]");
		return NULL;
	}
	cJSON *error_local_JSON =
		OpenAPI_problem_details_convertToJSON(hsmf_update_error->error);
	...
	if(hsmf_update_error->is_pti) {
		if(cJSON_AddNumberToObject(item, "pti",
		                           hsmf_update_error->pti) == NULL)
		{
			ogs_error(
				"OpenAPI_hsmf_update_error_convertToJSON() failed [pti]");
			goto end;
		}
	}
	...
	if(hsmf_update_error->n1sm_cause) {
		if(cJSON_AddStringToObject(item, "n1smCause",
		                           hsmf_update_error->n1sm_cause) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_hsmf_update_error_convertToJSON() failed [n1sm_cause]");
			goto end;
		}
	}
	...
	if(hsmf_update_error->n1_sm_info_to_ue) {
		...
		if(n1_sm_info_to_ue_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_hsmf_update_error_convertToJSON() failed [n1_sm_info_to_ue]");
			goto end;
		}
	...
	if(hsmf_update_error->is_back_off_timer) {
		if(cJSON_AddNumberToObject(item, "backOffTimer",
		                           hsmf_update_error->back_off_timer) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_hsmf_update_error_convertToJSON() failed [back_off_timer]");
			goto end;
		}
	}
	...
	if(hsmf_update_error->recovery_time) {
		if(cJSON_AddStringToObject(item, "recoveryTime",
		                           hsmf_update_error->recovery_time) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_hsmf_update_error_convertToJSON() failed [recovery_time]");
			goto end;
		}
	}
...
OpenAPI_hsmf_update_error_t *OpenAPI_hsmf_update_error_parseFromJSON(
	cJSON *hsmf_update_errorJSON) {
	...
	error = cJSON_GetObjectItemCaseSensitive(hsmf_update_errorJSON,
	                                         "error");
	if(!error) {
		ogs_error(
			"OpenAPI_hsmf_update_error_parseFromJSON() failed [error]");
		goto end;
	}
	...
	if(pti) {
		if(!cJSON_IsNumber(pti)) {
			ogs_error(
				"OpenAPI_hsmf_update_error_parseFromJSON() failed [pti]");
			goto end;
		}
	}

	n1sm_cause = cJSON_GetObjectItemCaseSensitive(hsmf_update_errorJSON,
	                                              "n1smCause");
	if(n1sm_cause) {
		...
		{
			ogs_error(
				"OpenAPI_hsmf_update_error_parseFromJSON() failed [n1sm_cause]");
			goto end;
		}
	}

	n1_sm_info_to_ue =
		cJSON_GetObjectItemCaseSensitive(hsmf_update_errorJSON,
		                                 "n1SmInfoToUe");
	if(n1_sm_info_to_ue) {
	...
	back_off_timer = cJSON_GetObjectItemCaseSensitive(hsmf_update_errorJSON,
	                                                  "backOffTimer");
	if(back_off_timer) {
		if(!cJSON_IsNumber(back_off_timer)) {
			ogs_error(
				"OpenAPI_hsmf_update_error_parseFromJSON() failed [back_off_timer]");
			goto end;
		}
	}

	recovery_time = cJSON_GetObjectItemCaseSensitive(hsmf_update_errorJSON,
	                                                 "recoveryTime");
	if(recovery_time) {
		...
		{
			ogs_error(
				"OpenAPI_hsmf_update_error_parseFromJSON() failed [recovery_time]");
			goto end;
		}
	}
...
OpenAPI_hsmf_update_error_t *OpenAPI_hsmf_update_error_copy(
	OpenAPI_hsmf_update_error_t *dst, OpenAPI_hsmf_update_error_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
