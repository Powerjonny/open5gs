...
OpenAPI_address_list_t *OpenAPI_address_list_create(
	...
	OpenAPI_list_t	    *
	ipv6_addrs) {
	OpenAPI_address_list_t *address_list_local_var =
		ogs_malloc(sizeof(OpenAPI_address_list_t));
...
cJSON *OpenAPI_address_list_convertToJSON(
	OpenAPI_address_list_t *address_list) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(address_list == NULL) {
		ogs_error(
			"OpenAPI_address_list_convertToJSON() failed [AddressList]");
		return NULL;
	}
	...
	if(address_list->ipv4_addrs) {
		...
		if(ipv4_addrsList == NULL) {
			ogs_error(
				"OpenAPI_address_list_convertToJSON() failed [ipv4_addrs]");
			goto end;
		}
		OpenAPI_list_for_each(address_list->ipv4_addrs, node) {
	...
	if(address_list->ipv6_addrs) {
		...
		if(ipv6_addrsList == NULL) {
			ogs_error(
				"OpenAPI_address_list_convertToJSON() failed [ipv6_addrs]");
			goto end;
		}
		OpenAPI_list_for_each(address_list->ipv6_addrs, node) {
...
OpenAPI_address_list_t *OpenAPI_address_list_parseFromJSON(
	...
	if(ipv4_addrs) {
		...
		if(!cJSON_IsArray(ipv4_addrs)) {
			ogs_error(
				"OpenAPI_address_list_parseFromJSON() failed [ipv4_addrs]");
			goto end;
		}
	...
	if(ipv6_addrs) {
		...
		if(!cJSON_IsArray(ipv6_addrs)) {
			ogs_error(
				"OpenAPI_address_list_parseFromJSON() failed [ipv6_addrs]");
			goto end;
		}
