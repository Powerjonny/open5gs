...
OpenAPI_sm_policy_control_t *OpenAPI_sm_policy_control_create(
	...
	OpenAPI_sm_policy_control_t *sm_policy_control_local_var =
		ogs_malloc(sizeof(OpenAPI_sm_policy_control_t));
...
void OpenAPI_sm_policy_control_free(
	OpenAPI_sm_policy_control_t *sm_policy_control) {
...
cJSON *OpenAPI_sm_policy_control_convertToJSON(
	...
	if(sm_policy_control == NULL) {
		ogs_error(
			"OpenAPI_sm_policy_control_convertToJSON() failed [SmPolicyControl]");
		return NULL;
	}
	...
	if(!sm_policy_control->context) {
		ogs_error(
			"OpenAPI_sm_policy_control_convertToJSON() failed [context]");
		return NULL;
	}
	cJSON *context_local_JSON =
		OpenAPI_sm_policy_context_data_convertToJSON(
			sm_policy_control->context);
	...
	if(!sm_policy_control->policy) {
		ogs_error(
			"OpenAPI_sm_policy_control_convertToJSON() failed [policy]");
		return NULL;
	}
	cJSON *policy_local_JSON =
		OpenAPI_sm_policy_decision_convertToJSON(
			sm_policy_control->policy);
...
OpenAPI_sm_policy_control_t *OpenAPI_sm_policy_control_parseFromJSON(
	cJSON *sm_policy_controlJSON) {
	...
	context = cJSON_GetObjectItemCaseSensitive(sm_policy_controlJSON,
	                                           "context");
	if(!context) {
		ogs_error(
			"OpenAPI_sm_policy_control_parseFromJSON() failed [context]");
		goto end;
	}
	context_local_nonprim =
		OpenAPI_sm_policy_context_data_parseFromJSON(context);
	if(!context_local_nonprim) {
		ogs_error(
			"OpenAPI_sm_policy_context_data_parseFromJSON failed [context]");
		goto end;
	}

	policy = cJSON_GetObjectItemCaseSensitive(sm_policy_controlJSON,
	                                          "policy");
	if(!policy) {
		ogs_error(
			"OpenAPI_sm_policy_control_parseFromJSON() failed [policy]");
		goto end;
	}
	policy_local_nonprim = OpenAPI_sm_policy_decision_parseFromJSON(policy);
	if(!policy_local_nonprim) {
		ogs_error(
			"OpenAPI_sm_policy_decision_parseFromJSON failed [policy]");
		goto end;
	}
...
OpenAPI_sm_policy_control_t *OpenAPI_sm_policy_control_copy(
	OpenAPI_sm_policy_control_t *dst, OpenAPI_sm_policy_control_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
