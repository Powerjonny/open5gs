...
OpenAPI_nr_v2x_auth_t *OpenAPI_nr_v2x_auth_create(
	...
	OpenAPI_nr_v2x_auth_t *nr_v2x_auth_local_var =
		ogs_malloc(sizeof(OpenAPI_nr_v2x_auth_t));
...
cJSON *OpenAPI_nr_v2x_auth_convertToJSON(OpenAPI_nr_v2x_auth_t *nr_v2x_auth) {
	...
	if(nr_v2x_auth == NULL) {
		ogs_error(
			"OpenAPI_nr_v2x_auth_convertToJSON() failed [NrV2xAuth]");
		return NULL;
	}
	...
	if(nr_v2x_auth->vehicle_ue_auth != OpenAPI_ue_auth_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_nr_v2x_auth_convertToJSON() failed [vehicle_ue_auth]");
			goto end;
		}
	}
	...
	if(nr_v2x_auth->pedestrian_ue_auth != OpenAPI_ue_auth_NULL) {
		...
		{
			ogs_error(
				"OpenAPI_nr_v2x_auth_convertToJSON() failed [pedestrian_ue_auth]");
			goto end;
		}
	}
...
OpenAPI_nr_v2x_auth_t *OpenAPI_nr_v2x_auth_parseFromJSON(
	...
	if(vehicle_ue_auth) {
		if(!cJSON_IsString(vehicle_ue_auth)) {
			ogs_error(
				"OpenAPI_nr_v2x_auth_parseFromJSON() failed [vehicle_ue_auth]");
			goto end;
		}
	...
	if(pedestrian_ue_auth) {
		if(!cJSON_IsString(pedestrian_ue_auth)) {
			ogs_error(
				"OpenAPI_nr_v2x_auth_parseFromJSON() failed [pedestrian_ue_auth]");
			goto end;
		}
