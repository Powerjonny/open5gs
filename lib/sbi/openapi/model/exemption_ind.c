...
OpenAPI_exemption_ind_t *OpenAPI_exemption_ind_create(
	...
	int	dnn_congestion,
	bool
	is_snssai_only_congestion,
	int snssai_only_congestion,
	bool is_snssai_dnn_congestion, int snssai_dnn_congestion) {
	OpenAPI_exemption_ind_t *exemption_ind_local_var =
		ogs_malloc(sizeof(OpenAPI_exemption_ind_t));
	...
	exemption_ind_local_var->is_snssai_only_congestion =
		is_snssai_only_congestion;
	exemption_ind_local_var->snssai_only_congestion =
		snssai_only_congestion;
	exemption_ind_local_var->is_snssai_dnn_congestion =
		is_snssai_dnn_congestion;
...
cJSON *OpenAPI_exemption_ind_convertToJSON(
	...
	if(exemption_ind == NULL) {
		ogs_error(
			"OpenAPI_exemption_ind_convertToJSON() failed [ExemptionInd]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(exemption_ind->is_dnn_congestion) {
		if(cJSON_AddBoolToObject(item, "dnnCongestion",
		                         exemption_ind->dnn_congestion) == NULL)
		{
			ogs_error(
				"OpenAPI_exemption_ind_convertToJSON() failed [dnn_congestion]");
			goto end;
		}
	}

	if(exemption_ind->is_snssai_only_congestion) {
		if(cJSON_AddBoolToObject(item, "snssaiOnlyCongestion",
		                         exemption_ind->snssai_only_congestion)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_exemption_ind_convertToJSON() failed [snssai_only_congestion]");
			goto end;
		}
	}

	if(exemption_ind->is_snssai_dnn_congestion) {
		if(cJSON_AddBoolToObject(item, "snssaiDnnCongestion",
		                         exemption_ind->snssai_dnn_congestion)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_exemption_ind_convertToJSON() failed [snssai_dnn_congestion]");
			goto end;
		}
	}
...
OpenAPI_exemption_ind_t *OpenAPI_exemption_ind_parseFromJSON(
	cJSON *exemption_indJSON) {
	...
	dnn_congestion = cJSON_GetObjectItemCaseSensitive(exemption_indJSON,
	                                                  "dnnCongestion");
	if(dnn_congestion) {
		if(!cJSON_IsBool(dnn_congestion)) {
			ogs_error(
				"OpenAPI_exemption_ind_parseFromJSON() failed [dnn_congestion]");
			goto end;
		}
	}

	snssai_only_congestion =
		cJSON_GetObjectItemCaseSensitive(exemption_indJSON,
		                                 "snssaiOnlyCongestion");
	if(snssai_only_congestion) {
		if(!cJSON_IsBool(snssai_only_congestion)) {
			ogs_error(
				"OpenAPI_exemption_ind_parseFromJSON() failed [snssai_only_congestion]");
			goto end;
		}
	}

	snssai_dnn_congestion =
		cJSON_GetObjectItemCaseSensitive(exemption_indJSON,
		                                 "snssaiDnnCongestion");
	if(snssai_dnn_congestion) {
		if(!cJSON_IsBool(snssai_dnn_congestion)) {
			ogs_error(
				"OpenAPI_exemption_ind_parseFromJSON() failed [snssai_dnn_congestion]");
			goto end;
		}
	}
...
OpenAPI_exemption_ind_t *OpenAPI_exemption_ind_copy(
	OpenAPI_exemption_ind_t *dst, OpenAPI_exemption_ind_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
