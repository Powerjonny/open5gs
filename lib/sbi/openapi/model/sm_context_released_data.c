...
OpenAPI_sm_context_released_data_t *OpenAPI_sm_context_released_data_create(
	...
	OpenAPI_sm_context_released_data_t *sm_context_released_data_local_var =
		ogs_malloc(sizeof(OpenAPI_sm_context_released_data_t));
	ogs_assert(sm_context_released_data_local_var);

	sm_context_released_data_local_var->small_data_rate_status =
...
void OpenAPI_sm_context_released_data_free(
	OpenAPI_sm_context_released_data_t *sm_context_released_data) {
	...
	if(sm_context_released_data->small_data_rate_status) {
		OpenAPI_small_data_rate_status_free(
			sm_context_released_data->small_data_rate_status);
	...
	if(sm_context_released_data->apn_rate_status) {
		OpenAPI_apn_rate_status_free(
			sm_context_released_data->apn_rate_status);
		sm_context_released_data->apn_rate_status = NULL;
	}
	ogs_free(sm_context_released_data);
}

cJSON *OpenAPI_sm_context_released_data_convertToJSON(
	OpenAPI_sm_context_released_data_t *sm_context_released_data) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(sm_context_released_data == NULL) {
		ogs_error(
			"OpenAPI_sm_context_released_data_convertToJSON() failed [SmContextReleasedData]");
		return NULL;
	}
	...
	if(sm_context_released_data->small_data_rate_status) {
		...
		if(small_data_rate_status_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_context_released_data_convertToJSON() failed [small_data_rate_status]");
			goto end;
		}
	...
	if(sm_context_released_data->apn_rate_status) {
		...
		if(apn_rate_status_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_sm_context_released_data_convertToJSON() failed [apn_rate_status]");
			goto end;
		}
...
OpenAPI_sm_context_released_data_parseFromJSON(
	...
	small_data_rate_status =
		cJSON_GetObjectItemCaseSensitive(sm_context_released_dataJSON,
		                                 "smallDataRateStatus");
	...
	sm_context_released_data_local_var =
...
OpenAPI_sm_context_released_data_t *OpenAPI_sm_context_released_data_copy(
	OpenAPI_sm_context_released_data_t	*dst,
	OpenAPI_sm_context_released_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_sm_context_released_data_convertToJSON() failed");
		return NULL;
	}
