...
OpenAPI_service_name_cond_t *OpenAPI_service_name_cond_create(
	char *service_name) {
	OpenAPI_service_name_cond_t *service_name_cond_local_var =
		ogs_malloc(sizeof(OpenAPI_service_name_cond_t));
...
cJSON *OpenAPI_service_name_cond_convertToJSON(
	...
	if(service_name_cond == NULL) {
		ogs_error(
			"OpenAPI_service_name_cond_convertToJSON() failed [ServiceNameCond]");
		return NULL;
	}
	...
	if(!service_name_cond->service_name) {
		ogs_error(
			"OpenAPI_service_name_cond_convertToJSON() failed [service_name]");
		return NULL;
	}
...
OpenAPI_service_name_cond_t *OpenAPI_service_name_cond_parseFromJSON(
	...
	service_name = cJSON_GetObjectItemCaseSensitive(service_name_condJSON,
	                                                "serviceName");
	if(!service_name) {
		ogs_error(
			"OpenAPI_service_name_cond_parseFromJSON() failed [service_name]");
		goto end;
	}
