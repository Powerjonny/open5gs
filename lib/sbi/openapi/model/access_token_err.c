...
char *OpenAPI_erroraccess_token_err_ToString(
	OpenAPI_access_token_err_error_e error) {
	const char *errorArray[] = { "NULL", "invalid_request",
...
OpenAPI_access_token_err_error_e OpenAPI_erroraccess_token_err_FromString(
	char *error) {
...
OpenAPI_access_token_err_t *OpenAPI_access_token_err_create(
	OpenAPI_access_token_err_error_e error, char *error_description,
	char *error_uri) {
	OpenAPI_access_token_err_t *access_token_err_local_var =
		ogs_malloc(sizeof(OpenAPI_access_token_err_t));
...
void OpenAPI_access_token_err_free(
	OpenAPI_access_token_err_t *access_token_err) {
...
cJSON *OpenAPI_access_token_err_convertToJSON(
	OpenAPI_access_token_err_t *access_token_err) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(access_token_err == NULL) {
		ogs_error(
			"OpenAPI_access_token_err_convertToJSON() failed [AccessTokenErr]");
		return NULL;
	}
	...
	if(access_token_err->error == OpenAPI_access_token_err_ERROR_NULL) {
		ogs_error(
			"OpenAPI_access_token_err_convertToJSON() failed [error]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "error",
	                           OpenAPI_erroraccess_token_err_ToString(
	...
	if(access_token_err->error_description) {
		if(cJSON_AddStringToObject(item, "error_description",
		   ...
		   == NULL) {
			ogs_error(
				"OpenAPI_access_token_err_convertToJSON() failed [error_description]");
			goto end;
		}
	}
	...
	if(access_token_err->error_uri) {
		if(cJSON_AddStringToObject(item, "error_uri",
		   ...
		   NULL) {
			ogs_error(
				"OpenAPI_access_token_err_convertToJSON() failed [error_uri]");
			goto end;
		}
	}
...
OpenAPI_access_token_err_t *OpenAPI_access_token_err_parseFromJSON(
	cJSON *access_token_errJSON) {
	...
	if(!error) {
		ogs_error(
			"OpenAPI_access_token_err_parseFromJSON() failed [error]");
		goto end;
	}
	...
	errorVariable =
		OpenAPI_erroraccess_token_err_FromString(error->valuestring);

	error_description =
		cJSON_GetObjectItemCaseSensitive(access_token_errJSON,
		                                 "error_description");
	if(error_description) {
		...
		{
			ogs_error(
				"OpenAPI_access_token_err_parseFromJSON() failed [error_description]");
			goto end;
		}
	}
	...
	if(error_uri) {
		...
		{
			ogs_error(
				"OpenAPI_access_token_err_parseFromJSON() failed [error_uri]");
			goto end;
		}
	}
	...
	access_token_err_local_var = OpenAPI_access_token_err_create(
		...
		error_description &&
		!cJSON_IsNull(error_description) ?
		ogs_strdup(error_description->valuestring) : NULL,
		error_uri &&
		!cJSON_IsNull(error_uri) ? ogs_strdup(error_uri->valuestring) :
		NULL
		);
...
OpenAPI_access_token_err_t *OpenAPI_access_token_err_copy(
	OpenAPI_access_token_err_t *dst, OpenAPI_access_token_err_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
