...
OpenAPI_qos_monitoring_data_t *OpenAPI_qos_monitoring_data_create(char *qm_id,
                                                                  OpenAPI_list_t
                                                                  *
                                                                  req_qos_mon_params,
                                                                  OpenAPI_list_t
                                                                  *rep_freqs,
                                                                  ...
                                                                  rep_thresh_dl,
                                                                  bool
                                                                  is_rep_thresh_ul_null,
                                                                  bool
                                                                  is_rep_thresh_ul,
                                                                  int
                                                                  rep_thresh_ul,
                                                                  bool
                                                                  is_rep_thresh_rp_null,
                                                                  bool
                                                                  is_rep_thresh_rp,
                                                                  int
                                                                  rep_thresh_rp,
                                                                  ...
                                                                  direct_notif_ind)
{
	OpenAPI_qos_monitoring_data_t *qos_monitoring_data_local_var =
		ogs_malloc(sizeof(OpenAPI_qos_monitoring_data_t));
	...
	qos_monitoring_data_local_var->is_notify_corre_id_null =
		is_notify_corre_id_null;
	qos_monitoring_data_local_var->notify_corre_id = notify_corre_id;
	qos_monitoring_data_local_var->is_direct_notif_ind =
		is_direct_notif_ind;
...
void OpenAPI_qos_monitoring_data_free(
	OpenAPI_qos_monitoring_data_t *qos_monitoring_data) {
...
cJSON *OpenAPI_qos_monitoring_data_convertToJSON(
	...
	if(qos_monitoring_data == NULL) {
		ogs_error(
			"OpenAPI_qos_monitoring_data_convertToJSON() failed [QosMonitoringData]");
		return NULL;
	}
	...
	if(!qos_monitoring_data->qm_id) {
		ogs_error(
			"OpenAPI_qos_monitoring_data_convertToJSON() failed [qm_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "qmId",
	                           qos_monitoring_data->qm_id) == NULL)
	{
		ogs_error(
			"OpenAPI_qos_monitoring_data_convertToJSON() failed [qm_id]");
	...
	{
		ogs_error(
			"OpenAPI_qos_monitoring_data_convertToJSON() failed [req_qos_mon_params]");
		return NULL;
	}
	...
	if(qos_monitoring_data->rep_freqs == OpenAPI_reporting_frequency_NULL) {
		ogs_error(
			"OpenAPI_qos_monitoring_data_convertToJSON() failed [rep_freqs]");
		return NULL;
	}
	...
	OpenAPI_list_for_each(qos_monitoring_data->rep_freqs, node) {
		if(cJSON_AddStringToObject(rep_freqsList, "",
		                           OpenAPI_reporting_frequency_ToString(
						   (intptr_t) node->data)) ==
		   NULL)
		{
			ogs_error(
	...
	if(qos_monitoring_data->is_rep_thresh_dl) {
		if(cJSON_AddNumberToObject(item, "repThreshDl",
		                           qos_monitoring_data->rep_thresh_dl)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_qos_monitoring_data_convertToJSON() failed [rep_thresh_dl]");
			goto end;
		}
	...
	if(qos_monitoring_data->is_rep_thresh_ul) {
		if(cJSON_AddNumberToObject(item, "repThreshUl",
		                           qos_monitoring_data->rep_thresh_ul)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_qos_monitoring_data_convertToJSON() failed [rep_thresh_ul]");
			goto end;
		}
	...
	if(qos_monitoring_data->is_rep_thresh_rp) {
		if(cJSON_AddNumberToObject(item, "repThreshRp",
		                           qos_monitoring_data->rep_thresh_rp)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_qos_monitoring_data_convertToJSON() failed [rep_thresh_rp]");
			goto end;
		}
	...
	if(qos_monitoring_data->is_wait_time) {
		if(cJSON_AddNumberToObject(item, "waitTime",
		                           qos_monitoring_data->wait_time) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_qos_monitoring_data_convertToJSON() failed [wait_time]");
			goto end;
		}
	...
	if(qos_monitoring_data->is_rep_period) {
		if(cJSON_AddNumberToObject(item, "repPeriod",
		                           qos_monitoring_data->rep_period) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_qos_monitoring_data_convertToJSON() failed [rep_period]");
			goto end;
		}
	...
	if(qos_monitoring_data->notify_uri) {
		if(cJSON_AddStringToObject(item, "notifyUri",
		                           qos_monitoring_data->notify_uri) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_qos_monitoring_data_convertToJSON() failed [notify_uri]");
			goto end;
		}
	...
	if(qos_monitoring_data->notify_corre_id) {
		if(cJSON_AddStringToObject(item, "notifyCorreId",
		                           qos_monitoring_data->notify_corre_id)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_qos_monitoring_data_convertToJSON() failed [notify_corre_id]");
			goto end;
		}
	...
	if(qos_monitoring_data->is_direct_notif_ind) {
		if(cJSON_AddBoolToObject(item, "directNotifInd",
		                         qos_monitoring_data->direct_notif_ind)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_qos_monitoring_data_convertToJSON() failed [direct_notif_ind]");
			goto end;
		}
	}
...
OpenAPI_qos_monitoring_data_t *OpenAPI_qos_monitoring_data_parseFromJSON(
	...
	qm_id = cJSON_GetObjectItemCaseSensitive(qos_monitoring_dataJSON,
	                                         "qmId");
	if(!qm_id) {
		ogs_error(
			"OpenAPI_qos_monitoring_data_parseFromJSON() failed [qm_id]");
		goto end;
	}
	...
	req_qos_mon_params =
		cJSON_GetObjectItemCaseSensitive(qos_monitoring_dataJSON,
		                                 "reqQosMonParams");
	if(!req_qos_mon_params) {
		ogs_error(
			"OpenAPI_qos_monitoring_data_parseFromJSON() failed [req_qos_mon_params]");
		goto end;
	}
	...
	if(req_qos_mon_paramsList->count == 0) {
		ogs_error(
			"OpenAPI_qos_monitoring_data_parseFromJSON() failed: Expected req_qos_mon_paramsList to not be empty (after ignoring unsupported enum values).");
		goto end;
	}

	rep_freqs = cJSON_GetObjectItemCaseSensitive(qos_monitoring_dataJSON,
	                                             "repFreqs");
	if(!rep_freqs) {
		ogs_error(
			"OpenAPI_qos_monitoring_data_parseFromJSON() failed [rep_freqs]");
		goto end;
	}
	...
	cJSON_ArrayForEach(rep_freqs_local, rep_freqs) {
		OpenAPI_reporting_frequency_e localEnum =
			OpenAPI_reporting_frequency_NULL;
		...
		localEnum =
			OpenAPI_reporting_frequency_FromString(
				rep_freqs_local->valuestring);
		if(!localEnum) {
			ogs_info(
				"Enum value \"%s\" for field \"rep_freqs\" is not supported. Ignoring it ...",
				rep_freqs_local->valuestring);
		} else {
	...
	if(rep_freqsList->count == 0) {
		ogs_error(
			"OpenAPI_qos_monitoring_data_parseFromJSON() failed: Expected rep_freqsList to not be empty (after ignoring unsupported enum values).");
		goto end;
	}
	...
	if(rep_thresh_dl) {
		if(!cJSON_IsNull(rep_thresh_dl)) {
			if(!cJSON_IsNumber(rep_thresh_dl)) {
				ogs_error(
					"OpenAPI_qos_monitoring_data_parseFromJSON() failed [rep_thresh_dl]");
				goto end;
			}
		}
	}
	...
	if(rep_thresh_ul) {
		if(!cJSON_IsNull(rep_thresh_ul)) {
			if(!cJSON_IsNumber(rep_thresh_ul)) {
				ogs_error(
					"OpenAPI_qos_monitoring_data_parseFromJSON() failed [rep_thresh_ul]");
				goto end;
			}
		}
	}
	...
	wait_time = cJSON_GetObjectItemCaseSensitive(qos_monitoring_dataJSON,
	                                             "waitTime");
	if(wait_time) {
		if(!cJSON_IsNull(wait_time)) {
			if(!cJSON_IsNumber(wait_time)) {
				ogs_error(
					"OpenAPI_qos_monitoring_data_parseFromJSON() failed [wait_time]");
				goto end;
			}
		}
	}

	rep_period = cJSON_GetObjectItemCaseSensitive(qos_monitoring_dataJSON,
	                                              "repPeriod");
	if(rep_period) {
		if(!cJSON_IsNull(rep_period)) {
			if(!cJSON_IsNumber(rep_period)) {
				ogs_error(
					"OpenAPI_qos_monitoring_data_parseFromJSON() failed [rep_period]");
				goto end;
			}
		}
	}

	notify_uri = cJSON_GetObjectItemCaseSensitive(qos_monitoring_dataJSON,
	                                              "notifyUri");
	if(notify_uri) {
		if(!cJSON_IsNull(notify_uri)) {
			if(!cJSON_IsString(notify_uri) &&
			   !cJSON_IsNull(notify_uri))
			{
				ogs_error(
					"OpenAPI_qos_monitoring_data_parseFromJSON() failed [notify_uri]");
				goto end;
			}
		}
	}
	...
	if(notify_corre_id) {
		if(!cJSON_IsNull(notify_corre_id)) {
			if(!cJSON_IsString(notify_corre_id) &&
			   !cJSON_IsNull(notify_corre_id))
			{
				ogs_error(
					"OpenAPI_qos_monitoring_data_parseFromJSON() failed [notify_corre_id]");
				goto end;
			}
		}
	}
	...
	if(direct_notif_ind) {
		if(!cJSON_IsBool(direct_notif_ind)) {
			ogs_error(
				"OpenAPI_qos_monitoring_data_parseFromJSON() failed [direct_notif_ind]");
			goto end;
		}
	}
	...
	qos_monitoring_data_local_var = OpenAPI_qos_monitoring_data_create(
		...
		rep_freqsList,
		rep_thresh_dl &&
		cJSON_IsNull(rep_thresh_dl) ? true : false,
		rep_thresh_dl ? true : false,
		rep_thresh_dl ? rep_thresh_dl->valuedouble : 0,
		rep_thresh_ul &&
		cJSON_IsNull(rep_thresh_ul) ? true : false,
		rep_thresh_ul ? true : false,
		rep_thresh_ul ? rep_thresh_ul->valuedouble : 0,
		rep_thresh_rp &&
		cJSON_IsNull(rep_thresh_rp) ? true : false,
		rep_thresh_rp ? true : false,
		rep_thresh_rp ? rep_thresh_rp->valuedouble : 0,
		wait_time &&
		cJSON_IsNull(wait_time) ? true : false,
		wait_time ? true : false,
		wait_time ? wait_time->valuedouble : 0,
		rep_period &&
		cJSON_IsNull(rep_period) ? true : false,
		rep_period ? true : false,
		rep_period ? rep_period->valuedouble : 0,
		notify_uri &&
		cJSON_IsNull(notify_uri) ? true : false,
		notify_uri &&
		!cJSON_IsNull(notify_uri) ?
		ogs_strdup(notify_uri->valuestring) : NULL,
		notify_corre_id &&
		cJSON_IsNull(notify_corre_id) ? true : false,
		notify_corre_id &&
		!cJSON_IsNull(notify_corre_id) ?
		ogs_strdup(notify_corre_id->valuestring) : NULL,
...
OpenAPI_qos_monitoring_data_t *OpenAPI_qos_monitoring_data_copy(
	OpenAPI_qos_monitoring_data_t	*dst,
	OpenAPI_qos_monitoring_data_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
