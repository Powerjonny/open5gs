...
char * OpenAPI_user_consent_ToString(OpenAPI_user_consent_e user_consent) {
	const char *user_consentArray[] = { "NULL", "CONSENT_NOT_GIVEN",
		                            "CONSENT_GIVEN" };
	size_t sizeofArray = sizeof(user_consentArray) /
	                     sizeof(user_consentArray[0]);
...
OpenAPI_user_consent_e OpenAPI_user_consent_FromString(char *user_consent) {
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(user_consent,
		          user_consentArray[stringToReturn]) == 0) {
