...
OpenAPI_id_translation_result_t *OpenAPI_id_translation_result_create(
	char *supported_features, char *supi, char *gpsi,
	OpenAPI_list_t *additional_supis, OpenAPI_list_t *additional_gpsis) {
	OpenAPI_id_translation_result_t *id_translation_result_local_var =
		ogs_malloc(sizeof(OpenAPI_id_translation_result_t));
	ogs_assert(id_translation_result_local_var);

	id_translation_result_local_var->supported_features =
		supported_features;
...
void OpenAPI_id_translation_result_free(
	OpenAPI_id_translation_result_t *id_translation_result) {
	...
	if(id_translation_result->additional_supis) {
		OpenAPI_list_for_each(id_translation_result->additional_supis,
		                      node) {
			ogs_free(node->data);
		}
	...
	if(id_translation_result->additional_gpsis) {
		OpenAPI_list_for_each(id_translation_result->additional_gpsis,
		                      node) {
			ogs_free(node->data);
		}
...
cJSON *OpenAPI_id_translation_result_convertToJSON(
	OpenAPI_id_translation_result_t *id_translation_result) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(id_translation_result == NULL) {
		ogs_error(
			"OpenAPI_id_translation_result_convertToJSON() failed [IdTranslationResult]");
		return NULL;
	}
	...
	if(id_translation_result->supported_features) {
		if(cJSON_AddStringToObject(item, "supportedFeatures",
		                           id_translation_result->
		                           supported_features) == NULL)
		{
			ogs_error(
				"OpenAPI_id_translation_result_convertToJSON() failed [supported_features]");
			goto end;
		}
	}
	...
	if(!id_translation_result->supi) {
		ogs_error(
			"OpenAPI_id_translation_result_convertToJSON() failed [supi]");
		return NULL;
	}
	...
	if(id_translation_result->gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_id_translation_result_convertToJSON() failed [gpsi]");
			goto end;
		}
	}

	if(id_translation_result->additional_supis) {
		cJSON *additional_supisList = cJSON_AddArrayToObject(item,
		                                                     "additionalSupis");
		if(additional_supisList == NULL) {
			ogs_error(
				"OpenAPI_id_translation_result_convertToJSON() failed [additional_supis]");
			goto end;
		}
		OpenAPI_list_for_each(id_translation_result->additional_supis,
		                      node) {
			if(cJSON_AddStringToObject(additional_supisList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_id_translation_result_convertToJSON() failed [additional_supis]");
	...
	if(id_translation_result->additional_gpsis) {
		cJSON *additional_gpsisList = cJSON_AddArrayToObject(item,
		                                                     "additionalGpsis");
		if(additional_gpsisList == NULL) {
			ogs_error(
				"OpenAPI_id_translation_result_convertToJSON() failed [additional_gpsis]");
			goto end;
		}
		OpenAPI_list_for_each(id_translation_result->additional_gpsis,
		                      node) {
			if(cJSON_AddStringToObject(additional_gpsisList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_id_translation_result_convertToJSON() failed [additional_gpsis]");
...
OpenAPI_id_translation_result_t *OpenAPI_id_translation_result_parseFromJSON(
	cJSON *id_translation_resultJSON) {
	...
	supported_features =
		cJSON_GetObjectItemCaseSensitive(id_translation_resultJSON,
		                                 "supportedFeatures");
	if(supported_features) {
		...
		{
			ogs_error(
				"OpenAPI_id_translation_result_parseFromJSON() failed [supported_features]");
			goto end;
		}
	}

	supi = cJSON_GetObjectItemCaseSensitive(id_translation_resultJSON,
	                                        "supi");
	if(!supi) {
		ogs_error(
			"OpenAPI_id_translation_result_parseFromJSON() failed [supi]");
		goto end;
	}
	...
	gpsi = cJSON_GetObjectItemCaseSensitive(id_translation_resultJSON,
	                                        "gpsi");
	if(gpsi) {
		...
		{
			ogs_error(
				"OpenAPI_id_translation_result_parseFromJSON() failed [gpsi]");
			goto end;
		}
	}
	...
	if(additional_supis) {
		...
		if(!cJSON_IsArray(additional_supis)) {
			ogs_error(
				"OpenAPI_id_translation_result_parseFromJSON() failed [additional_supis]");
			goto end;
		}
		...
		cJSON_ArrayForEach(additional_supis_local, additional_supis) {
			...
			OpenAPI_list_add(additional_supisList,
			                 ogs_strdup(
						 additional_supis_local->
						 valuestring));
		}
	}
	...
	if(additional_gpsis) {
		...
		if(!cJSON_IsArray(additional_gpsis)) {
			ogs_error(
				"OpenAPI_id_translation_result_parseFromJSON() failed [additional_gpsis]");
			goto end;
		}
		...
		cJSON_ArrayForEach(additional_gpsis_local, additional_gpsis) {
			...
			OpenAPI_list_add(additional_gpsisList,
			                 ogs_strdup(
						 additional_gpsis_local->
						 valuestring));
		}
	}
...
OpenAPI_id_translation_result_t *OpenAPI_id_translation_result_copy(
	OpenAPI_id_translation_result_t *dst,
	OpenAPI_id_translation_result_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_id_translation_result_convertToJSON() failed");
		return NULL;
	}
