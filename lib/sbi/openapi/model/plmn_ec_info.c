...
{
	OpenAPI_plmn_ec_info_t *plmn_ec_info_local_var =
		ogs_malloc(sizeof(OpenAPI_plmn_ec_info_t));
	...
	plmn_ec_info_local_var->is_ec_restriction_data_nb =
		is_ec_restriction_data_nb;
...
cJSON *OpenAPI_plmn_ec_info_convertToJSON(
	OpenAPI_plmn_ec_info_t *plmn_ec_info) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;

	if(plmn_ec_info == NULL) {
		ogs_error(
			"OpenAPI_plmn_ec_info_convertToJSON() failed [PlmnEcInfo]");
		return NULL;
	}
	...
	if(!plmn_ec_info->plmn_id) {
		ogs_error(
			"OpenAPI_plmn_ec_info_convertToJSON() failed [plmn_id]");
		return NULL;
	}
	cJSON *plmn_id_local_JSON =
		OpenAPI_plmn_id_convertToJSON(plmn_ec_info->plmn_id);
	...
	if(plmn_ec_info->ec_restriction_data_wb) {
		...
		if(ec_restriction_data_wb_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_plmn_ec_info_convertToJSON() failed [ec_restriction_data_wb]");
			goto end;
		}
	...
	if(plmn_ec_info->is_ec_restriction_data_nb) {
		if(cJSON_AddBoolToObject(item, "ecRestrictionDataNb",
		                         plmn_ec_info->ec_restriction_data_nb)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_plmn_ec_info_convertToJSON() failed [ec_restriction_data_nb]");
			goto end;
		}
	}
...
OpenAPI_plmn_ec_info_t *OpenAPI_plmn_ec_info_parseFromJSON(
	...
	if(!plmn_id) {
		ogs_error(
			"OpenAPI_plmn_ec_info_parseFromJSON() failed [plmn_id]");
		goto end;
	}
	...
	ec_restriction_data_wb =
		cJSON_GetObjectItemCaseSensitive(plmn_ec_infoJSON,
		                                 "ecRestrictionDataWb");
	...
	if(ec_restriction_data_nb) {
		if(!cJSON_IsBool(ec_restriction_data_nb)) {
			ogs_error(
				"OpenAPI_plmn_ec_info_parseFromJSON() failed [ec_restriction_data_nb]");
			goto end;
		}
	}
