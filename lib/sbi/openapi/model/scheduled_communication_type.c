...
char * OpenAPI_scheduled_communication_type_ToString(
	...
	const char *scheduled_communication_typeArray[] = { "NULL",
		                                            "DOWNLINK_ONLY",
		                                            "UPLINK_ONLY",
		                                            "BIDIRECTIONAL" };
	size_t sizeofArray = sizeof(scheduled_communication_typeArray) /
	                     sizeof(scheduled_communication_typeArray[0]);
	if(scheduled_communication_type < sizeofArray) {
		return (char *) scheduled_communication_typeArray[
...
OpenAPI_scheduled_communication_type_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(scheduled_communication_type,
		          scheduled_communication_typeArray[stringToReturn]) ==
		   0) {
