...
OpenAPI_presence_info_rm_t *OpenAPI_presence_info_rm_create(char *pra_id,
                                                            char *
                                                            additional_pra_id,
                                                            ...
                                                            globale_nb_id_list)
{
	OpenAPI_presence_info_rm_t *presence_info_rm_local_var =
		ogs_malloc(sizeof(OpenAPI_presence_info_rm_t));
	...
	presence_info_rm_local_var->global_ran_node_id_list =
		global_ran_node_id_list;
...
void OpenAPI_presence_info_rm_free(
	...
	if(presence_info_rm->tracking_area_list) {
		OpenAPI_list_for_each(presence_info_rm->tracking_area_list,
		                      node) {
	...
	if(presence_info_rm->globale_nb_id_list) {
		OpenAPI_list_for_each(presence_info_rm->globale_nb_id_list,
		                      node) {
			OpenAPI_global_ran_node_id_free(node->data);
		}
...
cJSON *OpenAPI_presence_info_rm_convertToJSON(
	OpenAPI_presence_info_rm_t *presence_info_rm) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(presence_info_rm == NULL) {
		ogs_error(
			"OpenAPI_presence_info_rm_convertToJSON() failed [PresenceInfoRm]");
		return NULL;
	}
	...
	if(presence_info_rm->pra_id) {
		...
		{
			ogs_error(
				"OpenAPI_presence_info_rm_convertToJSON() failed [pra_id]");
			goto end;
		}
	}
	...
	if(presence_info_rm->additional_pra_id) {
		if(cJSON_AddStringToObject(item, "additionalPraId",
		                           presence_info_rm->additional_pra_id)
		   == NULL)
		{
			ogs_error(
				"OpenAPI_presence_info_rm_convertToJSON() failed [additional_pra_id]");
			goto end;
		}
	}

	if(presence_info_rm->presence_state != OpenAPI_presence_state_NULL) {
		if(cJSON_AddStringToObject(item, "presenceState",
		                           OpenAPI_presence_state_ToString(
						   presence_info_rm->
						   presence_state)) == NULL)
		{
			ogs_error(
				"OpenAPI_presence_info_rm_convertToJSON() failed [presence_state]");
			goto end;
		}
	}
	...
	if(presence_info_rm->tracking_area_list) {
		...
		if(tracking_area_listList == NULL) {
			ogs_error(
				"OpenAPI_presence_info_rm_convertToJSON() failed [tracking_area_list]");
			goto end;
		}
		OpenAPI_list_for_each(presence_info_rm->tracking_area_list,
		                      node) {
	...
	if(presence_info_rm->ecgi_list) {
		...
		if(ecgi_listList == NULL) {
			ogs_error(
				"OpenAPI_presence_info_rm_convertToJSON() failed [ecgi_list]");
			goto end;
		}
		OpenAPI_list_for_each(presence_info_rm->ecgi_list, node) {
			cJSON *itemLocal =
				OpenAPI_ecgi_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_presence_info_rm_convertToJSON() failed [ecgi_list]");
				goto end;
	...
	if(presence_info_rm->ncgi_list) {
		...
		if(ncgi_listList == NULL) {
			ogs_error(
				"OpenAPI_presence_info_rm_convertToJSON() failed [ncgi_list]");
			goto end;
		}
		OpenAPI_list_for_each(presence_info_rm->ncgi_list, node) {
			cJSON *itemLocal =
				OpenAPI_ncgi_convertToJSON(node->data);
			if(itemLocal == NULL) {
				ogs_error(
					"OpenAPI_presence_info_rm_convertToJSON() failed [ncgi_list]");
				goto end;
	...
	if(presence_info_rm->global_ran_node_id_list) {
		cJSON *global_ran_node_id_listList =
			cJSON_AddArrayToObject(item, "globalRanNodeIdList");
		if(global_ran_node_id_listList == NULL) {
			ogs_error(
				"OpenAPI_presence_info_rm_convertToJSON() failed [global_ran_node_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(presence_info_rm->global_ran_node_id_list,
		                      node) {
	...
	if(presence_info_rm->globale_nb_id_list) {
		...
		if(globale_nb_id_listList == NULL) {
			ogs_error(
				"OpenAPI_presence_info_rm_convertToJSON() failed [globale_nb_id_list]");
			goto end;
		}
		OpenAPI_list_for_each(presence_info_rm->globale_nb_id_list,
		                      node) {
...
OpenAPI_presence_info_rm_t *OpenAPI_presence_info_rm_parseFromJSON(
	...
	pra_id = cJSON_GetObjectItemCaseSensitive(presence_info_rmJSON,
	                                          "praId");
	if(pra_id) {
		...
		{
			ogs_error(
				"OpenAPI_presence_info_rm_parseFromJSON() failed [pra_id]");
			goto end;
		}
	}
	...
	if(additional_pra_id) {
		...
		{
			ogs_error(
				"OpenAPI_presence_info_rm_parseFromJSON() failed [additional_pra_id]");
			goto end;
		}
	}

	presence_state = cJSON_GetObjectItemCaseSensitive(presence_info_rmJSON,
	                                                  "presenceState");
	if(presence_state) {
		if(!cJSON_IsString(presence_state)) {
			ogs_error(
				"OpenAPI_presence_info_rm_parseFromJSON() failed [presence_state]");
			goto end;
		}
		presence_stateVariable =
			OpenAPI_presence_state_FromString(
				presence_state->valuestring);
	}
	...
	if(tracking_area_list) {
		...
		if(!cJSON_IsArray(tracking_area_list)) {
			ogs_error(
				"OpenAPI_presence_info_rm_parseFromJSON() failed [tracking_area_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(tracking_area_list_local,
			...
			OpenAPI_tai_t *tracking_area_listItem =
				OpenAPI_tai_parseFromJSON(
					tracking_area_list_local);
			if(!tracking_area_listItem) {
				ogs_error("No tracking_area_listItem");
				goto end;
			}
			OpenAPI_list_add(tracking_area_listList,
			                 tracking_area_listItem);
		}
	}

	ecgi_list = cJSON_GetObjectItemCaseSensitive(presence_info_rmJSON,
	                                             "ecgiList");
	if(ecgi_list) {
		cJSON *ecgi_list_local = NULL;
		if(!cJSON_IsArray(ecgi_list)) {
			ogs_error(
				"OpenAPI_presence_info_rm_parseFromJSON() failed [ecgi_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ecgi_list_local, ecgi_list) {
			...
			OpenAPI_ecgi_t *ecgi_listItem =
				OpenAPI_ecgi_parseFromJSON(ecgi_list_local);
			if(!ecgi_listItem) {
	...
	ncgi_list = cJSON_GetObjectItemCaseSensitive(presence_info_rmJSON,
	                                             "ncgiList");
	if(ncgi_list) {
		cJSON *ncgi_list_local = NULL;
		if(!cJSON_IsArray(ncgi_list)) {
			ogs_error(
				"OpenAPI_presence_info_rm_parseFromJSON() failed [ncgi_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(ncgi_list_local, ncgi_list) {
			...
			OpenAPI_ncgi_t *ncgi_listItem =
				OpenAPI_ncgi_parseFromJSON(ncgi_list_local);
			if(!ncgi_listItem) {
	...
	if(global_ran_node_id_list) {
		...
		if(!cJSON_IsArray(global_ran_node_id_list)) {
			ogs_error(
				"OpenAPI_presence_info_rm_parseFromJSON() failed [global_ran_node_id_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(global_ran_node_id_list_local,
			...
			OpenAPI_list_add(global_ran_node_id_listList,
			                 global_ran_node_id_listItem);
		}
	}
	...
	if(globale_nb_id_list) {
		...
		if(!cJSON_IsArray(globale_nb_id_list)) {
			ogs_error(
				"OpenAPI_presence_info_rm_parseFromJSON() failed [globale_nb_id_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(globale_nb_id_list_local,
			...
			OpenAPI_global_ran_node_id_t *globale_nb_id_listItem =
				OpenAPI_global_ran_node_id_parseFromJSON(
			...
			OpenAPI_list_add(globale_nb_id_listList,
	...
	presence_info_rm_local_var = OpenAPI_presence_info_rm_create(
		pra_id &&
		!cJSON_IsNull(pra_id) ? ogs_strdup(pra_id->valuestring) : NULL,
		additional_pra_id &&
		!cJSON_IsNull(additional_pra_id) ?
		ogs_strdup(additional_pra_id->valuestring) : NULL,
...
OpenAPI_presence_info_rm_t *OpenAPI_presence_info_rm_copy(
	OpenAPI_presence_info_rm_t *dst, OpenAPI_presence_info_rm_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
