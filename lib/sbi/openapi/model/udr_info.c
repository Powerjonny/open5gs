...
OpenAPI_udr_info_t *OpenAPI_udr_info_create(char		*group_id,
	...
	OpenAPI_udr_info_t *udr_info_local_var =
		ogs_malloc(sizeof(OpenAPI_udr_info_t));
	...
	udr_info_local_var->external_group_identifiers_ranges =
		external_group_identifiers_ranges;
...
void OpenAPI_udr_info_free(OpenAPI_udr_info_t *udr_info) {
	...
	if(udr_info->external_group_identifiers_ranges) {
		OpenAPI_list_for_each(
			udr_info->external_group_identifiers_ranges, node) {
			OpenAPI_identity_range_free(node->data);
		}
...
cJSON *OpenAPI_udr_info_convertToJSON(OpenAPI_udr_info_t *udr_info) {
	...
	if(udr_info->group_id) {
		...
		{
			ogs_error(
				"OpenAPI_udr_info_convertToJSON() failed [group_id]");
			goto end;
		}
	}
	...
	if(udr_info->supi_ranges) {
		...
		if(supi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_udr_info_convertToJSON() failed [supi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(udr_info->supi_ranges, node) {
			cJSON *itemLocal =
	...
	if(udr_info->gpsi_ranges) {
		...
		if(gpsi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_udr_info_convertToJSON() failed [gpsi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(udr_info->gpsi_ranges, node) {
			cJSON *itemLocal =
	...
	if(udr_info->external_group_identifiers_ranges) {
		...
		if(external_group_identifiers_rangesList == NULL) {
			ogs_error(
				"OpenAPI_udr_info_convertToJSON() failed [external_group_identifiers_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(
			udr_info->external_group_identifiers_ranges, node) {
	...
	if(udr_info->supported_data_sets != OpenAPI_data_set_id_NULL) {
		...
		if(supported_data_setsList == NULL) {
			ogs_error(
				"OpenAPI_udr_info_convertToJSON() failed [supported_data_sets]");
			goto end;
		}
	...
	if(udr_info->shared_data_id_ranges) {
		...
		if(shared_data_id_rangesList == NULL) {
			ogs_error(
				"OpenAPI_udr_info_convertToJSON() failed [shared_data_id_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(udr_info->shared_data_id_ranges, node) {
			cJSON *itemLocal =
...
OpenAPI_udr_info_t *OpenAPI_udr_info_parseFromJSON(cJSON *udr_infoJSON) {
	...
	if(group_id) {
		...
		{
			ogs_error(
				"OpenAPI_udr_info_parseFromJSON() failed [group_id]");
			goto end;
		}
	}

	supi_ranges = cJSON_GetObjectItemCaseSensitive(udr_infoJSON,
	                                               "supiRanges");
	if(supi_ranges) {
		cJSON *supi_ranges_local = NULL;
		if(!cJSON_IsArray(supi_ranges)) {
			ogs_error(
				"OpenAPI_udr_info_parseFromJSON() failed [supi_ranges]");
			goto end;
		}
	...
	gpsi_ranges = cJSON_GetObjectItemCaseSensitive(udr_infoJSON,
	                                               "gpsiRanges");
	if(gpsi_ranges) {
		cJSON *gpsi_ranges_local = NULL;
		if(!cJSON_IsArray(gpsi_ranges)) {
			ogs_error(
				"OpenAPI_udr_info_parseFromJSON() failed [gpsi_ranges]");
			goto end;
		}
	...
	external_group_identifiers_ranges =
		cJSON_GetObjectItemCaseSensitive(udr_infoJSON,
		                                 "externalGroupIdentifiersRanges");
	if(external_group_identifiers_ranges) {
		...
		if(!cJSON_IsArray(external_group_identifiers_ranges)) {
			ogs_error(
				"OpenAPI_udr_info_parseFromJSON() failed [external_group_identifiers_ranges]");
			goto end;
		}
	...
	supported_data_sets = cJSON_GetObjectItemCaseSensitive(udr_infoJSON,
	                                                       "supportedDataSets");
	if(supported_data_sets) {
		cJSON *supported_data_sets_local = NULL;
		if(!cJSON_IsArray(supported_data_sets)) {
			ogs_error(
				"OpenAPI_udr_info_parseFromJSON() failed [supported_data_sets]");
			goto end;
		}
		...
		if(supported_data_setsList->count == 0) {
			ogs_error(
				"OpenAPI_udr_info_parseFromJSON() failed: Expected supported_data_setsList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}

	shared_data_id_ranges = cJSON_GetObjectItemCaseSensitive(udr_infoJSON,
	                                                         "sharedDataIdRanges");
	if(shared_data_id_ranges) {
		cJSON *shared_data_id_ranges_local = NULL;
		if(!cJSON_IsArray(shared_data_id_ranges)) {
			ogs_error(
				"OpenAPI_udr_info_parseFromJSON() failed [shared_data_id_ranges]");
			goto end;
		}
...
OpenAPI_udr_info_t *OpenAPI_udr_info_copy(OpenAPI_udr_info_t	*dst,
                                          OpenAPI_udr_info_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
