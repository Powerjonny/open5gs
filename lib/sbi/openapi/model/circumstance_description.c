...
OpenAPI_circumstance_description_t *OpenAPI_circumstance_description_create(
	bool is_freq, float freq, char *tm,
	OpenAPI_network_area_info_t *loc_area, bool is_vol, long vol) {
	OpenAPI_circumstance_description_t *circumstance_description_local_var =
		ogs_malloc(sizeof(OpenAPI_circumstance_description_t));
...
void OpenAPI_circumstance_description_free(
	OpenAPI_circumstance_description_t *circumstance_description) {
...
cJSON *OpenAPI_circumstance_description_convertToJSON(
	OpenAPI_circumstance_description_t *circumstance_description) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(circumstance_description == NULL) {
		ogs_error(
			"OpenAPI_circumstance_description_convertToJSON() failed [CircumstanceDescription]");
		return NULL;
	}
	...
	if(circumstance_description->is_freq) {
		...
		{
			ogs_error(
				"OpenAPI_circumstance_description_convertToJSON() failed [freq]");
			goto end;
		}
	}
	...
	if(circumstance_description->tm) {
		...
		{
			ogs_error(
				"OpenAPI_circumstance_description_convertToJSON() failed [tm]");
			goto end;
		}
	}

	if(circumstance_description->loc_area) {
		cJSON *loc_area_local_JSON =
			OpenAPI_network_area_info_convertToJSON(
				circumstance_description->loc_area);
		if(loc_area_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_circumstance_description_convertToJSON() failed [loc_area]");
			goto end;
		}
	...
	if(circumstance_description->is_vol) {
		...
		{
			ogs_error(
				"OpenAPI_circumstance_description_convertToJSON() failed [vol]");
			goto end;
		}
	}
...
OpenAPI_circumstance_description_parseFromJSON(
	...
	if(freq) {
		if(!cJSON_IsNumber(freq)) {
			ogs_error(
				"OpenAPI_circumstance_description_parseFromJSON() failed [freq]");
			goto end;
		}
	}

	tm = cJSON_GetObjectItemCaseSensitive(circumstance_descriptionJSON,
	                                      "tm");
	if(tm) {
		...
		{
			ogs_error(
				"OpenAPI_circumstance_description_parseFromJSON() failed [tm]");
			goto end;
		}
	}
	...
	loc_area =
		cJSON_GetObjectItemCaseSensitive(circumstance_descriptionJSON,
		                                 "locArea");
	if(loc_area) {
		loc_area_local_nonprim =
			OpenAPI_network_area_info_parseFromJSON(loc_area);
		if(!loc_area_local_nonprim) {
			ogs_error(
				"OpenAPI_network_area_info_parseFromJSON failed [loc_area]");
			goto end;
		}
	}
	...
	if(vol) {
		if(!cJSON_IsNumber(vol)) {
			ogs_error(
				"OpenAPI_circumstance_description_parseFromJSON() failed [vol]");
			goto end;
		}
	}

	circumstance_description_local_var =
		OpenAPI_circumstance_description_create(
			freq ? true : false,
			freq ? freq->valuedouble : 0,
			tm &&
			!cJSON_IsNull(tm) ? ogs_strdup(tm->valuestring) : NULL,
...
OpenAPI_circumstance_description_t *OpenAPI_circumstance_description_copy(
	OpenAPI_circumstance_description_t	*dst,
	OpenAPI_circumstance_description_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_circumstance_description_convertToJSON() failed");
		return NULL;
	}
