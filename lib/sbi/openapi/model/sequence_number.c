...
OpenAPI_sequence_number_t *OpenAPI_sequence_number_create(
	...
	OpenAPI_sequence_number_t *sequence_number_local_var =
		ogs_malloc(sizeof(OpenAPI_sequence_number_t));
...
cJSON *OpenAPI_sequence_number_convertToJSON(
	...
	if(sequence_number == NULL) {
		ogs_error(
			"OpenAPI_sequence_number_convertToJSON() failed [SequenceNumber]");
		return NULL;
	}

	item = cJSON_CreateObject();
	if(sequence_number->sqn_scheme != OpenAPI_sqn_scheme_NULL) {
		if(cJSON_AddStringToObject(item, "sqnScheme",
		                           OpenAPI_sqn_scheme_ToString(
						   sequence_number->sqn_scheme))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_sequence_number_convertToJSON() failed [sqn_scheme]");
			goto end;
		}
	}

	if(sequence_number->sqn) {
		if(cJSON_AddStringToObject(item, "sqn",
		                           sequence_number->sqn) == NULL)
		{
			ogs_error(
				"OpenAPI_sequence_number_convertToJSON() failed [sqn]");
			goto end;
		}
	}

	if(sequence_number->last_indexes) {
		cJSON *last_indexes = cJSON_AddObjectToObject(item,
		                                              "lastIndexes");
		if(last_indexes == NULL) {
			ogs_error(
				"OpenAPI_sequence_number_convertToJSON() failed [last_indexes]");
			goto end;
		}
		cJSON *localMapObject = last_indexes;
		if(sequence_number->last_indexes) {
			OpenAPI_list_for_each(sequence_number->last_indexes,
			                      node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				if(localKeyValue->value == NULL) {
					ogs_error(
						"OpenAPI_sequence_number_convertToJSON() failed [inner]");
					goto end;
				}
	...
	if(sequence_number->is_ind_length) {
		if(cJSON_AddNumberToObject(item, "indLength",
		                           sequence_number->ind_length) == NULL)
		{
			ogs_error(
				"OpenAPI_sequence_number_convertToJSON() failed [ind_length]");
			goto end;
		}
	}

	if(sequence_number->dif_sign != OpenAPI_sign_NULL) {
		if(cJSON_AddStringToObject(item, "difSign",
		                           OpenAPI_sign_ToString(
						   sequence_number->dif_sign))
		   == NULL)
		{
			ogs_error(
				"OpenAPI_sequence_number_convertToJSON() failed [dif_sign]");
			goto end;
		}
	}
...
OpenAPI_sequence_number_t *OpenAPI_sequence_number_parseFromJSON(
	...
	sqn_scheme = cJSON_GetObjectItemCaseSensitive(sequence_numberJSON,
	                                              "sqnScheme");
	if(sqn_scheme) {
		if(!cJSON_IsString(sqn_scheme)) {
			ogs_error(
				"OpenAPI_sequence_number_parseFromJSON() failed [sqn_scheme]");
			goto end;
		}
		sqn_schemeVariable =
			OpenAPI_sqn_scheme_FromString(sqn_scheme->valuestring);
	}
	...
	if(sqn) {
		...
		{
			ogs_error(
				"OpenAPI_sequence_number_parseFromJSON() failed [sqn]");
			goto end;
		}
	}

	last_indexes = cJSON_GetObjectItemCaseSensitive(sequence_numberJSON,
	                                                "lastIndexes");
	if(last_indexes) {
		cJSON *last_indexes_local_map = NULL;
		if(!cJSON_IsObject(last_indexes) &&
		   !cJSON_IsNull(last_indexes))
		{
			ogs_error(
				"OpenAPI_sequence_number_parseFromJSON() failed [last_indexes]");
			goto end;
		}
		if(cJSON_IsObject(last_indexes)) {
			last_indexesList = OpenAPI_list_create();
			OpenAPI_map_t *localMapKeyPair = NULL;
			cJSON_ArrayForEach(last_indexes_local_map,
			                   last_indexes) {
				...
				if(!cJSON_IsNumber(localMapObject)) {
					ogs_error(
						"OpenAPI_sequence_number_parseFromJSON() failed [inner]");
					goto end;
				}
				...
				*localDouble = localMapObject->valuedouble;
				localMapKeyPair =
					OpenAPI_map_create(ogs_strdup(
								   ...
								   ->string),
					                   localDouble);
				OpenAPI_list_add(last_indexesList,
				                 localMapKeyPair);
			}
		}
	}

	ind_length = cJSON_GetObjectItemCaseSensitive(sequence_numberJSON,
	                                              "indLength");
	if(ind_length) {
		if(!cJSON_IsNumber(ind_length)) {
			ogs_error(
				"OpenAPI_sequence_number_parseFromJSON() failed [ind_length]");
			goto end;
		}
	}

	dif_sign = cJSON_GetObjectItemCaseSensitive(sequence_numberJSON,
	                                            "difSign");
	if(dif_sign) {
		if(!cJSON_IsString(dif_sign)) {
			ogs_error(
				"OpenAPI_sequence_number_parseFromJSON() failed [dif_sign]");
			goto end;
		}
		dif_signVariable =
			OpenAPI_sign_FromString(dif_sign->valuestring);
	}

	sequence_number_local_var = OpenAPI_sequence_number_create(
		sqn_scheme ? sqn_schemeVariable : 0,
		sqn &&
		!cJSON_IsNull(sqn) ? ogs_strdup(sqn->valuestring) : NULL,
