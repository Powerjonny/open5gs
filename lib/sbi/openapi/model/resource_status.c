...
char * OpenAPI_resource_status_ToString(
	...
	const char *resource_statusArray[] = { "NULL", "RELEASED", "UNCHANGED",
		                               "TRANSFERRED", "UPDATED",
		                               "ALT_ANCHOR_SMF" };
	size_t sizeofArray = sizeof(resource_statusArray) /
	                     sizeof(resource_statusArray[0]);
...
OpenAPI_resource_status_e OpenAPI_resource_status_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(resource_status,
		          resource_statusArray[stringToReturn]) == 0) {
