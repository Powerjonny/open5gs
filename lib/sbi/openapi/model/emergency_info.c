...
OpenAPI_emergency_info_t *OpenAPI_emergency_info_create(char *pgw_fqdn,
                                                        ...
                                                        *smf_instance_id,
                                                        ...
                                                        OpenAPI_plmn_id_t
                                                        *plmn_id) {
	OpenAPI_emergency_info_t *emergency_info_local_var =
		ogs_malloc(sizeof(OpenAPI_emergency_info_t));
...
cJSON *OpenAPI_emergency_info_convertToJSON(
	...
	if(emergency_info == NULL) {
		ogs_error(
			"OpenAPI_emergency_info_convertToJSON() failed [EmergencyInfo]");
		return NULL;
	}
	...
	if(emergency_info->pgw_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_emergency_info_convertToJSON() failed [pgw_fqdn]");
			goto end;
		}
	}

	if(emergency_info->pgw_ip_address) {
		cJSON *pgw_ip_address_local_JSON =
			OpenAPI_ip_address_convertToJSON(
				emergency_info->pgw_ip_address);
		if(pgw_ip_address_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_emergency_info_convertToJSON() failed [pgw_ip_address]");
			goto end;
		}
		cJSON_AddItemToObject(item, "pgwIpAddress",
		                      pgw_ip_address_local_JSON);
		if(item->child == NULL) {
			ogs_error(
				"OpenAPI_emergency_info_convertToJSON() failed [pgw_ip_address]");
			goto end;
	...
	if(emergency_info->smf_instance_id) {
		if(cJSON_AddStringToObject(item, "smfInstanceId",
		                           emergency_info->smf_instance_id) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_emergency_info_convertToJSON() failed [smf_instance_id]");
			goto end;
		}
	}
	...
	if(emergency_info->is_epdg_ind) {
		if(cJSON_AddBoolToObject(item, "epdgInd",
		                         emergency_info->epdg_ind) == NULL)
		{
			ogs_error(
				"OpenAPI_emergency_info_convertToJSON() failed [epdg_ind]");
			goto end;
		}
	}
	...
	if(emergency_info->plmn_id) {
		cJSON *plmn_id_local_JSON =
			OpenAPI_plmn_id_convertToJSON(emergency_info->plmn_id);
		if(plmn_id_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_emergency_info_convertToJSON() failed [plmn_id]");
			goto end;
		}
...
OpenAPI_emergency_info_t *OpenAPI_emergency_info_parseFromJSON(
	...
	pgw_fqdn = cJSON_GetObjectItemCaseSensitive(emergency_infoJSON,
	                                            "pgwFqdn");
	if(pgw_fqdn) {
		...
		{
			ogs_error(
				"OpenAPI_emergency_info_parseFromJSON() failed [pgw_fqdn]");
			goto end;
		}
	}

	pgw_ip_address = cJSON_GetObjectItemCaseSensitive(emergency_infoJSON,
	                                                  "pgwIpAddress");
	if(pgw_ip_address) {
		pgw_ip_address_local_nonprim =
			OpenAPI_ip_address_parseFromJSON(pgw_ip_address);
		if(!pgw_ip_address_local_nonprim) {
			ogs_error(
				"OpenAPI_ip_address_parseFromJSON failed [pgw_ip_address]");
			goto end;
		}
	}

	smf_instance_id = cJSON_GetObjectItemCaseSensitive(emergency_infoJSON,
	                                                   "smfInstanceId");
	if(smf_instance_id) {
		...
		{
			ogs_error(
				"OpenAPI_emergency_info_parseFromJSON() failed [smf_instance_id]");
			goto end;
		}
	}

	epdg_ind = cJSON_GetObjectItemCaseSensitive(emergency_infoJSON,
	                                            "epdgInd");
	if(epdg_ind) {
		if(!cJSON_IsBool(epdg_ind)) {
			ogs_error(
				"OpenAPI_emergency_info_parseFromJSON() failed [epdg_ind]");
			goto end;
		}
	}

	plmn_id = cJSON_GetObjectItemCaseSensitive(emergency_infoJSON,
	                                           "plmnId");
	if(plmn_id) {
