...
OpenAPI_hss_subscription_item_t *OpenAPI_hss_subscription_item_create(
	char *hss_instance_id, char *subscription_id,
	OpenAPI_context_info_t *context_info) {
	OpenAPI_hss_subscription_item_t *hss_subscription_item_local_var =
		ogs_malloc(sizeof(OpenAPI_hss_subscription_item_t));
...
void OpenAPI_hss_subscription_item_free(
	OpenAPI_hss_subscription_item_t *hss_subscription_item) {
...
cJSON *OpenAPI_hss_subscription_item_convertToJSON(
	OpenAPI_hss_subscription_item_t *hss_subscription_item) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(hss_subscription_item == NULL) {
		ogs_error(
			"OpenAPI_hss_subscription_item_convertToJSON() failed [HssSubscriptionItem]");
		return NULL;
	}
	...
	if(!hss_subscription_item->hss_instance_id) {
		ogs_error(
			"OpenAPI_hss_subscription_item_convertToJSON() failed [hss_instance_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "hssInstanceId",
	                           hss_subscription_item->hss_instance_id) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_hss_subscription_item_convertToJSON() failed [hss_instance_id]");
	...
	if(!hss_subscription_item->subscription_id) {
		ogs_error(
			"OpenAPI_hss_subscription_item_convertToJSON() failed [subscription_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "subscriptionId",
	                           hss_subscription_item->subscription_id) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_hss_subscription_item_convertToJSON() failed [subscription_id]");
	...
	if(hss_subscription_item->context_info) {
		cJSON *context_info_local_JSON =
			OpenAPI_context_info_convertToJSON(
				hss_subscription_item->context_info);
		if(context_info_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_hss_subscription_item_convertToJSON() failed [context_info]");
			goto end;
		}
...
OpenAPI_hss_subscription_item_t *OpenAPI_hss_subscription_item_parseFromJSON(
	...
	hss_instance_id =
		cJSON_GetObjectItemCaseSensitive(hss_subscription_itemJSON,
		                                 "hssInstanceId");
	if(!hss_instance_id) {
		ogs_error(
			"OpenAPI_hss_subscription_item_parseFromJSON() failed [hss_instance_id]");
		goto end;
	}
	...
	if(!subscription_id) {
		ogs_error(
			"OpenAPI_hss_subscription_item_parseFromJSON() failed [subscription_id]");
		goto end;
	}
...
OpenAPI_hss_subscription_item_t *OpenAPI_hss_subscription_item_copy(
	OpenAPI_hss_subscription_item_t *dst,
	OpenAPI_hss_subscription_item_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_hss_subscription_item_convertToJSON() failed");
		return NULL;
	}
