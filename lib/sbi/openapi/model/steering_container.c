...
OpenAPI_steering_container_t *OpenAPI_steering_container_create() {
	OpenAPI_steering_container_t *steering_container_local_var =
		ogs_malloc(sizeof(OpenAPI_steering_container_t));
...
cJSON *OpenAPI_steering_container_convertToJSON(
	...
	if(steering_container == NULL) {
		ogs_error(
			"OpenAPI_steering_container_convertToJSON() failed [SteeringContainer]");
		return NULL;
	}
