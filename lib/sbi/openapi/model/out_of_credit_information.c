...
OpenAPI_out_of_credit_information_t *OpenAPI_out_of_credit_information_create(
	OpenAPI_final_unit_action_t *fin_unit_act, OpenAPI_list_t *flows) {
	OpenAPI_out_of_credit_information_t *out_of_credit_information_local_var
	        = ogs_malloc(sizeof(OpenAPI_out_of_credit_information_t));
...
void OpenAPI_out_of_credit_information_free(
	OpenAPI_out_of_credit_information_t *out_of_credit_information) {
...
cJSON *OpenAPI_out_of_credit_information_convertToJSON(
	OpenAPI_out_of_credit_information_t *out_of_credit_information) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(out_of_credit_information == NULL) {
		ogs_error(
			"OpenAPI_out_of_credit_information_convertToJSON() failed [OutOfCreditInformation]");
		return NULL;
	}
	...
	if(out_of_credit_information->flows) {
		...
		if(flowsList == NULL) {
			ogs_error(
				"OpenAPI_out_of_credit_information_convertToJSON() failed [flows]");
			goto end;
		}
		OpenAPI_list_for_each(out_of_credit_information->flows, node) {
			cJSON *itemLocal =
...
OpenAPI_out_of_credit_information_parseFromJSON(
	...
	fin_unit_act =
		cJSON_GetObjectItemCaseSensitive(out_of_credit_informationJSON,
		                                 "finUnitAct");
	if(!fin_unit_act) {
		ogs_error(
	...
	flows = cJSON_GetObjectItemCaseSensitive(out_of_credit_informationJSON,
	                                         "flows");
	if(flows) {
		cJSON *flows_local = NULL;
		if(!cJSON_IsArray(flows)) {
			ogs_error(
				"OpenAPI_out_of_credit_information_parseFromJSON() failed [flows]");
			goto end;
		}
	...
	out_of_credit_information_local_var =
...
OpenAPI_out_of_credit_information_t *OpenAPI_out_of_credit_information_copy(
	OpenAPI_out_of_credit_information_t	*dst,
	OpenAPI_out_of_credit_information_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_out_of_credit_information_convertToJSON() failed");
		return NULL;
	}
