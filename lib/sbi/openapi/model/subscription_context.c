...
OpenAPI_subscription_context_t *OpenAPI_subscription_context_create(
	char *subscription_id, OpenAPI_subscr_cond_t *subscr_cond) {
	OpenAPI_subscription_context_t *subscription_context_local_var =
		ogs_malloc(sizeof(OpenAPI_subscription_context_t));
...
void OpenAPI_subscription_context_free(
...
cJSON *OpenAPI_subscription_context_convertToJSON(
	OpenAPI_subscription_context_t *subscription_context) {
	cJSON *item = NULL;
	OpenAPI_lnode_t *node = NULL;
	...
	if(subscription_context == NULL) {
		ogs_error(
			"OpenAPI_subscription_context_convertToJSON() failed [SubscriptionContext]");
		return NULL;
	}
	...
	if(!subscription_context->subscription_id) {
		ogs_error(
			"OpenAPI_subscription_context_convertToJSON() failed [subscription_id]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "subscriptionId",
	                           subscription_context->subscription_id) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_subscription_context_convertToJSON() failed [subscription_id]");
	...
	if(subscription_context->subscr_cond) {
		cJSON *subscr_cond_local_JSON =
			OpenAPI_subscr_cond_convertToJSON(
				subscription_context->subscr_cond);
		if(subscr_cond_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_subscription_context_convertToJSON() failed [subscr_cond]");
			goto end;
		}
...
OpenAPI_subscription_context_t *OpenAPI_subscription_context_parseFromJSON(
	cJSON *subscription_contextJSON) {
	...
	subscription_id =
		cJSON_GetObjectItemCaseSensitive(subscription_contextJSON,
		                                 "subscriptionId");
	if(!subscription_id) {
		ogs_error(
			"OpenAPI_subscription_context_parseFromJSON() failed [subscription_id]");
		goto end;
	}
	...
	subscr_cond = cJSON_GetObjectItemCaseSensitive(subscription_contextJSON,
	                                               "subscrCond");
	if(subscr_cond) {
...
OpenAPI_subscription_context_t *OpenAPI_subscription_context_copy(
	OpenAPI_subscription_context_t	*dst,
	OpenAPI_subscription_context_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
	...
	if(!item) {
		ogs_error(
			"OpenAPI_subscription_context_convertToJSON() failed");
		return NULL;
	}
