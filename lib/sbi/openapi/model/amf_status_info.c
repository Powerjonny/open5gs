...
OpenAPI_amf_status_info_t *OpenAPI_amf_status_info_create(
	OpenAPI_list_t *guami_list, OpenAPI_status_change_e status_change,
	char *target_amf_removal, char *target_amf_failure) {
	OpenAPI_amf_status_info_t *amf_status_info_local_var =
		ogs_malloc(sizeof(OpenAPI_amf_status_info_t));
...
cJSON *OpenAPI_amf_status_info_convertToJSON(
	...
	if(amf_status_info == NULL) {
		ogs_error(
			"OpenAPI_amf_status_info_convertToJSON() failed [AmfStatusInfo]");
		return NULL;
	}
	...
	if(!amf_status_info->guami_list) {
		ogs_error(
			"OpenAPI_amf_status_info_convertToJSON() failed [guami_list]");
		return NULL;
	}
	...
	if(amf_status_info->status_change == OpenAPI_status_change_NULL) {
		ogs_error(
			"OpenAPI_amf_status_info_convertToJSON() failed [status_change]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "statusChange",
	                           OpenAPI_status_change_ToString(
					   amf_status_info->status_change)) ==
	   NULL)
	{
		ogs_error(
			"OpenAPI_amf_status_info_convertToJSON() failed [status_change]");
	...
	if(amf_status_info->target_amf_removal) {
		...
		{
			ogs_error(
				"OpenAPI_amf_status_info_convertToJSON() failed [target_amf_removal]");
			goto end;
		}
	}
	...
	if(amf_status_info->target_amf_failure) {
		...
		{
			ogs_error(
				"OpenAPI_amf_status_info_convertToJSON() failed [target_amf_failure]");
			goto end;
		}
	}
...
OpenAPI_amf_status_info_t *OpenAPI_amf_status_info_parseFromJSON(
	cJSON *amf_status_infoJSON) {
	...
	if(!guami_list) {
		ogs_error(
			"OpenAPI_amf_status_info_parseFromJSON() failed [guami_list]");
		goto end;
	}
	...
	if(!status_change) {
		ogs_error(
			"OpenAPI_amf_status_info_parseFromJSON() failed [status_change]");
		goto end;
	}
	...
	status_changeVariable =
		OpenAPI_status_change_FromString(status_change->valuestring);

	target_amf_removal =
		cJSON_GetObjectItemCaseSensitive(amf_status_infoJSON,
		                                 "targetAmfRemoval");
	if(target_amf_removal) {
		...
		{
			ogs_error(
				"OpenAPI_amf_status_info_parseFromJSON() failed [target_amf_removal]");
			goto end;
		}
	}
	...
	if(target_amf_failure) {
		...
		{
			ogs_error(
				"OpenAPI_amf_status_info_parseFromJSON() failed [target_amf_failure]");
			goto end;
		}
	}
	...
	amf_status_info_local_var = OpenAPI_amf_status_info_create(
		...
		target_amf_removal &&
		!cJSON_IsNull(target_amf_removal) ?
		ogs_strdup(target_amf_removal->valuestring) : NULL,
		target_amf_failure &&
		!cJSON_IsNull(target_amf_failure) ?
		ogs_strdup(target_amf_failure->valuestring) : NULL
		);
...
OpenAPI_amf_status_info_t *OpenAPI_amf_status_info_copy(
	OpenAPI_amf_status_info_t *dst, OpenAPI_amf_status_info_t *src) {
	cJSON *item = NULL;
	char *content = NULL;
