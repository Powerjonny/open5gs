...
char *OpenAPI_condition_typenef_cond_ToString(
...
OpenAPI_nef_cond_condition_type_e OpenAPI_condition_typenef_cond_FromString(
	char *condition_type) {
...
OpenAPI_nef_cond_t *OpenAPI_nef_cond_create(
	OpenAPI_nef_cond_condition_type_e condition_type,
	OpenAPI_list_t *af_events, OpenAPI_list_t *snssai_list,
	OpenAPI_pfd_data_t *pfd_data, OpenAPI_list_t *gpsi_ranges,
	OpenAPI_list_t *external_group_identifiers_ranges,
	OpenAPI_list_t *served_fqdn_list) {
	OpenAPI_nef_cond_t *nef_cond_local_var =
		ogs_malloc(sizeof(OpenAPI_nef_cond_t));
	...
	nef_cond_local_var->external_group_identifiers_ranges =
...
void OpenAPI_nef_cond_free(OpenAPI_nef_cond_t *nef_cond) {
	...
	if(nef_cond->external_group_identifiers_ranges) {
		OpenAPI_list_for_each(
			nef_cond->external_group_identifiers_ranges, node) {
			OpenAPI_identity_range_free(node->data);
		}
...
cJSON *OpenAPI_nef_cond_convertToJSON(OpenAPI_nef_cond_t *nef_cond) {
	...
	if(nef_cond->condition_type == OpenAPI_nef_cond_CONDITIONTYPE_NULL) {
		ogs_error(
			"OpenAPI_nef_cond_convertToJSON() failed [condition_type]");
		return NULL;
	}
	if(cJSON_AddStringToObject(item, "conditionType",
	                           OpenAPI_condition_typenef_cond_ToString(
					   nef_cond->condition_type)) == NULL)
	{
		ogs_error(
			"OpenAPI_nef_cond_convertToJSON() failed [condition_type]");
		goto end;
	...
	if(nef_cond->af_events != OpenAPI_af_event_NULL) {
		...
		if(af_eventsList == NULL) {
			ogs_error(
				"OpenAPI_nef_cond_convertToJSON() failed [af_events]");
			goto end;
		}
		OpenAPI_list_for_each(nef_cond->af_events, node) {
			if(cJSON_AddStringToObject(af_eventsList, "",
			                           OpenAPI_af_event_ToString(
							   (intptr_t) node->data))
			   == NULL)
			{
				ogs_error(
	...
	if(nef_cond->snssai_list) {
		...
		if(snssai_listList == NULL) {
			ogs_error(
				"OpenAPI_nef_cond_convertToJSON() failed [snssai_list]");
			goto end;
		}
		OpenAPI_list_for_each(nef_cond->snssai_list, node) {
			cJSON *itemLocal =
	...
	if(nef_cond->pfd_data) {
		cJSON *pfd_data_local_JSON =
			OpenAPI_pfd_data_convertToJSON(nef_cond->pfd_data);
		if(pfd_data_local_JSON == NULL) {
			ogs_error(
				"OpenAPI_nef_cond_convertToJSON() failed [pfd_data]");
			goto end;
		}
	...
	if(nef_cond->gpsi_ranges) {
		...
		if(gpsi_rangesList == NULL) {
			ogs_error(
				"OpenAPI_nef_cond_convertToJSON() failed [gpsi_ranges]");
			goto end;
		}
		OpenAPI_list_for_each(nef_cond->gpsi_ranges, node) {
			cJSON *itemLocal =
	...
	if(nef_cond->served_fqdn_list) {
		...
		if(served_fqdn_listList == NULL) {
			ogs_error(
				"OpenAPI_nef_cond_convertToJSON() failed [served_fqdn_list]");
			goto end;
		}
		OpenAPI_list_for_each(nef_cond->served_fqdn_list, node) {
...
OpenAPI_nef_cond_t *OpenAPI_nef_cond_parseFromJSON(cJSON *nef_condJSON) {
	...
	condition_type = cJSON_GetObjectItemCaseSensitive(nef_condJSON,
	                                                  "conditionType");
	if(!condition_type) {
		ogs_error(
			"OpenAPI_nef_cond_parseFromJSON() failed [condition_type]");
		goto end;
	}
	...
	condition_typeVariable =
		OpenAPI_condition_typenef_cond_FromString(
	...
	if(af_events) {
		...
		if(!cJSON_IsArray(af_events)) {
			ogs_error(
				"OpenAPI_nef_cond_parseFromJSON() failed [af_events]");
			goto end;
		}
		...
		cJSON_ArrayForEach(af_events_local, af_events) {
			...
			localEnum =
				OpenAPI_af_event_FromString(
					af_events_local->valuestring);
			if(!localEnum) {
				ogs_info(
					"Enum value \"%s\" for field \"af_events\" is not supported. Ignoring it ...",
					af_events_local->valuestring);
			} else {
				OpenAPI_list_add(af_eventsList,
				                 (void *) localEnum);
			}
		}
		if(af_eventsList->count == 0) {
			ogs_error(
				"OpenAPI_nef_cond_parseFromJSON() failed: Expected af_eventsList to not be empty (after ignoring unsupported enum values).");
			goto end;
		}
	}

	snssai_list = cJSON_GetObjectItemCaseSensitive(nef_condJSON,
	                                               "snssaiList");
	if(snssai_list) {
		cJSON *snssai_list_local = NULL;
		if(!cJSON_IsArray(snssai_list)) {
			ogs_error(
				"OpenAPI_nef_cond_parseFromJSON() failed [snssai_list]");
			goto end;
		}
	...
	gpsi_ranges = cJSON_GetObjectItemCaseSensitive(nef_condJSON,
	                                               "gpsiRanges");
	if(gpsi_ranges) {
		cJSON *gpsi_ranges_local = NULL;
		if(!cJSON_IsArray(gpsi_ranges)) {
			ogs_error(
				"OpenAPI_nef_cond_parseFromJSON() failed [gpsi_ranges]");
			goto end;
		}
	...
	external_group_identifiers_ranges =
		cJSON_GetObjectItemCaseSensitive(nef_condJSON,
		                                 "externalGroupIdentifiersRanges");
	if(external_group_identifiers_ranges) {
		...
		if(!cJSON_IsArray(external_group_identifiers_ranges)) {
			ogs_error(
				"OpenAPI_nef_cond_parseFromJSON() failed [external_group_identifiers_ranges]");
			goto end;
		}
	...
	served_fqdn_list = cJSON_GetObjectItemCaseSensitive(nef_condJSON,
	                                                    "servedFqdnList");
	if(served_fqdn_list) {
		cJSON *served_fqdn_list_local = NULL;
		if(!cJSON_IsArray(served_fqdn_list)) {
			ogs_error(
				"OpenAPI_nef_cond_parseFromJSON() failed [served_fqdn_list]");
			goto end;
		}
...
OpenAPI_nef_cond_t *OpenAPI_nef_cond_copy(OpenAPI_nef_cond_t	*dst,
                                          OpenAPI_nef_cond_t	*src) {
	cJSON *item = NULL;
	char *content = NULL;
