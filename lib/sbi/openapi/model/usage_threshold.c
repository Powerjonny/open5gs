...
OpenAPI_usage_threshold_t *OpenAPI_usage_threshold_create(bool	is_duration,
                                                          ...
                                                          long	uplink_volume) {
	OpenAPI_usage_threshold_t *usage_threshold_local_var =
		ogs_malloc(sizeof(OpenAPI_usage_threshold_t));
...
cJSON *OpenAPI_usage_threshold_convertToJSON(
	...
	if(usage_threshold == NULL) {
		ogs_error(
			"OpenAPI_usage_threshold_convertToJSON() failed [UsageThreshold]");
		return NULL;
	}
	...
	if(usage_threshold->is_duration) {
		if(cJSON_AddNumberToObject(item, "duration",
		                           usage_threshold->duration) == NULL)
		{
			ogs_error(
				"OpenAPI_usage_threshold_convertToJSON() failed [duration]");
			goto end;
		}
	}
	...
	if(usage_threshold->is_total_volume) {
		if(cJSON_AddNumberToObject(item, "totalVolume",
		                           usage_threshold->total_volume) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_usage_threshold_convertToJSON() failed [total_volume]");
			goto end;
		}
	}
	...
	if(usage_threshold->is_downlink_volume) {
		if(cJSON_AddNumberToObject(item, "downlinkVolume",
		                           usage_threshold->downlink_volume) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_usage_threshold_convertToJSON() failed [downlink_volume]");
			goto end;
		}
	}
	...
	if(usage_threshold->is_uplink_volume) {
		if(cJSON_AddNumberToObject(item, "uplinkVolume",
		                           usage_threshold->uplink_volume) ==
		   NULL)
		{
			ogs_error(
				"OpenAPI_usage_threshold_convertToJSON() failed [uplink_volume]");
			goto end;
		}
	}
...
OpenAPI_usage_threshold_t *OpenAPI_usage_threshold_parseFromJSON(
	...
	duration = cJSON_GetObjectItemCaseSensitive(usage_thresholdJSON,
	                                            "duration");
	if(duration) {
		if(!cJSON_IsNumber(duration)) {
			ogs_error(
				"OpenAPI_usage_threshold_parseFromJSON() failed [duration]");
			goto end;
		}
	}

	total_volume = cJSON_GetObjectItemCaseSensitive(usage_thresholdJSON,
	                                                "totalVolume");
	if(total_volume) {
		if(!cJSON_IsNumber(total_volume)) {
			ogs_error(
				"OpenAPI_usage_threshold_parseFromJSON() failed [total_volume]");
			goto end;
		}
	}

	downlink_volume = cJSON_GetObjectItemCaseSensitive(usage_thresholdJSON,
	                                                   "downlinkVolume");
	if(downlink_volume) {
		if(!cJSON_IsNumber(downlink_volume)) {
			ogs_error(
				"OpenAPI_usage_threshold_parseFromJSON() failed [downlink_volume]");
			goto end;
		}
	}

	uplink_volume = cJSON_GetObjectItemCaseSensitive(usage_thresholdJSON,
	                                                 "uplinkVolume");
	if(uplink_volume) {
		if(!cJSON_IsNumber(uplink_volume)) {
			ogs_error(
				"OpenAPI_usage_threshold_parseFromJSON() failed [uplink_volume]");
			goto end;
		}
	}
