...
OpenAPI_identity_data_t *OpenAPI_identity_data_create(
	...
	OpenAPI_identity_data_t *identity_data_local_var =
		ogs_malloc(sizeof(OpenAPI_identity_data_t));
...
cJSON *OpenAPI_identity_data_convertToJSON(
	...
	if(identity_data == NULL) {
		ogs_error(
			"OpenAPI_identity_data_convertToJSON() failed [IdentityData]");
		return NULL;
	}
	...
	if(identity_data->supi_list) {
		...
		if(supi_listList == NULL) {
			ogs_error(
				"OpenAPI_identity_data_convertToJSON() failed [supi_list]");
			goto end;
		}
		OpenAPI_list_for_each(identity_data->supi_list, node) {
			if(cJSON_AddStringToObject(supi_listList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_identity_data_convertToJSON() failed [supi_list]");
	...
	if(identity_data->gpsi_list) {
		...
		if(gpsi_listList == NULL) {
			ogs_error(
				"OpenAPI_identity_data_convertToJSON() failed [gpsi_list]");
			goto end;
		}
		OpenAPI_list_for_each(identity_data->gpsi_list, node) {
	...
	if(identity_data->allowed_af_ids) {
		cJSON *allowed_af_idsList = cJSON_AddArrayToObject(item,
		                                                   "allowedAfIds");
		if(allowed_af_idsList == NULL) {
			ogs_error(
				"OpenAPI_identity_data_convertToJSON() failed [allowed_af_ids]");
			goto end;
		}
		OpenAPI_list_for_each(identity_data->allowed_af_ids, node) {
			if(cJSON_AddStringToObject(allowed_af_idsList, "",
			                           (char *) node->data) == NULL)
			{
				ogs_error(
					"OpenAPI_identity_data_convertToJSON() failed [allowed_af_ids]");
	...
	if(identity_data->application_port_ids) {
		cJSON *application_port_ids = cJSON_AddObjectToObject(item,
		                                                      "applicationPortIds");
		if(application_port_ids == NULL) {
			ogs_error(
				"OpenAPI_identity_data_convertToJSON() failed [application_port_ids]");
			goto end;
		}
		cJSON *localMapObject = application_port_ids;
		if(identity_data->application_port_ids) {
			OpenAPI_list_for_each(
				identity_data->application_port_ids, node) {
				OpenAPI_map_t *localKeyValue =
					(OpenAPI_map_t *) node->data;
				...
				{
					ogs_error(
						"OpenAPI_identity_data_convertToJSON() failed [inner]");
					goto end;
				}
			}
		}
...
OpenAPI_identity_data_t *OpenAPI_identity_data_parseFromJSON(
	...
	supi_list = cJSON_GetObjectItemCaseSensitive(identity_dataJSON,
	                                             "supiList");
	if(supi_list) {
		cJSON *supi_list_local = NULL;
		if(!cJSON_IsArray(supi_list)) {
			ogs_error(
				"OpenAPI_identity_data_parseFromJSON() failed [supi_list]");
			goto end;
		}
		...
		cJSON_ArrayForEach(supi_list_local, supi_list) {
			...
			OpenAPI_list_add(supi_listList,
			                 ogs_strdup(
						 supi_list_local->valuestring));
		}
	}

	gpsi_list = cJSON_GetObjectItemCaseSensitive(identity_dataJSON,
	                                             "gpsiList");
	if(gpsi_list) {
		cJSON *gpsi_list_local = NULL;
		if(!cJSON_IsArray(gpsi_list)) {
			ogs_error(
				"OpenAPI_identity_data_parseFromJSON() failed [gpsi_list]");
			goto end;
		}
	...
	allowed_af_ids = cJSON_GetObjectItemCaseSensitive(identity_dataJSON,
	                                                  "allowedAfIds");
	if(allowed_af_ids) {
		cJSON *allowed_af_ids_local = NULL;
		if(!cJSON_IsArray(allowed_af_ids)) {
			ogs_error(
				"OpenAPI_identity_data_parseFromJSON() failed [allowed_af_ids]");
			goto end;
		}
		...
		cJSON_ArrayForEach(allowed_af_ids_local, allowed_af_ids) {
			...
			OpenAPI_list_add(allowed_af_idsList,
			                 ogs_strdup(
						 allowed_af_ids_local->
						 valuestring));
		}
	}

	application_port_ids =
		cJSON_GetObjectItemCaseSensitive(identity_dataJSON,
		                                 "applicationPortIds");
	if(application_port_ids) {
		cJSON *application_port_ids_local_map = NULL;
		if(!cJSON_IsObject(application_port_ids) &&
		   !cJSON_IsNull(application_port_ids))
		{
			ogs_error(
				"OpenAPI_identity_data_parseFromJSON() failed [application_port_ids]");
			goto end;
		}
		if(cJSON_IsObject(application_port_ids)) {
			...
			cJSON_ArrayForEach(application_port_ids_local_map,
				...
				cJSON *localMapObject =
					application_port_ids_local_map;
				double *localDouble = NULL;
				int *localInt = NULL;
				if(!cJSON_IsString(localMapObject)) {
					ogs_error(
						"OpenAPI_identity_data_parseFromJSON() failed [inner]");
					goto end;
				}
				localMapKeyPair =
					OpenAPI_map_create(ogs_strdup(
								   ...
								   localMapObject
								   ->valuestring));
				OpenAPI_list_add(application_port_idsList,
				                 localMapKeyPair);
			}
		}
	}
