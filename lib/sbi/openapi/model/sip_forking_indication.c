...
char * OpenAPI_sip_forking_indication_ToString(
	...
	const char *sip_forking_indicationArray[] = { "NULL", "SINGLE_DIALOGUE",
		                                      "SEVERAL_DIALOGUES" };
	size_t sizeofArray = sizeof(sip_forking_indicationArray) /
	                     sizeof(sip_forking_indicationArray[0]);
	if(sip_forking_indication < sizeofArray) {
		return (char *) sip_forking_indicationArray[
...
OpenAPI_sip_forking_indication_e OpenAPI_sip_forking_indication_FromString(
	...
	while(stringToReturn < sizeofArray) {
		if(strcmp(sip_forking_indication,
		          sip_forking_indicationArray[stringToReturn]) == 0) {
