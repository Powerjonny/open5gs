...
OpenAPI_nf_set_cond_t *OpenAPI_nf_set_cond_create(char *nf_set_id) {
	OpenAPI_nf_set_cond_t *nf_set_cond_local_var =
		ogs_malloc(sizeof(OpenAPI_nf_set_cond_t));
...
cJSON *OpenAPI_nf_set_cond_convertToJSON(OpenAPI_nf_set_cond_t *nf_set_cond) {
	...
	if(nf_set_cond == NULL) {
		ogs_error(
			"OpenAPI_nf_set_cond_convertToJSON() failed [NfSetCond]");
		return NULL;
	}
	...
	if(!nf_set_cond->nf_set_id) {
		ogs_error(
			"OpenAPI_nf_set_cond_convertToJSON() failed [nf_set_id]");
		return NULL;
	}
...
OpenAPI_nf_set_cond_t *OpenAPI_nf_set_cond_parseFromJSON(
	...
	nf_set_id = cJSON_GetObjectItemCaseSensitive(nf_set_condJSON,
	                                             "nfSetId");
	if(!nf_set_id) {
		ogs_error(
			"OpenAPI_nf_set_cond_parseFromJSON() failed [nf_set_id]");
		goto end;
	}
