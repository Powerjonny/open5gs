...
ogs_pkbuf_t *ogs_lpp_encode(ogs_lpp_message_t *message)
{
    ...
    if (ogs_log_get_domain_level(OGS_LOG_DOMAIN) >= OGS_LOG_TRACE)
        asn_fprint(stdout, &asn_DEF_LPP_LPP_Message, message);

    pkbuf = ogs_asn_encode_uper(&asn_DEF_LPP_LPP_Message, message);
    if (!pkbuf) {
...
{
    ogs_assert(message);
    ogs_asn_free(&asn_DEF_LPP_LPP_Message, message);
}
